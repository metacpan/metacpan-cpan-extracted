...
sub num2ita_cardinal :Export { goto &number_to_it }

sub num2ita_ordinal :Export {
    my $number = shift;
    ...
    # For 11+, get the cardinal form and apply the rule:
    # drop final vowel (if any) + "esimo"
    # special case: if cardinal ends in "tre", keep the "e" and add accent -> "treesimo" -> "tré" + "esimo"
    my $cardinal = number_to_it($number);

    # Handle "tre" at the end: treesimo with accent -> tré + esimo
    if ($cardinal =~ /tre$/) {
...
=head1 SYNOPSIS
 ...
 # Procedural Style
 use Lingua::ITA::Num2Word qw(number_to_it);
 ...
 # OO Style
 use Lingua::ITA::Num2Word;
 my $number = Lingua::ITA::Num2Word->new( 123 );
...
Lingua::ITA::Num2Word converts arbitrary numbers into human-oriented
...
Remark that Lingua::ITA::Num2Word object, created by the B<new> constructor
described below, are I<Two-face scalars> as described in L<overload>:
when a Lingua::ITA::Num2Word object is used as a number, then it is a number,
...
=item B<num2ita_cardinal($number,[options])>

Alias for B<number_to_it>.

=item B<num2ita_ordinal($number)>

Converts a number to its Italian ordinal representation (e.g. 1 => "primo",
11 => "undicesimo", 23 => "ventitreesimo").

=item B<number_to_it($number,[options])>

Converts a number to its Italian string representation without building
the Lingua::ITA::Num2Word instance.
...
is equivalent to
  ...
  $string = do {
    my $tmp = Lingua::ITA::Num2Word->new($number,...);
    $tmp->get_string();
  };
...
The following method compose the OO interface to Lingua::ITA::Num2Word.

=over

=item B<Lingua::ITA::Num2Word->new($number,[options])>
...
The representation of numbers by Lingua::ITA::Num2Word can be influenced by means
...
element is taken to be the singular form while the second is the plural. So that
    ...
    my $euro = Lingua::ITA::Num2Word->new(253,name => "euro",
                                            decmode => 'fract',
                                            decimal => 2);
...
will print 'duecentocinquantatre euro e zero centesimi', while
    ...
    my $dollar = Lingua::ITA::Num2Word->new(253,name => [qw(dollaro dollari)],
                                              decmode => 'fract',
                                              decimal => 2);
...
As stated above, instances of Lingua::ITA::Num2Word are I<Two-face scalars> like
those described in L<overload>. This means that you can do something like:

    my $first = Lingua::ITA::Num2Word->new(123);
    my $second = Lingua::ITA::Num2Word->new(321);
