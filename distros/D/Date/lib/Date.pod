...
=head1 SYNOPSIS
    ...
    "$date"             # same as prev.
    $date->to_string(Date::Format::iso8601); # 2000-02-29T12:21:11+03:00
    ...
    if ($a) { # valid date
      ...
      if ($a->error == Date::errc::parser_error) { ... }
...
INPUT_FORMAT_ALL(the default), INPUT_FORMAT_ISO, INPUT_FORMAT_ISO8601, INPUT_FORMAT_RFC1123, INPUT_FORMAT_RFC850, INPUT_FORMAT_ANSI_C, INPUT_FORMAT_DOT.

If C<$string> is an invalid date or a valid date but in format other than specified, then C<error()> is set to C<Date::errc::parser_error>
...
=head4 to_string($format = Date::Format::iso)
...
=item Date::Format::iso_tz

C<iso> with timezone offset

    2019-12-31 23:59:59.123+03:00

=item Date::Format::iso_date

Only date

    2019-12-31

=item Date::Format::iso8601

    2019-12-31T23:59:59.123+03:00

=item Date::Format::iso8601_notz

Same as C<iso8601> but omits printing timezone offset

    2019-12-31T23:59:59.123

=item Date::Format::rfc1123

    Tue, 10 Dec 2019 00:00:00 GMT
    
=item Date::Format::cookie

same as rfc1123

=item Date::Format::rfc850

    Tuesday, 01-Jan-19 03:04:00 GMT

=item Date::Format::ansi_c
...
=item Date::Format::dot

    31.12.2019

=item Date::Format::hms
...
If second operand is a date object, then returns L<Date::Rel> object representing exact interval from left operand till right operand.
...
when range_check is set, then a date "2001-02-31" became invalid date and error() is set to C<Date::errc::out_of_range>.

One exception from this rule is when format has a weekday name (rfc1123, etc) and it doesn't match represented date, C<Date::errc::out_of_range> will be always set.
...
=head4 Date::errc::parser_error

Wrong date string format

=head4 Date::errc::out_of_range
