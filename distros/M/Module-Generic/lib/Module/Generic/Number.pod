...
=head1 SYNOPSIS
    ...
    my $m = Module::Generic::Number->new( 1281284,
    {
        precision    => 2,
    ...
    my $loose = Module::Generic::Number->new( 1000, { posix_strict => 0 } );
...
The purpose of this class/package is to provide a lightweight object-oriented approach to number manipulation.
...
Specifies the monetary decimal separator, used by L</format_money>. When not provided, it is resolved from the C<LC_MONETARY> category of the current locale. This can also be changed or retrieved with the method L</mon_decimal>

=item C<mon_grouping>

The size of the groups of digits for monetary values, used by L</format_money>. When not provided, it is resolved from the C<mon_grouping> field of the C<LC_MONETARY> category. This can also be changed or retrieved with the method L</mon_grouping>

=item C<mon_thousand>

Specifies the monetary thousand separator, used by L</format_money>. When not provided, it is resolved from the C<LC_MONETARY> category. This can also be changed or retrieved with the method L</mon_thousand>
...
When true, plain number formatting follows the C<LC_NUMERIC> category and monetary formatting (L</format_money>) follows the C<LC_MONETARY> category, with no fall back from one category to the other. This is the strict POSIX-conformant behaviour: under a mixed locale such as C<LC_NUMERIC=C> with C<LC_MONETARY=en_US.UTF-8>, a plain number is not grouped whereas money is.

When false, a more tolerant resolution is used: a value absent from its own category is filled from the sibling category. This is meant for incompletely configured locales, where a missing value is treated as a misconfiguration to be worked around rather than honoured.

This can also be changed or retrieved with the method L</posix_strict>
...
Returns the object as a regular perl number.

Internally, it returns the underlying number + 0 to ensure perl converts it to a number.
...
Sets or gets the monetary decimal separator used by L</format_money>. When not set explicitly, it is resolved from the C<LC_MONETARY> category of the current locale.

=head2 mon_grouping

Sets or gets the size of the groups of digits for monetary values, used by L</format_money>. When not set explicitly, it is resolved from the C<mon_grouping> field of the C<LC_MONETARY> category.

=head2 mon_thousand

Sets or gets the monetary thousand separator used by L</format_money>. When not set explicitly, it is resolved from the C<LC_MONETARY> category of the current locale.
...
Boolean, true by default. Controls how the number and monetary formatting properties are resolved from the locale. See the C<posix_strict> option in the constructor L</new> for the full description.
...
Gets or sets the position of the sign symbol (typically "-") relative to the currency symbol and value for negative monetary amounts. Corresponds to C<POSIX::localeconv()-E<gt>{n_sign_posn}>. Accepted values:
...
Gets or sets the sign (typically "", i.e. empty, but could be set to "+") position for non-negative monetary values. Corresponds to C<POSIX::localeconv()-E<gt>{p_sign_posn}>. See L</position_neg> for the accepted values. Returns a L<Module::Generic::Scalar> object.
...
Sets or gets the I<precede> property of this object. This is used by L<Module::Generic::Number::Format> to determine if the currency symbol should be set before or after the number

=head2 precede_neg

Sets or gets the I<precede_neg> property of this object. This is used by L<Module::Generic::Number::Format> to determine if the currency symbol should be set before or after the number when it is a negative number.
