...
BEGIN {
    ...
    $subcommand = 'restore';              # <<<--- Change this to match the command you are testing against.

    use_ok( 'OpenVZ::Vzctl', qw( vzctl known_options subcommand_specs ), $subcommand );

}

my @parms = sort qw( restore_dumpfile );  # <<<--- Change this to match the parameters you are expecting (checked against
                                          # known_options).
...
my $expected_spec = {                     # <<<--- Change this to match expected hash from subcommand_specs
    ...
    restore_dumpfile => { type => SCALAR, optional => 1, callbacks => { 'does file exist?' => ignore(), }, },
