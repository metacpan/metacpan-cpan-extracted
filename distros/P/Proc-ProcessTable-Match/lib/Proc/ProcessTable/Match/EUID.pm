package Proc::ProcessTable::Match::EUID;
...
Proc::ProcessTable::Match::EUID - Check if the EUID of a process matches.
...
=head1 SYNOPSIS
    ...
    use Proc::ProcessTable::Match::EUID;
...
that is 'euids', which is a array of EUIDs to match.

The EUID values can be prefixed with the equalities below for doing
...
sub new{
	...
	if ( ! defined( $args{euids} ) ){
		die ('No euids key specified in the argument hash');
	}
	if ( ref( \$args{euids} ) eq 'ARRAY' ){
		die ('The euids key is not a array');
	}
	if ( ! defined $args{euids}[0] ){
		die ('Nothing defined in the euids array');
	}
...
sub match{
	...
	my $proc_euid;
	eval{
		$proc_euid=$object->euid;
	};

	# don't bother proceeding, the object won't match ever
	# as it does not have a EUID
	if ( ! defined( $proc_euid ) ){
		return 0;
	}

	# use while as foreach will reference the value
	my $euid_int=0;
	while (defined( $self->{euids}[$euid_int] )){
		my $euid=$self->{euids}[$euid_int];
		if (
			( $euid =~ /^[0-9]+$/ ) &&
			( $euid eq $proc_euid )
			){
