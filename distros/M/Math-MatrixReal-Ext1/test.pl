...
eval{ Math::MatrixReal::Ext1->new_from_rows( { foo=> 'bar'} ) };
warn $@ if $@ && $verbose;
if ($@ =~ /array of rows/) {
    print "ok 13\n";
}
...
eval { Math::MatrixReal::Ext1->new_from_rows( [$foo_matrix, $row82, $matrix8->column(3)] ) };
warn $@ if $@ && $verbose;
...
# print appropriately
eval { Math::MatrixReal::Ext1->new_from_cols( [$foo_matrix, $row82, ] ) };
warn $@ if $@ && $verbose;
...
eval { Math::MatrixReal::Ext1->new_from_rows( [$foo_matrix, $row82,  [ 1 ]] ) };
warn $@ if $@ && $verbose;
...
eval { Math::MatrixReal::Ext1->new_from_columns( [ [ 1, 2, 3], [ 1, 2], ] ) };
warn $@ if $@ && $verbose;
...
eval { Math::MatrixReal::Ext1->new_from_columns( [ [ 1, 2, 3], [], ] ) };
warn $@ if $@ && $verbose;
if ($@ =~ /no elements/ ) {
    print "ok 18\n";
}
