...
void
DESTROY(perl_nghttp2_session_t *sess)
CODE:
    if (sess) {
...
int
is_xs(perl_nghttp2_session_t *sess)
CODE:
    RETVAL = 1;
OUTPUT:
    RETVAL
...
void
_xs_set_callbacks(perl_nghttp2_session_t *sess, SV *on_headers, SV *on_data, SV *on_trailers, SV *on_stream_close)
CODE:
    if (SvOK(on_headers)) {
        if (sess->on_headers_cb) SvREFCNT_dec(sess->on_headers_cb);
...
int
_xs_submit_request(perl_nghttp2_session_t *sess, SV *headers_av_ref, SV *data_sv, int end_stream)
CODE:
...
void
feed_input(perl_nghttp2_session_t *sess, SV *input_sv)
CODE:
...
SV *
get_output(perl_nghttp2_session_t *sess)
CODE:
