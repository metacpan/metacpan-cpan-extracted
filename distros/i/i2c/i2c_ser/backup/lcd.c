...
int lcd_wchar(unsigned char data)
{
...
int lcd_rchar(unsigned char *data)
{
 unsigned char ret;
...
/* Liest einen String in lcd_STRING ab der aktuellen Adresse, mit Anzahl len */
int lcd_read_str(int len)
{
 int ret,n,i,c;
 unsigned char ch;
 ret = 1;
...
int lcd_busy(void)
{
  unsigned char busy, time_cnt, time_out,data_1,data_2;
