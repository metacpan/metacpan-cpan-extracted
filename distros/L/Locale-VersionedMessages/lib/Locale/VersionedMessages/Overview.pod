...
process, I'll concentrate on three: L<Locale::gettext>, L<Locale::Maketext>,
...
Another note is that gettext does handle plural values, but it is done
in a very tedious manner.  You have to have a full translation for each
case, which makes translating a long message entail a lot of repetition.
As an example:
...
that this is a real case rather than a made-up one illustrates how
complex localization can be, and the need for a very flexible tool.
...
namespace and to accidentally create conflicts with other modules.
...
it involves both changes to the lexicons AND changes to every program
that uses the lexicons.  This greatly impacts the practicality of
having universal message sets.
...
=head1 Locale::VersionedMessages TOOLS
...
be a hierarchy F<Locale/Message/Sets> and the modules will be created
