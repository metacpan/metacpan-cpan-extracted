...
files could be changed. See L<perlfunc/chmod> for more details.
...
Make sure that the file I<$filename> has the content I<$content>. The
given content should be unencoded (raw, binary, octets). If you need a
specific encoding, then it has to be encoded before:
...
=head3 system
    ...
    $doit->system({show_cwd => $bool}, "command", "arg" ...);

Execute a command. If C<show_cwd> is set to a true value, then logging
shows also the current working directory. See L<"system" in
...
created and done using L<Net::OpenSSH>. The following options are
available:
...
interactive input (e.g. password input).
...
Additional options to pass. See L<Net::OpenSSH/master_opts>.
