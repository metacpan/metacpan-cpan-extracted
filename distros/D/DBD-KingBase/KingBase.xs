...
MODULE = DBD::KingBase    PACKAGE = DBD::KingBase
...
constant(name=Nullch)
    ...
    CODE:
        if (0==ix) {
            ...
            croak("Unknown DBD::KingBase constant '%s'", name);
        }
        else {
            RETVAL = ix;
        }
...
INCLUDE: KingBase.xsi


# ------------------------------------------------------------
# db functions
# ------------------------------------------------------------
MODULE=DBD::KingBase    PACKAGE = DBD::KingBase::db
...
quote(dbh, to_quote_sv, type_sv=Nullsv)
    ...
    CODE:
    {
        ...
        /* Reject references other than overloaded objects (presumed
          stringifiable) and arrays (will make a Kingbase array). */
...
void do(dbh, statement_sv, attr=Nullsv, ...)
    ...
    CODE:
    {
        ...
        if (items < 4) { /* No bind arguments */
            /* Quick run via KCIStatementExecute */
...
# -- end of DBD::KingBase::db


# ------------------------------------------------------------
# statement level interface PG specific
# ------------------------------------------------------------
MODULE = DBD::KingBase    PACKAGE = DBD::KingBase::st
