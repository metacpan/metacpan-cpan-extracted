...
# ----------------------------------------------------
# Exit script if operating system Windows is detected.
# ----------------------------------------------------
if ($^O eq "MSWin32") {
    exit 1;
};
...
=head1 SYNOPSIS
  ...
  # Get the terminal window size using /sys/ioctl.ph or stty. 
  ...
  # Get the terminal window size using Inline C code using sys/ioctl.h.
...
There are two areas of application for the methods presented. They are useful
in the terminal emulator or terminal for short. However, they also work in the
system consoles.
...
=head1 EXAMPLES
  ...
  my ($rows, $cols) = window_size_chars($timeout);
  my ($xpix, $ypix) = windows_size_pixels($timeout);
  my ($rows, $cols) = screen_size_chars($timeout);
  my ($xpix, $ypix) = screen_size_pixels($timeout);

  # Get the window size using Inline C code.
  my ($rows, $cols, $xpix, $ypix) = Terminal::Control::WinSize();
  printf ("%s\n%s\n%s\n%s\n", $rows, $cols, $xpix, $ypix);
...
=head2 Remark on Xterm control sequences  

The use of Xterm escape sequences is still considered experimental.

Reading from STDIN is blocking with respect to the request of escape sequences,
if no data are responded. To overcome the problem of waiting endless for an user
input a timeout is programmed. 

If the standard value is not applicable, reduce the timeout step by step. If the
timeout is to short the method is not able to catch the input from STDIN. 
...
escape sequences take their name from the terminal emulator of the same name.
This means that the Xterm escape sequences should primarily be able to run
...
succession, the data from STDIN will be read in incorrectly. This is only
possible by deleting STDIN. A method of deleting or resetting STDIN is not 
known yet.
