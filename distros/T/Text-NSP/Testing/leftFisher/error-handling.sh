...
# shell program to test leftFisher.pm's responses to various erroneous
# conditions. note of course that leftFisher never halts on errors/warnings
# but sets error codes that statistic.pl can request later on.

# -------------------------------------------------------------------
# Subtest 1: what happens when ngram > 2 is provided to leftFisher
...
# test-1.sub-1.cnt has trigrams in it. so leftFisher.pm should complain
# during the initialization step, since leftFisher is for bigrams only!
...
# now the test!
echo "Test:   statistic.pl --ngram 3 leftFisher test-1.out $TESTFILE" 
statistic.pl --ngram 3 leftFisher test-1.out $TESTFILE >& error.out
...
# now the test!
echo "Test:   statistic.pl --set_freq_combo $FREQCOMBFILE leftFisher test-1.out $TESTFILE" 
statistic.pl --set_freq_combo $FREQCOMBFILE leftFisher test-1.out $TESTFILE >& error.out
...
statistic.pl leftFisher test-1.out $TESTFILE >& error.out
