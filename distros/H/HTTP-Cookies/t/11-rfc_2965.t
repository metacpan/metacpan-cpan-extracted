...
# Example 1
{
    ...
    $res->header( 'Set-Cookie2' =>
            q{Customer="WILE_E_COYOTE"; Version="1"; Path="/acme"} );
    ...
    is(
        count_cookies_for('www.acme.com'), 1,
        '1.1-2: res: found the cookie'
    );
    ...
    $res->header( 'Set-Cookie2' =>
            q{Part_Number="Rocket_Launcher_0001"; Version="1"; Path="/acme"}
    );
    $res->request($req);
    $jar->extract_cookies($res);
    is(
        count_cookies_for('www.acme.com'), 2,
        '1.3-4: res: found the cookies'
    );
    ...
    like(
        $h, qr/Part_Number="?Rocket_Launcher_0001"?/,
        '1.5-6: req: contains part'
    );
    $res->header(
        'Set-Cookie2' => q{Shipping="FedEx"; Version="1"; Path="/acme"} );
    $res->request($req);
    $jar->extract_cookies($res);
    is(
        count_cookies_for('www.acme.com'), 3,
        '1.5-6: res: found the cookies'
    );
...
#   user agent has no stored cookies.
{
    ...
    $res->push_header( 'Set-Cookie2' =>
            q{Part_Number="Riding_Rocket_0023"; Version="1"; Path="/acme/ammo"}
    );
