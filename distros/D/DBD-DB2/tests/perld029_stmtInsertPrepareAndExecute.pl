####################################################################
# TESTCASE: 		perld029_stmtInsertPrepareAndExecute.pl
# DESCRIPTION: 		Prepare and execute an INSERT statement to 
#                       populate table perld1t1.
...
#
$stmt = get_insert_stmt("perld1t1", $row, FALSE);
...
#
$stmt = get_insert_stmt("perld1t1", undef, TRUE);
$sth = $dbh->prepare($stmt);
check_error("PREPARE");
...
for ($row = 1; $row < 5; $row++)
{
  @bind_values = get_values("perld1t1", $row);
