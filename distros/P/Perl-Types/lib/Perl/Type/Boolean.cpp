...
#ifndef __CPP__INCLUDED__Perl__Type__Boolean_cpp
#define __CPP__INCLUDED__Perl__Type__Boolean_cpp 0.003_000

// [[[ INCLUDES ]]]
#include <Perl/HelperFunctions.cpp>  // -> HelperFunctions.h
#include <Perl/Type/Boolean.h>  // -> NULL (relies on native C type)
...
nonsigned_integer boolean_to_nonsigned_integer(boolean input_boolean) {
    return (nonsigned_integer) input_boolean;
}
...
SV* boolean_typetest0() {
    ...
    SV* retval = newSViv(1);
    if (SvIV(Perl__Type__Boolean__MODE_ID()) == 0) { retval = newSViv(0); }
//	SV* retval = integer_to_boolean(Perl__Type__Boolean__MODE_ID());
...
//fprintf(stderr, "in CPPOPS_PERLTYPES boolean_typetest1(), received lucky_boolean = %"INTEGER"\n", SvIV(lucky_boolean));
    SV* retval = newSViv(1);
    if ((SvIV(lucky_boolean) + SvIV(Perl__Type__Boolean__MODE_ID())) == 0) { retval = newSViv(0); }
	return retval;
//	return integer_to_boolean(newSViv( SvIV(lucky_boolean) + SvIV(Perl__Type__Boolean__MODE_ID()) ) );
}

# elif defined __CPP__TYPES

boolean boolean_typetest0() {
//	boolean retval = integer_to_boolean(Perl__Type__Boolean__MODE_ID());
	boolean retval = 1;
	if (Perl__Type__Boolean__MODE_ID() == 0) { retval = 0; }
...
//fprintf(stderr, "in CPPOPS_CPPTYPES boolean_typetest1(), received lucky_boolean = %"INTEGER"\n", lucky_boolean);
    boolean retval = 1;
    if ((lucky_boolean + Perl__Type__Boolean__MODE_ID()) == 0) { retval = 0; }
//fprintf(stderr, "in CPPOPS_CPPTYPES boolean_typetest0(), have retval = %"INTEGER"\n", retval);
	return retval;
//	return(integer_to_boolean(lucky_boolean + Perl__Type__Boolean__MODE_ID()));
}
