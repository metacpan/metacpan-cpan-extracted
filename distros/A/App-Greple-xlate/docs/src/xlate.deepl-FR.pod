...
Le module B<Greple> B<xlate> recherche les blocs de texte souhaités et les remplace par le texte traduit. Actuellement, les modules DeepL (F<deepl.pm>), ChatGPT 4.1 (F<gpt4.pm>) et GPT-5 (F<gpt5.pm>) sont mis en œuvre en tant que moteur dorsal.
...
Les opérations Docker sont gérées par le script C<dozo>, qui peut également être utilisé comme une commande autonome. Le script C<dozo> prend en charge le fichier de configuration C<.dozorc> pour les paramètres persistants des conteneurs.
