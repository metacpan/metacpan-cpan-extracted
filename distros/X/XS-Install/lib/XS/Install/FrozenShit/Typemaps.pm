package 
    XS::Install::FrozenShit::Typemaps;
use 5.006001;
use strict;
use warnings;
our $VERSION = '3.57';

require XS::Install::FrozenShit::ParseXS;
require XS::Install::FrozenShit::ParseXS::Constants;
require XS::Install::FrozenShit::Typemaps::InputMap;
require XS::Install::FrozenShit::Typemaps::OutputMap;
require XS::Install::FrozenShit::Typemaps::Type;

=head1 NAME

XS::Install::FrozenShit::Typemaps - Read/Write/Modify Perl/XS typemap files

=head1 SYNOPSIS

  # read/create file
  my $typemap = XS::Install::FrozenShit::Typemaps->new(file => 'typemap');
  # alternatively create an in-memory typemap
  # $typemap = XS::Install::FrozenShit::Typemaps->new();
  # alternatively create an in-memory typemap by parsing a string
  # $typemap = XS::Install::FrozenShit::Typemaps->new(string => $sometypemap);
...
an C<XS::Install::FrozenShit::Typemaps::Type> object as first argument, a copy of which will be
...
sub add_typemap {
  ...
  if ((@_ % 2) == 1) {
    ...
    $type = XS::Install::FrozenShit::Typemaps::Type->new(
      xstype      => $xstype,
...
As an alternative to the named parameters usage, you may pass in
an C<XS::Install::FrozenShit::Typemaps::InputMap> object as first argument, a copy of which will be
added to the typemap. In that case, only the C<replace> or C<skip> named parameters
may be used after the object. Example:
...
sub add_inputmap {
  ...
  if ((@_ % 2) == 1) {
    ...
    $input = XS::Install::FrozenShit::Typemaps::InputMap->new(
      xstype => $xstype,
      code   => $code,
    );
  }
...
sub add_outputmap {
  ...
  if ((@_ % 2) == 1) {
    ...
    $output = XS::Install::FrozenShit::Typemaps::OutputMap->new(
      xstype => $xstype,
      code   => $code,
    );
  }
...
sub add_string {
  ...
  my $other = XS::Install::FrozenShit::Typemaps->new(string => $args{string});
...
Alternatively, you may pass a single C<XS::Install::FrozenShit::Typemaps::Type> object.
...
Alternatively, you may pass a single C<XS::Install::FrozenShit::Typemaps::InputMap> object.
...
Alternatively, you may pass a single C<XS::Install::FrozenShit::Typemaps::OutputMap> object.
...
Returns the C<XS::Install::FrozenShit::Typemaps::Type>
...
Returns the C<XS::Install::FrozenShit::Typemaps::InputMap>
object for the entry if found.
...
sub as_string {
  ...
  foreach my $entry (@$typemap) {
    # type kind proto
    # /^(.*?\S)\s+(\S+)\s*($XS::Install::FrozenShit::ParseXS::Constants::PrototypeRegexp*)$/o
...
This is documented because it is used by C<XS::Install::FrozenShit::ParseXS>,
...
sub _get_inputmap_hash {
  ...
  foreach my $xstype (keys %$lookup) {
    ...
    # to the original XS::Install::FrozenShit::ParseXS.
    $rv{$xstype} =~ s/\s*\z/\n/;
  }
...
Moved from C<XS::Install::FrozenShit::ParseXS>.
...
sub _parse {
  ...
  # Heavily influenced by XS::Install::FrozenShit::ParseXS
  my $section = 'typemap';
  ...
  while ($$stringref =~ /^(.*)$/gcm) {
    ...
    if ($section eq 'typemap') {
      ...
      s/^\s+//; s/\s+$//;
      next if $_ eq '' or /^#/;
      my($type, $kind, $proto) = /^(.*?\S)\s+(\S+)\s*($XS::Install::FrozenShit::ParseXS::Constants::PrototypeRegexp*)$/o
      ...
      $self->add_typemap(
        XS::Install::FrozenShit::Typemaps::Type->new(
          xstype => $kind,
  ...
  foreach my $inexpr (@input_expr) {
    $self->add_inputmap( XS::Install::FrozenShit::Typemaps::InputMap->new(%$inexpr), @add_params );
  }
  foreach my $outexpr (@output_expr) {
    $self->add_outputmap( XS::Install::FrozenShit::Typemaps::OutputMap->new(%$outexpr), @add_params );
  }

  return 1;
}

# taken from XS::Install::FrozenShit::ParseXS
sub _valid_proto_string {
  my $string = shift;
  if ($string =~ /^$XS::Install::FrozenShit::ParseXS::Constants::PrototypeRegexp+$/o) {
    return $string;
  }
...
# taken from XS::Install::FrozenShit::ParseXS (C_string)
...
Inherits some evil code from C<XS::Install::FrozenShit::ParseXS>.

=head1 SEE ALSO

The parser is heavily inspired from the one in L<XS::Install::FrozenShit::ParseXS>.
