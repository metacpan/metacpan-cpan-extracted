######################################################################
#
# 06_yasohachi_db.pl - Yaso-Hachi (rice-grain manager) on PSGI::Handy
#
# A rebuild of the standalone "Yaso-Hachi" web app as a PSGI::Handy
# example. The original drove HTTP::Handy directly with a hand-rolled
# dispatcher; here the same application is expressed through the
# framework: PSGI::Handy routing, a Context for params/redirect/html,
# and DB::Handy as the injected model. HTTP::Handy is only the delivery
# layer, exactly as in eg/03 and eg/04.
#
# The kanji for rice ("E<31>") hides the figure 88 ("eight-ten-eight"),
...
#
# Run: perl -Ilib eg/06_yasohachi_db.pl
# Then open http://127.0.0.1:8080/ to register and track grains.
# (A throwaway database is created under the system temp directory and
#  removed on exit, so the example leaves nothing behind.)
#
# Demonstrates:
...
#   DB::Handy two-table CRUD with CREATE INDEX, and US-ASCII source that
#   renders Japanese via &#xNNNN; numeric character references.
#
######################################################################
use strict;
...
use DB::Handy;     # model layer (DBI-like flat-file RDBMS)

######################################################################
# Inlined model classes (kept in-file so the example is one script).
# Both are written to run on Perl 5.005_03 and later.
######################################################################
...
my $DBDIR = File::Spec->catfile(File::Spec->tmpdir(), "psgi_eg05_$$");
mkdir($DBDIR, 0700) or die "Cannot mkdir $DBDIR: $!\n";

my $dbh = DB::Handy->connect($DBDIR, 'yasohachi',
                             { RaiseError => 0, PrintError => 0 });
die "DB connect failed\n" unless $dbh;

# rice: one row per grain. planted_on is VARCHAR(13) so it can hold the
# extended range "-12000-04-15" (BCE) to "1000000-12-31" (far future).
$dbh->do(
    'CREATE TABLE rice ('
  . 'rice_id    VARCHAR(40)  PRIMARY KEY,'
  . 'variety    VARCHAR(80),'
  . 'grower     VARCHAR(80),'
  . 'location   VARCHAR(80),'
  . 'planted_on VARCHAR(13),'
  . 'note       VARCHAR(255),'
  . 'created_at VARCHAR(20)'
  . ')'
);

# rice_status: one row per (grain x step); 88 rows per grain.
$dbh->do(
    'CREATE TABLE rice_status ('
  . 'rice_id  VARCHAR(40),'
  . 'step_no  INT,'
  . 'step_key VARCHAR(40),'
  . 'status   INT'
...
# Speed up the per-grain status lookups used on every detail page.
$dbh->do('CREATE INDEX idx_status_rice_id ON rice_status (rice_id)');
...
# Insert the 88 status rows (all status=0) for a freshly created grain.
sub insert_initial_status {
    my ($h, $id) = @_;
    my $all = RiceProcess->new->all;
    my $ins = $h->prepare(
        'INSERT INTO rice_status (rice_id, step_no, step_key, status)'
      . ' VALUES (?, ?, ?, ?)');
    my $step;
    for $step (@$all) {
        $ins->execute($id, $step->{number}, $step->{key}, 0);
    }
    $ins->finish;
}
...
$app->get('/', sub {
    my $c   = shift;
    my $sth = $c->db->prepare(
        'SELECT rice_id, variety, grower, location, planted_on, created_at'
      . ' FROM rice ORDER BY created_at DESC');
    $sth->execute;
    my @grains;
    my $r;
    while ($r = $sth->fetchrow_hashref) { push @grains, $r }
    $sth->finish;

    my %done;
    my $ssth = $c->db->prepare('SELECT rice_id, status FROM rice_status');
    $ssth->execute;
    while ($r = $ssth->fetchrow_hashref) {
        $done{ $r->{rice_id} } += 0;
        $done{ $r->{rice_id} }++ if defined $r->{status} && $r->{status} == 2;
    }
    $ssth->finish;
...
$app->post('/grain/create', sub {
    ...
    my $ins = $c->db->prepare(
        'INSERT INTO rice'
      . ' (rice_id, variety, grower, location, planted_on, note, created_at)'
      . ' VALUES (?, ?, ?, ?, ?, ?, ?)');
    $ins->execute(
        $id,
        defined $c->param('variety')  ? $c->param('variety')  : '',
        defined $c->param('grower')   ? $c->param('grower')   : '',
        defined $c->param('location') ? $c->param('location') : '',
        $planted,
        defined $c->param('note')     ? $c->param('note')     : '',
        now(),
    );
    $ins->finish;

    insert_initial_status($c->db, $id);
...
$app->get('/grain/:id', sub {
    ...
    my $sth = $c->db->prepare('SELECT * FROM rice WHERE rice_id = ?');
    $sth->execute($id);
    my $grain = $sth->fetchrow_hashref;
    $sth->finish;
    return $c->html(layout('404',
        '<h2>404 Not Found</h2><p>' . h($id) . '</p>'), 404)
        unless defined $grain;

    my $ssth = $c->db->prepare(
        'SELECT step_no, step_key, status FROM rice_status'
      . ' WHERE rice_id = ? ORDER BY step_no');
    $ssth->execute($id);
    my @steps;
    my $sr;
    while ($sr = $ssth->fetchrow_hashref) { push @steps, $sr }
    $ssth->finish;
...
$app->post('/grain/:id/update', sub {
    ...
    my $id  = $c->param('id');
    my $upd = $c->db->prepare(
        'UPDATE rice_status SET status = ? WHERE rice_id = ? AND step_no = ?');
    my $step;
    for $step (@{ RiceProcess->new->all }) {
        my $v = $c->param('s_' . $step->{number});
        $v = 0 unless defined $v && $v =~ /^[012]$/;
        $upd->execute(int($v), $id, $step->{number});
    }
    $upd->finish;
    return $c->redirect("/grain/$id");
});
...
$app->get('/grain/:id/delete', sub {
    my $c  = shift;
    my $id = $c->param('id');
    my $d1 = $c->db->prepare('DELETE FROM rice_status WHERE rice_id = ?');
    $d1->execute($id);
    $d1->finish;
    my $d2 = $c->db->prepare('DELETE FROM rice WHERE rice_id = ?');
    $d2->execute($id);
    $d2->finish;
    return $c->redirect('/');
});
...
print "Yaso-Hachi on PSGI::Handy  http://127.0.0.1:8080/\n";
my $psgi = $app->to_app;
