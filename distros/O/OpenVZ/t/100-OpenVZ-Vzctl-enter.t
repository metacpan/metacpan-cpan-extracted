...
BEGIN {
    ...
    $subcommand = 'enter';                # <<<--- Change this to match the command you are testing against.

    use_ok( 'OpenVZ::Vzctl', qw( vzctl known_options subcommand_specs ), $subcommand );

}

my @parms = sort qw( exec );              # <<<--- Change this to match the parameters you are expecting (checked against
                                          # known_options).
...
my $expected_spec = {                     # <<<--- Change this to match expected hash from subcommand_specs
    ...
    exec => { type => SCALAR | ARRAYREF, optional => 1, callbacks => { 'do not want empty values' => ignore() }, },
