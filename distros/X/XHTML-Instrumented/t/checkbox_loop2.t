...
$output = $x->output(
   likely_phrase_loop => $x->loop(
       ...
       data => [
           [ $x->replace( text => 'one' ),  $x->replace( args => { value => 123 } ) ],
           [ $x->replace( text => 'two' ),  $x->replace( args => { value => 321 } ) ],
       ],
   ),
   myform => $form,
);
...
<div>
  <form name="myform" id="myform" method="post">
    ...
    <ul id="likely_phrase_loop">
      <li>
        <input type="checkbox" id="likely_phrase_id.1" name="likely_phrase_ids" value="123"/>
        <a id="likely_phrase_text.1" href="./phrase.html" name= "likely_phrase_text">one</a>
      </li>
      <li>
        <input type="checkbox" id="likely_phrase_id.2" name="likely_phrase_ids" value="321"/>
