...
my @responses = (
':listen.server.irc 015 bobbins :listen.server.irc[1FU] ----------------------------------------- | Users:     1 (33.33%)',
':listen.server.irc 015 bobbins :|-harpo.server.irc[9T9] ---------------------------------------- | Users:     1 (33.33%)',
':listen.server.irc 015 bobbins :`-groucho.server.irc[7UP] -------------------------------------- | Users:     1 (33.33%)',
':listen.server.irc 015 bobbins :  `-fake.server.irc[4AK] --------------------------------------- | Users:     0 (0.00%)',
':listen.server.irc 017 bobbins :End of /MAP',
);
...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^bobbins' && $params->[1] eq '+i' ) {
    pass($cmd);
    $poe_kernel->post( $sender, 'send_to_server', { command => 'OPER', params => [ 'bobbins', 'fishdont' ], colonify => 0 } );
  }
  if ( $cmd eq '381' ) {
    pass("IRC$cmd");
    $poe_kernel->post( $sender, 'send_to_server', { command => 'MAP' } );
    return;
  }
  if ( $cmd =~ m!^(01[567])$! ) {
