...
Этот учебник описывает использование потоков(threads) интерпретатором Perl (иногда
именуемых, как I<ithreads>). В этой модели каждый поток выполняется в собственном интерпретаторе Perl и любой обмен данными
между потоками должен быть явным. На уровне пользователя интерфейс для I<ithreads>
использует класс L<threads>.
