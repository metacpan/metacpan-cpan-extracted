...
#   P15 No remainder (%) by a value of 2**31 or more
...
# Counts: P1-P9 and P15 for all files, P10-P14 for .pm only
my $total = scalar(@all_files) * 10
          + scalar(@pm_files)  * 5;
plan_tests($total);
...
for my $f (@all_files) {
    ...
    # P15: no remainder by 2**31 or more.
    #
    # Perl's % operator casts both operands to an integer first. Where
    # an integer is 32 bits wide -- Windows Perl, and any other 32 bit
    # build -- a divisor of 2**32 does not fit and becomes zero, so
    # "$x % 4294967296" dies with "Illegal modulus zero" there while
    # working perfectly on a 64 bit build. Take such a remainder by
    # subtraction, or with int() division, instead.
    my $limit = 65536 * 32768;          # 2**31, itself written safely
    my %big_name;
    while ($code =~ /my\s+\$(\w+)\s*=\s*([\d_]+)\s*;/g) {
        my ($name, $value) = ($1, $2);
        $value =~ s/_//g;
        $big_name{$name} = 1 if $value >= $limit;
    }
    while ($code =~ /my\s+\$(\w+)\s*=\s*(\d+)\s*\*\s*(\d+)\s*;/g) {
        $big_name{$1} = 1 if $2 * $3 >= $limit;
    }
    my @p15;
    {
        my $lineno = 0;
        for my $raw (split /\n/, $code) {
            ...
            while ($cl =~ /%\s*(\d[\d_]*)/g) {
                my $value = $1;
                $value =~ s/_//g;
                $bad = 1 if $value >= $limit;
            }
            while ($cl =~ /%\s*\$(\w+)/g) {
                $bad = 1 if $big_name{$1};
            }
            $bad = 1 if $cl =~ /%\s*\(\s*(\d+)\s*\*\s*(\d+)\s*\)/
                     && ($1 * $2) >= $limit;
            push @p15, { line => $lineno, text => $raw } if $bad;
        }
    }
    ok(!@p15, "P15 - no remainder by 2**31 or more: $f");
    diag("  line $_->{line}: $_->{text}") for @p15;
}
