...
sub harpo_connected {
  ...
  $kernel->post( $sender, 'send_to_server', { prefix => '9T9', command => 'UID', params => [ 'bobbins', '1', $uidts, '+i', '~bobbins', 'listen.server.irc', '192.168.1.40', '9T9AAAAAB', '0', 'bobbins along' ], colonify => 1 } );
...
sub client_input {
  ...
  if ( $cmd eq 'KILL' ) {
    ...
    is( $params->[0], 'bobbins', 'Nick okay' );
    is( $params->[1], 'Nick Collision', 'Nicks have collide' );
    return;
  }
  if ( $cmd eq 'ERROR' ) {
    pass($cmd);
