package QualysGuard::Response::AssetRangeInfo;
...
sub new {
    ...
    if ( $self->exists('/ASSET_RANGE_INFO/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/ASSET_RANGE_INFO/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/ASSET_RANGE_INFO/ERROR');
...
QualysGuard::Response::AssetRangeInfo
