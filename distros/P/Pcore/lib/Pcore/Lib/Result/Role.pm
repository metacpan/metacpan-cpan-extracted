package Pcore::Lib::Result::Role;

use Pcore -role, -const;
use Pcore::Lib::Scalar qw[is_res is_ref is_plain_arrayref is_plain_hashref];
use overload
  ...
  REDO:
    if ( is_plain_arrayref $args->{status} ) {
        ...
        if ( !defined $args->{reason} ) {
            $args->{reason} = Pcore::Lib::Result::resolve_reason( $args->{status}, $self->get_status_reason );
        }
        elsif ( is_plain_hashref $args->{reason} ) {
            $args->{reason} = Pcore::Lib::Result::resolve_reason( $args->{status}, $args->{reason} );
        }
    }
...
sub set_status ( $self, $status, $reason = undef ) {
  ...
  REDO:
    if ( is_plain_arrayref $status) {
        ...
        if ( !defined $reason ) {
            $self->{reason} = Pcore::Lib::Result::resolve_reason( $status, $self->get_status_reason );
        }
        elsif ( is_plain_hashref $reason) {
            $self->{reason} = Pcore::Lib::Result::resolve_reason( $status, $reason );
        }
        else {
            $self->{reason} = $reason;
        }
