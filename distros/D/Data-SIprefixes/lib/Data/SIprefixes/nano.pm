...
=head1 SYNOPSIS
    ...
    use Data::SIprefixes::nano;
    
    my $prefix=Data::SIprefixes::nano->new;

    my $origMeasure='nanometer';
...
sub shortMatch{
    ...
    if ( $measure=~/^n/ ){
        ...
        if ( $measure =~ /^ / ){
            $self->{error}=2;
            $self->{errorString}='Space found after prefix, /^n/, in "'.$origMeasure.'"' ;
            $self->warn;
            return undef;
        }
