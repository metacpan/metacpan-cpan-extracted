...
sub new {
	...
	$self->propertyContainerName( 'ODO::RDFS::List::PropertiesContainer' );
	$self->properties(bless {}, 'ODO::RDFS::List::PropertiesContainer');
	...
	if(   exists($properties{'rest'})
	   && defined($properties{'rest'})) {
		...
		unless(UNIVERSAL::isa($properties{'rest'}, 'ODO::RDFS::Properties::rest')) {
			return undef;
		}
	...
	if(   exists($properties{'first'})
	   && defined($properties{'first'})) {
		...
		unless(UNIVERSAL::isa($properties{'first'}, 'ODO::RDFS::Properties::first')) {
			return undef;
		}
...
sub __to_statement_array {
	...
	my %properties = (''=> '', 'rest'=> 'ODO::RDFS::Properties::rest',  'first'=> 'ODO::RDFS::Properties::first', );
