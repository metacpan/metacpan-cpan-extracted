package MediaWiki::Bot::Plugin::ImageTester;
...
MediaWiki::Bot::Plugin::ImageTester - a plugin for MediaWiki::Bot which contains image copyright checking and analysis for the english wikipedia
...
MediaWiki::Bot is a framework that can be used to write Wikipedia bots. MediaWiki::Bot::Plugin::ImageTester can be used to check and tag images for common copyright issues. It should not be assumed to be perfect, caveat emptor.
...
sub optout {
	my ($user)=@_;
	if ($user=~/User:(23skidoo|TreasuryTag|Matt Fitzpatrick|Commdor)/) {
		return 1;
	} else {
		return 0;
	}
