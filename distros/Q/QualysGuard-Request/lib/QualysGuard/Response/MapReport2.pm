package QualysGuard::Response::MapReport2;
...
sub new {
    ...
    if ( $self->exists('/MAP_REQUEST/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/MAP_REQUEST/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/MAP_REQUEST/ERROR');
...
sub get_ip_address_list {
    my $self    = shift;
    my @nodes   = $self->findnodes('/MAP_REQUEST/MAP/IP');
    my @rv      = (); 
...
QualysGuard::Response::MapReport2
...
Returns an arrayref of the @value attribute for each /MAP_REQUEST/MAP/IP node.
