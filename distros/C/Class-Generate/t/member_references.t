...
Test {
    ...
    (Arrays_Equal([sort { $a cmp $b } $v->Hash_keys], [qw(e1 e2)]) &&
     ...
     Arrays_Equal([sort { $a cmp $b } $v->Hash], [sort { $a cmp $b } @sample]))
};
...
Test {
    ...
    $v->delete_Hash('e1');
    Arrays_Equal [sort { $a <=> $b } map substr($_,1), $v->Hash_keys], [2..10]
};

Test {
    $v->delete_Hash(map { "e$_" } 2..4);
    Arrays_Equal [sort { $a <=> $b } map substr($_,1), $v->Hash_keys], [5..10]
};
