...

{
   ...
   is( [ sort keys %$meta ], [sort qw( t.Colourable t.Ball )], 'keys of t/Ball.tan' );
   ...
   is( [ sort keys %$props ], [qw( colour )], 't.Colourable direct props' );
   ...
   is( [ sort keys %{ $colourable->properties } ], [qw( colour )], 't.Colourable props' );

   ref_is( $colourable->property( "colour" ), $props->{colour}, 't.Colourable ->property' );
   ...
   is( [ sort keys %$methods ], [qw( bounce )], 't.Ball direct methods' );

   isa_ok( $methods->{bounce}, [ "Tangence::Meta::Method" ], 't.Ball method bounce' );
   ref_is( $methods->{bounce}->class, $ball, 't.Ball method bounce class' );
   ...
   is( [ map $_->sig, $methods->{bounce}->argtypes ], [qw( str )], 't.Ball method bounce argtypes sigs' );
   ...
   is( [ sort keys %{ $ball->methods } ], [qw( bounce )], 't.Ball methods' );

   ref_is( $ball->method( "bounce" ), $methods->{bounce}, 't.Ball ->method' );

   $events = $ball->direct_events;

   is( [ sort keys %$events ], [qw( bounced )], 't.Ball direct events' );

   isa_ok( $events->{bounced}, [ "Tangence::Meta::Event" ], 't.Ball event bounced' );
   ref_is( $events->{bounced}->class, $ball, 't.Ball event bounced class' );
   ...
   is( [ map $_->sig, $events->{bounced}->argtypes ], [qw( str )], 't.Ball event bounced argtypes sigs' );

   is( [ sort keys %{ $ball->events } ], [qw( bounced )], 't.Ball events' );

   ref_is( $ball->event( "bounced" ), $events->{bounced}, 't.Ball ->event' );

   $props = $ball->direct_properties;

   is( [ sort keys %$props ], [qw( size )], 't.Ball direct props' );

   ref_is( $props->{size}->class, $ball, 't.Ball prop size class' );
   ...
   is( [ sort keys %{ $ball->properties } ], [qw( colour size )], 't.Ball props' );

   ref_is( $ball->property( "size" ), $props->{size}, 't.Ball ->property' );

   is( [ map { $_->name } $ball->direct_superclasses ], [qw( t.Colourable )], 't.Ball direct superclasses' );
   is( [ map { $_->name } $ball->superclasses ], [qw( t.Colourable )], 't.Ball superclasses' );
}
