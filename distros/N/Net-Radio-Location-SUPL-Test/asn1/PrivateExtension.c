/*
 * Generated by asn1c-0.9.23 (http://lionet.info/asn1c)
 * From ASN.1 module "MAP-ExtensionDataTypes"
 * 	found in "../asn1src/MAP-ExtensionDataTypes.asn"
 * 	`asn1c -gen-PER -fskeletons-copy -fnative-types`
 */

#include "PrivateExtension.h"

static int
memb_extId_constraint_1(asn_TYPE_descriptor_t *td, const void *sptr,
			asn_app_constraint_failed_f *ctfailcb, void *app_key) {
	
	if(!sptr) {
		_ASN_CTFAIL(app_key, td, sptr,
			"%s: value not given (%s:%d)",
			td->name, __FILE__, __LINE__);
		return -1;
	}
	
	
	if(1 /* No applicable constraints whatsoever */) {
		/* Nothing is here. See below */
	}
	
	return td->check_constraints(td, sptr, ctfailcb, app_key);
}

static int
memb_extType_constraint_1(asn_TYPE_descriptor_t *td, const void *sptr,
			asn_app_constraint_failed_f *ctfailcb, void *app_key) {
	
	if(!sptr) {
		_ASN_CTFAIL(app_key, td, sptr,
			"%s: value not given (%s:%d)",
			td->name, __FILE__, __LINE__);
		return -1;
	}
	
	
	if(1 /* No applicable constraints whatsoever */) {
		/* Nothing is here. See below */
	}
	
	return td->check_constraints(td, sptr, ctfailcb, app_key);
}

static asn_per_constraints_t asn_PER_memb_extId_constr_2 = {
	{ APC_UNCONSTRAINED,	-1, -1,  0,  0 },
	{ APC_UNCONSTRAINED,	-1, -1,  0,  0 },
	0, 0	/* No PER value map */
};
static asn_per_constraints_t asn_PER_memb_extType_constr_3 = {
	{ APC_UNCONSTRAINED,	-1, -1,  0,  0 },
	{ APC_UNCONSTRAINED,	-1, -1,  0,  0 },
	0, 0	/* No PER value map */
};
static asn_TYPE_member_t asn_MBR_PrivateExtension_1[] = {
	{ ATF_NOFLAGS, 0, offsetof(struct PrivateExtension, extId),
		(ASN_TAG_CLASS_UNIVERSAL | (6 << 2)),
		0,
		&asn_DEF_OBJECT_IDENTIFIER,
		memb_extId_constraint_1,
		&asn_PER_memb_extId_constr_2,
		0,
		"extId"
		},
	{ ATF_OPEN_TYPE | ATF_POINTER, 1, offsetof(struct PrivateExtension, extType),
		-1 /* Ambiguous tag (ANY?) */,
		0,
		&asn_DEF_ANY,
		memb_extType_constraint_1,
		&asn_PER_memb_extType_constr_3,
		0,
		"extType"
		},
};
static int asn_MAP_PrivateExtension_oms_1[] = { 1 };
static ber_tlv_tag_t asn_DEF_PrivateExtension_tags_1[] = {
	(ASN_TAG_CLASS_UNIVERSAL | (16 << 2))
};
static asn_TYPE_tag2member_t asn_MAP_PrivateExtension_tag2el_1[] = {
    { (ASN_TAG_CLASS_UNIVERSAL | (6 << 2)), 0, 0, 0 } /* extId at 46 */
};
static asn_SEQUENCE_specifics_t asn_SPC_PrivateExtension_specs_1 = {
	sizeof(struct PrivateExtension),
	offsetof(struct PrivateExtension, _asn_ctx),
	asn_MAP_PrivateExtension_tag2el_1,
	1,	/* Count of tags in the map */
	asn_MAP_PrivateExtension_oms_1,	/* Optional members */
	1, 0,	/* Root/Additions */
	-1,	/* Start extensions */
	-1	/* Stop extensions */
};
asn_TYPE_descriptor_t asn_DEF_PrivateExtension = {
	"PrivateExtension",
	"PrivateExtension",
	SEQUENCE_free,
	SEQUENCE_print,
	SEQUENCE_constraint,
	SEQUENCE_decode_ber,
	SEQUENCE_encode_der,
	SEQUENCE_decode_xer,
	SEQUENCE_encode_xer,
	SEQUENCE_decode_uper,
	SEQUENCE_encode_uper,
	0,	/* Use generic outmost tag fetcher */
	asn_DEF_PrivateExtension_tags_1,
	sizeof(asn_DEF_PrivateExtension_tags_1)
		/sizeof(asn_DEF_PrivateExtension_tags_1[0]), /* 1 */
	asn_DEF_PrivateExtension_tags_1,	/* Same as above */
	sizeof(asn_DEF_PrivateExtension_tags_1)
		/sizeof(asn_DEF_PrivateExtension_tags_1[0]), /* 1 */
	0,	/* No PER visible constraints */
	asn_MBR_PrivateExtension_1,
	2,	/* Elements count */
	&asn_SPC_PrivateExtension_specs_1	/* Additional specs */
};

