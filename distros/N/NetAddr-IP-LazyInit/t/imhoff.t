...
use NetAddr::IP::LazyInit qw(Compact);
...
my $sortnum = Compact(map { NetAddr::IP::LazyInit->new($_) } @sortnum);
my $sorttag = Compact(map { NetAddr::IP::LazyInit->new($_) } @sortreg);
my $sortdec = Compact(map { NetAddr::IP::LazyInit->new($_) } @sortdec);
