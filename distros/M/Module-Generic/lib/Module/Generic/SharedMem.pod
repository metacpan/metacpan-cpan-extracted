...
If you provide a number, it will be used as the project id passed to L<IPC::SysV/ftok>.

If you provide a string key, a stable positive 31-bit System V IPC key is derived from the string together with the selected path. This avoids reducing arbitrary string keys to the small project-id space used by C<ftok()> and therefore substantially reduces avoidable key collisions.

Either way, the resulting numeric value is used to create or access a shared memory segment and its semaphore by L</open>.
...
For numeric keys, the serial is created with L<IPC::SysV/ftok>. For string keys, it is derived directly as a stable positive 31-bit System V IPC key from the configured path and string key.
