...
sub read_handle {
    ...
    _read( [ map { chomp; $_ } shift->getlines ], @_ );
}
...
the C<:all> tag can be used to import all of them into your namespace in one
...
result of calling B<credentials_file> (if it is a readable file) or the result
of calling B<config_file> otherwise.

B<read_all> will return the results of parsing all of the content in the source,
for all profiles that may be defined in it.

B<read> will instead return the data I<for a single profile only>. This profile
can be specified as the second argument. If no profile is provided, B<read>
will use the result of calling B<default_profile> as the default.
...
the AWS CLI interface. These can be imported with the C<:aws> tag.
...
Returns the contents of the C<AWS_SHARED_CREDENTIALS_FILE> environment variable,
or C<~/.aws/credentials> if undefined.
...
of L<Config::INI>. The B<read_file>, B<read_string>, and B<read_handle>
functions behave like those described in the documentation for that
distribution. They can be imported with the C<:ini> tag.
