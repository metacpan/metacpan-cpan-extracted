...
1;
# ABSTRACT: Run a Getopt::Long::More-based script but only to dump the spec
...
This document describes version 0.001 of Getopt::Long::More::Dump (from Perl distribution Getopt-Long-More-Dump), released on 2016-12-05.

=head1 FUNCTIONS


=head2 dump_getopt_long_more_script(%args) -> [status, msg, result, meta]

Run a Getopt::Long::More-based script but only to dump the spec.

This function runs a CLI script that uses C<Getopt::Long::More> but
monkey-patches beforehand so that C<GetOptions()> will dump the object and then
exit. The goal is to get the object without actually running the script.
...
CLI script needs to use C<Getopt::Long::More>. This is detected currently by a
simple regex. If script is not detected as using C<Getopt::Long::More>, status
412 is returned.
