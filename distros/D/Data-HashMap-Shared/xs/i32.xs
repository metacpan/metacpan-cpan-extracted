...
freeze(SV* self_sv)
    CODE:
        ...
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::I32->freeze: cannot freeze a read-only handle");
    ...
    CODE:
        ...
        if (h->shard_handles) {
            ...
            for (int i = 1; i < items; i += 2) {
                ...
                int32_t _v = (int32_t)SvIV(ST(i + 1));
                REEXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
                count += shm_i32_put_inner(h, _k, _v, SHM_TTL_USE_DEFAULT);
            }
        }
        RETVAL = count;
    OUTPUT:
    ...
    CODE:
        ...
        if (h->shard_handles) {
            ...
            for (int i = 1; i < items; i++) {
                int32_t _k = (int32_t)SvIV(ST(i));
                REEXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
                count += shm_i32_remove_inner(h, _k);
            }
