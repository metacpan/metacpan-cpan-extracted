...
=item B<-banner> I<bool>

If true, a banner is included at the bottom of the generated
...
sub pod2html {
    ...
    for(qw(-suffix -navigation -localtoc -toc -tocname -toctitle -idx
        -idxname -idxtitle -idxopt -stylesheet -verbose -banner)) {
        $conv_opts{$_} = $opts{$_};
    }
...
sub customize {
    ...
    $self->{_body}->push_content("Generated by Marek::Pod::HTML $VERSION on " . localtime() . "\n")
      if($self->{-banner});
}
