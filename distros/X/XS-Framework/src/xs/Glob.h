...
struct Glob : Scalar {
    ...
    Glob& operator= (const CallProxy& p) { return operator=(p.sv()); }
    ...
    template <typename T = SV> one_of_t<T,SV,GV>* get () const { return (T*)sv; }
    ...
    void slot (const CallProxy& p) { slot(p.scalar()); }
    void slot (GV*)          = delete;
    ...
    std::string_view name           () const { return std::string_view(GvNAME((GV*)sv), GvNAMELEN((GV*)sv)); }
    std::string_view effective_name () const { return std::string_view(GvENAME((GV*)sv), GvENAMELEN((GV*)sv)); }
