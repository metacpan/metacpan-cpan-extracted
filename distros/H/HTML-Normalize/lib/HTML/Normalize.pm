...
C<elements> takes no parameters and returns the list of HTML::Element instances
generated by C<cleanup>. C<elements> should only be called after C<cleanup>. It
will return C<undef> if C<cleanup> was not called or failed.
...
sub elements {
    ...
    while ($root->implicit()) {
        ($root) = grep {$_->tag() ne 'head'} $root->content_list();
        last if $root->tag() eq 'body';
        next;
    }

    return $root->content_list();
}
