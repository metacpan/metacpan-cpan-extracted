...
# `make test'. After `make install' it should work as 'perl 005-captitle-preserve_anycaps.t'
...

{
  foreach my $case (sort keys %{ $basic_test_cases }) {
    ...
    my $result = capitalize_title($case, PRESERVE_ANYCAPS => 1);
...
# Regression test: make sure $_ isn't munged by unlocalized use
{
  ...
  my $keeper = "abc123";
  local $_ = $keeper;
  capitalize_title($anything, PRESERVE_ANYCAPS => 1);
...
sub define_basic_test_cases {
  ...
  my %expect_capitalize_title_PRESERVE_ANYCAPS = (
     'This And ThAt'=>
        'This And ThAt',
     "Revenge is Doom's" =>
        "Revenge Is Doom's",
  ...
  return \%expect_capitalize_title_PRESERVE_ANYCAPS;
}

sub define_basic_test_cases_i18n {
  my %expect_capitalize_title_PRESERVE_ANYCAPS_i18n =
    (
  ...
  return \%expect_capitalize_title_PRESERVE_ANYCAPS_i18n;
}
