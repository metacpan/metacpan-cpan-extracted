...
trap {my $sb_file = SeaBASS::File->new(\$DATA[0], {strict => (STRICT_READ | STRICT_WRITE)});};
is($trap->leaveby, 'return', "bats92 pass");

trap {my $sb_file = SeaBASS::File->new(\$DATA[1], {strict => (STRICT_READ | STRICT_WRITE)});};
like($trap->die, qr/Error\(s\) reading SeaBASS file/, "bare minimum fail");

trap {
    my $sb_file = SeaBASS::File->new(\$DATA[3]);
    is($sb_file->delim, undef, "delim not strict 1");
};
is($trap->leaveby, 'return', "delim not strict trap");

trap {
    my $sb_file = SeaBASS::File->new(\$DATA[3], {strict => (STRICT_READ | STRICT_WRITE)});
};
