...
sub client_input {
  ...
  if ( $cmd eq '331' ) {
    ...
    if ( $heap->{topic} ) {
      $heap->{ircd}->daemon_server_mode('#potato','+c');
    }
    else {
  ...
  if ( $cmd eq 'MODE' ) {
    pass($cmd);
    is( $prefix, 'listen.server.irc', 'It is I, bobbins' );
    is( $params->[0], '#potato', 'Channel is #potato' );
  ...
  if ( $cmd eq '408' ) {
    ...
    $heap->{ircd}->daemon_server_remove('#potato','bobbins','Suckers');
    return;
  }
  if ( $cmd eq 'PART' ) {
    pass($cmd);
    is( $prefix, 'bobbins!~bobbins@listen.server.irc', 'It is I, bobbins' );
    is( $params->[0], '#potato', 'Channel name is #potato' );
    is( $params->[1], 'Enforced PART "Suckers"', 'So long suckers' );
