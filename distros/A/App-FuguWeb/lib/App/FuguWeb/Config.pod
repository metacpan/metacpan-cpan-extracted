...
a second C<keys> block appears, or a C<key> block stands with no
C<keys> block;
...
an absolute URL, so a description that names no C<url> writes no such
field.

=head2 keys_path

    my $dir  = $config->keys_path;
    my $file = $config->keys_path('SHA256');

The source key directory, or one file in it. The method answers
C<undef> when the description holds no C<keys> block.

=head2 site_keys

The C<key> blocks, in file order. Each entry is a hashref:

    name         the key file in the directory
    stem         the block name, which is the name without the
                 extension
    type         signify or openpgp, from the extension
...
Every path of the key directory in the output, relative to the output
directory, or the empty list when the description holds no C<keys>
block. L</inventory> holds the same paths.
