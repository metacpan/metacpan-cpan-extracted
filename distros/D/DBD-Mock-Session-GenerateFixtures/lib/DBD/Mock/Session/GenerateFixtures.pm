...
sub _override_dbi_begin_work {
    my $self              = shift;
    my $mocked_begin_work = shift;

    my $original_begin_work = \&$mocked_begin_work,;
    $self->get_override_object()->replace(
        $mocked_begin_work,
        sub {
            my $dbh = shift;
            ...
            $self->_write_to_file();
            return $original_begin_work->($dbh);
        }
    );
}

sub _override_dbi_commit {
    my $self          = shift;
    my $mocked_commit = shift;

    my $original_commit = \&$mocked_commit;
    $self->get_override_object()->replace(
        $mocked_commit,
        sub {
            my $dbh = shift;
...
sub _override_dbi_rollback {
    my $self            = shift;
    my $mocked_rollback = shift;

    my $original_rollback = \&$mocked_rollback;
    $self->get_override_object()->replace(
        $mocked_rollback,
        sub {
            my $dbh = shift;
...
sub _write_to_file {
    ...
    if ( $override_flag && scalar @{$result} ) {
        ...
        my $fh        = IO::File->new( $fixture_file, 'w' )
          or croak "cannot open file:$fixture_file  $!\n";
        say $fh $json_data;
...
=head1 SYNOPSIS
	...
	# support for begin_work, autocommit and rollback
...
=head2 _override_dbi_begin_work

	_override_dbi_begin_work($dbi_begin_work);

	This method overrides the `DBD::Mock::db::begin` method. It is used for testing purposes to add in fxtures an 'BEGIN WORK' statement.

=head2 _override_dbi_commit

	_override_dbi_commit($commit);

	This method overrides the `DBD::Mock::db::commit` method. It is used for testing purposes to add in fxtures an 'COMMIT' statement.

=head2 _override_dbi_rollback

	_override_dbi_rollback($rollback);

	This method overrides the `DBD::Mock::db::rollback` method. It is used for testing purposes to add in fxtures an 'ROLLBACK' statement.
