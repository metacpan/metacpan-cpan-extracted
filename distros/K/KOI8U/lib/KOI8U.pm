...
#
# http://search.cpan.org/dist/Char-KOI8U/
#
...
perl $0 KOI8-U_script.pl > Escaped_script.pl.e
END
    }

    print KOI8U::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Ekoi8u::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
    ...
    my $fh = gensym();
    Ekoi8u::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub KOI8U::escape_script {
    ...
    # read KOI8-U script
    my $fh = gensym();
    Ekoi8u::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Ekoi8u(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ekoi8u '%s.0'; # 'quote' for perl5.6\n", $KOI8U::VERSION); # require run-time routines version

        # use KOI8U version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | KOI8U ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $KOI8U::VERSION) {
                    my @file = grep -e, map {qq{$_/KOI8U.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects KOI8U.pm $version, but @{[__FILE__]} is version $KOI8U::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Ekoi8u.pm $version, but \\\$Ekoi8u::VERSION is \$Ekoi8u::VERSION" if \$Ekoi8u::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $KOI8U::VERSION) {
                    die "Script $0 required KOI8U.pm $version, but @{[__FILE__]} is only version $KOI8U::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Ekoi8u::function_ord     = 'KOI8U::ord'     if /\A ord \z/oxms;
                    $Ekoi8u::function_ord_    = 'KOI8U::ord_'    if /\A ord \z/oxms;
                    $Ekoi8u::function_reverse = 'KOI8U::reverse' if /\A reverse \z/oxms;
                    $Ekoi8u::function_getc    = 'KOI8U::getc'    if /\A getc \z/oxms;

                    # avoid: used only once: possible typo at ...
                    $Ekoi8u::function_ord     = $Ekoi8u::function_ord;
                    $Ekoi8u::function_ord_    = $Ekoi8u::function_ord_;
                    $Ekoi8u::function_reverse = $Ekoi8u::function_reverse;
                    $Ekoi8u::function_getc    = $Ekoi8u::function_getc;
                }
            }
        }
    }

    $e_script .= KOI8U::escape();
...
=item * You'll have to download KOI8U.pm and Ekoi8u.pm and put it in your perl lib directory.

=item * You'll need to write "use KOI8U;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use KOI8U;
  use KOI8U ver.sion;             --- require minimum version
  use KOI8U ver.sion.0;           --- expects version (match or die)
...
KOI8U software is "middleware" between perl interpreter and your Perl script
written in KOI8-U.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain KOI8-U, Perl3 or later
can treat KOI8-U as binary data.

By "use KOI8U;", it automatically interpret your script as KOI8-U. The various
functions of perl including a regular expression can treat KOI8-U now.
...
This software is a source code filter to escape Perl script encoded by KOI8-U
...
=item * Remains One Language Handling Raw KOI8-U, Doesn't Use UTF8 flag like JPerl
...
import of Ekoi8u.pm works well.
...
=head1 Calling 'Ekoi8u::ignorecase()' (KOI8U software provides)

KOI8U software applies calling 'Ekoi8u::ignorecase()' instead of /i modifier.
...
=head1 Escaping Character Classes (Ekoi8u.pm provides)
...
Definitions in Ekoi8u.pm.
  ...
  ${Ekoi8u::not_lower_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Ekoi8u::not_lower_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
  ${Ekoi8u::not_print}      qr{(?>[^\x20-\x7F])};
  ${Ekoi8u::not_punct}      qr{(?>[^\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E])};
  ${Ekoi8u::not_space}      qr{(?>[^\s\x0B])};
  ${Ekoi8u::not_upper}      qr{(?>[^\x41-\x5A])};
  ${Ekoi8u::not_upper_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Ekoi8u::not_upper_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (KOI8U software provides)

KOI8U software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (KOI8U software provides)

Insert 'Ekoi8u::' at head of function name. Ekoi8u.pm provides your script Ekoi8u::*
subroutines.
...
=head1 Yada Yada Operator (KOI8U software provides)
...
=head1 Un-Escaping bytes::* Subroutines (KOI8U software provides)

KOI8U software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in KOI8U software and
...
KOI8U software remains one language and one interpreter.
