...
our @ISA = qw(Exporter ElmButtonPtr);
...
require XSLoader;
XSLoader::load('pEFL::Elm::Button');

sub add {
    my ($class,$parent) = @_;
    my $widget = elm_button_add($parent);
...
=head1 SYNOPSIS
  ...
  use pEFL::Elm;
  [...]
  my $button = pEFL::Elm::Button->add($parent);
...
For instructions, how to use pEFL::Elm::Button, please study this API reference for now. A perl-specific documentation will perhaps come in later versions. But applying the C documentation should be no problem. pEFL::Elm::Button gives you a nice object-oriented interface that is kept close to the C API. Please note, that the perl method names remove the "elm_button_" at the beginning of the c functions.
