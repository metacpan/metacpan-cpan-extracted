...
=item $obj-E<gt>B<cleanupString>($manual, $string, %options)

Format a single line piece of text.  The C<%options> are passed to L<cleanup()|OODoc::Format/"Page generation">.
...
Cast by C<showRequiredChapter()>

=item Error: no formatter specified

Cast by C<new()>

=item Error: no index specified
