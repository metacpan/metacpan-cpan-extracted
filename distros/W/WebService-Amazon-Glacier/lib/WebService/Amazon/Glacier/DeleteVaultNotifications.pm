use strict;
package WebService::Amazon::Glacier::DeleteVaultNotifications;
{
  $WebService::Amazon::Glacier::DeleteVaultNotifications::VERSION = '0.001';
}
...
sub run {
    ...
    try{
	$self->_delete_vault_notifications();
...
sub _delete_vault_notifications{
    my $self=shift;
    
    my $hr=HTTP::Request->new('DELETE',"https://glacier.".$self->get_region().".amazonaws.com/".$self->get_AccountID()."/vaults/".$self->get_vaultname()."/notification-configuration", [ 
...
WebService::Amazon::Glacier::DeleteVaultNotifications
...
=head2 _delete_vault_notifications

Requires vaultname to be set.  Deletes the selected vault
notifications.  Throws an exception if it can't be created for any
reason.
