...
perl $0 KOI8-R_script.pl > Escaped_script.pl.e
END
    }

    print KOI8R::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Ekoi8r::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
    ...
    my $fh = gensym();
    Ekoi8r::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub KOI8R::escape_script {
    ...
    # read KOI8-R script
    my $fh = gensym();
    Ekoi8r::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Ekoi8r(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ekoi8r '%s.0'; # 'quote' for perl5.6\n", $KOI8R::VERSION); # require run-time routines version

        # use KOI8R version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | KOI8R ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $KOI8R::VERSION) {
                    my @file = grep -e, map {qq{$_/KOI8R.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects KOI8R.pm $version, but @{[__FILE__]} is version $KOI8R::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Ekoi8r.pm $version, but \\\$Ekoi8r::VERSION is \$Ekoi8r::VERSION" if \$Ekoi8r::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $KOI8R::VERSION) {
                    die "Script $0 required KOI8R.pm $version, but @{[__FILE__]} is only version $KOI8R::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Ekoi8r::function_ord     = 'KOI8R::ord'     if /\A ord \z/oxms;
                    $Ekoi8r::function_ord_    = 'KOI8R::ord_'    if /\A ord \z/oxms;
                    $Ekoi8r::function_reverse = 'KOI8R::reverse' if /\A reverse \z/oxms;
                    $Ekoi8r::function_getc    = 'KOI8R::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= KOI8R::escape();
...
=item * You'll have to download KOI8R.pm and Ekoi8r.pm and put it in your perl lib directory.

=item * You'll need to write "use KOI8R;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use KOI8R;
  use KOI8R ver.sion;             --- require minimum version
  use KOI8R ver.sion.0;           --- expects version (match or die)

  # "no KOI8R;" not supported

  or

  $ perl KOI8R.pm KOI8-R_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  KOI8-R_script.pl  --- script written in KOI8-R
...
KOI8R software is "middleware" between perl interpreter and your Perl script
written in KOI8-R.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain KOI8-R, Perl3 or later
can treat KOI8-R as binary data.

By "use KOI8R;", it automatically interpret your script as KOI8-R. The various
functions of perl including a regular expression can treat KOI8-R now.
...
This software is a source code filter to escape Perl script encoded by KOI8-R
...
=item * Remains One Language Handling Raw KOI8-R, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. KOI8R.pm, Ekoi8r.pm
  +---------------------------------------+
...
import of Ekoi8r.pm works well.
...
=head1 Software Composition
   ...
   KOI8R.pm               --- source code filter to escape KOI8-R
   Ekoi8r.pm              --- run-time routines for KOI8R.pm
...
You need write 'use KOI8R;' in your script.
...
=head1 Calling 'Ekoi8r::ignorecase()' (KOI8R software provides)

KOI8R software applies calling 'Ekoi8r::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Ekoi8r::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Ekoi8r.pm provides)
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (KOI8R software provides)

KOI8R software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (KOI8R software provides)

Insert 'Ekoi8r::' at head of function name. Ekoi8r.pm provides your script Ekoi8r::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Ekoi8r::lc        Character
  lcfirst     Ekoi8r::lcfirst   Character
  uc          Ekoi8r::uc        Character
  ucfirst     Ekoi8r::ucfirst   Character
  fc          Ekoi8r::fc        Character
  chr         Ekoi8r::chr       Character
  glob        Ekoi8r::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (KOI8R software provides)
...
=head1 Un-Escaping bytes::* Subroutines (KOI8R software provides)

KOI8R software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Ekoi8r.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in KOI8R software and
...
KOI8R software remains one language and one interpreter.
