...
sub optimize {
	...
	if (int(grep { !$self->is_processed($_) } @ops) > 0) {
		# This is true, if any of the arguments are true.
		return 1 if (int(grep { $self->is_processed($_) && $_ } @ops) > 0);
		# All processed arguments, then should be eliminated, because they must be false.
