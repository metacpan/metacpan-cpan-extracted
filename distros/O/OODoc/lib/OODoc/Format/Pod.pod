...
Extends L<"Collected" in OODoc::Format|OODoc::Format/"Collected">.
...
The C<%options> are a collection of all options available to C<show*()> methods.
...
Create the text for a link which refers to the C<$object>.  The link will be
shown somewhere in the C<$manual>.  The C<$text> will be displayed is stead
...
The POD is produced in the specified C<$in> filename, but may contain some
...
L<createManual()|OODoc::Format::Pod/"Page generation"> or L<OODoc::formatter()|OODoc/"Actions">
...
=head4 » Example: appending text to a page
...
=head4 » Example: remove chapter inheritance
...
=head4 » Example: changing the chapter's output
...
=head4 » Example: adding to a chapter's output
...
When using 'pod2' in stead of 'pod' when L<OODoc::formatter()|OODoc/"Actions"> is called,
...
=head4 » Example: formatting with template
...
=head4 » Example: format options within template
...
=item Error: cannot link to a $package

Cast by C<link()>

=item Fault: cannot read prelimary pod from $file: $!

Cast by C<simplifyPod()>

=item Fault: cannot write final pod to $file: $!

Cast by C<simplifyPod()>

=item Fault: cannot write prelimary pod manual to $file: $!

Cast by C<createManual()>
...
=item Fault: write to $file failed: $!
