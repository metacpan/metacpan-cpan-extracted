...
$_results = $index->SearchByBounds( \%bad_lats );
is( $_results, undef, "Bad latitudes" );

$_results = $index->SearchByBounds( \%out_of_range );
is( $_results, undef, "Latitude or longitude out of range" );
...
$_results = $index->SearchByBounds( \%max_range );
@expected = sort @$_all;
@results = sort map { $_->{name}; } @$_results;
is_deeply( \@results, \@expected, "Max range (all points on globe)" );



$_results = $index->SearchByBounds( \%max_range, { quick_results=>1 } );
@expected = sort @$_all;
@results = ();
...
is_deeply( \@results, \@expected, "Max range (all points on globe), quick_results=>1" );



$_results = $index->SearchByBounds( \%max_range, { max_results=>5 } );
is( scalar @$_results, 5, "Max range (all points on globe), max_results=5" );



my $_europe = [ "Algiers, Algeria", "Amsterdam / The Hague (seat of Government), Netherlands", "Andorra la Vella, Andorra", "Ankara, Turkey", "Athens, Greece", 
                "Belgrade, Yugoslavia", "Berlin, Germany", "Bern, Switzerland", "Bratislava, Slovakia", "Brussels, Belgium", "Bucuresti, Romania", "Budapest, Hungary", "Chisinau, Moldova, Republic of", "Copenhagen, Denmark", "Dublin, Ireland", 
                "Helsinki, Finland", "Kiev, Ukraine", "Lisbon, Portugal", "Ljubljana, Slovenia", "London, United Kingdom of Great Britain and Northern Ireland", "Luxembourg, Luxembourg", "Madrid, Spain", "Minsk, Belarus", "Oslo, Norway", 
                "Paris, France", "Prague, Czech Republic", "Reykjavik, Iceland", "Riga, Latvia", "Rome, Italy", "San Marino, San Marino", "Sarajevo, Bosnia and Herzegovina", "Skopje, Macedonia (Former Yugoslav Republic)", "Sofia, Bulgaria", 
                "St. Peter Port, Guernsey", "Stockholm, Sweden", "Tallinn, Estonia", "Tirane, Albania", "Torshavn, Faroe Islands", "Tunis, Tunisia", "Vaduz, Liechtenstein", "Vienna, Austria", "Vilnius, Lithuania", "Warsaw, Poland", "Zagreb, Croatia" ];
$_results = $index->SearchByBounds( \%europe );
@expected = sort @$_europe;
@results = sort map { $_->{name}; } @$_results;
is_deeply( \@results, \@expected, "Europe (lat>0, lon>0)" );



$_results = $index->SearchByBounds( \%europe, { tile_adjust=>1 } );
@expected = sort @$_europe;
@results = sort map { $_->{name}; } @$_results;
is_deeply( \@results, \@expected, "Europe (lat>0, lon>0), tile_adjust=>1" );
...
my $_europe_a = [ "Algiers, Algeria", "Amsterdam / The Hague (seat of Government), Netherlands", "Andorra la Vella, Andorra", "Ankara, Turkey", "Athens, Greece" ];
$_results = $index->SearchByBounds( \%europe, { condition=>sub { return ($_[0]->{name} =~ /$_[2]/); }, user_data=>'^A' } );
@expected = sort @$_europe_a;
@results = sort map { $_->{name}; } @$_results;
is_deeply( \@results, \@expected, "Europe (lat>0, lon>0), condition" );


my $_caribbean = [ "Basse-Terre, Guadeloupe", "Basseterre, Saint Kitts and Nevis", "Bridgetown, Barbados", "Caracas, Venezuela", "Castries, Saint Lucia", "Charlotte Amalie, United States Virgin Islands", "Fort-de-France, Martinique", 
                   "George Town, Cayman Islands", "Havana, Cuba", "Kingston, Jamaica", "Kingstown, Saint Vincent and the Greeadines", "Managua, Nicaragua", "Nassau, Bahamas", "Oranjestad, Aruba", "Port-au-Prince, Haiti", 
                   "Road Town, British Virgin Islands", "Roseau, Dominica", "San Juan, Puerto Rico", "Santo Domingo, Dominica Republic", "West Indies, Antigua and Barbuda", "Willemstad, Netherlands Antilles" ];
$_results = $index->SearchByBounds( \%carribean );
@expected = sort @$_caribbean;
@results = sort map { $_->{name}; } @$_results;
is_deeply( \@results, \@expected, "Caribbean (lat>0, lon<0)" );



my $_africa = [ "Abu Dhabi, United Arab Emirates", "Abuja, Nigeria", "Accra, Ghana", "Addis Ababa, Ethiopia", "Algiers, Algeria", "Amman, Jordan", "Antananarivo, Madagascar", "Asmara, Eritrea", "Athens, Greece", "Baghdad, Iraq", "Bamako, Mali", 
                "Bangui, Central African Republic", "Banjul, Gambia", "Beirut, Lebanon", "Bissau, Guinea-Bissau", "Brazzaville, Congo", "Bujumbura, Burundi", "Cairo, Egypt", "Conakry, Guinea", "Dakar, Senegal", "Damascus, Syrian Arab Republic", 
                "Djibouti, Djibouti", "Dodoma, United Republic of Tanzania", "Doha, Qatar", "Freetown, Sierra Leone", "Gaborone, Botswana", "Harare, Zimbabwe", "Jerusalem, Israel", "Kampala, Uganda", "Khartoum, Sudan", "Kigali, Rawanda", 
                "Kinshasa, Democratic Republic of the Congo", "Kuwait, Kuwait", "Libreville, Gabon", "Lilongwe, Malawi", "Lome, Togo", "Luanda, Angola", "Lusaka, Zambia", "Malabo, Equatorial Guinea", "Mamoudzou, Mayotte", "Manama, Bahrain", 
                "Maputo, Mozambique", "Maseru, Lesotho", "Mbabane (administrative), Swaziland", "Mogadishu, Somalia", "Monrovia, Liberia", "Moroni, Comros", "N'Djamena, Chad", "Nairobi, Kenya", "Niamey, Niger", "Nicosia, Cyprus", 
                "Ouagadougou, Burkina Faso", "Porto Novo (constitutional) / Cotonou (seat of government), Benin", "Pretoria (administrative) / Cape Town (legislative) / Bloemfontein (judicial), South Africa", "Riyadh, Saudi Arabia", 
                "Sao Tome, Sao Tome and Principe", "Tehran, Iran (Islamic Republic of)", "Tripoli, Libyan Arab Jamahiriya", "Tunis, Tunisia", "Valletta, Malta", "Windhoek, Namibia", "Yamoussoukro, Cote d'Ivoire", "Yaounde, Cameroon" ];
$_results = $index->SearchByBounds( \%africa );
@expected = sort @$_africa;
@results = sort map { $_->{name}; } @$_results;
is_deeply( \@results, \@expected, "Africa (straddles equator and prime meridian)" );



my $_aus_nz = [ "Canberra, Australia", "Hobart, Tasmania, Australia", "Kingston, Norfolk Island", "Noumea, New Caledonia", "Port-Vila, Vanuatu", "Suva, Fiji", "Wellington, New Zealand" ];
$_results = $index->SearchByBounds( \%aus_nz );
@expected = sort @$_aus_nz;
@results = sort map { $_->{name}; } @$_results;
is_deeply( \@results, \@expected, "Australia and New Zealand (lat < 0, straddles antimeridian)" );


my $_antarctica = [ "McMurdo Station, Antarctica", "South pole, Antarctica" ];
$_results = $index->SearchByBounds( \%antarctica );
@expected = sort @$_antarctica;
@results = sort map { $_->{name}; } @$_results;
is_deeply( \@results, \@expected, "Antarctica (south polar region)" );



my $_arctic = [ "Helsinki, Finland", "North pole, Arctic", "Nuuk, Greenland", "Reykjavik, Iceland", "Svalbard, Norway", "Torshavn, Faroe Islands" ];
$_results = $index->SearchByBounds( \%arctic );
@expected = sort @$_arctic;
@results = sort map { $_->{name}; } @$_results;
is_deeply( \@results, \@expected, "Arctic (north polar region)" );
