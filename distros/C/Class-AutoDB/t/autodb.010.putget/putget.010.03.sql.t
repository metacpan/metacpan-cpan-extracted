########################################
# retrieve objects stored by 010.00 using get & find with sql arg. also tests count
########################################
use t::lib;
use strict;
use Carp;
...
my @objects=($joe,$mary,$jane,$barb);
$test->test_get(labelprefix=>"$get_type with SQL argument",get_type=>$get_type,
		get_args=>{sql=>qq(SELECT oid FROM Person WHERE name='Joe' OR sex='F')},
		correct_objects=>\@objects);
my @objects=($jane);
$test->test_get(labelprefix=>"$get_type with search key and SQL arguments",get_type=>$get_type,
		get_args=>{collection=>'Person',sex=>'F',
			   sql=>qq(SELECT Student.oid FROM Student, Place
                                   WHERE Student.school=Place.oid AND Place.name='MIT')},
		correct_objects=>\@objects);
