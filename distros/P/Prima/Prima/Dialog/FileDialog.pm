...
sub create
{
	...
	if ( $params{system} && ( $win32 || $gtk))  {
		...
		if ( $class =~ /^Prima::Dialog::(Open|Save|File)Dialog$/) {
			undef $@;
			eval "use Prima::sys::${sys}::FileDialog";
			die $@ if $@;
			$class =~ s/(Prima)::Dialog/$1::sys::$sys/;
...
package Prima::Dialog::OpenDialog;
use vars qw( @ISA);
@ISA = qw(Prima::Dialog::FileDialog);
...
package Prima::Dialog::SaveDialog;
use vars qw( @ISA);
@ISA = qw(Prima::Dialog::FileDialog);
...
package Prima::Dialog::ChDirDialog;
use vars qw(@ISA);
@ISA = qw(Prima::Dialog);
