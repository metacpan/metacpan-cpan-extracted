...
Sah::Schema::phone::num::int - International phone number, e.g. +628123456789

=head1 VERSION

This document describes version 0.001 of Sah::Schema::phone::num::int (from Perl distribution Sah-Schemas-Phone), released on 2022-09-23.
...
=head2 Sample data and validation results against this schema
 ...
 "08123456789"  # INVALID (No country code)
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("phone::num::int*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("phone::num::int", {return_type=>'str_errmsg'});
 ...
 $data = "";
 my $errmsg = $validator->($data); # => "Invalid phone number "

Often a schema has coercion rule or default value, so after validation the
validated value is different. To return the validated (set-as-default, coerced,
prefiltered) value:

 my $validator = gen_validator("phone::num::int", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = "";
 my $res = $validator->($data); # => ["Invalid phone number ",""]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("phone::num::int*");
     $validator->(\@args);
     ...
 }
