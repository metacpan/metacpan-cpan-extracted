...
our $template = XML::XPathScript::Stylesheet::DocBook2LaTeX->new;
...
sub tc_text {
	...
	foreach my $axis (qw(preceding-sibling following-sibling)) {
		$neighbours++ if grep {
			is_text_node($_) ?
			  (xpath_to_string($_) =~ m/\S/) :
				is_element_node($_)
		} (findnodes("${axis}::node()",$self));
	};
...
sub tc_varlisentry {
	my ($self,$t)=@_;
	my $header=join(", ",map {apply_templates_under($_) }
					(findnodes("term",$self)));
...
sub verbatimcode {
	...
	if (findvalue("name(..)", $self) =~ m/^(programlisting|screen)co$/) {
...
sub tc_para {
	...
	my ($brother)=findnodes("following-sibling::*[1]", $self);
	$t->{post}=($brother ? "\n\n": undef );
	return 1;
};
...
sub tc_section {
    ...
    my $titleabbrev = apply_templates_under( findnodes('titleabbrev',$n) )
                        || $title ;
...
sub id2label {
	my ($node,$attrname)=@_;
	$attrname ||= "id";
	my ($attr)=findnodes("\@$attrname",$node);
...
PREAMBLE
	...
	foreach my $pos (all_paths_for_header_element("title")) {
		last if (($titlenode)=findnodes($pos,$self));
	};
...
TITLE
   ...
   $t->{pre}.= ( $firstpage? $firstpage : $maketitle ? (&$maketitle($title,\@subtitles,$author)) :
	...
	foreach my $subnode (all_paths_for_header_element("abstract"),
						 ...
						 "epigraph") {
		if (my ($node)=findnodes($subnode, $self)) {
			$t->{pre}.=apply_templates($node);
		}
	}
...
sub apply_templates_under {
	...
	my @subnodes=findnodes($xpath, $node);
	@subnodes=grep {
		my $n=$_;
		(is_element_node($n) || is_text_node($n));
	} (@subnodes);

	return "" unless @subnodes;
	return apply_templates(@subnodes);
}
...
sub render_authors {
	...
	if (ref($authors[0])) { # Not quite the same meaning
		...
		if ($nodename =~ m/^(?:artheader|(article|book)(info)?)/) {
			...
			@authors=map {apply_templates($_) }
			  (map { findnodes($_, $self) } @paths);
		}
	}
