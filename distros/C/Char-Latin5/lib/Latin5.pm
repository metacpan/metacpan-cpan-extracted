...
=item * You'll have to download Latin5.pm and Elatin5.pm and put it in your perl lib directory.

=item * You'll need to write "use Latin5;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Latin5;
  use Latin5 ver.sion;             --- require minimum version
  use Latin5 ver.sion.0;           --- expects version (match or die)

  # "no Latin5;" not supported

  or

  $ perl Latin5.pm Latin-5_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Latin-5_script.pl  --- script written in Latin-5
...
Latin5 software is "middleware" between perl interpreter and your Perl script
written in Latin-5.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Latin-5, Perl3 or later
can treat Latin-5 as binary data.

By "use Latin5;", it automatically interpret your script as Latin-5. The various
functions of perl including a regular expression can treat Latin-5 now.
...
This software is a source code filter to escape Perl script encoded by Latin-5
...
=item * Remains One Language Handling Raw Latin-5, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Latin5.pm, Elatin5.pm
  +---------------------------------------+
...
import of Elatin5.pm works well.
...
=head1 Software Composition
   ...
   Latin5.pm               --- source code filter to escape Latin-5
   Elatin5.pm              --- run-time routines for Latin5.pm
...
You need write 'use Latin5;' in your script.
...
=head1 Calling 'Elatin5::ignorecase()' (Latin5 software provides)

Latin5 software applies calling 'Elatin5::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Elatin5::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Elatin5.pm provides)
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Latin5 software provides)

Latin5 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Latin5 software provides)

Insert 'Elatin5::' at head of function name. Elatin5.pm provides your script Elatin5::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Elatin5::lc        Character
  lcfirst     Elatin5::lcfirst   Character
  uc          Elatin5::uc        Character
  ucfirst     Elatin5::ucfirst   Character
  fc          Elatin5::fc        Character
  chr         Elatin5::chr       Character
  glob        Elatin5::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Latin5 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Latin5 software provides)

Latin5 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Elatin5.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Latin5 software and
...
Latin5 software remains one language and one interpreter.
