...
The C<discard-is-fallback> adverb indicates ESLIF that C<:discard> should not be tried whenever scan/resume is entered. The default is the safe implementation, i.e. it is always tried, and no grammar terminal will match if C<:discard> matches an equal number of bytes or longer. Nevertheless, if you are sure that the grammar's C<:discard> cannot conflict with any other grammar terminal, then C<:discard>, if successful, will always hit as a fallback alternative. The C<discard-is-fallback> enforces this mode, providing a safe performance improvement for grammars that has no conflict between C<:discard> and any other grammar terminal.
...
Lua actions are loaded dynamcally and can have two forms:
