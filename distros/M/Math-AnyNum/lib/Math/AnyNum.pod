...
holding a complex number, such as C<"3+4i">.
...
=head2 faulhaber_sum
    ...
    faulhaber_sum(n, p)                           #=> Int | NaN

Computes the sum C<1**p + 2**p + 3**p +...+ n**p>, using Faulhaber's formula.

C<p> must be a non-negative integer. Returns NaN otherwise.
...
Example:
    ...
    say ipolygonal_root2($n, 5);                   # integer second pentagonal root
