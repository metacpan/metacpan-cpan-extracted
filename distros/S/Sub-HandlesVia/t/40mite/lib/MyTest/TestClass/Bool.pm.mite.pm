...
sub new {
    ...
    my $no_build = delete $args->{__no_BUILD__};
        ...
        # Attribute: attr
    do { my $value = exists( $args->{"attr"} ) ? $args->{"attr"} : do { my $method = $MyTest::TestClass::Bool::__attr_DEFAULT__; $self->$method }; (!ref $value and (!defined $value or $value eq q() or $value eq '0' or $value eq '1')) or MyTest::Mite::croak "Type check failed in constructor: %s should be %s", "attr", "Bool"; $self->{"attr"} = $value; }; 
