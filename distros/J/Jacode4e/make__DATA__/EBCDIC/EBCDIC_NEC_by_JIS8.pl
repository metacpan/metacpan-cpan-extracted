...
for my $jis (sort keys %jis2ebcdic_ne) {
    if ($jis2ebcdic_nec{$jis} ne $jis2ebcdic_ne{$jis}) {
        die "jis($jis): ($jis2ebcdic_nec{$jis}) ne ($jis2ebcdic_ne{$jis})";
    }
}
...
sub keys_of_EBCDIC_NEC_by_JIS8 {
    return keys %EBCDIC_NEC_by_JIS8;
}

sub values_of_EBCDIC_NEC_by_JIS8 {
    return values %EBCDIC_NEC_by_JIS8;
}
