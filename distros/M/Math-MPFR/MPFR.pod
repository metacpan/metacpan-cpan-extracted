...
=head1 FUNCTIONS
   ...
   Where a function is documented as returning "$si" it is (unless
   otherwise stated) returning a ternary value where:
    $si == 0 indicates that the function's result was exact;
    $si < 0 indicates that the function's result was rounded to a
   ...
   $si = Rmpfr_set_IV($rop, $iv, $rnd); # $iv is $Config{ivtype}
   $si = Rmpfr_set_d ($rop, $double, $rnd);
   $si = Rmpfr_set_ld($rop, $ld, $rnd); # long double
   $si = Rmpfr_set_NV($rop, $nv, $rnd); # $nv is $Config{nvtype}
   ...
   $si = Rmpfr_set_FLOAT128($rop, $F128, $rnd);# mpfr-4.0.0 and later
    ...
    NOTE:
         Rmpfr_set_IV requires that $iv has it's IOK flag set, and
   ...
   $si = Rmpfr_strtofr($rop, $str, $base, $rnd);
   ...
   ($rop, $si) = Rmpfr_init_set_ld        ($longdouble, $rnd);
   ($rop, $si) = Rmpfr_init_set_ld_nobless($longdouble, $rnd);
   ($rop, $si) = Rmpfr_init_set_float128        ($float128, $rnd);
   ($rop, $si) = Rmpfr_init_set_float128_nobless($float128, $rnd);
   ...
   ($rop, $si) = Rmpfr_init_set_IV        ($IV,$rnd);# $IV is $Config{ivtype}
   ($rop, $si) = Rmpfr_init_set_IV_nobless($IV,$rnd);# $IV is $Config{ivtype}
   ($rop, $si) = Rmpfr_init_set_IV        ($NV,$rnd);# $NV is $Config{nvtype}
   ($rop, $si) = Rmpfr_init_set_IV_nobless($NV,$rnd);# $NV is $Config{nvtype}
   ...
   $si = Rmpfr_pow_uj($rop, $op, $uj, $rnd);
   $si = Rmpfr_pow_si($rop, $op, $si, $rnd);
   $si = Rmpfr_pow_sj($rop, $op, $sj, $rnd);
   $si = Rmpfr_pown($rop, $op, $sj, $rnd); # same as Rmpfr_pow_sj
   $si = Rmpfr_ui_pow_ui($rop, $ui, $ui, $rnd);
   $si = Rmpfr_ui_pow($rop, $ui,  $op,  $rnd);
   $si = Rmpfr_pow   ($rop, $op1, $op2, $rnd);
   $si = Rmpfr_pow_z ($rop, $op,  $z,   $rnd); # $z is a mpz object

   $si = Rmpfr_pow_IV($rop, $op,  $IV,  $rnd); # $IV is $Config{ivtype}
   ...
   $si = Rmpfr_cmp_NV($op, $nv); # $nv is $Config{nvtype}
    ...
    NOTE:
         Rmpfr_cmp_IV() requires that the 2nd argument has its IOK
         flag set, and Rmpfr_cmp_NV() requires that the 2nd
   ...
   $si = Rmpfr_total_order_p($op1, $op2); # new with mpfr-4.1.0
   ...
   $si = Rmpfr_cosu($rop, $op, $ui, $rnd); # mpfr-4.2.0 and later only
   $si = Rmpfr_sinu($rop, $op, $ui, $rnd); # mpfr-4.2.0 and later only
   $si = Rmpfr_tanu($rop, $op, $ui, $rnd); # mpfr-4.2.0 and later only
   ...
   $si = Rmpfr_acosu($rop, $op, $ui, $rnd); # mpfr-4.2.0 and later only
   $si = Rmpfr_asinu($rop, $op, $ui, $rnd); # mpfr-4.2.0 and later only
   $si = Rmpfr_atanu($rop, $op, $ui, $rnd); # mpfr-4.2.0 and later only
   ...
   $si = Rmpfr_cospi($rop, $op, $rnd); # mpfr-4.2.0 and later only
   $si = Rmpfr_sinpi($rop, $op, $rnd); # mpfr-4.2.0 and later only
   $si = Rmpfr_tanpi($rop, $op, $rnd); # mpfr-4.2.0 and later only
   ...
   $si = Rmpfr_acospi($rop, $op, $rnd); # mpfr-4.2.0 and later only
   $si = Rmpfr_asinpi($rop, $op, $rnd); # mpfr-4.2.0 and later only
   $si = Rmpfr_atanpi($rop, $op, $rnd); # mpfr-4.2.0 and later only
   ...
   $si = Rmpfr_atan2u($rop, $op1, $op2, $ui, $rnd);
    Same as taking the Rmpfr_atan2() result and multiplying it
    by $ui/(2*Pi).

   $si = Rmpfr_atan2pi($rop, $op1, $op2, $rnd);
   ...
   $si = Rmpfr_fac_ui($rop, $ui, $rnd);
    Set $rop to the factorial of $ui, rounded to the direction
    $rnd with the precision of $rop.

   $si = Rmpfr_log1p  ($rop, $op, $rnd);
   $si = Rmpfr_log2p1 ($rop, $op, $rnd);
   $si = Rmpfr_log10p1($rop, $op, $rnd);
   ...
   $si = Rmpfr_expm1  ($rop, $op, $rnd);
   $si = Rmpfr_exp2m1 ($rop, $op, $rnd);
   $si = Rmpfr_exp10m1($rop, $op, $rnd);
    In Rmpfr_expm1, set $rop to the exponential of $op followed
    by a subraction of 1. $rop = (e  ** $op) - 1.
   ...
   $si2 = Rmpfr_check_range($op, $si1, $rnd);
    ...
    range, and that $si1 is the corresponding ternary value. Thus $si1
    ...
    $si1 may be used to avoid a double rounding. This function returns
    ...
    Note: If $op is an infinity and $si1 is different from zero (i.e.,
   ...
   $mask = nvtoa_test($str, $arg, $debug); # $debug can be omitted.
    ...
    Secondly, we chop the least significant digit off the mantissa of
    $str and add 2 to $mask if this chopped $str assigns to a value
    less than $arg.
    Thirdly, we increment the least siginificant mantissa digit of this
    chopped $str, and add 4 to $mask if this chopped-and-incremented
    $str assigns to a value greater than $arg.
    $mask is then returned. If all is correct, it's value will be 7.
    If the value of the returned $mask is not 7, then we can determine
    which part(s) of the test has failed by examining $mask's three
    lowest bits.

    If a third (debug) arg is provided and is TRUE, then some debug
    info is provided during the runnng of nvtoa_test().

    For inf, nan, and zero, we do only the initial test and return 7 if
    that test has passed, or 0 if that test has failed. (The other 2
   ...
   $ret = decimalize($op [, $anything]); # 2nd (optional) arg
    ...
    representation", as estmated by decimalize(). This value could be
   ...
   $si = Rmpfr_fmod_ui($rop, $op1, $ui, $rnd);
   ...
   $bytes = Math::MPFR::bytes($val, $bits);
    ...
    as an extended precison 10-byte long double.
    ...
    as the quad precison 16-byte IEEE long double (or __float128,
