...
# RCS:      $Id: instance.pl,v 1.2 2000/02/08 02:33:45 winters Exp $
#
...
eval {
  ...
  $person = new Persistent::File('data/people.txt', '|');
  ...
  print "------------------------------------------------\n";
  $person->restore_where("lastname = 'Flintstone' and telnum =~ /^[(]?650/");
  while ($person->restore_next()) {
    print "Restored: ";  print_person($person);
  }
  print "\n";
  ...
  unlink <data/people.dbm.*>;

  ### allocate new object and define attributes ###
  my $new_person = new Persistent::DBM('data/people.dbm');
  ...
  $person->restore_all('lastname asc, firstname, telnum');
  while ($person->restore_next()) {
