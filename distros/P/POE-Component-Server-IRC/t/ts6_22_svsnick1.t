...
sub client_connected {
  ...
  pass($_[STATE]);
  $kernel->post( $sender, 'send_to_server', { command => 'CAP',  params => [ 'REQ', 'account-notify extended-join' ], colonify => 0 } );
...
sub client_input {
  ...
  if ( $cmd eq 'ACCOUNT' ) {
    pass($cmd);
    is($prefix,'custard!~custard@listen.server.irc','custard!~custard@listen.server.irc');
    is($params->[0], 'custard', 'Account is custard');
    return;
  }
  if ( $cmd eq 'JOIN' ) {
    pass($cmd);
    if ( $prefix =~ m{^rubbarb!} ) {
      is( $params->[1], 'rubbarb', 'Account is rubbarb' );
      return;
    }
    if ( $prefix =~ m{^custard!} ) {
      is( $params->[1], '*', 'Account is *' );
      return;
    }
    return;
  }
...
sub client2_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^rubbarb' && $params->[1] eq '+i' ) {
    pass($cmd);
    $poe_kernel->post( $sender, 'send_to_server', { command => 'PRIVMSG', params => [ 'NickServ', 'IDENTIFY foobar' ] } );
    return;
  }
  if ( $cmd eq 'MODE' && $prefix =~ m'^rubbarb' && $params->[1] ne '+i' ) {
    pass($cmd);
    return;
  }
  if ( $cmd eq '366' ) {
...
sub client3_input {
  ...
  if ( $cmd eq 'NICK' && $prefix =~ m'^custard' ) {
    pass($cmd);
...
sub groucho_input {
  ...
  if ( $cmd eq 'PRIVMSG' && $prefix eq '1FUAAAAAB' ) {
      pass($cmd);
      my $ts = $heap->{ircd}{state}{uids}{$prefix}{ts};
      $poe_kernel->post($sender, 'send_to_server', { prefix => '4AK', command => 'SVSMODE', params => [ $prefix, $ts, '+dr', 'rubbarb' ] } );
      $poe_kernel->post($sender, 'send_to_server', { prefix => '4AKAAAAAA', command => 'NOTICE', params => [ $prefix, 'You are identified' ] } );
      return;
  }
  if ( $cmd eq 'PRIVMSG' && $prefix eq '1FUAAAAAC' ) {
      pass($cmd);
      my $ts = $heap->{ircd}{state}{uids}{$prefix}{ts};
      $poe_kernel->post($sender, 'send_to_server', { prefix => '4AK', command => 'SVSMODE', params => [ $prefix, $ts, '+dr', 'custard' ] } );
      $poe_kernel->post($sender, 'send_to_server', { prefix => '4AKAAAAAA', command => 'NOTICE', params => [ $prefix, 'You are identified' ] } );
      return;
  }
  return;
