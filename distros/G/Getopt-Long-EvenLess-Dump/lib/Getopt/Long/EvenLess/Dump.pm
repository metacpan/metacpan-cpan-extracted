package Getopt::Long::EvenLess::Dump;
...
our @EXPORT_OK = qw(dump_getopt_long_evenless_script);

our %SPEC;

$SPEC{dump_getopt_long_evenless_script} = {
    v => 1.1,
    summary => 'Run a Getopt::Long::EvenLess-based script but only to '.
        'dump the spec',
    description => <<'_',
...
CLI script needs to use `Getopt::Long::EvenLess`. This is detected currently by
a simple regex. If script is not detected as using `Getopt::Long::EvenLess`,
...
sub dump_getopt_long_evenless_script {
    ...
    my $detres;
    if ($args{skip_detect}) {
        $detres = [200, "OK (skip_detect)", 1, {"func.module"=>"Getopt::Long::EvenLess", "func.reason"=>"skip detect, forced"}];
    } else {
        $detres = Getopt::Long::Util::detect_getopt_long_script(
            filename => $filename);
        return $detres if $detres->[0] != 200;
        return [412, "File '$filename' is not script using Getopt::Long::EvenLess (".
    ...
    my @cmd = (
        $^X, (map {"-I$_"} @$libs),
        "-MGetopt::Long::EvenLess::Patch::DumpAndExit=-tag,$tag",
        $filename,
        "--version",
    );
    my ($stdout, $stderr, $exit) = Capture::Tiny::capture(
        sub { local $ENV{GETOPT_LONG_EVENLESS_DUMP} = 1; system @cmd },
    );
    ...
    if ($stdout =~ /^# BEGIN DUMP $tag\s+(.*)^# END DUMP $tag/ms) {
        ...
        if ($@) {
            return [500, "Script '$filename' looks like using ".
                        "Getopt::Long::EvenLess, but I got an error in eval-ing ".
                            "captured option spec: $@, raw capture: <<<$1>>>"];
        }
        if (ref($spec) ne 'HASH') {
            return [500, "Script '$filename' looks like using ".
                        "Getopt::Long::EvenLess, but I didn't get a hash option spec, ".
                            "raw capture: stdout=<<$stdout>>"];
        }
    } else {
        return [500, "Script '$filename' looks like using Getopt::Long::EvenLess, ".
...
1;
# ABSTRACT: Run a Getopt::Long::EvenLess-based script but only to dump the spec
...
Getopt::Long::EvenLess::Dump - Run a Getopt::Long::EvenLess-based script but only to dump the spec

=head1 VERSION

This document describes version 0.001 of Getopt::Long::EvenLess::Dump (from Perl distribution Getopt-Long-EvenLess-Dump), released on 2017-08-10.

=head1 FUNCTIONS


=head2 dump_getopt_long_evenless_script
...
This function runs a CLI script that uses C<Getopt::Long::EvenLess> but
...
CLI script needs to use C<Getopt::Long::EvenLess>. This is detected currently by
a simple regex. If script is not detected as using C<Getopt::Long::EvenLess>,
status 412 is returned.

Will return the C<Getopt::Long::EvenLess> specification.
...
=head2 GETOPT_LONG_EVENLESS_DUMP => bool
