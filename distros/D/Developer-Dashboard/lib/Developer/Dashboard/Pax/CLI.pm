...
sub _run_dispatch {
    ...
    my $result = Developer::Dashboard::Pax::RuntimeDispatcher->new->dispatch_i64(
...
sub _capture {
    ...
    my $capture = Developer::Dashboard::Pax::Capture->new(mode => $mode)->capture($entrypoint);
...
sub _compile {
    ...
    my $tier1 = Developer::Dashboard::Pax::Tier1->new;
...
sub _build_artifacts {
    ...
    my $capture = Developer::Dashboard::Pax::Capture->new(mode => $capture_mode)->capture($entrypoint);
    my $manifest = Developer::Dashboard::Pax::Manifest->new(capture => $capture)->to_hash;
    my $regions = Developer::Dashboard::Pax::RegionSelector->new(manifest => $manifest)->select;
    ...
    print _json({
        ...
        operation_policy => Developer::Dashboard::Pax::Mode->policy($operation_mode),
...
sub _diff {
    ...
    my $result = Developer::Dashboard::Pax::Differential->new(pax_bin => _pax_bin())->compare_capture($entrypoint);
    print _json($result, $pretty);
...
sub _bench {
    ...
    my $result = Developer::Dashboard::Pax::Benchmark->new(
        pax_bin => _pax_bin(),
...
sub _bench_matrix {
    ...
    my $result = Developer::Dashboard::Pax::BenchmarkMatrix->new(
...
sub _run_native {
    ...
    my $result = Developer::Dashboard::Pax::NativeRunner->new->run_i64_binary(
...
sub _corpus {
    ...
    my $result = Developer::Dashboard::Pax::Corpus->new(manifest_path => $manifest_path)->run;
    print _json($result, $pretty);
    return $result->{passed} ? 0 : 1;
}
...
sub _core_suite {
    ...
    my $result = Developer::Dashboard::Pax::CoreSuite->new(manifest_path => $manifest_path)->run;
    print _json($result, $pretty);
    return $result->{passed} ? 0 : 1;
}
...
sub _cpan_matrix {
    ...
    my $result = Developer::Dashboard::Pax::CPANMatrix->new(manifest_path => $manifest_path)->run;
    print _json($result, $pretty);
    return $result->{passed} ? 0 : 1;
}
...
sub _profile {
    ...
    my $store = Developer::Dashboard::Pax::ProfileStore->new(threshold => $threshold);
    my @events;
    my $dispatcher = Developer::Dashboard::Pax::RuntimeDispatcher->new(
        profile_store => $store,
        inline_cache => Developer::Dashboard::Pax::InlineCache->new,
        threshold => $threshold,
    );
    for (1 .. $iterations) {
...
sub _why_not {
    ...
    for my $unit (@$ssa) {
        ...
        my $artifact = Developer::Dashboard::Pax::Tier1->new->compile($unit);
        push @native_reasons, {
...
sub _trace_guards {
    ...
    my ($capture, $manifest, $regions, $hir, $ssa) = _pipeline($entrypoint);
    my $guard_manager = Developer::Dashboard::Pax::GuardManager->new(epochs => $manifest->{runtime_epochs});
    my @traces;
    for my $unit (@$ssa) {
        next if defined $region && (($unit->{region_name} // '') ne $region && ($unit->{region_name} // '') ne "main::$region");
...
sub _gatekeeper {
    ...
    my $report = Developer::Dashboard::Pax::Gatekeeper->new(root => '.')->sow01_report;
    print _json($report, $pretty);
...
sub _pax_bin {
    # $0 is the currently-executing script's own path - correct whether this
    # is the shipped share/private-cli/pax source or its staged runtime copy
    # under ~/.developer-dashboard/cli/dd/pax, unlike the old FindBin::Bin
    # assumption (which required a sibling 'pax' file next to whatever
    # script happened to load this module - not true once vendored and
    # dispatched through this project's own staged-command mechanism).
    require Cwd;
...
sub _app_build {
    ...
    my $cfg = $no_paxfile ? {} : Developer::Dashboard::Pax::Paxfile->load_optional($paxfile);
    ...
    my $result = Developer::Dashboard::Pax::AppImage->new->build(
        name => $name,
        entrypoint => $entrypoint,
        lib_dirs => \@libs,
        assets => \@assets,
...
sub _app_start {
    ...
    my $image = Developer::Dashboard::Pax::AppImage->new->load(name => $name);
    if ($daemonize) {
        Developer::Dashboard::Pax::AppServer->new(image => $image)->start(daemonize => 1);
        print _json({ status => 'started', name => $name, socket_path => $image->{socket_path} }, $pretty);
        return 0;
    }
    return Developer::Dashboard::Pax::AppServer->new(image => $image)->start;
}
...
sub _app_run {
    ...
    my $image = Developer::Dashboard::Pax::AppImage->new->load(name => $name);
    return Developer::Dashboard::Pax::AppServer->run_client(image => $image, argv => \@argv);
}
...
sub _app_stop {
    ...
    my $image = Developer::Dashboard::Pax::AppImage->new->load(name => $name);
    return Developer::Dashboard::Pax::AppServer->stop(image => $image);
}
...
sub _standalone_build_from_config {
    ...
    my $result = eval {
        Developer::Dashboard::Pax::StandaloneImage->new->build(
            name => $cfg->{name},
            entrypoint => $entrypoint,
            lib_dirs => \@lib_dirs,
...
sub _standalone_build_progress {
    ...
    my $enabled = exists $ENV{PAX_PROGRESS}
        ...
        : 1;
    return if !$enabled;
    return Developer::Dashboard::Pax::CLI::Progress->new(
        title   => 'pax build progress',
        tasks   => Developer::Dashboard::Pax::StandaloneImage->build_progress_tasks,
        stream  => \*STDERR,
...
sub _standalone_run {
    ...
    my $image = Developer::Dashboard::Pax::StandaloneImage->new->load(name => $name);
...
sub _standalone_native_run {
    ...
    my $result = Developer::Dashboard::Pax::StandaloneDispatch->new->run_i64(
        name => $name,
        region_name => $region,
        left => $left,
        right => $right,
...
Developer::Dashboard::Pax::CLI - public PAX command facade
...
C<Developer::Dashboard::Pax::CLI> implements the SOW-03 public command surface for C<bin/pax>. The
...
executable through C<Developer::Dashboard::Pax::StandaloneImage>.
