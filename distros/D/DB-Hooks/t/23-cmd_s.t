...
is
	n( `$^X $lib -d:DbBatch='s;q' -e '$script'` )
	,$files->{ 'sbs' }
...
is
	n( `$^X $lib -d:DbBatch='s;s;q' -e '$script'` )
	,$files->{ 'step into sub' }
	,"Step into sub";

is
	n( `$^X $lib -d:DbBatch='go 2;s;q' -e '$script'` )
	,$files->{ 'step from sub' }
	,"Step from sub";

is
	n( `$^X $lib -d:DbBatch='s 4;q' -e '$script'` )
	,$files->{ 'do n steps' }
	,"Do N steps at once";
...
is
	n( `$^X $lib -d:DbBatch='s 2;s;s;s' -e '$script'` )
	,$files->{ 'chained steps' }
