...
This document describes version 0.009 of Sah::Schema::homs (from Perl distribution Sah-SchemaBundle-Collection), released on 2024-06-13.
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("homs", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("homs", {return_type=>'str_errmsg+val'});
 my $res = $validator->($data); # [$errmsg, $validated_val]
 
 # a sample valid data
 $data = {k=>undef};
 my $res = $validator->($data); # => ["",{k=>undef}]
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('homs*', name=>'Homs')
     );
 }

 use My::Types qw(Homs);
 Homs->assert_valid($data);
