...
=item $class-E<gt>B<new>($msgid, %options)
...
Extends L<"Attributes" in Mail::Box::Message|Mail::Box::Message/"Attributes">.

=over 4

=item $obj-E<gt>B<bodyType>()

Inherited, see L<Mail::Message/"Attributes">

=item $obj-E<gt>B<fieldType>()

Inherited, see L<Mail::Message/"Attributes">

=item $obj-E<gt>B<folder>( [$folder] )

Inherited, see L<Mail::Box::Message/"Attributes">

=item $obj-E<gt>B<headType>()
...
When C<undef> is specified for C<$head>, no change has to take place and
...
When C<undef> is specified for C<$body>, no change has to take place and
the method returns silently.  In all other cases, this method will
...
The message L<send()|Mail::Message/"The message"> mechanism had not enough information to
...
=item Error: destructed message can not be undeleted
...
=item Error: destructed message has no labels except 'deleted', requested is $label.

Cast by C<label()>

=item Error: destructed message has no labels except 'deleted', trying to set @labels.
...
=item Error: you cannot coerce a $class into destruction.
...
=item Error: you cannot set the body on a destructed message.

Cast by C<body()>

=item Error: you cannot set the head on a destructed message.

Cast by C<head()>

=item Error: you cannot set the modified flag on a destructed message.

Cast by C<modified()>

=item Error: you cannot take the body of a destructed message.

Cast by C<body()>

=item Error: you cannot take the head of a destructed message.
...
This module is part of Mail-Box version 4.02,
