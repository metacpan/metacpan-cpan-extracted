...
SKIP: {
    ...
    use_ok('Callback');
    is( version->parse($Callback::VERSION), version->parse(1.07), 'Callback version' );

    use_ok('Array::DeepUtils');
    is( version->parse($Array::DeepUtils::VERSION), version->parse(0.2), 'Array::DeepUtils version' );
}
