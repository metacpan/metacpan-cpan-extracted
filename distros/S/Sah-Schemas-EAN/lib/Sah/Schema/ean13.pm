...
our $schema = [str => {
    summary => 'EAN-13 number (e.g. 5-901234-123457)',
    description => <<'_',
...
Sah::Schema::ean13 - EAN-13 number (e.g. 5-901234-123457)

=head1 VERSION

This document describes version 0.009 of Sah::Schema::ean13 (from Perl distribution Sah-Schemas-EAN), released on 2023-01-27.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("ean13*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("ean13", {return_type=>'str_errmsg'});
 ...
 $data = "123-4567890-123";
 my $errmsg = $validator->($data); # => "Invalid checksum digit"

Often a schema has coercion rule or default value, so after validation the
validated value is different. To return the validated (set-as-default, coerced,
prefiltered) value:

 my $validator = gen_validator("ean13", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = "123-4567890-123";
 my $res = $validator->($data); # => ["Invalid checksum digit",undef]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("ean13*");
     $validator->(\@args);
     ...
 }
...
To create a type constraint and type library from a schema:
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('$sch_name*', name=>'Ean13')
     );
 }

 use My::Types qw(Ean13);
 Ean13->assert_valid($data);
