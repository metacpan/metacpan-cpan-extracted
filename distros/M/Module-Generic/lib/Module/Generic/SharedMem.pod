...
C<Module::Generic::SharedMem> is designed to provide multi-process safe access to shared memory using System V IPC. It uses semaphores to guard concurrent access.
...
Shared memory and semaphores are per-system resources — do not assume persistence across process restarts.

=item *

Always call C<remove()> manually if you want to explicitly clean up memory and semaphores, especially in daemonized or mod_perl environments.

=item *

Thread safety is supported where C<threads> is enabled, but this module is primarily designed for inter-process communication.
