...
#
#=======================================================================
package PDF::API3::Compat::API2::Basic::TTF::GSUB;
...
The structure of a GSUB table is the same as that given in L<PDF::API3::Compat::API2::Basic::TTF::Ttopen>.
...
@ISA = qw(PDF::API3::Compat::API2::Basic::TTF::Ttopen);
...
sub out_sub
{
    ...
    if ($type == 1)
    {
        $out = pack("nn", $fmt, PDF::API3::Compat::API2::Basic::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2));
        if ($fmt == 1)
    ...
    {
        $out = pack("nnn", $fmt, PDF::API3::Compat::API2::Basic::TTF::Ttopen::ref_cache($lookup->{'COVERAGE'}, $ctables, 2),
                            $num);
        $out .= pack('n*', 0 x $num);
        $offc = length($out);
        for ($i = 0; $i < $num; $i++)
    ...
    PDF::API3::Compat::API2::Basic::TTF::Ttopen::out_final($fh, $out, [[$ctables, 0]]);
    $lookup;
}
