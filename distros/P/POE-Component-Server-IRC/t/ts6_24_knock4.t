...
sub groucho_connected {
  ...
  $kernel->post( $sender, 'send_to_server', { prefix => '7UP', command => 'SJOIN', params => [ $chants, '#special', '+int', '@7UPAAAAAA' ], colonify => 1 } );
  $kernel->post( $sender, 'send_to_server', { prefix => '7UP', command => 'SJOIN', params => [ $chants, '#wotevs', '+int', '7UPAAAAAA' ], colonify => 1 } );
...
sub harpo_connected {
  ...
  $kernel->post( $sender, 'send_to_server', { prefix => '9T9', command => 'SJOIN', params => [ $chants, '#special', '+int', '@9T9AAAAAA' ], colonify => 1 } );
  $kernel->post( $sender, 'send_to_server', { prefix => '9T9', command => 'SJOIN', params => [ $chants, '#wotevs', '+int', '9T9AAAAAA' ], colonify => 1 } );
...
sub client_input {
  ...
  if ( $cmd eq '711' ) {
    ...
    my $chan = shift @{ $heap->{chans} };
    $poe_kernel->post( $sender, 'send_to_server', { command => 'KNOCK', params => [ $chan ] } ) if $chan;
    return;
  }
  if ( $cmd eq '712' ) {
    pass("IRC_$cmd");
    is($params->[2],'Too many KNOCKs (user).','Too many KNOCKs (user).');
  ...
  if ( $cmd eq 'ERROR' ) {
    ...
    is( scalar keys %{ $state->{chans} }, 3, 'Three channels' );
