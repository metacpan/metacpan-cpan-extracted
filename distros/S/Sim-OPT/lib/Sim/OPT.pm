...
$VERSION = '0.637';
$ABSTRACT =
'Sim::OPT is an optimization and parametric exploration program oriented to problem decomposition. It can be used with simulation programs receiving text files as input and emitting text files as output. It allows a free mix of sequential and parallel block coordinate searches.';
...
sub filterinsts_winsts {
    ...
    "FEWERINSTNAMES" . dump(@fewerinstnames);
...
sub fromopt_tosweep {
    ...
    foreach my $case (@casegroup) {
        ...
        foreach my $elt (@blockrefs) {
            ...
            my @sweepblock =
              @chances[ $attachpoint .. ( $attachpoint + $blocklength - 1 ) ];
    ...
    foreach (@varnumbers) {
        ...
        foreach ( keys %$_ ) {
            unless ( defined $mediumters[$countcase]{$_} ) {
                $mediters[$countcase]{$_} =
                  ( ceil( $varnumbers[$countcase]{$_} / 2 ) );
            }
        }
        $countcase++;
    }
...
sub getitersnum { my $countcase = shift;
    my $varnumber  = shift;
    my @varnumbers = @_;
...
sub getblockelts { my ( $sweeps_ref, $countcase, $countblock ) = @_;
...
sub extractcase {
    my ( $inst_r, $dowhat_ref, $transfile, $carrier_r, $file, $blockelts_r,
        $mypath, $instn )
      = @_;
    ...
    my %to =
      %{ makefilename( \%tempc, $mypath, $file, $instn, \%inst, \%dowhat ) };
    return ( \%to );
}
...
sub definerootcases {
    ...
    foreach my $sweep (@sweeps) {
        ...
        foreach $key ( sort { $a <=> $b } ( keys %$case ) ) {
            $rootname =
              $rootname . $key . "-" . $miditers[$countcase]{$key} . "_";
        }
...
sub givesize { my $sliceref = shift;
    my @slice        = @$sliceref;
    my $countcase    = shift;
...
sub integratebox {
    ...
    if ( ref( $arrelts[0] ) ) {
        foreach my $eltref (@arrelts) {
            ...
            push( @newbox,
                [ $righttarget, $elts[1], $elts[2], $rightorigin, $elts[4] ] );
        }
        return ( \@newbox );
    }
}
...
sub cleansweeps {
    ...
    "CLEANED \@outbag: " . dump(@outbag);
    "CLEANED \@sourcestruct: " . dump(@sourcestruct);
...
sub gencentres {
    ...
    while ( $c < ( $numelts - 1 ) ) {
        ...
        foreach my $key ( sort { $a <=> $b } ( keys %hsmax ) ) {
            my $newval =
              ( int( ( $hsmax{$key} - $hsmin{$key} ) / 2 ) + $hsmin{$key} );
            if ( ( $newval == $hsmax{$key} ) or ( $newval == $hsmin{$key} ) ) {
                ...
                if ( $answer eq "even" ) {
                    ...
                    if ( $inv == "no" ) {
                        @newpair = ( $hsmin{$key}, $hsmax{$key} );
                        "newpair 2: " . dump(@newpair);
                    }
                    else {
                        @newpair = ( $hsmax{$key}, $hsmin{$key} );
                        "newpair 3: " . dump(@newpair);
                    }
                    $newval = $newpair[0];
                }
                $signal++;
...
sub stararrange {
    my ( $sweeps_ref, $blockelts_ref, $varnumbers_ref, $countcase, $countblock,
        $file )
      = @_;
    my @sweeps     = @{$sweeps_ref};
...
sub solvestar {
    ...
    my ( @gencetres, @genextremes, @genpoints, $genextremes_ref,
        $gencentres_ref, @starpositions, @dummysweeps );

    if ( $dirfiles{stardivisions} ne "" ) {
        $dirfiles{starpositions} =
          genstar( \%dirfiles, \@varnumbers, \%carrier, \@blockelts );

        ( $dirfiles{dummysweeps}, $dirfiles{dummyelt} ) =
          @{ star_act( \@sweeps, \@blockelts, $countcase, $countblock ) };
    }
    ...
    if ( not( scalar( @{ $dirfiles{starpositions} } ) == 0 ) ) {
        $dirfiles{direction} => [ ["star"] ],
          $dirfiles{randomsweeps} => "no", $dirfiles{randominit} =>
          "no", $dirfiles{randomdirs} => "no",;
    }
    return ( \%dirfiles );
}
...
sub callblock {
    ...
    if ( $countcase > $#sweeps ) {
        if ( $dirfiles{checksensitivity} eq "yes" ) {
            Sim::OPT::sense( $dirfiles{ordtot}, $mypath,
                $dirfiles{objectivecolumn} );
        }
        exit( "#END RUN." );
    }

    "#Beginning a search on case "
      . ( $countcase + 1 )
      . ", block "
      . ( $countblock + 1 ) . ".";
    ...
    my @sourceblockelts =
      @{ getblockelts( \@sourcesweeps, $countcase, $countblock ) };
    ...
    if ( $sourceblockelts[0] =~ /^([A-Za-z]+)/ ) {
        ...
        if ( $sourceblockelts[0] =~ /ç/ ) {
            ...
            push( @{ $vehicles{nesting}{ $dirfiles{nestclock} } },
                $countblock );
        }
    }
    else {
    ...
    if ( $sourceblockelts[0] =~ /ç/ ) {
        ...
        "SLICENUM! " . dump( $dirfiles{slicenum} );
    ...
    if ( $sourceblockelts[0] =~ /é/ ) {
        ...
        "REVEALNUM! " . dump( $dirfiles{revealnum} );
    ...
    if ( $dirfiles{starsign} eq "yes" ) {
        my @newvarnumbers = @{
            stararrange(
                \@sweeps,   \@blockelts, \@varnumbers,
                $countcase, $countblock, $file
            )
        };
        ...
        my @newmiditers = @{ $dirfiles{starpositions} };
    ...
    $dirfiles{replist}     = "$mypath/$file-replist--$countcase";
    $dirfiles{descendlist} = "$mypath/$file-descendlist--$countcase";
    ...
    $dirfiles{repblock}    = "$mypath/$file-repblock--$countcase-$countblock";
    ...
    deffiles(
        {
            ...
            carrier         => \%carrier,
            carrier         => \%carrier,
            sourceblockelts => \@sourceblockelts,
            blocks          => \@blocks,
            blockelts       => \@blockelts,
            instn           => $instn,
            inst            => \%inst,
...
sub deffiles {
    ...
    $dirfiles{starstring} = $starstring;
    ...
    if (
        ...
        or
        ( ( $dirfiles{random} eq "yes" ) and ( $dowhat{metamodel} eq "yes" ) )
      ...
      )
    {
        if ( $dirfiles{starsign} eq "yes" ) {
            ...
            foreach my $elt (@blockelts) {
                my @dummys = ($elt);
                my @bix =
                  @{ toil( \@dummys, \%varnums, \@basket, $c, $mypath, $file )
                  };
                @bix = @{ $bix[0] };
                push( @bux, @bix );
                $c++;
          ...
          )
        {
            ...
            @bag          = ($bit);
            @fills        = @{
                Sim::OPT::Descend::prepareblank(
                    \%varnums, $tmpblankfile, \@blockelts,
                    \@bag,     $file,         \%carrier
                )
            };
        ...
        unless ( $dirfiles{ga} eq "yes" ) {
            @bux =
              @{ toil( \@blockelts, \%varnums, \@basket, "", $mypath, $file ) };
        }
        else {
            my @subst = (1);
            @bux =
              @{ toil( \@subst, \%carrier, \@basket, "", $mypath, $file ) };
        }
    }
    ...
    if ( $dirfiles{starsign} eq "yes" ) {
        ...
        foreach my $midsurrs_r (@miditers) {
            ...
            @bag = uniq(
                @{
                    integratebox(
                        \@bux,   \%midsurrs, $file,    \@blockelts,
                        $mypath, \%inst,     \%dowhat, $instn
                    )
                }
            );
            push( @finalbox, @bag );
...
sub setlaunch {
    ...
    foreach my $elt (@basket) {
        ...
        my %to = %{
            extractcase(
                \%inst, \%dowhat,    $newpars, \%carrier,
                $file,  \@blockelts, $mypath,  $instn
            )
        };

        {
            my %orig = %{
                extractcase(
                    \%inst, \%dowhat,    $oldpars, \%carrier,
                    $file,  \@blockelts, $mypath
                )
            };
            say $tee
"obtained \$instn: $instn, countinstance: $count, from: $from, \$to: $to, \%orig: "
              . dump( \%orig )
              . ", \%carrier: "
              . dump( \%carrier );
...
sub exe {
    ...
    "BEFORE PROCESSING, INSTANCES: " . dump(@instances);

    if ( $dirfiles{ga} eq "yes" ) {
        "SHOW \@instances: " . dump(@instances);
        "#Calling GAs on morphing operations for case "
          . ( $countcase + 1 )
          . ", block "
          . ( $countblock + 1 ) . ".";
        ...
        my $starttarget = Sim::OPT::Morph::giveback( \%mids );
        ...
        foreach $i (@instances) {
            ...
            if ( $count == 0 ) {
                ...
                if ( $dowhat{names} eq "long" ) {
                    ...
                    $to{crypto}        = "$mypath/$file" . "_" . "$starttarget";
                    $to{cleancrypto}   = $starttarget;
                    $orig{crypto}      = "$mypath/$file" . "_" . "$starttarget";
                    $orig{cleancrypto} = $starttarget;
                }
        ...
        my @juggle;
        my @prov = @instances;
        "BEFORE, INSTANCES: " . dump(@instances);
        my $pop      = pop(@prov);
        my %instance = %{$pop};
        ...
        `rm -f $dowhatstore`;
        `rm -f $dirfilesstore`;
        `rm -f $basestore-return`;
        `rm -f ./$relate.txt`;
        `rm -f ./$report.txt`;

        store \%instance, "$basestore";
        store \%dowhat,   "$dowhatstore";
        store \%dirfiles, "$dirfilesstore";

        say $tee
"python $dowhat{ga} $configfile $starttarget $countcase $basestore $dowhatstore $dirfilesstore $dirfiles{ganum}";
        my @returns =
`python $dowhat{ga} $configfile $starttarget $countcase $basestore $dowhatstore $dirfilesstore $dirfiles{ganum}`;
        ...
        say "\%mids!: " . dump( \%mids );
        say "\$countcase!: " . dump($countcase);
        say "\@juggle!: " . dump(@juggle);
        say "\%dowhat!: " . dump( \%dowhat );
        foreach my $name (@returns) {
            chomp $name;
            say "\$name!: " . dump($name);
            my $inss_r =
              Sim::OPT::Morph::setpickinst( $name, \%mids, $countcase,
                \@juggle, \%dowhat );
            my @inss = @{$inss_r};
            chomp $name;
            say "\@inss!: " . dump(@inss);
        ...
        foreach my $inst_r (@instances) {
            ...
            my %inst = %{ $i{inst} };
            my %to   = %{ $i{to} };
            say RELATE "out of setpickinst \%to " . dump(%to);
            my %orig = %{ $i{orig} };
            say RELATE "out of \%orig \$crypto " . dump(%orig);
            my $is = $i{is};
            say RELATE "out of setpickinst \$is " . dump($is);
            my $origin = $i{origin};
            say RELATE "out of setpickinst \$origin " . dump($origin);

            my $crypto = Sim::OPT::encrypt1($is);
            say RELATE "out of setpickinst \$crypto " . dump($crypto);
            my $cryptor = Sim::OPT::encrypt1($origin);
            say RELATE "out of setpickinst \$cryptor " . dump($cryptor);
        ...
        my $cryptolinks =
          "$mypath/$file" . "_" . "$countcase" . "_cryptolinks.pl";
        open( CRYPTOLINKS, ">$cryptolinks" ) or die;
        say CRYPTOLINKS "" . dump( \%inst );
        close CRYPTOLINKS;
        "ARRIVED 1 ";
        if ( ( $dowhat{simulate} eq "y" ) or ( $dowhat{newreport} eq "y" ) ) {
            "ARRIVED 2 ";
            say $tee
"#Calling simulations, reporting and retrieving for instance $instance{is} in case "
              . ( $countcase + 1 )
              . ", block "
              . ( $countblock + 1 ) . ".";
            (
        ...
        if ( $dowhat{metamodel} eq "yes" ) {
            $tmpblankfile = "$mypath/$file" . "_tmp_gen_blank.csv";
            "IN LATINHYPERCUBE \$tmpblankfile: " . dump($tmpblankfile);
            $bit   = $file . "_";
            @bag   = ($bit);
            @fills = @{
                Sim::OPT::Descend::prepareblank(
        ...
        while ( scalar(@sack) <= $randompicknum ) {
            ...
            $instances_r =
              Sim::OPT::Morph::setpickinst( $newstring, \%mids, $countcase,
                \@baseinsts, \%dowhat );
            @shortinstances = @{$instances_r};
            "ALL-IMPORTANT ARRIVED INSTANCES TO GO "
              . dump(@instances);

            foreach my $ir (@shortinstances) {
                my %i  = %{$ir};
                my %to = %{ $i{to} };
                "out of setpickinst \%to " . dump(%to);
                my %orig = %{ $i{orig} };
                "out of \%orig \$crypto " . dump(%orig);
                my $is = $i{is};
                "out of setpickinst \$is " . dump($is);
                my $origin = $i{origin};
                "out of setpickinst \$origin " . dump($origin);

                $to{to} = "$mypath/$file" . "_" . "$is";
                "A out of setpickinst \$to{to} " . dump( $to{to} );
                $to{cleanto} = "$is";
                "B out of setpickinst \$to{cleanto} "
                  . dump( $to{cleanto} );
                $orig{to} = "$mypath/$file" . "_" . "$origin";
                "C out of setpickinst \$orig{to} " . dump( $orig{to} );
                $orig{cleanto} = "$origin";
                "D out of setpickinst \$orig{cleanto} "
                  . dump( $orig{cleanto} );

                if ( $dowhat{names} eq "long" ) {
                    my $crypto = Sim::OPT::encrypt1($is);
                    "out of setpickinst \$crypto " . dump( \$crypto );
                    my $cryptor = Sim::OPT::encrypt1($origin);
                    "out of setpickinst \$cryptor "
                      . dump( \$cryptor );
                    $to{crypto}        = "$mypath/$file" . "_" . "$crypto";
                    $to{cleancrypto}   = $crypto;
                    $orig{crypto}      = "$mypath/$file" . "_" . "$cryptor";
            ...
            "FINALLY, \%inst" . dump( \%inst );

            my @pop          = @shortinstances;
            my $leadinst_ref = pop(@pop);

            say $tee
"16ARRIVED 3,LAUNCHING MORPHING WITH \$configfile $configfile, \@shortinstances, "
              . dump(@shortinstances)
			...
			\$fire $fire, \%dirfiles,  "
              ...
              . " \%inst "
              . dump(%inst),;
            Sim::OPT::Morph::morph( $configfile, \@shortinstances, \%dirfiles,
            ...
            "ARRIVED 4, WITH $precious";
            "LAUNCHING SIM WITH  \@lastinstance, "
              . dump(@lastinstance)
              . ", \$fire $fire, \%dirfiles,  "
              . dump(%dirfiles)
              . ", \%vehicles, "
            ...
            "#Descending to catch performance "
              . ( $countcase + 1 )
              . ", block "
              . ( $countblock + 1 ) . ".";
...
"#Calling simulations, reporting and retrieving for instance $instance{is} in case "
    ...
    elsif ( $dirfiles{OLDrandompick} eq "yes" ) {
        "#Calling randomly picked morphing operations for case "
          . ( $countcase + 1 )
          . ", block "
          . ( $countblock + 1 ) . ".";
        ...
        if ( $dowhat{morph} eq "y" ) {
            say $tee
              "#Calling morphing operations for instance $instance{is} in case "
              . ( $countcase + 1 )
              . ", block "
              . ( $countblock + 1 ) . ".";
            my @result = Sim::OPT::Morph::morph(
                $configfile, \@instances, \%dirfiles, \%dowhat,
                \%vehicles,  \%inst,      \@precedents
            );
...
"#Calling simulations, reporting and retrieving for instance $instance{is} in case "
        ...
        if ( $dowhat{morph} eq "y" ) {
            "#Calling morphing operations for case "
              . ( $countcase + 1 )
              . ", block "
              . ( $countblock + 1 ) . ".";
            my @result = Sim::OPT::Morph::morph(
        ...
        if ( ( $dowhat{simulate} eq "y" ) or ( $dowhat{newreport} eq "y" ) ) {
            ...
            "#Calling simulations, reporting and retrieving for case "
              . ( $countcase + 1 )
              . ", block "
              . ( $countblock + 1 ) . ".";
            (
            ...
            $dirfiles{mergecases}  = $mergecases_ref;
            say $tee
              "#Performed simulations, reporting and retrieving for case "
              . ( $countcase + 1 )
              . ", block "
              . ( $countblock + 1 ) . ".";
        }
    ...
    NOW ON THE BRINK";
    "\$dowhat{descend} $dowhat{descend}";
    if ( $dowhat{descend} eq "y" ) {
        "#Calling descent for case "
          . ( $countcase + 1 )
          . ", block "
          . ( $countblock + 1 ) . ".";
        Sim::OPT::Descend::descend(
...
sub opt {
    ...
    if ( $dowhat{justchecksensitivity} ne "" ) {
        "RECEIVED.";
        Sim::OPT::sense(
            $dowhat{justchecksensitivity}, $mypath,
            $dowhat{objectivecolumn},      "stop"
        );
    }

    if ( $dowhat{randomsweeps} eq "yes" ) {
        my @newsweeps;
    ...
    { foreach my $directionref ( @{ $dowhat{direction} } )
        ...
        "NEW RANDOMIZED DIRECTIONS: " . dump( $dowhat{direction} );
    }

    { my $i = 0;
        my @newarr;
        foreach my $dirref ( @{ $dowhat{direction} } ) {
            "112 \$dirref: " . dump($dirref);
    ...
    @caseseed =
      convcaseseed( { caseseed => \@caseseed, chanceseed => \@chanceseed } );

    my (@sweeps_);

    if ( ( $target eq "takechance" ) and (@chancedata) and ($dimchance) ) {
        my @obt = Sim::OPT::Takechance::takechance( \@caseseed, \@chanceseed,
        ...
        say MESSAGE
"\THESE VALUES OF \@sweeps IS EQUIVALENT TO THE FOLLOWING VALUES OF \@caseseed AND \@chanceseed: ";
        ...
        my @winneritems =
          populatewinners( \@rootnames, $countcase, $countblock );
        ...
        my (
            @gencentres,      @genextremes, @genpoints,
            $genextremes_ref, $gencentres_ref
        );

        if ( $dowhat{outstarmode} eq "yes" ) {
            if (    ( $dowhat{stardivisions} ne "" )
                and ( $dowhat{starpositions} eq "" ) )
            {
                $dowhat{starpositions} =
                  genstar( \%dowhat, \@varnumbers, $miditers[$countcase] );
            }
            ...
            if ( not( scalar( @{ $dowhat{starpositions} } ) == 0 ) ) {
                ...
                $dowhat{randomsweeps} => "no";
                $dowhat{randominit}   => "no";
                $dowhat{randomdirs}   => "no";
