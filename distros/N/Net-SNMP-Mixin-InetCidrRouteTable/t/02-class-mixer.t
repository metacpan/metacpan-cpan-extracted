...
ok(
  Net::SNMP->can('get_inet_cidr_route_table'),
  'get_inet_cidr_route_table() is now a class method'
);

eval {Net::SNMP->mixer('Net::SNMP::Mixin::InetCidrRouteTable')};
...
eval {$session->mixer("Net::SNMP::Mixin::InetCidrRouteTable")};
like( $@, qr/already mixed into/, 'mixed in twice is an error' );

ok( $session->can('get_inet_cidr_route_table'), '$session can get_inet_cidr_route_table' );

eval {$session->get_inet_cidr_route_table};
