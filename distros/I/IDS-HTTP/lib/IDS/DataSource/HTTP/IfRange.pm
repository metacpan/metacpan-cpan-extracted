# HTTP IfRange line; Section 14.27 of RFC 2616
#
# subclass of IDS::DataSource::HTTP; see that for interface requirements
#

package IDS::DataSource::HTTP::IfRange;
...
$IDS::DataSource::HTTP::IfRange::VERSION     = "1.0";
...
sub parse {
    ...
    # We can have either a date or an entity tag as a value.
    # Entity tags start or end with a "
    if ($data =~ /^"/ || $data =~ /"$/) {
	$self->{"sub"} = new IDS::DataSource::HTTP::ETag($self->{"params"}, $data);
    } else {
	$self->{"sub"} = new IDS::DataSource::HTTP::Date($self->{"params"}, $data);
    }
