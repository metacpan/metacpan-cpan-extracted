...
eval {
    ...
    $fig->arc(1);
    $fig->save("${dir}/arc1.fig");
    &FigCmp::figCmp("${dir}/arc1.fig", "t/arc1.fig") || die;
};
ok($@ eq "", "test1");

#
# Test 2: arc given radius and negative angle
#
eval {
    my $fig = Graphics::Fig->new({ position => [ 3, 2 ], arrowMode => "forw" });
    $fig->arc(2, -120);
...
#
# Test 3: arc given diameter, angle and rotation
#
eval {
    my $fig = Graphics::Fig->new({ position => [ 3, 2 ], arrowMode => "forw" });
...
#
# Test 4: arc given radius, rotation and control angle
#
eval {
    my $fig = Graphics::Fig->new({ position => [ 3, 2 ], arrowMode => "forw" });
...
#
# Test 5: arc given radius, angle, rotation and control angle
#
eval {
    my $fig = Graphics::Fig->new({ position => [ 3, 2 ], arrowMode => "forw" });
...
#
# Test 6: arc given starting point, center, angle and direction
#
eval {
    my $fig = Graphics::Fig->new({ position => [ 3, 2 ], arrowMode => "forw" });
...
#
# Test 7: arc given starting point, final point and angle
#
eval {
    my $fig = Graphics::Fig->new({ position => [ 3, 2 ], arrowMode => "forw" });
...
#
# Test 9: test extended arrow options
#
eval {
...
#
# Test 10: test remaining options
#
eval {
    my $fig = Graphics::Fig->new();
    $fig->arc({
...
#
# Test 11: translate, rotate, scale and bbox
#
eval {
    my $fig = Graphics::Fig->new();
