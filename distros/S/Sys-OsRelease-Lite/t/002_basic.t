...
#===============================================================================
#         FILE: 002_basic.t
#  DESCRIPTION: basic tests for Sys::OsRelease::Lite 
...
my @std_attrs = Sys::OsRelease::Lite::std_attrs();
plan tests => (26 + 3 * scalar @std_attrs);

{
    # instantiation (3 tests)
    is(Sys::OsRelease::Lite->defined_instance(), 0, "instance undefined before initialization");
    my $osrelease = Sys::OsRelease::Lite->instance();
    isa_ok($osrelease, "Sys::OsRelease::Lite", "correct type from instance()");
    my $osrelease2 = Sys::OsRelease::Lite->instance();
    is($osrelease, $osrelease2, "same instance from 2nd call to instance()");

    # module data (11+std_attrs tests)
    my @std_search_path = Sys::OsRelease::Lite::std_search_path();
    ...
    foreach my $attr (map {lc $_} @std_attrs) {
        ...
        ok(Sys::OsRelease::Lite->can($attr), "class->$attr() method found");
    }
    is($osrelease->has_config("platform"), 0, "before calling platform(): ref->has_attr(platform) is false");
    is(Sys::OsRelease::Lite->has_config("platform"), 0, "before calling platform(): class->has_attr(platform) is false");
    my $platform1 = $osrelease2->platform();
    my $platform2 = Sys::OsRelease::Lite->platform();
    is($osrelease->has_config("platform"), 1, "after calling platform(): ref->has_attr(platform) is true");
    is(Sys::OsRelease::Lite->has_config("platform"), 1, "after calling platform(): class->has_attr(platform) is true");
    ...
    is(scalar (Sys::OsRelease::Lite->found_attrs()), $expected_attr_count,
...
is(Sys::OsRelease::Lite->defined_instance(), 0, "cleared: instance undefined after clear_instance()");
my $osrelease3 = Sys::OsRelease::Lite->instance();
isa_ok($osrelease3, "Sys::OsRelease::Lite", "cleared: instance set again by instance()");

# test for empty object if os-release file wasn't found - set empty search path to force test (8+std_attrs tests)
Sys::OsRelease::Lite->clear_instance();
foreach my $attr (map {lc $_} @std_attrs) {
    ok((Sys::OsRelease::Lite->can($attr))?0:1, "empty: $attr() method does not exist");
}
my $osrelease4 = Sys::OsRelease::Lite->instance(search_path => []);
isa_ok($osrelease4, "Sys::OsRelease::Lite", "empty: instance set again by instance()");
