package  perfSONAR_PS::Datatypes::v2_0::nmtl4::Message::Metadata::Subject::EndPointPair;
use strict;
use warnings;
...
=head1 NAME
 ...
 perfSONAR_PS::Datatypes::v2_0::nmtl4::Message::Metadata::Subject::EndPointPair  - A base class, implements  'endPointPair'  element from the perfSONAR_PS RelaxNG schema
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::Datatypes::v2_0::nmtl4::Message::Metadata::Subject::EndPointPair->new($DOM_Obj);
...
use fields qw(nsmap idmap refidmap  endPoint  );

perfSONAR_PS::Datatypes::v2_0::nmtl4::Message::Metadata::Subject::EndPointPair->mk_accessors(perfSONAR_PS::Datatypes::v2_0::nmtl4::Message::Metadata::Subject::EndPointPair->show_fields('Public'));
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::Datatypes::v2_0::nmtl4::Message::Metadata::Subject::EndPointPair';
...
sub getDOM {
    ...
    if($self->endPoint && ref($self->endPoint) eq 'ARRAY' ) {
        foreach my $subel (@{$self->endPoint}) { 
            if(blessed  $subel  &&  $subel->can("getDOM")) { 
                 my  $subDOM =  $subel->getDOM($endPointPair);
                $subDOM?$endPointPair->appendChild($subDOM):$logger->error("Failed to append  endPoint elements  with value: " .  $subDOM->toString ); 
            }
         }
    }
    return $endPointPair;
...
sub addEndPoint {
    ...
    $self->endPoint && ref($self->endPoint) eq 'ARRAY'?push @{$self->endPoint}, $new:$self->endPoint([$new]); 
    $logger->debug("Added new to endPoint"); 
...
=head2  removeEndPointById()
     ...
     remove specific element from the array of endPoint elements by id ( if id is supported by this element )
...
sub removeEndPointById {
    ...
    $self->buildIdMap; ## rebuild index map 
    return $id;
    } elsif(!ref($self->endPoint)  || ref($self->endPoint) ne 'ARRAY')  {
        $logger->warn("Failed to remove  element because endPoint not an array for non-existent id:$id");  
    } else {
        $logger->warn("Failed to remove element for non-existant id:$id");  
    } 
    return;
}   
=head2  getEndPointByMetadataIdRef()

     get specific object from the array of endPoint elements by  MetadataIdRef( if  MetadataIdRef is supported by this element )
     accepts single param -  MetadataIdRef
     if there is no array then it will return just an object
     
=cut

sub getEndPointByMetadataIdRef {
    my $self = shift;
    my $id = shift;
    my $logger  = get_logger( $CLASSPATH ); 
    if(ref($self->endPoint) eq 'ARRAY' && $self->refidmap->{endPoint} && exists $self->refidmap->{endPoint}{$id}) {
        my $endPoint = $self->endPoint->[$self->refidmap->{endPoint}{$id}];
    return ($endPoint->can("metadataIdRef") &&   $endPoint->metadataIdRef eq  $id)?$endPoint:undef; 
    } elsif($self->endPoint && (!ref($self->endPoint) || 
                                    (ref($self->endPoint) ne 'ARRAY' &&
                                     blessed $self->endPoint && $self->endPoint->can("metadataIdRef") &&
                     $self->endPoint->metadataIdRef eq  $id)))  {
        return $self->endPoint;
    }  
...
sub getEndPointById {
    ...
    my $logger  = get_logger( $CLASSPATH ); 
    if(ref($self->endPoint) eq 'ARRAY' && $self->idmap->{endPoint} &&  exists $self->idmap->{endPoint}{$id} ) {
        return $self->endPoint->[$self->idmap->{endPoint}{$id}];
    } elsif(!ref($self->endPoint) || ref($self->endPoint) ne 'ARRAY')  {
        return $self->endPoint;
    }  
...
sub fromDOM {
    ...
    foreach my $childnode ($dom->childNodes) { 
        ...
        if ($tagname eq  'endPoint' && $nsid eq 'nmtl4' && $self->can($tagname)) { 
           my $element = undef;
           eval {
               $element = perfSONAR_PS::Datatypes::v2_0::nmtl4::Message::Metadata::Subject::EndPointPair::EndPoint->new($childnode) 
           };
           if($EVAL_ERROR || !($element  && blessed $element)) {
               $logger->error(" Failed to load and add  EndPoint : " . $dom->toString . " error: " . $EVAL_ERROR);
               return;
           }
           ($self->endPoint && ref($self->endPoint) eq 'ARRAY')?push @{$self->endPoint}, $element:$self->endPoint([$element]);; ### add another endPoint  
