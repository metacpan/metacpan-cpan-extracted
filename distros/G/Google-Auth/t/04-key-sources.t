...
note("Testing Google::Auth::IDTokens::KeySources, Perl $], $^X");
...
my $x509_hr = HTTP::Response->new('200', 'OK', ['Content-Type' => 'text/plain'],
  $certs_body_json);
$source =
  Google::Auth::IDTokens::X509CertHttpKeySource->new({uri => $certs_uri});
$ua->unmap_all();
...
my $correct_hr =
  HTTP::Response->new('200', 'OK', ['Content-Type' => 'text/plain'], $jwk_body);
$source = Google::Auth::IDTokens::JwkHttpKeySource->new($params);
