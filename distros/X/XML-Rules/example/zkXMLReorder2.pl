...
my $parser = XML::Rules->new(
	...
	rules => {
		_default => 'raw',
		'name,members' => 'raw extended',
		'Group' => 'raw extended array',
		record => sub {
			my @I_depend;
			if (exists $_[1]->{':members'} and exists $_[1]->{':members'}{':Group'}) {
				@I_depend = grep !exists $written{$_}, map $_->{_content}, @{$_[1]->{':members'}{':Group'}};
			}
			if (@I_depend) {
...
sub find_dependent {
	...
	foreach my $parent (@{$depends{$name}}) {
		...
		if (! %{$parent->{':I_depend'}}) { # if it doesn't depend on anything more, write it after this one
			...
			$written{$parent->{':name'}{_content}} = 1;
...
#if (%depends) {
#	print STDERR "Unsatisfied dependencies! Some records depend on these unknown groups: ",
#		join ", ", keys %depends;
#}
