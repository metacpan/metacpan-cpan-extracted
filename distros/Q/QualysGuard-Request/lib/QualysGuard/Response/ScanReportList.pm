package QualysGuard::Response::ScanReportList;
...
sub new {
    ...
    if ( $self->exists('/SCAN_REPORT/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/SCAN_REPORT/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/SCAN_REPORT/ERROR');
...
QualysGuard::Response::ScanReportList
