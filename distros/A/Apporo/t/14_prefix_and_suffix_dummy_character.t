...
my $index_path = "/tmp/p5_apporo_index_14.tsv";
my $out_index;
...
my $conf_path = "/tmp/p5_apporo_conf_14.tsv";
my $out_conf;
...
is_utf8	false
dist_threshold	0.0
index_path	/tmp/p5_apporo_index_14.tsv
dist_func	edit
entry_buf_len	1024
engine	tsubomi
result_num	10
...
__CONF__
    ...
    my $file_name = "configure file(ASCII, 1-gram, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 3-gram, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 4-gram, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
