...
F<Makefile>.  The target of the file will be added to the 'to clean' list.Each
element must be a hashref, with the following keys:
...
B<Optional>.  If present, this must be a hashref, with values being
environment variable names to set for this ruleset, and values being
the... value.
...
if ( defined *DEPENDS{CODE} ) {
  ...
  push @{$Config{clean}->{FILES}}, $_->{target}
    for @$depends;
  # Not quite sure what ExtUtils::MakeMaker is doing with this, but it breaks
  # (with Out of Memory) if an arrayref is returned here
  $Config{depend} = { x => $depends };
}

sub MY::depend {
  package MY;
  my $depends = $_[0]->{depend}->{x};
  ...
  for (@$depends) {
    ...
    if ( exists $_->{env} ) {
      die sprintf("Env argument to depends target $target must be a " .
                  "hashref (got a %s)\n", ref($_->{env}) || 'simple value')
        unless UNIVERSAL::isa($_->{env}, 'HASH');
      # perl5.00503 gets narky if the my is in the while clause
      my ($var, $val);
      while ( ($var, $val) = each %{$_->{env}} ) {
        push @rules, "export $var";
        push @rules, sprintf "%s : %s=%s", $target, $var, $val;
      }
    }

    push @rules, sprintf("%s : %s\n",
                         $target, join("\n\t", join(' ', @$reqs), @$rules));
  }
