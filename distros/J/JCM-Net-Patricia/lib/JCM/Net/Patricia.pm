...
sub new {
  ...
  if ($type == AF_INET) {
    return bless _new(32), 'JCM::Net::Patricia::AF_INET';
  }

  if ($type == AF_INET6) {
    if (JCM::Net::Patricia::have_ipv6()) {
      return bless _new(128), 'JCM::Net::Patricia::AF_INET6';
    } else {
...
sub _ip_bits {
  ...
  if (ref ($self) eq 'JCM::Net::Patricia::AF_INET6') { 
	$bits = ($str =~ s|/(\d+)$||) ? $1 : 128; 
  } else { 
	$bits = ($str =~ s|/(\d+)$||) ? $1 : 32; 
  }
...
package JCM::Net::Patricia::AF_INET;
...
BEGIN {
  require Exporter;
  require DynaLoader;
  @ISA = qw(Exporter DynaLoader JCM::Net::Patricia);
  @EXPORT = qw(AF_INET);
}
...
package JCM::Net::Patricia::AF_INET6;
...
=item B<new> - create a new JCM::Net::Patricia object

   $pt = new JCM::Net::Patricia;

This is the class' constructor - it returns a C<JCM::Net::Patricia> object
...
The C<JCM::Net::Patricia> object will be destroyed automatically when
...
JCM::Net::Patricia trees, unlike many classes with XS-level data, can be
...
the C<climb> and the C<climb_inorder> methods - the CIDR prefix being
