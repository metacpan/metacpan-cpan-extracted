...
my($ret, $ok, $mpz, $count, $prefix, $suffix);

$mpz = Math::GMPz->new(17);
...
for(1..5) {
   $ret = TRmpz_out_str($prefix, \*$WR4, 10, $mpz, $suffix);
...
for(1..5) {
   $ret = TRmpz_out_str($prefix, \*$WR5, 10, $mpz);
...
for(1..5) {
   $ret = TRmpz_out_str($prefix, \*$WR6, 10, $mpz, $suffix);
...
$ret = TRmpz_inp_str($mpz, \*$RD8, 10);
close $RD8 or die "Can't close RD8: $!";

if($ret == 5 && $mpz == 12351) {print "ok 8\n"}
else {print "not ok 8 $ret $mpz\n"}
