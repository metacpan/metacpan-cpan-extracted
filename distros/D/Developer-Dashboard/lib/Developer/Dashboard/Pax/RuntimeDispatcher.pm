...
sub new {
    ...
    return bless {
        mode => $args{mode} // 'live',
        profile_store => $args{profile_store} // Developer::Dashboard::Pax::ProfileStore->new(threshold => $args{threshold} // 2),
        inline_cache => $args{inline_cache} // Developer::Dashboard::Pax::InlineCache->new,
        hot_region_jit => $args{hot_region_jit} // Developer::Dashboard::Pax::HotRegionJIT->new(threshold => $args{threshold} // 2),
        osr => $args{osr} // Developer::Dashboard::Pax::OSR->new(threshold => $args{threshold} // 2),
        aot => $args{aot} // Developer::Dashboard::Pax::ProfileGuidedAOT->new(threshold => $args{threshold} // 2),
    }, $class;
}
...
sub dispatch_i64 {
    ...
    my $capture = Developer::Dashboard::Pax::Capture->new(mode => $self->{mode})->capture($entrypoint);
...
Developer::Dashboard::Pax::RuntimeDispatcher - runtime dispatch coordinator

=head1 SYNOPSIS

  use Developer::Dashboard::Pax::RuntimeDispatcher;

  my $obj = Developer::Dashboard::Pax::RuntimeDispatcher->new(...);
...
Example 1:
  ...
  perl -Ilib -MDeveloper::Dashboard::Pax::RuntimeDispatcher -e 1
