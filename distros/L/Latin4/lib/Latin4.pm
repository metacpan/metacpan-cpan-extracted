...
perl $0 Latin-4_script.pl > Escaped_script.pl.e
END
    }

    print Latin4::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Elatin4::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
    ...
    my $fh = gensym();
    Elatin4::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Latin4::escape_script {
    ...
    # read Latin-4 script
    my $fh = gensym();
    Elatin4::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Elatin4(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Elatin4 '%s.0'; # 'quote' for perl5.6\n", $Latin4::VERSION); # require run-time routines version

        # use Latin4 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Latin4 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Latin4::VERSION) {
                    my @file = grep -e, map {qq{$_/Latin4.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Latin4.pm $version, but @{[__FILE__]} is version $Latin4::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Elatin4.pm $version, but \\\$Elatin4::VERSION is \$Elatin4::VERSION" if \$Elatin4::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Latin4::VERSION) {
                    die "Script $0 required Latin4.pm $version, but @{[__FILE__]} is only version $Latin4::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Elatin4::function_ord     = 'Latin4::ord'     if /\A ord \z/oxms;
                    $Elatin4::function_ord_    = 'Latin4::ord_'    if /\A ord \z/oxms;
                    $Elatin4::function_reverse = 'Latin4::reverse' if /\A reverse \z/oxms;
                    $Elatin4::function_getc    = 'Latin4::getc'    if /\A getc \z/oxms;

                    # avoid: used only once: possible typo at ...
                    $Elatin4::function_ord     = $Elatin4::function_ord;
                    $Elatin4::function_ord_    = $Elatin4::function_ord_;
                    $Elatin4::function_reverse = $Elatin4::function_reverse;
                    $Elatin4::function_getc    = $Elatin4::function_getc;
                }
            }
        }
    }

    $e_script .= Latin4::escape();
...
=item * You'll have to download Latin4.pm and Elatin4.pm and put it in your perl lib directory.

=item * You'll need to write "use Latin4;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Latin4;
  use Latin4 ver.sion;             --- require minimum version
  use Latin4 ver.sion.0;           --- expects version (match or die)

  # "no Latin4;" not supported

  or

  $ perl Latin4.pm Latin-4_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Latin-4_script.pl  --- script written in Latin-4
...
Latin4 software is "middleware" between perl interpreter and your Perl script
written in Latin-4.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Latin-4, Perl3 or later
can treat Latin-4 as binary data.

By "use Latin4;", it automatically interpret your script as Latin-4. The various
functions of perl including a regular expression can treat Latin-4 now.
...
This software is a source code filter to escape Perl script encoded by Latin-4
...
=item * Remains One Language Handling Raw Latin-4, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Latin4.pm, Elatin4.pm
  +---------------------------------------+
...
import of Elatin4.pm works well.
...
=head1 Software Composition
   ...
   Latin4.pm               --- source code filter to escape Latin-4
   Elatin4.pm              --- run-time routines for Latin4.pm
...
=head1 Calling 'Elatin4::ignorecase()' (Latin4 software provides)

Latin4 software applies calling 'Elatin4::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Elatin4::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Elatin4.pm provides)
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Latin4 software provides)

Latin4 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Latin4 software provides)

Insert 'Elatin4::' at head of function name. Elatin4.pm provides your script Elatin4::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Elatin4::lc        Character
  lcfirst     Elatin4::lcfirst   Character
  uc          Elatin4::uc        Character
  ucfirst     Elatin4::ucfirst   Character
  fc          Elatin4::fc        Character
  chr         Elatin4::chr       Character
  glob        Elatin4::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Latin4 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Latin4 software provides)

Latin4 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Elatin4.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Latin4 software and
