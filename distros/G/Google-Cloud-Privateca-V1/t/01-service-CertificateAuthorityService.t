...
BEGIN {
    for my $pkg (qw( Google::Cloud::Security::Privateca::V1::Resources::Certificate )) {
        unless ($pkg->can('new')) {
            no strict 'refs';
...
my $client = Google::Cloud::Privateca::V1::CertificateAuthorityServiceClient->new( credentials => 'dummy' );
ok($client, 'Instantiated generated client');
isa_ok($client->transport, 'Google::gRPC::Client', 'Client transport');

subtest 'create_certificate method' => sub {
    $client->transport->{mock_call} = sub {
        my ($args) = @_;
        is($args->{service}, 'google.cloud.security.privateca.v1.CertificateAuthorityService', 'Correct service path');
