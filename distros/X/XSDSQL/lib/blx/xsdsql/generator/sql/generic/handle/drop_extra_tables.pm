package blx::xsdsql::generator::sql::generic::handle::drop_extra_tables;
...
sub table_header {
	...
	my $comm=$params{NO_EMIT_COMMENTS} ? '' : $table->get_comment;
	$self->{STREAMER}->put_line($self->_get_drop_prefix,' ',$table->get_sql_name,' ',$self->_get_drop_suffix,' ',$comm,$table->command_terminator);
	undef;
}
