...
sub test_new : Test(6) {
    ...
    $sql = Prty::Sql->new('SQLite');
    $self->is($sql->{'dbms'},'SQLite');

    $sql = Prty::Sql->new('MySQL');
    $self->is($sql->{'dbms'},'MySQL');

    $sql = Prty::Sql->new('Access');
    $self->is($sql->{'dbms'},'Access');

    eval { Prty::Sql->new('Unknown') };
...
sub test_dbmsTestVector : Test(5) {
    ...
    my @vec = Prty::Sql->new('Oracle')->dbmsTestVector;
    $self->isDeeply(\@vec,[1,0,0,0,0]);

    @vec = Prty::Sql->new('PostgreSQL')->dbmsTestVector;
    $self->isDeeply(\@vec,[0,1,0,0,0]);

    @vec = Prty::Sql->new('SQLite')->dbmsTestVector;
    $self->isDeeply(\@vec,[0,0,1,0,0]);

    @vec = Prty::Sql->new('MySQL')->dbmsTestVector;
    $self->isDeeply(\@vec,[0,0,0,1,0]);

    @vec = Prty::Sql->new('Access')->dbmsTestVector;
...
sub test_isOracle : Test(2) {
    ...
    my $bool = Prty::Sql->new('Oracle')->isOracle;
    $self->is($bool,1);

    $bool = Prty::Sql->new('PostgreSQL')->isOracle;
    $self->is($bool,0);
}
...
sub test_isPostgreSQL : Test(2) {
    ...
    my $bool = Prty::Sql->new('PostgreSQL')->isPostgreSQL;
    $self->is($bool,1);

    $bool = Prty::Sql->new('Oracle')->isPostgreSQL;
    $self->is($bool,0);
}
...
sub test_isSQLite : Test(2) {
    ...
    my $bool = Prty::Sql->new('SQLite')->isSQLite;
    $self->is($bool,1);

    $bool = Prty::Sql->new('PostgreSQL')->isSQLite;
    $self->is($bool,0);
}
...
sub test_isMySQL : Test(2) {
    ...
    my $bool = Prty::Sql->new('MySQL')->isMySQL;
    $self->is($bool,1);

    $bool = Prty::Sql->new('PostgreSQL')->isMySQL;
    $self->is($bool,0);
}
...
sub test_isAccess : Test(2) {
    ...
    my $bool = Prty::Sql->new('Access')->isAccess;
    $self->is($bool,1);

    $bool = Prty::Sql->new('PostgreSQL')->isAccess;
    $self->is($bool,0);
}
...
sub test_split : Test(2) {
    ...
    my ($stmt,@arr) = Prty::Sql->split($stmt1);
    $self->is($stmt,$stmt2);
...
sub test_dataType_scalarContext : Test(10) {
    ...
    for my $dbms (Prty::Sql->dbmsNames()) {
        my $sql = Prty::Sql->new($dbms);
...
sub test_select : Test(7) {
    ...
    my $stmt1 = Prty::String->removeIndentation(<< '    __SQL__');
    SELECT
        *
    __SQL__
    ...
    $stmt1 = Prty::String->removeIndentation(<< '    __SQL__');
    SELECT
        *
    FROM
        tab
    ...
    my $stmt2 = Prty::String->removeIndentation(<< '    __SQL__');
    SELECT
        col1,
        col2,
        col3
    ...
    $stmt2 = Prty::String->removeIndentation(<< '    __SQL__');
    SELECT
        sta_id,
        sta_name,
        par_id,
...
sub test_update : Test(3) {
    ...
    my $stmt0 = Prty::String->removeIndentation(<<'    __SQL__');
    UPDATE person SET
        per_vorname = 'Hanno'
    __SQL__
    ...
    $stmt0 = Prty::String->removeIndentation(<<'    __SQL__');
    UPDATE person SET
        per_vorname = 'Hanno',
        per_nachname = 'Seitz'
    WHERE
