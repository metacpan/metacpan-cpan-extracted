...
B<Note:> PAGI applications only require C<Future::AsyncAwait>. The event loop
(L<IO::Async>) is an implementation detail of L<PAGI::Server>. Using C<Future::IO>
for timers and I/O keeps your application code portable across different servers.
...
The lifespan protocol allows your application to initialize resources on startup and clean them up on shutdown. The key feature is C<$scope-E<gt>> - a hash that persists across all requests within a worker process.
