...
Math::Bacovia::Product - Represents a product of an arbitrary (finite) number of symbolic values.

=head1 SYNOPSIS

    use 5.014;
    use Math::Bacovia qw(Product Symbol);

    my $f = Product(Symbol('a'), Symbol('b'), Symbol('c'));
    say $f->inv->pretty;        #=> ((1/a) * (1/b) * (1/c))

=head1 METHODS

This section describes the methods provided by the B<Math::Bacovia::Product> module.

=head2 new

    my $obj = Math::Bacovia::Product->new($x, $y, $z, , ,);

Constructs and returns a new B<Math::Bacovia::Product> object.
...
This section describes the special methods provided by the B<Math::Bacovia::Product> module.

=head2 inside

    my @values = $obj->inside;
