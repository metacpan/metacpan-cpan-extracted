...
Sah::Schema::negfloat - Negative float
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("negfloat*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("negfloat", {return_type=>'str_errmsg'});
 ...
 $data = 0.1;
 my $errmsg = $validator->($data); # => "Must be smaller than 0"

Often a schema has coercion rule or default value, so after validation the
validated value is different. To return the validated (set-as-default, coerced,
prefiltered) value:

 my $validator = gen_validator("negfloat", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = 0.1;
 my $res = $validator->($data); # => ["Must be smaller than 0",0.1]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("negfloat*");
     $validator->(\@args);
     ...
 }
