...
for (1..10) {
    cmpthese(-1, {
        ...
        panda_nextcan => sub { $o->bcan_my },
    });
    cmpthese(-1, {
        perl_next_method  => sub { $o->bmethod_std },
        panda_next_method => sub { $o->bmethod_my },
        panda_super       => sub { $o->bsuper },
    });    
    cmpthese(-1, {
        perl_maybe_next_method       => sub { $o->bmaybe_method_std },
        perl_maybe_next_method_last  => sub { $o->bmaybe_method_std_ne },
        panda_maybe_next_method      => sub { $o->bmaybe_method_my },
        panda_maybe_next_method_last => sub { $o->bmaybe_method_my_ne },
        panda_maybe_super            => sub { $o->bmaybe_super },
        panda_maybe_super_last       => sub { $o->bmaybe_super_ne },
    });    
}
