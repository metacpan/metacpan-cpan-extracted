...
Returns the font's name (a.k.a. display-name).
...
Returns the font's alternative-name (a.k.a. windows-name for a postscript font).
...
Returns the font's subname (a.k.a. font-variant, schriftschnitt).
...
sub width {
    ...
    foreach my $n (unpack('C*',$text)) {
        ...
        if ($kern) {
            ...
            $width += $self->data()->{'kern'}->{$lastglyph.':'.$glyph};
            $lastglyph = $glyph;
        }
    }
    $width /= 1000;
...
sub textByStrKern {
    ...
    if ($self->{'-dokern'} && ref($self->data()->{'kern'})) {
        ...
        foreach my $n (unpack('C*', $text)) {
	    my $glyph = $self->data()->{'e2n'}->[$n];
            if (defined $self->data()->{'kern'}->{$lastglyph.':'.$glyph}) {
                $newtext .= ') ' if $tBefore;
                $newtext .= sprintf('%i ',-($self->data()->{'kern'}->{$lastglyph.':'.$glyph}));
                $tBefore = 0;
            }
            $lastglyph = $glyph;
            my $t = pack('C', $n);
