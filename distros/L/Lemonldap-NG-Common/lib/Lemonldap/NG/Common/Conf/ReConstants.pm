...
our $oidcOPMetaDataNodeKeys = 'oidcOPMetaData(?:Options(?:A(?:uth(?:nEndpointAuth(?:Method|SigAlg)|EndpointExtraParams)|crValues)|C(?:o(?:nfigurationURI|mment)|lient(?:Secret|ID)|heckJWTSignature)|U(?:se(?:r(?:infoSourc|Attribut)|Nonc)e|iLocales)|To(?:kenEndpointAuth(?:Method|SigAlg)|oltip)|S(?:toreIDToken|igningKey|ortNumber|cope)|Re(?:quire(?:Pkce|Iss)|solutionRule)|(?:JWKSTimeou|Promp)t|I(?:DTokenMaxAge|con)|Display(?:Name)?|NoJwtHeader|MaxAge)|ExportedVars|J(?:SON|WKS))';
our $oidcRPMetaDataNodeKeys = 'oidcRPMetaData(?:Options(?:A(?:uth(?:n(?:Require(?:Nonc|Stat)e|Level)|orizationCodeExpiration|RequiredForAuthorize|Method)|c(?:cessToken(?:E(?:nc(?:ContentEnc|KeyMgt)Alg|xpiration)|SignAlg|Claims|JWT)|tivation)|llow(?:(?:ClientCredentials|Password)Grant|NativeSso|Offline)|dditionalAudiences)|I(?:(?:ntrospection(?:Enc(?:ContentEnc|KeyMgt)|Sign)|dTokenEnc(?:ContentEnc|KeyMgt))Alg|DToken(?:ForceClaims|Expiration|SignAlg)|con)|User(?:I(?:nfo(?:Enc(?:ContentEnc|KeyMgt)|Sign)Alg|DAttr)|infoRequireHeaderToken)|R(?:e(?:freshToke(?:nRotatio)?n|qu(?:estUris|irePKCE)|directUris)|tActivity|ule)|Logout(?:Enc(?:ContentEnc|KeyMgt)Alg|SessionRequired|BypassConfirm|Type|Url)|P(?:ostLogoutRedirectUris|ublic)|C(?:lient(?:Secret|ID)|omment)|OfflineSessionExpiration|TokenXAuthorizedRP|BypassConsent|Jwks(?:Uri)?|DisplayName|ExtraClaims|NoJwtHeader|SigningKey)|(?:ExportedVar|ScopeRule|Macro)s)';
...
our $samlSPMetaDataNodeKeys = 'samlSPMetaData(?:Options(?:F(?:ederation(?:(?:Optional|Required)Attributes|EntityID)|orce(?:NameIDFormat|UTF8))|S(?:ign(?:ature(?:Method|Key)|S[LS]OMessage)|essionNotOnOrAfterTimeout)|N(?:ameID(?:SessionKey|Format)|otOnOrAfterTimeout)|C(?:heckS[LS]OMessageSignature|omment)|En(?:ableIDPInitiatedURL|cryptionMode)|A(?:ctivation|uthnLevel)|(?:OneTimeUs|Rul)e|URL)|(?:ExportedAttribute|Macro)s|XML)';
...
our $authParameters = {
  ...
  restParams => [qw(restAuthnLevel restAuthUrl restUserDBUrl restPwdConfirmUrl restPwdModifyUrl restAuthArgs restUserDBArgs restPwdConfirmArgs restPwdModifyArgs)],
  slaveParams => [qw(slaveAuthnLevel slaveUserHeader slaveDisplayLogo slaveMasterIP slaveCertificateField slaveCertificateRegexp slaveHeaderName slaveHeaderContent slaveExportedVars)],
