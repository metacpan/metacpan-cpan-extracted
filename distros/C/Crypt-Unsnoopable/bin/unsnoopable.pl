...
#
# $Id: bin/unsnoopable.pl v1.008 Sun Dec 10 01:36:27 PST 2017 $
...
capability, and a camera, such as the Noodle Unsnoopable
...
Unsnoopable, using the 'zbar' program. When running unsnoopable on a
Noodle Unsnoopable, use the '--noodlepi' command line switch.
...
screen of the air-gapped Noodle Unsnoopable using a regular connected
...
Unsnoopable (which already has the pre-shared OTP on it). The
ciphertext is then automatically decrypted and the plaintext message
displayed.
...
secure. Noodle Unsnoopable features a MicroSD card reader and boots
off the MicroSD card itself, which is also important as it enables
keeping the boot disk physically secure. An attacker with access to
the air-gapped machine's boot disk could put in trojans to
surreptitiously save OTPs or plaintexts for later retrieval.
