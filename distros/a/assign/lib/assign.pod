...
To preserve backwards compatability for early adopters, the module currently requires you to C<use assign::0;>. When the module becomes stable and vetted, it will become simply C<use assign;>.

=head1 USAGE

Simply add a C<use assign::0;> line to your program, and then you can use any of the assignment forms described below. You must C<use> the module I<before> the first line where you use one of the assignment forms.

If you need to turn off C<assign> after turning it on for some reason, you can use the line: C<no assign>.

=head2 DEBUGGING

To see how the C<assign> module turns the new style assignment forms into plain old Perl code in you program, you can use the C<assign::0->debug()> method.

    require assign::0;
    print assign::0->debug(<perl-file-name>);
    print assign::0->debug(<perl-code-string-as-scalar-ref>);

This will perform the C<assign> transformations on the Perl code and print the result.

You can do that as a perl one-liner like so:

    perl -e 'require assign::0; print assign::0->debug("program.pl")'

Remember to use C<require assign::0;>, not C<use assign::0;>.

=head1 DESTRUCTURING FORMS
...
=item * C<my [$foo, $bar] = $array_ref;>

Define 2 C<my> variables and assign the array-ref values in order.
...
=item * C<my [$foo, $bar] = (111, 222);> B<ERROR>
...
=item * C<our [$foo, $bar] = $array_ref;>

Define C<our> variables and assign the array-ref values.

=item * C<local [$foo, $bar] = $array_ref;>

Define C<local> variables and assign the array-ref values.

=item * C<my $foo; our $bar; [$foo, $bar] = $array_ref;>

Assign the array-ref values to 2 pre-defined variables.

=item * C<my {$foo, $bar} = {bar => 111, foo => 222};>

Define 2 C<my> variables and use the variable name as the key to extract from the hash-ref on the RHS.

=back

=head2 EXTENDED ARRAY REF FORMS

=over

=item * C<my [ $foo, _, _, $bar ] = $array_ref;>

You can skip array values with the C<_> symbol.

=item * C<my [ $foo, $_, $bar ] = $array_ref;>

Unpack into global C<$_>, not a my variable. The C<$foo> and C<$bar> here are still C<my> lexical vars.

=item * C<my [ 7, $foo, 42, $bar ] = $array_ref;>

You can skip any number of array values by using a positive integer.

=item * C<my [ $a, $b=42, $c="1\n2", $d=$a ] = $array_ref;>

You can define default values for variables. Currently the default must be a single token (string, number, variable). Also there must not be whitespace on either side of the C<=>.

=item * C<my [ $a, $b, @rest ] = $array_ref>

Slurp remaining elements into an array variable.

=item * C<my [ $a, $b, @$rest ] = $array_ref>

Slurp remaining elements into an array reference variable (C<$rest>).
...
All of the new assignment forms introduced here would cause Perl syntax or runtime errors without using C<assign.pm>.

Currently this module is a working prototype that uses Filter::Simple (Perl source code filtering) and PPI.pm (to parse and restructure Perl code).
...
Note: Code transformations adjust the line numbers with C<#line <num>> statements so that warnings and errors report line numbers that make sense.

=head1 PRIOR ART

Destructuring assignment is available in many common languages. The C<assign> module got many ideas from these.

=over

=item * L<CoffeeScript|https://alchaplinsky.github.io/hard-rock-coffeescript/syntax/destructuring_assignment.html>

=item * L<JavaScript|https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment>

=item * L<Python|https://riptutorial.com/python/example/14981/destructuring-assignment>

=item * L<Clojure|https://clojure.org/guides/destructuring>
...
=item * L<DestructAssign|https://metacpan.org/pod/DestructAssign>
...
=item * Ingy döt Net C<<<ingy@ingy.net>>>

=item * Kang-min Liu C<<<gugod@gugod.org>>>
