...
BorderStyle::UTF8::SpaceInnerOnly - No borders, but columns are still separated using spaces and data row separator is still drawn using horizontal line

=head1 VERSION

This document describes version 0.007 of BorderStyle::UTF8::SpaceInnerOnly (from Perl distribution BorderStyles-Standard), released on 2021-05-12.
...
To use with L<Text::ANSITable>:
 ...
 $t->border_style("UTF8::SpaceInnerOnly");
 $t->columns($rows->[0]);
...
To use with L<Text::Table::More>:
 ...
 generate_table(rows=>$rows, header_row=>1, separate_rows=>1, border_style=>"UTF8::SpaceInnerOnly");
...
To use with L<Text::Table::TinyBorderStyle>:
 ...
 generate_table(rows=>$rows, header_row=>1, separate_rows=>1, border_style=>"BorderStyle::UTF8::SpaceInnerOnly");
