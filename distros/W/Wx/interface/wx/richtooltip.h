...
%typemap{wxTipKind}{simple};
%typemap{wxRichToolTip*}{simple};
...
/**
    ...
    @since 2.9.3
 */
%name{Wx::RichToolTip} class wxRichToolTip
{
...
public:
    ...
    void SetBackgroundColour(const wxColour& col,
                             const wxColour& colEnd = wxNullColourPtr );
     ...
     */
    //@{
    %name{SetIconId} void SetIcon(int icon = wxICON_INFORMATION) %Overload;
    %name{SetIconIcon} void SetIcon(const wxIcon& icon) %Overload;
    //@}
     ...
     */
    ~wxRichToolTip()
        %code%{  wxPli_thread_sv_unregister( aTHX_ "Wx::RichToolTip", THIS, ST(0) );
                 delete THIS;
               %};
};
