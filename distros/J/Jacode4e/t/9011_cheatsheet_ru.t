...
#
#   $line             : строка для преобразования (передаётся по ссылке, перезаписывается)
#   $OUTPUT_encoding  : название выходной кодировки
#   $INPUT_encoding   : название входной кодировки
#   $char_count       : количество символов после преобразования
#
# [НАЗВАНИЯ КОДИРОВОК]
#
#   мнемоника   описание
#   ---------   ---------------------------------------------------
#   cp932x      CP932X (расширение до JIS X 0213, одиночный сдвиг 0x9C5A)
#   cp932       Microsoft CP932 (Windows-31J / имя IANA)
#   cp932ibm    IBM CP932
#   cp932nec    NEC CP932
...
#   jef         FUJITSU JEF (12pt, коды сдвига с OUTPUT_SHIFTING)
#   jef9p       FUJITSU JEF ( 9pt, коды сдвига с OUTPUT_SHIFTING)
#   jipsj       NEC JIPS(J)
#   jipse       NEC JIPS(E)
#   letsj       UNISYS LetsJ
#   utf8        UTF-8.0 (обычный UTF-8)
#   utf8.1      UTF-8.1 (преобразование на основе таблицы Shift_JIS-Unicode, не CP932)
#   utf8jp      UTF-8-SPUA-JP (JIS X 0213 отображён в зону SPUA Unicode)
#
# [ПАРАМЕТРЫ]
#
#   ключ              значение / описание
#   ---------------   ---------------------------------------------------
#   INPUT_LAYOUT      разметка входной записи ('S'=SBCS, 'D'=DBCS, со счётчиком)
#   OUTPUT_SHIFTING   если истина, добавляет коды сдвига вокруг последовательностей DBCS
#   SPACE             код пробела DBCS/MBCS (двоичная строка)
#   GETA              символ гэта для несопоставимых символов
#   OVERRIDE_MAPPING  замена по символу { "\x12\x34"=>"\x56\x78" }
#
# [ПРЕДУПРЕЖДЕНИЕ: ДВУНАПРАВЛЕННОЕ ПРЕОБРАЗОВАНИЕ]
#
#   Существуют потери при преобразовании (напр. CP932 имеет 398 необратимых отображений).
#   Для двунаправленного преобразования используйте модуль Jacode4e::RoundTrip.
#
# ======================================================================
...
BEGIN {
    ...
    @test = (
        ...
        # --- UTF-8.1: преобразование на основе Shift_JIS-Unicode, не CP932 ---
