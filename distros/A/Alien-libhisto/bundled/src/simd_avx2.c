...
bool histo_fill_uniform_w2_avx2(histo_t *h, const double *x, const double *weights, size_t n) {
    ...
    for (; i + 3 < n; i += 4) {
        __m256d v_x = _mm256_loadu_pd(&x[i]);
        __m256d v_w = _mm256_loadu_pd(&weights[i]);

        __m256d mask_under = _mm256_cmp_pd(v_x, v_min, _CMP_LT_OQ);
        __m256d mask_over  = _mm256_cmp_pd(v_x, v_max, _CMP_GE_OQ);
        __m256d mask_nan_x = _mm256_cmp_pd(v_x, v_x, _CMP_NEQ_UQ);

        __m256d v_w_sub    = _mm256_sub_pd(v_w, v_w);
        __m256d mask_bad_w = _mm256_cmp_pd(v_w_sub, v_w_sub, _CMP_NEQ_UQ);

        __m256d mask_any_bad = _mm256_or_pd(
            _mm256_or_pd(mask_under, mask_over),
            _mm256_or_pd(mask_nan_x, mask_bad_w)
        );
        int bad_mask = _mm256_movemask_pd(mask_any_bad);
        ...
        if (bad_mask != 0) {
            ...
            __m256d v_w2 = _mm256_mul_pd(v_w, v_w);
            __m256d v_idx = _mm256_floor_pd(_mm256_mul_pd(_mm256_sub_pd(v_x, v_min), v_inv_binsize));
            v_idx = _mm256_max_pd(v_idx, v_zero);
            v_idx = _mm256_min_pd(v_idx, v_nbins_minus_1);
            ...
            double w2_arr[4];
            _mm256_storeu_pd(idx_arr, v_idx);
            _mm256_storeu_pd(w_arr, v_w);
            _mm256_storeu_pd(w2_arr, v_w2);
...
bool histo2d_fill_uniform_w2_avx2(histo2d_t *h, const double *x, const double *y, const double *weights, size_t n) {
    ...
    for (; i + 3 < n; i += 4) {
        ...
        __m256d mask_any_bad = _mm256_or_pd(
            _mm256_or_pd(
                _mm256_or_pd(mask_under_x, mask_over_x),
                _mm256_or_pd(mask_under_y, mask_over_y)
            ),
            _mm256_or_pd(
                _mm256_or_pd(mask_nan_x, mask_nan_y),
                mask_bad_w
            )
        );
