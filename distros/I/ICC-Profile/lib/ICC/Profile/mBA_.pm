...
sub new {
	...
	if (@_) {
		...
		if (@_ == 1 && ref($_[0]) eq 'HASH') {
			...
			# make new mBA_ tag from attribute hash
...
# create mBA_ tag object from ICC profile
...
# writes mBA_ tag object to ICC profile
# parameters: (ref_to_parent_object, file_handle, ref_to_tag_table_entry)
sub write_fh {
	...
	# write mBA_ data to profile
	_writeICCmBA_($self, @_);
...
sub _check {
	...
	($#{$self->[1]} <= 4) || croak('\'mBA_\' object has too many processing elements');
	...
	for my $i (0 .. $#{$self->[1]}) {
		...
		if (defined($self->[1][$i])) {
			...
			(ref($self->[1][$i]) eq $class[$i]) || croak("'mBA_' processing element $i has wrong class");
			...
			if ($self->[1][$i]->can('cin')) {
				...
				if (defined($co)) {
					...
					($self->[1][$i]->cin() == $co) || croak("'mBA_' processing element $i has wrong number of channels");
	...
	if (defined($self->[1][1])) {
		...
		($ci == 3) || croak("matrix processing element not permitted with $ci input channels");
...
# equivalent to inverse of a matrix-based display profile
# parameters: (ref_to_mAB_object, ref_to_profile_object)
sub _newICCmatrix {
	...
	# make Math::Matrix object from primary tags (inverse)
	$mat = Math::Matrix->new(map {$_->XYZ()} @XYZ)->transpose->invert->multiply_scalar(65535/32768);
	...
	# set M-curves (inverse)
	$self->[1][2] = ICC::Profile::cvst->new([map {$_->inv()} @TRC]);
...
# read mBA_ tag from ICC profile
...
sub _readICCmBA_ {
	...
	($mft[0] eq 'mBA ') or croak('wrong tag type');
