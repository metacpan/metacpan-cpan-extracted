...
sub basic_checks {
    ...
    is( $xpc->findvalue('/nar:packageItem/nar:itemMeta/nar:itemClass/@qcode'),
        'ninat:composite', 'correct itemClass'
    );
    is( $xpc->findvalue('/nar:packageItem/nar:groupSet/@root'),
        'root_group', 'correct root group id' );
    ok( $xpc->find(
            '/nar:packageItem/nar:groupSet/nar:group[@id="root_group"]'),
        'group with correct id exists'
    );
...
cmp_ok( @{ $pi->root_group->items }, '==', 2,
    'root group has two items now' );

my %schemes = (
    pgrmod => XML::NewsML_G2::Scheme->new(
        alias => 'pgrmod',
        catalog =>
