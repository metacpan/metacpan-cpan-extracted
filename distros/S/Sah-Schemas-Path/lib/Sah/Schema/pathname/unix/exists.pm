...
Sah::Schema::pathname::unix::exists - Unix path name, must exist on filesystem

=head1 VERSION

This document describes version 0.030 of Sah::Schema::pathname::unix::exists (from Perl distribution Sah-Schemas-Path), released on 2024-01-08.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("pathname::unix::exists*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("pathname::unix::exists", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("pathname::unix::exists", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("pathname::unix::exists*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"pathname::unix::exists*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('pathname::unix::exists*', name=>'PathnameUnixExists')
     );
 }

 use My::Types qw(PathnameUnixExists);
 PathnameUnixExists->assert_valid($data);
