#!/usr/bin/perl
# This is OgreAL's "MultiChannel" demo ported to Perl.
# Read README.txt in this directory for how to set
# up the resources for this demo.
#
# A lot of the code here is duplicated from directional.pl.
#
# See Demos/MultiChannel/ in the OgreAL distribution.
...
sub keyPressed {
    ...
    if ($key == OIS::Keyboard->KC_ESCAPE) {
        ...
        Ogre::AL::SoundManager->getSingletonPtr->getSound("5.1 Test")->play();
    }
    elsif ($key == OIS::Keyboard->KC_F2) {
        Ogre::AL::SoundManager->getSingletonPtr->getSound("5.1 Test")->pause();
    }
    elsif ($key == OIS::Keyboard->KC_F3) {
        Ogre::AL::SoundManager->getSingletonPtr->getSound("5.1 Test")->stop();
    }
...
use Ogre 0.39 qw(:SceneType :ShadowTechnique :GuiMetricsMode);
...
sub createScene {
    ...
    my $sound = $self->{soundManager}->createSound("5.1 Test", "6chan.ogg", 1);
    $sound->setGain(2);
#XXX: TODO
#    $sound->addSoundFinishedHandler($self, $self->can('soundFinished'));
#    $sound->addSoundLoopedHandler($self, $self->can('soundLooped'));
