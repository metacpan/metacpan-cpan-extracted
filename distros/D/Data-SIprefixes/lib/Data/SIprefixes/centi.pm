...
Data::SIprefixes::centi - This provides centi matching for Data::SIprefixes.
...
=head1 SYNOPSIS
    ...
    my $prefix=Data::SIprefixes::centi->new;

    my $origMeasure='centimeter';
...
sub longMatch{
	...
	if ( $measure=~/^centi/ ){
		...
		if ( $measure =~ /^ / ){
			$self->{error}=2;
			$self->{errorString}='Space found after prefix, /^centi/, in "'.$origMeasure.'"' ;
			$self->warn;
			return undef;
		}
...
sub shortMatch{
    ...
    if ( $measure=~/^c/ ){
        ...
        if ( $measure =~ /^ / ){
            $self->{error}=2;
            $self->{errorString}='Space found after prefix, /^c/, in "'.$origMeasure.'"' ;
            $self->warn;
            return undef;
        }
...
1; # End of Data::SIprefixes::centi
