...
We have tested pmi.pm, a component of Bigram Statistics Package version
0.5. File pmi.pm has the implementation of the pointwise mutual  
information statistic. Note that pmi.pm is not a program by itself, but is  
a statistical library package that is plugged into statistic.pl. We test 
here the features of pmi.pm by running statistic.pl.

Following is a description of the aspects of pmi.pm that we have
tested. We provide the scripts and files used for testing so that
later versions of pmi.pm can be tested for backward compatibility.
...
Phase 1: Testing pmi.pm's response to erroneous conditions. 
Phase 2: Testing pmi.pm's behaviour under normal conditions. 


2.1. Phase 1: Testing pmi.pm's response to erroneous conditions:
----------------------------------------------------------------------
...
pmi.pm is meant only for bigrams. This test checks if pmi.pm throws
an error when provided with larger n-grams.

2.1.2. Subtest 2: 
-----------------

pmi.pm requires three frequency values: the frequency of the bigram,
...
not provided, pmi.pm should throw an error. This test checks for this
error. 
...
checks if pmi.pm does indeed throw an error when passed a total
bigrams value that is less than or equal to zero.

2.1.4. Subtest 4: 
-----------------

The numbers passed to pmi.pm should be "valid" in that they should
represent a possible two-by-two table. If this is not the case,
various warnings are thrown by pmi.pm. This test checks for these
warnings. 
...
the last bigram "three<>seven<>" has a ngram frequency greater than
its marginal totals, for which a warning should be produced. 


2.2. Phase 2: Testing pmi.pm's behaviour under normal conditions:
------------------------------------------------------------------
...
First, it checks if pmi.pm works when the frequencies are given in a
...
Second it checks the actual calculations of pmi.pm. The output target
...
Statistical library package pmi.pm has been tested for erroneous
