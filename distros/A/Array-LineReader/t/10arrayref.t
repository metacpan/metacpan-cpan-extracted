...
### access offset and content via a reference to an array
...
ok(tie(@lines, $TEST_CLASS, $TEST_FILE, result=>[]));
ok(join("",map{$_->[0]}@lines),join("",@testOffsets));	# equal offsets;
ok(join("",map{$_->[1]}@lines),join("",@testLines));	# equal content;
untie @lines;

### Test if the reversed lines contain equal contents ###
ok(tie(@lines, $TEST_CLASS, $TEST_FILE, result=>[]));
ok(join("",map{$_->[0]}reverse @lines),join("",reverse @testOffsets));	# equal offsets;
ok(join("",map{$_->[1]}reverse @lines),join("",reverse @testLines));	# equal content;
untie @lines;
