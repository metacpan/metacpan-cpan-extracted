...
The B<error> method returns the L<Net::API::Stripe::Exception> object, which inherits from L<Module::Generic::Exception>. Please refer to the manual page of L<Net::API::Stripe::Exception> for more information, but essentially, the following methods are available with the error objects:
...
The error param. For example: C<payment_method>
...
The error type, if any. This can be any of C<api_error>, C<card_error>, C<idempotency_error>, C<invalid_request_error>
