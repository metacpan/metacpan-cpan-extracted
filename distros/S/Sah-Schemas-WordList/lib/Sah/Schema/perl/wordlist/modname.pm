...
This document describes version 0.005 of Sah::Schema::perl::wordlist::modname (from Perl distribution Sah-Schemas-WordList), released on 2023-01-19.
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("perl::wordlist::modname", {return_type=>'str_errmsg'});
...
prefiltered) value:
 ...
 my $validator = gen_validator("perl::wordlist::modname", {return_type=>'str_errmsg+val'});
 my $res = $validator->($data); # [$errmsg, $validated_val]
 
 # a sample valid data
 $data = "EN/Enable";
 my $res = $validator->($data); # => ["","EN::Enable"]
...
To create a type constraint and type library from a schema:
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('$sch_name*', name=>'PerlWordlistModname')
     );
 }

 use My::Types qw(PerlWordlistModname);
 PerlWordlistModname->assert_valid($data);
