...
for my $eg (@eg_files) {
    ...
    # E6: Demonstrates comment lists only methods actually used in code
    # JSON::LINQ methods start with uppercase (From, Where, Select, etc.)
    # Extract tokens matching: "Word:" or "Word," at start of bullet item
    my %demo_methods;
    for my $line (@lines) {
        if ($line =~ /^#\s+-\s+(.+)/) {
            my $desc = $1;
            # Match "MethodName:" and "MethodName," patterns (method name hints)
            while ($desc =~ /\b([A-Z][A-Za-z]+)(?::|,)/g) {
                my $tok = $1;
                # Skip plain English words that are not JSON::LINQ methods
                next if $tok =~ /^(Load|Build|Sort|Group|Left|Right|Top|Sum|
                                    Any|All|Get|Set|The|For|Per|Via|And|Or|
                                    No|On|To|In|Of|By|Up|As|At)$/xi;
                $demo_methods{$tok}++;
            }
        }
    }
