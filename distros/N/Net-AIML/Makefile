# This Makefile is for the Net::AIML extension to perl.
#
...
#     ABSTRACT => q[Perl interface to the Pandorabots.com AIML server]
#     AUTHOR => q[Chris Prather  C<< <cpan@prather.org> >>]
#     DIR => []
#     DISTNAME => q[Net-AIML]
#     NAME => q[Net::AIML]
#     NO_META => q[1]
...
TO_INST_PM = lib/Net/AIML.pm

PM_TO_BLIB = lib/Net/AIML.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Net/AIML.pm $(INST_MAN3DIR)/Net::AIML.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	- $(RM_RF) \
	  t/000_standard__perl_critic.t t/000_standard__pod_coverage.t \
	  t/000_standard__compile.t t/000_standard__pod.t \
	  $(DISTVNAME) 
...
TEST_FILE = test.pl
TEST_FILES = t/00.load.t t/000_standard__compile.t t/000_standard__perl_critic.t t/000_standard__pod.t t/000_standard__pod_coverage.t
TESTDB_SW = -d
...
ppd:
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="HTTP-Request-Common" VERSION="1,26,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="LWP-Simple" VERSION="1,41,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="LWP-UserAgent" VERSION="2,033,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Test-Compile" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="XML-Smart" VERSION="1,006009,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/Net/AIML.pm blib/lib/Net/AIML.pm 
