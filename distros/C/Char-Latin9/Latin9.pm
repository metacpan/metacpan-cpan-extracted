...
######################################################################
#
# Latin9 - Source code filter to escape Latin-9 script
#
# http://search.cpan.org/dist/Char-Latin9/
#
...
perl $0 Latin-9_script.pl > Escaped_script.pl.e
END
    }

    print Latin9::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (CORE::eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Elatin9::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
        }
    ...
    my $fh = gensym();
    Elatin9::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Latin9::escape_script {
    ...
    # read Latin-9 script
    my $fh = gensym();
    Elatin9::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script\n";

    if (/^ use Elatin9(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Elatin9 '%s.0'; # 'quote' for perl5.6\n", $Latin9::VERSION); # require run-time routines version

        # use Latin9 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Latin9 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Latin9::VERSION) {
                    my @file = grep -e, map {qq{$_/Latin9.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Latin9.pm $version, but @{[__FILE__]} is version $Latin9::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Elatin9.pm $version, but \\\$Elatin9::VERSION is \$Elatin9::VERSION" if \$Elatin9::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Latin9::VERSION) {
                    die "Script $0 required Latin9.pm $version, but @{[__FILE__]} is only version $Latin9::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Elatin9::function_ord     = 'Latin9::ord'     if /\A ord \z/oxms;
                    $Elatin9::function_ord_    = 'Latin9::ord_'    if /\A ord \z/oxms;
                    $Elatin9::function_reverse = 'Latin9::reverse' if /\A reverse \z/oxms;
                    $Elatin9::function_getc    = 'Latin9::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Latin9::escape();
...
=item * You'll have to download Latin9.pm and Elatin9.pm and put it in your perl lib directory.

=item * You'll need to write "use Latin9;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Latin9;
  use Latin9 ver.sion;             --- require minimum version
  use Latin9 ver.sion.0;           --- expects version (match or die)

  # "no Latin9;" not supported

  or

  $ perl Latin9.pm Latin-9_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Latin-9_script.pl  --- script written in Latin-9
...
Latin9 software is "middleware" between perl interpreter and your Perl script
written in Latin-9.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Latin-9, Perl3 or later
can treat Latin-9 as binary data.

By "use Latin9;", it automatically interpret your script as Latin-9. The various
functions of perl including a regular expression can treat Latin-9 now.
...
This software is a source code filter to escape Perl script encoded by Latin-9
...
=item * Remains One Language Handling Raw Latin-9, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Latin9.pm, Elatin9.pm
  +---------------------------------------+
...
import of Elatin9.pm works well.
...
=head1 Software Composition
   ...
   Latin9.pm               --- source code filter to escape Latin-9
   Elatin9.pm              --- run-time routines for Latin9.pm
...
You need write 'use Latin9;' in your script.
...
=head1 Calling 'Elatin9::ignorecase()' (Latin9 software provides)

Latin9 software applies calling 'Elatin9::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Elatin9::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Elatin9.pm provides)
...
Also POSIX-style character classes.
  ...
  [:^alnum:]    ${Elatin9::not_alnum}
  [:^alpha:]    ${Elatin9::not_alpha}
  [:^ascii:]    ${Elatin9::not_ascii}
  [:^blank:]    ${Elatin9::not_blank}
  [:^cntrl:]    ${Elatin9::not_cntrl}
  [:^digit:]    ${Elatin9::not_digit}
  [:^graph:]    ${Elatin9::not_graph}
  [:^lower:]    ${Elatin9::not_lower}
                ${Elatin9::not_lower_i}    (/i modifier)
  [:^print:]    ${Elatin9::not_print}
  [:^punct:]    ${Elatin9::not_punct}
  [:^space:]    ${Elatin9::not_space}
  [:^upper:]    ${Elatin9::not_upper}
                ${Elatin9::not_upper_i}    (/i modifier)
  [:^word:]     ${Elatin9::not_word}
  [:^xdigit:]   ${Elatin9::not_xdigit}
  ---------------------------------------------------------------
...
Definitions in Elatin9.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Elatin9::dot}            qr{(?>[^\x0A])};
  ${Elatin9::dot_s}          qr{(?>[\x00-\xFF])};
  ${Elatin9::eD}             qr{(?>[^0-9])};
  ${Elatin9::eS}             qr{(?>[^\s])};
  ${Elatin9::eW}             qr{(?>[^0-9A-Z_a-z])};
  ${Elatin9::eH}             qr{(?>[^\x09\x20])};
  ${Elatin9::eV}             qr{(?>[^\x0A\x0B\x0C\x0D])};
  ${Elatin9::eR}             qr{(?>\x0D\x0A|[\x0A\x0D])};
  ${Elatin9::eN}             qr{(?>[^\x0A])};
  ${Elatin9::not_alnum}      qr{(?>[^\x30-\x39\x41-\x5A\x61-\x7A])};
  ${Elatin9::not_alpha}      qr{(?>[^\x41-\x5A\x61-\x7A])};
  ${Elatin9::not_ascii}      qr{(?>[^\x00-\x7F])};
  ${Elatin9::not_blank}      qr{(?>[^\x09\x20])};
  ${Elatin9::not_cntrl}      qr{(?>[^\x00-\x1F\x7F])};
  ${Elatin9::not_digit}      qr{(?>[^\x30-\x39])};
  ${Elatin9::not_graph}      qr{(?>[^\x21-\x7F])};
  ${Elatin9::not_lower}      qr{(?>[^\x61-\x7A])};
  ${Elatin9::not_lower_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Elatin9::not_lower_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
  ${Elatin9::not_print}      qr{(?>[^\x20-\x7F])};
  ${Elatin9::not_punct}      qr{(?>[^\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E])};
  ${Elatin9::not_space}      qr{(?>[^\s\x0B])};
  ${Elatin9::not_upper}      qr{(?>[^\x41-\x5A])};
  ${Elatin9::not_upper_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Elatin9::not_upper_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
  ${Elatin9::not_word}       qr{(?>[^\x30-\x39\x41-\x5A\x5F\x61-\x7A])};
  ${Elatin9::not_xdigit}     qr{(?>[^\x30-\x39\x41-\x46\x61-\x66])};
  
  # This solution is not perfect. I beg better solution from you who are reading this.
  ${Elatin9::eb}             qr{(?:\A(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[0-9A-Z_a-z])|(?<=[0-9A-Z_a-z])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]|\z))};
  ${Elatin9::eB}             qr{(?:(?<=[0-9A-Z_a-z])(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]))};
  ---------------------------------------------------------------------------------------------------------------------------------------------------------

=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Latin9 software provides)

Latin9 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Latin9 software provides)

Insert 'Elatin9::' at head of function name. Elatin9.pm provides your script Elatin9::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Elatin9::lc        Character
  lcfirst     Elatin9::lcfirst   Character
  uc          Elatin9::uc        Character
  ucfirst     Elatin9::ucfirst   Character
  fc          Elatin9::fc        Character
  chr         Elatin9::chr       Character
  glob        Elatin9::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Latin9 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Latin9 software provides)

Latin9 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Elatin9.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Latin9 software and
...
Latin9 software remains one language and one interpreter.
