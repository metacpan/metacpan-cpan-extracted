...
# regexp of character
my $qq_char   = qr/(?> \\c[\x40-\x5F] | \\? (?:[\x81-\xFE][^\x30-\x39]|[\x81-\xFE][\x30-\x39][\x81-\xFE][\x00-\xFF] | [\x00-\xFF]) )/oxms;
my  $q_char   = qr/(?> [\x81-\xFE][^\x30-\x39]|[\x81-\xFE][\x30-\x39][\x81-\xFE][\x00-\xFF] | [\x00-\xFF] )/oxms;
...
sub GB18030::escape_script {
    ...
    if (/^ use Egb18030(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        ...
        if (s/^ (?>\s*) use (?>\s+) (?: Char | GB18030 ) (?>\s*) ([^\x81-\xFE;]*) ; \s* \n? $//oxms) {
...
END
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Egb18030::function_ord     = 'GB18030::ord'     if /\A ord \z/oxms;
                    $Egb18030::function_ord_    = 'GB18030::ord_'    if /\A ord \z/oxms;
                    $Egb18030::function_reverse = 'GB18030::reverse' if /\A reverse \z/oxms;
                    $Egb18030::function_getc    = 'GB18030::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= GB18030::escape();
...
=head1 Escaping Multiple-Octet Code (GB18030 software provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (GB18030 software provides)

GB18030 software applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Escaping Second Octet (GB18030 software provides)

GB18030 software escapes second octet of multiple-octet character in regular
expression.
...
=head1 Multiple-Octet Character Regular Expression (GB18030 software provides)

GB18030 software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Egb18030::ignorecase()' (GB18030 software provides)

GB18030 software applies calling 'Egb18030::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Egb18030::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Egb18030::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Egb18030::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Egb18030::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
Definitions in Egb18030.pm.
  ...
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Egb18030::anchor}         qr{\G(?>[^\x81-\xFE]|[\x81-\xFE][^\x30-\x39]|[\x81-\xFE][\x30-\x39][\x81-\xFE][\x00-\xFF])*?}
                           for over 32766 octets string on ActivePerl5.6 and Perl5.10 or later
                           qr{\G(?(?=.{0,32766}\z)\G(?>[^\x81-\xFE]|[\x81-\xFE][^\x30-\x39]|[\x81-\xFE][\x30-\x39][\x81-\xFE][\x00-\xFF])*?|(?(?=[\x00-\x80\xFF]+\z).*?|(?:.*?[^\x30-\x39\x81-\xFE](?>[\x30-\x39]|[\x81-\xFE][\x81-\xFE]|[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39])*?)))}oxms
                           for over 32766 octets string on ActivePerl5.6 and Perl5.10 or later
                           qr{\G(?(?=.{0,32766}\z)\G(?>[^\x81-\xFE]|[\x81-\xFE][^\x30-\x39]|[\x81-\xFE][\x30-\x39][\x81-\xFE][\x00-\xFF])*?|(?(?=[$sbcs]+\z).*?|(?:.*?[$sbcs](?:[^$sbcs][^$sbcs])*?)))}oxms
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (GB18030 software provides)

GB18030 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (GB18030 software provides)
...
=head1 Escaping File Test Operators (GB18030 software provides)
...
=item * Eval GB18030 Script

  $eval = GB18030::eval { block };
  $eval = GB18030::eval $expr;
  $eval = GB18030::eval;

  The GB18030::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, GB18030::eval { block }
  and GB18030::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, GB18030::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of GB18030::eval, the value returned from an GB18030::eval is
  ...
  GB18030::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  GB18030::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      GB18030::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The GB18030::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow GB18030::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  GB18030::eval cannot trap syntax errors.

  The GB18030::eval $expr form can trap syntax errors because it parses the code
  ...
  after the GB18030::eval is complete, as do any subroutine or format definitions.
  The code of the GB18030::eval is treated as a block, so any locally scoped
  variables declared within the GB18030::eval last only until the GB18030::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  GB18030::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  GB18030::eval { block }    eval { block }
  GB18030::eval $expr        eval GB18030::escape $expr
  GB18030::eval              eval GB18030::escape
  -------------------------------------------------

  To tell the truth, the subroutine GB18030::eval does not exist. If it exists,
  you will troubled, when GB18030::eval has a parameter that is single quoted
  string included my variables. GB18030::escape is a subroutine that makes Perl
  ...
  Since GB18030::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because GB18030::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  GB18030::eval $expr and GB18030::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      GB18030::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      GB18030::eval { $answer = }; # WRONG

      # a runtime syntax error
      GB18030::eval '$answer =';   # sets $@
  ...
  Using the GB18030::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      GB18030::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          GB18030::eval { die "foo lives here" };
  ...
  With an GB18030::eval, you should be especially careful to remember what's being
  looked at when:

      GB18030::eval $x;        # CASE 1
      GB18030::eval "$x";      # CASE 2

      GB18030::eval '$x';      # CASE 3
      GB18030::eval { $x };    # CASE 4

      GB18030::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  The block of GB18030::eval { block } does not count as a loop, so the loop
...
=head1 Yada Yada Operator (GB18030 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (GB18030 software provides)

GB18030 software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in GB18030 software and
