...
sub showDetail{
	...
	foreach my $rowData( @$detailArray ){
                ...
                # Make copy of row data because we lop off the first item (which is in the title area)
                #   so it can be pasted using PasteHandler
                my @rowCopy = @$rowData;   
                shift @rowCopy;
                my $insertData = join("\t", @rowCopy);        # make insert data look like tcl array, so it
								# gets put in different cells
		#$self->set('row', "$rowNum,$colorigin", $insertData);
		$self->PasteHandler("$rowNum,".($colorigin+1), $insertData);
		$rowNum++;
	}
...
sub GeneralMotion{
	...
	$self->SUPER::GeneralMotion($mw,@_);
	
	my ($row,$col) = split(',',$rc);

	my @border = $self->border('mark',$x,$y);
