...
sub new {
    ...
    if ( exists($args->{q[attr]}) ) { do { package MyTest::Mite; ref($args->{q[attr]}) eq 'CODE' } or require Carp && Carp::croak(q[Type check failed in constructor: attr should be CodeRef]); $self->{q[attr]} = $args->{q[attr]};  } else { my $value = do { my $default_value = do { my $method = $MyTest::TestClass::Code::__attr_DEFAULT__; $self->$method }; (ref($default_value) eq 'CODE') or do { require Carp; Carp::croak(q[Type check failed in default: attr should be CodeRef]) }; $default_value }; $self->{q[attr]} = $value;  }
...
*_set_attr = sub { (ref($_[1]) eq 'CODE') or require Carp && Carp::croak(q[Type check failed in writer: value should be CodeRef]); $_[0]{q[attr]} = $_[1]; $_[0]; };
