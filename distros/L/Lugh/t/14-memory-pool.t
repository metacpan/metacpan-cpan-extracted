...
ok(scalar(@logits) > 0, 'forward_with_pool returns logits');
...
my @logits_standard = $inference->forward(\@tokens);
my $pool2 = $inference->create_memory_pool();
my @logits_pool2 = $inference->forward_with_pool($pool2, \@tokens);
...
my @logits_reuse = $inference->forward_with_pool($pool, \@tokens2);
...
my @logits_short = $inference->forward_with_pool($pool, \@short_tokens);
...
my @logits_long = $inference->forward_with_pool($pool, \@long_tokens);
...
my @logits_a = $inference->forward_with_pool($pool_a, \@tokens);
my @logits_b = $inference->forward_with_pool($pool_b, \@tokens);
...
push @tokens, $sampled;
my @next_logits = $inference->forward_with_pool($pool, \@tokens);
