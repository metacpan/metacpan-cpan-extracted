...
my @tests = (
	...
	{
		fen => '6kn/8/8/8/8/8/8/KBB5 w - - 10 20',
		name => 'KN vs. kbb',
		draw => 0,
	},
);
...
my $draw = Chess::Plisco::Engine::Tree::DRAW;
foreach my $test (@tests) {
	my $pos = Chess::Plisco::Engine::Position->new($test->{fen});
	if ($test->{draw}) {
		is $pos->evaluate, $draw, "$test->{name} should be draw";
	} else {
		isnt $pos->insufficientMaterial, $draw, "$test->{name} should not be draw";
	}
}
