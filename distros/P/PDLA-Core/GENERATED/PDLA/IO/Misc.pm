...
#
package PDLA::IO::Misc;

@EXPORT_OK  = qw(  rcols wcols swcols rgrep rdsa PDLA::PP bswap2 PDLA::PP bswap4 PDLA::PP bswap8  isbigendian  rasc rcube PDLA::PP _rasc );
...
# Internal routine to extend PDLA array by size $n along last dimension
...
# Implements PDLA->at() for either 1D PDLA or ARRAY arguments
...
sub _handle_types ($$$) {
    ...
    barf "Unknown PDLA type given for DEFTYPE.\n"
...
see L<PDLA::Core/Datatype_conversions>.
...
Options (case insensitive):
  ...
  stored in C<$PDLA::IO::Misc::deftype>, which starts off as B<double>).
  ...
  VERBOSE
  - be verbose about IO processing (default C<$PDLA::vebose>)

=for example

For example:

  $x      = PDLA->rcols 'file1';	        # file1 has only one column of data
  $x      = PDLA->rcols 'file2', [];	# file2 can have multiple columns, still 1 piddle output
  ...
  2. Columns are separated by whitespace by default, use the COLSEP option
     ...
     alternate default separator by setting C<$PDLA::IO::Misc::defcolsep> .
  
  3. Legacy support is present to use C<$PDLA::IO::Misc::colsep> to set the
     column separator but C<$PDLA::IO::Misc::colsep> is not defined by default.
...
RCOLS_IO: {
   ...
   while( defined($fhline = <$fh>) ) {
      ...
      # Prior to PDLA 2.003 the line checks were done BEFORE the
      # pattern matching
      #
      ...
      if ( $index == 0 ) {
         ...
         for (@explicit_cols) {
            ...
            # between memory usage (perl list) and speed of IO (PDLA operations)
            if (ref($_) eq "ARRAY") {
      ...
      # if necessary, extend PDLA in buffered manner
      $k = 0;
      if ( $pdlsize < $index ) {
...
sub PDLA::wcols {
   ...
   my $file = $_[-1];
   my $file_opened;
   my $is_handle = !UNIVERSAL::isa($file,'PDLA') &&
...
=for ref
  ...
  The default value is to assign C<$PDLA::undefval> to undefined values,
  but if C<UNDEFINED> is set this is used instead. This would normally 
  be set to a number, but if it is set to C<Bad> and PDLA is compiled
  with Badvalue support (see L<PDLA::Bad/>) then undefined values are set to
...
=cut
  ...
  sub rgrep (&@) {
     ...
     my (@ret,@v,$nret); my ($m,$n)=(-1,0); # Count/PDLA size
     my $pattern = shift;
     ...
     if ($undef_is_bad && !$PDLA::Bad::Status) {
	carp "UNDEFINED cannot be set to Badvalue when PDLA hasn't been compiled with Bad value support - \$PDLA::undefval used instead\n";
        $undef_is_bad = 0;
     }        
     barf "Unknown PDLA type given for DEFTYPE.\n"
     ...
     while(<$fh>) {
       ...
       if ($n<$m) {
           for (0..$nret) {
               _ext_lastD( $ret[$_]->[0], $$options{BUFFERSIZE} ); # Extend PDLA in buffered manner
           }
...
=for ref
  ...
  Requires non-PDLA DSA module. Contact Frossie (frossie@jach.hawaii.edu)
...
sub PDLA::isbigendian { return 0; };
*isbigendian = \&PDLA::isbigendian;
...
the file COPYING in the PDLA distribution. If this file is
separated from the PDLA distribution, the copyright notice
