...
set event listeners or callbacks will be discarded. For the interface see
C<event> function.
...
All previously set event listeners or callbacks will be discarded. For the
...
class having C<on_fs_start> and C<on_fs_poll> methods. The object
can optionally be weakened.
