...
map keycodes to 16-bit masks (or bitmaps).  (The customary names for lower bits of the mask are C<KBDSHIFT>,
C<KBDCTRL>, C<KBDALT>, C<KBDKANA>; two more bits are named C<KBDROYA> and C<KBDLOYA> — after
OYAYUBI 親指, meaning THUMB; ten more
bits are unnamed.)  The keycodes of the currently pressed keys (from the “internal” table) are translated to bitmaps, and
these bitmaps are ORed together.  (For the purpose of translation to C<WM_CHAR>/etc [done
...
The keyboard layout tables translate the ORed bitmaps to a number called “modification column”.
...
then the bit C<KBDALT> is removed from the ORed bitmaps.  (We call this “C<KBDALT>
stripping” below.  B<Note:> this does not happen on “key up” events — which is very
important in the context of “non-destructive C<ToUnicode[Ex]()> calls.)
...
(If the ORed bitmaps corresponds to a valid modification column, but the row does not
...
The resulting units are fed to the finite state machine.  When the state machine is in
0-state, a fed character unit is passed through, and a fed deadkey ID sets the state
of the state machine to this number.  In non-0 state, the IDs behave the
same as numerically equal character units; the behaviour is described by the keyboard layout
tables.  The state machine changes the state according to the input; it may also emit a character
(= 1 code unit; then it is always reset to 0 state).  When “unrecognized input” arrives, the state machine
emits the ID I<and> the input, and resets to 0 state.

(On KEYUP event, the changes to the state of the finite-state-machine are ignored.  This is only
...
state machine is restored.

=item 9

After UTF-16 codepoints are passed through the state machine, its output is returned by ToUnicode().
If the state machine is in non-0 state, the state ID becomes the output.
...
I<many> slots good for 4-chars strings.  This is limited by the type C<BYTE> used to specify
the maximal length of an entry in a ligature table; so this length cannot be more than 255.

B<Implementation detail:> to use “more than one slot”, one should expoit the fact that in all
4 CPU architectures supported by C<MSKLC>, the structures C<LIGATURE{n}> is little-endian and
aligned on 2-byte boundary — so its first element can be safely extended from C<BYTE> to
C<WORD>. — And then one can put a C<WCHAR> there!  B<Keep in mind:> when OS matches a “ligature
record” and a keypress, the padding byte is ignored.  However, if such “overhanging records”
are at the end, this can lead to problems very rarely only.

B<NOTE:> If the application uses the standard message pump
...
finite state machine, the transitions to non-0 state do not generate deadkey IDs in the output
...
B<NOTE:> How the kernel recognizes which key sequences contribute to
character-by-number input via numeric KeyPad?  First, the starter keydown must happen
when the ORed bitmaps contains C<KBDALT>, and no other bits except C<KBDSHIFT>
...
provided C<Alt> and C<NumPad1> together have ORed bitmaps “in between” of C<KBDALT>
...
(=C<Alt>) key.  (See
L<the historical reasons for such behaviour|https://retrocomputing.stackexchange.com/questions/25016/ibm-pc-alt-numpad-for-entering-character-codes-firmware-bios-or-os/25017#25017>.)
...
B<NOTE:> since keyboard layout normally map C<Alt> to the bitmap C<KBDALT>, and do not define
a modification column for the ORed bitmap C<=KBDALT>, and C<KBDALT> is B<NOT> stripped for
...
the deadkey-finite-state-machine.  This way, one gets a “I<non-destructive>” flavor of ToUnicode().
...
Additionally, the typical keyboards also define the following bindings:
  ...
  Enter		 ——→ ^M
  Ctrl-Enter	 ——→ ^J
  Ctrl-2	 ——→ 0x00	In console only; due to a bug: not checking for the match of “Shift being down”.
...
C<KBDKANA> or above.  C<CONCLUSION:> if the bitmap is below C<KBDKANA>, and C<Win> is down, then this character may be entered
...
sanity, this means that the bitmaps on the pressed keys “match their names”, and we can find exactly which combination of keys
...
B<Remark:> See the link above for how Emacs does it — it is copiously commented.  (However, keep in mind that this code wants to be
as compatible as possible with “the ideosyncratic ways of old versions of Emacs”.)

B<Note:> Obviously, the algorithm above would not work if a keypress comes after a prefix key.  So one should inspect
C<WM_[SYS]DEADKEY> messages — and it is safe to I<just assume that during a chord involving prefix keys, no bindable actions should
be called>.  (However,
L<in very rare situations bugs in Windows may spoil the picture|"It is impossible to find whether the keyboard state is “after a prefix key”">.)
...
combinations of C<lCtrl> and C<rAlt> were not “faked”, but actually keyed in by the user.  (See
L<the example working implementation|https://search.cpan.org/~ilyaz/UI-KeyboardLayout/examples/robust_input/>.)
...
B<Reminder:> the
L<non-destructive ToUnicode()|https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-tounicodeex> can be
...
I<precisely>?)  The second method may have an advantage (and related — still unresolved — challenges): since there is no C<KBDALT>
stripping on the key-up events.

(Compare with L<the example working implementation|https://search.cpan.org/~ilyaz/UI-KeyboardLayout/examples/robust_input/>.  It
uses the flags C<0x01|0x02> only.)

B<Beware:> L<this comment on Stack Exchange|https://stackoverflow.com/questions/2604541/how-can-i-use-tounicode-without-breaking-dead-key-support/78173637#comment134382035_2605828>
says about C<0x04> flag (untested):

  That flag sadly doesn't affect the kernel's Alt-Numpad buffer.
...
eventual result”, and proceed until an “insert event” or a bindable-event is detected — possibly stripping the “excessive”
C<Alt>-modifiers.  Then the engine would apply “another excessive modifier” C<Alt> to the resulting “insert event”. — And what to
do with bindable events — it is a judgement call…)  B<Warning:> one may choose to also apply this approach for C<Ctrl>
modifier — mutatis mutandis.

B<Warning:> (This concerns a completely fictional situation.)  We did not discuss what to do if a text input event corresponds to
...
B<Note:> One major shortcoming of different schemes of binding events is that they assume that “the language of” the keyboard
...
SCANCODE —→ VK_CODE —→ ORED_BITMAP —→ MODIFICATION_COLUMN, then operates in terms of
ORed bitmaps and modification columns.  The application can access only the first two levels
...
B<NOTE:> of course, if only C<Shift/Alt/Ctrl> keys are associated to non-0 ORed bitmaps,
...
key (call it C<Super>; assume its bitmap involves a non-SHIFT/ALT/CTRL/KANA bit),
...
restriction, there would be no reason for the kernel to go through the extra step of “the ORed bitmap”.
...
about ORed bitmaps, it has no way to distinguish that, for example, C<lCtrl-rCtrl-=> may be I<SUPPOSED> to be
...
In addition to 8-bitness, Emacs had also been suffering from check-for-specials-first syndrome…
...
We explained above that (without non-destructive C<ToUnicode[Ex]()> calls) L<it is not possible to make a bullet-proof algorithm
...
the C<lAlt-Menu-key> modifier combinations with L<the assignment of bitmap
...
So the net result is: one designed a nice assignment of bitmaps to the modifier 
...
contribute anything into the ORed bitmap; hence, C<ApplicationMenu-letter>
...
assignments of the ORed bitmaps.  On such keyboards, pressing right C<Alt> (i.e.,
...
which defines an ORed bitmap for C<ApplicationMenu>, pressing C<ApplicationMenu-letter>
...
(so that on future keypresses the deadkey finite state-machine behaves as expected).  Generate the
...
(Via assigning ORed bitmaps for modification bitmaps to radio-buttons, the radio-buttons and toggle-buttons above may affect the layout.
...
to the ORed bitmaps by the table in the C<MODIFIERS> structure.  One B<must> define all the entries 
...
Typical usages:
  ...
  KBDNLS_NUMPAD (VK_xxx for Numpad)          [NEC PC-9800 Only; exactly the opposite to KBDNUMPAD — possbily predates it?]
...
B<Warning:> for less confusion, it is probably advisable to implement the C<VK_>-code-switch above I<whenever> C<Alt> is down — so
...
bitmaps, one can define how the keyboard behaves when both these modifiers are down.  (However, this requires consuming 1 extra
bit in the modifiers-bitmap.  If it is indeed 16-bit as it seems, this should be OK even with the assignment from the next
section.)

Later L<we inspect possibities opened by the assignments above|"Almost orthogonality — with C<lCtrl>-C<AltGr>-combinations">.
...
Assign ORed bitmaps to the modifier keys as follows:X<AssignBitmapsSmart>
...
B<Caveat:> the latter assignment leads to possible complications (see B<Caveat> in L<"The bullet-proof method">).  So the
...
C<Lat>/C<Grk>/C<Cyr>/C<Hbr> mean 4 personalities (these particular names are just for illustrative purposes), C<L> and C<Gr> mean
two possible scripts of math alphabet letters.  The superscripts mean:
...
C<Bbb>/C<Mono>/C<Scr>/C<Frk> mean flavors of “fancy” math characters: C<Blackboard-Bold> 𝔸𝕒𝔹𝕓, C<Monospaced> 𝙰𝚊𝙱𝚋, C<Script> 𝒜𝒶ℬ𝒷,
...
B<Note>: the special role of Bold Italic Sans Serif math font is that though standards deny particular binds between fonts-in-math
and semantic (like ISO 80000-2 (3): “There are no strict rules for the use of letter fonts…”), many of them suggest denoting tensors
in this font.  (For example, ISO 80000-2 (17.19) = L<ISO 31-11 (13.14)|https://law.resource.org/pub/in/bis/S07/is.1890.11.1995.pdf#page=28>,
or L<IEC 60050 (102-03-39)|https://www.electropedia.org/iev/iev.nsf/display?openform&ievref=102-03-39>.)

B<Enhancement>: since there are a few unused slots in the table above, one may want to allow “short continuous” input of
Super/subscripts, phonetic symbols etc. by filling these slots.

=head2 Almost orthogonality — with C<lCtrl>-C<AltGr>-combinations

Recall that nowadays it seems that by making the C<VK_>-code of C<lCtrl> and of C<AltGr> dependant on modifiers,
L<it is possible to support combinations of C<lCtrl>-C<AltGr>|"Another redefinition to avoid problems with the C<KLLF_ALTGR> flag">
(and other modifiers) almost without user-visible glitches.  Therefore, it makes sense to inspect what kind of orthogonality is
achievable when such combinations become accessible for binding.

In the table above, C<〃>s disappear.  The best result I could arrive at is:

 ↓L R→|	  ∅    M    C    MC     A    AM     AC   AMC     Heuristics
  —————————————————————————————————————————————————
  ∅   |	 Lat  Grk   —    Scr   Latᴬ  Grkᴬ  BbB   Frk       Base (Two personalities + Weird-Math)
   A  |	  —   ¿?         Scrᴮ  Lᵗᵉⁿ  Grᵗᵉⁿ Mono  Frkᴮ     ±Heavy
  C   |	  —   Cyr  Hbr   Lᴵˢˢ  Lᴮˢˢ  Cyrᴬ  Hbrᴬ  Grᴮˢˢ	  Two more personalities (and — scattered — Sans Serif)  
  CA  |	  —   Lˢˢ   Lᴵ   Grᴵ    Lᴮ    Grᴮ  Lᴮⁱ   Grᴮᴵ     Orthogonal math: (and Lˢˢ)


As before, C<Lat>/C<Grk>/C<Cyr>/C<Hbr> mean 4 personalities (these particular names are just for illustrative purposes), C<L> and
C<Gr> mean two possible scripts of math alphabet letters.  The superscripts mean:
C<ᴬ>: C<AltGr>-flavor, C<ᴮ>: Bold, C<ᴵ>: Italic, C<ˢˢ>: SansSerif, C<ᵗᵉⁿ>: “Tensor” = C<ᴮᴵˢˢ>.  This time, C<¿?> stands for the
position where not I<very> sophisticated applications may interpret as “being C<Alt>-letters”.  (This is the only “usable” position
for which C<Alt> is down, but C<Ctrl> is not).

  ???  One should also inspect what happens with modifiers-changed lCtrl.
       If AltGr was pressed first, this does not matter — so at least there is a workaround — except for the left ½ of bottom row.
       (And there pressing lCtrl before lAlt is a workaround.)  What happens without workarounds???

Complete orthogonality is not possible; however:

=over 4

=item

For personalities, C<AltGr> behaves “as expected”.  (And in the “orthogonal math” it means “Bold”.)

=item

The “C<lAlt>” row is the “Heavy” variant of the first row.  (Only to get “tensor”, one needs another C<Alt> too…)

=item

“Two more” personalities are obtained “by adding C<lCtrl> and/or C<rCtrl>”. — This is not “fully orthogonal”.

=item

In the “orthogonal math” mode, C<AltGr> means “Bold”, C<rCtrl> means “Italic”, and C<Menu> means “Greek”.

=item

Non-”tensor” variants of Sans Serif math is obtained by removing C<lAlt> (and possibly removing Greek/Italic) from the “orthogonal”
variants (without Sans Serif).  (The only exception is C<Lˢˢ> — for which one “skips removing C<lAlt>” as well ☺.)

=back

B<Enhancement:> two unused slots (empty, and the “questionable C<¿?> one”) may be used as in the preceding section.
...
For example, C<rAlt> above would produce modifier mask C<CTRL|ALT|LOYA|X1>; 
...
for a certain character via C<VkKeyScanW()> would see C<CTRL|ALT>.  Since this is
...
and change the labels on the keys — below-right of the image. — Also try changing the layout — in the URL — to C<kbdbr>.)
...
not act on “the top-right extra-key” as intended. — But anyway, this happens even without using the scheme above!  B<TODO:> Can one
...
the keyboard.  (For another — and very robust — way to work, see
...
in the C<LAYOUT> section to make “the Menu key” into C<VK_KANA>), such scancodes are not included into “the special table for
C<e0>-bindings” of the generated C file.
...
L<a string with up to 125 (with “tricks” up to 255) characters|"Keyboard input on Windows, Part II: The semantic of ToUnicode()">.
...
keys).  In fact, one should better subclasses the target window to catch the
accelerator (if the system decides to deliver it — as happens without C<KLLF_ALTGR>).
(Compare with L<the example working implementation|https://search.cpan.org/~ilyaz/UI-KeyboardLayout/examples/robust_input/>.)

B<Obsolete:> Additionally, some system accelerators react on Windows key as a
...
down.  (Testing with the implementation above shows that this concern is irrelevant.)
...
=head2 It is impossible to find whether the keyboard state is “after a prefix key”

The effect of a keypress depends on whether a “prefix key” (= “deadkey”) was pressed before it.  For example, on many layouts typing
4 keys C<Compose ( a )> produces C<ⓐ≡U+24d0> (here C<Compose> stands for a certain layout-specific “prefix”).  Here the effect of
typing C<(> is very different — one may call it C<(-after-Compose> — than when C<(> is typed “standalone” — in “the base state” of
the keyboard.  (So the example shows a chain of three prefix keys, C<Compose> and C<(-after-Compose> and 
C<a-after-(-after-Compose> — and the final C<)> “produces a real character”.  A convenient way to think of it is that “a prefix
key” is just “a switch to a particular state in a Finite State Machine.  “The result” of a keypress depends on the state!)  

An application may want to know whether the keyboard is “in the base state or not” (for example,
L<to decide whether a keypress should “insert”, or “trigger a bindable action”.|"The ultra-simple approach">).
In the best world, the application would do it by checking whether the last non-0 result of C<ToUnicode[Ex]()> was
negative — indicating “a prefix key” (but
L<one needs to special-case the returned state being 0|"It is not documented how to make a with-prefix-key(s) combination produce 0-length string">).
However, while this works fine for the first prefix key, for a chain of prefix keys this is not bullet-proof: two bugs get in the
way.

First, for all the “prefix key” keypresses in a chain of prefix keys (as in the example above) C<ToUnicodeEx()> correctly returns
C<-1> to indicate that it is still not in the base state.  However, “the returned new state” is returned correctly only after the
first keypress in a chord — in the remaining cases the returned state is just unitialized junk.  B<CONCLUSION:> one cannot
check this state for being 0 — as it is needed for the robust algorithm above.

Second, when “the base action of a keypress” is to produce “several C<UTF-16> codepoints”, pressing this key after a prefix key
“behaves as expected”.  For example, if C<AltGr-a> produces the string C<(a>, then C<Compose AltGr-a )> would still produce
C<ⓐ≡U+24d0>.  However, C<ToUnicode[Ex]()> does not return C<-1> after C<Compose AltGr-a> — although this combination produces
a non-0 state in the finite state machine.  (The returned value is “0-characters produced”, so the application would conclude that
the state did not change — which would be wrong 
L<if the resulting state is 0|"It is not documented how to make a with-prefix-key(s) combination produce 0-length string">.)
...
is doing is making the kernel remember a word (the state of the finite state machine), and not
producing any output character.  Having no prefix key corresponds to the state being 0.

Hence making prefix_key=0 is the same as switching the finite state machine to the initial
state, and not producing any character — and this exactly what is requested in the question.
(However, one should keep in mind that this may break certain heuristics applications can make — L<due
to a bug in C<ToUnicode[Ex]()>|"It is impossible to find whether the keyboard state is “after a prefix key”">.
...
To add insult to injury, I<due to the exceptions described above> this does not I<happen always>.  For example, whether
C<lCtrl> “is faked” when pressing the chord C<rCtrl-rAlt> depends on the order in which this chord is pressed (if C<rCtrl> is
pressed first, then C<lCtrl> is not faked) — hence the bitmap produced for this chord may have two different values.

B<Aside:> Likewise, pressing
...
The (very limited — and nowadays potentially dangerous — see B<Possible complications> in L<"The bullet-proof method">) workaround
is to ensure that the bits in the modification bitmap one puts on C<AltGr> contain
...
The gist of this is: for I<the kernel> to handle C<Alt-Numpad> input correctly (without these keypresses I<inserting> [hex] digits)
the C<Alt>-keys should not produce any character without C<KBDALT> stripping.  On the other hand, for I<the applications> to treat
C<Alt>-keypresses correctly, they should better produce “the base characters” I<after> C<KBDALT> stripping.

C<Workaround:> one may still add C<KBDKANA> on C<lAlt>.  It looks like the combination
...
C<KBDSHIFT> added. — Moreover, one can still enhance this: see the “NOTE” after going through the next link.)
...
#      $prefix_in_base->{$_}++ for @{ $H->{'[dead_in_VK_array]'} || [] };
      ...
      for my $pass (1,2,3) {
       for my $deadKEY ( sort keys %{ $H->{'[deadkeyFace]'}} ) {
        ...
        $cDifc{$_} = 1, $haveChar{$_} = '1①2②3③9⑨'
           for grep defined, map {($_ and ref $_) ? $_->[0] : $_} grep {$_ and not (ref $_ and $_->[2])} values %$map;
        $havePref{$_} = 1
           for grep defined, map {($_ and ref $_) ? $_->[0] : $_} grep {$_ and     (ref $_ and $_->[2])} values %$map;
