...
# shell program to test ll.pm's responses to various erroneous
# conditions. note of course that ll never halts on errors/warnings
# but sets error codes that statistic.pl can request later on.

# Subtest 1: what happens when ngram > 2 is provided to ll
...
# test-1.sub-1.cnt has trigrams in it. so ll.pm should complain
# during the initialization step, since ll is for bigrams only!
...
# now the test!
echo "Test:   statistic.pl --ngram 3 ll test-1.out $TESTFILE" 
statistic.pl --ngram 3 ll test-1.out $TESTFILE >& error.out
...
# now the test!
echo "Test:   statistic.pl --set_freq_combo $FREQCOMBFILE ll test-1.out $TESTFILE" 
statistic.pl --set_freq_combo $FREQCOMBFILE ll test-1.out $TESTFILE >& error.out
...
# now the test!
echo "Test:   statistic.pl ll test-1.out $TESTFILE"
statistic.pl ll test-1.out $TESTFILE >& error.out
