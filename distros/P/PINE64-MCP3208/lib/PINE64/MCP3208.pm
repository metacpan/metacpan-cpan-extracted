...
PINE64::MCP3208 - Perl interface to the MCP3208 12-bit SPI
...
number, SPI chip select pin number, and returns a PINE64::MAX3208
object.  Pin numbers are valid PINE64::GPIO objects on the Pi-2
bus.

=head2 read3208($channel_number,$clk_pls_delay,$voltage_reference); 
...
So, when calling read3208, you would pass a reference of the 
...
the output code is 2048,  the method will return 25V.  

The reference voltage can be useful if you are using a voltage
divider circuit to step-down a voltage higher that the 
maximum input reference voltage of the MCP3208.  

read3208() returns an array that contains an array reference 
of the 10-bit binary result i.e. 101101001101, the output code 
in decimal i.e. 2024, 567, 311, etc., and the calculated
voltage based on the supplied reference voltage.   
		
Below is an example call to the read3208() method:
my ($reading, $binval, $voltage ) = $adc->read3208(\@ch0, 50, 5.01);
