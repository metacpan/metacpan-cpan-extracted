...
use Data::MARC::Field008::Utils qw(check_continuing_resource_type);
...
my $ret = check_continuing_resource_type($self, 'key');
is($ret, undef, 'Right continuing resource type is present (n).');

# Test.
$self = {
	'key' => ' ',
};
$ret = check_continuing_resource_type($self, 'key');
is($ret, undef, 'Right continuing resource type is present ( ).');
