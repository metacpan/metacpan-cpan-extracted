...
$VERSION = '0.161';
$ABSTRACT =
'Interlinear is a program for building metamodels from incomplete, multivariate, discrete dataseries on the basis of nearest-neighbouring gradients weighted by distance.';
...
sub tellstepsize {
    ...
    foreach my $fact ( sort { $a <=> $b } ( keys %{ $factlevels{pairs} } ) ) {
        ...
        if ( not( $factlevels{pairs}{$fact} == 1 ) ) {
            $stepsize = ( 1 / ( $factlevels{pairs}{$fact} - 1 ) ) * $lvconv;
        }
        else {
            $stepsize = 0;
        }
...
sub weightvals1 {
    my (
        $elt1,           $boxgrads_ref,   $boxdists_ref, $boxors_ref,
        $minreq_forcalc, $factbag_ref,    $levbag_ref,   $stepbag_ref,
        $elt2,           $factlevels_ref, $maxdist,      $elt3,
        $condweight
    ) = @_;
    ...
    foreach my $or (@boxors) {
        my $hsh_ref =
          calcdist( $elt1, $or, \%factlevels, $maxdist, $elt3, $condweight );
        my %hsh  = %{$hsh_ref};
        my $dist = $hsh{dist};
    ...
    if ( not( scalar(@boxgrads) == 0 ) ) {
        ...
        foreach my $grad (@boxgrads) {
            ...
            if (    ( $strength ne "" )
                and ( $sum_strengths ne "" )
                and ( $sum_strengths != 0 ) )
            {
                $soughtgrad =
                  ( $soughtgrad + ( $grad * ( $strength / $sum_strengths ) ) );
            }
            $in++;
        }
    }
    ...
    if (    ( $soughtinc ne "" )
        and ( $totdist ne "" )
        and ( $totstrength ne "" )
        and ( $totstrength >= $minreq_forcalc ) )
    {
...
sub weightvals_merge {
    ...
    if ( ( $sum_strengths ne "" ) and ( $sum_strengths > 0 ) ) {
        ...
        foreach my $val (@vals) {
            ...
            $soughtval =
              ( $soughtval + ( $val * ( $strength / $sum_strengths ) ) );
            $in++;
        }
        my $seedstrength = 1;
    ...
    if (    ( $soughtval ne "" )
        and ( $totstrength ne "" )
        and ( $totstrength >= $minreq_formerge ) )
    {
...
sub calcdist {
    my ( $el1, $elt1, $factlevels_ref, $maxdist, $el3, $elt3, $condweight ) =
      @_;
    my %factlevels = %{$factlevels_ref};
    ...
    if ( scalar(@diff1) > 0 ) {
        ...
        foreach my $e (@da1par) {
            ...
            if (    ( $fact ne "" )
                and ( $diffpar ne "" )
                and ( $factlevels{stepsizes}{$fact} ne "" ) )
            {
                push( @factbag, $fact );
                push( @levbag,  $diffpar );
...
sub calcdistgrad {
    my ( $el1, $elt1, $factlevels_ref, $minreq, $maxdist, $el3, $elt3,
        $condweight, $el, $elt )
      = @_;
    my %factlevels = %{$factlevels_ref};
    my @diff1 = diff( \@{$el1}, \@{$elt1} );
...
sub calcmaxdist {
    ...
    "raw max distance: " . dump( $hash{rawdist} );
...
sub wei {
    my (
        $arr_ref,           $relaxmethod,          $overweightnearest,
        $parconcurrencies,  $instconcurrencies,    $count,
        $factlevels_ref,    $minreq_forgrad,       $minreq_forinclusion,
        $minreq_forcalc,    $minreq_formerge,      $maxdist,
        $nfiltergrads,      $limit_checkdistgrads, $limit_checkdistpoints,
        $bank_ref,          $fulldo,               $first0,
        $last0,             $nears_ref,            $checkstop,
        $weldsprepared_ref, $weldaarrs_ref,        $parswelds_ref,
        $recedes_ref,       $nfilterpoints,        $limitgrads,
        $limitpoints
    ) = @_;
    my @arr = @{$arr_ref};
    ...
    my ( @arra, @arrah );
    if (    ( $limit_checkdistgrads ne "" )
        and ( $limit_checkdistgrads > $checkstop ) )
    {
    ...
    sub fillbank {
        my (
            $arr_r,        $minimumcertain, $minreq_forgrad,
            $maxdist,      $condweight,     $factlevels_r,
            $nfiltergrads, $arra_r,         $first0,
            $last0,        $nears_ref,      $limitgrads,
            $limitpoints
        ) = @_;
        my @arr        = @{$arr_r};
        my @arra       = @{$arra_r};
        ...
        foreach my $el (@arr) {
            if ( ( $el->[2] ne "" ) and ( $el->[3] >= $minimumcertain ) ) {
                ...
                foreach my $it (@neighbours) {
                    ...
                    if (    ( $elt->[2] ne "" )
                        and ( $elt->[0] ne $el->[0] )
                        and ( $el->[3] >= $minimumcertain ) )
                    {

                        my ($res_ref) = calcdistgrad(
                            $el->[1],        $elt->[1],   \%factlevels,
                            $minreq_forgrad, $maxdist,    $el->[3],
                            $elt->[3],       $condweight, $el,
                            $elt
                        );
    ...
    sub clean {
        ...
        foreach my $trio ( keys(%bank) ) {
            ...
            unless ( ( $message eq "principal" ) and ( $first ~~ @recedes ) ) {
                ...
                unless ( ( $bank{$trio}{grad} eq "" )
                    or ( $bank{$trio}{ordists} eq "" )
                    or ( $bank{$trio}{dists} eq "" )
                    or ( $bank{$trio}{strengths} eq "" ) )
                {
    ...
    unless ( ( $fulldo eq "$tight" ) and ( $count > 0 ) ) {
        "Entering gradients' \%bank.";
        my ( $bank_ref, $nears_ref ) = fillbank(
            \@arr__,       $minimumcertain, $minreq_forgrad,
            $maxdist,      $condweight,     \%factlevels,
            $nfiltergrads, \@arra,          $first0,
            $last0,        \%nears,         $limitgrads,
            $limitpoints
        );
        %bank  = %{$bank_ref};
        %nears = %{$nears_ref};
        %bank  = %{ clean( \%bank, $nfiltergrads, "principal", \@recedes ) };
        "BANK DONE.";
    }
    ...
    sub mixbank {
        ...
        "Mixing for welding " . dump(@parsws);
        foreach my $trio ( keys %weldbank ) {
            unless ( $trio eq "" ) {
                ...
                if ( $num ~~ @parsws ) {
                    push(
                        @{ $bank{$trio}{orderedtrio} },
                        @{ $weldbank{$trio}{orderedtrio} }
                    );
                    push( @{ $bank{$trio}{grad} },
                        @{ $weldbank{$trio}{grad} } );
                    push(
                    ...
                    push(
                        @{ $bank{$trio}{ordists} },
                        @{ $weldbank{$trio}{ordists} }
                    );
                    push(
                        @{ $bank{$trio}{strengths} },
                        @{ $weldbank{$trio}{strengths} }
                    );
                    push(
                        @{ $bank{$trio}{orvals} },
                        @{ $weldbank{$trio}{orvals} }
                    );
                    push(
                        @{ $bank{$trio}{origins} },
                        @{ $weldbank{$trio}{origins} }
                    );
                    push(
                        @{ $bank{$trio}{orstrings} },
                        @{ $weldbank{$trio}{orstrings} }
                    );
                    push(
                        @{ $bank{$trio}{orstring} },
                        @{ $weldbank{$trio}{orstring} }
                    );
                    "weld $num.";
                }
            }
    ...
    unless ( ( scalar(@weldsprepared) == 0 ) ) {
        "Entering gradients' bank for welding.";
        my $co = 0;
        foreach my $weldref (@weldaarrs) {
            my @parsws = @{ $parswelds[$co] };
            "\@parsws: " . dump(@parsws);
            my @weldarr = @{$weldref};
            my @weldarr__;
            if (   ( $limit_checkdistgrads ne "" )
                or ( $limit_checkdistpoints ne "" ) )
            {
            ...
            my ( $weldbank_ref, $weldnears_ref ) = fillbank(
                \@weldarr__,   $minimumcertain, $minreq_forgrad,
                $maxdist,      $condweight,     \%factlevels,
                $nfiltergrads, \@weldarra,      $first0,
                $last0,        \%nears,         $limitgrads,
                $limitpoints
            );
    ...
    sub cyclearr {
        my (
            $arr_r,     $minreq_forinclusion, $minreq_forgrad,
            $bank_r,    $factlevels,          $nfilterpoints,
            $arrb_r,    $first0,              $last0,
            $nears_ref, $limitgrads,          $limitpoints
        ) = @_;
        my @arr        = @{$arr_r};
        my @arrb       = @{$arrb_r};
        my %bank       = %{$bank_r};
        ...
        foreach my $el (@arr) {
            ...
            if ( $el->[2] eq "" ) {
                ...
                foreach my $it (@neighbours) {
                    my $elt = $nears{$it}{all};
                    if (    ( $elt->[2] ne "" )
                        and ( $el->[3] >= $minreq_forinclusion ) )
                    {
                        my @diff1 = diff( \@{ $el->[1] }, \@{ $elt->[1] } );

                        if (    ( scalar(@diff1) > 0 )
                            and ( scalar(@diff1) <= $minreq_forgrad->[2] ) )
                        {
                                        ...
                                        if ( ( $d10 eq $d20 ) ) {
                                                ...
                                                @{ $bank{$newtrio}{grads} } )
                                            {
                                                  ...
                                                  )
                                                {
                                                    push( @boxgrads, $grad );
                                                    my $ordist =
                                                      $bank{$newtrio}{ordists}
                                                      [$cn];
                                                    push( @boxdists, $ordist );
                                                    my $origin =
                                                      $bank{$newtrio}{origins}
                                                      [$cn];
                                                    push( @boxors, $origin );
                                                }
                                                $cn++;
                                            ...
                                            my ( $soughtinc, $totdist,
                                                $totstrength );

                                            unless (( scalar(@boxgrads) == 0 )
                                                and ( scalar(@boxdists) == 0 )
                                                and ( scalar(@boxors) == 0 ) )
                                            { (
                                                    $soughtinc, $totdist,
                                                    $totstrength
    ...
    "GOING TO CREATE NEW POINTS.";
    my ( $wand_ref, $nears_ref ) = cyclearr(
        \@arr__, $minreq_forinclusion, $minreq_forgrad,
        \%bank,  \%factlevels,         $nfiltergrads,
        \@arrb,  $first0,              $last0,
        \%nears, $limitgrads,          $limitpoints
    );
    "DONE.";
    my %wand = %{$wand_ref};
    "\nDONE.";
    ...
    foreach my $ke ( keys %wand ) {
        ...
        if ( ( $soughtval ne "" ) and ( $totstrength ne "" ) ) {
            push(
                @limb0,
                [
                    $wand{$ke}{name}, $wand{$ke}{bulk}, $soughtval,
                    $totstrength
                ]
            );
        }
...
sub purelin {
    my (
        $arr_refillf,       $relaxlimit,       $relaxmethod,
        $overweightnearest, $parconcurrencies, $instconcurrencies,
        $count,             $factlevels_ref,   $maxdist
    ) = @_;
    my @arr = @{$arr_ref};
    my (@linneabagbox);
    my ( %magic, %wand, %spell );
        ...
        if ( $el->[2] eq "" ) {
            foreach my $elt (@arr) {
                if ( not( $elt->[2] eq "" ) ) {
                    ...
                    elsif (( $relaxmethod eq "linear" )
                        or ( $relaxmethod eq "exponential" ) )
                    {
                        my $index = 0;
                        while ( $index <= $relaxlimit ) {
                            if (    ( scalar(@diff1) > 0 )
...
sub near {
    my ( $arr_ref, $nearrelaxlimit, $relaxmethod, $overweightnearest,
        $nearconcurrencies, $count, $factlevels_ref, $maxdist )
      = @_;
    my @arr        = @{$arr_ref};
    my %factlevels = $factlevels_ref;
    foreach my $el (@arr) {
            ...
            foreach my $elt (@arr) {
                if ( not( $elt->[2] eq "" ) ) {
                    ...
                    while ( $index <= $nearrelaxlimit ) {
                        if (    ( scalar(@diff1) > 0 )
                            and ( scalar(@diff1) <= ( 1 + $nearrelaxlimit ) ) )
                        {
                            my @diff2 = diff( \@{ $elt->[1] }, \@{ $el->[1] } );

                            my ( $da1_ref, $da1par_ref, $da2_ref, $da2par_ref )
                              = setvalues( \@diff1, \@diff2 );
                            my @da1 = @{$da1_ref};
                            my @da2 = @{$da2_ref};
...
sub mixlimbo {
    my ( $limbo1_ref, $limbo2_ref, $presence, $linearprecedence, $weights_ref )
      = @_;
...
sub interlinear {
    my ( $sourcef, $configf, $metafile, $blockelts_r, $reportf, $countblock ) =
      @_;
    ...
    close SOURCEFILE;
    say $tee
      "Preparing the dataseries, IN INTERLINEAR: \$countblock $countblock";
    ...
    "Checking factors and levels.";
    my %factlevels = %{ prepfactlev( \@aarr ) };
    "Done.";

    my ($factlev_ref) = tellstepsize( \%factlevels, $lvconversion );
    my %factlev = %{$factlev_ref};
    " \%factlev: " . dump(%factlev);
    "Understood step sizes.";
    ...
    while ( $count < $maxloops ) {
        ...
        foreach my $el (@arr) {
            if (
                ( $nears{ $el->[0] }{all}->[2] eq "" )
                or (    ( $nears{ $el->[0] }{all}->[3] < 1 )
                    and ( $nears{ $el->[0] }{all}->[2] != $el->[2] ) )
              )
            {
                $nears{ $el->[0] }{all} = $el;
            }
        ...
        if ( ( $mode__ eq "wei" ) or ( $mode__ eq "mix" ) ) {
            my ( $limbo_wei_ref, $bank_ref, $nears_ref ) = wei(
                \@arr,                  $relaxmethod,
                $overweightnearest,     $parconcurrencies,
                $instconcurrencies,     $count,
                \%factlev,              $minreq_forgrad,
                $minreq_forinclusion,   $minreq_forcalc,
                $minreq_formerge,       $maxdist,
                $nfiltergrads,          $limit_checkdistgrads,
                $limit_checkdistpoints, \%bank,
                $fulldo,                $first0,
                $last0,                 \%nears,
                $checkstop,             \@weldsprepared,
                \@weldaarrs,            \@parswelds,
                \@recedes,              $nfilterpoints,
                $limitgrads,            $limitpoints
            );
            ...
            "THERE ARE "
              . scalar(@limbo_wei)
              . " ITEMS IN THIS LOOP , NUMBER "
              . ( $count + 1 )
              . ", 1, FOR WEIGHTED GRADIENT INTERPOLATION OF THE NEAREST NEIGHBOUR.";
        }

        if ( ( $mode__ eq "purelin" ) or ( $mode__ eq "mix" ) ) {
            @limbo_purelin = purelin(
                \@arr,              $relaxlimit,       $relaxmethod,
                $overweightnearest, $parconcurrencies, $instconcurrencies,
                $count,             \%factlev,         $maxdist
            );
            "THERE ARE "
              . scalar(@limbo_purelin)
              . " ITEMS IN THIS LOOP , NUMBER "
              . ( $count + 1 )
              . ", 1, FOR PURE LINEAR INTERPOLATION.";
        }

        if ( ( $mode__ eq "near" ) or ( $mode__ eq "mix" ) ) {
            @limbo_near =
              near( @arr, $nearrelaxlimit, $relaxmethod, $overweightnearest,
                $nearconcurrencies, $count, \%factlev, $maxdist );
            "THERE ARE "
              . scalar(@limbo_near)
              . " ITEMS IN THIS LOOP, NUMBER "
              . ( $count + 1 )
              . ", 2, FOR THE NEAREST NEIGHBOUR STRATEGY.";
        }
        ...
        "MIXING THE ARRAY UPDATES "
          . ( $count + 1 )
          . " for $sourcefile";
        "THERE ARE "
          . scalar(@limbo)
          . " ITEMS COMING OUT FROM THIS MIX "
          . ( $count + 1 );
