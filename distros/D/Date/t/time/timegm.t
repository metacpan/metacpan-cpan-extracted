...
subtest 'read only values failure' => sub {
    dies_ok { timegmn(0,0,0,0,0,0) };
};
...
subtest 'normalization' => sub {
    ...
    is_deeply([$Y,$M,$D,$h,$m,$s], [1970,0,1,0,0,-1]);
    is(timegmn($s,$m,$h,$D,$M,$Y), -1);
    is_deeply([$Y,$M,$D,$h,$m,$s], [1969,11,31,23,59,59]);
    
    ($Y,$M,$D,$h,$m,$s) = (1970,234,-4643,2341,-34332,-1213213);
    is(timegmn($s,$m,$h,$D,$M,$Y), 219167267);
    is_deeply([$Y,$M,$D,$h,$m,$s], [1976,11,11,15,47,47]);
    ...
    is_deeply([$Y,$M,$D,$h,$m,$s], [2009,10,30,0,0,0]);
};
