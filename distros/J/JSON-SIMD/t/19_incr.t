...
splitter +JSON::SIMD->new->allow_nonref (0), '  ["x\\"","\\u1000\\\\n\\nx",1,{"\\\\" :5 , "": "x"}]';
splitter +JSON::SIMD->new->allow_nonref (0), '[ "x\\"","\\u1000\\\\n\\nx" , 1,{"\\\\ " :5 , "": " x"} ] ';
splitter +JSON::SIMD->new                  , '"test"';
splitter +JSON::SIMD->new                  , ' "5" ';
splitter +JSON::SIMD->new                  , '-1e5';
splitter +JSON::SIMD->new                  , ' 0.00E+00 ';

{
   my $text = '[5],{"":1} , [ 1,2, 3], {"3":null}';
   my $coder = new JSON::SIMD;
   for (0 .. length $text) {
      my $a = substr $text, 0, $_;
      my $b = substr $text, $_;
