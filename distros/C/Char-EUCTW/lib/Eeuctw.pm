...
# regexp of character
my $your_char = q{[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF]|[\x00-\xFF]};
...
sub import {
    ...
    *Char::ord           = \&EUCTW::ord;
    *Char::ord_          = \&EUCTW::ord_;
    *Char::reverse       = \&EUCTW::reverse;
    *Char::getc          = \&EUCTW::getc;
    *Char::length        = \&EUCTW::length;
    *Char::substr        = \&EUCTW::substr;
    *Char::index         = \&EUCTW::index;
    *Char::rindex        = \&EUCTW::rindex;
    *Char::eval          = \&EUCTW::eval;
    *Char::escape        = \&EUCTW::escape;
    *Char::escape_token  = \&EUCTW::escape_token;
    *Char::escape_script = \&EUCTW::escape_script;
}
...
${Eeuctw::anchor} = qr{\G(?>[^\x8E\xA1-\xFE]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])*?}oxms;
...
if (($] >= 5.010001) or
    ...
    # EUC-TW encoding
        ...
        ${Eeuctw::q_char_SADAHIRO_Tomoyuki_2002_01_17} = qr{.*?[^\x8E\xA1-\xFE]     (?>            [\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]       )*?}oxms;
    ...
    # other encoding
        ...
        ${Eeuctw::q_char_SADAHIRO_Tomoyuki_2002_01_17} = qr{.*?[$sbcs](?:[^$sbcs][^$sbcs])*?}oxms;
        #                                                     ******* octets not in multiple octet char (always char boundary)
        #                                                               **************** 2 octet chars
    }

    ${Eeuctw::anchor_SADAHIRO_Tomoyuki_2002_01_17} =
    qr{\G(?(?=.{0,32766}\z)(?:[^\x8E\xA1-\xFE]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])*?|(?(?=[$sbcs]+\z).*?|(?:${Eeuctw::q_char_SADAHIRO_Tomoyuki_2002_01_17})))}oxms;
#   qr{
#      \G # (1), (2)
#        (? # (3)
#          (?=.{0,32766}\z) # (4)
#                          (?:[^\x8E\xA1-\xFE]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])*?| # (5)
#                                                                                      (?(?=[$sbcs]+\z) # (6)
#                                                                                                      .*?| #(7)
...
# $PadA  = '  (?:\A|                                           [\x00-\x80\xA0-\xDF])(?:[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE]{2})*?';
# $PadG  = '\G(?:                                |[\x00-\xFF]*?[\x00-\x80\xA0-\xDF])(?:[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE]{2})*?';
# $PadGA = '\G(?:\A|(?:[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE]{2})+?|[\x00-\xFF]*?[\x00-\x80\xA0-\xDF] (?:[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE]{2})*?)';
...
# ${Eeuctw::eS}          = qr{(?>[^\x8E\xA1-\xFE\x09\x0A    \x0C\x0D\x20]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
# ${Eeuctw::eS}          = qr{(?>[^\x8E\xA1-\xFE\x09\x0A\x0B\x0C\x0D\x20]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
...
sub _last_subexpression_offsets {
    ...
    if ($modifier =~ /x/oxms) {
        @char = $pattern =~ /\G((?>
            ...
            \\ $q_char      |
            \# (?>[^\n]*) $ |
            \[ (?>(?:[^\x8E\xA1-\xFE\\\]]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF]|\\\\|\\\]|$q_char)+) \] |
            \(\?            |
                $q_char
        ))/oxmsg;
    }
...
sub _charlist {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # \P{PROPERTY} --> P\{PROPERTY}
        elsif ($char[$i] =~ /\A \\ ([bBNpP]) ( \{ ([^\x8E\xA1-\xFE0-9\}][^\x8E\xA1-\xFE\}]*) \} ) \z/oxms) {
            $char[$i] = $1 . '\\' . $2;
        }
...
sub charlist_qr {
    ...
    if (scalar(@singleoctet) >= 1) {
        ...
        my $not_anchor = '';
        $not_anchor = '(?![\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE])';
...
sub charlist_not_qr {
    ...
    if (scalar(@multipleoctet) >= 1) {
        if (scalar(@singleoctet) >= 1) {
            ...
            return '(?!' . join('|', @multipleoctet) . ')(?:[^\x8E\xA1-\xFE' . join('', @singleoctet) . ']|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])';
        }
        else {
        ...
        if (scalar(@singleoctet) >= 1) {
            ...
            return                                      '(?:[^\x8E\xA1-\xFE' . join('', @singleoctet) . ']|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])';
        }
        else {
...
sub _DOS_like_glob {
    ...
    # UNIX-like system
        ...
        $expr =~ s{ \A ~ ( (?:[^\x8E\xA1-\xFE/]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])* ) }
...
OUTER:
    for my $expr (@expr) {
        ...
        if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
            if ($expr =~ s# \A ((?:[A-Za-z]:)?) ([^\x8E\xA1-\xFE/\\]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF]) #$1./$2#oxms) {
...
sub _parse_line {
    ...
    my @piece = ();
    while ($line =~ /
        " ( (?>(?: [^\x8E\xA1-\xFE"]  |[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF] )* ) ) " (?>\s+) |
          ( (?>(?: [^\x8E\xA1-\xFE"\s]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF] )* ) )   (?>\s+)
        /oxmsg
    ) {
...
my $qq_paren   = qr{(?{local $nest=0}) (?>(?:
                       [^\x8E\xA1-\xFE\\()] |
                           \(  (?{$nest++}) |
                           \)  (?(?{$nest>0})(?{$nest--})|(?!)))*) (?(?{$nest!=0})(?!)) |
                       [\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF] |
                    \\ [^\x8E\xA1-\xFEc] |
                    \\c[\x40-\x5F] |
                    \\ [\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF] |
                       [\x00-\xFF]
...
my $qq_substr  = qr{(?> Char::substr | EUCTW::substr | CORE::substr | substr ) (?>\s*) \( $qq_paren \)
                 }xms;
...
sub e_string {
    ...
    my @char = $string =~ / \G (?>[^\x8E\xA1-\xFE\\]|\\$q_char|$q_char) /oxmsg;
...
sub e_qr {
    ...
    my @char = $string =~ /\G((?>
        ...
        \\x\{ (?>[0-9A-Fa-f]+) \}  |
        \\o\{ (?>[0-7]+)       \}  |
        \\[bBNpP]\{ (?>[^\x8E\xA1-\xFE0-9\}][^\x8E\xA1-\xFE\}]*) \} |
        \\  $q_char                |
        \$` | \$\{`\} | \$ (?>\s*) PREMATCH  | \$ (?>\s*) \{ (?>\s*) PREMATCH  (?>\s*) \} | \$ (?>\s*) \{\^PREMATCH\}  |
        \$& | \$\{&\} | \$ (?>\s*) MATCH     | \$ (?>\s*) \{ (?>\s*) MATCH     (?>\s*) \} | \$ (?>\s*) \{\^MATCH\}     |
                        \$ (?>\s*) POSTMATCH | \$ (?>\s*) \{ (?>\s*) POSTMATCH (?>\s*) \} | \$ (?>\s*) \{\^POSTMATCH\} |
...
sub e_s2_q {
    ...
    my @char = $string =~ / \G (?>[^\x8E\xA1-\xFE\\]|\\\\|$q_char) /oxmsg;
    for (my $i=0; $i <= $#char; $i++) {
        if (0) {
        }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                               1                                                2   3                                  4   5
                q<CORE::eval{local $Eeuctw::re_t=%s; local $Eeuctw::re_a=''; while($Eeuctw::re_t =~ %s){%s local $^W=0; local $Eeuctw::re_r=%s; %s$Eeuctw::re_t="$Eeuctw::re_a${1}$Eeuctw::re_r$'"; pos($Eeuctw::re_t)=length "$Eeuctw::re_a${1}$Eeuctw::re_r"; $Eeuctw::re_a=substr($Eeuctw::re_t,0,pos($Eeuctw::re_t)); } return $Eeuctw::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Eeuctw::re_r=CORE::eval $Eeuctw::re_r; ' x $e_modifier,          #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                               1                         2   3                                  4   5
                q<CORE::eval{local $Eeuctw::re_t=%s; while($Eeuctw::re_t =~ %s){%s local $^W=0; local $Eeuctw::re_r=%s; %s$Eeuctw::re_t="$`$Eeuctw::re_r$'"; pos($Eeuctw::re_t)=length "$`$Eeuctw::re_r"; } return $Eeuctw::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                  4   5  6                     7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Eeuctw::re_r=%s; %s"%s$Eeuctw::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<CORE::eval{local $Eeuctw::re_n=0; local $Eeuctw::re_a=''; while(%s =~ %s){%s local $^W=0; local $Eeuctw::re_r=%s; %s%s="$Eeuctw::re_a${1}$Eeuctw::re_r$'"; pos(%s)=length "$Eeuctw::re_a${1}$Eeuctw::re_r"; $Eeuctw::re_a=substr(%s,0,pos(%s)); $Eeuctw::re_n++} return %s$Eeuctw::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                        1     2   3                                  4   5 6                          7                                                   8
                q<CORE::eval{local $Eeuctw::re_n=0; while(%s =~ %s){%s local $^W=0; local $Eeuctw::re_r=%s; %s%s="$`$Eeuctw::re_r$'"; pos(%s)=length "$`$Eeuctw::re_r"; $Eeuctw::re_n++} return %s$Eeuctw::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                  5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Eeuctw::re_r=%s; %s%s="%s$Eeuctw::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                  5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Eeuctw::re_r=%s; %s%s="%s$Eeuctw::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Eeuctw::re_r=CORE::eval $Eeuctw::re_r; ' x $e_modifier,          #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
