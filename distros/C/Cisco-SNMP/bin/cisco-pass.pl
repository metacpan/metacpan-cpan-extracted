...
my $PASSWORD7 = "( password 7 )|(-server key 7 )|( key-string 7 )";

if (defined $opt{encrypt}) {
    if (my $passwd = Cisco::SNMP::Password->password_encrypt($ARGV[0], $opt{encrypt})) {
        print "$_\n" for (@{$passwd})
    } else {
        printf "Error - %s\n", Cisco::SNMP::Password->error
    }
} else {
    my @passwords;
    my $linecnt = 0;
    ...
    for (@passwords) {
        my @parts = split(/ /,$_);
        if (my $passwd = Cisco::SNMP::Password->password_decrypt($parts[$#parts])) {
            if ($linecnt > 0) {
                print "@parts = $passwd\n"
            } else {
                print "$passwd\n"
