This is the Security Policy for the Perl DBIO-DuckDB distribution.
...
[DBIO-DuckDB on CPAN](https://metacpan.org/dist/DBIO-DuckDB) or the main branch of the
[DBIO-DuckDB git repository](https://codeberg.org/dbio/dbio-duckdb).
...
DBIO-DuckDB is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO-DuckDB are covered by this policy.
...
they affect DBIO-DuckDB, or DBIO-DuckDB can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO-DuckDB, or plugins to it that are not included with the DBIO-DuckDB
distribution) are not covered by this policy.

## Supported Versions of DBIO-DuckDB

Only the latest release of DBIO-DuckDB will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO-DuckDB will run on older versions of Perl. If a
...
that are required for DBIO-DuckDB to work. However, some of these
