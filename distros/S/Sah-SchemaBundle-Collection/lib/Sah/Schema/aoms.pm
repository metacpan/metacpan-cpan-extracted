...
This document describes version 0.009 of Sah::Schema::aoms (from Perl distribution Sah-SchemaBundle-Collection), released on 2024-06-13.
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("aoms", {return_type=>'str_errmsg'});
 ...
 $data = ["a",[]];
 my $errmsg = $validator->($data); # => "\@[1]: Not of type text"

Often a schema has coercion rule or default value rules, so after validation the
validated value will be different from the original. To return the validated
(set-as-default, coerced, prefiltered) value:

 my $validator = gen_validator("aoms", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = ["a",[]];
 my $res = $validator->($data); # => ["\@[1]: Not of type text",["a",[]]]
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('aoms*', name=>'Aoms')
     );
 }

 use My::Types qw(Aoms);
 Aoms->assert_valid($data);
