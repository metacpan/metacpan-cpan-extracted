{
  ...
  sub PCSI_register {
    ...
    $ircd->plugin_register($self, 'SERVER', qw(daemon_dline));
    return 1;
  }
  ...
  sub IRCD_daemon_dline {
    my ($self, $ircd) = splice @_, 0, 2;
    my $alarm_id = $ircd->{state}{dlines}[0]{alarm};
...
sub ircd_daemon_expired {
    my ($heap,@args) = @_[HEAP,ARG0..$#_];
    is($args[0], 'd-line', 'Type is D-Line' );
    is($args[1], '127.0.0.1/32', 'The mask is right' );
    my $result = $heap->{ircd}->denied('127.0.0.1');
...
sub client_disconnected {
  ...
  $heap->{undline}++;
  diag("Waiting for D-Line to expire, should be 10 seconds or so\n");
  return;
}
...
sub ircd_daemon_eob {
  ...
  if ( $heap->{eob} >= 3 ) {
    $kernel->post( 'groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'DLINE', params => [ '*', '60', '127.0.0.1/32', 'Banhammer'] } );
  }
  return;
}
