...
# DMS::Parser::encode and DMS::XS::Parser::encode — the document shape
...
{ my $warned;
  sub to_dms {
      unless ($warned++) {
          Carp::carp(
              'DMS::Emitter::to_dms() is deprecated; use encode() instead. '
...
{ my $warned;
  sub to_dms_lite {
      unless ($warned++) {
          Carp::carp(
              'DMS::Emitter::to_dms_lite() is deprecated; use encode_lite() instead. '
...
sub _emit {
    ...
    # a Document that contains a DMS::UnorderedTable. The Document tree
    ...
    if (!$lite && _contains_unordered($doc->{body})) {
        die "encode (full-mode round-trip) refuses Document with DMS::UnorderedTable; "
...
# DMS::Index objects (list indices). We encode each segment with a
# distinct prefix so the join is unambiguous (a string key "0" doesn't
# collide with index 0).
sub _path_key {
    my ($p) = @_;
    return join("\0", map {
        ref($_) eq 'DMS::Index' ? "I:" . $$_ : "K:$_"
    } @$p);
}
...
sub _is_table {
    ...
    # SPEC §"Unordered tables": DMS::UnorderedTable is a blessed hashref
    ...
    if (blessed($v)) {
        return ref($v) eq 'DMS::UnorderedTable';
    }
...
sub _table_keys {
    ...
    # DMS::UnorderedTable (plain blessed hash) or plain HV: arbitrary
...
# is a DMS::UnorderedTable. Used by `to_dms` (full mode) to refuse
# round-trip on unordered Documents per SPEC §"Unordered tables".
sub _contains_unordered {
    my ($v) = @_;
    return 0 if !defined $v;
    if (blessed($v)) {
        # DMS::UnorderedTable is itself the marker — we don't need to
        # walk into it because finding the outer one is enough; any
        # nested UnorderedTable would still be detected via the body.
        return 1 if ref($v) eq 'DMS::UnorderedTable';
        # Other blessed sentinels (DMS::Integer, DMS::Bool, dates,
        # DMS::Float) are leaves.
        return 0;
    }
    if (ref($v) eq 'HASH') {
        for my $k (keys %$v) {
...
sub _emit_list_block {
    ...
    for (my $i = 0; $i < @$items; $i++) {
        my $v = $items->[$i];
        my $child_path = [ @$path, DMS::Index->new($i) ];
        my $child_pk = _path_key($child_path);
        my $nc = $self->{comments_by_path}{$child_pk};
        if ($nc) {
            for my $c (@{ $nc->{leading} }) {
...
sub _emit_value_inline {
    ...
    if (blessed($v)) {
        my $cls = ref($v);
        if    ($cls eq 'DMS::Bool')          { $self->{out} .= $v->value ? 'true' : 'false'; }
        elsif ($cls eq 'DMS::Integer')       { $self->_emit_integer($v, $path); }
        elsif ($cls eq 'DMS::Float')         { $self->_emit_float($v->value); }
        elsif ($cls eq 'DMS::OffsetDateTime'
            || $cls eq 'DMS::LocalDateTime'
            || $cls eq 'DMS::LocalDate'
            || $cls eq 'DMS::LocalTime')     { $self->{out} .= $v->value; }
    ...
    if (_is_list($v)) {
        ...
        for (my $i = 0; $i < @$v; $i++) {
            $self->{out} .= ', ' if $i > 0;
            my $sub = $lite ? undef : [ @$path, DMS::Index->new($i) ];
...
sub _emit_integer {
    my ($self, $iv, $path) = @_;
    if ($self->{lite}) {
        # DMS::Integer is `bless \$v, 'DMS::Integer'` where $v is an IV.
    ...
    # Default: canonical decimal. DMS::Integer's bstr stringifies the IV.
    $self->{out} .= $iv->bstr;
}
...
sub _emit_modifier_arg {
    ...
    if (blessed($v)) {
        my $cls = ref($v);
        if    ($cls eq 'DMS::Bool')    { $self->{out} .= $v->value ? 'true' : 'false'; }
        elsif ($cls eq 'DMS::Integer') { $self->{out} .= $v->bstr; }
        elsif ($cls eq 'DMS::Float')   { $self->_emit_float($v->value); }
        elsif ($cls eq 'DMS::OffsetDateTime'
            || $cls eq 'DMS::LocalDateTime'
            || $cls eq 'DMS::LocalDate'
