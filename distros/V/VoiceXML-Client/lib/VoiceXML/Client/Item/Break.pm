...
package VoiceXML::Client::Item::Break;
...
sub init {
	...
	$self->{'strength'} = $self->{'XMLElement'}->attribute('strength') || 'medium';
...
sub execute {
	...
	if ($self->{'strength'} && exists $StrengthToSeconds{$self->{'strength'}} && $StrengthToSeconds{$self->{'strength'}})
	{
		$handle->wait($StrengthToSeconds{$self->{'strength'}});
	}
