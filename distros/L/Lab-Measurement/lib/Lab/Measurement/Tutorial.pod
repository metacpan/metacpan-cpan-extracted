...
L<Lab::XPRESS> is provided in L<Lab::Measurement::Legacy::Tutorial>. These
...
Setting the C<date_prefix> attribute to C<1> will prefix the folder name with
the date:
...
This will create output folders with names F<YYYY-MM-DD_IV_curve_xxx>. The
exact timestamp with second precision can always be found in F<META.yml>.
