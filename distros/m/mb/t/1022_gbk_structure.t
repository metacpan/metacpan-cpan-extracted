...
# GBK multibyte structure, verified directly through the runtime API. mb is
...
# stays US-ASCII.
#
# GBK, UHC, Big5 and Big5-HKSCS share the SAME getc lead dispatch: every octet
# \x81-\xFE is a two-octet lead, everything else (\x00-\x80 and \xFF) is a
# one-octet unit. This file exercises that shared 2-octet structure through the
# 'gbk' encoding.
#
# GBK characters:
#   A  US-ASCII single octet               "Z"          (\x5A)
#   G  two octets, trailing octet "@"      \x81\x40
#   B  two octets, trailing octet "A"      \x81\x41     (\x41 buried inside)
#   D  two octets, trailing octet "\"      \x81\x5C     (DAMEMOJI: 2nd octet \)
# The trailing \x40, \x41 and \x5C octets must be kept inside their two-octet
# characters and must NOT be seen as character boundaries.
...
mb::set_script_encoding('gbk');

my $A = "\x5A";         # "Z"
my $G = "\x81\x40";     # 2-octet, trailing octet "@" (\x40)
my $B = "\x81\x41";     # 2-octet, trailing octet "A" (\x41)
my $D = "\x81\x5C";     # 2-octet, trailing octet "\" (\x5C)
my $s = $A . $G . $B . $D . $A;  # 1+2+2+2+1 octets, 5 characters

BEGIN { open(GETCFH,">@{[__FILE__]}.getc.tmp") or die $!; binmode GETCFH; print GETCFH "\x5A\x81\x40\x81\x41\x81\x5C\x5A"; close(GETCFH); }
END   { unlink("@{[__FILE__]}.getc.tmp") }

@test = (

# get/set round trip
    sub { mb::get_script_encoding() eq 'gbk' },
...
# getc-style lead dispatch: \x81-\xFE are leads, \x00-\x80 and \xFF single
    ...
    sub { ("\x80" =~ /\A [\x81-\xFE] \z/x) ? 0 : 1 },  # \x80 not a lead
    sub { ("\xFF" =~ /\A [\x81-\xFE] \z/x) ? 0 : 1 },  # \xFF not a lead
...
# mb::getc reads one whole character at a time from a real filehandle
    sub { open(GETCFH,"@{[__FILE__]}.getc.tmp") or die $!; binmode GETCFH;
          ...
          close(GETCFH);
          ($c1 eq $A) and ($c2 eq $G) and ($c3 eq $B) and
