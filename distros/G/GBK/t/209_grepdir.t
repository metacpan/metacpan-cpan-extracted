# encoding: GBK
# This file is encoded in GBK.
die "This file is not encoded in GBK.\n" if q{偁} ne "\x82\xa0";

use GBK;
print "1..2\n";

my $__FILE__ = __FILE__;

if ($^O !~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    print "ok - 1 # SKIP $^X $__FILE__\n";
    print "ok - 2 # SKIP $^X $__FILE__\n";
    exit;
}

$| = 1;

mkdir('dt',0777);
mkdir('dt/alphabet',0777);
mkdir('dt/擔杮岅',0777);

open(FILE,">dt/alphabet/alpha.txt") || die "Can't open file: dt/alphabet/alpha.txt\n";
print FILE <<'END';
aaa
bbb
ccc
ddd
eee
END
close(FILE);

open(FILE,">dt/擔杮岅/alpha.txt") || die "Can't open file: dt/擔杮岅/alpha.txt\n";
print FILE <<'END';
aaa
bbb
ccc
ddd
eee
END
close(FILE);

open(FILE,">dt/alphabet/sjis.txt") || die "Can't open file: dt/alphabet/sjis.txt\n";
print FILE <<'END';
aaa
偁偁偁
bbb
偄偄偄
ccc
偆偆偆
ddd
昞
eee
END
close(FILE);

open(FILE,">dt/擔杮岅/sjis.txt") || die "Can't open file: dt/擔杮岅/sjis.txt\n";
print FILE <<'END';
aaa
偁偁偁
bbb
偄偄偄
ccc
偆偆偆
ddd
昞
eee
END
close(FILE);

my $aaa = <<'END';
!!dt/alphabet!!
!!dt/alphabet/alpha.txt!!
dt/alphabet/alpha.txt:aaa
!!dt/alphabet/sjis.txt!!
dt/alphabet/sjis.txt:aaa
!!dt/擔杮岅!!
!!dt/擔杮岅/alpha.txt!!
dt/擔杮岅/alpha.txt:aaa
!!dt/擔杮岅/sjis.txt!!
dt/擔杮岅/sjis.txt:aaa
END

my $hyou = <<'END';
!!dt/alphabet!!
!!dt/alphabet/alpha.txt!!
!!dt/alphabet/sjis.txt!!
dt/alphabet/sjis.txt:昞
!!dt/擔杮岅!!
!!dt/擔杮岅/alpha.txt!!
!!dt/擔杮岅/sjis.txt!!
dt/擔杮岅/sjis.txt:昞
END

my $script = __FILE__ . '.pl';

open(FILE,">$script") || die "Can't open file: $script\n";
print FILE <DATA>;
close(FILE);

if (($ENV{'PERL5SHELL'}||$ENV{'COMSPEC'}) =~ / \\COMMAND\.COM \z/oxmsi) {
    $_ = `$^X $script aaa dt`;
}
else {
    $_ = `$^X $script aaa dt 2>NUL`;
}
sleep 1;
if ($_ eq $aaa) {
    print "ok - 1 $^X $__FILE__ aaa dt \n";
}
else {
    print "not ok - 1 $^X $__FILE__ aaa dt \n";
    print "($_)\n";
    print "($aaa)\n";
}

if (($ENV{'PERL5SHELL'}||$ENV{'COMSPEC'}) =~ / \\COMMAND\.COM \z/oxmsi) {
    $_ = `$^X $script 昞 dt`;
}
else {
    $_ = `$^X $script 昞 dt 2>NUL`;
}
sleep 1;
if ($_ eq $hyou) {
    print "ok - 2 $^X $__FILE__ 昞 dt\n";
}
else {
    print "not ok - 2 $^X $__FILE__ 昞 dt\n";
    print "($_)\n";
    print "($hyou)\n";
}
sleep 1;

unlink($script);
unlink("$script.e");

unlink('dt/alphabet/alpha.txt');
unlink('dt/alphabet/sjis.txt');
unlink('dt/擔杮岅/alpha.txt');
unlink('dt/擔杮岅/sjis.txt');
rmdir('dt/alphabet');
rmdir('dt/擔杮岅');
rmdir('dt');

__END__
# encoding: GBK
# This file is encoded in ShiftJIS.
die "This file is not encoded in ShiftJIS.\n" if q{偁} ne "\x82\xa0";

use GBK;

local $^W = 1;

if (@ARGV < 2) {
    die <<END;
幚峴曽朄:

perl $0 aaa dt
perl $0 昞  dt
END
}

&grepdir(@ARGV);

exit 0;

sub grepdir ($$) {
    my($pat,$dir) = @_;
    my($node);

    opendir(D,$dir);
    my @nodes = grep (!/^\./, readdir(D));
    closedir(D);

    foreach $node (@nodes) {
        my $path="$dir/$node";
        print "!!$path!!\n";
        if ( -f $path ) {
            grepfile($pat,$path);
        }
        elsif( -d $path) {
            &grepdir($pat,$path);
        }
        else {
            print STDERR "skip:$path\n";
        }
    }
}

sub grepfile ($$) {
    my($pat,$file) = @_;
    open(IN,$file) or die "Error:open($file):$!\n";
    while (<IN>) {
        chomp;

# 廋惓売強1
#       print "$file:$_\n" if (/$pat/);
        print "$file:$_\n" if (/\Q$pat\E/);
    }
}

__END__

Windows偱Perl 5.8/5.10傪巊偆儌儞偠傖側偄

偺乽偙偙偱徯夘偟偨僗僋儕僾僩偺僒儞僾儖乿偺 grepdir.pl 傪棙梡偟偰偄傑偡丅

椺: 惓婯昞尰傪巜掕偟偰丆巜掕偟偨僨傿儗僋僩儕攝壓偺僼傽僀儖偐傜庢傝弌偡僐乕僪傪彂偄偰傞丅

僐儅儞僪宍幃: perl grepdir.pl {僷僞乕儞} {僨傿儗僋僩儕}

師偺傛偆側僥僗僩娐嫬傪梡堄偡傞丅

C:\TEMP\TP> tree /F dt
僼僅儖僟 僷僗偺堦棗: 儃儕儏乕儉 vvvvv_vvvvvvvvv
儃儕儏乕儉 僔儕傾儖斣崋偼 vvvv-ssss 偱偡
C:\TEMP\TP\DT
劌劅alphabet
劆      alpha.txt
劆      sjis.txt
劆
劋劅擔杮岅
       alpha.txt
       sjis.txt

偙傟傪 perl 偵偰幚峴偝偣傞偲丆師偺傛偆偵側傞丅

C:\TEMP\TP\DT>perl grepdir.pl aaa dt
!!dt/alphabet!!
!!dt/alphabet/alpha.txt!!
dt/alphabet/alpha.txt:aaa
!!dt/alphabet/sjis.txt!!
dt/alphabet/sjis.txt:aaa
!!dt/擔杮岅!!
!!dt/擔杮岅/alpha.txt!!
dt/擔杮岅/alpha.txt:aaa
!!dt/擔杮岅/sjis.txt!!
dt/擔杮岅/sjis.txt:aaa

C:\TEMP\TP\DT>perl grepdir.pl 昞 dt
!!dt/alphabet!!
!!dt/alphabet/alpha.txt!!
!!dt/alphabet/sjis.txt!!
dt/alphabet/sjis.txt:昞
!!dt/擔杮岅!!
!!dt/擔杮岅/alpha.txt!!
!!dt/擔杮岅/sjis.txt!!
dt/擔杮岅/sjis.txt:昞

捈偝側偗傟偽側傜側偄偲偙傠偼丆埲壓偺傛偆側偲偙傠偵側傞丅

廋惓売強1
  惓婯昞尰撪偵曄悢傪婰弎偟丄曄悢偵奿擺偝傟偰偄傞撪梕偦偺傕偺偵儅僢僠
  偝偣偨偄偺偱偁傟偽 \Q ... \E 偱埻傓昁梫偑偁傞丅

    ----------------------------------------------
    print "$file:$_\n" if (/$pat/);
    ----------------------------------------------
                伀 彂偒姺偊
    ----------------------------------------------
    print "$file:$_\n" if (/\Q$pat\E/);
    ----------------------------------------------

埲忋
