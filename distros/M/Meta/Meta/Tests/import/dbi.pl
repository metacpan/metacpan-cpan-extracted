...
for(my($i)=0;$i<=$#driver_names;$i++) {
	my($name)=$driver_names[$i];
	Meta::Utils::Output::print("driver $i [".$name."]\n");
	if($name eq "mysql" || $name eq "Pg") {
		my(@data_sources);
		if($name eq "Pg") {
			@data_sources=DBI->data_sources($name);
		}
		if($name eq "mysql") {
			my($drh)=DBI->install_driver($name);
			@data_sources=$drh->func("database",'_ListDBs');
		}
		for(my($j)=0;$j<=$#data_sources;$j++) {
			my($src)=$data_sources[$j];
			Meta::Utils::Output::print("\tsource $j [".$src."]\n");
		}
	}
}
...
dbi.pl - test the external DBI module.
...
This is a test suite for the DBI.pm package.
...
DBI(3), Meta::Baseline::Test(3), Meta::Utils::Opts::Opts(3), Meta::Utils::Output(3), Meta::Utils::System(3), strict(3)

=head1 TODO

-why doesnt the data_sources method return the right stuff ?
