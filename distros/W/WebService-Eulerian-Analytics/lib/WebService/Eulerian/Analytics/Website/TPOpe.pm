package WebService::Eulerian::Analytics::Website::TPOpe;

# $Id: TPOpe.pm,v 1.1 2008-09-07 23:31:00 cvscore Exp $
...
WebService::Eulerian::Analytics::Website::TPOpe - access to the TPOpe service for a given website : campaigns for outbound traffic.

=head1 DESCRIPTION

This module allow you to access the TPOpe service holding information 
...
=head1 SYNOPSIS
	...
	#
	my $api = new WebService::Eulerian::Analytics::Website::TPOpe(
...
sub new {
 ...
 return         $class->SUPER::new(@_, service => 'Website/TPOpe');
}
...
=head2 getById : return a tpope given it's id
...
=item * hash reference containing the attributes of the tpope

=back

=head3 sample

	my $rh_tpope = $api->getById($my_website_id, 1);
	#
	if ( $api->fault ) {
	 die $api->faultstring();
	}
	#
	foreach ( keys %{ $rh_tpope } ) {
	 print $_." => ".$rh_tpope->{$_}."\n";
	}
...
=head2 getByName : return a tpope given it's name
...
=head3 sample
	...
	my $rh_tpope = $api->getByName($my_website_id, 'test');
	#
	if ( $api->fault ) {
	 die $api->faultstring();
	}
...
=head2 search : search for a tpope
...
o sortkey : key on which the sorting can be done, defaults to tpope_name
...
o results : array of hash containing the list of tpope matching the search
...
=head3 sample
	...
	for ( @{ $rh_result->{results} || [] } ) {
	  print "\t name=".$_->{tpope_name}." | id=".$_->{tpope_id}."<\n";
	}
