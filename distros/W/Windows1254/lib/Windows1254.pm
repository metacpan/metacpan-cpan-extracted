...
######################################################################
#
# Windows1254 - Source code filter to escape Windows-1254 script
#
...
perl $0 Windows-1254_script.pl > Escaped_script.pl.e
END
    }

    print Windows1254::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Ewindows1254::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
        ...
        my $e_script = Windows1254::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Ewindows1254::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Windows1254::escape_script {
    ...
    # read Windows-1254 script
    my $fh = gensym();
    Ewindows1254::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Ewindows1254(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ewindows1254 '%s.0'; # 'quote' for perl5.6\n", $Windows1254::VERSION); # require run-time routines version

        # use Windows1254 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Windows1254 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Windows1254::VERSION) {
                    my @file = grep -e, map {qq{$_/Windows1254.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Windows1254.pm $version, but @{[__FILE__]} is version $Windows1254::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Ewindows1254.pm $version, but \\\$Ewindows1254::VERSION is \$Ewindows1254::VERSION" if \$Ewindows1254::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Windows1254::VERSION) {
                    die "Script $0 required Windows1254.pm $version, but @{[__FILE__]} is only version $Windows1254::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Ewindows1254::function_ord     = 'Windows1254::ord'     if /\A ord \z/oxms;
                    $Ewindows1254::function_ord_    = 'Windows1254::ord_'    if /\A ord \z/oxms;
                    $Ewindows1254::function_reverse = 'Windows1254::reverse' if /\A reverse \z/oxms;
                    $Ewindows1254::function_getc    = 'Windows1254::getc'    if /\A getc \z/oxms;

                    # avoid: used only once: possible typo at ...
                    $Ewindows1254::function_ord     = $Ewindows1254::function_ord;
                    $Ewindows1254::function_ord_    = $Ewindows1254::function_ord_;
                    $Ewindows1254::function_reverse = $Ewindows1254::function_reverse;
                    $Ewindows1254::function_getc    = $Ewindows1254::function_getc;
                }
            }
        }
    }

    $e_script .= Windows1254::escape();
...
=item * You'll have to download Windows1254.pm and Ewindows1254.pm and put it in your perl lib directory.

=item * You'll need to write "use Windows1254;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Windows1254;
  use Windows1254 ver.sion;             --- require minimum version
  use Windows1254 ver.sion.0;           --- expects version (match or die)

  # "no Windows1254;" not supported

  or

  $ perl Windows1254.pm Windows-1254_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Windows-1254_script.pl  --- script written in Windows-1254
...
Windows1254 software is "middleware" between perl interpreter and your Perl script
written in Windows-1254.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Windows-1254, Perl3 or later
can treat Windows-1254 as binary data.

By "use Windows1254;", it automatically interpret your script as Windows-1254. The various
functions of perl including a regular expression can treat Windows-1254 now.
...
This software is a source code filter to escape Perl script encoded by Windows-1254
...
=item * Remains One Language Handling Raw Windows-1254, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Windows1254.pm, Ewindows1254.pm
  +---------------------------------------+
...
import of Ewindows1254.pm works well.
...
=head1 Software Composition
   ...
   Windows1254.pm               --- source code filter to escape Windows-1254
   Ewindows1254.pm              --- run-time routines for Windows1254.pm
...
You need write 'use Windows1254;' in your script.
...
=head1 Calling 'Ewindows1254::ignorecase()' (Windows1254 software provides)

Windows1254 software applies calling 'Ewindows1254::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Ewindows1254::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Ewindows1254.pm provides)
...
Definitions in Ewindows1254.pm.
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Windows1254 software provides)

Windows1254 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Windows1254 software provides)

Insert 'Ewindows1254::' at head of function name. Ewindows1254.pm provides your script Ewindows1254::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Ewindows1254::lc        Character
  lcfirst     Ewindows1254::lcfirst   Character
  uc          Ewindows1254::uc        Character
  ucfirst     Ewindows1254::ucfirst   Character
  fc          Ewindows1254::fc        Character
  chr         Ewindows1254::chr       Character
  glob        Ewindows1254::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Windows1254 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Windows1254 software provides)

Windows1254 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Ewindows1254.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Windows1254 software and
...
exist at time of be written this document and JPerl did exist.
                        ...
                        (a)     (b)     (c)     (d)     (e)
                                      JPerl,japerl    Encode,Windows1254
      +--------------+-------+-------+-------+-------+-------+
      | data         |  Old  |  Old  |  New  |  Old  |  New  |
      +--------------+-------+-------+-------+-------+-------+
      | script       |  Old  |      Old      |      New      |
...
Windows1254 software remains one language and one interpreter.
