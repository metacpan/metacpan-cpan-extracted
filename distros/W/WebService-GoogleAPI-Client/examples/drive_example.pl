...
if ($config->{do}{'about.get'}) {
  ...
  my $r = $gapi_client->api_query(
    api_endpoint_id => "$versioned_api.about.get",
    options         => {
      fields => '*'
      ,    #'user,canCreateTeamDrives,kind,maxUploadSize,appInstalled,canCreateTeamDrives,importFormats,exportFormats'
    }
  );
  #print Dumper  $r; # ->json;
  if ($config->{debug}) {
...
if ($config->{do}{'files.list'}) {
  ...
  while ($pagination->{is_more} == 1) {
    ...
    my $d = $gapi_client->api_query(
      api_endpoint_id => "$versioned_api.files.list",
      options         => $options
    )->json;
