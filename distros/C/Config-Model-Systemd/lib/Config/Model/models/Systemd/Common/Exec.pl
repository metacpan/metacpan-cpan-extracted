...
',
    ...
    'description' => {
      'AmbientCapabilities' => 'Controls which capabilities to include in the ambient capability set for the executed
...
C<+>.',
      'AppArmorProfile' => 'Takes a profile name as argument. The process executed by the unit will switch to
...
is not enabled. This setting does not affect commands prefixed with C<+>.',
      'BPFDelegateAttachments' => 'Accepts a list of BPF attach points to allow or C<any> to allow everything.
...
see C<PrivateBPF> more details.',
      'BPFDelegateCommands' => 'Accepts a list of BPF commands to allow or C<any> to allow everything.
Defaults to none. The accepted values are:
...
Requires C<PrivateBPF=yes> to be effective,
see C<PrivateBPF> more details.',
      'BPFDelegateMaps' => 'Accepts a list of BPF maps to allow or C<any> to allow everything.
Defaults to none. The accepted values are:
...
Requires C<PrivateBPF=yes> to be effective,
see C<PrivateBPF> more details.',
      'BPFDelegatePrograms' => 'Accepts a list of BPF programs to allow or C<any> to allow everything.
Defaults to none. The accepted values are:
...
Requires C<PrivateBPF=yes> to be effective,
see C<PrivateBPF> more details.',
      'BindLogSockets' => 'Takes a boolean argument. If true, sockets from L<systemd-journald.socket(8)>
...
in conjunction with either C<RootDirectory> or C<RootImage>.',
      'BindPaths' => 'Configures unit-specific bind mounts. A bind mount makes a particular file or
...
C<ProtectHome=tmpfs> should be used instead.',
      ...
      'CPUAffinity' => 'Controls the CPU affinity of the executed processes. Takes a list of CPU indices or ranges
...
details.',
      'CPUSchedulingPolicy' => 'Sets the CPU scheduling policy for executed processes. Takes one of C<other>,
...
for details.',
      'CPUSchedulingResetOnFork' => 'Takes a boolean argument. If true, elevated CPU scheduling priorities and policies
will be reset when the executed processes call
L<fork(2)>,
and can hence not leak into child processes. See
L<sched_setscheduler(2)>
for details. Defaults to false.',
      'CacheDirectory' => "These options take a whitespace-separated list of directory names. The specified
...
C</run/foo>.",
      'CacheDirectoryAccounting' => 'Takes a boolean argument. If true, a project ID is assigned to the directories specified in
...
or C<LogsDirectoryQuota> must be specified.',
      'CacheDirectoryMode' => 'Specifies the access mode of the directories specified in C<RuntimeDirectory>,
...
discussion of the meaning of permission bits.',
      'CacheDirectoryQuota' => 'Specifies the storage limits for the directories specified in C<StateDirectory>,
...
L<quotaon.|https://linux.die.net/man/8/quotaon>',
      'CapabilityBoundingSet' => 'Controls which capabilities to include in the capability bounding set for the
...
then, only C<CAP_A> is set.',
      ...
      'ConfigurationDirectoryMode' => '*CacheDirectoryMode',
      'CoredumpFilter' => 'Controls which types of memory mappings will be saved if the process dumps core
...
ORed. When not set, or if the empty value is assigned, the inherited value is not changed.',
      'DelegateNamespaces' => 'Delegates ownership of the given namespace types to the user namespace of the
...
is implicitly delegated.',
      'DynamicUser' => "Takes a boolean parameter. If set, a UNIX user and group pair is allocated
...
C<LogLevelMax> permitted it to be processed.',
      'LogNamespace' => 'Run the unit\'s processes in the specified journal namespace. Expects a short
...
output, unless the C<--namespace=> option is used.',
      'LogRateLimitBurst' => "Configures the rate limiting that is applied to log messages generated by this unit.
...
options instead, in particular C<PrivateMounts>, see above.",
      ...
      'MountImages' => 'This setting is similar to C<RootImage> in that it mounts a file
...
about environment variables.',
      'Personality' => 'Controls which kernel architecture L<uname(2)> shall
...
as C<m68k> (32-bit only) or C<alpha> (64-bit only).',
      'PrivateBPF' => 'Takes a boolean argument. If set, mount a private instance of the BPF filesystem
...
L<LWN post|https://lwn.net/Articles/947173/>.',
      'PrivateDevices' => 'Takes a boolean argument. If true, sets up a new C</dev/> mount for
...
L<systemd.resource-control(5)>.
',
      'PrivateIPC' => 'Takes a boolean argument. If true, sets up a new IPC namespace for the executed processes.
...
security.',
      'PrivateMounts' => "Takes a boolean parameter. If set, the processes of this unit will be run in their own private
...
request this behaviour if none of the other settings are used.",
      'PrivateNetwork' => 'Takes a boolean argument. If true, sets up a new network namespace for the executed processes
...
services.',
      'PrivatePIDs' => 'Takes a boolean argument. Defaults to false. If enabled, sets up a new PID namespace
...
instance of C</proc/>.',
      'PrivateTmp' => 'Takes a boolean argument, or C<disconnected>. If enabled, a new
...
security.',
      'PrivateUsers' => 'Takes a boolean argument or one of C<self>,
...
as if the option was not used.',
      'ProtectSystem' => "Takes a boolean argument or the special values C<full> or
...
C</var/tmp/> will be writable.",
      'ReadOnlyPaths' => '*ExecPaths',
      'ReadWritePaths' => '*ExecPaths',
      'RemoveIPC' => 'Takes a boolean parameter. If set, all System V and POSIX IPC objects owned by the user and
...
stopped. This setting is implied if C<DynamicUser> is set.',
      'RestrictAddressFamilies' => 'Restricts the set of socket address families accessible to the processes of this
...
C<SystemCallArchitectures=native> or similar.',
      'RestrictFileSystems' => 'Restricts the set of filesystems processes of this unit can open files on. Takes a space-separated
...
has no effect.',
      'RestrictNamespaces' => "Restricts access to Linux namespace functionality for the processes of this unit. For details
...
then, only C<ipc> is set.",
      'RestrictRealtime' => 'Takes a boolean argument. If set, any attempts to enable realtime scheduling in a process of
...
that actually require them. Defaults to off.',
      'RestrictSUIDSGID' => 'Takes a boolean argument. If set, any attempts to set the set-user-ID (SUID) or
...
defaults to off.',
      'RootDirectory' => 'Takes a directory path relative to the host\'s root directory (i.e. the root of the system
...
in case the service is configured to survive it.',
      'RootEphemeral' => 'Takes a boolean argument. If enabled, executed processes will run in an ephemeral
...
with support for reflinks should be used to ensure an efficient ephemeral copy.',
      'RootHash' => 'Takes a data integrity (dm-verity) root hash specified in hexadecimal, or the path to a file
...
system via the unit file directly.',
      'RootHashSignature' => 'Takes a PKCS7 signature of the C<RootHash> option as a path to a
...
to be added to the unit (see above).",
      ...
      'StateDirectoryQuota' => '*CacheDirectoryQuota',
      'SupplementaryGroups' => 'Sets the supplementary Unix groups the processes are executed as. This takes a space-separated
...
commands prefixed with C<+>.',
      'SyslogFacility' => 'Sets the syslog facility identifier to use when logging. One of
...
written to stdout or stderr. Defaults to C<daemon>.',
      'SyslogIdentifier' => 'Sets the process name ("syslog tag") to prefix log lines sent to
...
written to stdout or stderr.',
      'SyslogLevel' => 'The default syslog log level to use when logging to the logging system or
...
Defaults to C<info>.',
      'SyslogLevelPrefix' => 'Takes a boolean argument. If true and C<StandardOutput> or
C<StandardError> are set to C<journal> or C<kmsg> (or to
...
L<sd-daemon(3)>.
Defaults to true.',
      'SystemCallArchitectures' => "Takes a space-separated list of architecture identifiers to include in the system call
...
the timeout is reached, potentially leaving resources on disk.",
      'TimerSlackNSec' => 'Sets the timer slack in nanoseconds for the executed processes. The timer slack controls the
...
nano-seconds if no unit is specified. The usual time units are understood too.',
      'UMask' => "Controls the file mode creation mask. Takes an access mode in octal notation. See
L<umask(2)> for
...
module, such as L<pam_umask(8)>.",
      'UnsetEnvironment' => 'Explicitly unset environment variable assignments that would normally be passed from the
...
information about environment variables.',
      'User' => '*Group',
      'UserNamespacePath' => 'Takes an absolute file system path referring to a Linux user namespace
...
This option is only available for system services.',
      'UtmpIdentifier' => 'Takes a four character identifier string for an L<utmp(5)> and wtmp entry
...
service.',
      'UtmpMode' => 'Takes one of C<init>, C<login> or C<user>. If
...
leader. Defaults to C<init>.',
      'WorkingDirectory' => 'Takes a directory path relative to the service\'s root directory specified by
