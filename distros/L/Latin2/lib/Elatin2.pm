...
######################################################################
#
# Elatin2 - Run-time routines for Latin2.pm
#
...
#
# Latin-2 character range per length
#
my %range_tr = ();

#
# Latin-2 case conversion
#
my %lc = ();
...
sub import {
    ...
    *Char::ord           = \&Latin2::ord;
    *Char::ord_          = \&Latin2::ord_;
    *Char::reverse       = \&Latin2::reverse;
    *Char::getc          = \&Latin2::getc;
    *Char::length        = \&Latin2::length;
    *Char::substr        = \&Latin2::substr;
    *Char::index         = \&Latin2::index;
    *Char::rindex        = \&Latin2::rindex;
    *Char::eval          = \&Latin2::eval;
    *Char::escape        = \&Latin2::escape;
    *Char::escape_token  = \&Latin2::escape_token;
    *Char::escape_script = \&Latin2::escape_script;
}
...
sub Elatin2::rindex($$;$);
sub Elatin2::lcfirst(@);
sub Elatin2::lcfirst_();
sub Elatin2::lc(@);
sub Elatin2::lc_();
sub Elatin2::ucfirst(@);
sub Elatin2::ucfirst_();
sub Elatin2::uc(@);
sub Elatin2::uc_();
sub Elatin2::fc(@);
sub Elatin2::fc_();
sub Elatin2::ignorecase;
sub Elatin2::classic_character_class;
sub Elatin2::capture;
sub Elatin2::chr(;$);
sub Elatin2::chr_();
sub Elatin2::glob($);
...
# ${Elatin2::eS}          = qr{(?>[^\x09\x0A    \x0C\x0D\x20])};
# ${Elatin2::eS}          = qr{(?>[^\x09\x0A\x0B\x0C\x0D\x20])};
${Elatin2::eS}            = qr{(?>[^\s])};
...
#
# Latin-2 lower case first with parameter
#
sub Elatin2::lcfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Elatin2::lc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Elatin2::lc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Elatin2::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}

#
# Latin-2 lower case first without parameter
#
sub Elatin2::lcfirst_() {
    return Elatin2::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
}
...
#
# Latin-2 lower case with parameter
#
sub Elatin2::lc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# Latin-2 lower case without parameter
#
sub Elatin2::lc_() {
    my $s = $_;
    return join '', map {defined($lc{$_}) ? $lc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# Latin-2 upper case first with parameter
#
sub Elatin2::ucfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Elatin2::uc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Elatin2::uc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Elatin2::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}

#
# Latin-2 upper case first without parameter
#
sub Elatin2::ucfirst_() {
    return Elatin2::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
}
...
#
# Latin-2 upper case with parameter
#
sub Elatin2::uc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# Latin-2 upper case without parameter
#
sub Elatin2::uc_() {
    my $s = $_;
    return join '', map {defined($uc{$_}) ? $uc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# Latin-2 fold case with parameter
#
sub Elatin2::fc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# Latin-2 fold case without parameter
#
sub Elatin2::fc_() {
    my $s = $_;
    return join '', map {defined($fc{$_}) ? $fc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# Latin-2 regexp ignore case modifier
#
sub Elatin2::ignorecase {
    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # with /i modifier
                ...
                my $uc = Elatin2::uc($char[$i]);
                my $fc = Elatin2::fc($char[$i]);
                if ($uc ne $fc) {
                    if (CORE::length($fc) == 1) {
                        $char[$i] = '['   . $uc       . $fc . ']';
                    }
...
#
# prepare Latin-2 characters per length
#
...
#
# Latin-2 open character list for tr
#
sub _charlist_tr {
...
#
# Latin-2 open character list for qr and not qr
#
sub _charlist {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '${Elatin2::not_lower_i}',
                '[:^upper:]'  => '${Elatin2::not_upper_i}',
...
#
# Latin-2 hexadecimal escape sequence
#
sub hexchr {
    my($hexdigit) = @_;
...
#
# Latin-2 open character list for qr
#
sub charlist_qr {
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Elatin2::uc($char);
                        my $fc = Elatin2::fc($char);
                        if ($uc eq $fc) {
...
#
# Latin-2 open character list for not qr
#
sub charlist_not_qr {
...
#
# Latin-2 order to character (with parameter)
#
sub Elatin2::chr(;$) {
...
#
# Latin-2 order to character (without parameter)
#
sub Elatin2::chr_() {
...
#
# Latin-2 path globbing (with parameter)
#
sub Elatin2::glob($) {
...
#
# Latin-2 path globbing (without parameter)
#
sub Elatin2::glob_() {
...
#
# Latin-2 path globbing via File::DosGlob 1.10
#
...
#
# Latin-2 path globbing subroutine
#
sub _do_glob {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $fc = Elatin2::fc($char)) ne $char) {
                $pattern .= $fc;
            }
            else {
        ...
        my $matchsub = sub { Elatin2::fc($_[0]) =~ /\A $pattern \z/xms };
...
INNER:
        for my $leaf (@leaf) {
            ...
            if (Elatin2::index($leaf,'.') == -1 and   # if name does not have a dot in it *and*
                CORE::length($leaf) <= 8 and        # name is shorter than or equal to 8 chars *and*
                Elatin2::index($pattern,'\\.') != -1  # pattern has a dot.
            ) {
                if (&$matchsub("$leaf.")) {
                    push @matched, "$head$leaf";
                    next INNER;
...
#
sub Elatin2::POSTMATCH {
    return $';
}

#
# Latin-2 character to order (with parameter)
#
sub Latin2::ord(;$) {
...
#
# Latin-2 character to order (without parameter)
#
sub Latin2::ord_() {
...
#
# Latin-2 getc (with parameter, without parameter)
#
sub Latin2::getc(;*@) {

    my($package) = caller;
    my $fh = @_ ? qualify_to_ref(shift,$package) : \*STDIN;
    croak 'Too many arguments for Latin2::getc' if @_ and not wantarray;
    ...
    for my $length ($length[0] .. $length[-1]) {
        ...
        if (exists $range_tr{CORE::length($getc)}) {
            if ($getc =~ /\A ${Elatin2::dot_s} \z/oxms) {
...
#
# Latin-2 index by character
#
sub Latin2::index($$;$) {

    my $index;
    if (@_ == 3) {
        $index = Elatin2::index($_[0], $_[1], CORE::length(Latin2::substr($_[0], 0, $_[2])));
    }
    else {
        $index = Elatin2::index($_[0], $_[1]);
    }
    ...
    if ($index == -1) {
        ...
        return Latin2::length(CORE::substr $_[0], 0, $index);
    }
}

#
# Latin-2 rindex by character
#
sub Latin2::rindex($$;$) {

    my $rindex;
    if (@_ == 3) {
        $rindex = Elatin2::rindex($_[0], $_[1], CORE::length(Latin2::substr($_[0], 0, $_[2])));
    }
    else {
        $rindex = Elatin2::rindex($_[0], $_[1]);
    }
    ...
    if ($rindex == -1) {
        ...
        return Latin2::length(CORE::substr $_[0], 0, $rindex);
    }
}
...
# ord() to ord() or Latin2::ord()
my $function_ord = 'ord';

# ord to ord or Latin2::ord_
my $function_ord_ = 'ord';

# reverse to reverse or Latin2::reverse
my $function_reverse = 'reverse';

# getc to getc or Latin2::getc
...
my $qq_substr  = qr{(?> Char::substr | Latin2::substr | CORE::substr | substr ) (?>\s*) \( $qq_paren \)
                 }xms;
...
#
# escape Latin-2 token of script
#
sub Latin2::escape_token {
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Elatin2::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ (?>\s*) PREMATCH  \b | \$ (?>\s*) \{ (?>\s*) PREMATCH  (?>\s*) \} | \$ (?>\s*) \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin2::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Elatin2::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ (?>\s*) MATCH     \b | \$ (?>\s*) \{ (?>\s*) MATCH     (?>\s*) \} | \$ (?>\s*) \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin2::MATCH()};
    }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Elatin2::POSTMATCH()
    elsif (/\G (                 \$ (?>\s*) POSTMATCH \b | \$ (?>\s*) \{ (?>\s*) POSTMATCH (?>\s*) \} | \$ (?>\s*) \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin2::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Elatin2::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while (?>\s*) \( (?>\s*) glob (?>\s*) \) /oxgc) {
        return 'while ($_ = Elatin2::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while (?>\s*) \( (?>\s*) glob \b /oxgc) {
        return 'while ($_ = Elatin2::glob';
    }
...
# subroutines of package Elatin2
    elsif (/\G \b (CORE:: | ->(>?\s*) (?: atan2 | [a-z]{2,})) \b       /oxgc) { $slash = 'm//'; return $1;                  }
    elsif (/\G \b Char::eval       (?= (?>\s*) \{ )                    /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Latin2::eval       (?= (?>\s*) \{ )                    /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval Char::escape'; }
    elsif (/\G \b Latin2::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval Latin2::escape'; }
    elsif (/\G \b bytes::substr \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'substr';            }
    elsif (/\G \b chop \b          (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Elatin2::chop';       }
    elsif (/\G \b bytes::index \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'index';             }
    elsif (/\G \b Char::index \b   (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Char::index';       }
    elsif (/\G \b Latin2::index \b   (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Latin2::index';       }
    elsif (/\G \b index \b         (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Elatin2::index';      }
    elsif (/\G \b bytes::rindex \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'rindex';            }
    elsif (/\G \b Char::rindex \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Char::rindex';      }
    elsif (/\G \b Latin2::rindex \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Latin2::rindex';      }
    elsif (/\G \b rindex \b        (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Elatin2::rindex';     }
    elsif (/\G \b lc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin2::lc';         }
    elsif (/\G \b lcfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin2::lcfirst';    }
    elsif (/\G \b uc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin2::uc';         }
    elsif (/\G \b ucfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin2::ucfirst';    }
    elsif (/\G \b fc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin2::fc';         }
    ...
    elsif (/\G \b chr           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin2::chr';               }
    elsif (/\G \b bytes::ord    (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin2::glob';              }
    elsif (/\G \b lc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin2::lc_';               }
    elsif (/\G \b lcfirst \b       (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin2::lcfirst_';          }
    elsif (/\G \b uc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin2::uc_';               }
    elsif (/\G \b ucfirst \b       (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin2::ucfirst_';          }
    elsif (/\G \b fc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin2::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin2::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! (?>\s*) => )                          /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! (?>\s*) => )                          /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin2::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Elatin2::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Elatin2::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq           (\#) [ ] (\#) /oxgc) { return 'Elatin2::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq ((?>\s*)) (\() [ ] (\)) /oxgc) { return 'Elatin2::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\{) [ ] (\}) /oxgc) { return 'Elatin2::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\[) [ ] (\]) /oxgc) { return 'Elatin2::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\<) [ ] (\>) /oxgc) { return 'Elatin2::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\S) [ ] (\2) /oxgc) { return 'Elatin2::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q            (\#) [ ] (\#) /oxgc) { return 'Elatin2::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  ((?>\s*)) (\() [ ] (\)) /oxgc) { return 'Elatin2::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\{) [ ] (\}) /oxgc) { return 'Elatin2::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\[) [ ] (\]) /oxgc) { return 'Elatin2::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\<) [ ] (\>) /oxgc) { return 'Elatin2::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\S) [ ] (\2) /oxgc) { return 'Elatin2::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                    ' [ ] '    /oxgc) { return 'Elatin2::split' . $e . qq     {' '};  }
        elsif (/\G                    " [ ] "    /oxgc) { return 'Elatin2::split' . $e . qq     {" "};  }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Elatin2::PREMATCH()]}
            ...
            $e_string .= q{Elatin2::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Elatin2::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ (?>\s*) MATCH     \b | \$ (?>\s*) \{ (?>\s*) MATCH     (?>\s*) \} | \$ (?>\s*) \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Elatin2::MATCH()};
            $slash = 'div';
        }
...
sub character_class {
    ...
    if ($char eq '.') {
        ...
        return Elatin2::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Elatin2::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Elatin2::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Elatin2::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Elatin2::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Elatin2::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Elatin2::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Elatin2::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Elatin2::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Elatin2::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Elatin2::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Elatin2::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Elatin2::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Elatin2::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Elatin2::uc($char[$i]) ne Elatin2::fc($char[$i]))) {
            if (CORE::length(Elatin2::fc($char[$i])) == 1) {
                $char[$i] = '['   . Elatin2::uc($char[$i])       . Elatin2::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Elatin2::uc($char[$i]) . '|' . Elatin2::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin2::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ (?>\s*) 0 (?>\s*) \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Elatin2::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin2::ignorecase(Elatin2::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin2::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Elatin2::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin2::ignorecase(Elatin2::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin2::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Elatin2::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin2::ignorecase(Elatin2::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin2::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                               1                         2   3                                  4   5
                q<CORE::eval{local $Elatin2::re_t=%s; while($Elatin2::re_t =~ %s){%s local $^W=0; local $Elatin2::re_r=%s; %s$Elatin2::re_t="$`$Elatin2::re_r$'"; pos($Elatin2::re_t)=length "$`$Elatin2::re_r"; } return $Elatin2::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Elatin2::re_r=CORE::eval $Elatin2::re_r; ' x $e_modifier,          #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                  4   5  6                     7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Elatin2::re_r=%s; %s"%s$Elatin2::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                        1     2   3                                  4   5 6                          7                                                   8
                q<CORE::eval{local $Elatin2::re_n=0; while(%s =~ %s){%s local $^W=0; local $Elatin2::re_r=%s; %s%s="$`$Elatin2::re_r$'"; pos(%s)=length "$`$Elatin2::re_r"; $Elatin2::re_n++} return %s$Elatin2::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                  5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Elatin2::re_r=%s; %s%s="%s$Elatin2::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                  5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Elatin2::re_r=%s; %s%s="%s$Elatin2::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Elatin2::re_r=CORE::eval $Elatin2::re_r; ' x $e_modifier,          #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Elatin2::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Elatin2::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
=head1 SYNOPSIS
  ...
  # "no Elatin2;" not supported

=head1 ABSTRACT

This module has run-time routines for use Latin2 software automatically, you
do not have to use.
...
some bugs. Thus, if you happen to find a bug that's in Latin2 software and not
...
This Elatin2 module first appeared in ActivePerl Build 522 Built under
...
=item * Split string
  ...
  @split = Elatin2::split(/pattern/,$string,$limit);
  @split = Elatin2::split(/pattern/,$string);
  @split = Elatin2::split(/pattern/);
  @split = Elatin2::split('',$string,$limit);
  @split = Elatin2::split('',$string);
  @split = Elatin2::split('');
  @split = Elatin2::split();
  @split = Elatin2::split;
  ...
  at all, Elatin2::split returns the original string as a single substring, If it
  ...
  @chars  = Elatin2::split(//,  $word);
  @fields = Elatin2::split(/:/, $line);
  @words  = Elatin2::split(" ", $paragraph);
  @lines  = Elatin2::split(/^/, $buffer);
  ...
  print join(":" => Elatin2::split(/ */, "hi there"));
  ...
  Elatin2::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Elatin2::split(/:/, $_, 3);
  ...
  splits on whitespace just as Elatin2::split with no arguments does. Thus,
  Elatin2::split(" ") can be used to emulate awk's default behavior. In contrast,
  Elatin2::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Elatin2::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Elatin2::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Elatin2::split()) {
          $count{$word}++;
      }
  }

  The inverse of Elatin2::split is join, except that join can only join with the
...
=item * Transliteration
  ...
  $tr = Elatin2::tr($variable,$bind_operator,$searchlist,$replacementlist,$modifier);
  $tr = Elatin2::tr($variable,$bind_operator,$searchlist,$replacementlist);
  ...
  This subroutine scans a Latin-2 string character by character and replaces all
  occurrences of the characters found in $searchlist with the corresponding character
  in $replacementlist. It returns the number of characters replaced or deleted.
  If no Latin-2 string is specified via =~ operator, the $_ variable is translated.
  $modifier are:
  ...
  print Elatin2::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'

=item * Chop string

  $chop = Elatin2::chop(@list);
  $chop = Elatin2::chop();
  $chop = Elatin2::chop;

  This subroutine chops off the last character of a string variable and returns the
  character chopped. The Elatin2::chop subroutine is used primary to remove the newline
  ...
  since Elatin2::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Elatin2::chop a literal, only a variable. If you Elatin2::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Elatin2::chop(@lines);

  You can Elatin2::chop anything that is an lvalue, including an assignment:

  Elatin2::chop($cwd = `pwd`);
  Elatin2::chop($answer = <STDIN>);

  This is different from:

  $answer = Elatin2::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Elatin2::chop returns the character
  ...
  In the most general case, Elatin2::chop can be expressed using substr:

  $last_code = Elatin2::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Elatin2::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Elatin2::chop;
...
=item * Index string
  ...
  $byte_pos = Elatin2::index($string,$substr,$byte_offset);
  $byte_pos = Elatin2::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Elatin2::index($string, $lookfor, $byte_pos)) > -1) {
...
=item * Reverse index string
  ...
  $byte_pos = Elatin2::rindex($string,$substr,$byte_offset);
  $byte_pos = Elatin2::rindex($string,$substr);

  This subroutine works just like Elatin2::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Elatin2::index).
  ...
  while (($byte_pos = Latin2::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item * Lower case string
  ...
  $lc = Elatin2::lc($string);
  $lc = Elatin2::lc_;

  This subroutine returns a lowercased version of Latin-2 $string (or $_, if
  $string is omitted). This is the internal subroutine implementing the \L escape
  in double-quoted strings.

  You can use the Elatin2::fc subroutine for case-insensitive comparisons via Latin2
  software.

=item * Lower case first character of string

  $lcfirst = Elatin2::lcfirst($string);
  $lcfirst = Elatin2::lcfirst_;

  This subroutine returns a version of Latin-2 $string with the first character
  lowercased (or $_, if $string is omitted). This is the internal subroutine
  implementing the \l escape in double-quoted strings.

=item * Upper case string

  $uc = Elatin2::uc($string);
  $uc = Elatin2::uc_;

  This subroutine returns an uppercased version of Latin-2 $string (or $_, if
  $string is omitted). This is the internal subroutine implementing the \U escape
  in interpolated strings. For titlecase, use Elatin2::ucfirst instead.
...
=item * Upper case first character of string
  ...
  $ucfirst = Elatin2::ucfirst($string);
  $ucfirst = Elatin2::ucfirst_;
  ...
  $titlecase = Elatin2::ucfirst(substr($word,0,1)) . Elatin2::lc(substr($word,1));
  ...
  $do_not_use = Elatin2::ucfirst(Elatin2::lc($word));
...
=item * Fold case string
  ...
  $fc = Elatin2::fc($string);
  $fc = Elatin2::fc_;

  New to Latin2 software, this subroutine returns the full Unicode-like casefold of
  Latin-2 $string (or $_, if omitted). This is the internal subroutine implementing
  ...
  Elatin2::fc($a) eq Elatin2::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because Latin2 software has always used casefolding semantics for
...
=item * Make ignore case string
  ...
  @ignorecase = Elatin2::ignorecase(@string);

  This subroutine is internal use to m/ /i, s/ / /i, split / /i, and qr/ /i.

=item * Make capture number

  $capturenumber = Elatin2::capture($string);

  This subroutine is internal use to m/ /, s/ / /, split / /, and qr/ /.

=item * Make character

  $chr = Elatin2::chr($code);
  $chr = Elatin2::chr_;

  This subroutine returns a programmer-visible character, character represented by
  that $code in the character set. For example, Elatin2::chr(65) is "A" in either
  ASCII or Latin-2, not Unicode. For the reverse of Elatin2::chr, use Latin2::ord.

=item * Filename expansion (globbing)

  @glob = Elatin2::glob($string);
  @glob = Elatin2::glob_;
  ...
  The Elatin2::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Elatin2::glob('"*e f*"');
  @spacies = Elatin2::glob(q("*e f*"));

  If you had to get a variable through, you could do this:

  @spacies = Elatin2::glob("'*${var}e f*'");
  @spacies = Elatin2::glob(qq("*${var}e f*"));
