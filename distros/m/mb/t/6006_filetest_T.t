...
# 1
    ...
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; not CORE::eval(q{ -T "6006.0B.A"          }) xor mb::eval(q{ -T "6006.0B.ソ"          }) },
    sub { return 'SKIP' unless $MSWin32_MBCS; not CORE::eval(q{ -T "6006.1B.binary.A"   }) xor mb::eval(q{ -T "6006.1B.binary.ソ"   }) },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; not CORE::eval(q{ -T "6006.1B.text.A"     }) xor mb::eval(q{ -T "6006.1B.text.ソ"     }) },
...
# 11
    ...
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; not mb::eval(q{ (-T "6006.0B.A"         ) xor (-T "6006.0B.ソ"         ) }) },
    sub { return 'SKIP' unless $MSWin32_MBCS; not mb::eval(q{ (-T "6006.1B.binary.A"  ) xor (-T "6006.1B.binary.ソ"  ) }) },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; not mb::eval(q{ (-T "6006.1B.text.A"    ) xor (-T "6006.1B.text.ソ"    ) }) },
...
# 21
    ...
    sub { return 'SKIP' unless $MSWin32_MBCS; not CORE::eval(q{ opendir(DH1,"6006.777.A"     ); my $r = eval q{ -T DH1 }; closedir DH1; $r                                   }) },
    sub { return 'SKIP' unless $MSWin32_MBCS; not CORE::eval(q{ opendir(DH1,"6006.000.A"     ); my $r = eval q{ -T DH1 }; closedir DH1; $r                                   }) },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; not CORE::eval(q{ open(FH1,"6006.0B.A"         ); my $r = -T FH1; close FH1;    $r }) xor mb::eval(q{ -T "6006.0B.ソ"          }) },
    ...
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; not CORE::eval(q{ open(FH1,"6006.1B.text.A"    ); my $r = -T FH1; close FH1;    $r }) xor mb::eval(q{ -T "6006.1B.text.ソ"     }) },
...
# 31
    ...
    sub { return 'SKIP' unless $MSWin32_MBCS; not                                              mb::eval(q{ opendir(DH2,"6006.777.ソ"     ); -T DH2 }) },
    sub { return 'SKIP' unless $MSWin32_MBCS; not                                              mb::eval(q{ opendir(DH2,"6006.000.ソ"     ); -T DH2 }) },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; not CORE::eval(q{ -T "6006.0B.A"          }) xor mb::eval(q{ open(FH2,"6006.0B.ソ"         ); -T FH2 }) },
    sub { return 'SKIP' unless $MSWin32_MBCS; not CORE::eval(q{ -T "6006.1B.binary.A"   }) xor mb::eval(q{ open(FH2,"6006.1B.binary.ソ"  ); -T FH2 }) },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; not CORE::eval(q{ -T "6006.1B.text.A"     }) xor mb::eval(q{ open(FH2,"6006.1B.text.ソ"    ); -T FH2 }) },
...
# 41
    ...
    sub { return 'SKIP' unless $MSWin32_MBCS; not mb::eval(q{ opendir(DH1,"6006.777.A"     ); opendir(DH2,"6006.777.ソ"     ); (-T DH1) xor (-T DH2) }) },
    sub { return 'SKIP' unless $MSWin32_MBCS; not mb::eval(q{ opendir(DH1,"6006.000.A"     ); opendir(DH2,"6006.000.ソ"     ); (-T DH1) xor (-T DH2) }) },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; not mb::eval(q{ open(FH1,"6006.0B.A"         ); open(FH2,"6006.0B.ソ"         ); (-T FH1) xor (-T FH2) }) },
    sub { return 'SKIP' unless $MSWin32_MBCS; not mb::eval(q{ open(FH1,"6006.1B.binary.A"  ); open(FH2,"6006.1B.binary.ソ"  ); (-T FH1) xor (-T FH2) }) },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; not mb::eval(q{ open(FH1,"6006.1B.text.A"    ); open(FH2,"6006.1B.text.ソ"    ); (-T FH1) xor (-T FH2) }) },
