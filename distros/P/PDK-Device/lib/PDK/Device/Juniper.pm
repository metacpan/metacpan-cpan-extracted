...
sub waitfor {
  ...
  my @ret = $exp->expect(
    ...
    [
      qr/Are you sure you want to commit\?.*$/mi => sub {
        $exp->send("yes\n");
    ...
    [
      qr/Delete this inactive statement\?.*$/mi => sub {
        $exp->send("yes\n");
        $buff .= $exp->before() . $exp->match();
        exp_continue;
      }
...
sub execCommands {
  ...
  for my $cmd (@commands) {
    ...
    if ($buff =~ /(missing argument|unknown command|Invalid input detected|^Error:)/i) {
      return {success => 0, failCommand => $cmd, reason => $result . $buff};
    }
    elsif ($buff =~ /(Couldn\'t initiate connection|Unrecognized command)/i) {
      return {success => 0, failCommand => $cmd, reason => $result . $buff};
    }
    elsif ($buff =~ /(command is not valid|syntax error|incomplete command)/i) {
      return {success => 0, failCommand => $cmd, reason => $result . $buff};
    }
    elsif ($buff =~ /(permission denied|commit failed|commit check failed|invalid value)/i) {
      return {success => 0, failCommand => $cmd, reason => $result . $buff};
    }
    else {
      $result .= $buff;
