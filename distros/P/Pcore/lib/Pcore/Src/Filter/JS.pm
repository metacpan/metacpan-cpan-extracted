...
sub decompress ( $self, % ) {
    ...
    if ( 0 && $self->{file}->{path}->mime_type eq 'application/json' ) {
        ...
        my $proc = P->sys->run_proc1( qq[js-beautify $js_beautify_args --replace "$temp"], win32_create_no_window => 1 )->wait;
...
sub run_js_hint ($self) {
    ...
    my $proc = P->sys->run_proc1( qq[jshint  $js_hint_args "$in_temp" > "$out_temp"], win32_create_no_window => 1 )->wait;
