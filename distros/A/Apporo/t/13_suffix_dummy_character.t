...
my $index_path = "/tmp/p5_apporo_index_13.tsv";
my $out_index;
...
my $conf_path = "/tmp/p5_apporo_conf_13.tsv";
my $out_conf;
...
is_utf8	false
dist_threshold	0.0
index_path	/tmp/p5_apporo_index_13.tsv
dist_func	edit
entry_buf_len	1024
engine	tsubomi
result_num	10
...
__CONF__
    ...
    my $file_name = "configure file(ASCII, 1-gram, insert dummy character to tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of ngram_num:1 from the indexes using '$query' query with suffix dummy character");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, insert dummy character to tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    my @res = (
        "1	sourceinstall	20-Jul-2008 22:25",
        "0.384615	GNUsBulletins	24-Mar-2003 18:00",
        "0.307692	dismal	03-Apr-2007 18:23",
        "0.307692	smalltalk	22-Mar-2011 03:40",
        "0.307692	gnu-c-manual	05-Nov-2011 17:55",
        "0.307692	gnusound	06-Jul-2008 05:00",
        "0.307692	spell	21-Jul-2011 15:30",
        "0.307692	goptical	07-Jan-2012 18:45",
        "0.307692	libunistring	02-May-2010 17:45",
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of ngram_num:2 from the indexes using '$query' query with suffix dummy character");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 3-gram, insert dummy character to tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    my @res = (
        "1	sourceinstall	20-Jul-2008 22:25",
        "0.384615	GNUsBulletins	24-Mar-2003 18:00",
        "0.307692	aspell	04-Jul-2011 06:20",
        "0.307692	libffcall	16-Jun-2008 12:35",
        "0.307692	smalltalk	22-Mar-2011 03:40",
        "0.307692	gnusound	06-Jul-2008 05:00",
        "0.307692	gnu-c-manual	05-Nov-2011 17:55",
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of ngram_num:3 from the indexes using '$query' query with suffix dummy character");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 4-gram, insert dummy character to tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    my @res = (
        ...
        "0.307692	goptical	07-Jan-2012 18:45",
        "0.307692	spell	21-Jul-2011 15:30",
        "0.230769	metahtml	02-Aug-2003 17:08",
        "0.230769	libtool	18-Oct-2011 04:25",
        "0.230769	jacal	09-Apr-2012 23:20",
    );
    my %hash_res = ();
    for (my $i = 0; $i <= $#res; $i++) {
        $res[$i] = $res[$i];
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of ngram_num:4 from the indexes using '$query' query with suffix dummy character");
}
