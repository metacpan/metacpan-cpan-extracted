...
Defines the provider name to use by the framework plugin when no one has
been specified in the call to the C<oidc($provider)> method.
...
=head2 provider."provider".refresh_scope

String with space separators, used only for the C<refresh_token> grant-type, defining
the desired scope of rights of the requested renewed token.

May be useful, depending on your needs, to restrict scopes relative to the initial
request. For example, removing the C<openid> scope means that no new ID token will
be received, only a new access token with its new refresh token.
...
a mocked object representing an L<OIDC::Client::Identity> object.
...
=head2 provider."provider".mocked_access_token

For local use only, allows the verification of a token to be bypassed by directly
defining a mocked object representing an L<OIDC::Client::AccessToken> object.
Also used when retrieving an L<OIDC::Client::AccessToken> object from the store.
