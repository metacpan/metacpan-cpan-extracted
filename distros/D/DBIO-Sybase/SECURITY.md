This is the Security Policy for the Perl DBIO-Sybase distribution.
...
[DBIO-Sybase on CPAN](https://metacpan.org/dist/DBIO-Sybase) or the main branch of the
[DBIO-Sybase git repository](https://codeberg.org/dbio/dbio-sybase).
...
DBIO-Sybase is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO-Sybase are covered by this policy.
...
they affect DBIO-Sybase, or DBIO-Sybase can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO-Sybase, or plugins to it that are not included with the DBIO-Sybase
distribution) are not covered by this policy.

## Supported Versions of DBIO-Sybase

Only the latest release of DBIO-Sybase will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO-Sybase will run on older versions of Perl. If a
...
that are required for DBIO-Sybase to work. However, some of these
