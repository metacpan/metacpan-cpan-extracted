...
IMDB::Local::Actor - Object representation of Actor information.
...
use constant DB_TABLE => 'Actors';
use constant DB_KEY   => 'ActorID';
...
sub createNew($%)
{
    ...
    return new IMDB::Local::Actor(imdbdb=>$imdbdb, TableID=>$id);
}
...
sub findIdsBySearchableName($$)
{
    my ($imdbdb, $name)=@_;

    my $search=$imdbdb->makeSearchableTitle($name);

    my @ids;
    my $res=$imdbdb->selectall_arrayref("SELECT ".DB_KEY." from ".DB_TABLE." where SearchName='$search'");
    if ( !defined($res) ) {
	# return empty
	return(@ids);
    }
    ...
    # user can now new IMDB::Local::Actor(imdbdb=>$imdbdb, DB_KEY, $id);
    return(@ids);
}
