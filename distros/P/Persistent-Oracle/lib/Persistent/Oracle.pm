...
# RCS:      $Id: Oracle.pm,v 1.2 2000/02/10 01:43:00 winters Exp winters $
#
# A class that implements object persistence using an Oracle database.
#
...
This is a Persistent class that uses an Oracle database table to store
...
This class is part of the Persistent Oracle package which is available
from:
...
C<DBD::Oracle> documentation for valid data source names.
...
#              The returned SQL is Oracle specific.
...
sub _get_sql_for_date_to_string {
  ...
  "TO_CHAR($dt_col, 'YYYY-MM-DD HH24:MI:SS') AS $dt_col";
}
...
sub _get_column_name {
  ...
  ### Oracle returns all column names in UPPER case ###
  uc $attribute;
}
...
And you know, include the regular stuff: OS, Perl, Oracle versions,
snippet of code, etc.
