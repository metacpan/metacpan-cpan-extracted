# This test has been largely, ah, "borrowed" from
# t/catalyst-traitfor-request-rest-forbrowsers.t in Catalyst-Action-REST.
...
my $anon_class = Moose::Meta::Class->create_anon_class(
    ...
    roles        => ['Catalyst::TraitFor::Request::REST::ForBrowsers::AndPJAX'],
    cache        => 1,
)->name;

# We run the tests twice to make sure Catalyst::Request::REST::ForBrowsers is
# 100% back-compatible.
for my $class ( $anon_class ) {
    {
...

    {
        for my $with (qw( HTTP.Request XMLHttpRequest )) {
            ...
            ok(
                $req->looks_like_browser(),
                "a browser making a pjax request (X-Request-With = $with)"
            );
        }
    }
