...
perl $0 Latin-7_script.pl > Escaped_script.pl.e
END
    }

    print Latin7::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (CORE::eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Elatin7::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
        }
    ...
    my $fh = gensym();
    Elatin7::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Latin7::escape_script {
    ...
    # read Latin-7 script
    my $fh = gensym();
    Elatin7::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script\n";

    if (/^ use Elatin7(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Elatin7 '%s.0'; # 'quote' for perl5.6\n", $Latin7::VERSION); # require run-time routines version

        # use Latin7 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Latin7 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Latin7::VERSION) {
                    my @file = grep -e, map {qq{$_/Latin7.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Latin7.pm $version, but @{[__FILE__]} is version $Latin7::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Elatin7.pm $version, but \\\$Elatin7::VERSION is \$Elatin7::VERSION" if \$Elatin7::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Latin7::VERSION) {
                    die "Script $0 required Latin7.pm $version, but @{[__FILE__]} is only version $Latin7::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Elatin7::function_ord     = 'Latin7::ord'     if /\A ord \z/oxms;
                    $Elatin7::function_ord_    = 'Latin7::ord_'    if /\A ord \z/oxms;
                    $Elatin7::function_reverse = 'Latin7::reverse' if /\A reverse \z/oxms;
                    $Elatin7::function_getc    = 'Latin7::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Latin7::escape();
...
=item * You'll have to download Latin7.pm and Elatin7.pm and put it in your perl lib directory.

=item * You'll need to write "use Latin7;" at head of the script.
...
=head1 SYNOPSIS
  ...
  $ perl Latin7.pm Latin-7_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Latin-7_script.pl  --- script written in Latin-7
...
Latin7 software is "middleware" between perl interpreter and your Perl script
written in Latin-7.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Latin-7, Perl3 or later
can treat Latin-7 as binary data.

By "use Latin7;", it automatically interpret your script as Latin-7. The various
functions of perl including a regular expression can treat Latin-7 now.
...
This software is a source code filter to escape Perl script encoded by Latin-7
...
=item * Remains One Language Handling Raw Latin-7, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Latin7.pm, Elatin7.pm
  +---------------------------------------+
...
import of Elatin7.pm works well.
...
=head1 Software Composition
   ...
   Latin7.pm               --- source code filter to escape Latin-7
   Elatin7.pm              --- run-time routines for Latin7.pm
...
=head1 Calling 'Elatin7::ignorecase()' (Latin7 software provides)
...
=head1 Escaping Character Classes (Elatin7.pm provides)
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Latin7 software provides)

Latin7 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Latin7 software provides)

Insert 'Elatin7::' at head of function name. Elatin7.pm provides your script Elatin7::*
subroutines.
...
=head1 Yada Yada Operator (Latin7 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Latin7 software provides)
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Elatin7.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Latin7 software and
