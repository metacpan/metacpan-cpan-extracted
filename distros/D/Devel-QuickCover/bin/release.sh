...
name=$(cat /tmp/release_$$_make | egrep "^Created ${NAME}-.*.gz" | awk '{print $2}' | sed 's/\.tar\.gz$//g')
...
git grep -E '(Version|VERSION).*[0-9]' | egrep -v '^(quickcover.[hc]|quickcover.*.[hc]|ppport.h|Makefile.PL|bin/release.sh)' | egrep -v 'XSLoader|head' >>$errors
...
git grep -E '^[ \t]*use' | egrep '\.(pm|pl):' | egrep -v '^(bin)/' | sed 's/::/@@/g' | awk -F: '{print $2}' | sed 's/qw.*//g' | sed 's/#.*//g' | tr -d ';' | sed 's/ *$//g' | sed 's/@@/::/g' | egrep -v "use (strict|warnings|utf8|parent|${NAME})" | sed 's/^use //g' | sort -u >/tmp/release_$$_a &
...
mkfifo /tmp/release_$$_a
git grep use t | sed 's/::/@@/g' | awk -F: '{print $2}' | sed 's/qw.*//g' | sed 's/#.*//g' | tr -d ';' | sed 's/ *$//g' | sed 's/@@/::/g' | egrep -v "use (strict|warnings|utf8|parent|${NAME})|^[ \t]*use_ok" | sed 's/^use //g' | sort -u >/tmp/release_$$_a &
mkfifo /tmp/release_$$_b
