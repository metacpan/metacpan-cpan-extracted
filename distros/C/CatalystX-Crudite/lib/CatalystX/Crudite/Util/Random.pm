package CatalystX::Crudite::Util::Random;
use strict;
use warnings;
...
our %EXPORT_TAGS = (
    util => [
        qw(
          rand_words rand_phrase rand_url rand_int rand_date rand_el pick_n
          unique
          )
    ],
);
...
sub rand_date {
    DateTime::Event::Random->datetime(@_);
}
...
sub pick_n {
    ...
    my @result;
    for (1 .. $n) {
        push @result => splice @el, int(rand @el), 1;
    }
    @result;
}
...
sub unique {
    ...
    sub {
        my $value;
        do { $value = $generator->() } until !$seen{$value}++;
        $value;
    };
}
1;
