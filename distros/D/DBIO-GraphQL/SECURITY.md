This is the Security Policy for the Perl DBIO-GraphQL distribution.
...
[DBIO-GraphQL on CPAN](https://metacpan.org/dist/DBIO-GraphQL) or the main branch of the
[DBIO-GraphQL git repository](https://codeberg.org/dbio/dbio-graphql).
...
DBIO-GraphQL is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO-GraphQL are covered by this policy.
...
they affect DBIO-GraphQL, or DBIO-GraphQL can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO-GraphQL, or plugins to it that are not included with the DBIO-GraphQL
distribution) are not covered by this policy.

## Supported Versions of DBIO-GraphQL

Only the latest release of DBIO-GraphQL will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO-GraphQL will run on older versions of Perl. If a
...
that are required for DBIO-GraphQL to work. However, some of these
