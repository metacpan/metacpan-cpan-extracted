...
sub run_i64 {
    ...
    my $guard = Developer::Dashboard::Pax::GuardManager->new(epochs => \%epochs)->validate_or_deopt(
        {
    ...
    my $deopt = $guard->{status} eq 'deopt'
        ...
        : Developer::Dashboard::Pax::DeoptEngine->new->reconstruct(
            ssa_unit => {
                region_id => $region->{region_id},
                region_name => $region->{region_name},
                deopt => $region->{deopt} // {},
...
require Developer::Dashboard::Pax::StandaloneRuntime;
my $rv = Developer::Dashboard::Pax::StandaloneRuntime->run(entrypoint => $entry, argv => []);
...
Developer::Dashboard::Pax::StandaloneDispatch - standalone command and region dispatch helper

=head1 SYNOPSIS

  use Developer::Dashboard::Pax::StandaloneDispatch;

  my $obj = Developer::Dashboard::Pax::StandaloneDispatch->new(...);
...
Example 1:
  ...
  perl -Ilib -MDeveloper::Dashboard::Pax::StandaloneDispatch -e 1
