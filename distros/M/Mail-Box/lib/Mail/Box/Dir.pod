...
For rare folder types, the C<$directory> name may differ from the folder
name.
...
Extends L<"Attributes" in Mail::Box|Mail::Box/"Attributes">.
...
C<$folderdir> to replace a leading “C<< = >>”.
...
C<$directory> and represent a message.  The filenames are returned as
relative path.
...
The folder C<$directory> does already exist and is write protected, which may
...
=item Warning: folder directory $dir is write-protected
...
=item Error: no directory $dir for folder of $type.
