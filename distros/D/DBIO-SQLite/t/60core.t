...
is($new_again->ID, 'DBIO::Test::Artist|artist|artistid=4', 'unique object id generated correctly');
...
SKIP: {
    ...
    isa_ok($tdata{'cd'}, 'DBIO::Test::CD', 'cd is CD object');
...
# test columns_info
{
  ...
  cmp_deeply (
    $schema->source('Artist')->columns_info,
    {
      ...
      charfield => {
        data_type => re(qr/^char$/i),
        default_value => undef,
        is_nullable => 1,
        size => 10
      },
      name => {
        data_type => re(qr/^varchar$/i),
        default_value => undef,
        is_nullable => 1,
        is_numeric => 0,
        size => 100
...
# make sure that obsolete handle-based source tracking continues to work for the time being
{
  ...
  isa_ok( $rs_result, 'DBIO::Test::Artist' );
  is_deeply (
  ...
  my $row = DBIO::Test::Artist->new({ -source_handle => $handle });
  is_deeply(
    { $row->get_columns },
    {},
    'No columns yet'
