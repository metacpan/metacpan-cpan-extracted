    ...
    $cgi->set_error_handler(sub {
      ...
      unless ($cgi->headers_rendered) {
        if ($cgi->response_status_code == 413) {
          ...
          $cgi->set_response_status(500)->render(json => {error => 'Internal server error'});
        }
      }
    });
...
error handlers may choose to render some response if L</"headers_rendered"> is
...
Returns true if response headers have been rendered, such as by L</"render"> or
the first call to L</"render_chunk">.
...
Renders response headers (setting L</"headers_rendered">) and then fixed-length
response data of a type indicated by the first parameter, if any. A
C<Content-Length> header will be set to the length of the encoded response
content, and further calls to C<render> or L</"render_chunk"> will throw an
exception.
...
rendered, and will set a response status of 302 unless a different status has
been set with L</"set_response_status">. It will set a C<Content-Length> of 0,
and it will not set a C<Content-Type> response header.
...
Renders response headers (setting L</"headers_rendered">) the first time it is
called, and then chunked response data of a type indicated by the first
parameter, if any. No C<Content-Length> header will be set, and C<render_chunk>
may be called additional times with more response data.
