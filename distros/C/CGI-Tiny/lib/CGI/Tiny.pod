...
false. L</"response_status_code"> may be set by some errors and can be
inspected if headers have not been rendered yet.
...
=head3 set_response_fixed_length

  $cgi = $cgi->set_response_fixed_length(1);

If set to a true value before rendering response headers, a C<Content-Length>
header will be set to the size of the encoded content on the first call to
L</"render">, unless it streams the response data from a C<handle> and thus
can't predetermine the content size. L</"render"> cannot be used to provide
additional response data if a C<Content-Length> header was rendered.
...
or C<handle> response body with L</"render">. Defaults to the value of the
C<CGI_TINY_RESPONSE_BODY_BUFFER> environment variable or 131072 (128 KiB). A
...
L</"render">. C<undef> will remove the override. No effect after response
headers have been rendered.
...
=head3 render
  ...
  $cgi = $cgi->render(handle   => $filehandle);
...
L</"headers_rendered">. It may be called additional times with more response
data, unless a C<Content-Length> header was generated due to
L</"set_response_fixed_length">.
...
the first call to C<render>, or to C<application/octet-stream> if there is no
...
L</"set_response_status">. It will set a C<Content-Length> of 0, and it will
not set a C<Content-Type> response header. If response headers have already
been rendered a warning will be emitted.
...
automatically, and L</"render"> provides methods to encode response data from
