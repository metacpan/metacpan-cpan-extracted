package Slackware::Slackget::Network::Backend::Base64;
...
sub backend_decode {
	my $self = shift;
	my $Base64_msg = shift;
	print "[Slackware::Slackget::Network::Backend::Base64] call backend_decode($Base64_msg).\n" if($ENV{SG_DAEMON_DEBUG}) ;
	
	my $data = MIME::Base64::decode( $Base64_msg->data );
	print "[Slackware::Slackget::Network::Backend::Base64] decoded data are :\n".$data."\n" if($ENV{SG_DAEMON_DEBUG}) ;
...
sub backend_encode {
	my $self = shift;
	my $message = shift ;
	print "[Slackware::Slackget::Network::Backend::Base64] call backend_encode($message).\n" if($ENV{SG_DAEMON_DEBUG}) ;
	my $Base64 = MIME::Base64::encode($message->data());
	print "[Slackware::Slackget::Network::Backend::Base64] encoded Base64:\n$Base64\n" if($ENV{SG_DAEMON_DEBUG});
