...
sub test_encode : Test(3) {
    ...
    my $val = Prty::Url->encode('-*._1ABCabc');
...
sub test_decode : Test(3) {
    ...
    my $val = Prty::Url->decode('-*._1ABCabc');
    $self->is($val,'-*._1ABCabc','decode: Nicht-kodierte Zeichen (ASCII)');

    $val = Prty::Url->decode('%21%22%A7%24%25%26%2F%28%29%3D%3F');
    $self->is($val,'!"§$%&/()=?','decode: Einige kodierte Zeichen (ASCII)');

    $val = Prty::Url->decode('%C4%D6%DC%E4%F6%FC%DF');
...
sub test_queryEncode : Test(9) {
    ...
    my $val = Prty::Url->queryEncode(a=>1,b=>2,c=>3);
    $self->is($val,'a=1;b=2;c=3');

    # Query-String mit Fragezeichen

    $val = Prty::Url->queryEncode('?',a=>1,b=>2,c=>3);
    $self->is($val,'?a=1;b=2;c=3');

    # Query-String ohne leere Werte

    $val = Prty::Url->queryEncode('?',a=>1,b=>2,c=>'',d=>4,e=>undef);
    $self->is($val,'?a=1;b=2;d=4');

    # Query-String ohne leere Werte

    $val = Prty::Url->queryEncode(a=>1,b=>[2,'',3,undef],c=>'',d=>4,e=>undef);
    $self->is($val,'a=1;b=2;b=3;d=4');

    # Query-String mit leeren Werten

    $val = Prty::Url->queryEncode('?',-null=>1,a=>1,b=>2,c=>'',d=>4,e=>undef);
    $self->is($val,'?a=1;b=2;c=;d=4;e=');

    # Query-String mit leeren Werten

    $val = Prty::Url->queryEncode(-null=>1,a=>1,b=>[2,'',3,undef],c=>'',
        d=>4,e=>undef);
    $self->is($val,'a=1;b=2;b=;b=3;b=;c=;d=4;e=');

    # Umlaute

    $val = Prty::Url->queryEncode(a=>1,'für'=>'Möller');
    $self->is($val,'a=1;f%FCr=M%F6ller');

    # Array-Referenz als Wert

    $val = Prty::Url->queryEncode(a=>1,b=>[1,2,3],c=>3);
    $self->is($val,'a=1;b=1;b=2;b=3;c=3');

    # & als Trennzeichen

    $val = Prty::Url->queryEncode(-separator=>'&',a=>1,b=>[1,2,3],c=>3);
    $self->is($val,'a=1&b=1&b=2&b=3&c=3');
}
...
sub test_queryDecode : Test(3) {
    ...
    my $arr = Prty::Url->queryDecode('');
    $self->isDeeply($arr,[]);

    $arr = Prty::Url->queryDecode('a=1&b=2');
