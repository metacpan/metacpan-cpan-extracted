package Pcore::Lib::URI;

use Pcore -class, -const;
use Pcore::Lib::Net qw[get_free_port];
use Pcore::Lib::Scalar qw[is_path is_uri is_ref];
use Pcore::Lib::Data qw[:URI to_b64];
...
around new => sub ( $orig, $self, $uri = undef, %args ) {
    ...
    if ( $scheme ne $EMPTY ) {
        ...
        $class->{$scheme} = eval { P->class->load( $scheme, ns => 'Pcore::Lib::URI' ) } if !exists $class->{$scheme};
