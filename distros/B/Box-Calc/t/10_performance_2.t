...
$t = [gettimeofday()];
my $plane = $calc->add_item(2*8000, {x => 2.5, y => 2.25, z => 0.5, name => 'Plane', weight => 1});
my $car = $calc->add_item(2*1150, {x => 2, y => 0.25, z => 0.125, name => 'Car', weight => 0.1});
my $bills = $calc->add_item(2*662, {x => 3, y => 2, z => 0.125, name => 'Bills', weight => 0.1});
my $die = $calc->add_item(2*558, {x => 0.5, y => 0.5, z => 0.5, name => 'Die', weight => 0.1});
note "Time to add 20,000+ parts: ".tv_interval($t);
