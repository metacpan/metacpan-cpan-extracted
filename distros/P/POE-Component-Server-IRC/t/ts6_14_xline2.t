...
sub groucho_input {
  ...
  if ( $cmd eq 'XLINE' ) {
    ...
    is( $params->[2], '*NastyBot*', 'Mask is correct' );
...
sub client_disconnected {
  ...
  $heap->{ircd}->yield('daemon_cmd_unxline','OperServ','*NastyBot*','ON','*');
  return;
}
...
sub ircd_daemon_eob {
  ...
  if ( $heap->{eob} >= 3 ) {
    #$poe_kernel->yield('_launch_client');
    $heap->{ircd}->yield( 'daemon_cmd_xline', 'OperServ', '10', '*NastyBot*', 'ON', '*', 'Banhammer' );
  }
  return;
}
