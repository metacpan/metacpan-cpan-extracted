...
We have tested tscore.pm, a component of Ngram Statistics Package version
0.59. File tscore.pm implements the T-score. Note that tscore.pm is not a  
program by itself, but is a statistical library package that is plugged  
into statistic.pl. We test here the features of tscore.pm by running 
statistic.pl.

Following is a description of the aspects of tscore.pm that we have
tested. We provide the scripts and files used for testing so that
later versions of tscore.pm can be tested for backward compatibility.
...
Phase 1: Testing tscore.pm's response to erroneous conditions. 
Phase 2: Testing tscore.pm's behaviour under normal conditions. 


2.1. Phase 1: Testing tscore.pm's response to erroneous conditions:
----------------------------------------------------------------------
...
tscore.pm is meant only for bigrams. This test checks if tscore.pm throws
an error when provided with larger n-grams.

2.1.2. Subtest 2: 
-----------------

tscore.pm requires three frequency values: the frequency of the bigram,
...
not provided, tscore.pm should throw an error. This test checks for this
error. 
...
checks if tscore.pm does indeed throw an error when passed a total
bigrams value that is less than or equal to zero.

2.1.4. Subtest 4: 
-----------------

The numbers passed to tscore.pm should be "valid" in that they should
represent a possible two-by-two table. If this is not the case,
various warnings are thrown by tscore.pm. This test checks for these
warnings. 
...
2.2. Phase 2: Testing tscore.pm's behaviour under normal conditions:
------------------------------------------------------------------
...
First, it checks if tscore.pm works when the frequencies are given in a
...
Second it checks the actual calculations of tscore.pm. The output target
...
Statistical library package tscore.pm has been tested for erroneous
