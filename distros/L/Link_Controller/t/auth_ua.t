...
This is based on and almost exactly the test run script "local/http.t"
from libwww-perl except that it's only running our AuthUA.

Basically, our AuthUA should exactly match a normal user agent in
normal use.

Changed lines have the prepended with "#mikedlr"
...
#    open(DAEMON, "$perl local/http.t daemon |") or die "Can't exec daemon: $!";
    open(DAEMON, "$perl t/auth_ua.t daemon |") or die "Can't exec daemon: $!";
}
...
#mikedlr: require LWP::UserAgent;
require LWP::Auth_UA;
require HTTP::Request;
#mikedlr: $ua = new LWP::UserAgent;
$ua = new LWP::Auth_UA;
$ua->agent("Mozilla/0.01 " . $ua->agent);
$ua->from('link-controller@scotclimb.org.uk');
...
#mikedlr: print "not " unless /^From:\s*gisle\@aas\.no$/m
print "not " unless /^From:\s*link-controller\@scotclimb\.org\.uk$/m
                and /^Host:/m
                and @accept == 3
...
{
#mikedlr:   package MyUA; @ISA=qw(LWP::UserAgent);
   package MyUA; @ISA=qw(LWP::Auth_UA);
...
#mikedlr: # Let's try to set credentials for this realm
#mikedlr: $ua->credentials($req->url->host_port, "libwww-perl", "ok 12", "xyzzy");
#mikedlr: $res = $ua->request($req);
#mikedlr: print "not " unless $res->is_success;
print "ok 14\n";
#mikedlr: 
#mikedlr: # Then illegal credentials
#mikedlr: $ua->credentials($req->url->host_port, "libwww-perl", "user", "passwd");
#mikedlr: $res = $ua->request($req);
#mikedlr: print "not " unless $res->code == 401;
print "ok 15\n";
