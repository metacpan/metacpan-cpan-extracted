...
Sah::Schema::cpan::pause_id - PAUSE author ID, e.g. 'PERLANCAR'

=head1 VERSION

This document describes version 0.014 of Sah::Schema::cpan::pause_id (from Perl distribution Sah-Schemas-CPAN), released on 2022-09-25.
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("cpan::pause_id", {return_type=>'str_errmsg'});
 ...
 $data = "";
 my $errmsg = $validator->($data); # => "Must match regex pattern qr(\\A(?^:[A-Z][A-Z0-9]{1,8})\\z)"

Often a schema has coercion rule or default value, so after validation the
validated value is different. To return the validated (set-as-default, coerced,
prefiltered) value:

 my $validator = gen_validator("cpan::pause_id", {return_type=>'str_errmsg+val'});
 my $res = $validator->($data); # [$errmsg, $validated_val]
 
 # a sample valid data
 $data = "perlancar";
 my $res = $validator->($data); # => ["","PERLANCAR"]
 
 # a sample invalid data
 $data = "";
 my $res = $validator->($data); # => ["Must match regex pattern qr(\\A(?^:[A-Z][A-Z0-9]{1,8})\\z)",""]
