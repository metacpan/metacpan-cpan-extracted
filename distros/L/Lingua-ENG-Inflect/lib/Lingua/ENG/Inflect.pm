...
$rcfile = $INC{'Lingua//ENG/Inflect.pm'} || '';
...
return if $count=~/^($PL_count_one)$/io;
...
$word =~ /^($PL_v_special_s)$/      and return;
$word =~ /\s/               and return;
...
=head1 SYNOPSIS
 ...
 use Lingua::ENG::Inflect qw ( PL PL_N PL_V PL_ADJ NO NUM
                  PL_eq PL_N_eq PL_V_eq PL_ADJ_eq
                  A AN
                  PART_PRES
                  ORD NUMWORDS
...
a preceding count, Lingua::ENG::Inflect provides an exportable subroutine
...
Lingua::ENG::Inflect therefore provides an exportable subroutine
...
Lingua::ENG::Inflect also provides a solution to the problem
...
C<Lingua::ENG::Inflect> also provides the C<PART_PRES> subroutine,
...
Lingua::ENG::Inflect provides two exportable subroutines (C<A($;$)> and
...
a string is returned. Hence:
    ...
    use Lingua::ENG::Inflect qw( NUMWORDS );
...
When it is imported, Lingua::ENG::Inflect executes (as Perl code)
the contents of any file named F<.inflectrc> which it finds in the
in the directory where F<Lingua/ENG/Inflect.pm> is installed,
or in the current home directory (C<$ENV{HOME}>), or in both.
Note that the code is executed within the Lingua::ENG::Inflect
namespace.
...
C<Lingua::ENG::Inflect::NUMWORDS()> can C<die> with the following messages:
...
Therefore, Lingua::ENG::Text contents itself with the following compromise: If
...
Lingua::ENG::Inflect handles such words in two ways:
