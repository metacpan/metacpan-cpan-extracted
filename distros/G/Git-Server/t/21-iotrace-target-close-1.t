...
# Test target close STDOUT (fd 1) behavior (Run 6 seconds)
my $test_prog = q{
    $|=1;                                    #LineA
    sub p{sleep 1}                           #LineB
    ...
    p;r;                                     #LineD
    p;print "OUT-ONE:$_\n";                  #LineE
    p;close STDOUT;                          #LineF
    p;print "OUT-CLOSED:$_\n";               #LineG
    p;warn "ERR-TWO:$_\n";                   #LineH
    p;exit 0;                                #LineI
};
...
SKIP: for my $try (@filters) {
    ...
    # Test #LineF: close STDOUT
    # Test #LineG: print OUT
    ...
    $line = <$out_fh>;
    ok(!$line, t." $prog: back2 eof out: $! ".($line || "(eof)"));

    # Test #LineH: p;TWO
    alarm 5;
    ok(!canread($err_fh), t." $prog: MID: STDERR is still empty: $!");
    alarm 5;
    ok(canread($err_fh,2.2),t." $prog: MID: STDERR ready: $!");
    alarm 5;
    chomp($line = <$err_fh>);
    ok($line, t." $prog: back3: $line");
    like($line, qr{TWO.*uno}, t." $prog: END: STDERR done");
    ...
    alarm 5;
    # STDERR should implicitly close in about 1 seconds when prog exits
    ...
    my $explicit_close = "";
    while (<$tmp>) {
        $explicit_close .= " [$1]" if /(.*\bclose\(2\).*)/;
    }
    ok(!$explicit_close, t." $prog: END: STDERR implicitly closed:$explicit_close");
}
