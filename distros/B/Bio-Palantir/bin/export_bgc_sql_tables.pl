...
# ABSTRACT: Exports SQL tables of BGC results (antiSMASH or Palantir version)
...
for my $infile ($ARGV_file_table ? @infiles : @ARGV_infiles) {
        ...
        if ($gca) {
            ...
            push @lineage_elmts,
                ...
                (map { $tax->get_term_at_level($taxid, $_) }
                    qw(order family genus species subspecies)),
            ;
    ...
    fill_bgc_tables($infile, 'antismash', $lineage_id, \%table_for);
    fill_bgc_tables($infile, 'palantir' , $lineage_id, \%table_for);
...
sub fill_bgc_tables {
    ...
    my ($report_file, $annotation, $lid, $table_ref) = @_;

    my ($suffix, @clusters);
    if ($annotation eq 'antismash') {

        my $parser = Parser->new( file => $report_file );
        my $root = $parser->root;
        push @clusters, $root->all_clusters;
    ...
    for my $cluster (@clusters) {
        ...
        next CLUSTER 
            if $cluster->type eq 'other';

        my @cluster_elmts = (
            $cluster->uui, $cluster->rank, $cluster->type, 
...
sub get_function {
    my $seq = shift;
    my $hmmdb = '/media/vol2/home/lmeunier/tools/bio-palantir/'
        . 'data/generic_domains.hmm';

    my $tbout = do_hmmscan($seq, $hmmdb);

    # parsing of domtblout hmmscan report 
    my $hmmscan_report = DomTable->new( file => $tbout->filename );
    ...
    HIT:
    while (my $hit = $hmmscan_report->next_hit) {   #TODO the method risks to be changed /BUG!!!\ 
        ...
        $hit_for{'hit' . $i2}{score}  = $hit->dom_score;
        $hit_for{'hit' . $i2}{evalue} = $hit->i_evalue;     # i-value = independent evalue ('evalue' return cumulative evalues for the sequence, and c-evalue may be deleted because potentially e-value)
    }
...
sub do_hmmscan {
    ...
    my $template = 'tempfile_XXXXXXXXXXXXXXXX';
    my $query = File::Temp->new($template, suffix => '.faa', unlock => 1); 
    print $query '>query' . "\n" . $seq;

    my $pgm = 'hmmscan';

    my $ARGV_cpu = 1;
    my $tbout = File::Temp->new($template, suffix => '_domtblout.tsv'); #, unlock => $ARGV_mode_debug ? 0 : 1);  
    my $opt = ' --domtblout ' . $tbout . ' --cpu ' . $ARGV_cpu;
    my $log = File::Temp->new($template, suffix => '_hmmscan.log', unlock => 1);
...
# resistance gene search

# $hmmdb = '/media/vol2/home/lmeunier/data/resistance_genes/Resfams.hmm'; # use full version?
# $query = $proteome; 
# $pgm = 'hmmscan';
# 
# $tbout 
#   = File::Temp->new(
#       $template, 
#       suffix => '_domtblout.tsv', 
#       unlock => $ARGV_mode_debug ? 0 : 1
#     )
# ;  
# $opt = '--domtblout ' . $tbout . ' --cpu ' . $ARGV_cpu;
# 
# $cmd = "$pgm $opt $hmmdb $query"; 
# system $cmd;
# 
# # parse domtblout hmmer report 
# $report = DomTable->new( file => $tbout->filename );
# 
# my %rg_for; # resistance gene
# $i;
# 
# HIT:
...
#         next HIT if $hit->evalue > $ARGV{'--evalue-threshold'};
#     }
#     
#     $i++;
# 
#     $rg_for{ $hit->query_name }{ 'hit_' . $i } = {
#         map { $_ => $hit->$_ } 
#         qw(target_name ali_from ali_to hmm_from hmm_to tlen score evalue )
#     };
# 
# }
# 
...
export_bgc_sql_tables.pl - This tool creates a sql database for the AntiSMASH
results and allow to make queries on it.
...
Path to biosynML.xml or regions.js  files, including at least the directory
containing this file. This option can takes multiple values.

=item --file-table [=] <tsv_file>

TSV (Tabulation separated values) format file to give non ambiguously the path
...
Path to an id mapper file to retrieve the assemblies accession numbers. The
file should be in tabular format with accession number in the secund column.
...
with the basename of your xml file.  For example, if you use my_org.xml, name
...
=for Euclid: name.default: 'bgc_db'
