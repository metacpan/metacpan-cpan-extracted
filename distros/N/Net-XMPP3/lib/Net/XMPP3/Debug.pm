...
sub Init
{
    ...
    else
    {
        ...
        if (exists($args{file}))
        {
            ...
            {
                $self->{HANDLE} = $Net::XMPP3::Debug::HANDLES{$args{file}};
                $self->{HANDLE}->autoflush(1);
            }
            else
            {
                if (-e $args{file})
                {
                    if (-w $args{file})
                    {
                        ...
                        {
                            $self->{HANDLE}->autoflush(1);
                            $Net::XMPP3::Debug::HANDLES{$args{file}} = $self->{HANDLE};
                        }
                        else
                        {
    ...
    {
        $Net::XMPP3::Debug::DEFAULT = $self->{HANDLE};
        $Net::XMPP3::Debug::DEFAULTLEVEL = $self->{LEVEL};
        $Net::XMPP3::Debug::DEFAULTTIME = $self->{TIME};
    }
...
sub Log
{
    ...
    my $fh = $self->{HANDLE};
    $fh = $Net::XMPP3::Debug::DEFAULT if exists($self->{USEDEFAULT});

    my $string = "";

    my $testTime = $self->{TIME};
    $testTime = $Net::XMPP3::Debug::DEFAULTTIME if exists($self->{USEDEFAULT});

    $string .= "[".&Net::XMPP3::GetTimeStamp("local",time,"short")."] "
        if ($testTime == 1);
    $string .= $self->{HEADER}.": ";
...
sub AUTOLOAD
{
    ...
    return 0 if ($level > (exists($self->{USEDEFAULT}) ? $Net::XMPP3::Debug::DEFAULTLEVEL : $self->{LEVEL}));
    $self->Log(@_);
}
