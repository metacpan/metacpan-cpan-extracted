...
# CLASSES    : LaTeX::BibTeX::Entry
# RELATIONS  : base class for LaTeX::BibTeX::StructuredEntry, and 
...
use UNIVERSAL 'isa';
use Carp;
use LaTeX::BibTeX qw(:metatypes :nodetypes);

=head1 NAME

LaTeX::BibTeX::Entry - read and parse BibTeX files

=head1 SYNOPSIS

   use LaTeX::BibTeX;            # do not use LaTeX::BibTeX::Entry alone!

   # ...assuming that $bibfile and $newbib are both objects of class
   # LaTeX::BibTeX::File, opened for reading and writing (respectively):
   ...
   $entry = new LaTeX::BibTeX::Entry $filename, $filehandle;
...
C<LaTeX::BibTeX::Entry> does all the real work of reading and parsing
...
BibTeX entries can be read either from C<LaTeX::BibTeX::File> objects (using
...
the extra functionality provided by the C<LaTeX::BibTeX::File> class.
...
database structure.  See L<LaTeX::BibTeX::Structure> for details on database
structures.
...
C<LaTeX::BibTeX::File> object, a filehandle or a string.  (A filehandle or
...
Creates a new C<LaTeX::BibTeX::Entry> object.  If the SOURCE parameter is
supplied, it must be one of the following: a C<LaTeX::BibTeX::File> (or
descendant class) object, a filename/filehandle pair, or a string.  Calls
C<read> to read from a C<LaTeX::BibTeX::File> object, C<parse> to read from
...
C<FileHandle> objects, since C<LaTeX::BibTeX> requires Perl 5.004, which
always includes the C<IO> modules.)  You can I<not> pass in the name of
a filehandle as a string, though, because C<LaTeX::BibTeX::Entry>
...
C<$filename> (error handling ignored):
   ...
   $entry = new LaTeX::BibTeX::Entry ($filename, \*BIBFILE);

   # newfangled IO::File thingy
   $file = new IO::File $filename;
   $entry = new LaTeX::BibTeX::Entry ($filename, $file);

But using a C<LaTeX::BibTeX::File> object is simpler and preferred:

   $file = new LaTeX::BibTeX::File $filename;
   $entry = new LaTeX::BibTeX::Entry $file;
...
sub new
{
   ...
   if (@source)
   {
      ...
      if (@source == 1 && isa ($source[0], 'LaTeX::BibTeX::File'))
      { 
         my $file = $source[0];
         ...
         if (my $structure = $file->structure)
         {
          ...
          { croak "new: source argument must be either a LaTeX::BibTeX::File " .
...
C<LaTeX::BibTeX::File> object (or descendant).  The next entry will be read
...
sub read
{
   ...
   croak "`source' argument must be ref to open LaTeX::BibTeX::File " .
         "(or descendant) object"
      unless (isa ($source, 'LaTeX::BibTeX::File'));
...
Perl hash, which coincidentally is the C<LaTeX::BibTeX::Entry> object
...
sub _preserve
{
   ...
   $preserve = $self->{'file'}->preserve_values
      if ! defined $preserve && 
         defined $self->{'file'} &&
         isa ($self->{'file'}, 'LaTeX::BibTeX::File');
   require LaTeX::BibTeX::Value if $preserve;
   $preserve;
}
...
C<LaTeX::BibTeX> exports four constants for these metatypes: C<BTE_COMMENT>,
...
C<LaTeX::BibTeX::Value> objects.  Each C<Value> object is a list of
C<LaTeX::BibTeX::SimpleValue> objects, which in turn consists of a simple
...
a C<LaTeX::BibTeX::Entry> object, but these are actually just front ends
to more general functions in C<LaTeX::BibTeX>.  (More general in that you
...
L<LaTeX::BibTeX> (the C<split_list> and C<split_name> functions in
...
Returns a list of C<LaTeX::BibTeX::Name> objects, each of which represents
one name.  Use the C<part> method to query these objects; see
L<LaTeX::BibTeX::Name> for details on the interface to name objects (and on
...
has been parsed into a C<LaTeX::BibTeX::Entry> object C<$entry>, then

   @names = $entry->names ('author');

will put a list of C<LaTeX::BibTeX::Name> objects in C<@names>.  These can
be queried individually as described in L<LaTeX::BibTeX::Name>; for instance,
...
#      unless defined $filename;         # undefined $filename
   LaTeX::BibTeX::split_list ($self->{'values'}{$field}, $delim,
                             $filename, $line, $desc);
}
...
#      unless defined $filename;         # undefined $filename
   for $i (0 .. $#names)
   {
      $names[$i] = new LaTeX::BibTeX::Name ($names[$i], $filename, $line, $i);
   }
   @names;
}
...
are all optionally exported from C<LaTeX::BibTeX>).
...
VALUE can be either a simple string or a C<LaTeX::BibTeX::Value> object;
...
should be a C<LaTeX::BibTeX::File> object, opened for output).  Currently
...
sub print_s
{
   ...
   sub value_to_string
   {
      ...
      else                              # a LaTeX::BibTeX::Value object
      {
         confess "value is a reference, but not to LaTeX::BibTeX::Value object"
            unless isa ($value, 'LaTeX::BibTeX::Value');
...
works if the file is represented by a C<LaTeX::BibTeX::File> object---if
...
L<LaTeX::BibTeX>, L<LaTeX::BibTeX::File>, L<LaTeX::BibTeX::Structure>
