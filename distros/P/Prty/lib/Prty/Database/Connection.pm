package Prty::Database::Connection;
...
sub new {
    ...
    $sqlClass = 'Prty::Sql' if !defined $sqlClass;
    ...
    my $udlObj = ref $udl? $udl: Prty::Udl->new($udl);
    my $apiObj = Prty::Database::Api->connect($udlObj,
        -utf8=>$utf8,
        -handle=>$handle,
    );
    ...
    $self = $class->SUPER::new(
        ...
        sqlClass=>$sqlClass,
        sqlObj=>$sqlObj,
        titleListCache=>Prty::Hash->new->unlockKeys,
        nullRowCache=>Prty::Hash->new->unlockKeys,
        utf8=>$utf8,
        schema=>undef,
        log=>$log,
        logfile=>$logfile,
...
=head3 newFromSbit() - Instantiiere Prty::Database::Connection-Datenbankobjekt aus Sbit-Datenbankobjekt
...
sub defaultRowClass {
    return $_[1]? 'Prty::Database::Row::Array': 'Prty::Database::Row::Object';
}
...
sub exportTable {
    ...
    $fh->print(Prty::Array->dump($titleA,$colSep),"\n");
    while (my $rowA = $cur->fetch) {
        $fh->print(Prty::Array->dump($rowA,$colSep),"\n");
    }
    $fh->close;
    $cur->close;
...
sub importTable {
    ...
    my $line = scalar <$fh>;
    chomp $line;
    my $titleA = Prty::Array->restore($line,$colSep);
    ...
    while (my $line = <$fh>) {
        chomp $line;
        $cur->bind(Prty::Array->restore($line,$colSep));
    }
    $fh->close;
    $cur->close;
...
Row-Objekte. Als Default-Rowklasse verwende Prty::Database::Row::Array
statt Prty::Database::Row::Object (der Parameter -rowClass überschreibt
diesen Default).
...
Bei Raw-Datensätzen ist Prty::Database::ResultSet::Array der Default,
ansonsten Prty::Database::ResultSet::Object.
...
sub sql {
    ...
    return Prty::Database::Cursor->new(
        apiCur=>$apiCur,
        bindVars=>$bindVars,
...
# FIXME: Methode in Prty::Database::Row::Object::Table auf diese zurückführen?
...
=item -rowClass => $class (Default: 'Prty::Database::Row::Object')
...
Dies ist ein Objekt der Klasse Prty::Sql, das beim
Verbindungsaufbau passend zum DBMS instantiiert wurde.

Alle SQL-Generierungsmethoden der Klasse Prty::Sql können
...
Die meisten Statements der Klasse Prty::Sql können auch
...
und Ausführung vorzuziehen, da die Prty::Database::Connection-Methoden bei der
Ausführung teilweise DBMS-abhängige Sonderbehandlungen vornehmen.

=head2 Erweiterte Statement-Generierung

Anstelle der Default Sql-Klasse Prty::Sql kann beim Verbindungsaufbau
