package Slackware::Slackget::Network::Backend::TEST;
...
Slackware::Slackget::Network::Backend::TEST - TEST backend for slack-get network protocol
...
sub backend_decode {
	my $self = shift;
	my $test_msg = shift;
	print "[Slackware::Slackget::Network::Backend::TEST] call backend_decode($test_msg).\n" if($ENV{SG_DAEMON_DEBUG}) ;
	my $data = $test_msg->data ;
	print "[Slackware::Slackget::Network::Backend::TEST] got data:\n$data.\n" if($ENV{SG_DAEMON_DEBUG}) ;
	$data =~ s/\?\?TEST\?\?//;
	$data =~ s/!!TEST!!//;
	print "[Slackware::Slackget::Network::Backend::TEST] return data:\n$data.\n" if($ENV{SG_DAEMON_DEBUG}) ;
	return Slackware::Slackget::Network::Message->new(
		action => $test_msg->{Enveloppe}->{Action}->{content},
		action_id => $test_msg->{Enveloppe}->{Action}->{id},
		raw_data => $data,
	);
}
...
sub backend_encode {
	my $self = shift;
	my $message = shift ;
	print "[Slackware::Slackget::Network::Backend::TEST] call backend_encode($message).\n" if($ENV{SG_DAEMON_DEBUG}) ;
	$message->data(  "??TEST??\n".$message->data()."\n!!TEST!!" );
	return $message;
}
...
1; # End of Slackware::Slackget::Network::Backend::TEST