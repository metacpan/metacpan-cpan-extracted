...
######################################################################
#
# Latin8 - Source code filter to escape Latin-8 script
#
# http://search.cpan.org/dist/Char-Latin8/
#
...
perl $0 Latin-8_script.pl > Escaped_script.pl.e
END
    }

    print Latin8::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Elatin8::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
    ...
    my $fh = gensym();
    Elatin8::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Latin8::escape_script {
    ...
    # read Latin-8 script
    my $fh = gensym();
    Elatin8::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Elatin8(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Elatin8 '%s.0'; # 'quote' for perl5.6\n", $Latin8::VERSION); # require run-time routines version

        # use Latin8 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Latin8 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Latin8::VERSION) {
                    my @file = grep -e, map {qq{$_/Latin8.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Latin8.pm $version, but @{[__FILE__]} is version $Latin8::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Elatin8.pm $version, but \\\$Elatin8::VERSION is \$Elatin8::VERSION" if \$Elatin8::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Latin8::VERSION) {
                    die "Script $0 required Latin8.pm $version, but @{[__FILE__]} is only version $Latin8::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Elatin8::function_ord     = 'Latin8::ord'     if /\A ord \z/oxms;
                    $Elatin8::function_ord_    = 'Latin8::ord_'    if /\A ord \z/oxms;
                    $Elatin8::function_reverse = 'Latin8::reverse' if /\A reverse \z/oxms;
                    $Elatin8::function_getc    = 'Latin8::getc'    if /\A getc \z/oxms;

                    # avoid: used only once: possible typo at ...
                    $Elatin8::function_ord     = $Elatin8::function_ord;
                    $Elatin8::function_ord_    = $Elatin8::function_ord_;
                    $Elatin8::function_reverse = $Elatin8::function_reverse;
                    $Elatin8::function_getc    = $Elatin8::function_getc;
                }
            }
        }
    }

    $e_script .= Latin8::escape();
...
=item * You'll have to download Latin8.pm and Elatin8.pm and put it in your perl lib directory.

=item * You'll need to write "use Latin8;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Latin8;
  use Latin8 ver.sion;             --- require minimum version
  use Latin8 ver.sion.0;           --- expects version (match or die)

  # "no Latin8;" not supported

  or

  $ perl Latin8.pm Latin-8_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Latin-8_script.pl  --- script written in Latin-8
...
Latin8 software is "middleware" between perl interpreter and your Perl script
written in Latin-8.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Latin-8, Perl3 or later
can treat Latin-8 as binary data.

By "use Latin8;", it automatically interpret your script as Latin-8. The various
functions of perl including a regular expression can treat Latin-8 now.
...
This software is a source code filter to escape Perl script encoded by Latin-8
...
=item * Remains One Language Handling Raw Latin-8, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Latin8.pm, Elatin8.pm
  +---------------------------------------+
...
import of Elatin8.pm works well.
...
=head1 Software Composition
   ...
   Latin8.pm               --- source code filter to escape Latin-8
   Elatin8.pm              --- run-time routines for Latin8.pm
...
You need write 'use Latin8;' in your script.
...
=head1 Calling 'Elatin8::ignorecase()' (Latin8 software provides)

Latin8 software applies calling 'Elatin8::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Elatin8::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Elatin8.pm provides)
...
Definitions in Elatin8.pm.
  ...
  ${Elatin8::not_lower_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Elatin8::not_lower_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
  ${Elatin8::not_print}      qr{(?>[^\x20-\x7F])};
  ${Elatin8::not_punct}      qr{(?>[^\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E])};
  ${Elatin8::not_space}      qr{(?>[^\s\x0B])};
  ${Elatin8::not_upper}      qr{(?>[^\x41-\x5A])};
  ${Elatin8::not_upper_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Elatin8::not_upper_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Latin8 software provides)

Latin8 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Latin8 software provides)

Insert 'Elatin8::' at head of function name. Elatin8.pm provides your script Elatin8::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Elatin8::lc        Character
  lcfirst     Elatin8::lcfirst   Character
  uc          Elatin8::uc        Character
  ucfirst     Elatin8::ucfirst   Character
  fc          Elatin8::fc        Character
  chr         Elatin8::chr       Character
  glob        Elatin8::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Latin8 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Latin8 software provides)

Latin8 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Elatin8.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Latin8 software and
...
Latin8 software remains one language and one interpreter.
