...
This document is a guide to writing plugins for Dist::Man.  Currently, as
is evident, it isn't very comprehensive.  It should provide enough information
for writing effective plugins, though.  After all, Dist::Man's guts are
nice and simple.

=head2 C<< Dist::Man->import >>

Dist::Man provides an import method, the arguments to which are plugins,
in the order in which they should be loaded.  If no plugins are given,
L<Dist::Man::Simple> (and only Dist::Man::Simple) is loaded.

By default, the given modules are required and arranged in an I<is-a> chain.
That is, Dist::Man subclasses the last plugin given, which subclasses the
...
implementation.  The only engine included with Dist::Man is
Dist::Man::Simple, and I'm not sure any more will be seen in the wild any
time soon.
...
features to Dist::Man engines, or may just provide general APIs for other
plugins to exploit (like L<Dist::Man::Plugin::Template>.)
...
Dist::Man::Plugin::Template will be available on the CPAN.
