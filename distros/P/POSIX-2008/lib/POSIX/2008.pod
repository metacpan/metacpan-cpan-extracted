...
(Intel/AMD), or C<RNDR> (ARM64) if available (picked in this order at build
time).
...
C<O_DIRECT> C<O_DIRECTORY> C<O_DSYNC> C<O_EMPTY_PATH> C<O_EXEC> C<O_EXCL>
C<O_EXLOCK> C<O_LARGEFILE> C<O_NDELAY> C<O_NOATIME> C<O_NOCTTY> C<O_NOFOLLOW>
C<O_NONBLOCK> C<O_NOSIGPIPE> C<O_PATH> C<O_RDONLY> C<O_RDWR>
C<O_RESOLVE_BENEATH> C<O_REGULAR> C<O_RSYNC> C<O_SEARCH> C<O_SHLOCK> C<O_SYNC>
...
C<SOCK_DGRAM> C<SOCK_RAW> C<SOCK_SEQPACKET> C<SOCK_STREAM> C<SOCK_NONBLOCK>
C<SOCK_CLOEXEC> C<SOCK_CLOFORK>
...
C<INFTIM> C<POLLERR> C<POLLFREE> C<POLLHUP> C<POLLIN> C<POLLINIGNEOF>
C<POLLMSG> C<POLLNORM> C<POLLNVAL> C<POLLOUT> C<POLLPRI> C<POLLRDBAND>
C<POLLRDHUP> C<POLLRDNORM> C<POLLREMOVE> C<POLLSTANDARD> C<POLLWRBAND>
C<POLLWRNORM> C<POLL_BUSY_LOOP>
