...
sub get_expanded_text_ads {
  ...
  # Create predicates.
  my $ad_group_predicate = Google::Ads::AdWords::v201702::Predicate->new({
      field    => "AdGroupId",
      operator => "IN",
      values   => [$ad_group_id]});
  my $status_predicate = Google::Ads::AdWords::v201702::Predicate->new({
      field    => "Status",
      operator => "IN",
      values   => ["ENABLED", "PAUSED"]});
  my $ad_type_predicate = Google::Ads::AdWords::v201702::Predicate->new({
      field    => "AdType",
      operator => "EQUALS",
