...
#if ( my $x = $chi->get('https://www.googleapis.com/discovery/v1/apis') )
{
  ...
  #croak('fdfd');
  my $y = WebService::GoogleAPI::Client->new(chi => $chi)
      ->discovery->get_api_discovery_for_api_version({ api => 'gmail', version => 'v1' });
...
if (1 == 0) {
  ...
  for my $api (@{ $discover_all->{items} }) {
    if ($api->{preferred}) {
      ...
      print my $v1
          = qq{$api->{preferred} $api->{name} $api->{version} https://www.googleapis.com/discovery/v1/apis/$key \n};
...
if (1 == 0) {
  ...
  my $x = WebService::GoogleAPI::Client->new()
      ->discovery->extract_method_discovery_detail_from_api_spec('gmail.users.messages.list-breakme');

  #print Dumper $x;
  say "empty hashref" if (keys %$x == 0);
  say "empty hashref as expected"
      if (
    keys %{ WebService::GoogleAPI::Client->new()
          ->discovery->extract_method_discovery_detail_from_api_spec('gmail.users.messages.list-breakme')
    } == 0
      );
  say join("\n\t", keys %$x);
...
######################################
if ('NO     -PULL-ALL-API-SPECS-AND-EXTRACT-METHODS' eq
  'YES-PULL-ALL-API-SPECS-AND-EXTRACT-METHODS')    ## NB - network load unless cached
{
...
#say "Processed a total of " . scalar(@methods) . ' services';
  ...
  foreach my $service_id (WebService::GoogleAPI::Client->new(chi => $chi)->list_of_available_google_api_ids()) {
    my $service_methods
        = WebService::GoogleAPI::Client->new(chi => $chi)->methods_available_for_google_api_id($service_id);
...
#  my $scopes = "email profile https://www.googleapis.com/auth/plus.profile.emails.read https://www.googleapis.com/auth/calendar https://www.googleapis.com/auth/contacts.readonly https://mail.google.com";
  ...
  my %configured_scopes_hash
      = map { s/\/$//xr, 1 } @$configured_scopes;    ## NB r switch as per https://www.perlmonks.org/?node_id=613280
                                                     #print Dumper \%configured_scopes_hash;exit;
  my @aapis = $gapi_agent->list_of_available_google_api_ids()
      ;                                              ## nb - actually returns an array .. perhaps make aref standard?
                                                     #print Dumper \@aapis;
  ...
  foreach my $api (qw/gmail/) {
    ...
    foreach my $method_spec_key (keys %$api_methods) {
      ...
      foreach my $method_scope (map {s/\/$//xr}
        @{ $api_methods->{$method_spec_key}{scopes} }
          )    ## as earlier remove trailing '/' - NB r switch as per https://www.perlmonks.org/?node_id=613280
...
#say " method $method_spec_key Permission on currently configured scopes = " . ('Denied','Granted')[$granted] if $granted==1;
    ...
    print Dumper $api_spec_scope_counts
        ;    ## nb - the set venn gets fubarred by last - also need to really look to interpret these results
  }
  exit;
}
######################################
...
if (1 == 1)    ## TODO: - Audit all service method param
{
  ...
  foreach my $api (@aapis) {
    if (1 == 0)    ## check expected top level fields for pi_discovery_for_api_id of $api_id
    {
      ...
      $meth_spec_variations->{spec}{documentationLink} = ''
          unless defined $meth_spec_variations->{spec}{documentationLink};

      foreach my $expected_key (
        qw/title rest ownerName canonicalName version id discoveryVersion revision documentationLink description/) {
...
#      }
    ...
    foreach my $meth (keys %{$meths_by_id}) {
      ...
      if (defined $meths_by_id->{$meth}{parameterOrder} && scalar(@{ $meths_by_id->{$meth}{parameterOrder} }) > 0) {
        $meth_spec_variations->{parameters}{parameterOrder_ttl_count}
            += scalar(@{ $meths_by_id->{$meth}{parameterOrder} });
...
my $ret = WebService::GoogleAPI::Client::Discovery->new(debug => 1)
    ->get_resource_meta('WebService::GoogleAPI::Client::Gmail::Users');    ## ::CalendarList::delete
...
for my $api (@{$aapis}) {
  ...
  my @clean_doclinks = grep { defined $_ }
      @{ $api->{doclinks} };    ## was seeing undef in doclinks array - eg 'surveys'causing warnings in join
  my %seen     = ();
...
if (1 == 0
    ) ## initial play to explore how CHI works and whether could be applied - informed integration of CHI into WebService::GoogleAPI::Client::Discovery
{
...
sub render_services_as_formatted_table {
  ...
  foreach my $r (@{ $d->{items} }) {
    push @$rows,
        [ $r->{version}, $r->{title}, $r->{description} ];    ## documentationLink icon-s->{x32|16}  id discoveryRestUrl

  }
  my $tb = Text::Table->new
      ( ## currently use Text::Table - perhaps should use Text::MarkdownTable so can put images and links in without bloaring the col lengths
...
if (1 == 0)    ## used to info teapot reponse code in Client->api_query
{
  ...
  $res->body(
    'A Valid Google API Service end-point was included in params but the token does not have the required scope');
  say $res->to_string;
...
sub api_query {
  my ($self, $params) = @_;
  my @teapot_errors
      = ();    ## used to collect pre-query validation errors - if set we return a response with 418 I'm a teapot
  ...
  if (defined $params->{api_endpoint_id}) {
    ...
    if (keys %{$method_discovery_struct} > 0)    ## method discovery struct ok
    {
      ...
      if (not defined $params->{method}) {
        $params->{method} = $method_discovery_struct->{httpMethod}
            || croak("API Endpoint discovered specification didn't include expected httpMethod value");
      ...
      foreach my $meth_param_spec (keys %{ $method_discovery_struct->{parameters} }) {
        ...
        if ($params->{path} =~ /\{.+\}/xms
            )    ## there are un-interpolated variables in the path - try to fill them for this param if reqd
        {
...
#carp("DEBUG: $method_discovery_struct->{parameters}{$meth_param_spec}{'location'}");
          if ($method_discovery_struct->{parameters}{$meth_param_spec}{'location'} eq 'path') ## this is a path variable
          {
            ...
            if ($params->{path} =~ /\{$meth_param_spec\}/xg) ## eg match {jobId} in 'v1/jobs/{jobId}/reports/{reportId}'
            {
              ...
              ## else if not provided as an option but a default value is provided in the spec
                ...
                $params->{path}
                    =~ s/\{$meth_param_spec\}/$method_discovery_struct->{parameters}{$meth_param_spec}{default}/xsmg;
        ...
        } elsif ($method_discovery_struct->{parameters}{$meth_param_spec}{'location'} eq
          'query')    ## check post form variables .. assume not get?
        {
          if (!defined $params->{options}{$meth_param_spec}) {
            $params->{options}{$meth_param_spec} = $method_discovery_struct->{parameters}{$meth_param_spec}{default}
      ...
      $params->{path} = "$api_discovery_struct->{baseUrl}/$params->{path}"
          unless $params->{path} =~ /^$api_discovery_struct->{baseUrl}/ixsmg;
      ...
      if (@teapot_errors) {
        ...
        $method_discovery_struct->{canonicalName} = $method_discovery_struct->{title}
            unless defined $method_discovery_struct->{canonicalName};
        $method_discovery_struct->{documentationLink} = '??'
            unless defined $method_discovery_struct->{documentationLink};
