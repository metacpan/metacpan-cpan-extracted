package blx::xsdsql::xsd_parser::node::restriction;
...
sub trigger_at_start_node {
	my ($self,%params)=@_;
	my ($ns,$base)=map { $self->get_attrs_value($_); } qw(NAMESPACE base);
	$self->set_attrs_value(base => $ns.':string')  if $base eq $ns.':NOTATION'; #force the type in string
	return $self;
}
