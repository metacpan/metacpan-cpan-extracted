...
sub client2_connected {
  my ($kernel,$heap,$sender) = @_[KERNEL,HEAP,SENDER];
  pass($_[STATE]);
  $kernel->post( $sender, 'send_to_server', { command => 'NICK', params => [ 'mustard' ], colonify => 0 } );
  $kernel->post( $sender, 'send_to_server', { command => 'USER', params => [ 'mustard', '*', '*', 'colonel mustard' ], colonify => 1 } );
  return;
}

sub _svsnick {
  $poe_kernel->post('groucho', 'send_to_server', { prefix => '4AK', command => 'SVSNICK', params => [ '1FUAAAAAA', 'mustard', time()] } );
  return;
}
...
sub client_input {
  ...
  if ($cmd eq 'ERROR') {
    pass($cmd);
    is($params->[0],'Closing Link: 127.0.0.1 (SVSNICK Collide)','Closing Link: 127.0.0.1 (SVSNICK Collide)');
    return;
  }
  return;
}
...
sub client2_input {
  ...
  my $cmd    = $in->{command};
  my $params = $in->{params};
  if ( $cmd eq 'MODE' && $prefix =~ m'^mustard' && $params->[1] eq '+i' ) {
    pass($cmd);
    $poe_kernel->yield('_svsnick');
    return;
  }
  return;
}
...
sub groucho_input {
  ...
  if ( $cmd eq 'QUIT' ) {
    pass($cmd);
    is($prefix,'1FUAAAAAA','Prefix okay');
    is($params->[0],'SVSNICK Collide','SVSNICK Collide');
    return;
  }
  return;
}
