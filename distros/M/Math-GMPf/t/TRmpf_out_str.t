...
my($ret, $ok, $mpf, $count, $prefix, $suffix);

$mpf = Math::GMPf->new(17);
...
for(1..5) {
   $ret = TRmpf_out_str(\*$WR1, 10, 0, $mpf);
...
for(1..5) {
   $ret = TRmpf_out_str($prefix, \*$WR4, 10, 0, $mpf, $suffix);
...
for(1..5) {
   $ret = TRmpf_out_str($prefix, \*$WR5, 10, 0, $mpf);
...
for(1..5) {
   $ret = TRmpf_out_str($prefix, \*$WR6, 10, 0, $mpf, $suffix);
