...
If no field is given the internal representation of certificate (X509 handle)
from Net::SSLeay is returned, with reference owned internally (i.e. should not
be freed).
...
If no B<$field> is given the X509 handle from the underlying OpenSSL will
be returned. This handle is owned internally and will only be valid as long as
the SSL connection exists. If used afterwards it might result in strange crashes
of the application.

=item B<peer_certificates>

This returns all the certificates send by the peer as X509 handles, e.g. first
the peers own certificate and then the rest of the chain. You might use
B<CERT_asHash> from L<IO::Socket::SSL::Utils> to inspect each of the
certificates.

Like with B<sock_certificate> and B<peer_certificates> the handles are owned
internally and will only be valid as long as the SSL connection exists.
