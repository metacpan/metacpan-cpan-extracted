package Developer::Dashboard::Pax::GuardedSSA;
...
sub build_unit {
    ...
    my $source_text = _source_text_for_unit($unit);
    my $type_annotations = Developer::Dashboard::Pax::TypeAnnotationExtractor->new(
        source_text => $source_text,
        native_shape => $unit->{native_shape},
        region_name => $unit->{region_name},
    )->extract;
    my $typed_ir = Developer::Dashboard::Pax::TypedIR->new->lower_unit(
        $unit,
...
sub _source_text_for_unit {
    my ($unit) = @_;
    return $unit->{source}{source_text} if defined $unit->{source}{source_text};
    return $unit->{source}{body_source} if defined $unit->{source}{body_source};
    return;
}
...
Developer::Dashboard::Pax::GuardedSSA - guard-aware SSA lowerer

=head1 SYNOPSIS

  use Developer::Dashboard::Pax::GuardedSSA;

  my $obj = Developer::Dashboard::Pax::GuardedSSA->new(...);
...
Example 1:
  ...
  perl -Ilib -MDeveloper::Dashboard::Pax::GuardedSSA -e 1
