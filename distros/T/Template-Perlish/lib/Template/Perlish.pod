...
By default, landing on an (existent) undefined value makes C<traverse>
return an empty string; this is to simplify usage in templates and is in
line with the default value for C<missing>. As of version 1.64 it's
possible to pass option C<undef> to set a different value, defaulting to
