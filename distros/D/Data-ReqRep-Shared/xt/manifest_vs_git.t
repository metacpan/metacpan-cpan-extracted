...
chomp(my $top = `git rev-parse --show-toplevel 2>/dev/null` // '');
plan skip_all => 'not a git checkout' unless $top && abs_path($top) eq $root;

my %git = map { chomp; $_ => 1 } qx(git ls-files);
delete $git{'.gitignore'};
delete $git{$_} for grep /^\.github\//, keys %git;   # CI files optional
delete $git{$_} for grep /^MANIFEST(\.SKIP)?$/, keys %git;
