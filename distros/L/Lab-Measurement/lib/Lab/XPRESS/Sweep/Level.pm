...
$Lab::XPRESS::Sweep::Level::VERSION = '3.760';
...
sub go_to_sweep_start {
    ...
    # go to start:
    print "Setting level to start value ... ";
...
sub go_to_next_step {
    ...
    if ( $self->{config}->{jump} == 1 ) {
        $self->{config}->{instrument}->set_level(
            ${ $self->{config}->{points} }[ $self->{iterator} ] );
    }
    else {
        $self->{config}->{instrument}->config_sweep(
...
array of level values  that defines the characteristic points of the sweep.
...
Case mode => 'step' :
Same as in 'continuous' but level will be swept in stop and go mode. I.e. level source approaches values between start and stop at the interval defined in 'stepwidth'. A measurement is performed, when level source is idle.

Case mode => 'list' :
Array of levels, with minimum length 1, that are approached in sequence to perform a measurment.

=head2 rate [float array] (mandatory if not defined duration)

array of rates, at which the level is swept (V / sec).
...
The following values define the rates to approach the levels defined by the points-array.
...
The second value defines the duration to approach the level value defined by the second value of the points-array.
...
...
1 : the level is set without sweeping, given that gateprotect does not trigger a sweep.

=head2 id [string] (default = 'Level_sweep')
...
This software is copyright (c) 2021 by the Lab::Measurement team; in detail:
  ...
  Copyright 2012       Stefan Geissler
            ...
            2014       Alois Dirnaichner, Andreas K. HÃ¼ttel
