...
#  Portions Copyright (c) 2025 Jeffrey H.Q.Wang for DBD::KB
...

{
    ...
    use version; our $VERSION = qv('3.18.5');
    ...
    {
        package DBD::KB::DefaultValue;
        sub new { my $self = {}; return bless $self, shift; }
    }
    our $DBDPG_DEFAULT = DBD::KB::DefaultValue->new();
   ...
   ## Both CLONE and driver are required by DBI, see perldoc DBI::DBD
    ...
    sub driver {
        ...
        $drh = DBI::_new_drh($class, {
            ...
            'Version'     => $VERSION,
            'Err'         => \$DBD::KB::err,
            'Errstr'      => \$DBD::KB::errstr,
            'State'       => \$DBD::KB::sqlstate,
            'Attribution' => "DBD::KB $VERSION by Greg Sabino Mullane and others",
        });

        if (!$methods_are_installed) {
            DBD::KB::db->install_method('pg_cancel');
            DBD::KB::db->install_method('pg_endcopy');
            DBD::KB::db->install_method('pg_error_field');
            DBD::KB::db->install_method('pg_getline');
            DBD::KB::db->install_method('pg_getcopydata');
            DBD::KB::db->install_method('pg_getcopydata_async');
            DBD::KB::db->install_method('pg_notifies');
            DBD::KB::db->install_method('pg_putcopydata');
            DBD::KB::db->install_method('pg_putcopyend');
            DBD::KB::db->install_method('pg_ping');
            DBD::KB::db->install_method('pg_putline');
            DBD::KB::db->install_method('pg_ready');
            DBD::KB::db->install_method('pg_release');
            DBD::KB::db->install_method('pg_result'); ## NOT duplicated below!
            DBD::KB::db->install_method('pg_rollback_to');
            DBD::KB::db->install_method('pg_savepoint');
            DBD::KB::db->install_method('pg_server_trace');
            DBD::KB::db->install_method('pg_server_untrace');
            DBD::KB::db->install_method('pg_type_info');

            DBD::KB::st->install_method('pg_cancel');
            DBD::KB::st->install_method('pg_result');
            DBD::KB::st->install_method('pg_ready');
            DBD::KB::st->install_method('pg_canonical_ids');
            DBD::KB::st->install_method('pg_canonical_names');

            DBD::KB::db->install_method('pg_lo_creat');
            DBD::KB::db->install_method('pg_lo_open');
            DBD::KB::db->install_method('pg_lo_write');
            DBD::KB::db->install_method('pg_lo_read');
            DBD::KB::db->install_method('pg_lo_lseek');
            DBD::KB::db->install_method('pg_lo_tell');
            DBD::KB::db->install_method('pg_lo_truncate');
            DBD::KB::db->install_method('pg_lo_close');
            DBD::KB::db->install_method('pg_lo_unlink');
            DBD::KB::db->install_method('pg_lo_import');
            DBD::KB::db->install_method('pg_lo_import_with_oid');
            DBD::KB::db->install_method('pg_lo_export');
...

{
    ...
    sub data_sources {
        ...
        if ($ENV{DBI_DSN}) {
            ($connstring = $ENV{DBI_DSN}) =~ s/dbi:KB://i;
        }
        if (length $attr) {
            $connstring .= ";$attr";
        }

        my $dbh = DBD::KB::dr::connect($drh, $connstring) or return;
        $dbh->{AutoCommit}=1;
        my $SQL = 'SELECT pg_catalog.quote_ident(datname) FROM pg_catalog.pg_database ORDER BY 1';
        my $sth = $dbh->prepare($SQL);
        $sth->execute() or die $DBI::errstr;
        $attr and $attr = ";$attr";
        my @sources = map { "dbi:KB:dbname=$_->[0]$attr" } @{$sth->fetchall_arrayref()};
        $dbh->disconnect;
        return @sources;
    }
    ...
    sub connect { ## no critic (ProhibitBuiltinHomonyms)
        ...
        DBD::KB::db::_login($dbh, $dbname, $user, $pass, $attr) or return undef;
...

{
    ...
    sub parse_trace_flag {
        my ($h, $flag) = @_;
        return DBD::KB->parse_trace_flag($flag);
    }
    ...
    sub prepare {
        ...
        DBD::KB::st::_prepare($sth, $statement, @attribs) || 0;
    ...
    sub pg_ping {
        my $dbh = shift;
        local $SIG{__WARN__} = sub { } if $dbh->FETCH('PrintError');
        return DBD::KB::db::_ping($dbh);
    }

    sub pg_type_info {
        my($dbh,$pg_type) = @_;
        local $SIG{__WARN__} = sub { } if $dbh->FETCH('PrintError');
        my $ret = DBD::KB::db::_pg_type_info($pg_type);
        return $ret;
    }
    ...
    sub column_info {
        ...
        for my $row (@$data) {
            ...
            # Replace the KB type with the SQL_ type
            $row->[$col_map{DATA_TYPE}] = DBD::KB::db::pg_type_info($dbh,$row->[$col_map{DATA_TYPE}]);
...
## Driver information:
   ...
   10021 => ['SQL_ASYNC_MODE',                      2                         ], ## SQL_AM_STATEMENT
     ...
     121 => ['SQL_BATCH_SUPPORT',                   3                         ], ## 12 SELECT_PROC + ROW_COUNT_PROC
       2 => ['SQL_DATA_SOURCE_NAME',                sub { sprintf 'dbi:KB:%s', shift->{Name} } ],
     ...
     135 => ['SQL_DRIVER_HDESC',                    0                         ], ## not applicable
      ...
      76 => ['SQL_DRIVER_HLIB',                     0                         ], ## not applicable
       ...
       ## Not clear what should go here. Some things suggest 'KB', others 'KB.pm'. We'll use DBD::KB for now
       6 => ['SQL_DRIVER_NAME',                     'DBD::KB'                 ],
...
## Supported SQL
   ...
   10003  => ['SQL_CATALOG_NAME',                   'N'                       ],
     ...
     148  => ['SQL_INDEX_KEYWORDS',                 0                         ], ## not needed for KB
...
## Conversion information - all but BIT, LONGVARBINARY, and LONGVARCHAR
    ...
    sub get_info {
        ...
        if (ref $ans eq 'CODE') {
            ...
            my $simpleversion = $DBD::KB::VERSION;
            $simpleversion =~ s/_/./g;
...
版本： 3.18.4 由王海清(161263@qq.com)根据DBD::Pg V3.18.0改造而来

=head1 DESCRIPTION

DBD::KB is a Perl module that works with the DBI module to provide access to
Kingbase databases.
...
The following connect statement shows almost all possible parameters:
  ...
  $dbh = DBI->connect("dbi:KB:dbname=$dbname;host=$host;port=$port;options=$options",
                      $username,
                      $password,
...
around the value e.g. "dbi:KB:dbname='spacey name';host=$host"
...
default socket directory from the one used to compile DBD::KB. 
Use the complete path to the socket directory as the name of the 
host, like this:

  $dbh = DBI->connect('dbi:KB:dbname=foo;host=/var/tmp/socket',
    $username,
    $password,
...
=head3 B<connect_cached>
  ...
  $dbh = DBI->connect_cached("dbi:KB:dbname=$dbname", $username, $password, \%options);
...
an alternate port and host:
  ...
  @data_sources = DBI->data_sources('KB', 'port=5824;host=example.com');
...
C<KCIConnectionGetStatus>, which is a number used by I<libkci> (the Postgres connection library). A value of 0 
...
The list of codes used by Kingbase can be found at:
...
flags that DBD::KB uses, see L<parse_trace_flag|/parse_trace_flag and parse_trace_flags>.
...
=head3 B<parse_trace_flag> and B<parse_trace_flags>
  ...
  my $value = DBD::KB->parse_trace_flag('pglibkci');
  DBI->trace($value);
...
DBD::KB currently supports the DBI-specific flag, C<SQL>, 
...
the call to C<< DBD::KB->parse_trace_flags >> provides a number than can 
be fed to C<< DBI->trace >> before you create a database handle.

DBD::KB supports the following trace flags:
...
as server-side prepared statements are used extensively by DBD::KB.
...
to DBD::KB (all those below which start with the letters 'pg').
...
Outputs the name of each internal DBD::KB function, and other information such as 
...
Outputs a simple message at the very end of each internal DBD::KB function. This is also 
...
DBD::KB uses the C<func> method to support a variety of functions. 
...
This is only available when DBD::KB is compiled against a Postgres 
...
Returns a hash of all private attributes used by DBD::KB, for either 
...
Supported by DBD::KB as proposed by DBI. This method is similar to the
...
WARNING: DBD::KB now (as of version 1.40) uses true prepared statements by sending them 
...
prepared statements, which enables DBD::KB to only send the query once, and
simply send the arguments for every subsequent call to L</execute>.
DBD::KB can use these server-side prepared statements, or it can
...
Kingbase backend only have to prepare the query only once, but DBD::KB no
...
there are methods to instruct DBD::KB to not treat some symbols as placeholders. First, you 
may simply add a backslash before the start of a placeholder, and DBD::KB will strip the 
backslash and not treat the character as a placeholder. 

You can also tell DBD::KB to ignore any non-dollar sign placeholders by setting the 
...
placeholders, you can tell DBD::KB to ignore just colon placeholders by setting
...
name with optional schema, and DBD::KB will attempt to find the sequence itself. 
...
DBD::KB will attempt to return the correct value. To do this, there must be at
...
looking up of things in the system table, so DBD::KB will cache the sequence
...
exported by DBD::KB (such as PG_BYTEA). In addition to string, bytea, char, bool, 
...
B<'DBD::KB ping test v3.10.4'>. It returns 0 (false) if the connection is not valid, 
...
This is a DBD::KB-specific extension to the L</ping> method. This will check the 
...
L<https://www.postgresql.org/docs/current/libkci-exec.html#LIBPQ-PQRESULTERRORFIELD>
...
Four additional fields specific to DBD::KB are returned:
...
not used by DBD::KB. For information on the format of the standard rows returned, please 
...
Supported by DBD::KB as proposed by DBI. According to the classification of
...
DBD::KB specific attribute. If true, boolean values will be returned
...
DBD::KB specific attribute. Indicates if DBD::KB should attempt to use server-side 
...
DBD::KB specific attribute. Indicates when DBD::KB will internally switch from using 
...
Setting pg_switch_prepared to 0 will force DBD::KB to always use KCIStatementExecuteParams.

=head3 B<pg_placeholder_dollaronly> (boolean)

DBD::KB specific attribute. Defaults to false. When true, question marks inside of statements 
...
DBD::KB specific attribute. Defaults to false. When true, colons inside of statements
...
DBD::KB specific attribute. The behavior of DBD::KB with regards to this flag has 
...
pg_enable_utf8 to -1 to force DBD::KB to read in the new client_encoding and 
act accordingly.

=head3 B<pg_errorlevel> (integer)

DBD::KB specific attribute. Sets the amount of information returned by the server's 
...
DBD::KB specific attribute. Indicates which version of Kingbase that 
DBD::KB was compiled against. In other words, which libraries were used. 
...
"dbi:KB:" part. Before version 2.0.0, this only returned the bare database name 
...
DBD::KB specific attribute. Returns the name of the current database.

=head3 B<pg_user> (string, read-only)

DBD::KB specific attribute. Returns the name of the user that
connected to the server.

=head3 B<pg_host> (string, read-only)

DBD::KB specific attribute. Returns the host of the current
server connection. Locally connected hosts will return an empty
string.

=head3 B<pg_port> (integer, read-only)

DBD::KB specific attribute. Returns the port of the connection to
the server.

=head3 B<pg_socket> (integer, read-only)

DBD::KB specific attribute. Returns the file description number of
the connection socket to the server.

=head3 B<pg_pass> (string, read-only)

DBD::KB specific attribute. Returns the password used to connect
to the server.

=head3 B<pg_options> (string, read-only)

DBD::KB specific attribute. Returns the command-line options passed
to the server. May be an empty string.

=head3 B<pg_default_port> (integer, read-only)

DBD::KB specific attribute. Returns the default port used if none is
specifically given.

=head3 B<pg_pid> (integer, read-only)

DBD::KB specific attribute. Returns the process id (PID) of the
backend server process handling the connection.

=head3 B<pg_prepare_now> (boolean)

DBD::KB specific attribute. Default is off. If true, then the L</prepare> method will 
immediately prepare commands, rather than waiting until the first execute.

=head3 B<pg_expand_array> (boolean)

DBD::KB specific attribute. Defaults to true. If false, arrays returned from the server will 
not be changed into a Perl arrayref, but remain as a string.

=head3 B<pg_async_status> (integer, read-only)

DBD::KB specific attribute. Returns the current status of an L<asynchronous|/Asynchronous Queries>
...
DBD::KB specific attribute. Returns true if the server is currently using 
...
DBD::KB specific attribute. Returns the version of the Kingbase server.
If DBD::KB is unable to figure out the version, it will return a "0". Otherwise,
a "3" is returned.
...
use one of the values provided by DBI or by DBD::KB. To use a SQL value,
...
data type to something else, DBD::KB will re-prepare the statement for you before
...
DBD::KB also supports passing in arrays to execute: simply pass in an arrayref, 
and DBD::KB will flatten it into a string suitable for input on the backend.
...
Supported by DBD::KB. This method is implemented by DBI but not
...
about Large Objects in the Kingbase Programmer's Guide at
...
DBD::KB specific method. It returns Oid of table and position in table for
...
DBD::KB specific method. It returns array of original (or canonical) names
...
DBD::KB specific attribute. Returns the number of the tuple (row) that was
most recently fetched. Returns zero before and after fetching is performed.

=head3 B<pg_numbound> (integer, read-only)

DBD::KB specific attribute. Returns the number of placeholders
that are currently bound (via bind_param).

=head3 B<pg_bound> (hashref, read-only)

DBD::KB specific attribute. Returns a hash of all named placeholders. The
...
DBD::KB specific attribute. It returns a reference to an array of integer
...
DBD::KB specific attribute. It returns a reference to an array of strings
for each column. The string shows the name of the data_type.

=head3 B<pg_segments> (arrayref, read-only)

DBD::KB specific attribute. Returns an arrayref of the query split on the 
placeholders.

=head3 B<pg_oid_status> (integer, read-only)

DBD::KB specific attribute. It returns the OID of the last INSERT command.

=head3 B<pg_cmd_status> (integer, read-only)

DBD::KB specific attribute. It returns the type of the last
command. Possible types are: "INSERT", "DELETE", "UPDATE", "SELECT".

=head3 B<pg_direct> (boolean)

DBD::KB specific attribute. Default is false. If true, the query is passed 
directly to the backend without parsing for placeholders.

=head3 B<pg_prepare_now> (boolean)

DBD::KB specific attribute. Default is off. If true, the query will be immediately 
prepared, rather than waiting for the L</execute> call.

=head3 B<pg_prepare_name> (string)

DBD::KB specific attribute. Specifies the name of the prepared statement to use for this 
...
DBD::KB specific attribute. Defaults to false. When true, question marks inside of the query 
...
DBD::KB specific attribute. Indicates the current behavior for asynchronous queries. See the section 
...
Not used by DBD::KB. See the note about L</Cursors> elsewhere in this document.

=head1 FURTHER INFORMATION

=head2 Encoding

DBD::KB has extensive support for a client_encoding of UTF-8, and most 
...
rest of the transaction. DBD::KB encourages using the following methods to 
control savepoints:
...
There are currently three asynchronous constants automatically exported by DBD::KB.
...
so you may need to ROLLBACK or ROLLBACK TO a savepoint. As of version 2.17.0 of DBD::KB, rollbacks are 
...
DBD::KB allows for quick (bulk) reading and storing of data by using 
...
Kingbase documentation for COPY can be found at:
...
For convenience, DBD::KB can export certain constants representing the limits of 
Postgres data types. To use them, just add C<:pg_limits> when DBD::KB is used:

  use DBD::KB qw/:pg_limits/;
...
DBD::KB supports all largeobject functions provided by libkci via the
...
choice. DBD::KB therefore translates the result for the C<BOOL> data type in a
...
Kingbase documentation for more details:

L<http://www.postgresql.org/docs/current/static/ddl-schemas.html>

DBD::KB does not provide explicit support for Kingbase schemas.
...
http://rt.cpan.org/Public/Dist/Display.html?Name=DBD-KB
...
DBD::KB by H.Q.Wang <161263@qq.com>

The original DBD-KB was by H.Q.Wang <161263@qq.com> and DBD::Pg groups
Edmund Mergl (E.Mergl@bawue.de) and Jeffrey W. Baker
...
file (L<http://search.cpan.org/dist/DBD-KB/Changes>) for a complete list.

Parts of this package were originally copied from DBI and DBD-Oracle.

B<Mailing List>

The current maintainers may be reached through the 'dbd-kb' mailing list:
<dbd-kb@perl.org>. Subscribe by sending an email to dbd-pg-subscribe@perl.org. 
Visit the archives at http://grokbase.com/g/perl/dbd-pg

=head1 COPYRIGHT AND LICENSE

Copyright (C) 2025, H.Q.Wang
