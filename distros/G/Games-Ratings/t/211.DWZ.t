...
## * http://schachbund.de/dwz/turniere/2007.html?code=A540-336-OPN
##   (Player Bartolomaeus, Christian)

my $player = Games::Ratings::Chess::DWZ->new();
$player->set_rating(2181);
$player->set_coefficient(30);
$player->add_game( {
                     opponent_rating => 1712,
                     result          => 'win',
                   }
                 );
$player->add_game( {
                     opponent_rating => 1950,
                     result          => 'win',
                   }
                 );
$player->add_game( {
...
$player->add_game( {
                     opponent_rating => 1987,
                     result          => 'draw',
                   }
                 );
$player->add_game( {
                     opponent_rating => 1968,
                     result          => 'loss',
                   }
                 );
$player->add_game( {
                     opponent_rating => 1796,
                     result          => 'win',
                   }
                 );
$player->add_game( {
                     opponent_rating => 2014,
                     result          => 'draw',
                   }
                 );
...
$expected{t08_expected_points_all_games} = '6.468';
$computed{t08_expected_points_all_games} = $player->get_points_expected();

## test 3: check performance
$expected{t12_performance} = '2032';
