...
# hand (ForgeOps::Tracker::http_span for an outgoing HTTP call, which also hands it the
# traceparent header to send).
#
...
# every error reported during the request carries it. An error that escapes the app is remembered
# with the request's context (ForgeOps::Tracker::snapshot_onto) before the trace is finished, so
# Integrations::PSGI, enabled outside this middleware, still reports it with the trace id.
#
...
# pattern and so avoids a distinct id exploding into its own separate transaction. For the same
# reason an error reported during a plain PSGI request carries the trace id but no transaction
# name or endpoint (a raw path is neither), unless the app names the request itself with
# ForgeOps::Tracker::set_request_route.
sub call {
    my ($self, $env) = @_;

    ForgeOps::Tracker::clear_breadcrumbs();
    ForgeOps::Tracker::start_trace($env->{HTTP_TRACEPARENT});
    ...
    ForgeOps::Tracker::snapshot_onto($error) if $error;
