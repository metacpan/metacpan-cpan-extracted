#
# Test case for WebService::Recruit::CarSensor::Color
#
...
my $service = new WebService::Recruit::CarSensor::Color();

ok( ref $service, 'new WebService::Recruit::CarSensor::Color()' );
...
# Test[0]
{
    ...
    my $res = new WebService::Recruit::CarSensor::Color();
    ...
    can_ok( $data, 'color' );
    ok( eval { $data->color }, 'Test[0]: color' );
    ok( eval { ref $data->color } eq 'ARRAY', 'Test[0]: color' );
    can_ok( $data->color->[0], 'code' );
    ok( eval { $data->color->[0]->code }, 'Test[0]: code' );
    can_ok( $data->color->[0], 'name' );
    ok( eval { $data->color->[0]->name }, 'Test[0]: name' );
}
