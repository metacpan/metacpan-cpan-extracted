...
$test->for('synopsis', sub {
  ...
  ok $result->isa('Faker::Plugin::JaJp::JargonNoun');
...
$test->for('example', 1, 'execute', sub {
  ...
  is $result->execute, '市場';
  ok $result->faker->random->pick; # reset randomizer
  is $result->execute, 'パートナーシップ';
  ok $result->faker->random->pick; # reset randomizer
  is $result->execute, '眼球';
...
$test->render('lib/Faker/Plugin/JaJp/JargonNoun.pod') if $ENV{RENDER};
