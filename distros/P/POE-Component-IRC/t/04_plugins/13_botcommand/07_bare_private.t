...
sub irc_join {
    ...
    # try command
    $bot2->yield(privmsg => $bot1->nick_name(), "cmd1 foo bar");

    # and one with color
    $bot2->yield(privmsg => $bot1->nick_name(), "\x02cmd2\x0f");
}
...
sub irc_botcmd_cmd1 {
    ...
    is($where, $bot2->nick_name(), 'Normal command (channel)');
...
sub irc_botcmd_cmd2 {
    ...
    is($where, $bot2->nick_name(), 'Colored command (channel)');
