...
sub parse {
    return DMS::XS::Parser::decode_document($_[0]);
}

sub path_strs {
    my $aref = shift;
    return [ map { ref($_) eq 'DMS::Index' ? '#' . $_->value : $_ } @$aref ];
}
...
# 8. List-item leading with DMS::Index breadcrumb
{
    my $doc = parse("+ 1\n# before-second\n+ 2\n");
    ...
    isa_ok($ac->{path}[0], 'DMS::Index', 'list-leading: path[0] is DMS::Index');
