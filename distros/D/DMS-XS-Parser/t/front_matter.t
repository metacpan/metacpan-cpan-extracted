...
# DMS::XS::Parser::decode_front_matter($src). Same contract as the
# pure-Perl tier (DMS/t/front_matter.t):
#   - No FM at all → undef
...
# Strip the lite-mode order sidecar so we can compare just user keys.
# DMS-XS uses "\0__dms_keys"; DMS uses "\0_keys". We tolerate both so
# the same helper can run against either backend.
sub user_keys {
    my $h = shift;
    return [ sort grep { $_ ne "\0_keys" && $_ ne "\0__dms_keys" } keys %$h ];
}

# 1. No front matter → undef.
{
    my $fm = DMS::XS::Parser::decode_front_matter("port: 8080\n");
    is($fm, undef, 'no FM: returns undef');

    $fm = DMS::XS::Parser::decode_front_matter("");
    is($fm, undef, 'empty source: returns undef');

    # Trivia only (comments, blank lines) without a `+++`: still undef.
    $fm = DMS::XS::Parser::decode_front_matter("# just a comment\n\n");
    is($fm, undef, 'trivia-only source: returns undef');
}

# 2. Empty FM (`+++\n+++\n`) → defined empty hashref.
{
    my $fm = DMS::XS::Parser::decode_front_matter("+++\n+++\nport: 8080\n");
...
body: 1
    ...
    my $fm = DMS::XS::Parser::decode_front_matter($src);
...
{
    my $src = "+++\n_dms_tier: 1\n+++\n";
    eval { DMS::XS::Parser::decode_front_matter($src); };
    my $err = $@;
...
broken: """END
  no closing label ever
    ...
    my $fm;
    my $ok = eval { $fm = DMS::XS::Parser::decode_front_matter($src); 1 };
