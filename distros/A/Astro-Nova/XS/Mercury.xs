...
ln_get_mercury_helio_coords(double JD)
    ...
    CODE:
      ...
      ln_get_mercury_helio_coords(JD, RETVAL);
    OUTPUT:
      RETVAL

struct ln_equ_posn*
ln_get_mercury_equ_coords(double JD)
    INIT:
      const char* CLASS = "Astro::Nova::EquPosn";
    CODE:
      Newx(RETVAL, 1, struct ln_equ_posn);
      ln_get_mercury_equ_coords(JD, RETVAL);
    OUTPUT:
      RETVAL
...
struct ln_rect_posn*
ln_get_mercury_rect_helio(double JD)
    INIT:
      const char* CLASS = "Astro::Nova::RectPosn";
    CODE:
      Newx(RETVAL, 1, struct ln_rect_posn);
      ln_get_mercury_rect_helio(JD, RETVAL);
    OUTPUT:
      RETVAL
