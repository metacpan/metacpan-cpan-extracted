...
# page is loaded but event was not found
...
for my $type ('&&', '||'){
	...
	$ret = domops_wait_for_page_to_load({
	  'mech-obj' => $mech_obj,
	  'elements-must-be-present' => '//div[@id="div-id-1"]/div[@id="div-id-1-1"]',
	...
	$ret = domops_wait_for_page_to_load({
	  ...
	  'elements-must-be-present' => [
	    '//div[@id="div-id-2"]/div[@id="div-id-2-1"]',
	  ],
	  'elements-must-be-present-op' => $type,
	});
	is($ret, 0, 'domops_wait_for_page_to_load()'." : called and got good result for type '$type'/2.") or BAIL_OUT;

	# ANY: give it an element's xpath to be visible (array with 2 element2)
	$ret = domops_wait_for_page_to_load({
	  'mech-obj' => $mech_obj,
	  'elements-must-be-present' => [
	    '//div[@id="div-id-1"]/div[@id="div-id-1-1"]',
	    '//div[@id="div-id-2"]/div[@id="div-id-2-1"]',
	   ],
	  'elements-must-be-present-op' => $type,
	});
