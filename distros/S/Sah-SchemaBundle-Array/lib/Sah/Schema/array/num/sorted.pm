...
valid, a non-empty error message otherwise):
 ...
 $data = [1,3,2];
 my $errmsg = $validator->($data); # => "Elements not numerically sorted (check element[2]) "
...
(set-as-default, coerced, prefiltered) value:
 ...
 # a sample valid data
 $data = [1,2,2];
 my $res = $validator->($data); # => ["",[1,2,2]]
 
 # a sample invalid data
 $data = [1,3,2];
 my $res = $validator->($data); # => ["Elements not numerically sorted (check element[2]) ",[1,3,2]]
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"array::num::sorted*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('array::num::sorted*', name=>'ArrayNumSorted')
     );
 }
