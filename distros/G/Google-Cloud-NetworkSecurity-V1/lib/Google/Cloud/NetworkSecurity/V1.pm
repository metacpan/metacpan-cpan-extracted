...
sub BUILD {
    ...
    my $client = Google::gRPC::Client->new(
        target     => 'networksecurity.googleapis.com:443',
        auth_token => $token,
    );
...
Google::Cloud::NetworkSecurity::V1 - Google Cloud Network Security V1 API Client

=head1 SYNOPSIS

    use Google::Cloud::NetworkSecurity::V1;
    use Google::Auth;

    my $auth = Google::Auth->default();
    my $client = Google::Cloud::NetworkSecurity::V1->new(credentials => $auth);

=head1 DESCRIPTION

Google Cloud Network Security V1 API Client over high-performance gRPC transport.
