...
#   Verifies two things:
#
#   1. HTTPS::Handy module load and interface
#      - The module loads without error
#      - $VERSION is defined and looks like a version number
#      - The sub-packages (Input, BigInt, Crypt, RSA, X509, TLS) are present
...
#
# COMPATIBILITY
#   Perl 5.005_03 and later.  The distribution has no dependency
#   outside the Perl core, so nothing here is skipped.
#
######################################################################
...
my @sub_packages = qw(
    HTTPS::Handy::TLS    HTTPS::Handy::EC     HTTPS::Handy::ChaCha
    HTTPS::Handy::Crypt  HTTPS::Handy::X509   HTTPS::Handy::RSA
    HTTPS::Handy::BigInt
);

my $total = 4                           # load + VERSION + packages
          + scalar(@sub_packages)       # the pure Perl TLS packages
          + scalar(@handy_methods)      # HTTPS::Handy methods
          + scalar(@input_methods)      # HTTPS::Handy::Input methods
          + 4;                          # INA_CPAN_Check section
plan_tests($total);

######################################################################
# Section 1: HTTPS::Handy
######################################################################

# ok 1: module loads
eval { require HTTPS::Handy };
ok(!$@, 'HTTPS::Handy loads without error');
diag("load error: $@") if $@;

# ok 2-3: VERSION
ok(defined $HTTPS::Handy::VERSION,         'HTTPS::Handy: $VERSION defined');
ok(($HTTPS::Handy::VERSION =~ /^\d+\.\d+/) ? 1 : 0, 'HTTPS::Handy: $VERSION looks like a version number');

# ok 4: HTTPS::Handy::Input present
ok(defined $HTTPS::Handy::Input::{new}, 'HTTPS::Handy::Input package present');

# The packages that make up the TLS implementation
for my $p (@sub_packages) {
    no strict 'refs';
    ok(scalar(keys %{$p . '::'}) > 0, "$p package present");
}

# ok 5+: HTTPS::Handy public methods
for my $m (@handy_methods) {
    ok(HTTPS::Handy->can($m), "HTTPS::Handy->can('$m')");
}

# HTTPS::Handy::Input methods
for my $m (@input_methods) {
    ok(HTTPS::Handy::Input->can($m), "HTTPS::Handy::Input->can('$m')");
}
