...
BEGIN {
    use_ok('Win32::SharedFileOpen', qw(SH_DENYNO));
}
...
MAIN: {
    ...
    ok(!eval { SH_DENYRD(); 1 }, 'SH_DENYRD is not imported');
    ok(!eval { SH_DENYWR(); 1 }, 'SH_DENYWR is not imported');
    ok(!eval { SH_DENYRW(); 1 }, 'SH_DENYRW is not imported');
