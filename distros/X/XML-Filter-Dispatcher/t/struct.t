...
use XML::Filter::Dispatcher::AsStructHandler;
use UNIVERSAL;
...
my @ab_tests = (
sub { $h->{a}->[0]->{""} =~ /^\s+\z/ ? ok 1 : ok $h->{a}->[0]->{""}, qr/^\s+\z/ },
sub { ok $h->{'@a'},                   "A"   },
sub { ok $h->{a}->[0]->{'@aa1'},       "AA1" },
sub { ok $h->{a}->[0]->{'@aa2'},       "AA2" },
sub { ok $h->{a}->[0]->{b}->[0]->{""}, "B1"  },
sub { ok $h->{a}->[0]->{b}->[1]->{""}, "B2"  },
);
...
sub { ok $h->{'@bar:a'},                     "FOOA" },
sub { ok $h->{a}->[0]->{'@aa1'},             "AA1"  },
sub { ok $h->{a}->[0]->{'@bar:aa1'},         "AA2"  },
sub { ok $h->{a}->[0]->{b}->[0]->{""},       "B1"   },
sub { ok $h->{a}->[0]->{'bar:b'}->[0]->{""}, "B2"   },
);

my @tests = (
sub {
    $h = $ab->playback( XML::Filter::Dispatcher::AsStructHandler->new );
