...
C<path> returns the backing path (C<undef> for anonymous, memfd, or
fd-reopened trees) and C<memfd> the backing descriptor. C<frozen> and
C<readonly> report whether the tree has been sealed and whether this handle is
a read-only view, respectively (see L</"FROZEN (READ-ONLY) MODE">).
...
disturb the handle. Any process can add intervals; the first query after an
add rebuilds the shared tree once (under the write lock), and subsequent
...
to be frozen> (it croaks on a file that was never C<freeze>d). Because
C<freeze> guarantees the balanced tree is already built, and a sealed tree's
intervals and links are immutable, C<stab>, C<overlaps>, C<count> and C<stats>
read them B<directly, taking no reader lock and never rebuilding> -- the
...
apart from a corrupt one and C<new> croaks with C<incomplete interval tree
