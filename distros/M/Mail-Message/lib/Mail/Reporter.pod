...
the following C<%options> related to error reports.
...
Add the report from other C<$object> to the report of this object. This is
...
C<$level> is set for both loglevel as tracelevel.

With two arguments, the second determines which configuration you like.  If
the second argument is a CODE reference, you install a C<$callback>.  The loglevel
will be set to NONE, and all warnings produced in your program will get
passed to the C<$callback> function.  That function will get the problem level,
...
Each log-entry has a C<$level> and a text string which will
be constructed by joining the C<$strings>.  If there is no newline, it will
be added.
...
Get logged reports, as list of strings.  If a C<$level> is specified, the log
for that level is returned.

In case no C<$level> is specified, you get all messages each as reference
...
Change the trace C<$level> of the object. When no arguments are specified, the
