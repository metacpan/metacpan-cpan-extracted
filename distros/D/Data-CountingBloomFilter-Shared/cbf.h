...
static inline void cbf_init_header(void *base, uint32_t k, uint64_t m_ctr,
    ...
    /* Publish magic LAST, as a release store: it is the commit point, so a
       ...
       hand. */
    __atomic_store_n(&hdr->magic, CBF_MAGIC, __ATOMIC_RELEASE);
    ...
    if (anonymous) {
        ...
        if (!is_new) {
            if (!cbf_validate_header((CbfHeader *)base, (uint64_t)st.st_size)) {
                ...
                if (((CbfHeader *)base)->magic == 0 && (uint64_t)st.st_size == total
                    && st.st_uid == geteuid()) {
                    CBF_ERR("%s: incomplete counting Bloom filter file left by an interrupted create; remove it and retry", path);
                    munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
                }
                CBF_ERR("invalid counting Bloom filter file"); munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
            }
            if (((CbfHeader *)base)->sealed) {
                CBF_ERR("%s is frozen (read-only); open it with new_readonly", path);
                munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
            }
            flock(fd, LOCK_UN); close(fd);
            return cbf_setup(base, map_size, path, -1);
...
static CbfHandle *cbf_open_fd(int fd, char *errbuf) {
    ...
    if (((CbfHeader *)base)->sealed) {
        CBF_ERR("this counting Bloom filter is frozen (read-only); open it with new_readonly");
 ...
 * file). */
static CbfHandle *cbf_open_readonly(const char *path, char *errbuf) {
    if (errbuf) errbuf[0] = '\0';
    int fd = open(path, O_RDONLY|O_NOFOLLOW|O_CLOEXEC);
    if (fd < 0) { CBF_ERR("open %s: %s", path, strerror(errno)); return NULL; }
    struct stat st;
    ...
    if (base == MAP_FAILED) { CBF_ERR("mmap %s: %s", path, strerror(errno)); return NULL; }
    if (!cbf_validate_header((CbfHeader *)base, (uint64_t)st.st_size)) {
        CBF_ERR("%s: invalid counting Bloom filter file", path); munmap(base, ms); return NULL;
    }
    if (!((CbfHeader *)base)->sealed) {
        CBF_ERR("%s is not frozen: call ->freeze on the producer before opening read-only", path);
        munmap(base, ms); return NULL;
    }
    CbfHandle *h = cbf_setup(base, ms, path, -1);   /* munmaps on OOM */
    if (!h) { CBF_ERR("out of memory"); return NULL; }
    h->readonly = 1;
    return h;
}
...
static int cbf_freeze(CbfHandle *h) {
    cbf_rwlock_wrlock(h);
    h->hdr->sealed = 1;
    cbf_rwlock_wrunlock(h);
    if (h->path || h->backing_fd >= 0) return cbf_msync(h);  /* durability for file/memfd-backed */
