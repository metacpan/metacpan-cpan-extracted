...
our @EXPORT_OK = qw(dump_getopt_std_script);

our %SPEC;

$SPEC{dump_getopt_std_script} = {
    v => 1.1,
    summary => 'Run a Getopt::Std-based script but only to '.
        'dump the spec',
    description => <<'_',

This function runs a CLI script that uses `Getopt::Std` but monkey-patches
beforehand so that `getopts()` or `getopt()` will dump the spec and then exit.
...
CLI script needs to use `Getopt::Std`. This is detected currently by a simple
regex. If script is not detected as using `Getopt::Std`, status 412 is
returned.

Will return an array: `[$which, $spec]` where `$which` is either `getopt` or
`getopts`.
...
sub dump_getopt_std_script {
    require Capture::Tiny;
    require Getopt::Std::Util;
    require UUID::Random;
    ...
    my $detres;
    if ($args{skip_detect}) {
        $detres = [200, "OK (skip_detect)", 1, {"func.module"=>"Getopt::Std", "func.reason"=>"skip detect, forced"}];
    } else {
        $detres = Getopt::Std::Util::detect_getopt_std_script(
            filename => $filename);
        return $detres if $detres->[0] != 200;
        return [412, "File '$filename' is not script using Getopt::Std (".
    ...
    my @cmd = (
        $^X, (map {"-I$_"} @$libs),
        "-MGetopt::Std::Patch::DumpAndExit=-tag,$tag",
        $filename,
        "--version",
    );
    my ($stdout, $stderr, $exit) = Capture::Tiny::capture(
        sub { local $ENV{GETOPT_STD_DUMP} = 1; system @cmd },
    );
    ...
    if ($stdout =~ /^# BEGIN DUMP $tag\s+(.*)^# END DUMP $tag/ms) {
        ...
        if ($@) {
            return [500, "Script '$filename' looks like using ".
                        "Getopt::Std, but I got an error in eval-ing ".
                            "captured option spec: $@, raw capture: <<<$1>>>"];
        }
        if (ref($spec) ne 'ARRAY') {
            return [500, "Script '$filename' looks like using ".
                        "Getopt::Std, but I didn't get an array dump, ".
                            "raw capture: stdout=<<$stdout>>"];
        }
    } else {
        return [500, "Script '$filename' looks like using Getopt::Std, ".
...
Getopt::Std::Dump - Run a Getopt::Std-based script but only to dump the spec
...
=head2 dump_getopt_std_script(%args) -> [status, msg, result, meta]
