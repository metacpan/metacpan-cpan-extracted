...
foreach my $static (undef, 'off', 'on')
{
    subtest 'default installer (MakeMaker::Fallback, ModuleBuildTiny::Fallback), StaticInstall.mode = ' . ($static // '<undef>') => sub {
        ...
        cmp_deeply(
            ...
            'installer configuration settings are properly added to installer payload(s)',
        );
        ...
        my @files = recursive_child_files($build_dir);
        cmp_deeply(
            \@files,
            superbagof(qw(Build.PL)),
            'Build.PL was generated by the pluginbundle',
        );

        ok((!grep $_ eq 'Makefile.PL', @files), 'Makefile.PL was not included by default');
