...
sub edit {
  my $c = shift;
  my $id = $c->param('id');
  my $param = {name => 'New York', foto => 'https://www.flickr.com/search/?text=New%20York' };
  $c->qupdate('models', {id => $id}, $param);
  $c->redirect_to('/');
}

sub insert {
  my $c = shift;
  my $param = {name => 'London', foto => 'https://www.flickr.com/search/?text=London' };
  # details record
  #my $id = $c->qinsert('models', { name => 'Moscow', foto => 'https://www.flickr.com/search/?text=Moscow' } );
  # short record (as example $param = $s->req->json;)
  my $id = $c->qinsert('models', $param );
  $c->app->log->info($id);
  $c->redirect_to('/');
}
