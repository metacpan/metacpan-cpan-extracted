 ...
 *
 * $Id: tixGrid.c,v 1.7 2010/08/13 21:47:56 hobbs Exp $
 */
...
{
    Tix_DItem *iPtr = chPtr->iPtr;
    if (iPtr) {
	if (Tix_DItemType(iPtr) == TIX_DITEM_WINDOW) {
	    /*
	     * Remove items from mappedWindows list. [Bug 856808]
	     */
	    WidgetPtr wPtr = (WidgetPtr) iPtr->base.clientData;
	    if (wPtr) {
		Tix_WindowItemListRemove(&wPtr->mappedWindows, iPtr);
	    }
	}
	Tix_DItemFree(iPtr);
    }
