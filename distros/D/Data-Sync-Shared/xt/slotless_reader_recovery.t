...
# reader-slot table is full, a reader holds the rwlock word without a slot and
# is invisible to dead-reader recovery. Force-reset could then zero the lock
# under that live reader and admit a writer (writer-exclusion violation).
#
# Reproduced deterministically by compiling sync.h with SYNC_READER_SLOTS=1 so
# the single slot fills after one reader and the next reader goes slotless at
# just two processes. The fix tracks slotless readers in hdr->slotless_readers
# so recovery treats them as live.

plan skip_all => 'Linux only' unless $^O eq 'linux';
my $cc = $Config{cc} || 'cc';
system("$cc --version >/dev/null 2>&1") == 0
    or plan skip_all => "no working C compiler ($cc)";
-f 'sync.h' or plan skip_all => 'run from the distribution root (sync.h not found)';
...
static struct flags *F; static SyncHandle *L;
int main(void) {
    char eb[256];
    L = sync_create(NULL, SYNC_TYPE_RWLOCK, 0, 0, 0600, eb);
    if (!L) { fprintf(stderr, "create: %s\n", eb); return 2; }
    F = mmap(NULL, sizeof *F, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0);
    memset(F, 0, sizeof *F);
    pid_t a = fork();
    if (a == 0) { sync_rwlock_rdlock(L);
    ...
    if (b == 0) { sync_rwlock_rdlock(L);
        ...
        __atomic_sub_fetch(&F->reader_active,1,__ATOMIC_RELEASE); sync_rwlock_rdunlock(L); _exit(0); }
    ...
    if (c == 0) { sync_rwlock_wrlock(L);
        ...
        __atomic_store_n(&F->writer_active,0,__ATOMIC_RELEASE); sync_rwlock_wrunlock(L); _exit(0); }
    waitpid(c, NULL, 0); kill(b, SIGKILL); waitpid(b, NULL, 0);
    return __atomic_load_n(&F->violation, __ATOMIC_ACQUIRE) ? 1 : 0;
}
C
close $fh;

my $bin = "$dir/repro";
my $cflags = "-O2 -D_GNU_SOURCE -DSYNC_READER_SLOTS=1 -I.";
my $log = `$cc $cflags -o $bin $src -lpthread 2>&1`;
