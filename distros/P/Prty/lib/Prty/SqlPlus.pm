...
sub script {
    my $class = shift;
    my $name = shift;
    my $sql = Prty::Unindent->trim(shift);
    # @_: @opt
    ...
    my $template = Prty::Unindent->string(q|
        __COMMENT__
    ...
    my $tpl = Prty::Template->new('text',\$template);

    my $comment = Prty::Unindent->string("
        -- NAME
        --     $name
    ");
    if ($description) {
        $description = Prty::Unindent->trim($description);
        $description =~ s/^/--     /mg;
        $comment .= "--\n-- DESCRIPTION\n$description\n";
    }
    if ($author) {
        $author = Prty::Unindent->trim($author);
        $author =~ s/^/--     /mg;
        $comment .= "--\n-- AUTHOR\n$author\n";
    }
    ...
    if ($beforeAndAfter) {
        $beforeAndAfter = Prty::Unindent->trim($beforeAndAfter);
        if ($before) {
            $before .= "\n";
        }
    ...
    if ($before) {
        $before = Prty::Unindent->trim($before);
        $sql =
            "-- ZUSTAND VORHER\n\n$before\n\n$sql";
    }
    if ($after) {
        $after = Prty::Unindent->trim($after);
        $sql =
            "$sql\n\n-- ZUSTAND NACHHER\n\n$after\n";
    }
    if ($commit) {
