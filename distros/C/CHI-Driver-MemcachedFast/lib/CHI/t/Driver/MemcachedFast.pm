package CHI::t::Driver::MemcachedFast;
use strict;
use warnings;
use CHI::Test;
...
sub new_cache_options {
    ...
    # CHI::Test::MemcachedFast::Driver defines get_keys for testing purposes
    return (
        $self->SUPER::new_cache_options(),
        driver       => undef,
        driver_class => 'CHI::Test::MemcachedFast::Driver',
        servers      => [$testaddr]
    );
}
...
sub test_get_keys : Test(1) {
    ...
    my $cache =
      $self->SUPER::new_cache( driver => 'MemcachedFast', servers => [$testaddr] );
    throws_ok(
        sub { $cache->get_keys() },
        qr/not supported/,
