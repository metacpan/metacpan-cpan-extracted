...
# (the file mode in new(), rows/cols in new_memfd(), fd in new_from_fd()) runs
...
# ref($obj) is the decoy ('Data::Fenwick2D::ShareQ', one character off so it
# is the same string length and lands in the same malloc size class). With
# the fix, the constructor re-reads ST(0) after all magic and blesses into
# Data::Fenwick2D::Shared.
#
# The assertion is on the specific symptom: the returned object must be
# blessed into Data::Fenwick2D::Shared AND be callable, not merely "no crash".
use strict;
use warnings;
use Test::More;
use Data::Fenwick2D::Shared;

my $CLASS = 'Data::Fenwick2D::Shared';
# Decoy: 'Data::Fenwick2D::ShareQ' - one character off $CLASS so it is the
# same string length and lands in the same malloc size class.
...
{ package ReallocClass;
  ...
  sub FETCH {
      ...
      $s->{decoy} = join '', 'Data::Fenwick2D', '::ShareQ';  # private PV: reuses the freed chunk
...
sub fresh_class { join '', 'Data::Fenwick2D', '::Shared' }

sub check_obj {
    my ($obj, $what) = @_;
    is ref($obj), $CLASS, "$what: blessed into $CLASS (not the decoy)"
        or diag "ref was: ", ref($obj);
    my $ok = eval { $obj->update(1, 1, 5); $obj->point(1, 1) == 5 };
...
# new(): magic on the optional file-mode argument runs after class capture.
{
    ...
    my $obj = $class->new(undef, 4, 3, $mode);
    check_obj($obj, 'new (mode magic)');
}

# new_memfd(): magic on rows runs in INPUT, after class capture.
{
    my $class = fresh_class();
    tie my $rows, 'ReallocClass', \$class, 4;
    my $obj = $class->new_memfd('magic', $rows, 3);
    check_obj($obj, 'new_memfd (rows magic)');
}

# new_from_fd(): magic on fd runs in INPUT, after class capture.
{
    my $donor = $CLASS->new_memfd('donor', 4, 3);
    my $fd = $donor->memfd;
