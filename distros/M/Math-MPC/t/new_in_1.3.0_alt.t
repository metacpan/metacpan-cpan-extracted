...
# mpc_agm. Testing of the ball functions, also new in mpc-1.3.0,
# is conducted in the (new) test scripts Radius.t and Ball.t.
...
if(MPC_VERSION < 66304) {
  ...
  $ok = 1
    if($op1 == Math::MPC->new(-0.3, 1.2));
  cmp_ok($ok, '==', 1, "'$op1' == " . Math::MPC->new(-0.3, 1.2));
  $ok = 0;
  #cmp_ok($op1, '==', Math::MPC->new(-0.3, 1.2), '\$op1 was not modified by Rmpc_eta_fund()');

  my $inex = Rmpc_agm($rop, $op1, $op2, MPC_RNDAA);
  $ok = 1
    if "$rop" eq '(2.7191494731957273 6.4237609338121771e-1)';
  cmp_ok($ok, '==', 1, "'$rop' eq '(2.7191494731957273 6.4237609338121771e-1)'");
  $ok = 0;
  ...
  $ok = 1
    if $re0 == $re1;
  cmp_ok($ok, '==', 1, "Rmpcr_agm RE1: '$re0' == '$re1'");
  $ok = 0;
  #cmp_ok($re0, '==', $re1, "Rmpcr_agm: first two reals are equivalent");

  $ok = 1
    if $re1 == $re2;
  cmp_ok($ok, '==', 1, "Rmpcr_agm RE2: '$re1' == '$re2'");
  $ok = 0;
  ...
  $ok = 1
    if $im0 == $im1;
  cmp_ok($ok, '==', 1, "Rmpcr_agm IM1: '$im0' == '$im1'");
  $ok = 0;
  #cmp_ok($im0, '==', $im1, "Rmpcr_agm: first two imaginaries are equivalent");

  $ok = 1
    if $im1 == $im2;
  cmp_ok($ok, '==', 1, "Rmpcr_agm IM2: '$im1' == '$im2'");
  $ok = 0;
