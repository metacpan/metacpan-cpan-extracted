...
sub list_code_package_revisions {
    ...
    my $connection = $self->{'connection'};
    my ( $http_status_code, $response_message ) =
      $connection->perform_iron_action( IO::Iron::IronWorker::Api::IRONWORKER_LIST_CODE_PACKAGE_REVISIONS(),
        { '{Code ID}' => $params{'id'}, } );
    my @revisions;
    foreach ( @{$response_message} ) {
        push @revisions, $_;
...
sub queue {
    ...
    my %params = validate( @_, {%validate_params} );
...
sub get_info_about_task {
    ...
    my $connection = $self->{'connection'};
    my ( $http_status_code, $response_message ) =
      $connection->perform_iron_action( IO::Iron::IronWorker::Api::IRONWORKER_GET_INFO_ABOUT_A_TASK(),
        { '{Task ID}' => $params{'id'}, } );
    $self->{'last_http_status_code'} = $http_status_code;
    my $info = $response_message;
...
sub scheduled_tasks {
    ...
    my $connection = $self->{'connection'};
    my ( $http_status_code, $response_message ) =
      $connection->perform_iron_action( IO::Iron::IronWorker::Api::IRONWORKER_LIST_SCHEDULED_TASKS(), {} );
    $self->{'last_http_status_code'} = $http_status_code;
    my @tasks;
    foreach ( @{$response_message} ) {
        $log->debugf( 'task info:%s', $_ );
...
sub get_info_about_scheduled_task {
    ...
    my $connection = $self->{'connection'};
    my ( $http_status_code, $response_message ) =
      $connection->perform_iron_action( IO::Iron::IronWorker::Api::IRONWORKER_GET_INFO_ABOUT_A_SCHEDULED_TASK(),
...
sub list_available_stacks {
    ...
    my $connection = $self->{'connection'};
    my ( $http_status_code, $response_message ) =
      $connection->perform_iron_action( IO::Iron::IronWorker::Api::IRONWORKER_LIST_OF_AVAILABLE_STACKS(), {} );
    $self->{'last_http_status_code'} = $http_status_code;
    my @stacks;
    foreach ( @{$response_message} ) {
        push @stacks, $_;
