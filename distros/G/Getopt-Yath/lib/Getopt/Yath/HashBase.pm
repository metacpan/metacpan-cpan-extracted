...
{
    no warnings 'once';
    $Getopt::Yath::HashBase::HB_VERSION = '0.015';
    *Getopt::Yath::HashBase::ATTR_SUBS = \%Object::HashBase::ATTR_SUBS;
    *Getopt::Yath::HashBase::ATTR_LIST = \%Object::HashBase::ATTR_LIST;
    *Getopt::Yath::HashBase::VERSION   = \%Object::HashBase::VERSION;
    *Getopt::Yath::HashBase::CAN_CACHE = \%Object::HashBase::CAN_CACHE;
}
...
sub do_import {
    ...
    my $ver = $Getopt::Yath::HashBase::HB_VERSION || $Getopt::Yath::HashBase::VERSION;
    $Getopt::Yath::HashBase::VERSION{$into} = $ver if !$Getopt::Yath::HashBase::VERSION{$into} || $Getopt::Yath::HashBase::VERSION{$into} > $ver;

    my $isa = _isa($into);
    my $attr_list = $Getopt::Yath::HashBase::ATTR_LIST{$into} ||= [];
    my $attr_subs = $Getopt::Yath::HashBase::ATTR_SUBS{$into} ||= {};
    ...
    if (my $have_new = $into->can('new')) {
        my $new_lookup = $Getopt::Yath::HashBase::NEW_LOOKUP //= {};
        $add_new = 0 unless $new_lookup->{$have_new};
    }

    my %subs = (
        ($add_new ? ($class->_build_new($into, \@pre_init, \@post_init)) : ()),
        (map %{$Getopt::Yath::HashBase::ATTR_SUBS{$_} || {}}, @{$isa}[1 .. $#$isa]),
...
sub attr_list {
    ...
    my @list = grep { !$seen{$_}++ } map {
        ...
        if (0.004 > ($Getopt::Yath::HashBase::VERSION{$_} || 0)) {
            Carp::carp("$_ uses an inlined version of Getopt::Yath::HashBase too old to support attr_list()");
        }
        else {
            my $list = $Getopt::Yath::HashBase::ATTR_LIST{$_};
            @out = $list ? @$list : ()
        }
...
sub _build_new {
    ...
    my $new = sub {
        ...
        $Getopt::Yath::HashBase::CAN_CACHE{$class} = $self->can('init')
            unless exists $Getopt::Yath::HashBase::CAN_CACHE{$class};

        $self->$_() for $_pre_init->();
        $self->init() if $Getopt::Yath::HashBase::CAN_CACHE{$class};
...
Getopt::Yath::HashBase - Build hash based classes.
...
Subclass it
    ...
    use base 'My::Class';
    use Getopt::Yath::HashBase qw/bub/;
...
It is also helpful for extending Getopt::Yath::HashBase.
...
declare it before loading L<Getopt::Yath::HashBase>.
...
B<Note:> Getopt::Yath::HashBase checks for an init using C<< $class->can('init') >>
...
Getopt::Yath::HashBase provides a function for retrieving a list of attributes for an
Getopt::Yath::HashBase class.

=over 4

=item @list = Getopt::Yath::HashBase::attr_list($class)

=item @list = $class->Getopt::Yath::HashBase::attr_list()
