package  perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src;
use strict;
use warnings;
...
=head1 NAME
 ...
 perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src  - A base class, implements  'src'  element from the perfSONAR_PS RelaxNG schema
  
=head1 DESCRIPTION

   Object representation of the src element.
   Object fields are:
    Scalar:     value, 
    Scalar:     type, 
    Scalar:     port, 
   
   The constructor accepts only single parameter, it could be a hashref to parameters hash  or DOM with  'src' element 
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src->new($DOM_Obj);
...
perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src->mk_accessors(perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src->show_fields('Public'));
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src';
...
=head2   getDOM ($) 
       ...
       accept parent DOM
       return src object DOM, generated from object contents 
...
sub getDOM {
    ...
    my $parent = shift; 
    my $logger  = get_logger( $CLASSPATH ); 
    my $src = getElement({name =>   $LOCALNAME, parent => $parent , ns => [$self->nsmap->mapname( $LOCALNAME )],
                             attributes => [
...
sub  querySQL {
    ...
    $query->{metaData}{ip_name_src}= [ 'perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src' ] if!(defined $query->{metaData}{ip_name_src}) || ref($query->{metaData}{ip_name_src});
    $query->{host}{ip_name}= [ 'perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src' ] if!(defined $query->{host}{ip_name}) || ref($query->{host}{ip_name});
    $query->{host}{ip_number}= [ 'perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src' ] if!(defined $query->{host}{ip_number}) || ref($query->{host}{ip_number});
    $query->{metaData}{ip_name_src}= [ 'perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src' ] if!(defined $query->{metaData}{ip_name_src}) || ref($query->{metaData}{ip_name_src});
    $query->{host}{ip_name}= [ 'perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src' ] if!(defined $query->{host}{ip_name}) || ref($query->{host}{ip_name});
    $query->{host}{ip_number}= [ 'perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src' ] if!(defined $query->{host}{ip_number}) || ref($query->{host}{ip_number});
    eval { 
            ...
            foreach my $entry (@{$defined_table{$table}}) {  
                if(ref($query->{$table}{$entry}) eq 'ARRAY') {
                    foreach my $classes (@{$query->{$table}{$entry}}) {  
                        if($classes && $classes eq 'perfSONAR_PS::Datatypes::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src' ) { 
                            if    ($self->value && ( (  ( ($self->type eq 'hostname')  && $entry eq 'ip_name_src')) || (  ( ($self->type eq 'hostname')  && $entry eq 'ip_name') or  ( ($self->type eq 'ipv4')  && $entry eq 'ip_number')) )) {
                                $query->{$table}{$entry} =  $self->value;
                                $logger->debug(" Got value for SQL query $table.$entry: " . $self->value);
                                last;  
                            }
                            elsif ($self->text && ( (  ( ($self->type eq 'hostname')  && $entry eq 'ip_name_src')) || (  ( ($self->type eq 'hostname')  && $entry eq 'ip_name') or  ( ($self->type eq 'ipv4')  && $entry eq 'ip_number')) )) {
...
=head2 merge
      ...
      merge with another src ( append + overwrite if exists )
...
sub merge {
    ...
    my $new_src = shift;
    my $logger  = get_logger( $CLASSPATH );  
    unless($new_src && blessed $new_src && $new_src->can("getDOM")) {
        $logger->error(" Please supply defined object of src  ");
        return;
    } 
    ...
    if($self->can($member_name)) {
        ...
        $logger->error(" This field $member_name,  found in supplied  src  is not supported by src class");
        return;
        }
    }
    return $self;
...
=head2  asString()
   ...
   returns XML string  representation of the  src object
...
=head2 registerNamespaces ()
   ...
   will parse all subelements and register all namepspaces within the src namespace
