...
=head3 Mounting Components and Class Names

Mount targets (and route handlers) accept more than coderefs: anything
L<PAGI::Utils/to_app> can coerce composes directly, compiled once at
mount time.
...
=item * C<$ctx> - L<PAGI::Context> - read the request via C<< $ctx->req >>, build
the response with C<< $ctx->json >> / C<< $ctx->html >> / C<< $ctx->text >> and
return it
...
WebSocket handlers receive C<($self, $ctx)>; call C<< $ctx->websocket >> for the channel:
...
Middleware methods receive C<($self, $ctx, $next)>. C<< await $next->() >> runs
the rest of the chain and returns the handler's response; return it (optionally
after decorating), or return your own response to short-circuit.
