...
$Lab::Instrument::HP34420A::VERSION = '3.899';
use v5.20;
    ...
    if ( $error !~ /\+0,/ ) {
        if ( $error =~ /^(\+[0-9]*)\,\"?(.*)\"?$/ ) {
            ...
            Lab::Exception::DeviceError->throw(
                error =>
                    "Reading the error status of the device failed in Instrument::HP34401A::get_error(). Something's going wrong here.\n",
            );
        }
    }
    else {
...
sub configure_voltage_dc_trigger {
    my $self = shift;
    my $tint = shift
        ; # integration time in sec, Default is 10PLC*pl_freq(), "MIN" = 0.02PLC , "MAX" = 200PLC
    my $range   = shift;    # in V, or "DEF"(Default), "MIN", "MAX"
    my $count   = shift;    # Measurment count, Default = 1
    my $delay   = shift;    # in seconds, Default = 'MIN'
    my $res_cmd = shift
        ; # Resolution. Decimal (not number of digits!). NOT VALID FOR AC MEASUREMENT
    ...
    if ( !defined($tint) ) {
        ...
        Lab::Exception::CorruptParameter->throw( error =>
                "Integration time has to be set to a positive value, 'MIN' or 'MAX' in HP34401A::configure_voltage_dc()\n"
        );
    }
    ...
    $self->write( "VOLT:DC:NPLC ${tint}",             error_check => 1 )
        if $res_cmd eq
        '';    # integration time implicitly set if resolution given
