...
sub decode_jwt 
{
    my %args = @_;
    my( $header, $payload );
    print( STDERR __PACKAGE__ . "::decode_jwt() [", __LINE__, "]: Token to decode is: $args{token}\n" ) if( $args{debug} );

    if( !$args{token} ) 
    {
        die( "JWT: missing token" );
    }
    elsif( $args{token} =~ /^([a-zA-Z0-9_-]+)=*\.([a-zA-Z0-9_-]*)=*\.([a-zA-Z0-9_-]+)=*\.([a-zA-Z0-9_-]+)=*\.([a-zA-Z0-9_-]+)=*$/ ) 
    {
        # JWE token (5 segments)
        print( STDERR __PACKAGE__ . "::decode_jwt() [", __LINE__, "]: Token is an encrypted token\n" ) if( $args{debug} );
        ( $header, $payload ) = _decode_jwe( $1, $2, $3, $4, $5, undef(), {}, {}, %args );
    }
    elsif( $args{token} =~ /^([a-zA-Z0-9_-]+)=*\.([a-zA-Z0-9_-]+)=*\.([a-zA-Z0-9_-]*)=*$/ ) 
    {
        # JWS token (3 segments)
        print( STDERR __PACKAGE__ . "::decode_jwt() [", __LINE__, "]: Token is a 3-segment token\n" ) if( $args{debug} );
        ( $header, $payload ) = _decode_jws( $1, $2, $3, {}, %args );
    }
    elsif( $args{token} =~ /^\s*\{.*?\}\s*$/s ) 
    {
        print( STDERR __PACKAGE__ . "::decode_jwt() [", __LINE__, "]: Token provided is wrapped in json, decoding first.\n" ) if( $args{debug} );
        ...
        {
            die( "JWT: unsupported JWS/JWT JSON Serialization" );
        }
    }
    else 
    {
        print( STDERR __PACKAGE__ . "::decode_jwt() [", __LINE__, "]: Unknown token, dying...\n" ) if( $args{debug} );
        die( "JWT: invalid token format" );
    }
    print( STDERR __PACKAGE__ . "::decode_jwt() [", __LINE__, "]: Returning header and payload\n" ) if( $args{debug} );
...
sub _kid_lookup 
{
    ...
    die( "JWT: key type '$found->{kty}' cannot be used with alg '$alg'" );
}
...
sub _verify_claims 
{
    ...
    print( STDERR __PACKAGE__ . "::_verify_claims() [", __LINE__, "]: Ignoring claim verification, nothing to do here, returning\n" ) if( $args{debug} && $args{ignore_claims} );
    ...
    {
        print( STDERR __PACKAGE__ . "::_verify_claims() [", __LINE__, "]: Checking expiration $payload->{exp}\n" ) if( $args{debug} );
        if( !defined( $args{verify_exp} ) || $args{verify_exp} == 1 ) 
        {
            die( "JWT: exp claim check failed ($payload->{exp}/$leeway vs. $now)" ) if( $payload->{exp} + $leeway <= $now );
        }
    }
    elsif( $args{verify_exp} && $args{verify_exp} == 1 ) 
    {
        die( "JWT: exp claim required but missing" );
    }

    ### nbf
    if( defined( $payload->{nbf} ) ) 
    {
        print( STDERR __PACKAGE__ . "::_verify_claims() [", __LINE__, "]: Checking mbf $payload->{nbf}\n" ) if( $args{debug} );
        if( !defined( $args{verify_nbf} ) || $args{verify_nbf} == 1 ) 
        {
            die( "JWT: nbf claim check failed ($payload->{nbf}/$leeway vs. $now)" ) if( $payload->{nbf} - $leeway > $now );
        }
    }
    elsif( $args{verify_nbf} && $args{verify_nbf} == 1 ) 
    {
        die( "JWT: nbf claim required but missing" );
    }

    ### iat
    #default (non existing verify_iat) == no iat check
    if( exists( $args{verify_iat} ) ) 
    {
        if( defined( $payload->{iat} ) ) 
        {
            print( STDERR __PACKAGE__ . "::_verify_claims() [", __LINE__, "]: Checking iat $payload->{iat}\n" ) if( $args{debug} );
            if( !defined( $args{verify_iat} ) || $args{verify_iat} == 1 ) 
            {
                die( "JWT: iat claim check failed ($payload->{iat}/$leeway vs. $now)" ) if( $payload->{iat} - $leeway > $now );
            }
        }
        elsif( $args{verify_iat} && $args{verify_iat} == 1 ) 
        {
            die( "JWT: iat claim required but missing" );
        }
    }
    ...
    if( exists( $args{verify_iss} ) ) 
    {
        ...
        {
            print( STDERR __PACKAGE__ . "::_verify_claims() [", __LINE__, "]: Checking issuer $payload->{iss}\n" ) if( $args{debug} );
            if( ref( $args{verify_iss} ) eq 'Regexp' ) 
            {
                die( "JWT: iss claim re check failed" ) unless( $payload->{iss} =~ $args{verify_iss} );
            }
            elsif( ref( $args{verify_iss} ) eq 'CODE' ) 
            {
                die( "JWT: iss claim check failed" ) unless( $args{verify_iss}->( $payload->{iss} ) );
            }
            else 
            {
                die( "JWT: verify_iss must be Regexp or CODE" );
            }
        }
        elsif( $args{verify_iss} ) 
        {
            die( "JWT: iss claim required but missing" );
        }
    }
    ...
    if( exists( $args{verify_sub} ) )
    {
        ...
        {
            print( STDERR __PACKAGE__ . "::_verify_claims() [", __LINE__, "]: Checking sub $payload->{sub}\n" ) if( $args{debug} );
            if( ref( $args{verify_sub} ) eq 'Regexp' ) 
            {
                die( "JWT: sub claim re check failed" ) unless( $payload->{sub} =~ $args{verify_sub} );
            }
            elsif( ref( $args{verify_sub} ) eq 'CODE' ) 
            {
                die( "JWT: sub claim check failed" ) unless( $args{verify_sub}->( $payload->{sub} ) );
            }
            else 
            {
                die( "JWT: verify_sub must be Regexp or CODE" );
            }
        }
        elsif( $args{verify_sub} ) 
        {
            die( "JWT: sub claim required but missing" );
        }
    }
    ...
    if( exists( $args{verify_aud} ) ) 
    {
        ...
        {
            print( STDERR __PACKAGE__ . "::_verify_claims() [", __LINE__, "]: Checking aud $payload->{aud}\n" ) if( $args{debug} );
            if( ref( $args{verify_aud} ) eq 'Regexp' ) 
            {
                die( "JWT: aud claim re check failed" ) unless( $payload->{aud} =~ $args{verify_aud} );
            }
            elsif( ref( $args{verify_aud} ) eq 'CODE' ) 
            {
                die( "JWT: aud claim check failed" ) unless( $args{verify_aud}->( $payload->{aud} ) );
            }
            else 
            {
                die( "JWT: verify_aud must be Regexp or CODE" );
            }
        }
        elsif( $args{verify_aud} ) 
        {
            die( "JWT: aud claim required but missing" );
        }
    }
    ...
    if( exists( $args{verify_jti} ) ) 
    {
        ...
        {
            print( STDERR __PACKAGE__ . "::_verify_claims() [", __LINE__, "]: Checking jti $payload->{jti}\n" ) if( $args{debug} );
            if( ref( $args{verify_jti} ) eq 'Regexp' ) 
            {
                die( "JWT: jti claim re check failed" ) unless( $payload->{jti} =~ $args{verify_jti} );
            }
            elsif( ref( $args{verify_jti} ) eq 'CODE' ) 
            {
                die( "JWT: jti claim check failed" ) unless( $args{verify_jti}->( $payload->{jti} ) );
            }
            else 
            {
                die( "JWT: verify_jti must be Regexp or CODE" );
            }
        }
        elsif( $args{verify_jti} ) 
        {
            die( "JWT: jti claim required but missing" );
        }
    }
}
...
sub _payload_zip 
{
    ...
    if( $zip[0] eq 'deflate' ) 
    {
        ...
        my $output = '';
        $d->deflate( $payload, $output ) == Z_OK or die( "JWT: deflate failed" );
        $d->flush( $output ) == Z_OK             or die( "JWT: deflate/flush failed" );
        die( "JWT: deflate/output failed" ) unless( $output );
        $payload = $output;
    }
    else 
    {
        die( "JWT: unknown zip method '$zip[0]'" );
    }
    return( $payload );
}
...
sub _payload_dec 
{
    ...
    if( $decode_payload ) 
    {
        die( "JWT: payload not a valid JSON" ) unless( $de );
        return( $de );
    }
    else 
    {
...
sub _encode_jwe 
{
    ...
    # compress payload
    # may set some header items
    $payload = _payload_zip( $payload, $header, $args{zip} ) if( $args{zip} );
    # prepare header
    $header->{alg} = $alg;
    $header->{enc} = $enc;
    # key
    die( "JWE: missing 'key'" ) if( !$args{key} );
...
sub _decode_jwe 
{
    ...
    die( "JWE: invalid header part" ) if( $b64u_header && !$header );
    die( "JWE: invalid ecek part" )   if( $b64u_ecek   && !$ecek );
    die( "JWE: invalid ct part" )     if( $b64u_ct     && !$ct );
    die( "JWE: invalid iv part" )     if( $b64u_iv     && !$iv );
    die( "JWE: invalid tag part" )    if( $b64u_tag    && !$tag );
    ...
    elsif( exists( $args{kid_keys} ) ) 
    {
        ...
        die( "JWE: kid_keys lookup failed" ) if( !defined( $k ) );
        $key = $k;
    }
    die( "JWE: missing key" ) if( !defined( $key ) );

    my $aa = $args{accepted_alg};
    if( ref( $aa ) eq 'Regexp' ) 
    {
        die( "JWE: alg '$header->{alg}' does not match accepted_alg" ) if( $header->{alg} !~ $aa );
    }
    elsif( $aa && ( ref( $aa ) eq 'ARRAY' || !ref( $aa ) ) ) 
    {
        my %acca = ref( $aa ) ? map{ $_ => 1 } @$aa : ( $aa => 1 );
        die( "JWE: alg '$header->{alg}' not in accepted_alg" ) if( !$acca{$header->{alg}} );
    }

    my $ae = $args{accepted_enc};
    if( ref( $ae ) eq 'Regexp' ) 
    {
        die( "JWE: enc '$header->{enc}' does not match accepted_enc" ) if( $header->{enc} !~ $ae );
    }
    elsif( $ae && ( ref( $ae ) eq 'ARRAY' || !ref( $ae ) ) ) 
    {
        my %acce = ref( $ae ) ? map{ $_ => 1 } @$ae : ( $ae => 1 );
        die( "JWE: enc '$header->{enc}' not in accepted_enc" ) if( !$acce{$header->{enc}} );
    }
    ...
    # croaks on error
    _verify_claims( $payload, %args ) if( ref( $payload ) eq 'HASH' );
...
sub _decode_jws 
{
    ...
    my $header = _b64u_to_hash( $b64u_header );
    print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Decoded header is: ", Dumper( $header ), "\n" ) if( $args{debug} );
    die( "JWS: invalid header part" ) if( $b64u_header && !$header );
    $unprotected_header = {} if( ref( $unprotected_header ) ne 'HASH' );

    if( !$args{ignore_signature} ) 
    {
        my $alg = $header->{alg};
        print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Checking signature with algorithm '$alg'\n" ) if( $args{debug} );
        die( "JWS: missing header 'alg'" ) unless( $alg );
        die( "JWS: alg 'none' not allowed" ) if( $alg eq 'none' && !$args{allow_none} );
        die( "JWS: alg 'none' expects no signature" ) if( $alg eq 'none' && defined( $b64u_sig ) && length( $b64u_sig ) > 0 );

        my $aa = $args{accepted_alg};
        print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Accepted algorithms are: ", Dumper( $aa ), "\n" ) if( $args{debug} );
        if( ref( $aa ) eq 'Regexp' ) 
        {
            die( "JWS: alg '$alg' does not match accepted_alg" ) if( $alg !~ $aa );
        }
        elsif( ref( $aa ) eq 'ARRAY' ) 
        {
            my %acca = map { $_ => 1 } @$aa;
            die( "JWS: alg '$alg' not in accepted_alg" ) if( !$acca{$alg} );
        }
        elsif( defined( $aa ) ) 
        {
            die( "JWS: alg '$alg' not accepted_alg" ) if( $aa ne $alg );
        }
        ...
        if( $alg ne 'none' ) 
        {
            ...
            {
                print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Using kid_keys of ", Dumper( $args{kid_keys} ), "\n" ) if( $args{debug} );
                # BEWARE: stricter approach since 0.023
                # when 'kid_keys' specified it croaks if header doesn't contain 'kid' value or if 'kid' wasn't found in 'kid_keys'
                my $kid = exists( $header->{kid} ) ? $header->{kid} : $unprotected_header->{kid};
                print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Kid found in header '$kid', looking it up\n" ) if( $args{debug} );
                my $k = _kid_lookup( $kid, $args{kid_keys}, $alg );
                print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Could not find kid among keys provided.\n" ) if( $args{debug} && !defined( $k ) );
                die( "JWS: kid_keys lookup failed" ) if( !defined( $k ) );
                $key = $k;
            }
            elsif( $args{key_from_jwk_header} ) 
            {
                ...
                my $k = $header->{jwk};
                die( "JWS: jwk header does not contain a key" ) if( !defined( $k ) || ref( $k ) ne 'HASH' || !defined( $k->{kty} ) );
                die( "JWS: jwk header allowed only for RSA/ECDSA" ) if( $alg !~ /^(RS|PS|ES)/ || $k->{kty} !~ /^(RSA|EC)$/ );
                die( "JWS: jwk header must be a public key" ) if( $k->{d} || $k->{p} || $k->{q} || $k->{dp} || $k->{dq} || $k->{qi} );
                $key = $k;
            }
            print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Key set to '$key'\n" ) if( $args{debug} );
            die( "JWS: missing key" ) if( !defined( $key ) );

            print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Verifying the JWS\n" ) if( $args{debug} );
            my $valid = _verify_jws( $b64u_header, $b64u_payload, $b64u_sig, $alg, $key );
            print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: JWS found to be invalid!\n" ) if( $args{debug} && !$valid );
            die( "JWS: invalid signature" ) if( !$valid );
        }
    }
    print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Decoding payload...\n" ) if( $args{debug} );
    my $payload = decode_b64u( $b64u_payload );
    print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Decoded payload is: ", Dumper( $payload ), "\n" ) if( $args{debug} );
    die( "JWS: invalid payload part" ) if( $b64u_payload && !$payload );
    $payload = _payload_unzip( $payload, $header->{zip} ) if( $header->{zip} );
    $payload = _payload_dec( $payload, $args{decode_payload} );
    # croaks on error
    print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Verify claim of payload\n" ) if( $args{debug} );
    _verify_claims( $payload, %args ) if( ref( $payload ) eq 'HASH' );
    print( STDERR __PACKAGE__ . "::_decode_jws() [", __LINE__, "]: Returning header and payload verified\n" ) if( $args{debug} );
    # merge headers
    $header = { %$unprotected_header, %$header };
    return( $header, $payload );
}
...
You can export selected functions:
  ...
  use Net::API::REST::JWT qw(decode_jwt encode_jwt);

Or all of them at once:

  use Net::API::REST::JWT ':all';
