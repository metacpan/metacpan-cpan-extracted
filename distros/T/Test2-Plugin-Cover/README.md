...
exclude a sibling `library`. A relative path is resolved against the coverage
...
- $event = $class->report(%options)
    ...
    The event's `coverage` facet carries the processed file data from `data()`
    under `files`, and under `root` the canonical absolute directory a relative
