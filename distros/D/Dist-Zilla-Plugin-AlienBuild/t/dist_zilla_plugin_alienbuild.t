...
subtest 'mm with specific location' => sub {

  skip_all 'Test requires Dist::Zilla::Plugin::MakeMaker::Custom'
    unless eval q{ use Dist::Zilla::Plugin::MakeMaker::Custom; 1 };

  my $tzil = Builder->from_config({ dist_root => 'corpus/Alien-Foo1-New' }, {
    add_files => {
      'source/dist.ini' => simple_ini(
        { name => 'Alien-Foo1' },
        [ 'GatherDir'  => {} ],
        [ 'MakeMaker::Custom'  => { eumm_version => '6.64', } ],
        [ 'MetaJSON'   => {} ],
        [ 'AlienBuild' => { eumm_hash_var => '%args' } ],
      ),
    },
  });
...
subtest 'mm reprocessed' => sub {

  skip_all 'Test requires Dist::Zilla::Plugin::MakeMaker::Custom'
    unless eval q{ use Dist::Zilla::Plugin::MakeMaker::Custom; 1 };

  my $tzil = Builder->from_config({ dist_root => 'corpus/Alien-Foo1-Previous' }, {
    add_files => {
      'source/dist.ini' => simple_ini(
        { name => 'Alien-Foo1' },
        [ 'GatherDir'  => {} ],
