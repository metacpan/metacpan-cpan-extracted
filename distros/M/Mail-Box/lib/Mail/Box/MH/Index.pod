...
The C<$file> which is used to store the headers of all the e-mails for
...
Append C<@messages> to the index file.

=item $obj-E<gt>B<get>($msgfile)

Look if there is header info for the specified C<$msgfile>.  The filename
...
Write an index file containing the headers specified C<@messages>
...
=item Error: MH index requires a filename.
