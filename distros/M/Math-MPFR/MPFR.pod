...
=head1 FUNCTIONS
   ...
   $nv = atonv($str); # Needs mpfr-3.1.6 or later
    ...
    NOTE: ORIGINALLY TOOK 2 ARGUMENTS - NOW TAKES ONLY 1.

   $str = nvtoa($nv);
    Requires at least version 4.0 of the mpfr library.
    ...
    *minimum* number of digits such that the condition ($str == $nv)
    is true.
    NOTE 1: Perl versions prior to 5.29.4, or subsequent versions that
            don't define $Config{d_strtod} (or, respectively,
            $Config{d_strtold} for-Duselongdouble builds) are buggy in
            their assignment of values to the NV. The nvtoa() function
            looks only at the value that was passed to it - which,
            with such buggy perls, may differ from the intended value.
            When it matters, assign with atonv().
