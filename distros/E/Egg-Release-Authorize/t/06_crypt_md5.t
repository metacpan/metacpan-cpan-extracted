...
if ($@) {
	...
	if ($@) {
		plan skip_all=> "Digest::MD5 is not installed."
	} else {
		test();
	}
}
...
my $psw    = '%test%';
my $salt   = '12345';
my $passwd = Digest::MD5::md5_hex($psw. $salt);
...
isa_ok $a, 'Egg::Model::Auth::Crypt::MD5';
...
value: |
  ...
  __PACKAGE__->setup_api( File => 'Crypt::MD5' );
