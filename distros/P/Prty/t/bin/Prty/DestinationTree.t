...
package Prty::DestinationTree::Test;
...
sub test_loadClass : Init(1) {
    shift->useOk('Prty::DestinationTree');
}
...
sub unit_setup : Setup(2) Group(^unit_) {
    ...
    Prty::Path->mkdir("$fixDir/a",-recursive=>1);
    Prty::Path->write("$fixDir/a/hello.c","main(){}\n");
    Prty::Path->write("$fixDir/a/hello.o","xyz\n");
    $self->ok(-d $fixDir,'Fixture-Verzeichnis erzeugt');

    my $tree = Prty::DestinationTree->new("$fixDir/a",
        -exclude=>qr/\.o$/,
        -quiet=>1,
    );
    $self->is(ref($tree),'Prty::DestinationTree');
...
package main;
Prty::DestinationTree::Test->runTests;
