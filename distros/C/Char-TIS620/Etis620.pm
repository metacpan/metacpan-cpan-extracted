...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Etis620::index($name, '::') == -1) && (Etis620::index($name, "'") == -1)) {
            my $pkg;
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $TIS620::re_t=%s; while($TIS620::re_t =~ %s){%s local $^W=0; local $TIS620::re_r=%s; %s$TIS620::re_t="$`$TIS620::re_r$'"; pos($TIS620::re_t)=length "$`$TIS620::re_r"; } return $TIS620::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$TIS620::re_r=CORE::eval $TIS620::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $TIS620::re_r=%s; %s"%s$TIS620::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $TIS620::re_n=0; while(%s =~ %s){%s local $^W=0; local $TIS620::re_r=%s; %s%s="$`$TIS620::re_r$'"; pos(%s)=length "$`$TIS620::re_r"; $TIS620::re_n++} return %s$TIS620::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $TIS620::re_r=%s; %s%s="%s$TIS620::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $TIS620::re_r=%s; %s%s="%s$TIS620::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$TIS620::re_r=CORE::eval $TIS620::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
