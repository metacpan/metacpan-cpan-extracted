...
#                                                                              #
# Description:                                                                 #
# Get the information about rewards from the Tron blockchain using the         #
# full-node HTTP Tron API that a witness or a user has not yet withdrawn.      #
#                                                                              #
...
CryptoTron::GetReward - Perl extension for use with the blockchain of the crypto coin Tron.
...
The module requests the information about rewards from the Tron blockchain
using the full-node HTTP Tron API from the Tron developer that a witness or
a user has not yet withdrawn. The module is designed for use with Tron
Mainnet. For HTTP requests the methods C<POST> or C<GET> can be used in
general. For the method C<GetAccount> the needed method is C<POST>. The
payload of the HTTP request consists of the key 'address' and the key
...
parsing of the reward information is done by a separate module.
...
=head2 Example 2
  ...
  # Request the account info from the mainnet.
  my $response = GetReward({PublicKey => $PublicKeyBase58});
