...
B<Greple> B<xlate> 模块会查找目标文本块并将其替换为翻译后的文本。主要引擎是 GPT-5.5 (F<llm/gpt5.pm>)，它会调用 L<llm|https://llm.datasette.io/> 命令； DeepL (F<deepl.pm>) 以及基于 B<gpty> 的旧版引擎也包含在内。
...
指定要发送给翻译引擎的自定义提示词。此选项适用于 LLM 引擎（C<gpt3>、C<gpt4o>、C<gpt5>），但不适用于 DeepL。 您可以通过向 AI 模型提供具体指令来自定义翻译行为。如果提示语中包含 C<%s>，它将被目标语言名称替换。
