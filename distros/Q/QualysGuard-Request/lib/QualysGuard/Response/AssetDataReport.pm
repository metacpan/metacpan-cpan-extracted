package QualysGuard::Response::AssetDataReport;
...
sub new {
    ...
    if ( $self->exists('/ASSET_DATA_REPORT/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/ASSET_DATA_REPORT/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/ASSET_DATA_REPORT/ERROR');
...
QualysGuard::Response::AssetDataReport 
