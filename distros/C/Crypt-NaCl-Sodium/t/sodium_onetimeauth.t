
use strict;
use warnings;
use Test::More;


use Crypt::NaCl::Sodium qw(:utils);

my $rs = join('', map { chr($_) }
        0xee, 0xa6, 0xa7, 0x25, 0x1c, 0x1e, 0x72, 0x91, 0x6d, 0x11, 0xc2,
        0xcb, 0x21, 0x4d, 0x3c, 0x25, 0x25, 0x39, 0x12, 0x1d, 0x8e, 0x23,
        0x4e, 0x65, 0x2d, 0x65, 0x1f, 0xa4, 0xc8, 0xcf, 0xf8, 0x80
);

my $c = join('', map { chr($_) }
        
);

my $exp = join('', map { chr($_) }
     0xf3,0xff,0xc7,0x70,0x3f,0x94,0x00,0xe5
    ,0x2a,0x7d,0xfb,0x4b,0x3d,0x33,0x05,0xd9
);

my $crypto_onetimeauth = Crypt::NaCl::Sodium->onetimeauth();

my $mac = $crypto_onetimeauth->mac( $c, $rs );

is(bin2hex($mac), bin2hex($exp), "mac as expected");

ok( $crypto_onetimeauth->verify( $mac, $c, $rs ), "...and verified");

my $hasher = $crypto_onetimeauth->init($rs);
$hasher->update( substr($c, 0, 100) );
$hasher->update( substr($c, 100) );
my $final = $hasher->final();

is(bin2hex($final), bin2hex($exp), "multi-part mac as expected");
ok( $crypto_onetimeauth->verify( $final, $c, $rs ), "...and verified");

done_testing();
