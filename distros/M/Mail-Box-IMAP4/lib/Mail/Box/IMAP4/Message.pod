...
speed.  This has the same dangers as setting C<write_labels> to C<false>.
...
Extends L<"Attributes" in Mail::Box::Net::Message|Mail::Box::Net::Message/"Attributes">.
...
Inherited, see L<Mail::Box::Net::Message/"Attributes">
...
With only one argument, the value related to C<$label> is returned.  With
more that one argument, the list is interpreted a label-value C<PAIRS>
to be set.
...
message.  The C<$info> request is passed to L<Mail::Box::IMAP4::fetch()|Mail::Box::IMAP4/"Internals">.
Without C<$info>, C<ALL> information is retrieved and returned as a HASH.
...
The C<$imap> argument is a Mail::IMAPClient which has the right folder
already selected.
...
=head4 » Example: of label translations
...
=item Error: unable to read delayed body for $msgid.

Cast by C<loadBody()>

=item Error: unable to read delayed head for $msgid.
...
This module is part of Mail-Box-IMAP4 version 4.000,
built on December 10, 2025. Website: F<http://perl.overmeer.net/CPAN/>
