...
perl $0 Big5-HKSCS_script.pl > Escaped_script.pl.e
END
    }

    print Big5HKSCS::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (Ebig5hkscs::e("$filename.e")) {
        if (exists $ENV{'CHAR_DEBUG'}) {
            Ebig5hkscs::unlink "$filename.e";
        }
        elsif (Ebig5hkscs::z("$filename.e")) {
            Ebig5hkscs::unlink "$filename.e";
        }
        else {
    ...
    my $fh = gensym();
    Ebig5hkscs::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Big5HKSCS::escape_script {
    ...
    # read Big5-HKSCS script
    my $fh = gensym();
    Ebig5hkscs::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Ebig5hkscs(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ebig5hkscs '%s.0'; # 'quote' for perl5.6\n", $Big5HKSCS::VERSION); # require run-time routines version

        # use Big5HKSCS version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Big5HKSCS ) (?>\s*) ([^\x81-\xFE;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Big5HKSCS::VERSION) {
                    my @file = grep -e, map {qq{$_/Big5HKSCS.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Big5HKSCS.pm $version, but @{[__FILE__]} is version $Big5HKSCS::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Ebig5hkscs.pm $version, but \\\$Ebig5hkscs::VERSION is \$Ebig5hkscs::VERSION" if \$Ebig5hkscs::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Big5HKSCS::VERSION) {
                    die "Script $0 required Big5HKSCS.pm $version, but @{[__FILE__]} is only version $Big5HKSCS::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Ebig5hkscs::function_ord     = 'Big5HKSCS::ord'     if /\A ord \z/oxms;
                    $Ebig5hkscs::function_ord_    = 'Big5HKSCS::ord_'    if /\A ord \z/oxms;
                    $Ebig5hkscs::function_reverse = 'Big5HKSCS::reverse' if /\A reverse \z/oxms;
                    $Ebig5hkscs::function_getc    = 'Big5HKSCS::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Big5HKSCS::escape();
...
=item * You'll have to download Big5HKSCS.pm and Ebig5hkscs.pm and put it in your perl lib directory.

=item * You'll need to write "use Big5HKSCS;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Big5HKSCS;
  use Big5HKSCS ver.sion;             --- require minimum version
  use Big5HKSCS ver.sion.0;           --- expects version (match or die)
  use Big5HKSCS qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Big5HKSCS ver.sion qw(ord reverse getc);
  use Big5HKSCS ver.sion.0 qw(ord reverse getc);

  # "no Big5HKSCS;" not supported

  or

  $ perl Big5HKSCS.pm Big5-HKSCS_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Big5-HKSCS_script.pl  --- script written in Big5-HKSCS
...
Big5HKSCS software is "middleware" between perl interpreter and your Perl script
written in Big5-HKSCS.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Big5-HKSCS, Perl3 or later
can treat Big5-HKSCS as binary data.

By "use Big5HKSCS;", it automatically interpret your script as Big5-HKSCS. The various
functions of perl including a regular expression can treat Big5-HKSCS now.
...
This software is a source code filter to escape Perl script encoded by Big5-HKSCS
...
=item * Remains One Language Handling Raw Big5-HKSCS, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Big5HKSCS.pm, Ebig5hkscs.pm
  +---------------------------------------+
...
import of Ebig5hkscs.pm works well.
...
=head1 Software Composition
   ...
   Big5HKSCS.pm               --- source code filter to escape Big5-HKSCS
   Ebig5hkscs.pm              --- run-time routines for Big5HKSCS.pm
...
You need write 'use Big5HKSCS;' in your script.
...
=head1 Escaping Multiple-Octet Code (Big5HKSCS software provides)
...
subroutines.
  ...
  lstat       Ebig5hkscs::lstat     Character
  opendir     Ebig5hkscs::opendir   Character
  stat        Ebig5hkscs::stat      Character
  unlink      Ebig5hkscs::unlink    Character
  chdir       Ebig5hkscs::chdir     Character
  do          Ebig5hkscs::do        Character
  require     Ebig5hkscs::require   Character
  -------------------------------------------
...
=item * Ordinal Value of Character
  ...
  $ord = Big5HKSCS::ord($string);

  This subroutine returns the numeric value (ASCII or Big5-HKSCS character) of the
  first character of $string, not Unicode. If $string is omitted, it uses $_.
  The return value is always unsigned.

  If you import ord "use Big5HKSCS qw(ord);", ord of your script will be rewritten in
  Big5HKSCS::ord. Big5HKSCS::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = Big5HKSCS::reverse(@list);
  $reverse = Big5HKSCS::reverse(@list);
  ...
  If you import reverse "use Big5HKSCS qw(reverse);", reverse of your script will be
  rewritten in Big5HKSCS::reverse. Big5HKSCS::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  $getc = Big5HKSCS::getc(FILEHANDLE);
  $getc = Big5HKSCS::getc($filehandle);
  $getc = Big5HKSCS::getc;
...
=item * Length by Big5-HKSCS Character

  $length = Big5HKSCS::length($string);
  $length = Big5HKSCS::length();

  This subroutine returns the length in characters (programmer-visible characters)
  of the scalar value $string. If $string is omitted, it returns the Big5HKSCS::length
  of $_.

  Do not try to use Big5HKSCS::length to find the size of an array or hash. Use scalar
...
=item * Substr by Big5-HKSCS Character

  $substr = Big5HKSCS::substr($string,$offset,$length,$replacement);
  $substr = Big5HKSCS::substr($string,$offset,$length);
  $substr = Big5HKSCS::substr($string,$offset);
  ...
  my $color  = Big5HKSCS::substr $s, 4, 5;      # black
  my $middle = Big5HKSCS::substr $s, 4, -11;    # black cat climbed the
  my $end    = Big5HKSCS::substr $s, 14;        # climbed the green tree
  my $tail   = Big5HKSCS::substr $s, -4;        # tree
  my $z      = Big5HKSCS::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Big5HKSCS::substr() subroutine as an
  ...
  end of the string, Big5HKSCS::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Big5HKSCS::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Big5HKSCS::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Big5HKSCS::substr $name, 7;      # returns undef, with warning
  Big5HKSCS::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Big5HKSCS::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Big5HKSCS::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Big5HKSCS::substr() acts
...
=item * Index by Big5-HKSCS Character

  $index = Big5HKSCS::index($string,$substring,$offset);
  $index = Big5HKSCS::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Big5HKSCS::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }

=item * Rindex by Big5-HKSCS Character

  $rindex = Big5HKSCS::rindex($string,$substring,$offset);
  $rindex = Big5HKSCS::rindex($string,$substring);

  This subroutine works just like Big5HKSCS::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Big5HKSCS::index).
  ...
  $pos = Big5HKSCS::length($string);
  while (($pos = Big5HKSCS::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }

=item * Eval Big5-HKSCS Script

  $eval = Big5HKSCS::eval { block };
  $eval = Big5HKSCS::eval $expr;
  $eval = Big5HKSCS::eval;

  The Big5HKSCS::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Big5HKSCS::eval { block }
  and Big5HKSCS::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Big5HKSCS::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Big5HKSCS::eval, the value returned from an Big5HKSCS::eval is
  ...
  Big5HKSCS::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Big5HKSCS::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Big5HKSCS::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Big5HKSCS::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Big5HKSCS::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Big5HKSCS::eval cannot trap syntax errors.

  The Big5HKSCS::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Big5HKSCS::eval is complete, as do any subroutine or format definitions.
  The code of the Big5HKSCS::eval is treated as a block, so any locally scoped
  variables declared within the Big5HKSCS::eval last only until the Big5HKSCS::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Big5HKSCS::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Big5HKSCS::eval { block }    eval { block }
  Big5HKSCS::eval $expr        eval Big5HKSCS::escape $expr
  Big5HKSCS::eval              eval Big5HKSCS::escape
  -------------------------------------------------

  To tell the truth, the subroutine Big5HKSCS::eval does not exist. If it exists,
  you will troubled, when Big5HKSCS::eval has a parameter that is single quoted
  string included my variables. Big5HKSCS::escape is a subroutine that makes Perl
  ...
  Since Big5HKSCS::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Big5HKSCS::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Big5HKSCS::eval $expr and Big5HKSCS::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Big5HKSCS::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Big5HKSCS::eval { $answer = }; # WRONG

      # a runtime syntax error
      Big5HKSCS::eval '$answer =';   # sets $@
  ...
  Using the Big5HKSCS::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Big5HKSCS::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          Big5HKSCS::eval { die "foo lives here" };
  ...
  With an Big5HKSCS::eval, you should be especially careful to remember what's being
  looked at when:

      Big5HKSCS::eval $x;        # CASE 1
      Big5HKSCS::eval "$x";      # CASE 2

      Big5HKSCS::eval '$x';      # CASE 3
      Big5HKSCS::eval { $x };    # CASE 4

      Big5HKSCS::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  a temporary is required if you want to mask some but not all errors:
      ...
      # alter $@ on nefarious repugnancy only
      {
          my $e;
          {
              ...
              Big5HKSCS::eval { test_repugnancy() };
  ...
  The block of Big5HKSCS::eval { block } does not count as a loop, so the loop
...
=item * Filename Globbing
  ...
  This operator is implemented via the Ebig5hkscs::glob() subroutine. See Ebig5hkscs::glob
  of Ebig5hkscs.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Big5HKSCS qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Big5HKSCS qw(reverse);" exists or not.
...
=head1 Yada Yada Operator (Big5HKSCS software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Big5HKSCS software provides)

Big5HKSCS software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Ebig5hkscs.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Big5HKSCS software and
...
=item * Big5HKSCS::substr as Lvalue

If Perl version is older than 5.14, Big5HKSCS::substr differs from CORE::substr, and
...
Big5HKSCS::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Ebig5hkscs::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${`}            Ebig5hkscs::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Ebig5hkscs::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Ebig5hkscs::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Ebig5hkscs::MATCH()       $1
  ${&}            Ebig5hkscs::MATCH()       $1
  $MATCH          Ebig5hkscs::MATCH()       $1
  ${^MATCH}       Ebig5hkscs::MATCH()       $1
  $'              $'                   $'
  ${'}            ${'}                 $'
  $POSTMATCH      Ebig5hkscs::POSTMATCH()   $'
  ${^POSTMATCH}   Ebig5hkscs::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
Big5HKSCS software remains one language and one interpreter.
