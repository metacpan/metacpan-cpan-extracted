...
# Ported from OrePAN2 0.54 t/04_repository.t. Exercises the full
# Repository built by hand off Class::Accessor::Fast: construction (eager
# builders for cache/indexer/injector), inject, make_index, and gc (both
# default-unlink and callback forms). This is the regression net for the
# Moo -> CAF conversion of Repository and Cache.
#
# File::Touch (upstream) replaced with a plain open to avoid a test dep.

sub _touch {
    my ($path) = @_;
    open my $fh, '>', $path or die "touch $path: $!";
    close $fh;
    return;
}
...

{
    ...
    $repo->gc(
        sub { my $file = shift; unlink $file; diag "unlinked $file"; } );
    test_repo($tmpdir);
}
...
sub make_repo {
    ...
    $repo->make_index();
    _touch("$tmpdir/authors/id/D/DU/DUMMY/foo.tar.gz");
