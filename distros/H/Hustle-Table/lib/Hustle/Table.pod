...
B<Version 0.4.0 and later>
C<given>/C<when> and thus the use of smart matching is removed. This gives a
...
matcher like C<^exact$>.

B<Version 0.3.0 and later>
...
Alternatively, a C<sub> and C<ctx> argument can be provided:
...
via the dynamically scoped numbered capture group variables (i.e. C<$1> et al.)
...
C<count> This is a dual purpose attributes. When adding entries to the list, it
...
C<ctx> Represents a user context which is applicable to the matcher. It would
