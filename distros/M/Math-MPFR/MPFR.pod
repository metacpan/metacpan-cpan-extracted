...
=head1 PASSING _Decimal64 VALUES

   Install Math::Decimal64 and build the mpfr library (version
   3.1.0 or later) with the --enable-decimal-float option, and
   build Math::MPFR by providing the "D64=1" arg to the
   Makefile.PL (which will define MPFR_WANT_DECIMAL_FLOATS):

     perl Makefile.PL D64=1

   You can then pass _Decimal64 values between Math::MPFR and
   Math::Decimal64 using:

      Rmpfr_set_DECIMAL64() and Rmpfr_get_DECIMAL64()

   NOTE: It is allowable to pass both "D64=1" and "F128=1" args
     to the Makefile.PL:
...
=head1 FUNCTIONS
   ...
   $nv = atonv($ws, $str); # $ws is a Math::MPFR object.
                           # Needs mpfr-3.1.6 or later
    Sets the NV $nv to the value represented by the string $str.
    $ws needs to be initialized (once only) with a precision of
    $Math::MPFR::BITS - after which atonv() can be called repeatedly
    without incurring the overhead of having to repeatedly create the
    Math::MPFR object that it needs. Ie:
