...
sub client4_connected {
  my ($kernel,$heap,$sender) = @_[KERNEL,HEAP,SENDER];
  pass($_[STATE]);
  $poe_kernel->post( 'client', 'send_to_server', { command => 'CLOSE', colonify => 0 } );
  return;
}
...
sub client_input {
  ...
  if ( $cmd eq '362' ) {
    pass("IRC$cmd");
    is($params->[1],'[unknown@127.0.0.1]','[unknown@127.0.0.1]');
    is($params->[2],'Closed: status = unknown','Closed: status = unknown');
    return;
  }
  # :listen.server.irc 363 bobbins 1 :Connections closed
  if ( $cmd eq '363' ) {
    pass("IRC$cmd");
    is($params->[1],1,'One connection closed');
    is($params->[2],'Connections closed','Connections closed');
...
sub client4_input {
  ...
  my $params = $in->{params};
  is($cmd,'ERROR','ERROR!');
  is($params->[0],'Closing Link: 127.0.0.1 (Oper Closing)','Closing Link: 127.0.0.1 (Oper Closing)');
  return;
}
...
sub groucho_input {
  ...
  if ( $cmd eq 'TRACE' ) {
    pass($cmd);
    is($in->{raw_line},':1FUAAAAAA TRACE :4AKAAAAAA',':1FUAAAAAA TRACE :4AKAAAAAA');
    return;
  }
  return;
}
