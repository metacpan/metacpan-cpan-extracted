...
package Context::Singleton::Context;
$Context::Singleton::Context::VERSION = '1.0.3';
...
sub _build_rule_promise_for {
	...
	$self->{promises}{$rule} // do {
		...
		for my $builder ($self->db->search_builder_for ($rule)) {
			$promise->add_dependencies (
...
sub _execute_triggers {
	...
	$_->($value) for $self->db->search_trigger_for ($rule);
}
