...
C<CPAN::Maker::Bootstrapper> scaffolds a new Perl project that
leverages L<CPAN::Maker> to create a ready-built CPAN distribution. It
installs a project Makefile, a C<buildspec.yml> pre-populated and
ready to feed to C<CPAN::Maker>, optional stub source file and test
file, and supporting makefile snippets that implement the bootstrapper
framework. It then runs C<make> to generate your distribution tarball.

The result is a project that can produce a distributable tarball with
a single C<make> invocation. As you iterate on your application, new
...
optional POD B<generation>, and AI-generated release notes. See L</LLM
Commands> and L</THE REVIEW WORKFLOW> for details.

I<NOTE: Check out the
L<release-notes|https://github.com/rlauer6/CPAN-Maker-Bootstrapper/tree/main/release-notes>
directory in the GitHub project for examples of release notes generated by the LLM.>
...
I<By default the final build step applies full linting: syntax
checking (C<perl -wc>), perltidy conformance, and perlcritic at its
...
or fails to compile, the build - and therefore C<install> - will fail.
...
=item Review the generated files

...particularly F<buildspec.yml> which controls how the distribution
is built as well as F<requires> and F<test-requires> which list your
module's dependencies. Your git identity is pre-populated from
F<~/.gitconfig> but you may want to adjust the description or resource
URLs.

See L<CPAN::Maker> for details regarding F<buildspec.yml>.

=item Edit the generated stub in F<lib/My/Module.pm.in>.

This is your primary source file - never edit the generated F<.pm> file
directly as it will be overwritten on the next C<make>.

=item Add More Modules and Scripts
...
=item Rebuild Your Distribution
...
=item Verify Your Tarball Installs Cleanly

To verify your distribution installs cleanly:

 cpanm --local-lib=$HOME My-Module-*.tar.gz

=item Put Your Project Under Source Control

To initialize version control and make your first commit:

 make git

Set C<NO_COMMIT=1> if you don't want to commit yet.

 make git NO_COMMIT=1

=item Learn More About C<CPAN::Maker::Bootstrapper>
...
=head1 WHY YOU SHOULD CONSIDER USING CPAN::Maker::Bootstrapper
...
C<perltidy>, and C<perlcritic>, C<podchecker> run automatically on
every build, stopping bad code before it enters the
distribution. Gates can be selectively disabled via your configuration
file or on the command line (C<make LINT=off>) when you need a faster
...
By default the recipes in the C<Makefile> will perform the following
actions:
...
Suppose you have a project that used C<CLI::Simple> as base class and
now want to use the C<CPAN::Maker::Bootstrapper> framework.
...
C<cmb> can read your global F<.gitconfig> file or
...
Then point C<cmb> at it by setting the
...
=item C<make bash-completion>

Generates and installs a bash completion function for your project's
modulino, then prints the C<source> line to enable it. The function is
produced by C<< <modulino> -generate-completion >> (available to any
C<CLI::Simple>-based modulino) and written to
F<~/.local/share/bash-completion/completions/E<lt>aliasE<gt>>.

 make bash-completion
 # then, as it instructs:
 source ~/.local/share/bash-completion/completions/<alias>

The target depends on the modulino, so it will build F<bin/E<lt>aliasE<gt>>
...
=item C<make recommends> / C<make suggests>

Companion targets to C<make requires>. The dependency scanner classifies
each discovered module into one of three tiers: hard C<requires>,
C<recommends> (soft, non-eval conditional dependencies), and C<suggests>
...
C<clean-local> target with a double-colon.
...
=item C<make workflow>

Installs a CI build script (F<builder>) and a GitHub Actions workflow
(F<.github/workflows/build.yml>) into your project, templated with
your module and project name. Also merges any build-only dependencies
C<builder> needs into F<build-requires>.

 make workflow
 git add build-requires builder .github/workflows/build.yml

Commit these files - GitHub Actions will then run C<./builder> on
every push to C<main> or C<dev>. See L</Continuous Integration> for
what C<builder> does and how to run it outside of GitHub Actions.

=item C<make build-ci>

Runs F<builder> locally inside Docker, against your current branch,
to reproduce a CI build without pushing. Requires C<docker> and a
F<builder> script (run C<make workflow> first if you don't have one).

 make build-ci

See L</Continuous Integration> for the variables that control this
target.
...
config to customize the behavior of C<cmb>.
...
B<inter-module> dependencies within your distribution -- i.e. which of
your own C<.pm> files C<use> which others. Uses
L<Module::ScanDeps::Static> to scan each source module, then prints
C<target: prerequisite> lines (in F<deps.mk> form) for the internal
packages only, so C<make> rebuilds a dependent module when a module it
...
Runs L<Perl::Critic> over the given files (or a newline-delimited
C<--file-list>). Defaults to the C<pbp> theme at severity 5; override
with the C<PERLCRITIC_THEME>, C<PERLCRITIC_SEVERITY>, and
...
C<file>. On first run it generates an F<.annotate> file alongside the
review for you to edit; re-run it to apply your edited annotations back
into the review. Pairs with C<code-review>/C<annotate> in the review
workflow.
...
=head2 Immutability Is a Feature

The managed build system is deliberately B<immutable>: the F<Makefile>,
everything under F<.includes/>, and the generated F<.pm>/F<.pl> files are
write-protected on purpose. This is a feature, not a restriction. It lets
C<make update> replace those files with newer, better versions without
clobbering anything of yours, and it guarantees that two projects on the
same bootstrapper version build I<identically> -- there is no per-project
drift hiding in a locally-edited rule.

You I<can> override any of it -- these are your files, and nothing stops you
from C<chmod +w> and editing a generated module or a managed include. But
you should not need to, and if you do, C<make update> will overwrite your
change. Every legitimate customization has a sanctioned hook that survives
C<make update>:

=over 4

=item * B<Project-specific targets, rules, and build ordering> --
F<project.mk> (always writable; never touched by C<make update>).

Reach for F<project.mk> when your project needs to do something the
generic build system can't know about, for example:

=over 4

=item * B<Build a companion artifact> the managed build doesn't produce --
generate a F<.pm.in> from a JSON/YAML schema, render documentation, compile
assets, or (as C<Amazon::API> does) build a Storable data file consumed at
runtime.

=item * B<Declare inter-module build order> the scanner can't infer --
C<lib/Foo/Bar.pm: lib/Foo.pm> when one module must be built before another.

=item * B<Deploy or publish> -- an C<scp>/upload/notify target that runs
after C<all>.

=item * B<Extend cleanup> -- a C<clean-local::> double-colon rule to remove
your own generated files, and C<CLEANFILES +=> for anything else.

=back

See L</What Belongs in F<project.mk>> for worked examples of each, and
L</What does NOT belong in project.mk> for the line between your extensions
and the managed core.

=item * B<Build-behavior toggles> (dependency scanning, linting, syntax
checking, version-drift strictness) -- make variables set on the command
line or in F<config.mk> (see L</CONFIGURATION> and the variable list below).

=item * B<Template tokens in your source> -- declare them in
C<TEMPLATE_VARS> and let C<cmb resolve-vars> fill them, rather than
hand-editing a generated F<.pm> (see L</Custom Template Tokens>).

=item * B<Custom cleanup> -- the C<clean-local::> double-colon target in
F<project.mk>.

=item * B<Extra distribution files> -- list them in F<buildspec.yml>;
C<extra-files.mk> wires them into the tarball automatically.

=item * B<Dependencies the scanner cannot see> -- the sticky C<+> prefix in
F<requires> (see L</Dependencies Management>).
...
I<Why the generated F<.pm>/F<.pl> files are read-only:> they are regenerated
from their F<.pm.in>/F<.pl.in> sources on every build, so any edit you make
directly to a generated F<.pm> would be silently lost on the next C<make>.
The C<chmod -w> is there to stop you from making that mistake. Edit the
F<.pm.in> source, not the generated F<.pm>.
...
=item C<SKIP_TESTS=1> - interpreted by L<CPAN::Maker>; skips running
the test suite when building the distribution tarball

=item C<PERLWC_SKIP="file1 file2">  - space-separated list of files
to exclude from syntax and POD checks

=item C<POD=extract|remove> - extract POD to a companion F<.pod> file
or strip it entirely from the built F<.pm>

=item C<PERLINCLUDE="-I path"> - additional include paths used during
the C<perl -wc> syntax check. Defaults to C<-I lib -I local/lib/perl5>
for hermetic checking; see L</The local dependency library>.

=item C<CPAN_INSTALLER=cpm|carton> - selects the installer used to
populate F<local/> for hermetic syntax checking. Auto-detected (C<cpm>
preferred) if unset.

=back

Two further toggles, C<CMB_UPDATE_CHECK> and C<CMB_VERSION_DRIFT>, are set
in F<config.mk> rather than on the command line; see L</Automatic Drift and
Update Checks>. F<config.mk> is read on every invocation of C<make> and is
the right place for durable, machine- or project-wide build settings such as
C<SYNTAX_CHECKING=off> on a box without an installer.

=head2 What Belongs in F<project.mk>
...
=head2 Custom Template Tokens

The build generates each F<.pm>/F<.pl> from its F<.pm.in>/F<.pl.in> source
by substituting C<E<64>TOKENE<64>> placeholders through C<cmb resolve-vars>.
The standard tokens (C<E<64>PACKAGE_VERSIONE<64>>, C<E<64>MODULE_NAMEE<64>>,
C<E<64>GIT_SHAE<64>>, and the other git-metadata variables) are always
available, but the mechanism is extensible: a project can define its own
tokens without editing any managed file.

To add a token, declare its name in C<TEMPLATE_VARS> (in F<project.mk>) and
provide a value -- as a make variable, an exported environment variable, or
via the C<.vars> sidecar. For example, to stamp a build timestamp:
...
C<cmb resolve-vars> then fills C<E<64>BUILD_DATEE<64>> from the value on
every build. The token grammar is uppercase-only (C<E<64>[A-Z0-9_]+E<64>>),
so placeholders never collide with real Perl such as C<@_> or C<@ISA>.

The substitution is B<fail-loud>: if a source contains a
C<E<64>TOKENE<64>> that is not resolved -- because it was never declared, or
...
=head2 Automatic Drift and Update Checks

Every build runs two checks before proceeding, so you don't have to
remember to run C<make check-upgrade> yourself:

=over 4

=item *

Is a newer C<CPAN::Maker::Bootstrapper> published on CPAN than the
one installed on this machine?

=item *

Do this project's managed files still match what the I<currently
installed> C<CPAN::Maker::Bootstrapper> would produce?
...
commonly because the project hasn't been through C<make update>
...
I<Drift> can happen for either of two reasons: your installed
C<CPAN::Maker::Bootstrapper> was upgraded since this project last ran
C<make update>, or a managed file was hand-edited despite the
warnings not to (see L</What You Should Never Modify>). C<make>
doesn't try to tell these apart - the fix is the same either way:

 make update

Two variables, set in F<config.mk>, control how strict these checks
are:

=over 4

=item C<CMB_UPDATE_CHECK> (C<on>|C<off>, default C<on>)

Set to C<off> to skip the MetaCPAN lookup - useful in CI or offline
environments where the network call would just fail or slow things
down.

=item C<CMB_VERSION_DRIFT> (C<fail>|C<warn>|C<ignore>, default C<fail>)

Controls what happens when a project's managed files no longer match
the installed bootstrapper. C<fail> stops the build until you run
C<make update>; C<warn> prints a message and continues; C<ignore>
...
=head3 The local dependency library

Syntax checking is performed in a I<hermetic> environment: each generated
F<.pm> is compiled with C<perl -wc> against C<lib> and a project-local
library at F<local/lib/perl5> only, with C<PERL5LIB> explicitly cleared for
...
in your C<PERL5LIB> or system C<@INC>. A dependency that compiles on your
machine but is missing from F<requires> will fail the build here instead of
surprising you on a clean install or CI box.

To populate that library, the build installs your declared dependencies into
F<local/> using C<cpm> (preferred) or C<carton>:
...
normal C<make> installs dependencies first, then syntax-checks against
them. C<cpm> is preferred because it supports multiple mirrors (see
F<build-mirrors>); C<carton> is supported but resolves against a
single mirror -- its default, or the one named in
C<PERL_CARTON_MIRROR>. Multiple mirrors may be specified in the
F<cpanfile> for both C<carton> and C<cpm>.

The use of a F<build-mirrors> file versus specifying the mirrors in
the dependency files determines the scope of their
use. F<build-mirrors> (and F<cpanfile> mirror declarations) set the
resolvers used for B<every> dependency in the build. To route a
B<single> module to a specific mirror, URL, or distribution -- without
affecting how anything else resolves -- annotate that module's entry
in F<requires> with C<mirror=>, C<url=>, or C<dist=> (see
L</build-mirrors>).

The C<+> prefix has a second use beyond protecting mirror annotations.
Because the check is hermetic, B<a module reached only at runtime will not
be found unless it is declared>. Static scanning (C<scandeps-static.pl>)
cannot see a dependency loaded dynamically -- for example a module pulled
in through a method call rather than a C<use> statement -- so it will never
add it to F<requires>. Declare such modules explicitly with C<+>, which
...
When using the preferred CPAN installer (C<cpm>), the build system
reads mirror URLs, one per line, from a F<build-mirrors> file in the
...
of) the public CPAN. If F<build-mirrors> is absent, the installer uses
its default mirror.

When C<carton> is used, because it does not support multiple mirrors
when setting the mirror using an environment variable, the build
system will use the first mirror in your F<build-mirrors> file if
present. C<carton> supports multiple mirrors only by specifying them
in the F<cpanfile>.

Entries in F<requires>, F<suggests>, and F<recommends> may carry optional
...
Only C<dist=>, C<url=>, and C<mirror=> are permitted after the version;
anything else is an error. Prefix such entries with the sticky C<+> so the
scanner does not strip the annotation on a later rescan. See L<CPAN::Maker>
for the full format.
...
C<make workflow> installs F<builder>, a self-contained bash script
that performs a clean-room build: it installs a minimal Perl
toolchain, installs your distribution's dependencies, and runs
C<make>. It's designed to run unmodified in GitHub Actions, in any
other CI runner, or by hand from the command line.

=head3 Running builder manually

F<builder> can build any git repository, not just the one it's
...
I<Note: a repository URL is currently required - F<builder> does not
...
Which tool F<builder> uses to install dependencies. Accepts C<cpm> or
C<cpanm>-based values.

default: C<cpm install -g --show-build-log-on-failure --verbose>

=item C<PERLTIDYRC> / C<PERLCRITICRC>

F<builder> searches the checked-out repository for F<.perltidyrc> or
F<perltidyrc> (respectively F<.perlcriticrc>/F<perlcriticrc>) and
exports them automatically. When found, F<builder> also installs
C<Perl::Tidy> or C<Perl::Critic> (plus the community/compatibility
policy modules) as extra build dependencies before running C<make>.

=item C<BUILD_BRANCH> / C<GITHUB_REF_NAME>

The branch to check out and build. C<GITHUB_REF_NAME> is set
automatically by GitHub Actions; C<BUILD_BRANCH> is the equivalent
override for manual or non-GitHub runs. Falls back to the current
branch if neither is set.

=back

=head3 Override files

Commit these to your project root to customize a CI build without
touching F<builder> itself:

=over 4

=item F<build-apt-deps>

Whitespace-separated list of additional Debian packages to install
before the build (F<builder> always installs a minimal base:
C<git gcc make perl curl ca-certificates libexpat-dev libssl-dev
libzip-dev>).

=item F<build-mirrors>

One CPAN mirror URL per line - for example, your DarkPAN. Merged with
C<https://cpan.metacpan.org> when resolving dependencies.

=back

=head3 make build-ci variables

C<make build-ci> wraps the manual docker invocation above. It accepts:
...
L</make workflow>, L</make build-ci>
...
=item * C<cpm> or C<carton> - installs declared dependencies into a
project-local library (F<local/lib/perl5>) for hermetic syntax checking

=back

You can set make variables like C<SYNTAX_CHECKING> in F<config.mk>, which
is included on every invocation of C<make>, to alter build behavior -- for
example C<SYNTAX_CHECKING=off> to skip the check when neither installer is
present (undeclared dependencies then go undetected).

I<Note: neither C<cpm> nor C<carton> is a hard prerequisite of
C<CPAN::Maker::Bootstrapper> itself; they are needed only to populate the
local library for hermetic checking. C<cpm> is preferred for its
multi-mirror support (see L</build-mirrors>).>
...
Makefile by filtering them through a C<cmb resolve-vars> command that
replaces certain tokens like C<E<64>PACKAGE_VERSIONE<64>> with
values. The generated files are read-only. Always edit the F<.in> file
version.
...
that simply isn't installed in F<local/>. Because syntax checking runs
against F<local/lib/perl5> with C<PERL5LIB> cleared, a dependency that is
present elsewhere on your system but not declared will fail here. Confirm it
is in F<requires> (add it with a sticky C<+> if the scanner can't see it --
see L</Dependencies Management>), then C<make local> to install it. This is
...
=head2 Why does my build say it has drifted from the installed bootstrapper?

This means your project's managed files (F<Makefile>,
F<.includes/*.mk>) no longer match what your I<currently installed>
C<CPAN::Maker::Bootstrapper> would generate. There are two ways this
happens - upgrading your bootstrapper (e.g. via C<cpanm
...
C<CMB_VERSION_DRIFT=warn> (or C<=ignore>) in that project's
F<config.mk>. See L</Automatic Drift and Update Checks>.
...
=item * The version of C<CPAN::Maker::Bootstrapper> (C<cmb --version>
