...
B<v0.4.0> the concept of redirected files is also supported. It give an
'application' layer emulation of linked file, but works on file systems that
don't support links. This is implemented using L<File::Path::Redirect>.
...
This are constants defined in the C<File::Meta::Cache> package, which can be
...
The path to the actual file opened. This could be different to C<KEY> if redirection is
enabled.
...
Returns a new  L<File::Meta::Cache> object. Each object is a unique cache and
...
The entry (array ref) is returned on success, or C<undef> is returned if the
...
currently used for the file. This will force another C<stat> on the file and
...
C<enable_redirect> will test files for redirection using
L<File::Path::Redirect>. The cache entry will still be keyed with the
C<$file_path>, but the actual path opened will be stored in entry as C<PATH> 
...
The returned code reference takes the same arguments as the C<open> method. 
...
Once a file is open, subsequent opens are only a hash lookup. No C<open> or
C<stat> call is issued. 
...
There is a PSGI specific module L<Plack::Middleware::Static::OpenFileCache>
...
L<GitHub|https://github.com/drclaw1394/perl-file-meta-cache>
