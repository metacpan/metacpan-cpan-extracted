...
# If any of the next modulo three test ever fail for anybody, I'd like to hear it.
is(-7 % 10, 3, 'negative modulo positive >= 0');
is(-8 % -3, -2, 'negative modulo negative <= 0');
is(20 % -3, -1, 'positive modulo negative <= 0');
...
__BOOTSTRAP_NAV__
ok($navbar = $tree->find('div.navbar-nav-scroll'), 'navbar element');
is(scalar(@li = $navbar->find('ul li')), 7, 'all items');
is($ref = $navbar->find('.active')->look_up( _tag => 'li'), $li[2], 'reference item');
is($li = $navbar->find('ul li:nth-of-type(3)'), $ref, 'third item');
my $s = HTML::Selector::Element->new('ul li:nth-of-type(3n)');
is_deeply([ $navbar->find('ul li:nth-of-type(3n)') ], [ @li[2, 5] ], 'every third item');
is_deeply([ $navbar->find('ul li:nth-of-type(2n+1)') ], [ @li[0, 2, 4, 6] ], 'every second item starting from 1');
is_deeply([ $navbar->find('ul li:nth-of-type(even)') ], [ @li[1, 3, 5] ], 'every even item');
is_deeply([ $navbar->find('ul li:nth-of-type(odd)') ], [ @li[0, 2, 4, 6] ], 'every odd item');
is_deeply([ $navbar->find('ul li:first-child') ], [ @li[0] ], 'first child');
is_deeply([ $navbar->find('ul li:last-child') ], [ @li[-1] ], 'last child');
is_deeply([ $navbar->find('ul li:nth-of-type(-n+5)') ], [ @li[0, 1, 2, 3, 4] ], 'every item up to the fifth');
is_deeply([ $navbar->find('ul li:nth-last-of-type(2)') ], [ @li[-2] ], 'second to last item');
is_deeply([ $navbar->find('ul li:nth-last-child(even)') ], [ @li[-6, -4, -2] ], 'every even item starting from end');
