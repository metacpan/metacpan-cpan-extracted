...
Note: YAMLScript.pm has Alien support built in, so you don't really need to install C<libyamlscript.so> yourself for Perl.

One simple way to do that is with:

    $ curl https://yamlscript.org/install | sudo PREFIX=/usr/local bash

Note: The above command will install the latest version of the YAMLScript command line utility, C<ys>, and the shared library, C<libyamlscript.so>, into C</usr/local/bin> and C</usr/local/lib> respectively.
...
Copyright 2022-2024 Ingy döt Net L<ingy@ingy.net|mailto:ingy@ingy.net>
