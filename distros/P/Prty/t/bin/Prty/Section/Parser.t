...
package Prty::Section::Parser::Test;
...
sub test_loadClass : Init(1) {
    shift->useOk('Prty::Section::Parser');
}
...
sub test_unitTest_statistic : Ignore(1) {
    ...
    $self->is(ref($par),'Prty::Section::Parser');

    my @files = Prty::Path->find('yeah',
        -type=>'f',
    );
    for my $file (@files) {
        $par->parse($file);
...
sub test_unitTest_content1 : Test(2) {
    ...
    my $obj = Prty::Section::Parser->new->parse(\$text)->[0];
    $self->is($obj->content,"\n\nDies ist ein beliebiger\nTest-Text.");
    $self->is($obj->source,$text);
}
...
package main;
Prty::Section::Parser::Test->runTests;
