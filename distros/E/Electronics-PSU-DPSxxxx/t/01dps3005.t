...
# Output state
{
   $test_fio->expect_syswrite( "\x01\x03\x00\x08\x00\x01\x05\xC8" );
   $test_fio->expect_sysread( 7 )
      ->returns( "\x01\x03\x02\x00\x00\xB8\x44" );
...
# Read multiple
{
   $test_fio->expect_syswrite( "\x01\x03\x00\x02\x00\x02\x65\xCB" );
   $test_fio->expect_sysread( 9 )
      ->returns( "\x01\x03\x04\x01\xF9\x00\x14\x2B\xF1" );
