use strict;
use warnings;

use Test2::Bundle::Extended;
use Test2::Tools::Subtest qw/subtest_buffered/;

use HTTP::Response;
use HTTP::Headers;
use JSON;
use WWW::LetsEncrypt::JWK::RSA;
use WWW::LetsEncrypt::Message::Certificate;

my $rsa_private_key =<<'END';
-----BEGIN RSA PRIVATE KEY-----

Jwd0k2SrxvrrIE9ieWlbHzV1Acw1AL1jSZ6sVZcXEkZi
-----END RSA PRIVATE KEY-----
END

my $JWK = WWW::LetsEncrypt::JWK::RSA::load_cert({
	private_key => $rsa_private_key,
	key_id      => 1,
	alg         => 'RS256',
});

my $Message;

sub _reset_data {
	$Message = WWW::LetsEncrypt::Message::Certificate->new({
		JWK  => $JWK,
		cert => 'DER encoding string',
	});
	$Message->_step('poll');
	$Message->_prep_step();
	return;
}

subtest_buffered cert_not_ready => sub {
	_reset_data;
	my $mock_retry_time = 10;
	my $Response = HTTP::Response->new(
		202,
		'',
		HTTP::Headers->new('Retry-After' => $mock_retry_time)
	);

	my $result_ref = $Message->_process_response($Response);
	is(
		$result_ref,
		{
			successful => 1,
			finished   => 0,
		},
		'Processed 202 response correctly.'
	);
	is($Message->retry_time, $mock_retry_time, 'retry_time set correctly');
};

subtest_buffered submitted_ready => sub {
	_reset_data;
	my $mock_cert = 'new der cert';
	my $Response = HTTP::Response->new('200');
	$Response->content($mock_cert);

	my $result_ref = $Message->_process_response($Response);
	is(
		$result_ref,
		{
			successful => 1,
			finished   => 1,
			cert       => $mock_cert,
		},
		'Processed 200 response correctly.'
	);
};

subtest_buffered error => sub {
	_reset_data;
	my $Response = HTTP::Response->new(404);
	my $result_ref = $Message->_process_response($Response);
	is($result_ref, {error => 1}, 'Correctly reported error');
};

done_testing;

