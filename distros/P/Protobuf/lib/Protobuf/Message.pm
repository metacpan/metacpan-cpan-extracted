...
sub FREEZE {
    ...
    if (ref($self) eq 'HASH' || (eval { Scalar::Util::reftype($self) } || '') eq 'HASH') {
...
sub THAW {
    my ($class_or_self, $serializer, @args) = @_;
    my $serialized = $args[0];
    if (!defined($serialized) && ref($class_or_self) && (eval { Scalar::Util::reftype($class_or_self) } || '') eq 'HASH') {
        $serialized = delete $class_or_self->{'_serialized'};
        delete $class_or_self->{'_upb_ptr'};
        delete $class_or_self->{'_mdef_ptr'};
        delete $class_or_self->{'_arena_sv'};
        delete $class_or_self->{'_engine'};
    }
    if (defined $serialized && length($serialized)) {
        my $class = ref($class_or_self) || $class_or_self;
        return $class->parse($serialized);
    }
