...
#     NO_META => q[1]
#     PREREQ_PM => { Params::Util=>q[1.01], LWP::UserAgent=>q[5.835], Test::Exception=>q[0.32], MIME::Base64=>q[3.13], ExtUtils::MakeMaker=>q[6.42], XML::Simple=>q[2.18], JSON=>q[2.27], REST::Client=>q[171], Carp=>q[1.11] }
#     VERSION => v2.2.1
...
DISTVNAME = Net-CampaignMonitor-v2.2.1
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="v2.2.1">' > $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::Exception" VERSION="0.32" />' >> $(DISTNAME).ppd
