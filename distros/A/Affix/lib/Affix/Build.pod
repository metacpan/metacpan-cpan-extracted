...
Affix::Build - Polyglot Compiler Frontend for Building FFI Extensions in Everything from Assembly to Zig
...
Compile source code from 20+ languages into shared libraries ready for Affix binding. C<Affix::Build> is a
cross-platform compilation utility designed to generate shared libraries (C<.dll>, C<.so>, C<.dylib>) from source code
in over 20 different programming languages.

It abstracts away the complexity of invoking various compilers, normalizing object file extensions, handling
platform-specific linker flags (such as MinGW vs. MSVC on Windows), and ensuring that language runtimes (like the Go GC
or .NET Runtime) are correctly initialized.

It serves as a powerful, polyglot alternative to C<Inline::*> modules. This allows you to write native code in your
language of choice and instantly bind to it from Perl using L<Affix>.

=head2 Build Strategies

The builder automatically picks the right approach based on your sources:
...
Add source files, then compile and link into a shared library:
...
=item * The Go runtime spins up background threads (for GC and scheduling) that do not shut down cleanly when a shared library is unloaded. This can cause access violations on Windows during program exit. If you encounter segmentation faults during global destruction, you can force a safe exit using C<POSIX::_exit(0)> to skip global destructors.
...
=item * B<Eiffel> (C<.e>) - via C<se> (SmartEiffel)

=item * B<Futhark> (C<.fut>) - Transpiles to C automatically

=back

=head1 PLATFORM NOTES

Platform-specific quirks and workarounds. The Windows/MinGW environment deserves special attention:

=head2 Windows (MinGW / Strawberry Perl)

This environment is notoriously considered "Polyglot Hard Mode." C<Affix::Build> performs several behind-the-scenes
tricks to make it work:

=over

=item 1. B<Rust Compatibility:> Forces C<rustc> to use the C<x86_64-pc-windows-gnu> target to ensure the resulting static libraries can link against Perl's GCC.

=item 2. B<Whole Archive:> When linking static libraries (C<.a>) into a shared library, passes C<-Wl,--whole-archive> to the linker to ensure symbols aren't stripped out prematurely.

=item 3. B<Exports:> Automatically adds C<-Wl,--export-all-symbols> when linking to ensure Fortran/Assembly symbols are visible without explicit decoration.
