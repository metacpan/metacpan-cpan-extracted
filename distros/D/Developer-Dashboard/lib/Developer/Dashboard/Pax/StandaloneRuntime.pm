...
sub _state {
    ...
    return $STATE = {
        ...
        native_runner => Developer::Dashboard::Pax::NativeRunner->new,
        wrapped => {},
        namespace_aliases => {},
        by_region => \%by_region,
    ...
    for my $full (sort keys %{ $state->{by_region} }) {
        ...
        *{$full} = sub {
            ...
            if (_eligible_i64_args(\@args)) {
                my $guard = Developer::Dashboard::Pax::GuardManager->new(
...
sub _compiled_script_sub_source {
    ...
    return sprintf(
        "sub %s%s {\n    my (%s) = \@_;\n    return Developer::Dashboard::Pax::StandaloneRuntime::_run_native_shape_sub(%s, %s, \@_);\n}\n",
        $short,
        $proto || '',
        $args,
        _perl_literal($full),
...
Developer::Dashboard::Pax::StandaloneRuntime - embedded runtime loader for standalone binaries

=head1 SYNOPSIS

  use Developer::Dashboard::Pax::StandaloneRuntime;

  my $result = Developer::Dashboard::Pax::StandaloneRuntime->run(...);
...
Example 1:
  ...
  perl -Ilib -MDeveloper::Dashboard::Pax::StandaloneRuntime -e 1
