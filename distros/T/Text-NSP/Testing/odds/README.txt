...
We have tested odds.pm, a component of Ngram Statistics Package version
0.59. File odds.pm implements the odds ratio. Note that odds.pm is not a  
program by itself, but is a statistical library package that is plugged  
into statistic.pl. We test here the features of odds.pm by running 
statistic.pl.

Following is a description of the aspects of odds.pm that we have
tested. We provide the scripts and files used for testing so that
later versions of odds.pm can be tested for backward compatibility.
...
Phase 1: Testing odds.pm's response to erroneous conditions. 
Phase 2: Testing odds.pm's behaviour under normal conditions. 


2.1. Phase 1: Testing odds.pm's response to erroneous conditions:
----------------------------------------------------------------------
...
odds.pm is meant only for bigrams. This test checks if odds.pm throws
an error when provided with larger n-grams.

2.1.2. Subtest 2: 
-----------------

odds.pm requires three frequency values: the frequency of the bigram,
...
not provided, odds.pm should throw an error. This test checks for this
error. 
...
checks if odds.pm does indeed throw an error when passed a total
bigrams value that is less than or equal to zero.

2.1.4. Subtest 4: 
-----------------

The numbers passed to odds.pm should be "valid" in that they should
represent a possible two-by-two table. If this is not the case,
various warnings are thrown by odds.pm. This test checks for these
warnings. 
...
2.2. Phase 2: Testing odds.pm's behaviour under normal conditions:
------------------------------------------------------------------
...
First, it checks if odds.pm works when the frequencies are given in a
...
Second it checks the actual calculations of odds.pm. The output target
...
Statistical library package odds.pm has been tested for erroneous
