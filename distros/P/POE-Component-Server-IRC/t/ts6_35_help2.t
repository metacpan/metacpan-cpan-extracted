...
my @responses = (
':listen.server.irc 704 bobbins index :Available HELP topics:',
':listen.server.irc 705 bobbins index :',
':listen.server.irc 705 bobbins index :ACCEPT          ADMIN           AWAY            CLOSE',
':listen.server.irc 705 bobbins index :CMODE           CONNECT         DIE             DLINE',
':listen.server.irc 705 bobbins index :ETRACE          HASH            HELP            INFO',
':listen.server.irc 705 bobbins index :INVITE          ISON            JOIN            KICK',
':listen.server.irc 705 bobbins index :KILL            KLINE           KNOCK           LINKS',
':listen.server.irc 705 bobbins index :LIST            LOCOPS          LUSERS          MAP',
':listen.server.irc 705 bobbins index :MODULE          MOTD            NAMES           NICK',
':listen.server.irc 705 bobbins index :NOTICE          OPER            PART',
':listen.server.irc 705 bobbins index :PASS            PING            PONG            POST',
':listen.server.irc 705 bobbins index :PRIVMSG         QUIT            REHASH          RESTART',
':listen.server.irc 705 bobbins index :RESV            SET             SQUIT           STATS',
':listen.server.irc 705 bobbins index :TIME            TOPIC           TRACE           UMODE',
':listen.server.irc 705 bobbins index :UNDLINE         UNKLINE         UNRESV          UNXLINE',
':listen.server.irc 705 bobbins index :USER            USERHOST        VERSION         WALLOPS',
':listen.server.irc 705 bobbins index :WHO             WHOIS           WHOWAS          XLINE',
':listen.server.irc 706 bobbins index :End of /HELP.',
);
...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^bobbins' && $params->[1] eq '+i' ) {
    $poe_kernel->post( $sender, 'send_to_server', { command => 'HELP' } );
    $poe_kernel->post( $sender, 'send_to_server', { command => 'HELP', params => [ 'accept' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'ERROR' ) {
    pass($cmd);
  ...
  # :listen.server.irc 263 bobbins HELP :Server load is temporarily too heavy. Please wait a while and try again.
  if ( $cmd eq '263' ) {
    pass("IRC$cmd");
