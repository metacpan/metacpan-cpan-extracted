package Prty::Database::ResultSet::Object;
...
Liefere einen Hash bzw. eine Hashreferenz (Prty::Hash) mit den
...
sub values {
    ...
    if (wantarray) {
        ...
        return Prty::Hash->new({@arr})->unlockKeys;
    }
    else {
        return Prty::Array->new(\@arr);
    }
}
...
sub normalizeNumber {
    ...
    for my $row (@{$self->rows}) {
        for my $title (@_) {
            my $val = $row->$title;
            $val = Prty::Formatter->normalizeNumber($val);
            $row->$title($val);
        }
    }
