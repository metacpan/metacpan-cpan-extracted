...
Sah::Schema::array::int::contiguous - An array of a single contiguous range of integers

=head1 VERSION

This document describes version 0.003 of Sah::Schema::array::int::contiguous (from Perl distribution Sah-Schemas-Array), released on 2023-02-03.
...
=head2 Sample data and validation results against this schema
 ...
 [1,2,4]  # INVALID (Not contiguous)

 [1,3,2]  # INVALID (Not sorted)

 [1]  # valid

 [1,2,3]  # valid

 [1.1,2.1]  # INVALID (Contains non-integers)
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("array::int::contiguous*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("array::int::contiguous", {return_type=>'str_errmsg'});
 ...
 $data = [1,"a"];
 my $errmsg = $validator->($data); # => "Elements not contiguous (check element[1]) "

Often a schema has coercion rule or default value, so after validation the
validated value is different. To return the validated (set-as-default, coerced,
prefiltered) value:

 my $validator = gen_validator("array::int::contiguous", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = [1,"a"];
 my $res = $validator->($data); # => ["Elements not contiguous (check element[1]) ",[1,"a"]]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("array::int::contiguous*");
     $validator->(\@args);
     ...
 }
...
To create a type constraint and type library from a schema:
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('$sch_name*', name=>'ArrayIntContiguous')
     );
 }

 use My::Types qw(ArrayIntContiguous);
 ArrayIntContiguous->assert_valid($data);
