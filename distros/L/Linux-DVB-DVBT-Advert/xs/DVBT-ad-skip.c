...
int
dvb_ad_cut(char *filename, char *ofilename, SV *cuts_aref, HV *settings_href=NULL)
	INIT:
	...
	CODE:
		...
		|| (SvTYPE(SvRV(cuts_aref)) != SVt_PVAV))
		{
		 	croak("Linux::DVB::DVBT::Advert::dvb_ad_cut requires a valid cuts array ref") ;
		}
		...
		if (num_entries <= 0)
		{
		 	croak("Linux::DVB::DVBT::Advert::dvb_ad_cut requires a list of cuts hashes") ;
		}
		...
		for (i=0; i <= num_entries ; i++)
		{
			if ((item = av_fetch((AV *)SvRV(cuts_aref), i, 0)) && SvOK (*item))
			{
	  			...
	  			{
	 			 	croak("Linux::DVB::DVBT::Advert::dvb_ad_cut requires a list of cut hashes") ;
	 			}
	 			href = (HV *)SvRV(*item) ;
...
int
dvb_ad_split(char *filename, char *ofilename, SV *cuts_aref, HV *settings_href=NULL)
	INIT:
		struct TS_settings settings;
		SV **val;
		struct list_head   	cut_list;
	...
	CODE:
		...
		|| (SvTYPE(SvRV(cuts_aref)) != SVt_PVAV))
		{
		 	croak("Linux::DVB::DVBT::Advert::dvb_ad_split requires a valid cuts array ref") ;
		}
		...
		for (i=0; i <= num_entries ; i++)
		{
			if ((item = av_fetch((AV *)SvRV(cuts_aref), i, 0)) && SvOK (*item))
			{
	  			if ( SvTYPE(SvRV(*item)) != SVt_PVHV )
	  			{
	 			 	croak("Linux::DVB::DVBT::Advert::dvb_ad_split requires a list of cut hashes") ;
	 			}
	 			href = (HV *)SvRV(*item) ;
