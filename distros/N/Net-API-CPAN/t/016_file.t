...
my $this;
my $obj = Net::API::CPAN::File->new( $test_data );
isa_ok( $obj => 'Net::API::CPAN::File' );
if( !defined( $obj ) )
{
    BAIL_OUT( Net::API::CPAN::File->error );
}

# To generate this list:
# egrep -E '^sub ' ./lib/Net/API/CPAN/File.pm | perl -lnE 'my $m = [split(/\s+/, $_)]->[1]; say "can_ok( \$obj, ''$m'' );"'
can_ok( $obj, 'abstract' );
can_ok( $obj, 'author' );
...
if( defined( $this ) )
{
    if( ref( $this ) )
    {
        isa_ok( $this => 'Net::API::CPAN::Author', 'author returns a Net::API::CPAN::Author object' );
    }
    else
    {
        is( $this => $test_data->{author}, 'author returns a string' );
    }
}
$this = $obj->authorized;
...
if( defined( $this ) )
{
    if( ref( $this ) )
    {
        isa_ok( $this => 'Net::API::CPAN::Release', 'release returns a Net::API::CPAN::Release object' );
    }
    else
    {
        is( $this => $test_data->{release}, 'release returns a string' );
    }
}
$this = $obj->sloc;
