...
UI::Various::PoorTerm::Dialog - concrete implementation of L<UI::Various::Dialog>
...
L<UI::Various::Dialog>.  It manages and hides everything specific to the last
resort UI.
...
require Exporter;
our @ISA = qw(UI::Various::Dialog UI::Various::PoorTerm::container);
our @EXPORT_OK = qw();
...
Show the complete dialogue by printing its title and all its elements.  Active
...
sub _show($)
{
    ...
    print $self->_wrap(sprintf($pre_active, 0), msg('leave_dialog')), "\n\n";
...
Handle the action of the UI element.  For a C<PoorTerm>'s dialogue this
means a loop of printing the dialogue's elements and allowing to select one
of the active ones for processing until the dialogue is exited or destroyed.
