######################################################################
#
# 07_yasohachi_db_hp.pl - Yaso-Hachi (rice-grain manager), full four-layer build
#
# Version (2): the same Yaso-Hachi application driven by all three
# companion modules at once --
#   model  = DB::Handy   (DBI-like flat-file RDBMS, two-table CRUD)
#   view   = HP::Handy   (Jinja2-style templates with inheritance)
#   server = HTTP::Handy (PSGI delivery layer)
# all wired together by PSGI::Handy. HP::Handy exposes render_string
# (not render), so it is injected through a CODE renderer exactly as in
# eg/04; $c->render then works as in any PSGI::Handy app.
#
# Versus eg/05 the model layer is unchanged: the difference is the view.
# Where eg/05 built HTML by hand, here every page is an HP::Handy
# template that {% extends "base.html" %}, with user input auto-escaped
# by {{ }} and the pre-encoded Japanese (&#xNNNN;) marked | safe.
#
...
#
# Run: perl -Ilib eg/07_yasohachi_db_hp.pl
# Then open http://127.0.0.1:8080/ to register and track grains.
# (A throwaway database and a throwaway template directory are created
#  under the system temp directory and removed on exit.)
#
# Demonstrates:
#   PSGI::Handy new(renderer=>CODE, db=>...)/get/post/to_app, Context
#   render/db/param/redirect, HP::Handy render_string with extends/blocks/
#   for/if and the | safe filter, DB::Handy two-table CRUD with CREATE
#   INDEX, and US-ASCII source that renders Japanese via &#xNNNN;.
#
######################################################################
use strict;
...
use HP::Handy;     # view layer    (Jinja2-style templates)
...
# --- view: HP::Handy templates -------------------------------------
# base.html lives on disk so {% extends "base.html" %} can find it; the
# page templates are kept in %VIEW and rendered with render_string.
my $TMPLDIR = File::Spec->catfile(File::Spec->tmpdir(), "psgi_eg05hp_tmpl_$$");
mkdir($TMPLDIR, 0700) or die "Cannot mkdir $TMPLDIR: $!\n";
...
<head>
<meta charset="utf-8">
<title>{{ title | safe }} - Yaso-Hachi</title>
<style>
...
local *BASE_FH;
open(BASE_FH, '>' . File::Spec->catfile($TMPLDIR, 'base.html'))
    or die "Cannot write base.html: $!\n";
print BASE_FH $BASE_HTML;
close(BASE_FH);
...
{% for g in grains %}
<tr>
<td><a href="/grain/{{ g.id }}"><code>{{ g.id_short }},,,</code></a></td>
<td>{{ g.variety }}</td>
<td>{{ g.grower }}</td>
<td>{{ g.location }}</td>
<td>{{ g.planted_html | safe }}</td>
<td><span class='bar-wrap'><span class='bar' style='width:{{ g.pct }}%'></span></span> {{ g.done }}/88 ({{ g.pct }}%)</td>
<td><a class="btn btn-sm" href="/grain/{{ g.id }}">&#x8a73;&#x7d30;</a></td>
</tr>
{% endfor %}
{% else %}
<tr><td colspan="7">&#x767b;&#x9332;&#x30c7;&#x30fc;&#x30bf;&#x306a;&#x3057;</td></tr>
{% endif %}
</tbody></table>
{% endblock %}
TMPL
...
<p><a href='/'>&#x4e00;&#x89a7;&#x306b;&#x623b;&#x308b;</a> |
   <a class='btn btn-sm btn-del' href='/grain/{{ id }}/delete'
      onclick="return confirm('&#x524a;&#x9664;&#x3057;&#x307e;&#x3059;&#x304b;?')">&#x524a;&#x9664;</a></p>
<table class='data'>
<tr><th>Rice-ID</th><td><code>{{ id }}</code></td></tr>
<tr><th>&#x54c1;&#x7a2e;</th><td>{{ grain.variety }}</td></tr>
<tr><th>&#x751f;&#x7523;&#x8005;</th><td>{{ grain.grower }}</td></tr>
<tr><th>&#x5834;&#x6240;</th><td>{{ grain.location }}</td></tr>
<tr><th>&#x7530;&#x690d;&#x65e5;</th><td>{{ planted_html | safe }}</td></tr>
<tr><th>&#x5099;&#x8003;</th><td>{{ grain.note }}</td></tr>
<tr><th>&#x767b;&#x9332;&#x65e5;&#x6642;</th><td>{{ grain.created_at }}</td></tr>
<tr><th>&#x9032;&#x6357;</th><td><span class='bar-wrap'><span class='bar' style='width:{{ pct }}%'></span></span> {{ done }}/88 ({{ pct }}%)</td></tr>
</table>
<h2>&#x516b;&#x5341;&#x516b;&#x306e;&#x624b;&#x9593;</h2>
<form method='post' action='/grain/{{ id }}/update'>
{% for step in steps %}
{% if step.cat %}<div class='section-head'>{{ step.cat | safe }}</div>{% endif %}
<div class='step-row'><div class='step-no'>{{ step.no }}</div><div class='step-name'>{{ step.name | safe }}</div><div class='step-sel'><select name='s_{{ step.no }}'>
<option value='0'{% if step.v == 0 %} selected{% endif %}>&#x672a;&#x5b9f;&#x65bd;</option>
<option value='1'{% if step.v == 1 %} selected{% endif %}>&#x4f5c;&#x696d;&#x4e2d;</option>
<option value='2'{% if step.v == 2 %} selected{% endif %}>&#x5b8c;&#x4e86;</option>
</select></div></div>
{% endfor %}
<p style='margin-top:12px'><button class='btn' type='submit'>&#x72b6;&#x614b;&#x3092;&#x66f4;&#x65b0;</button></p>
</form>
{% endblock %}
TMPL
...
# One HP::Handy engine, auto-escaping {{ }} output; base.html is on disk.
my $HP = HP::Handy->new(template_dir => $TMPLDIR, auto_escape => 1);
...
my $DBDIR = File::Spec->catfile(File::Spec->tmpdir(), "psgi_eg05hp_$$");
...
my $app = PSGI::Handy->new(
    ...
    not_found => sub {
        my $c = shift;
        my $html = render_view('notfound.html',
                               { title => '404', path => $c->req->path });
...
$app->get('/', sub {
    ...
    for $g (@grains) {
        ...
        push @view, {
            id           => $id,
            id_short     => substr($id, 0, 12),
            variety      => $g->{variety},
            grower       => $g->{grower},
            location     => $g->{location},
            planted_html => format_planted_on($g->{planted_on}),
            done         => $d,
            pct          => $pct,
        };
    }
...
$app->get('/grain/new', sub {
    my $c = shift;
    return $c->render('new.html', { title => '&#x65b0;&#x898f;&#x767b;&#x9332;' });
});
...
$app->get('/grain/:id', sub {
    ...
    unless (defined $grain) {
        my $html = render_view('notfound.html', { title => '404', path => $id });
        return $c->html($html, 404);
    }
    ...
    for $sd (@steps) {
        ...
        my $nm = defined $i->{name} ? $i->{name} : $sd->{step_key};
        push @sv, {
            no   => $sd->{step_no},
            name => $nm,
            cat  => $show,
    ...
    return $c->render('detail.html', {
        title        => '&#x8a73;&#x7d30;',
        id           => $id,
        grain        => $grain,
        planted_html => format_planted_on($grain->{planted_on}),
        done         => $done,
        pct          => $pct,
        steps        => [ @sv ],
    });
...
END {
    ...
    _rmtree($TMPLDIR) if defined $TMPLDIR && -d $TMPLDIR;
}
...
print "Yaso-Hachi on PSGI::Handy (DB::Handy + HP::Handy)  http://127.0.0.1:8080/\n";
my $psgi = $app->to_app;
