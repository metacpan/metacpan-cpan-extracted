...
#   F  eg/ examples exist
#   G  POD structure of lib/*.pm, including duplicate link targets
#   H  README required sections
#   I  generated metadata is well-formed
#   J  test suite and prerequisite conventions
#   K  reference, punctuation and test-argument style of the Perl
#      sources listed in MANIFEST
...
@EXPORT_OK = qw(
    ...
    _yaml_str _json_str _pod_link_targets
    check_A count_A
    check_B count_B
    check_C count_C
    check_D count_D
...
# ----------------------------------------------------------------------
# _pod_link_targets -- every POD link target a source declares, in order.
#
# A target is named by the FIRST LINE of an =head1..=head4 or =item
# command paragraph, which is what Pod::Checker keys on: a continuation
# line that tells two otherwise identical items apart in prose does not
# make their targets distinct.  The leading bullet of "=item * text" is
# dropped, formatting codes are removed innermost first so that C<foo>
# and foo name one target, and whitespace is squeezed.
# ----------------------------------------------------------------------
sub _pod_link_targets {
    my ($text) = @_;
    return () unless defined $text && $text ne '';
    my @target;
    for my $line (split /\n/, $text) {
        next unless $line =~ /^=(?:item|head[1-4])[ \t]+(.*)$/;
        my $t = $1;
        $t =~ s/\r$//;
        $t =~ s/^\*[ \t]*//;
        my $prev = '';
...
#   G8  no POD link target is declared twice
#
# G6 accepts any heading that contains the word LICENSE, so both
# "=head1 LICENSE" and "=head1 COPYRIGHT AND LICENSE" pass.
#
# G8 is here because the Pod::Checker check that would otherwise catch a
# duplicate target is only as strict as the Perl the tester happens to
# run.  Every =head and =item is a link target, and two of them that name
# the same target are a defect: the Pod::Parser-based Pod::Checker that
# Perls up to 5.24 ship reports "multiple occurrence of link target" as a
# warning, and the Pod::Simple-based one that came later says nothing at
# all.  A distribution whose own pod test asserts that Pod::Checker is
# quiet therefore passes every run on a modern Perl and collects a FAIL
# report from a 5.18 smoker -- which is how this check came to be written.
# Doing it here makes it hold whatever Pod::Checker is installed, or none.
#
...
sub check_G {
    ...
    for my $pm (_lib_pm_files($root)) {
        ...
        my %seen;
        my @dup;
        for my $t (_pod_link_targets($text)) {
            push @dup, $t if $seen{$t}++ == 1;
        }
        ok(!@dup, "G8 - no duplicate POD link target: $pm"
                . (@dup ? ' (' . join('; ', @dup) . ')' : ''));
    }
}
...
#   K4  a match handed to a locally declared ok()/is() is forced into
#       scalar context
#
# Options:
...
sub check_K {
    ...
    for my $pm (_style_files($root)) {
        ...
        # K4 -- a bare match as the first argument of a test helper that
        # was declared in this file without a prototype.
        #
        # ok(), is() and their kin take a plain list, so their arguments
        # are evaluated in LIST context.  A match in list context returns
        # the empty list when it fails and the capture list when it
        # succeeds -- not the 1/'' a boolean argument is meant to be.  So
        #
        #     ok($str =~ /wanted/, 'the message says so');
        #
        # passes 'the message says so' as the condition and nothing as the
        # name whenever the match FAILS: the assertion prints itself as a
        # nameless "ok" and can never fail.  With a capture group the error
        # runs the other way, and a successful match whose capture is '0'
        # or '' reports a failure.  Either way the assertion is inert and
        # the defect it was written to catch goes unnoticed.
        #
        # The module's own ok() carries an ($;$) prototype, which imposes
        # scalar context and makes it immune, so only helpers declared in
        # the file being checked are examined.  Write the argument as
        # ($str =~ /wanted/) ? 1 : 0, or as !!($str =~ /wanted/), or wrap
        # it in scalar().
        my %own;
        for my $line (@lines) {
            next unless $line =~ /^[ \t]*sub[ \t]+(ok|is|isnt)[ \t]*(\S?)/;
            $own{$1} = 1 unless $2 eq '(';   # ($;$) imposes scalar context
        }
        my @k4_bad;
        if (keys %own) {
            ...
            for my $line (@lines) {
                ...
                next if $arg =~ /(?:^|[\(,])\s*!/; # ! or !! forces boolean
                push @k4_bad, $n;
            }
        }
        ok(!@k4_bad, "K4 - match as a test argument needs scalar context: $pm"
                   . _first_lines(\@k4_bad));
    }
}
