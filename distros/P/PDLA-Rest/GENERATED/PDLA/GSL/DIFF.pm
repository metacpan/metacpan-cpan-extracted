...
#
package PDLA::GSL::DIFF;

@EXPORT_OK  = qw( gsldiff PDLA::PP diff_central PDLA::PP diff_backward PDLA::PP diff_forward );
...
sub gsldiff{
  ...
  if($$opt{Method}=~/cent/i){
   ($res,$abserr) = PDLA::GSL::DIFF::diff_central($x,$f);
  }
  elsif($$opt{Method}=~/back/i){
    ($res,$abserr) = PDLA::GSL::DIFF::diff_backward($x,$f);
  }
  elsif($$opt{Method}=~/forw/i){
    ($res,$abserr) = PDLA::GSL::DIFF::diff_forward($x,$f);
  }
  else{
...
*diff_central = \&PDLA::diff_central;
...
*diff_backward = \&PDLA::diff_backward;
...
*diff_forward = \&PDLA::diff_forward;
