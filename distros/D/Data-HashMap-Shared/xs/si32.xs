...
new(char* class, SV* path_sv, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        ...
        char errbuf[SHM_ERR_BUFLEN]; const char* path = SHM_PATH_ARG(path_sv, "path", "Data::HashMap::Shared::SI32"); ShmHandle* map = shm_si32_create(path, (uint32_t)max_entries, (uint32_t)lru_max, (uint32_t)ttl_default, (uint32_t)lru_skip, (uint64_t)arena_cap, (mode_t)file_mode, errbuf);
        if (!map) croak("Data::HashMap::Shared::SI32: %s", errbuf[0] ? errbuf : "unknown error");
        CK_MAX_SIZE(map, "Data::HashMap::Shared::SI32");
...
new_sharded(char* class, SV* path_prefix_sv, UV num_shards, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        const char* path_prefix = SHM_PATH_ARG(path_prefix_sv, "path prefix", "Data::HashMap::Shared::SI32");
        CK_U32(max_entries, "max_entries", "Data::HashMap::Shared::SI32"); CK_U32(lru_max, "lru_max", "Data::HashMap::Shared::SI32"); CK_U32(ttl_default, "ttl_default", "Data::HashMap::Shared::SI32"); CK_U32(lru_skip, "lru_skip", "Data::HashMap::Shared::SI32");
...
freeze(SV* self_sv)
    CODE:
        ...
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI32->freeze: cannot freeze a read-only handle");
...
unlink(SV* self_or_class, ...)
    CODE:
        if (sv_isobject(self_or_class) && sv_derived_from(self_or_class, "Data::HashMap::Shared::SI32")) {
            ...
            RETVAL = shm_unlink_path(shm_path_arg(aTHX_ ST(1), "path", "Data::HashMap::Shared::SI32"));
        }
    OUTPUT:
        RETVAL
