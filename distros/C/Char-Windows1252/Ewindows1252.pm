...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Ewindows1252::index($name, '::') == -1) && (Ewindows1252::index($name, "'") == -1)) {
            my $pkg;
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Windows1252::re_t=%s; while($Windows1252::re_t =~ %s){%s local $^W=0; local $Windows1252::re_r=%s; %s$Windows1252::re_t="$`$Windows1252::re_r$'"; pos($Windows1252::re_t)=length "$`$Windows1252::re_r"; } return $Windows1252::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Windows1252::re_r=CORE::eval $Windows1252::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Windows1252::re_r=%s; %s"%s$Windows1252::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Windows1252::re_n=0; while(%s =~ %s){%s local $^W=0; local $Windows1252::re_r=%s; %s%s="$`$Windows1252::re_r$'"; pos(%s)=length "$`$Windows1252::re_r"; $Windows1252::re_n++} return %s$Windows1252::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Windows1252::re_r=%s; %s%s="%s$Windows1252::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Windows1252::re_r=%s; %s%s="%s$Windows1252::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Windows1252::re_r=CORE::eval $Windows1252::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
