#!/usr/bin/perl
#
# $Id: tcp-scan-multi.pl,v 8a883d4cca4b 2015/02/20 06:38:19 gomor $
#
use strict;
use warnings;
...
use Getopt::Std;
my %opts;
getopts('t:p:r:n:TDF6d:s:', \%opts);

if (!$opts{t}) {
   die("Usage: $0 -t targets -p ports -r rate -n ntimes [-D] [-F] [-6] [-d device] [-s source IP]\n");
}
...
if (! $opts{d}) {
   my $nr;
   (my $nr_target = $targets[0]) =~ s/\/\d+$//;
   if ($opts{6}) {
      $nr = Net::Routing->new(target => $nr_target, family => NR_FAMILY_INET6());
   }
   else {
      $nr = Net::Routing->new(target => $nr_target, family => NR_FAMILY_INET4());
   }
   my $routes = $nr->get or die("get: $Net::Routing::Error");
   $interface = $routes->[0]->{interface};
   if (@$routes != 1) {
      eval("use Data::Dumper;");
      die("Routing not found in: ".Dumper($routes)."\n");
   }
}
else {
   $interface = $opts{d};
...
my $dev = Net::Frame::Device->new(dev => $interface);
my $ip;
my $ip6;
if ($opts{6}) {
   $ip6 = $opts{s} || $dev->ip6;
   print("Using source IPv6 address: $ip6\n");
}
else {
   $ip = $opts{s} || $dev->ip;
   print("Using source IPv4 address: $ip\n");
}
...
if ($opts{6}) {
   $d->filter(
      'ip6 and src host '.$target.' and dst host '.$ip6,
   );
}
else {
   $d->filter(
...
my $runtime = estimate_runtime({
   targets => \@targets,
   ports => \@send,
   pps => $pps,
   try => $pass,
}) or die("estimate_runtime failed");

my $string = runtime_as_string($runtime);
print $string."\n";
