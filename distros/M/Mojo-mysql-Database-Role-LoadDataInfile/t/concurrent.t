...
note q{Hard to test CONCURRENT. Just make sure it doesn't break anything and expected data is present};
my $res = $db->load_data_infile(table => 'people', rows => $people, concurrent => 1);
...
note 'Test CONCURRENT is placed in query where expected';
my $module = Test::MockModule->new('Mojo::mysql::Database');

my $query;
$module->mock('query' => sub { $query = $_[1] });
$db->load_data_infile(table => 'people', rows => $people, concurrent => 1);

like $query, qr/\s*LOAD DATA\s+CONCURRENT\s+LOCAL INFILE '(.*?)'/, 'query contains CONCURRENT in the correct spot for load_data_infile';

undef $query;
is $query, undef, 'query is undef';

$db->load_data_infile_p(table => 'people', rows => $people, concurrent => 1);
like $query, qr/\s*LOAD DATA\s+CONCURRENT\s+LOCAL INFILE '(.*?)'/, 'query contains CONCURRENT in the correct spot for load_data_infile_p';
