...
for my $cp (128..255) {
    ...
    if (exists $tests{$char}) {
        print +($uc eq $tests{$char} ? "ok " : "not ok "), ++$i, 
            " # uc('$char') eq '$uc'\n";
    } else {
        print +($uc eq $char ? "ok " : "not ok "), ++$i, 
            " # uc('$char') same as $char itself\n";
    }
}
...
U+0053 LATIN CAPITAL LETTER S, U+0053 LATIN CAPITAL LETTER S
