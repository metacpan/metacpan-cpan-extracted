...
#
# $Id: ObjectInfo.pm,v 1.4 2000/10/22 16:12:15 spragues Exp $
#
...
=head1 SYNOPSIS
 ...
 %syb_info = grab Sybase::ObjectInfo($dbh, $database, \@table_list) 
 ...
 returns a hash for the form:
    {$db_name}->{$table_name}->{$field_name}->{col_id}          # column order
...
=head1 MISC
 ...
 When supplying the parameters to grab, $database can be null. If so, then tables must be 
 of the form "database.owner.table" (owner can be null)
...
=head1 AUTHOR
 ...
 Stephen Sprague   Morgan Stanley Dean Witter
