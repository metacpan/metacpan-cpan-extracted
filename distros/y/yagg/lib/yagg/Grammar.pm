# Module yagg::Grammar
#
# David Coppit. This code is based on code by:
#
...
# use Data::Dumper; die Dumper $self;
    ...
    my($parser)=new yagg::NonterminalParser;
...
#    use Data::Dumper; die Dumper $values;
...
sub _ReduceGrammar {
    ...
    for my $ruleno (0..$#$rules) {
        ...
        my $tempref = [ @{$$rules[$ruleno]}[0,1] ];
        if (ref($$rules[$ruleno]) =~ /^(GENERATOR|EQUIVALENCE)$/)
        {
          bless($tempref,$1);
        }

        push(@{$$grammar{UURULES}}, $tempref);
    }
