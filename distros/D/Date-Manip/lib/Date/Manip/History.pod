# Copyright (c) 2015-2023 Sullivan Beck. All rights reserved.
...
Filter is no longer available on CPAN (though the author is still active).

GD is still maintained but by a new author.
