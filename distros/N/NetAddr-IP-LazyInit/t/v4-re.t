...
die "# Cannot continue without NetAddr::IP::LazyInit\n"
    unless use_ok('NetAddr::IP::LazyInit');

my @addrs = map { new NetAddr::IP::LazyInit $_ } @ips;

for my $a (@addrs)
{
    isa_ok($a, 'NetAddr::IP::LazyInit');
    my $re = $a->re;
    my $rx;
    ...
    ok(NetAddr::IP::LazyInit->new('default') !~ m/$rx/, "0/0 does not match");
}
