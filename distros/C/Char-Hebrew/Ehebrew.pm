...
######################################################################
#
# Ehebrew - Run-time routines for Hebrew.pm
#
...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Ehebrew::index($name, '::') == -1) && (Ehebrew::index($name, "'") == -1)) {
            my $pkg;
...
#
# Hebrew character range per length
#
my %range_tr = ();
...
sub import {
    ...
    *Char::ord           = \&Hebrew::ord;
    *Char::ord_          = \&Hebrew::ord_;
    *Char::reverse       = \&Hebrew::reverse;
    *Char::getc          = \&Hebrew::getc;
    *Char::length        = \&Hebrew::length;
    *Char::substr        = \&Hebrew::substr;
    *Char::index         = \&Hebrew::index;
    *Char::rindex        = \&Hebrew::rindex;
    *Char::eval          = \&Hebrew::eval;
    *Char::escape        = \&Hebrew::escape;
    *Char::escape_token  = \&Hebrew::escape_token;
    *Char::escape_script = \&Hebrew::escape_script;
}
...
sub Ehebrew::rindex($$;$);
sub Ehebrew::lcfirst(@);
sub Ehebrew::lcfirst_();
sub Ehebrew::lc(@);
sub Ehebrew::lc_();
sub Ehebrew::ucfirst(@);
sub Ehebrew::ucfirst_();
sub Ehebrew::uc(@);
sub Ehebrew::uc_();
sub Ehebrew::fc(@);
sub Ehebrew::fc_();
sub Ehebrew::ignorecase;
sub Ehebrew::classic_character_class;
sub Ehebrew::capture;
sub Ehebrew::chr(;$);
sub Ehebrew::chr_();
sub Ehebrew::glob($);
...
# ${Ehebrew::eS}          = qr{(?>[^\x09\x0A    \x0C\x0D\x20])};
# ${Ehebrew::eS}          = qr{(?>[^\x09\x0A\x0B\x0C\x0D\x20])};
${Ehebrew::eS}            = qr{(?>[^\s])};
...
# avoid: Name "Ehebrew::foo" used only once: possible typo at here.
${Ehebrew::dot}         = ${Ehebrew::dot};
${Ehebrew::dot_s}       = ${Ehebrew::dot_s};
${Ehebrew::eD}          = ${Ehebrew::eD};
${Ehebrew::eS}          = ${Ehebrew::eS};
${Ehebrew::eW}          = ${Ehebrew::eW};
${Ehebrew::eH}          = ${Ehebrew::eH};
${Ehebrew::eV}          = ${Ehebrew::eV};
${Ehebrew::eR}          = ${Ehebrew::eR};
${Ehebrew::eN}          = ${Ehebrew::eN};
${Ehebrew::not_alnum}   = ${Ehebrew::not_alnum};
${Ehebrew::not_alpha}   = ${Ehebrew::not_alpha};
${Ehebrew::not_ascii}   = ${Ehebrew::not_ascii};
${Ehebrew::not_blank}   = ${Ehebrew::not_blank};
${Ehebrew::not_cntrl}   = ${Ehebrew::not_cntrl};
${Ehebrew::not_digit}   = ${Ehebrew::not_digit};
${Ehebrew::not_graph}   = ${Ehebrew::not_graph};
${Ehebrew::not_lower}   = ${Ehebrew::not_lower};
${Ehebrew::not_lower_i} = ${Ehebrew::not_lower_i};
${Ehebrew::not_print}   = ${Ehebrew::not_print};
${Ehebrew::not_punct}   = ${Ehebrew::not_punct};
${Ehebrew::not_space}   = ${Ehebrew::not_space};
${Ehebrew::not_upper}   = ${Ehebrew::not_upper};
${Ehebrew::not_upper_i} = ${Ehebrew::not_upper_i};
${Ehebrew::not_word}    = ${Ehebrew::not_word};
${Ehebrew::not_xdigit}  = ${Ehebrew::not_xdigit};
${Ehebrew::eb}          = ${Ehebrew::eb};
${Ehebrew::eB}          = ${Ehebrew::eB};
...
#
# Hebrew lower case first with parameter
#
sub Ehebrew::lcfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Ehebrew::lc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Ehebrew::lc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Ehebrew::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}

#
# Hebrew lower case first without parameter
#
sub Ehebrew::lcfirst_() {
    return Ehebrew::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
}
...
#
# Hebrew lower case with parameter
#
sub Ehebrew::lc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# Hebrew lower case without parameter
#
sub Ehebrew::lc_() {
    my $s = $_;
    return join '', map {defined($lc{$_}) ? $lc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# Hebrew upper case first with parameter
#
sub Ehebrew::ucfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Ehebrew::uc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Ehebrew::uc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Ehebrew::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}

#
# Hebrew upper case first without parameter
#
sub Ehebrew::ucfirst_() {
    return Ehebrew::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
}
...
#
# Hebrew upper case with parameter
#
sub Ehebrew::uc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# Hebrew upper case without parameter
#
sub Ehebrew::uc_() {
    my $s = $_;
    return join '', map {defined($uc{$_}) ? $uc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# Hebrew fold case with parameter
#
sub Ehebrew::fc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# Hebrew fold case without parameter
#
sub Ehebrew::fc_() {
    my $s = $_;
    return join '', map {defined($fc{$_}) ? $fc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# Hebrew regexp ignore case modifier
#
sub Ehebrew::ignorecase {
    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # with /i modifier
                ...
                my $uc = Ehebrew::uc($char[$i]);
                my $fc = Ehebrew::fc($char[$i]);
                if ($uc ne $fc) {
                    if (CORE::length($fc) == 1) {
                        $char[$i] = '['   . $uc       . $fc . ']';
                    }
...
#
# prepare Hebrew characters per length
#
...
#
# Hebrew open character list for tr
#
sub _charlist_tr {
...
#
# Hebrew open character list for qr and not qr
#
sub _charlist {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '${Ehebrew::not_lower_i}',
                '[:^upper:]'  => '${Ehebrew::not_upper_i}',
...
#
# Hebrew hexadecimal escape sequence
#
sub hexchr {
    my($hexdigit) = @_;
...
#
# Hebrew open character list for qr
#
sub charlist_qr {
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Ehebrew::uc($char);
                        my $fc = Ehebrew::fc($char);
                        if ($uc eq $fc) {
...
#
# Hebrew open character list for not qr
#
sub charlist_not_qr {
...
#
# Hebrew order to character (with parameter)
#
sub Ehebrew::chr(;$) {
...
#
# Hebrew order to character (without parameter)
#
sub Ehebrew::chr_() {
...
#
# Hebrew path globbing (with parameter)
#
sub Ehebrew::glob($) {
...
#
# Hebrew path globbing (without parameter)
#
sub Ehebrew::glob_() {
...
#
# Hebrew path globbing via File::DosGlob 1.10
#
...
#
# Hebrew path globbing subroutine
#
sub _do_glob {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $fc = Ehebrew::fc($char)) ne $char) {
                $pattern .= $fc;
            }
            else {
        ...
        my $matchsub = sub { Ehebrew::fc($_[0]) =~ /\A $pattern \z/xms };
...
INNER:
        for my $leaf (@leaf) {
            ...
            if (Ehebrew::index($leaf,'.') == -1 and   # if name does not have a dot in it *and*
                CORE::length($leaf) <= 8 and        # name is shorter than or equal to 8 chars *and*
                Ehebrew::index($pattern,'\\.') != -1  # pattern has a dot.
            ) {
                if (&$matchsub("$leaf.")) {
                    push @matched, "$head$leaf";
                    next INNER;
...
#
sub Ehebrew::POSTMATCH {
    return $';
}

#
# Hebrew character to order (with parameter)
#
sub Hebrew::ord(;$) {
...
#
# Hebrew character to order (without parameter)
#
sub Hebrew::ord_() {
...
#
# Hebrew getc (with parameter, without parameter)
#
sub Hebrew::getc(;*@) {

    my($package) = caller;
    my $fh = @_ ? qualify_to_ref(shift,$package) : \*STDIN;
    croak 'Too many arguments for Hebrew::getc' if @_ and not wantarray;
    ...
    for my $length ($length[0] .. $length[-1]) {
        ...
        if (exists $range_tr{CORE::length($getc)}) {
            if ($getc =~ /\A ${Ehebrew::dot_s} \z/oxms) {
...
#
# Hebrew index by character
#
sub Hebrew::index($$;$) {

    my $index;
    if (@_ == 3) {
        $index = Ehebrew::index($_[0], $_[1], CORE::length(Hebrew::substr($_[0], 0, $_[2])));
    }
    else {
        $index = Ehebrew::index($_[0], $_[1]);
    }
    ...
    if ($index == -1) {
        ...
        return Hebrew::length(CORE::substr $_[0], 0, $index);
    }
}

#
# Hebrew rindex by character
#
sub Hebrew::rindex($$;$) {

    my $rindex;
    if (@_ == 3) {
        $rindex = Ehebrew::rindex($_[0], $_[1], CORE::length(Hebrew::substr($_[0], 0, $_[2])));
    }
    else {
        $rindex = Ehebrew::rindex($_[0], $_[1]);
    }
    ...
    if ($rindex == -1) {
        ...
        return Hebrew::length(CORE::substr $_[0], 0, $rindex);
    }
}
...
my $qq_substr  = qr{(?> Char::substr | Hebrew::substr | CORE::substr | substr ) (?>\s*) \( $qq_paren \)
                 }xms;
...
#
# escape Hebrew token of script
#
sub Hebrew::escape_token {
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Ehebrew::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ (?>\s*) PREMATCH  \b | \$ (?>\s*) \{ (?>\s*) PREMATCH  (?>\s*) \} | \$ (?>\s*) \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Ehebrew::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Ehebrew::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ (?>\s*) MATCH     \b | \$ (?>\s*) \{ (?>\s*) MATCH     (?>\s*) \} | \$ (?>\s*) \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Ehebrew::MATCH()};
    }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Ehebrew::POSTMATCH()
    elsif (/\G (                 \$ (?>\s*) POSTMATCH \b | \$ (?>\s*) \{ (?>\s*) POSTMATCH (?>\s*) \} | \$ (?>\s*) \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Ehebrew::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Ehebrew::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while (?>\s*) \( (?>\s*) glob (?>\s*) \) /oxgc) {
        return 'while ($_ = Ehebrew::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while (?>\s*) \( (?>\s*) glob \b /oxgc) {
        return 'while ($_ = Ehebrew::glob';
    }
...
# subroutines of package Ehebrew
    elsif (/\G \b (CORE:: | ->(>?\s*) (?: atan2 | [a-z]{2,})) \b       /oxgc) { $slash = 'm//'; return $1;                  }
    elsif (/\G \b Char::eval       (?= (?>\s*) \{ )                    /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Hebrew::eval       (?= (?>\s*) \{ )                    /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval Char::escape'; }
    elsif (/\G \b Hebrew::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval Hebrew::escape'; }
    elsif (/\G \b bytes::substr \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'substr';            }
    elsif (/\G \b chop \b          (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Ehebrew::chop';       }
    elsif (/\G \b bytes::index \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'index';             }
    elsif (/\G \b Char::index \b   (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Char::index';       }
    elsif (/\G \b Hebrew::index \b   (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Hebrew::index';       }
    elsif (/\G \b index \b         (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Ehebrew::index';      }
    elsif (/\G \b bytes::rindex \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'rindex';            }
    elsif (/\G \b Char::rindex \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Char::rindex';      }
    elsif (/\G \b Hebrew::rindex \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Hebrew::rindex';      }
    elsif (/\G \b rindex \b        (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Ehebrew::rindex';     }
    elsif (/\G \b lc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ehebrew::lc';         }
    elsif (/\G \b lcfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ehebrew::lcfirst';    }
    elsif (/\G \b uc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ehebrew::uc';         }
    elsif (/\G \b ucfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ehebrew::ucfirst';    }
    elsif (/\G \b fc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ehebrew::fc';         }
    ...
    elsif (/\G \b chr           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ehebrew::chr';               }
    elsif (/\G \b bytes::ord    (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ehebrew::glob';              }
    elsif (/\G \b lc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ehebrew::lc_';               }
    elsif (/\G \b lcfirst \b       (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ehebrew::lcfirst_';          }
    elsif (/\G \b uc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ehebrew::uc_';               }
    elsif (/\G \b ucfirst \b       (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ehebrew::ucfirst_';          }
    elsif (/\G \b fc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ehebrew::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ehebrew::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! (?>\s*) => )                          /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! (?>\s*) => )                          /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ehebrew::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Ehebrew::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Ehebrew::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq           (\#) [ ] (\#) /oxgc) { return 'Ehebrew::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq ((?>\s*)) (\() [ ] (\)) /oxgc) { return 'Ehebrew::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\{) [ ] (\}) /oxgc) { return 'Ehebrew::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\[) [ ] (\]) /oxgc) { return 'Ehebrew::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\<) [ ] (\>) /oxgc) { return 'Ehebrew::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\S) [ ] (\2) /oxgc) { return 'Ehebrew::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q            (\#) [ ] (\#) /oxgc) { return 'Ehebrew::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  ((?>\s*)) (\() [ ] (\)) /oxgc) { return 'Ehebrew::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\{) [ ] (\}) /oxgc) { return 'Ehebrew::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\[) [ ] (\]) /oxgc) { return 'Ehebrew::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\<) [ ] (\>) /oxgc) { return 'Ehebrew::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\S) [ ] (\2) /oxgc) { return 'Ehebrew::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                    ' [ ] '    /oxgc) { return 'Ehebrew::split' . $e . qq     {' '};  }
        elsif (/\G                    " [ ] "    /oxgc) { return 'Ehebrew::split' . $e . qq     {" "};  }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Ehebrew::PREMATCH()]}
            ...
            $e_string .= q{Ehebrew::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Ehebrew::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ (?>\s*) MATCH     \b | \$ (?>\s*) \{ (?>\s*) MATCH     (?>\s*) \} | \$ (?>\s*) \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Ehebrew::MATCH()};
            $slash = 'div';
        }
...
# subroutines of package Ehebrew
        ...
        elsif ($string =~ /\G \b Hebrew::eval       (?= (?>\s*) \{ )                    /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::eval \b                                        /oxgc) { $e_string .= 'eval Char::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b Hebrew::eval \b                                        /oxgc) { $e_string .= 'eval Hebrew::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::substr \b                                     /oxgc) { $e_string .= 'substr';            $slash = 'm//'; }
        elsif ($string =~ /\G \b chop \b                                              /oxgc) { $e_string .= 'Ehebrew::chop';       $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                                      /oxgc) { $e_string .= 'index';             $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::index \b                                       /oxgc) { $e_string .= 'Char::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b Hebrew::index \b                                       /oxgc) { $e_string .= 'Hebrew::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                             /oxgc) { $e_string .= 'Ehebrew::index';      $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b Hebrew::rindex \b                                      /oxgc) { $e_string .= 'Hebrew::rindex';      $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                            /oxgc) { $e_string .= 'Ehebrew::rindex';     $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ehebrew::lc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ehebrew::lcfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ehebrew::uc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ehebrew::ucfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ehebrew::fc';         $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ehebrew::chr';           $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ehebrew::glob';          $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                                      /oxgc) { $e_string .= 'Ehebrew::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                                 /oxgc) { $e_string .= 'Ehebrew::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                                      /oxgc) { $e_string .= 'Ehebrew::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                                 /oxgc) { $e_string .= 'Ehebrew::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                                      /oxgc) { $e_string .= 'Ehebrew::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                                     /oxgc) { $e_string .= 'Ehebrew::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                              /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                                     /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                                    /oxgc) { $e_string .= 'Ehebrew::glob_';             $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Ehebrew::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Ehebrew::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq           (\#) [ ] (\#) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\() [ ] (\)) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\{) [ ] (\}) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\[) [ ] (\]) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\<) [ ] (\>) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\S) [ ] (\2) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q            (\#) [ ] (\#) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\() [ ] (\)) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\{) [ ] (\}) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\[) [ ] (\]) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\<) [ ] (\>) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\S) [ ] (\2) /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                    ' [ ] '    /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                    " [ ] "    /oxgc) { $e_string .= 'Ehebrew::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        ...
        return Ehebrew::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Ehebrew::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Ehebrew::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Ehebrew::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Ehebrew::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Ehebrew::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Ehebrew::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Ehebrew::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Ehebrew::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Ehebrew::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Ehebrew::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Ehebrew::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Ehebrew::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Ehebrew::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Ehebrew::uc($char[$i]) ne Ehebrew::fc($char[$i]))) {
            if (CORE::length(Ehebrew::fc($char[$i])) == 1) {
                $char[$i] = '['   . Ehebrew::uc($char[$i])       . Ehebrew::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Ehebrew::uc($char[$i]) . '|' . Ehebrew::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ehebrew::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ (?>\s*) 0 (?>\s*) \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Ehebrew::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ehebrew::ignorecase(Ehebrew::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Ehebrew::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Ehebrew::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ehebrew::ignorecase(Ehebrew::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Ehebrew::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Ehebrew::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ehebrew::ignorecase(Ehebrew::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Ehebrew::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Hebrew::re_t=%s; while($Hebrew::re_t =~ %s){%s local $^W=0; local $Hebrew::re_r=%s; %s$Hebrew::re_t="$`$Hebrew::re_r$'"; pos($Hebrew::re_t)=length "$`$Hebrew::re_r"; } return $Hebrew::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Hebrew::re_r=CORE::eval $Hebrew::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Hebrew::re_r=%s; %s"%s$Hebrew::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Hebrew::re_n=0; while(%s =~ %s){%s local $^W=0; local $Hebrew::re_r=%s; %s%s="$`$Hebrew::re_r$'"; pos(%s)=length "$`$Hebrew::re_r"; $Hebrew::re_n++} return %s$Hebrew::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Hebrew::re_r=%s; %s%s="%s$Hebrew::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Hebrew::re_r=%s; %s%s="%s$Hebrew::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Hebrew::re_r=CORE::eval $Hebrew::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Ehebrew::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Ehebrew::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
=head1 SYNOPSIS
  ...
  # "no Ehebrew;" not supported

=head1 ABSTRACT

This module has run-time routines for use Hebrew software automatically, you
do not have to use.
...
some bugs. Thus, if you happen to find a bug that's in Hebrew software and not
...
This Ehebrew module first appeared in ActivePerl Build 522 Built under
...
=item Split string
  ...
  @split = Ehebrew::split(/pattern/,$string,$limit);
  @split = Ehebrew::split(/pattern/,$string);
  @split = Ehebrew::split(/pattern/);
  @split = Ehebrew::split('',$string,$limit);
  @split = Ehebrew::split('',$string);
  @split = Ehebrew::split('');
  @split = Ehebrew::split();
  @split = Ehebrew::split;
  ...
  at all, Ehebrew::split returns the original string as a single substring, If it
  ...
  @chars  = Ehebrew::split(//,  $word);
  @fields = Ehebrew::split(/:/, $line);
  @words  = Ehebrew::split(" ", $paragraph);
  @lines  = Ehebrew::split(/^/, $buffer);
  ...
  print join(":" => Ehebrew::split(/ */, "hi there"));
  ...
  Ehebrew::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Ehebrew::split(/:/, $_, 3);
  ...
  splits on whitespace just as Ehebrew::split with no arguments does. Thus,
  Ehebrew::split(" ") can be used to emulate awk's default behavior. In contrast,
  Ehebrew::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Ehebrew::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Ehebrew::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Ehebrew::split()) {
          $count{$word}++;
      }
  }

  The inverse of Ehebrew::split is join, except that join can only join with the
...
=item Transliteration
  ...
  $tr = Ehebrew::tr($variable,$bind_operator,$searchlist,$replacementlist,$modifier);
  $tr = Ehebrew::tr($variable,$bind_operator,$searchlist,$replacementlist);
  ...
  This subroutine scans a Hebrew string character by character and replaces all
  occurrences of the characters found in $searchlist with the corresponding character
  in $replacementlist. It returns the number of characters replaced or deleted.
  If no Hebrew string is specified via =~ operator, the $_ variable is translated.
  $modifier are:
  ...
  print Ehebrew::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'

=item Chop string

  $chop = Ehebrew::chop(@list);
  $chop = Ehebrew::chop();
  $chop = Ehebrew::chop;

  This subroutine chops off the last character of a string variable and returns the
  character chopped. The Ehebrew::chop subroutine is used primary to remove the newline
  ...
  since Ehebrew::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Ehebrew::chop a literal, only a variable. If you Ehebrew::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Ehebrew::chop(@lines);

  You can Ehebrew::chop anything that is an lvalue, including an assignment:

  Ehebrew::chop($cwd = `pwd`);
  Ehebrew::chop($answer = <STDIN>);

  This is different from:

  $answer = Ehebrew::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Ehebrew::chop returns the character
  ...
  In the most general case, Ehebrew::chop can be expressed using substr:

  $last_code = Ehebrew::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Ehebrew::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Ehebrew::chop;
...
=item Index string
  ...
  $byte_pos = Ehebrew::index($string,$substr,$byte_offset);
  $byte_pos = Ehebrew::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Ehebrew::index($string, $lookfor, $byte_pos)) > -1) {
...
=item Reverse index string
  ...
  $byte_pos = Ehebrew::rindex($string,$substr,$byte_offset);
  $byte_pos = Ehebrew::rindex($string,$substr);

  This subroutine works just like Ehebrew::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Ehebrew::index).
  ...
  while (($byte_pos = Hebrew::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item Lower case string
  ...
  $lc = Ehebrew::lc($string);
  $lc = Ehebrew::lc_;

  This subroutine returns a lowercased version of Hebrew $string (or $_, if
  $string is omitted). This is the internal subroutine implementing the \L escape
  in double-quoted strings.

  You can use the Ehebrew::fc subroutine for case-insensitive comparisons via Hebrew
  software.

=item Lower case first character of string

  $lcfirst = Ehebrew::lcfirst($string);
  $lcfirst = Ehebrew::lcfirst_;

  This subroutine returns a version of Hebrew $string with the first character
  lowercased (or $_, if $string is omitted). This is the internal subroutine
  implementing the \l escape in double-quoted strings.

=item Upper case string

  $uc = Ehebrew::uc($string);
  $uc = Ehebrew::uc_;

  This subroutine returns an uppercased version of Hebrew $string (or $_, if
  $string is omitted). This is the internal subroutine implementing the \U escape
  in interpolated strings. For titlecase, use Ehebrew::ucfirst instead.
...
=item Upper case first character of string
  ...
  $ucfirst = Ehebrew::ucfirst($string);
  $ucfirst = Ehebrew::ucfirst_;
  ...
  $titlecase = Ehebrew::ucfirst(substr($word,0,1)) . Ehebrew::lc(substr($word,1));
  ...
  $do_not_use = Ehebrew::ucfirst(Ehebrew::lc($word));
...
=item Fold case string
  ...
  $fc = Ehebrew::fc($string);
  $fc = Ehebrew::fc_;

  New to Hebrew software, this subroutine returns the full Unicode-like casefold of
  Hebrew $string (or $_, if omitted). This is the internal subroutine implementing
  ...
  Ehebrew::fc($a) eq Ehebrew::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because Hebrew software has always used casefolding semantics for
...
=item Make ignore case string
  ...
  @ignorecase = Ehebrew::ignorecase(@string);

  This subroutine is internal use to m/ /i, s/ / /i, split / /i, and qr/ /i.

=item Make capture number

  $capturenumber = Ehebrew::capture($string);

  This subroutine is internal use to m/ /, s/ / /, split / /, and qr/ /.

=item Make character

  $chr = Ehebrew::chr($code);
  $chr = Ehebrew::chr_;

  This subroutine returns a programmer-visible character, character represented by
  that $code in the character set. For example, Ehebrew::chr(65) is "A" in either
  ASCII or Hebrew, not Unicode. For the reverse of Ehebrew::chr, use Hebrew::ord.

=item Filename expansion (globbing)

  @glob = Ehebrew::glob($string);
  @glob = Ehebrew::glob_;
  ...
  The Ehebrew::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Ehebrew::glob('"*e f*"');
  @spacies = Ehebrew::glob(q("*e f*"));

  If you had to get a variable through, you could do this:

  @spacies = Ehebrew::glob("'*${var}e f*'");
  @spacies = Ehebrew::glob(qq("*${var}e f*"));
