...
SKIP: {
  ...
  ok( $session->can('cisco_trunk_ports2vlan_ids'),
    'can $session->cisco_trunk_ports2vlan_ids' );

  eval { $session->init_mixins };
  ok( !$@, 'init_mixins without error' );
  ok( $session->init_ok('Net::SNMP::Mixin::CiscoDot1qVlanStaticTrunks'), 'initialization successful completetd' );
  ...
  eval { $lldp_local_system_data = $session->cisco_vlan_ids2names };
  ok( !$@, 'cisco_vlan_ids2names' );

  my $lldp_loc_port_table;
  eval { $lldp_loc_port_table = $session->cisco_vlan_ids2trunk_ports };
  ok( !$@, 'cisco_vlan_ids2trunk_ports' );

  my $lldp_rem_table;
  eval { $lldp_rem_table = $session->cisco_trunk_ports2vlan_ids };
