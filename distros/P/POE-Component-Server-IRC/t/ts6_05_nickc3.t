...
sub harpo_connected {
  ...
  $kernel->post( $sender, 'send_to_server', { prefix => '9T9', command => 'UID', params => [ 'bobbins', '1', $bobts, '+i', '~pobbins', 'bobbins.irc.not', '192.168.1.40', '9T9AAAAAB', '0', 'bobbins along' ], colonify => 1 } );
...
sub groucho_input {
  ...
  if ( $cmd eq 'KILL' ) {
    ...
    is( $params->[0], '1FUAAAAAA', 'Correct UID: 1FUAAAAAAA' );
    is( $params->[1], q{listen.server.irc!bobbins (Nick Collision)}, 'listen.server.irc!bobbins (Nick Collision)' );
    return;
  }
  if ( $cmd eq 'SQUIT' ) {
    pass($cmd);
  ...
  if ( $cmd eq 'UID' && $params->[7] eq '1FUAAAAAA' ) {
    pass($cmd);
    $heap->{bobbins} = $params->[2];
    return;
  }
  if ( $cmd eq 'EOB' && $prefix eq '1FU' ) {
...
sub harpo_input {
  ...
  if ( $cmd eq 'KILL' ) {
    pass($cmd);
    is( $prefix, '1FU', 'Prefix is correct: 1FU' );
    is( $params->[0], '9T9AAAAAB', 'Target is correct' );
    is( $params->[1], 'Nick Collision', 'It is a NICK collision' );
    return;
  }
  return;
}
