	...
	I32 deepcopy, I32 quotekeys, SV *bless, I32 maxdepth, SV *sortkeys)
{
    ...
    if (SvROK(val)) {
        ...
        /* If a freeze method is provided and the object has it, call
           it.  Warn on errors. */
       // fprintf(stderr,"NG before freezer 'if' val=0x%0x\n",val);
                         ...
                         SvCUR(freezer), -1) != NULL)
	{
	    ...
	    XPUSHs(val); PUTBACK;
            // fprintf(stderr,"NG before perl_call_method val=0x%0x\n",val);
            /* NG 10--01-02: change G_VOID to G_SCALAR so we'll get return value */
	    i = perl_call_method(SvPVX_const(freezer), G_EVAL|G_SCALAR);
	    SPAGAIN;
	    if (SvTRUE(ERRSV)) {
		warn("WARNING(Freezer method call failed): %"SVf"", ERRSV);
		}
	    // fprintf(stderr,"NG after perl_call_method i=%d val=0x%0x\n",i,val);
            /* NG 10-01-02: this is the body of our patch. assign return value to val */
            if (i) {
	          // fprintf(stderr,"NG in 'if' i=%d val=0x%0x\n",i,val);
		  val = newSVsv(POPs);
	          // fprintf(stderr,"NG after assign to val i=%d val=0x%0x\n",i,val);
		  //(void)sv_2mortal(val); - don't know if this is necessary
		}
            /* end of patch */ 
	    PUTBACK; FREETMPS; LEAVE;
	}
        // fprintf(stderr,"NG after freezer 'if' val=0x%0x\n",val);
...
#if PERL_VERSION < 8
                    sortkeys = sv_2mortal(newSVpvn("Class::AutoDB::Dumper::_sortkeys", 23));
#else
		    keys = newAV();
...
# endif
	...
	if (realpack && !no_bless) {  /* free blessed allocs */
	    ...
	    sv_catpvn(retval, "' )", 3);
            /* NG 10-01-02: add term to check that object can do toaster method */
	    if (toaster && SvPOK(toaster) && SvCUR(toaster) &&
		gv_fetchmeth(SvSTASH(SvRV(val)), SvPVX_const(toaster), 
			     SvCUR(toaster), -1)!=NULL) {
                // fprintf(stderr,"NG in toaster 'if' val=0x%0x\n",val);
...
MODULE = Class::AutoDB::Dumper		PACKAGE = Class::AutoDB::Dumper         PREFIX = AutoDB_Dumper_
...
AutoDB_Dumper_Dumpxs(href, ...)
	...
	PPCODE:
	{
		...
		if (items < 2)
		    croak("Usage: Class::AutoDB::Dumper::Dumpxs(PACKAGE, VAL_ARY_REF, [NAME_ARY_REF])");
