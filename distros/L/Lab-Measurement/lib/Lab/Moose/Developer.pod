...
=head3 Building automated instrument tests

Lab::Moose has extensive support to build automated tests for instrument
drivers and this is what enables B<long-term maintainability> of the code base.
...
C<< $ perl -Ilib t/Moose/Instrument/SR830.t --connection=LinuxGPIB --connection_options='{pad: 1}'  >>

where the argument of C<--connection_options> is given as a YAML hash.
...
Know with the connection log in place, you can run the test without the real
...
=head3 Roles for SCPI instruments

TODO

=head3 Writing connections

TODO

=head2 Data files and plotting
