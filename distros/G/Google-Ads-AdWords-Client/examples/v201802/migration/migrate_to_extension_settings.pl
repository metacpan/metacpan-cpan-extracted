...
use Google::Ads::AdWords::v201802::ExtensionSetting;
use Google::Ads::AdWords::v201802::ExtensionSetting::Platform;
...
sub get_platform_restrictions_for_campaign() {
  ...
  if ($matching_function->get_operator() eq 'AND') {
    foreach my $argument (@{$matching_function->get_lhsOperand()}) {
      if ( $argument->isa("Google::Ads::AdWords::v201802::FunctionOperand")
        && $argument->get_value()->get_operator() eq 'EQUALS')
      {
          ...
          )
        {
          ...
          return Google::Ads::AdWords::v201802::ExtensionSetting::Platform->new(
...
sub get_feed_items_from_argument() {
  ...
  if ( $lhs_operand_size == 1
    && $lhs_operand->[0]
    ->isa("Google::Ads::AdWords::v201802::RequestContextOperand")
...
sub create_extension_setting() {
  ...
  foreach my $feed_item_id (@{$feed_item_ids}) {
    ...
    my $sitelink_feed_item =
      Google::Ads::AdWords::v201802::SitelinkFeedItem->new({
  ...
  $campaign_extension_setting->set_extensionSetting(
    Google::Ads::AdWords::v201802::ExtensionSetting->new({
...
sub delete_old_feed_items() {
  ...
  foreach my $feed_item_id (@{$feed_item_ids}) {
    push @operations,
      Google::Ads::AdWords::v201802::FeedItemOperation->new({
        operator => 'REMOVE',
        operand  => Google::Ads::AdWords::v201802::FeedItem->new({
            feedId     => $feed->get_id(),
