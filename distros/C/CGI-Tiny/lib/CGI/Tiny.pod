...
name/value pairs, represented as two-element array references.

=head3 query_param_names

  my $arrayref = $cgi->query_param_names;

Retrieve URL query string parameter names as an unsorted array reference.

=head3 query_param

  my $value = $cgi->query_param('foo');

Retrieve value of a named URL query string parameter. If the parameter name was
...
Retrieve values of a named URL query string parameter as an ordered array
reference.
...
Retrieve request cookie names as an unsorted array reference.
...
Retrieve C<x-www-form-urlencoded> body parameters as an ordered array reference
of name/value pairs, represented as two-element array references.
...
Retrieve C<x-www-form-urlencoded> body parameter names as an unsorted array
reference.
...
parameter name was passed multiple times, returns the last value. Use
...
Retrieve values of a named C<x-www-form-urlencoded> body parameter as an
...
be encoded according to L</"set_response_charset"> (UTF-8 by default).
...
characters automatically, and L</"render"> provides methods to encode response
data from Unicode characters to UTF-8 by default.
...
parameters, without decoding them from UTF-8 bytes.

=item *

L<CGI::Minimal> has context-sensitive parameter accessors, and only implements
parsing of request query/body parameters (without decoding them from UTF-8
bytes) and uploads.
