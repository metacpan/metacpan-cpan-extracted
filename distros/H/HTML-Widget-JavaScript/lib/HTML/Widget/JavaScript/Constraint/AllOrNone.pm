package HTML::Widget::JavaScript::Constraint::AllOrNone;

use warnings;
use strict;

use base 'HTML::Widget::Constraint::AllOrNone';

=head1 NAME

HTML::Widget::JavaScript::Constraint::AllOrNone - JavaScript AllOrNone Constraint
...
See L<HTML::Widget::Constraint::AllOrNone>.
...
sub emit_javascript {
	...
	return 
		"($not((" . 
		join(' && ', map {"${var_name}.$_.value != ''"} @{$self->names}) . 
		') || (' . 
		join(' && ', map {"${var_name}.$_.value == ''"} @{$self->names})
		. ')))';
}
