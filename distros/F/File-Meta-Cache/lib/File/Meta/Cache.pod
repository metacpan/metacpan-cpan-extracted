...
is up to the user to implement a timer that calls the C<sweep> API.
...
Each cached entry contains a C<USER> field, which allows the user to store
...
B<NOTE:> from v0.3.0, the field index contants have been renamed. The old names
