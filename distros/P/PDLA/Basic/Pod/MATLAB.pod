...
differences between MATLAB and PDLA to help you get going as quickly
as possible.
...
it highlights the key differences between MATLAB and PDLA.

=head1 Perl

The key difference between MATLAB and PDLA is B<Perl>.
...
At the same time, most syntax differences between MATLAB and PDLA are a
...
Unlike MATLAB, PDLA does not come with a dedicated IDE. It does however
...
C<perldl> or C<pdl2> shell) you must start the program with C<use PDLA;>.
...
PDLA program:
  ...
  use PDLA::AutoLoader; # Import additional PDLA module.
...
In current versions of PDLA (version 2.4.7 or later) there is
...
Provides a MATLAB-style autoloader for PDLA. If an unknown function
C<foo()> is called, PDLA looks for a file called C<foo.pdl>. If it
...
This section explains how PDLA's syntax differs from MATLAB. Most
...
In PDLA, indices start at '0' (like C and Java), not 1 (like MATLAB or FORTRAN).
For example, if C<$b> is an array with 5 elements, the elements would be
numbered from 0 to 4.

=item Displaying an object

MATLAB normally displays object contents automatically. In the PDLA shells you
...
MATLAB has a single function call for this. On the other hand, PDLA's
...
PDLA and MATLAB have a similar syntax for slicing, but there are two
important differences:

1) PDLA indices start at 0, as in C and Java. MATLAB starts indices at 1.

2) In MATLAB you think "rows and columns". In PDLA, think "x and y".
...
previous section "B<MODULES FOR MATLAB USERS>" for more information.
  ...
  use PDLA::AutoLoader; # MATLAB-like autoloader.
...
PDLA take a different approach:
...
=item PDLA offers two functions for each feature.
...
Notice that PDLA aggregates horizontally while MATLAB aggregates
...
B<TIP>: In MATLAB you think "rows and columns". In PDLA, think "x and y".

=back

=head2 Higher dimensional data sets

A related issue is how MATLAB and PDLA understand data sets of higher
dimension. MATLAB was designed for 1D vectors and 2D matrices. Higher
dimensional objects ("N-D arrays") were added on top. In contrast, PDLA
was designed for N-dimensional piddles from the start. This leads to
a few surprises in MATLAB that don't occur in PDLA:
...
MATLAB sees C<[1,2,3,4]> as a 2D matrix (1x4 matrix). PDLA sees it
...
available for N-D arrays. In PDLA, just about any feature supported by
...
PDLA's syntax for declaring functions differs significantly from MATLAB's.
...
one uses the PDLA I<rcols> in L<PDLA::IO::Misc>.  For a general
review of the IO functionality available in PDLA, see the
documentation for L<PDLA::IO>, e.g., C<help PDLA::IO> in the I<pdl2>
shell or C< pdldoc PDLA::IO > from the shell command line.
...
PDLA has full-featured plotting abilities. Unlike MATLAB, PDLA relies more on
...
PDLA and Perl do not have a direct equivalent to MATLAB's Simulink.
If this feature is important to you, then take a look at B<Scilab>:

L<http://www.scilab.org>

Scilab is another numerical analysis software. Like PDLA, it is free
and open source. It doesn't have PDLA's unique features, but it is
