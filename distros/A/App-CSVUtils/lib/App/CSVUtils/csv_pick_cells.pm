...
1;
# ABSTRACT: Get one or more random cells from CSV
...
App::CSVUtils::csv_pick_cells - Get one or more random cells from CSV

=head1 VERSION

This document describes version 1.034 of App::CSVUtils::csv_pick_cells (from Perl distribution App-CSVUtils), released on 2024-02-02.

=head1 FUNCTIONS


=head2 csv_pick_cells

Usage:

 csv_pick_cells(%args) -> [$status_code, $reason, $payload, \%result_meta]
...
=item * Pick a random cell value from CSV:

 csv_pick_cells(input_filename => "file.csv");

=item * Pick 5 random cell values from CSV, do not output header row:

 csv_pick_cells(input_filename => "file.csv", num_cells => 5, output_header => 0);
...
=item * B<num_cells> => I<posint> (default: 1)

Number of cells to pick.
