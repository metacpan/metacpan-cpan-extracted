...
new(char* class, SV* path_sv, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        ...
        char errbuf[SHM_ERR_BUFLEN]; const char* path = SHM_WRITE_PATH_ARG(path_sv, "path", "Data::HashMap::Shared::I32", "new"); ShmHandle* map = shm_i32_create(path, (uint32_t)max_entries, (uint32_t)lru_max, (uint32_t)ttl_default, (uint32_t)lru_skip, (uint64_t)arena_cap, (mode_t)file_mode, errbuf);
        if (!map) croak("Data::HashMap::Shared::I32: %s", errbuf[0] ? errbuf : "unknown error");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)map);
        CK_MAX_SIZE(map, "Data::HashMap::Shared::I32", RETVAL);
    OUTPUT:
        RETVAL
...
new_sharded(char* class, SV* path_prefix_sv, UV num_shards, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        const char* path_prefix = SHM_WRITE_PATH_ARG(path_prefix_sv, "path prefix", "Data::HashMap::Shared::I32", "new_sharded");
        CK_U32(max_entries, "max_entries", "Data::HashMap::Shared::I32"); CK_U32(lru_max, "lru_max", "Data::HashMap::Shared::I32"); CK_U32(ttl_default, "ttl_default", "Data::HashMap::Shared::I32"); CK_U32(lru_skip, "lru_skip", "Data::HashMap::Shared::I32");
...
new_from_fd(char* class, int fd)
    CODE:
        char errbuf[SHM_ERR_BUFLEN];
        SHM_TAINT_CHECK("Data::HashMap::Shared::I32->new_from_fd");
...
freeze(SV* self_sv)
    CODE:
        ...
        if (h->readonly || shm_is_fully_sealed(h)) croak("Data::HashMap::Shared::I32->freeze: map is frozen (read-only)");
        if (shm_freeze(h) != 0) croak("Data::HashMap::Shared::I32->freeze: msync: %s", strerror(errno));
        shm_mark_readonly(h);
...
sync(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
        if (shm_msync(h) != 0) croak("Data::HashMap::Shared::I32 sync: %s", strerror(errno));

void
DESTROY(SV* self_sv)
    CODE:
        if (!sv_isobject(self_sv) || !sv_derived_from(self_sv, "Data::HashMap::Shared::I32")) return;
    ...
    CODE:
        ...
        if (h->shard_handles) {
            for (int i = 1; i < items; i += 2) {
                int32_t _k = (int32_t)SvIV(ST(i));
                int32_t _v = (int32_t)SvIV(ST(i + 1));
                REEXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
                count += shm_i32_put(h, _k, _v);
            }
        } else {
            ...
            Newx(_vs, _n ? _n : 1, int32_t); SAVEFREEPV(_vs);
            for (int i = 1, j = 0; i < items; i += 2, j++) {
                _ks[j] = (int32_t)SvIV(ST(i));
                _vs[j] = (int32_t)SvIV(ST(i + 1));
            }
            REEXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
            WRSEQ_GUARD(h);
            for (int j = 0; j < _n; j++)
                count += shm_i32_put_inner(h, _ks[j], _vs[j], SHM_TTL_USE_DEFAULT);
        }
        RETVAL = count;
    OUTPUT:
        RETVAL
    ...
    CODE:
        ...
        if (h->shard_handles) {
            ...
            for (int i = 1, j = 0; i < items; i++, j++)
                _ks[j] = (int32_t)SvIV(ST(i));   /* FETCH before the lock */
            REEXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
            WRSEQ_GUARD(h);
            for (int j = 0; j < _n; j++)
                count += shm_i32_remove_inner(h, _ks[j]);
...
get_multi(SV* self_sv, ...)
    PPCODE:
        ...
        if (h->shard_handles) {
            ...
            SAVEFREEPV(keys);
            for (int i = 0; i < nkeys; i++)
                keys[i] = (int32_t)SvIV(ST(i + 1));   /* FETCH the keys before the lock */
            REEXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
            RDLOCK_GUARD(h);
...
stats(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
        HV *hv = newHV();
        hv_store(hv, "size", 4, newSVuv(shm_i32_size(h)), 0);
        hv_store(hv, "capacity", 8, newSVuv(shm_i32_capacity(h)), 0);
        hv_store(hv, "max_entries", 11, newSVuv(shm_i32_max_entries(h)), 0);
        hv_store(hv, "tombstones", 10, newSVuv(shm_i32_tombstones(h)), 0);
        hv_store(hv, "mmap_size", 9, newSVuv(shm_i32_mmap_size(h)), 0);
        hv_store(hv, "arena_used", 10, newSVuv(shm_i32_arena_used(h)), 0);
        hv_store(hv, "arena_cap", 9, newSVuv(shm_i32_arena_cap(h)), 0);
        hv_store(hv, "evictions", 9, newSVuv(shm_i32_stat_evictions(h)), 0);
        hv_store(hv, "expired", 7, newSVuv(shm_i32_stat_expired(h)), 0);
        hv_store(hv, "recoveries", 10, newSVuv(shm_i32_stat_recoveries(h)), 0);
        hv_store(hv, "max_size", 8, newSVuv(shm_i32_max_size(h)), 0);
        hv_store(hv, "ttl", 3, newSVuv(shm_i32_ttl(h)), 0);
...
bool
cas(SV* self_sv, int32_t key, int32_t expected, int32_t desired)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::I32: map is frozen (read-only)");
...
SV*
max(SV* self_sv, int32_t key, int32_t desired)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::I32: map is frozen (read-only)");
        int ok;
        int32_t val = shm_i32_set_minmax(h, key, desired, 1, &ok);
        if (!ok) croak("Data::HashMap::Shared::I32: max failed");
        RETVAL = newSViv(val);
    OUTPUT:
        RETVAL

SV*
min(SV* self_sv, int32_t key, int32_t desired)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::I32: map is frozen (read-only)");
        int ok;
        int32_t val = shm_i32_set_minmax(h, key, desired, 0, &ok);
        if (!ok) croak("Data::HashMap::Shared::I32: min failed");
        RETVAL = newSViv(val);
    OUTPUT:
        RETVAL
...
reserve(SV* self_sv, UV target)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::I32: map is frozen (read-only)");
        CK_U32(target, "target", "Data::HashMap::Shared::I32");
...
arena_used(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
        RETVAL = (UV)shm_i32_arena_used(h);
    OUTPUT:
        RETVAL
...
arena_cap(SV* self_sv)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
        RETVAL = (UV)shm_i32_arena_cap(h);
    OUTPUT:
        RETVAL

bool
add(SV* self_sv, int32_t key, int32_t value)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::I32: map is frozen (read-only)");
        RETVAL = shm_i32_add(h, key, value);
    OUTPUT:
        RETVAL
...
SV*
swap(SV* self_sv, int32_t key, int32_t value)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::I32", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::I32: map is frozen (read-only)");
        int32_t out_value;
        int rc = shm_i32_swap(h, key, value, &out_value);
...
unlink(SV* self_or_class, ...)
    CODE:
        if (sv_isobject(self_or_class) && sv_derived_from(self_or_class, "Data::HashMap::Shared::I32")) {
            EXTRACT_MAP("Data::HashMap::Shared::I32", self_or_class);
            if (h->path) {
                SHM_TAINT_CHECK("Data::HashMap::Shared::I32->unlink");
                REEXTRACT_MAP("Data::HashMap::Shared::I32", self_or_class);
            }
            RETVAL = shm_unlink_sharded(h);
        } else {
            if (items < 2) croak("Usage: Data::HashMap::Shared::I32->unlink($path)");
            SvGETMAGIC(ST(1));
            RETVAL = shm_unlink_path(shm_write_path_arg(aTHX_ ST(1), "path", "Data::HashMap::Shared::I32", "Data::HashMap::Shared::I32->unlink"));
        }
    OUTPUT:
        RETVAL
...
DESTROY(SV* self_sv)
    CODE:
        if (!sv_isobject(self_sv) || !sv_derived_from(self_sv, "Data::HashMap::Shared::I32::Cursor")) return;
