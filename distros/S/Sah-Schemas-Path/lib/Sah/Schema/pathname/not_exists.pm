...
Sah::Schema::pathname::not_exists - Path name, must not already exist on filesystem

=head1 VERSION

This document describes version 0.025 of Sah::Schema::pathname::not_exists (from Perl distribution Sah-Schemas-Path), released on 2023-11-23.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("pathname::not_exists*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("pathname::not_exists", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("pathname::not_exists", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("pathname::not_exists*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"pathname::not_exists*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('pathname::not_exists*', name=>'PathnameNotExists')
     );
 }

 use My::Types qw(PathnameNotExists);
 PathnameNotExists->assert_valid($data);
