...
sub set_param_index {
	...
	if ( CORE::length($index) ) {
		...
		$param_flow_pink->{_param_index} = $index;
		print(
"param_flow_color, set_param_index, $param_flow_pink->{_param_index}\n"
		);
	}
	else {
...
# print("param_flow_pink,set_param_value param_flow_pink->{_selection_index}= $param_flow_pink->{_selection_index}\n");

	if (   CORE::length($value)
		&& CORE::length( $param_flow_pink->{_param_index} )
		&& CORE::length( $param_flow_pink->{_selection_index} ) )
	{
		my $param_index  = $param_flow_pink->{_param_index};
		my $flow_index   = $param_flow_pink->{_selection_index};
#		print("param_flow_pink,set_param_value, flow index: $flow_index\n");
#        print("param_flow_pink,set_param_value, param index: $param_index\n");
#		print("param_flow_pink,set_param_value, values =$value\n");

		$cloned_values_aref2[$flow_index] =
		  clone( \@{ @{ $param_flow_pink->{_values_aref2} }[$flow_index] } );

#		my $length = scalar @{ $cloned_values_aref2[$flow_index] };
#		print(" OK 1. param_flow_pink, set_param_value:length=$length\n");
		...
		@{ @{ $param_flow_pink->{_values_aref2} }[$flow_index] } =
		  @{ $cloned_values_aref2[$flow_index] };

#        print("1. param_flow_pink, set_param_value:@{@{ $param_flow_pink->{_values_aref2} }[$flow_index]}\n");
#         print("1. param_flow_pink, set_param_value:@{$cloned_values_aref2[$flow_index]}\n");

	}
	else {
		# print("param_flow_pink, set_param_value: selection index < 0 NADA\n");
	}

	# print("param_flow_pink,set_param_value :values_aref[0] are @$value[0]\n");
	return ();
}
...
# print("param_flow_pink,set_values param_flow_pink->{_selection_index}= $param_flow_pink->{_selection_index}\n");
	...
	if ( $param_flow_pink->{_selection_index} >= 0 ) {
		...
		if ( $index >= 0 ) {
			...
			@{ @{ $param_flow_pink->{_values_aref2} }[$index] } =
			  @{ $cloned_values_aref2[$index] };
