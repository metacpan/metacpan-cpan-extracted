...
sub new
{
	...
	if(not defined $AsposeCellsCloud::Configuration::app_sid or $AsposeCellsCloud::Configuration::app_sid eq ''){
		croak("Aspose Cloud App SID key is missing.");
    }
	
	if(not defined $AsposeCellsCloud::Configuration::api_key or $AsposeCellsCloud::Configuration::api_key eq ''){
    ...
    my (%args) = (
        ...
        'base_url' => $AsposeCellsCloud::Configuration::api_server,
        @_
    );
...
sub call_api {
	...
	$resource_path =~ s/\Q{appSid}\E/$AsposeCellsCloud::Configuration::app_sid/g;
  
    my $_url = $self->{base_url} . $resource_path;
  
    # update signature
    my $signature = $self->sign($_url, $AsposeCellsCloud::Configuration::app_sid, $AsposeCellsCloud::Configuration::api_key);
    ...
    $self->{ua}->timeout($self->{http_timeout} || $AsposeCellsCloud::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $AsposeCellsCloud::Configuration::http_user_agent);
