...
1;
# ABSTRACT: An array of numbers with monotonically increasing elements
...
Sah::Schema::array::num::monotonically_increasing - An array of numbers with monotonically increasing elements

=head1 VERSION

This document describes version 0.004 of Sah::Schema::array::num::monotonically_increasing (from Perl distribution Sah-SchemaBundle-Array), released on 2024-02-13.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("array::num::monotonically_increasing*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("array::num::monotonically_increasing", {return_type=>'str_errmsg'});
 ...
 $data = [1,3,2];
 my $errmsg = $validator->($data); # => "Elements not monotonically increasing (check element[2]) "

Often a schema has coercion rule or default value rules, so after validation the
validated value will be different from the original. To return the validated
(set-as-default, coerced, prefiltered) value:

 my $validator = gen_validator("array::num::monotonically_increasing", {return_type=>'str_errmsg+val'});
 my $res = $validator->($data); # [$errmsg, $validated_val]
 
 # a sample valid data
 $data = [1,2.9,3];
 my $res = $validator->($data); # => ["",[1,2.9,3]]
 
 # a sample invalid data
 $data = [1,3,2];
 my $res = $validator->($data); # => ["Elements not monotonically increasing (check element[2]) ",[1,3,2]]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("array::num::monotonically_increasing*");
     $validator->(\@args);
     ...
 }
 ...
 $SPEC{myfunc} = {
     ...
     args => {
         arg1 => {
             ...
             schema => ['array::num::monotonically_increasing*'],
         },
         ...
     },
 };
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"array::num::monotonically_increasing*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('array::num::monotonically_increasing*', name=>'ArrayNumMonotonicallyIncreasing')
     );
 }

 use My::Types qw(ArrayNumMonotonicallyIncreasing);
 ArrayNumMonotonicallyIncreasing->assert_valid($data);
