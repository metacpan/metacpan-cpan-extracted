...
my $index_path = "/tmp/p5_apporo_index_04.tsv";
my $out_index;
...
system("apporo_indexer -i $index_path -bh");
{
    ...
    my $file_name = "apporo ASCII ary index for head of each line of sample data file";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
...
my $conf_path = "/tmp/p5_apporo_conf_04.tsv";
my $out_conf;
...
is_utf8	false
dist_threshold	0.0
index_path	/tmp/p5_apporo_index_04.tsv
dist_func	edit
entry_buf_len	1024
engine	tsubomi
result_num	10
...

{
    ...
    my @res = (
        ...
        "0.4	acm	08-Dec-2010 14:00",
        "0.333333	mailutils	08-Sep-2010 08:55",
        "0.3	macchanger	11-May-2004 10:49",
        "0.285714	maverik	11-Jan-2009 15:50",
        "0.285714	mailman	15-Jun-2012 12:50",
        "0.2	acct	05-Nov-2010 15:45",
    );
    my %hash_res = ();
    for (my $i = 0; $i <= $#res; $i++) {
        $res[$i] = $res[$i];
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result from the indexes whose index points are only heads of line using using '$query' query");
}

{
    ...
    my @arr = @{$app->retrieve($query)};
    my @res = ();
    is_deeply(\@arr, \@res, "'$query' is not included in first colmun of target data");
}
