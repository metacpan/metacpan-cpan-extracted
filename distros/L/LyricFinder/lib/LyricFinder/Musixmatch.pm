...
sub fetch {
	...
	(my $song = $song_in) =~ s#\s*\/\s*#\-#;  #FIX SONGS WITH "/" IN THEM!
	$artist =~ s/\&/feat/;    #DON'T ASK ME WHY "&" => "feat"? BUT IT DOES!
	$song =~ s/\&/feat/;
	$artist =~ s/ +/\-/g;
	$song =~ s/ +/\-/g;
	$artist =~ tr/A-Z/a-z/;
...
LyricFinder::Musixmatch accepts an artist name and song title, searches 
https://www.musixmatch.com for song lyrics, and, if found, returns them as a 
...
=item B<new> I<LyricFinder::Musixmatch>([ I<options> ])
...
Specifies an alternate "user-agent" string sent to www.musixmatch.com when 
attempting to fetch lyrics.  Set the desired user-agent (ie. browser name) to 
pass to www.musixmatch.com.  Some sites are pickey about receiving a user-agent 
...
(this module's) name, either as "Musixmatch" or ("Musixmatch").
...
This site's base URL on success is always:  "I<https://www.musixmatch.com>".
...
This site's module name on success is always:  "I<Musixmatch>".
...
Please report any bugs or feature requests to C<bug-lyricFinder-musixmatch 
at rt.cpan.org>, or through the web interface at 
L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=LyricFinder-Musixmatch>.  
...
L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=LyricFinder-Musixmatch>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/LyricFinder-Musixmatch>

=item * Search CPAN

L<http://search.cpan.org/dist/LyricFinder-Musixmatch/>
