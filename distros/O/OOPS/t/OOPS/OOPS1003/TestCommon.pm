...
package OOPS::OOPS1003::TestCommon;
...
our $dbms = OOPS::OOPS1003->initial_setup(%args) || confess;

$OOPS::OOPS1003::bigcutoff = 50;
$OOPS::OOPS1003::sqlite::big_blob_size = 60;
our $ocut = $OOPS::OOPS1003::bigcutoff;
our $bbs = $OOPS::OOPS1003::sqlite::big_blob_size;
...
sub rvnotied
{
	...
	for my $i (keys %OOPS::OOPS1003::tiedvars) {
		next if $i eq $r1;
		next if $i eq $r2;
		next if $r1 && $i eq tied %{$r1->{named_objects}};
		next if $r2 && $i eq tied %{$r2->{named_objects}};
		$bad++;
		print "TIED $i FROM $OOPS::OOPS1003::tiedvars{$i}\n" if $debug;
	}
	return $bad;
}
...
sub notied 
{
	...
	unless ($OOPS::OOPS1003::debug_tiedvars) {
		print "ok $okay # Skip \$OOPS::OOPS1003::debug_tiedvars not set\n";
		$okay++;
		return;
	}
...
sub qcheck
{
	...
	my $dbh = OOPS::OOPS1003->dbiconnect(%args) || confess;
...
sub db_drop
{
	if ($test_dsn =~ m{^DBI:SQLite:dbname=(/tmp/OOPStest.\d+.db)$}) {
		...
		eval {
			OOPS::OOPS1003->db_domany(\%OOPS::OOPS1003::TestCommon::args, <<END);
				DROP TABLE TP_object;
				DROP TABLE TP_attribute;
				DROP TABLE TP_big;
END
		};
		eval { OOPS::OOPS1003->db_domany(\%OOPS::OOPS1003::TestCommon::args, "DROP TABLE TP_counters") }
...
sub resetall
{
	...
	OOPS::OOPS1003->db_domany({ %args }, 
		$x 
		. OOPS::OOPS1003->db_initial_values() 
	...
	$r1 = OOPS::OOPS1003->new(%args) || confess;
	$fe = OOPS::OOPS1003::FrontEnd->new($r1);
}
...
sub groupmangle
{
	...
	my $r = OOPS::OOPS1003->new(%args) || confess;
	my $dbh = $r->{dbh};
...
{
	rcon(($_[0] || 0)+1);
	$r2 = OOPS::OOPS1003->new(%args) || confess;
}
