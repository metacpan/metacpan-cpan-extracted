...
$str = anytoa($op); # $op precision must be 53 or 64 or 113 or 2098
   ...
   NOTE: Bear in mind that, if the variable to be formatted by
         ...
         NV) then the formatting will actually be done by the
         underlying C functionality.
         For example, if perl's nvtype is 'long double' then doing:
         Rmpfr_printf("%a", $nv);
         will output rubbish. Instead, you would need to:
         Rmpfr_printf("%La", $nv).
         (But then ... why would you invoke Rmpfr_printf to do
         something of which perl's printf function is capable ?)
