...
You can use this module to return a Class::Error object instead of a simple
false value (e.g. when a sub or a property may return an object OR the undef
value on failure).

That feature allows to check on the object itself, or delay the checking after
calling any method on the object.

   $obj = AnyClass->new or die $obj->error
   AnyClass->new->any_method or die Class::Error->error  # static

For example, compare the difference between the behaviour of C<obj_A>
and C<obj_B> if the C<< AnyClass->new >> would return false:
...
All the methods called on the Class::Error object (regardless the arguments)
return a reference to the object itself, thus allowing you to call methods
on methods:
...
I<error> sets the error, which could be a simple string or any other
value (also stored in C<$Class::Error::error>), I<errnum> sets the
error number (also stored in C<$Class::Error::errnum>) which you can
retrieve with the C<error> and C<errnum> static or dynamic methods.

You can also pass a third argument (which must be false) to the new
method or leave it undef: the scalar reference to the I<false> argument
will be used as the object value in any contexts (internally using
C<overload>).

For example, if you leave the I<false> argument as C<undef>, the
Class::Error object itself is evaluated as undef in any contexts
(e.g. false in boolean context like the undef value), but unlike
the undef value, it is defined and allows you to call any methods on it.

B<Note>: If you want to avoid the "use of uninitialized value..."
warning when you use the object itself (or the result of its methods)
in string context, you can pass an empty string to the constructor,
or the C<0> value for numeric context. Use that feature only if you
know what you are doing, since a defined false value might make more
difficult the debgging of real errors.
...
request, please use this link: http://perl.4pro.net/?Class::Error.
