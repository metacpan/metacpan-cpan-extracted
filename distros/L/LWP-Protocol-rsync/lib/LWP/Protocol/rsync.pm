...
comparing the desired time.  If server time is not newer then the response
is usual "304 Not Modified".

C<Last-Modified> response is the modification time of the file on the
server.  For C<:content_file>, the file modification time is set too.
...
directly, or perhaps C<File::Rsync> front-end or C<File::RsyncP> protocol.
...
If the username or password is incorrect the response is "401 Unauthorized"
in the usual way.  The server checks authorization before other module
restrictions or path existence, so expect "Unauthorized" for anything
...
but that's just the filenames, not the dates, sizes and perms text of the
listing.
...
The descriptions are from the C<comment> part of F<rsyncd.conf>.  Eg.
...
Temporary directory as per C<File::Temp> and C<File::Spec> (and their usual
other variables etc on various systems).
...
to the C<rsync> program in C<$ENV{'RSYNC_PASSWORD'}> (since rsync doesn't
take a password part in a command line C<rsync://> URL).  C<rsync> expects
either $ENV{'RSYNC_PASSWORD'} or prompts the user (with L<getpass(3)>).
A prompt is avoided as this interface is meant to be non-interactive.

Any C<%20> etc URL escapes are unescaped to relevant characters since
C<rsync> doesn't take those forms in its C<rsync://> command line.  (Any %
is a literal part of the filename.)

C<*> C<?> C<[> characters as literals in filenames probably needs help from
C<rsync> itself, perhaps even on the server side.  Some C<\*> or C<[*]>
escaping can read an existing file, but will result in reading a file of
actual C<\*> or C<[*]> if there's no C<*>.  It would be bad to read or write
a wrong file.

The C<rsync --checksum> option is always used so the file contents are
...
meaning the daemon, and should be usual for publicly available URLs.  An
C<ssh> or C<rsh> mode would be for point-to-point with a specific host where
you have an account.  L<LWP::Protocol::sftp> might be useful for such C<ssh>
(without C<rsync> synchronizing).  Some generic way to specify C<rsync>
...
present when mirroring a directory.  Can it be used to delete an individual
file?  If so perhaps a C<DELETE> method could be implemented.
...
tighter grip on which part is the filename etc when strange characters or a
C<-E<gt>> sequence for symlink etc.
