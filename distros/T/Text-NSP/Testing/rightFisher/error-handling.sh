...
# shell program to test rightFisher.pm's responses to various erroneous
# conditions. note of course that rightFisher never halts on errors/warnings
# but sets error codes that statistic.pl can request later on.

# Subtest 1: what happens when ngram > 2 is provided to rightFisher
...
# test-1.sub-1.cnt has trigrams in it. so rightFisher.pm should complain
# during the initialization step, since rightFisher is for bigrams only!
...
echo "Test:   statistic.pl --ngram 3 rightFisher test-1.out $TESTFILE" 
statistic.pl --ngram 3 rightFisher test-1.out $TESTFILE >& error.out
...
# now the test!
echo "Test:   statistic.pl --set_freq_combo $FREQCOMBFILE rightFisher test-1.out $TESTFILE" 
statistic.pl --set_freq_combo $FREQCOMBFILE rightFisher test-1.out $TESTFILE >& error.out
...
# now the test!
echo "Test:   statistic.pl rightFisher test-1.out $TESTFILE"
statistic.pl rightFisher test-1.out $TESTFILE >& error.out
