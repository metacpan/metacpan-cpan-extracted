...
Dataflow is new to PDLA2.0. The basic philosophy
...
a flag in $c: "PDLA_PARENTDIMSCHANGED" and $b just raises the same flag
...
A better stack trace for errors is in the works for PDLA, probably
so that you can toggle a switch $PDLA::traceevals and get a good trace
...
This is what PDLA actually has in memory after the first three lines.
...
Notice how the callbacks only get called during PDLA::dowhenidle.
