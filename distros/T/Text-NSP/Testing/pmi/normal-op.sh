...
# shell program to test pmi.pm's output during normal operation (that
...
# test-2.sub-1-a.cnt has all the three frequency values in the default
...
# now the test!
echo "Test:   statistic.pl pmi test-2.out $TESTFILE" 
statistic.pl pmi test-2.out $TESTFILE 
...
# test-2.sub-1-b.cnt has the three frequency values in the following
# order: 0, 1, 0-1
...
# now the test!
echo "Test:   statistic.pl --set_freq_combo $FREQCOMBFILE pmi test-2.out $TESTFILE" 
statistic.pl --set_freq_combo $FREQCOMBFILE pmi test-2.out $TESTFILE
