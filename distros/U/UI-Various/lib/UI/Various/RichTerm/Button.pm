package UI::Various::RichTerm::Button;

# Author, Copyright and License: see end of file

=head1 NAME

UI::Various::RichTerm::Button - concrete implementation of L<UI::Various::Button>
...
require Exporter;
our @ISA = qw(UI::Various::Button UI::Various::RichTerm::base);
our @EXPORT_OK = qw();
...
sub _prepare($$)
{
    my ($self, $content_width) = @_;		# v- - 2 chars decoration
    my ($w, $h) = $self->_size($self->text, $content_width - 2);
    return ($w + 2, $h);			# + 2 chars decoration
}
...
sub _show($$$$)
{
    ...
    return $self->_format($prefix, $D{BL}, $self->text, $D{BR},
