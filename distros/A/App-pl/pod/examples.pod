...
just a dot-file.  Store in sort-by-key-and-stringify-at-end hash C<%R(ESULT)>.
...
has an almost fixed format, except with extreme member sizes.  The regexp
picks up only those lines which refer to files:
...
group:
    ...
    pl -o 'piped { keydiff $4 if s!^[^d]\S+ \K(.+?) +(\d+) (.{16}) [^/]+/(.+)!sprintf "%-20s %10d %s", $1, $2, $3!e } "tar", "-tvf", $_' *.tar *.tgz *.txz
    pl -o 'p { k $4 if s!^[^d]\S+ \K(.+?) +(\d+) (.{16}) [^/]+/(.+)!sprintf "%-20s %10d %s", $1, $2, $3!e } "tar", "-tvf", $_' *.tar *.tgz *.txz
...
Again without the date and owner/group, which can also vary:
    ...
    pl -o 'piped { keydiff $2 if s!^[^d]\S+ \K.+? +(\d+) .{16} [^/]+/(.+)!sprintf "%10d", $1!e; } "tar", "-tvf", $_' *.tar *.tgz *.txz
    pl -o 'p { k $2 if s!^[^d]\S+ \K.+? +(\d+) .{16} [^/]+/(.+)!sprintf "%10d", $1!e; } "tar", "-tvf", $_' *.tar *.tgz *.txz
...
format takes only the integral parts of the random numbers:
    ...
    >   2511e5aa-aefc-ad66-8568-163adce60f2a
...
standard values.  Note that Shell strings can span more than one line:
    ...
    >   7537e587-72d2-47d2-971e-8db53d0e7f41
...
argument to the pre-prototypes C<printf>.  But C<f(orm)> has a prototype.  So
use the C<&>-syntax to prevent it giving 2 (the length of the list):
