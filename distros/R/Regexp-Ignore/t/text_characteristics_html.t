...
BEGIN { 
    ...
    my $number_of_tests = 
	$number_of_tests_per_file * scalar(@test_files) * 4 + 1;
...
for (my $k = 0; $k <= 3; $k++) {
	...
	my $test_file_cleaned = 
	    "text_characteristics_html_cleaned_".$i."_".$k.".html";
	my $test_file_delimited = 
	    "text_characteristics_html_delimited_".$i."_".$k.".html";    
	my $test_file_merged = 
	    "text_characteristics_html_merged_".$i."_".$k.".html";    
	my $test_file_counters = 
	    "text_characteristics_html_counters_".$i."_".$k;    
	...
	# create the Regexp::IgnoreTextCharacteristicsHTML object (that 
	# implements the get_tokens method)
	my $rei = 
	    new Regexp::IgnoreTextCharacteristicsHTML($original_text, 
						      "<!-- __INDEX__ -->");
	
	if ($k == 0) { # stay with the defaults
	    $rei->ignore_word_remarks(1);
	    $rei->ignore_html_remarks(1);
	}
	elsif ($k == 1) {
	    $rei->ignore_word_remarks(0);
	    $rei->ignore_html_remarks(1);
	}
	elsif ($k == 2) {
	    $rei->ignore_word_remarks(1);
