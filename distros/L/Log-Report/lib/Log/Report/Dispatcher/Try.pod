...
Returns C<true> if the previous try block did produce a terminal
...
Extends L<"Attributes" in Log::Report::Dispatcher|Log::Report::Dispatcher/"Attributes">.
...
=item $obj-E<gt>B<died>( [$text] )

The exit C<$text> or object (C<$@>) of the eval'ed block, in its unprocessed state.
They will always return C<true> when they where deadly, and it always stringifies
into something useful.

=item $obj-E<gt>B<exceptions>()

Returns all collected L<Log::Report::Exception|Log::Report::Exception> objects.  The last of
...
The C<$message> is either a STRING or a L<Log::Report::Message|Log::Report::Message>.
Improves base, see L<Log::Report::Dispatcher/"Logging">

=item $obj-E<gt>B<reportAll>(%options)

Re-cast the messages in all collect exceptions into the defined
dispatchers, which were disabled during the try block. The C<%options>
will end-up as HASH of C<%options> to L<Log::Report::report()|Log::Report/"Report production and configuration">; see
L<Log::Report::Exception::throw()|Log::Report::Exception/"Processing"> which does the job.

=item $obj-E<gt>B<reportFatal>(%options)

Re-cast only the fatal message to the defined dispatchers.  If the
block was left without problems, then nothing will be done.  The C<%options>
will end-up as HASH of C<%options> to L<Log::Report::report()|Log::Report/"Report production and configuration">; see
L<Log::Report::Exception::throw()|Log::Report::Exception/"Processing"> which does the job.
...
=item $obj-E<gt>B<translate>(\%options, $reason, $message)
...
Returns C<true> if the block was left with an fatal message.
...
the specified C<$tag> (as string) or matches the REGEX.
See L<Log::Report::Message::taggedWith()|Log::Report::Message/"Processing">
