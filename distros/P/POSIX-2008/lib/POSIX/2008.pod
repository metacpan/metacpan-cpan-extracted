...
The C<openat2()> system call is an extension of L<C<openat()>|"openat"> and
provides a superset of its functionality. It is Linux-specific and thus not
part of POSIX, but I include it anyway.
...
C<preadv()> is similar to L<C<preadv()>|"preadv"> but adds an optional I<flags>
...
C<pwritev()> is similar to L<C<writev()>|"writev"> but adds an optional I<flags>
argument, which is a bitwise OR of zero or more of the C<RWF_*> flags (see the
manpage for details). I<flags> is set to 0 if omitted or undef.
...
This could be fixed like this: C<unlink $_[0] or ($!{EISDIR} || $!{EPERM} ?
rmdir $_[0] : undef)> (correct errno check depends on OS), or by using the
library call right away.
...
C<AT_SYMLINK_FOLLOW> C<AT_SYMLINK_NOFOLLOW> C<RESOLVE_BENEATH>
...
C<openat2()>, C<preadv2()> and C<pwritev2()> are nonstandard extensions
present in Linux.
