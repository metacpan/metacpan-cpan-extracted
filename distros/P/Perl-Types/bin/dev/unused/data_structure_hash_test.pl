...
use Perl::Structure::Hash_cpp;  Perl::Structure::Hash_cpp::cpp_load();

$Perl::DEBUG = 1;
Perl::diag(q{in hash_test.pl, have Perl__Type__Integer__MODE_ID() = '} . Perl__Type__Integer__MODE_ID() . "'\n");
Perl::diag(q{in hash_test.pl, have Perl__Type__Number__MODE_ID() = '} . Perl__Type__Number__MODE_ID() . "'\n");
Perl::diag(q{in hash_test.pl, have Perl__Type__String__MODE_ID() = '} . Perl__Type__String__MODE_ID() . "'\n");
Perl::diag(q{in hash_test.pl, have hash__MODE_ID() = '} . hash__MODE_ID() . "'\n");

# variable declarations
my string $retval_stringify;
my hashref::integer $retval_integer_hashref;
my hashref::number $retval_number_hashref;
my hashref::string $my_peeps = {};
my hashref::string $retval_jeffys;

# loop to test for memory leaks
my integer $i_MAX = 0;  # CONSTANT
for my integer $i ( 0 .. $i_MAX ) {
	Perl::diag("in hash_test.pl, top of for() loop $i/$i_MAX\n");
...
#	$retval_stringify = integer_hashref_to_string({a_key => 23});  # TIVHVRV20
	...
	Perl::diag("in hash_test.pl $i/$i_MAX, have \$retval_stringify =\n$retval_stringify\n");
...
#	Perl::diag("in hash_test.pl $i/$i_MAX, have \$retval_integer_hashref = \n" . Dumper($retval_integer_hashref) . "\n");
...
#	Perl::diag("in hash_test.pl $i/$i_MAX, have \$retval_number_hashref = \n" . Dumper($retval_number_hashref) . "\n");
...
#	Perl::diag("in hash_test.pl, have \$my_peeps =\n" . Perl::DUMPER($my_peeps) . "\n");
...
#	Perl::diag("in hash_test.pl, have \$retval_jeffys =\n" . Perl::DUMPER($retval_jeffys) . "\n");
