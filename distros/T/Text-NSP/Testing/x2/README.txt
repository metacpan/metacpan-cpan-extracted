...
We have tested x2.pm, a component of Bigram Statistics Package version
0.5. File x2.pm implements the chi-squared measure of
association. Note that x2.pm is not a program by itself, but is a
statistical library package that is plugged into statistic.pl. We test
here the features of x2.pm by running statistic.pl.

Following is a description of the aspects of x2.pm that we have
tested. We provide the scripts and files used for testing so that
later versions of x2.pm can be tested for backward compatibility.
...
Phase 1: Testing x2.pm's response to erroneous conditions. 
Phase 2: Testing x2.pm's behaviour under normal conditions. 


2.1. Phase 1: Testing x2.pm's response to erroneous conditions:
----------------------------------------------------------------------
...
X2.pm is meant only for bigrams. This test checks if x2.pm throws
an error when provided with larger n-grams.

2.1.2. Subtest 2: 
-----------------

X2.pm requires three frequency values: the frequency of the bigram,
...
not provided, x2.pm should throw an error. This test checks for this
error. 
...
checks if x2.pm does indeed throw an error when passed a total
bigrams value that is less than or equal to zero.

2.1.4. Subtest 4: 
-----------------

The numbers passed to x2.pm should be "valid" in that they should
represent a possible two-by-two table. If this is not the case,
various warnings are thrown by x2.pm. This test checks for these
warnings. 
...
2.2. Phase 2: Testing x2.pm's behaviour under normal conditions:
------------------------------------------------------------------
...
First, it checks if x2.pm works when the frequencies are given in a
...
Second it checks the actual calculations of x2.pm. The output target
...
Statistical library package x2.pm has been tested for erroneous
