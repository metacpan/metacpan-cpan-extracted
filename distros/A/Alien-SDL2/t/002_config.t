...
like( Alien::SDL2->config('version'), qr/([0-9]+\.)*[0-9]+/, "Testing config('version')" );
like( Alien::SDL2->config('prefix'), qr/.+/, "Testing config('prefix')" );

### check if prefix is a real directory
my $p = Alien::SDL2->config('prefix');
diag ("Prefix='$p'");
is( (-d Alien::SDL2->config('prefix')), 1, "Testing existence of 'prefix' directory" );

### check if list of ld_shared_libs contains existing files
my $l_result = 1;
foreach (@{Alien::SDL2->config('ld_shared_libs')}) {
...
my $m_result = 1;
foreach (values %{Alien::SDL2->config('ld_shlib_map')}) {
...
my $p_result = 1;
foreach (@{Alien::SDL2->config('ld_paths')}) {
