...
my($handle)=Meta::IO::File->new_reader($input);
while(!$handle->eof()) {
	my($line)=$handle->cgetline();
	Meta::Utils::Output::println($line);
}
$handle->close();
...
Error(3), Meta::IO::File(3), Meta::Utils::Opts::Opts(3), Meta::Utils::Output(3), Meta::Utils::System(3), strict(3)
