...
stdout_is_eq($cmd_full, $stdout, "-i=file");
stderr_is_eq($cmd_full, "", $cmd_full);

$cmd_full = "cat $input_file | " . $cmd_base . " -i=- " . $cmd_suffix;
exit_is_num($cmd_full, 0);
stdout_is_eq($cmd_full, $stdout, "-i=-");
stderr_is_eq($cmd_full, "", $cmd_full);

$cmd_full = "cat $input_file | " . $cmd_base . " " . $cmd_suffix;
exit_is_num($cmd_full, 0);
stdout_is_eq($cmd_full, $stdout, "-i is ommited");
stderr_is_eq($cmd_full, "", $cmd_full);


stdout_is_eq($cmd_base . " --input=$input_file " . $cmd_suffix, $stdout, "--input=file");
stdout_is_eq("cat $input_file | " . $cmd_base . " --input=- " . $cmd_suffix, $stdout, "--input=-");
stdout_is_eq("cat $input_file | " . $cmd_base . " " . $cmd_suffix, $stdout, "--input is ommited");
