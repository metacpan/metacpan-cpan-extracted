...
L<Date> is almost fully compatible with L<Class::Date>, but has several more features and much greater perfomance.
It is written fully in C++.

L<Date> supports dates between -2147483648/01/01 00:00:00 and 2147483647/12/31 23:59:59 (on 64bit perls).

With L<Date> you can perform some operations even faster than in plain C program using stdlibc functions.
See L<Time::XS> why.
...
=item YYYY-MM-DD hh:mm[:ss][.mks][±hh[:]mm]

=item YYYY/MM/DD hh:mm[:ss][.mks][±hh[:]mm]

=item YYYYMMDDThhmmss[.mks][±hh[:]mm]

=item YYYY-MM-DDThh:mm:ss[.mks][±hh[:]mm]

=back

If timezone offset designator is present, the date is created in unnamed timezone with constant offset from GMT (no transition rules). If you pass your
own timezone (second parameter to constructor or set()) in this case, it will be ignored (your parameter).
...
If there is any error while creating an object, properties error() and errstr() will be set and the date is set to epoch=0.
...
With no arguments returns information about object's timezone. See L<Time::XS/tzget([$zone])>.
...
Returns key-value list. Keys are 'year', 'month', 'day', 'hour', 'min', 'sec' and optionally 'mksec'
...
Hash keys: 'year' (YYYY), 'month' [1-12], 'day', 'hour', 'min', 'sec', 'mksec'.
Array: [$year (YYYY), $month [1-12], $day, $hour, $min, $sec, $mksec]
...
Adds a relative date to date object. If second operand is not an object then it's created (L<Date::Rel>).
...
Subtracts a relative date or another date from the date object. In case of relative date the result is a L<Date> object.
Otherwise the result is L<Date::Int>.
If second operand is not an object then it's created (L<Date::Rel> or L<Date>).

=head4 '-=', subtract($reldate | $rel_string | $seconds | \@rel_array | \%rel_hash)

Same as C<subtract()> but changes current object instead of creating new one. Operand can only be a L<Date::Rel> object.
...
=item L<JSON::XS> serialization with convert_blessed
...
L<Date> operates 40-70x faster than Class::Date, tests were performed on Core i7 3.2Ghz, MacOSX Lion, perl 5.12.4
