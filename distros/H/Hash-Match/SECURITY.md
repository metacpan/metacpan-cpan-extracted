...
Any security vulnerabilities in Hash-Match are covered
by this policy.
...
by this policy unless they affect Hash-Match, or
Hash-Match can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that
uses Hash-Match, or plugins to it that are not included
with the Hash-Match distribution) are not covered by
this policy.

## Which Versions of this Software are Supported?

The maintainer(s) will only commit to releasing security fixes for the
latest version of Hash-Match.

Note that the Hash-Match project only supports major
versions of Perl released in the past ten (10) years, even though
Hash-Match will run on older versions of Perl.  If a
