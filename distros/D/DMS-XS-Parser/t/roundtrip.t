...
sub roundtrip {
    my ($src) = @_;
    my $doc = DMS::XS::Parser::decode_document($src);
    return DMS::XS::Parser::encode($doc);
}
...
# forms not guaranteed without C-side original_forms recording).
{
    ...
    my $d1 = DMS::XS::Parser::decode_document($src);
    my $d2 = DMS::XS::Parser::decode_document($out);
...
# blessed DMS::Integer scalar refs, identical in shape to the pure-Perl
...
#    travel with it.
{
    ...
    my $doc = DMS::XS::Parser::decode_document($src);
    $doc->{body}->{port} = DMS::Integer->new(5432);
    my $emitted = DMS::XS::Parser::encode($doc);
    my $doc2 = DMS::XS::Parser::decode_document($emitted);
...
# 10. Combined: update one value, delete one key, insert a new one.
{
    my $src = "# A\na: 1   # a-trail\n# B\nb: 2   # b-trail\n# C\nc: 3   # c-trail\n";
    my $doc = DMS::XS::Parser::decode_document($src);
    $doc->{body}->{a} = DMS::Integer->new(100);   # update
    delete $doc->{body}->{b};                     # delete
    $doc->{body}->{d} = DMS::Integer->new(4);     # insert
    my $emitted = DMS::XS::Parser::encode($doc);
    my $doc2 = DMS::XS::Parser::decode_document($emitted);
...
#     still work and produce the same result as decode / encode.
{
    ...
    my $src = "alpha: 1\nbeta: 2\n";
    my $body_new = DMS::XS::Parser::decode($src);
    my $body_old = DMS::XS::Parser::parse($src);
    is_deeply($body_old, $body_new, 'parse() alias matches decode()');

    my $doc_new = DMS::XS::Parser::decode_document($src);
    my $doc_old = DMS::XS::Parser::parse_document($src);
    is_deeply($doc_old->{body}, $doc_new->{body},
        'parse_document alias still callable, returns equivalent body');

    my $emit_new = DMS::XS::Parser::encode($doc_new);
    my $emit_old = DMS::XS::Parser::to_dms($doc_new);
    is($emit_old, $emit_new, 'to_dms() alias matches encode()');

    my $emit_lite_new = DMS::XS::Parser::encode_lite($doc_new);
    my $emit_lite_old = DMS::XS::Parser::to_dms_lite($doc_new);
