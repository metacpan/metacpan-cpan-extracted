...
=item * If reputation falls to B<20 or below>, the peer is automatically disconnected and blacklisted for the session.
...
Checks if the peer is still connected.
    ...
    say 'Peer is alive' if $peer->connected;

This method returns true if the connection to this peer has not been closed. Once disconnected, this method returns
false.
