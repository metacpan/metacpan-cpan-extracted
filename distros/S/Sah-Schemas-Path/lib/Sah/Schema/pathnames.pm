...
1;
# ABSTRACT: List of filesystem path names
...
Sah::Schema::pathnames - List of filesystem path names

=head1 VERSION

This document describes version 0.021 of Sah::Schema::pathnames (from Perl distribution Sah-Schemas-Path), released on 2023-10-29.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("pathnames*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("pathnames", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("pathnames", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("pathnames*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"pathnames*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('pathnames*', name=>'Pathnames')
     );
 }

 use My::Types qw(Pathnames);
 Pathnames->assert_valid($data);
