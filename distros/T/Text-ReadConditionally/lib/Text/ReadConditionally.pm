package Text::ReadConditionally;
...
# Author Alain D D Williams <addw@phcomp.co.uk> March 2015, 2016 Copyright (C) the author.
...
Text::ReadConditionally - Macro processing built into IO::File replacement

=head1 SYNOPSIS

    use Text::ReadConditionally;

    my $fh = new Text::ReadConditionally(File => './input.file') or
        die "Startup error: $Text::ReadConditionally::errstr\n";

    while(<$fh>) {
        print $_;    # Or other processing of input
    }

    die($Text::ReadConditionally::errstr . "\n")
        if($fn->error);
...
C<Text::ReadConditionally> returns lines from the input stream.
...
C<new> returns an C<Text::ReadConditionally> object, C<undef> on error.
...
If you share a C<Math::Expression> object between different C<Text::ReadConditionally> objects
...
If you wish to change this, provide this option. So to use directives like C<< #if >> go:
    ...
    new Text::ReadConditionally(File => 'fred', DirStart> => '#')
...
Eg: allow the start sequence to be either C<.> or C<%>:
    ...
    new Text::ReadConditionally(File => 'fred', DirStartRE> => '[.%]')
...
Do nothing. The application should check the method C<error> and look at C<$Text::ReadConditionally::errstr>.
...
C<open> returns an C<Text::ReadConditionally> object, C<undef> on error.
...
If there is an error in opening a file look at C<$Text::ReadConditionally::errstr>;
...
C<close> returns an C<Text::ReadConditionally> object, C<undef> on error.
...
Most methods return C<undef> if there is an error. There will be a reason in C<$Text::ReadConditionally::errstr>.
...
what it reads.
    ...
    my $fh = new Text::ReadConditionally(Fd => \*STDIN, File => 'Standard input') or
        die "Startup error: $Text::ReadConditionally::errstr\n";
...
Please report any bugs or feature requests to C<bug-text-readconditionally at rt.cpan.org>, or through
the web interface at L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=Text-ReadConditionally>.  I will be notified, and then you'll
...
You can find documentation for this module with the perldoc command.
    ...
    perldoc Text::ReadConditionally
...
L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=Text-ReadConditionally>

=item * AnnoCPAN: Annotated CPAN documentation

L<http://annocpan.org/dist/Text-ReadConditionally>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/Text-ReadConditionally>

=item * Search CPAN

L<http://search.cpan.org/dist/Text-ReadConditionally/>
...
Alain Williams, C<< <addw@phcomp.co.uk> >> April 2015.
