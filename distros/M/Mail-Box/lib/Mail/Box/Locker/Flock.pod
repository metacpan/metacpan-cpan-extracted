...
Improves base, see L<Mail::Box::Locker/"Locking">
...
The C<$folder> is already locked, but you attempt to lock it again.  The
...
To check whether the C<$filename> is used to C<flock> a C<$folder>, the file must be
opened.  Apparently this fails, which does not mean that the folder is
locked neither that it is unlocked.
Cast by C<isLocked()>

=item Error: Unable to open flock file $file for $folder: $!

For C<flock>-ing a C<$folder> it must be opened, which does not succeed for the
specified reason.
Cast by C<lock()>

=item Error: Will never get a flock at $file for $folder: $!

Tried to C<flock> the C<$folder>, but it did not succeed.  The error code received
