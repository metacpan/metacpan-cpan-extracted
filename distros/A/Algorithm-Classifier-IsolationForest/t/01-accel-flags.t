...
ok( !$has_openmp || $has_c,
    'OpenMP implies Inline::C (parallel scoring needs the C backend)' );
ok( !$has_simd || $has_openmp,
    'SIMD implies OpenMP (omp simd pragma is gated on _OPENMP)' );

diag( sprintf 'Active backend flags: HAS_C=%d HAS_OPENMP=%d HAS_SIMD=%d',
