...
This option is ignored on systems where it's not supported.
...
the same socket in multiple child processes).

C<sock> and C<host>, C<port>, C<reuse_port>, C<domain> are mutually exclusive.
