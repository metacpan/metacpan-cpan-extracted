...
## it's much better.
## Same license as T::RL::Perl. But awesome.
## Fixes: UTF-8 safe now (crosses fingers) on Mac OS X, Ubuntu and more
## Upgrades: Repaintable readline/removeable readline, signal chaining,
##           character count.
##
############################################################################
##
...
sub preinit
{
    ...
    *rl_delete_selection = \$var_DeleteSelection; # Alias
    ...
    $rl_scroll_nextline = 0 unless defined $rl_scroll_nextline;
...
# These functions are custom for TTYtter and are used by the client.
sub rl_removereadline
# call to remove the readline from the screen temporarily.
{
    my $out = "\r" . (" " x ($rl_screen_width || 79))  . "\r";
    print $term_OUT $out;
}

sub rl_redisplay
# call to force a repaint. won't respond if we're not displaying a prompt.
{
    return unless ($in_readline);
    $force_redraw = 1;
    redisplay();
}
sub rl_hook_background_control
# call to bind background control to a pid (USR1 = halt output, USR2 = ok).
{
...
sub get_ornaments_selected {
    ...
    local $^W=0;
    my $Orig = $Term::ReadLine::TTYtter::term->ornaments(); 
    eval {
	...
	my @set = @$rl_term_set;
        ...
        $Term::ReadLine::TTYtter::term->ornaments
            (join(',', (split(/,/, $Orig))[0,1]) . ',mr,me') ;
        @set[4,5] = @$rl_term_set[2,3];
        $Term::ReadLine::TTYtter::term->ornaments($Orig);
	@$rl_term_set = @set;
    } else {
...
sub readline_dumb {
	...
	local $/ = "\n";
	return undef
          if !defined($line = $Term::ReadLine::TTYtter::term->get_line);
	chomp($line);
	$| = $oldbar;
	select $old;
	return $line;
...
#    }
	...
	*KeyMap = $var_EditingMode;           # JP: added
    ...
    $AcceptLine = "\x{263A}".$AcceptLine;
#print STDOUT "hex line is ", unpack("H*", $AcceptLine), "\n";
    $AcceptLine = pack("U0H*", substr(unpack("U0H*", $AcceptLine), 6));
#print STDOUT "line is ",unpack("H*", $AcceptLine), "\n"; 
...
$dline = $dline.($dont_use_counter ? '       ' :
    ...
    #print $term_OUT "xyz" . $prompt . $dline;
	#$force_redraw = 1;
...
# use our stubbed out getc_hex to safely return hex fragments of a UTF-8 char.
	...
	# this is a gross hack to deal with ISO-8859-1 getting in our
	# stream. it should work even if we get multiple Latin-1 chars
	# back to back, but could spew invalid UTF-8, so deal or fix your
	# keymap locale, beyotches.
	my $i = &getc_hex;
	last if (hex($i) < 128 || hex($i) > 191);
	$key .= $i;
    }
    $key = (length($key) == 2)
	? ((hex($key) < 128) ? pack("H*", $key) : 
	   pack("U*", unpack("C*", pack("H*", $key)))) # AUGH THE PAIN THE PAIN
	: pack("U0H*", $key);

#print STDOUT "result = ", unpack("H*", $key), "\n";
