...
# Testing that DIBitmap.dll has the same version as DIBitmap.pm
...
my $version = $Win32::GUI::DIBitmap::VERSION . '00';
if($version =~ m/^(\d+)\.(\d\d)(\d\d)/) {
    ($maj_pm, $min_pm, $rc_pm) = ($1, $2, $3);
}

my ($maj_rc, $min_rc, $rc_rc, $extra) = Win32::GUI::GetDllVersion('DIBitmap.' . $Config{dlext});
