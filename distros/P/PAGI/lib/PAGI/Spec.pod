...
C<version> -- the core PAGI specification version, e.g. C<'0.5'>

=item -

C<spec_version> -- the protocol sub-spec version, e.g. C<'0.6'>. It may be
...
key names an extension (e.g. C<tls>, C<fullflush>) mapping to a
...
is B<finished with the event>: it has validated it and consumed it -- including
any application-owned resource whose lifetime is tied to that send -- and has
either accepted the resulting output into its outbound processing path or
discarded it because the connection had ended. The application may release such
a resource after the Future resolves. Whether output was accepted or discarded
is not reported by the Future; applications that care consult the protocol's
...
A send Future still pending when the server detects that the connection -- or,
on a multiplexed transport, this scope's stream -- has ended MUST settle
promptly, as part of the server's disconnect processing, and MUST settle by
resolving successfully once the server has stopped using the event and any
application-owned resource it references. The server MUST NOT fail a pending
send with a disconnect-derived error and MUST NOT cancel it. A send Future that
has already settled is never rewritten: in particular, a send that failed for
an event-validation or event-resource reason stays failed. Because an
application must await every send (see L</Application Completion Contract>), a
server that left a send Future pending across a disconnect would deadlock a
conforming application; prompt settlement is what makes the required awaiting
safe.
...
Applications SHOULD NOT cancel the Future returned by C<$send>; the effect of
cancelling one is unspecified.
...
B<The value the Future resolves to is inert.> Completion is signalled by the
Future resolving, errors by it failing, and every client-visible result by
events sent through C<$send> (with connection facts such as C<response_started>
exposed on the C<pagi.connection> object). The server assigns the resolved
value no meaning and B<must not> act on it; a conformant application resolves to
nothing meaningful.

Frameworks layered above the gateway B<may> use the resolved value for their own
composition -- for example, returning a response object up an internal call
chain -- but B<must> convert any such value into protocol events at the gateway
boundary. A framework value B<must not> escape to the server as the
...
A middleware propagates the inner application's B<Future> -- its completion and
any exception -- not a value. The C<return> hands the inner application's
resolution outward so completion and errors flow unchanged; the resolved value
itself is inert (see L</Application Completion Contract>). A middleware that
needs to observe or rewrite the response does so by wrapping C<$send> or
C<$receive>, never by reading the return value.
...
Every receive Future already pending when the server detects the
disconnect B<must> be resolved with that disconnect event, however many are
pending: the event reports the scope's end, so no waiting receive is left
behind.
...
use the protocol's disconnect event and the C<pagi.connection> state
object instead.
...
0.5 (Draft): Send Completion Contract: successful resolution redefined as the
server being finished with the event and its resources (accepted into
outbound processing, or discarded because the connection ended); a send
Future still pending when the connection or stream ends settles promptly by
resolving successfully -- never failed with a disconnect-derived error, never
cancelled, an already-settled send never rewritten; a receive Future pending
at disconnect detection resolves with the protocol's disconnect event;
applications should not cancel send Futures. See L<PAGI::Spec::Www> for the
protocol-level 0.4 changes.
