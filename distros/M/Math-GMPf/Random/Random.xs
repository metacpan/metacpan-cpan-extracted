...
SV * Rgmp_randinit_default(pTHX) {
     ...
     if(rand_obj == NULL) croak("Failed to allocate memory in Math::GMPf::Random::Rgmp_randinit_default function");
     obj_ref = newSV(0);
     obj = newSVrv(obj_ref, "Math::GMPf::Random");
...
SV * Rgmp_randinit_mt(pTHX) {
     ...
     New(1, rand_obj, 1, gmp_randstate_t);
     if(rand_obj == NULL) croak("Failed to allocate memory in Math::GMPf::Random::Rgmp_randinit_mt function");
     obj_ref = newSV(0);
     obj = newSVrv(obj_ref, "Math::GMPf::Random");
...
SV * Rgmp_randinit_lc_2exp(pTHX_ mpz_t* a, SV * c, SV * m2exp) {
     ...
     New(1, rand_obj, 1, gmp_randstate_t);
     if(rand_obj == NULL) croak("Failed to allocate memory in Math::GMPf::Random::Rgmp_randinit_lc_2exp function");
     obj_ref = newSV(0);
     obj = newSVrv(obj_ref, "Math::GMPf::Random");
...
SV * Rgmp_randinit_lc_2exp_size(pTHX_ SV * size) {
     ...
     New(1, rand_obj, 1, gmp_randstate_t);
     if(rand_obj == NULL) croak("Failed to allocate memory in Math::GMPf::Random::Rgmp_randinit_lc_2exp_size function");
     obj_ref = newSV(0);
     obj = newSVrv(obj_ref, "Math::GMPf::Random");
...
SV * Rgmp_randinit_set(pTHX_ gmp_randstate_t * op) {
     ...
     New(1, rand_obj, 1, gmp_randstate_t);
     if(rand_obj == NULL) croak("Failed to allocate memory in Math::GMPf::Random::Rgmp_randinit_set function");
     obj_ref = newSV(0);
     obj = newSVrv(obj_ref, "Math::GMPf::Random");
...
SV * Rgmp_urandomb_ui(pTHX_ gmp_randstate_t * state, SV * n) {
     ...
     if(req > max) croak("In Math::GMPf::Random::Rgmp_urandomb_ui, requested %u bits, but %u is the maximum allowed", (int)req, (int)max);
...
SV * Rgmp_randinit_default_nobless(pTHX) {
     ...
     New(1, rand_obj, 1, gmp_randstate_t);
     if(rand_obj == NULL) croak("Failed to allocate memory in Math::GMPf::Random::Rgmp_randinit_default_nobless function");
     obj_ref = newSV(0);
...
SV * Rgmp_randinit_mt_nobless(pTHX) {
     ...
     New(1, rand_obj, 1, gmp_randstate_t);
     if(rand_obj == NULL) croak("Failed to allocate memory in Math::GMPf::Random::Rgmp_randinit_mt_nobless function");
     obj_ref = newSV(0);
     obj = newSVrv(obj_ref, NULL);
     gmp_randinit_mt(*rand_obj);
...
SV * Rgmp_randinit_lc_2exp_nobless(pTHX_ mpz_t* a, SV * c, SV * m2exp) {
     ...
     New(1, rand_obj, 1, gmp_randstate_t);
     if(rand_obj == NULL) croak("Failed to allocate memory in Math::GMPf::Random::Rgmp_randinit_lc_2exp_nobless function");
     obj_ref = newSV(0);
     obj = newSVrv(obj_ref, NULL);
     gmp_randinit_lc_2exp(*rand_obj, *a, (unsigned long)SvUV(c), (mp_bitcnt_t)SvUV(m2exp));
...
SV * Rgmp_randinit_lc_2exp_size_nobless(pTHX_ SV * size) {
     ...
     New(1, rand_obj, 1, gmp_randstate_t);
     if(rand_obj == NULL) croak("Failed to allocate memory in Math::GMPf::Random::Rgmp_randinit_lc_2exp_size_nobless function");
     obj_ref = newSV(0);
     obj = newSVrv(obj_ref, NULL);
     ret = gmp_randinit_lc_2exp_size(*rand_obj,(mp_bitcnt_t)SvUV(size));
...
SV * Rgmp_randinit_set_nobless(pTHX_ gmp_randstate_t * op) {
     ...
     New(1, rand_obj, 1, gmp_randstate_t);
     if(rand_obj == NULL) croak("Failed to allocate memory in Math::GMPf::Random::Rgmp_randinit_set_nobless function");
     obj_ref = newSV(0);
     obj = newSVrv(obj_ref, NULL);
     gmp_randinit_set(*rand_obj, *op);
...
/* Provide a duplicate of Math::GMPf::_has_pv_nv_bug. *
 * This allows GMPf.pm to determine the value of      *
 * the constant GMPF_PV_NV_BUG at compile time.       */
