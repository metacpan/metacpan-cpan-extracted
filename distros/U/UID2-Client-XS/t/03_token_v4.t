...
use Test::More;
use Crypt::Misc qw(encode_b64 encode_b64u decode_b64 decode_b64u);
...
subtest InvalidPayload => sub {
    ...
    my $payload = decode_b64u($advertising_token);
    $payload =~ s/.\z//;
    $advertising_token = encode_b64u($payload);
    is $client->decrypt($advertising_token, $now)->{status}, UID2::Client::XS::DecryptionStatus::INVALID_PAYLOAD;
};
