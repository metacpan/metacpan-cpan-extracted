...
1;
# ABSTRACT: Left-trim (remove leading whitespace) values and/or fields in CSV
...
App::CSVUtils::csv_ltrim - Left-trim (remove leading whitespace) values and/or fields in CSV

=head1 VERSION

This document describes version 1.021 of App::CSVUtils::csv_ltrim (from Perl distribution App-CSVUtils), released on 2023-03-02.
...
Usage:
 ...
 csv_ltrim(%args) -> [$status_code, $reason, $payload, \%result_meta]

Left-trim (remove leading whitespace) values andE<sol>or fields in CSV.

Examples:

=over

=item * Left-trim values in a CSV:

 csv_ltrim(input_filename => "FILE.csv");
