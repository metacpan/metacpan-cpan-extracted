...
Locale::Intl::Exception - Locale International Exception Class

=head1 SYNOPSIS

    my $ex = Locale::Intl::Exception->new({
        code => 400,
        type => $error_type,
...
or, providing a list of string that will be concatenated:
    ...
    my $ex = Locale::Intl::Exception->new( "Some error", "has occurred:", $details );
...
This method is called by perl when you call L<perlfunc/die> with no parameters and C<$@> is set to a L<Locale::Intl::Exception> object.
...
Provided with a message string, this will create a new L<Locale::Intl::Exception> object and call L<perlfunc/"die"> with it.
