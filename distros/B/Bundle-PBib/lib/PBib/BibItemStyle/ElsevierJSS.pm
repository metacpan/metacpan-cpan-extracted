# --*-Perl-*--
# $Id: ElsevierJSS.pm 10 2004-11-02 22:14:09Z tandler $
#

package PBib::BibItemStyle::ElsevierJSS;
use strict;
use warnings;
#use English;

=head1 package PBib::BibItemStyle::ElsevierJSS;

Based on the vague description at http://www.elsevier.com/....
...
sub new {
  ...
  $self->{'keywords'} = $self->ElsevierJSS_keywords();
  return $self;
}
...
sub format_in_ed_booktitle { my ($self, $check) = @_;
  ...
  return $self->entryNotEmpty('Editors') ?
	"In: " . $self->format_editors() . ", $Booktitle" :
	"In: $Booktitle";
}
