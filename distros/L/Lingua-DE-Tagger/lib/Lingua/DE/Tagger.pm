...
our %hash_memoize_replacement = ();

sub _assign_tag {

        my ( $self, $prev_tag, $word) = @_;

	if ( $hash_memoize_replacement{ $prev_tag . ' ' . $word } ) {
#		return $hash_memoize_replacement{ $prev_tag . ' ' . $word };
	}
...
#                $self->{'relax'} = 1;
                ...
                if( $probability > $best_so_far ) { 
                        ...
                        $best_tag = $tag;
			print 'best_tag set to: ' . $best_tag . "\n";
                }
        }

	$hash_memoize_replacement{ $prev_tag . ' ' . $word } = $best_tag;

	print 'best_tag is: ' . $best_tag . "\n";
