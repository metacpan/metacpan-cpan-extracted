...
subtest 'and the tree holds what it should' => sub {
	cmp_ok(scalar @MODULES, '>=', 7, 'seven modules or more');
	ok((grep { $_ eq 'Game::Mahjong' } @MODULES), 'the top module is one of them');
	ok((grep { $_ eq 'Game::Mahjong::Tiles' } @MODULES), 'and so is the table');
	ok(defined &Game::Mahjong::Tiles::is_green, 'and the XS predicates are installed');
};
