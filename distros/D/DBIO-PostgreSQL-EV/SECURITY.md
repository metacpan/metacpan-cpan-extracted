This is the Security Policy for the Perl DBIO-PostgreSQL-EV distribution.
...
[DBIO-PostgreSQL-EV on CPAN](https://metacpan.org/dist/DBIO-PostgreSQL-EV) or the main branch of the
[DBIO-PostgreSQL-EV git repository](https://codeberg.org/dbio/dbio-postgresql-async).
...
DBIO-PostgreSQL-EV is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO-PostgreSQL-EV are covered by this policy.
...
they affect DBIO-PostgreSQL-EV, or DBIO-PostgreSQL-EV can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO-PostgreSQL-EV, or plugins to it that are not included with the DBIO-PostgreSQL-EV
distribution) are not covered by this policy.

## Supported Versions of DBIO-PostgreSQL-EV

Only the latest release of DBIO-PostgreSQL-EV will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO-PostgreSQL-EV will run on older versions of Perl. If a
...
that are required for DBIO-PostgreSQL-EV to work. However, some of these
