...
The first section defines the most common data types for and its acceptable
values. Please note that different chart L<Types|Chart::Manual::Types>
react to different of properties. Therefore the 
L<section section|/"BY CHART TYPE"> groups all properties by chart type
or set of types, where they have an effect in a tabular manner.
