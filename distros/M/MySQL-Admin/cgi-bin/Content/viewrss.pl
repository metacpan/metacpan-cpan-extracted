...
if (defined $url) {
    $url = $url =~ /(\d+)/ ? $1 : 1;
    my @o = $m_oDatabase->fetch_array("select url from blogs where id = ? && `right` <= ?",
                                      $url, $m_nRight);
    $url = $o[0] ? $o[0] : $url;
    my $rss = new XML::RSS;
    use LWP::UserAgent;
    ...
    push @content,
      qq(</td></tr></table></td></tr></table><A href="$url"><img border="0" align="right" src="../images/rss.png" alt=""/></a>);
    ...
    my @menu =
      $m_oDatabase->fetch_AoH('select id,name from blogs  where `right` <= ? order by id',
                              $m_nRight);
    my @ret;
    for (my $i = 0 ; $i <= $#menu ; $i++) {
        use HTML::Entities;
        my $url =
          "requestURI('$ENV{SCRIPT_NAME}?action=viewrss&url=$menu[$i]->{id}','feeds','feeds')";
        push @ret,
          {
            text    => $menu[$i]->{name},
            onclick => $url,
