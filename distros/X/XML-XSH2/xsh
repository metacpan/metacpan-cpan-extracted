...
use vars qw($opt_q $opt_w $opt_i $help $opt_V $opt_E $opt_e $opt_d
            ...
            $opt_l $opt_n $opt_p $opt_P $usage $manpage $opt_HTML $opt_F
...
if ($process and ($input or $output)) {
  print STDERR "Options -P is incompatible with -I and -O!\n";
  $usage=1;
}
if ($opt_F and ($process or $input or $output)) {
  print STDERR "Options -F is incompatible with -I, -O, and -P!\n";
  $usage=1;
}
...
my $string;
$opt_a=1 if $opt_F;
if (!$opt_C and !$opt_F and !$opt_a and $opt_l eq "" and -f $ARGV[0]) {
  # suppose the 1st argument is a script,
  # and the rest is its parameters
  $opt_l = shift @ARGV;
  @XML::XSH2::Map::ARGV=@ARGV;
} elsif ($opt_C and ($opt_F or $opt_a)) {
  $string  = shift @ARGV;
  @XML::XSH2::Map::ARGV=@ARGV;
} elsif ($opt_F and !$opt_l) {
  $string  = shift @ARGV;
  @XML::XSH2::Map::ARGV=@ARGV;
} elsif ($opt_a) {
  @XML::XSH2::Map::ARGV=@ARGV;
...
print STDERR "\$INPUT_FILENAME := '$input'\n" if $opt_d;
print STDERR "\$INPUT := open \$INPUT_FILENAME\n" if $opt_d;
xsh_set_script('<CMD-LINE>');
if ($input) {
  $XML::XSH2::Map::INPUT_FILENAME=$input;
  xsh("\$INPUT := open \$INPUT_FILENAME;");
}
if ($output) {
  $XML::XSH2::Map::OUTPUT_FILENAME=$output;
}
...
if ($opt_l) {
  ...
  if ($opt_F) {
    ...
    xsh("for \$INPUT_FILENAME in {\@ARGV} { \$INPUT := open \$INPUT_FILENAME; do { $s\n};\n$string\n}");
    undef $string;
    exit unless $opt_i;
  } else {
    xsh($s);
...
if ($string) {
  ...
  if ($opt_F) {
    $string = "for \$INPUT_FILENAME in {\@ARGV} { \$INPUT := open \$INPUT_FILENAME; $string }";
    print "xsh> $string\n" if ($opt_i and not $opt_q);
    xsh($string);
    exit unless $opt_i;
  } else {
...
xsh_set_script('<CMD-LINE>');
if ($output) {
  xsh('save --file $OUTPUT_FILENAME $INPUT');
}
...
C<--commands> or without C<--load>, the first argument should contain XSH commands and the
rest are the passed to via C<@ARGV>.

=item B<--files|-F>

Indicate, that the command-line arguments should be treated as input
filenames. If used with C<--commands> or without C<--load>, the first
argument should contain XSH commands. A XSH script specified with
C<--load> or XSH commands specified with C<--commands> (or both in
this order) are evaluated repeatedly on each input file. For example,
running
