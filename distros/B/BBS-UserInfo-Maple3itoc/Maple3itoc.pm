package BBS::UserInfo::Maple3itoc;
...
Create a BBS::UserInfo::Maple3itoc object, there are some parameters
that you can define:
...
sub _login {
    ...
    $bot->expect($self->{'timeout'}, '-re', '±zªº±b¸¹');
...
sub query {
    ...
    $bot->expect($timeout, '-re', '\[¼ÊºÙ\]\s(.+?)\s+\[¤W¯¸\]\s*(\d+)\s*¦¸\s*\[¤å³¹\]\s*?(\d+)\s*½g');
...
1; # End of BBS::UserInfo::Maple3itoc
