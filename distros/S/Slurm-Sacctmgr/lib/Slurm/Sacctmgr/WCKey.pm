...
#Overloaded to match on wckey, user, and cluster.
#Sort of pointless, think we need to match everything
{	my $obj = shift;
	croak "Must be called as an instance method at "
		unless $obj && ref($obj);

	#Do everything, because nothing else is necessarily unique
	my @fields = @rw_accessors;
...
=item B<wckey>: Workload Charactization Key

=item B<cluster>: cluster for the WCKey

=item B<user>: user for the WCKey
...
all have a very similar wckey interface (basically the differences are what
operations sacctmgr allows on different entity types, and how the results
of list/show operations get parsed).  

Since sacctmgr only allows one to show/list WCKeys, the only real method
provided by this class is B<sacctmgr_list($sacctmgr, %where)>.
This will return a list reference of all objects of entity type "wckey"
