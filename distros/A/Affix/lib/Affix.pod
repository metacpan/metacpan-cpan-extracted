...
=item * C<own($p, 1)>: Perl owns the memory; `free()` is called when `$p` goes out of scope.

=item * C<own($p, 0)>: Perl releases ownership. Useful when handing memory off to a C function.
