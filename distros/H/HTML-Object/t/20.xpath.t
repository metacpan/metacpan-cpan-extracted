...
# warn $tree->as_xml, "\n\n";
diag( "tree is '$tree' -> ", $tree->as_xml ) if( $DEBUG );
...

{
    ...
    if( $DEBUG && scalar( @kid_nodes ) != 2 )
    {
        diag( "\@kid_nodes contains: ", join( '', map{ $_->as_xml } @kid_nodes ) );
    }
}
...
{
    is( $xp->findvalue( '//*[@att2="vv"]', $tree ), 'gvkid1gvkid2gvkid3gvkid4gvkid5', q{findvalue( '//*[@att2="vv"]', $tree)} );
    is( $xp->findvalue( '//*[@att2]', $tree ), 'gvkid1gkid2 1gvkid2gkid2 2gvkid3gkid2 3gvkid4gkid2 4gvkid5gkid2 5',     q{findvalue( '//*[@att2]', $tree)} );
}
...
{
    use lib './t';
    use parent qw( minitree );
};

sub as_string { return( shift->as_xml ); }
...
sub as_xml
{
    ...
    return(
        "<" . $elt->getName . 
        join( '', map{ ' ' . $_->getName . '="' . $_->getValue . '"' } $elt->getAttributes ) . 
...
{
    use lib './t';
    use parent -norequire, qw( attribute );
};

sub as_string { return( sprintf( '%s="%s"', $_[0]->name, $_[0]->value ) ); }

sub as_xml { return( shift->as_string ); }
