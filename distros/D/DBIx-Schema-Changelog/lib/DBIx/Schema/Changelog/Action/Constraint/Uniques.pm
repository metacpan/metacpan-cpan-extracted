package DBIx::Schema::Changelog::Action::Constraint::Uniques;

=head1 NAME

DBIx::Schema::Changelog::Action::Constraint::Uniques - Action handler for constraint
...
sub add {
    ...
    return _replace_spare( $actions->{unique}, [ qq~unique_$name~, join( ',', @{ $col->{unique} } ) ] );
}
...
sub list_from_schema {
    my ( $self, $schema ) = @_;
    my $uniques = [];
    foreach ( @{ $self->do( $self->driver->actions->{list_unique_keys}, [] ) } ) {
        push(
            @$uniques,
            {
                type          => 'altertable',
