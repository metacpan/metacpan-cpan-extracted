...
if ('偁' =~ m'(.)'b) {
    if (length($1) == 1) {
        print qq{ok - 5 '偁'=~m'(.)'b; length(\$1)==1 $^X $__FILE__\n};
    }
    else {
        print qq{not ok - 5 '偁'=~m'(.)'b; length(\$1)==1 $^X $__FILE__\n};
    }
}
else {
    print qq{not ok - 5 '偁'=~m'(.)'b; length(\$1)==1 $^X $__FILE__\n};
...
if (@_ = '偁' =~ m'(.)'bg) {
    if (scalar(@_) == length('偁')) {
        if (grep( ! /^1$/, map { length($_) } @_)) {
            print qq{not ok - 6 \@_='偁'=~m'(.)'bg; grep(!/^1\$/,map{length(\$_)} \@_) $^X $__FILE__\n};
        }
        else {
            print qq{ok - 6 \@_='偁'=~m'(.)'bg; grep(!/^1\$/,map{length(\$_)} \@_) $^X $__FILE__\n};
        }
    }
    else {
        print qq{not ok - 6 \@_='偁'=~m'(.)'bg; grep(!/^1\$/,map{length(\$_)} \@_) $^X $__FILE__\n};
