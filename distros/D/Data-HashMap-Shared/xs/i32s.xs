...
new(char* class, SV* path_sv, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        ...
        char errbuf[SHM_ERR_BUFLEN]; const char* path = SHM_WRITE_PATH_ARG(path_sv, "path", "Data::HashMap::Shared::I32S", "new"); ShmHandle* map = shm_i32s_create(path, (uint32_t)max_entries, (uint32_t)lru_max, (uint32_t)ttl_default, (uint32_t)lru_skip, (uint64_t)arena_cap, (mode_t)file_mode, errbuf);
        if (!map) croak("Data::HashMap::Shared::I32S: %s", errbuf[0] ? errbuf : "unknown error");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)map);
        CK_MAX_SIZE(map, "Data::HashMap::Shared::I32S", RETVAL);
    OUTPUT:
        RETVAL
...
new_sharded(char* class, SV* path_prefix_sv, UV num_shards, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        const char* path_prefix = SHM_WRITE_PATH_ARG(path_prefix_sv, "path prefix", "Data::HashMap::Shared::I32S", "new_sharded");
        CK_U32(max_entries, "max_entries", "Data::HashMap::Shared::I32S"); CK_U32(lru_max, "lru_max", "Data::HashMap::Shared::I32S"); CK_U32(ttl_default, "ttl_default", "Data::HashMap::Shared::I32S"); CK_U32(lru_skip, "lru_skip", "Data::HashMap::Shared::I32S");
...
new_from_fd(char* class, int fd)
    CODE:
        char errbuf[SHM_ERR_BUFLEN];
        SHM_TAINT_CHECK("Data::HashMap::Shared::I32S->new_from_fd");
...
freeze(SV* self_sv)
    CODE:
        ...
        if (h->readonly || shm_is_fully_sealed(h)) croak("Data::HashMap::Shared::I32S->freeze: map is frozen (read-only)");
...
set_multi(SV* self_sv, ...)
    CODE:
        ...
        if (h->shard_handles) {
            ...
            WRSEQ_GUARD(h);
            for (int j = 0; j < _n; j++)
                count += shm_i32s_put_inner(h, _ks[j], _vs[j], (uint32_t)_vl[j], _vu[j], SHM_TTL_USE_DEFAULT);
        }
        RETVAL = count;
    OUTPUT:
        RETVAL
...
remove_multi(SV* self_sv, ...)
    CODE:
        ...
        if (h->shard_handles) {
            ...
            WRSEQ_GUARD(h);
            for (int j = 0; j < _n; j++)
                count += shm_i32s_remove_inner(h, _ks[j]);
...
unlink(SV* self_or_class, ...)
    CODE:
        if (sv_isobject(self_or_class) && sv_derived_from(self_or_class, "Data::HashMap::Shared::I32S")) {
            EXTRACT_MAP("Data::HashMap::Shared::I32S", self_or_class);
            if (h->path) {
                SHM_TAINT_CHECK("Data::HashMap::Shared::I32S->unlink");
                REEXTRACT_MAP("Data::HashMap::Shared::I32S", self_or_class);
            }
            RETVAL = shm_unlink_sharded(h);
        } else {
            if (items < 2) croak("Usage: Data::HashMap::Shared::I32S->unlink($path)");
            SvGETMAGIC(ST(1));
            RETVAL = shm_unlink_path(shm_write_path_arg(aTHX_ ST(1), "path", "Data::HashMap::Shared::I32S", "Data::HashMap::Shared::I32S->unlink"));
        }
    OUTPUT:
        RETVAL
