...
=head2 Multi-valued query parameters

Query parameters will be automatically added to the C<cgi_request> span
as tags with a C<http.query.> prefix. Since span tags cannot contain
references, query parameters appearing multiple times will be concatenated
with a comma.

You can use a different format by defining an C<opentracing_format_query_params>
method in your CGI Application. If present, this method will be called whenever
a query parameter is assigned to a tag, with the parameter name and all values
as an array reference (even if the parameter only has a single value) as arguments.
