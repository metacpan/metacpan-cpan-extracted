package QualysGuard::Response::TicketListDeleted;
...
sub new {
    ...
    if ( $self->exists('/TICKET_LIST_DELETED_OUTPUT/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/TICKET_LIST_DELETED_OUTPUT/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/TICKET_LIST_DELETED_OUTPUT/ERROR');
...
QualysGuard::Response::TicketListDeleted
