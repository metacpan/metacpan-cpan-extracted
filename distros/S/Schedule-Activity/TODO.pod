...
=item Per-activity goals (maybe)

=item Script incremental build supports configuration overwrite (maybe)
...
Setting goals per activity is being considered with/before 0.3.0.  This may not include script support since this can already be achieved with incremental construction and there are potential parsing issues from the commandline.
...
Rudimentary attribute reporting from the commandline is supported starting with 0.2.2, with additional options in 0.2.6.
...
Floating-point modulus is available starting with 0.2.6.
