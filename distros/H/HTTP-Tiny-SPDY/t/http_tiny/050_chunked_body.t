...
use t::http_tiny::Util    qw[tmpfile rewind $CRLF];
use HTTP::Tiny::SPDY;

{
    no warnings 'redefine';
    sub HTTP::Tiny::Handle::SPDY::can_read  { 1 };
    sub HTTP::Tiny::Handle::SPDY::can_write { 1 };
}

{
    my $body    = join($CRLF, map { sprintf('%x', length $_) . $CRLF . $_ } 'A'..'Z', '') . $CRLF;
    my $fh      = tmpfile($body);
    my $handle  = HTTP::Tiny::Handle::SPDY->new(fh => $fh);
    my $exp     = ['A'..'Z'];
    my $got     = [];
