package Net::Telnet::Wrapper::Device::Cisco::CATOS;

## ----------------------------------------------------------------------------------------------
## Net::Telnet::Wrapper::Device::Cisco::CATOS
##
## Device class for Cisco switches based on CATOS
##
## $Id: CATOS.pm 39 2007-07-11 14:29:01Z mwallraf $
## $Author: mwallraf $
...
sub new()  {
	...
	*$self->{'net_telnet_wrapper'}->{'device_class'} = "Cisco::CATOS";
	...
	*$self->{'net_telnet_wrapper'}->{'prompt_enable'} = '(enable)';	# when we see this we're in enable mode
	*$self->{'net_telnet_wrapper'}->{'prompt_config'} = '(config)';	# when we see this we're in config mode
	*$self->{'net_telnet_wrapper'}->{'terminal_length_cmd'} = 'set length 0';	# if set then the terminal length will be set to 0 before each command
...
Cisco::CATOS device class template.
