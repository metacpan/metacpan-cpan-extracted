...
Affix::Wrap - Automatic Generation of FFI Wrappers for Shared Libraries Written in C or C++
...
Automatically generate Affix bindings from C/C++ header files. C<Affix::Wrap> parses headers to extract functions,
structs, enums, typedefs, macros, and global variables, converting them into L<Affix> definitions.
...
Choose between Clang (recommended, compiler-grade accuracy) or a pure-Perl regex fallback:
...
Create a new binder instance with the header files and settings you need:
...
The internal object hierarchy returned by C<parse()>. Each node represents a parsed C entity.
