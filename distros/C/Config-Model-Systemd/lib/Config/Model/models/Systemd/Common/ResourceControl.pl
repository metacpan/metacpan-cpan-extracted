...
',
    ...
    'description' => {
      'AllowedCPUs' => 'This setting controls the C<cpuset> controller in the unified hierarchy.
...
This setting is supported only with the unified control group hierarchy.',
      'AllowedMemoryNodes' => 'These settings control the C<cpuset> controller in the unified hierarchy.
...
This setting is supported only with the unified control group hierarchy.',
      'BPFProgram' => 'C<BPFProgram> allows attaching custom BPF programs to the cgroup of a
...
',
      'BindNetworkInterface' => 'Takes the name of a network interface. This option causes every socket created by processes of this
...
20% CPU time on one CPU.',
      'CPUQuotaPeriodSec' => 'This setting controls the C<cpu> controller in the unified hierarchy.
...
configuration, so users should be careful to not mistake one for the other.',
      'CoredumpReceive' => 'Takes a boolean argument. This setting is used to enable coredump forwarding for containers
...
L<systemd-coredump(8)>.',
      'Delegate' => 'Turns on delegation of further resource control partitioning to processes of the unit. Units
...
Delegation|https://systemd.io/CGROUP_DELEGATION>.',
      'DelegateSubgroup' => 'Place unit processes in the specified subgroup of the unit\'s control group. Takes a valid
...
',
      'DisableControllers' => 'Disables controllers from being enabled for a unit\'s children. If a controller listed is
...
of the cgroup hierarchy is unmanaged by systemd.',
      'IOAccounting' => 'This setting controls the C<io> controller in the unified hierarchy.
...
in
L<systemd-system.conf(5)>.',
      'IODeviceLatencyTargetSec' => 'This setting controls the C<io> controller in the unified hierarchy.
...
Similar restrictions on block device discovery as for C<IODeviceWeight> apply, see above.',
      'IODeviceWeight' => 'This setting controls the C<io> controller in the unified hierarchy.
...
Similar restrictions on block device discovery as for C<IODeviceWeight> apply, see above.',
      'IOReadIOPSMax' => 'These settings control the C<io> controller in the unified hierarchy.
...
Similar restrictions on block device discovery as for C<IODeviceWeight> apply, see above.',
      'IOWeight' => 'These settings control the C<io> controller in the unified hierarchy.
...
and shutdown differently than during runtime.',
      'IOWriteBandwidthMax' => '*IOReadBandwidthMax',
      'IOWriteIOPSMax' => '*IOReadIOPSMax',
      'IPAccounting' => "Takes a boolean argument. If true, turns on IPv4 and IPv6 network traffic accounting for packets sent
...
per-user services.",
      'IPAddressAllow' => "Turn on network traffic filtering for IP packets sent and received over
...
them for IP security.",
      ...
      'IPEgressFilterPath' => 'Add custom network traffic filters implemented as BPF programs, applying to all IP packets
...
(requires C<Delegate>=C<yes>) instead of using this setting.',
      ...
      'ManagedOOMMemoryPressure' => 'Specifies how
...
for systemd-oomd to terminate.',
      'ManagedOOMMemoryPressureDurationSec' => "Overrides the default memory pressure duration set by
L<oomd.conf(5)> for
...
",
      'ManagedOOMMemoryPressureLimit' => 'Overrides the default memory pressure limit set by
L<oomd.conf(5)> for
...
This property is ignored unless C<ManagedOOMMemoryPressure>=C<kill>.
',
      'ManagedOOMPreference' => 'Allows deprioritizing or omitting this unit\'s cgroup as a candidate when
...
',
      ...
      'MemoryAccounting' => 'This setting controls the C<memory> controller in the unified hierarchy.
...
L<systemd-system.conf(5)>.',
      'MemoryHigh' => 'These settings control the C<memory> controller in the unified hierarchy.
...
boot-up and shutdown differently than during normal runtime.',
      'MemoryMax' => 'These settings control the C<memory> controller in the unified hierarchy.
...
boot-up and shutdown differently than during normal runtime.',
      'MemoryMin' => 'These settings control the C<memory> controller in the unified hierarchy.
...
boot-up and shutdown differently than during normal runtime.',
      'MemoryPressureThresholdSec' => "Sets the memory pressure threshold time for memory pressure monitor as configured via
...
L<systemd.time(7)> for
details on the permitted syntax.",
      'MemoryPressureWatch' => 'Controls memory pressure monitoring for invoked processes. Takes a boolean or one of
C<auto> and C<skip>. If C<no>, tells the service not
...
L<systemd-system.conf(5)>.',
      'MemorySwapMax' => 'These settings control the C<memory> controller in the unified hierarchy.
...
boot-up and shutdown differently than during normal runtime.',
      'MemoryZSwapMax' => 'These settings control the C<memory> controller in the unified hierarchy.
...
"Default Dependencies" for details.',
      'SocketBindAllow' => "Configures restrictions on the ability of unit processes to invoke L<bind(2)> on a
...
",
      ...
      'StartupAllowedCPUs' => '*AllowedCPUs',
      'StartupAllowedMemoryNodes' => '*AllowedMemoryNodes',
      'StartupCPUWeight' => '*CPUWeight',
      'StartupIOWeight' => '*IOWeight',
      'StartupMemoryHigh' => '*MemoryHigh',
      'StartupMemoryLow' => '*MemoryMin',
      'StartupMemoryMax' => '*MemoryMax',
      'StartupMemorySwapMax' => '*MemorySwapMax',
      'StartupMemoryZSwapMax' => '*MemoryZSwapMax',
      'TasksAccounting' => 'This setting controls the C<pids> controller in the unified hierarchy.
...
L<systemd-system.conf(5)>.',
      'TasksMax' => 'This setting controls the C<pids> controller in the unified hierarchy.
