...
Prty::File::Image - Informationen über Bild-Datei
...
sub new {
    ...
    return $class->SUPER::new(
        ...
        type=>undef,
        # Properties
        propertyH=>Prty::Hash->new->unlockKeys,
    );
}
...
sub extension {
    ...
    return $self->memoize('extension',sub {
        my ($self,$key) = @_;
        return Prty::Path->extension($self->path);
    });
}
...
sub type {
    ...
    return $self->memoize('type',sub {
        ...
        my $type = Prty::Image->type($self->path);
        if ($type eq 'jpeg') {
            $type = 'jpg';
        }
...
sub aspectRatio {
    my $self = shift;
    return Prty::Image->aspectRatio($self->width,$self->height);
}
...
Der Property-Hash ist ein Prty::Hash-Objekt, dessen Schlüssel
