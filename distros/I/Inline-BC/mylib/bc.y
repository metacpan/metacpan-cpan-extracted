...
argument_list 		: expression
			    {
			      if ($1 & 2) my_warn ("comparison in argument");
			...
			| argument_list ',' expression
			    {
			      if ($3 & 2) my_warn ("comparison in argument");
			      $$ = nextarg ($1,0,FALSE);
			    }
			| argument_list ',' NAME '[' ']'
			    {
...
expression		:  named_expression ASSIGN_OP 
			  ...
			  expression
			    {
			      if ($4 & 2) my_warn("comparison in assignment");
			      if ($2 != '=')
				{
				  sprintf (genstr, "%c", $2);
				  generate (genstr);
...
named_expression	: NAME
			...
			| NAME '[' expression ']'
			    {
			      if ($3 > 1) my_warn("comparison in subscript");
			      $$ = lookup($1,ARRAY);
			    }
			| Ibase
			    { $$ = 0; }
...
required_eol		: { my_warn ("End of line required"); }
			| ENDOFLINE
