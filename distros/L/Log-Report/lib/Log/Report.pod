...
=item B<report>( [\%options], $reason, $message|<$text,%params>, )
...
The C<$message> is a L<Log::Report::Message|Log::Report::Message> object (which is created with
the special translation syntax like L<__x()|Log::Report/"Messages (optionally translatable)">).

The message may also be a plain piece of C<$text>, or a
L<Log::Report::Exception|Log::Report::Exception> object. The C<%params> can be used to influence
...
=head2 Messages with Context

In L<Log::Report|Log::Report>, the message context (C<mgsctxt> in the PO-files --in the
...
=item import => $function|\@functions

[0.998] When not specified, the C<syntax> option determines the list
of C<@functions> which are being exported.  With this option, the C<syntax>
option is ignored and only the specified C<@functions> are imported.
...
=head3 Lightweight Log::Report::Optional

When you want to use L<Log::Report|Log::Report> features as cheap as possible, you
may use L<Log::Report::Optional|Log::Report::Optional> in your module.  In this case, your
...
initiate the full L<Log::Report|Log::Report> functionality, then your module will
also switch to full support mode.

=head3 Background ideas
...
=head4 » Example: extract run mode from Getopt::Long
...
=head4 » Example: the mode of a dispatcher

  my $mode = dispatcher(find => 'myname')->mode;

=head4 » Example: run-time change mode of a dispatcher
...
=head2 Interpolation via formats
...
create a L<Log::Report::Message|Log::Report::Message> object.
...
Distribution L<Log::Report::Lexicon|Log::Report::Lexicon> handles translations.  It will help
you manage all L<__x()|Log::Report/"Messages (optionally translatable)"> (and friends) format strings in GNU's PO file
format (F<https://www.gnu.org/software/gettext/manual/html_node/PO-Files.html>)

With L<Log::Report::Template|Log::Report::Template>, you can use L<Log::Report|Log::Report> with translations
in various website template systems.

Then, L<Dancer2::Plugin::LogReport|Dancer2::Plugin::LogReport> brings L<Log::Report|Log::Report> with exceptions
and translations to the Dancer2 web framework.

=head2 Compared to other solutions in Perl
...
In SCALAR context, only one dispatcher name accepted The L<dispatcher()|Log::Report/"Report production and configuration">
method returns the L<Log::Report::Dispatcher|Log::Report::Dispatcher> objects which it has accessed.
