...
This documents version 0.23 of Gzip::Faster
corresponding to git commit L<3ddd14cb6e3eae87e4d6ee8140474a26d39d41d2|https://github.com/benkasminbullock/gzip-faster/commit/3ddd14cb6e3eae87e4d6ee8140474a26d39d41d2> made on Fri Sep 18 15:46:02 2026 +0900.
...
A limitation of the module is that it is designed around the idea of
keeping the entire input and output in memory, so it is not suitable
for streaming compressing of huge entities. For these cases I suggest
piping the stream through the command-line gzip utility.
...
using L</copy_perl_flags>. The size of the output of L</unzip> can be
limited with L</max_size>.
...
For an example using L</deflate> to write a PNG image file, see L<F<t/png.t>|https://fastapi.metacpan.org/source/BKB/Gzip-Faster-0.23/t/png.t> in the module's tests.
...
(This example is included as L<F<file-name.pl>|https://fastapi.metacpan.org/source/BKB/Gzip-Faster-0.23/examples/file-name.pl> in the distribution.)
...
=head2 max_size
    ...
    $gf->max_size (1_000_000);

Limit the output of L</unzip> to a number of bytes. If the
uncompressed data would be longer, L</unzip> stops and throws a fatal
error. A value of 0, or no value, removes the limit, which is the
default. The functions like L</gunzip> are not limited.

C<max_size> was added in version 0.23.
...
As of version 0.23, there is no way to read or write the
...
=item Out of memory during inflate

(Fatal)

=item Internal error in zlib

(Fatal)

=item Unknown status %d from inflate

(Fatal) zlib returned an error code as given which is not one of the
known options.

=item Uncompressed data exceeds max_size of %d bytes

(Fatal) The user set a non-zero maximum size with L</max_size> and the
output exceeded that limit.

=item Zlib did not finish processing the string: %d bytes left

(Fatal) zlib failed to process all of the input buffer.

=item Zlib did not come to the end of the string: zlib_status = %d

(Fatal) zlib finished processing for a reason other than coming to the
end of the string. The status number reported is from zlib.
...
be found in the file L<F<README>|https://fastapi.metacpan.org/source/BKB/Gzip-Faster-0.23/README> in the distribution. The
...
(This example is included as L<F<inflate.pl>|https://fastapi.metacpan.org/source/BKB/Gzip-Faster-0.23/examples/inflate.pl> in the distribution.)
