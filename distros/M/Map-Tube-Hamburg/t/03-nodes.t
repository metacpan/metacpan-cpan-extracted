...

{
  ...
  is( $ret->id( ),   'hh_u211', 'Node id not correct for station named Schlump' );
  is( $ret->name( ), 'Schlump', 'Node name not correct for station named Schlump' );
  is( $ret->link( ), 'hh_u210,hh_u212,hh_u306,hh_u307', 'Links not correct for station named Schlump' );
  is( join( ',', sort map { $_->name( ) } @{ $ret->line( ) } ),  'U2,U3', 'Lines not correct for station named Schlump' );
}

{
  my $ret = $map->get_node_by_id('hh_u211');
  isa_ok( $ret,      'Map::Tube::Node' );
  is( $ret->id( ),   'hh_u211', 'Node id not correct for station id u211' );
  is( $ret->name( ), 'Schlump', 'Node name not correct for station id u211' );
}
