...
$VERSION = '0.72.71';
$ABSTRACT = 'Sim::OPT is an optimization and morphing program oriented to problem decomposition. It can be used with simulation programs receiving text files as input and emitting text files as output. It allows a free mix of sequential and parallel block coordinate searches.';
...
sub calcmediumiters
{
	...
	foreach ( @varinumbers )
	{
		...
		foreach ( keys %$_ )
		{
			...
			{
				$mediumiters[$countcase]{$_} = ( ceil( $varinumbers[$countcase]{$_} / 2 ) );
			}
		}
		$countcase++;
