...
=head2 _prune_unused_joins

Remove non-selecting/non-restricting joins from C<{from}> specs, aiding the
RDBMS query optimizer.

=head2 _adjust_select_args_for_complex_prefetch

Produce joined subqueries for complex prefetches, e.g.
C<SELECT me.*, other.* FROM ( SELECT me.* FROM ... ) JOIN other ON ...>

=head2 _resolve_aliastypes_from_select_args

=head2 _group_over_selection

=head2 _minmax_operator_for_datatype

=head2 _resolve_ident_sources

=head2 _resolve_column_info

=head2 _inner_join_to_node

=head2 _find_join_path_to_node

=head2 _extract_order_criteria

=head2 _order_by_is_stable

=head2 _columns_comprise_identifying_set

=head2 _extract_colinfo_of_stable_main_source_order_by_portion

=head2 _collapse_cond

=head2 _collapse_cond_unroll_pairs

=head2 _extract_fixed_condition_columns
