...
sub _get_names_aref {
	...
	if ( $item_index >= 0 ) {
		...
		print("param_flow_blue, _get_names_aref, @names_aref, index=$item_index\n");
		return ( \@names_aref );
	}
}
	...
	if ( $param_flow_blue->{_selection_index} >= 0 ) {
		...
		if ( $index >= 0 ) {
			...
			for ( $i = 1, $j = 0; $i < $length; $i = $i + 2, $j++ ) {
				...
				# print("1. param_flow_blue, set_values_aref :index $j values: $values[$j]\n");
			}
			
			# print("2. param_flow_blue,set_values_aref: @values\n");
		}
	}
	else {
