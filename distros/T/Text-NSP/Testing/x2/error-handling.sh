...
# shell program to test x2.pm's responses to various erroneous
# conditions. note of course that x2 never halts on errors/warnings
# but sets error codes that statistic.pl can request later on.

# Subtest 1: what happens when ngram > 2 is provided to x2
...
# test-1.sub-1.cnt has trigrams in it. so x2.pm should complain
# during the initialization step, since x2 is for bigrams only!
...
# now the test!
echo "Test:   statistic.pl --ngram 3 x2 test-1.out $TESTFILE" 
statistic.pl --ngram 3 x2 test-1.out $TESTFILE >& error.out
...
# now the test!
echo "Test:   statistic.pl --set_freq_combo $FREQCOMBFILE x2 test-1.out $TESTFILE" 
statistic.pl --set_freq_combo $FREQCOMBFILE x2 test-1.out $TESTFILE >& error.out
...
# now the test!
echo "Test:   statistic.pl x2 test-1.out $TESTFILE"
statistic.pl x2 test-1.out $TESTFILE >& error.out
