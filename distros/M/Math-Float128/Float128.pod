...
=head1 BUGS
   ...
   Old mingw64 compilers (less than version 4) have buggy coshq(), expq(),
   fmaq(), tgammaq() and nearbyintq() functions that crash when called.
   If such a mingw64 compiler is detected, this module uses workarounds for
   those problem functions. See the documentation (above) for cosh_F128(),
   exp_F128(), fma_F128(), nearbyint_F128() and tgamma_F128() for an
   outline of the workarounds involved.
...
=head1 LICENSE
   ...
   Copyright 2013-18, 2020, 2026 Sisyphus
