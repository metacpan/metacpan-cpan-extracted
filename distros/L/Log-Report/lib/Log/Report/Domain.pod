...
distribution, each containing a number of packages.  So separate the L<Log::Report|Log::Report>
...
formatting configuration (L<String::Print|String::Print> options).  The administration
for the configuration is managed in this package.

This class extends L<Log::Report::Minimal::Domain|Log::Report::Minimal::Domain>, which can be used when your
program does not want any overhead from the powerful L<Log::Report|Log::Report> features.
In that case, your program invokes L<Log::Report::Optional|Log::Report::Optional>.
...
These C<%options> are used for all packages which use the same DOMAIN.
See chapter L</Configuring> below.
Improves base, see L<Log::Report::Minimal::Domain/"Attributes">
...
Read the settings from the C<$file>.  The parameters found in the file are
...
=item context_rules => \%rules|OBJECT

When C<%rules> are provided, the translator will use the C<msgctxt> fields
...
=item formatter => CODE|\%config|'PRINTI'
...
=item translator => Log::Report::Translator|\%config
...
Helper method, which simply parses the content C<$filename> into a HASH to be
...
Extends L<"Translating" in Log::Report::Minimal::Domain|Log::Report::Minimal::Domain/"Translating">.
...
not modify the content of that HASH: change it by called L<setContext()|Log::Report::Domain/"Translating"> or
L<updateContext()|Log::Report::Domain/"Translating">.
...
can be retrieved with L<defaultContext()|Log::Report::Domain/"Translating">.

Contexts are totally ignored then there are no C<context_rules>.  When
you do not wish to change settings, you may simply provide an empty C<HASH>.
...
Translate the C<$message> into the C<$language>.

=item $obj-E<gt>B<updateContext>(STRING|HASH|ARRAY|PAIRS)

[1.10] Make changes and additions to the active context (see L<setContext()|Log::Report::Domain/"Translating">).
...
[0.91] The C<PRINTI> is a special constants for L<configure(formatter)|Log::Report::Minimal::Domain/"Attributes">, and
