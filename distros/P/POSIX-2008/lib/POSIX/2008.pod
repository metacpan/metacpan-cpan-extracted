...
accept Perl file or directory handles. C<openat()> even returns a handle if
you passed it one.
...
unchanged. These are C<read()>, C<write()>, C<writev()>, C<pread()>,
C<pwrite()>, C<pwritev()>. C<openat()> gets a special treatment in this
module, see below.
...
Returns a file handle associated with the file descriptor I<fd>. I<mode> is
one of the values C<"r">, C<"w">, C<"a"> with an optional C<"+"> and/or
C<"b">.

It's similar to C<IO::Handle::new_from_fd()> but it I<really> calls
C<fdopen(3)>, expects letter mode strings (e.g. C<"r">, not C<"<">) and fails
if the I<mode> is not compatible with the flags of I<fd> (none of which
C<new_from_fd()> does).
...
Returns a directory handle associated with the file descriptor I<fd>. Usage
example:
...
specified to place the read data at that position in I<buf>. Defaults to 0. If
I<buf_offset> is past the end of I<buf>, I<buf> will be padded with zeros
before appending the data. If I<buf_offset> is negative, it is counted from
the end of the string. If I<buf_offset> plus I<bytes_read> does not exceed the
current string length of I<buf>, I<buf> will be truncated accordingly. This
latter detail is missing from the sysread() documentation.

Returns the number of bytes read, 0 at EOF, undef on error.

Note that the order of I<offset> and I<count> is inverted compared to
pread(2). This is to be consistent with pwrite() (see below).
...
I<count> may be specified to write only that much bytes (not characters). If
not specified or undef, everything from I<buf_offset> up to the end of I<buf>
is written.
...
Note that the order of I<offset> and I<count> is inverted compared to
pwrite(2). This is to allow omitting I<count> and I<buf_offset> if you just
want to write the whole buffer.
...
C<pwritev()> behaves like C<writev()> but adds an I<offset> argument, which
specifies the file position at which the data is to be written.
...
If I<count> is omitted, it defaults to the length of I<buf>

=item writev

bytes_written = writev(fd, arrayref);

writev() writes multiple buffers from I<arrayref> to the file associated with
the file descriptor I<fd>. The buffers are processed in array order. Undefined
or empty buffers are skipped.
