...
use_ok('HTML::Selector::Element', '!DEFAULT');
is(HTML::Selector::Element::Trait->can('find'), HTML::Element->can('find'), 'monkeypatch');
ok($tree = HTML::Tree->new_from_content(<<'__BOOTSTRAP_NAV__'), 'parse html');
...
__BOOTSTRAP_NAV__
is(scalar(@link = $tree->find('a')), 23, 'all links');
is(scalar(@r = $tree->find('.navbar-nav a[onclick]')), 7, 'attribute exists');
is(scalar(@r = $tree->find('.navbar-nav a[href="#"]')), 1, 'attribute exact value');
is(scalar(@r = $tree->find('a[href^="https://"]')), 15, 'attribute starts with https://');
is(scalar(@r = $tree->find('a[href$="/"]')), 20, 'attribute ends with /');
is(scalar(@r = $tree->find('a[href*="/docs/"]')), 10, 'attribute contains substring');
is(scalar(@r = $tree->find('[class~=dropdown]')), 1, 'attribute contains word');
is(scalar(@r = $tree->find('[class|="dropdown"]')), 12, 'attribute is word or starts with word followed by hyphen');
is(scalar(@r = $tree->find('.navbar-nav a[href!="#"]')), 17, 'attribute is not');
