...
1;
# ABSTRACT: Hash of (defined-)hash-of-(defined-)strings
...
Sah::Schema::hohos - Hash of (defined-)hash-of-(defined-)strings

=head1 VERSION

This document describes version 0.009 of Sah::Schema::hohos (from Perl distribution Sah-SchemaBundle-Collection), released on 2024-06-13.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("hohos*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("hohos", {return_type=>'str_errmsg'});
 ...
 $data = {k=>{},k2=>{k=>undef}};
 my $errmsg = $validator->($data); # => "\@[k2][k]: Required but not specified"

Often a schema has coercion rule or default value rules, so after validation the
validated value will be different from the original. To return the validated
(set-as-default, coerced, prefiltered) value:

 my $validator = gen_validator("hohos", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = {k=>{},k2=>{k=>undef}};
 my $res = $validator->($data); # => ["\@[k2][k]: Required but not specified",{k=>{},k2=>{k=>undef}}]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("hohos*");
     $validator->(\@args);
     ...
 }
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('hohos*', name=>'Hohos')
     );
 }

 use My::Types qw(Hohos);
 Hohos->assert_valid($data);
