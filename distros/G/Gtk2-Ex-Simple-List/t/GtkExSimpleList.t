#
# $Header: /cvsroot/gtk2-perl-ex/Gtk2-Ex/Simple/List/t/GtkExSimpleList.t,v 1.1.1.1 2004/10/21 00:00:58 rwmcfa1 Exp $
#

#########################
# GtkSimple::List Tests
# 	- rm
#########################

use Gtk2::TestHelper tests => 46;

require_ok( 'Gtk2::Ex::Simple::List' );
...
$win->set_title('19.GtkSimple::List.t test');
...
ok( my $list = Gtk2::Ex::Simple::List->new(
			'Text Field'    => 'text',
			'Int Field'     => 'int',
			'Double Field'  => 'double',
			'Bool Field'    => 'bool',
...
# end exercise of Simple::List

Gtk2->main;
ok(1);

# each of these should result in exceptions.
eval { Gtk2::Ex::Simple::List->new; };
ok( $@ =~ m/no columns/i, 'no args' );

eval { Gtk2::Ex::Simple::List->new ('foo'); };
ok( $@ =~ m/no columns/i, 'odd number of params' );

eval { Gtk2::Ex::Simple::List->new ('foo' => 'bar'); };
ok( $@ =~ m/unknown column type/i, 'bad column type' );

eval { Gtk2::Ex::Simple::List->new_from_treeview; };
ok( $@ =~ m/not a Gtk2::TreeView/i, 'no args triggers invalid treeview first' );

eval { Gtk2::Ex::Simple::List->new_from_treeview ('foo'); };
ok( $@ =~ m/not a Gtk2::TreeView/i, 'invalid treeview reference' );

my $tv = Gtk2::TreeView->new;
eval { Gtk2::Ex::Simple::List->new_from_treeview ($tv, 'bar'); };
ok( $@ =~ m/no columns/i, 'odd number of params' );

eval { Gtk2::Ex::Simple::List->new_from_treeview ($tv, 'bar', 'baz'); };
ok( $@ =~ m/unknown column type/i, 'unknown column type' );

eval { Gtk2::Ex::Simple::List->new_from_treeview ($tv, 'bar', 'text', 'baz'); };
