...
$str = anytoa($op); # $op precision must be 53 or 64 or 113 or 2098
   ...
   OPERATOR OVERLOADING
    ...
    Math::GMPq, Math::GMPf or Math::GMP object. Then it is now
    permissible to do:
    ...
    (But note that such operations involving Math::GMP will
    silently fail or crash if used.)
    Again, each of those operations returns a Math::MPFR object
    containing the result of the operation.
    Each operation is performed to "infinite: precision, and then
    rounded to default precision using default Math::MPFR rounding.
    The precision of the returned Math::MPFR object will therefore
    differ from the precision of $M unless the precision of $M is
    the default.

    With Math::GMPz (0.63 or later) or Math::GMPq (0.58 or later)
    it is also now permissible to do:

    $G % $M:

    NOTE: In overloading a ** (power) operation that involves a
    Math::GMPq object, the value of the Math::GMPq object is
    assigned to a Math::MPFR object. This assignment is done using
    current default precision and current default rounding mode.

    The following is allowed for Math::GMPq objects (if
    $Math::GMPq::VERSION >= 0.54 && $Math::GMPq::RETYPE == 1) and
    for Math::GMPz (if $Math::GMPz::VERSION >= 0.57 &&
    $Math::GMPz::RETYPE == 1), but not for Math::GMPf or
    Math::GMP :
     ...
     $G *= $M;
     $G /= $M;
     $G %= $M; (Math::GMPz >= 0.63; Math::GMPq >= 0.58)
     $G **= $M;

    When this usage is permitted, the result of the operation is
    returned as a Math::MPFR object, calculated to "infinite"
    precision and rounded to Math::MPFR default precision, using
    Math::MPFR default rounding mode.

    To determine the type and value of the second argument received
    by the overloading subroutine, it works through the following
    steps (in order), using the first value it finds, or croaking if
    it gets to step 6:
