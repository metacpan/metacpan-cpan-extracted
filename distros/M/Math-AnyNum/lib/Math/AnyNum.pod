...
=head1 FUNCTIONS
  ...
  :ntheory
    ...
    invmod(n,k)               multiplicative inverse of n modulo k
...
When an additional argument is provided, it returns the the n-th Bernoulli polynomial: C<B_n(x)>.
...
When an additional argument is provided, it returns the the n-th Euler polynomial: C<E_n(x)>.
...
Returns a true value when C<n> is the product of a subset of powers of prime factors of C<k>,
where C<k> is a square-free integer, whose prime factors represent a set C<A>, such that C<n>
is said to be smooth over a set C<A> if there exists a factorization of C<n> where the factors
are powers of elements in C<A>.
...
Returns the number of digits of the integer part of C<n>.

Returns C<undef> when C<n> cannot be truncated to an integer.

For C<-1234.56>, it returns C<4>, which is the length of C<1234>.
...
Return C<undef> when C<n> cannot be truncated to an integer.
...
Return C<undef> when C<n> or C<k> cannot be truncated to an integer.
...
Return C<undef> when C<n> cannot be truncated to an integer or when C<k> is negative.
...
Returns a true value when C<n> is exactly divisible by C<k> (i.e.: when the remainder C<n % k> is zero).
...
Returns a true value when C<n> is congruent to C<k> modulo C<m> (i.e.: when the remainder C<n % m> equals C<k % m>).
...
Copyright 2017-2019 Daniel Șuteu.
