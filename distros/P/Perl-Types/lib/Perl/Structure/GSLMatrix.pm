...
#use parent qw(Math::GSL::Matrix Perl::Structure::Array::Reference);  # NEED UPGRADE, CORRELATION #rp023: Inline::CPP support for multiple inheritance
...
#use Perl::Structure::Array::Reference;  # NEED ANSWER: do we actually inherit or use any part of Array::Reference?  is this inheritance actually correct logically?
...
our @EXPORT = qw(
    ...
    gsl_matrix_to_arrayref_number
    gsl_matrix_to_arrayref_arrayref_number
    gsl_matrix_to_string
    arrayref_number_to_gsl_matrix
);
our @EXPORT_OK = qw(
...
# [[[ INCLUDES ]]]
#use RPerl::Operation::Expression::Operator::GSLFunctions;  # NEED UPDATE, RPERL REFACTOR
...
#    Perl::diag("in PERLOPS_PERLTYPES gsl_matrix_CHECK(), top of subroutine\n");
    ...
    if ( not( main::PerlTypes_SvHROKp($possible_gsl_matrix) ) ) {
...
#    Perl::diag("in PERLOPS_PERLTYPES gsl_matrix_CHECK(), bottom of subroutine\n");
    return;
}
...
#    Perl::diag("in PERLOPS_PERLTYPES gsl_matrix_CHECKTRACE(), top of subroutine\n");
#    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_CHECKTRACE(), received $variable_name = ' . $variable_name . "\n");
#    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_CHECKTRACE(), received $subroutine_name = ' . $subroutine_name . "\n");
...
#    Perl::diag("in PERLOPS_PERLTYPES gsl_matrix_CHECKTRACE(), bottom of subroutine\n");
    return;
}
...
sub gsl_matrix_to_arrayref_number {
    { my arrayref::number $RETURN_TYPE };
    ( my gsl_matrix $input_gsl_matrix ) = @ARG;

#    gsl_matrix_CHECK($input_gsl_matrix);
    gsl_matrix_CHECKTRACE( $input_gsl_matrix, '$input_gsl_matrix', 'gsl_matrix_to_arrayref_number()' );

    my arrayref::number $retval = [];
...
sub gsl_matrix_to_arrayref_arrayref_number {
    { my arrayref::arrayref::number $RETURN_TYPE };
    ( my gsl_matrix $input_gsl_matrix ) = @ARG;

#    gsl_matrix_CHECK($input_gsl_matrix);
    gsl_matrix_CHECKTRACE( $input_gsl_matrix, '$input_gsl_matrix', 'gsl_matrix_to_arrayref_arrayref_number()' );
...
sub gsl_matrix_to_string {
    ...
    ( my gsl_matrix $input_gsl_matrix ) = @ARG;
    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_to_string(), top of subroutine', "\n");
 
#    gsl_matrix_CHECK($input_gsl_matrix);
    gsl_matrix_CHECKTRACE( $input_gsl_matrix, '$input_gsl_matrix', 'gsl_matrix_to_string()' );

#    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_to_string(), have $input_gsl_matrix = ', Dumper($input_gsl_matrix), "\n");
#    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_to_string(), have gsl_matrix_get($input_gsl_matrix, 0, 0) = ', gsl_matrix_get($input_gsl_matrix, 0, 0), "\n");
#    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_to_string(), have gsl_matrix_get($input_gsl_matrix, 0, 1) = ', gsl_matrix_get($input_gsl_matrix, 0, 1), "\n");
#    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_to_string(), have gsl_matrix_get($input_gsl_matrix, 1, 0) = ', gsl_matrix_get($input_gsl_matrix, 1, 0), "\n");
#    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_to_string(), have gsl_matrix_get($input_gsl_matrix, 1, 1) = ', gsl_matrix_get($input_gsl_matrix, 1, 1), "\n");
...
#    my integer $i_max = 1;
#    my integer $j_max = 1;
#    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_to_string(), have $i_max = ', $i_max, ', $j_max = ', $j_max, "\n");

    for my integer $i (0 .. $i_max) { 
#        Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_to_string(), have $i = ', $i, "\n");
        for my integer $j (0 .. $j_max) {
#            Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_to_string(), have $j = ', $j, "\n");
...
# so that loading will all be controlled by the 'use perlgsl;' directive

sub arrayref_number_to_gsl_matrix {
    { my gsl_matrix $RETURN_TYPE };
    ( my arrayref::number $input_arrayref_number, my integer $rows, my integer $cols ) = @ARG;
#    Perl::diag('in PERLOPS_PERLTYPES arrayref_number_to_gsl_matrix(), top of subroutine', "\n");

#    ::arrayref_number_CHECK($input_arrayref_number);
    ::arrayref_number_CHECKTRACE( $input_arrayref_number, '$input_arrayref_number', 'arrayref_number_to_gsl_matrix()' );

#    Perl::diag('in PERLOPS_PERLTYPES arrayref_number_to_gsl_matrix(), received $rows = ', $rows, ', $cols = ', $cols, "\n");

    my gsl_matrix $retval = gsl_matrix_alloc($rows, $cols);

    for my integer $i (0 .. ($rows - 1)) {
        for my integer $j (0 .. ($cols - 1)) {
            gsl_matrix_set($retval, $i, $j, $input_arrayref_number->[($i * $cols) + $j]);
        }
    }
#    Perl::diag('in PERLOPS_PERLTYPES arrayref_number_to_gsl_matrix(), bottom of subroutine', "\n");
    return $retval;
}
...
sub gsl_matrix_typetest0 {
    { my gsl_matrix $RETURN_TYPE };
    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_typetest0(), top of subroutine' . "\n");

    my gsl_matrix $retval = gsl_matrix_alloc(21, 12);
    gsl_matrix_set($retval, 0, 0, (main::Perl__Type__Integer__MODE_ID() + 23));
    return ($retval);
}
...
sub gsl_matrix_typetest1 {
    ...
    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_typetest1(), top of subroutine' . "\n");

#    ::gsl_matrix_CHECK($lucky_gsl_matrix);
    ::gsl_matrix_CHECKTRACE( $lucky_gsl_matrix, '$lucky_gsl_matrix', 'gsl_matrix_typetest1()' );

    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_typetest1(), received $lucky_gsl_matrix = ', "\n", gsl_matrix_to_string($lucky_gsl_matrix), "\n");

    gsl_matrix_add_constant($lucky_gsl_matrix, main::Perl__Type__Integer__MODE_ID());

    Perl::diag('in PERLOPS_PERLTYPES gsl_matrix_typetest1(), about to return $lucky_gsl_matrix = ', "\n", gsl_matrix_to_string($lucky_gsl_matrix), "\n");
