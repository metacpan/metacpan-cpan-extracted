...
This document describes version 0.001 of Role::Versioning::Scheme (from Perl distribution Versioning-Scheme), released on 2018-10-04.
...
Usage:
 ...
 my $valid = $vs->is_valid('1.2.3'); # bool
...
Usage:
 ...
 my $normalized = $vs->normalize('1.2.3'); # bool
...
Number of version numbers to bump, for example:
 ...
 my $v2 = $vs->bump('1.2.3', {num=>2}); # => '1.2.5'

It can be negative:

 my $v2 = $vs->bump('1.2.3', {num=>-1}); # => '1.2.2'
 my $v2 = $vs->bump('1.2.3', {num=>-3}); # => '1.2.0'

It must die when an ambiguous number is specified:

 my $v2 = $vs->bump('1.2.3', {num=>-4}); # dies
...
the second smallest part, and so on). For example in dotted version:
 ...
 my $v2 = $vs->bump('1.2.3', {part=>-2}); # => '1.3.0'
...
previous example). Setting this option to 0 prevents that:
 ...
 my $v2 = $vs->bump('1.2.3', {part=>-2, reset_smaller=>0}); # => '1.3.3'
