...
#
#	$Id: DirList.tcl,v 1.7 2011/07/01 00:12:13 hobbs Exp $
#
# DirList.tcl --
#
...
proc tixDirList:Command {w args} {
    ...
    # Ensure we fully resolve link-based paths. [sereno]
    set vpath [file normalize $vpath]
...
proc tixDirList:ListHierachy {w npath vpath} {
	...
	$data(w:hlist) add $vpath -itemtype imagetext \
	    -text $text -data $curpath -image $img
    }
}
...
proc tixDirList:ListSubDirs {w npath vpath} {
	...
	if {![$data(w:hlist) info exists $vp]} {
	    $data(w:hlist) add $vp -itemtype imagetext \
		-text $ent -data $curpath -image $img
	}
    }
}
