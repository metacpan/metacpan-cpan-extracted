...
BEGIN { CORE::require Ewindows1252; }
...
perl $0 Windows-1252_script.pl > Escaped_script.pl.e
END
    }

    print Windows1252::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (CORE::eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Ewindows1252::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
        }
    ...
    my $fh = gensym();
    Ewindows1252::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Windows1252::escape_script {
    ...
    # read Windows-1252 script
    my $fh = gensym();
    Ewindows1252::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script\n";

    if (/^ use Ewindows1252(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ewindows1252 '%s.0'; # 'quote' for perl5.6\n", $Windows1252::VERSION); # require run-time routines version

        # use Windows1252 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Windows1252 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Windows1252::VERSION) {
                    my @file = grep -e, map {qq{$_/Windows1252.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Windows1252.pm $version, but @{[__FILE__]} is version $Windows1252::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Ewindows1252.pm $version, but \\\$Ewindows1252::VERSION is \$Ewindows1252::VERSION" if \$Ewindows1252::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Windows1252::VERSION) {
                    die "Script $0 required Windows1252.pm $version, but @{[__FILE__]} is only version $Windows1252::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Ewindows1252::function_ord     = 'Windows1252::ord'     if /\A ord \z/oxms;
                    $Ewindows1252::function_ord_    = 'Windows1252::ord_'    if /\A ord \z/oxms;
                    $Ewindows1252::function_reverse = 'Windows1252::reverse' if /\A reverse \z/oxms;
                    $Ewindows1252::function_getc    = 'Windows1252::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Windows1252::escape();
...
=item * You'll have to download Windows1252.pm and Ewindows1252.pm and put it in your perl lib directory.

=item * You'll need to write "use Windows1252;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Windows1252;
  use Windows1252 ver.sion;             --- require minimum version
  use Windows1252 ver.sion.0;           --- expects version (match or die)

  # "no Windows1252;" not supported

  or

  $ perl Windows1252.pm Windows-1252_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Windows-1252_script.pl  --- script written in Windows-1252
...
Windows1252 software is "middleware" between perl interpreter and your Perl script
written in Windows-1252.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Windows-1252, Perl3 or later
can treat Windows-1252 as binary data.

By "use Windows1252;", it automatically interpret your script as Windows-1252. The various
functions of perl including a regular expression can treat Windows-1252 now.
...
This software is a source code filter to escape Perl script encoded by Windows-1252
...
=item * Remains One Language Handling Raw Windows-1252, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Windows1252.pm, Ewindows1252.pm
  +---------------------------------------+
...
import of Ewindows1252.pm works well.
...
=head1 Software Composition
   ...
   Windows1252.pm               --- source code filter to escape Windows-1252
   Ewindows1252.pm              --- run-time routines for Windows1252.pm
...
You need write 'use Windows1252;' in your script.
...
=head1 Calling 'Ewindows1252::ignorecase()' (Windows1252 software provides)

Windows1252 software applies calling 'Ewindows1252::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Ewindows1252::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Ewindows1252.pm provides)
...
Definitions in Ewindows1252.pm.
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Windows1252 software provides)

Windows1252 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Windows1252 software provides)

Insert 'Ewindows1252::' at head of function name. Ewindows1252.pm provides your script Ewindows1252::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Ewindows1252::lc        Character
  lcfirst     Ewindows1252::lcfirst   Character
  uc          Ewindows1252::uc        Character
  ucfirst     Ewindows1252::ucfirst   Character
  fc          Ewindows1252::fc        Character
  chr         Ewindows1252::chr       Character
  glob        Ewindows1252::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Windows1252 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Windows1252 software provides)

Windows1252 software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Windows1252 software and
...
exist at time of be written this document and JPerl did exist.
                        ...
                        (a)     (b)     (c)     (d)     (e)
                                      JPerl,japerl    Encode,Windows1252
      +--------------+-------+-------+-------+-------+-------+
      | data         |  Old  |  Old  |  New  |  Old  |  New  |
      +--------------+-------+-------+-------+-------+-------+
      | script       |  Old  |      Old      |      New      |
...
Windows1252 software remains one language and one interpreter.
