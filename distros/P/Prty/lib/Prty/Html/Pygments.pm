...
Prty::Html::Pygments - Syntax Highlighting in HTML
...
eingebettet werden, damit die oben erzeugten CSS-Regeln greifen:
    ...
    $html = Prty::Html::Pygments->html('perl',$code);

Liefere die Namen aller Pygments-Styles:

    @styles = Prty::Html::Pygments->styles;
...
der Programmiersprache $lang:
    ...
    $html = Prty::Html::Pygments->stylesPage($h,$lang,$code);
...
Gib die CSS-Regeln für den Pyments-Style 'emacs' aus:
    ...
    print scalar Prty::Html::Pygments->css('emacs');
    __END__
...
Parent-Elemente der Klasse 'highlight' aus:
    ...
    print scalar Prty::Html::Pygments->css('emacs','.syntax');
    __END__
...
sub css {
    ...
    my $c = Prty::CommandLine->new('pygmentize');
    $c->addOption(
        -f => 'html',
        -S => $style,
        -a => $selector,
    ...
    my $rules = Prty::Shell->exec($c->command,-capture=>'stdout');
...
sub html {
    ...
    my $html = Prty::Ipc->filter($c->command,$code);
...
sub styles {
    ...
    my $text = Prty::Shell->exec($c->command,-capture=>'stdout');
...
Generiere eine Seite mit allen Styles und schreibe sie auf Datei $file:
    ...
    my $html = Prty::Html::Pygments->stylesPage($h,'perl',q~
        PERL-CODE
    ~));
    Prty::Path->write($file,$html);
...
sub stylesPage {
    ...
    $code = Prty::Unindent->trimNl($code);
