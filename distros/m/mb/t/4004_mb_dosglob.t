...
# 21
    ...
    sub {1},
# 31
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; mb::eval(q{ chdir($FindBin::Bin); @_ = mb::dosglob('"*.1表" "*.2表" "*.3表"'); scalar(@_) == 3   }) },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; mb::eval(q{ chdir($FindBin::Bin); @_ = mb::dosglob('"*.1表" "*.2表" "*.3表"'); $_[0] =~ /\.1表$/ }) },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; mb::eval(q{ chdir($FindBin::Bin); @_ = mb::dosglob('"*.1表" "*.2表" "*.3表"'); $_[1] =~ /\.2表$/ }) },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; mb::eval(q{ chdir($FindBin::Bin); @_ = mb::dosglob('"*.1表" "*.2表" "*.3表"'); $_[2] =~ /\.3表$/ }) },
    sub {1},
    sub {1},
    sub {1},
    sub {1},
...
# 51
    ...
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; mb::_unlink("@{[__FILE__]}.1表\"); },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; mb::_unlink("@{[__FILE__]}.2表\"); },
    sub {1}, # sub { return 'SKIP' unless $MSWin32_MBCS; mb::_unlink("@{[__FILE__]}.3表\"); },
    sub {1},
    sub {1},
    sub {1},
);
