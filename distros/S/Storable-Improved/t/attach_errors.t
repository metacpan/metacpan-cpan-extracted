...
# Good Case - should not die
{
	...
	my $frozen = undef;
	eval {
		$frozen = Storable::Improved::freeze( $goodfreeze );
	};
	ok( ! $@, 'Storable::Improved does not die when STORABLE_freeze does not return references' );
	ok( $frozen, 'Storable::Improved freezes to a string successfully' );
...
# Error Case - should die on freeze
{
	...
	eval {
		Storable::Improved::freeze( $badfreeze );
	};
	ok( $@, 'Storable::Improved dies correctly when STORABLE_freeze returns a reference' );
	# Check for a unique substring of the error message
	ok( $@ =~ /cannot return references/, 'Storable::Improved dies with the expected error' );
...
# Good Case - should not die
{
	...
	my $frozen = undef;
	eval {
		$frozen = Storable::Improved::freeze( $goodthaw );
	};
	ok( $frozen, 'Storable::Improved freezes to a string as expected' );
	my $thawed = eval {
		Storable::Improved::thaw( $frozen );
	};
...
# Bad Case - should die on thaw
{
	...
	my $frozen = undef;
	eval {
		$frozen = Storable::Improved::freeze( $badthaw );
	};
...
# Good Case - should not die
{
	...
	my $frozen = Storable::Improved::freeze( $goodattach );
...
# Good case - multiple references to the same object should be attached properly
{
	...
	my $frozen = Storable::Improved::freeze($arr);
...
# Bad Cases - die on thaw
{
	...
	my $frozen = Storable::Improved::freeze( $badattach );
