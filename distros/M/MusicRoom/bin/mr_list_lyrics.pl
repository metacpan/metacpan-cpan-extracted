# Create a summary list (in CSV format) of which lyric files will
...
my $target_file = MusicRoom::File::new_name("lyric","csv");
...
sub process_track
  {
    ...
    foreach my $attr (@columns)
      {
        my $val;
        if($attr eq "lyrics_file")
          {
            $val = MusicRoom::Lyrics::locate($track);
          ...
          {
            my @vals = MusicRoom::Lyrics::search_for($track);
            $val = join(', ',@vals);
          }
