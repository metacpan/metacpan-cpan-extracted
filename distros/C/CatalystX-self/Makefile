# This Makefile is for the CatalystX::self extension to perl.
#
...
#     ABSTRACT => q[A customized self for Catalyst controllers]
#     AUTHOR => q[Jason M. Mills <jmmills at cpan.org>]
#     DIR => []
#     DISTNAME => q[CatalystX-self]
#     NAME => q[CatalystX::self]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], parent=>q[0], ExtUtils::MakeMaker=>q[6.30], Sub::Exporter=>q[0], self=>q[0] }
#     VERSION => q[0.01]
#     clean => { FILES=>q[META.yml] }
#     dist => { PREOP=>q[$(PERL) -I. "-MModule::Install::Admin" -e "dist_preop(q($(DISTVNAME)))"] }
#     test => { TESTS=>q[t/00-load.t t/01-basic.t t/02-values.t t/03-aliases.t t/pod-coverage.t t/pod.t] }
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/CatalystX/self.pm
...
TO_INST_PM = lib/CatalystX/self.pm

PM_TO_BLIB = lib/CatalystX/self.pm \
	blib/lib/CatalystX/self.pm
...
DISTNAME = CatalystX-self
DISTVNAME = CatalystX-self-0.01
...
manifypods : pure_all  \
	lib/CatalystX/self.pm \
	lib/CatalystX/self.pm
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/CatalystX/self.pm $(INST_MAN3DIR)/CatalystX::self.$(MAN3EXT) 
...
TEST_FILE = test.pl
TEST_FILES = t/00-load.t t/01-basic.t t/02-values.t t/03-aliases.t t/pod-coverage.t t/pod.t
TESTDB_SW = -d
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>A customized self for Catalyst controllers</ABSTRACT>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/CatalystX/self.pm blib/lib/CatalystX/self.pm 
