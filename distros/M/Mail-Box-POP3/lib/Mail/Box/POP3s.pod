...
=item lock_type => $class|$name|\@names
...
Extends L<"Attributes" in Mail::Box::POP3|Mail::Box::POP3/"Attributes">.
...
=item $any-E<gt>B<create>($folder, %options)
...
=item Error: Cannot find head back for $id on POP3 server $name.
...
Cast by C<getHeadAndBody()>

=item Error: Cannot read body for $id on POP3 server $name.
...
=item Warning: Message $id on POP3 server $name disappeared.
...
=item Warning: message $id disappeared from POP3 server $name.
...
=item Warning: update of $count messages ignored for POP3 folder $name.
...
=item Error: you cannot write messages to a pop server (yet)

Cast by C<addMessages()>

=back

=head1 SEE ALSO

This module is part of Mail-Box-POP3 version 4.02,
