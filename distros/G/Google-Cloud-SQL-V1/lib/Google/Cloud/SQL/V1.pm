...
sub BUILD {
    ...
    my $client = Google::gRPC::Client->new(
        target     => 'sqladmin.googleapis.com:443',
        auth_token => $token,
    );
...
Google::Cloud::SQL::V1 - Google Cloud SQL Admin V1 API Client

=head1 SYNOPSIS

    use Google::Cloud::SQL::V1;
    use Google::Auth;

    my $auth = Google::Auth->default();
    my $client = Google::Cloud::SQL::V1->new(credentials => $auth);

=head1 DESCRIPTION

Google Cloud SQL Admin V1 API Client over high-performance gRPC transport.
