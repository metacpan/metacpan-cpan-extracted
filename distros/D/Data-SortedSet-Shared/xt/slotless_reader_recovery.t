...
# compiling sortedset.h with SS_READER_SLOTS=1 so the 2nd reader goes slotless.

plan skip_all => 'Linux only' unless $^O eq 'linux';
my $cc = $Config{cc} || 'cc';
system("$cc --version >/dev/null 2>&1") == 0
    or plan skip_all => "no working C compiler ($cc)";
-f 'sortedset.h' or plan skip_all => 'run from the distribution root (sortedset.h not found)';
...
int main(void) {
    ...
    if (b == 0) { ss_rwlock_rdlock(L);
        ...
        __atomic_sub_fetch(&F->reader_active,1,__ATOMIC_RELEASE); ss_rwlock_rdunlock(L); _exit(0); }
    ...
    if (c == 0) { ss_rwlock_wrlock(L);
        ...
        __atomic_store_n(&F->writer_active,0,__ATOMIC_RELEASE); ss_rwlock_wrunlock(L); _exit(0); }
...
my $bin = "$dir/repro";
my $log = `$cc -O2 -D_GNU_SOURCE -DSS_READER_SLOTS=1 -I. -o $bin $src -lpthread  2>&1`;
