...
#ifndef __CPP__INCLUDED__Perl__Type__Number_cpp
#define __CPP__INCLUDED__Perl__Type__Number_cpp 0.008_000

// [[[ INCLUDES ]]]
#include <Perl/Type/Number.h>  // -> NULL (relies on native C type)
...
SV* number_to_nonsigned_integer(SV* input_number) {
//  number_CHECK(input_number);
...
nonsigned_integer number_to_nonsigned_integer(number input_number) {
    if (input_number < 0) { return (nonsigned_integer) (input_number * -1); }
    else { return (nonsigned_integer) input_number; }
}
...
SV* number_typetest0() {
	SV* retval = newSVnv((22.0 / 7.0) + SvIV(Perl__Type__Number__MODE_ID()));
...
//fprintf(stderr, "in CPPOPS_PERLTYPES number_typetest1(), have received lucky_number = %"NUMBER"\n", (number)SvNV(lucky_number));
	return newSVnv((SvNV(lucky_number) * 2.0) + SvIV(Perl__Type__Number__MODE_ID()));
}

# elif defined __CPP__TYPES

number number_typetest0() {
	number retval = (22.0 / 7.0) + Perl__Type__Number__MODE_ID();
...
number number_typetest1(number lucky_number) {
	...
	return (lucky_number * 2.0) + Perl__Type__Number__MODE_ID();
}
