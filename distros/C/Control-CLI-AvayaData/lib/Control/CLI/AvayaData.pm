...
my %MorePrompt = ( # The following characters are automatically backslashed in more_prompt(): ().
	...
	$Prm{xirrus}		=>	'--MORE--\x10?',
	$Prm{generic}		=>	'----More (q=Quit, space/return=Continue)----|--More-- (q = quit) |Press any key to continue (q : quit) :\x00|press any key to continue, q to quit.|--MORE--',
);
	...
	if ($familyType eq $Prm{pers}) {
		$attrib->{attribute} eq 'is_ha' && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show ha-state\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->cmdPrivExec($pkgsub, 'show ha-state', 'show ha-state');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		$attrib->{attribute} eq 'sw_version' && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show sys sw / show sys software\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->cmdPrivExec($pkgsub, 'show sys sw', 'show sys software');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		$attrib->{attribute} eq 'fw_version' && do {
			if ($attrib->{stage} < 4) { # 4th stage
				unless ($attrib->{debugMsg}) {
					$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show bootconfig info / show boot config general\n");
					$attrib->{debugMsg} = 1;
				}
				my ($ok, $outref) = $self->cmdPrivExec($pkgsub, 'show bootconfig info', 'show boot config general');
				return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
			...
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show bootconfig info / show boot config info\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->cmdPrivExec($pkgsub, 'show bootconfig info', 'show boot config info'); # On 8300 it's 'show boot config info'
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
			if ($$outref =~ /Version:\s+(?i:v|REL)?(.+)/) {
				$self->_setAttrib('fw_version', $1);
			}
		...
		$attrib->{attribute} eq 'stp_mode' && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show bootconfig flags / show boot flags\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->cmdPrivExec($pkgsub, 'show bootconfig flags', 'show boot config flags');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		$attrib->{attribute} eq 'baudrate' && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show bootconfig sio / show boot config sio\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->cmdPrivExec($pkgsub, 'show bootconfig sio', 'show boot config sio', 1);
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		if ($self->{$Package}{ATTRIB}{'is_master_cpu'}) { # On Master CPU
			($attrib->{attribute} eq 'is_dual_cpu' || $attrib->{attribute} eq 'base_mac') && do {
				unless ($attrib->{debugMsg}) {
					$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show sys info / show sys-info (4 pages)\n");
					$attrib->{debugMsg} = 1;
				}
			...
			($attrib->{attribute} eq 'slots' || $attrib->{attribute} eq 'ports') && do {
				if ($self->{$Package}{ATTRIB}{'is_nncli'} && $self->{$Package}{ATTRIB}{'model'} =~ /(?:Passport|ERS)-8[36]\d\d/) { # 8300/8600 NNCLI case
					if ($attrib->{stage} < 4) { # 4th stage
						unless ($attrib->{debugMsg}) {
							$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show interfaces fastEthernet name\n");
							$attrib->{debugMsg} = 1;
						}
						my ($ok, $outref) = $self->cmdPrivExec($pkgsub, undef, 'show interfaces fastEthernet name');
						return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
					...
					unless ($attrib->{debugMsg}) {
						$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show interfaces gigabitEthernet name\n");
						$attrib->{debugMsg} = 1;
					}
					my ($ok, $outref) = $self->cmdPrivExec($pkgsub, undef, 'show interfaces gigabitEthernet name');
					return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
					$self->_setSlotPortAttrib($outref);
					$self->{POLL}{output_result} = $self->{$Package}{ATTRIB}{$attrib->{attribute}};
					return $self->poll_return(1);
					...
					unless ($attrib->{debugMsg}) {
						$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show port info\n");
						$attrib->{debugMsg} = 1;
					}
					my ($ok, $outref) = $self->cmdPrivExec($pkgsub, 'show ports info name', 'show interfaces gigabitEthernet high-secure');
					return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
					$self->_setSlotPortAttrib($outref);
					$self->{POLL}{output_result} = $self->{$Package}{ATTRIB}{$attrib->{attribute}};
					return $self->poll_return(1);
			...
			$attrib->{attribute} =~ /^(?:is_)?oob_/ && do {
				if ($self->{$Package}{ATTRIB}{'is_voss'}) { # VSP based PassportERS (VOSS)
					...
					my ($ok, $outref) = $self->cmdPrivExec($pkgsub, undef, 'show ip interface vrf MgmtRouter');
					return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
					my ($ip1, $ip2, $ipv);
					...
					if ($attrib->{stage} < 4) { # 4th stage
						unless ($attrib->{debugMsg}) {
							$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show sys info / show sys-info\n");
							$attrib->{debugMsg} = 1;
						}
					...
					unless ($attrib->{debugMsg}) {
						$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show bootconfig config / show boot config running-config\n");
						$attrib->{debugMsg} = 1;
					}
					my ($ok, $outref) = $self->cmdPrivExec($pkgsub, 'show bootconfig config', 'show boot config running-config');
					return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
					my ($ip1, $ip2);
		...
		($attrib->{attribute} eq 'fw_version' || $attrib->{attribute} eq 'sw_version' || $attrib->{attribute} eq 'switch_mode' ||
		 ...
		 $attrib->{attribute} eq 'model' || $attrib->{attribute} eq 'sysname' || $attrib->{attribute} eq 'base_mac') && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show sys-info\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->cmdPrivExec($pkgsub, undef, 'show sys-info');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		($attrib->{attribute} eq 'slots' || $attrib->{attribute} eq 'ports') && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show interfaces\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->cmdPrivExec($pkgsub, undef, 'show interfaces');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
			$self->_setSlotPortAttrib($outref);
			$self->{POLL}{output_result} = $self->{$Package}{ATTRIB}{$attrib->{attribute}};
			return $self->poll_return(1);
		...
		$attrib->{attribute} eq 'stp_mode' && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show spanning-tree mode\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->cmdPrivExec($pkgsub, undef, 'show spanning-tree mode');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		$attrib->{attribute} eq 'mgmt_vlan' && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show vlan mgmt\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->cmdPrivExec($pkgsub, undef, 'show vlan mgmt');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		($attrib->{attribute} eq 'mgmt_ip' || $attrib->{attribute} eq 'oob_ip' || $attrib->{attribute} eq 'is_oob_connected') && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show ip\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->cmdPrivExec($pkgsub, undef, 'show ip');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		$attrib->{attribute} eq 'baudrate' && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show terminal\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->poll_cmd($pkgsub, 'show terminal'); # Don't need to be in privExec for this
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		$attrib->{attribute} eq 'max_baud' && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: terminal speed ?\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->poll_cmd($pkgsub, "terminal speed ?$CTRL_C"); # Don't need to be in privExec for this
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
			my $baudRate;
			while ($$outref =~ /^  (\d+)\s*$/mg) {
		...
		$attrib->{attribute} eq 'model' && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show chassis\n");
				$attrib->{debugMsg} = 1;
			}
		...
		($attrib->{attribute} eq 'fw_version' || $attrib->{attribute} eq 'sw_version') && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show version\n");
				$attrib->{debugMsg} = 1;
			}
		...
		($attrib->{attribute} eq 'slots' || $attrib->{attribute} eq 'ports') && do {
			if ($attrib->{stage} < 4) { # 4th stage
				unless ($attrib->{debugMsg}) {
					$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show interface ethernets\n");
					$attrib->{debugMsg} = 1;
				}
				my ($ok, $outref) = $self->poll_cmd($pkgsub, 'show interface ethernets');
				return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
				$self->_setSlotPortAttrib($outref);
				$attrib->{stage}++; # Move to next stage
				$attrib->{debugMsg} = 0;
			...
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show module configuration all\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->poll_cmd($pkgsub, 'show module configuration all');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
			$self->_setSlotPortAttrib($outref);
			$self->{POLL}{output_result} = $self->{$Package}{ATTRIB}{$attrib->{attribute}};
			return $self->poll_return(1);
		...
		$attrib->{attribute} eq 'sysname' && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show hostname\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->poll_cmd($pkgsub, 'show hostname');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		$attrib->{attribute} eq 'base_mac' && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show system configuration\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->poll_cmd($pkgsub, 'show system configuration', 1);
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		($attrib->{attribute} eq 'model' || $attrib->{attribute} eq 'sysname' || $attrib->{attribute} eq 'base_mac') && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show system\n");
				$attrib->{debugMsg} = 1;
			}
		...
		($attrib->{attribute} eq 'slots' || $attrib->{attribute} eq 'ports') && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show port status\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->poll_cmd($pkgsub, 'show port status');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
			$self->_setSlotPortAttrib($outref);
			$self->{POLL}{output_result} = $self->{$Package}{ATTRIB}{$attrib->{attribute}};
			return $self->poll_return(1);
		...
		($attrib->{attribute} eq 'model' || $attrib->{attribute} eq 'fw_version' || $attrib->{attribute} eq 'sw_version')&& do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show config\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->poll_cmd($pkgsub, 'show config', 1);
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		($attrib->{attribute} eq 'is_dual_cpu' || $attrib->{attribute} eq 'sysname') && do {
			...
			my ($ok, $outref) = $self->poll_cmd($pkgsub, 'show sys info', 3);
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		($attrib->{attribute} eq 'slots' || $attrib->{attribute} eq 'ports') && do {
			unless ($attrib->{debugMsg}) {
	 			$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show ports info arp\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->poll_cmd($pkgsub, 'show ports info arp');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
			$self->_setSlotPortAttrib($outref);
			$self->{POLL}{output_result} = $self->{$Package}{ATTRIB}{$attrib->{attribute}};
			return $self->poll_return(1);
		...
		($attrib->{attribute} eq 'model' || $attrib->{attribute} eq 'base_mac' ||
		 $attrib->{attribute} eq 'fw_version' || $attrib->{attribute} eq 'sw_version')&& do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show system-info\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->poll_cmd($pkgsub, 'show system-info');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
		...
		($attrib->{attribute} eq 'sysname') && do {
			unless ($attrib->{debugMsg}) {
				$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show contact-info\n");
				$attrib->{debugMsg} = 1;
			}
		...
		($attrib->{attribute} eq 'slots' || $attrib->{attribute} eq 'ports') && do {
			unless ($attrib->{debugMsg}) {
	 			$self->debugMsg(4,"Seeking attribute $attrib->{attribute} value by issuing command: show ethernet\n");
				$attrib->{debugMsg} = 1;
			}
			my ($ok, $outref) = $self->poll_cmd($pkgsub, 'show ethernet');
			return $self->poll_return($ok) unless $ok; # Come out if error or if not done yet in non-blocking mode
			$self->_setSlotPortAttrib($outref);
			$self->{POLL}{output_result} = $self->{$Package}{ATTRIB}{$attrib->{attribute}};
			return $self->poll_return(1);
...
sub poll_change_baudrate { # Method to handle change_baudrate for poll methods (used for both blocking & non-blocking modes)
	...
	if ($changeBaud->{stage} < 2) { # 2nd stage
		...
		else { # Other Avaya family types not supported
...
There is no need to set the prompt string in any of this module's methods since it knows exactly what to expect from any of the supported AvayaData products. Furthermore the prompt string is automatically internally set to match the actual prompt of the connected device (rather than using a generic regular expression such as '*[#>]$'). This greatly reduces the risk that the generic regular expression might trigger on a fake prompt embedded in the output stream from the device.
...
A number of attributes are made available to find out basic information about the connected Avaya device.
...
Note that all the extra functionality that this module offers over and above Control::CLI, is only possible if connected to an Avaya (or ex Nortel) device. To make sure that the connected device is supported, the family_type attribute can be inspected to make sure it is not set to 'generic'; see attribute().
...
The Control::CLI::AvayaData constructor accpets all arguments supported by the Control::CLI constructor (which are passed to it) and defines some new arguments specific to itself.
...
I<baudrate>: Console port configured baudrate. This attribute only works with devices where the baudrate is configurable (i.e. only PassportERS and BaystackERS devices with the exceptions of ERS-4000 units and standby CPU of a VSP9000). On these devices this attribute will return a defined value. On other devices where the baudrate is not configurable an undef value is returned, and in this case it is safe to assume that the valid baudrate is 9600.
...
Alternatively, to force wake_console to be sent even on regular Telnet/SSH connections, you may set it with this method to a true value.
...
=head2 Methods for modules sub-classing Control::CLI::AvayaData inherited from Control::CLI
...
=head2 Methods for modules sub-classing Control::CLI::AvayaData
...
These should be called using their fully qualified package name or else they can be expressly imported when loading this module:
	...
	use Control::CLI::AvayaData qw(:prompt);
