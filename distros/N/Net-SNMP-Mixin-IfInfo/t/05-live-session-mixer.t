...
SKIP: {
  ...
  isa_ok( $session->mixer('Net::SNMP::Mixin::IfInfo'), 'Net::SNMP' );
  ok(
    $session->can('get_if_entries'),
    'can $session->get_if_entries'
  );

  eval { $session->init_mixins };
  ok( !$@, 'init_mixins without error' );
  ok( $session->init_ok('Net::SNMP::Mixin::IfInfo'), 'initialization successful completetd' );
