...
This document describes version 0.010 of Sah::Schema::datasize (from Perl distribution Sah-SchemaBundle-DataSizeSpeed), released on 2024-08-03.

=head1 SAH SCHEMA DEFINITION

 [
   "float",
   { "min" => 0, "x.perl.coerce_rules" => ["From_str::suffix_datasize"] },
   {},
 ]
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("datasize*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("datasize", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("datasize", {return_type=>'str_errmsg+val'});
 my $res = $validator->($data); # [$errmsg, $validated_val]
 
 # a sample valid data
 $data = "2mb";
 my $res = $validator->($data); # => ["",2097152]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("datasize*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"datasize*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('datasize*', name=>'Datasize')
     );
 }

 use My::Types qw(Datasize);
 Datasize->assert_valid($data);
