...
sub new {
    ...
    if ( exists($args->{q[attr]}) ) { do { package MyTest::Mite; ref($args->{q[attr]}) eq 'ARRAY' } or require Carp && Carp::croak(q[Type check failed in constructor: attr should be ArrayRef]); $self->{q[attr]} = $args->{q[attr]};  } else { my $value = do { my $default_value = do { my $method = $MyTest::TestClass::Array::__attr_DEFAULT__; $self->$method }; (ref($default_value) eq 'ARRAY') or do { require Carp; Carp::croak(q[Type check failed in default: attr should be ArrayRef]) }; $default_value }; $self->{q[attr]} = $value;  }
...
*_set_attr = sub { (ref($_[1]) eq 'ARRAY') or require Carp && Carp::croak(q[Type check failed in writer: value should be ArrayRef]); $_[0]{q[attr]} = $_[1]; $_[0]; };
