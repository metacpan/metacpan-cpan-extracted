...
When a C<$name> (a string) given, the chapter with that name is returned, or
C<undef> when it is not known.  With an C<$object>, that object is added to
...
Returns the subroutine with the specified C<$name> as object reference.  When
...
Call L<OODoc::Text::Structure::all()|OODoc::Text::Structure/"Collected"> on all chapters, passing the C<$method>
and C<$parameters>.  In practice, this means that you can simply collect
...
Returns whether the C<$subroutine> or C<$option> was defined by this manual page,
...
using Object::Realize::Later magic.  When C<$packages> (names or objects)
...
a trick of the Object::Realize::Later module.  The C<$package> (name or
...
When C<$packages> (names or objects) are specified, they are first added
to the list.
...
All C<%options> are passed to L<OODoc::Parser::Markov::finalizeManual()|OODoc::Parser::Markov/"Producing manuals">.

=item $obj-E<gt>B<mergeStructure>(%options)

Merge two lists of structured text objects: the C<this> and C<super> lists.
The C<this> objects are defined on this level of inheritance, where the
C<super> objects are from an inheritence level higher (super class).
...
The C<$object> (a text element) is located in some subsection, section or
chapter.  But the C<$object> may also be an extension to a piece of
...
=item Error: No chapter NAME in scope of package $pkg in $file
...
=item Error: The NAME chapter does not have the right format in $file
...
=item Warning: order conflict '$take' before '$insert' in $file line $linenr
...
=item Warning: section without location in $manual: $section
...
Cast by C<mostDetailedLocation()>

=item Warning: subroutine '$name' location conflict:\n  $p1 in $man1\n  $p2 in $man2

Cast by C<mostDetailedLocation()>

=item Error: two chapters named $name in $file line $line1 and $line2
