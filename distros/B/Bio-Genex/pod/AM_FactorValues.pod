...
Bio::Genex::AM_FactorValues - Methods for processing data from the GeneX DB
linking table: AM_FactorValues

=head1 SYNOPSIS

  use Bio::Genex::AM_FactorValues;

  # instantiating a linking table instance
  my $AM_FactorValues = Bio::Genex::AM_FactorValues->new(id=>47,pkey_link=>'ef_fk');
  # or 
  my $AM_FactorValues = Bio::Genex::AM_FactorValues->new(id=>47,pkey_link=>'am_fk');

  # retrieve data from the DB for all columns
  $AM_FactorValues->fetch();

  # creating an instance, without pre-fetching all columns
  my $AM_FactorValues = new Bio::Genex::AM_FactorValues(id=>47,
                                              'pkey_link'=>'ef_fk');
  ...
  my @objects = Bio::Genex::AM_FactorValues->get_objects('pkey_link'=>'ef_fk',
                                                  23,57,98);


  # retrieving all instances from a table
  my @objects = Bio::Genex::AM_FactorValues->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $AM_FactorValues->id();

  # retrieving other DB column attributes
  my $ef_fk_val = $AM_FactorValues->ef_fk();
  $AM_FactorValues->ef_fk($value);

  my $am_fk_val = $AM_FactorValues->am_fk();
  $AM_FactorValues->am_fk($value);

  my $value_val = $AM_FactorValues->value();
  $AM_FactorValues->value($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::AM_FactorValues is
AM_FactorValues).


B<NOTE>: Class Bio::Genex::AM_FactorValues is linking table class. This means
...
value from the database. In Bio::Genex::AM_FactorValues, those two columns are
'ef_fk' and 'ef_fk'. When an instance of
Bio::Genex::AM_FactorValues is instantiated, either via C<new()> or
...
Bio::Genex::AM_FactorValues can access: I<raw> foreign key attributes,
...
Class Bio::Genex::AM_FactorValues defines the following utility variables for assisting
programmers to access the AM_FactorValues table.

=over 4

=item $Bio::Genex::AM_FactorValues::LIMIT
...
=item $Bio::Genex::AM_FactorValues::USE_CACHE
...
For example:
  ...
  my $AM_FactorValues = Bio::Genex::AM_FactorValues->new(id=>47);
  my $val = $AM_FactorValues->ef_fk();
...
instance of the class via the Bio::Genex::AM_FactorValues->methodname() syntax.
...
DB. For class Bio::Genex::AM_FactorValues, 'C<pkey_link>' can be either 'C<ef_fk>' or 'C<am_fk>'.
...
Bio::Genex::AM_FactorValues if a method in class C<ExperimentFactors> invokes its
foreign key accessor function, the column 'am_fk' will be
used to fetch values from the DB.

=item linking_table()

Used by generic functions to determine if a specified class is a
linking table class. For Bio::Genex::AM_FactorValues it returns 1, since it I<is>
...
Bio::Genex::AM_FactorValues it returns 'AM_FactorValues';
...
=item get_objects({pkey_link=>'ef_fk'}, @id_list)
...
Bio::Genex::AM_FactorValues simultaneously. For linking tables, there are three
different ways to invoke this method.

By passing in an C<@id_list>, get_objects() uses each element of the
list as a primary key for the AM_FactorValues table and returns a single
...
a warning from C<Bio::Genex::AM_FactorValues::initialize()>. Objects will be
...
instance of class Bio::Genex::AM_FactorValues.
...
instance. Class Bio::Genex::AM_FactorValues is a linking table and therefore it
can I<two> possible columns that represent the 'id'
('C<ef_fk>' and 'C<am_fk>'). When an instance of
class Bio::Genex::AM_FactorValues is created using either C<new()> or
...
Methods for the ef_fk attribute. This attribute is the first
...
Methods for the am_fk attribute. This attribute is the second
possible value for the 'C<pkey_link>' attribute at object creation
time.
...
on Tue Jan 23 22:53:43 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=AM_FactorValues
