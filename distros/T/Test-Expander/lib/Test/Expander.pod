...
B<Test::Expander> - A convenience layer for Test2::V1 that automates common test boilerplate.

=head1 SYNOPSIS

=head2 Basic Usage

Most tests need to reference the class and method under test. B<Test::Expander>
...
B<Test::Expander> replaces this with a standardized, automated mechanism that
reduces maintenance effort, improves readability, and allows test files to
focus on asserting behavior.

=head1 DESCRIPTION

B<Test::Expander> provides a convenience layer on top of L<Test2::V1|https://metacpan.org/pod/Test2::V1>.
...
As a result, the function B<test_table> is exported by default providing a possibility to avoid copy-and-paste
...
The function B<test_table> can be used in two different ways:
...
Assuming such a test file should verify whether the function B<Foo::Bar::baz> returns

=over 2

=item B<0>

if the first parameter contains the string B<'abc'> and the second parameter is omitted,

=item B<1>

if both the first and the second parameters contain B<1>,

=item and, finally, B<0>

if both the first and the second parameters contain B<0>.
...
providing the methods / functions B<tempdir> and B<tempfile>.
...
B<Test::Expander> combines all advanced possibilities provided by L<Test2::V1|https://metacpan.org/pod/Test2::V1>
...
(see variable B<$CLASS> below) so that in contrast to L<Test2::V1|https://metacpan.org/pod/Test2::V1>
...
B<-target> - identical to the same-named option of L<Test2::V1|https://metacpan.org/pod/Test2::V1> and, if contains
a defined value, has the same purpose namely the explicit definition of the class / module to be tested as the value.
However, if its value is B<undef>, this is not passed through to L<Test2::V1|https://metacpan.org/pod/Test2::V1> so that
