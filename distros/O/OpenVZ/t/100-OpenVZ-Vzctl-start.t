...
BEGIN {
    ...
    $subcommand = 'start';                # <<<--- Change this to match the command you are testing against.

    use_ok( 'OpenVZ::Vzctl', qw( vzctl known_options subcommand_specs ), $subcommand );

}

my @parms = sort qw( force wait );        # <<<--- Change this to match the parameters you are expecting (checked against
                                          # known_options).
