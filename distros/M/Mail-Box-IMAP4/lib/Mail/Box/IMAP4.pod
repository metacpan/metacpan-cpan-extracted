...
The C<new> can have many C<%options>.  Not only the ones listed here below,
but also all the C<%options> for L<Mail::Transport::IMAP4::new()|Mail::Transport::IMAP4/"METHODS"> can be
passed.
...
Improves base, see L<Mail::Box/"Constructors">
...
Within this L<Mail::Box::IMAP4|Mail::Box::IMAP4> class is registered which transporters are
...
The name of the C<$class> which will interface with the connection.  When you
implement your own extension to L<Mail::Transport::IMAP4|Mail::Transport::IMAP4>, you can either
specify a fully instantiated transporter C<$object>, or the name of your own
C<$class>.  When an C<$object> is given, most other options will be ignored.
...
Extends L<"Attributes" in Mail::Box::Net|Mail::Box::Net/"Attributes">.
...
C<$class> defines the exact object type.  As C<%options>, everything which is
...
See L<new(join_connection)|Mail::Box::IMAP4/"METHODS">.  When C<false>, the connection will never be shared
...
An ARRAY of C<$messages> may be specified or some message C<$selection>,
...
The C<$info> contains one or more attributes as defined by the IMAP protocol.
...
if the C<$message> disappeared from the server.
...
When an C<$object> is specified, it is set to be the transporter from
that moment on.  The C<$object> must extend L<Mail::Transport::IMAP4|Mail::Transport::IMAP4>.
