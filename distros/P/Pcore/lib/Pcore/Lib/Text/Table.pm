package Pcore::Lib::Text::Table;

use Pcore -const, -class;
use Pcore::Lib::Text::Table::Column;
use Pcore::Lib::List qw[pairs];
use Pcore::Lib::Text qw[wrap];
...
has cols => ( required => 1 );    # ArrayRef [ InstanceOf ['Pcore::Lib::Text::Table::Column'] ]
...
sub BUILDARGS ( $self, $args ) {
    ...
    for my $col ( pairs $args->{cols}->@* ) {
        push $cols->@*, Pcore::Lib::Text::Table::Column->new( { $col->value->%*, id => $col->key, idx => $idx++ } );
    }
