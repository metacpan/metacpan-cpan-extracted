=comment Beginning of file pod/perlvar.pod

=begin pod

=head X<NAME;manpages/perlvar;perlvar>

perlvar - Perl predefined variables

=head DESCRIPTION

=head X<Predefined Names>
...
the methods do nothing to the current value, except for I<R<autoflush()>>, 
...
Here are the places where Perl will assume C<R<$_>> even if you don't use 
it:
...
Various unary functions, including functions like I<R<ord()>> and I<R<int()>>, 
as well as the all file tests (C<-f>, C<-d>) except for C<-t>, which 
defaults to STDIN.

=item * 

Various list functions like I<R<print()>> and I<R<unlink()>>.
...
The default iterator variable in a C<foreach> loop if no other variable is 
supplied.

=item * 

The implicit iterator variable in the I<R<grep()>> and I<R<map()>> 
functions.
...
=item X<$E<lt>I<digit>;$<digit>E<gt>
...
matches hidden within a BLOCK or I<R<eval()>> enclosed by the current 
...
match (not counting any matches hidden within a BLOCK or I<R<eval()>> 
...
newlines can produce confusing results when C<$*> is 0. Default is 0. 
(Mnemonic: * matches multiple things.) Note that this variable only 
influences the interpretation of C<^> and C<$>. A literal newline can be 
searched for even when C<$* == 0>.

Use of C<$*> is deprecated in Perl 5.

=item X<input_line_number> HANDLE EXPR

=item X<$INPUT_LINE_NUMBER>
...
explicit close on the filehandle resets the line number. Since C<E<lt>E<gt>> 
never does an explicit close, line numbers increase across ARGV files (but 
see examples under I<R<eof()>>). Localizing C<$.> has the effect of also 
...
more consecutive blank lines as a single blank line. Setting it to C<"\n\n"> 
...
order to get behavior more like B<awk>, set this variable as you would set 
B<awk>'s OFS variable to specify what is printed between fields. 
(Mnemonic: what is printed when there is a , in your print statement.)

=item X<output_record_separator> HANDLE EXPR

=item X<$OUTPUT_RECORD_SEPARATOR>
...
more like B<awk>, set this variable as you would set B<awk>'s ORS variable 
to specify what is printed at the end of the print. (Mnemonic: you set C<$\> 
...
Default is "\034", the same as SUBSEP in B<awk>. Note that if your keys 
contain binary data there might not be any safe value for C<$;>. 
(Mnemonic: comma (the syntactic subscript separator) is a semi-semicolon. 
Yeah, I know, it's pretty lame, but C<$,> is already taken for something 
more important.)
...
attempt to emulate B<awk>'s OFMT variable. There are times, however, when 
B<awk> and Perl have differing notions of what is in fact numeric. Also, 
the initial value is %.20g rather than %.6g, so you need to set C<$#> 
...
channel. Default is name of the filehandle. (Mnemonic: brother to C<$^>.)

=item X<format_top_name> HANDLE EXPR

=item X<$FORMAT_TOP_NAME>
...
The current value of the I<R<write()>> accumulator for I<R<format()>> 
lines. A format contains I<R<formline()>> commands that put their result 
into C<$^A>. After calling its format, I<R<write()>> prints out the 
contents of C<$^A> and empties. So you never actually see the contents of 
C<$^A> unless you call I<R<formline()>> yourself and then look at it. See 
...
The status returned by the last pipe close, backtick (C<``>) command, or I<R<system()>> 
operator. Note that this is the status word returned by the I<R<wait()>> 
system call, so the exit value of the subprocess is actually (C<$? E<gt>E<gt> 
8>). Thus on many systems, C<$? & 255> gives which signal, if any, the 
...
the usual caveats. (This means that you shouldn't depend on the value of C<$!> 
...
corresponding system error string. You can assign to C<$!> in order to set 
I<errno> if, for instance, you want C<$!> to return the string for error I<n>, 
or you want to set the exit value for the I<R<die()>> operator. (Mnemonic: 
What just went bang?)
...
The Perl syntax error message from the last I<R<eval()>> command. If null, 
the last I<R<eval()>> parsed and executed correctly (although the 
...
however, set up a routine to process warnings by setting C<R<$SIG>>{__WARN__} 
below.
...
The real uid of this process. (Mnemonic: it's the uid you came I<FROM>, if 
...
(Mnemonic: it's the uid you went I<TO>, if you're running setuid.) Note: C<$E<lt>> 
and C<$E<gt>> can only be swapped on machines supporting I<R<setreuid()>>.
...
of groups you are in. The first number is the one returned by I<R<getgid()>>, 
and the subsequent ones by I<R<getgroups()>>, one of which may be the same 
as the first number. (Mnemonic: parentheses are used to I<GROUP> things. 
The real gid is the group you I<LEFT>, if you're running setgid.)
...
membership in multiple groups simultaneously, gives a space separated list 
of groups you are in. The first number is the one returned by I<R<getegid()>>, 
and the subsequent ones by I<R<getgroups()>>, one of which may be the same 
as the first number. (Mnemonic: parentheses are used to I<GROUP> things. 
The effective gid is the group that's I<RIGHT> for you, if you're running 
setgid.)

Note: C<$E<lt>>, C<$E<gt>>, C<$(> and C<$)> can only be set on machines 
that support the corresponding I<set[re][ug]I<R<id()>>> routine. C<$(> and 
C<$)> can only be swapped on machines supporting I<R<setregid()>>. Because 
Perl doesn't currently use I<R<initgroups()>>, you can't set your group 
...
Assigning to C<$0> modifies the argument area that the I<R<ps(1)>> program 
sees. This is more useful as a way of indicating the current program state 
than it is for hiding the program you're running. (Mnemonic: same as B<sh> 
and B<ksh>.)
...
more like B<awk> (or Fortran) when subscripting and when evaluating the I<R<index()>> 
and I<R<substr()>> functions. (Mnemonic: [ begins subscripts.)

As of Perl 5, assignment to C<$[> is treated as a compiler directive, and 
...
The string printed out when you say C<perl -v>. (This is currently I<BROKEN>). 
...
The current value of the debugging flags. (Mnemonic: value of S<R<-D;switches/-D;-D>> 
switch.)
...
are passed to I<R<exec()>>ed processes, while higher file descriptors are 
not. Also, during an I<R<open()>>, system file descriptors are preserved 
even if the I<R<open()>> fails. (Ordinary file descriptors are closed 
before the I<R<open()>> is attempted.) Note that the close-on-exec status 
of a file descriptor will be decided according to the value of C<$^F> at 
...
inplace editing. (Mnemonic: value of S<R<-i;switches/-i;-i>> switch.)
...
(beginning of 1970). The values returned by the S<R<-M;switches/-M;-M>>, S<R<-A;switches/-A;-A>> 
and S<R<-C;switches/-C;-C>> filetests are based on this value.
...
related to the S<R<-w;switches/-w;-w>> switch.)
...
The array C<R<@ARGV>> contains the command line arguments intended for the 
...
The array C<R<@INC>> contains the list of places to look for Perl scripts 
to be evaluated by the C<do EXPR>, C<require>, or C<use> constructs. It 
initially consists of the arguments to any S<R<-I;switches/-I;-I>> command 
line switches, followed by the default Perl library, probably 
"/usr/local/lib/perl", followed by ".", to represent the current 
directory. If you need to modify this at runtime, you should use the C<use 
...
The hash C<R<%INC>> contains entries for each filename that has been 
included via C<do> or C<require>. The key is the filename you specified, 
and the value is the location of the file actually found. The C<require> 
command uses this array to determine whether a given file has already been 
included.

=item X<$ENV{expr}>

The hash C<R<%ENV>> contains your current environment. Setting a value in 
C<ENV> changes the environment for child processes.

=item X<$SIG{expr}>

The hash C<R<%SIG>> is used to set signal handlers for various signals. 
Example:
...
The C<R<%SIG>> array only contains values for the signals actually set 
...
Certain internal hooks can be also set using the C<R<%SIG>> hash. The 
routine indicated by C<R<$SIG>>{__WARN__} is called when a warning message 
...
The routine indicated by C<R<$SIG>>{__DIE__} is called when a fatal 
...
routine itself exits via a C<goto>, a loop exit, or a I<R<die()>>. The 
...
=comment End of file pod/perlvar.pod
