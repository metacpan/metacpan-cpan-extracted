...
BEGIN {
  plan skip_all =>
    ...
    . 'deployment_statements() no longer generates SQL.';
}

my $custom_deployment_statements_called = 0;

sub DBIO::Test::Schema::deployment_statements {
...
{
  my $schema = DBIO::SQLite::Test->init_schema(no_deploy => 1, quote_names => 1);
...

{
  ...
  local $DBIO::Test::Schema::Track::hook_cb = sub {
    my ($class, $sqlt_table) = @_;

    $deploy_hook_called = 1;

    is ($class, 'DBIO::Test::Track', 'Result class passed to plain hook');
  ...
  local $DBIO::Test::DeployComponent::hook_cb = sub {
...
# KEY constraints to DBIO::Test::Schema, add tests here if you think the existing
...
my %unique_constraints = (
  ...
  # Employee
# Constraint is commented out in DBIO::Test::Schema::Employee
#  employee => [
#    {
