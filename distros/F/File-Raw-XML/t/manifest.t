...
my @skip = (
    qr{^blib/}, qr{^File-Raw-XML-}, qr{^Makefile$}, qr{^Makefile\.old$},
    qr{^MYMETA\.}, qr{^META\.}, qr{^pm_to_blib$}, qr{^ignore\.txt$},
    qr{^Install/},                      # written by Makefile.PL, not shipped
    qr{^plan_full_file_xml/},           # the checkout's plan, not shipped
    qr{^t/xmlconf/(?:xmlconf/|xmlts.*\.tar\.gz$)},   # the suite, fetched not shipped
    qr{^MANIFEST\.bak$},
    qr{\.(?:c|o|obj|bs|tar\.gz)$},
    # what tools/fuzz/build.sh leaves behind (the same line as MANIFEST.SKIP)
    qr{^tools/fuzz/(?:core_only$|corpus/|corpus-min/|crash-|leak-|timeout-|slow-unit-|fuzz-.*\.log$|.*\.dSYM)},
