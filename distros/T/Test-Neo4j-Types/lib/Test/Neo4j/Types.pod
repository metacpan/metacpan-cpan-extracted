...
module certainly won't cover all requirements, they will cover
many of them.
...
You probably shouldn't be using this module at this time, because
the requirements it checks for are evolving and might shift under
your feet at any moment. For further information, see
...
on the elements provided in the array ref C<$elements>.
The elements are alternating between references that
represent nodes and relationships in the exact same order
as for L<Neo4j::Types::Path/"elements">:
...
Note that unlike the other test tools, this one doesn't
need a constructor sub reference. L<Neo4j::Types::Point>
requires one anyway because Point is a Neo4j property type,
which means it should in theory be possible to construct
points without using Cypher.
(However, passing of Point values as parameters is not
currently implemented in any known CPAN module.)

With all that said, this test tool may change in future, even
independent of the reasons stated in L</"BUGS AND LIMITATIONS">.
...
If these tools are ever developed further than this, switching
to L<Test2::Manual::Tooling> should be considered.
