...
Perl-Tests-Covering says which tests of a distribution load a given file, so
that a pre-commit hook can run only those.  It runs each test once under
Devel::Cover, keeps what each test loaded in a cache with a digest per file, and
runs a test again only when one of those files changed.

Read these first, in this order: `lib/Perl/Tests/Covering.pm`, whose POD is the
contract; `lib/Perl/Tests/Covering/Recorder.pm`, which covers what Devel::Cover
cannot see; `t/Perl-Tests-Covering.t`, which says when a test runs again and
what is reported.
...
| | |
|---|---|
| `perldoc Perl-Tests-Covering` | what this is for and how a caller uses it |
...
| `perldoc bin/tests-covering` | the command line, and the pre-commit hook built on it |

`t/integration-Perl-Tests-Covering.t` runs tests under Devel::Cover for real,
and only with `RELEASE_TESTING=1`.  Run it whenever you touch how a run starts
or how its database is read: the unit tests mock both.
...
Then the mechanical ones:
    ...
    RELEASE_TESTING=1 prove -lm -j8 t/
