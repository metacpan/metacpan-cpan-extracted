...
=head1 FUNCTIONS
   ...
   '<<' operators has now been added.
    ...
    or 'Math::MPFR'/'Math::GMPz'/'Math::GMP' will not work. (Support
    for Math::GMP objects is limited.)

    Math::MPFR objects can be provided as the second arg with '+',
    '-', '*', '/', '%' and '**' operators if Math::MPFR is at version
    3.13 or greater. (Support for '%' overloading requires
    Math-MPFR-4.35 or later.) In these cases, the operation will
