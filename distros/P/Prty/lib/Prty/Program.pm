package Prty::Program;
use base qw/Prty::Process Prty::Hash/;
...
Prty::Program - Basisklasse für Programme
...
Programm:
    ...
    use Prty::Program;
    exit Prty::Program->run('MyProg')->exitCode;
...
=head4 Synopsis
    ...
    $prg = Prty::Program->run($programClass,@options);
...
sub run {
    ...
    my $self = $class->new(@_);
    eval {
        Prty::Perl->loadClass($programClass);
        ...
        while (@arr) {
            ...
            if ($key eq 'programClass') {
                ...
                Prty::Perl->loadClass($subProgramClass);
...
sub new {
    ...
    Prty::Perl->binmode(*STDIN,":encoding($encoding)");
    Prty::Perl->binmode(*STDOUT,":encoding($encoding)");
    Prty::Perl->binmode(*STDERR,":encoding($encoding)");
