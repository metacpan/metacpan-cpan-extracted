...
L<Form::Tiny> is a form validation engine which can use all the type constraints you're already familiar with. The module does not ship with any field definitions on its own, instead it provides tools to reuse any type constraints from L<Type::Tiny> and other similar systems.
...
Form fields end up as instances of L<Form::Tiny::FieldDefinition> or its descendants. Also see full keyword docs at L<Form::Tiny/form_field>.
...
Value that is a coderef will be passed C<($self, $value)>. It is required to make its own checks and return a scalar which will replace the old value.
...
The default value does not support nested arrays, like C<array.*>. An exception will be thrown at compile time if this condition is not met.
...
Validators consist of two parts - an error message that should be added to form in case validation fails, and a subroutine that will perform the check. This subroutine is passed C<($self, $value)>, and should return a boolean value - validation result.
...
B<before_validate> is fired just once for the form, before any field is validated. It is passed a hashref - the input data. This method is free to do anything with the input.
...
This hook does not require you to return anything and is passed the same data as C<cleanup>. Note that it will not be fired if the input data was not a hash reference.
...
=item * it is ran regardless of previous validation being successful or not, but only if the data format is correct
...
Filter subroutines should accept two parameters: C<($self, $value)>, and must return a C<$value>, changed or unchanged.
...
A filter can be also narrowed down to a single form field with C<field_filter> keyword, which will apply to the last field declared:
...
Added behavior is entirely up to the plugin. See L<Form::Tiny::Plugin> for details on how to implement a plugin.
