...

{
  ...
  sub said {
    ...
    if ($message->{who} eq 'RevolutionaryBot') {
      like($message->{body}, qr/^Nous sommes le.*, il est/, 'Spread to Liberachat for channel ' . $message->{channel});
      $channels_said{$message->{channel}}++;
    }

    if (scalar(grep { $channels_said{$_} } keys %channels_said) == scalar($self->channels)) {
      ok(1, "Spread to all Liberachat channels");
      $self->shutdown;
    }
  ...
  sub tick {
    ...
    $nb_ticks++;
    if ($nb_ticks > 5) {
      ok(0, "Spread only to " . scalar(grep { $channels_said{$_} } keys %channels_said) . "/" . scalar($self->channels) . " Liberachat channels");
...
my $channels = $spread_revolutionary_date->config->liberachat_test_channels;
