package Proc::ProcessTable::Match::Size;
...
Proc::ProcessTable::Match::Size - Check if the Size of a process matches.
...
=head1 SYNOPSIS
    ...
    use Proc::ProcessTable::Match::Size;
...
that is 'sizes', which is a array of Sizes to match.

The Size values can be prefixed with the equalities below for doing
...
sub match{
	...
	my $proc_size;
	eval{
		$proc_size=$object->size;
	};

	# don't bother proceeding, the object won't match ever
	# as it does not have a Size
	if ( ! defined( $proc_size ) ){
		return 0;
	}

	# use while as foreach will reference the value
	my $size_int=0;
	while (defined( $self->{sizes}[$size_int] )){
		my $size=$self->{sizes}[$size_int];
