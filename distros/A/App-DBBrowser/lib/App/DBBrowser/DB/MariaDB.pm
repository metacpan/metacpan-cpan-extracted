...
sub get_databases {
    ...
    #my $file_MariaDB_dbs = sprintf $sf->{i}{db_cache_file_fmt}, $sf->{i}{plugin}; ##
    #my $cache = $ax->read_json( $file_MariaDB_dbs ) // {};
    ...
    my $user_db   = [];
    my $system_db = [];
    for my $database ( map { s/^DBI:MariaDB://r } @$databases ) {
    ...
    #};
    #$ax->write_json( $file_MariaDB_dbs, $cache );
