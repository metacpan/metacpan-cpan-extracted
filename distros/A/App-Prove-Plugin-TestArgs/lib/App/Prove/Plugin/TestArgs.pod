...
to a L<functions/sprintf> like format. The allowed conversions are C<%s> (full
path to the test script) and C<%a> (test alias). The format tells C<prove> what
to use as the test name in its test output. The C<name> key is optional. If it
is omitted the C<scripts> key can be omitted too and this brings us back to the
original configuration scheme.
