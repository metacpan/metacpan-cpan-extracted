This is the Security Policy for the Perl DBIO-Oracle distribution.
...
[DBIO-Oracle on CPAN](https://metacpan.org/dist/DBIO-Oracle) or the main branch of the
[DBIO-Oracle git repository](https://codeberg.org/dbio/dbio-oracle).
...
DBIO-Oracle is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO-Oracle are covered by this policy.
...
they affect DBIO-Oracle, or DBIO-Oracle can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO-Oracle, or plugins to it that are not included with the DBIO-Oracle
distribution) are not covered by this policy.

## Supported Versions of DBIO-Oracle

Only the latest release of DBIO-Oracle will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO-Oracle will run on older versions of Perl. If a
...
that are required for DBIO-Oracle to work. However, some of these
