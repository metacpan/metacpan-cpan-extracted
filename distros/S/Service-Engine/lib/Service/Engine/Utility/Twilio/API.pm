package Service::Engine::Utility::Twilio::API;
...
sub NOTIFY_API_URL     { 'https://notify.twilio.com/v1' }
...
sub new {
    ...
    $service_sid  {$self} = $args{ServiceSid}   || '';
...
sub _do_request {
    ...
    $lwp->agent("perl-service-engine-API/$VERSION");
    ...
    if ($service_sid{$self}) {
    	$url = NOTIFY_API_URL();
    	$url .= "/Services/" . $service_sid{$self} . "/$api";
    } else {
...
HMG::Twilio::API - Accessing Twilio's REST API with Perl

=head1 SYNOPSIS

  use WWW::Twilio::API;

  my $twilio = HMG::Twilio::API->new(AccountSid => 'AC12345...',
                                     AuthToken  => '1234567...');
...
Versions of B<HMG::Twilio::API> prior to 0.15 defaulted to
...
against a different API, you may pass it into the constructor:
  ...
  $t = HGM::Twilio::API->new( AccountSid  => '...',
                              AuthToken   => '...',
