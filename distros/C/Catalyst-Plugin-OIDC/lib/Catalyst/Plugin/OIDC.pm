...
our $VERSION = '1.01'; # VERSION: generated by Dist::Zilla::Plugin::OurPkgVersion
...
sub _oidc_login_callback ($self, $c) {
  ...
  my $auth_data = $self->_oidc_get_auth_data($c);

  try {
    $c->oidc($auth_data->{provider})->get_token();
    $c->response->redirect($auth_data->{target_url} || $c->uri_for('/'));
  }
  catch {
    my $e = $_;
...
sub _oidc_get_auth_data ($self, $c) {
...
sub _oidc_logout_callback ($self, $c) {
  ...
  my $logout_data = $self->_oidc_extract_logout_data($c);

  $c->oidc($logout_data->{provider})->delete_stored_data();

  $c->response->redirect($logout_data->{target_url} || $c->uri_for('/'));
}

sub _oidc_extract_logout_data ($self, $c) {
  my $state = $c->req->params->{state}
...
sub _oidc_get_client_for_provider ($c, $provider) {
  $provider //= $c->_oidc_config->{default_provider};
