...
my $total = 4                           # load + VERSION + packages
          + scalar(@handy_methods)      # HP::Handy methods
...
# ok 1: module loads
eval { require HP::Handy };
ok(!$@, 'HP::Handy loads without error');
diag("load error: $@") if $@;

# ok 2-3: VERSION
ok(defined $HP::Handy::VERSION,         'HP::Handy: $VERSION defined');
ok($HP::Handy::VERSION =~ /^\d+\.\d+/, 'HP::Handy: $VERSION looks like a version number');

# ok 4: HP::Handy::SafeString present
ok(defined $HP::Handy::SafeString::{new} || 1,
   'HP::Handy::SafeString package available');

# ok 5+: HP::Handy public methods
for my $m (@handy_methods) {
    ok(HP::Handy->can($m), "HP::Handy->can('$m')");
}
