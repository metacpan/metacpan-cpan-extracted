...
Data::SIprefixes::yotta - This provides yotta matching for Data::SIprefixes.
...
=head1 SYNOPSIS
    ...
    use Data::SIprefixes::yotta;
    
    my $prefix=Data::SIprefixes::yotta->new;

    my $origMeasure='yottameter';
...
sub longMatch{
	...
	if ( $measure=~/^yotta/ ){
		...
		if ( $measure =~ /^ / ){
			$self->{error}=2;
			$self->{errorString}='Space found after prefix, /^yetta/, in "'.$origMeasure.'"' ;
			$self->warn;
			return undef;
		}
...
sub shortMatch{
    ...
    if ( $measure=~/^Y/ ){
        ...
        if ( $measure =~ /^ / ){
            $self->{error}=2;
            $self->{errorString}='Space found after prefix, /^Y/, in "'.$origMeasure.'"' ;
            $self->warn;
            return undef;
        }
...
1; # End of Data::SIprefixes::yotta
