...
our $VERSION = '1.08'; # VERSION: generated by Dist::Zilla::Plugin::OurPkgVersion
...
enum 'CodeChallengeMethod'   => [qw/plain S256/];
...
The PKCE code challenge, derived from a code verifier. When provided,
it is added as the C<code_challenge> parameter in the authorization URL.

=item code_challenge_method

The method used to derive the C<code_challenge> from the code verifier.
...
sub auth_url {
  ...
  my (%params) = validated_hash(
    ...
    code_challenge        => { isa => 'Str', optional => 1 },
    code_challenge_method => { isa => 'CodeChallengeMethod', optional => 1 },
  ...
  foreach my $param_name (qw/state nonce code_challenge code_challenge_method/) {
...
The PKCE code verifier. The value must be exactly the same as the one
used to generate the C<code_challenge> sent in the authorization request.

Optional, but required when the authorization request included a
C<code_challenge>.
...
sub get_token {
  ...
  my (%params) = validated_hash(
    ...
    code_verifier => { isa => 'Str', optional => 1 },
    redirect_uri  => { isa => 'Str', optional => 1 },
  ...
  if ($grant_type eq 'authorization_code') {
    ...
    if (my $code_verifier = $params{code_verifier}) {
      $form->{code_verifier} = $code_verifier;
    }
    elsif ($self->use_pkce) {
      $self->log_msg(warning => "OIDC: PKCE is enabled but no code_verifier provided in get_token()");
    }
