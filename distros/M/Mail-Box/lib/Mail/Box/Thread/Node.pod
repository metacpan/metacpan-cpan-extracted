...
Indicates the C<$class> name of dummy messages. Dummy messages are
placeholders in a L<Mail::Box::Thread::Manager|Mail::Box::Thread::Manager> data structure.

=item message => $message

The C<$message> which is stored in this node.  The C<$message> object
must be a L<Mail::Box::Message|Mail::Box::Message>.

=item messageId => $msgid

The C<$msgid> for the message which is stored in this node.  Only
...
Returns C<true> if the message is a dummy. A dummy is a "hole" in a thread
...
Register that the C<$threads> are follow-ups to this message. These
...
Register that the current thread is a reply to the specified C<$thread>. The
C<$quality> of the relation is specified by the second argument.  The method
...
relation is used. For example, if a reply (C<$quality> equals C<REPLY>) is
specified, later calls to the follow method will have no effect. If
C<follows> is called with a C<$quality> that matches the current quality, the
...
Execute a function for all sub-threads.  If the subroutine returns C<true>,
...
The optional C<CODE> argument is a reference to a routine which will be called
...
=item Error: thread node needs a message object or msgid.
