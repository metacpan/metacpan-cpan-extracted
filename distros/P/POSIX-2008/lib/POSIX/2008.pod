...
For system calls that return -1 on failure and 0 on success the return value
is mapped to undef on failure and "0 but true" on success. This is the
majority of system calls.

For system calls where 0 is a valid return value it is left unchanged. These
are C<read()>, C<write()>, C<pread()>, C<pwrite()>. C<openat()> gets a special
treatment in this module, see its description below.
...
C<pread()> reads I<nbytes> bytes of data from the file descriptor I<fd> at
file offset I<offset> into the scalar I<buf> without changing the file
offset. I<buf> will be enlarged automatically if necessary.
...
If I<nbytes> is omitted, it defaults to the length of I<buf>
...
available:
 ...
 :prw    pread() and pwrite()
 :clock  All clock_* functions like clock_gettime() etc. plus all CLOCK_ constants
