...
Multiple proposals are being considered for rescheduling, scheduling incrementally (C<begin>, C<push>, C<pop>), scheduling adjustment to target different attributes, as well as attribute-level goals.  Other feature improvements below are related to scheduling.
...
Some annotations may occur before an action but not when after certain other actions.  Annotations should support node filtering for this situation.

Other improvements include helpers to merge annotation attributes into schedules.  This also requires options to control the order/materialization of annotation generation.  Proposed is annotation configuration per-node (with a keyname, possibly duplicated in the upper-level configuration).
...
In 0.1.9, some annotation handling is provided.
