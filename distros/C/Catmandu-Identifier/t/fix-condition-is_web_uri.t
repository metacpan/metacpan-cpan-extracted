...
my $pkg;
BEGIN {
    $pkg = 'Catmandu::Fix::Condition::is_web_uri';
    use_ok $pkg;
}
...
is_deeply
    $cond->fix({uri => "http://foo.bar/file.txt"}),
    {uri => "http://foo.bar/file.txt", test => 'pass'},
    "is valid";

is_deeply
    $cond->fix({uri => "https://foo.bar/file.txt"}),
    {uri => "https://foo.bar/file.txt", test => 'pass'},
    "is valid";
