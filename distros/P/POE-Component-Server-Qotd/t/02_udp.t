...
BEGIN { use_ok('POE::Component::Server::Qotd') };

use Socket;
use POE;

my $self = POE::Component::Server::Qotd->spawn( Alias => 'Qotd-Server', BindPort => 0,
			BindAddress => '127.0.0.1' );

isa_ok ( $self, 'POE::Component::Server::Qotd' );
...
sub _get_datagram {
  ...
  ok( $message eq 'Parts that positively cannot be assembled in improper order will be.', 'Qotd test' );
  $kernel->select_read( $socket );
  $kernel->post( 'Qotd-Server' => 'shutdown' );
  return;
}
