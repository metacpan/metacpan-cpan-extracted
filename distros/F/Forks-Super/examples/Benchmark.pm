# Forks-Super-x.xx/examples/Benchmark.pm
#
# modification of Benchmark.pm file from Benchmark distribution v1.22
# that supports parallel evaluation of benchmark code with Forks::Super.
#
# To enable parallelization, set @INC to make this file available, and
# call
#
...
July, 2018, by Marty O'Brien: add ':parallel' special tag
...
sub import {
	...
	*mytime = $hirestime if defined $hirestime;
    ...
    if (grep { $_ eq ":parallel" } @_) {
	@_ = grep { $_ ne ":parallel" } @_;
        our $USE_PARALLEL = eval { use Forks::Super 'pmap'; 1 };
        warn "Benchmark: parallel service not available: $@" if $@;
    }
