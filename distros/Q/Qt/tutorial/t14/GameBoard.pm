...
sub new {
    ...
    my $frame = new QFrame($self, 'cannonFrame');
    ...
    my $accel = new QAccel($self);
    $accel->connectItem($accel->insertItem($Key{Space}), $self, 'fire()');
    $accel->connectItem($accel->insertItem($Key{Q}), $qApp, 'quit()');
