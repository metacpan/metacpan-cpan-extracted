...
######################################################################
#
# KSC5601 - Source code filter to escape KSC5601 script
#
...
# regexp of character
my $qq_char   = qr/(?> \\c[\x40-\x5F] | \\? (?:[\xA1-\xFE][\x00-\xFF] | [\x00-\xFF]) )/oxms;
my  $q_char   = qr/(?> [\xA1-\xFE][\x00-\xFF] | [\x00-\xFF] )/oxms;
...
perl $0 KSC5601_script.pl > Escaped_script.pl.e
END
    }

    print KSC5601::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Eksc5601::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
        ...
        my $e_script = KSC5601::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Eksc5601::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub KSC5601::escape_script {
    ...
    # read KSC5601 script
    my $fh = gensym();
    Eksc5601::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Eksc5601(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Eksc5601 '%s.0'; # 'quote' for perl5.6\n", $KSC5601::VERSION); # require run-time routines version

        # use KSC5601 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | KSC5601 ) (?>\s*) ([^\xA1-\xFE;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $KSC5601::VERSION) {
                    my @file = grep -e, map {qq{$_/KSC5601.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects KSC5601.pm $version, but @{[__FILE__]} is version $KSC5601::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Eksc5601.pm $version, but \\\$Eksc5601::VERSION is \$Eksc5601::VERSION" if \$Eksc5601::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $KSC5601::VERSION) {
                    die "Script $0 required KSC5601.pm $version, but @{[__FILE__]} is only version $KSC5601::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Eksc5601::function_ord     = 'KSC5601::ord'     if /\A ord \z/oxms;
                    $Eksc5601::function_ord_    = 'KSC5601::ord_'    if /\A ord \z/oxms;
                    $Eksc5601::function_reverse = 'KSC5601::reverse' if /\A reverse \z/oxms;
                    $Eksc5601::function_getc    = 'KSC5601::getc'    if /\A getc \z/oxms;

                    # avoid: used only once: possible typo at ...
                    $Eksc5601::function_ord     = $Eksc5601::function_ord;
                    $Eksc5601::function_ord_    = $Eksc5601::function_ord_;
                    $Eksc5601::function_reverse = $Eksc5601::function_reverse;
                    $Eksc5601::function_getc    = $Eksc5601::function_getc;
                }
            }
        }
    }

    $e_script .= KSC5601::escape();
...
=item * You'll have to download KSC5601.pm and Eksc5601.pm and put it in your perl lib directory.

=item * You'll need to write "use KSC5601;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use KSC5601;
  use KSC5601 ver.sion;             --- require minimum version
  use KSC5601 ver.sion.0;           --- expects version (match or die)
  use KSC5601 qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use KSC5601 ver.sion qw(ord reverse getc);
  use KSC5601 ver.sion.0 qw(ord reverse getc);

  # "no KSC5601;" not supported

  or

  $ perl KSC5601.pm KSC5601_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  KSC5601_script.pl  --- script written in KSC5601
...
KSC5601 software is "middleware" between perl interpreter and your Perl script
written in KSC5601.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain KSC5601, Perl3 or later
can treat KSC5601 as binary data.

By "use KSC5601;", it automatically interpret your script as KSC5601. The various
functions of perl including a regular expression can treat KSC5601 now.
...
This software is a source code filter to escape Perl script encoded by KSC5601
...
=item * Remains One Language Handling Raw KSC5601, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. KSC5601.pm, Eksc5601.pm
  +---------------------------------------+
...
import of Eksc5601.pm works well.
...
=head1 Software Composition
   ...
   KSC5601.pm               --- source code filter to escape KSC5601
   Eksc5601.pm              --- run-time routines for KSC5601.pm
...
You need write 'use KSC5601;' in your script.
...
=head1 Multiple-Octet Anchoring of Regular Expression (KSC5601 software provides)

KSC5601 software applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Eksc5601::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Multiple-Octet Character Regular Expression (KSC5601 software provides)

KSC5601 software clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Eksc5601::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Eksc5601::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Eksc5601::ignorecase()' (KSC5601 software provides)

KSC5601 software applies calling 'Eksc5601::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Eksc5601::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Eksc5601::matched/
  m/regexp/               m/ditto$Eksc5601::matched/
  ?regexp?                m?ditto$Eksc5601::matched?
  m?regexp?               m?ditto$Eksc5601::matched?
 
  $_ =~                   ($_ =~ m/ditto$Eksc5601::matched/) ?
  s/regexp/replacement/   CORE::eval{ Eksc5601::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Eksc5601::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Eksc5601::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Eksc5601::split(qr/regexp/)
  split(m/regexp/)        Eksc5601::split(qr/regexp/)
  split(qr/regexp/)       Eksc5601::split(qr/regexp/)
  qr/regexp/              qr/ditto$Eksc5601::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Eksc5601::matched/
  m/regexp/b              m/(?:regexp)$Eksc5601::matched/
  ?regexp?b               m?regexp$Eksc5601::matched?
  m?regexp?b              m?regexp$Eksc5601::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Eksc5601::matched/) ?
  s/regexp/replacement/b  CORE::eval{ Eksc5601::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Eksc5601::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Eksc5601.pm provides)
...
Definitions in Eksc5601.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Eksc5601::anchor}         qr{\G(?>[^\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF])*?}
                           for over 32766 octets string on ActivePerl5.6 and Perl5.10 or later
                           qr{\G(?(?=.{0,32766}\z)\G(?>[^\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF])*?|(?(?=[$sbcs]+\z).*?|(?:.*?[$sbcs](?:[^$sbcs][^$sbcs])*?)))}oxms
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (KSC5601 software provides)

KSC5601 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (KSC5601 software provides)

Insert 'Eksc5601::' at head of function name. Eksc5601.pm provides your script Eksc5601::*
subroutines.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Eksc5601::split     Character
  tr///       Eksc5601::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Eksc5601::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Eksc5601::chop      Character
  index       Eksc5601::index     Character
  rindex      Eksc5601::rindex    Character
  lc          Eksc5601::lc        Character
  lcfirst     Eksc5601::lcfirst   Character
  uc          Eksc5601::uc        Character
  ucfirst     Eksc5601::ucfirst   Character
  fc          Eksc5601::fc        Character
  chr         Eksc5601::chr       Character
  glob        Eksc5601::glob      Character
  -------------------------------------------
...
You need write 'KSC5601::' at head of function name when you want character-
oriented subroutine. See 'Character-Oriented Subroutines'.
  ...
  --------------------------------------------------------
  ord        KSC5601::ord
  reverse    KSC5601::reverse
  getc       KSC5601::getc
  length     KSC5601::length
  substr     KSC5601::substr
  index      KSC5601::index          See 'About Indexes'
  rindex     KSC5601::rindex         See 'About Rindexes'
  eval       KSC5601::eval
  --------------------------------------------------------
  ...
  (same as Eksc5601::index)
  KSC5601::index    Character   Character    Character-oriented semantics
  ...
  (same as Eksc5601::rindex)
  KSC5601::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  $ord = KSC5601::ord($string);

  This subroutine returns the numeric value (ASCII or KSC5601 character) of the
  first character of $string, not Unicode. If $string is omitted, it uses $_.
  The return value is always unsigned.

  If you import ord "use KSC5601 qw(ord);", ord of your script will be rewritten in
  KSC5601::ord. KSC5601::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = KSC5601::reverse(@list);
  $reverse = KSC5601::reverse(@list);
  ...
  If you import reverse "use KSC5601 qw(reverse);", reverse of your script will be
  rewritten in KSC5601::reverse. KSC5601::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  If you import getc "use KSC5601 qw(getc);", getc of your script will be rewritten
  in KSC5601::getc. KSC5601::getc is not compatible with getc of JPerl.

=item * Length by KSC5601 Character

  $length = KSC5601::length($string);
  $length = KSC5601::length();

  This subroutine returns the length in characters (programmer-visible characters)
  of the scalar value $string. If $string is omitted, it returns the KSC5601::length
  of $_.

  Do not try to use KSC5601::length to find the size of an array or hash. Use scalar
...
=item * Substr by KSC5601 Character

  $substr = KSC5601::substr($string,$offset,$length,$replacement);
  $substr = KSC5601::substr($string,$offset,$length);
  $substr = KSC5601::substr($string,$offset);
  ...
  my $end    = KSC5601::substr $s, 14;        # climbed the green tree
  my $tail   = KSC5601::substr $s, -4;        # tree
  my $z      = KSC5601::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the KSC5601::substr() subroutine as an
  ...
  end of the string, KSC5601::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  KSC5601::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = KSC5601::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = KSC5601::substr $name, 7;      # returns undef, with warning
  KSC5601::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using KSC5601::substr() as an lvalue is to specify the replacement
  ...
  Note that the lvalue returned by the three-argument version of KSC5601::substr() acts
...
=item * Index by KSC5601 Character

  $index = KSC5601::index($string,$substring,$offset);
  $index = KSC5601::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = KSC5601::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }

=item * Rindex by KSC5601 Character

  $rindex = KSC5601::rindex($string,$substring,$offset);
  $rindex = KSC5601::rindex($string,$substring);

  This subroutine works just like KSC5601::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse KSC5601::index).
  ...
  $pos = KSC5601::length($string);
  while (($pos = KSC5601::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }

=item * Eval KSC5601 Script

  $eval = KSC5601::eval { block };
  $eval = KSC5601::eval $expr;
  $eval = KSC5601::eval;

  The KSC5601::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, KSC5601::eval { block }
  and KSC5601::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, KSC5601::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of KSC5601::eval, the value returned from an KSC5601::eval is
  ...
  KSC5601::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  KSC5601::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      KSC5601::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The KSC5601::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow KSC5601::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  KSC5601::eval cannot trap syntax errors.

  The KSC5601::eval $expr form can trap syntax errors because it parses the code
  ...
  after the KSC5601::eval is complete, as do any subroutine or format definitions.
  The code of the KSC5601::eval is treated as a block, so any locally scoped
  variables declared within the KSC5601::eval last only until the KSC5601::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  KSC5601::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  KSC5601::eval { block }    eval { block }
  KSC5601::eval $expr        eval KSC5601::escape $expr
  KSC5601::eval              eval KSC5601::escape
  -------------------------------------------------

  To tell the truth, the subroutine KSC5601::eval does not exist. If it exists,
  you will troubled, when KSC5601::eval has a parameter that is single quoted
  string included my variables. KSC5601::escape is a subroutine that makes Perl
  ...
  Since KSC5601::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because KSC5601::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  KSC5601::eval $expr and KSC5601::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      KSC5601::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      KSC5601::eval { $answer = }; # WRONG

      # a runtime syntax error
      KSC5601::eval '$answer =';   # sets $@
  ...
  Using the KSC5601::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      KSC5601::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          KSC5601::eval { die "foo lives here" };
  ...
  With an KSC5601::eval, you should be especially careful to remember what's being
  looked at when:

      KSC5601::eval $x;        # CASE 1
      KSC5601::eval "$x";      # CASE 2

      KSC5601::eval '$x';      # CASE 3
      KSC5601::eval { $x };    # CASE 4

      KSC5601::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  The block of KSC5601::eval { block } does not count as a loop, so the loop
...
=item * Filename Globbing
  ...
  This operator is implemented via the Eksc5601::glob() subroutine. See Eksc5601::glob
  of Eksc5601.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use KSC5601 qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use KSC5601 qw(reverse);" exists or not.
...
=head1 Yada Yada Operator (KSC5601 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (KSC5601 software provides)

KSC5601 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Eksc5601.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in KSC5601 software and
...
=item * KSC5601::substr as Lvalue

If Perl version is older than 5.14, KSC5601::substr differs from CORE::substr, and
...
KSC5601::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Eksc5601::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${`}            Eksc5601::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Eksc5601::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Eksc5601::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Eksc5601::MATCH()       $1
  ${&}            Eksc5601::MATCH()       $1
  $MATCH          Eksc5601::MATCH()       $1
  ${^MATCH}       Eksc5601::MATCH()       $1
  $'              $'                   $'
  ${'}            ${'}                 $'
  $POSTMATCH      Eksc5601::POSTMATCH()   $'
  ${^POSTMATCH}   Eksc5601::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
KSC5601 software remains one language and one interpreter.
