...
if (eval "use Hash::Util; 1") {
  ...
  for $Storable::Improved::downgrade_restricted (0, 1, undef, "cheese") {
  ...
  $hash = thaw_hash ('Locked keys placeholder', \%R_HASH);
  test_newkey ($hash);
  local $Storable::Improved::downgrade_restricted = 0;
...
# Please send the output of perl -V to perlbug\@perl.org");
  ...
  local $Storable::Improved::drop_utf8 = 1;
...
# Please send the output of perl -V to perlbug\@perl.org");
    ...
    local $Storable::Improved::drop_utf8 = 0;
    thaw_fail ('Locked hash with utf8 keys', $RESTRICTED_CROAK);
  }
}
__END__
