...
#
# http://search.cpan.org/dist/Char-EUCJP/
#
...
# regexp of character
my $qq_char   = qr/(?> \\c[\x40-\x5F] | \\? (?:[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF] | [\x00-\xFF]) )/oxms;
my  $q_char   = qr/(?> [\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF] | [\x00-\xFF] )/oxms;
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Eeucjp::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
    ...
    my $fh = gensym();
    Eeucjp::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub EUCJP::escape_script {
    ...
    # read EUC-JP script
    my $fh = gensym();
    Eeucjp::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script\n";

    if (/^ use Eeucjp(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Eeucjp '%s.0'; # 'quote' for perl5.6\n", $EUCJP::VERSION); # require run-time routines version

        # use EUCJP version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | EUCJP ) (?>\s*) ([^\x8E\x8F\xA1-\xFE;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $EUCJP::VERSION) {
                    my @file = grep -e, map {qq{$_/EUCJP.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects EUCJP.pm $version, but @{[__FILE__]} is version $EUCJP::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Eeucjp.pm $version, but \\\$Eeucjp::VERSION is \$Eeucjp::VERSION" if \$Eeucjp::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $EUCJP::VERSION) {
                    die "Script $0 required EUCJP.pm $version, but @{[__FILE__]} is only version $EUCJP::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Eeucjp::function_ord     = 'EUCJP::ord'     if /\A ord \z/oxms;
                    $Eeucjp::function_ord_    = 'EUCJP::ord_'    if /\A ord \z/oxms;
                    $Eeucjp::function_reverse = 'EUCJP::reverse' if /\A reverse \z/oxms;
                    $Eeucjp::function_getc    = 'EUCJP::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= EUCJP::escape();
...
=head1 SYNOPSIS
  ...
  use EUCJP qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use EUCJP ver.sion qw(ord reverse getc);
  use EUCJP ver.sion.0 qw(ord reverse getc);
...
10% everything else. Even if this "text" doesn't contain EUC-JP, Perl3 or later
can treat EUC-JP as binary data.
...
=item * Remains One Language Handling Raw EUC-JP, Doesn't Use UTF8 flag like JPerl
...
import of Eeucjp.pm works well.
...
=head1 Multiple-Octet Anchoring of Regular Expression (EUCJP software provides)

EUCJP software applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Multiple-Octet Character Regular Expression (EUCJP software provides)

EUCJP software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Eeucjp::ignorecase()' (EUCJP software provides)

EUCJP software applies calling 'Eeucjp::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Eeucjp::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Eeucjp::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Eeucjp::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Eeucjp::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
Definitions in Eeucjp.pm.
  ...
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Eeucjp::anchor}         qr{\G(?>[^\x8E\x8F\xA1-\xFE]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])*?}
                           for over 32766 octets string on ActivePerl5.6 and Perl5.10 or later
                           qr{\G(?(?=.{0,32766}\z)\G(?>[^\x8E\x8F\xA1-\xFE]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])*?|(?(?=[\x00-\x8D\x90-\xA0\xFF]+\z).*?|(?:.*?[^\x8E\x8F\xA1-\xFE](?>[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\xA1-\xFE])*?)))}oxms
                           for over 32766 octets string on ActivePerl5.6 and Perl5.10 or later
                           qr{\G(?(?=.{0,32766}\z)\G(?>[^\x8E\x8F\xA1-\xFE]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])*?|(?(?=[$sbcs]+\z).*?|(?:.*?[$sbcs](?:[^$sbcs][^$sbcs])*?)))}oxms
  ${Eeucjp::dot}            qr{(?>[^\x8E\x8F\xA1-\xFE\x0A]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::dot_s}          qr{(?>[^\x8E\x8F\xA1-\xFE]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::eD}             qr{(?>[^\x8E\x8F\xA1-\xFE0-9]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::eS}             qr{(?>[^\x8E\x8F\xA1-\xFE\s]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::eW}             qr{(?>[^\x8E\x8F\xA1-\xFE0-9A-Z_a-z]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::eH}             qr{(?>[^\x8E\x8F\xA1-\xFE\x09\x20]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::eV}             qr{(?>[^\x8E\x8F\xA1-\xFE\x0A\x0B\x0C\x0D]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::eR}             qr{(?>\x0D\x0A|[\x0A\x0D])};
  ${Eeucjp::eN}             qr{(?>[^\x8E\x8F\xA1-\xFE\x0A]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_alnum}      qr{(?>[^\x8E\x8F\xA1-\xFE\x30-\x39\x41-\x5A\x61-\x7A]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_alpha}      qr{(?>[^\x8E\x8F\xA1-\xFE\x41-\x5A\x61-\x7A]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_ascii}      qr{(?>[^\x8E\x8F\xA1-\xFE\x00-\x7F]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_blank}      qr{(?>[^\x8E\x8F\xA1-\xFE\x09\x20]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_cntrl}      qr{(?>[^\x8E\x8F\xA1-\xFE\x00-\x1F\x7F]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_digit}      qr{(?>[^\x8E\x8F\xA1-\xFE\x30-\x39]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_graph}      qr{(?>[^\x8E\x8F\xA1-\xFE\x21-\x7F]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_lower}      qr{(?>[^\x8E\x8F\xA1-\xFE\x61-\x7A]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_lower_i}    qr{(?>[^\x8E\x8F\xA1-\xFE\x41-\x5A\x61-\x7A]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])}; # Perl 5.16 compatible
# ${Eeucjp::not_lower_i}    qr{(?>[^\x8E\x8F\xA1-\xFE]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};                   # older Perl compatible
  ${Eeucjp::not_print}      qr{(?>[^\x8E\x8F\xA1-\xFE\x20-\x7F]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_punct}      qr{(?>[^\x8E\x8F\xA1-\xFE\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_space}      qr{(?>[^\x8E\x8F\xA1-\xFE\s\x0B]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_upper}      qr{(?>[^\x8E\x8F\xA1-\xFE\x41-\x5A]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_upper_i}    qr{(?>[^\x8E\x8F\xA1-\xFE\x41-\x5A\x61-\x7A]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])}; # Perl 5.16 compatible
# ${Eeucjp::not_upper_i}    qr{(?>[^\x8E\x8F\xA1-\xFE]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};                   # older Perl compatible
  ${Eeucjp::not_word}       qr{(?>[^\x8E\x8F\xA1-\xFE\x30-\x39\x41-\x5A\x5F\x61-\x7A]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
  ${Eeucjp::not_xdigit}     qr{(?>[^\x8E\x8F\xA1-\xFE\x30-\x39\x41-\x46\x61-\x66]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (EUCJP software provides)

EUCJP software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (EUCJP software provides)
...
=item * Ordinal Value of Character
  ...
  This subroutine returns the numeric value (ASCII or EUC-JP character) of the
...
=item * Returns Next Character
  ...
  If you import getc "use EUCJP qw(getc);", getc of your script will be rewritten
  in EUCJP::getc. EUCJP::getc is not compatible with getc of JPerl.
...
=item * Length by EUC-JP Character
  ...
  of the scalar value $string. If $string is omitted, it returns the EUCJP::length
  of $_.
...
=item * Substr by EUC-JP Character
  ...
  If Perl version 5.14 or later, you can use the EUCJP::substr() subroutine as an
...
=item * Rindex by EUC-JP Character
  ...
  This subroutine works just like EUCJP::index except that it returns the character
...
=item * Eval EUC-JP Script

  $eval = EUCJP::eval { block };
  $eval = EUCJP::eval $expr;
  $eval = EUCJP::eval;

  The EUCJP::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, EUCJP::eval { block }
  and EUCJP::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, EUCJP::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of EUCJP::eval, the value returned from an EUCJP::eval is
  ...
  EUCJP::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  EUCJP::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      EUCJP::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The EUCJP::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow EUCJP::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  EUCJP::eval cannot trap syntax errors.

  The EUCJP::eval $expr form can trap syntax errors because it parses the code
  ...
  after the EUCJP::eval is complete, as do any subroutine or format definitions.
  The code of the EUCJP::eval is treated as a block, so any locally scoped
  variables declared within the EUCJP::eval last only until the EUCJP::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  EUCJP::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  EUCJP::eval { block }    eval { block }
  EUCJP::eval $expr        eval EUCJP::escape $expr
  EUCJP::eval              eval EUCJP::escape
  -------------------------------------------------

  To tell the truth, the subroutine EUCJP::eval does not exist. If it exists,
  you will troubled, when EUCJP::eval has a parameter that is single quoted
  string included my variables. EUCJP::escape is a subroutine that makes Perl
  ...
  Since EUCJP::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because EUCJP::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  EUCJP::eval $expr and EUCJP::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      EUCJP::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      EUCJP::eval { $answer = }; # WRONG

      # a runtime syntax error
      EUCJP::eval '$answer =';   # sets $@
  ...
  Using the EUCJP::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      EUCJP::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  With an EUCJP::eval, you should be especially careful to remember what's being
  looked at when:
  ...
  The block of EUCJP::eval { block } does not count as a loop, so the loop
...
=item * Filename Globbing
  ...
  This operator is implemented via the Eeucjp::glob() subroutine. See Eeucjp::glob
  of Eeucjp.pm for details.
...
=head1 Yada Yada Operator (EUCJP software provides)
...
=head1 Un-Escaping bytes::* Subroutines (EUCJP software provides)

EUCJP software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in EUCJP software and
...
If Perl version is older than 5.14, EUCJP::substr differs from CORE::substr, and
...
EUCJP software remains one language and one interpreter.
