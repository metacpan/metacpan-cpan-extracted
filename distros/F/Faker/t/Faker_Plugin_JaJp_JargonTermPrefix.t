...
Faker::Plugin::JaJp::JargonTermPrefix
...
$test->for('synopsis', sub {
  ...
  ok $result->isa('Faker::Plugin::JaJp::JargonTermPrefix');
...
$test->for('example', 1, 'execute', sub {
  ...
  is $result->execute, 'エグゼクティブ';
  ok $result->faker->random->pick; # reset randomizer
  is $result->execute, '明示的';
  ok $result->faker->random->pick; # reset randomizer
  is $result->execute, 'フルレンジ';
...
$test->render('lib/Faker/Plugin/JaJp/JargonTermPrefix.pod') if $ENV{RENDER};
