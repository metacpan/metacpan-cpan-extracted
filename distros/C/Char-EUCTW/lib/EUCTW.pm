...
# regexp of character
my $qq_char   = qr/(?> \\c[\x40-\x5F] | \\? (?:[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF] | [\x00-\xFF]) )/oxms;
my  $q_char   = qr/(?> [\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF] | [\x00-\xFF] )/oxms;
...
perl $0 EUC-TW_script.pl > Escaped_script.pl.e
END
    }

    print EUCTW::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Eeuctw::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
    ...
    my $fh = gensym();
    Eeuctw::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub EUCTW::escape_script {
    ...
    # read EUC-TW script
    my $fh = gensym();
    Eeuctw::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Eeuctw(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Eeuctw '%s.0'; # 'quote' for perl5.6\n", $EUCTW::VERSION); # require run-time routines version

        # use EUCTW version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | EUCTW ) (?>\s*) ([^\x8E\xA1-\xFE;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $EUCTW::VERSION) {
                    my @file = grep -e, map {qq{$_/EUCTW.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects EUCTW.pm $version, but @{[__FILE__]} is version $EUCTW::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Eeuctw.pm $version, but \\\$Eeuctw::VERSION is \$Eeuctw::VERSION" if \$Eeuctw::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $EUCTW::VERSION) {
                    die "Script $0 required EUCTW.pm $version, but @{[__FILE__]} is only version $EUCTW::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Eeuctw::function_ord     = 'EUCTW::ord'     if /\A ord \z/oxms;
                    $Eeuctw::function_ord_    = 'EUCTW::ord_'    if /\A ord \z/oxms;
                    $Eeuctw::function_reverse = 'EUCTW::reverse' if /\A reverse \z/oxms;
                    $Eeuctw::function_getc    = 'EUCTW::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= EUCTW::escape();
...
=item * You'll have to download EUCTW.pm and Eeuctw.pm and put it in your perl lib directory.

=item * You'll need to write "use EUCTW;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use EUCTW;
  use EUCTW ver.sion;             --- require minimum version
  use EUCTW ver.sion.0;           --- expects version (match or die)
  use EUCTW qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use EUCTW ver.sion qw(ord reverse getc);
  use EUCTW ver.sion.0 qw(ord reverse getc);

  # "no EUCTW;" not supported

  or

  $ perl EUCTW.pm EUC-TW_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  EUC-TW_script.pl  --- script written in EUC-TW
...
EUCTW software is "middleware" between perl interpreter and your Perl script
written in EUC-TW.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain EUC-TW, Perl3 or later
can treat EUC-TW as binary data.

By "use EUCTW;", it automatically interpret your script as EUC-TW. The various
functions of perl including a regular expression can treat EUC-TW now.
...
This software is a source code filter to escape Perl script encoded by EUC-TW
...
=item * Remains One Language Handling Raw EUC-TW, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. EUCTW.pm, Eeuctw.pm
  +---------------------------------------+
...
import of Eeuctw.pm works well.
...
=head1 Software Composition
   ...
   EUCTW.pm               --- source code filter to escape EUC-TW
   Eeuctw.pm              --- run-time routines for EUCTW.pm
...
You need write 'use EUCTW;' in your script.
...
=head1 Multiple-Octet Anchoring of Regular Expression (EUCTW software provides)

EUCTW software applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Eeuctw::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Multiple-Octet Character Regular Expression (EUCTW software provides)

EUCTW software clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Eeuctw::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Eeuctw::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Eeuctw::ignorecase()' (EUCTW software provides)

EUCTW software applies calling 'Eeuctw::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Eeuctw::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Eeuctw::matched/
  m/regexp/               m/ditto$Eeuctw::matched/
  ?regexp?                m?ditto$Eeuctw::matched?
  m?regexp?               m?ditto$Eeuctw::matched?
 
  $_ =~                   ($_ =~ m/ditto$Eeuctw::matched/) ?
  s/regexp/replacement/   CORE::eval{ Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Eeuctw::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Eeuctw::split(qr/regexp/)
  split(m/regexp/)        Eeuctw::split(qr/regexp/)
  split(qr/regexp/)       Eeuctw::split(qr/regexp/)
  qr/regexp/              qr/ditto$Eeuctw::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Eeuctw::matched/
  m/regexp/b              m/(?:regexp)$Eeuctw::matched/
  ?regexp?b               m?regexp$Eeuctw::matched?
  m?regexp?b              m?regexp$Eeuctw::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Eeuctw::matched/) ?
  s/regexp/replacement/b  CORE::eval{ Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Eeuctw::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Eeuctw.pm provides)
...
Definitions in Eeuctw.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Eeuctw::anchor}         qr{\G(?>[^\x8E\xA1-\xFE]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])*?}
                           for over 32766 octets string on ActivePerl5.6 and Perl5.10 or later
                           qr{\G(?(?=.{0,32766}\z)\G(?>[^\x8E\xA1-\xFE]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])*?|(?(?=[\x00-\x8D\x8F-\xA0\xFF]+\z).*?|(?:.*?[^\x8E\xA1-\xFE](?>[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE])*?)))}oxms
                           for over 32766 octets string on ActivePerl5.6 and Perl5.10 or later
                           qr{\G(?(?=.{0,32766}\z)\G(?>[^\x8E\xA1-\xFE]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])*?|(?(?=[$sbcs]+\z).*?|(?:.*?[$sbcs](?:[^$sbcs][^$sbcs])*?)))}oxms
  ${Eeuctw::dot}            qr{(?>[^\x8E\xA1-\xFE\x0A]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::dot_s}          qr{(?>[^\x8E\xA1-\xFE]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::eD}             qr{(?>[^\x8E\xA1-\xFE0-9]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::eS}             qr{(?>[^\x8E\xA1-\xFE\s]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::eW}             qr{(?>[^\x8E\xA1-\xFE0-9A-Z_a-z]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::eH}             qr{(?>[^\x8E\xA1-\xFE\x09\x20]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::eV}             qr{(?>[^\x8E\xA1-\xFE\x0A\x0B\x0C\x0D]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::eR}             qr{(?>\x0D\x0A|[\x0A\x0D])};
  ${Eeuctw::eN}             qr{(?>[^\x8E\xA1-\xFE\x0A]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_alnum}      qr{(?>[^\x8E\xA1-\xFE\x30-\x39\x41-\x5A\x61-\x7A]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_alpha}      qr{(?>[^\x8E\xA1-\xFE\x41-\x5A\x61-\x7A]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_ascii}      qr{(?>[^\x8E\xA1-\xFE\x00-\x7F]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_blank}      qr{(?>[^\x8E\xA1-\xFE\x09\x20]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_cntrl}      qr{(?>[^\x8E\xA1-\xFE\x00-\x1F\x7F]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_digit}      qr{(?>[^\x8E\xA1-\xFE\x30-\x39]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_graph}      qr{(?>[^\x8E\xA1-\xFE\x21-\x7F]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_lower}      qr{(?>[^\x8E\xA1-\xFE\x61-\x7A]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_lower_i}    qr{(?>[^\x8E\xA1-\xFE\x41-\x5A\x61-\x7A]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])}; # Perl 5.16 compatible
# ${Eeuctw::not_lower_i}    qr{(?>[^\x8E\xA1-\xFE]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};                   # older Perl compatible
  ${Eeuctw::not_print}      qr{(?>[^\x8E\xA1-\xFE\x20-\x7F]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_punct}      qr{(?>[^\x8E\xA1-\xFE\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_space}      qr{(?>[^\x8E\xA1-\xFE\s\x0B]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_upper}      qr{(?>[^\x8E\xA1-\xFE\x41-\x5A]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_upper_i}    qr{(?>[^\x8E\xA1-\xFE\x41-\x5A\x61-\x7A]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])}; # Perl 5.16 compatible
# ${Eeuctw::not_upper_i}    qr{(?>[^\x8E\xA1-\xFE]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};                   # older Perl compatible
  ${Eeuctw::not_word}       qr{(?>[^\x8E\xA1-\xFE\x30-\x39\x41-\x5A\x5F\x61-\x7A]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
  ${Eeuctw::not_xdigit}     qr{(?>[^\x8E\xA1-\xFE\x30-\x39\x41-\x46\x61-\x66]|[\xA1-\xFE][\xA1-\xFE]|\x8E[\xA1-\xB0][\xA1-\xFE][\x00-\xFF])};
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (EUCTW software provides)

EUCTW software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (EUCTW software provides)

Insert 'Eeuctw::' at head of function name. Eeuctw.pm provides your script Eeuctw::*
subroutines.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Eeuctw::split     Character
  tr///       Eeuctw::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Eeuctw::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Eeuctw::chop      Character
  index       Eeuctw::index     Character
  rindex      Eeuctw::rindex    Character
  lc          Eeuctw::lc        Character
  lcfirst     Eeuctw::lcfirst   Character
  uc          Eeuctw::uc        Character
  ucfirst     Eeuctw::ucfirst   Character
  fc          Eeuctw::fc        Character
  chr         Eeuctw::chr       Character
  glob        Eeuctw::glob      Character
  -------------------------------------------
...
You need write 'EUCTW::' at head of function name when you want character-
oriented subroutine. See 'Character-Oriented Subroutines'.
  ...
  --------------------------------------------------------
  ord        EUCTW::ord
  reverse    EUCTW::reverse
  getc       EUCTW::getc
  length     EUCTW::length
  substr     EUCTW::substr
  index      EUCTW::index          See 'About Indexes'
  rindex     EUCTW::rindex         See 'About Rindexes'
  eval       EUCTW::eval
  --------------------------------------------------------
  ...
  (same as Eeuctw::index)
  EUCTW::index    Character   Character    Character-oriented semantics
  ...
  (same as Eeuctw::rindex)
  EUCTW::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  $ord = EUCTW::ord($string);

  This subroutine returns the numeric value (ASCII or EUC-TW character) of the
  first character of $string, not Unicode. If $string is omitted, it uses $_.
  The return value is always unsigned.

  If you import ord "use EUCTW qw(ord);", ord of your script will be rewritten in
  EUCTW::ord. EUCTW::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = EUCTW::reverse(@list);
  $reverse = EUCTW::reverse(@list);
  ...
  If you import reverse "use EUCTW qw(reverse);", reverse of your script will be
  rewritten in EUCTW::reverse. EUCTW::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  If you import getc "use EUCTW qw(getc);", getc of your script will be rewritten
  in EUCTW::getc. EUCTW::getc is not compatible with getc of JPerl.

=item * Length by EUC-TW Character

  $length = EUCTW::length($string);
  $length = EUCTW::length();

  This subroutine returns the length in characters (programmer-visible characters)
  of the scalar value $string. If $string is omitted, it returns the EUCTW::length
  of $_.

  Do not try to use EUCTW::length to find the size of an array or hash. Use scalar
...
=item * Substr by EUC-TW Character

  $substr = EUCTW::substr($string,$offset,$length,$replacement);
  $substr = EUCTW::substr($string,$offset,$length);
  $substr = EUCTW::substr($string,$offset);
  ...
  my $color  = EUCTW::substr $s, 4, 5;      # black
  my $middle = EUCTW::substr $s, 4, -11;    # black cat climbed the
  my $end    = EUCTW::substr $s, 14;        # climbed the green tree
  my $tail   = EUCTW::substr $s, -4;        # tree
  my $z      = EUCTW::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the EUCTW::substr() subroutine as an
  ...
  end of the string, EUCTW::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  EUCTW::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = EUCTW::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = EUCTW::substr $name, 7;      # returns undef, with warning
  EUCTW::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using EUCTW::substr() as an lvalue is to specify the replacement
  ...
  Note that the lvalue returned by the three-argument version of EUCTW::substr() acts
...
=item * Index by EUC-TW Character

  $index = EUCTW::index($string,$substring,$offset);
  $index = EUCTW::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = EUCTW::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }

=item * Rindex by EUC-TW Character

  $rindex = EUCTW::rindex($string,$substring,$offset);
  $rindex = EUCTW::rindex($string,$substring);

  This subroutine works just like EUCTW::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse EUCTW::index).
  ...
  $pos = EUCTW::length($string);
  while (($pos = EUCTW::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }

=item * Eval EUC-TW Script

  $eval = EUCTW::eval { block };
  $eval = EUCTW::eval $expr;
  $eval = EUCTW::eval;

  The EUCTW::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, EUCTW::eval { block }
  and EUCTW::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, EUCTW::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of EUCTW::eval, the value returned from an EUCTW::eval is
  ...
  EUCTW::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  EUCTW::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      EUCTW::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The EUCTW::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow EUCTW::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  EUCTW::eval cannot trap syntax errors.

  The EUCTW::eval $expr form can trap syntax errors because it parses the code
  ...
  after the EUCTW::eval is complete, as do any subroutine or format definitions.
  The code of the EUCTW::eval is treated as a block, so any locally scoped
  variables declared within the EUCTW::eval last only until the EUCTW::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  EUCTW::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  EUCTW::eval { block }    eval { block }
  EUCTW::eval $expr        eval EUCTW::escape $expr
  EUCTW::eval              eval EUCTW::escape
  -------------------------------------------------

  To tell the truth, the subroutine EUCTW::eval does not exist. If it exists,
  you will troubled, when EUCTW::eval has a parameter that is single quoted
  string included my variables. EUCTW::escape is a subroutine that makes Perl
  ...
  Since EUCTW::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because EUCTW::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  EUCTW::eval $expr and EUCTW::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      EUCTW::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      EUCTW::eval { $answer = }; # WRONG

      # a runtime syntax error
      EUCTW::eval '$answer =';   # sets $@
  ...
  Using the EUCTW::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      EUCTW::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  With an EUCTW::eval, you should be especially careful to remember what's being
  looked at when:
  ...
  The block of EUCTW::eval { block } does not count as a loop, so the loop
...
=item * Filename Globbing
  ...
  This operator is implemented via the Eeuctw::glob() subroutine. See Eeuctw::glob
  of Eeuctw.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use EUCTW qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use EUCTW qw(reverse);" exists or not.
...
=head1 Yada Yada Operator (EUCTW software provides)
...
=head1 Un-Escaping bytes::* Subroutines (EUCTW software provides)

EUCTW software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Eeuctw.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in EUCTW software and
...
=item * EUCTW::substr as Lvalue

If Perl version is older than 5.14, EUCTW::substr differs from CORE::substr, and
...
EUCTW::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Eeuctw::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${`}            Eeuctw::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Eeuctw::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Eeuctw::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Eeuctw::MATCH()       $1
  ${&}            Eeuctw::MATCH()       $1
  $MATCH          Eeuctw::MATCH()       $1
  ${^MATCH}       Eeuctw::MATCH()       $1
  $'              $'                   $'
  ${'}            ${'}                 $'
  $POSTMATCH      Eeuctw::POSTMATCH()   $'
  ${^POSTMATCH}   Eeuctw::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
EUCTW software remains one language and one interpreter.
