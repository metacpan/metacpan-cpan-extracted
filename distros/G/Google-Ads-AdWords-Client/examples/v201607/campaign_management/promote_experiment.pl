...
use Google::Ads::AdWords::v201607::Experiment;
use Google::Ads::AdWords::v201607::ExperimentOperation;
...
sub promote_experiment {
  ...
  my $experiment = Google::Ads::AdWords::v201607::Experiment->new({
      id     => $experiment_id,
      status => "PROMOTED"
  });
  ...
  my $experiment_operation =
    Google::Ads::AdWords::v201607::ExperimentOperation->new({
      operator => "SET",
      operand  => $experiment
    });
  ...
  # Update experiment.
  my $result =
    $client->ExperimentService()
    ->mutate({operations => [$experiment_operation]});
