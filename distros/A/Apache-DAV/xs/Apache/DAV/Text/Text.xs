...
void Apache__DAV__Text_new_init (pTHX_ Apache__DAV__Text  obj, SV * item, int overwrite) {
    ...
    if (SvTYPE(item) == SVt_PVMG) 
        ...
        if ((tmpsv = hv_fetch((HV *)item, "text", sizeof("text") - 1, 0)) || overwrite) {
            const char * tmpobj = ((const char *)davxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef)));
            if (tmpobj)
                obj -> text = (const char *)strdup(tmpobj);
            else
                obj -> text = NULL ;
        }
        if ((tmpsv = hv_fetch((HV *)item, "next", sizeof("next") - 1, 0)) || overwrite) {
            obj -> next = (struct dav_text *)davxs_sv2_Apache__DAV__Text((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef)) ;
        }
   ; }

    else
        croak ("initializer for Apache::DAV::Text::new is not a hash or object reference") ;
...
PREINIT:
    SV * svobj ;
    Apache__DAV__Text  cobj ;
    SV * tmpsv ;
CODE:
    davxs_Apache__DAV__Text_create_obj(cobj,svobj,RETVAL,malloc(sizeof(*cobj))) ;

    if (initializer) {
        if (!SvROK(initializer) || !(tmpsv = SvRV(initializer))) 
            croak ("initializer for Apache::DAV::Text::new is not a reference") ;

        if (SvTYPE(tmpsv) == SVt_PVHV || SvTYPE(tmpsv) == SVt_PVMG)  
            Apache__DAV__Text_new_init (aTHX_ cobj, tmpsv, 0) ;
