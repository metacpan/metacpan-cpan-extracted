...
Problems on X11:
  ...
  http://www.x.org/releases/X11R7.7/doc/kbproto/xkbproto.html			(definition of XKB???)
...
#warn "Massaging face `$f'...";
    for my $key ( qw( Flip_AltGr_Key Diacritic_if_undef DeadChar_DefaultTranslation DeadChar_32bitTranslation extra_report_DeadChar
		      ...
		      layers_mods_keys mods_keys_KBD AltGrInv_AltGr_as_Ctrl
...
# Dumpvalue->new()->dumpValue($data);
    ...
    for my $F (keys %{ $data->{faces} }) {	# Fine-tune inverted-AltGr faces
      next if 'HASH' ne ref $data->{faces}{$F} or $F =~ /\bVK$/;			# "parent" taking keys for a child
      next if $F =~ /#\@?#\@?(Inv)?#\@?/;		# Face-on-a-deadkey

      my $D = $data->{faces}{$F}{'[deadkeyFace]'};
      my $Ex = $data->{faces}{$F}{'[AltGr_Invert_Show]'};
      for my $d (keys %$D) {
        $data->{faces}{$F}{'[deadkeyFaceHexMap]'}{$d} = $data->linked_faces_2_hex_map($F, $D->{$d});
        defined (my $auto_inv_AltGr = $data->{faces}{$F}{'[deadkeyInvAltGrKey]'}{$d}) or next;
	my $b1 = $data->{faces}{$F}{'[deadkeyFaceInvAltGr]'}{my $a = $data->charhex2key($auto_inv_AltGr)};
        $data->{faces}{$F}{'[deadkeyFaceHexMapInv]'}{$d} = $data->linked_faces_2_hex_map($F, $b1) if $b1;
        my $D = $data->{faces}{$F}{'[prefixDocs]'}{$d};
        $data->{faces}{$F}{'[prefixDocs]'}{$data->key2hex($a)} = 'AltGr-inverted: ' . (defined $D ? $D : "[[$d]]");
        my $S = $data->{faces}{$F}{'[Show]'}{$d};
        $data->{faces}{$F}{'[Show]'}{$data->key2hex($a)} = (defined $S ? $S : $data->charhex2key($d)) . $Ex;
      }

      my($flip_AltGr, @protect_chr) = $data->{faces}{$F}{'[Flip_AltGr_Key]'};	# Who put it into deadkeyFace???
      if (defined $flip_AltGr) {
        $flip_AltGr = $data->key2hex($data->charhex2key($flip_AltGr));
        push @protect_chr, $flip_AltGr;
        $data->{faces}{$F}{'[prefixDocs]'}{$flip_AltGr} = 'AltGr-inverted base face'
          unless defined $data->{faces}{$F}{'[prefixDocs]'}{$flip_AltGr};
        $data->{faces}{$F}{'[Show]'}{$flip_AltGr} = $Ex unless defined $data->{faces}{$F}{'[Show]'}{$flip_AltGr};
      }
      my $expl = $data->{faces}{$F}{'[Explicit_AltGr_Invert]'} || [];
      for my $i (1..(@$expl/2)) {
        my @C = map $data->key2hex($expl->[2*$i + $_]), -2, -1;
        push @protect_chr, $C[1];
        my $D = $data->{faces}{$F}{'[prefixDocs]'}{$C[0]};
        $data->{faces}{$F}{'[prefixDocs]'}{$C[1]} = 'AltGr-inverted: ' . (defined $D ? $D : "[[$C[0]]]");
        my $S = $data->{faces}{$F}{'[Show]'}{$C[0]};
        $data->{faces}{$F}{'[Show]'}{$C[1]} = (defined $S ? $S : $data->charhex2key($C[0])) . $Ex;
      }
      $data->{faces}{$F}{'[auto_dead]'}{ord $data->charhex2key($_)}++ for @protect_chr;
#      warn "   Keys HexMap: ", join ', ', sort keys %{$data->{faces}{$F}{'[deadkeyFaceHexMap]'}};
    ...
    for my $F (keys %{ $data->{faces} }) {	# Finally, collect the stats
      ...
      my %is_extra = map { ($data->charhex2key($_), 1) } @extras;	# extra layers (on bizarre modifiers)
      for my $deadKEY ( sort keys %{ $data->{faces}{$F}{'[deadkeyFace]'}} ) {
        ...
        my $cov1 = $data->{faces}{$FFF}{'[coverage0]'}	# XXXX not layer0coverage0 - may slide down to layer0
          or warn("Deadkey `$deadKey' on face `$F' -> unmassaged face"), next;
        $seenExtra{$_}++
          for map {ref() ? $_->[0] : $_} grep !(ref and $_->[2]), @$cov1;	# Skip 2nd level deadkeys
      }
      $data->{faces}{$F}{'[coverageExtra]'} = \%seenExtra;
    ...
    for my $F (keys %{ $data->{faces} }) {	# Finally, collect the stats
      next if 'HASH' ne ref $data->{faces}{$F} or $F =~ /\bVK$/;			# "parent" taking keys for a child
      next if $F =~ /#\@?#\@?(Inv)?#\@?/;		# Face-on-a-deadkey
      my($seen_prefix, %seen0, %seen00, %seen1, %seen1only, %seenExtra) = $data->{faces}{$F}{'[coverage0_prefix]'};
...
# warn 'Key: <', join('> <', @C), '>';
  ...
  if (@extra) {
    ...
    for my $l (0 .. $#{ $self->{faces}{$f}{layers} } ) {	# Assume that in every layer a few positions after end of the 
      ...
      my $add = $start_append + @{ $extra[$l] } - @L;
      $L[$start_append+$_] = [] for 0..$add-1;		# Avoid splicing after the end of array
...
{ my(%seen, %seen_scan, %seen_VK, @add_scan_VK, @ligatures, @decimal);
  ...
  sub output_unit00 ($$$$$$$;$$) {
    ...
    my @KK = map [$_->[2], $_->[0],	# 0:layer#, 1:shift#, 2:char, 3:deadkeys(layer), 4:char_array ==> 0:char, 1:layer, 2:dead
    ...
    ($_->[2] || 0) >= 3 and $_->[0] = $self->dead_with_inversion(!'hex', $_, $face, $self->{faces}{$face}) for @KK;
#($KK[$_][2] || 0) >= 3 and warn "face=$face N=$N c=$_ <$KK[$_][0]> --> $KK[$_][2]\n" for 0..$#KK;
#my @UUU = map $U->[$_->[0]][$_->[1]], map +([$_, 0], [$_, 1]), 0..$#$u;
#ref $UUU[$_] and ($UUU[$_][2] || 0) >= 3 and warn "face=$face N=$N cc=$_ <$UUU[$_][0]> --> $UUU[$_][2]\n" for 0..$#UUU;
...
sub layers_by_face_recipe ($$$;$) {
  ...
  die "No face recipe for `$face' found" unless my $R = $self->{face_recipes}{$face} or defined $r;
  $r = $R if $R;
  $r = $self->recipe2str($r);
...
#warn "Translation sub-spec: recipe = <$recipe>, ARG=<$ARG>";
  if ($recipe =~ /^If(Not)?Prefix\[(.*)\]$/s) {	# No embedded \\]
    my $neg = $1;
    my @prefix = map $self->charhex2key($_), split /,/, "$2";
    return $self->make_translated_layers($ARG, $face, $NN, $deadkey, $noid) if $neg xor grep $_ eq $deadkey, @prefix;
...
#  warn "Basekeys: max=$max; cnt=$cnt";
  my(@o, @oo);
  for my $u (0..$max) {
    my $c = $self->base_unit($basesub, $u, $u >= $cnt);	# [0 || 1, VK]
...
# Dumpvalue->new()->dumpValue($self);
  ...
  my $file = $self->{'[file]'};
  $file = (defined $file) ? "keyboard layout file $file" : 'string descriptor';
  $file .= " version $v" if defined $v;
  $file .= " Unicode tables version $self->{uniVersion}" if defined $self->{uniVersion};
...
sub AppleMap_Base ($$) {
  ...
  my $o = $F->{'[VK_off]'};
  for my $b ($B, $BB) {			# Explicitly add via-VK keys
    for my $vk (keys %$o) {
