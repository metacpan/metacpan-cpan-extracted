package Prty::Database::Row::Object;
use base qw/Prty::Database::Row/;
...
Prty::Database::Row::Object - Datensatz als Objekt
...
our $TableClass = 'Prty::Database::ResultSet::Object';
...
sub new {
    ...
    if (ref $_[0]) {
        if (Scalar::Util::blessed($_[0]) &&
                $_[0]->isa('Prty::Database::Connection')) {
    ...
    my $hash = Prty::Hash->new($titleA,$valueA? $valueA: (),'')
        ->unlockKeys;
...
sub getSet {
    ...
    if (@_) {
        ...
        if ($val ne $oldVal) {
            ...
            unless ($self->[3]) {
                $self->[3] = Prty::Hash->new->unlockKeys;
            }
...
sub addChildType {
    ...
    return $self->[4]->{$type} =
        Prty::Database::ResultSet->new($rowClass,$titleA);
}
...
Basisklasse Prty::Database::Row. Die Methoden hat die Klasse mit
der Klasse Prty::Database::Row::Array gemeinsam.
...
sub copyData {
    ...
    if ($ignore) {
        $ignore = Prty::Hash->new($ignore,1)->unlockKeys;
    }
