...
subtest 'pure-Perl backend is reproducible for a fixed seed' => sub {
    for my $mode (qw(axis extended)) {
        my %opts = (
            n_trees => 25, sample_size => 40, seed => 31,
            mode    => $mode, use_c => 0,
        );
        is( fit_trees(%opts), fit_trees(%opts),
            "[$mode] two pure-Perl fits with the same seed match" );
    }
};
...
subtest 'serial-C backend is reproducible for a fixed seed' => sub {
    ...
    for my $mode (qw(axis extended)) {
        my %opts = (
            n_trees => 25, sample_size => 40, seed => 32,
            mode    => $mode, use_c => 1,
        );
        is( fit_trees(%opts), fit_trees(%opts),
            "[$mode] two serial-C fits with the same seed match" );
    }
};
...
# ------------------------------------------------------------------------
subtest 'pure-Perl and serial-C build identical trees for the same seed' =>
    sub {
    plan skip_all => 'no Inline::C backend compiled in' unless $HAS_C;
    ...
    for my $missing (qw(zero impute nan)) {
        my %opts = (
            n_trees => 20, sample_size => 40, seed => 34,
            missing => $missing, data => \@data_nan,
        );
        is(
            fit_trees( %opts, use_c => 1 ),
            fit_trees( %opts, use_c => 0 ),
...
subtest 'use_openmp_fit is reproducible in-process for a fixed seed' => sub {
    ...
    for my $mode (qw(axis extended)) {
        my %opts = (
            n_trees => 40, sample_size => 48, seed => 41,
            mode    => $mode, use_c => 1, use_openmp_fit => 1,
        );
        is( fit_trees(%opts), fit_trees(%opts),
            "[$mode] two use_openmp_fit fits with the same seed match" );
    }
};

subtest 'use_openmp_fit trees are valid, but not required to match Drand01 backends'
    => sub {
    plan skip_all => 'OpenMP not linked in' unless $HAS_C && $HAS_OPENMP;
    ...
    my $f = $CLASS->new(
        n_trees => 40, sample_size => 48, seed => 42,
        mode    => 'extended', use_c => 1, use_openmp_fit => 1,
    )->fit( \@data );
...
# ------------------------------------------------------------------------
subtest 'use_openmp_fit reproducibility does not depend on OMP_NUM_THREADS' =>
    sub {
    plan skip_all => 'OpenMP not linked in' unless $HAS_C && $HAS_OPENMP;
...
END_CHILD
    ...
    is( $out1, $out4,
        'same seed + n_trees builds identical trees under OMP_NUM_THREADS=1 vs 4'
    );
    };
...
# ordering.
# ------------------------------------------------------------------------
subtest 'use_openmp_fit + parallel_fit does not hang and is reproducible'
    => sub {
    ...
    is( $err2, '', 'second parallel_fit + use_openmp_fit run does not hang' )
        or diag("error: $err2");
    is( $r1, $r2,
        'two parallel_fit + use_openmp_fit runs with the same seed and '
            . 'worker count match' );
    };
