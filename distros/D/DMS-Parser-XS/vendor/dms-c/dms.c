...
typedef struct {
    ...
    int unordered;
    /* Tier-1 mode: allow _dms_tier = 1 and _dms_imports in FM.
       Set only by dms_decode_front_matter_t1(). Regular decode paths
       keep this 0 and continue to reject those reserved keys. */
    int tier1_mode;
} parser;
        ...
        for (size_t idx = 0; idx < tbl_val->u.t.len; idx++) {
            ...
            if (k[0] == '_') {
                if (strcmp(k, "_dms_tier") == 0) {
                    ...
                    if (p->tier1_mode) {
                        /* Tier-1 decode path: accept 0 or 1, transfer to meta. */
                        if (v->u.i < 0 || v->u.i > 1) {
                            char msg[192];
                            snprintf(msg, sizeof msg,
                                "_dms_tier: %lld is not supported",
                                (long long)v->u.i);
                        ...
                        /* Tier-0 decode path: only _dms_tier = 0 is accepted. */
                        if (v->u.i < 0) {
                ...
                } else if (strcmp(k, "_dms_imports") == 0) {
                    if (p->tier1_mode) {
                        /* Transfer _dms_imports to meta for tier-1 encoder. */
                        if (meta->len == meta->cap) {
                            size_t nc = meta->cap ? meta->cap * 2 : 8;
                            meta->items = (dms_kv *)xrealloc(meta->items, nc * sizeof(dms_kv));
                            meta->cap = nc;
