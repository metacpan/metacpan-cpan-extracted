...
sub build {
    ...
    my $analysis = Developer::Dashboard::Pax::StandaloneAnalysis->new;
...
sub _launcher_source {
    ...
    my $bootstrap_code = _c_string('use Developer::Dashboard::Pax::StandaloneRuntime; my $entry = shift @ARGV; my @args = @ARGV; Developer::Dashboard::Pax::StandaloneRuntime->run(entrypoint => $entry, argv => \@args);');
    return <<"C";
#define _GNU_SOURCE
#include <errno.h>
#include <stdio.h>
...
int main(int argc, char **argv) {
    ...
    if (!next) return 111;
    next[0] = perl_exec;
    next[1] = "-MDeveloper::Dashboard::Pax::StandaloneRuntime";
    next[2] = "-e";
...
sub _pax_runtime_helper_payloads {
    ...
    for my $rel (@helpers) {
        ...
        if ($rel eq 'Developer/Dashboard/Pax/StandaloneRuntime.pm' && $app_namespace) {
...
sub _runtime_inc_dirs {
    ...
    for my $dir (@INC) {
        ...
        next if $exclude{$abs};
        next if $abs =~ m{\A\Q$PAX_OWN_LIB_ROOT\E(?:/|\z)};
        next if $seen{$abs}++;
        push @dirs, $abs;
    }
    return @dirs;
...
sub _pax_runtime_helper_relative_paths {
    return qw(
        Developer/Dashboard/Pax/StandaloneRuntime.pm
        Developer/Dashboard/Pax/NativeRunner.pm
        Developer/Dashboard/Pax/GuardManager.pm
        Developer/Dashboard/Pax/DeoptEngine.pm
    );
}
...
sub _pax_runtime_helper_lib_roots {
    ...
    my $loaded = $INC{'Developer/Dashboard/Pax/StandaloneImage.pm'} || __FILE__;
    my $abs = abs_path($loaded) || $loaded;
    my @parts = File::Spec->splitdir($abs);
    while (@parts) {
        my $candidate = File::Spec->catdir(@parts);
        if (-f File::Spec->catfile($candidate, 'Developer', 'Dashboard', 'Pax', 'StandaloneImage.pm')) {
    ...
    for my $inc (@INC) {
        ...
        push @roots, $abs_inc if -f File::Spec->catfile($abs_inc, 'Developer', 'Dashboard', 'Pax', 'StandaloneRuntime.pm')
            && !$seen{$abs_inc}++;
    }
...
sub _pax_runtime_helper_modules {
    ...
    for my $rel (@helpers) {
        ...
        while ($source =~ /^\s*use\s+([A-Za-z_][A-Za-z0-9_:]*)\b/gm) {
            my $module = $1;
            next if $module =~ /^Developer::Dashboard::Pax::/;
...
Developer::Dashboard::Pax::StandaloneImage - build standalone PAX executable images
