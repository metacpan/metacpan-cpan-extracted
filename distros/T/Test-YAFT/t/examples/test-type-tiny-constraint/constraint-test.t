...
subtest q (constraint PositiveOrZeroNum) => sub {
	...
	this_constraint q (should accept zero)
		=> value  => 0
		=> expect => expect_true
		;

	this_constraint q (should accept positive number)
		=> value  => 100.885
		=> expect => expect_true
		;

	this_constraint q (should not accept negative value)
		=> value  => -100.885
		=> throws => expect_failed_constraint_exception (q (Must be a number greater than or equal to zero))
		;

	this_constraint q (should accept very small positive number)
		=> value => 0.0000000001
		=> expect => expect_true
		;
};
