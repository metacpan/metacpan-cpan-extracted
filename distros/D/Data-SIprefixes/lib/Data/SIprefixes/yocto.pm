...
Data::SIprefixes::yocto - This provides yocto matching for Data::SIprefixes.
...
=head1 SYNOPSIS
    ...
    use Data::SIprefixes::yocto;
    
    my $prefix=Data::SIprefixes::yocto->new;
...
sub longMatch{
	...
	if ( $measure=~/^yocto/ ){
		...
		if ( $measure =~ /^ / ){
			$self->{error}=2;
			$self->{errorString}='Space found after prefix, /^yocto/, in "'.$origMeasure.'"' ;
			$self->warn;
			return undef;
		}
...
1; # End of Data::SIprefixes::yocto
