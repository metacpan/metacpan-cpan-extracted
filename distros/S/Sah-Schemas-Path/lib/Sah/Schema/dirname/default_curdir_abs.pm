...
Sah::Schema::dirname::default_curdir_abs - Directory name, default to current directory (absolutified)

=head1 VERSION

This document describes version 0.030 of Sah::Schema::dirname::default_curdir_abs (from Perl distribution Sah-Schemas-Path), released on 2024-01-08.
...
=head1 SAH SCHEMA DEFINITION
 ...
 [
   ...
   {
     "summary" => "Directory name, default to current directory (absolutified)",
     "x.perl.default_value_rules" => ["Path::curdir_abs"],
   },
 ]
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("dirname::default_curdir_abs*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("dirname::default_curdir_abs", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("dirname::default_curdir_abs", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("dirname::default_curdir_abs*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"dirname::default_curdir_abs*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('dirname::default_curdir_abs*', name=>'DirnameDefaultCurdirAbs')
     );
 }

 use My::Types qw(DirnameDefaultCurdirAbs);
 DirnameDefaultCurdirAbs->assert_valid($data);
