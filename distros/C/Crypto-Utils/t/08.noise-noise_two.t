...
our $noise_conf = {
    ...
    enc_func => sub {  #encrypt: key, iv, aad, plaintext  -> ciphertext, authtag
        ...
        my $tag_len = 16;
        my $res =
          aead_encrypt( 'aes-256-gcm', $plain, $aad, $key, $iv, $tag_len );
        my $ciphertext = $res->[0];
        my $tag        = $res->[1];
    ...
    dec_func => sub {   #decrypt: key, iv, aad, ciphertext, authtag -> plaintext
        ...
        my $plain =
          aead_decrypt( 'aes-256-gcm', $ciphertext, $aad, $tag, $key, $iv );
...
my @test_psk =
  ( [ undef, undef ], [ 'test_psk', 0 ], [ 'test_psk', 1 ], [ 'test_psk', 2 ] );

for my $pattern_name (qw/NN NK KK IK IX KN NX KX IN NK1/) {
    ...
    for my $psk_r (@test_psk) {
        ...
        my $a_hs = new_handshake_state(
            $noise_conf,
            {
                ...
                s_priv =>
                  read_key_from_pem( $FindBin::Bin . '/noise-a_s_priv.pem' ),
                s_pub =>
                  read_pubkey_from_pem( $FindBin::Bin . '/noise-a_s_pub.pem' ),
                rs_pub =>
                  read_pubkey_from_pem( $FindBin::Bin . '/noise-b_s_pub.pem' ),
                ...
                rs_pub_bin  => pack(
                    ...
                    read_pubkey(
                        read_pubkey_from_pem(
                            $FindBin::Bin . '/noise-a_rs_pub.pem'
                        )
                    )
                ),
            },
        ...
        my $b_hs = new_handshake_state(
            $noise_conf,
            {
                ...
                s_priv =>
                  read_key_from_pem( $FindBin::Bin . '/noise-b_s_priv.pem' ),
                s_pub =>
                  read_pubkey_from_pem( $FindBin::Bin . '/noise-b_s_pub.pem' ),
                rs_pub =>
                  read_pubkey_from_pem( $FindBin::Bin . '/noise-a_s_pub.pem' ),
                ...
                rs_pub_bin  => pack(
                    ...
                    read_pubkey(
                        read_pubkey_from_pem(
                            $FindBin::Bin . '/noise-b_rs_pub.pem'
                        )
                    )
                ),
            }
        ...
        my ( $b_msg, $b_c1, $b_c2 ) =
          write_message( $noise_conf, $b_hs, [], $b_msg_src );
...
###   $b_c2
        ...
        ### a recv msg from b
        my ( $a_recv_b_msg_r, $a_c1, $a_c2 ) =
          read_message( $noise_conf, $a_hs, [], $b_msg );
        ### $a_recv_b_msg_r->[0]
###   $a_hs
###  $a_c1
###  $a_c2
        ...
        my ( $a_c1_key, $a_c1_iv ) =
          derive_session_key_iv( $noise_conf, $a_c1->{k}, '' );
        my $a_trans_cipherinfo_b =
          session_encrypt( $noise_conf, $a_c1_key, $a_c1_iv, $a_hs->{ss}{h},
            $plain_a );
        ### b recv comm msg from a
        my ( $b_c1_key, $b_c1_iv ) =
          derive_session_key_iv( $noise_conf, $b_c1->{k}, '' );
        my $b_recv_plaintext_a =
          session_decrypt( $noise_conf, $b_c1_key, $b_c1_iv, $b_hs->{ss}{h},
            $a_trans_cipherinfo_b );
        ### $b_recv_plaintext_a
        ...
        my ( $b_c2_key, $b_c2_iv ) =
          derive_session_key_iv( $noise_conf, $b_c2->{k}, '' );
        my $b_trans_cipherinfo_a =
          session_encrypt( $noise_conf, $b_c2_key, $b_c2_iv, $b_hs->{ss}{h},
            $plain_b );
        ### a recv comm msg from b
        my ( $a_c2_key, $a_c2_iv ) =
          derive_session_key_iv( $noise_conf, $a_c2->{k}, '' );
        my $a_recv_plaintext_b =
          session_decrypt( $noise_conf, $a_c2_key, $a_c2_iv, $a_hs->{ss}{h},
            $b_trans_cipherinfo_a );
        ### $a_recv_plaintext_b
