This is the Security Policy for the Perl DBIO-SQLite distribution.
...
[DBIO-SQLite on CPAN](https://metacpan.org/dist/DBIO-SQLite) or the main branch of the
[DBIO-SQLite git repository](https://codeberg.org/dbio/dbio-sqlite).
...
DBIO-SQLite is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO-SQLite are covered by this policy.
...
they affect DBIO-SQLite, or DBIO-SQLite can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO-SQLite, or plugins to it that are not included with the DBIO-SQLite
distribution) are not covered by this policy.

## Supported Versions of DBIO-SQLite

Only the latest release of DBIO-SQLite will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO-SQLite will run on older versions of Perl. If a
...
that are required for DBIO-SQLite to work. However, some of these
