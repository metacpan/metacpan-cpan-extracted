...
=head2 PACKAGE: pEFL::Edje::Message::StringFloat
...
=head2 PACKAGE: EdjeMessageStringFloatPtr

=over 4

=item * B<Context:> Base class for all EdjeMessageStringFloat methods. Used directly in the C-handler (C<call_perl_edje_message_handler>) to pass read-only pointers from C to Perl.
...
When Perl creates the message, C<pEFL::Edje::Message::StringFloat> inherits from this package to share the C<str()> method. Since Perl is the owner in that case, its own C<DESTROY> block safely handles the cleanup.
