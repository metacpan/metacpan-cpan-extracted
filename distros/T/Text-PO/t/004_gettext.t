...
ok( $rv->length == 1, 'fetchLocale -> array length' );
...
# diag( "Is ", $po->locale_unix( 'ja_JP' ), " same as ", $po->locale_unix, " ?" );
$rv = $po->getLanguageDict( 'ja_JP' );
# is( ref( $rv ), 'HASH', 'getLanguageDict' );
ok( !defined( $rv ), "getLanguageDict() cannot be called to load different locale data" );
is( $po->getLanguageDict( 'fr_BE' ), undef, 'getLanguageDict(fr_BE) -> fail' );
is( $po->getLocale(), 'fr_FR', 'getLocale' );
is( $po->getLocales( 'Bad Request' ), "<span lang=\"fr-FR\">Mauvaise requête</span>", 'getLocales' );
is( $po->getLocalesf( 'Unknown properties: %s', 'pouec' ), "<span lang=\"fr-FR\">Propriété inconnue: pouec</span>", 'getLocalesf' );
# $rv = $po->getMetaKeys();
...
diag( "Supported language is ", $po->locale );
...
# isa_ok( $rv, 'DateTime::Lite', 'poRevisionDate returns a DateTime::Lite object' );
# is( "$rv", '2019-10-03 19-44+0000', 'poRevisionDate' );
# $rv = $po->potCreationDate;
# isa_ok( $rv, 'DateTime::Lite', 'potCreationDate returns a DateTime::Lite object' );
