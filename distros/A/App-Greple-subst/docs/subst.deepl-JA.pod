...
オプション B<--dictpair> を使用すると、コマンドラインから生の辞書エントリを指定できます。この場合、空白、コメント、または DEFINE の展開に関する処理は一切行われません。
...
定義されたパターンは、C<(?&name)> 構文を使用して辞書エントリ内で参照できます。
...
一部の端末 (例: "Apple_Terminal" や "iTerm") は自動的に検出され、何もする必要はありません。それ以外の場合は、B<TERM_BGCOLOR>環境変数に#000000（黒）〜#FFFFFF（白）の数字を設定し、端末の背景色に依存します。
