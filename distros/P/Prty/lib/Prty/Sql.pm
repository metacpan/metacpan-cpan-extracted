...
So verhält es sich im Detail:
    ...
    $script = Prty::Sql->stmtListToScript(
        '-- TEXT1',
        'STMT1',
        "STMT2\n...',
        "STMT3 (\n....\n)",
...
sub insert {
    ...
    if (ref $_[0]) {
        if (Scalar::Util::blessed($_[0]) &&
                $_[0]->isa('Prty::Database::Row::Object')) { # $row
            my $row = shift;
...
sub opIN {
    ...
    if (@_ == 1 && $_[0] =~ /SELECT/i) {
        # Subselect
        $str = Prty::String->removeIndentationNl($_[0]);
        $str =~ s/^/        /mg;
        return "IN (\n$str    )";
    }
...
sub fromClause {
    ...
    for my $expr (@from) {
        ...
        $expr = Prty::String->removeIndentation($expr);
        $expr =~ s/\n/\n    /g;
    }
...
sub whereClause {
    ...
    while (@_) {
        if ($_[0] =~ /[\s=><]/) {
            ...
            my $clause = Prty::String->removeIndentation(shift);
            if ($clause =~ /\n/) {
...
sub diff {
    ...
    my @opt = Prty::Option->extractAll(\@_);
    my @where = (@_,'('.join(" OR\n    ",@or).')');
