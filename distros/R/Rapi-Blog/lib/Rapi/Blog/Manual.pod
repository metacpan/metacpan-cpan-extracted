...
As of the current version of L<Rapi::Blog>, besides B<Author>, there are two other general purpose, built-in 
(i.e. defined by the default database schema) taxonomies, 
B<Tags> and B<Categories>. Both of these operate as C<many-to-many> links, meaning Posts can have
multiple Tags and be in multiple Categories. The difference between the two is the manner in which
they are set in the admin interface:
