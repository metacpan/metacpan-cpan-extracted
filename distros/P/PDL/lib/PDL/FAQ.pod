...
striking examples for this point is probably L<PDL::PP> 
...
=head2 Q: 4.11   I have developer access to Git, how do I upload my changes?
...
Until 2.075, "threading" was used to refer to two ideas,
but that ambiguity has now been resolved by using the now (as of
2022) industry-standard term "broadcasting" for the vectorisation /
...
C<$y> results in a (5,4) ndarray where each
...
in common PDL parlance often called I<ndarrays> (for example, check the
mailing list archives).  Err, clear?  If not, simply use the term
I<ndarray> when you refer to a PDL variable (an instance of a PDL
...
split ndarrays using the command C<dog>.
...
As of PDL 2.099, all functions' inplace capability are automatically
...
=head2 Q: 6.9    How do I set a set of values in an ndarray?  
...
=head2 Q: 6.10   Can I use an ndarray in a conditional expression?  
...
=head2 Q: 6.11   Logical operators and ndarrays -  '||' and '&&' don't work!  
...
C<null> is a special token for 'empty ndarray'. A null pdl 
...
On CPAN you can find the L<PDL::NetCDF> module that works with PDL 2.007.
...
C<rpic> (see L<PDL::IO::Pic> ) you could say:
...
for the slice assignment using the new L<PDL::NiceSlice> syntax and lvalue
assignments.
...
appropriate slices of the 3D ndarray C<$cube> .
...
with the L<PGPLOT> module itself.
...
The fifth element in the C<$package> array-ref is true. This tells PDL
to generate one C file per PP function, which with the right C<make>
...
with the L<PDL::PP> perldoc page this should allow you to quickly
