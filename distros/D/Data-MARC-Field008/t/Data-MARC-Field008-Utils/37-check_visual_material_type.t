...
use Data::MARC::Field008::Utils qw(check_visual_material_type);
...
my $ret = check_visual_material_type($self, 'key');
is($ret, undef, 'Right visual material type is present (a).');

# Test.
$self = {
	'key' => '|',
};
$ret = check_visual_material_type($self, 'key');
is($ret, undef, 'Right visual material type is present (|).');
