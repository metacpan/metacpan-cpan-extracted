...
#
package PeakForest::REST_Client::ApiClient;
...
use PeakForest::REST_Client::Configuration;

sub new {
    my $class = shift;

    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'PeakForest::REST_Client::Configuration' ) {
        $config = $_[0];
    } else {
        $config = PeakForest::REST_Client::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("PeakForest::REST_Client::Object::$class")->new;
        if (ref $data eq "HASH") {
