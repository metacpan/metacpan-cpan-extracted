...
# invoke lazyinit directly
{
   ...
   is( $obj->m_get_value, 1234, 'direct field read yields lazyinit result' );
   is( $init_called, 1, 'lazyinit called once' );

   is( $obj->m_get_value, 1234, 'direct field read yields result a second time' );
...
# setting field value cancels lazyinit
{
   undef $init_called;
...
# providing field value via param cancels lazyinit
{
   undef $init_called;
