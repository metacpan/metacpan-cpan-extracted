...
We've got a couple of methods that are helpers that allow you to physically see
which Raspberry Pi board you're currently working on.

WARNING: These methods call system command line commands using C<sudo>
internally!
    ...
    # turn the green disk IO activity light on full-time

    $pi->io_led(1);

    sleep 10;

    # restore the red power LED back to normal operation

    $pi->pwr_led;

    # restore the green disk IO activity LED back to normal operation
...
=head2 MCP23017 GPIO Expander Testing
...
(4, 20) are for four row, 20 column units. If you only have a two row by four
