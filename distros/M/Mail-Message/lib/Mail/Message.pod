...
When you use Mail::Box subroutine addMessage with the cloned message at hand,
then the clone will automatically be coerced into the right message type
to be added.

See also Mail::Box::Message subroutine copyTo and Mail::Box::Message subroutine moveTo.
...
when Mail::Box::Message subroutine moveTo is used and you are sure that the
...
for possible options Mail::Transport::Send subroutine new and
Mail::Transport::Send subroutine send.
...
located in folders: their Mail::Box::Message subroutine destruct works quite
differently.
...
a search for spam in a folder.  The Mail::Box subroutine messages method
...
Mail::Box subroutine addMessage process, the message will get L<coerce()|Mail::Message/"Internals">-d
