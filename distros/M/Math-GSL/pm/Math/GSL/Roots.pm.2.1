...
type $T. $T must be one of the constant included with this module. If there is
...
=item * C<gsl_root_fsolver_set($s, $f, $x_lower, $x_upper)> -

This function initializes, or reinitializes, an existing solver $s to use the
function $f and the initial search interval [$x_lower, $x_upper]. $f has to be
of this form :
...
is a valid value for $f.  Don't apply this function twice to the same fsolver.
...
=item * C<gsl_root_fdfsolver_set($s, $fdf, $root)> -

This function initializes, or reinitializes, an existing solver $s to use the
function and derivative $fdf and the initial guess $root. $f has to be of this
form :
