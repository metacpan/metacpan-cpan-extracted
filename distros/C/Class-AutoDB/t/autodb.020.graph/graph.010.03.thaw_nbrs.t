...
sub do_test {
  ...
  for my $node (@actual_nodes) {
    my @neighbors=@{$node->neighbors}; # also forces thaw
    push(@correct_thawed,$node);
    $ok&&=_cmp_thawed(\@all_actual_objects,\@correct_thawed,
		      "$label thawed node before neighbors",__FILE__,__LINE__);
    my @neighbor_names=map {$_->name} @neighbors; # force thaw
    push(@correct_thawed,@neighbors);
    $ok&&=_cmp_thawed(\@all_actual_objects,\@correct_thawed,
		      "$label thawed neighbors",__FILE__,__LINE__);
  }
  report_pass($ok,"$label thawed neighbors");
