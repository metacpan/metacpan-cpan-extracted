...
sub groucho_connected {
  ...
  $kernel->post( $sender, 'send_to_server', { prefix => '7UP', command => 'UID', params => [ 'noddy', '1', $uidts, '+i', 'noddy', 'toy.town', '0', '7UPAAAAAB', '0', 'Noddy' ], colonify => 1 } );
  ...
  $kernel->post( $sender, 'send_to_server', { prefix => '7UP', command => 'SJOIN', params => [ ( $uidts - 10 ), '#floobar', '+nt', '@7UPAAAAAB' ], colonify => 1 } );
...
sub client3_input {
  ...
  if ( $cmd eq '366' ) {
    $poe_kernel->post('groucho', 'send_to_server', { prefix => '7UPAAAAAB', command => 'PART', params => [ '#floobar', 'Filthy ape' ], colonify => 1 } );
    return;
  }
  if ( $cmd eq 'PART' ) {
    pass($cmd);
    is($prefix,'noddy!noddy@toy.town','noddy!noddy@toy.town');
    is($params->[0],'#floobar','Channel is correct');
    is($params->[1],'Filthy ape','Pithy comment');
