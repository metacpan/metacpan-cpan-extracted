...
B<Greple> B<xlate> modul menemukan blok teks yang diinginkan dan menggantinya dengan teks terjemahan. Mesin utama adalah GPT-5.5 (F<llm/gpt5.pm>), yang memanggil perintah L<llm|https://llm.datasette.io/>; DeepL (F<deepl.pm>) dan mesin lama berbasis B<gpty> juga disertakan.

Terjemahan di-cache per file, sehingga menjalankan ulang perintah tidak memerlukan biaya untuk teks yang tidak berubah. Ketika dokumen diedit, hanya paragraf yang berubah yang dikirim lagi ke API; mesin yang sadar konteks juga menerima terjemahan di sekitarnya, teks sumber mentah di sekitar perubahan, dan versi sebelumnya dari paragraf yang diedit, sehingga terjemahan baru mempertahankan pilihan kata yang sudah ditetapkan (lihat B<--xlate-context-window>). String sensitif dapat disembunyikan sebelum transmisi (lihat L</ANONYMIZATION AND TEMPLATES>).

Jika Anda ingin menerjemahkan blok teks normal dalam dokumen yang ditulis dalam gaya pod Perl, gunakan perintah B<greple> dengan C<--xlate-engine gpt5> dan modul C<perl> seperti ini:
...
Lalu tambahkan opsi C<--xlate> untuk menerjemahkan area yang dipilih. Kemudian, ini akan menemukan bagian yang diinginkan dan menggantinya dengan keluaran mesin terjemahan.
...
String sensitif dapat disembunyikan sebelum dikirim ke API terjemahan dan dipulihkan dalam output. Tersedia tiga sumber aturan anonimisasi: berkas kamus (B<--xlate-anonymize>), tanda inline dalam dokumen itu sendiri (B<--xlate-anonymize-mark>), dan nilai front matter YAML (B<--xlate-frontmatter>). Setiap string diganti dengan tag kategori seperti C<< <person id=1 /> >> selama transmisi. Target penyembunyian hanya transmisi API: berkas cache lokal menyimpan teks biasa yang telah dipulihkan. Gunakan B<--xlate-dryrun> untuk memeriksa secara persis apa yang akan ditransmisikan.
...
Terjemahkan templat sekali per bahasa dengan C<--xlate-template> (dan C<--xlate-frontmatter> ketika nilainya disimpan di berkas), lalu render setiap kasus dengan mode mandiri B<pandoc-embedz> -- nilai di bawah C<global:> dalam konfigurasi eksternal sama sekali tidak pernah mencapai API terjemahan:
...
Modul mesin dicari terlebih dahulu di namespace backend (C<llm>, lalu C<gpty>), kemudian langsung di bawah C<App::Greple::xlate>. Jadi C<gpt5> memuat C<App::Greple::xlate::llm::gpt5> yang memanggil perintah C<llm>, sementara C<gpt4o> kembali menggunakan C<App::Greple::xlate::gpty::gpt4o>. Gunakan C<--xlate-setopt backend=gpty> untuk memaksa backend tertentu.
...
Tentukan bahasa target. Mesin LLM menerima nama atau kode bahasa apa pun yang dipahami model; nilai tersebut diinterpolasikan ke dalam prompt terjemahan. Anda bisa mendapatkan bahasa yang tersedia dengan perintah C<deepl languages> saat menggunakan mesin B<DeepL>.
...
Tentukan panjang maksimum teks yang akan dikirim ke API sekaligus. Nilai default 0 berarti batas milik engine sendiri: untuk layanan akun DeepL gratis, yaitu 128K untuk API (B<--xlate>) dan 5000 untuk antarmuka papan klip (B<--xlate-labor>). Anda mungkin dapat mengubah nilai-nilai ini jika menggunakan layanan Pro.
...
Tentukan prompt khusus untuk dikirim ke mesin penerjemahan. Opsi ini tersedia untuk mesin LLM (C<gpt3>, C<gpt4o>, C<gpt5>) tetapi tidak untuk DeepL. Anda dapat menyesuaikan perilaku penerjemahan dengan memberikan instruksi spesifik kepada model AI. Jika prompt berisi C<%s>, itu akan diganti dengan nama bahasa target.
...
Inisialisasi cache dokumen baru dari file cache dokumen lain. Berguna untuk laporan berkala: isi awal cache edisi baru dengan cache edisi sebelumnya, sehingga paragraf yang tidak berubah tidak diterjemahkan ulang dan paragraf yang diedit mempertahankan susunan kata edisi sebelumnya. Seed hanya digunakan ketika cache target kosong; jika tidak, seed diabaikan dengan peringatan. Dengan C<--xlate-cache=auto> default, menentukan seed juga menyiratkan pembuatan file cache dokumen baru.
...
atau dalam format baris sederhana (C<category pattern>, C</.../> untuk regex). Setiap item diganti dengan tag kategori seperti C<< <person id=1 /> >>; string yang sama selalu mendapatkan tag yang sama, sehingga model dapat melacak siapa itu siapa. Field JSON yang tidak dikenal diabaikan, sehingga generator (mis. LLM lokal yang mengekstrak entitas) dapat menambahkan anotasi mereka sendiri. Kategori C<lit> dicadangkan. File cache lokal tetap menyimpan teks biasa yang dipulihkan: target penyembunyian hanya transmisi API.
...
Kumpulkan entri anonimisasi dari mark inline dalam dokumen itu sendiri. Tandai kemunculan pertama seperti C<{{ person("山田太郎") }}> dan setiap kemunculan string di seluruh dokumen dianonimkan. Mark itu sendiri tetap berada di sumber dan dalam terjemahan, sehingga dokumen juga dapat diproses oleh pemroses makro bergaya Jinja2 (definisikan makro C<person> untuk mencetak atau menyunting nama). I<regex> kustom harus berisi named capture C<< (?<category>...) >> dan C<< (?<text>...) >>.

Perhatikan bahwa dengan opsi bernilai opsional seperti ini, argumen file berikutnya akan dianggap sebagai nilainya: tulis C<--xlate-anonymize-mark=> (dengan C<=> di akhir) saat menggunakan notasi default.

Notasi alternatif dapat dikonfigurasi, misalnya C<< --xlate-anonymize-mark='@@(?<category>[a-z][a-z0-9_]*):(?<text>[^\n]+?)@@' >> untuk mark bergaya C<@@person:NAME@@>, atau bentuk komentar HTML yang tetap tidak terlihat dalam Markdown yang dirender. Aturan mark dikumpulkan per dokumen: string yang ditandai dalam satu file input tidak disembunyikan dalam file lain pada run yang sama (berbeda dengan nilai front matter, yang terakumulasi di seluruh file).

=item B<--xlate-template>[=I<regex>]

Perlakukan ekspresi templat (default: Jinja2 C<{{ ... }}>, C<{% ... %}>, C<{# ... #}>) sebagai placeholder buram: instruksikan model untuk menyalinnya tanpa perubahan dan verifikasi, per blok, bahwa respons berisi ekspresi yang sama persis, masing-masing dengan jumlah kemunculan yang sama. Urutannya boleh berubah, karena terjemahan secara sah menyusun ulang ekspresi tersebut agar mengikuti urutan kata bahasa target. Ekspresi yang rusak membatalkan run; cache di-checkpoint dan dibekukan, sehingga tidak ada yang sudah dibayar menjadi hilang.

Perhatikan bahwa dengan opsi bernilai opsional seperti ini, argumen file berikutnya akan diambil sebagai nilainya: tulis C<--xlate-template=> (dengan C<=> di akhir) saat menggunakan notasi default.

=item B<--xlate-frontmatter>

Perlakukan blok awal C<---> ... C<---> sebagai YAML front matter: kecualikan dari terjemahan dan dari irisan konteks fase-2, serta tambahkan nilai C<key: value> datarnya ke aturan anonimisasi (kategori C<var>) sebagai jaring pengaman. Dengan beberapa file input, nilai yang dikumpulkan akan terakumulasi (lebih memilih sisi penyembunyian).

Selalu sisakan satu baris kosong setelah penutup C<--->. Dengan pola pencocokan bergaya paragraf, front matter yang langsung menyambung ke teks isi membentuk satu blok yang melintang yang tidak dapat ditekan oleh pengecualian (peringatan dicetak dalam kasus tersebut); nilainya tetap dianonimkan, tetapi front matter itu sendiri akan dikirim untuk diterjemahkan.
...
Lihat hasil terjemahan secara waktu nyata di keluaran STDERR. Payload C<From> ditampilkan seperti yang dikirimkan, setelah anonimisasi dan masking.
...
Operasi Docker ditangani oleh L<App::dozo>, yang juga dapat digunakan sebagai perintah mandiri. Perintah C<dozo> mendukung berkas konfigurasi C<.dozorc> untuk pengaturan kontainer yang persisten.
...
Kunci autentikasi OpenAI, digunakan oleh mesin B<gpty> legacy. Mesin B<gpt5> berbasis C<llm> juga membaca variabel ini, tetapi kunci yang disimpan dengan C<llm keys set openai> juga berfungsi.

=item GREPLE_XLATE_CACHE

Atur strategi cache default (lihat L</CACHE OPTIONS>).
...
Pasang alat baris perintah untuk mesin yang Anda gunakan: C<llm> untuk mesin B<gpt5>, C<deepl> untuk DeepL, C<gpty> untuk mesin GPT legacy.
...
Pustaka C<getoptlong.sh> digunakan untuk penguraian opsi dalam skrip C<xlate> dan L<App::dozo>.

=item * L<https://llm.datasette.io/>

Perintah C<llm> yang digunakan oleh mesin B<gpt5> untuk mengakses model LLM.
