...
@colors = ( @Graphics::VTK::Colors::flesh, @Graphics::VTK::Colors::banana, @Graphics::VTK::Colors::grey, @Graphics::VTK::Colors::pink, @Graphics::VTK::Colors::carrot, @Graphics::VTK::Colors::gainsboro, @Graphics::VTK::Colors::tomato, @Graphics::VTK::Colors::gold, @Graphics::VTK::Colors::thistle, @Graphics::VTK::Colors::chocolate );
@types = (qw/UnsignedChar Char Short UnsignedShort Int UnsignedInt Long UnsignedLong Float Double/);
$i = 1;
$c = 0;
foreach $vtkType (@types)
 {
  ...
  $castTo{$vtkType} = Graphics::VTK::ImageCast->new;
   $method = "SetOutputScalarTypeTo$vtkType";
  $castTo{$vtkType}->$method;
  ...
  $isoMapper{$vtkType}->ImmediateModeRenderingOn;
  $isoActor{$vtkType} = Graphics::VTK::Actor->new;
  $isoActor{$vtkType}->SetMapper($isoMapper{$vtkType});
  $isoActor{$vtkType}->GetProperty->SetDiffuseColor($colors[$c],$colors[($c + 1)],$colors[($c + 2)]);
