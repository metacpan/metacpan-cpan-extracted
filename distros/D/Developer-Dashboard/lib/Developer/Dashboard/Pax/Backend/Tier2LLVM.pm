...
sub metadata {
    ...
    return {
        ...
        contract => 'guarded_ssa_to_typed_ir_to_llvm_ir_module_emission',
    };
}
...
sub module_for {
    ...
    if (($typed_ir->{status} // '') eq 'typed_ir' && ($typed_ir->{ops}[0]{op} // '') eq 'typed_i64_binary_leaf') {
        my $op = $shape->{op} // '';
        my $body = _llvm_binary_body($op);
        return _module($region_id, $region_name, $body, "LLVM IR emitted for typed guarded i64 $op leaf");
    }

    if (($typed_ir->{status} // '') eq 'typed_ir' && ($typed_ir->{ops}[0]{op} // '') eq 'typed_i64_sum_loop') {
        return _module($region_id, $region_name, _llvm_sum_loop_body(), 'LLVM IR emitted for typed guarded i64 sum loop');
    }

    if (($typed_ir->{status} // '') eq 'typed_ir' && ($typed_ir->{ops}[0]{op} // '') eq 'typed_i64_masked_mix_accum_loop') {
        return _module($region_id, $region_name, _llvm_masked_mix_accum_loop_body(), 'LLVM IR emitted for typed guarded i64 masked mix accumulation loop');
    }

    return {
        status => 'fallback',
        reason => 'no typed LLVM lowering for this guarded SSA shape',
    };
}
...
Developer::Dashboard::Pax::Backend::Tier2LLVM - tier-2 backend planning stub for optimized native code generation

=head1 SYNOPSIS

  use Developer::Dashboard::Pax::Backend::Tier2LLVM;

  my $obj = Developer::Dashboard::Pax::Backend::Tier2LLVM->new(...);
...
Example 1:
  ...
  perl -Ilib -MDeveloper::Dashboard::Pax::Backend::Tier2LLVM -e 1
