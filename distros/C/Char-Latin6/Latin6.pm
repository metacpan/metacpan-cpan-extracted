...
######################################################################
#
# Latin6 - Source code filter to escape Latin-6 script
#
...
perl $0 Latin-6_script.pl > Escaped_script.pl.e
END
    }

    print Latin6::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (CORE::eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Elatin6::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
        }
    ...
    my $fh = gensym();
    Elatin6::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Latin6::escape_script {
    ...
    if (/^ use Elatin6(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        ...
        $e_script .= sprintf("use Elatin6 '%s.0'; # 'quote' for perl5.6\n", $Latin6::VERSION); # require run-time routines version

        # use Latin6 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Latin6 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Latin6::VERSION) {
                    my @file = grep -e, map {qq{$_/Latin6.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Latin6.pm $version, but @{[__FILE__]} is version $Latin6::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Elatin6.pm $version, but \\\$Elatin6::VERSION is \$Elatin6::VERSION" if \$Elatin6::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Latin6::VERSION) {
                    die "Script $0 required Latin6.pm $version, but @{[__FILE__]} is only version $Latin6::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Elatin6::function_ord     = 'Latin6::ord'     if /\A ord \z/oxms;
                    $Elatin6::function_ord_    = 'Latin6::ord_'    if /\A ord \z/oxms;
                    $Elatin6::function_reverse = 'Latin6::reverse' if /\A reverse \z/oxms;
                    $Elatin6::function_getc    = 'Latin6::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Latin6::escape();
...
=item * You'll have to download Latin6.pm and Elatin6.pm and put it in your perl lib directory.

=item * You'll need to write "use Latin6;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Latin6;
  use Latin6 ver.sion;             --- require minimum version
  use Latin6 ver.sion.0;           --- expects version (match or die)

  # "no Latin6;" not supported

  or

  $ perl Latin6.pm Latin-6_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Latin-6_script.pl  --- script written in Latin-6
...
Latin6 software is "middleware" between perl interpreter and your Perl script
written in Latin-6.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Latin-6, Perl3 or later
can treat Latin-6 as binary data.

By "use Latin6;", it automatically interpret your script as Latin-6. The various
functions of perl including a regular expression can treat Latin-6 now.
...
This software is a source code filter to escape Perl script encoded by Latin-6
...
=item * Remains One Language Handling Raw Latin-6, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Latin6.pm, Elatin6.pm
  +---------------------------------------+
...
import of Elatin6.pm works well.
...
=head1 Software Composition
   ...
   Latin6.pm               --- source code filter to escape Latin-6
   Elatin6.pm              --- run-time routines for Latin6.pm
...
You need write 'use Latin6;' in your script.
...
=head1 Calling 'Elatin6::ignorecase()' (Latin6 software provides)

Latin6 software applies calling 'Elatin6::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Elatin6::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Elatin6.pm provides)
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Latin6 software provides)

Latin6 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Latin6 software provides)

Insert 'Elatin6::' at head of function name. Elatin6.pm provides your script Elatin6::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Elatin6::lc        Character
  lcfirst     Elatin6::lcfirst   Character
  uc          Elatin6::uc        Character
  ucfirst     Elatin6::ucfirst   Character
  fc          Elatin6::fc        Character
  chr         Elatin6::chr       Character
  glob        Elatin6::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Latin6 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Latin6 software provides)

Latin6 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Elatin6.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Latin6 software and
...
Latin6 software remains one language and one interpreter.
