# This Makefile is for the Mobile::Location extension to perl.
#
...
#	NAME => q[Mobile::Location]
#	PREREQ_PM => { constant=>q[0], Sys::Hostname=>q[0], IO::Socket=>q[0], Crypt::RSA=>q[1.5], POSIX=>q[0], Socket=>q[0], Devel::Scooby=>q[4.07], HTTP::Status=>q[1.26], HTTP::Daemon=>q[1.26] }
#	VERSION_FROM => q[Location.pm]
...
DISTNAME = Mobile-Location
NAME_SYM = Mobile_Location
VERSION = 4.02
VERSION_SYM = 4_02
XS_VERSION = 4.02
INST_BIN = blib/bin
...
FULLEXT = Mobile/Location
BASEEXT = Location
PARENT_NAME = Mobile
DLBASE = $(BASEEXT)
VERSION_FROM = Location.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
...
TO_INST_PM = Location.pm

PM_TO_BLIB = Location.pm \
	$(INST_LIBDIR)/Location.pm
...
manifypods : pure_all Location.pm
	@$(POD2MAN) \
	Location.pm \
	$(INST_MAN3DIR)/Mobile::Location.$(MAN3EXT)
