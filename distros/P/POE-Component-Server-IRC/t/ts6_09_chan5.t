...
sub client2_input {
  ...
  if ( $cmd eq '366' ) {
    $poe_kernel->post( 'client', 'send_to_server', { command => 'MODE', params => [ '#floobar', '+h', 'rubbarb' ], colonify => 0 } );
    $poe_kernel->yield('_launch_client3');
    return;
  }
  if ( $cmd eq 'PRIVMSG' && $params->[1] =~ m!ACTION! ) {
    pass($cmd);
    is($params->[1],"\001ACTION hopes for the best\001", 'CTCP ACTION which is allowed');
    $poe_kernel->post('client', 'send_to_server', { command => 'MODE', params => [ '#floobar', '+M' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'MODE' && $params->[1] eq '+b' ) {
    pass($cmd);
    $poe_kernel->post( 'client3', 'send_to_server', { command => 'MODE', params => [ '#floobar', '+b' ] } );
    return;
  }
  return;
}
...
sub client3_input {
  ...
  if ( $cmd eq '366' ) {
    $poe_kernel->post('client', 'send_to_server', { command => 'MODE', params => [ '#floobar', '+C' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'MODE' && $params->[1] eq '+C' ) {
    my $msg = "\001ACTION hopes for the best\001";
    $poe_kernel->post($sender, 'send_to_server', { command => 'PRIVMSG', params => [ '#floobar', $msg ], colonify => 1 } );
    return;
  }
  if ( $cmd eq 'MODE' && $params->[1] eq '+M' ) {
    $poe_kernel->post($sender, 'send_to_server', { command => 'PRIVMSG', params => [ '#floobar', 'Testing' ], colonify => 1 } );
    return;
  }
  if ( $cmd eq 'MODE' && $params->[1] eq '-M+T' ) {
    $poe_kernel->post($sender, 'send_to_server', { command => 'NOTICE', params => [ '#floobar', 'Testing' ], colonify => 1 } );
    return;
  }
  if ( $cmd eq 'MODE' && $params->[1] eq '-T+N' ) {
    $poe_kernel->post($sender, 'send_to_server', { command => 'NICK', params => [ 'mustard' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'MODE' && $params->[1] eq '-N+u' ) {
    $poe_kernel->post('client', 'send_to_server', { command => 'MODE', params => [ '#floobar', '+b', 'bads!bads@bads.net' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'MODE' && $params->[1] eq '+R' ) {
    $poe_kernel->post($sender, 'send_to_server', { command => 'PART', params => [ '#floobar' ], colonify => 0 } );
    $poe_kernel->post($sender, 'send_to_server', { command => 'JOIN', params => [ '#floobar' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq '477' ) {
    pass("IRC$cmd");
    is($params->[0], 'custard','Our nickname is correct');
    is($params->[1], '#floobar', 'Channel is okay');
    is($params->[2], 'You need to identify to a registered nick to join or speak in that channel.',
      'You need to identify to a registered nick to join or speak in that channel.');
    if ( !$heap->{477} ) {
      $poe_kernel->post('client', 'send_to_server', { command => 'MODE', params => [ '#floobar', '-M+T' ], colonify => 0 } );
      $heap->{477}++;
      return;
    }
  ...
  if ( $cmd eq '404' ) {
    ...
    is($params->[0], 'custard','Our nickname is correct');
    is($params->[1], '#floobar', 'Channel is okay');
    is($params->[2], 'Cannot send to channel', 'Cannot send to channel' );
    $poe_kernel->post('client', 'send_to_server', { command => 'MODE', params => [ '#floobar', '-T+N' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq '447' ) {
    pass("IRC$cmd");
    is($params->[0], 'custard','Our nickname is correct');
    is($params->[1], 'Cannot change nickname while on #floobar (+N)', 'Cannot change nickname while on #floobar (+N)' );
    $poe_kernel->post('client', 'send_to_server', { command => 'MODE', params => [ '#floobar', '-N+u' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq '368' ) {
    pass("IRC$cmd");
    is($params->[0], 'custard','Our nickname is correct');
    is($params->[1], '#floobar', 'Channel is okay');
    is($params->[2], 'End of Channel Ban List', 'End of Channel Ban List' );
    $poe_kernel->post('client', 'send_to_server', { command => 'MODE', params => [ '#floobar', '+R' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq '367' ) {
    fail("We should not see a 367");
    return;
  }
  return;
}
