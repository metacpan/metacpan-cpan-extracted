...
This document describes version 0.050 of Sah::Schema::perl::identifier::qualified_ascii (from Perl distribution Sah-SchemaBundle-Perl), released on 2024-02-16.
...
(set-as-default, coerced, prefiltered) value:
 ...
 # a sample valid data
 $data = "foo::0";
 my $res = $validator->($data); # => ["","foo::0"]
 
 # a sample invalid data
 $data = "foo::bar-baz";
 my $res = $validator->($data); # => ["Must match regex pattern qr(\\A[A-Za-z_][A-Za-z_0-9]*(?:::[A-Za-z_0-9]+)+\\z)","foo::bar-baz"]
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"perl::identifier::qualified_ascii*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('perl::identifier::qualified_ascii*', name=>'PerlIdentifierQualifiedAscii')
     );
 }
