...
sub main {
    ...
    $self->process_template_chapter($book, $_, %opt) for @{$book->{item_list}};
    return $opt{output};
}
...
sub process_template_index {
    ...
    $_->{fid}  = sprintf("%04d", $_->{id}) for @{$book->{item_list}};
    my $n = $#{$book->{item_list}} + 1 + 1;
    my $m = "%0".(int(log($n)/log(10))+1)."d";
    $_->{tid}  = sprintf($m, $_->{id}) for @{$book->{item_list}};

    my $toc = qq{<div id="toc"><ul>
    [% FOREACH r IN item_list %][% IF r.content %]
