...
This document describes version 0.016 of Sah::Schema::latin_alphanum (from Perl distribution Sah-Schemas-Str), released on 2023-10-23.
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("latin_alphanum", {return_type=>'str_errmsg'});
 ...
 $data = "foo bar";
 my $errmsg = $validator->($data); # => "Must match regex pattern qr(\\A[A-Za-z0-9]*\\z)"

Often a schema has coercion rule or default value rules, so after validation the
validated value will be different from the original. To return the validated
(set-as-default, coerced, prefiltered) value:

 my $validator = gen_validator("latin_alphanum", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = "foo bar";
 my $res = $validator->($data); # => ["Must match regex pattern qr(\\A[A-Za-z0-9]*\\z)","foo bar"]
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"latin_alphanum*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('latin_alphanum*', name=>'LatinAlphanum')
     );
 }

 use My::Types qw(LatinAlphanum);
 LatinAlphanum->assert_valid($data);
