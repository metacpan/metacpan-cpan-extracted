...
`Catalyst::Plugin::MCP` is a Model Context Protocol server plugin for Catalyst,
layered on `Catalyst::Plugin::JSONRPC::Server`. The engine is in
`lib/Catalyst/Plugin/MCP/` (the plugin, `Server`, and the provider roles); tests
are under `t/`.
...
- Perl 5.36+; subroutine signatures. Moo and Moo::Role for OO.
