...
A lexeme or a terminal can have a C<if-action> setting, that applies I<only> when the lexeme or a terminal is discovered via the automatic scan. The C<if-action> must refer to a callback in user-land's recognizer interface or in embedded lua interpreter, and must return a boolean:
...
MarpaX::ESLIF BNF can be expressed in itself:
  ...
  <separator specification>      ::= 'separator' /=>|\x{21D2}/u <single symbol>
