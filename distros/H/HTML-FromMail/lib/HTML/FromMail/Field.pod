...
=item $obj-E<gt>B<addressField>($field, $how, \%options)

Produce text for a header C<$field> containing addresses.  On C<$how> this
is done is defining the result.  Possible values are C<'ADDRESS'>,
C<'PHRASE'>, C<'PLAIN'>, C<'MAILTO'>, or C<'LINK'>.  See L</address HOW>
for details.

=item $obj-E<gt>B<fromHead>($head, $name, \%options)

Returns the fields from the header with C<$name>.  Some fields appear
more than once, some may not be present.

=item $obj-E<gt>B<htmlAddresses>($field, \%options)

Returns an array with address info.

=item $obj-E<gt>B<htmlBody>(\%options)
...
=item $obj-E<gt>B<htmlName>(\%options)
...
Overrules the default from L<new(settings)|HTML::FromMail::Object/"Constructors"> C<names>.
See L</names HOW>.
...
=head4 » Example: using HTML::FromMail::Format::Magic
