package Net::FreeDB2::Response::Hello;
...
# This allows declaration	use Net::FreeDB2::Response::Hello ':all';
...
sub read {
	...
	exists ($opt->{content_ref}) || throw Error::Simple ('ERROR: Net::FreeDB2::Response::Hello::read, option \'content_ref\' not defined.');
	...
	defined ($code) || throw Error::Simple ('ERROR: Net::FreeDB2::Response::Hello::read, first line of specified \'content_ref\' does not contain a code.');
	$code == 200 || $code == 431 || $code == 431 || throw Error::Simple ('ERROR: Net::FreeDB2::Response::Hello::read, first line of specified \'content_ref\' does not contain a code.');
	if ($code == 200) {
		$self->setError (0);
		$self->setResult ('Handshake successful');
	} elsif ($code == 402) {
		$self->setError (0);
		$self->setResult ('Already shook hands');
	} elsif ($code == 431) {
		$self->setError (1);
		$self->setResult ('Handshake not successful, closing connection');
	} else {
		throw Error::Simple ("ERROR: Net::FreeDB2::Response::Hello::read, unknown code '$code' returned.");
	}
}
...
The C<Net::FreeDB2::Response::Hello> class contains FreeDB/CDDB response information for the hello process. It is a subclass of C<Net::FreeDB2::Response>.
...
Creates a new C<Net::FreeDB2::Response::Hello> object. Calls C<read ()> if option C<content_ref> is passed through C<OPT_HASH_REF>.
