...
ok( $Locales::DB::Language::nl::VERSION eq ( $Locales::VERSION - 0.25 ), 'VERSION is correct' );
...
my %uniq = ();
grep { not $uniq{$_}++ } @{ $Locales::DB::Language::nl::misc_info{'plural_forms'}->{'category_list'} };
is_deeply(
    [ sort @{ $Locales::DB::Language::nl::misc_info{'plural_forms'}->{'category_list'} } ],
    [ sort keys %uniq ],
    "'category_list' contains no duplicates"
);

ok( grep( m/^other$/, @{ $Locales::DB::Language::nl::misc_info{'plural_forms'}->{'category_list'} } ), "'category_list' has 'other'" );

is_deeply(
    [ grep !m/^other$/, sort @{ $Locales::DB::Language::nl::misc_info{'plural_forms'}->{'category_list'} } ],
    [ grep !m/^other$/, sort keys %{ $Locales::DB::Language::nl::misc_info{'plural_forms'}->{'category_rules'} } ],
    "'category_rules' has necessary 'category_list' items"
);

is_deeply(
    [ sort keys %{ $Locales::DB::Language::nl::misc_info{'plural_forms'}->{'category_rules'} } ],
    [ sort keys %{ $Locales::DB::Language::nl::misc_info{'plural_forms'}->{'category_rules_compiled'} } ],
    "each 'category_rules' has a 'category_rules_compiled'"
);
my $ok_rule_count = 0;
my $error         = '';
for my $rule ( keys %{ $Locales::DB::Language::nl::misc_info{'plural_forms'}->{'category_rules_compiled'} } ) {
    if ( ref( $Locales::DB::Language::nl::misc_info{'plural_forms'}->{'category_rules_compiled'}{$rule} ) eq 'CODE' ) {
        $ok_rule_count++;
        next;
    }
    eval $Locales::DB::Language::nl::misc_info{'plural_forms'}->{'category_rules_compiled'}{$rule};
    if ($@) {
        $error .= $@;
        next;
    }
...
ok( $ok_rule_count == keys %{ $Locales::DB::Language::nl::misc_info{'plural_forms'}->{'category_rules_compiled'} }, "each 'category_rules_compiled' eval without error - count" );
is( $error, '', "each 'category_rules_compiled' is a code ref or evals without error - errors" );

my $self_obj = Locales->new('nl');
ok( ref($self_obj), 'nl object created OK' );
