...
{
    package DBD::KingBase;

    use version; our $VERSION = qv('3.10.12');
    ...
    {
        package DBD::KingBase::DefaultValue;
        sub new { my $self = {}; return bless $self, shift; }
    }
    $DBDPG_DEFAULT = DBD::KingBase::DefaultValue->new();
    ...
    bootstrap DBD::KingBase $VERSION;
    ...
    sub parse_trace_flag {
        ...
        return 0x01000000 if $flag eq 'pglibkci';
    ...
    sub driver {
        ...
        $drh = DBI::_new_drh($class, {
            'Name'        => 'KingBase',
            'Version'     => $VERSION,
            'Err'         => \$DBD::KingBase::err,
            'Errstr'      => \$DBD::KingBase::errstr,
            'State'       => \$DBD::KingBase::sqlstate,
            'Attribution' => "DBD::KingBase $VERSION by Greg Sabino Mullane and others",
        });

        if (!$methods_are_installed) {
            DBD::KingBase::db->install_method('pg_cancel');
            DBD::KingBase::db->install_method('pg_endcopy');
            DBD::KingBase::db->install_method('pg_error_field');
            DBD::KingBase::db->install_method('pg_getline');
            DBD::KingBase::db->install_method('pg_getcopydata');
            DBD::KingBase::db->install_method('pg_getcopydata_async');
            DBD::KingBase::db->install_method('pg_notifies');
            DBD::KingBase::db->install_method('pg_putcopydata');
            DBD::KingBase::db->install_method('pg_putcopyend');
            DBD::KingBase::db->install_method('pg_ping');
            DBD::KingBase::db->install_method('pg_putline');
            DBD::KingBase::db->install_method('pg_ready');
            DBD::KingBase::db->install_method('pg_release');
            DBD::KingBase::db->install_method('pg_result'); ## NOT duplicated below!
            DBD::KingBase::db->install_method('pg_rollback_to');
            DBD::KingBase::db->install_method('pg_savepoint');
            DBD::KingBase::db->install_method('pg_server_trace');
            DBD::KingBase::db->install_method('pg_server_untrace');
            DBD::KingBase::db->install_method('pg_type_info');

            DBD::KingBase::st->install_method('pg_cancel');
            DBD::KingBase::st->install_method('pg_result');
            DBD::KingBase::st->install_method('pg_ready');
            DBD::KingBase::st->install_method('pg_canonical_ids');
            DBD::KingBase::st->install_method('pg_canonical_names');

            DBD::KingBase::db->install_method('pg_lo_creat');
            DBD::KingBase::db->install_method('pg_lo_open');
            DBD::KingBase::db->install_method('pg_lo_write');
            DBD::KingBase::db->install_method('pg_lo_read');
            DBD::KingBase::db->install_method('pg_lo_lseek');
            DBD::KingBase::db->install_method('pg_lo_tell');
            DBD::KingBase::db->install_method('pg_lo_truncate');
            DBD::KingBase::db->install_method('pg_lo_close');
            DBD::KingBase::db->install_method('pg_lo_unlink');
            DBD::KingBase::db->install_method('pg_lo_import');
            DBD::KingBase::db->install_method('pg_lo_import_with_oid');
            DBD::KingBase::db->install_method('pg_lo_export');
...
} ## end of package DBD::KingBase


{
    package DBD::KingBase::dr;
    ...
    sub data_sources {
        ...
        my $drh = shift;
        my $attr = shift || '';
        my $connstring = 'dbname=kingbase';
        if ($ENV{DBI_DSN}) {
            ($connstring = $ENV{DBI_DSN}) =~ s/dbi:KingBase://i;
        }
        if (length $attr) {
            $connstring .= ";$attr";
        }

        my $dbh = DBD::KingBase::dr::connect($drh, $connstring) or return;
        $dbh->{AutoCommit}=1;
        my $SQL = 'SELECT pg_catalog.quote_ident(datname) FROM pg_catalog.pg_database ORDER BY 1';
        my $sth = $dbh->prepare($SQL);
        $sth->execute() or die $DBI::errstr;
        $attr and $attr = ";$attr";
        my @sources = map { "dbi:KingBase:dbname=$_->[0]$attr" } @{$sth->fetchall_arrayref()};
        $dbh->disconnect;
        return @sources;
    }
    ...
    sub connect { ## no critic (ProhibitBuiltinHomonyms)
        ...
        DBD::KingBase::db::_login($dbh, $dbname, $user, $pass, $attr) or return undef;
...
} ## end of package DBD::KingBase::dr


{
    package DBD::KingBase::db;
    ...
    sub parse_trace_flag {
        my ($h, $flag) = @_;
        return DBD::KingBase->parse_trace_flag($flag);
    }
    ...
    sub prepare {
        ...
        DBD::KingBase::st::_prepare($sth, $statement, @attribs) || 0;
    ...
    sub ping {
        ...
        my $ret = DBD::KingBase::db::_ping($dbh);
        return $ret < 1 ? 0 : $ret;
    }
    ...
    sub pg_ping {
        my $dbh = shift;
        local $SIG{__WARN__} = sub { } if $dbh->FETCH('PrintError');
        return DBD::KingBase::db::_ping($dbh);
    }

    sub pg_type_info {
        my($dbh,$pg_type) = @_;
        local $SIG{__WARN__} = sub { } if $dbh->FETCH('PrintError');
        my $ret = DBD::KingBase::db::_pg_type_info($pg_type);
        return $ret;
    }
    ...
    sub column_info {
		...
		$table = lc($table);		#KingBase 数据库中表名称存储的为小写
        my @search;
        ...
        for my $row (@$data) {
            ...
            # Replace the KingBase type with the SQL_ type
            $row->[$col_map{DATA_TYPE}] = DBD::KingBase::db::pg_type_info($dbh,$row->[$col_map{DATA_TYPE}]);
...
## Driver information:
   ...
   10021 => ['SQL_ASYNC_MODE',                      2                         ], ## SQL_AM_STATEMENT
     ...
     121 => ['SQL_BATCH_SUPPORT',                   3                         ], ## 12 SELECT_PROC + ROW_COUNT_PROC
       2 => ['SQL_DATA_SOURCE_NAME',                sub { sprintf 'dbi:KingBase:%s', shift->{Name} } ],
     ...
     135 => ['SQL_DRIVER_HDESC',                    0                         ], ## not applicable
      ...
      76 => ['SQL_DRIVER_HLIB',                     0                         ], ## not applicable
       ...
       ## Not clear what should go here. Some things suggest 'KingBase', others 'KingBase.pm'. We'll use DBD::KingBase for now
       6 => ['SQL_DRIVER_NAME',                     'DBD::KingBase'                 ],
...
## Supported SQL
   ...
   10003  => ['SQL_CATALOG_NAME',                   'N'                       ],
     ...
     148  => ['SQL_INDEX_KEYWORDS',                 0                         ], ## not needed for KingBase
...
## Conversion information - all but BIT, LONGVARBINARY, and LONGVARCHAR
    ...
    sub get_info {
        ...
        if (ref $ans eq 'CODE') {
            ...
            my $simpleversion = $DBD::KingBase::VERSION;
            $simpleversion =~ s/_/./g;
            no if $] >= 5.022, warnings => 'redundant';
            return sprintf '%02d.%02d.%1d%1d%1d%1d', split (/\./, "$simpleversion.0.0.0.0.0.0");
        }
         elsif ($ans eq 'KEYWORDS') {
            ## Basically, we want ones that are 'reserved' for Kingbase but not 'reserved' in SQL:2011
...
DBD::KingBase - Kingbase database driver for the DBI module

=head1 SYNOPSIS

  use DBI;

  # For some advanced uses you may need Kingbase type values:
  use DBD::KingBase qw(:pg_types);
  
  $dbh = DBI->connect("dbi:KingBase:dbname=$dbname", '', '', {AutoCommit => 0});
...
DBD::KingBase is a Perl module that works with the DBI module to provide access to
Kingbase databases.

=head1 VERSION

KingBase Version 3.10.x 系列版本，由王海清（161263@qq.com）基于原来DBD:KB V3.10.5改造而来。

  - DBD:KB是纯DBD::Pg改造的版本，没有基于KingBase接口做任何改造，相对比较稳定，但是无KingBase的特有能力和参数。

KingBase Version 3.10.9 (released June 16, 2026)

  - 发布到cpan官网

KingBase Version 3.10.8 (released June 16, 2026)

  - 注意： 经过甘肃移动现场测试，相对比较稳定的版本，支持相对比较复制的链接配置
  
    DBI:KingBase:                           #dsn参数设置
    dbname=gzglzxtemp;                      #数据库名称
    host=10.215.225.9,10.215.225.8;         #支持主备服务器
    port=54329,54329;                       #支持主备服务器端口
    connect_timeout=3;                      #链接超时
    fastswitch=on;                          #快速切换（主备切换）
    retries=3;                              #重试次数
    delay=5;                                #重试延迟5秒，于retries配合使用
    statement_rollback=1;                   #语句级回滚
    target_session_attrs=read-write         #默认是备机只读，参数设置备机可以读写

KingBase Version 3.10.7 (released May 30, 2026)

  - 解决MERGE，INSERT，MOVE，DELETE，UPDATE，SELECT的rows的问题

KingBase Version 3.10.6 (released May 10, 2026)

  - 支持参数statement_rollback=1，解决KingBase/Pg数据库的事务出错后续SQL不能执行的问题

KingBase Version 3.10.5  (released April 28, 2026)

  - 基于官方DBD::Pg v3.18.0和原来DBD::KB v3.18.5，并结合KingBase官方提供lib改造

原Pg数据库：

  BEGIN;
  INSERT INTO t VALUES (1);
  INSERT INTO t VALUES ('abc');  -- 出错，比如类型不匹配
  INSERT INTO t VALUES (3);      -- ❌ 不会执行
  COMMIT;

KingBase数据库，支持参数：statement_rollback=1：

  use DBI;
  use DBD::KingBase;
  # options 参数内容必须以 -c 开头，用空格分隔多个配置项
  my $options = "-c statement_timeout=5000 -c log_statement=none";
  
  $dbh = DBI->connect("dbi:KingBase:dbname=$dbname;statement_rollback=1;options='$options'", '', '', {AutoCommit => 0});
...
再执行以上SQL不会出错：
  ...
  INSERT INTO t VALUES (1);
  INSERT INTO t VALUES ('abc');  -- 出错，比如类型不匹配
  INSERT INTO t VALUES (3);      -- 会继续执行执行
  COMMIT;
...
equivalent of the "new" method. To connect to a Kingbase database with a minimum of parameters, 
use the following syntax:

  $dbh = DBI->connect("dbi:KingBase:dbname=$dbname", '', '', {AutoCommit => 0});

This connects to the database named in the C<$dbname> variable on the default port (usually 54321) 
without any user authentication.

The following connect statement shows almost all possible parameters:

  $dbh = DBI->connect("dbi:KingBase:dbname=$dbname;host=$host;port=$port;options=$options",
                      $username,
                      $password,
...
around the value e.g. "dbi:KingBase:dbname='spacey name';host=$host"
...
specific environment variables, and then fall back to hard-coded defaults:
  ...
  -----------------------------------------------------------------------------
  host                   KBHOST                  local domain socket,eg.10.215.225.9,10.215.225.8(支持主备服务器)
  hostaddr               KBHOSTADDR              local domain socket,eg.54329,54329(支持主备服务器端口)
  port                   KBPORT                  54321
  dbname*                KBDATABASE              current userid
  username               KBUSER                  current userid
  password               KBPASSWORD              (none)
  options                KBOPTIONS               (none)
  service                KBSERVICE               (none)
  sslmode                KBSSLMODE               (none)
  
  以下是KingBase特有参数：
  -----------------------------------------------------------------------------
  statement_rollback     KB特有参数               默认：0，设置为1时，事务sql出错继续执行
  connect_timeout        链接超时                 默认：0
  retries                重试次数                 链接超时，重试次数
  delay                  重试延迟                 重试延迟5秒，与 retries配合使用
  fastswitch             快速切换                 off/on，主备切换参数，默认:off
  target_session_attrs   备用机器模式              read/write/read-write，参数设置备机可以读写 

注意：以上参数为链接参数，不同于options参数，不支持SQL命令设置，
...
Pg支持的options参数说明：
  ...
  用途                                     示例                                                          说明             
  -----------------------------------------------------------------------------------------------------------------------------
  设置自定义运行参数                         options='-c statement_timeout=5000'                           设置执行超时时间 5 秒
  指定 search_path                         options='-c search_path=myschema,public'                      默认 schema 搜索路径
  禁用某些特性                              options='-c enable_seqscan=off'                               调优时关闭顺序扫描
  调试日志                                 options='-c log_statement=all'                                 打印所有 SQL 语句到日志
  设置 work_mem / maintenance_work_mem     options='-c work_mem=16MB'                                    临时内存调整
  指定事务隔离级别                           options='-c default_transaction_isolation=serializable'       设置默认隔离级别
  设置 client_min_messages                 options='-c client_min_messages=WARNING'                      控制服务器消息级别

注意：

options参数可以通过SQL命令设置，比如：
...
service named "zephyr", you could use:
  ...
  $dbh = DBI->connect("dbi:KingBase:service=zephyr", '', '');

You could also set C<$ENV{PGSERVICE}> to "zephyr" and connect like this:

  $dbh = DBI->connect("dbi:KingBase:", '', '');
...
default socket directory from the one used to compile DBD::KingBase. 
Use the complete path to the socket directory as the name of the 
host, like this:

  $dbh = DBI->connect('dbi:KingBase:dbname=foo;host=/var/tmp/socket',
    $username,
    $password,
...
=head3 B<connect_cached>
  ...
  $dbh = DBI->connect_cached("dbi:KingBase:dbname=$dbname", $username, $password, \%options);

Implemented by DBI, no driver-specific impact.

=head3 B<data_sources>

  @data_sources = DBI->data_sources('KingBase');
...
an alternate port and host:
  ...
  @data_sources = DBI->data_sources('KingBase', 'port=5824;host=example.com');
...
C<KCIConnectionGetStatus>, which is a number used by I<libkci> (the Kingbase connection library). A value of 0 
indicates no error (CONNECTION_OK), while any other number indicates a failed connection. The 
only other number commonly seen is 1 (CONNECTION_BAD). See the libkci documentation for the 
complete list of return codes.

In all other non-connect methods C<< $h->err >> returns the C<KCIResultGetStatusCode> of the current
handle. This is a number used by libkci and is one of:
...
Returns the last error that was reported by Kingbase. This message is affected 
...
usually because the connection to the Kingbase server has been lost.
...
of them will be used by Kingbase.
...
flags that DBD::KingBase uses, see L<parse_trace_flag|/parse_trace_flag and parse_trace_flags>.
...
=head3 B<parse_trace_flag> and B<parse_trace_flags>
  ...
  $h->trace($h->parse_trace_flags('SQL|pglibkci'));
  $h->trace($h->parse_trace_flags('1|pgstart'));

  ## Simpler:
  $h->trace('SQL|pglibkci');
  $h->trace('1|pgstart');

  my $value = DBD::KingBase->parse_trace_flag('pglibkci');
  DBI->trace($value);
...
DBD::KingBase currently supports the DBI-specific flag, C<SQL>, 
...
the call to C<< DBD::KingBase->parse_trace_flags >> provides a number than can 
be fed to C<< DBI->trace >> before you create a database handle.

DBD::KingBase supports the following trace flags:
...
necessarily be in a form suitable to passing directly to Kingbase, 
as server-side prepared statements are used extensively by DBD::KingBase.
...
to DBD::KingBase (all those below which start with the letters 'pg').

=item pglibkci

Outputs the name of each libkci function (without arguments) immediately 
...
Outputs the name of each internal DBD::KingBase function, and other information such as 
...
Outputs a simple message at the very end of each internal DBD::KingBase function. This is also 
...
DBD::KingBase uses the C<func> method to support a variety of functions. 
...
since Kingbase version 8.1. For backwards compatibility, however, you should 
...
This is only available when DBD::KingBase is compiled against a Kingbase 
...
Returns a hash of all private attributes used by DBD::KingBase, for either 
...
Kingbase backend are only handled by one of the processes. If you don't set things up 
...
Supported by DBD::KingBase as proposed by DBI. This method is similar to the
...
WARNING: DBD::KingBase now (as of version 1.40) uses true prepared statements by sending them 
to the backend to be prepared by the Kingbase server. Statements 
that were legal before may no longer work. See below for details.

The prepare method prepares a statement for later execution. Kingbase supports 
prepared statements, which enables DBD::KingBase to only send the query once, and
simply send the arguments for every subsequent call to L</execute>.
DBD::KingBase can use these server-side prepared statements, or it can
...
Kingbase backend only have to prepare the query only once, but DBD::KingBase no
...
plan will probably work out well. Programs such as KingBaseBouncer which cache connections 
at a low level should not use prepared statements via DBD::KingBase, or must take 
extra care in the application to account for the fact that prepared statements 
are not shared across database connections. 

Only certain commands will be sent to a server-side prepare: currently these
include C<SELECT>, C<INSERT>, C<UPDATE>, and C<DELETE>. DBD::KingBase uses a simple
...
You can force DBD::KingBase to send your query directly to the server by adding
...
There are three types of placeholders that can be used in DBD::KingBase. The first is
...
that Kingbase uses internally and is overall probably the best method to use
if you do not need compatibility with other database systems. DBD::KingBase, like
...
syntax is supported by DBD::KingBase, its use is discouraged in favor of 
dollar-sign numbers.
...
Kingbase geometric operators and JSON operators) or array slices (e.g. C<data[100:300]>), 
there are methods to instruct DBD::KingBase to not treat some symbols as placeholders. First, you 
may simply add a backslash before the start of a placeholder, and DBD::KingBase will strip the 
backslash and not treat the character as a placeholder. 

You can also tell DBD::KingBase to ignore any non-dollar sign placeholders by setting the 
...
placeholders, you can tell DBD::KingBase to ignore just colon placeholders by setting
...
name with optional schema, and DBD::KingBase will attempt to find the sequence itself. 
The current value of the sequence is returned by a call to the C<CURRVAL()> 
Kingbase function. This will fail if the sequence has not yet been used in the 
current database connection.

If you do not know the name of the sequence, you can provide a table name and
DBD::KingBase will attempt to return the correct value. To do this, there must be at
...
looking up of things in the system table, so DBD::KingBase will cache the sequence
...
Disconnects from the Kingbase database. Any uncommitted changes will be rolled back upon disconnection. It's 
...
suitable for passing into Kingbase array columns.
...
exported by DBD::KingBase (such as PG_BYTEA). In addition to string, bytea, char, bool, 
...
PG_PATH, PG_POLYGON, and PG_CIRCLE respectively). To quote a Kingbase-specific 
...
Payloads will always be an empty string unless you are connecting to a Kingbase 
...
B<'DBD::KingBase ping test v3.10.7'>. It returns 0 (false) if the connection is not valid, 
...
This is a DBD::KingBase-specific extension to the L</ping> method. This will check the 
...
return the following:
  ...
  --------------------------------------------------
   ...
   -3      The test query failed (KCIStatementExecute returned null)
   -4      KCIConnectionGetStatus returned a CONNECTION_BAD
...
sent to Kingbase (via $dbh or $sth) will reset all the error information. Note that 
...
=item severity_nonlocal (only for Kingbase 10 and above)

=item state

=item primary

=item detail (does not work well for Kingbase < 9.2)
...
=item schema (only for Kingbase 9.3 and above)

=item table (only for Kingbase 9.3 and above)

=item column (only for Kingbase 9.3 and above)

=item type (only for Kingbase 9.3 and above)

=item constraint (only for Kingbase 9.3 and above)
...
This returns all items reserved by Kingbase but NOT reserved by SQL:2011 standard.
...
Four additional fields specific to DBD::KingBase are returned:
...
not used by DBD::KingBase. For information on the format of the standard rows returned, please 
see the DBI documentation.

L<DBI section on statistics_info|DBI/statistics_info>

In addition, the following Kingbase specific columns are returned:

=over 4

=item pg_expression

Kingbase allows indexes on functions and scalar expressions based on one or more columns. This field 
...
Kingbase typename and the SQL92 datatype (if possible) has been done
...
Writes debugging information from the Kingbase backend to a file. This is
...
Supported by DBD::KingBase as proposed by DBI. According to the classification of
DBI, Kingbase is a database in which a transaction must be explicitly
...
DBD::KingBase specific attribute. If true, boolean values will be returned
...
DBD::KingBase specific attribute. Indicates if DBD::KingBase should attempt to use server-side 
...
DBD::KingBase specific attribute. Indicates when DBD::KingBase will internally switch from using 
KCIStatementExecuteParams to KCIStatementExecutePrepared. In other words, when it will start using server-side 
...
time execute is called. To always use KCIStatementExecutePrepared instead of KCIStatementExecuteParams, set 
pg_switch_prepared to 1 (this was the default behavior in earlier versions). 
Setting pg_switch_prepared to 0 will force DBD::KingBase to always use KCIStatementExecuteParams.

=head3 B<pg_placeholder_dollaronly> (boolean)

DBD::KingBase specific attribute. Defaults to false. When true, question marks inside of statements 
...
DBD::KingBase specific attribute. Defaults to false. When true, colons inside of statements
...
DBD::KingBase specific attribute. The behavior of DBD::KingBase with regards to this flag has 
...
pg_enable_utf8 to -1 to force DBD::KingBase to read in the new client_encoding and 
act accordingly.

=head3 B<pg_errorlevel> (integer)

DBD::KingBase specific attribute. Sets the amount of information returned by the server's 
...
DBD::KingBase specific attribute. Indicates which version of Kingbase that 
DBD::KingBase was compiled against. In other words, which libraries were used. 
...
"dbi:KingBase:" part. Before version 2.0.0, this only returned the bare database name 
...
DBD::KingBase specific attribute. Returns the name of the current database.

=head3 B<pg_user> (string, read-only)

DBD::KingBase specific attribute. Returns the name of the user that
connected to the server.

=head3 B<pg_host> (string, read-only)

DBD::KingBase specific attribute. Returns the host of the current
server connection. Locally connected hosts will return an empty
string.

=head3 B<pg_port> (integer, read-only)

DBD::KingBase specific attribute. Returns the port of the connection to
the server.

=head3 B<pg_socket> (integer, read-only)

DBD::KingBase specific attribute. Returns the file description number of
the connection socket to the server.

=head3 B<pg_pass> (string, read-only)

DBD::KingBase specific attribute. Returns the password used to connect
to the server.

=head3 B<pg_options> (string, read-only)

DBD::KingBase specific attribute. Returns the command-line options passed
to the server. May be an empty string.

=head3 B<pg_default_port> (integer, read-only)

DBD::KingBase specific attribute. Returns the default port used if none is
specifically given.

=head3 B<pg_pid> (integer, read-only)

DBD::KingBase specific attribute. Returns the process id (PID) of the
backend server process handling the connection.

=head3 B<pg_prepare_now> (boolean)

DBD::KingBase specific attribute. Default is off. If true, then the L</prepare> method will 
immediately prepare commands, rather than waiting until the first execute.

=head3 B<pg_expand_array> (boolean)

DBD::KingBase specific attribute. Defaults to true. If false, arrays returned from the server will 
not be changed into a Perl arrayref, but remain as a string.

=head3 B<pg_async_status> (integer, read-only)

DBD::KingBase specific attribute. Returns the current status of an L<asynchronous|/Asynchronous Queries>
...
DBD::KingBase specific attribute. Returns true if the server is currently using 
...
DBD::KingBase specific attribute. Returns the version of the Kingbase server.
If DBD::KingBase is unable to figure out the version, it will return a "0". Otherwise,
a "3" is returned.
...
use one of the values provided by DBI or by DBD::KingBase. To use a SQL value,
...
To use Kingbase data types, import the list of values like this:

  use DBD::KingBase qw(:pg_types);

You can then set the data types by setting the value of the C<pg_type>
key in the hash passed to L</bind_param>.
The current list of Kingbase data types exported is:
...
data type to something else, DBD::KingBase will re-prepare the statement for you before
...
The "prepare/bind/execute" process has changed significantly for Kingbase
...
option will force server-side prepares off until such time as Kingbase 
supports using DEFAULT in prepared statements.

DBD::KingBase also supports passing in arrays to execute: simply pass in an arrayref, 
and DBD::KingBase will flatten it into a string suitable for input on the backend.

If you are using Kingbase version 8.2 or greater, you can also use any of the 
...
Supported by DBD::KingBase. This method is implemented by DBI but not
...
table can have only one blob (datatype LONG), Kingbase handles its blobs
...
See also the Kingbase-specific functions concerning blobs, which are
...
DBD::KingBase specific method. It returns Oid of table and position in table for
...
DBD::KingBase specific method. It returns array of original (or canonical) names
...
only be expressed by a Kingbase type. The value is the internal number corresponding to the type originally 
...
DBD::KingBase specific attribute. Returns the number of the tuple (row) that was
most recently fetched. Returns zero before and after fetching is performed.

=head3 B<pg_numbound> (integer, read-only)

DBD::KingBase specific attribute. Returns the number of placeholders
that are currently bound (via bind_param).

=head3 B<pg_bound> (hashref, read-only)

DBD::KingBase specific attribute. Returns a hash of all named placeholders. The
...
DBD::KingBase specific attribute. It returns a reference to an array of integer
...
DBD::KingBase specific attribute. It returns a reference to an array of strings
for each column. The string shows the name of the data_type.

=head3 B<pg_segments> (arrayref, read-only)

DBD::KingBase specific attribute. Returns an arrayref of the query split on the 
placeholders.

=head3 B<pg_oid_status> (integer, read-only)

DBD::KingBase specific attribute. It returns the OID of the last INSERT command.

=head3 B<pg_cmd_status> (integer, read-only)

DBD::KingBase specific attribute. It returns the type of the last
command. Possible types are: "INSERT", "DELETE", "UPDATE", "SELECT".

=head3 B<pg_direct> (boolean)

DBD::KingBase specific attribute. Default is false. If true, the query is passed 
directly to the backend without parsing for placeholders.

=head3 B<pg_prepare_now> (boolean)

DBD::KingBase specific attribute. Default is off. If true, the query will be immediately 
prepared, rather than waiting for the L</execute> call.

=head3 B<pg_prepare_name> (string)

DBD::KingBase specific attribute. Specifies the name of the prepared statement to use for this 
...
DBD::KingBase specific attribute. Defaults to false. When true, question marks inside of the query 
...
DBD::KingBase specific attribute. Indicates the current behavior for asynchronous queries. See the section 
...
Not used by DBD::KingBase. See the note about L</Cursors> elsewhere in this document.

=head1 FURTHER INFORMATION

=head2 Encoding

DBD::KingBase has extensive support for a client_encoding of UTF-8, and most 
...
Kingbase version 8.0 introduced the concept of savepoints, which allows 
transactions to be rolled back to a certain point without affecting the 
rest of the transaction. DBD::KingBase encourages using the following methods to 
control savepoints:

=head3 C<pg_savepoint>

Creates a savepoint. This will fail unless you are inside of a transaction. The 
only argument is the name of the savepoint. Note that Kingbase DOES allow 
...
There are currently three asynchronous constants automatically exported by DBD::KingBase.
...
so you may need to ROLLBACK or ROLLBACK TO a savepoint. As of version 2.17.0 of DBD::KingBase, rollbacks are 
...
long-running query.
  ...
  use DBD::KingBase ':async';

  my $dbh = DBI->connect('dbi:KingBase:dbname=kingbase', 'kingbase', '', {AutoCommit=>0,RaiseError=>1});
...
as you don't need it anymore.
  ...
  my $dbhslave1 = DBI->connect('dbi:KingBase:dbname=kingbase;host=slave1', 'kingbase', '', {AutoCommit=>0,RaiseError=>1});
  my $dbhslave2 = DBI->connect('dbi:KingBase:dbname=kingbase;host=slave2', 'kingbase', '', {AutoCommit=>0,RaiseError=>1});
...
DBD::KingBase allows arrays (as arrayrefs) to be passed in to both 
the L</quote> and the L</execute> methods. In both cases, the array is 
flattened into a string representing a Kingbase array.
...
DBD::KingBase allows for quick (bulk) reading and storing of data by using 
...
receive information via the L</pg_putcopydata> method.
...
=head2 Kingbase limits

For convenience, DBD::KingBase can export certain constants representing the limits of 
Kingbase data types. To use them, just add C<:pg_limits> when DBD::KingBase is used:

  use DBD::KingBase qw/:pg_limits/;
...
DBD::KingBase supports all largeobject functions provided by libkci via the
...
implementation. When DBD::KingBase was created, cursors in Kingbase could only be
...
The current implementation of Kingbase returns 't' for true and 'f' for
false. From the Perl point of view, this is a rather unfortunate
choice. DBD::KingBase therefore translates the result for the C<BOOL> data type in a
...
Boolean values can be passed to Kingbase as TRUE, 't', 'true', 'y', 'yes' or
'1' for true and FALSE, 'f', 'false', 'n', 'no' or '0' for false.

=head2 Schema support

The Kingbase schema concept may differ from those of other databases. In a nutshell,
a schema is a named collection of objects within a single database.

DBD::KingBase does not provide explicit support for Kingbase schemas.
...
L<The B<DBD::Pg> module|DBD::Pg>
L<The B<DBD::KB> module|DBD::KB>

=head1 BUGS

To report a bug, or view the current list of bugs, please visit 
http://rt.cpan.org/Public/Dist/Display.html?Name=DBD-KingBase

=head1 DEVELOPMENT

Patches can be submitted to rt.cpan.org. Detailed information on how to 
help out with this module can be found in the README.dev file.

=head1 AUTHORS

DBI by Tim Bunce L<http://www.tim.bunce.name>

The original DBD-KingBase was by Edmund Mergl (E.Mergl@bawue.de) and Jeffrey W. Baker
...
file (L<http://search.cpan.org/dist/DBD-KingBase/Changes>) for a complete list.
