...
Prty::Time::RFC822 (siehe Abschnitt L</EXAMPLES>). Fehlt die Angabe,
...
von Prty::Time::RFC822->get() in eine gültige RFC822-Datumsangabe
...
sub asString {
    ...
    if (defined $expires) {
        ...
        $str .= sprintf "; expires=%s",Prty::Time::RFC822->get($expires);
    }
    if ($secure) {
        $str .= '; secure';
    }
