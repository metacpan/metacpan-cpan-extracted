package PVTests::Standard_Array;
...
# derived from Standard.pm, but calls PVA::validate(@_, [ ... ] ) with square
# brackets to cause them to return args as an array.
...
sub run_tests {
    ...
    for my $test (@Tests) {
        if ( $test->{options} ) {
            ...
            # P::V::A can't handle per-package invocations, so skip
            next;
