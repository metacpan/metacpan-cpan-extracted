package Proc::ProcessTable::Match::Idle;
...
Proc::ProcessTable::Match::Idle - Attempts to match the kernel idle process.
...
=head1 SYNOPSIS
    ...
    my $checker=Proc::ProcessTable::Match::Idle->new;
    
    if ( $checker->match( $proc ) ){
        print "It matches.\n";
    }

This modules functions via checking that the process matches the following.
...
sub match{
	...
	my $proc_cmd;
	eval{
		$proc_cmd=$object->cmndline;
	};

	# don't bother proceeding, the object won't match ever
	# as it does not have a cmndline
	if ( ! defined( $proc_cmd ) ){
		return 0;
	}
