package Dimedis::SqlDriver::Sybase;
...
my $exc = "Dimedis::SqlDriver::Sybase:";	# Exception Prefix
...
sub db_join_cond {
	...
	my ($left_table, $left_alias) = split (/\s/, $left);
	$left_alias ||= $left_table;

	$cond =~ s/($left_alias\.[^\s]+)\s*=\s*(\w)/$1 *= $2/g;
 
#------ Diese Ersetzung erzeugt teilweise einen Syntaxfehler		
#	und zwar genau dann, wenn in der geouter jointen Tabelle
#	auchnoch Werte eingeschränkt werden
