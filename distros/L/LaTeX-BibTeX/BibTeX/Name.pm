...
LaTeX::BibTeX::Name - interface to BibTeX-style author names

=head1 SYNOPSIS

   $name = new LaTeX::BibTeX::Name;
   $name->split('J. Random Hacker');
   # or:
   $name = new LaTeX::BibTeX::Name ('J. Random Hacker');

   @firstname_tokens = $name->part ('first');
   $lastname = join (' ', $name->part ('last'));

   $format = new LaTeX::BibTeX::NameFormat;
   # ...customize $format...
   $formatted = $name->format ($format);

=head1 DESCRIPTION

C<LaTeX::BibTeX::Name> provides an abstraction for BibTeX-style names and
...
just a string.  Writing this example in actual code is simple:
   ...
   $name = new LaTeX::BibTeX::Name ("John Smith");  # or "Smith, John"
...
algorithm in L<LaTeX::BibTeX::BibSort> is a slavish imitiation of BibTeX
...
way is the job of another module: see L<LaTeX::BibTeX::NameFormat>.
...
Creates a new C<LaTeX::BibTeX::Name> object.  If NAME is supplied, it
...
the C<LaTeX::BibTeX::Name> object for later retrieval or formatting with
...
C<split>.  For example, suppose a C<LaTeX::BibTeX::Name> object is created and
...
should be a C<LaTeX::BibTeX::NameFormat> (or descendant) object.  (In short,
it must supply a method C<apply> which takes a C<LaTeX::BibTeX::NameFormat>
object as its only argument.)  Returns the formatted name as a string.

See L<LaTeX::BibTeX::NameFormat> for full details on formatting names.
...
L<LaTeX::BibTeX::Entry>, L<LaTeX::BibTeX::NameFormat>, L<bt_split_names>.
