package Pg::SQL::PrettyPrinter::Node::DeleteStmt;
...
sub new {
    ...
    $self->objectify(
        qw( returningList usingClause whereClause ),
        [ 'withClause', 'ctes' ],
    );
...
sub as_text {
    my $self     = shift;
    my @elements = ();
    push @elements, 'DELETE FROM';
    push @elements, $self->relname;
    if ( exists $self->{ 'usingClause' } ) {
        push @elements, 'USING';
        push @elements, join( ', ', map { $_->as_text } @{ $self->{ 'usingClause' } } );
    }
...
sub pretty_print {
    my $self  = shift;
    my @lines = ();
    push @lines, 'DELETE FROM ' . $self->relname;

    if ( exists $self->{ 'usingClause' } ) {
        push @lines, 'USING';
        push @lines, map { $self->increase_indent( $_->pretty_print ) . ',' } @{ $self->{ 'usingClause' } };
