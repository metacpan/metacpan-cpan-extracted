#!/usr/bin/perl
package SQL::QueryBuilder::Pretty::Database::ANSI::Punctuation;
...
sub action {
    ...
    if ( $token =~ /^,$/ ) {
        ...
        $print->indent_multiplier(1);
        $print->skip_next_space(1);
    }
    elsif ( $token =~ /^\)$/ ) {
        $print->new_line;
        $print->unindent( $print->indent_multiplier );
        $print->var($token);

        $print->indent_multiplier(1);
    }
    else {
        $self->SUPER::action($print, $token, @_);     
    }
...
SQL::QueryBuilder::Pretty::Database::ANSI::Punctuation - Rule to process ANSI 
punctuations.
...
Rule to process ANSI punctuations.
