...
int DistanceYN00(int is, int js, double*S, double*N, double*dS,double*dN,double *SEdS, double *SEdN, double *t,double space[]);
int GetKappa (void);
...
int DistanceF84(double n, double P, double Q, double pi[],double*k_HKY, double*t, double*SEt);
...
/* new variables */
int damage = 0;	// for exit(-1)
double *kaks_values;
/* leave program: Back to PERL!
 *	damage = 1;
 *	kaks_values[0] = -1;
 *	break;
 * return;
...
double *start(char seq_file[96], int ch_icode, int ch_ite, int ch_f)
{
   FILE *fseq;
   ...
   int num_kaks = 0;
	...
	com.fcommon=0;  
	com.kcommon=1;
	kaks_values = (double *)malloc(1*sizeof(double));

	/* parameters */
   strcpy(com.seqf, seq_file); //com.seqf = "abglobin.nuc";
	com.verbose = 0;
	com.icode = ch_icode;
	com.iteration = ch_ite;
	com.fcommon = ch_f;
   ...
   if((fseq=fopen (com.seqf,"r"))==NULL) 
	{  /* exit(-1); */
      printf ("\nSequence file %s not found!\n", com.seqf);
		damage = 1; kaks_values[0] = -1; return kaks_values;
   }
   ...
   for (idata=0; idata<com.ndata; idata++) 
	{
      if (com.ndata>1) 
		...
		/* remove the function of verbose */
      if ((damage = ReadSeq (NULL, fseq)) == 1)
		{
			kaks_values[0] = -1;
			return kaks_values;
		}
      ...
      if ((damage = EncodeSeqCodon())==1)   /* ls changed to ls/3 in this */
		{
			//kaks_values[0] = -1;
			//return kaks_values;
		}
      ...
      if(com.kcommon)	GetKappa();
		...
		/* new variable */
		/* need to allocate *kaks_values */
      num_kaks = com.ns * (com.ns-1) / 2 * 6 + 1;
		if ((kaks_values = (double*)malloc(num_kaks*sizeof(double))) == NULL) error2("out of memory");

		for (i = 0; i < num_kaks; i++)
			kaks_values[i] = .0;
      ...
      FOR(is,com.ns) 
		{
         FOR (js,is) 
            ...
            if(!com.kcommon) GetKappa();
				    // j
            if ((damage = DistanceYN00(is, js, &S, &N, &dS,&dN, &SEdS, &SEdN, &t,space))== 1)
				{	printf("! - Skip DistanceYN00\n");
					kaks_values[tmp_ct++] = 0;		
					kaks_values[tmp_ct++] = 0;		
					kaks_values[tmp_ct++] = 0; 	
					kaks_values[tmp_ct++] = 0;
					kaks_values[tmp_ct++] = 0;  
				...
				{	//printf("%7.1f %7.1f %8.4f %7.4f %7.4f %6.4f +- %6.4f %7.4f +- %6.4f\n",S,N,t,com.kappa,com.omega,dN,SEdN,dS,SEdS);
					/* return to cal_kaks */
					kaks_values[tmp_ct++] = S;		
					kaks_values[tmp_ct++] = N;		
					kaks_values[tmp_ct++] = dN; 	
					kaks_values[tmp_ct++] = SEdN;
					kaks_values[tmp_ct++] = dS;  
					kaks_values[tmp_ct++] = SEdS;
				}
         }    /* for (js) */
      }       /* for (is) */
		kaks_values[tmp_ct] = -1;
   }

	//if (damage == 1) kaks_values[0] = -1;
   return kaks_values;
}
...
*/
   ...
   for (ir=0; ir<(com.iteration?nround:1); ir++) {   /* iteration */
      ...
      if (damage = CountDiffs(com.z[is],com.z[js], &Sdts,&Sdtv,&Ndts,&Ndtv, PMat))
		{
			return (1);
		}
