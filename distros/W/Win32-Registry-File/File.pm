...
automatic conversion from native Perl to Windows registry data encoding
and vice versa.
...
registry formats.  For special situations you can use the
B<decode_reg_value> subroutine for manual decoding.
...
Constructor.  If a filename is supplied it will be opened as an .reg
...
Open the .reg file and read in all valid entries.  New entries will be
...
Save the current configuration into file in the .reg format.  Both
...
line that conforms to the .reg format.
...
to be treated as a registry file or .reg file.  If this flag is true
all subsequent file operations will work in registry file mode,
otherwise the .reg mode is used.  Normally this flag need not be altered
...
=item B<get(self, [ section[, key[, value]] ])>
