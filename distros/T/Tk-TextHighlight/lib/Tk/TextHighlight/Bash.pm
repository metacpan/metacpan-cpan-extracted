...
sub parseIString {
	...
	if ($text =~ s/^(`)//o) { #backticked
		$self->stackPush('Backticked');
		$self->snippetParse($1);
		return $text;
	}
	if ($text =~ s/^(")//o) { #string stop
		$self->snippetParse($1);
		$self->stackPull;
		return $text;
	}
	if ($text =~ s/^([^"|\$|`]+)//o) { #string content
		$self->snippetParse($1);
		return $text;
	}
	return $self->parserError($text);
...
sub parseText {
	...
	if ($text =~ s/^(")//o) { #string intrapolated
		$self->stackPush('String intrapolated');
		$self->snippetParse($1);
		return $text;
	}
	if ($text =~ s/^('[^']*)//o) { #string start
		$self->snippet($1);
	...
	if ($text =~ s/^([\|\||\||&&|&|;;|;|(|)])//o) { #operator
		$self->snippetParse($1, 'Operator');
		return $text
	}
	if ($text =~ s/^([<|>])//o) { #remaining separators
		$self->snippetParse($1);
		return $text
	}
	if ($text =~ s/^(\\.)//o) { #escaped character
