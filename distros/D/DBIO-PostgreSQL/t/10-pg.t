...
Set $ENV{DBIO_TEST_PG_DSN}, _USER and _PASS to run this test
...
'nonpkid_seq'. as well as following schemas: 'dbio_t_schema',
'dbio_t_schema_2', 'dbio_t_schema_3', 'dbio_t_schema_4', and 'dbio_t_schema_5')
EOM
...
###  pre-connect tests (keep each test separate as to make sure rebless() runs)
  {
    ...
    SKIP: {
        eval { require DateTime::Format::Pg; 1 }
          or skip 'DateTime::Format::Pg not installed', 2;
  ...
  {
    my $s = DBIO::Test::Schema->connect($dsn, $user, $pass);
    # make sure sqlt_type overrides work (DBIO::PostgreSQL::Storage does this)
    ok (!$s->storage->_dbh, 'definitely not connected');
...
### type_info tests
  ...
  my $type_info = $schema->storage->columns_info_for('dbio_t_schema.artist');
...
####### Array tests
  ...
  BEGIN {
    package DBIO::Test::Schema::ArrayTest;
    ...
    __PACKAGE__->table('dbio_t_schema.array_test');
...
########## Case check
  ...
  BEGIN {
    package DBIO::Test::Schema::Casecheck;
    push @main::test_classes, __PACKAGE__;
    ...
    __PACKAGE__->table('dbio_t_schema.casecheck');
...
## Test SELECT ... FOR UPDATE
  SKIP: {
      ...
      for my $t (
        ...
        # create a new schema
        my $schema2 = DBIO::Test::Schema->connect($dsn, $user, $pass);
        $schema2->source("Artist")->name("dbio_t_schema.artist");
...
######## other older Auto-pk tests
  ...
  $schema->source("SequenceTest")->name("dbio_t_schema.sequence_test");
  for (1..5) {
...
######## test non-serial auto-pk
  ...
  if ($schema->storage->_use_insert_returning) {
    $schema->source('TimestampPrimaryKey')->name('dbio_t_schema.timestamp_primary_key_test');
...
######## test with_deferred_fk_checks
  ...
  $schema->source('CD')->name('dbio_t_schema.cd');
  $schema->source('Track')->name('dbio_t_schema.track');
  lives_ok {
...
EOS
      ...
      $dbh->do("CREATE SCHEMA dbio_t_schema");
      $dbh->do("CREATE TABLE dbio_t_schema.artist $std_artist_table");

      $dbh->do(<<EOS);
CREATE TABLE dbio_t_schema.timestamp_primary_key_test (
  id timestamp default current_timestamp
)
EOS
      $dbh->do(<<EOS);
CREATE TABLE dbio_t_schema.cd (
...
EOS
      $dbh->do(<<EOS);
CREATE TABLE dbio_t_schema.track (
  trackid int,
  cd int REFERENCES dbio_t_schema.cd(cdid) DEFERRABLE,
  position int,
  title varchar(255),
...
EOS
      ...
      $dbh->do(<<EOS);
CREATE TABLE dbio_t_schema.sequence_test (
    pkid1 integer
    , pkid2 integer
    , nonpkid integer
    , name VARCHAR(100)
...
EOS
      ...
      $dbh->do(<<EOS);
CREATE TABLE dbio_t_schema.casecheck (
...
EOS
      $dbh->do(<<EOS);
CREATE TABLE dbio_t_schema.array_test (
    id serial PRIMARY KEY
    , arrayfield INTEGER[]
)
EOS
      $dbh->do("CREATE SCHEMA dbio_t_schema_2");
      $dbh->do("CREATE TABLE dbio_t_schema_2.artist $std_artist_table");
      $dbh->do("CREATE SCHEMA dbio_t_schema_3");
      $dbh->do("CREATE TABLE dbio_t_schema_3.artist $std_artist_table");
      $dbh->do('set search_path=dbio_t_schema,public');
      $dbh->do("CREATE SCHEMA dbio_t_schema_4");
      $dbh->do("CREATE SCHEMA dbio_t_schema_5");
      $dbh->do(<<EOS);
 CREATE TABLE dbio_t_schema_4.artist
 (
...
EOS
      $dbh->do('set search_path=public,dbio_t_schema,dbio_t_schema_3');
      $dbh->do('create sequence public.artist_artistid_seq'); #< in the public schema
      $dbh->do(<<EOS);
 CREATE TABLE dbio_t_schema_5.artist
 (
...
sub drop_test_schema {
    ...
    $schema->storage->dbh_do(sub {
        ...
        for my $stat (
                      'DROP SCHEMA dbio_t_schema_5 CASCADE',
                      'DROP SEQUENCE public.artist_artistid_seq CASCADE',
                      'DROP SCHEMA dbio_t_schema_4 CASCADE',
                      'DROP SCHEMA dbio_t_schema CASCADE',
                      'DROP SEQUENCE pkid1_seq CASCADE',
                      'DROP SEQUENCE pkid2_seq CASCADE',
                      'DROP SEQUENCE nonpkid_seq CASCADE',
                      'DROP SCHEMA dbio_t_schema_2 CASCADE',
                      'DROP SCHEMA dbio_t_schema_3 CASCADE',
                     ) {
            eval { $dbh->do ($stat) };
...
sub run_apk_tests {
    ...
    my @test_schemas = ( [qw| dbio_t_schema_2    1  |],
                         [qw| dbio_t_schema_3    1  |],
                         [qw| dbio_t_schema_4    2  |],
                         [qw| dbio_t_schema_5    1  |],
                       );
    foreach my $t ( @test_schemas ) {
        my ($sch_name, $start_num) = @$t;
        #test with dbio_t_schema_2
    ...
    lives_ok {
        apk_t_set($schema,'dbio_t_schema.artist');
...
my @eapk_id_columns;
BEGIN {
  package DBIO::Test::Schema::ExtAPK;
  push @main::test_classes, __PACKAGE__;
...
my @eapk_schemas;
BEGIN{ @eapk_schemas = map "dbio_apk_$_", 0..5 }
my %seqs; #< hash of schema.table.col => currval of its (DBIO) primary key sequence
...
# do a DBIO create on the apk table in the given schema number (which is an
