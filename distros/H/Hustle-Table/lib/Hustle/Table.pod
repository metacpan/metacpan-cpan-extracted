...
Entries are added in either anonymous hash or array format, using the C<add>
method.  Array entries must contain four elements, in the order of:
	
	[$matcher,$sub,$label,$count]

Hashes only need to specify the matcher and sub pairs

	{matcher=>$matcher, sub=>$sub, label=>$label, count=>$count}

In either case, C<$sub> is the only field/item that must be defined. It must
also be a CODE reference. 
...
Note the auto generated labels are only unique in the lifetime of a single
