...
for my $key_type ( keys %test_keys ) {
    eval {
        ...
        $ak_string = $options . q{ } . $test_keys{$key_type}->{key} . q{ } . $comment;
...
# named option method tests
{
    ...
    eval {
        ...
        is( $ak_entry->as_string,
            'no-agent-forwarding,from="127.0.0.1" ' . $test_keys{ssh_rsa_key}->{key},
