package Pcore::Lib::UUID;

use Pcore -export, -const;
use Pcore::Lib::UUID::Obj;
...
sub v1mc : prototype() {
    return bless { bin => v1mc_bin() }, 'Pcore::Lib::UUID::Obj';
}
...
sub v4 : prototype() {
    return bless { bin => $UUID_V4->() }, 'Pcore::Lib::UUID::Obj';
}
...
sub from_bin : prototype($) ($bin) {
    return bless { bin => $bin }, 'Pcore::Lib::UUID::Obj';
}

sub from_str : prototype($) ($str) {
    return bless { str => $str }, 'Pcore::Lib::UUID::Obj';
}

sub from_hex : prototype($) ($hex) {
    return bless { hex => $hex }, 'Pcore::Lib::UUID::Obj';
}
