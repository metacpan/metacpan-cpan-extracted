...
int te_identify_fn(const void *fn) {
    if (fn == (const void*)add) return TE_OP_ADD;
    if (fn == (const void*)sub) return TE_OP_SUB;
    if (fn == (const void*)mul) return TE_OP_MUL;
    if (fn == (const void*)divide) return TE_OP_DIV;
    if (fn == (const void*)negate) return TE_OP_NEGATE;
    if (fn == (const void*)comma) return TE_OP_COMMA;
    return TE_OP_UNKNOWN;
}
