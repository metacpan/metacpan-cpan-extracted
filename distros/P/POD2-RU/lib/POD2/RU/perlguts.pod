...
Для каждого типа имеются спецефические процедуры, которые манипулируют
своими типами данных.

=head2 Что такое "IV"?

Perl использует определитель типа (typedef) IV для определения обычного знакового integer,что 
...
которых всегда будет как минимум 32 и 16 бит, соответственно. (Аналогично есть 
U32 и U16.) Обычно эти типы равны точно 32 и 16 битам, но на ОС Crays они оба 
будут занимать 64 бита.

=head2 Работа с SV

SV можно создать и инициализировать  одной командой. ПОддерживается загрузка 
пяти типов значений: integer value  (IV), беззнаковый unsigned integer value (UV), double (NМ),
