package Pg::SQL::PrettyPrinter::Node::RangeVar;
...
sub as_text {
    my $self     = shift;
    my @elements = map { $self->quote_ident( $self->{ $_ } ) }
        grep { exists $self->{ $_ } } qw{ catalogname schemaname relname };
    my $full_name = join( '.', @elements );
