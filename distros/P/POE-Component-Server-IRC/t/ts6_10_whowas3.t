...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^bobbins' && $params->[1] eq '+i' ) {
    $poe_kernel->post( 'groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'WHOWAS', params => [ 'CowPig', '25', '9T9' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'ERROR' ) {
    pass($cmd);
...
sub harpo_input {
  ...
  if ( $cmd eq 'WHOWAS' ) {
    pass($cmd);
    is( $prefix, '7UPAAAAAA', 'Prefix is good' );
    is( $params->[0], 'CowPig', 'CowPig' );
    is( $params->[1], '25', 'Max is okay' );
    is( $params->[2], '9T9', 'Correct target' );
