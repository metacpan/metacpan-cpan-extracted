...
subtest 'viable' => sub {
    ok($CLASS->viable, "SharedMem is viable when IPC::SysV is available");
};

subtest 'spawn and unspawn' => sub {
    my $route = $CLASS->spawn(serializer => $SERIALIZER);
    ok($route, "spawn returns a route");
    like($route, qr/^\d+:\d+$/, "route matches shmid:semid format");

    my ($shm_id, $sem_id) = split /:/, $route;
    ok($shm_id =~ /^\d+$/, "shm_id is numeric");
    ok($sem_id =~ /^\d+$/, "sem_id is numeric");
...
subtest 'unspawn cleans up resources' => sub {
    my $route = $CLASS->spawn(serializer => $SERIALIZER);
    my ($shm_id, $sem_id) = split /:/, $route;

    $CLASS->unspawn($route);

    # Trying to read from the removed segment should fail
    my $buf = '';
    ok(!shmread($shm_id, $buf, 0, 4), "shared memory segment removed after unspawn");

    # Semaphore should also be gone
    require IPC::SysV;
    my $result = semctl($sem_id, 0, IPC::SysV::IPC_STAT(), $buf);
    ok(!$result, "semaphore removed after unspawn");
};
