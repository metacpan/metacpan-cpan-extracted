...
#pod
#pod If C<copyright_year> is not given, the current year is used.  If it is given,
#pod it is used as a string, so a range like C<2008 - 2012> is fine.  The string
#pod C<$this_year>, wherever it appears in C<copyright_year>, is replaced with the
#pod current year when the license is built, so C<2008 - $this_year> would become
#pod C<2008 - 2026> when built in 2026.
#pod
#pod =cut
