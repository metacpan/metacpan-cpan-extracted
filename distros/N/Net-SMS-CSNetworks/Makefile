# This Makefile is for the Net::SMS::CSNetworks extension to perl.
#
...
#     ABSTRACT => q[Send SMS messages via CS Networks HTTP API]
#     AUTHOR => [q[CS Networks <support@cs-networks.net>]]
#     BUILD_REQUIRES => { Test::More=>q[0] }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker=>q[6.30] }
#     DISTNAME => q[Net-SMS-CSNetworks]
#     EXE_FILES => []
#     LICENSE => q[bsd]
#     NAME => q[Net::SMS::CSNetworks]
#     PREREQ_PM => { Test::More=>q[0], LWP::UserAgent=>q[0], Data::Dumper=>q[0], Carp=>q[0], HTTP::Request::Common=>q[0] }
#     VERSION => q[0.07]
#     test => { TESTS=>q[t/*.t] }
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Net/SMS/CSNetworks.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Net/SMS
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Net/SMS
...
TO_INST_PM = lib/Net/SMS/CSNetworks.pm

PM_TO_BLIB = lib/Net/SMS/CSNetworks.pm \
	blib/lib/Net/SMS/CSNetworks.pm
...
DISTVNAME = Net-SMS-CSNetworks-0.07
...
manifypods : pure_all  \
	lib/Net/SMS/CSNetworks.pm
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/Net/SMS/CSNetworks.pm $(INST_MAN3DIR)/Net::SMS::CSNetworks.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               Net-SMS-CSNetworks' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.07' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           Send SMS messages via CS Networks HTTP API' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - CS Networks <support@cs-networks.net>' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Send SMS messages via CS Networks HTTP API</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>CS Networks &lt;support@cs-networks.net&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Net/SMS/CSNetworks.pm blib/lib/Net/SMS/CSNetworks.pm 
