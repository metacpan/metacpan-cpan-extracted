...
Calls L<sprinti()|String::Print/"FUNCTIONS"> to fill the C<%data> into C<$format>, and
then sends it to the C<$fh> (by default the selected file)
...
=item B<printp>( [$fh], $format, @params, %options )

Calls L<sprintp()|String::Print/"FUNCTIONS"> to fill the C<@params> in C<$format>, and
then sends it to the C<$fh> (by default the selected file)
...
=head4 POSIX modifier extensions '%[+ -]?[0-9]*[_,.]d'
...
=head3 Modifiers: YEAR, DATE, TIME, DT, and DT()
...
This module is part of String-Print version 0.96,
