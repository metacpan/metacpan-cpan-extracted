...
Important: If you want to implement file inclusion or exclusion in the style
of L<.gitignore|https://git-scm.com/docs/gitignore>, have a look at
L<File::Globstar::ListMatch> instead.
...
=item B<globstar PATTERN>

Return all files and directories matching B<PATTERN>.  Relative paths are
interpreted relatively to the current working directory.

An invalid B<PATTERN> matches nothing and B<globstar()> returns an empty
list for it.

The function I<only> expands the sequence "**" (at the appropriate) positions.
All other heavy lifting is done by L<File::Glob::bsd_glob()> (which is also
the backend for the standard Perl B<glob> operator <PATTERN>).

Only the (forward!) slash is accepted as a path separator!
...
supported option is "ignoreCase" at the moment.  As the name suggests, it
...
A character range or a negated character range marked by a leading
...
(according to the current locale).  See L<perlre> for a list of supported
character classes.
...
does not support them and in Perl regular expression character classes
...
=item B<*> If B<PATTERN> begins with an exclamation mark, the exclamation mark is discarded, and the B<PATTERN> is marked as negated (current "/src/**/.deps/").

=item B<*> If B<PATTERN> ends with a slash, the slash is discarded, and the B<PATTERN> is marked as a "directory match" (current "/src/**/.deps").

=item B<*> If B<PATTERN> begins with a slash, the slash is discarded, and the B<PATTERN> is marked as a "full path match" (current "src/**/.deps").

=item B<*> If B<PATTERN> is not already marked as a "full path match", every remaining slash will activate "full path match" (no modification in this last step).

=back

Note that the pre-processing step is skipped, if you pass a compiled regular
expression instead of a (scalar) string for B<PATTERN>.  And actually,
...
=item B<*> If full path mode is B<NOT> active (either because of a leading or innner slash in B<PATTERN>), a possible leading "directory portion" of B<STRING> is discarded.  The "directory portion" is everything until and including the last slash in B<STRING>.  The example string would have been truncated to just "Globstar" by now.

=back

The resulting B<STRING> is then compared to B<PATTERN>.  If it matches, and
B<PATTERN> was not negated, 1 is returned.  Likewise, if it does not match and
...
B<STRING> - that is the last slash and everything following it - is discarded,
...
This behaviour may change in the future.  The main reason why the backslash
...
=item B<UNIFORM BEHAVIOUR OF GLOBSTAR() AND FNMATCHSTAR()>
...
The behaviour of B<globstar()> completely depends on the behavior of
...
L<File::Glob>, L<File::Globstar::Listmatch>, L<perlre>, glob(3), glob(7),
fnmatch(3), glob(1), perl(1)
