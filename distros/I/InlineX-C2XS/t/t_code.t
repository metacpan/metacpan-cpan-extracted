...
if($ok) {
  if(!open(RD2, "expected_code.txt")) {
    warn "unable to open expected_code.txt for reading: $!\n";
    print "not ok 1\n";
    $ok = 0;
  }
}
...
if(!$ok2) {
  warn "testc.txt does not match expected_code.txt\n";
  print "not ok 1\n";
}
...
close(RD2) or warn "Unable to close expected_code.txt after reading: $!\n";
