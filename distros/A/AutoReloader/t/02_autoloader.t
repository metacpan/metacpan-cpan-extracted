...
package Foo;
require AutoReloader;
AutoReloader->import( 'AUTOLOAD' );
...
# to go wrong because AutoReloader used regular expressions to generate
# autoloaded filename.
'foo' =~ /(\w+)/;
...
# exception, including compilation error.  Now AutoReloader only
...
package Bar;
AutoReloader->import();
::ok( ! defined &AUTOLOAD, 'AutoReloader should not export AUTOLOAD by default' );
::ok( ! defined &can,      '... nor can()' );

package Foo;
AutoReloader->unimport();
eval { Foo->baz() };
...
::ok(1, "AutoReloader shouldn't loop forever if \%INC is modified");
