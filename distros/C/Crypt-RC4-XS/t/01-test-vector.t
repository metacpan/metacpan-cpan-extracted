...
$in  = "\x01\x23\x45\x67\x89\xab\xcd\xef";
$out = "\x75\xb7\x87\x80\x99\xe0\xc5\x96";
test_encrypt($key, $in, $out);

# Test vecotr 1
$key = "\x01\x23\x45\x67\x89\xab\xcd\xef";
$in  = "\x00\x00\x00\x00\x00\x00\x00\x00";
...
# Test vecotr 3
$key = "\xef\x01\x23\x45";
$in  = "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
$out = "\xd6\xa1\x41\xa7\xec\x3c\x38\xdf\xbd\x61";
test_encrypt($key, $in, $out);

# Test vector 4
$key = "\x01\x23\x45\x67\x89\xab\xcd\xef";
$in = "\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01".
...
"\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01".
"\x01";
$out = "\x75\x95\xc3\xe6\x11\x4a\x09\x78\x0c\x4a\xd4".
...
sub test_encrypt {
    my ($key, $in, $out) = @_;
    my $result = Crypt::RC4::XS::RC4($key, $in);
    ok($result eq $out);
    $result= Crypt::RC4::XS::RC4($key, $out);
    ok($result eq $in);
}
