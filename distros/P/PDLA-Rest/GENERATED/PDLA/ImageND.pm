...
sub PDLA::convolve{
    ...
    $c = PDLA->null if $#_<2;
    &PDLA::_convolve_int( $a->clump(-1), $b->clump(-1),
    ...
    foreach $i (0..$#odims) {
      ...
      # 1->n and n->1 separately for speed
         ...
         if (!($odims[$i] % $idims[$i])) {      # Cells map 1 -> n
             ...
             &PDLA::_rebin_int($a->mv($i,0), $b = null, $odims[$i]);
         }
         $a = $b->mv(0,$i);
      }
    }
...
sub circ_mean_p {
 ...
 if (defined $opt) {
   $rad = long PDLA::rvals($a,$opt);
 }
 else {
   $rad = long rvals $a;
 }
 $sum = zeroes($rad->max+1);
 PDLA::indadd $a->clump(-1), $rad->clump(-1), $sum; # this does the real work
...
sub PDLA::convolveND {
  ...
  if($fft) {
    ...
    &PDLA::_convolveND_int( $k->flat->index(0), $k, $aa, $a );
