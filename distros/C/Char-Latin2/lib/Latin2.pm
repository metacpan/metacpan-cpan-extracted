...
#
# http://search.cpan.org/dist/Char-Latin2/
#
...
END
    ...
    print Latin2::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Elatin2::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
    ...
    my $fh = gensym();
    Elatin2::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Latin2::escape_script {
    ...
    # read Latin-2 script
    my $fh = gensym();
    Elatin2::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Elatin2(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Elatin2 '%s.0'; # 'quote' for perl5.6\n", $Latin2::VERSION); # require run-time routines version

        # use Latin2 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Latin2 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Latin2::VERSION) {
                    my @file = grep -e, map {qq{$_/Latin2.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Latin2.pm $version, but @{[__FILE__]} is version $Latin2::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Elatin2.pm $version, but \\\$Elatin2::VERSION is \$Elatin2::VERSION" if \$Elatin2::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Latin2::VERSION) {
                    die "Script $0 required Latin2.pm $version, but @{[__FILE__]} is only version $Latin2::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Elatin2::function_ord     = 'Latin2::ord'     if /\A ord \z/oxms;
                    $Elatin2::function_ord_    = 'Latin2::ord_'    if /\A ord \z/oxms;
                    $Elatin2::function_reverse = 'Latin2::reverse' if /\A reverse \z/oxms;
                    $Elatin2::function_getc    = 'Latin2::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Latin2::escape();
...
=item * You'll have to download Latin2.pm and Elatin2.pm and put it in your perl lib directory.

=item * You'll need to write "use Latin2;" at head of the script.
...
Latin2 software is "middleware" between perl interpreter and your Perl script
written in Latin-2.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Latin-2, Perl3 or later
can treat Latin-2 as binary data.

By "use Latin2;", it automatically interpret your script as Latin-2. The various
functions of perl including a regular expression can treat Latin-2 now.
...
=item * Remains One Language Handling Raw Latin-2, Doesn't Use UTF8 flag like JPerl
...
import of Elatin2.pm works well.
...
=head1 Calling 'Elatin2::ignorecase()' (Latin2 software provides)
...
=head1 Escaping Character Classes (Elatin2.pm provides)
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Latin2 software provides)

Latin2 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Latin2 software provides)

Insert 'Elatin2::' at head of function name. Elatin2.pm provides your script Elatin2::*
subroutines.
...
=head1 Yada Yada Operator (Latin2 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Latin2 software provides)
...
some bugs. Thus, if you happen to find a bug that's in Latin2 software and
