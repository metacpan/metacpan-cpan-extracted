...
sub Big5::escape_script {
    ...
    # read Big5 script
    my $fh = gensym();
    Ebig5::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Ebig5(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ebig5 '%s.0'; # 'quote' for perl5.6\n", $Big5::VERSION); # require run-time routines version

        # use Big5 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Big5 ) (?>\s*) ([^\x81-\xFE;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Big5::VERSION) {
                    my @file = grep -e, map {qq{$_/Big5.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Big5.pm $version, but @{[__FILE__]} is version $Big5::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Ebig5.pm $version, but \\\$Ebig5::VERSION is \$Ebig5::VERSION" if \$Ebig5::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Big5::VERSION) {
                    die "Script $0 required Big5.pm $version, but @{[__FILE__]} is only version $Big5::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Ebig5::function_ord     = 'Big5::ord'     if /\A ord \z/oxms;
                    $Ebig5::function_ord_    = 'Big5::ord_'    if /\A ord \z/oxms;
                    $Ebig5::function_reverse = 'Big5::reverse' if /\A reverse \z/oxms;
                    $Ebig5::function_getc    = 'Big5::getc'    if /\A getc \z/oxms;

                    # avoid: used only once: possible typo at ...
                    $Ebig5::function_ord     = $Ebig5::function_ord;
                    $Ebig5::function_ord_    = $Ebig5::function_ord_;
                    $Ebig5::function_reverse = $Ebig5::function_reverse;
                    $Ebig5::function_getc    = $Ebig5::function_getc;
                }
            }
        }
    }

    $e_script .= Big5::escape();
...
=item * You'll have to download Big5.pm and Ebig5.pm and put it in your perl lib directory.

=item * You'll need to write "use Big5;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Big5;
  use Big5 ver.sion;             --- require minimum version
  use Big5 ver.sion.0;           --- expects version (match or die)
  use Big5 qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Big5 ver.sion qw(ord reverse getc);
  use Big5 ver.sion.0 qw(ord reverse getc);

  # "no Big5;" not supported

  or

  $ perl Big5.pm Big5_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Big5_script.pl  --- script written in Big5
...
Big5 software is "middleware" between perl interpreter and your Perl script
written in Big5.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Big5, Perl3 or later
can treat Big5 as binary data.

By "use Big5;", it automatically interpret your script as Big5. The various
functions of perl including a regular expression can treat Big5 now.
...
This software is a source code filter to escape Perl script encoded by Big5
...
=item * Remains One Language Handling Raw Big5, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Big5.pm, Ebig5.pm
  +---------------------------------------+
...
=head1 Software Composition
   ...
   Big5.pm               --- source code filter to escape Big5
   Ebig5.pm              --- run-time routines for Big5.pm
...
You need write 'use Big5;' in your script.
...
=head1 Escaping Multiple-Octet Code (Big5 software provides)
...
subroutines.
  ...
  lstat       Ebig5::lstat     Character
  opendir     Ebig5::opendir   Character
  stat        Ebig5::stat      Character
  unlink      Ebig5::unlink    Character
  chdir       Ebig5::chdir     Character
  do          Ebig5::do        Character
  require     Ebig5::require   Character
  -------------------------------------------

  ------------------------------------------------------------------------------------------------------------------------
  Before                   After
  ------------------------------------------------------------------------------------------------------------------------
  use Perl::Module;        BEGIN { Ebig5::require 'Perl/Module.pm'; Perl::Module->import() if Perl::Module->can('import'); }
  use Perl::Module @list;  BEGIN { Ebig5::require 'Perl/Module.pm'; Perl::Module->import(@list) if Perl::Module->can('import'); }
  use Perl::Module ();     BEGIN { Ebig5::require 'Perl/Module.pm'; }
  no Perl::Module;         BEGIN { Ebig5::require 'Perl/Module.pm'; Perl::Module->unimport() if Perl::Module->can('unimport'); }
  no Perl::Module @list;   BEGIN { Ebig5::require 'Perl/Module.pm'; Perl::Module->unimport(@list) if Perl::Module->can('unimport'); }
  no Perl::Module ();      BEGIN { Ebig5::require 'Perl/Module.pm'; }
  ------------------------------------------------------------------------------------------------------------------------

=head1 Escaping File Test Operators (Big5 software provides)

Insert 'Ebig5::' instead of '-' of operator.

  Available in MSWin32, MacOS, and UNIX-like systems
  --------------------------------------------------------------------------
  Before   After      Meaning
  --------------------------------------------------------------------------
  -r       Ebig5::r   File or directory is readable by this (effective) user or group
  -w       Ebig5::w   File or directory is writable by this (effective) user or group
  -e       Ebig5::e   File or directory name exists
  -x       Ebig5::x   File or directory is executable by this (effective) user or group
  -z       Ebig5::z   File exists and has zero size (always false for directories)
  -f       Ebig5::f   Entry is a plain file
  -d       Ebig5::d   Entry is a directory
  -t       -t         The filehandle is a TTY (as reported by the isatty() system function;
                      filenames can't be tested by this test)
  -T       Ebig5::T   File looks like a "text" file
  -B       Ebig5::B   File looks like a "binary" file
  -M       Ebig5::M   Modification age (measured in days)
  -A       Ebig5::A   Access age (measured in days)
  -C       Ebig5::C   Inode-modification age (measured in days)
  -s       Ebig5::s   File or directory exists and has nonzero size
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -R       Ebig5::R   File or directory is readable by this real user or group
  -W       Ebig5::W   File or directory is writable by this real user or group
  -X       Ebig5::X   File or directory is executable by this real user or group
  -l       Ebig5::l   Entry is a symbolic link
  -S       Ebig5::S   Entry is a socket
  --------------------------------------------------------------------------
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -o       Ebig5::o   File or directory is owned by this (effective) user
  -O       Ebig5::O   File or directory is owned by this real user
  -p       Ebig5::p   Entry is a named pipe (a "fifo")
  -b       Ebig5::b   Entry is a block-special file (like a mountable disk)
  -c       Ebig5::c   Entry is a character-special file (like an I/O device)
  -u       Ebig5::u   File or directory is setuid
  -g       Ebig5::g   File or directory is setgid
  -k       Ebig5::k   File or directory has the sticky bit set
  --------------------------------------------------------------------------
...
You need write 'Big5::' at head of function name when you want character-
oriented subroutine. See 'Character-Oriented Subroutines'.
  ...
  --------------------------------------------------------
  ord        Big5::ord
  reverse    Big5::reverse
  getc       Big5::getc
  length     Big5::length
  substr     Big5::substr
  index      Big5::index          See 'About Indexes'
  rindex     Big5::rindex         See 'About Rindexes'
  eval       Big5::eval
  --------------------------------------------------------
  ...
  (same as Ebig5::index)
  Big5::index    Character   Character    Character-oriented semantics
  ...
  (same as Ebig5::rindex)
  Big5::rindex   Character   Character    Character-oriented semantics
...
=item * Substr by Big5 Character
  ...
  If Perl version 5.14 or later, you can use the Big5::substr() subroutine as an
  ...
  end of the string, Big5::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Big5::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Big5::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Big5::substr $name, 7;      # returns undef, with warning
  Big5::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Big5::substr() as an lvalue is to specify the replacement
  ...
  Note that the lvalue returned by the three-argument version of Big5::substr() acts
...
=item * Index by Big5 Character

  $index = Big5::index($string,$substring,$offset);
  $index = Big5::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Big5::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }

=item * Rindex by Big5 Character

  $rindex = Big5::rindex($string,$substring,$offset);
  $rindex = Big5::rindex($string,$substring);

  This subroutine works just like Big5::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Big5::index).
  ...
  $pos = Big5::length($string);
  while (($pos = Big5::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }

=item * Eval Big5 Script

  $eval = Big5::eval { block };
  $eval = Big5::eval $expr;
  $eval = Big5::eval;

  The Big5::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Big5::eval { block }
  and Big5::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Big5::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Big5::eval, the value returned from an Big5::eval is
  ...
  Big5::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Big5::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Big5::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Big5::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Big5::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Big5::eval cannot trap syntax errors.

  The Big5::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Big5::eval is complete, as do any subroutine or format definitions.
  The code of the Big5::eval is treated as a block, so any locally scoped
  variables declared within the Big5::eval last only until the Big5::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Big5::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Big5::eval { block }    eval { block }
  Big5::eval $expr        eval Big5::escape $expr
  Big5::eval              eval Big5::escape
  -------------------------------------------------

  To tell the truth, the subroutine Big5::eval does not exist. If it exists,
  you will troubled, when Big5::eval has a parameter that is single quoted
  string included my variables. Big5::escape is a subroutine that makes Perl
  ...
  Since Big5::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Big5::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Big5::eval $expr and Big5::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Big5::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Big5::eval { $answer = }; # WRONG

      # a runtime syntax error
      Big5::eval '$answer =';   # sets $@
  ...
  Using the Big5::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Big5::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  With an Big5::eval, you should be especially careful to remember what's being
  looked at when:
  ...
  The block of Big5::eval { block } does not count as a loop, so the loop
...
=item * Filename Globbing
  ...
  This operator is implemented via the Ebig5::glob() subroutine. See Ebig5::glob
  of Ebig5.pm for details.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Ebig5.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Big5 software and
...
=item * Big5::substr as Lvalue

If Perl version is older than 5.14, Big5::substr differs from CORE::substr, and
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Ebig5::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${`}            Ebig5::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Ebig5::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Ebig5::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Ebig5::MATCH()       $1
  ${&}            Ebig5::MATCH()       $1
  $MATCH          Ebig5::MATCH()       $1
...
Big5 software remains one language and one interpreter.
