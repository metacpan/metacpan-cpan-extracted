# 3159mLT: Math::BaseConvert.pm created by Pip Stuart <Pip@CPAN.Org> to CoNVert between arbitrary number Bases.  I'm totally addicted to bass!
package Math::BaseConvert;
require Exporter;
use strict;
use warnings;
...
# only export cnv() for 'use Math::BaseConvert;' && all other stuff optionally
our @EXPORT      =             qw(cnv                                    )    ;
...
sub cnv__10 { # convert from some number base to decimal fast
  my $t = shift || '0'; my $s = shift || 64; my $n = Math::BigInt->new(0);
  $nega = ''; $nega = '-' if($t =~ s/^-//);
...
diginit(); # initialize the Dflt digit set whenever BaseConvert is used
127;
=head1 NAME

Math::BaseConvert - fast functions to CoNVert between number Bases

=head1 VERSION

This documentation refers to version 1.8 of Math::BaseConvert, which was released on Thu Apr 14 2016.
...
BaseConvert provides a few simple functions for converting between arbitrary number bases.  It is as fast as I currently know how to make it (of course
...
The reason I created BaseConvert was that I needed a simple way to convert quickly between the 3 number bases I use most (10, 16, && 64).  It turned out
...
BaseConvert can simply && realiably convert between any possible base between 2 && 64 (or 85).  I'm happy with it && employ b64() in places I probably
shouldn't now =).
...
cnv() is the only function that is exported from a normal 'use Math::BaseConvert;' command.  The other functions below can be imported to local namespaces
...
little as is necessary by default. So to be responsible, I have limited BaseConvert exporting to only cnv() under normal circumstances.  Please
...
I hope you find Math::BaseConvert useful.  Please feel free to e-mail me any suggestions or coding tips or notes of appreciation ("app-ree-see-ay-shun").
Thank you.  TTFN.
...
=item -    What else does BaseConvert need?
...
=item - 1.7 Tue May 29 2012

* forked from Math::BaseCnv to fix version number

=item - 1.1.6.A6FGHKE Tue Jun 15 16:17:20:14 2010
...
Forked from Math::BaseCnv which is (c) 2003-2007, Pip Stuart.
