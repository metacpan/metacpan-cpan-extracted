package QualysGuard::Response::IScannerList;
...
sub new {
    ...
    if ( $self->exists('/ISCANNER_LIST/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/ISCANNER_LIST/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/ISCANNER_LIST/ERROR');
...
QualysGuard::Response::IScannerList
