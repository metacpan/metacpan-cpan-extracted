...
sub try_apply_to {
  my $to = shift;
  eval { Jojo::Role->apply_role_to_package($to, 'MyRole'); 1 }
    and return undef;
  return $@ if $@;
...
eval {
  Jojo::Role->apply_role_to_package('IntermediaryRole', 'MyRole');
  Jojo::Role->apply_role_to_package('ExtraClass', 'IntermediaryRole');
  1;
} or $@ ||= "false exception!";
...
my $new_class;
eval {
    $new_class = Jojo::Role->create_class_with_roles('MyClass', 'ExtraRole');
} or $@ ||= "false exception!";
...
ok(Jojo::Role->is_role('MyRole'), 'is_role true for roles');
ok(!Jojo::Role->is_role('MyClass'), 'is_role false for classes');
