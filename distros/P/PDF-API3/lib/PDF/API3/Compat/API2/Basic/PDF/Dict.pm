...
#
#=======================================================================
package PDF::API3::Compat::API2::Basic::PDF::Dict;
...
PDF::API3::Compat::API2::Basic::PDF::Dict - PDF Dictionaries and Streams. Inherits from L<PDF::Objind>
...
sub outobjdeep
{
    ...
    if (defined $self->{' stream'} or defined $self->{' streamfile'} or defined $self->{' streamloc'})
    {
        ...
        {
            $self->{Length}||= PDF::API3::Compat::API2::Basic::PDF::Number->new(length($self->{' stream'}));
        }
        elsif($self->{'Filter'} || !defined $self->{' stream'})
        {
            $self->{'Length'} = PDF::API3::Compat::API2::Basic::PDF::Number->new(0) unless (defined $self->{'Length'});
            $pdf->new_obj($self->{'Length'}) unless ($self->{'Length'}->is_obj($pdf));
        } 
        else 
        {
            $self->{'Length'} = PDF::API3::Compat::API2::Basic::PDF::Number->new(length($self->{' stream'}));
            ## $self->{'Length'} = PDF::API3::Compat::API2::Basic::PDF::Number->new(length($self->{' stream'}) + 1);
    ...
    foreach ('Type', 'Subtype')
    {
        ...
        if (defined $self->{$_})
        {
            $fh->print('/'.PDF::API3::Compat::API2::Basic::PDF::Name::string_to_name($_).' ');
    ...
    while (($key, $val) = each %{$self})
    {
        ...
        next if (($val || '') eq '');
        $key = PDF::API3::Compat::API2::Basic::PDF::Name::string_to_name ($key, $pdf);
        $fh->print("/$key ");
            ...
            && defined $self->{'Filter'})
    {
        ...
        for ($i = 0; $i < scalar @{$self->{'Filter'}{' val'}}; $i++)
        {
            ...
            { $hasflate = -2; }
            $temp1 = "PDF::API3::Compat::API2::Basic::PDF::$temp";
...
sub read_stream
{
    ...
    if (defined $self->{'Filter'})
    {
        ...
        {
            my ($temp) = "PDF::API3::Compat::API2::Basic::PDF::" . $f->val;
            push(@filts, $temp->new());
        }
    }
