...
freeze(SV* self_sv)
    CODE:
        ...
        if (h->readonly || shm_is_fully_sealed(h)) croak("Data::HashMap::Shared::SI->freeze: map is frozen (read-only)");
...
set_multi(SV* self_sv, ...)
    CODE:
        ...
        if (h->shard_handles) {
            ...
            int _n = (items - 1) / 2;
            const char **_ks; STRLEN *_kl; bool *_ku; int64_t *_vs;
...
unlink(SV* self_or_class, ...)
    CODE:
        if (sv_isobject(self_or_class) && sv_derived_from(self_or_class, "Data::HashMap::Shared::SI")) {
            EXTRACT_MAP("Data::HashMap::Shared::SI", self_or_class);
            if (h->path) {
                SHM_TAINT_CHECK("Data::HashMap::Shared::SI->unlink");
                REEXTRACT_MAP("Data::HashMap::Shared::SI", self_or_class);
            }
            RETVAL = shm_unlink_sharded(h);
        } else {
            if (items < 2) croak("Usage: Data::HashMap::Shared::SI->unlink($path)");
            SvGETMAGIC(ST(1));
            RETVAL = shm_unlink_path(shm_write_path_arg(aTHX_ ST(1), "path", "Data::HashMap::Shared::SI", "Data::HashMap::Shared::SI->unlink"));
        }
    OUTPUT:
        RETVAL
