...
my $csv = Text::AutoCSV->new(in_file => "t/${ww}dates1-0.csv", fields_dates => ['D']);
my $d = $csv->_dds();
is_deeply($d,
	{ '.' => 1, 'D' => '%d/%m/%Y'},
...
$csv = Text::AutoCSV->new(in_file => "t/${ww}dates1-0.csv", fields_dates_auto => 1);
$d = $csv->_dds();
is_deeply($d,
	{'.' => 0, 'D' => '%d/%m/%Y', 'E' => 'N', 'F' => 'Z'},
...
$csv = Text::AutoCSV->new(in_file => "t/${ww}dates1-0.csv", fields_dates => ['D']);
$d = $csv->_dds();
is_deeply($d,
	{'.' => 1, 'D' => '%d/%m/%Y'},
...
eval {
	...
	$csv = Text::AutoCSV->new(in_file => "t/${ww}dates1-0.csv", fields_dates => ['E'],
		croak_if_error => 0);
	$d = $csv->_dds();
} or $w = -10;
is_deeply($d,
...
is($w, 2, "BA05 - check a warning got raised due to the format not found");

$w = 0;
eval {
	local $SIG{__WARN__} = sub { $w++ };
	$csv = Text::AutoCSV->new(in_file => "t/${ww}dates1-0.csv", fields_dates => ['D', 'E', 'F'],
		croak_if_error => 0);
	$d = $csv->_dds();
} or $w = -10;
is_deeply($d,
...
is($w, 3, "BA07 - check a warning got raised due to the format not found");
...
is($w, 102, "BA09 - check a warning got raised due to the format not found");

$w = 0;
eval {
	local $SIG{__WARN__} = sub { $w++ };
	$csv = Text::AutoCSV->new(in_file => "t/${ww}dates1-0.csv", fields_dates => ['D', 'E', 'BADNAME'],
		infoh => undef)->_dds();
} or $w += 100;
is($w, 101, "BA10 - check an error is raised if wrong field name given to fields_dates");
}

note("");
note("[SU]pplementary formats");

my $s = [ Text::AutoCSV->new(in_file => "t/${ww}dates1-1.csv", fields_dates_auto => 1)->_dds() ];
is_deeply($s,
	[{'.' => 2, 'A' => '%d/%m/%y', 'B' => 'N', 'C' => 'N', 'D' => 'N'}],
	"SU01 - t/dates1-1.csv': attribute dates_formats_to_try_supp (1)"
);

$s = [ Text::AutoCSV->new(in_file => "t/${ww}dates1-1.csv", fields_dates_auto => 1,
	dates_formats_to_try_supp => ['XX%d%m%yYY'])->_dds() ];
is_deeply($s,
	[{'.' => 2, 'A' => '%d/%m/%y', 'B' => 'XX%d%m%yYY', 'C' => 'N', 'D' => 'N'}],
	"SU02 - t/dates1-1.csv': attribute dates_formats_to_try_supp (2)"
);

$s = [ Text::AutoCSV->new(in_file => "t/${ww}dates1-1.csv", fields_dates_auto => 1,
	dates_formats_to_try_supp => ['XX%d%m%yYY', '%Y+%m+%d'])->_dds() ];
is_deeply($s,
	[{'.' => 2, 'A' => '%d/%m/%y', 'B' => 'XX%d%m%yYY', 'C' => 'N', 'D' => '%Y+%m+%d'}],
	"SU03 - t/dates1-1.csv': attribute dates_formats_to_try_supp (3)"
);

$s = [ Text::AutoCSV->new(in_file => "t/${ww}dates1-1.csv", fields_dates_auto => 1,
	dates_formats_to_try => ['XX%d%m%yYY', '%Y+%m+%d'])->_dds() ];
is_deeply($s,
	[{'.' => 1, 'A' => 'N', 'B' => 'XX%d%m%yYY', 'C' => 'N', 'D' => '%Y+%m+%d'}],
	"SU04 - t/dates1-1.csv': attribute dates_formats_to_try_supp (4)"
);

$s = [ Text::AutoCSV->new(in_file => "t/${ww}dates1-2.csv", fields_dates_auto => 1)->_dds() ];
is_deeply($s,
