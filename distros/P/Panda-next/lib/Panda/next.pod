...
C<Panda::next> replaces C<next::method>, C<maybe::next::method> and C<next::can> with very fast versions which are nearly the same speed as
C<SUPER::subname> calls and behave exactly as original methods. It replaces them globally even for modules that were loaded before C<Panda::next>.
...
C<Panda::next> introduces the new keyword <super::subname> which goes to the next method according to MRO of object's class
...
See L<Panda::Install> to find out how to make headers visible for your module.
...
See L<Panda::XS> for a more convenient way to call next method.
