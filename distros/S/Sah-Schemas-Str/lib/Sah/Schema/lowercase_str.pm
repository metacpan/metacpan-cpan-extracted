...
1;
# ABSTRACT: String containing only zero or more lowercase letters
...
Sah::Schema::lowercase_str - String containing only zero or more lowercase letters

=head1 VERSION

This document describes version 0.016 of Sah::Schema::lowercase_str (from Perl distribution Sah-Schemas-Str), released on 2023-10-23.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("lowercase_str*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("lowercase_str", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("lowercase_str", {return_type=>'str_errmsg+val'});
 my $res = $validator->($data); # [$errmsg, $validated_val]
 
 # a sample valid data
 $data = "12_3";
 my $res = $validator->($data); # => ["","12_3"]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("lowercase_str*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"lowercase_str*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('lowercase_str*', name=>'LowercaseStr')
     );
 }

 use My::Types qw(LowercaseStr);
 LowercaseStr->assert_valid($data);
