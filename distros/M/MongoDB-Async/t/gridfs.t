...
eval {
    ...
    $m = MongoDB::Async::MongoClient->new(host => $host, ssl => $ENV{MONGO_SSL});
};
...
is($file->{'chunkSize'}, $MongoDB::Async::GridFS::chunk_size);
...
isa_ok($file, 'MongoDB::Async::GridFS::File');
...
for (my $i=0; $i<3; $i++) {
    isa_ok($list[$i], 'MongoDB::Async::GridFS::File');
}
