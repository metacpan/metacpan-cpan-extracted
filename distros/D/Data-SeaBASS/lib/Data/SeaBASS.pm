...
To read SeaBASS files:
    ...
    use Data::SeaBASS qw(STRICT_READ STRICT_WRITE INSERT_BEGINNING INSERT_END);
...
C<Data::SeaBASS> provides an easy to use, object-oriented interface for
...
Creates a C<Data::SeaBASS> object.  If the file specified exists, the object
...
C<add_headers> is used to add or override metadata for a C<Data::SeaBASS>, as
...
#						print '/', $header, '=', $self->{'headers'}{"$slash$header"}, "\n";
						...
						next if ($OMIT_EMPTY_HEADERS{$header});

						my $value = $HEADER_DEFAULTS{$header} || $add_missing_headers;
						if ( $header =~ /_latitude|_longitude/ ) {
							print "/$header=$value\[deg]\n";
						} elsif ( $header =~ /_time/ ) {
							print "/$header=$value\[gmt]\n";
						} else {
							print "/$header=$value\n";
						}
					}
