...
=head2 Command Line Usage (Syntax check and run)
...
=head3 CLI Syntax Checking Options
...
B<From v0.3.0:> In the forth form (4), an explicit single argument of C<undef>,
...
C<Error::Show-E<gt>context(...)> if prefered, with the same arguemtn handling
as the other forms.
...
=item 1. String Errors (perl errors)

Error string, as per C<die> and C<warn>, containing file and line number. These
...
An reference to an array of call frames in C<caller()> or
C<Devel::StackTrace::Frame> format can also be supplied as the error or frames.
Again the C<message> option needs to be provided if error string is required in
the output.
...
The most reliable way is usually to explicitly set the B<message> and B<frames>
options explicitly. This works with a single frame (for the latest exception)
or ref to array, for a complete trace

=head3 Mojo::Exception

B<FYI:> L<Mojo::Exception> does provide it's own facility to show the code
...
=head3 Syntax Checking String C<eval>, Without Execution

A block eval will have it's syntax checked during normal compilation time. A
string eval is checked at run time and it uses the same variable C<$@> to
...
To work around these limitations, and have C<Error::Show> still provide context
information, start by wrapping your eval string with "sub { ... }.  The eval
result will be code ref if syntactically correct. Otherwise the error in C<$@>
is the syntax error string, which can be feed directly into C<Error::Show>.
The code ref is executed in a block eval, which if dies from an exception will
place the runtime error in C<$@>, which again can be used in C<Error::Show>.
As an example:
