...
sub AUTOLOAD {
    ...
    croak "&Net::CUPS::IPP::constant not defined" if $constname eq 'constant';
...
sub new
{
	...
	if( defined( $request ) && ( $request ne "" ) )
	{
		return( NETCUPS_newIPPRequest( $request ) );
	}
	else
	{
		return( NETCUPS_newIPP() );
	}
}
...
sub addString
{
	my( $self, $group, $type, $name, $charset, $value ) = @_;

	$charset = "" if( !defined( $charset ) );

	NETCUPS_addString( $self, $group, $type, $name, $charset, $value );
...
sub getAttributes
{
	...
	return( NETCUPS_getAttributes( $self ) );
}
...
sub getAttributeValue
{
	...
	return( NETCUPS_getAttributeValue( $self, $name ) );
}
...
L<Net::CUPS>, L<Net::CUPS::PPD>, L<Net::CUPS::Destination>
