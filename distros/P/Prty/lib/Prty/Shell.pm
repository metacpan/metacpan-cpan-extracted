...
sub exec {
    ...
    unless ($dryRun) {
        ...
        if ($log && $self->{'time'}) {
            ...
            # printf $fd "/* %.2f */\n",$t1-$t0;
            printf "%s%s\n",$self->{'timePrefix'},
                Prty::Converter->epochToDuration($t1-$t0,1,3);
        }
        if ($except) {
    ...
    if ($capture) {
        if ($capture eq 'stderr+stdout') {
            my $stdout = Prty::Path->read($stdoutFile,-delete=>1);
            my $stderr = Prty::Path->read($stderrFile,-delete=>1);
