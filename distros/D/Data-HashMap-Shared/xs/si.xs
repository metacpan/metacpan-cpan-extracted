...
new(char* class, SV* path_sv, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        ...
        char errbuf[SHM_ERR_BUFLEN]; const char* path = SHM_PATH_ARG(path_sv, "path", "Data::HashMap::Shared::SI"); ShmHandle* map = shm_si_create(path, (uint32_t)max_entries, (uint32_t)lru_max, (uint32_t)ttl_default, (uint32_t)lru_skip, (uint64_t)arena_cap, (mode_t)file_mode, errbuf);
        if (!map) croak("Data::HashMap::Shared::SI: %s", errbuf[0] ? errbuf : "unknown error");
        CK_MAX_SIZE(map, "Data::HashMap::Shared::SI");
...
new_sharded(char* class, SV* path_prefix_sv, UV num_shards, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        const char* path_prefix = SHM_PATH_ARG(path_prefix_sv, "path prefix", "Data::HashMap::Shared::SI");
        CK_U32(max_entries, "max_entries", "Data::HashMap::Shared::SI"); CK_U32(lru_max, "lru_max", "Data::HashMap::Shared::SI"); CK_U32(ttl_default, "ttl_default", "Data::HashMap::Shared::SI"); CK_U32(lru_skip, "lru_skip", "Data::HashMap::Shared::SI");
...
new_readonly(char* class, SV* path_sv)
    CODE:
        ...
        ShmHandle* map = shm_si_open_readonly(path, errbuf);
        if (!map) croak("Data::HashMap::Shared::SI: %s", errbuf[0] ? errbuf : "unknown error");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)map);
    OUTPUT:
        RETVAL
...
freeze(SV* self_sv)
    CODE:
        ...
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI->freeze: cannot freeze a read-only handle");
        if (shm_freeze(h) != 0) croak("Data::HashMap::Shared::SI->freeze: msync: %s", strerror(errno));
        shm_mark_readonly(h);
...
DESTROY(SV* self_sv)
    CODE:
        if (!sv_isobject(self_sv) || !sv_derived_from(self_sv, "Data::HashMap::Shared::SI")) return;
...
SV*
cas_take(SV* self_sv, SV* key_sv, int64_t expected)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
...
SV*
max(SV* self_sv, SV* key_sv, int64_t desired)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
...
SV*
min(SV* self_sv, SV* key_sv, int64_t desired)
    CODE:
        EXTRACT_MAP("Data::HashMap::Shared::SI", self_sv);
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::SI: map is frozen (read-only)");
        EXTRACT_STR_KEY(key_sv);
...
unlink(SV* self_or_class, ...)
    CODE:
        if (sv_isobject(self_or_class) && sv_derived_from(self_or_class, "Data::HashMap::Shared::SI")) {
            ...
            RETVAL = shm_unlink_path(shm_path_arg(aTHX_ ST(1), "path", "Data::HashMap::Shared::SI"));
        }
    OUTPUT:
        RETVAL
...
DESTROY(SV* self_sv)
    CODE:
        if (!sv_isobject(self_sv) || !sv_derived_from(self_sv, "Data::HashMap::Shared::SI::Cursor")) return;
