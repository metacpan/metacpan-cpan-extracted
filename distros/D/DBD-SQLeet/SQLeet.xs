...
MODULE = DBD::SQLeet          PACKAGE = DBD::SQLeet::db

PROTOTYPES: DISABLE

BOOT:
    init_cxt();
    sv_setpv(get_sv("DBD::SQLeet::sqlite_version",        TRUE|GV_ADDMULTI), SQLITE_VERSION);
    sv_setiv(get_sv("DBD::SQLeet::sqlite_version_number", TRUE|GV_ADDMULTI), SQLITE_VERSION_NUMBER);
...
last_insert_rowid(dbh)
    SV *dbh
    ALIAS:
        DBD::SQLeet::db::sqlite_last_insert_rowid = 1
    CODE:
    {
        D_imp_dbh(dbh);
...
create_function(dbh, name, argc, func, flags = 0)
    ...
    int flags
    ALIAS:
        DBD::SQLeet::db::sqlite_create_function = 1
    CODE:
    {
...
enable_load_extension(dbh, onoff)
    ...
    int onoff
    ALIAS:
        DBD::SQLeet::db::sqlite_enable_load_extension = 1
    CODE:
    {
...
load_extension(dbh, file, proc = 0)
    ...
    const char *proc
    ALIAS:
        DBD::SQLeet::db::sqlite_load_extension = 1
    CODE:
    {
...
create_aggregate(dbh, name, argc, aggr, flags = 0)
    ...
    int flags
    ALIAS:
        DBD::SQLeet::db::sqlite_create_aggregate = 1
    CODE:
    {
...
create_collation(dbh, name, func)
    ...
    SV *func
    ALIAS:
        DBD::SQLeet::db::sqlite_create_collation = 1
    CODE:
    {
...
collation_needed(dbh, callback)
    ...
    SV *callback
    ALIAS:
        DBD::SQLeet::db::sqlite_collation_needed = 1
    CODE:
    {
...
progress_handler(dbh, n_opcodes, handler)
    ...
    SV *handler
    ALIAS:
        DBD::SQLeet::db::sqlite_progress_handler = 1
    CODE:
    {
...
profile(dbh, callback)
    ...
    SV *callback
    ALIAS:
        DBD::SQLeet::db::sqlite_profile = 1
    CODE:
    {
...
commit_hook(dbh, hook)
    ...
    SV *hook
    ALIAS:
        DBD::SQLeet::db::sqlite_commit_hook = 1
    CODE:
    {
...
rollback_hook(dbh, hook)
    ...
    SV *hook
    ALIAS:
        DBD::SQLeet::db::sqlite_rollback_hook = 1
    CODE:
    {
...
update_hook(dbh, hook)
    ...
    SV *hook
    ALIAS:
        DBD::SQLeet::db::sqlite_update_hook = 1
    CODE:
    {
...
backup_to_file(dbh, filename)
    ...
    char *filename
    ALIAS:
        DBD::SQLeet::db::sqlite_backup_to_file = 1
    CODE:
...
table_column_metadata(dbh, dbname, tablename, columnname)
    ...
    SV* columnname
    ALIAS:
        DBD::SQLeet::db::sqlite_table_column_metadata = 1
    CODE:
...
db_filename(dbh)
    SV* dbh
    ALIAS:
        DBD::SQLeet::db::sqlite_db_filename = 1
    CODE:
...
register_fts3_perl_tokenizer(dbh)
    SV *dbh
    ALIAS:
        DBD::SQLeet::db::sqlite_register_fts3_perl_tokenizer = 1
    CODE:
...
db_status(dbh, reset = 0)
    ...
    int reset
    ALIAS:
        DBD::SQLeet::db::sqlite_db_status = 1
    CODE:
...
create_module(dbh, name, perl_class)
    ...
    char *perl_class
    ALIAS:
        DBD::SQLeet::db::sqlite_create_module = 1
    CODE:
    {
...
MODULE = DBD::SQLeet          PACKAGE = DBD::SQLeet::st
...
st_status(sth, reset = 0)
    ...
    int reset
    ALIAS:
        DBD::SQLeet::st::sqlite_st_status = 1
    CODE:
        RETVAL = (HV*)_sqlite_st_status(aTHX_ sth, reset);
    OUTPUT:
        RETVAL

MODULE = DBD::SQLeet          PACKAGE = DBD::SQLeet
