# PANT::Svn - Provide support for Svn operations
...
sub HasUpdate {
	...
	#Restored
	next if ($line =~ /^At /);
	# treat these two as updates
	return 1 if ($line =~ /^Updated to /);
	return 1 if ($line =~ /^Restored /);
	if ($line =~ /^[ADUM][A-Z]?\s+/) { # Its a change, one out, all out.
	    return 1;
	}
    }
    return 0;
...
sub HasConflict {
    my $self = shift;
    foreach my $line (@{ $self->{lines} }) {
	if ($line =~ /^[C][A-Z]?\s+/) { # Its a conflict
	    return 1;
	}
    }
    return 0;
...
is designed to provide support for svn. Most svn operations can
...
construction. The PANT function SVN calls this constructor with the
current xml stream. So normally
you would call it via the accessor.

=head2 Run(command)

This command will run the given svn command, and will collect
