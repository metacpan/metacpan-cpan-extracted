...
This example is in F<examples/synopsis.pl> in the distribution.


=head1 VERSION

This document describes Image::PNG::Libpng version 0.44,
corresponding to git commit 430def21322365ef035c98689e28ac74526c3b7b at Wed Jun 28 15:10:26 2017 +0900.
...
use the "libpng" library for reading and writing files in the PNG
...
mirrors the C interface of libpng.
...
This example is in F<examples/get-www-png.pl> in the distribution.
...
This example is in F<examples/png-cgi.pl> in the distribution.
...
This example is in F<examples/copy-png.pl> in the distribution.
...
This example is in F<examples/color-type-name.pl> in the distribution.
...
Alias for get_image_width. This is not exported, it's intended for use
with the object only.
...
Alias for get_image_height. This is not exported, it's intended for use
with the object only.
...
Get the number of channels, from one to four. The return value is 1
for colour type PNG_COLOR_TYPE_GRAY and PNG_COLOR_TYPE_PALETTE, 2 for
PNG_COLOR_TYPE_GRAY_ALPHA, 3 for PNG_COLOR_TYPE_RGB and 4 for
PNG_COLOR_TYPE_RGB_ALPHA or PNG_COLOR_TYPE_RGB with a filler byte.
...
height of the PNG image. Each row consists of the actual binary
data. Pixel values must be extracted using a function such as
C<substr> or C<unpack>. This binary data is likely to contain bytes
equal to zero.
...
chunks do not contain a time zone: according to the PNG specification,
...
PNG_TEXT_COMPRESSION_NONE is applied. The compression field can take the following values:
...
Provisionally, see L</set_sPLT> for documentation, the return value is
...
prints a message describing the first difference found, for example
the height is different, or row 0 of the image data is different,
etc. This function is used in testing this module.
...
This is allows XS routines to directly set the value of the row
pointers for the PNG image. The memory is copied so whatever is in the
row pointers can be freed immediately after calling this. However, the
...
should contain a pointer to memory stored as an SvIV allocated with
Newx or a similar function. This transfers ownership of the memory to
C<$png>, which will free it with Safefree when C<$png> is
...
you can find each instance L<in the file C<perl-libpng.c> in the top directory of the distribution|http://cpansearch.perl.org/src/BKB/Image-PNG-Libpng-0.44/perl-libpng.c> in the macro
C<UNUSED_ZERO_ARG>.
