...
######################################################################
#
# Hebrew - Source code filter to escape Hebrew script
#
# http://search.cpan.org/dist/Char-Hebrew/
#
...
perl $0 Hebrew_script.pl > Escaped_script.pl.e
END
    }

    print Hebrew::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Ehebrew::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
    ...
    my $fh = gensym();
    Ehebrew::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Hebrew::escape_script {
    ...
    # read Hebrew script
    my $fh = gensym();
    Ehebrew::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Ehebrew(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ehebrew '%s.0'; # 'quote' for perl5.6\n", $Hebrew::VERSION); # require run-time routines version

        # use Hebrew version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Hebrew ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Hebrew::VERSION) {
                    my @file = grep -e, map {qq{$_/Hebrew.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Hebrew.pm $version, but @{[__FILE__]} is version $Hebrew::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Ehebrew.pm $version, but \\\$Ehebrew::VERSION is \$Ehebrew::VERSION" if \$Ehebrew::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Hebrew::VERSION) {
                    die "Script $0 required Hebrew.pm $version, but @{[__FILE__]} is only version $Hebrew::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Ehebrew::function_ord     = 'Hebrew::ord'     if /\A ord \z/oxms;
                    $Ehebrew::function_ord_    = 'Hebrew::ord_'    if /\A ord \z/oxms;
                    $Ehebrew::function_reverse = 'Hebrew::reverse' if /\A reverse \z/oxms;
                    $Ehebrew::function_getc    = 'Hebrew::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Hebrew::escape();
...
=item * You'll have to download Hebrew.pm and Ehebrew.pm and put it in your perl lib directory.

=item * You'll need to write "use Hebrew;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Hebrew;
  use Hebrew ver.sion;             --- require minimum version
  use Hebrew ver.sion.0;           --- expects version (match or die)

  # "no Hebrew;" not supported

  or

  $ perl Hebrew.pm Hebrew_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Hebrew_script.pl  --- script written in Hebrew
...
Hebrew software is "middleware" between perl interpreter and your Perl script
written in Hebrew.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Hebrew, Perl3 or later
can treat Hebrew as binary data.

By "use Hebrew;", it automatically interpret your script as Hebrew. The various
functions of perl including a regular expression can treat Hebrew now.
...
This software is a source code filter to escape Perl script encoded by Hebrew
...
=item * Remains One Language Handling Raw Hebrew, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Hebrew.pm, Ehebrew.pm
  +---------------------------------------+
...
import of Ehebrew.pm works well.
...
=head1 Software Composition
   ...
   Hebrew.pm               --- source code filter to escape Hebrew
   Ehebrew.pm              --- run-time routines for Hebrew.pm
...
You need write 'use Hebrew;' in your script.
...
=head1 Calling 'Ehebrew::ignorecase()' (Hebrew software provides)

Hebrew software applies calling 'Ehebrew::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Ehebrew::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Ehebrew.pm provides)
...
Also POSIX-style character classes.
  ...
  [:^alnum:]    ${Ehebrew::not_alnum}
  [:^alpha:]    ${Ehebrew::not_alpha}
  [:^ascii:]    ${Ehebrew::not_ascii}
  [:^blank:]    ${Ehebrew::not_blank}
  [:^cntrl:]    ${Ehebrew::not_cntrl}
  [:^digit:]    ${Ehebrew::not_digit}
  [:^graph:]    ${Ehebrew::not_graph}
  [:^lower:]    ${Ehebrew::not_lower}
                ${Ehebrew::not_lower_i}    (/i modifier)
  [:^print:]    ${Ehebrew::not_print}
  [:^punct:]    ${Ehebrew::not_punct}
  [:^space:]    ${Ehebrew::not_space}
  [:^upper:]    ${Ehebrew::not_upper}
                ${Ehebrew::not_upper_i}    (/i modifier)
  [:^word:]     ${Ehebrew::not_word}
  [:^xdigit:]   ${Ehebrew::not_xdigit}
  ---------------------------------------------------------------
...
Definitions in Ehebrew.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Ehebrew::dot}            qr{(?>[^\x0A])};
  ${Ehebrew::dot_s}          qr{(?>[\x00-\xFF])};
  ${Ehebrew::eD}             qr{(?>[^0-9])};
  ${Ehebrew::eS}             qr{(?>[^\s])};
  ${Ehebrew::eW}             qr{(?>[^0-9A-Z_a-z])};
  ${Ehebrew::eH}             qr{(?>[^\x09\x20])};
  ${Ehebrew::eV}             qr{(?>[^\x0A\x0B\x0C\x0D])};
  ${Ehebrew::eR}             qr{(?>\x0D\x0A|[\x0A\x0D])};
  ${Ehebrew::eN}             qr{(?>[^\x0A])};
  ${Ehebrew::not_alnum}      qr{(?>[^\x30-\x39\x41-\x5A\x61-\x7A])};
  ${Ehebrew::not_alpha}      qr{(?>[^\x41-\x5A\x61-\x7A])};
  ${Ehebrew::not_ascii}      qr{(?>[^\x00-\x7F])};
  ${Ehebrew::not_blank}      qr{(?>[^\x09\x20])};
  ${Ehebrew::not_cntrl}      qr{(?>[^\x00-\x1F\x7F])};
  ${Ehebrew::not_digit}      qr{(?>[^\x30-\x39])};
  ${Ehebrew::not_graph}      qr{(?>[^\x21-\x7F])};
  ${Ehebrew::not_lower}      qr{(?>[^\x61-\x7A])};
  ${Ehebrew::not_lower_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Ehebrew::not_lower_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
  ${Ehebrew::not_print}      qr{(?>[^\x20-\x7F])};
  ${Ehebrew::not_punct}      qr{(?>[^\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E])};
  ${Ehebrew::not_space}      qr{(?>[^\s\x0B])};
  ${Ehebrew::not_upper}      qr{(?>[^\x41-\x5A])};
  ${Ehebrew::not_upper_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Ehebrew::not_upper_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
  ${Ehebrew::not_word}       qr{(?>[^\x30-\x39\x41-\x5A\x5F\x61-\x7A])};
  ${Ehebrew::not_xdigit}     qr{(?>[^\x30-\x39\x41-\x46\x61-\x66])};
  
  # This solution is not perfect. I beg better solution from you who are reading this.
  ${Ehebrew::eb}             qr{(?:\A(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[0-9A-Z_a-z])|(?<=[0-9A-Z_a-z])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]|\z))};
  ${Ehebrew::eB}             qr{(?:(?<=[0-9A-Z_a-z])(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]))};
  ---------------------------------------------------------------------------------------------------------------------------------------------------------

=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Hebrew software provides)

Hebrew software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Hebrew software provides)

Insert 'Ehebrew::' at head of function name. Ehebrew.pm provides your script Ehebrew::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Ehebrew::lc        Character
  lcfirst     Ehebrew::lcfirst   Character
  uc          Ehebrew::uc        Character
  ucfirst     Ehebrew::ucfirst   Character
  fc          Ehebrew::fc        Character
  chr         Ehebrew::chr       Character
  glob        Ehebrew::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Hebrew software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Hebrew software provides)

Hebrew software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Ehebrew.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Hebrew software and
...
Hebrew software remains one language and one interpreter.
