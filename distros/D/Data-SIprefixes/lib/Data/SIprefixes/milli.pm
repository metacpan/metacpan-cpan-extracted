...
Data::SIprefixes::milli - This provides milli matching for Data::SIprefixes.
...
=head1 SYNOPSIS
    ...
    my $prefix=Data::SIprefixes::milli->new;

    my $origMeasure='millimeter';
...
sub shortMatch{
    ...
    if ( $measure=~/^m/ ){
        ...
        if ( $measure =~ /^ / ){
            $self->{error}=2;
            $self->{errorString}='Space found after prefix, /^m/, in "'.$origMeasure.'"' ;
            $self->warn;
            return undef;
        }
...
1; # End of Data::SIprefixes::milli
