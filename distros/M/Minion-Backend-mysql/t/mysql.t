...
subtest 'Migrate up and down' => sub {
  is $minion->backend->mysql->migrations->active, 15, 'active version is 15';
  is $minion->backend->mysql->migrations->migrate(0)->active, 0, 'active version is 0';
  is $minion->backend->mysql->migrations->migrate->active, 15, 'active version is 15';
};
...
subtest 'Repair missing worker' => sub {
  ...
  $minion->backend->mysql->db->query(
    ...
    set notified = date_sub( now(), interval ? second ) where id = ?", $minion->missing_after + 1, $id
  );
  $minion->repair;
...
subtest 'Repair old jobs' => sub {
  ...
  $minion->backend->mysql->db->query('update minion_jobs set finished = from_unixtime(?) where id = ?',
...
subtest 'Repair stuck jobs' => sub {
  ...
  $minion->backend->mysql->db->query("update minion_jobs set `delayed` = date_sub( now(), interval ? second ) where id = ?",
...
subtest 'List locks' => sub {
  ...
  $minion->backend->mysql->db->query(
    "update minion_locks set expires = date_sub( now(), interval 1 second )
   where name = 'yada'",
  );
...
subtest 'Delayed jobs' => sub {
  ...
  $minion->backend->mysql->db->query("update minion_jobs set `delayed` = date_sub( now(), interval 1 day ) where id = ?", $id);
  ok $job = $worker->dequeue(0), 'job dequeued';
  is $job->id, $id, 'right id';
...
subtest 'Multiple attempts while processing' => sub {
  ...
  $minion->backend->mysql->db->query('update minion_jobs set `delayed` = now() where id = ?', $id);
  ok $job = $worker->dequeue(0), 'job dequeued';
  is $job->id, $id, 'right id';
...
subtest 'Job dependencies' => sub {
  ...
  ok $job = $worker->dequeue(0), 'job dequeued';
  is $job->id, $id, 'right id';
  is_deeply $job->info->{parents}, [-2, -1], 'right parents';
  ok $job->finish, 'job finished';
  $worker->unregister;
};
...
subtest 'Expiring jobs' => sub {
  ...
  $id = $minion->enqueue('test' => [] => {expire => 300});
  is $minion->repair->jobs({states => ['inactive']})->total, 1, 'job has not expired yet';
  my $mysql = $minion->backend->mysql;
  $mysql->db->query("update minion_jobs set expires = date_sub( now(), interval 1 day ) where id = ?", $id);
  is $minion->jobs({states => ['inactive']})->total, 0, 'job has expired';
  ok !$worker->dequeue(0), 'job has expired';
  ok $mysql->db->select('minion_jobs', '*', {id => $id})->hash, 'job still exists in database';
  $minion->repair;
  ok !$mysql->db->select('minion_jobs', '*', {id => $id})->hash, 'job no longer exists in database';
