...
This document describes version 0.003 of Sah::Schema::dns::zone (from Perl distribution Sah-SchemaBundle-DNS), released on 2024-06-26.
...
=head1 SAH SCHEMA DEFINITION
 ...
 [
   "hash",
   {
     "keys" => { records => ["dns::records_of_known_types", { req => 1 }] },
     "req_keys" => ["records"],
     "keys.restrict" => 0,
   },
 ]
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("dns::zone*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("dns::zone", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("dns::zone", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("dns::zone*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"dns::zone*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('dns::zone*', name=>'DnsZone')
     );
 }

 use My::Types qw(DnsZone);
 DnsZone->assert_valid($data);
