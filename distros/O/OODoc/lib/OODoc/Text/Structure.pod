...
The L<OODoc::Text::Structure|OODoc::Text::Structure> class is used as base class for
...
Find the chapter, section or subsection with this C<$name>.  The object found
is returned.
...
Call the C<$method> recursively on this object and all its sub-structures.
For instance, when called on a chapter, it first will call the C<$method> on
that chapter, than on all its sections, subsections, and subsubsections.
The C<%parameters> are passed with each call.  The results of all calls is
returned as LIST.
...
One or more subroutines (L<OODoc::Text::Subroutine|OODoc::Text::Subroutine> C<@objects>) are added to the
