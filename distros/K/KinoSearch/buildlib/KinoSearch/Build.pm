...
my $XS_SOURCE_DIR        = 'xs';
my $LIB_DIR              = 'lib';
my $XS_FILEPATH          = catfile( $LIB_DIR, "KinoSearch.xs" );
my $AUTOBIND_PM_PATH     = catfile( $LIB_DIR, 'KinoSearch', 'Autobinding.pm' );
...
sub ACTION_charmonizer {
    ...
    my $cbuilder
        = KinoSearch::Build::CBuilder->new( config => { cc => $self->config('cc') },
        );
...
sub ACTION_compile_custom_xs {
    ...
    # .xs => .c
    my $perl_binding_c_file = catfile( $LIB_DIR, 'KinoSearch.c' );
    ...
    my $perl_binding_o_file = catfile( $LIB_DIR, "KinoSearch$Config{_o}" );
