{
  ...
  sub PCSI_register {
    ...
    $ircd->plugin_register($self, 'SERVER', qw(daemon_kline));
    return 1;
  }
  ...
  sub IRCD_daemon_kline {
    my ($self, $ircd) = splice @_, 0, 2;
    my $alarm_id = $ircd->{state}{klines}[0]{alarm};
...
sub ircd_daemon_nick {
    ...
    $heap->{ircd}->yield( 'daemon_cmd_kline', 'OperServ', 1, '*bobb*@127.0.0.1', 'Banhammer' );
...
sub ircd_daemon_expired {
    ...
    is($args[0], 'k-line', 'Type is K-Line' );
    is($args[1], '*bobb*@127.0.0.1', 'The usermask is right' );
...
sub client_disconnected {
  ...
  diag("Waiting for K-Line to expire, should be 10 seconds or so\n");
  return;
}
