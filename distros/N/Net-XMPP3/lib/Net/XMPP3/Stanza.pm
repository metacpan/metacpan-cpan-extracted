...
$DEBUG = new Net::XMPP3::Debug(usedefault=>1,
                              header=>"XMPP");
...
sub debug
{
    ...
    print "debug ",$self,":\n";
    &Net::XMPP3::printData("debug: \$self->{CHILDREN}->",$self->{CHILDREN});
}
...
##############################################################################
#
# NewChild - calls AddChild to create a new Net::XMPP3::Stanza object, sets the
...
sub NewChild
{
    ...
    return unless exists($Net::XMPP3::Namespaces::NS{$xmlns});

    if (!defined($tag))
    {
        $tag = "x";
        $tag = $Net::XMPP3::Namespaces::NS{$xmlns}->{tag}
...
##############################################################################
#
# AddChild - creates a new Net::XMPP3::packet object, pushes it on the child
...
##############################################################################
#
# NewFirstChild - calls AddFirstChild to create a new Net::XMPP3::Stanza
...
##############################################################################
#
# AddFirstChild - creates a new Net::XMPP3::packet object, puts it on the child
...
sub _xpath_AUTOLOAD
{
    ...
    elsif (exists($funcs->{$var}->{child}))
    {
        ...
        if (exists($child->{ns}))
        {
            ...
            my $addFuncs = $Net::XMPP3::Namespaces::NS{$addXMLNS}->{xpath};
            my @calls =
                grep
                {
...
#                or from Net::XMPP3::Namespaces::NS.
#
##############################################################################
sub _xpath_funcs
{
    ...
    my $nsFuncs = {};
    $nsFuncs = $Net::XMPP3::Namespaces::NS{$xmlns}->{xpath}
        if (defined($xmlns) && exists($Net::XMPP3::Namespaces::NS{$xmlns}));
...
sub _xpath_set
{
    ...
    if ($type eq "timestamp")
    {
        ...
        if ($value eq "") {
            $value = &Net::XMPP3::GetTimeStamp("utc","","stamp");
        }
    }
    ...
    foreach my $val (@values)
    {
        ...
        if ((ref($val) ne "") && ($val->isa("Net::XMPP3::JID")))
        {
            $val = $val->GetJID("full");
        }
...
sub _check_skip_xmlns
{
    ...
    foreach my $skipns (keys(%Net::XMPP3::Namespaces::SKIPNS))
    {
...
##############################################################################
#
# _debug - helper function for printing debug messages using Net::XMPP3::Debug
#
##############################################################################
sub _debug
{
