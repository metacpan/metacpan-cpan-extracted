...
run {
    ...
    # Foreach match abbreviation return the key
    ...
    # Foreach normalized book return the key
    while ( my ($key, $value) = each(%{$bookconfig->{books}})) {
	foreach my $mb (@{$value->{Match}->{Book}}) {
	    my $result = $r->key($mb);
	    my $expected = $key;
	    is_deeply($result, $expected, $block->name . ": with the book='$mb', result='$result', expected='$expected' ");
	}
    }
