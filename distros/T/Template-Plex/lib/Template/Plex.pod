...
numbers maybe incorrect when block_fix is active, as it effectivly removes a
...
B<NOTE:> in the case of a syntax error present in the template, the line
numbers maybe incorrect if C<include> is used, as it effectivly adds lines to
the template source.
...
Enabling lexically scoped features (ie C<use feature "say">) is only in the
