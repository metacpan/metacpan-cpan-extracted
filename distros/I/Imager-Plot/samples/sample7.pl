...
my @tr = map { [$time[$_], $rate[$_]] } map { $_*12 } 0..int($cnt/12-1);
...
sub bar_style {
  ...
  for (0..$#x) {
    $img->box(color=>'blue', xmin=>$x[$_]-5, xmax=>$x[$_]+5, ymin=>$y[$_], ymax=>$bottom, filled=>1);
    $img->box(color=>'black', xmin=>$x[$_]-5, xmax=>$x[$_]+5, ymin=>$y[$_], ymax=>$bottom, filled=>0);
  }
...
$img->write(file => "sampleout/sample7.ppm");
