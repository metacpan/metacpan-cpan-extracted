...
static SV *
hv_clone (pTHX_ SV * ref, SV * target, HV* hseen, int depth) {
    HV *clone = (HV *) target;
    HV *self = (HV *) ref;
    HE *next = NULL;
    ...
    while ((next = hv_iternext(self))) {
        ...
        hv_store_ent(clone, key, sv_clone(aTHX_ hv_iterval(self, next), hseen, recur), 0);
    }
...
static SV *
av_clone (pTHX_ SV * ref, SV * target, HV* hseen, int depth) {
    AV *clone = (AV *) target;
    AV *self = (AV *) ref;
    SV **svp;
    SV *val = NULL;
    ...
    for (i = 0; i <= arrlen; i++) {
        ...
        if (svp) av_store(clone, i, sv_clone(aTHX_ *svp, hseen, recur));
    }
...
rv_clone (pTHX_ SV * ref, HV* hseen, int depth) {
    ...
    if (!SvROK(ref)) return NULL;

    if (sv_isobject (ref)) {
        clone = newRV_noinc(sv_clone(aTHX_ SvRV(ref), hseen, depth));
        sv_2mortal(sv_bless(clone, SvSTASH(SvRV(ref))));
    }
    else
        clone = newRV_inc(sv_clone(aTHX_ SvRV(ref), hseen, depth));
...
sv_clone (pTHX_ SV * ref, HV* hseen, int depth) {
        ...
        for (mg = SvMAGIC(ref); mg; mg = mg->mg_moremagic) {
            ...
            if ( mg->mg_obj != NULL ) {
                switch (mg->mg_type) {
                    ...
                    /* fall through */
                    default:
                        obj = sv_clone(aTHX_ mg->mg_obj, hseen, -1); 
                }
            }
            else {
    ...
    if ( magic_ref ) {
        ...
        clone = hv_clone(aTHX_ ref, clone, hseen, depth);
    else if ( SvTYPE(ref) == SVt_PVAV )
        clone = av_clone(aTHX_ ref, clone, hseen, depth);
    /* 3: REFERENCE (inlined for speed) */
    else if (SvROK (ref)) {
        TRACEME(("clone = 0x%x(%d)\n", clone, SvREFCNT(clone)));
        SvREFCNT_dec(SvRV(clone));
        SvRV(clone) = sv_clone(aTHX_ SvRV(ref), hseen, depth); /* Clone the referent */
        if (sv_isobject(ref)) {
