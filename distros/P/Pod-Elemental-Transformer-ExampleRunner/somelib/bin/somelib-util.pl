...
use Pod::Usage;
BEGIN {
eval " use SomeLib::Devel 1.0; ";

if ($@) {
  print <<END;
To use the SomeLib development tools including somelib-util.pl and the
generated script/myapp_create.pl you need SomeLib::Helper, which is
part of the SomeLib-Devel distribution. Please install this via a
vendor package or by running one of -

  perl -MCPAN -e 'install SomeLib::Devel'
  perl -MCPANPLUS -e 'install SomeLib::Devel'
END
...
somelib-util - Bootstrap a SomeLib application

=head1 SYNOPSIS

somelib-util.pl [options] application-name

'somelib-util.pl' creates a skeleton for a new application, and allows you to
upgrade the skeleton of your old application.
 ...
 Examples:
    somelib-util.pl My::App
    somelib-util.pl MyApp

 To upgrade your app to a new version of SomeLib:
    somelib-util.pl -force -scripts MyApp


=head1 DESCRIPTION

The C<somelib-util.pl> script bootstraps a SomeLib application, creating a
...
runs the generated application within a SomeLib test server, which can be
...
The application module generated by the C<somelib-util.pl> script is functional,
although it reacts to all requests by outputting a friendly welcome screen.

=head1 NOTE

Neither C<somelib-util.pl> nor the generated helper script will overwrite existing
