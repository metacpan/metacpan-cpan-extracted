...
# Program:  03-basic-off_on.t
#   Does a very basic OFF fish trace with fish turned on.
...
BEGIN {
   ...
   unless (use_ok ("Fred::Fish::DBUG::OFF")) {     # Test # 1
      BAIL_OUT ( "Can't load Fred::Fish::DBUG::OFF" );
      exit (0);
   }
}
...
BEGIN {
   ...
   # Level check doesn't work with the OFF module!  Always returns -1.
   my $lvl = -1;

   DBUG_ENTER_FUNC ();

   # Test # 2 ...
   $start_level = Fred::Fish::DBUG::OFF::dbug_level ();
   is2 ($start_level, $lvl, "In the BEGIN block ...");   # Test # 2

   ok2 (! DBUG_ACTIVE(), "Fish is turned OFF.");      # Test # 3
...
END {
   ...
   my $end_level = Fred::Fish::DBUG::OFF::dbug_level ();
...
# --------------------------------------
{
   ...
   # Test # 8 ...
   ok2 (Fred::Fish::DBUG::OFF::dbug_level(), $start_level, "Level Check");
