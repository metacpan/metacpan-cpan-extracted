...
# tag: test for JOAP Server Class subclass instance method handling
...

{
    ...
    my $q =
	  ...
	  type => 'methodCall',
	  methodName => 'walk',
	  params => [-13]);     # walk can't take negative values
    ...
    is($members{faultCode}->GetI4, 5440, "Has the special fault code from MyPerson.");

    ok($members{faultString}, "Has a fault string");
    ok($members{faultString}->DefinedString, "Has a string for the fault code.");
    is($members{faultString}->GetString, "Never go back.", "Has the special fault string from MyPerson.");
}
