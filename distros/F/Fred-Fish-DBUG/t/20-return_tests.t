...
# --------------------------------------
{
   ...
   dbug_ok ( (! defined $res), "Void Return Test # 1 succeeded!");

   $res = void_return_test_2 ();
   dbug_ok ( (! defined $res), "Void Return Test # 2 succeeded!");

   @lst = void_return_test_2 ();
   dbug_ok ( ($#lst == -1),    "Void Return Test # 3 succeeded!");

   $res = void_return_test_3 ();
   dbug_ok ( (! defined $res), "Void Return Test # 4 succeeded!");

   @lst = void_return_test_3 ();
   dbug_ok ( ($#lst == -1),    "Void Return Test # 5 succeeded!");

   $lvl = test_fish_level ();
   dbug_is ($lvl, $start_level, "Level 1 check!");

   # --------------------------------------------------------------
   DBUG_PRINT ("----", "The Value Return Tests ...\n%s", '-'x50);
   # --------------------------------------------------------------
   $res = return_scalar_test ();
   dbug_is ( $res, "scalar",        "Value Return Test # 1 succeeded!");

   @lst = return_scalar_test ();
   dbug_ok ( ($#lst == 0),          "Value Return Test # 2 succeeded!");
   dbug_ok ( ($lst[0] eq "scalar"), "Value Return Test # 3 succeeded!");

   $b = return_scalar_test (0);
   dbug_is ( $b, $res, "Value Return Test # 4, masking successful!");

   $lvl = test_fish_level ();
   dbug_is ($lvl, $start_level, "Level 2 check!");

   # --------------------------------------------------------------
   DBUG_PRINT ("----", "The List Return Tests ...\n%s", '-'x50);
   # --------------------------------------------------------------
   $res = return_list_test_1 ();
   dbug_is ($res, "a",     "List Return Test # 1 succeeded!");

   @lst = return_list_test_1 ();
   $cnt = @lst;
   dbug_is ( $cnt, 10,     "List Return Test # 2 succeeded!");
   dbug_ok ( ( $lst[0] eq "a" && $lst[1] eq "b" && (! defined $lst[2]) &&
   ...
   dbug_ok ( ($a eq "a" && $b eq "b"), "List Return Test # 4 - collected 1st 2 vals as scalars!");

   $res = return_list_test_2 ();
   dbug_is ($res, "a",       "List Return Test # 5 succeeded!");

   @lst = return_list_test_2 ();
   $cnt = @lst;
   dbug_is ( $#lst, 6,     "List Return Test # 6 succeeded!");
   dbug_is ( $lst[0], "a", "List Return Test # 7 succeeded!");

   ($a, $b) = return_list_test_2 ();
   dbug_ok ( ($a eq "a" && $b eq "b"), "List Return Test # 8 - collected 1st 2 vals as scalars!");

   my @lst2 = return_list_test_2 (1, 3, 5, 7, 9, 11);
   $cnt = test_mask_return(3);
   dbug_ok ( compare (\@lst, \@lst2 ), "List Retrun Test # 9 - Masking works OK!" );
   dbug_is ( $cnt, 3, "Masked ${cnt} return values!");

   @lst2 = return_list_test_2 (-1);
   $cnt = test_mask_return(7);
   dbug_ok ( compare (\@lst, \@lst2 ), "List Retrun Test # 10 - Masking works OK!" );
   dbug_is ( $cnt, 7, "Masked ${cnt} return values!");

   return_list_test_2 (2, 4, 5, 8, 10);
   $cnt = test_mask_return(0);
   dbug_ok ( 1, "List Return test # 11 - Masking with no return value expected!");
   dbug_is ( $cnt, 0, "Masked ${cnt} return values!");

   $lvl = test_fish_level ();
   dbug_is ($lvl, $start_level, "Level 3 check!");
   ...
   $a = keys %h;
   $cnt = test_mask_return(0);
   dbug_is ( $a, 4, "Hash Return Test # 1 - Collected the right number of entries ($a keys)!");
   dbug_is ( $cnt, 0, "Masked ${cnt} return values!");

   $res = return_hash_test ();
   $cnt = test_mask_return(0);
   dbug_ok ( exists $h{$res}, "Hash Return Test # 2 - Returned a valid hash key! ($res, $h{$res})!");
   dbug_is ( $cnt, 0, "Masked ${cnt} return values!");

   ($a, $b) = return_hash_test ();
   $cnt = test_mask_return(0);
   dbug_ok ( exists $h{$a} && $b == $h{$a}, "Hash Return Test # 3 - Returned the 1st key/value pair!");
   dbug_is ( $cnt, 0, "Masked ${cnt} return values!");

   my %h2 = return_hash_test ("BIG", "!", "BIG-FOOT");
   $cnt = test_mask_return(2);
   dbug_ok (compare_hash (\%h, \%h2), "Hash Return Test # 4 - Masking the hash works OK!");
   dbug_is ( $cnt, 2, "Masked ${cnt} return values!");

   my %h3 = return_hash_test ( -1, "BIG" );
   $cnt = test_mask_return(8);
   dbug_ok (compare_hash (\%h, \%h3), "Hash Return Test # 5 - Masking everything in the hash works OK!");
   dbug_is ( $cnt, 8, "Masked ${cnt} return values!");

   $lvl = test_fish_level ();
   dbug_is ($lvl, $start_level, "Level 4 check!");
