...
Returns 0 on success, negative wslay error code on failure.

=head2 queue_msg_ex( message, opcode, rsv )

Queue message, opcode is optional default is 1 (text message).
rsv is optional default is WS_RSV1_BIT.
Returns 0 on success, negative wslay error code on failure.
...
=head2 start_write() and stop_write()

=head2 start_read() and stop_read()

=head2 shutdown_read() and shutdown_write()

Permanently disable read or write at the wslay protocol level. Unlike stop_*/start_* which only pause the event watcher, shutdown is irreversible for the lifetime of the connection
