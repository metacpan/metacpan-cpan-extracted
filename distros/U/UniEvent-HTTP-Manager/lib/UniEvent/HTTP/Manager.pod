...
=head1 SYNOPSIS
    ...
    my $mgr = UniEvent::HTTP::Manager->new({
        ...
        server       => { locations => [{host => 'localhost', port => 8080}], },
    });
...
L<UniEvent>, C<track_load_average()> for details what this value means. If average busy load on workers becomes higher that configured value, manager
...
=item bind_model [BIND_REUSE_PORT on Unix, BIND_DUPLICATE on Windows]

Controls how manager will create listening sockets.
Should be either C<UniEvent::HTTP::Manager::BIND_REUSE_PORT> or C<UniEvent::HTTP::Manager::BIND_DUPLICATE>.

If C<UniEvent::HTTP::Manager::BIND_DUPLICATE> is set, manager will create listening sockets for all locations in master process once and use the same
sockets in each worker. So that they will be accepting the same sockets.
The advantage of this method is that it allows to start web server with some advanced privileges, bind sockets to ports like 80, 443, etc... 
...
If C<UniEvent::HTTP::Manager::BIND_REUSE_PORT> is set, manager will not create any sockets, instead each worker will create listening sockets by itself
and bind it to the same addresses and ports making use of C<REUSE_PORT> feature. This way each worker will accept its own sockets and the OS will
...
Alternatively, you can create sockets by yourself and pass it in C<sock> property of an item in L<UniEvent::HTTP::Server>'s C<locations> part of config.
In this case C<bind_model> is ignored (or applied only to those locations for which you didn't pass C<sock>). You can even create sockets in worker
...
Reconfigures the web server. Supported parameters are the same as in C<new()>, except for C<bind_model> and C<worker_model> which can't be
changed. All other parameters can be changed on-the-fly. Manager may gracefully restart all workers during this process (or may not, if significant
