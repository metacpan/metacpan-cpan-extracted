...
my $refactoring = hoist_to_lexical( $CODE, { closure=>0, from => $from, to => $to, all=>1 } );

#diag $refactoring->{code};
#diag $refactoring->{call};

is eval($CODE), 7  => 'Original returned expected value';
...
is eval($REFACTORED), 7  => 'Refactored returned expected value';
