package Firewall::Config::Element::Schedule::H3c;
  ...
  if ( $self->schType eq 'onetime' ) {
    ...
    if ( defined $self->day and defined $self->startTime and defined $self->endTime ) {
      ...
      for my $day (@days) {
        ...
        elsif ( $day eq 'weekdays' ) {
          push @weekDays, ( "Mon", "Tue", "Wed", "Thu", "Fri" );
        }
        elsif ( $day eq 'weekend' ) {
          push @weekDays, ( "Sun", "Sat" );
        }
        else {
          push @weekDays, $day;
        }
...
sub getSecondFromEpoch {
  ...
  #17:44 2/22/2018
  my ( $hour, $min, $mon, $mday, $year ) = split( '[\s+:/]', $string );
...
sub getEnddateStr {
  my $self = shift;
  my ( $mon, $mday, $year, $hour, $min ) = split( '[\s+:/]', $self->endDate );
