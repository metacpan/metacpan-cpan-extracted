...
struct imp_dbh_st {
	...
	int     pg_protocol;       /* value of KCIConnectionGetProtocolVersion, usually 3 (could also be 0) */
	...
	int     copystate;         /* 0=none EXECUTE_COPY_IN EXECUTE_COPY_OUT */
	bool    copybinary;        /* whether the copy is in binary format */
	int     pg_errorlevel;     /* KCISetVerbosity. Set by user, defaults to 1 */
	bool    server_prepare;    /* do we want to use KCIStatementExecutePrepared? Can be changed by user */
	int     switch_prepared;   /* how many executes until we switch to KCIStatementExecutePrepared */
	...
	KCIConnection  *conn;             /* connection structure */
    ...
    KCIResult  *last_result;     /* KCIResult structure from the last executed query (can be from imp_dbh or imp_sth) */
    ...
    int     fetch_size;          /* rows per fetch batch, 0 = fetch all at once */
};
...
struct ph_st {
	...
	char  *quoted;              /* quoted version of the value, for KCIStatementExecute only */
...
struct imp_sth_st {
	...
	KCIResult  *result;       /* result structure from the executed query */
	...
	int     chunk_size;          /* snapshot of dbh FetchSize at execute time */
	bool    fetch_chunked;       /* true when using FetchSize */
};
...
/* 
   Everything else should map back to the DBI version, or be handled by KingBase.pm
