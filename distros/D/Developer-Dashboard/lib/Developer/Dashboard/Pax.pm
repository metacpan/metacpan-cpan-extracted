package Developer::Dashboard::Pax;
...
to every C<lib/Developer/Dashboard/Pax/*.pm> module before release.
...
C<Developer::Dashboard::Pax::CLI> owns the public operator contract behind C<pax build> and
C<pax run>.

=head2 Manifest Loading

C<Developer::Dashboard::Pax::Paxfile> loads repeatable build inputs from C<paxfile.yml>.

=head2 Standalone Image Builder

C<Developer::Dashboard::Pax::StandaloneImage> collects dependencies, packages runtime payloads,
embeds assets, and writes the standalone launcher.

=head2 Code Unit Compilation

C<Developer::Dashboard::Pax::CodeUnitCompiler> lowers supported Perl source shapes into PAX code unit
records. Unsupported regions remain on explicit fallback paths instead of being
silently miscompiled.

=head2 Packaged Runtime

C<Developer::Dashboard::Pax::StandaloneRuntime> provides the packaged helper runtime used after the
standalone executable starts.

=head2 Native Dispatch

C<Developer::Dashboard::Pax::StandaloneDispatch> and related runtime pieces execute packaged native
...
C<#!/usr/local/bin/pax>. In that path, C<Developer::Dashboard::Pax::CLI> treats the script argument
...
C<Developer::Dashboard::Pax::CLI> is a small public facade. It resolves C<build> and C<run> inputs
from CLI arguments plus C<Developer::Dashboard::Pax::Paxfile>, then delegates to the standalone image
builder.

=head2 Compilation and Code Units

C<Developer::Dashboard::Pax::CodeUnitCompiler> compiles supported Perl source shapes into PCU records.
...
=head2 Guarded SSA, Typed IR, and LLVM Planning

For native-capable regions, the current lowering path is:

  Perl capture
    -> HIR
    -> Guarded SSA
    -> typed contract extraction / typed IR
    -> LLVM IR planning
    -> packaged native artifact or fallback runtime path

The typed stage does not claim that all Perl is statically typed. It extracts
explicit type hints when source carries them and otherwise infers a minimal
typed contract from the native-capable region shapes PAX already understands.
That keeps the LLVM-oriented path reusable across arbitrary Perl applications
instead of baking project names into the compiler.

=head2 Dependency Discovery

C<Developer::Dashboard::Pax::StandaloneImage> follows entrypoints, library directories, source roots,
