...
Data::SIprefixes::zepto - This provides zepto matching for Data::SIprefixes.
...
=head1 SYNOPSIS
    ...
    use Data::SIprefixes::zepto;
    
    my $prefix=Data::SIprefixes::atto->new;

    my $origMeasure='zeptometer';
...
sub longMatch{
	...
	if ( $measure=~/^zepto/ ){
		...
		if ( $measure =~ /^ / ){
			$self->{error}=2;
			$self->{errorString}='Space found after prefix, /^zepto/, in "'.$origMeasure.'"' ;
			$self->warn;
			return undef;
		}
...
1; # End of Data::SIprefixes::zepto
