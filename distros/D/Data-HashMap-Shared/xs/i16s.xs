...
new(char* class, SV* path_sv, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        ...
        char errbuf[SHM_ERR_BUFLEN]; const char* path = SHM_PATH_ARG(path_sv, "path", "Data::HashMap::Shared::I16S"); ShmHandle* map = shm_i16s_create(path, (uint32_t)max_entries, (uint32_t)lru_max, (uint32_t)ttl_default, (uint32_t)lru_skip, (uint64_t)arena_cap, (mode_t)file_mode, errbuf);
        if (!map) croak("Data::HashMap::Shared::I16S: %s", errbuf[0] ? errbuf : "unknown error");
        CK_MAX_SIZE(map, "Data::HashMap::Shared::I16S");
...
new_sharded(char* class, SV* path_prefix_sv, UV num_shards, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0, UV arena_cap = 0, UV file_mode = 0600)
    CODE:
        const char* path_prefix = SHM_PATH_ARG(path_prefix_sv, "path prefix", "Data::HashMap::Shared::I16S");
        CK_U32(max_entries, "max_entries", "Data::HashMap::Shared::I16S"); CK_U32(lru_max, "lru_max", "Data::HashMap::Shared::I16S"); CK_U32(ttl_default, "ttl_default", "Data::HashMap::Shared::I16S"); CK_U32(lru_skip, "lru_skip", "Data::HashMap::Shared::I16S");
...
freeze(SV* self_sv)
    CODE:
        ...
        if (h->readonly || shm_is_sealed(h)) croak("Data::HashMap::Shared::I16S->freeze: cannot freeze a read-only handle");
    ...
    CODE:
        ...
        if (h->shard_handles) {
            ...
            for (int i = 1; i < items; i += 2) {
                ...
                if (_vl > SHM_MAX_STR_LEN) croak("value too long (max 1GB)");
                REEXTRACT_MAP("Data::HashMap::Shared::I16S", self_sv);
                count += shm_i16s_put_inner(h, _key, _vs, (uint32_t)_vl, _vu, SHM_TTL_USE_DEFAULT);
            }
        }
        RETVAL = count;
    OUTPUT:
    ...
    CODE:
        ...
        if (h->shard_handles) {
            ...
            for (int i = 1; i < items; i++) {
                int16_t _k = (int16_t)SvIV(ST(i));
                REEXTRACT_MAP("Data::HashMap::Shared::I16S", self_sv);
                count += shm_i16s_remove_inner(h, _k);
            }
...
get_multi(SV* self_sv, ...)
    PPCODE:
        ...
        if (h->shard_handles) {
            ...
            for (int i = 0; i < nkeys; i++) {
                hashes[i] = shm_hash_int64((int64_t)(int16_t)SvIV(ST(i + 1)));
...
unlink(SV* self_or_class, ...)
    CODE:
        if (sv_isobject(self_or_class) && sv_derived_from(self_or_class, "Data::HashMap::Shared::I16S")) {
            ...
            RETVAL = shm_unlink_path(shm_path_arg(aTHX_ ST(1), "path", "Data::HashMap::Shared::I16S"));
        }
    OUTPUT:
        RETVAL
