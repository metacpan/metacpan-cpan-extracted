package LyricFinder::ChartLyrics;
...
my $Source = 'ChartLyrics';
my $Site   = 'http://api.chartlyrics.com';
my $DEBUG  = 0;
...
sub new
{
	...
	unless ($XMLavailable) {
		carp($self->{'Error'} = "u:$Source - Required module XML::Simple not installed.");
		return 0;
	}
...
sub fetch {
	...
	$self->_debug("ChartLyrics::fetch($artist_in, $song_in)!");
	...
	# http://api.chartlyrics.com/apiv1.asmx/SearchLyric?artist<artist>=&song=<title>
	($self->{'Url'} = $artist_in) =~ s#\s*\/\s*# and #;  #ONLY USE 1ST ARTIST, IF MORE THAN ONE!
	my $artist = $artist_in;
	(my $song = $song_in) =~ s#\s*\/\s*#\-#g;                  #FIX SONGS WITH "/" IN THEM!
	$self->{'Url'} .= "&song=${song}-lyrics";
	$self->{'Url'} = $Site . '/apiv1.asmx/SearchLyric?artist=' . $self->{'Url'};

	$self->_debug("--fetch: will fetch lookupURL=".$self->{'Url'}."= ARTIN=$artist_in= TTLIN=$song_in=");
...
sub _parse {
	...
	$self->_debug("ChartLyrics::_parse()!");
	my $xmldoc;
	my $xs1 = XML::Simple->new();

	eval {$xmldoc = $xs1->XMLin($html, ForceArray => ['SearchLyricResult']); };
	unless ($xmldoc && !$@ && $xmldoc->{'SearchLyricResult'}) {
		carp($self->{'Error'} = "e:$Source - No xml returned ($@) - no match.");
		return '';
	}

	my $lyricID = 0; my $lyricChecksum = 0;
	for (my $i=0;$i<$#{$xmldoc->{'SearchLyricResult'}};$i++) {
		if ($xmldoc->{'SearchLyricResult'}->[$i]->{'Artist'} =~ /^\Q$artist_in\E$/i
				&& $xmldoc->{'SearchLyricResult'}->[$i]->{'Song'} =~ /^\Q$song_in\E/i
				&& $xmldoc->{'SearchLyricResult'}->[$i]->{'LyricId'}
				&& $xmldoc->{'SearchLyricResult'}->[$i]->{'LyricChecksum'}) {
			$lyricID = $xmldoc->{'SearchLyricResult'}->[$i]->{'LyricId'};
			$lyricChecksum = $xmldoc->{'SearchLyricResult'}->[$i]->{'LyricChecksum'};
			$self->_debug("--_parse1($i) found ID=$lyricID, CK=$lyricChecksum!");
			last;
		}
	}
	unless ($lyricID && $lyricChecksum) {
		for (my $i=0;$i<$#{$xmldoc->{'SearchLyricResult'}};$i++) {
			if ($xmldoc->{'SearchLyricResult'}->[$i]->{'Artist'} =~ /\Q$artist_in\E/i
					&& $xmldoc->{'SearchLyricResult'}->[$i]->{'Song'} =~ /^\Q$song_in\E/i
					&& $xmldoc->{'SearchLyricResult'}->[$i]->{'LyricId'}
				...
				$lyricID = $xmldoc->{'SearchLyricResult'}->[$i]->{'LyricId'};
				$lyricChecksum = $xmldoc->{'SearchLyricResult'}->[$i]->{'LyricChecksum'};
				$self->_debug("--_parse2($i) found ID=$lyricID, CK=$lyricChecksum!");
				last;
			}
		}
	}
	...
	#ChartLyrics requires a 2-step fetch:  1: fetch the lyricID & checksum, then 2:  use those to
	#fetch the lyrics and cover-art.  Step 2 is done here:

	my $songUrl = $Site . "/apiv1.asmx/GetLyric?lyricId=$lyricID&lyricCheckSum=$lyricChecksum";
	$self->_debug("--looking up songURL=$songUrl=");
	...
	my $res = $ua->get($songUrl);
	if ($res->is_success) {
		$self->_debug("--success fetching song/lyric page.");
		eval {$xmldoc = $xs1->XMLin($res->decoded_content); };
		unless ($xmldoc && !$@ && $xmldoc->{'Lyric'}) {
			carp($self->{'Error'} = "e:$Source - Failed to identify lyrics on xml page($@).");
			return '';
		}
		my $text = $xmldoc->{'Lyric'};
		$self->{'image_url'} = $xmldoc->{'LyricCovertArtUrl'};
		$self->_debug("--cover art url=".$self->{'image_url'}."=");
		return $self->_normalize_lyric_text($text)  if ($text =~ /[a-z]/i);
		...
		if ($res->status_line =~ /^404/) {
			...
			carp($self->{'Error'} = "e:$Source - Failed to retrieve $songUrl ("
					.$res->status_line.').');
		}
		return '';
	}
...
LyricFinder::ChartLyrics - Fetch song lyrics from chartlyrics.com.

=head1 AUTHOR

This module is Copyright (c) 2024-2025 by
...
=head1 SYNOPSIS
    ...
    my $finder = new LyricFinder::ChartLyrics();

    # fetch lyrics for a song from http://api.chartlyrics.com:
    ...
    #-or-
    my $localfinder = new LyricFinder::ChartLyrics(-cache => '/tmp/lyrics');


=head1 DESCRIPTION

LyricFinder::ChartLyrics accepts an artist name and song title, searches 
http://api.chartlyrics.com for song lyrics, and, if found, returns them as a 
...
I<"ChartLyrics"> as the third argument of the B<fetch>() method.
...
=item B<new> I<LyricFinder::ChartLyrics>([ I<options> ])
...
Specifies an alternate "user-agent" string sent to chartlyrics.com when 
attempting to fetch lyrics.  Set the desired user-agent (ie. browser name) to 
pass to chartlyrics.com.  Some sites are pickey about receiving a user-agent 
...
Get / Set the desired user-agent (ie. browser name) to pass to chartlyrics.com.  
...
(this module's) name, either as "ChartLyrics" or ("ChartLyrics").
...
This site's base URL on success is always:  "I<http://api.chartlyrics.com>".
...
This site's module name on success is always:  "I<ChartLyrics>".
...
L<XML::Simple>, L<HTTP::Request>, L<LWP::UserAgent>

=head1 BUGS

Please report any bugs or feature requests to C<bug-lyricFinder-chartlyrics 
at rt.cpan.org>, or through the web interface at 
L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=LyricFinder-ChartLyrics>.  
...
You can find documentation for this module with the perldoc command.
    ...
    perldoc LyricFinder::ChartLyrics
...
L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=LyricFinder-ChartLyrics>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/LyricFinder-ChartLyrics>

=item * Search CPAN

L<http://search.cpan.org/dist/LyricFinder-ChartLyrics/>

=back

=head1 LICENSE AND COPYRIGHT

Copyright (c) 2024-2025 Jim Turner.
