...
#
# ... Tests via check_template-WebTransact-with-client-certificate.pl
#
# $Id: check_template-WebTransact-with-client-certificate.t, v 1.0 2006/02/01 Alex Peeters Exp $
#
...
my $plugin = 'check_template-WebTransact-with-client-certificate.pl';
...
if ( -x "$prefix/$plugin" ) {
  ...
  unless ( defined $ASNMTAP_PROXY and ( $ASNMTAP_PROXY eq '0.0.0.0' or $ASNMTAP_PROXY eq '' ) ) {
    ...
    $t += checkCmd( "$prefix/$plugin $proxy", [2, 3], "/ERROR: GET https:\/\/secure.citap.com\/certificate\/");
  } else {
