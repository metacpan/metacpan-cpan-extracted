...
sub client_input {
  ...
  if ( $cmd eq '251' ) {
    ...
    is( $params->[1], 'There are 1 users and 2 invisible on 4 servers', 'Correct text' );
    return;
  }
  if ( $cmd eq '252' ) {
    pass("IRC$cmd");
    is( $params->[0], 'bobbins', 'It is me, bobbins' );
    is( $params->[1], '2', 'Number of IRC opers' );
    is( $params->[2], 'IRC Operators online', 'Correct text' );
    return;
  }
  if ( $cmd eq '254' ) {
    pass("IRC$cmd");
    is( $params->[0], 'bobbins', 'It is me, bobbins' );
    is( $params->[1], '1', 'Number of channels formed' );
    is( $params->[2], 'channels formed', 'Correct text' );
    return;
  }
  if ( $cmd eq '255' ) {
    pass("IRC$cmd");
    is( $params->[0], 'bobbins', 'It is me, bobbins' );
    is( $params->[1], 'I have 1 clients and 2 servers', 'Correct text' );
    return;
  }
  if ( $cmd eq '265' ) {
    pass("IRC$cmd");
    is( $params->[0], 'bobbins', 'It is me, bobbins' );
    is( $params->[1], 'Current local users: 1  Max: 1', 'Correct text' );
    return;
  }
  if ( $cmd eq '266' ) {
    pass("IRC$cmd");
    is( $params->[0], 'bobbins', 'It is me, bobbins' );
    is( $params->[1], 'Current global users: 3  Max: 3', 'Correct text' );
    return;
  }
  if ( $cmd eq '250' ) {
    pass("IRC$cmd");
    is( $params->[0], 'bobbins', 'It is me, bobbins' );
    is( $params->[1], 'Highest connection count: 3 (1 clients) (3 connections received)', 'Correct text' );
    return;
  }
  return;
}
...
sub client_input2 {
  ...
  if ( $cmd eq '366' ) {
    ...
    $poe_kernel->post( $sender, 'send_to_server', { command => 'LUSERS' } );
    return;
  }
  if ( $cmd eq '251' ) {
    pass("IRC$cmd");
    is( $params->[0], 'bobbins', 'It is me, bobbins' );
    is( $params->[1], 'There are 0 users and 3 invisible on 4 servers', 'Correct text' );
    return;
  }
  if ( $cmd eq '252' ) {
    pass("IRC$cmd");
  ...
  if ( $cmd eq '254' ) {
    pass("IRC$cmd");
    is( $params->[0], 'bobbins', 'It is me, bobbins' );
    is( $params->[1], '2', 'Number of channels formed' );
    is( $params->[2], 'channels formed', 'Correct text' );
    return;
  }
  if ( $cmd eq '255' ) {
