...
sub new {
    ...
    $args{filename_method} //= sub {
        my $node = shift;
        return undef unless $node->can("title"); ## no critic: Subroutines::ProhibitExplicitReturnUndef
        my $name = $node->title;
        $name =~ s/\[.*?\]//g;
        $name =~ s/\s{2,}/ /g;
        $name =~ s/^\s+//;
...
This document describes version 0.004 of Tree::FSMethods::Org (from Perl distribution Tree-FSMethods), released on 2021-10-07.
