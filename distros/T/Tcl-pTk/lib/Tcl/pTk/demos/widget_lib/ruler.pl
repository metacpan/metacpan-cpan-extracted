...
sub ruler {
    ...
    $c->bind('well', '<1>' => [\&ruler_new_tab, \%rinfo, Ev('x'), Ev('y')]);
    $c->bind('tab', '<1>' => [\&ruler_select_tab, \%rinfo, Ev('x'), Ev('y')]);
    $c->CanvasBind('<B1-Motion>' => [\&ruler_move_tab, \%rinfo, Ev('x'), Ev('y')]);
    $c->CanvasBind('<Any-ButtonRelease-1>', [\&ruler_release_tab, \%rinfo]);
