...
PDLA's documentation is extensive. Some sections cover deep core magic
while others cover more usual topics like IO and numerical computation.
How are these related? Where should you begin?

This document is an attempt to pull all the key PDLA documentation
...
If you are new to PDLA, these documentation modules will get
you started down the right path for using PDLA.

=head2 Documentation

Modules that tell you how to start using PDLA.  Many of these
are library modules technically, but they are included when
you C<use PDLA>, so I've included them for their documentation.
...
familiarize yourself with the basic capabilities of PDLA.

=over

=item * L<PDLA::Philosophy|PDLA::Philosophy>, L<PDLA::QuickStart|PDLA::QuickStart>

A couple of brief introductions to PDLA. The second one is a bit more
hands-on. If you are new to PDLA, you should start with these.
...
PDLA came of age right around the turn of the millennium and
...
=item * Selections from L<PDLA::Core|PDLA::Core>

Like PDLA::Primitive, defines a large set of useful functions.
...
=item * The L<perldla|perldla> or L<pdla2|PDLA::Perldl2> Shell

The Perldl Shell is a REPL (Read-Evaluate-Print-Loop, in other words,
a prompt or shell) that allows you to work with PDLA (or any Perl, for
that matter) in 'real time', loading data from files, plotting,
manipulating... Anything you can do in a script, you can do in the PDLA
...
The main workhorse module. You'll include this in nearly every PDLA
program you write.
...
or from within the perldla shell.  Some of these modules you may never
...
for which you can usually use L<PDLA::NiceSlice|PDLA::NiceSlice> - this also covers
...
need to grasp to really use PDLA to its full potential.  It gets
...
PDLA has quite a few IO modules, most of which are discussed
in this summary module.

=item * L<PDLA::Tips|PDLA::Tips>

A collection of some of Tuomas's ideas for making good use of PDLA.

=item * L<PDLA::BadValues|PDLA::BadValues>

Explains what bad values are and how and why they are implemented.

=item * Selections from L<Inline::Pdlapp|Inline::Pdlapp>

Although writing PDLA::PP code is considered an Advanced topic, and is
covered in the next section, you should be aware that it is possible 
(and surprisingly simple) to write PDLA-aware code.  You needn't read
...
documentation is contained in L<PDLA::PP-Inline>.
...
file that lists all of the PDLA modules on your computer. There are many
...
Complex number support.  No, PDLA does not have complex number
support built into the core, but this should help you out.

=item * L<PDLA::FFT|PDLA::FFT>

PDLA's own Fast Fourier Transform.  If you have FFTW, then you
should probably make use of it; this is PDLA's internal
implementation and should always be available.

=item * GSL

PDLA does not have bindings for every sub-library in the GNU Scientific
Library, but it has quite a few. If you have GSL installed on your
machine then chances are decent that your PDLA has the GSL bindings.
For a full list of the GSL bindings, check L<PDLA::Index|PDLA::Index>.

=item * L<PDLA::Func|PDLA::Func>

A somewhat uniform interface to the different interpolation modules in PDLA.
...
other places, bad values are used in L<PDLA::Graphics::PLplot|PDLA::Graphics::PLplot>'s
...
order, you should consider using PDLA::DiskCache.

=item * L<PDLA::Char|PDLA::Char>

A PDLA subclass that allows you to store and manipulate collections
of fixed-length character strings using PDLA.
...
others.  This module is included in the standard call to C<use PDLA>, but
not in the Lite versions.

=item * L<PDLA::Matrix|PDLA::Matrix>

Provides a few functions that use the standard mathematical Matrix
notation of row-column indexing rather than the PDLA-standard column-row.
...
a system of linear equations.  Though it is not built on L<PDLA::Matrix|PDLA::Matrix>,
...
function, instead of complaining and croaking, PDLA will go hunt
...
PDLA scripts and/or perldla shell commands.

=item * L<PDLA::Options|PDLA::Options>
...
you face the problem of processing that hashref.  L<PDLA::Options|PDLA::Options>
...
couldn't find any.)  Note this module does not depend on PDLA for its
usage or installation.

=item * L<PDLA::pdladoc|PDLA::pdladoc>

Ever fired-up the perldla shell just to look up the help for a particular
function?  You can use C<pdladoc> instead.  This shell script extracts
information from the help index without needing to start the perldla
shell.
...
modules that use PDLA, or if you work on PDLA maintenance.  These modules
can be difficult to use, but enable you to tackle some of your harder
problems.

=over

=item * L<PDLA::Lite|PDLA::Lite>, L<PDLA::LiteF|PDLA::LiteF>

Lite-weight replacements for C<use PDLA>, from the standpoint of
namespace pollution and load time.

=item * L<Inline::Pdlapp|Inline::Pdlapp>

This was mentioned earlier.  Before you begin reading about L<PDLA::PP|PDLA::PP>
(next), you should remind yourself about how to use this.
L<Inline::Pdlapp|Inline::Pdlapp> will help you experiment with L<PDLA::PP|PDLA::PP> without having
to go through the trouble of building a module and constructing
makefiles (but see L<PDLA::pptemplate|PDLA::pptemplate> for help on that).

=item * L<PDLA::PP|PDLA::PP>

The PDLA Pre-Processor, which vastly simplifies making you
C or Fortran code play with Perl and piddles.  Most of PDLA's
basic functionality is written using PDLA::PP, so if you're
thinking about how you might integrate some numerical
library written in C, look no further.

=item * L<PDLA::pptemplate|PDLA::pptemplate>

A script that automates the creation of modules that use L<PDLA::PP|PDLA::PP>,
...
libraries.  This is an alternative to using L<PDLA::PP|PDLA::PP>.
The major difference between PDLA::PP and PDLA::CallExt is
that the former will handle threading over implicit thread
dimensions for you, whereas PDLA::CallExt simply calls an
external function.  PDLA::PP is generally the recommended
way to interface your code with PDLA, but it wouldn't be Perl
if there wasn't another way to do it.

=item * PDLA::Config

Defines the C<%PDLA::Config> hash, which has lots of useful
information pertinent to your PDLA build.

=item * L<PDLA::Doc|PDLA::Doc>

Explanation of the PDLA documentation conventions, and an
interface to the PDLA Documentation parser.  Following these
guidelines when writing documentation for PDLA functions will
ensure that your wonderful documentation is accessible from
the perldla shell and from calls to C<barf>.  (Did you notice
that C<barf> used your documentation?  Time to reread
L<PDLA::Core|PDLA::Core>...)

=item * L<PDLA::Exporter|PDLA::Exporter>
...
Simply defines the scalar C<$PDLA::Version::Version> with the current
version of PDLA, as defined in PDLA.pm.  This is most useful if you
distribute your own module on CPAN, use L<PDLA::Lite|PDLA::Lite> or L<PDLA::LiteF|PDLA::LiteF>
and want to make sure that your users have a recent-enough version of
PDLA.  Since the variable is defined in PDLA.pm, you don't need this
module if you C<use PDLA>.
...
=item * L<PDLA::Core::Dev|PDLA::Core::Dev>

Provides some decently useful functions that are pretty
much only needed by the PDLA Porters.

=item * L<PDLA::API|PDLA::API>

Explains how to make a piddle I<by hand>, from Perl or your
C source code, using the PDLA API.

=item * L<PDLA::Internals|PDLA::Internals>

Explains the nitty-gritty of the PDLA data structures.  After reading
this (a few times :), you should be able to create a piddle completely
from scratch (i.e. without using the PDLA API).  Put a little
differently, if you want to understand how PDLA::PP works, you'll need
to read this.
...
These documents are part of the PDLA distribution but don't
seem to fit anywhere above.

=over

=item * L<PDLA::Delta|PDLA::Delta>

Explains how things changed from PDLA 1.x to PDLA 2.x

=item * L<PDLA::Graphics2D|PDLA::Graphics2D>

Supposed to be a uniform interface to plotting 2D graphics.

=item * L<PDLA::Dataflow|PDLA::Dataflow>

Covers data flow, a cool feature to be sure, but from what I can tell
it is not currently supported.

=item * L<PDLA::Lvalues|PDLA::Lvalues>

PDLA started before Perl 5.6, which introduce subs that could return
lvalues. I'm pretty sure that this module has been rolled into the core
and is no longer necessary.


=item * L<PDLA::Tests|PDLA::Tests>

Defines a few PDLA::PP tests.  This code should be moved into the testing
...
=item * L<PDLA::Gaussian|PDLA::Gaussian>
