...
sub BUILD {
    ...
    my $client = Google::gRPC::Client->new(
        target     => 'dataplex.googleapis.com:443',
        auth_token => $token,
    );
...
Google::Cloud::Dataplex::V1 - Google Cloud Dataplex V1 API Client

=head1 SYNOPSIS

    use Google::Cloud::Dataplex::V1;
    use Google::Auth;

    my $auth = Google::Auth->default();
    my $client = Google::Cloud::Dataplex::V1->new(credentials => $auth);

=head1 DESCRIPTION

Google Cloud Dataplex V1 API Client over high-performance gRPC transport.
