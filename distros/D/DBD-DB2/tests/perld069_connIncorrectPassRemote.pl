####################################################################
# TESTCASE: 		perld069_connIncorrectPassRemote.pl
# DESCRIPTION: 		Incorrect password in conn. string
...
$string = "dbi:DB2:DATABASE=$DATABASE; HOSTNAME=$HOSTNAME; PORT=$PORT; PROTOCOL=$PROTOCOL; UID=$USERID; PWD=$fake_password;";
$dbh = DBI->connect($string, $userid, $fake_password, {PrintError => 0});
