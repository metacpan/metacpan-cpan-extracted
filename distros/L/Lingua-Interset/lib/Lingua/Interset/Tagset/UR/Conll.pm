# ABSTRACT: Driver for the tagset of the Hyderabad Urdu Treebank, as used in the CoNLL data format.
# Documentation:
...
###!!! There are only subtle differences between the Hindi and the Urdu tagsets (e.g. Urdu has NN-Ez for the ezafe construction).
###!!! They have yet to be accounted for. The following table has been copied over from the older driver; it is not used currently
###!!! here but it shall serve as a reference for future improvements of this driver.
my %pos_map = (
		"CC"	=> 'conj',
		"DEM"	=> 'adj',
		"INJ"	=> 'int',
...
Interset driver for the tagset of the Urdu treebank from Hyderabad,
...
In the case of Urdu, the CoNLL data had to be filtered before collecting the input tags.
...
Unlike in other CoNLL tagsets, some of the features in the Urdu treebank must not be
