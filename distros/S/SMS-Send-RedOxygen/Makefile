# This Makefile is for the SMS::Send::RedOxygen extension to perl.
#
...
#     ABSTRACT => q[SMS::Send driver for RedOxygen.com RedSMS]
#     AUTHOR => [q[Craig Ringer]]
#     BUILD_REQUIRES => { Test::More=>q[0.47], ExtUtils::MakeMaker=>q[6.6302] }
#     CONFIGURE_REQUIRES => {  }
#     DISTNAME => q[SMS-Send-RedOxygen]
#     LICENSE => q[perl]
...
MAKE = make
FULLEXT = SMS/Send/RedOxygen
BASEEXT = RedOxygen
PARENT_NAME = SMS::Send
DLBASE = $(BASEEXT)
VERSION_FROM = lib/SMS/Send/RedOxygen.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/SMS/Send/RedOxygen.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/SMS/Send
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/SMS/Send
...
TO_INST_PM = lib/SMS/Send/RedOxygen.pm

PM_TO_BLIB = lib/SMS/Send/RedOxygen.pm \
...
DISTVNAME = SMS-Send-RedOxygen-1.06
...
manifypods : pure_all  \
	lib/SMS/Send/RedOxygen.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/SMS/Send/RedOxygen.pm $(INST_MAN3DIR)/SMS::Send::RedOxygen.$(MAN3EXT) 
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>SMS::Send driver for RedOxygen.com RedSMS</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Craig Ringer</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,010,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Carp::" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="LWP::" VERSION="5.69" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/SMS/Send/RedOxygen.pm blib/lib/SMS/Send/RedOxygen.pm 
