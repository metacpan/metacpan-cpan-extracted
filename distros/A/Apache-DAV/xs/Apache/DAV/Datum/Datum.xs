...
void Apache__DAV__Datum_new_init (pTHX_ Apache__DAV__Datum  obj, SV * item, int overwrite) {
    ...
    if (SvTYPE(item) == SVt_PVMG) 
        ...
        if ((tmpsv = hv_fetch((HV *)item, "dptr", sizeof("dptr") - 1, 0)) || overwrite) {
            char * tmpobj = ((char *)davxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef)));
            if (tmpobj)
                obj -> dptr = (char *)strdup(tmpobj);
            else
                obj -> dptr = NULL ;
        }
        if ((tmpsv = hv_fetch((HV *)item, "dsize", sizeof("dsize") - 1, 0)) || overwrite) {
            obj -> dsize = (size_t)davxs_sv2_IV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef)) ;
        }
   ; }

    else
        croak ("initializer for Apache::DAV::Datum::new is not a hash or object reference") ;
...
PREINIT:
    SV * svobj ;
    Apache__DAV__Datum  cobj ;
    SV * tmpsv ;
CODE:
    davxs_Apache__DAV__Datum_create_obj(cobj,svobj,RETVAL,malloc(sizeof(*cobj))) ;

    if (initializer) {
        if (!SvROK(initializer) || !(tmpsv = SvRV(initializer))) 
            croak ("initializer for Apache::DAV::Datum::new is not a reference") ;

        if (SvTYPE(tmpsv) == SVt_PVHV || SvTYPE(tmpsv) == SVt_PVMG)  
            Apache__DAV__Datum_new_init (aTHX_ cobj, tmpsv, 0) ;
