...
sub find_and_remove_criteria_from_shared_set {
  ...
  $paging = Google::Ads::AdWords::v201705::Paging->new({
      startIndex    => 0,
      numberResults => PAGE_SIZE
  });
  $selector = Google::Ads::AdWords::v201705::Selector->new({
      fields => [
  ...
  my @remove_criterion_operations =
    ...
    sub {
      ...
      if ($criterion->isa("Google::Ads::AdWords::v201705::Keyword")) {
        printf "Shared negative keyword with ID %d and text '%s' was " .
          "found.\n",
          $criterion->get_id(),
          $criterion->get_text();
      } elsif ($criterion->isa("Google::Ads::AdWords::v201705::Placement")) {
      ...
      my $shared_criterion_operation =
        Google::Ads::AdWords::v201705::SharedCriterionOperation->new({
          operator => 'REMOVE',
          operand  => Google::Ads::AdWords::v201705::SharedCriterion->new({
              criterion => Google::Ads::AdWords::v201705::Criterion->new(
                {id => $criterion->get_id()}
              ),
