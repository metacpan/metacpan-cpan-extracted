package DBIx::Schema::Changelog::Action::Constraint::PrimaryKeys;
...
sub add {
    ...
    my $gentime = time;
    $gentime =~ s/\.//g;
    my $name = ( defined $col->{name} ) ? $col->{name} : $gentime . '_gen';
    return _replace_spare( $actions->{primary},
        [ qq~pkey_multi_$name~, join( ',', @{ $col->{primarykey} } ) ] );
}
...
sub list_from_schema {
    my ( $self, $schema ) = @_;
    my $raw_keys =
      $self->do( $self->driver->actions->{list_table_primary_keys}, [] );
    my $cols_for_table = {};
    foreach (@$raw_keys) {
        push( @{ $cols_for_table->{ $_->{table_name} } }, $_->{column_name} );
    }

    my $pkeys = [];
    foreach my $key ( keys %$cols_for_table ) {
        push(
            @$pkeys,
            {
                type          => 'altertable',
