...
Create a new formatter object.  All C<%options> are used as global set of
...
=item $obj-E<gt>B<process>(STRING|SCALAR|ARRAY, \%params|%params)

The template is rendered inserting the C<%params>.

With a C<STRING> or C<SCALAR> (reference to a string), the template will
be parsed first with L<parseTemplate()|OODoc::Template/"Parsing">.  With an C<ARRAY>, this work
has already been done.

=item $obj-E<gt>B<processFile>(FILENAME, HASH|PAIRS)

Process the content of the file with specified C<FILENAME>.  The current
value of the C<search> path is used as path to find it.  The returns
behaves the same as L<process()|OODoc::Template/"Processing">.

If the C<FILENAME> is not found, then C<undef> is returned as output.
...
=item $obj-E<gt>B<allValuesFor>( $tag, [$attrs, $then, $else] )

Collects all values related to C<$tag> in all nestings of values.  The most
preferred is listed first.

=item $obj-E<gt>B<defineMacro>($tag, $attrs, $then, $else)

Z<>

=item $obj-E<gt>B<includeTemplate>($tag, $attrs, $then, $else)
...
=item $obj-E<gt>B<pushValues>(\%values)

Add new level of C<%values> to the known list.  The data in the HASH is
copied, and a reference to the copy returned.  The copy may be changed
afterwards.

=item $obj-E<gt>B<valueFor>( $tag, [$attrs, $then, $else] )

Lookup the value for C<$tag> in the known data.  See section L</values>
about the way this is done.  The C<$attrs> (HASH of key-value pairs)
and C<$then>/C<$else> content text references are used when the C<$tag> relates to
...
the called code fragments.  The C<STRING> syntax is described in the
DETAILS section of this manual page.

=item $obj-E<gt>B<parseTemplate>(STRING)

Try to understand the C<STRING>. Returned is a reference to a tree which
...
values C<2>, C<true>, C<monkey>, and the looked-up value for C<cnr>.
...
=item Fault: Cannot read from $fn in $file: $!

Z<>

=item Error: attribute error in '$tag'

Z<>

=item Error: cannot find macro $name

Z<>

=item Error: cannot find template file $file
...
=item Error: file or macro attribute required for template in $source

Z<>

=item Error: macro requires a name

Z<>

=item Error: macros cannot have an else part ($macro)

Z<>

=item Error: template is not a container

Z<>

=item Error: value for $tag is $value, must be single
...
This module is part of OODoc-Template version 0.19,
built on August 15, 2025. Website: F<http://perl.overmeer.net/oodoc-template/>
