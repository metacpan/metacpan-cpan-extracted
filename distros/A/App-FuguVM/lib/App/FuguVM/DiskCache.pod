...
truncated SHA-256 over each input that shapes an installed disk, and
the record follows the C<install_mode> of the configuration, because
each mode shapes the disk with different inputs:

=over 4

=item expect

The OpenBSD version, the architecture, the disk size, the mode, the
contents of the F<install.exp> script that L<App::FuguVM::Console>
resolves, and the contents of F<share/fuguvm/cache-generation>.

=item autoinstall

The same inputs, with two differences: the script is
F<autoinstall.exp>, and the contents of the response file join the
record. The response file answers every installer question, so it
shapes the disk as much as an expect script does. Two response files
give two keys, two entry directories and two immutable base images.

=item import

The OpenBSD version, the architecture, the mode, and the generation
...
The guest permits root login with a password. QEMU binds the
forwarded SSH port and the serial console port of the guest to the
configured C<bind_address>. The default is C<127.0.0.1>. With
C<bind_address 0.0.0.0>, the password is not a localhost-only secret.
See L<fuguvm(1)>.
