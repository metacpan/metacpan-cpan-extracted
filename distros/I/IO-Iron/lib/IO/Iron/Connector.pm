...
# ABSTRACT: REST API Connector, HTTP interface class.
...
sub perform_http_action {
    ...
    $log->debugf( 'REST Request: [verb=%s; href=%s; body=%s; Headers: Content-Type=%s; Authorization=%s]',
        $action_verb, $href, $request_body, $content_type, $authorization );
    $client->request(
        $action_verb,
        $href,
        $request_body,
    ...
    if ( $client->responseCode() >= $HTTP_CODE_OK_MIN && $client->responseCode() <= $HTTP_CODE_OK_MAX ) {
        ...
        $log->tracef(
            'Throwing exception in perform_http_action(): status_code=%s, response_message=%s',
            $client->responseCode(),
            $response_message
        );
        IronHTTPCallException->throw(
            ...
            error => 'IronHTTPCallException: status_code=' . $client->responseCode() . ' response_message=' . $response_message,
        );
    }
...
=for stopwords API http Params params Mikko Koivunalho
