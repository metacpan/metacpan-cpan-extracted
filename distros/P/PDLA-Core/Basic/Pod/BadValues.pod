...
within L<perldla|PDLA::perldla> or L<pdla2|PDLA::Perldl2> gives a demonstration of some of the things
...
L<perldla|PDLA::perldla> or L<pdla2|PDLA::Perldl2> or the C<-b> option to
L<pdladoc|PDLA::pdladoc>.  This facility is currently a 'proof of concept'
...
the I<children> of a piddle, so
   ...
   pdla> $a = zeroes(20,30);
   pdla> $b = $a->slice('0:10,0:10');
   pdla> $c = $b->slice(',(2)');
   pdla> print ">>c: ", $c->badflag, "\n";
   >>c: 0
   pdla> $a->badflag(1);
   pdla> print ">>c: ", $c->badflag, "\n";
   >>c: 1
...
pdl structure) - this would then remove the current problem of:
 ...
 pdla> $a = byte( 1, 2, byte->badvalue, 4, 5 );
 pdla> p $a;
 [1 2 255 4 5]
 pdla> $a->badflag(1)
 pdla> p $a;
 [1 2 BAD 4 5]
 pdla> byte->badvalue(0);
 pdla> p $a;
 [1 2 255 4 5]
...
accessible from the C<perldla> or C<pdla2> shells - using the C<badinfo> command - and the C<pdladoc>
