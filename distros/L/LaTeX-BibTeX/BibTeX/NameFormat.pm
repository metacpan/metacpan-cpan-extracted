...
method.  NAME must be a C<LaTeX::BibTeX::Name> object (i.e., a pre-split
name); C<apply> returns a string containing the parts of the name
formatted according to the C<LaTeX::BibTeX::NameFormat> structure it is
called on.
...
L<LaTeX::BibTeX::Name>), it's actually quite simple.  There are really
only three steps to worry about: split the name (create a
C<LaTeX::BibTeX::Name> object), create and customize the format
(C<LaTeX::BibTeX::NameFormat> object), and apply the format to the name.

The first step is covered in L<LaTeX::BibTeX::Name>; here's a brief
example:

   $orig_name = 'Charles Louis Xavier Joseph de la Vall{\'e}e Poussin';
   $name = new LaTeX::BibTeX::Name $orig_name;

The various parts of the name can now be accessed through
C<LaTeX::BibTeX::Name> methods; for instance C<$name-E<gt>part('von')>
returns the list C<("de","la")>.

Creating the name format is equally simple:

   $format = new LaTeX::BibTeX::NameFormat ('vljf', 1);
...
done with C<LaTeX::BibTeX::NameFormat>'s C<apply> method:

   $formatted_name = $format->apply ($name);

while the second uses C<LaTeX::BibTeX::Name>'s C<format> method:

   $formatted_name = $name->format ($format);

which is just a wrapper around C<LaTeX::BibTeX::NameFormat::apply>.  In
...
L<LaTeX::BibTeX::Entry>, L<LaTeX::BibTeX::Name>, L<bt_format_names>.
