...
This document describes QRCode::Encoder version 0.005.
...
=head2 B<< qr_best_params >>

   my %args = qr_best_params($octets, %opts);  # OR
   my %args = qr_best_params(%opts);

Guess the best params that are not already defined in C<%opts>.
...
=head2 B<< qr_mode >>
   ...
   my $mode = qr_mode($octets);

Guess the best mode from the octets, returns a string among C<numeric>,
C<alphanumeric>, C<kanji>, and C<byte>.
