...
BEGIN
{
    use_ok( 'Net::API::Stripe::Event::Data' ) || BAIL_OUT( "Unable to load perl module 'Net::API::Stripe::Event::Data'" );
};
can_ok( 'Net::API::Stripe::Event::Data', 'object' );
can_ok( 'Net::API::Stripe::Event::Data', 'previous_attributes' );
...
my $json_file = $sample_dir->child( 'data.json' ) ||
...
if( $json_file->exists )
{
    if( !$json_file->can_read )
    {
        ...
        BAIL_OUT( "Unable to read json file $rel for Stripe class 'data'" );
    }
    elsif( !$json_file->is_empty )
    {
        $code = $json_file->load_json ||
            BAIL_OUT( "Failed to load json data for Stripe class 'data': " . $json_file->error );
    }
}
my $obj = scalar( keys( %$code ) ) ? Net::API::Stripe::Event::Data->new( $code ) : Net::API::Stripe::Event::Data->new;
isa_ok( $obj => 'Net::API::Stripe::Event::Data' );
