...
void Apache__DAV__PropsResult_new_init (pTHX_ Apache__DAV__PropsResult  obj, SV * item, int overwrite) {
    ...
    if (SvTYPE(item) == SVt_PVMG) 
        ...
        if ((tmpsv = hv_fetch((HV *)item, "propstats", sizeof("propstats") - 1, 0)) || overwrite) {
            obj -> propstats = (dav_text *)davxs_sv2_Apache__DAV__Text((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef)) ;
        }
        if ((tmpsv = hv_fetch((HV *)item, "xmlns", sizeof("xmlns") - 1, 0)) || overwrite) {
            obj -> xmlns = (dav_text *)davxs_sv2_Apache__DAV__Text((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef)) ;
        }
   ; }

    else
        croak ("initializer for Apache::DAV::PropsResult::new is not a hash or object reference") ;
...
PREINIT:
    SV * svobj ;
    Apache__DAV__PropsResult  cobj ;
    SV * tmpsv ;
CODE:
    davxs_Apache__DAV__PropsResult_create_obj(cobj,svobj,RETVAL,malloc(sizeof(*cobj))) ;

    if (initializer) {
        if (!SvROK(initializer) || !(tmpsv = SvRV(initializer))) 
            croak ("initializer for Apache::DAV::PropsResult::new is not a reference") ;

        if (SvTYPE(tmpsv) == SVt_PVHV || SvTYPE(tmpsv) == SVt_PVMG)  
            Apache__DAV__PropsResult_new_init (aTHX_ cobj, tmpsv, 0) ;
