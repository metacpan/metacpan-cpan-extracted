...
sub import {
    ...
    *Char::ord           = \&Windows1258::ord;
    *Char::ord_          = \&Windows1258::ord_;
    *Char::reverse       = \&Windows1258::reverse;
    *Char::getc          = \&Windows1258::getc;
    *Char::length        = \&Windows1258::length;
    *Char::substr        = \&Windows1258::substr;
    *Char::index         = \&Windows1258::index;
    *Char::rindex        = \&Windows1258::rindex;
    *Char::eval          = \&Windows1258::eval;
    *Char::escape        = \&Windows1258::escape;
    *Char::escape_token  = \&Windows1258::escape_token;
    *Char::escape_script = \&Windows1258::escape_script;
}
...
# ${Ewindows1258::eS}          = qr{(?>[^\x09\x0A\x0B\x0C\x0D\x20])};
...
${Ewindows1258::not_lower_i}   = qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Ewindows1258::not_lower_i} = qr{(?>[\x00-\xFF])};                   # older Perl compatible
...
${Ewindows1258::not_upper_i}   = qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Ewindows1258::not_upper_i} = qr{(?>[\x00-\xFF])};                   # older Perl compatible
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '${Ewindows1258::not_lower_i}',
                '[:^upper:]'  => '${Ewindows1258::not_upper_i}',
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Ewindows1258::uc($char);
                        my $fc = Ewindows1258::fc($char);
                        if ($uc eq $fc) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $fc = Ewindows1258::fc($char)) ne $char) {
                $pattern .= $fc;
            }
            else {
        ...
        my $matchsub = sub { Ewindows1258::fc($_[0]) =~ /\A $pattern \z/xms };
...
sub Windows1258::getc(;*@) {
    ...
    for my $length ($length[0] .. $length[-1]) {
        ...
        if (exists $range_tr{CORE::length($getc)}) {
            if ($getc =~ /\A ${Ewindows1258::dot_s} \z/oxms) {
...
my $qq_substr  = qr{(?> Char::substr | Windows1258::substr | CORE::substr | substr ) (?>\s*) \( $qq_paren \)
                 }xms;
...
sub Windows1258::escape(;$) {
    ...
    while (not /\G \z/oxgc) { # member
        $e_script .= Windows1258::escape_token();
    }
...
#
# escape Windows-1258 token of script
#
sub Windows1258::escape_token {
...
# subroutines of package Ewindows1258
    ...
    elsif (/\G \b Windows1258::eval       (?= (?>\s*) \{ )                    /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval Char::escape'; }
    elsif (/\G \b Windows1258::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval Windows1258::escape'; }
...
# subroutines of package Ewindows1258
        ...
        elsif ($string =~ /\G \b Windows1258::eval       (?= (?>\s*) \{ )                    /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::eval \b                                        /oxgc) { $e_string .= 'eval Char::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b Windows1258::eval \b                                        /oxgc) { $e_string .= 'eval Windows1258::escape'; $slash = 'm//'; }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Windows1258::re_t=%s; while($Windows1258::re_t =~ %s){%s local $^W=0; local $Windows1258::re_r=%s; %s$Windows1258::re_t="$`$Windows1258::re_r$'"; pos($Windows1258::re_t)=length "$`$Windows1258::re_r"; } return $Windows1258::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Windows1258::re_r=CORE::eval $Windows1258::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Windows1258::re_r=%s; %s"%s$Windows1258::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Windows1258::re_n=0; while(%s =~ %s){%s local $^W=0; local $Windows1258::re_r=%s; %s%s="$`$Windows1258::re_r$'"; pos(%s)=length "$`$Windows1258::re_r"; $Windows1258::re_n++} return %s$Windows1258::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Windows1258::re_r=%s; %s%s="%s$Windows1258::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Windows1258::re_r=%s; %s%s="%s$Windows1258::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Windows1258::re_r=CORE::eval $Windows1258::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item Split string
  ...
  at all, Ewindows1258::split returns the original string as a single substring, If it
  ...
  @chars  = Ewindows1258::split(//,  $word);
  @fields = Ewindows1258::split(/:/, $line);
  @words  = Ewindows1258::split(" ", $paragraph);
  @lines  = Ewindows1258::split(/^/, $buffer);
  ...
  print join(":" => Ewindows1258::split(/ */, "hi there"));
  ...
  Ewindows1258::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Ewindows1258::split(/:/, $_, 3);
  ...
  splits on whitespace just as Ewindows1258::split with no arguments does. Thus,
  Ewindows1258::split(" ") can be used to emulate awk's default behavior. In contrast,
  Ewindows1258::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Ewindows1258::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Ewindows1258::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Ewindows1258::split()) {
          $count{$word}++;
      }
  }

  The inverse of Ewindows1258::split is join, except that join can only join with the
...
=item Transliteration
  ...
  print Ewindows1258::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'
...
=item Chop string
  ...
  character chopped. The Ewindows1258::chop subroutine is used primary to remove the newline
  ...
  since Ewindows1258::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Ewindows1258::chop a literal, only a variable. If you Ewindows1258::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Ewindows1258::chop(@lines);

  You can Ewindows1258::chop anything that is an lvalue, including an assignment:

  Ewindows1258::chop($cwd = `pwd`);
  Ewindows1258::chop($answer = <STDIN>);

  This is different from:

  $answer = Ewindows1258::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Ewindows1258::chop returns the character
  ...
  In the most general case, Ewindows1258::chop can be expressed using substr:

  $last_code = Ewindows1258::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Ewindows1258::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Ewindows1258::chop;
...
=item Index string
  ...
  $byte_pos = Ewindows1258::index($string,$substr,$byte_offset);
  $byte_pos = Ewindows1258::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Ewindows1258::index($string, $lookfor, $byte_pos)) > -1) {
...
=item Reverse index string
  ...
  $byte_pos = Ewindows1258::rindex($string,$substr,$byte_offset);
  $byte_pos = Ewindows1258::rindex($string,$substr);

  This subroutine works just like Ewindows1258::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Ewindows1258::index).
  ...
  while (($byte_pos = Windows1258::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item Lower case string
  ...
  You can use the Ewindows1258::fc subroutine for case-insensitive comparisons via Windows1258
  software.
...
=item Upper case string
  ...
  in interpolated strings. For titlecase, use Ewindows1258::ucfirst instead.
...
=item Upper case first character of string
  ...
  $titlecase = Ewindows1258::ucfirst(substr($word,0,1)) . Ewindows1258::lc(substr($word,1));
  ...
  $do_not_use = Ewindows1258::ucfirst(Ewindows1258::lc($word));
...
=item Fold case string
  ...
  $fc = Ewindows1258::fc($string);
  $fc = Ewindows1258::fc_;

  New to Windows1258 software, this subroutine returns the full Unicode-like casefold of
  ...
  Ewindows1258::fc($a) eq Ewindows1258::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because Windows1258 software has always used casefolding semantics for
...
=item Make character
  ...
  that $code in the character set. For example, Ewindows1258::chr(65) is "A" in either
  ASCII or Windows-1258, not Unicode. For the reverse of Ewindows1258::chr, use Windows1258::ord.
...
=item Filename expansion (globbing)
  ...
  The Ewindows1258::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Ewindows1258::glob('"*e f*"');
  @spacies = Ewindows1258::glob(q("*e f*"));
