...
Bio::Genex::UserSec - Methods for processing data from the GeneX DB
 table: UserSec
...
=head1 SYNOPSIS
  ...
  my $UserSec = Bio::Genex::UserSec->new(id=>47);

  # retrieve data from the DB for all columns
  $UserSec->fetch();

  # creating an instance, without pre-fetching all columns
  my $UserSec = new Bio::Genex::UserSec(id=>47);

  # creating an instance with pre-fetched data
  my $UserSec = new Bio::Genex::UserSec(id=>47, 'fetch_all'=>1);

  # retrieving multiple instances via primary keys
  my @objects = Bio::Genex::UserSec->get_objects(23,57,98)


  # retrieving all instances from a table
  my @objects = Bio::Genex::UserSec->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $UserSec->id();

  # or specifically
  my $us_pk_val = $UserSec->us_pk();

  # retreving other DB column attributes
  my $con_fk_val = $UserSec->con_fk();
  $UserSec->con_fk($value);

  my $login_val = $UserSec->login();
  $UserSec->login($value);

  my $password_val = $UserSec->password();
  $UserSec->password($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::UserSec is
UserSec).


Most applications will first create an instance of Bio::Genex::UserSec
...
Bio::Genex::UserSec can access: I<raw> foreign key attributes,
...
Class Bio::Genex::UserSec defines the following utility variables for assisting
programmers to access the UserSec table.

=over 4

=item $Bio::Genex::UserSec::LIMIT
...
=item $Bio::Genex::UserSec::USE_CACHE
...
instance of the class via the Bio::Genex::UserSec->methodname() syntax.
...
linking table class. For Bio::Genex::UserSec it returns 0, since it is
...
table classes, and for Bio::Genex::UserSec it returns the value 'us_pk';


=item table_name()

Returns the name of the DB table represented by this class. For
Bio::Genex::UserSec it returns 'UserSec';
...
C<insert_db()>.
  ...
  my UserSec = Bio::Genex::UserSec->new();
  UserSec->con_fk('some_value');
  UserSec->insert_db($dbh);
...
C<update_db()>.
  ...
  my $dbh = Bio::Genex::current_connection(USER=>$SU_USERNAME,
                                      PASSWORD=>$SU_PASSWORD);
  my UserSec = Bio::Genex::UserSec->new(id=>43);
  UserSec->con_fk('some_value');
  UserSec->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('us_pk' for module Bio::Genex::UserSec).
...
This method is used to retrieve multiple instances of class Bio::Genex::UserSec
...
list as a primary key for the UserSec table and returns a single
instance for each entry.

B<WARNING>: Passing incorrect id values to C<get_objects()> will cause
a warning from C<Bio::Genex::UserSec::initialize()>. Objects will be
...
instance of class Bio::Genex::UserSec.
...
=item @id_list = grouplink_fk()

=item @obj_list = grouplink_obj()

This is an attribute of type ONE_TO_MANY_LINK and refers to class
L<Bio::Genex::GroupLink>. The raw accessor method, C<grouplink_fk()> returns a list of
foreign key ids. The OO accessor method, C<grouplink_obj()> returns a
list of objects of class Bio::Genex::GroupLink.
...
(and for class Bio::Genex::UserSec it is synonomous with the
C<us_pk()>method). The C<id()> method can be useful in writing
generic methods because it avoids having to know the name of the
primary key column. 

=item us_pk()

This is the primary key attribute for Bio::Genex::UserSec. It has no setter method. 
...
Methods for the login attribute.
...
Methods for the password attribute.
...
on Tue Jan 23 22:53:40 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=UserSec --support=GroupLink
