package LCFG::Build::Tool::DevDebianPkg;    # -*-perl-*-
use strict;
use warnings;
...
# $Revision: 35384 $
# $HeadURL: https://svn.lcfg.org/svn/source/tags/LCFG-Build-Tools/LCFG_Build_Tools_0_9_19/lib/LCFG/Build/Tool/DevDebianPkg.pm.in $
...
override 'abstract' => sub {
    return q{Build binary debs from the development source tree};
};
...
override 'execute' => sub {
    ...
    my $dscname = $spec->deb_dscname;
    my $dscfile = File::Spec->catfile( $outdir, $dscname );

    my $result = LCFG::Build::Utils::Debian->build( $outdir, $dscfile, \%opts );

    my @packages = @{ $result->{packages} };
    if ( !$self->sourceonly && scalar @packages > 0 ) {
        $self->log("Successfully built packages for $module");

        for my $pkg ( @packages ) {
            $self->log("Package: $pkg");
        }
    }
...
=head1 NAME
    ...
    LCFG::Build::Tool::DevDebianPkg - LCFG software packaging tool

=head1 VERSION

    This documentation refers to LCFG::Build::Tool::DevDebianPkg version 0.9.19

=head1 SYNOPSIS

    my $tool = LCFG::Build::Tool::DevDebianPkg->new( dir => '.' );

    $tool->execute;

    my $tool2 = LCFG::Build::Tool::DevDebianPkg->new_with_options();
...
out prior to distribution. Binary Debian packages will then be
generated from the gzipped source tar file using the generated
metadata.
...
binary .deb files, etc) are stored into a directory named after the
...
should be built as well as the source.

=item sign

This is a boolean value which controls whether C<debuild> should
embed a GPG signature in the package.

=item deps

This is a boolean value which controls whether C<debuild> checks the
dependencies specified in the C<debian/control> file when it builds
the source or binary packages.
...
L<LCFG::Build::Tool::Pack> to generate a gzipped source tar file and
the build metadata files for the various supported platforms (e.g. a
specfile for creating binary RPMs).  Once that is done C<debuild> is
used to create binary packages from the generated source tar file and
specfile.
...
For building Debian packages you will also need
L<LCFG::Build::Utils::Debian>. This uses the debuild(1) and
dpkg-source(1) utilities. You will need to install, at least, the
devscripts package.
