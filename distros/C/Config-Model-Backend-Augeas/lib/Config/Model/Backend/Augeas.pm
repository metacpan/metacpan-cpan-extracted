...
sub read {
    ...
    $self->{augeas_obj} ||= Config::Augeas->new(
        root => $args{root},
        save => $args{save} );
...
sub augeas_deep_match {
    ...
    while (@worklist) {
        ...
        $logger->debug("read-augeas $p/* matches paths: @newpath");
...
sub write {
    ...
    foreach my $param (qw/config_dir file/) {
        if ( not defined $args{$param} ) {
            Config::Model::Exception::Model->throw(
                error  => "write_augeas error: model " . "does not specify '$param' for Augeas ",
                object => $self->{node} );
        }
    }
    ...
    my %to_set = $self->copy_in_augeas( $augeas_obj, $mainpath, $set_in, $args{sequential_lens} );
...
sub save {
    ...
    $self->{augeas_obj}->save
        || die "Augeas save failed" . $self->{augeas_obj}->print("/augeas/$mainpath/*");
}
...
sub copy_in_augeas {
    ...
    $view_scanner->scan_node( [ $mainpath, $augeas_obj, $set_in, \%is_seq_lens ], $self->{node} );
}
...
sub list_element_cb {
    ...
    @matches = sort map { $augeas_obj->match( $_ . "/*[label() != '#comment']" ); } @matches
        unless $is_seq;
    ...
    if ($is_seq) {
        ...
        foreach my $idx (@idx) {
            ...
            $logger->debug( "copy_in_augeas: New list path $match{$i} " . "for index $i" );
        }
    }
...
sub hash_element_cb {
    ...
    my $logger = get_logger('Data::Write');
    $logger->debug(
        "copy_in_augeas: Hash path $p matches (seq $is_seq):\n\t" . join( "\n\t", @matches ) );
...
sub node_content_cb {
    ...
    if ( scalar @element == 1 and $element[0] eq $set_in ) {
        ...
        $scanner->scan_element( [ $p, $augeas_obj, $set_in, $is_seq_lens ], $node, $element[0] );
    }
    else {
        my @matches = $augeas_obj->match( $p . "/*[label() != '#comment']" );
        ...
        foreach (@element) {
            ...
            $scanner->scan_element( [ $scan_path, $augeas_obj, $set_in, $is_seq_lens ], $node, $_ );
        }
