...
Please note that the C<:discard[on]> and C<:discard[off]> events will I<always> happen if specified in the grammar, at any level of parsing, regardless if you the grammar's C<parse()> method. This is because these are internal events, not visible by the user.
