...
$rows = 0;
eval {
    my $sth = $mysql->prepare(q{SELECT id, name FROM test WHERE ? = ?});
    $sth->execute(1,0);
    while (my $record = $sth->fetch()) {
        ++$rows;
    }
...
$rows = 0;
eval {
    my $sth = $mysql->prepare(q{SELECT id, name FROM test WHERE id = ?});
    $sth->execute(1);
    while (my $record = $sth->fetch()) {
        if (defined($record->[0]) && defined($record->[1])) {
            ++$rows;
...
$rows = 0;
eval {
    my $sth = $mysql->prepare(q{SELECT id, name FROM test WHERE name=?});
    $sth->execute('foo');
    while (my $record = $sth->fetch()) {
        if (defined($record->[0]) && defined($record->[1])) {
            ++$rows;
