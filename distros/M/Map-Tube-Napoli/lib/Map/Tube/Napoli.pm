# -*- perl -*-
#
# Author: Gisbert W. Selke, TapirSoft Selke & Selke GbR.
#
# Copyright (C) 2025--2026 Gisbert W. Selke. All rights reserved.
# This package is free software; you can redistribute it and/or
# modify it under the same terms as Perl itself.
#
# Mail: gws@cpan.org
#
package Map::Tube::Napoli;
use 5.14.0;
use version 0.77 ( );
use strict;
use warnings;

our $VERSION = version->declare('v0.1.2');

=encoding utf8

=head1 NAME

Map::Tube::Napoli - Interface to the Napoli tube and funicular map

=cut

use File::Share ':all';
use Moo;
use namespace::clean;

has xml => ( is => 'ro', default => sub { return dist_file('Map-Tube-Napoli', 'napoli-map.xml') } );

with 'Map::Tube';

=head1 SYNOPSIS

	use Map::Tube::Napoli;
	my $tube = Map::Tube::Napoli->new();

	my $route = $tube->get_shortest_route( 'Materdei', 'Palazzolo');

    print "Route: $route\n";

=head1 DESCRIPTION

This module allows to find the shortest route between any two given tube or tram
stations in Napoli. All interesting methods are provided by the role L<Map::Tube>.

=head1 METHODS

=head2 CONSTRUCTOR

	use Map::Tube::Napoli;
	my $tube = Map::Tube::Napoli->new();

The only argument, C<xml>, is optional; if specified, it should be a code ref
to a function that returns either the path the XML map file, or a string
containing this XML content. The default is the path to F<napoli-map.xml>
that is a part of this distribution. For further information see L<Map::Tube>.

=head2 xml()

This read-only accessor returns whatever was specified as the XML source at
construction.

=head1 ERRORS

If something goes wrong, maybe because the map information file was corrupted,
the constructor will die.

=head1 BUGS

Please report any bugs or feature requests through the web interface at
L<https://github.com/gwselke/Map-Tube-Napoli/issues>. I will be
notified and then you'll automatically be notified of progress on your
bug when (and if) I make changes.

=head1 AUTHOR

Gisbert W. Selke, TapirSoft Selke & Selke GbR.

=head1 COPYRIGHT AND LICENCE

The data for the XML file were taken from
https://www.metroitalia.info/napoli/trasporti/metropolitana/,
https://www.metroitalia.info/napoli/trasporti/tram/,
https://www.metroitalia.info/napoli/trasporti/funicolare/,
http://www.oraricircumvesuviana.it/fermate,
https://www.napolike.it/trasporti-pubblici-a-napoli/linea-cumana-di-napoli.
The module itself is free software; you may redistribute and/or modify
it under the same terms as Perl itself.

=head1 SEE ALSO

L<Map::Tube>, L<Map::Tube::GraphViz>.

=cut

1;
