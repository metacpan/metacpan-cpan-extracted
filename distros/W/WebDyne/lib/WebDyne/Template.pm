...
C<WebDyne::Template> is a chaining and filter module that compiles a wrapper template page and inserts the current page output into named blocks inside that template.

When imported from a page C<__PERL__> block, it switches the page to C<WebDyne::Chain>, registers itself as both a chain component and filter, and stores the nominated template name in page metadata.
...
Resolve the active template filename from page metadata or C<dir_config('WebDyneTemplate')>.



=item *

B<source_mtime($srce_mtime)>
...
B<filter($self, $data_content_ar, $meta_content_hr)>
