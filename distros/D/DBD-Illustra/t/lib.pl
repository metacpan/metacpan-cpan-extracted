...
$verbose=!!$ENV{DBD_ILLUSTRA_VERBOSE};

#
#   Driver names; EDIT THIS!
#
$mdriver = 'Illustra';
$dbdriver = 'Illustra';  # $dbdriver is usually just the same as $mdriver.
...
sub ErrMsg (@) { if ($verbose) { print (@_); } }
sub ErrMsgF (@) { if ($verbose) { printf (@_); } }
