...
#
package PDLA::Image2D;

@EXPORT_OK  = qw(  PDLA::PP conv2d PDLA::PP med2d PDLA::PP med2df PDLA::PP box2d PDLA::PP patch2d PDLA::PP patchbad2d PDLA::PP max2d_ind PDLA::PP centroid2d  cc8compt cc4compt PDLA::PP ccNcompt polyfill  pnpoly  polyfillv  rotnewsz PDLA::PP rot2d PDLA::PP bilin2d PDLA::PP rescale2d  fitwarp2d applywarp2d PDLA::PP warp2d  warp2d_kernel PDLA::PP warp2d_kernel );
...
use PDLA;  # ensure qsort routine available
use PDLA::Math;
use Carp;
...
such as L<fftconvolve()|PDLA::FFT/fftconvolve()> in C<PDLA::FFT>,
will be quicker.
...
*cc8compt = \&PDLA::cc8compt;

sub PDLA::cc4compt{
return ccNcompt(shift,4);
}
*cc4compt = \&PDLA::cc4compt;
...
sub PDLA::polyfill {
	...
	if($opt) {
		...
		if( $parsed->current->{'Method'} eq 'pnpoly' ) {
			PDLA::pnpolyfill_pp($im,$ps,$colour);
		}
	}
	else
	{
		PDLA::polyfill_pp($im,$ps,$colour);
	}
	return $im;

}

*polyfill = \&PDLA::polyfill;
...
and vectorized for PDLA by Karl Glazebrook.
...
sub PDLA::pnpoly {
	barf('Usage: $mask = pnpoly($img, $ps);') unless(@_==2 || @_==4);
 	...
 	unless( defined $vertx ) {
		...
		return PDLA::pnpoly_pp($tx,$ty);
	}

	my $testx = PDLA::Core::topdl($tx)->dummy(0);
	my $testy = PDLA::Core::topdl($ty)->dummy(0);
	my $vertxj = PDLA::Core::topdl($vertx)->rotate(1);
	my $vertyj = PDLA::Core::topdl($verty)->rotate(1);
...
tracking, consider using L<PDLA::Transform::map|PDLA::Transform/map>
...
sub PDLA::fitwarp2d {
    ...
    my $opts = PDLA::Options->new( { FIT => ones(byte,$nf,$nf), THRESH => 1.0e-5 } );
	...
	unless UNIVERSAL::isa($fit,"PDLA") and
...
*fitwarp2d = \&PDLA::fitwarp2d;

sub PDLA::applywarp2d {
    # checks
...
sub PDLA::warp2d {
    my $opts = PDLA::Options->new( { KERNEL => "tanh", NOVAL => 0 } );
    ...
    my $px  = shift;
    my $py  = shift;
    my $out = $#_ == -1 ? PDLA->null() : shift;

    # safety checks
    my $copt   = $opts->current();
    my $kernel = _check_kernel( $$copt{KERNEL}, "warp2d" );

    &PDLA::_warp2d_int( $img, $px, $py, $out, $kernel, $$copt{NOVAL} );
    return $out;
}
...
sub PDLA::warp2d_kernel ($) {
    ...
    &PDLA::_warp2d_kernel_int( $x, $k, $kernel );
    return ( $x, $k );

#    return _get_kernel( $kernel );
}
*warp2d_kernel = \&PDLA::warp2d_kernel;
