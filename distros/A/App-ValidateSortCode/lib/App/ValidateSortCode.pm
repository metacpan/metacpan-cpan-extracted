...
This is the only method provided by the package L<App::ValidateSortCode>.It does
not expect any parameter. Code from the supplied C<validate-sort-code> script.
...
sub run {
    ...
    if ($country eq $DEFAULT_COUNTRY) {
        ...
        if ($validator->is_valid($sort_code, $account_number)) {
            print sprintf("%s is a valid sort code.\n", $self->sort_code);
        }
        else {
            print sprintf("Sorry, %s is an invalid sort code.\n", $self->sort_code);
        }
...
L<http://search.cpan.org/dist/App-ValidateSortCode/>
