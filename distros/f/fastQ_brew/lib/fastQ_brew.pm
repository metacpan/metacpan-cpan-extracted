...
=head2 load_fastQ_brew()
 ...
 Args    :
 -library_type, either sanger or illumina - defaults to sanger
 -file_path, path to sequences e.g. "./username/" - defaults to cwd()
...
sub load_fastQ_brew {
    ...
    $self->{library_type}  = $arg{library_type}  || "sanger";
    $self->{file_path}     = $arg{file_path}     || cwd();
...
sub _summary_stats {
    ...
    if ( $self->{summary} ne "N" ) {
        ...
        # the 1st line 3rd, line contain
        # the read sequence and scores
        print "\ncalculating stats...\n\n\n";
        ...
        while (<$fh>) {
            ...
            push @read_len,   length( $temp[1] );

            # Calculate phred score
            my $calc_phred = phred_calc( $temp[3], $self->{library_type} );

            # Calculate read probability
            my $calc_prob = prob_calc( $temp[3], $self->{library_type} );
