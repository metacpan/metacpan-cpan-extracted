...
You may specify an C<$object> of a type which extends L<Mail::Box::MH::Index|Mail::Box::MH::Index>
(at least implements a C<get()> method), as alternative for an index file
reader as created by C<Mail::Box::MH>.

=item index_filename => $file
...
You may specify an C<$object> of a type which extends L<Mail::Box::MH::Labels|Mail::Box::MH::Labels>
(at least implements the C<get()> method), as alternative for labels file
reader as created by C<Mail::Box::MH>.

=item labels_filename => $file
...
C<$file> which is usually called C<.mh_sequences>, but that name can
...
Permit renumbering of message.  By default this is C<true>, but for some
...
=item Fault: cannot create MH folder $name: $!

For some reason, it is impossible to create the folder.  Check the permissions
and the C<$name> of the folder.  Does the path to the directory to be created
exist?
Cast by C<create()>

=item Fault: cannot create directory $dir for subfolder $name: $!

Cast by C<openSubFolder()>
