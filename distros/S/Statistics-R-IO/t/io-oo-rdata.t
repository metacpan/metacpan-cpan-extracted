...
sub check_rdata {
    ...
    subtest 'rdata ' . $message => sub {
        plan tests => 2 * (keys(%{$expected}) + 1);
        
        ## test with filename
        my $rdata = Statistics::R::IO::RData->new($file);
        my %actual = $rdata->read;
        ...
        $rdata = Statistics::R::IO::RData->new({ fh => $fh });
        %actual = $rdata->read;
...
like(exception {
    Statistics::R::IO::RData->new('foobard')
