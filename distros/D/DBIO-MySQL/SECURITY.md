This is the Security Policy for the Perl DBIO-MySQL distribution.
...
[DBIO-MySQL on CPAN](https://metacpan.org/dist/DBIO-MySQL) or the main branch of the
[DBIO-MySQL git repository](https://codeberg.org/dbio/dbio-mysql).
...
DBIO-MySQL is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO-MySQL are covered by this policy.
...
they affect DBIO-MySQL, or DBIO-MySQL can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO-MySQL, or plugins to it that are not included with the DBIO-MySQL
distribution) are not covered by this policy.

## Supported Versions of DBIO-MySQL

Only the latest release of DBIO-MySQL will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO-MySQL will run on older versions of Perl. If a
...
that are required for DBIO-MySQL to work. However, some of these
