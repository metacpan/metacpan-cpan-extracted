#ifndef __FUTURE_ASYNCAWAIT_FROZEN_H__
#define __FUTURE_ASYNCAWAIT_FROZEN_H__
...
#define future_asyncawait_frozen_wrap_suspendhook(newfunc, oldhookp) S_future_asyncawait_frozen_wrap_suspendhook(aTHX_ newfunc, oldhookp)
static void S_future_asyncawait_frozen_wrap_suspendhook(pTHX_ SuspendHookFunc *newfunc, SuspendHookFunc **oldhookp)
{
  if(*oldhookp)
    return;
  ...
  if(!*oldhookp) {
    SV **hookp = hv_fetchs(PL_modglobal, "Future::AsyncAwait::Frozen/suspendhook", TRUE);
    if(hookp && SvOK(*hookp))
