...
trap {
    my $sb_file = SeaBASS::File->new(\$DATA[1], {});
    $sb_file->write();
};
is($trap->leaveby, 'die', "strict write trap 1");

trap {
    my $sb_file = SeaBASS::File->new(\$DATA[0], {strict => 0});
    $sb_file->write();
};
is($trap->leaveby, 'return', "nostrict write trap 1");

trap {
    my $sb_file = SeaBASS::File->new({strict => 0, add_empty_headers => 1, preserve_case => 0});
...
trap {
    my $sb_file = SeaBASS::File->new({strict => 0});
    $sb_file->add_field('lat', 'degrees');
    $sb_file->add_field('lon', 'degrees');
    $sb_file->append({'lat' => 1, 'lon' => 2});
    $sb_file->append("3,4");
...
trap {
    my $sb_file = SeaBASS::File->new({strict => 0, keep_slashes => 1});
    $sb_file->add_field('lat', 'degrees');
    $sb_file->add_field('lon', 'degrees');
    $sb_file->append({'lat' => 1, 'lon' => 2});
    $sb_file->append("3 4");
...
trap {
    my $sb_file = SeaBASS::File->new(\$DATA[6], {strict => 0});
