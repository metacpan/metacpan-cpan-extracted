...
perl $0 Greek_script.pl > Escaped_script.pl.e
END
    }

    print Greek::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Egreek::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
    ...
    my $fh = gensym();
    Egreek::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Greek::escape_script {
    ...
    # read Greek script
    my $fh = gensym();
    Egreek::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Egreek(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Egreek '%s.0'; # 'quote' for perl5.6\n", $Greek::VERSION); # require run-time routines version

        # use Greek version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Greek ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Greek::VERSION) {
                    my @file = grep -e, map {qq{$_/Greek.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Greek.pm $version, but @{[__FILE__]} is version $Greek::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Egreek.pm $version, but \\\$Egreek::VERSION is \$Egreek::VERSION" if \$Egreek::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Greek::VERSION) {
                    die "Script $0 required Greek.pm $version, but @{[__FILE__]} is only version $Greek::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Egreek::function_ord     = 'Greek::ord'     if /\A ord \z/oxms;
                    $Egreek::function_ord_    = 'Greek::ord_'    if /\A ord \z/oxms;
                    $Egreek::function_reverse = 'Greek::reverse' if /\A reverse \z/oxms;
                    $Egreek::function_getc    = 'Greek::getc'    if /\A getc \z/oxms;

                    # avoid: used only once: possible typo at ...
                    $Egreek::function_ord     = $Egreek::function_ord;
                    $Egreek::function_ord_    = $Egreek::function_ord_;
                    $Egreek::function_reverse = $Egreek::function_reverse;
                    $Egreek::function_getc    = $Egreek::function_getc;
                }
            }
        }
    }

    $e_script .= Greek::escape();
...
=item * You'll have to download Greek.pm and Egreek.pm and put it in your perl lib directory.

=item * You'll need to write "use Greek;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Greek;
  use Greek ver.sion;             --- require minimum version
  use Greek ver.sion.0;           --- expects version (match or die)

  # "no Greek;" not supported

  or

  $ perl Greek.pm Greek_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Greek_script.pl  --- script written in Greek
...
Greek software is "middleware" between perl interpreter and your Perl script
written in Greek.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Greek, Perl3 or later
can treat Greek as binary data.

By "use Greek;", it automatically interpret your script as Greek. The various
functions of perl including a regular expression can treat Greek now.
...
This software is a source code filter to escape Perl script encoded by Greek
...
=item * Remains One Language Handling Raw Greek, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Greek.pm, Egreek.pm
  +---------------------------------------+
...
import of Egreek.pm works well.
...
=head1 Software Composition
   ...
   Greek.pm               --- source code filter to escape Greek
   Egreek.pm              --- run-time routines for Greek.pm
...
You need write 'use Greek;' in your script.
...
=head1 Calling 'Egreek::ignorecase()' (Greek software provides)

Greek software applies calling 'Egreek::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Egreek::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Egreek.pm provides)
...
Definitions in Egreek.pm.
  ...
  ${Egreek::not_lower_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Egreek::not_lower_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
  ...
  ${Egreek::not_upper_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Egreek::not_upper_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Greek software provides)

Greek software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Greek software provides)

Insert 'Egreek::' at head of function name. Egreek.pm provides your script Egreek::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Egreek::lc        Character
  lcfirst     Egreek::lcfirst   Character
  uc          Egreek::uc        Character
  ucfirst     Egreek::ucfirst   Character
  fc          Egreek::fc        Character
  chr         Egreek::chr       Character
  glob        Egreek::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Greek software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Greek software provides)

Greek software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Egreek.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Greek software and
...
Greek software remains one language and one interpreter.
