...
The C<new()> constructur takes the following arguments:

=over 4

=item * uri - A HTTP or HTTPS URI to the firewall.

=item * username - a username to authenticate to the device.

=item * password - a password for the username.
...
sub _validate_and_return_response {
    ...
    my $response_structure = $xml_parser->safe_parse( $http_response->decoded_content )->simplify( forcearray => ['entry']);
