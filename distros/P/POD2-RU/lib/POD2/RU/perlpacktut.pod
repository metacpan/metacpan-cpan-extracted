...
perlpacktut - Учебник по C<pack> и C<unpack>

=head1 DESCRIPTION

C<pack> и C<unpack> это две функции для преобразования данных в соответствии с
пользовательским шаблоном, между способом хранить значения, который охраняет Perl 
и некоторыми четко определенными представлениями этой строки, как она может хранится по требованию окружающей среды Perl-программы. К сожалению они также две из
наиболее недооцененных и наиболее часто упускаемых из виду функции, которые  предоставляет Perl. Этот учебник будет демистификатором этих функций для вас.

=head1 Основной принцип

Большинство языков программирования не раскрывают память где сохраняется переменная. 
В C, например, вы можете взять адрес некоторой переменной,
и оператор C<sizeof> скажет вам сколько байтов выделены для
переменной. Используя адрес и размер, вы может получить доступ к хранилищу
 сердца вашей программы.

В Perl, вы просто не можете получить доступ к памяти наугад, но структурные и
представительские преобразования C<pack> и C<unpack> - это
отличная альтернатива. Функция C<pack> преобразует значения - в последовательность байтов
, которая содержит представление согласно данной спецификации,
аргумент, так называемый "шаблон". C<unpack> — это обратный процесс получения
некоторых значений из содержимого строки байтов. (Будьбе осторожны,
 что не все, что было упакован вместе может быть аккуратно распаковано -
 у нас есть очень общий опыт и мы, как опытные путешественники, это поджтверждаем.)

Почему вы можете спросить, вам потребуется кусок памяти, содержащих некоторые значения
в двоичном представлении? Одна хорошая причина заключается в том, чтобы получить доступ
 для ввода и вывода некоторого файла, устройства, или сетевого подключения, в двоичном представлении для вас, что даст вам некоторую пользу в обработке. 
 Другой причиной является передача данных в некоторым системным вызовам
не доступным, как функции Perl: C<syscall> требует предоставить
параметры, хранящиеся в том виде, с каким это происходит в программе C. Даже обработка текста 
(как показано в следующем разделе) может быть упрощена разумным использованием
 этих двух функций.

 Чтобы увидеть, как работает (рас)упаковка ((un)packing), мы начнем с простого шаблона
кода, где преобразование идет на низкой передаче (low gear): между содержимым последовательности байтов и строкой шестнадцатеричных цифр.
Давайте использовать C<unpack>, распаковать, так как
она, вероятно, напоминает нам о программе дампа, или о каком-то отчаянном последнем
сообщении несчастной программы, которое имеют обыкновение бросать на вас, прежде чем они истекают
в Далёкой синей выси (Wild Blue Yonder -  научно-фантастический псевдодокументальный фильм режиссёра Вернера Херцога, вышедший на экраны в 2005 году. Фильм разделён автором на десять частей-глав.
Инопланетянин, неотличимый от человека, рассказывает свою историю. Несколько сотен лет назад он с сородичами из Туманности Андромеды прилетел на Землю…)
  Предположим, что переменная C<$mem> содержит 
последовательность байтов, которые мы хотели бы проверить не предполагая ничего
о их значении, мы можем написать

   my( $hex ) = unpack( 'H*', $mem );
   print "$hex\n";

после чего мы могли бы увидеть что-то вроде этого, где каждой паре шестнадцатеричных цифр
соответствует байт:   
...
corresponding to a byte:)
   ...
   Что было в этом куске памяти? Числа, символы, или их смесь
? Если предположить, что мы находимся на компьютере, где используется кодирование ASCII (или аналогичное)
 : шестнадцатеричные значения в диапазоне  C<0x40> - C<0x5A>
указывают на заглавную букву, и C<0x20> кодирует пробел. Таким образом, мы могли бы
предположить, что это кусок текста, который некоторые могут читать как таблоид;
но другие должны будут овладеть ASCII таблицей и пережить чувство
первоклассника (relive that firstgrader feeling). Не заботясь слишком много о пути, который позволит прочитать это, отметим, что C<unpack> с кодом шаблона C<Н> преобразует содержимое
последовательности байтов в обычную шестнадцатеричную нотацию (customary hexadecimal notation). Поскольку "последовательность" является довольно расплывчатым указание количества, С<Н> 
определяется для преобразования только одной шестнадцатеричной цифры, еслиза ним не следует
 число повторений. Звездочка для счетчика повторов означает использовать все
оставшееся. (An asterisk for the repeat count means to use whatever
remains.)

Обратная операция - упаковка содержимого байтов из строки шестнадцатеричных
цифр - это так же легко написать. Например:
...
Поскольку мы скармливаем (feed) C<pack> список из десяти 2-значных шестнадцатеричный строк,
pack шаблон должен содержать десять pack кодов. Если это выполняется на компьютере
с ASCII кодированием символов, то он напечатает C<0123456789>.

=head1 Упаковка текста (Packing Text)

Давайте предположим, что у нас есть данные для чтения из файла такого рода:
...
Как мы это делаем? Вы можете подумать сначала использовать C<split>; однако, поскольку
C<split> разбивает поля с пробелами, то вы никогда не будете знать, была ли запись
доходов или расходов (income or expenditure). УПС. Ну вы всегда могли использовать C<substr>:
	...
	А вот это уже не смешно, не правда ли? (It's not really a barrel of laughs, is it?) В самом деле это хуже, чем это может показаться; проницательные могут заметить, что первое поле должно быть только из 10 символов и ошибка распространяется направо через другие
числа - которые нам пришлось рассчитывать вручную. Так что эти ошибки ужасно недружественные.
...
Брр. Ну, это немного лучше, но - Ну, вы хотите поддерживать что?
(Urgh. Well, it's a bit better, but - well, would you want to maintain
that?)

Эй, разве не Perl предполагает делать такого рода вещи легко? Да, он это делает,
если вы используете правильные инструменты. C<pack> и C<unpack> призваны помочь
вам, когда вы имеете дело с данными фиксированной ширины как выше. Давайте
посмотрим на решение с C<unpack>:
...
Это выглядит немного лучше; но для разбора у нас используется странный шаблон.
Где тут что и откуда извлекать?

ОК, давайте взглянем на некоторые из наших данных снова; в самом деле мы будем включать
заголовки и удобное правило, так что мы сможем отслеживать, где мы находимся.
