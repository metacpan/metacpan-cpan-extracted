package Proc::ProcessTable::Match::Cmndline;
...
Proc::ProcessTable::Match::Cmndline - Check if the cmndline of a process matches via regexp.
...
=head1 SYNOPSIS
    ...
    my $checker=Proc::ProcessTable::Match::Cmndline->new( \%args );
...
that is 'cmndlines', which is a array of cmndline values to match.

The matching is done via regexp.

Atleast one cmndline must be specified.
...
sub new{
	...
	if ( ! defined( $args{cmndlines} ) ){
		die ('No cmndlines key specified in the argument hash');
	}
	if ( ref( $args{cmndlines} ) ne 'ARRAY' ){
		die ('The cmndlines key is not a array');
	}
	if ( ! defined $args{cmndlines}[0] ){
		die ('Nothing defined in the cmndlines array');
	}

	# make sure each specified value compiles as a regex
	my $regex_int=0;
	while ( defined( $args{cmndlines}[$regex_int] ) ){
		if ( ! eval{ qr/$args{cmndlines}[$regex_int]/ } ){
			die (q{The value "}.$args{cmndlines}[$regex_int].q{" in the cmndlines array does not compile as a regex... }.$@);
		}
		$regex_int++;
	}
...
sub match{
	...
	if ( ! defined( $proc_cmndline ) ){
		return 0;
	}

	foreach my $cmndline ( @{ $self->{cmndlines} } ){
		if ( $proc_cmndline =~ /$cmndline/ ){
			return 1;
		}
	}
