package Time::Zone::Olson::Win32;

use strict;
use warnings;
use Carp();
use English qw( -no_match_vars );
use parent qw(Time::Zone::Olson);
...
"Failed to read LOCAL_MACHINE\\$timezone_specific_registry_path\\TZI:$EXTENDED_OS_ERROR"
    ...
    $self->{_comments}->{$tz} = $olson_to_win32_timezones{$tz};
...
sub _unpack_win32_tzi_structure {
    ...
    my $negative_one        = $self->SUPER::_NEGATIVE_ONE();
    my $minutes_in_one_hour = $self->SUPER::_MINUTES_IN_ONE_HOUR();
    my $tz_definition       = {
        ( ( $bias + $standard_bias ) < 0 ? ( std_sign => q[-] ) : () ),
        std_hours => int(
            ( $bias + $standard_bias ) /
              $self->SUPER::_SECONDS_IN_ONE_MINUTE() *
...
=item *
L<Win32API::Registry|Win32API::Registry>
