...
sub bnorm {
    # bnorm() can't support rounding, bround() and bfround() call bnorm().
    ...
    # inf, nan etc
    if ($x->{sign} !~ /^[+-]$/) {
        return $downgrade -> new($x -> bdstr()) if defined $downgrade;
        return $x;
    }
...
sub to_ieee754 {
    my ($class, $x, $format) = ref($_[0]) ? (ref($_[0]), @_) : objectify(1, @_);
