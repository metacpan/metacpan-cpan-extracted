...
Sah::Schema::dirname::unix::exists - Unix directory name, must exist on filesystem

=head1 VERSION

This document describes version 0.021 of Sah::Schema::dirname::unix::exists (from Perl distribution Sah-Schemas-Path), released on 2023-10-29.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("dirname::unix::exists*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("dirname::unix::exists", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("dirname::unix::exists", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("dirname::unix::exists*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"dirname::unix::exists*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('dirname::unix::exists*', name=>'DirnameUnixExists')
     );
 }

 use My::Types qw(DirnameUnixExists);
 DirnameUnixExists->assert_valid($data);
