...
static void stage_post_blockstart(pTHX)
{
...
static OP *stage_pre_blockend(pTHX_ OP *body)
{
...
static void stage_post_newcv(pTHX_ CV *cv)
{
  if(hv_fetchs(GvHV(PL_hintgv), "t::stages/post_newcv-capture-cv", 0)) {
    sv_setrv(get_sv("t::stages::captured", GV_ADD), SvREFCNT_inc(cv));
  }
}
