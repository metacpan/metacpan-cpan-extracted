...
1;
# ABSTRACT: Directory name, must exist on filesystem
...
Sah::Schema::dirname::exists - Directory name, must exist on filesystem

=head1 VERSION

This document describes version 0.031 of Sah::Schema::dirname::exists (from Perl distribution Sah-SchemaBundle-Path), released on 2024-02-16.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("dirname::exists*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("dirname::exists", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("dirname::exists", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("dirname::exists*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"dirname::exists*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('dirname::exists*', name=>'DirnameExists')
     );
 }

 use My::Types qw(DirnameExists);
 DirnameExists->assert_valid($data);
