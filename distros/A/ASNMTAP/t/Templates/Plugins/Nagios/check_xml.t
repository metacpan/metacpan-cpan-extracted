...
if ( -x "$prefix/$plugin" ) {
  ...
  $t += checkCmd( "$prefix/$plugin -e P -F $prefix/xml/nagios-1.0.xml -i 1 -H hostname -s service", 3, '/ERROR: Content Error: - Host: Host Name ... ne hostname - Service: Service Name ... ne service - Environment: LOCAL ne P/');
  $t += checkCmd( "$prefix/$plugin -e P -F $prefix/xml/nagios-1.0.xml -i 10 -H 'Host Name ...' -s service", 3, '/ERROR: Content Error: - Service: Service Name ... ne service - Environment: LOCAL ne P/');
  $t += checkCmd( "$prefix/$plugin -e P -F $prefix/xml/nagios-1.0.xml -i 10 -H 'Host Name ...' -s 'Service Name ...'", 3, '/ERROR: Content Error: - Environment: LOCAL ne P/');
  $t += checkCmd( "$prefix/$plugin -e L -F $prefix/xml/nagios-1.0.xml -i 10 -H 'Host Name ...' -s 'Service Name ...'", 3, '/Result into XML file \'[//\w\-\.]+\' are out of date:/');
} else {
