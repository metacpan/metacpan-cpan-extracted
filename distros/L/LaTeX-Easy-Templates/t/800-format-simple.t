...
###################################################################
#### NOTE the complex version of this test in now in xt/400-format.t
#### it can be run by : 
###################################################################
		...
		'params' => {
		  'processors' => {
			...
			'xelatex-in-memory' => {
			   ...
			   'template' => {
				# content will be read from file and set and filename will be deleted
				'filepath' => File::Spec->catfile($curdir, 'templates', 'multi-language-xelatex', 'main.tex.tx'),
				'auxfiles' => [ File::Spec->catdir($curdir, 'templates', 'multi-language-xelatex') ],
				'content' => undef,
			   },
			   'output' => {
				'filename' => undef, # it will go somewhere
