package QualysGuard::Response::TicketDelete;
...
sub new {
    ...
    if ( $self->exists('/TICKET_DELETE_OUTPUT/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/TICKET_DELETE_OUTPUT/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/TICKET_DELETE_OUTPUT/ERROR');
...
QualysGuard::Response::TicketDelete
