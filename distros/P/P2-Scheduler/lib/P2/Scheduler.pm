...
# experiments per priority
has 'experiments' => (is => 'ro', isa => 'ArrayRef', required => 1);
...
sub month_schedule {
    ...
    for (my $day_index=0;$day_index<scalar(@bd) - $self->spare_days_per_month;$day_index++) {
        next unless ( $bd[$day_index] >= $self->starting_from );
        my $exp_index=1;
        for (my $priority=0;$priority<$self->pri_per_month->[$month-1];$priority++) {
            next unless $self->experiments->[$priority]; # skip this priority if no experiments there...
            for ( my $index=0;$index<$self->weight->[$priority];$index++) {
                my $exp_mins = $self->{mins_interval} * $exp_index;
                push @schedule, { 
                    year    => $self->year, 
                    month   => sprintf("%02d", $month),
                    day     => sprintf("%02d", $bd[$day_index]),
                    experiment   => $self->experiments->[$priority][$experiment_index[$priority]],
                    priority    => $priority+1,
                ...
                $experiment_index[$priority] = ($experiment_index[$priority] + 1) % 
                    scalar( @{$self->experiments->[$priority]} );
                $exp_index++;
            }
        }
    }
    ...
    return map{{ 
        exp_id   => lc( $_->{experiment}.'-'.substr(uuid(), -8) ),
        context  => $_->{context},
        filepath => $_->{experiment}, 
