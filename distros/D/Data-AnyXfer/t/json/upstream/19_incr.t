...
if ($] >= 5.008) {
  splitter +Data::AnyXfer::JSON->new->canonical   , '  ["x\\"","\\u1000\\\\n\\nx",1,{"\\\\" :5 , "": "x"}]';
  splitter +Data::AnyXfer::JSON->new->canonical   , '[ "x\\"","\\u1000\\\\n\\nx" , 1,{"\\\\ " :5 , "": " x"} ] ';
}
splitter +Data::AnyXfer::JSON->new->allow_nonref->canonical, '"test"';
splitter +Data::AnyXfer::JSON->new->allow_nonref->canonical, ' "5" ';
...

{
   ...
   my $coder = new Data::AnyXfer::JSON;
   for (0 .. length $text) {
      my $a = substr $text, 0, $_;
      my $b = substr $text, $_;
...
{
   my $coder = Data::AnyXfer::JSON->new->max_size (5);
   ok (!$coder->incr_parse ("[    "), "incsize1");
   eval { !$coder->incr_parse ("]  ") }; ok ($@ =~ /6 bytes/, "incsize2 $@");
}

{
   my $coder = Data::AnyXfer::JSON->new->max_depth (3);
...
{
   my $coder = Data::AnyXfer::JSON->new;
