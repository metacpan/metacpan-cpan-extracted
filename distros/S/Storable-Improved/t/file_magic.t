...
use strict;
use Test::More;
use Storable::Improved qw(store nstore);
...
for my $f (<data_*>) {
    ...
    my $x = dump(Storable::Improved::file_magic($f));
    $x =~ s/^/        /gm;
    print "$x,\n";
...
is(Storable::Improved::file_magic(__FILE__), undef, "not an image");

store({}, $file);
{
    my $info = Storable::Improved::file_magic($file);
    unlink($file);
    ok($info, "got info");
    is($info->{file}, $file, "file set");
    is($info->{hdrsize}, 11 + length($Config{byteorder}), "hdrsize");
    like($info->{version}, q{/^2\.\d+$/}, "sane version");
    is($info->{version_nv}, Storable::Improved::BIN_WRITE_VERSION_NV, "version_nv match");
    is($info->{major}, 2, "sane major");
    ok($info->{minor}, "have minor");
    ok($info->{minor} >= Storable::Improved::BIN_WRITE_MINOR, "large enough minor");
...
for my $test (@tests) {
    ...
    $expected->{file} = 1;
    is_deeply(Storable::Improved::read_magic($data), $expected, "read magic $name");
}
