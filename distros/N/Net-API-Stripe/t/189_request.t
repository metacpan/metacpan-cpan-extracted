...
BEGIN
{
    use_ok( 'Net::API::Stripe::Event::Request' ) || BAIL_OUT( "Unable to load perl module 'Net::API::Stripe::Event::Request'" );
};
can_ok( 'Net::API::Stripe::Event::Request', 'id' );
can_ok( 'Net::API::Stripe::Event::Request', 'idempotency_key' );
...
my $json_file = $sample_dir->child( 'request.json' ) ||
...
if( $json_file->exists )
{
    if( !$json_file->can_read )
    {
        ...
        BAIL_OUT( "Unable to read json file $rel for Stripe class 'request'" );
    }
    elsif( !$json_file->is_empty )
    {
        $code = $json_file->load_json ||
            BAIL_OUT( "Failed to load json data for Stripe class 'request': " . $json_file->error );
    }
}
my $obj = scalar( keys( %$code ) ) ? Net::API::Stripe::Event::Request->new( $code ) : Net::API::Stripe::Event::Request->new;
isa_ok( $obj => 'Net::API::Stripe::Event::Request' );
