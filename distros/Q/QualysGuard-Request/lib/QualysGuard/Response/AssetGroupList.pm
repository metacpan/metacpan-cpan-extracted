...
sub new {
    ...
    if ( $self->exists('/ASSET_GROUP_LIST/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/ASSET_GROUP_LIST/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/ASSET_GROUP_LIST/ERROR');
...
QualysGuard::Response::AssetGroupList
