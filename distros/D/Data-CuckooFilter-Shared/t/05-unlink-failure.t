...
my $h = Data::CuckooFilter::Shared->new( $p, 10_000 );
...
like $err, qr/unlink/, '  ... and the message names unlink'
    or diag "got: $err";
ok -e $p, 'the filter file is still there, as the error said';
...
ok $twice, q{unlink on an already-removed file is benign (ENOENT)}
    or diag $@;
my $cls = eval { 'Data::CuckooFilter::Shared'->unlink("$sub/never-existed.bin"); 1 };
