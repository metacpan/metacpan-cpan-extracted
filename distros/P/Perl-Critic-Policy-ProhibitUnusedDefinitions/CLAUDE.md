...
Perl-Critic-Policy-ProhibitUnusedDefinitions is a Perl::Critic policy that
reports subs nothing in `bin/` or `lib/` calls, and `our` globals and `use
constant` constants nothing in `bin/`, `lib/`, `t/` or `xt/` uses.  Unlike most
policies it reads the whole distribution around the file it is critiquing:
once, on the first check, into an index every later check reads.

Three files, in this order:
`lib/Perl/Critic/Policy/ProhibitUnusedDefinitions.pm` is the whole of it and
its POD is the contract; `t/Perl-Critic-Policy-ProhibitUnusedDefinitions.t`
says what it reports and, as importantly, what it deliberately does not;
`.perlcriticrc` is the house style, made enforceable.
...
| | |
|---|---|
| `perldoc Perl-Critic-Policy-ProhibitUnusedDefinitions` | what this is for and how a caller uses it |
...
| `prereqs.yaml` | what it needs at runtime and to test, read by `[PrereqsFile]` |

This policy's own `.perlcriticrc` does not enable it.  Perl::Critic calls
`violates`, `applies_to` and the rest from outside, so every one would be
reported.
