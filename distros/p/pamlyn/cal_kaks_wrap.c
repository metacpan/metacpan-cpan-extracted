...
#define SWIG_init    boot_YN00

#define SWIG_name   "YN00c::boot_YN00"
#define SWIG_prefix "YN00c::"
...
extern double *cal_kaks(int,char *[]);
#ifdef PERL_OBJECT
#define MAGIC_CLASS _wrap_YN00_var::
class _wrap_YN00_var : public CPerlObj {
public:
#else
#define MAGIC_CLASS
#endif
...
XS(_wrap_cal_kaks) {
    {
        ...
        arg2 = 0;
        if ((items < 1) || (items > 1)) {
            SWIG_croak("Usage: cal_kaks(argc,argv);");
        }
        {
            AV *tempav;
            SV **tv;
            ...
            int i;
            if (!SvROK(ST(0))) {
                SWIG_exception(SWIG_ValueError,"ST(0) is not an array.");
            }
        ...
        result = (double *)cal_kaks(arg1,arg2);
        
        {
            ...
            while ((result[len]) && (result[len] > 0)) len++;
            
            svs = (SV**) malloc(len*sizeof(SV *));
            for (i = 0; i < len; i++) {
                svs[i] = sv_newmortal();
                sv_setnv((SV*)svs[i], result[i]);
            };
