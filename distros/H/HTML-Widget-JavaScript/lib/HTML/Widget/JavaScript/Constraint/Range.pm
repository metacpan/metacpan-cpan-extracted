package HTML::Widget::JavaScript::Constraint::Range;

use warnings;
use strict;

use base 'HTML::Widget::Constraint::Range';

=head1 NAME

HTML::Widget::JavaScript::Constraint::Range - JavaScript Range Constraint
...
See L<HTML::Widget::Constraint::Range>.
...
sub emit_javascript {
	...
	for my $param (@{$self->names}) {
		if ($min) {
			push(@js_constraints, "$not (${var_name}.${param}.value < $min)");
		}

		if ($max) {
			push(@js_constraints, "$not (${var_name}.${param}.value > $max)");
		}
	}
