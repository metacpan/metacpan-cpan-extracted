...
L<PDLA::Slatec|PDLA::Slatec> or any of the PDLA::GSL family of modules).
...
These routines should work OK with L<PDLA::Matrix|PDLA::Matrix> objects
...
order, you can try using the L<PDLA::Matrix|PDLA::Matrix> object, which
...
=cut
   ...
   sub PDLA::eigens {
      ...
      barf "Need real square matrix for eigens" 
            if $#d < 1 or $d[0] != $d[1];
      my $deviation = PDLA::max(abs($a - $a->mv(0,1)))/PDLA::max(abs($a));
      if ( $deviation <= 1e-5 ) {
          #taken from eigens_sym code

          my $lt  = PDLA::indexND($a,
			     scalar(PDLA::whichND(PDLA->xvals($n,$n) <=
						 PDLA->yvals($n,$n)))
			     )->copy;
          my $ev  = PDLA->zeroes($a->dims);
          my $e   = PDLA->zeroes($a->index(0)->dims);
	  ...
	  if( !$PDLA::eigens_bug_ack && !$ENV{PDLA_EIGENS_ACK} ) {
	    print STDERR "WARNING: using sketchy algorithm for PDLA::eigens asymmetric case -- you might\n".
          ...
          my $ev  = PDLA->zeroes(2, $a->dims);
          my $e   = PDLA->zeroes(2, $a->index(0)->dims);

          &PDLA::_eigens_int($a->clump(0,1), $ev, $e);
...
Here's a trivial example (used in L<PDLA::Transform::map|PDLA::Transform/map>)
...
C<lu_decomp> is ported from I<Numerical Recipes> to PDLA. It
...
C<lu_decomp2> is ported from I<Numerical Recipes> into PDLA.
...
sub lu_backsub {
   ...
   barf("Usage: \$x = lu_backsub(\$lu,\$perm,\$b); all must be PDLAs\n") 
   unless(UNIVERSAL::isa($lu,'PDLA') &&
      UNIVERSAL::isa($perm,'PDLA') &&
      UNIVERSAL::isa($b,'PDLA'));
