...
# tests fetch by oid (as number) when Oid already in object cache
########################################
use t::lib;
use strict;
use Carp;
...
is($@,'','p0 fetch when Oid exists');
my $actual_p1=eval{Class::AutoDB::Serialize->fetch($p1_oid);};
is($@,'','p1 fetch when Oid exists');

ok_objcache($actual_p0,$p0_oid,'object','Persistent',
	    'p0 fetched as object (not just Oid) when Oid exists',__FILE__,__LINE__);
ok_objcache($actual_p1,$p1_oid,'object','Persistent',
	    'p1 fetched as object (not just Oid) when Oid exists',__FILE__,__LINE__);

cmp_deeply($actual_p0,$p0,'p0 contents when Oid exists');
cmp_deeply($actual_p1,$p1,'p1 contents when Oid exists');
...
is((grep {@$_==1} values %actual_id2p),2,'persistent objects: 1 copy each when Oid exists');
is((grep {@$_==2} values %actual_id2np),2,'non-persistent objects: 2 copies each when Oid exists');
