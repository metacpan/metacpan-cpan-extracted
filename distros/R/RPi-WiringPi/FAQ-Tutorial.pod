...
NOTE: If you receive errors about the handler callback not being found, simply
specify the callback with the full package (eg. C<'main::callback'>).
...
number of runs if desired.
    ...
    bbdispatch -r https://github.com/stevieb9/rpi-wiringpi -a
...
restart the dispatcher with the C<--rpi> or the equivalent C<-R> flag:
    ...
    bbdispatch -r https://github.com/stevieb9/rpi-wiringpi -a 0 --rpi
