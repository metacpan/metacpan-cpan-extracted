...
sub client4_connected {
  my ($kernel,$heap,$sender) = @_[KERNEL,HEAP,SENDER];
  pass($_[STATE]);
  $poe_kernel->post( 'groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'ETRACE', params => [ '9T9' ] } );
  return;
}
...
sub client_input {
  ...
  if ( $cmd eq 'NOTICE' ) {
    ...
    is($params->[1],'*** Notice -- ETRACE requested by groucho (groucho@groucho.marx) [groucho.server.irc]',
...
sub groucho_input {
  ...
  if ( $cmd eq '759' ) {
    ...
    is($params->[2],'End of ETRACE','End of ETRACE');
    is($heap->{709},3,'Correct number of 709');
...
sub harpo_input {
  ...
  if ( $cmd eq 'ETRACE' ) {
    ...
    $poe_kernel->post( 'groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'ETRACE', params => [ '1FU' ] } );
    return;
  }
  return;
}
