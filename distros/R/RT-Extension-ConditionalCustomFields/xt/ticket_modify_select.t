...
ok($ticket_cf_conditioned_by->is_displayed, 'Show ConditionalCF when Select condition val with is operator is met');
...
ok($ticket_cf_conditioned_by->is_hidden, 'Hide ConditionalCF when Select condition val with is operator is updated to not met');
...
ok($ticket_cf_conditioned_by->is_hidden, 'Hide ConditionalCF when Select condition val with is operator is not met');
...
$mjs->get($m->rt_base_url . 'Ticket/Modify.html?id=' . $ticket->id);
$ticket_cf_conditioned_by = $mjs->by_id('Object-RT::Ticket-' . $ticket->id . '-CustomField:Grouptwo-' . $cf_conditioned_by->id . '-Value', single => 1);
ok($ticket_cf_conditioned_by->is_displayed, "Show ConditionalCF when Select condition val with isn't operator is met");

# Operator: isn't, condition not met
$ticket->AddCustomFieldValue(Field => $cf_condition->id , Value => $cf_values->[1]->Name);
$mjs->get($m->rt_base_url . 'Ticket/Modify.html?id=' . $ticket->id);
$ticket_cf_conditioned_by = $mjs->by_id('Object-RT::Ticket-' . $ticket->id . '-CustomField:Grouptwo-' . $cf_conditioned_by->id . '-Value', single => 1);
ok($ticket_cf_conditioned_by->is_hidden, "Hide ConditionalCF when Select condition val with is operator isn't not met");
