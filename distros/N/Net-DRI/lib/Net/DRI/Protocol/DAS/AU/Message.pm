## Domain Registry Interface, .AU DAS Message
##
...
package Net::DRI::Protocol::DAS::AU::Message;
...
Net::DRI::Protocol::DAS::AU::Message - .AU DAS Message for Net::DRI
...
{
 my ($self)=@_;
 my $s=sprintf("%s\x0d\x0a",$self->command_param());
 return $s;
}
...
sub parse
{
 my ($self,$dc,$rinfo)=@_;
 my @d=$dc->as_array();
 Net::DRI::Exception->die(0,'protocol/DAS',1,'Unsuccessfull parse, not exactly one line in server reply') unless (@d==1);
 $self->errcode(0);
 $self->errmsg($d[0]);
 $self->response($d[0]);
}
