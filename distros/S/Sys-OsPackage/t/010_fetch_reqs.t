...
sub init_tempdir
{
    ...
    $paths{temp_dir} = File::Temp->newdir(
    ...
    if ($debug_mode) {
        foreach my $varname ( sort @local_lib_vars ) {
            printf STDERR "get_test_dir(%03d): %s = %s\n", $test_num, $varname,
                ( exists $ENV{$varname} ) ? $ENV{$varname} : "undef";
        }
    }
...
sub test_exist
{
    ...
    is( $retval >> 8,
        0, sprintf( "%03d/%s/%s: install %s, install should work", $test_num, $pipe_label, $notest_label, $mod ) );
...
SKIP: {
        ...
        if ($use_notest) {
            ok( not( $log_content =~ qr/Result: (PASS|FAIL)/ ),
                sprintf( "%03d/%s/%s: tests inhibited as expected", $test_num, $pipe_label, $notest_label ) );
        } else {
            ok( $log_content =~ qr/Result: (PASS|FAIL)/,
                sprintf( "%03d/%s/%s: tests ran as expected", $test_num, $pipe_label, $notest_label ) );
        }
    }
    ...
    if ( ( exists $params->{files} ) and ref $params->{files} eq "ARRAY" ) {
        foreach my $filepath ( @{ $params->{files} } ) {
            ok(
                -e $paths_ref->{current_link} . "/" . $filepath,
                sprintf "%03d/%s/%s: found %s",
                $test_num, $pipe_label, $notest_label, $filepath
            );
        }
    }
...
sub test_nonexist
{
    ...
    my $retval = system $test_cmd;
    is( $retval >> 8, 1,
        sprintf( "%03d/%s: non-existent %s install fails as expected", $test_num, $pipe_label, $mod ) );
    return;
}
...
foreach my $mod ( sort keys %{ $tests{existent} } ) {
    ...
    foreach my $use_pipe (qw(0 1)) {
        foreach my $use_notest (qw(0 1)) {
            test_exist( $paths_ref, $test_num++, $mod, { %$params, use_pipe => $use_pipe, use_notest => $use_notest } );
        }
    }
}
