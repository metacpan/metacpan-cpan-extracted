...
differences between Scilab and PDLA to help you get going as quickly
as possible.

B<This document is not a tutorial>. For that, go to L<PDLA::QuickStart|
PDLA::QuickStart>. This document B<complements> the Quick Start guide, as
it highlights the key differences between Scilab and PDLA.

=head1 Perl

The key difference between Scilab and PDLA is B<Perl>.

Perl is a general purpose programming language with thousands of modules
freely available on the web. PDLA is an extension of Perl. This gives PDLA
programs access to more features than most numerical tools can dream of.
At the same time, most syntax differences between Scilab and PDLA are a
result of its Perl foundation.

B<You do not have to learn much Perl to be effective with PDLA>. But
...
PDLA does not come with a dedicated IDE. It does however come with an
interactive shell and you can use a Perl IDE to develop PDLA programs.

=head2 PDLA interactive shell

To start the interactive shell, open a terminal and run C<perldla> or C<pdla2>.
As in Scilab, the interactive shell is the best way to learn the
language. To exit the shell, type C<exit>, just like Scilab.

=head2 Writing PDLA programs

One popular IDE for Perl is called Padre (L<http://padre.perlide.org>).
It is cross platform and easy to use.

Whenever you write a stand-alone PDLA program (i.e. outside the
C<perldla> or C<pdla2> shells) you must start the program with C<use PDLA;>.
This command imports the PDLA module into Perl. Here is a sample
PDLA program:

  use PDLA;             # Import main PDLA module.
  use PDLA::NiceSlice;  # Import additional PDLA module.
...
In very recent versions of PDLA (version 2.4.7 or later) there is
...
PDLA should "do what you mean". So you can write in a syntax that is
...
Gives PDLA a syntax for slices (sub-matrices) that is shorter and
more familiar to Scilab users.

  // Scilab
  b(1:5)            -->  Selects the first 5 elements from b.
  
  # PDLA without NiceSlice
...
This section explains how PDLA's syntax differs from Scilab. Most
...
In PDLA, indices start at '0' (like C and Java), not 1 (like Scilab).
For example, if C<$y> is an array with 5 elements, the elements would be
numbered from 0 to 4.

=item Displaying an object

Scilab normally displays object contents automatically. In PDLA you display
...
PerlDL:
 ...
 pdla> $x = 12    # No output.
 pdla> print $x   # Print object.
 12
 pdla> p $x       # "p" is a shorthand for "print" in the shell.
 12
...
Notice that in PDLA the parenthesis in a function call are often optional.
...
=item Linearly spaced sequences
  ...
  PerlDL:   pdla> p zeroes(5)->xlinvals(2,10)
            [2 4 6 8 10]

B<Explanation>: Start with a 1-dimensional piddle of 5 elements and give
it equally spaced values from 2 to 10.

Scilab has a single function call for this. On the other hand, PDLA's
method is more flexible:

  pdla> p zeros(5,5)->xlinvals(2,10)
  [
   [ 2  4  6  8 10]
   [ 2  4  6  8 10]
   [ 2  4  6  8 10]
  ...
  pdla> p zeros(5,5)->ylinvals(2,10)
  [
   [ 2  2  2  2  2]
   [ 4  4  4  4  4]
   [ 6  6  6  6  6]
  ...
  pdla> p zeros(3,3,3)->zlinvals(2,6)
  [
   [
    [2 2 2]
    [2 2 2]
...
The PDLA shell and Scilab have a similar syntax for slicing, but there
are two important differences:

1) PDLA indices start at 0, as in C and Java. Scilab starts indices at 1.

2) In Scilab you think "rows and columns". In PDLA, think "x and y".
...
When you write a stand-alone PDLA program you have
to include the L<PDLA::NiceSlice|PDLA::NiceSlice> module. See the
previous section "B<MODULES FOR SCILAB USERS>" for more information.

  use PDLA;             # Import main PDLA module.
  use PDLA::NiceSlice;  # Nice syntax for slicing.
...
for an N-dimensional data set. Scilab and PDLA both give you the
...
PDLA offers two functions for each feature.
...
A related issue is how Scilab and PDLA understand data sets of higher
dimension. Scilab was designed for 1D vectors and 2D matrices with
higher dimensional objects added on top. In contrast, PDLA was designed
for N-dimensional piddles from the start. This leads to a few surprises
in Scilab that don't occur in PDLA:
...
=item Scilab sees a vector as a 2D matrix.
  ...
  Scilab                       PerlDL
  ------                       ------
  --> vector = [1,2,3,4];       pdla> $vector = pdl [1,2,3,4]
  --> size(vector)              pdla> p $vector->dims
  ans = 1 4                    4

Scilab sees C<[1,2,3,4]> as a 2D matrix (1x4 matrix). PDLA sees it
...
=item But Scilab ignores the last dimension of a 4x1x1 matrix.
  ...
  Scilab                       PerlDL
  ------                       ------
  --> A = ones(4,1,1);          pdla> $A = ones 4,1,1
  --> size(A)                   pdla> p $A->dims
  ans = 4 1                    4 1 1
...
=item And Scilab treats a 4x1x1 matrix differently from a 1x1x4 matrix.
  ...
  Scilab                       PerlDL
  ------                       ------
  --> A = ones(1,1,4);          pdla> $A = ones 1,1,4
  --> size(A)                   pdla> p $A->dims
  ans = 1 1 4                  1 1 4

=item Scilab has no direct syntax for N-D arrays.

  pdla> $A = pdl [ [[1,2,3],[4,5,6]], [[2,3,4],[5,6,7]] ]
  pdla> p $A->dims
  3 2 2

=item Feature support.

In Scilab, several features are not available for N-D arrays. In PDLA,
...
supported by N-dimensional piddles. There is usually no distinction:
  ...
  Scilab                       PerlDL
  ------                       ------
  --> A = ones(3,3,3);         pdla> $A = ones(3,3,3);
  --> A'                       pdla> transpose $A
      => ERROR                         => OK
...
PDLA has many tools that facilitate writing vectorized programs.
These are beyond the scope of this guide. To learn more, see:
L<PDLA::Indexing|PDLA::Indexing>, L<PDLA::Threading|PDLA::Threading>,
and L<PDLA::PP|PDLA::PP>.
...
One of the most interesting differences between PDLA and other tools
...
PDLA's syntax for declaring functions differs significantly from Scilab's.
...
a function declaration in PDLA:
...
Perl hashes are similar to Scilab's structure arrays:
  ...
  PerlDL
  ------
  pdla> %drink = ( type => 'coke' , size => 'large', mypiddle => ones(3,3,3) )
  pdla> $drink{type} = 'sprite'
  pdla> $drink{price} = 12   # Add new field to hash.
...
PDLA has powerful performance features, some of which are not normally
...
PDLA's "vectorization" feature goes beyond what most numerical software
...
generates real C code which can be compiled. With PDLA:PP
...
PDLA has full-featured plotting abilities. Unlike Scilab, PDLA relies more on
...
PDLA has three main plotting modules:
...
The native PDLA 3D graphics library using OpenGL as a backend
...
Through Perl, PDLA has access to all the major toolkits for creating
...
simulator. It is part of the standard Scilab distribution. PDLA and
