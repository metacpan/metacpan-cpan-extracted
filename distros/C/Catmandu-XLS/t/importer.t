...
$importer = Catmandu::Importer::XLS->new( file => './t/test.xls', columns => 1);
$rows = $importer->to_array();
...
$importer = Catmandu::Importer::XLSX->new( file => './t/test.xlsx', columns => 1);
$rows = $importer->to_array();
is_deeply ($rows->[0], {A => 1,B => 'a',C => 0.01}, 'XLSX option columns');
is_deeply ($rows->[-1], {A => 27,C => 'Ümlaut'}, 'XLSX option columns');

$importer = Catmandu::Importer::XLSX->new( file => './t/test.xlsx', header => 0);
$rows = $importer->to_array();
