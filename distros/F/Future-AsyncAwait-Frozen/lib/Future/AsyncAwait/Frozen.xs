...
{
  fprintf(stderr, "Future::AsyncAwait::Frozen panic: ");
  vfprintf(stderr, fmt, args);
  raise(SIGABRT);
}
...
static SV *MY_future_classname(pTHX)
{
  ...
  SV *class = cop_hints_fetch_pvs(PL_curcop, "Future::AsyncAwait::Frozen/future", 0);
...
static int async_keyword_plugin(pTHX_ OP **op_ptr)
{
  ...
  hv_stores(GvHV(PL_hintgv), "Future::AsyncAwait::Frozen/PL_compcv", newSVuv(PTR2UV(PL_compcv)));
...
{
  SV **asynccvp = hv_fetchs(GvHV(PL_hintgv), "Future::AsyncAwait::Frozen/PL_compcv", 0);
...
static int my_keyword_plugin(pTHX_ char *kw, STRLEN kwlen, OP **op_ptr)
{
  ...
  if(kwlen == 5 && strEQ(kw, "async") &&
      hv_fetchs(hints, "Future::AsyncAwait::Frozen/async", 0))
...
MODULE = Future::AsyncAwait::Frozen    PACKAGE = Future::AsyncAwait::Frozen
