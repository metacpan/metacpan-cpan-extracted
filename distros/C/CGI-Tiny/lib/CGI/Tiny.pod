...
=head3 set_request_body_buffer
  ...
  $cgi = $cgi->set_request_body_buffer(128*1024);

Sets the buffer size for reading the request body (number of bytes to read at
once). Defaults to the value of the C<CGI_TINY_REQUEST_BODY_BUFFER> environment
...
characters, as an unsorted array reference.
