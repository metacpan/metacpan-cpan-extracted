...
for my $filename (map bsd_glob($_), @ARGV) {
	my $outname = "$filename.hex";
	say "$filename => $outname";
	open my $in, '<:raw', $filename or die "$filename: $!";
	open my $out, '>', $outname or die "$outname: $!";
	local $/;
	print $out unpack 'H*', <$in>;
}
...
tohex.pl - hex encode files to be able to ship them with intact newlines

=head1 SYNOPSIS

	perl tohex.pl <file or pattern> ...

=head1 DESCRIPTION

Stores the hexified content of the given files with the added extension .hex .
