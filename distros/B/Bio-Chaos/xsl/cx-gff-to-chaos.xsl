...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:key name="k-feature" match="//gff_feature" use="feature_id"/>

  <xsl:template match="/gffset">
    <chaos>
      <xsl:for-each select="//gff_feature">
        <!-- nest all features directly under chado element -->
        <feature>
          <feature_id>
            <xsl:apply-templates mode="make-id" select="."/>
          </feature_id>
          <featureloc>
          </featureloc>
