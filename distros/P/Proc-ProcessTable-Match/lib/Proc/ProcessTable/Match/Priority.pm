package Proc::ProcessTable::Match::Priority;
...
Proc::ProcessTable::Match::Priority - Check if the Priority of a process matches.
...
=head1 SYNOPSIS
    ...
    use Proc::ProcessTable::Match::Priority;
    ...
    my $checker=Proc::ProcessTable::Match::Priority->new( \%args );
...
that is 'priorities', which is a array of priority values to match.

The Priority values can be prefixed with the equalities below for doing
...
sub match{
	...
	my $proc_priority;
	eval{
		$proc_priority=$object->priority;
	};

	# don't bother proceeding, the object won't match ever
	# as it does not have a Priority
