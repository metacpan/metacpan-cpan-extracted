...
(This example is included as L<F<synopsis-cpp.pl>|https://fastapi.metacpan.org/source/BKB/C-Tokenize-0.17/examples/synopsis-cpp.pl> in the distribution.)  
...
(This example is included as L<F<synopsis-comment.pl>|https://fastapi.metacpan.org/source/BKB/C-Tokenize-0.17/examples/synopsis-comment.pl> in the distribution.)  

=head1 VERSION

This documents version 0.17 of C::Tokenize corresponding
to git commit L<e7734a52fb15da79a33967349ffcfbec72127ef8|https://github.com/benkasminbullock/C-Tokenize/commit/e7734a52fb15da79a33967349ffcfbec72127ef8> released on Thu Aug 31 09:22:10 2017 +0900.

=head1 DESCRIPTION

This module provides a tokenizer, L</tokenize>, which breaks C source
code into its smallest meaningful components, and the regexs which
match each of these components. For example, the module supplies a
regex L</$comment_re> which matches a C comment.

As well as the basic components of C, it also supplies regexes for,
for example, local include statements, L</$include_local>, or C
variables, L</$cvar_re>, as well as extra functions, like
L</decomment> to remove the comment syntax of traditional C comments,
and L</strip_comments> which removes all comments from a C program.
...
Most of the following regular expressions do not do any capturing,
except where noted. If you want to capture, add your own parentheses
...
(This example is included as L<F<includes.pl>|https://fastapi.metacpan.org/source/BKB/C-Tokenize-0.17/examples/includes.pl> in the distribution.)  
...
Match an octal number, a number with digits 0 to 7 only which begins
with a leading zero.
...
(This example is included as L<F<cvar.pl>|https://fastapi.metacpan.org/source/BKB/C-Tokenize-0.17/examples/cvar.pl> in the distribution.)  
...
(This example is included as L<F<strip-comments.pl>|https://fastapi.metacpan.org/source/BKB/C-Tokenize-0.17/examples/strip-comments.pl> in the distribution.)  
...
(This example is included as L<F<strip-json.pl>|https://fastapi.metacpan.org/source/BKB/C-Tokenize-0.17/examples/strip-json.pl> in the distribution.)  
