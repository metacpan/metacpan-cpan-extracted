# encoding: GBK
# This file is encoded in GBK.
die "This file is not encoded in GBK.\n" if q{偁} ne "\x82\xa0";

use strict;
use GBK;
print "1..8\n";

my $__FILE__ = __FILE__;

# Test::Harness::runtests() and reset() make "Error: Runtime exception".
#
# t/304_m_once_reset........Error: Runtime exception
# Error: Runtime exception
# Can't spawn "cmd.exe": No such file or directory at Foo.pm line NNN.
# Callback called exit at t/304_m_once_reset.t line NNN.
# BEGIN failed--compilation aborted at t/304_m_once_reset.t line NNN.
# dubious
#         Test returned status 2 (wstat 512, 0x200)
# DIED. FAILED tests 1-8
#         Failed 8/8 tests, 0.00% okay

if ($] =~ /^5\.005/) {
    for my $tno (1 .. 8) {
        print qq{ok - $tno # SKIP $^X/$] $^O $__FILE__\n};
    }
    exit;
}

my @m_once = ();

@m_once = ();
for (qw(偁偁偁 偁偁偁 偁偁偁)) {
    if ($_ =~ m?偁偁偁?) {
        push @m_once, 1;
    }
    else {
        push @m_once, 0;
    }
    reset;
}
if (join(',',@m_once) eq '1,1,1') {
    print qq{ok - 1 m?偁偁偁?; reset $^X $__FILE__\n};
}
else {
    print qq{not ok - 1 m?偁偁偁?; reset $^X $__FILE__\n};
}

@m_once = ();
my $re = '';
for (qw(偁偁偁 偄偄偄 偆偆偆)) {
    $re = $_;
    if ($_ =~ m?$re?o) {
        push @m_once, 1;
    }
    else {
        push @m_once, 0;
    }
    reset;
}
if (join(',',@m_once) eq '1,0,0') {
    print qq{ok - 2 m?\$re?o; reset $^X $__FILE__\n};
}
else {
    if (($^O eq 'MSWin32') and ($] eq '5.006001')) {
        print qq{ok - 2 # SKIP m?\$re?o; reset $^X/$] $^O $__FILE__\n};
    }
    else {
        print qq{not ok - 2 m?\$re?o; reset $^X $__FILE__\n};
    }
}

@m_once = ();
for (qw(偁偁偁 偁偁偁 偁偁偁)) {
    if ($_ =~ m? 偁 偁 偁 ?x) {
        push @m_once, 1;
    }
    else {
        push @m_once, 0;
    }
    reset;
}
if (join(',',@m_once) eq '1,1,1') {
    print qq{ok - 3 m? 偁 偁 偁 ?x; reset $^X $__FILE__\n};
}
else {
    print qq{not ok - 3 m? 偁 偁 偁 ?x; reset $^X $__FILE__\n};
}

@m_once = ();
for ("偁偁偁\n偄偄偄","偁偁偁\n偄偄偄","偁偁偁\n偄偄偄") {
    if ($_ =~ m?^偄偄偄?m) {
        push @m_once, 1;
    }
    else {
        push @m_once, 0;
    }
    reset;
}
if (join(',',@m_once) eq '1,1,1') {
    print qq{ok - 4 m?^偄偄偄?m; reset $^X $__FILE__\n};
}
else {
    print qq{not ok - 4 m?^偄偄偄?m; reset $^X $__FILE__\n};
}

@m_once = ();
for ("偁\n偄","偁\n偄","偁\n偄") {
    if ($_ =~ m?偁.偄?s) {
        push @m_once, 1;
    }
    else {
        push @m_once, 0;
    }
    reset;
}
if (join(',',@m_once) eq '1,1,1') {
    print qq{ok - 5 m?偁.偄?s; reset $^X $__FILE__\n};
}
else {
    print qq{not ok - 5 m?偁.偄?s; reset $^X $__FILE__\n};
}

@m_once = ();
for (qw(AAA AAA AAA)) {
    if ($_ =~ m?aaa?i) {
        push @m_once, 1;
    }
    else {
        push @m_once, 0;
    }
    reset;
}
if (join(',',@m_once) eq '1,1,1') {
    print qq{ok - 6 m?aaa?i; reset $^X $__FILE__\n};
}
else {
    print qq{not ok - 6 m?aaa?i; reset $^X $__FILE__\n};
}

@m_once = ();
for (qw(偁偁偁 偁偁偁 偁偁偁)) {
    if ($_ =~ m?偁偁偁?g) {
        push @m_once, 1;
    }
    else {
        push @m_once, 0;
    }
    reset;
}
if (join(',',@m_once) eq '1,1,1') {
    print qq{ok - 7 m?偁偁偁?g; reset $^X $__FILE__\n};
}
else {
    print qq{not ok - 7 m?偁偁偁?g; reset $^X $__FILE__\n};
}

@m_once = ();
for (qw(偁偁偁 偁偁偁 偁偁偁)) {
    if ($_ =~ m?偁偁偁?gc) {
        push @m_once, 1;
    }
    else {
        push @m_once, 0;
    }
    reset;
}
if (join(',',@m_once) eq '1,1,1') {
    print qq{ok - 8 m?偁偁偁?gc; reset $^X $__FILE__\n};
}
else {
    print qq{not ok - 8 m?偁偁偁?gc; reset $^X $__FILE__\n};
}

__END__

