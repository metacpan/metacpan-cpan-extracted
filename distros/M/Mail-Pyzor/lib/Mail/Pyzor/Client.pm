...
sub _send_receive_msg {
    ...
    my $thread_id = $msg_ref->{'Thread'} or die 'No thread ID?';
...
sub _receive_packet {
    ...
    while (1) {
        ...
        my $found = IO::SigGuard::select( $rout = $rin, undef, undef, $time_left );
...
sub _send_packet {
    ...
    $sock->blocking(1);
    IO::SigGuard::syswrite( $sock, $packet ) or warn "write to socket: $!";
...
sub _get_user_pass_hash_key {
    ...
    return lc Mail::Pyzor::SHA::sha1_hex( $self->{'_username'} . ':' . $self->{'_password'} );
}
