...
my $measure = "path";
my $configfile = "rel-config.fma.par-chd";		      
my $keyfile = "error.sab.rel";
    
my $config  = File::Spec->catfile('t','config', $configfile);
my $key     = File::Spec->catfile('t', 'key', $version, $keyfile);
    
my $output = `$perl $util_prg --realtime --config $config --measure $measure arm foot 2>&1`;
...
$measure = "lesk";
$configfile = "config.fma.par-chd";		      
$keyfile = "error.sabdef.reldef";
  
$config  = File::Spec->catfile('t','config', $configfile);
$key     = File::Spec->catfile('t', 'key', $version, $keyfile);
    
$output = `$perl $util_prg --config $config --measure $measure arm foot 2>&1`;
