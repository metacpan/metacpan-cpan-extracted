...
Data::SIprefixes::micro - This provides micro matching for Data::SIprefixes.
...
=head1 SYNOPSIS
    ...
    use Data::SIprefixes::centi;
    
    my $prefix=Data::SIprefixes::micro->new;

    my $origMeasure='micrometer';
...
sub longMatch{
	...
	if ( $measure=~/^milli/ ){
		...
		if ( $measure =~ /^ / ){
			$self->{error}=2;
			$self->{errorString}='Space found after prefix, /^milli/, in "'.$origMeasure.'"' ;
			$self->warn;
			return undef;
		}
...
sub shortMatch{
    ...
    if ( $measure=~/^u/ ){
        ...
        if ( $measure =~ /^ / ){
            $self->{error}=2;
            $self->{errorString}='Space found after prefix, /^u/, in "'.$origMeasure.'"' ;
            $self->warn;
            return undef;
        }
...
1; # End of Data::SIprefixes::micro
