...
(This example is included as L<F<synopsis.pl>|https://api.metacpan.org/source/BKB/Lingua-JA-FindDates-0.024/examples/synopsis.pl> in the distribution.)


=head1 VERSION

This documents Lingua::JA::FindDates version 0.024
corresponding to git commit L<ba6a0c6a9ec653333dbabf2076dd5f71a17b64b9|https://github.com/benkasminbullock/Lingua--JA--FindDates/commit/ba6a0c6a9ec653333dbabf2076dd5f71a17b64b9> released on Tue Mar 14 16:36:13 2017 +0900.
...
If a code reference is supplied as C<replace>, subsjdate calls it as a
subroutine with user-defined data L</data>, and C<$before>, the
matched date and C<$after> the string with which it is to be replaced.

If C<replace> is not supplied, subsjdate substitutes the dates itself.

=item data

Any data you want to pass to L</replace>, above. If nothing is
supplied, C<subsjdate> simply passes the undefined value.
...
12, 1998". To change this to dates in the form "Th 2008/7/3", use a
...
(This example is included as L<F<subsjdate-make-date.pl>|https://api.metacpan.org/source/BKB/Lingua-JA-FindDates-0.024/examples/subsjdate-make-date.pl> in the distribution.)


The first two arguments passed to the user-defined routine are
C<$data>, user-defined data as described in L</data>, and
C<$original>, the original Japanese-language date. The following
argument is the date as a hash reference, with the fields C<year>
(Western-style year), C<month> (1-12), C<date> (1-31), and C<wday>
(1-7 for Monday to Sunday). Your routine must check whether the fields
C<year>, C<month>, C<date>, and C<wday> are defined, since
L</subsjdate> matches all kinds of dates including year only,
month/day only, and year/month only dates.
...
C<$original> as for C<make_date>, and the two dates in the form of
...
(This example is included as L<F<subsjdate-make-interval.pl>|https://api.metacpan.org/source/BKB/Lingua-JA-FindDates-0.024/examples/subsjdate-make-interval.pl> in the distribution.)
...
(This example is included as L<F<seireki-to-nengo.pl>|https://api.metacpan.org/source/BKB/Lingua-JA-FindDates-0.024/examples/seireki-to-nengo.pl> in the distribution.)
...
(This example is included as L<F<nengo-to-seireki.pl>|https://api.metacpan.org/source/BKB/Lingua-JA-FindDates-0.024/examples/nengo-to-seireki.pl> in the distribution.)
...
with kanji numbers up to 9,999. Please see L</Lingua::JA::Numbers> for
a fuller system.
...
(This example is included as L<F<make-date.pl>|https://api.metacpan.org/source/BKB/Lingua-JA-FindDates-0.024/examples/make-date.pl> in the distribution.)
...
(This example is included as L<F<my-date.pl>|https://api.metacpan.org/source/BKB/Lingua-JA-FindDates-0.024/examples/my-date.pl> in the distribution.)
...
(This example is included as L<F<default-make-date-interval.pl>|https://api.metacpan.org/source/BKB/Lingua-JA-FindDates-0.024/examples/default-make-date-interval.pl> in the distribution.)
...
request. All the functions can be exported using the tag C<:all> as in
