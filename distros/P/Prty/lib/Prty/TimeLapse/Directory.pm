package Prty::TimeLapse::Directory;
...
sub new {
    ...
    my @images;
    for my $file (Prty::Path->find($dir,-type=>'f')) {
        if ($file =~ /range.def$/) {
            next;
        }
        push @images,Prty::TimeLapse::File->new($file);
    }
    ...
    return $class->SUPER::new(
        ...
        imageA=>\@images,
        imageH=>\%hash,
        rangeDef=>Prty::TimeLapse::RangeDef->new($dir),
    );
}
...
sub importImages {
    ...
    my @images = Prty::Image->findImages(
        -sort=>$sort,
        -object=>'Prty::File::Image',
        $srcDir,
    );
    ...
    if ($subDir) {
        ...
        if (!-d $destDir) {
            Prty::Path->mkdir($destDir,-recursive=>1);
        }
    }
    ...
    my $pro = Prty::Progress->new(scalar(@images),
        -show=>$verbose,
    );
    my $i = 0;
    for my $img (@images) {
...
sub reorganize {
    ...
    for my $img ($tdr->images('all')) {
        ...
        if (!-e $subDir) {
            if (!$dryRun) {
                Prty::Path->mkdir($subDir,-recursive=>1);
            }
        ...
        if ($destFile ne $srcFile) {
            ...
            if (!$dryRun) {
                Prty::Path->duplicate($mode,$srcFile,$destFile);
            }
        }
    }
    ...
    for my $dir (sort Prty::Path->find($dir,-type=>'d')) {
        if (Prty::Path->isEmpty($dir)) {
            if (!$dryRun) {
                Prty::Path->rmdir($dir);
            }
