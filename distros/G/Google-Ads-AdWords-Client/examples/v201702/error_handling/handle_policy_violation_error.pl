...
use Google::Ads::AdWords::v201702::ExemptionRequest;
...
sub handle_policy_violation_error {
  ...
  my $exemptable_text_ad = Google::Ads::AdWords::v201702::ExpandedTextAd->new({
      ...
      if ($error->get_ApiError__Type() =~ "PolicyViolationError") {
        ...
        if ($error->get_isExemptable()) {
          ...
          $operation->set_exemptionRequests([
              new Google::Ads::AdWords::v201702::ExemptionRequest(
                {key => $error->get_key()})]);
        } else {
