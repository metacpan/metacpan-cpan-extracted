...
sub _dump
{
    ...
    if ($class && $ref) {
	if ($OBJECT_AS eq 'default_string') {
            $out = _dump(_obj_as_default_string($rval));
	} elsif ($OBJECT_AS eq 'string') {
            $out = _dump("$rval");
        } else {
...
1;
# ABSTRACT: Like Data::Dump but with some more options
...
In addition to L<Data::Dump>'s options, there are a few package variables that
can be set to modify the output generated by the dump functions. It's wise to
localize the setting of these.

=over

=item $Data::Dump::Options::OBJECT_AS

If set to "string", then objects (blessed references) will just be dumped as:
...
unless the object overloads stringification (see L<overload>).

If set to "default_string", then object will be dumped as the standard:
...
Please visit the project's homepage at L<https://metacpan.org/release/Data-Dump-Options>.

=head1 SOURCE

Source repository is at L<https://github.com/perlancar/perl-Data-Dump-Options>.

=head1 BUGS

Please report any bugs or feature requests on the bugtracker website L<https://rt.cpan.org/Public/Dist/Display.html?Name=Data-Dump-Options>
