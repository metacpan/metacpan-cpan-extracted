...
Returns the numerator as a signed C<Math::AnyNum> object. When C<x> is not a rational number,
...
Returns the numerator as an unsigned C<Math::AnyNum> object. When C<x> is not a rational number,
it tries to convert it to a rational. Returns NaN when this is not possible.
