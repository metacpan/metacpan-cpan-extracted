...
Send request to Zusaar API.
Also, this method has supported a fetch like 'Auto-Pager'.

=over 4

=item * $api_path = Path of request to Zusaar API. Currently available: "event" or "event/user".
...
In the case of default, you can fetch max 10 items by single request to Zusaar API.
...
L<https://github.com/mugifly/p5-WebService-Zusaar/> - Your feedback is highly appreciated.
