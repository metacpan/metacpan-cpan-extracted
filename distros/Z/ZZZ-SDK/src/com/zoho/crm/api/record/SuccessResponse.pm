...
package record::SuccessResponse;
use Moose;
sub new
{
	my ($class) = shift;
...
sub get_duplicate_field
{
	my ($self) = shift;
	return $self->{duplicate_field}; 
}

sub set_duplicate_field
{
	my ($self,$duplicate_field) = @_;
	$self->{duplicate_field} = $duplicate_field; 
	$self->{key_modified}{"duplicate_field"} = 1; 
}
...
{
	my ($self,$action) = @_;
	if(!(($action)->isa("Choice")))
	{
		die SDKException->new($Constants::DATA_TYPE_ERROR, "KEY: action EXPECTED TYPE: Choice", undef, undef); 
	}
	$self->{action} = $action; 
