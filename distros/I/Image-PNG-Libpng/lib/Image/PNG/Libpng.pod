...
(This example is included as L<F<synopsis.pl>|https://fastapi.metacpan.org/source/BKB/Image-PNG-Libpng-0.48/examples/synopsis.pl> in the distribution.)


=head1 VERSION

This document describes Image::PNG::Libpng version 0.48, corresponding to git commit L<7d88f3787cb6c47f9571491a82190f31125885fd|https://github.com/benkasminbullock/image-png-libpng/commit/7d88f3787cb6c47f9571491a82190f31125885fd> at Sun Nov 22 12:23:42 2020 +0900.
...
format. Image::PNG::Libpng does not contain libpng. It must be
installed prior to installing Image::PNG::Libpng.
...
(This example is included as L<F<get-www-png.pl>|https://fastapi.metacpan.org/source/BKB/Image-PNG-Libpng-0.48/examples/get-www-png.pl> in the distribution.)
...
(This example is included as L<F<png-cgi.pl>|https://fastapi.metacpan.org/source/BKB/Image-PNG-Libpng-0.48/examples/png-cgi.pl> in the distribution.)
...
(This example is included as L<F<copy-png.pl>|https://fastapi.metacpan.org/source/BKB/Image-PNG-Libpng-0.48/examples/copy-png.pl> in the distribution.)
...
L</get_rows> instead. Returns undef if given unknown chunk. This is
used by L<pnginspect> to get the various valid chunks.
...
instead of the several pointers to integers used in libpng. It does
not return the fields C<filter_method> and C<compression_method>,
which are always 0.
...
(This example is included as L<F<color-type-name.pl>|https://fastapi.metacpan.org/source/BKB/Image-PNG-Libpng-0.48/examples/color-type-name.pl> in the distribution.)
...
to zero. If month or day are omitted, these are set to 1. 
PNG tIME chunks do not contain a time zone. According to L<this subsection|http://www.w3.org/TR/PNG/#11tIME>
of L</The PNG specification>, "Universal Time (UTC) should be specified rather
than local time."
...
(C<ITXT>), Image::PNG::Libpng automatically puts it into Perl's
internal Unicode encoding (UTF-8). This is valid because PNG
international text C<ITXT> is required to be in the UTF-8 encoding,
and non-international text is required to contain whitespace and
printable ASCII characters only. See L</The PNG specification> for more on the
requirements of a PNG text section.

This function corresponds to C<png_get_text> in libpng, with a Perl array of hash
...
If the compression field is not set, a default value of
PNG_TEXT_COMPRESSION_NONE is applied. The compression field can take
...
Whether or not it's an ITXT field is also decided by the compression
value.

The C<text_length> value is
generated from the C<text> input field. Optionally the following
fields can be added:
...
This function corresponds to C<png_get_libpng_ver> in libpng. However, it
...
Get the maximum width the user will allow, as set by
L</set_user_limits>.
...
Get the maximum height the user will allow, as set by
L</set_user_limits>.
...
These functions aren't fully documented and are subject to change. You
can examine the source code or the libpng documentation to discover
their function.
...
you can find each instance L<in the file C<perl-libpng.c> in the top directory of the distribution|http://cpansearch.perl.org/src/BKB/Image-PNG-Libpng-0.48/perl-libpng.c> in the macro
C<UNUSED_ZERO_ARG>.
...
C<png_write_chunk>, C<png_write_end>, C<png_write_info>, C<png_write_row>, or
C<png_write_rows>.
