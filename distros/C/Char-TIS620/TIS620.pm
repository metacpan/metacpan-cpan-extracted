...
######################################################################
#
# TIS620 - Source code filter to escape TIS-620 script
#
...
perl $0 TIS-620_script.pl > Escaped_script.pl.e
END
    }

    print TIS620::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (CORE::eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Etis620::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
        }
    ...
    my $fh = gensym();
    Etis620::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub TIS620::escape_script {
    ...
    # read TIS-620 script
    my $fh = gensym();
    Etis620::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script\n";

    if (/^ use Etis620(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Etis620 '%s.0'; # 'quote' for perl5.6\n", $TIS620::VERSION); # require run-time routines version

        # use TIS620 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | TIS620 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $TIS620::VERSION) {
                    my @file = grep -e, map {qq{$_/TIS620.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects TIS620.pm $version, but @{[__FILE__]} is version $TIS620::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Etis620.pm $version, but \\\$Etis620::VERSION is \$Etis620::VERSION" if \$Etis620::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $TIS620::VERSION) {
                    die "Script $0 required TIS620.pm $version, but @{[__FILE__]} is only version $TIS620::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Etis620::function_ord     = 'TIS620::ord'     if /\A ord \z/oxms;
                    $Etis620::function_ord_    = 'TIS620::ord_'    if /\A ord \z/oxms;
                    $Etis620::function_reverse = 'TIS620::reverse' if /\A reverse \z/oxms;
                    $Etis620::function_getc    = 'TIS620::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= TIS620::escape();
...
=item * You'll have to download TIS620.pm and Etis620.pm and put it in your perl lib directory.

=item * You'll need to write "use TIS620;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use TIS620;
  use TIS620 ver.sion;             --- require minimum version
  use TIS620 ver.sion.0;           --- expects version (match or die)

  # "no TIS620;" not supported

  or

  $ perl TIS620.pm TIS-620_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  TIS-620_script.pl  --- script written in TIS-620
...
TIS620 software is "middleware" between perl interpreter and your Perl script
written in TIS-620.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain TIS-620, Perl3 or later
can treat TIS-620 as binary data.

By "use TIS620;", it automatically interpret your script as TIS-620. The various
functions of perl including a regular expression can treat TIS-620 now.
...
This software is a source code filter to escape Perl script encoded by TIS-620
...
=item * Remains One Language Handling Raw TIS-620, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. TIS620.pm, Etis620.pm
  +---------------------------------------+
...
import of Etis620.pm works well.
...
=head1 Software Composition
   ...
   TIS620.pm               --- source code filter to escape TIS-620
   Etis620.pm              --- run-time routines for TIS620.pm
...
You need write 'use TIS620;' in your script.
...
=head1 Calling 'Etis620::ignorecase()' (TIS620 software provides)

TIS620 software applies calling 'Etis620::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Etis620::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Etis620.pm provides)
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (TIS620 software provides)

TIS620 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (TIS620 software provides)

Insert 'Etis620::' at head of function name. Etis620.pm provides your script Etis620::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Etis620::lc        Character
  lcfirst     Etis620::lcfirst   Character
  uc          Etis620::uc        Character
  ucfirst     Etis620::ucfirst   Character
  fc          Etis620::fc        Character
  chr         Etis620::chr       Character
  glob        Etis620::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (TIS620 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (TIS620 software provides)

TIS620 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Etis620.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in TIS620 software and
...
TIS620 software remains one language and one interpreter.
