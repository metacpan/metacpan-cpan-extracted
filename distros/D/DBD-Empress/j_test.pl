...
my $test_dbname	= 'j_test_db';
my $conn_dbname	= 'j_test_db';
my $test_tabname = 'テスト_テーブル';
...
for $icreate (1 .. $ncreate) {
	...
	( $sth = $dbh->prepare( "CREATE TABLE $test_tabname ( 名前 nlschar, 名字 nlschar, 年齢 integer, 識別子 longinteger )" ) )
...
print "Testing: \$dbh->prepare( 'INSERT INTO $test_tabname VALUES ( \'Mr. マイク\', \'マグー\', 73, 99)' ): ";
( $sth = $dbh->prepare ( "INSERT INTO $test_tabname VALUES( \'Mr. マイク\', \'マグー\', 73, 99, \'Mr. マギラ\', \'ゴリラ\', 13, 98, \'Mr. バーニー\', \'ラブル\', 35, 97 )" ) )
...
print "Testing: \$dbh->prepare ( 'UPDATE $test_tabname SET 識別子 = 22  WHERE 名字 match \'マ*\'' ): ";
( $sth = $dbh->prepare ( "UPDATE $test_tabname SET 識別子 = 22 WHERE 名字 match \'マ*\'" ) )
...
print "Testing: \$dbh->prepare( 'DELETE FROM $test_tabname WHERE 識別子 < 5' ): ";
( $sth = $dbh->prepare( "DELETE FROM $test_tabname WHERE 識別子 < 5" ) )
...
print "Testing: \$cursor = \$dbh->prepare( 'SELECT FROM $test_tabname WHERE 識別子 = 1' ): ";
( $cursor = $dbh->prepare( "SELECT * FROM $test_tabname WHERE 識別子 = 1" ) )
