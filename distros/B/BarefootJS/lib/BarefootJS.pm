...
for my $attr (qw(
    c config backend
    _scripts _script_seen _preloads _preload_seen _portal_elements _scope_id _is_child _bf_parent _bf_mount _props
...
# other caller (marker IDs like "cond-start:s0", "loop:l0", ...), which are
# entirely compiler-generated, but the whole-item-conditional loop's
...
sub _register_manifest_child ($self, $slot_key, $marked, $signal_init, $manifest_defaults) {
    ...
    $self->register_child_renderer($slot_key, sub {
        ...
        # Shares the SAME root-level arrayref as `_scripts` above — every
        # child renderer registered here closes over `$parent` (the
        # instance `register_components_from_manifest` was called on, the
        # page root), so this reaches ANY "use client" nesting depth for
        # free, unlike the Go adapter's `PropagatePortals`, which needed an
        # explicit recursive walk because Go structs copy fields by value
        # rather than sharing a mutable reference (#3119).
