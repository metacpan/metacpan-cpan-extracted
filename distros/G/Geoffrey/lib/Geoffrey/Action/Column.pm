...
$Geoffrey::Action::Column::VERSION = '0.000101';
...
sub add {
    ...
    return $self->s_appending( $hr_params->{table}, $hr_params, $constraint )
      if $self->for_table;
        ...
        map {
            $self->do(
                Geoffrey::Utils::replace_spare(
                    $table->drop_column, [ $hr_params->{table}, $_ ]
                )
              )
...
sub s_appending {
    ...
    my $must_nn = ( defined $hr_params->{primarykey} ) ? 1 : 0;
    my $is_value = ( exists $hr_params->{default} || exists $hr_params->{foreignkey} ) ? 1 : 0;
    if ( $must_nn && !$is_value ) {
        require Geoffrey::Exception::RequiredValue;
        Geoffrey::Exception::RequiredValue::throw_column_default( $s_table_name,
            $hr_params->{name} );
    }
    return join q/ /,
      (
