#########################################################################################
# Package        HiPi::Interface::MonoOLED::DrawingContext
...
use constant {
    ...
    DEFAULT_FONT => HiPi::Interface::MonoOLED::Font::MONO_OLED_DEFAULT_FONT,
};
...
sub get_font {
    my($self, $fontname) = @_;
    HiPi::Interface::MonoOLED::Font->get_font( $fontname );
}

sub draw_circle {
    my( $self, $x0, $y0, $radius, $fill) = @_;
    return $self->draw_arc($x0, $y0, $radius, $radius, 0, 360, 0, $fill);
}
...
sub _get_line_points {
    ...
    if( $x1 == $x2 ) {
        ...
        if( abs($hdist) > abs($vdist) ) {
            my $ychange = $vdist / abs($hdist);
            my $xchange = ( $hdist > 0 ) ? 1 : -1;
            my $x = ( $hdist > 0 ) ? $x1 + 1 : $x1 - 1;
            my $end = abs($hdist);
            my $n = 1;
            while( $n < $end) {
                my $y = int( 0.5 + $y1 + $ychange * ( $n ) );
                push @points , [$x, $y];
                $x += $xchange;
                $n ++;
            }
        } else {
            my $xchange = $hdist / abs($vdist);
            my $ychange = ( $vdist > 0 ) ? 1 : -1;
            my $y = ( $vdist > 0 ) ? $y1 + 1 : $y1 - 1;
            my $end = abs($vdist);
            my $n = 1;
            while( $n < $end) {
                my $x = int( 0.5 + $x1 + $xchange * ( $n ) );
                push @points , [$x, $y];
                $y += $ychange;
                $n ++;
            }
        }
        push ( @points , [$x2, $y2] ) if $ep;
    }
