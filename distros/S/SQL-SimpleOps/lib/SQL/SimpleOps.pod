...
Sets the filename string  for writes log file process.
...
Enable/Disable the commit after a specific update command.

=item B<message_log:>

Enable/Disable the written on System Syslog Services.
...
Enable/Disable the written for SQL Log Files.
...
Sets the key that will use to search.
