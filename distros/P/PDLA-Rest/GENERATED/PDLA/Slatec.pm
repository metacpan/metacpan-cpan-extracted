...
=for example
  ...
  $v_in = pdl(1,0,1,0);
  ($azero,$a,$b) = PDLA::Slatec::fft($v_in);
...
=for example
  ...
  $v_out = PDLA::Slatec::rfft($azero,$a,$b);
  print $v_in, $vout
  [1 0 1 0] [1 0 1 0]
...
sub fft (;@) {
	...
	ezfftf($v,(my $az = PDLA->null), (my $a = PDLA->null),
		  (my $b = PDLA->null), $ws);
	return ($az,$a,$b);
}
...
sub PDLA::polyfit {
  ...
  my $a =      PDLA::new_from_specification('PDLA',$x_in->type,$sz,@otherdims);
