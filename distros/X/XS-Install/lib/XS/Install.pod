...
For development: by default, if any of header files changed, C<Makefile> will rebuild everything. If enabled (this is the default)
C<make> will only rebuild those C files that are dependent on changed headers.

This option has no effect if C<make> is neither C<GNU make> nor C<BSD make>. 
