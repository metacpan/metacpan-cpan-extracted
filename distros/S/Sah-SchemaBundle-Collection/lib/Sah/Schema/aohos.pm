...
This document describes version 0.009 of Sah::Schema::aohos (from Perl distribution Sah-SchemaBundle-Collection), released on 2024-06-13.
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("aohos", {return_type=>'str_errmsg'});
 ...
 $data = [{},{k=>undef}];
 my $errmsg = $validator->($data); # => "\@[1][k]: Required but not specified"

Often a schema has coercion rule or default value rules, so after validation the
validated value will be different from the original. To return the validated
(set-as-default, coerced, prefiltered) value:

 my $validator = gen_validator("aohos", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = [{},{k=>undef}];
 my $res = $validator->($data); # => ["\@[1][k]: Required but not specified",[{},{k=>undef}]]
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('aohos*', name=>'Aohos')
     );
 }

 use My::Types qw(Aohos);
 Aohos->assert_valid($data);
