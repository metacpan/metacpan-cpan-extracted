...
Break contact with the server, if that (still) exists.  Returns C<true> if
...
method will always return C<undef> because it is not possibly to reliably
...
specified C<$id>.  A C<undef> is returned if something has gone wrong.

The optional integer C<$bodylines> specifies the number of lines from the body
...
Translates the unique C<$id> of a message into a sequence number which
...
message with the specified C<$id>.  Returns C<undef> if something has gone
wrong.
...
Returns the size of the message which is indicated by the C<$id>, in octets.
...
Send C<$data> to the indicated socket and return the first line read from
...
Sends the indicated C<$command> to the specified socket, and retrieves the
...
=item Fault: cannot connect to $service for POP3: $!

Unsuccessful in connecting to the remote POP3 server.
Cast by C<login()>

=item Error: cannot get all messages of pop3 at once via messages().
...
=item Fault: cannot read POP3 from socket: $!
...
=item Error: could not authenticate using '$type' method.
...
=item Error: server at $service does not seem to be talking POP3.
