package MongoHosting::Provider::DigitalOcean;
...
use MongoHosting::Box::DigitalOcean;
...
sub _build_boxes {
  ...
  foreach my $host (@not_created) {
    ...
    $droplets{$droplet->name}
      = MongoHosting::Box::DigitalOcean->new(machine => $droplet);
  }
  print color('reset');
  print "\n";
...
sub _find_droplets {
  ...
  while ($obj = $droplets_collection->next) {
    ...
    $droplets{$obj->name} = MongoHosting::Box::DigitalOcean->new(machine => $obj);
  }
  return %droplets;
}
