package Proc::ProcessTable::Match::Fname;
...
Proc::ProcessTable::Match::Fname - Check if the fname of a process matches via regexp.
...
=head1 SYNOPSIS
    ...
    my $checker=Proc::ProcessTable::Match::Fname->new( \%args );
...
that is 'fnames', which is a array of fnames to match.

The matching is done via regexp.

Atleast one fname must be specified.
...
sub match{
	...
	if ( ! defined( $proc_fname ) ){
		return 0;
	}

	foreach my $fname ( @{ $self->{fnames} } ){
		if ( $proc_fname =~ /$fname/ ){
			return 1;
		}
	}
