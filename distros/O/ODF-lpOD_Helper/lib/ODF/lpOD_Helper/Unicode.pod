...
ODF::lpOD by itself is incompatible with the above paradigm by default
...
using them.  Therefore you must work with binary rather than character strings;
...
internal encoding & decoding, i.e. the original ODF::lpOD behavior
will be retained.

It is also possible to toggle between the old behavior and 
...
required a B<:chars> import tag.  This tag is now deprecated and
produces a warning. 
...
require Unicode processing to always work correctly.
