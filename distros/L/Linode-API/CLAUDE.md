...
Linode-API is a client for the Linode API: `OpenAPI::Client` pointed at the
specification Linode publishes, which ships in `share/openapi.json`.  Read
`lib/Linode/API.pm` and `t/Linode-API.t` first.  The module is small, and most
of it rewrites the specification where `OpenAPI::Client` would otherwise send
something wrong.
...
| | |
|---|---|
| `perldoc Linode-API` | what this is for and how a caller uses it |
