...
sub groucho_input {
  ...
  if ( $cmd eq 'NOTICE' ) {
    ...
    if ( $heap->{unkline} ) {
      ...
      is( $params->[1], 'Added temporary 10 min. K-Line [*bobb*@127.0.0.1]', 'Added temporary 10 min. K-Line [*bobb*@127.0.0.1]' );
    }
  }
  return;
}
...
sub client_disconnected {
  ...
  $heap->{unkline}++;
  $poe_kernel->post( 'groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'UNKLINE', params => [ '*', '*bobb*', '127.0.0.1' ], colonify => 0 } );
  return;
}
...
sub ircd_daemon_eob {
  ...
  if ( $heap->{eob} >= 3 ) {
    #$poe_kernel->yield('_launch_client');
    $kernel->post( 'groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'KLINE', params => [ '*', '600', '*bobb*', '127.0.0.1', 'Banhammer'] } );
  }
  return;
}
