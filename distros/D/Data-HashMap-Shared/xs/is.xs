...
new(char* class, char* path, UV max_entries, UV lru_max = 0, UV ttl_default = 0, UV lru_skip = 0)
    CODE:
        char errbuf[SHM_ERR_BUFLEN]; ShmHandle* map = shm_is_create(path, (uint32_t)max_entries, (uint32_t)lru_max, (uint32_t)ttl_default, (uint32_t)lru_skip, errbuf);
...
drain(SV* self_sv, UV limit)
    PPCODE:
        ...
        shm_is_drain_entry *entries = (shm_is_drain_entry *)calloc(limit, sizeof(shm_is_drain_entry));
