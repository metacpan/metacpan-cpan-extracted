...
use IRC::Utils qw[BOLD YELLOW NORMAL unparse_mode_line normalize_mask uc_irc];
...
while (<DATA>) {
  ...
  push @{ $bmasks{$type} },  $_ unless scalar @{ $bmasks{$type} }  > 5;
  push @{ $bmarsks{$type} }, $_ unless scalar @{ $bmarsks{$type} } > 2;
}
...
sub ircd_daemon_nick {
    ...
    my $user = 'OperServ!OperServ@listen.server.irc';

    foreach my $btype ( @types ) {
      my @marsk_list = @{ $bmarsks{$btype} };
      foreach my $marsk ( @marsk_list ) {
         my $mask = normalize_mask($marsk);
         my $umask = uc_irc($mask);
         $chanrec->{ $map{ $btype } }{$umask} =
              [ $mask, $user, time() ];
        }

    }
    $heap->{ircd}{state}{chans}{'#MARXBROS'} = $chanrec;
    $heap->{ircd}{state}{uids}{'1FUAAAAAA'}{chans}{'#MARXBROS'} = 'o';
...
sub client_input {
  ...
  if ( $cmd eq 'ERROR' ) {
    ...
    is( scalar keys %{ $state->{peers}{'LISTEN.SERVER.IRC'}{users} }, 1, 'One local users' );
    is( scalar keys %{ $state->{sids}{'1FU'}{uids} }, 1, 'One local UIDs' );
  ...
  if ( $cmd eq 'JOIN' && $prefix =~ m!^bobbins! ) {
    ...
    is( scalar keys %{ $state->{uids} }, 2, 'Two UID' );
    is( scalar keys %{ $state->{users} }, 2, 'Two user' );
    is( scalar keys %{ $state->{peers}{'LISTEN.SERVER.IRC'}{users} }, 2, 'Two local user' );
    is( scalar keys %{ $state->{sids}{'1FU'}{uids} }, 2, 'Two local UID' );
  ...
  if ( $cmd eq '353' ) {
    ...
    is( $params->[3], '@OperServ bobbins', 'I am not chanop' );
    return;
  }
  if ( $cmd eq '366' ) {
    pass("IRC$cmd");
