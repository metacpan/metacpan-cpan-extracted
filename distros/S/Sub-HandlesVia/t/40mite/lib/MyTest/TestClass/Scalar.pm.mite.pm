...
sub new {
    ...
    if ( exists($args->{q[attr]}) ) { (!!1) or require Carp && Carp::croak(q[Type check failed in constructor: attr should be Any]); $self->{q[attr]} = $args->{q[attr]};  } else { my $value = do { my $default_value = do { my $method = $MyTest::TestClass::Scalar::__attr_DEFAULT__; $self->$method }; (!!1) or do { require Carp; Carp::croak(q[Type check failed in default: attr should be Any]) }; $default_value }; $self->{q[attr]} = $value;  }
...
*_set_attr = sub { (!!1) or require Carp && Carp::croak(q[Type check failed in writer: value should be Any]); $_[0]{q[attr]} = $_[1]; $_[0]; };
