...
Las operaciones Docker son manejadas por el script C<dozo>, que también puede ser utilizado como un comando independiente. El script C<dozo> soporta el archivo de configuración C<.dozorc> para la configuración persistente del contenedor.
...
La biblioteca C<getoptlong.sh> se utiliza para el análisis sintáctico de opciones en los scripts C<xlate> y C<dozo>.
