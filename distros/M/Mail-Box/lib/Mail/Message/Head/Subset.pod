...
Count the number of fields with this C<$name>.  If the C<$name> cannot be found,
the full header get loaded.  In case we find any C<$name> field, it is
...
characters in C<$name> does not matter.  When a C<$name> is used which is not known
...
Improves base, see L<Mail::Message::Head/"About the body">
