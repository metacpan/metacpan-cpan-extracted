...
ok(
  do {
    eval {
      $addrGroup
        = PDK::Firewall::Element::AddressGroup::Srx->new(fwId => 1, addrGroupName => 'zzz', zone => 'xxx', description => 'yyy');
    };
    warn $@ if !!$@;
    $addrGroup->isa('PDK::Firewall::Element::AddressGroup::Srx');
  },
  ' 生成 PDK::Firewall::Element::AddressGroup::Srx 对象'
);
...
ok(
  do {
    my $address;
    eval {
      $address = PDK::Firewall::Element::Address::Srx->new(
        fwId        => 1,
        addrName    => 'xxxx',
        ip          => '192.168.8.1',
        mask        => '32',
  ...
  do {
    ...
    eval {
      ...
      $addrGroup
        = PDK::Firewall::Element::AddressGroup::Srx->new(fwId => 1, addrGroupName => 'zzz', zone => 'xxx', description => 'yyy');
      $addrGroup1
        = PDK::Firewall::Element::AddressGroup::Srx->new(fwId => 1, addrGroupName => 'mmm', zone => 'nnn', description => 'jjj');
    };
    warn $@ if !!$@;
    $addrGroup->addAddrGroupMember("member1", $address);
