...
The L<GB Standard|http://metacpan.org/pod/Dita::GB::Standard> name looks like this:
  ...
  human_readable_part_derived_from_content + _ + md5_sum_of_content + ext
...
The L<md4 sum|https://en.wikipedia.org/wiki/MD5> component of the file name is calculated from the content of the
file and presented as lowercase hexadecimal.

The B<ext> file extension component is obtained from:
L<https://en.wikipedia.org/wiki/List_of_filename_extensions>
...
If the option to present the L<md4 sum|https://en.wikipedia.org/wiki/MD5> sum as five letter English words is chosen
then the standardized name for this content becomes:
...
=head2 The Binary vs Utf8 GB Standard
...
The name is unique when computed by competing parallel processes so files that
have the same name have the same content and can be safely overwritten by
another process without attempting to coordinate names between processes.
...
Self constructed Content Management Systems using BitBucket, GitHub or Gitlab
that rely on guidization to differentiate files placed in these repositories
benefit immensely: the L<guid|https://en.wikipedia.org/wiki/Universally_unique_identifier> to use can be easily derived from the L<md4 sum|https://en.wikipedia.org/wiki/MD5> sum
...
The L<GB Standard|http://metacpan.org/pod/Dita::GB::Standard> has been implemented as a Perl package at:
