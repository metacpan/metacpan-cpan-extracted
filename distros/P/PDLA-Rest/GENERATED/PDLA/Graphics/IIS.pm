...
#
package PDLA::Graphics::IIS;

@EXPORT_OK  = qw(  iis iiscur iiscirc $stdimage $iisframe saoimage ximtool PDLA::PP _iis PDLA::PP _iiscirc );
...
are autoscaled. A good demonstration of PDLA threading can be had
...
sub iis {
    ...
    if ( $#_ == 1 ) { $min = $_[0]; $max = $_[1]; }
      ...
      my $opt = new PDLA::Options( { MIN => undef, MAX => undef, TITLE => $title, FRAME => $frame } );
      $opt->options( shift );
...
this module uses 
L<PDLA::PP|PDLA::PP> threading you can supply lists of points via
1D arrays, etc.

An amusing PDLA idiom is:
...
sub iiscirc {
   ...
   PDLA::_iiscirc($x, $y, $radius, $colour);
   1;
}
...
*_iis = \&PDLA::Graphics::IIS::_iis;





*_iiscirc = \&PDLA::Graphics::IIS::_iiscirc;
