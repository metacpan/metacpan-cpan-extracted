...
#
#   $line             : สตริงที่ต้องการแปลง (ส่งผ่านการอ้างอิง เขียนทับในตำแหน่งเดิม)
#   $OUTPUT_encoding  : ชื่อการเข้ารหัสเอาต์พุต
#   $INPUT_encoding   : ชื่อการเข้ารหัสอินพุต
#   $char_count       : จำนวนอักขระหลังการแปลง
#
# [รายชื่อการเข้ารหัส]
#
#   ตัวย่อ      ความหมาย
#   ---------   ---------------------------------------------------
#   cp932x      CP932X (ขยายไปยัง JIS X 0213 ใช้ single shift 0x9C5A)
#   cp932       Microsoft CP932 (Windows-31J / ชื่อ IANA)
#   cp932ibm    IBM CP932
#   cp932nec    NEC CP932
#   sjis2004    JISC Shift_JIS-2004
#   cp00930     IBM CP00930 (CP00290+CP00300, CCSID 5026 คาตาคานะ)
#   keis78      HITACHI KEIS78
#   keis83      HITACHI KEIS83
#   keis90      HITACHI KEIS90
#   jef         FUJITSU JEF (12pt ใช้ OUTPUT_SHIFTING สำหรับ shift codes)
#   jef9p       FUJITSU JEF ( 9pt ใช้ OUTPUT_SHIFTING สำหรับ shift codes)
#   jipsj       NEC JIPS(J)
#   jipse       NEC JIPS(E)
#   letsj       UNISYS LetsJ
#   utf8        UTF-8.0 (UTF-8 ทั่วไป)
#   utf8.1      UTF-8.1 (แปลงโดยใช้การจับคู่ Shift_JIS-Unicode ไม่ใช่ CP932)
#   utf8jp      UTF-8-SPUA-JP (JIS X 0213 แมปไปยัง SPUA ของ Unicode)
#
# [ตัวเลือก]
#
#   คีย์              ค่า / คำอธิบาย
#   ---------------   ---------------------------------------------------
#   INPUT_LAYOUT      รูปแบบเรคอร์ดอินพุต ('S'=SBCS, 'D'=DBCS พร้อมจำนวนซ้ำ)
#   OUTPUT_SHIFTING   ถ้าเป็นจริง จะแทรก shift codes รอบลำดับ DBCS ในเอาต์พุต
#   SPACE             รหัสช่องว่าง DBCS/MBCS (สตริงไบนารี)
#   GETA              สัญลักษณ์ geta สำหรับอักขระที่แมปไม่ได้
#   OVERRIDE_MAPPING  การแทนที่ต่ออักขระ { "\x12\x34"=>"\x56\x78" }
#
# [คำเตือน: การแปลงสองทิศทาง]
#
#   มีการแปลงแบบสูญเสีย (เช่น CP932 มี 398 การแมปที่ไม่สามารถย้อนกลับได้)
# [ความแตกต่างระหว่าง Jacode4e และ Jacode4e::RoundTrip]
#
#   Jacode4e              : แปลงทิศทางเดียวรวดเร็ว มีการสูญเสียสำหรับบางอักขระ
#                           (เช่น CP932 มี 398 การแมปที่ไม่สามารถย้อนกลับได้)
#   Jacode4e::RoundTrip   : รับประกันความเที่ยงตรงไปกลับผ่าน Unicode pivot
#                           A->B->A เหมือนกันเสมอ ช้ากว่า Jacode4e
#                           แต่ใช้เมื่อข้อมูลต้องไปกลับ
#
#   หากไม่จำเป็นต้องแปลงสองทิศทาง ให้ใช้โมดูล Jacode4e
#
# ======================================================================
...
BEGIN {
    ...
    @test = (
        ...
        # --- UTF-8.1: แปลงจาก Shift_JIS-Unicode ไม่ใช่ CP932 ---
