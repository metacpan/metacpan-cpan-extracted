...
sub client4_connected {
  my ($kernel,$heap,$sender) = @_[KERNEL,HEAP,SENDER];
  pass($_[STATE]);
  $kernel->post( $sender, 'send_to_server', { command => 'NICK', params => [ 'herman' ], colonify => 0 } );
  $kernel->post( $sender, 'send_to_server', { command => 'USER', params => [ 'herman', '*', '*', 'herman munster' ], colonify => 1 } );
  return;
}
...
sub client4_input {
  ...
  my $cmd    = $in->{command};
  my $params = $in->{params};
  if ( $cmd eq 'MODE' && $prefix =~ m'^herman' && $params->[1] eq '+i' ) {
    pass($cmd);
    $poe_kernel->post($sender, 'send_to_server', { command => 'USERHOST', params => [ qw[bobbins custard herman] ], colonify => 0 } );
    return;
  }
  if ( $cmd eq '302' ) {
    pass("IRC$cmd");
    is($in->{raw_line},':listen.server.irc 302 herman :bobbins*=+~bobbins@listen.server.irc custard=+~custard@listen.server.irc herman=+~herman@127.0.0.1',
      'Correct USERHOST response');
