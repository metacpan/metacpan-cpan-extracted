package Plugtools::Plugins::Samba::makeSambaAccount;
...
This plugin is responsible for taking standard POSIX account and using
Samba::SIDhelper to turn it into a usable accounf for Samba.
...
sub plugin{
	...
	if ($pts->{error}) {
		...
		warn('Plugtools-Plugins-Samba-makeSambaAccount plugin:1: '.$returned{errorString});
		return %returned;
	}
	...
	if ($pts->{error}) {
		$returned{error}=1;
		$returned{errorString}='Plugtools::Plugins::Samba->makeSambaAccountEntry errored. $pts->{error}="'.
		                       $pts->{error}.'" $pts->{errorString}="'.$pts->{errorString}.'"';
		warn('Plugtools-Plugins-Samba-makeSambaAccount plugin:1: '.$returned{errorString});
		return %returned;
	}
