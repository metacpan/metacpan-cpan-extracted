...
BEGIN
{
    use_ok( 'Net::API::Stripe::Error' ) || BAIL_OUT( "Unable to load perl module 'Net::API::Stripe::Error'" );
};
can_ok( 'Net::API::Stripe::Error', 'message' );
...
my $json_file = $sample_dir->child( 'error.json' ) ||
...
if( $json_file->exists )
{
    if( !$json_file->can_read )
    {
        ...
        BAIL_OUT( "Unable to read json file $rel for Stripe class 'error'" );
    }
    elsif( !$json_file->is_empty )
    {
        $code = $json_file->load_json ||
            BAIL_OUT( "Failed to load json data for Stripe class 'error': " . $json_file->error );
    }
}
my $obj = scalar( keys( %$code ) ) ? Net::API::Stripe::Error->new( $code ) : Net::API::Stripe::Error->new;
isa_ok( $obj => 'Net::API::Stripe::Error' );
