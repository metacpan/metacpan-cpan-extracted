...
# 5-15) Test cddbp proto synopsis
&test_syn();
...
sub test_syn {
    SKIP: {
        # Check for skip
        if ( ! $::opt{test_cddbp_proto_syn} ) {
            my $fn = File::Spec->catfile( 't', 'options' );
            my_skip( "test_cddbp_proto_syn is not set in file '$fn'" );
        }

        # 2) Require synopsis
        my $fn = File::Spec->catfile( 'gen', 'syn-cddbp-proto.pl' );
        try {
            require( $fn );
        }
