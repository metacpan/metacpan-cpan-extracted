...
my $passphrase2 = Crypt::Passphrase::Scrypt->new(cost => 15, block_size => 32);
...
my $passphrase3 = Crypt::Passphrase::Scrypt->new(cost => 15, block_size => 32, format => 'libcrypt');
my $hash4 = '$7$DU..../....2Q9obwLhin8qvQl6sisAO/$57x.voCZ7KkOfnXM7iv7973ues4B2.dMPicRyKJIfX2';
ok($passphrase3->verify_password('password', $hash4), '$7$ hash verifies');
ok(!$passphrase3->needs_rehash($hash4), '$7$ hash need not rehash');

my $hash5 = $passphrase2->recode_hash($hash4);
isnt($hash5, $hash4, 'recode has updated the hash');
ok($passphrase->verify_password('password', $hash5), 'Recoded hash verifies');
ok($passphrase3->verify_password('password', $hash5), 'Recoded hash verifies');
ok($passphrase3->needs_rehash($hash5), 'Recoded hash does need rehash');
