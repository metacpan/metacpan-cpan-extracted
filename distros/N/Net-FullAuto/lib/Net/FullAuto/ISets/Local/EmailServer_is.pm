...
END
   ...
   ($stdout,$stderr)=$handle->cmd($sudo.
      ...
      '--allow-root --path=/var/www/html/emailserver',
      '__display__');
   ($stdout,$stderr)=$handle->cmd($sudo.
      '/usr/local/bin/wp plugin install really-simple-ssl '.
      '--path=/var/www/html/emailserver','__display__');
   ($stdout,$stderr)=$handle->cmd($sudo.
...
if ($do==1) {
   ...
   ($stdout,$stderr)=$handle->cmd($sudo.
      ...
      "--activate --path=/var/www/html/emailserver",
      '__display__');
   ($stdout,$stderr)=$handle->cmd($sudo.
      "/usr/local/bin/wp theme install ${theme}-child.zip --allow-root ".
      "--activate --path=/var/www/html/emailserver",'__display__');
   ($stdout,$stderr)=$handle->cwd('/var/www/html/emailserver');
   ($stdout,$stderr)=$handle->cmd($sudo.
...
END
   ($stdout,$stderr)=$handle->cmd("echo -e \"$oc_style\" > ".
      "/var/www/html/emailserver/wp-content/themes/${theme}-child/style.css");
my $oc_func=<<END;
<?php
//* Code goes here
...
END
   ($stdout,$stderr)=$handle->cmd("echo -e \"$googlefont\" >> ".
      "/var/www/html/emailserver/wp-content/themes/${theme}-child/functions.php");
   $sudo='sudo -u www-data ';
   ($stdout,$stderr)=$handle->cmd($sudo.
      "sed -i '\$ d' ".
      "/var/www/html/emailserver/wp-content/themes/${theme}-child/functions.php");
...
| emailserver-importer                         | active   | none      | 0.6.4    |
| emailserver-popular-posts                    | active   | none      | 5.0.1    |
| emailserver-social-login                     | active   | none      | 2.3.3    |
...
| emailserver-seo                              | active   | none      | 12.8     |
+--------------------------------------------+----------+-----------+----------+
END
...
if ($do==1) {
   ...
   #   '--allow-root --activate --path=/var/www/html/emailserver',
   #   '__display__');
...
while (1) {
   ...
   last if $output=~/$prompt|Bye/;
   if (!$cmd_sent && $output=~/MariaDB.*?>\s*$/) {
      my $cmd='UPDATE emailserver.wp_options SET option_value = \'a:5:{s:18:"nav_menu_locations";a:0:{}s:18:"custom_css_post_id";i:-1;s:10:"meta_login";b:1;s:15:"nav_menu_search";b:1;s:20:"columns_ratio_header";s:3:"7-5";}\' WHERE option_name = \'theme_mods_memberlite-child\';';
      print "$cmd\n";
      $handle->print($cmd);
      $cmd_sent++;
      sleep 1;
      ...
      my $cmd='UPDATE emailserver.wp_options SET option_value = \'1\' WHERE option_name = \'users_can_register\';';
      print "$cmd\n";
      $handle->print($cmd);
      $cmd_sent++;
      sleep 1;
...
if ($do==1) {
   ...
   # https://www.cloudways.com/blog/elasticsearch-on-emailserver/
   ($stdout,$stderr)=$handle->cmd($sudo.
   ...
   ($stdout,$stderr)=$handle->cmd($sudo.
       "sed -i \'/DB_COLLATE/a$ep\' /var/www/html/emailserver/wp-config.php");
   ($stdout,$stderr)=$handle->cmd($sudo.
       "sed -i \"s/%SQ%/\'/g\" ".
       '/var/www/html/emailserver/wp-config.php');
   ($stdout,$stderr)=$handle->cmd($sudo.
       "sed -i \'s/%NL%/\'\"`echo \\\\\\n`/g\" ".
       '/var/www/html/emailserver/wp-config.php');
}
