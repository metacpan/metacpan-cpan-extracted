...
is_deeply([$user->_find_sql], ['SELECT id,name,email FROM users WHERE id=?', qw( 42 )], 'find');

is_deeply([$user->expand_sql('%t \\\%t')], ['users \%t'],     'escaped');
is_deeply([$user->expand_sql('%pc')],      ['id,name,email'], 'pc');

is_deeply([$user->expand_sql('%c.x from %t.x')],  ['x.name,x.email from users x'],      'alias x');
is_deeply([$user->expand_sql('%pc.x from %t.x')], ['x.id,x.name,x.email from users x'], 'alias x');

is_deeply([$user->_insert_sql],
  ['INSERT INTO users (name,email) VALUES (?,?) RETURNING id', qw( Bruce bruce@wayneenterprise.com )], 'insert');

is_deeply([$user->_update_sql],
