...
subtest 'tmpdir: C_DOCUMENT_ROOT/tmp used when it exists and is writable' => sub {
	...
	is($dir, $tmp, 'C_DOCUMENT_ROOT/tmp returned when it exists and is writable');
};
...
subtest '_find_paths: DOCUMENT_ROOT + SCRIPT_NAME builds script_path' => sub {
	...
	like($path, qr/app\.cgi$/, 'script_path built from DOCUMENT_ROOT + SCRIPT_NAME');
	like($path, qr/\Q$tmp\E/,  'script_path rooted at DOCUMENT_ROOT');
};
