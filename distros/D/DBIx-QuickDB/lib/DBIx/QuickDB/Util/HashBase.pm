package DBIx::QuickDB::Util::HashBase;
use strict;
use warnings;
...
{
    no warnings 'once';
    $DBIx::QuickDB::Util::HashBase::HB_VERSION = '0.009';
    *DBIx::QuickDB::Util::HashBase::ATTR_SUBS = \%Object::HashBase::ATTR_SUBS;
    *DBIx::QuickDB::Util::HashBase::ATTR_LIST = \%Object::HashBase::ATTR_LIST;
    *DBIx::QuickDB::Util::HashBase::VERSION   = \%Object::HashBase::VERSION;
    *DBIx::QuickDB::Util::HashBase::CAN_CACHE = \%Object::HashBase::CAN_CACHE;
}
...
sub import {
    ...
    my $ver = $DBIx::QuickDB::Util::HashBase::HB_VERSION || $DBIx::QuickDB::Util::HashBase::VERSION;
    $DBIx::QuickDB::Util::HashBase::VERSION{$into} = $ver if !$DBIx::QuickDB::Util::HashBase::VERSION{$into} || $DBIx::QuickDB::Util::HashBase::VERSION{$into} > $ver;

    my $isa = _isa($into);
    my $attr_list = $DBIx::QuickDB::Util::HashBase::ATTR_LIST{$into} ||= [];
    my $attr_subs = $DBIx::QuickDB::Util::HashBase::ATTR_SUBS{$into} ||= {};

    my %subs = (
        ($into->can('new') ? () : (new => \&_new)),
        (map %{$DBIx::QuickDB::Util::HashBase::ATTR_SUBS{$_} || {}}, @{$isa}[1 .. $#$isa]),
        (
            map {
                my $p = substr($_, 0, 1);
                my $x = $_;
...
sub attr_list {
    ...
    my @list = grep { !$seen{$_}++ } map {
        ...
        if (0.004 > ($DBIx::QuickDB::Util::HashBase::VERSION{$_} || 0)) {
            Carp::carp("$_ uses an inlined version of DBIx::QuickDB::Util::HashBase too old to support attr_list()");
        }
        else {
            my $list = $DBIx::QuickDB::Util::HashBase::ATTR_LIST{$_};
            @out = $list ? @$list : ()
        }
...
sub _new {
    ...
    $DBIx::QuickDB::Util::HashBase::CAN_CACHE{$class} = $self->can('init')
        unless exists $DBIx::QuickDB::Util::HashBase::CAN_CACHE{$class};

    $self->init if $DBIx::QuickDB::Util::HashBase::CAN_CACHE{$class};
...
DBIx::QuickDB::Util::HashBase - Build hash based classes.
...
Subclass it
    ...
    use base 'My::Class';
    use DBIx::QuickDB::Util::HashBase qw/bub/;
...
declare it before loading L<DBIx::QuickDB::Util::HashBase>.
...
B<Note:> DBIx::QuickDB::Util::HashBase checks for an init using C<< $class->can('init') >>
...
DBIx::QuickDB::Util::HashBase provides a function for retrieving a list of attributes for an
DBIx::QuickDB::Util::HashBase class.

=over 4

=item @list = DBIx::QuickDB::Util::HashBase::attr_list($class)

=item @list = $class->DBIx::QuickDB::Util::HashBase::attr_list()
