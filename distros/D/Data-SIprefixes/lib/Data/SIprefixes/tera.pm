...
Data::SIprefixes::tera - This provides tera matching for Data::SIprefixes.
...
=head1 SYNOPSIS
    ...
    use Data::SIprefixes::tera;
    
    my $prefix=Data::SIprefixes::tera->new;
...
sub longMatch{
	...
	if ( $measure=~/^tera/ ){
		...
		if ( $measure =~ /^ / ){
			$self->{error}=2;
			$self->{errorString}='Space found after prefix, /^tera/, in "'.$origMeasure.'"' ;
			$self->warn;
			return undef;
		}
...
sub shortMatch{
    ...
    if ( $measure=~/^T/ ){
        ...
        if ( $measure =~ /^ / ){
            $self->{error}=2;
            $self->{errorString}='Space found after prefix, /^T/, in "'.$origMeasure.'"' ;
            $self->warn;
            return undef;
        }
