...
static inline void dd_init_header(void *base, uint32_t num_buckets, double alpha, uint64_t total) {
    ...
    /* Zero the header, reader slots and occupancy bitmap (through the 8-byte
       alignment pad at L.neg).  The two count stores are NOT zeroed here: every
       caller hands us a region that is already zero -- a fresh ftruncate, a fresh
       anonymous/memfd map, or a file the recovery path just scanned -- so writing
       them would only fault in and allocate every page of an otherwise sparse
       file, which on a nearly-full filesystem turns create into a SIGBUS. This
       matches the other sketches (Histogram, CountMinSketch, HyperLogLog, TopK). */
    memset(base, 0, (size_t)L.neg);
    ...
    /* Publish magic LAST, as a release store: it is the commit point, so a
       ...
       hand. */
    __atomic_store_n(&hdr->magic, DD_MAGIC, __ATOMIC_RELEASE);
    ...
    if (anonymous) {
        ...
        if (!is_new) {
            if (!dd_validate_header((DdHeader *)base, (uint64_t)st.st_size)) {
                ...
                if (((DdHeader *)base)->magic == 0 && (uint64_t)st.st_size == total
                    && st.st_uid == geteuid()) {
                    DD_ERR("%s: incomplete DDSketch file left by an interrupted create; remove it and retry", path);
                    munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
                }
                DD_ERR("invalid DDSketch file"); munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
            }
            if (((DdHeader *)base)->sealed) {
                DD_ERR("%s is frozen (read-only); open it with new_readonly", path);
                munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
            }
            flock(fd, LOCK_UN); close(fd);
            return dd_setup(base, map_size, path, -1);
...
static DdHandle *dd_open_fd(int fd, char *errbuf) {
    ...
    if (((DdHeader *)base)->sealed) {
        DD_ERR("this DDSketch is frozen (read-only); open it with new_readonly");
 ...
 * file). */
static DdHandle *dd_open_readonly(const char *path, char *errbuf) {
    if (errbuf) errbuf[0] = '\0';
    int fd = open(path, O_RDONLY|O_NOFOLLOW|O_CLOEXEC);
    if (fd < 0) { DD_ERR("open %s: %s", path, strerror(errno)); return NULL; }
    struct stat st;
    ...
    if (base == MAP_FAILED) { DD_ERR("mmap %s: %s", path, strerror(errno)); return NULL; }
    if (!dd_validate_header((DdHeader *)base, (uint64_t)st.st_size)) {
        DD_ERR("%s: invalid DDSketch file", path); munmap(base, ms); return NULL;
    }
    if (!((DdHeader *)base)->sealed) {
        DD_ERR("%s is not frozen: call ->freeze on the producer before opening read-only", path);
        munmap(base, ms); return NULL;
    }
    DdHandle *h = dd_setup(base, ms, path, -1);   /* munmaps on OOM */
    if (!h) { DD_ERR("out of memory"); return NULL; }
    h->readonly = 1;
    return h;
}

/* Seal a sketch: make it permanently immutable so it can be shipped and opened
 * read-only.  Takes the write lock so no add/merge/clear is in flight, publishes
 * the seal, then flushes it (file/memfd-backed).  Afterwards every mutator
 * croaks and a read-write reopen is refused. */
static int dd_freeze(DdHandle *h) {
    dd_rwlock_wrlock(h);
    h->hdr->sealed = 1;
    dd_rwlock_wrunlock(h);
    if (h->path || h->backing_fd >= 0) return dd_msync(h);  /* durability for file/memfd-backed */
