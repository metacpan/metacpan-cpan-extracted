...
sub decrypt_test {
    for (@encryptions) {
        if (my $passwd = cisco_decrypt($_)) {
            if ($passwd ne "cisco") {
                print "Error: cisco_decrypt($_) returned $passwd ne cisco\n";
                return 1
            }
        }
    }
...
sub encrypt_test {
    if (my $passwd = cisco_encrypt('cisco')) {
        for (@encryptions) {
            if ($passwd eq $_) {
                return 0
            }
...
sub encrypt_test_arg {
    if ((my $passwd = cisco_encrypt('cisco',5)) ne $encryptions[5]) {
            printf "Error: cisco_encrypt('cisco',5) returned %s ne %s\n", $passwd, $encryptions[5];
            return 1
    }
    return 0
}
    ...
    if (my $passwd = cisco_encrypt('cisco','*')) {
        for (0..$#{$passwd}) {
            ...
            if ($passwd->[$_] ne $encryptions[$index]) {
                printf "Error: cisco_encrypt('cisco','*') returned %s ne %s\n", $passwd->[$_], $encryptions[$index];
                return 1
            }
        }
        return 0
