...
sub new {
    ...
    my $no_build = delete $args->{__no_BUILD__};
        ...
        # Attribute: attr
    do { my $value = exists( $args->{"attr"} ) ? $args->{"attr"} : do { my $method = $MyTest::TestClass::Scalar::__attr_DEFAULT__; $self->$method }; (!!1) or MyTest::Mite::croak "Type check failed in constructor: %s should be %s", "attr", "Any"; $self->{"attr"} = $value; }; 
...
sub _set_attr { (!!1) or MyTest::Mite::croak( "Type check failed in %s: value should be %s", "writer", "Any" ); $_[0]{"attr"} = $_[1]; $_[0]; }
