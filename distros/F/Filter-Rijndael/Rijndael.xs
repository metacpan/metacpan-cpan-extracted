...
#define HEADERSIZE	6
#define BLOCKSIZE	16
#define CHECKSUMSIZE	20
#define KEYSIZE		32
#define SHAALGORITHM	0
#define SHAMAXWRITESIZE	16384

unsigned char checksum[CHECKSUMSIZE];
UINT8 iv[BLOCKSIZE];
RIJNDAEL_context ctx;

static int ix2alg[] = {1,224,256,384,512,512224,512256};
...
Decrypt(SV *in_sv, SV *out_sv)
{
	...
	out_buffer = (unsigned char *) SvPVX(out_sv) ;
        ...
        /* call rijndael block_decrypt */
        block_decrypt( &ctx, in_buffer, size, out_buffer, iv );
...
static int ReadBlock(int idx, SV *sv, unsigned int size) {
    /* read *exactly* size bytes from the next filter */
    unsigned int i = size;
    while (1) {
        unsigned int n = FILTER_READ(idx, sv, i) ;
...
MODULE = Filter::Rijndael	PACKAGE = Filter::Rijndael
