...
$Shipment::Purolator::VERSION = '3.11';
use strict;
use warnings;
...
sub _build_services {
    ...
    if ($self->from_address && $self->to_address) {
        ...
        try {
            ...
            try {
                warn $response->get_ResponseInformation()
                  ->get_Errors()
                  ->get_Error()->[0]->get_Description
                  if $self->debug;
                $self->error($response->get_ResponseInformation()
                      ->get_Errors()
        ...
        $self->service(
            Shipment::Service->new(
                ...
                etd  => $response->get_ShipmentEstimates()->[0]
                  ->get_ShipmentEstimate()
                  ->get_EstimatedTransitDays()
                  ->get_value,
                ship_date => $ship_date,
                eta       => $eta,
