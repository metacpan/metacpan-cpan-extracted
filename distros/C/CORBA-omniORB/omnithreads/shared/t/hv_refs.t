...
omnithreads->create(sub{${$foo{foo}} = "test2";})->join();
...
ok(8, omnithreads::shared::_id($$gg) == omnithreads::shared::_id($$gg2),
       sprintf("Check we get the same thing (%x vs %x)",
       omnithreads::shared::_id($$gg),omnithreads::shared::_id($$gg2)));
...
ok(10, keys %foo == 0, "And make sure we realy have deleted the values");
{
    ...
    omnithreads->create(sub { $hash2{"bar2"} = "foo2"})->join();
    ok(12, $hash1{hash}->{bar2} eq "foo2", "Check hash references work");
    omnithreads->create(sub { my (%hash3); share(%hash3); $hash2{hash} = \%hash3; $hash3{"thread"} = "yes"})->join();
