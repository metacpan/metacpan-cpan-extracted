...
sub login {
    ...
    && $self->stash("session_authenticated") == 0 )
  {
    ...
    if ( $type eq "Basic" ) {
      ...
      my $user_o = Rex::Repositorio::Server::Docker::Helper::Auth->create(
        "Plain",
        {
          user_path => $user_dir,
        }
      ...
      if ( $user_o->login( $user, $pass ) ) {
        ...
        $self->res->headers->add( 'X-Docker-Token' => 'Token signature='
            . $token
            . ',repository="'
            . $repo_name
            . '",access=write' );
        $self->res->headers->add( 'WWW-Authenticate' => 'Token signature='
            . $token
            . ',repository="'
            . $repo_name
            . '",access=write' );
