...
All methods, including L</new>, C<AUTOLOAD>, and serialisation subroutines (C<FREEZE>, C<THAW>), operate exclusively on the object’s internal hash, which is unique to each instance. This ensures thread-safe and process-safe behaviour without shared resources:
...
Serialisation methods (C<FREEZE>, C<THAW>) copy the object’s hash, which is per-object and thread-safe, supporting L<CBOR::XS>, L<Sereal>, and L<Storable::Improved>:
