...
my @chars = split //, 'AaEFfOoCcDdeHhIiKkLlQqSsTtUvXxYyz?';
...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^bobbins' && $params->[1] eq '+i' ) {
    ...
    $poe_kernel->state('_delay_char', sub {
          my $char = $_[ARG0];
          $poe_kernel->post('client', 'send_to_server', { command => 'STATS', params => [ $char ], colonify => 0 } );
      },
    );
    $poe_kernel->post($sender, 'send_to_server', { command => 'STATS', params => [ $char ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'ERROR' ) {
    pass($cmd);
...
sub client_input2 {
  ...
  my $char = shift @chars;
  if (!$char) {
    $poe_kernel->state('client_input', \&client_input);
    $poe_kernel->post($sender, 'send_to_server', { command => 'QUIT', params => [ 'Connection reset by fear' ], colonify => 1 } );
    return;
  }
  $poe_kernel->delay('_delay_char',1,$char);
  #$poe_kernel->post($sender, 'send_to_server', { command => 'STATS', params => [ $char ], colonify => 0 } );
  return;
}
