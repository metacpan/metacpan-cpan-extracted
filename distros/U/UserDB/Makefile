# This Makefile is for the UserDB extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/UserDB.pm]
#     AUTHOR => [q[Patrick Lambert <dendory@live.ca>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[UserDB]
#     PREREQ_PM => { Digest::SHA=>q[5.85], DBI=>q[1.630] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/UserDB.pm]
...
MAKEMAKER   = C:/Perl64/lib/ExtUtils/MakeMaker.pm
MM_VERSION  = 6.84
MM_REVISION = 68400
...
TO_INST_PM = lib/UserDB.pm

PM_TO_BLIB = lib/UserDB.pm \
	blib\lib\UserDB.pm


# --- MakeMaker platform_constants section:
MM_Win32_VERSION = 6.84
...
UserDB.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"UserDB\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/UserDB.pm $(INST_MAN3DIR)\UserDB.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) "abstract: 'A simple users and groups management interface'" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "generated_by: 'ExtUtils::MakeMaker version 6.84, CPAN::Meta::Converter version 2.132830'" >> META_new.yml
	$(NOECHO) $(ECHO) "license: unknown" >> META_new.yml
	$(NOECHO) $(ECHO) meta-spec: >> META_new.yml
	$(NOECHO) $(ECHO) "  url: http://module-build.sourceforge.net/META-spec-v1.4.html" >> META_new.yml
	$(NOECHO) $(ECHO) "  version: 1.4" >> META_new.yml
	$(NOECHO) $(ECHO) "name: UserDB" >> META_new.yml
	$(NOECHO) $(ECHO) no_index: >> META_new.yml
	$(NOECHO) $(ECHO) "  directory:" >> META_new.yml
	$(NOECHO) $(ECHO) "    - t" >> META_new.yml
	$(NOECHO) $(ECHO) "    - inc" >> META_new.yml
	$(NOECHO) $(ECHO) requires: >> META_new.yml
	$(NOECHO) $(ECHO) "  DBI: 1.630" >> META_new.yml
	$(NOECHO) $(ECHO) "  Digest::SHA: 5.85" >> META_new.yml
	$(NOECHO) $(ECHO) "version: 1.00" >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) {{ > META_new.json
	$(NOECHO) $(ECHO) "   \"abstract\" : \"A simple users and groups management interface\"," >> META_new.json
	$(NOECHO) $(ECHO) "   \"author\" : [" >> META_new.json
	$(NOECHO) $(ECHO) "      \"Patrick Lambert ^<dendory^@live.ca^>\"" >> META_new.json
	$(NOECHO) $(ECHO) "   ]," >> META_new.json
	$(NOECHO) $(ECHO) "   \"dynamic_config\" : 1," >> META_new.json
	$(NOECHO) $(ECHO) "   \"generated_by\" : \"ExtUtils::MakeMaker version 6.84, CPAN::Meta::Converter version 2.132830\"," >> META_new.json
	...
	$(NOECHO) $(ECHO) "   }}," >> META_new.json
	$(NOECHO) $(ECHO) "   \"name\" : \"UserDB\"," >> META_new.json
	...
	$(NOECHO) $(ECHO) "         \"requires\" : {{" >> META_new.json
	$(NOECHO) $(ECHO) "            \"DBI\" : \"1.630\"," >> META_new.json
	$(NOECHO) $(ECHO) "            \"Digest::SHA\" : \"5.85\"" >> META_new.json
	$(NOECHO) $(ECHO) "         }}" >> META_new.json
	$(NOECHO) $(ECHO) "      }}" >> META_new.json
	$(NOECHO) $(ECHO) "   }}," >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) "    <ABSTRACT>A simple users and groups management interface</ABSTRACT>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>Patrick Lambert &lt;dendory@live.ca&gt;</AUTHOR>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"DBI::\" VERSION=\"1.63\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Digest::SHA\" VERSION=\"5.85\" />" >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e "pm_to_blib({{@ARGV}}, '$(INST_LIB)\auto', q[$(PM_FILTER)], '$(PERM_DIR)')" -- \
	  lib/UserDB.pm blib\lib\UserDB.pm 
