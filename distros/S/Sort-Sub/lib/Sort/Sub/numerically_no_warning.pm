...
1;
# ABSTRACT: Sort numerically (without warning when comparing non-numbers)
...
Sort::Sub::numerically_no_warning - Sort numerically (without warning when comparing non-numbers)

=head1 VERSION

This document describes version 0.121 of Sort::Sub::numerically_no_warning (from Perl distribution Sort-Sub), released on 2024-07-17.

=head1 SYNOPSIS

Generate sorter (accessed as variable) via L<Sort::Sub> import:

 use Sort::Sub '$numerically_no_warning'; # use '$numerically_no_warning<i>' for case-insensitive sorting, '$numerically_no_warning<r>' for reverse sorting
 my @sorted = sort $numerically_no_warning ('item', ...);

Generate sorter (accessed as subroutine):

 use Sort::Sub 'numerically_no_warning<ir>';
 my @sorted = sort {numerically_no_warning} ('item', ...);

Generate directly without Sort::Sub:

 use Sort::Sub::numerically_no_warning;
 my $sorter = Sort::Sub::numerically_no_warning::gen_sorter(
...
Use in shell/CLI with L<sortsub> (from L<App::sortsub>):
 ...
 % some-cmd | sortsub numerically_no_warning
 % some-cmd | sortsub numerically_no_warning --ignore-case -r
