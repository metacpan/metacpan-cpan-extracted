...
my $module = 'App::GUI::Harmonograph::Color::Constant';

eval "use $module qw/:all/";
is( not($@), 1, 'could load the module');

my @names = App::GUI::Harmonograph::Color::Constant::all_names();
is( @names > 700, 1, 'get a large list of names, all_names seems to working');

my $taken          = \&App::GUI::Harmonograph::Color::Constant::name_taken;
my $add_rgb        = \&App::GUI::Harmonograph::Color::Constant::add_rgb;
my $add_hsl        = \&App::GUI::Harmonograph::Color::Constant::add_hsl;
my $get_name_range = \&App::GUI::Harmonograph::Color::Constant::names_in_hsl_range;
...
is( name_from_rgb( 1, 2, 3 ), 'one',            'retrieve added color' );
...
is( name_from_rgb( 255 ,255, 255 ), 'white',      'could get a color def');        
is( scalar name_from_rgb( 255, 215,   0 ), 'gold','selects shorter name: gold instead of gold1');
is( scalar name_from_rgb( [255, 215,   0]),'gold','array ref arg format works too');
is( scalar name_from_rgb( 255,   0,   0 ), 'red', 'selects shorter name red instead of inserted lucky');
is( name_from_hsl(  0, 100,  50 ), 'red',         'found red by hsl');
is( name_from_hsl( 14,  10,  50 ), 'blob',        'found inserted color by hsl');

my @rgb = rgb_from_name('white');
my @hsl = hsl_from_name('white');
...
@rgb = rgb_from_name('one');
@hsl = hsl_from_name('one');
