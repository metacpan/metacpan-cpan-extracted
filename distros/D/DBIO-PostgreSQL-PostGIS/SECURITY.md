This is the Security Policy for the Perl DBIO-PostgreSQL-PostGIS distribution.
...
[DBIO-PostgreSQL-PostGIS on CPAN](https://metacpan.org/dist/DBIO-PostgreSQL-PostGIS) or the main branch of the
[DBIO-PostgreSQL-PostGIS git repository](https://codeberg.org/dbio/dbio-postgresql-postgis).
...
DBIO-PostgreSQL-PostGIS is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO-PostgreSQL-PostGIS are covered by this policy.
...
they affect DBIO-PostgreSQL-PostGIS, or DBIO-PostgreSQL-PostGIS can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO-PostgreSQL-PostGIS, or plugins to it that are not included with the DBIO-PostgreSQL-PostGIS
distribution) are not covered by this policy.

## Supported Versions of DBIO-PostgreSQL-PostGIS

Only the latest release of DBIO-PostgreSQL-PostGIS will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO-PostgreSQL-PostGIS will run on older versions of Perl. If a
...
that are required for DBIO-PostgreSQL-PostGIS to work. However, some of these
