...
This document describes version 0.009 of Sah::Schema::hohoms (from Perl distribution Sah-SchemaBundle-Collection), released on 2024-06-13.
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("hohoms", {return_type=>'str_errmsg'});
 ...
 $data = {k=>undef};
 my $errmsg = $validator->($data); # => "\@[k]: Required but not specified"

Often a schema has coercion rule or default value rules, so after validation the
validated value will be different from the original. To return the validated
(set-as-default, coerced, prefiltered) value:

 my $validator = gen_validator("hohoms", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = {k=>undef};
 my $res = $validator->($data); # => ["\@[k]: Required but not specified",{k=>undef}]
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('hohoms*', name=>'Hohoms')
     );
 }

 use My::Types qw(Hohoms);
 Hohoms->assert_valid($data);
