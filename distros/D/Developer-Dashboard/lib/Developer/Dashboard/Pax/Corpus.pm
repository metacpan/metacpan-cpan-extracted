...
sub run {
    ...
    for my $case (@{ $manifest->{cases} // [] }) {
        my $capture = Developer::Dashboard::Pax::Capture->new(mode => $case->{mode} // 'live')->capture($case->{path});
        my $pax_manifest = Developer::Dashboard::Pax::Manifest->new(capture => $capture)->to_hash;
...
Developer::Dashboard::Pax::Corpus - corpus runner for grouped validation workloads

=head1 SYNOPSIS

  use Developer::Dashboard::Pax::Corpus;

  my $obj = Developer::Dashboard::Pax::Corpus->new(...);
  my $result = $obj->run(...);
...
Example 1:
  ...
  perl -Ilib -MDeveloper::Dashboard::Pax::Corpus -e 1
