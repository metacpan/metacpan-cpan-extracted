package TableDataRole::Source::SQLite;
...
around new => sub {
    ...
    my $dsn = "dbi:SQLite:dbname=".delete($args{filename});
    ...
    if (defined $sth) {
        ...
        die "You specify 'query' or 'table', but you don't specify ".
            "dbh/filename, so I cannot create a statement handle"
            unless $dbh;
        if (defined $query) {
        } elsif (defined $table) {
...
1;
# ABSTRACT: Role to access table data from SQLite database table/query
...
TableDataRole::Source::SQLite - Role to access table data from SQLite database table/query

=head1 VERSION

This document describes version 0.001 of TableDataRole::Source::SQLite (from Perl distribution TableDataRole-Source-SQLite), released on 2024-05-14.
...
One of L</filename>, L</dbh>, or L</sth>, is required.
