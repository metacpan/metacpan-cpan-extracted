...
sub run_test {
  ...
  dies_ok { $results = $badthumb->get_thumbnail('http://digg.com')} 'Bad Connection';
  ...
  dies_ok { $results = $thumb->get_thumbnail()} 'No Url';

  my @domains = ('perl.com', 'http://perlmonks.org');
  lives_ok { $results = $thumb->get_thumbnail(\@domains)} 'Amazon connection';
  is(scalar @{ $results }, 2);
  like  (${ $results }[0], qr/http/);
  like  (${ $results }[1], qr/http/);
  return if($args);

  lives_ok { $results = $thumb->get_thumbnail('yahoo.com')} 'Amazon connection 2';
  is(scalar @{ $results }, 1);
  like  (${ $results }[0], qr/http/);

  lives_ok { $results = $thumb->get_thumbnail('http://stonehenge.com')} 'Amazon connnection 3';
  is(scalar @{ $results }, 1);
  like  (${ $results }[0], qr/http/);
  ...
  lives_ok { $results = $thumb->get_thumbnail('http://perlmonks.org')} 'Amazon connection 4';
  ...
  lives_ok { $results = $thumb->get_thumbnail(\%domains)} 'Amazon connection 5';
