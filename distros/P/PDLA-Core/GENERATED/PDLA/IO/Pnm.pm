...
@EXPORT_OK  = qw(  rpnm wpnm PDLA::PP pnminraw PDLA::PP pnminascii PDLA::PP pnmout );
...
use PDLA::Core qw/howbig convert/;
use PDLA::Types;
use PDLA::Basic;  # for max/min
use PDLA::IO::Misc;
use Carp;
use File::Temp qw( tempfile );

# return the upper limit of data values an integer PDLA data type
# can hold
sub dmax {
    my $type = shift;
    my $sz = 8*howbig($type);
    $sz-- if ($type == $PDLA_S || $type == $PDLA_L);  # signed types
    return ((1 << $sz)-1);
}
...
always PDLA_B). rpnm tries to read chopped files by zero padding the
...
sub PDLA::rpnm {
    ...
    # close(STDERR);
    open(STDERR, ">&PDLA::IO::Pnm::SAVEERR");
    ...
    do {
TYPES:	{  my $pdlt;
	   foreach $pdlt ($PDLA_B,$PDLA_US,$PDLA_L){
	     if ($dims[2] <= dmax($pdlt))
	       { $type = $pdlt;
	         last TYPES;
	       }
    ...
    unless ( isbigendian() ) {
       if ($israw ) {
          $pdl->bswap2 if $type==$PDLA_US or $pdl->type == ushort;
          $pdl->bswap4 if $type==$PDLA_L;  # not likely, but supported anyway
       }
    }
    return $pdl;
}
...
sub PDLA::wpnm {
	...
	if ($pdl->get_datatype == $PDLA_F) || ($pdl->get_datatype == $PDLA_D);
    barf "wpnm: expecting prescaled data"
	if (($pdl->get_datatype != $PDLA_B) || ($pdl->get_datatype != $PDLA_US)) &&
	    ($pdl->min < 0);

    # check for raw format
    $israw = 1 if (($pdl->get_datatype == $PDLA_B) || ($pdl->get_datatype == $PDLA_US) || ($type =~ /PBM/));
    ...
    pnmout($out,$israw,$type eq "PBM",'PDLA::IO::Pnm::PNM');
