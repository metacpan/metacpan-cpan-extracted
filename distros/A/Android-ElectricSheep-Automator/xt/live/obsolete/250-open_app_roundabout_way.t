...
$res = $mother->open_app_roundabout_way({'name' => 'Calendar'});
ok(defined($res), 'Android::ElectricSheep::Automator->open_app_roundabout_way()'." : called and got good result.") or BAIL_OUT;
is(ref($res), 'HASH', 'Android::ElectricSheep::Automator->open_app_roundabout_way()'." : called and got good result which is a HASH.") or BAIL_OUT("no it is '".ref($res)."'");
for my $k ('name', 'bounds'){
	ok(exists($res->{$k}), 'Android::ElectricSheep::Automator->open_app_roundabout_way()'." : called and got good result which is a HASH and contains key '$k'.") or BAIL_OUT("no it has these keys: '".join("', '", keys %$res)."'");
	# name can be undef if position was specified
	#ok(defined($res->{$k}), 'Android::ElectricSheep::Automator->open_app_roundabout_way()'." : called and got good result which is a HASH and contains key '$k' which is defined.") or BAIL_OUT;
}
