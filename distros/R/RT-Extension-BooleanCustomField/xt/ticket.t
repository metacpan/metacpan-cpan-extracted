...
(my $cf_checkbox) = grep {$_->{name} =~ /Object-RT::Ticket--CustomField-\d+-Value/} @inputs;
ok(!$cf_checkbox, 'No checkbox without CF Boolean');
...
($cf_checkbox) = grep {$_->{name} eq "Object-RT::Ticket--CustomField-$cf_id-Value"} @inputs;
ok($cf_checkbox, 'Checkbox with checked CF Boolean');

is($cf_checkbox->value, undef, 'Checkbox is unchecked with checked CF Boolean');
$m->tick("Object-RT::Ticket--CustomField-$cf_id-Value", '1');
is($cf_checkbox->value, '1', 'Checkbox is checked with checked CF Boolean');
...
ok($ticket->Load($ticket_id), 'Load ticket');

is($ticket->FirstCustomFieldValue('Active'), 1, 'Current value is true');
$ticket->DeleteCustomFieldValue(Field => 'Active', Value => 1);
is($ticket->FirstCustomFieldValue('Active'), undef, 'Current value is false');

$m->get_ok($m->rt_base_url . "Ticket/Display.html?id=$ticket_id", 'Asset display');
if (RT::Handle::cmp_version($RT::VERSION, '6.0.0') >= 0) {
