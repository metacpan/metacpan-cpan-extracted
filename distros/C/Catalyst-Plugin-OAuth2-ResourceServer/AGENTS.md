...
`Catalyst::Plugin::OAuth2::ResourceServer` is an MCP-profile OAuth 2.1 Resource
Server plugin for Catalyst. The engine is in
`lib/Catalyst/Plugin/OAuth2/ResourceServer/` (the `Server` and an `Error` class);
tests are under `t/`.
