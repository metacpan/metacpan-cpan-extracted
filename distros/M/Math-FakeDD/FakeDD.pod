...
=head1 OTHER FUNCTIONS (not exported and might later be removed or altered)

  $obj = mpfr2dd($mpfr);

   Return the value held by the 2098-bit precision Math::MPFR object $mpfr
   as a Math::FakeDD object $obj.
   This conversion can lose precision - ie, can be subject to rounding.
