...
/, 'replica section created in config file after init'
    ...
    $output   = run_command(qw(search --type Bug --regex .));
    $expected = qr/new/;
...
/, 'publish-url variable created correctly in config'
    ...
    ok(
        run_command( qw(publish --to), $new_published ),
...
EOF
    ...
    ok(
        run_command( qw(publish --to), $bogus_name ),
        'publish to bogus name',
    );
    ok(
        !-f Prophet::Util->catfile( $new_published, 'config' )
...
as_bob {
    ok( run_command( 'clone', '--from', "file://$path" ), 'clone as bob', );
    my $config_contents = Prophet::Util->slurp( $ENV{PROPHET_APP_CONFIG} );
    $config_contents =~ s!\\\\!\\!g;
    my $replica_uuid = replica_uuid();
    like(
...
as_alice {
    ...
    ok(
        run_command( qw(publish --to), $alice_published ),
...
is(
    database_uuid_for('alice'),
    database_uuid_for('charlie'),
    "pull propagated the database uuid properly"
);
isnt(
    replica_uuid_for('alice'),
    replica_uuid_for('charlie'),
    "pull created a new replica uuid"
);
