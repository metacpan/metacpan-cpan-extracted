...
sub HList2 {
    ...
    # Get real hlist (non-scrolled) for creating embedded windows
    my $realHlist = $h->Subwidget('scrolled');
	...
	for my $col (0 .. 3) {
	    ...
	    my $btn;
	    my $style = $realHlist->ItemStyle($style_type);
	    if ($style_type eq 'window') {
		$style->configure(-pady => 0, -padx => 0, -anchor => "center");
		if ($window eq 'Button') {
		    $btn = $realHlist->Button
		      (-text => 'Click me!',
		       -command => sub {
