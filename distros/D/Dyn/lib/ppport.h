...
----------------------------------------------------------------------
    ...
    lib/ppport.h -- Perl/Pollution/Portability Version 3.63
    ...
    Use 'perldoc lib/ppport.h' to view the documentation below.
...
=head1 SYNOPSIS
  ...
  perl lib/ppport.h [options] [source files]
...
This version of F<lib/ppport.h> is designed to support operation with Perl
...
Display the version of F<lib/ppport.h>.
...
Tell F<lib/ppport.h> to check for compatibility with the given
Perl version. The default is to check for compatibility with Perl
version 5.003_07. You can use this option to reduce the output
of F<lib/ppport.h> if you intend to be backward compatible only
down to a certain Perl version.

=head2 --cplusplus

Usually, F<lib/ppport.h> will detect C++ style comments and
replace them with C style comments for portability reasons.
Using this option instructs F<lib/ppport.h> to leave C++
comments untouched.
...
Strip all script and documentation functionality from F<lib/ppport.h>.
This reduces the size of F<lib/ppport.h> dramatically and may be useful
if you want to include F<lib/ppport.h> in smaller modules without
increasing their distribution size too much.

The stripped F<lib/ppport.h> will have a C<--unstrip> option that allows
...
F<lib/ppport.h>. Also lists if it must be explicitly requested,
if it has dependencies, and if there are hints or warnings for it.

=head2 --list-unsupported

Lists the API elements that are known not to be FULLY supported by F<lib/ppport.h>,
...
version of Perl that F<lib/ppport.h> supports at all.
...
earlier Perl releases. Use
    ...
    perl lib/ppport.h --list-provided

to see which API elements are provided by lib/ppport.h.
...
also deprecated. You can safely use the supported API, as F<lib/ppport.h>
will provide wrappers for older Perl versions.

=item *

Although the purpose of F<lib/ppport.h> is to keep you from having to concern
...
more C<#define>s in your source code before the inclusion of F<lib/ppport.h>.
...
macro. Just C<#define> the macro before including C<lib/ppport.h>:

    #define DPPP_NAMESPACE MyOwnNamespace_
    #include "lib/ppport.h"
...
F<lib/ppport.h> on your source code. See the next section for
details.

=head1 EXAMPLES

To verify whether F<lib/ppport.h> is needed for your module, whether you
should make any changes to your code, and whether any special defines
should be used, F<lib/ppport.h> can be run as a Perl script to check your
...
use something like this:
    ...
    perl lib/ppport.h --compat-version=5.6.0 --cplusplus --patch=test.diff

If you only want your code to be scanned without any suggestions
for changes, use:

    perl lib/ppport.h --nochanges

You can specify a different C<diff> program or options, using
the C<--diff> option:

    perl lib/ppport.h --diff='diff -C 10'

This would output context diffs with 10 lines of context.

If you want to create patched copies of your files instead, use:

    perl lib/ppport.h --copy=.new

To display portability information for the C<newSVpvn> function,
use:

    perl lib/ppport.h --api-info=newSVpvn

Since the argument to C<--api-info> can be a regular expression,
you can use

    perl lib/ppport.h --api-info=/_nomg$/

to display portability information for all C<_nomg> functions or

    perl lib/ppport.h --api-info=/./
...
If this version of F<lib/ppport.h> is causing failure during
the compilation of this module, please check if newer versions
of either this module or C<Devel::PPPort> are available on CPAN
before sending a bug report.

If F<lib/ppport.h> was generated using the latest version of
...
# These are tools that must be included in lib/ppport.h.  It doesn't work if given
# a .pl suffix.
#
...
unless $base; if ($todo) { if (! $base || $todo < $base) { my $additionally = ""; $additionally .= "
additionally" if $base; print "$ppport$additionally provides support at least back to perl-",
                    format_version($todo),
                    ".\n";
            }
        }
            ...
            if (exists $depends{$f}) {
                my $max = 0;
                for (@{$depends{$f}}) {
                    $max = int_parse_version($API{$_}{todo}) if $API{$_}{todo} && $API{$_}{todo} >
$max; # XXX What to assume unspecified values are?  This effectively makes them MIN_PERL
                }
                $todo = $max if $max;
            }
...
#endif
#if !defined(PERL_VERSION) && !defined(PERL_VERSION_MAJOR) &&                                      \
...
#define D_PPP_SUBVERSION 201
     ...
     && ((defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L) || defined(_STDC_C99) ||       \
         defined(__c99)))
#define D_PPP_STRINGIFY(x) #x
#define D_PPP_deprecate(xyz)                                                                       \
    _Pragma(D_PPP_STRINGIFY(GCC warning(D_PPP_STRINGIFY(xyz) " is deprecated")))
...
#define PERL_BCDVERSION D_PPP_JNP_TO_BCD(D_PPP_MAJOR, D_PPP_MINOR, D_PPP_PATCH)
...
#define PERL_VERSION_EQ(j, n, p)                                                                   \
    (((p) == '*') ? ((j) == D_PPP_VERSION_MAJOR && (n) == D_PPP_VERSION_MINOR)                     \
...
#define PERL_VERSION_LT(j, n, p) /* p=='*' means _LT(j,n,0) */                                     \
    (PERL_BCDVERSION < D_PPP_JNP_TO_BCD((j), (n), (((p) == '*') ? 0 : (p))))
#endif
...
#define PERL_VERSION_LE(j, n, p) /* p=='*' means _LT(j,n+1,0) */                                   \
    (PERL_BCDVERSION <                                                                             \
     D_PPP_JNP_TO_BCD((j), (((p) == '*') ? ((n) + 1) : (n)), (((p) == '*') ? 0 : (p))))
#endif
 ...
 * 5.005 perls, you can use aTHXR (supplied by lib/ppport.h), which will
...
#define D_PPP_my_PL_parser_var(var)                                                                \
    ((PL_parser                                                                                    \
          ? PL_parser                                                                              \
          : (croak("panic: PL_parser == NULL in %s:%d", __FILE__, __LINE__), (yy_parser *)NULL))   \
         ->var)
#else
...
#endif
#define D_PPP_my_PL_parser_var(var)                                                                \
    ((PL_parser ? PL_parser : (D_PPP_parser_dummy_warning(var) & DPPP_(dummy_PL_parser)))->var)
...
/* PL_expect, PL_copline, PL_rsfp, PL_rsfp_filters, PL_linestr, PL_bufptr, PL_bufend, PL_lex_state,
 * PL_lex_stuff, PL_tokenbuf depends on PL_parser */
/* Warning: PL_expect, PL_copline, PL_rsfp, PL_rsfp_filters, PL_linestr, PL_bufptr, PL_bufend,
 * PL_lex_state, PL_lex_stuff, PL_tokenbuf Do not use this variable unless you know exactly what
 * you're doing. It is internal to the perl parser and may change or even be removed in the future.
 * As of perl 5.9.5, you have to check for (PL_parser != NULL) for this variable to have any effect.
...
#define D_PPP_PL_copline PL_copline
    ...
    if (stash) PL_curstash = PL_curcop->cop_stash = stash;
...
#define D_PPP_IS_GENERIC_UTF8_SAFE(s, e, macro)                                                    \
    (((e) - (s)) <= 0              ? 0                                                             \
     : UTF8_IS_INVARIANT((s)[0])   ? is##macro##_L1((s)[0])                                        \
     : (((e) - (s)) < UTF8SKIP(s)) ? 0                                                             \
     : UTF8_IS_DOWNGRADEABLE_START(                                                                \
           (s)[0]) /* The cast in the line below is only to silence warnings */                    \
         ? is##macro##_L1((WIDEST_UTYPE)LATIN1_TO_NATIVE(                                          \
               UTF8_ACCUMULATE(NATIVE_UTF8_TO_I8((s)[0]) & UTF_START_MASK(2), (s)[1])))            \
         : is##macro##_utf8(s))
...
#define D_PPP_IS_GENERIC_LC_UTF8_SAFE(s, e, macro)                                                 \
    (((e) - (s)) <= 0              ? 0                                                             \
     : UTF8_IS_INVARIANT((s)[0])   ? is##macro##_LC((s)[0])                                        \
     : (((e) - (s)) < UTF8SKIP(s)) ? 0                                                             \
     : UTF8_IS_DOWNGRADEABLE_START(                                                                \
           (s)[0]) /* The cast in the line below is only to silence warnings */                    \
...
#ifndef SvRX
#define SvRX(rv)                                                                                   \
    (SvROK((rv)) ? (SvMAGICAL(SvRV((rv))) ? (mg_find(SvRV((rv)), PERL_MAGIC_qr)                    \
                                                 ? mg_find(SvRV((rv)), PERL_MAGIC_qr)->mg_obj      \
                                                 : NULL)                                           \
                                          : NULL)                                                  \
                 : NULL)
#endif
...
/* Hint: isPSXSPC, isPSXSPC_A, isPSXSPC_L1, isPSXSPC_utf8_safe
    ...
    lib/ppport.h SPACE version does match VT in all perl releases.  Since VT's are
    extremely rarely found in real-life files, this difference effectively
    doesn't matter */

/* Hint: isSPACE, isSPACE_A, isSPACE_L1, isSPACE_utf8_safe
    Until Perl 5.18, this did not match the vertical tab (VT).  The lib/ppport.h
...
#define isCNTRL(c)                                                                                 \
    ((c) == '\0' || (c) == '\a' || (c) == '\b' || (c) == '\f' || (c) == '\n' || (c) == '\r' ||     \
     (c) == '\t' || (c) == '\v' || ((c) <= 3 && (c) >= 1) /* SOH, STX, ETX */                      \
     || (c) == 7                                          /* U+7F DEL */                           \
     || ((c) <= 0x13 && (c) >= 0x0E) /* SO, SI */         /* DLE, DC[1-3] */                       \
     || (c) == 0x18                                       /* U+18 CAN */                           \
     || (c) == 0x19                                       /* U+19 EOM */                           \
...
#ifndef isCNTRL_L1
#define isCNTRL_L1(c) ((WIDEST_UTYPE)(c) < ' ' || (WIDEST_UTYPE)(c) == D_PPP_OUTLIER_CONTROL)
#endif
...
#ifndef isCNTRL_L1
#define isCNTRL_L1(c) ((WIDEST_UTYPE)(c) < ' ' || inRANGE((c), 0x7F, 0x9F))
#endif
...
#ifndef isBLANK_L1
#define isBLANK_L1(c) (isBLANK(c) || (FITS_IN_8_BITS(c) && NATIVE_TO_LATIN1((U8)c) == 0xA0))
#endif
...
#ifndef isGRAPH_L1
#define isGRAPH_L1(c) (isPRINT_L1(c) && (c) != ' ' && NATIVE_TO_LATIN1((U8)c) != 0xA0)
#endif
...
#ifndef isLOWER_L1
#define isLOWER_L1(c)                                                                              \
    (isLOWER(c) || (FITS_IN_8_BITS(c) &&                                                           \
                    ((NATIVE_TO_LATIN1((U8)c) >= 0xDF && NATIVE_TO_LATIN1((U8)c) != 0xF7) ||       \
                     NATIVE_TO_LATIN1((U8)c) == 0xAA || NATIVE_TO_LATIN1((U8)c) == 0xBA ||         \
...
#define isPUNCT_L1(c)                                                                              \
    (isPUNCT(c) ||                                                                                 \
     (FITS_IN_8_BITS(c) && (NATIVE_TO_LATIN1((U8)c) == 0xA1 || NATIVE_TO_LATIN1((U8)c) == 0xA7 ||  \
                            NATIVE_TO_LATIN1((U8)c) == 0xAB || NATIVE_TO_LATIN1((U8)c) == 0xB6 ||  \
                            NATIVE_TO_LATIN1((U8)c) == 0xB7 || NATIVE_TO_LATIN1((U8)c) == 0xBB ||  \
...
#define isSPACE_L1(c)                                                                              \
    (isSPACE(c) ||                                                                                 \
     (FITS_IN_8_BITS(c) && (NATIVE_TO_LATIN1((U8)c) == 0x85 || NATIVE_TO_LATIN1((U8)c) == 0xA0)))
#endif

#ifndef isUPPER_L1
#define isUPPER_L1(c)                                                                              \
    (isUPPER(c) ||                                                                                 \
     (FITS_IN_8_BITS(c) && (NATIVE_TO_LATIN1((U8)c) >= 0xC0 && NATIVE_TO_LATIN1((U8)c) <= 0xDE &&  \
...
#ifndef isXDIGIT
#define isXDIGIT(c) (isDIGIT(c) || inRANGE((c), 'a', 'f') || inRANGE((c), 'A', 'F'))
#endif
...
#define D_PPP_is_ctype(upper, lower, c)                                                            \
    (FITS_IN_8_BITS(c) ? is##upper##_L1(c) : is##upper##_uni((UV)(c))) /* _uni is old synonym */
#else
#define D_PPP_is_ctype(upper, lower, c)                                                            \
    (FITS_IN_8_BITS(c) ? is##upper##_L1(c) : is_uni_##lower((UV)(c))) /* is_uni_ is even older */
#endif
...
#else
#ifndef isBLANK_uvchr
#define isBLANK_uvchr(c)                                                                           \
    (FITS_IN_8_BITS(c)                                                                             \
         ? isBLANK_L1(c)                                                                           \
         : ((UV)(c) == 0x1680                                        /* Unicode 3.0 */             \
            || inRANGE((UV)(c), 0x2000, 0x200A) || (UV)(c) == 0x202F /* Unicode 3.0 */             \
...
#define isWORDCHAR_uvchr(c) (FITS_IN_8_BITS(c) ? isWORDCHAR_L1(c) : isALPHANUMERIC_uvchr(c))
#endif
...
#define isALPHANUMERIC_utf8_safe(s, e) D_PPP_IS_GENERIC_UTF8_SAFE(s, e, ALPHANUMERIC)
#endif

#else
#ifndef isALPHANUMERIC_utf8_safe
#define isALPHANUMERIC_utf8_safe(s, e) (isALPHA_utf8_safe(s, e) || isDIGIT_utf8_safe(s, e))
#endif
    ...
    ((LIKELY((e) > (s))) ? /* Machine generated */                                                 \
         ((0x09 == ((const U8 *)s)[0] || 0x20 == ((const U8 *)s)[0]) ? 1                           \
          : (LIKELY(((e) - (s)) >= UTF8SKIP(s)))                                                   \
              ? ((0xC2 == ((const U8 *)s)[0]) ? ((0xA0 == ((const U8 *)s)[1]) ? 2 : 0)             \
                 : (0xE1 == ((const U8 *)s)[0])                                                    \
                     ? (((0x9A == ((const U8 *)s)[1]) && (0x80 == ((const U8 *)s)[2])) ? 3 : 0)    \
                 : (0xE2 == ((const U8 *)s)[0])                                                    \
                     ? ((0x80 == ((const U8 *)s)[1])                                               \
                            ? ((inRANGE(((const U8 *)s)[2], 0x80, 0x8A) ||                         \
                                0xAF == ((const U8 *)s)[2])                                        \
                                   ? 3                                                             \
                                   : 0)                                                            \
                        : ((0x81 == ((const U8 *)s)[1]) && (0x9F == ((const U8 *)s)[2])) ? 3       \
                                                                                         : 0)      \
                 : (((0xE3 == ((const U8 *)s)[0]) && (0x80 == ((const U8 *)s)[1])) &&              \
                    (0x80 == ((const U8 *)s)[2]))                                                  \
                     ? 3                                                                           \
                     : 0)                                                                          \
              : 0)                                                                                 \
...
#define isBLANK_utf8_safe(s, e)                                                                    \
    ((LIKELY((e) > (s)))                                                                           \
         ? ((0x05 == ((const U8 *)s)[0] || 0x40 == ((const U8 *)s)[0]) ? 1                         \
            : (LIKELY(((e) - (s)) >= UTF8SKIP(s)))                                                 \
                ? ((0x80 == ((const U8 *)s)[0]) ? ((0x41 == ((const U8 *)s)[1]) ? 2 : 0)           \
                   : (0xBC == ((const U8 *)s)[0])                                                  \
                       ? (((0x63 == ((const U8 *)s)[1]) && (0x41 == ((const U8 *)s)[2])) ? 3 : 0)  \
                   : (0xCA == ((const U8 *)s)[0])                                                  \
                       ? ((0x41 == ((const U8 *)s)[1])                                             \
                              ? ((inRANGE(((const U8 *)s)[2], 0x41, 0x4A) ||                       \
                                  0x51 == ((const U8 *)s)[2])                                      \
                                     ? 3                                                           \
                                     : 0)                                                          \
                          : (0x42 == ((const U8 *)s)[1]) ? ((0x56 == ((const U8 *)s)[2]) ? 3 : 0)  \
                          : ((0x43 == ((const U8 *)s)[1]) && (0x73 == ((const U8 *)s)[2])) ? 3     \
                                                                                           : 0)    \
                   : (((0xCE == ((const U8 *)s)[0]) && (0x41 == ((const U8 *)s)[1])) &&            \
                      (0x41 == ((const U8 *)s)[2]))                                                \
                       ? 3                                                                         \
                       : 0)                                                                        \
                : 0)                                                                               \
...
#define isBLANK_utf8_safe(s, e)                                                                    \
    ((LIKELY((e) > (s)))                                                                           \
         ? ((0x05 == ((const U8 *)s)[0] || 0x40 == ((const U8 *)s)[0]) ? 1                         \
            : (LIKELY(((e) - (s)) >= UTF8SKIP(s)))                                                 \
                ? ((0x78 == ((const U8 *)s)[0]) ? ((0x41 == ((const U8 *)s)[1]) ? 2 : 0)           \
                   : (0xBD == ((const U8 *)s)[0])                                                  \
                       ...
                       ? ((0x41 == ((const U8 *)s)[1])                                             \
                              ? ((inRANGE(((const U8 *)s)[2], 0x41, 0x4A) ||                       \
                                  0x51 == ((const U8 *)s)[2])                                      \
                                     ...
                                     : 0)                                                          \
                          : (0x42 == ((const U8 *)s)[1]) ? ((0x56 == ((const U8 *)s)[2]) ? 3 : 0)  \
                          : ((0x43 == ((const U8 *)s)[1]) && (0x72 == ((const U8 *)s)[2])) ? 3     \
                                                                                           : 0)    \
                   : (((0xCE == ((const U8 *)s)[0]) && (0x41 == ((const U8 *)s)[1])) &&            \
                      (0x41 == ((const U8 *)s)[2]))                                                \
                       ? 3                                                                         \
...
#else
#ifndef isWORDCHAR_utf8_safe
#define isWORDCHAR_utf8_safe(s, e) (isALPHANUMERIC_utf8_safe(s, e) || (*(s)) == '_')
#endif
...
#define isXDIGIT_utf8_safe(s, e)                                                                   \
    ((LIKELY((e) > (s)))                                                                           \
         ? ((inRANGE(((const U8 *)s)[0], 0x30, 0x39) || inRANGE(((const U8 *)s)[0], 0x41, 0x46) || \
             inRANGE(((const U8 *)s)[0], 0x61, 0x66))                                              \
                ? 1                                                                                \
            : ((LIKELY(((e) - (s)) >= UTF8SKIP(s))) && (0xEF == ((const U8 *)s)[0]))               \
                ? ((0xBC == ((const U8 *)s)[1]) ? ((inRANGE(((const U8 *)s)[2], 0x90, 0x99) ||     \
                                                    inRANGE(((const U8 *)s)[2], 0xA1, 0xA6))       \
                                                       ? 3                                         \
                                                       : 0)                                        \
                   : ((0xBD == ((const U8 *)s)[1]) && (inRANGE(((const U8 *)s)[2], 0x81, 0x86)))   \
                       ? 3                                                                         \
                       : 0)                                                                        \
                : 0)                                                                               \
         : 0)
...
#define isXDIGIT_utf8_safe(s, e)                                                                   \
    ((LIKELY((e) > (s)))                                                                           \
         ? ((inRANGE(((const U8 *)s)[0], 0x81, 0x86) || inRANGE(((const U8 *)s)[0], 0xC1, 0xC6) || \
             inRANGE(((const U8 *)s)[0], 0xF0, 0xF9))                                              \
                ? 1                                                                                \
            : (((LIKELY(((e) - (s)) >= UTF8SKIP(s))) && (0xDD == ((const U8 *)s)[0])) &&           \
               (0x73 == ((const U8 *)s)[1]))                                                       \
                ? ((0x67 == ((const U8 *)s)[2]) ? ((inRANGE(((const U8 *)s)[3], 0x57, 0x59) ||     \
                                                    inRANGE(((const U8 *)s)[3], 0x62, 0x68))       \
                                                       ? 4                                         \
                                                       : 0)                                        \
                   : ((inRANGE(((const U8 *)s)[2], 0x68, 0x69)) &&                                 \
                      (inRANGE(((const U8 *)s)[3], 0x42, 0x47)))                                   \
                       ? 4                                                                         \
                       : 0)                                                                        \
                : 0)                                                                               \
         : 0)
    ...
    ((LIKELY((e) > (s)))                                                                           \
         ? ((inRANGE(((const U8 *)s)[0], 0x81, 0x86) || inRANGE(((const U8 *)s)[0], 0xC1, 0xC6) || \
            ...
            : (((LIKELY(((e) - (s)) >= UTF8SKIP(s))) && (0xDD == ((const U8 *)s)[0])) &&           \
               (0x72 == ((const U8 *)s)[1]))                                                       \
                ? ((0x66 == ((const U8 *)s)[2])                                                    \
                       ? ((inRANGE(((const U8 *)s)[3], 0x57, 0x59) ||                              \
                           0x5F == ((const U8 *)s)[3] || inRANGE(((const U8 *)s)[3], 0x62, 0x67))  \
                              ? 4                                                                  \
                              : 0)                                                                 \
                   : ((inRANGE(((const U8 *)s)[2], 0x67, 0x68)) &&                                 \
                      (inRANGE(((const U8 *)s)[3], 0x42, 0x47)))                                   \
                       ? 4                                                                         \
                       : 0)                                                                        \
                : 0)                                                                               \
...
#define isALPHANUMERIC_LC_utf8_safe(s, e) D_PPP_IS_GENERIC_LC_UTF8_SAFE(s, e, ALPHANUMERIC)
#endif

#else
#ifndef isALPHANUMERIC_LC_utf8_safe
#define isALPHANUMERIC_LC_utf8_safe(s, e) (isALPHA_LC_utf8_safe(s, e) || isDIGIT_LC_utf8_safe(s, e))
#endif

#endif
#ifndef isBLANK_LC_utf8_safe
#define isBLANK_LC_utf8_safe(s, e) D_PPP_IS_GENERIC_LC_UTF8_SAFE_BROKEN(s, e, BLANK)
#endif
...
#else
#ifndef isWORDCHAR_LC_utf8_safe
#define isWORDCHAR_LC_utf8_safe(s, e) (isALPHANUMERIC_LC_utf8_safe(s, e) || (*(s)) == '_')
#endif

#endif
#ifndef isXDIGIT_LC_utf8_safe
#define isXDIGIT_LC_utf8_safe(s, e) D_PPP_IS_GENERIC_LC_UTF8_SAFE_BROKEN(s, e, XDIGIT)
#endif
...
#              */
#define D_PPP_TO_LOWER_CALLEE(s, r, l) Perl__to_utf8_lower_flags(aTHX_ s, r, l, 0, NULL)
#define D_PPP_TO_TITLE_CALLEE(s, r, l) Perl__to_utf8_title_flags(aTHX_ s, r, l, 0, NULL)
#define D_PPP_TO_UPPER_CALLEE(s, r, l) Perl__to_utf8_upper_flags(aTHX_ s, r, l, 0, NULL)
#define D_PPP_TO_FOLD_CALLEE(s, r, l) Perl__to_utf8_fold_flags(aTHX_ s, r, l, FOLD_FLAGS_FULL, NULL)
#endif
...
#define D_PPP_GENERIC_MULTI_ARG_TO(name, s, e, r, l)                                               \
    (((((e) - (s)) <= 0) /* We could just do nothing, but modern perls croak */                    \
          ? (croak("Attempting case change on zero length string"),                                \
             0) /* So looks like it returns something, and will compile */                         \
          : ((e) - (s)) < UTF8SKIP(s))                                                             \
         ? (croak(D_PPP_TOO_SHORT_MSG, s[0], (int)((e) - (s)), (int)UTF8SKIP(s)), 0)               \
         : D_PPP_TO_##name##_CALLEE(s, r, l))
#ifndef toUPPER_utf8_safe
#define toUPPER_utf8_safe(s, e, r, l) D_PPP_GENERIC_MULTI_ARG_TO(UPPER, s, e, r, l)
#endif

#ifndef toLOWER_utf8_safe
#define toLOWER_utf8_safe(s, e, r, l) D_PPP_GENERIC_MULTI_ARG_TO(LOWER, s, e, r, l)
#endif

#ifndef toTITLE_utf8_safe
#define toTITLE_utf8_safe(s, e, r, l) D_PPP_GENERIC_MULTI_ARG_TO(TITLE, s, e, r, l)
#endif

#ifndef toFOLD_utf8_safe
#define toFOLD_utf8_safe(s, e, r, l) D_PPP_GENERIC_MULTI_ARG_TO(FOLD, s, e, r, l)
#endif
...
#define D_PPP_GENERIC_SINGLE_ARG_TO_UVCHR(name, c, s, l)                                           \
    (*(l) = (D_PPP_UV_TO_UTF8(s, UNI_TO_NATIVE(to_uni_##name(NATIVE_TO_UNI(c)))) - (s)),           \
     UNI_TO_NATIVE(to_uni_##name(NATIVE_TO_UNI(c))))
#ifndef toLOWER_uvchr
#define toLOWER_uvchr(c, s, l) D_PPP_GENERIC_SINGLE_ARG_TO_UVCHR(lower, c, s, l)
#endif

#ifndef toUPPER_uvchr
#define toUPPER_uvchr(c, s, l) D_PPP_GENERIC_SINGLE_ARG_TO_UVCHR(upper, c, s, l)
#endif

#ifndef toTITLE_uvchr
#define toTITLE_uvchr(c, s, l) D_PPP_GENERIC_SINGLE_ARG_TO_UVCHR(title, c, s, l)
#endif
...
#define D_PPP_GENERIC_SINGLE_ARG_TO_UTF8(name, s, e, r, l)                                         \
    (((((e) - (s)) <= 0) ? (croak("Attempting case change on zero length string"),                 \
                            0) /* So looks like it returns something, and will compile */          \
                         : ((e) - (s)) < UTF8SKIP(s))                                              \
         ? (croak(D_PPP_TOO_SHORT_MSG, s[0], (int)((e) - (s)), (int)UTF8SKIP(s)),                  \
            0) /* Get the changed code point and store its UTF-8 */                                \
         : D_PPP_UV_TO_UTF8(                                                                       \
               r,                                                                                  \
               to_utf8_##name(s)), /* Then store its length, and re-get code point for return */   \
...
#ifndef toUPPER_utf8_safe
#define toUPPER_utf8_safe(s, e, r, l) D_PPP_GENERIC_SINGLE_ARG_TO_UTF8(upper, s, e, r, l)
#endif

#ifndef toLOWER_utf8_safe
#define toLOWER_utf8_safe(s, e, r, l) D_PPP_GENERIC_SINGLE_ARG_TO_UTF8(lower, s, e, r, l)
#endif

#ifndef toTITLE_utf8_safe
#define toTITLE_utf8_safe(s, e, r, l) D_PPP_GENERIC_SINGLE_ARG_TO_UTF8(title, s, e, r, l)
#endif
...
#if defined(PERL_USE_GCC_BRACE_GROUPS)
#define vnewSVpvf(pat, args)                                                                       \
    ({                                                                                             \
        SV *_sv = newSV(0);                                                                        \
        sv_vsetpvfn(_sv, (pat), strlen((pat)), (args), Null(SV **), 0, Null(bool *));              \
        _sv;                                                                                       \
    })
#else
#define vnewSVpvf(pat, args)                                                                       \
    ((PL_Sv = newSV(0)),                                                                           \
     sv_vsetpvfn(PL_Sv, (pat), strlen((pat)), (args), Null(SV **), 0, Null(bool *)), PL_Sv)
#endif
#endif

#if (PERL_BCDVERSION >= 0x5004000) && !defined(sv_vcatpvf)
#define sv_vcatpvf(sv, pat, args)                                                                  \
    sv_vcatpvfn(sv, pat, strlen(pat), args, Null(SV **), 0, Null(bool *))
#endif

#if (PERL_BCDVERSION >= 0x5004000) && !defined(sv_vsetpvf)
#define sv_vsetpvf(sv, pat, args)                                                                  \
...
/* Hint: SvPVbyte
 * Does not work in perl-5.6.1, lib/ppport.h implements a version
...
#define SvPVbyte(sv, lp)                                                                           \
    ((SvFLAGS(sv) & (SVf_POK | SVf_UTF8)) == (SVf_POK) ? ((lp = SvCUR(sv)), SvPVX(sv))             \
                                                       : sv_2pvbyte(sv, &lp))
...
#define sv_2pv_flags(sv, lp, flags)                                                                \
    ({                                                                                             \
        SV *_sv = (sv);                                                                            \
        const I32 _flags = (flags);                                                                \
        STRLEN *_lp = lp;                                                                          \
        _lp = _lp ?: &PL_na;                                                                       \
        (!(_flags & SV_GMAGIC) && SvGMAGICAL(_sv)) ? ({                                            \
            char *_pv;                                                                             \
            SvGMAGICAL_off(_sv);                                                                   \
            _pv = sv_2pv(_sv, _lp);                                                                \
            SvGMAGICAL_on(_sv);                                                                    \
...
#define sv_2pv_flags(sv, lp, flags)                                                                \
    ((PL_Sv = (sv)),                                                                               \
     (!((flags)&SV_GMAGIC) && SvGMAGICAL(PL_Sv))                                                   \
         ? (SvGMAGICAL_off(PL_Sv), (PL_Xpv = (XPV *)sv_2pv(PL_Sv, (lp) ? (lp) : &PL_na)),          \
            SvGMAGICAL_on(PL_Sv), (char *)PL_Xpv)                                                  \
...
#define sv_pvn_force_flags(sv, lp, flags)                                                          \
    ((PL_Sv = (sv)),                                                                               \
     (!((flags)&SV_GMAGIC) && SvGMAGICAL(PL_Sv))                                                   \
         ? (SvGMAGICAL_off(PL_Sv), (PL_Xpv = (XPV *)sv_pvn_force(PL_Sv, (lp) ? (lp) : &PL_na)),    \
            SvGMAGICAL_on(PL_Sv), (char *)PL_Xpv)                                                  \
         : sv_pvn_force(PL_Sv, (lp) ? (lp) : &PL_na))
#endif

#endif

#if (PERL_BCDVERSION < 0x5008008) ||                                                               \
    ((PERL_BCDVERSION >= 0x5009000) && (PERL_BCDVERSION < 0x5009003))
...
#define SvPV_flags(sv, lp, flags)                                                                  \
    ((SvFLAGS(sv) & (SVf_POK)) == SVf_POK ? ((lp = SvCUR(sv)), SvPVX(sv))                          \
...
#define SvPV_flags_mutable(sv, lp, flags)                                                          \
    ((SvFLAGS(sv) & (SVf_POK)) == SVf_POK ? ((lp = SvCUR(sv)), SvPVX_mutable(sv))                  \
...
#define SvPV_force_flags(sv, lp, flags)                                                            \
    ((SvFLAGS(sv) & (SVf_POK | SVf_THINKFIRST)) == SVf_POK ? ((lp = SvCUR(sv)), SvPVX(sv))         \
...
#define SvPV_force_flags_nolen(sv, flags)                                                          \
    ((SvFLAGS(sv) & (SVf_POK | SVf_THINKFIRST)) == SVf_POK                                         \
         ? SvPVX(sv)                                                                               \
         : sv_pvn_force_flags(sv, D_PPP_SVPV_NOLEN_LP_ARG, flags))
#endif
...
#define SvPV_nolen(sv)                                                                             \
    ((SvFLAGS(sv) & (SVf_POK)) == SVf_POK ? SvPVX(sv)                                              \
                                          : sv_2pv_flags(sv, D_PPP_SVPV_NOLEN_LP_ARG, SV_GMAGIC))
#endif
#ifndef SvPV_nolen_const
#define SvPV_nolen_const(sv)                                                                       \
    ((SvFLAGS(sv) & (SVf_POK)) == SVf_POK                                                          \
         ? SvPVX_const(sv)                                                                         \
         : sv_2pv_flags(sv, D_PPP_SVPV_NOLEN_LP_ARG, SV_GMAGIC | SV_CONST_RETURN))
#endif
...
#define SvPV_nomg_nolen(sv)                                                                        \
    ((SvFLAGS(sv) & (SVf_POK)) == SVf_POK ? SvPVX(sv)                                              \
                                          : sv_2pv_flags(sv, D_PPP_SVPV_NOLEN_LP_ARG, 0))
#endif
#ifndef SvPV_renew
#define SvPV_renew(sv, n)                                                                          \
    STMT_START {                                                                                   \
        SvLEN_set(sv, n);                                                                          \
        SvPV_set((sv), (char *)saferealloc((Malloc_t)SvPVX(sv), (MEM_SIZE)((n))));                 \
    }                                                                                              \
    STMT_END
#endif
#ifndef SvPVCLEAR
...
#define Perl_ck_warner DPPP_(my_ck_warner)
    ...
    if (!ckWARN((err)&0xFF) && !ckWARN((err >> 8) & 0xFF) && !ckWARN((err >> 16) & 0xFF) &&
        !ckWARN((err >> 24) & 0xFF)) {
        return;
    }
...
#define Perl_ck_warner_d DPPP_(my_ck_warner_d)
    ...
    if (!ckWARN_d((err)&0xFF) && !ckWARN_d((err >> 8) & 0xFF) && !ckWARN_d((err >> 16) & 0xFF) &&
...
#define sv_2uv(sv)                                                                                 \
    ({                                                                                             \
        SV *_sv = (sv);                                                                            \
        (UV)(SvNOK(_sv) ? SvNV(_sv) : sv_2nv(_sv));                                                \
    })
#endif
...
#ifndef croak_sv
#if (PERL_BCDVERSION >= 0x5007003) ||                                                              \
    ((PERL_BCDVERSION >= 0x5006001) && (PERL_BCDVERSION < 0x5007000))
...
#define D_PPP_FIX_UTF8_ERRSV_FOR_SV(sv)                                                            \
    STMT_START {                                                                                   \
        SV *_errsv = ERRSV;                                                                        \
        SvFLAGS(_errsv) = (SvFLAGS(_errsv) & ~SVf_UTF8) | (SvFLAGS(sv) & SVf_UTF8);                \
    }                                                                                              \
    STMT_END
#else
#define D_PPP_FIX_UTF8_ERRSV_FOR_SV(sv)                                                            \
...
#if (PERL_BCDVERSION >= 0x5009002) ||                                                              \
    ((PERL_BCDVERSION >= 0x5008006) && (PERL_BCDVERSION < 0x5009000))
...
#define D_PPP_CROAK_IF_ERROR(cond)                                                                 \
    ({                                                                                             \
        SV *_errsv;                                                                                \
        ((cond) && (_errsv = ERRSV) && (SvROK(_errsv) || SvTRUE(_errsv)) &&                        \
         (croak_sv(_errsv), 1));                                                                   \
    })
#else
#define D_PPP_CROAK_IF_ERROR(cond)                                                                 \
    ((cond) && (SvROK(ERRSV) || SvTRUE(ERRSV)) && (croak_sv(ERRSV), 1))
#endif
...
#define call_sv(sv, flags)                                                                         \
    ((flags)&G_METHOD ? perl_call_method((char *)SvPV_nolen_const(sv), (flags) & ~G_METHOD)        \
                      : perl_call_sv(sv, flags))
#else
#define call_sv(sv, flags)                                                                         \
    ((flags)&G_METHOD ? Perl_call_method(aTHX_(char *) SvPV_nolen_const(sv), (flags) & ~G_METHOD)  \
...
#define eval_sv(sv, flags)                                                                         \
    ({                                                                                             \
        I32 _flags = (flags);                                                                      \
        I32 _ret = Perl_eval_sv(aTHX_ sv, (_flags & ~G_RETHROW));                                  \
        D_PPP_CROAK_IF_ERROR(_flags &G_RETHROW);                                                   \
        _ret;                                                                                      \
    })
#else
#define eval_sv(sv, flags)                                                                         \
    ((PL_na = Perl_eval_sv(aTHX_ sv, ((flags) & ~G_RETHROW))),                                     \
     D_PPP_CROAK_IF_ERROR((flags)&G_RETHROW), (I32)PL_na)
#endif
#endif
...
#undef eval_pv
#if defined(PERL_USE_GCC_BRACE_GROUPS)
#define eval_pv(p, croak_on_error)                                                                 \
    ({                                                                                             \
        SV *_sv = Perl_eval_pv(aTHX_ p, 0);                                                        \
        D_PPP_CROAK_IF_ERROR(croak_on_error);                                                      \
        _sv;                                                                                       \
    })
#else
#define eval_pv(p, croak_on_error)                                                                 \
    ((PL_Sv = Perl_eval_pv(aTHX_ p, 0)), D_PPP_CROAK_IF_ERROR(croak_on_error), PL_Sv)
#endif
#endif
#endif
...
#define Perl_vload_module DPPP_(my_vload_module)
    ...
    if (ver) { veop = newSVOP(OP_CONST, 0, ver); }
    else
        veop = NULL;
...
#define dMY_CXT_SV SV *my_cxt_sv = get_sv(MY_CXT_KEY, FALSE)
#else /* >= perl5.004_68 */
#define dMY_CXT_SV SV *my_cxt_sv = *hv_fetch(PL_modglobal, MY_CXT_KEY, sizeof(MY_CXT_KEY) - 1, TRUE)
#endif /* < perl5.004_68 */
...
#endif
#ifndef newSVpvn
#define newSVpvn(data, len) ((data) ? ((len) ? newSVpv((data), (len)) : newSVpv("", 0)) : newSV(0))
#endif
...
#define newSVpvn_flags(s, len, flags)                                                              \
    ({                                                                                             \
        SV *_sv = newSVpvn(D_PPP_CONSTPV_ARG((s)), (len));                                         \
        SvFLAGS(_sv) |= ((flags)&SVf_UTF8);                                                        \
        ((flags)&SVs_TEMP) ? sv_2mortal(_sv) : _sv;                                                \
    })
#else
#define newSVpvn_flags(s, len, flags)                                                              \
    ((PL_Sv = newSVpvn(D_PPP_CONSTPV_ARG((s)), (len))), SvFLAGS(PL_Sv) |= ((flags)&SVf_UTF8),      \
     (((flags)&SVs_TEMP) ? sv_2mortal(PL_Sv) : PL_Sv))
#endif
#endif
#ifndef SV_NOSTEAL
...
((((flags)&SV_NOSTEAL) && (sstr) && (SvFLAGS((SV *)(sstr)) & SVs_TEMP))
     ? (SvTEMP_off((SV *)(sstr)), Perl_sv_setsv_flags(aTHX_(dstr), (sstr), (flags) & ~SV_NOSTEAL),
        SvTEMP_on((SV *)(sstr)), 1)
     : (Perl_sv_setsv_flags(aTHX_(dstr), (sstr), (flags) & ~SV_NOSTEAL), 1))
#endif
#endif
    ...
    ((((flags)&SV_NOSTEAL) && (sstr) && (SvFLAGS((SV *)(sstr)) & SVs_TEMP))                        \
         ? (SvTEMP_off((SV *)(sstr)),                                                              \
            (!((flags)&SV_GMAGIC) && (sstr) && SvGMAGICAL((SV *)(sstr)))                           \
                ? (SvGMAGICAL_off((SV *)(sstr)), sv_setsv((dstr), (sstr)),                         \
                   SvGMAGICAL_on((SV *)(sstr)), 1)                                                 \
                : (sv_setsv((dstr), (sstr)), 1),                                                   \
            SvTEMP_on((SV *)(sstr)), 1)                                                            \
         : ((!((flags)&SV_GMAGIC) && (sstr) && SvGMAGICAL((SV *)(sstr)))                           \
                ? (SvGMAGICAL_off((SV *)(sstr)), sv_setsv((dstr), (sstr)),                         \
                   SvGMAGICAL_on((SV *)(sstr)), 1)                                                 \
...
#define Perl_newSVpvn_share DPPP_(my_newSVpvn_share)
    ...
    SV *sv;
    if (len < 0) len = -len;
    if (!hash) PERL_HASH(hash, (char *)s, len);
...
#if (PERL_BCDVERSION >= 0x5009002) &&                                                              \
    (PERL_BCDVERSION <= 0x5009003) /* 5.9.2 and 5.9.3 ignore the length param */
...
#define gv_fetchpvn_flags(name, len, flags, sv_type)                                               \
    gv_fetchpv(SvPVX(sv_2mortal(newSVpvn((name), (len)))),                                         \
               ((flags)&D_PPP_GV_NOADD_MASK) ? FALSE : TRUE, (I32)(sv_type))
#endif
#ifndef GvSVn
...
#ifndef SvIV_nomg
#define SvIV_nomg(sv)                                                                              \
    (!SvGMAGICAL((sv)) ? SvIV((sv)) : ({                                                           \
        SV *_sviv = sv_mortalcopy_flags((sv), SV_NOSTEAL);                                         \
        IV _iv = SvIV(_sviv);                                                                      \
...
#ifndef SvUV_nomg
#define SvUV_nomg(sv)                                                                              \
    (!SvGMAGICAL((sv)) ? SvUV((sv)) : ({                                                           \
        SV *_svuv = sv_mortalcopy_flags((sv), SV_NOSTEAL);                                         \
        UV _uv = SvUV(_svuv);                                                                      \
        SvFLAGS((sv)) = (SvFLAGS((sv)) & ~SVf_IVisUV) | (SvFLAGS(_svuv) & SVf_IVisUV);             \
        _uv;                                                                                       \
    }))
#endif
...
#define SvIV_nomg(sv)                                                                              \
    (!SvGMAGICAL((sv))                                                                             \
         ? SvIV((sv))                                                                              \
         : ((PL_Sv = sv_mortalcopy_flags((sv), SV_NOSTEAL)), sv_upgrade(PL_Sv, SVt_PVIV),          \
            (SvIVX(PL_Sv) = SvIV(PL_Sv)),                                                          \
            (SvFLAGS((sv)) = (SvFLAGS((sv)) & ~SVf_IVisUV) | (SvFLAGS(PL_Sv) & SVf_IVisUV)),       \
            SvIVX(PL_Sv)))
#endif
...
#define Perl_sv_unmagicext DPPP_(my_sv_unmagicext)
    ...
    for (mg = *mgp; mg; mg = *mgp) {
        ...
        if (mg->mg_type == type && virt == vtbl) {
            *mgp = mg->mg_moremagic;
            if (virt && virt->svt_free) virt->svt_free(aTHX_ sv, mg);
            ...
            if (mg->mg_flags & MGf_REFCOUNTED) SvREFCNT_dec(mg->mg_obj);
            Safefree(mg);
        }
        else
            mgp = &mg->mg_moremagic;
...
#define CopSTASH_eq(c, hv)                                                                         \
    ((hv) && (CopSTASHPV(c) == HvNAME(hv) ||                                                       \
              (CopSTASHPV(c) && HvNAME(hv) && strEQ(CopSTASHPV(c), HvNAME(hv)))))
#endif
...
static I32 DPPP_dopoptosub_at(const PERL_CONTEXT *cxstk, I32 startingblock) {
    I32 i;
...
#define Perl_grok_bin DPPP_(my_grok_bin)
    ...
    for (; len-- && *s; s++) {
        ...
        if (bit == '_' && len && allow_underscores && (bit = s[1]) && (bit == '0' || bit == '1')) {
            --len;
            ++s;
            goto redo;
        }
        if (!(*flags & PERL_SCAN_SILENT_ILLDIGIT)) warn("Illegal binary digit '%c' ignored", *s);
        break;
    }
...
#define Perl_grok_oct DPPP_(my_grok_oct)
    ...
    for (; len-- && *s; s++) {
        ...
        if (digit == ('_' - '0') && len && allow_underscores && (digit = s[1] - '0') &&
            (digit >= 0 && digit <= 7)) {
            --len;
            ++s;
            goto redo;
        ...
        if (digit == 8 || digit == 9) {
            if (!(*flags & PERL_SCAN_SILENT_ILLDIGIT)) warn("Illegal octal digit '%c' ignored", *s);
        }
        break;
    }
...
#define UTF8_IS_DOWNGRADEABLE_START(c)                                                             \
    inRANGE(NATIVE_UTF8_TO_I8(c), UTF_MIN_START_BYTE, UTF_MIN_ABOVE_LATIN1_BYTE - 1)
#endif
...
#define UTF8_ACCUMULATE(base, added)                                                               \
    (((base) << UTF_ACCUMULATION_SHIFT) | ((NATIVE_UTF8_TO_I8(added)) & UTF_CONTINUATION_MASK))
#endif
...
#define UTF8_ALLOW_ANY                                                                             \
    (UTF8_ALLOW_CONTINUATION | UTF8_ALLOW_NON_CONTINUATION | UTF8_ALLOW_SHORT | UTF8_ALLOW_LONG |  \
     UTF8_ALLOW_OVERFLOW)
#endif
...
#define UTF8_SAFE_SKIP(s, e) (((((e) - (s)) <= 0) ? 0 : D_PPP_MIN(((e) - (s)), UTF8SKIP(s))))
#endif
#ifndef UTF8_CHK_SKIP
#define UTF8_CHK_SKIP(s)                                                                           \
    (s[0] == '\0' ? 1 : ((U8)D_PPP_MIN(my_strnlen((char *)(s), UTF8SKIP(s)), UTF8SKIP(s))))
#endif
...
#define D_PPP_UVCHR_SKIP_UPPER(c)                                                                  \
    (((WIDEST_UTYPE)(c)) < (((WIDEST_UTYPE)1) << (6 * D_PPP_BYTE_INFO_BITS)) ? 7 : 13)
#endif
#ifndef UVCHR_SKIP
#define UVCHR_SKIP(c)                                                                              \
...
#define D_PPP_utf8_to_uvchr_buf_callee utf8n_to_uvchr
    ...
    defined(utf8_to_uv) &&                                                                         \
    defined(utf8_to_uv_simple)
#define D_PPP_utf8_to_uvchr_buf_callee utf8_to_uv
#elif defined(utf8_to_uvchr) /* The below won't work well on error input */
#define D_PPP_utf8_to_uvchr_buf_callee(s, curlen, retlen, flags) utf8_to_uvchr((U8 *)(s), (retlen))
#else
#define D_PPP_utf8_to_uvchr_buf_callee(s, curlen, retlen, flags) utf8_to_uv((U8 *)(s), (retlen))
#endif
#endif
...
#define Perl_utf8_to_uvchr_buf DPPP_(my_utf8_to_uvchr_buf)
    ...
    if (curlen > 0 && UNLIKELY(*s >= 0xFE)) {
        ...
        if (sizeof(ret) < 8) {
            ...
            for (cur_s = s; cur_s < send; cur_s++, cur_h++) {
                if (UNLIKELY(*cur_s == *cur_h)) { continue; }
    ...
    if (UNLIKELY(overflows)) {
        ...
        if (!do_warnings && retlen) { *retlen = overflow_length; }
    }
    else
        ...
        ret = D_PPP_utf8_to_uvchr_buf_callee(
            ...
            s,
            curlen, retlen, (UTF8_ALLOW_ANYUV & ~(UTF8_ALLOW_LONG | UTF8_ALLOW_EMPTY)));
     ...
     * overflowing. */
    if (UNLIKELY(ret > IV_MAX)) { overflows = 1; }
    ...
    if (UNLIKELY(ret == 0 && (curlen == 0 || *s != '\0'))) {
        ...
        if (do_warnings) {
            ...
            ret = D_PPP_utf8_to_uvchr_buf_callee((U8 *)/* Early perls: no const */
                                                 s,
                                                 curlen, retlen, UTF8_ALLOW_ANY);
...
#define utf8_to_uvchr(s, lp)                                                                       \
    ((*(s) == '\0') ? utf8_to_uvchr_buf(s, ((s) + 1), lp) /* Handle single NUL specially */        \
...
#define sv_len_utf8_nomg(sv)                                                                       \
    ({                                                                                             \
        SV *_sv2 = (sv);                                                                           \
        (SvUTF8(_sv2) ? Perl_sv_len_utf8(aTHX_(                                                    \
                            !SvGMAGICAL(_sv2) ? _sv2 : sv_mortalcopy_flags(_sv2, SV_NOSTEAL)))     \
                      : ({                                                                         \
                            STRLEN _len;                                                           \
                            SvPV_nomg(_sv2, _len);                                                 \
                            _len;                                                                  \
...
#define sv_len_utf8_nomg(sv)                                                                       \
    (PL_Sv = (sv),                                                                                 \
     (SvUTF8(PL_Sv) ? Perl_sv_len_utf8(aTHX_(                                                      \
                          !SvGMAGICAL(PL_Sv) ? PL_Sv : sv_mortalcopy_flags(PL_Sv, SV_NOSTEAL)))    \
                    : (SvPV_nomg(PL_Sv, PL_na), PL_na)))
...
#define sv_len_utf8_nomg(sv)                                                                       \
    ((PL_Sv = (sv)),                                                                               \
     sv_len_utf8(!SvGMAGICAL(PL_Sv) ? PL_Sv : sv_mortalcopy_flags(PL_Sv, SV_NOSTEAL)))
#endif
...
static char *DPPP_(my_pv_escape)(pTHX_ SV *dsv, char const *const str, const STRLEN count,
                                 const STRLEN max, STRLEN *const escaped, const U32 flags);
static
#else
extern char *DPPP_(my_pv_escape)(pTHX_ SV *dsv, char const *const str, const STRLEN count,
                                 const STRLEN max, STRLEN *const escaped, const U32 flags);
#endif
...
#define Perl_pv_escape DPPP_(my_pv_escape)
    ...
    char *
    DPPP_(my_pv_escape)(pTHX_ SV *dsv, char const *const str, const STRLEN count, const STRLEN max,
    ...
    if (!(flags & PERL_PV_ESCAPE_NOCLEAR)) sv_setpvs(dsv, "");

#if defined(is_utf8_string) && defined(utf8_to_uvchr_buf)
    if ((flags & PERL_PV_ESCAPE_UNI_DETECT) && is_utf8_string((U8 *)pv, count)) isuni = 1;
#endif
    ...
    for (; pv < end && (!max || wrote < max); pv += readsize) {
        ...
        if (u > 255 || (flags & PERL_PV_ESCAPE_ALL)) {
            if (flags & PERL_PV_ESCAPE_FIRSTCHAR)
                chsize = my_snprintf(octbuf, sizeof octbuf, "%" UVxf, u);
            else
        ...
        if (flags & PERL_PV_ESCAPE_FIRSTCHAR) break;
    }
    if (escaped != NULL) *escaped = pv - str;
    return SvPVX(dsv);
}
...
static char *DPPP_(my_pv_pretty)(pTHX_ SV *dsv, char const *const str, const STRLEN count,
                                 const STRLEN max, char const *const start_color,
                                 char const *const end_color, const U32 flags);
static
#else
extern char *DPPP_(my_pv_pretty)(pTHX_ SV *dsv, char const *const str, const STRLEN count,
                                 const STRLEN max, char const *const start_color,
                                 char const *const end_color, const U32 flags);
#endif
...
#define Perl_pv_pretty DPPP_(my_pv_pretty)
    ...
    char *
    DPPP_(my_pv_pretty)(pTHX_ SV *dsv, char const *const str, const STRLEN count, const STRLEN max,
                        char const *const start_color, char const *const end_color,
                        const U32 flags) {
    const U8 dq = (flags & PERL_PV_PRETTY_QUOTE) ? '"' : '%';
    STRLEN escaped;

    if (!(flags & PERL_PV_PRETTY_NOCLEAR)) sv_setpvs(dsv, "");
    ...
    if (start_color != NULL) sv_catpv(dsv, D_PPP_CONSTPV_ARG(start_color));

    pv_escape(dsv, str, count, max, &escaped, flags | PERL_PV_ESCAPE_NOCLEAR);

    if (end_color != NULL) sv_catpv(dsv, D_PPP_CONSTPV_ARG(end_color));
    ...
    if ((flags & PERL_PV_PRETTY_ELLIPSES) && escaped < count) sv_catpvs(dsv, "...");
...
static char *DPPP_(my_pv_display)(pTHX_ SV *dsv, const char *pv, STRLEN cur, STRLEN len,
                                  STRLEN pvlim);
static
#else
extern char *DPPP_(my_pv_display)(pTHX_ SV *dsv, const char *pv, STRLEN cur, STRLEN len,
                                  STRLEN pvlim);
#endif
...
#define Perl_pv_display DPPP_(my_pv_display)
    ...
    if (len > cur && pv[cur] == '\0') sv_catpvs(dsv, "\\0");
    return SvPVX(dsv);
}
...
#define RESTORE_LC_NUMERIC() setlocale(LC_NUMERIC, LoC_);
#endif
...
#define sync_locale()                                                                              \
    (new_ctype(setlocale(LC_CTYPE, NULL)), new_collate(setlocale(LC_COLLATE, NULL)),               \
     set_numeric_local(), new_numeric(setlocale(LC_NUMERIC, NULL)), 1)
