...
{ my $b = Data::BloomFilter::Shared->new($p, 10000, 0.01); }
my $total = -s $p;
unlink $p;
...
# 1. Recovery: an abandoned all-zero file of exactly $total bytes is re-initialized.
{
    ...
    my $b = eval { Data::BloomFilter::Shared->new($p, 10000, 0.01) };
...
{
    my $a = Data::BloomFilter::Shared->new($p, 10000, 0.01); $a->add("keep"); undef $a;
    my $r = Data::BloomFilter::Shared->new($p, 10000, 0.01);
