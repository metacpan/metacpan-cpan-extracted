...
package MongoDB::Async::Connection;
{
  $MongoDB::Async::Connection::VERSION = '0.702.3';
}
...
has '_client' => (
    ...
    handles     => [ grep { $_ !~ /^(meta|new)$/ } 
                     map { $_->name } Class::MOP::Class->initialize( 'MongoDB::Async::MongoClient' )->get_all_methods 
                   ]
);
...
around 'new' => sub { 
    my ( $orig, $self, @args ) = @_;
    return $self->$orig( _client => MongoDB::Async::MongoClient->new( @args ) );
};
...
MongoDB::Async::Connection - A connection to a Mongo server
...
The MongoDB::Async::Connection class creates a connection to the MongoDB server.

By default, it connects to a single server running on the local machine
listening on the default port:

    # connects to localhost:27017
    my $connection = MongoDB::Async::Connection->new;

It can connect to a database server running anywhere, though:

    my $connection = MongoDB::Async::Connection->new(host => 'example.com:12345');
...
MongoDB::Async::Connection - A connection to a MongoDB server

=head1 DEPRECATED

NOTE: C<MongoDB::Async::Connection> is DEPRECATED as of version 0.502.0 of the MongoDB CPAN distribution. 
It is no longer maintained and will be removed in a future version. Use L<MongoDB::Async::MongoClient> instead. 
...
example, to connecting to C<localhost:27017> and C<localhost:27018>:
    ...
    $conn = MongoDB::Async::Connection->new("host" => "mongodb://localhost,localhost:27018");
...
=head2 query_timeout
    ...
    my $conn = MongoDB::Async::Connection->new(query_timeout => 1000);
...
L<MongoDB::Async::Cursor/timeout>.
...
This value overrides L<MongoDB::Async::Cursor/timeout>.

    $MongoDB::Async::Cursor::timeout = 1000;
    my $conn = MongoDB::Async::Connection->new(query_timeout => 10);
...
L<MongoDB::Async::Cursor/slave_okay> documentation for information on reading from a
slave.
...
Returns a L<MongoDB::Async::Database> instance for database with the given C<$name>.
...
L<MongoDB::Async::write_insert>, L<MongoDB::Async::write_update>, L<MongoDB::Async::write_remove>, or
L<MongoDB::Async::write_query> to create a valid string.
...
C<MongoDB::Async::Cursor>.  At the moment, the only required field for C<$info> is
"ns", although "request_id" is likely to be required in the future.  The
C<$info> hash will be automatically created for you by L<MongoDB::Async::write_query>.
