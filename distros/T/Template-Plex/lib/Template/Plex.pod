...
=item * Make examples of recursion, lexical scoping, etc...
...
calling the C<render> method on it.

B<Single  Argument Version>
...
This doesn't impact recursive calls to C<plex> when dynamically/conditionally
loading templates.
...
Note that the lexically aliased variables in C<plex> are independent to the
...
This is a special directive that substitutes the text B<@{[include("path")]}>
...
into the template at that point.  So instead you could use C<do{..}> execute
