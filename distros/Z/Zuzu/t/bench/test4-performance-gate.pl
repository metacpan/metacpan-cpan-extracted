...
my $threshold_file = File::Spec->catfile( $Bin, 'test4-thresholds.json' );
my $benchmark_file = File::Spec->catfile( $Bin, 'test4-benchmark.json' );

my $iterations = $ENV{TEST4_BENCH_ITERATIONS};
$iterations = 2000 if !defined $iterations or $iterations !~ /^\d+$/;

my $warmup = $ENV{TEST4_BENCH_WARMUP};
$warmup = 100 if !defined $warmup or $warmup !~ /^\d+$/;

my $cmd = join ' ',
	$^X,
	File::Spec->catfile( $Bin, 'core1-baseline.pl' ),
	'--json',
	'--iterations',
	$iterations,
	'--warmup',
...
if ( $? != 0 ) {
	die "Could not execute core1-baseline benchmark\n";
}
...
if ( !@ops ) {
	die "No performance thresholds were defined\n";
}

my $failures = 0;
print "TEST4 performance thresholds\n";
...
if ( $failures ) {
	die "Performance regression gate failed ($failures failures)\n";
}
