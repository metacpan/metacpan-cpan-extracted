...
(This example is included as L<F<synopsis.pl>|https://fastapi.metacpan.org/source/BKB/Image-PNG-Libpng-0.49/examples/synopsis.pl> in the distribution.)


=head1 VERSION

This document describes Image::PNG::Libpng version 0.49, corresponding to git commit L<f38ec837a4703027147409f4976d08a5f5d10efc|https://github.com/benkasminbullock/image-png-libpng/commit/f38ec837a4703027147409f4976d08a5f5d10efc> at Thu Dec 10 08:47:36 2020 +0900.
...
(This example is included as L<F<get-www-png.pl>|https://fastapi.metacpan.org/source/BKB/Image-PNG-Libpng-0.49/examples/get-www-png.pl> in the distribution.)
...
(This example is included as L<F<png-cgi.pl>|https://fastapi.metacpan.org/source/BKB/Image-PNG-Libpng-0.49/examples/png-cgi.pl> in the distribution.)
...
(This example is included as L<F<copy-png.pl>|https://fastapi.metacpan.org/source/BKB/Image-PNG-Libpng-0.49/examples/copy-png.pl> in the distribution.)
...
Get the specified chunk. Croaks if given C<IDAT> chunk, use
L</get_rows> instead. Returns undef if given unknown chunk. 
...
Set the specified chunk. Croaks if given C<IDAT> chunk, use
L</set_rows> instead. Croaks if given unknown chunk.
...
RGBA images or gray/alpha images.
...
⛐🤪⚠ NOTE: this list is untested and might be wrong. It was
...
See L<this subsection|http://www.w3.org/TR/PNG/#11IHDR>
...
L</bKGD>, L</cHRM>, L</gAMA>, L</hIST>, L</hIST>, L</iCCP>, L<IDAT|/Image data>, L<IHDR|/The image header>, L</iTXt>, L</oFFs>, L</pCAL>, L</pHYs>, L<PLTE|/Palettes>, L</sBIT>, L</sCAL>, L</sPLT>, L</sRGB>, L</tEXt>, L<tIME|/PNG timestamps>, L</tRNS>, and L</zTXt>.
...
⛐🤪⚠ It does not return the fields C<filter_method> and
C<compression_method>, which are always 0. See L<Unused arguments omitted>.
...
(This example is included as L<F<color-type-name.pl>|https://fastapi.metacpan.org/source/BKB/Image-PNG-Libpng-0.49/examples/color-type-name.pl> in the distribution.)
...
This sets the text chunks in an array reference C<$text_chunks>. The
input value is an array reference containing hash references. Each
hash reference must have the following keys:

=over

=item C<key>

The "key" value for the text. According to the PNG specification, this
should be between one and 79 bytes in length, so if you supply a key
longer than that the chunk cannot be added.
...
The optional key C<compression> controls the compression of the
...
Whether or not the value of C<text> is an ITXT field is also decided
by the compression value. People who want to fiddle with the text
compression can do so via L</set_text_compression_level> and the other
functions described below that.

If it is called more than once, the chunks are not overwritten but
appended to the existing ones. (This behaviour is copied from libpng
itself.)
...
(This example is included as L<F<set-text.pl>|https://fastapi.metacpan.org/source/BKB/Image-PNG-Libpng-0.49/examples/set-text.pl> in the distribution.)


=head3 Other fields

The following fields can also be used in C<set_text>:
...
specification. Note that the user needs to supply L</key> and L</lang>
...
length obtained from Perl. This key is used to set the length of the
string when reading text via L</get_text>. If the text is marked as
international text, L</itxt_length> is used to return its length in
...
Get the maximum width allowed. These values can be changed by
L</set_user_limits>.
...
Get the maximum height allowed. These values can be changed by
L</set_user_limits>.
...
(⛐🤪⚠ The libpng documentation, and online sites derived from it,
...
This function corresponds to C<png_get_cHRM> in libpng with a hash function instead of the C<double>
...
This function corresponds to C<png_set_cHRM> in libpng with a hash function instead of the C<double>
arguments.
...
This function corresponds to C<png_set_iCCP> in libpng.
...
⛐🤪⚠ Implemented but not tested. Return value is a hash with
the following keys:
...
⛐🤪⚠ Implemented but not tested. The input is just the same as
...
⛐🤪⚠ These functions aren't fully documented and are subject to
...
=head2 set_rgb_to_gray_fixed

    $png->set_gray_to_rgb ();

This function corresponds to C<png_set_gray_to_rgb_fixed> in libpng
...
⛐🤪⚠ This module eliminates all the unevaluated arguments of
...
you can find each instance L<in the file C<perl-libpng.c> in the top directory of the distribution|http://cpansearch.perl.org/src/BKB/Image-PNG-Libpng-0.49/perl-libpng.c> in the macro
C<UNUSED_ZERO_ARG>.
...
There is no support for the PNG fixed point functions in this Perl
module.
...
⛐🤪⚠ This is a useless function in libpng corresponding to the
...
⛐🤪⚠ This is not an exhaustive list of libpng functions.
...
=item The %s chunk is not supported in this libpng
...
=item Request for too many bytes %zu on a scalar of length %zu at read position %d.
...
=item set_PLTE: Empty array of colors in set_PLTE
...
=item Not a hash reference at position %d

(W) 



=item Could not get entries at position %d

(W) 



=item Could not get entry %d
...
=item Row %d is %zu bytes, which is too long; truncating to %d

(W) 



=item %s:%d: Mismatch %p != %p
...
=item Unknown whether '%s' is supported

(W) 
The user asked whether something was supported, but we don't know what that thing is.
...
=item chunk %i has bad length %zu: should be %d in chunk list
...
=head2 Text chunk addition fails silently

If some of your text chunks are incorrectly formatted, L</set_text>
just silently refuses to add them rather than warning you. This
behaviour may be modified in future versions.

=head2 Issues with old versions of libpng

⛐🤪⚠ Some operating systems, in particular Linux systems, use
...
⛐🤪⚠ The documentation which comes with libpng is rather sketchy. See
