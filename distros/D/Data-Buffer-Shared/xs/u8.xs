...
new(char* class, SV* path, UV capacity, UV file_mode = 0600)
    CODE:
        ...
        BufHandle* buf = buf_u8_create(p, (uint64_t)capacity, (mode_t)file_mode, errbuf);
...
DESTROY(SV* self_sv)
    CODE:
        if (!sv_isobject(self_sv) || !sv_derived_from(self_sv, "Data::Buffer::Shared::U8")) return;
...
set_slice(SV* self_sv, UV from, ...)
    CODE:
        ...
        REEXTRACT_BUF("Data::Buffer::Shared::U8", self_sv);
...
cas(SV* self_sv, UV idx, UV expected, UV desired)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::U8", self_sv);
        RETVAL = buf_u8_cas(h, (uint64_t)idx, (uint8_t)expected, (uint8_t)desired);
    OUTPUT:
        RETVAL
...
unlink(SV* self_or_class, ...)
    CODE:
        const char *p;
        if (sv_isobject(self_or_class) && sv_derived_from(self_or_class, "Data::Buffer::Shared::U8")) {
            BufHandle* h = INT2PTR(BufHandle*, SvIV(SvRV(self_or_class)));
            if (h) { if (!h->path) croak("cannot unlink anonymous buffer"); p = h->path; }
            else croak("Data::Buffer::Shared::U8: destroyed object");
        } else {
            if (items < 2) croak("Usage: Data::Buffer::Shared::U8->unlink($path)");
            p = SvPV_nolen(ST(1));
        }
...
get_raw(SV* self_sv, UV byte_off, UV nbytes)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::U8", self_sv);
        if (!buf_u8_raw_in_bounds(h, (uint64_t)byte_off, (uint64_t)nbytes))
...
new_memfd(char* class, SV* name, UV capacity)
    CODE:
        ...
        BufHandle* buf = buf_u8_create_memfd(nm, (uint64_t)capacity, errbuf);
        if (!buf) croak("Data::Buffer::Shared::U8: %s", errbuf[0] ? errbuf : "unknown error");
        REREAD_CLASS();
        RETVAL = sv_setref_pv(newSV(0), class, (void*)buf);
    OUTPUT:
...
new_from_fd(char* class, int fd)
    CODE:
        char errbuf[BUF_ERR_BUFLEN];
        BufHandle* buf = buf_u8_open_fd(fd, errbuf);
        if (!buf) croak("Data::Buffer::Shared::U8: %s", errbuf[0] ? errbuf : "unknown error");
        REREAD_CLASS();
        RETVAL = sv_setref_pv(newSV(0), class, (void*)buf);
    OUTPUT:
...
add_slice(SV* self_sv, UV from, ...)
    CODE:
        ...
        for (UV i = 0; i < count; i++)
            tmp[i] = (uint8_t)SvUV(ST(i + 2));
        REEXTRACT_BUF("Data::Buffer::Shared::U8", self_sv);
        RETVAL = buf_u8_add_slice(h, (uint64_t)from, (uint64_t)count, tmp);
    OUTPUT:
        RETVAL
...
create_eventfd(SV* self_sv)
    CODE:
        ...
        if (RETVAL < 0) croak("Data::Buffer::Shared::U8: eventfd: %s", strerror(errno));
    OUTPUT:
        RETVAL
...
sync(SV* self_sv)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::U8", self_sv);
        if (buf_msync(h) != 0) croak("Data::Buffer::Shared::U8 sync: %s", strerror(errno));
