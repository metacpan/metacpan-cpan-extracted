...
sub client2_input {
  ...
  if ( $cmd eq '441' ) {
    ...
    $poe_kernel->post( $sender, 'send_to_server', { command => 'REMOVE', params => [ '#floobar', 'bobbins', 'Filthy ape' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq '482' ) {
    pass("IRC$cmd");
...
sub client3_input {
  ...
  if ( $cmd eq '366' ) {
    $poe_kernel->post('client', 'send_to_server', { command => 'REMOVE', params => [ '#floobar', 'custard', 'Filthy ape' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq 'PART' ) {
    pass($cmd);
    is($params->[0],'#floobar','Channel is correct');
    is($params->[1],'Requested by bobbins "Filthy ape"','Pithy comment');
    $poe_kernel->post( 'client2', 'send_to_server', { command => 'REMOVE', params => [ '#floobar', 'custard', 'Filthy ape' ], colonify => 0 } );
    return;
  }
  return;
}
