...
Attribute::Handlers::Clean makes strenuous efforts to convert
...
Occasionally the strenuous efforts Attribute::Handlers::Clean makes to convert
the data argument (C<$_[4]>) to a usable form before passing it to
...
This causes Attribute::Handlers::Clean to define the C<Roo> attribute in the package
...
Damian Conway (damian@conway.org), 90% of the code that makes this module work.
Francisco Zarabozo (zarabozo@cpan.org), adaptation from the excellent L<Attribute::Handlers> to avoid touching L<UNIVERSAL>.
