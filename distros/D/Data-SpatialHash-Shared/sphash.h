 ...
 * Layout: Header -> reader_slots[1024] -> occ bitmap -> buckets -> bitmap -> entries
 */
 ...
 * cleared on clean release.  A writer scans these SPH_OCC_WORDS words to visit
 * only OCCUPIED slots (O(words + live readers)) instead of all SPH_READER_SLOTS. */
#define SPH_OCC_WORDS   (((SPH_READER_SLOTS) + 63) / 64)   /* 16 for 1024 slots */
#define SPH_OCC_BYTES   ((uint64_t)SPH_OCC_WORDS * 8)      /* 128 bytes */
...
struct SphHeader {
    ...
    uint32_t slotless_rdepth;         /* 84: readers holding with no reader-slot (documented residual).
                                             Was _pad0 (zeroed), so existing images stay compatible. */
...
typedef struct SpatialHandle {
    ...
    uint64_t      *occ;          /* SPH_OCC_WORDS-word slot-occupancy bitmap (trusted layout offset) */
    uint32_t      *buckets;
    uint64_t      *bitmap;
    SpatialEntry  *entries;
    uint32_t       max_entries;  /* cached at attach (trusted, validated); never reload from peer-writable hdr for a bound/index */
    uint32_t       num_buckets;  /* cached at attach (trusted, validated); drives the bucket mask + bucket-array bound */
...
static inline uint32_t sph_bucket_of_cell(const SpatialHandle *h, const int64_t cell[3]) {
    return (uint32_t)(XXH3_64bits(cell, 3 * sizeof(int64_t)) & (h->num_buckets - 1));
}
...
#define SPH_LOCK_TIMEOUT_SEC  2  /* FUTEX_WAIT timeout for stale-lock detection / drain re-scan */
...
static inline void sph_occ_set(SpatialHandle *h, uint32_t s) {
    __atomic_fetch_or(&h->occ[s >> 6], (uint64_t)1 << (s & 63), __ATOMIC_SEQ_CST);
}
static inline void sph_occ_clear(SpatialHandle *h, uint32_t s) {
...
static inline void sph_claim_reader_slot(SpatialHandle *h) {
    ...
    for (uint32_t i = 0; i < SPH_READER_SLOTS; i++) {
        ...
        if (__atomic_compare_exchange_n(&h->reader_slots[s].pid,
            ...
            sph_occ_set(h, s);   /* mark occupied BEFORE any rdlock can bump rdepth */
            h->my_slot_idx = s;
            return;
        }
    }
    ...
    for (uint32_t i = 0; i < SPH_READER_SLOTS; i++) {
        ...
        if (__atomic_compare_exchange_n(&h->reader_slots[i].pid, &expected, now_pid, 0,
            ...
            sph_occ_set(h, i);   /* mark occupied BEFORE any rdlock can bump rdepth */
            h->my_slot_idx = i;
            return;
        }
    }
...
static inline void sph_unpark(SpatialHandle *h) {
...
static inline void sph_rdepth_inc(SpatialHandle *h) {
...
static inline void sph_rdepth_dec(SpatialHandle *h) {
...
static inline void sph_reader_wake_drain(SpatialHandle *h) {
...
static inline void sph_rwlock_rdunlock(SpatialHandle *h) {
    sph_rdepth_dec(h);                 /* RELEASE: drop our entire contribution */
    sph_reader_wake_drain(h);          /* if a writer is draining, wake it to re-scan */
}
 ...
 * Header -> reader_slots[] -> occ bitmap -> buckets -> bitmap -> entries. */
typedef struct { uint64_t reader_slots, occ, buckets, bitmap, entries; } SphLayout;
...
static inline SphLayout sph_layout(uint32_t max_entries, uint32_t num_buckets) {
    ...
    L.occ          = L.reader_slots + (uint64_t)SPH_READER_SLOTS * sizeof(SphReaderSlot);
    L.buckets      = L.occ + SPH_OCC_BYTES;
...
static inline SpatialHandle *sph_setup(void *base, size_t map_size,
    ...
    h->hdr          = hdr;
    h->reader_slots = (SphReaderSlot *)((uint8_t *)base + sizeof(SphHeader));  /* trusted layout, not the peer-writable header offset */
    h->occ          = (uint64_t *)((uint8_t *)base + sph_layout(hdr->max_entries, hdr->num_buckets).occ);  /* trusted layout offset (occ offset is capacity-independent) */
    ...
    /* Freeze the two geometry fields at attach.  The header was validated
     * (sph_validate_header / sph_validate_create_args) so these match the mmap
     * size exactly; a later lock-violating peer that corrupts hdr->max_entries
     * or hdr->num_buckets cannot then drive an OOB access through the cached
     * copies (CWE-125/787). */
    h->max_entries  = hdr->max_entries;
...
static inline int sph_validate_world(const double *world, double cell_size, char *errbuf) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (world[i] > 0.0) {
            ...
            if (nx > (double)SPH_CELL_LIMIT) {   /* keep the setup (int64_t)floor cast well below 2^63 (UB) */
                SPH_ERR("wrap extent too large: cells per axis exceeds 2^62"); return 0;
            }
        }
    }
    return 1;
...
static SpatialHandle *sph_create(const char *path, uint32_t max_entries,
    ...
    if (anonymous) {
        ...
        if (is_new && (st.st_uid != geteuid() || fchmod(fd, mode) < 0)) {
            SPH_ERR("%s: refusing to initialize file not owned by us", path);
            flock(fd, LOCK_UN); close(fd); return NULL;
        }
...
/* Map a caller-supplied handle to an entry index without truncating the high
 * bits: an out-of-uint32 value becomes UINT32_MAX, which is never a live slot
 * (indices run 0..max_entries-1, max_entries <= UINT32_MAX). Prevents a handle
 * like 2**32+h from aliasing the live slot h. */
static inline uint32_t sph_idx(uint64_t handle) {
    return handle > 0xFFFFFFFFULL ? UINT32_MAX : (uint32_t)handle;
}

static inline int sph_is_live(const SpatialHandle *h, uint32_t idx) {
    if (idx >= h->max_entries) return 0;
...
static inline void sph_bucket_link(SpatialHandle *h, uint32_t idx) {
    ...
    if (head != SPH_NONE && head >= h->max_entries) head = SPH_NONE;
    h->entries[idx].prev = SPH_NONE;
...
static void sph_clear_locked(SpatialHandle *h) {
    uint32_t me = h->max_entries, nb = h->num_buckets;
...
static void sph_chain_stats(SpatialHandle *h, uint32_t *occupied, uint32_t *max_chain,
                            uint32_t *max_cell) {
    uint32_t occ = 0, mx = 0, mxc = 0, nb = h->num_buckets;
    uint32_t me = h->max_entries;
