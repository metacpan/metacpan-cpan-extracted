...
1;
# ABSTRACT: Right-trim (remove trailing whitespace) values and/or fields in CSV
...
App::CSVUtils::csv_rtrim - Right-trim (remove trailing whitespace) values and/or fields in CSV

=head1 VERSION

This document describes version 1.023 of App::CSVUtils::csv_rtrim (from Perl distribution App-CSVUtils), released on 2023-03-31.
...
Usage:
 ...
 csv_rtrim(%args) -> [$status_code, $reason, $payload, \%result_meta]

Right-trim (remove trailing whitespace) values andE<sol>or fields in CSV.

Examples:

=over

=item * Right-trim values in a CSV:

 csv_rtrim(input_filename => "FILE.csv");
...
=item * B<trim_fields> => I<bool>

Whether also to trim the field names.

=item * B<trim_values> => I<bool> (default: 1)

Whether to trim the values in the data rows.
