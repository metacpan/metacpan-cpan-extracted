...
require XSLoader;
XSLoader::load('pEFL::Elm::Spinner');
...
=head1 SYNOPSIS
  ...
  use pEFL::Elm;
  [...]
  my $widget = pEFL::Elm::Spinner->add($parent);
...
For instructions, how to use pEFL::Elm::Spinner, please study this API reference for now. A perl-specific documentation will perhaps come in later versions. But applying the C documentation should be no problem. pEFL::Elm::Spinner gives you a nice object-oriented interface that is kept close to the C API. Please note, that the perl method names remove the "elm_spinner_" at the beginning of the c functions.
