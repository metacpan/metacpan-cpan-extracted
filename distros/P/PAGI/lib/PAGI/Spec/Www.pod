...
PSGI's flat array format is more compatible with C<<< HTTP::Headers->flatten() >>>. If you need to interoperate:
 ...
 # PAGI tuples â flat array (for HTTP::Headers)
 my @flat = map { @$_ } @$pagi_headers;
 my $hh = HTTP::Headers->new(@flat);
 
 # Flat array â PAGI tuples
...
The server does not parse the cookie string into key-value pairs â parsing is left to middleware or application code. The server only guarantees RFC-compliant normalization.
...
C<pagi["spec_version"]> (String) -- PAGI HTTP spec version (default C<"0.1">)
...
Servers SHOULD use C<sendfile()> or similar zero-copy mechanisms when available


=item -

Servers MAY fall back to chunked read/write if C<sendfile()> is unavailable or inappropriate
...
Any subsequent C<$send> invocation B<must> fail its returned C<Future> (or throw) with a Perl exception class that indicates the disconnect (e.g., C<PAGI::Error::Disconnected>). Servers MUST NOT expose Python exceptions such as C<OSError>.
...
C<REQUEST_METHOD> â C<method>


=item -

C<SCRIPT_NAME> â C<root_path>


=item -

C<PATH_INFO> â C<path> minus C<root_path>


=item -

C<QUERY_STRING> â C<query_string>


=item -

C<CONTENT_TYPE> â extracted from C<headers>


=item -

C<CONTENT_LENGTH> â extracted from C<headers>


=item -

C<SERVER_NAME>, C<SERVER_PORT> â C<server>


=item -

C<REMOTE_ADDR>, C<REMOTE_PORT> â C<client>


=item -

C<SERVER_PROTOCOL> â C<http_version>


=item -

C<psgi.url_scheme> â C<scheme>


=item -

C<psgi.version> â C<[1, 1]> (PAGI servers MUST advertise the PSGI version they emulate when bridging)


=item -

C<psgi.input> â constructed from C<http.request> events


=item -

C<psgi.errors> â handled by the server as appropriate (often tied to the loop's logging sink)


=item -

C<psgi.streaming>, C<psgi.nonblocking>, C<psgi.multithread>, C<psgi.multiprocess> â derived from PAGI server capabilities and advertised via PSGI adapter docs
