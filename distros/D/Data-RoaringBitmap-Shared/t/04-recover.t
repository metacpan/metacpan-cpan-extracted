...
{ my $b = Data::RoaringBitmap::Shared->new($p, 256); }
my $total = -s $p;
unlink $p;
...
# 1. Recovery: an abandoned all-zero file of exactly $total bytes is re-initialized.
{
    ...
    my $b = eval { Data::RoaringBitmap::Shared->new($p, 256) };
    ok($b, "new() recovers an abandoned mid-init file instead of bricking") or diag $@;
  SKIP: {
        skip "no handle", 4 unless $b;
        is($b->cardinality, 0, "recovered bitmap starts empty (cardinality 0)");
        ok($b->is_empty, "recovered bitmap starts empty (is_empty)");
        is($b->add(42), 1, "add(42) on the recovered bitmap returns 1 (new)");
        ok($b->contains(42), "recovered bitmap works (add/contains)");
    }
    undef $b; unlink $p;
}
...
{
    my $a = Data::RoaringBitmap::Shared->new($p, 256); $a->add(999); undef $a;
    my $r = Data::RoaringBitmap::Shared->new($p, 256);
    ok($r->contains(999), "reopening a valid file preserves its data");
    undef $r; unlink $p;
}
