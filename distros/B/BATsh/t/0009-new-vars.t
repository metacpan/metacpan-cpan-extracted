...
my @tests = (
    ...
    sub {
        ...
        _ok(($out =~ /\AABC\s*\z/) ? 1 : 0, 'NV01: %VAR:~0,3% first 3 chars');
    },
    ...
    sub {
        ...
        _ok(($out =~ /\ADEFGH\s*\z/) ? 1 : 0, 'NV02: %VAR:~3% from offset to end');
    },
    ...
    sub {
        $BATsh::Env::STORE{'NV_STR'} = 'ABCDEFGH';
        my $out = _capture(sub { BATsh->run_string('ECHO %NV_STR:~-3%') });
        _ok(($out =~ /\AFGH\s*\z/) ? 1 : 0, 'NV03: %VAR:~-3% last 3 chars');
    },
    ...
    sub {
        $BATsh::Env::STORE{'NV_STR'} = 'ABCDEFGH';
        my $out = _capture(sub { BATsh->run_string('ECHO %NV_STR:~0,-3%') });
        _ok(($out =~ /\AABCDE\s*\z/) ? 1 : 0, 'NV04: %VAR:~0,-3% all but last 3');
    },
    ...
    sub {
        ...
        _ok(($out =~ /\ACDEF\s*\z/) ? 1 : 0, 'NV05: %VAR:~2,4% 4 chars from offset 2');
    },
    ...
    sub {
        ...
        _ok(($out =~ /\A\[\]\s*\z/) ? 1 : 0, 'NV06: %VAR:~99% beyond length returns empty');
    },
    ...
    sub {
        ...
        _ok(($out =~ /\A\[\]\s*\z/) ? 1 : 0, 'NV07: %VAR:~1,0% zero length returns empty');
    },
    ...
    sub {
        ...
        _ok(($out =~ /\Ahello there\s*\z/) ? 1 : 0, 'NV09: %VAR:str1=str2% basic substitution');
    },
    ...
    sub {
        ...
        _ok(($out =~ /\Ahello world\s*\z/) ? 1 : 0, 'NV10: %VAR:notfound=x% returns original');
    },
    ...
    sub {
        ...
        _ok(($out =~ /\Ahelloworld\s*\z/) ? 1 : 0, 'NV11: %VAR:str=% deletes substring');
    },
    ...
    sub {
        ...
        _ok(($out =~ /\AHi World\s*\z/) ? 1 : 0, 'NV12: %VAR:str1=str2% case-insensitive match');
    },
    ...
    sub {
        ...
        _ok(($out =~ /\AAFTERtwo:three\s*\z/) ? 1 : 0, 'NV14: %VAR:*str=val% star-prefix substitution');
    },
    ...
    sub {
        ...
        $out =~ s/\r?\n\z//;
        _ok(($out =~ /\A\d{4}-\d{2}-\d{2}\z/) ? 1 : 0, 'NV15: %DATE% expands to YYYY-MM-DD');
    },
    ...
    sub {
        ...
        $out =~ s/\r?\n\z//;
        _ok(($out =~ /\A\d{2}:\d{2}:\d{2}/) ? 1 : 0, 'NV16: %TIME% expands to HH:MM:SS...');
    },
    ...
    sub {
        ...
        _ok(($out =~ /EL=3/) ? 1 : 0, 'NV19: %ERRORLEVEL% reflects current ERRORLEVEL');
    },
    ...
    sub {
        ...
        _ok(($out =~ /\.t/) ? 1 : 0, 'NV20: SH echo *.t expands glob to .t files');
    },
    ...
    sub {
        ...
        $out =~ s/\r?\n\z//;
        _ok(($out =~ /zzz_no_such/) ? 1 : 0, 'NV21: SH glob no-match returns literal pattern');
    },
    ...
    sub {
        ...
        chdir($orig);
        _ok(($out =~ /\.t/) ? 1 : 0, 'NV22: SH for f in *.t iterates .t files');
    },
    ...
    sub {
        ...
        _ok(($out =~ /000\d-.*\.t/) ? 1 : 0, 'NV23: SH glob ? matches single character');
    },
