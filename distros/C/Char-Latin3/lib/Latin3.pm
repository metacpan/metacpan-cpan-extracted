...
######################################################################
#
# Latin3 - Source code filter to escape Latin-3 script
#
...
perl $0 Latin-3_script.pl > Escaped_script.pl.e
END
    }

    print Latin3::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Elatin3::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
    ...
    my $fh = gensym();
    Elatin3::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub Latin3::escape_script {
    ...
    # read Latin-3 script
    my $fh = gensym();
    Elatin3::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script\n";

    if (/^ use Elatin3(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Elatin3 '%s.0'; # 'quote' for perl5.6\n", $Latin3::VERSION); # require run-time routines version

        # use Latin3 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Latin3 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Latin3::VERSION) {
                    my @file = grep -e, map {qq{$_/Latin3.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Latin3.pm $version, but @{[__FILE__]} is version $Latin3::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Elatin3.pm $version, but \\\$Elatin3::VERSION is \$Elatin3::VERSION" if \$Elatin3::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Latin3::VERSION) {
                    die "Script $0 required Latin3.pm $version, but @{[__FILE__]} is only version $Latin3::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Elatin3::function_ord     = 'Latin3::ord'     if /\A ord \z/oxms;
                    $Elatin3::function_ord_    = 'Latin3::ord_'    if /\A ord \z/oxms;
                    $Elatin3::function_reverse = 'Latin3::reverse' if /\A reverse \z/oxms;
                    $Elatin3::function_getc    = 'Latin3::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Latin3::escape();
...
=item * You'll have to download Latin3.pm and Elatin3.pm and put it in your perl lib directory.

=item * You'll need to write "use Latin3;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Latin3;
  use Latin3 ver.sion;             --- require minimum version
  use Latin3 ver.sion.0;           --- expects version (match or die)

  # "no Latin3;" not supported

  or

  $ perl Latin3.pm Latin-3_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Latin-3_script.pl  --- script written in Latin-3
...
Latin3 software is "middleware" between perl interpreter and your Perl script
written in Latin-3.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Latin-3, Perl3 or later
can treat Latin-3 as binary data.

By "use Latin3;", it automatically interpret your script as Latin-3. The various
functions of perl including a regular expression can treat Latin-3 now.
...
This software is a source code filter to escape Perl script encoded by Latin-3
...
=item * Remains One Language Handling Raw Latin-3, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Latin3.pm, Elatin3.pm
  +---------------------------------------+
...
import of Elatin3.pm works well.
...
=head1 Software Composition
   ...
   Latin3.pm               --- source code filter to escape Latin-3
   Elatin3.pm              --- run-time routines for Latin3.pm
...
You need write 'use Latin3;' in your script.
...
=head1 Calling 'Elatin3::ignorecase()' (Latin3 software provides)

Latin3 software applies calling 'Elatin3::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Elatin3::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Elatin3.pm provides)
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Latin3 software provides)

Latin3 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Latin3 software provides)

Insert 'Elatin3::' at head of function name. Elatin3.pm provides your script Elatin3::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Elatin3::lc        Character
  lcfirst     Elatin3::lcfirst   Character
  uc          Elatin3::uc        Character
  ucfirst     Elatin3::ucfirst   Character
  fc          Elatin3::fc        Character
  chr         Elatin3::chr       Character
  glob        Elatin3::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Latin3 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Latin3 software provides)

Latin3 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Elatin3.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Latin3 software and
...
Latin3 software remains one language and one interpreter.
