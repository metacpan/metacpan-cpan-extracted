...
Happily, L<Mail::Box|Mail::Box> can be used as spam filter, in combination with
...
=item label => $label|undef

Mark all selected message with the specified C<$label>.  If this
...
=item sa_options => \%options

The C<%options> to create the internal Mail::SpamAssassin object; see its
...
Provide a Mail::SpamAssassin C<$object> to be used for searching spam.  If
