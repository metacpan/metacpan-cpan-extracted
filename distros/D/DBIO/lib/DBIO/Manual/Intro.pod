...
=head2 Using DBIO::Generate

L<DBIO::Generate> is part of the L<DBIO> distribution. It inspects your
database, and automatically creates classes for all the tables in your
schema.

The simplest way to use it is via the L<dbiogen> script. For example:
...
If you have a mixed-case database, use the C<preserve_case> option, e.g.:
    ...
    $ dbiogen -o dump_directory=./lib -o preserve_case=1 \
        -o components='["InflateColumn::DateTime"]' \
        MyApp::Schema dbi:mysql:mydb user pass
...
See the L<DBIO::Generate> documentation for more information on the many
generation options.
...
L<DBIO::Generate>, you can start interacting with your database.
