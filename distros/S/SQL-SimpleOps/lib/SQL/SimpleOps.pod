...
Before any implementation, we recommend seeing the section B<DESCRIPTION> for considerations and restrictions.
...
where the current version has built-in support for the databases: I<MySQL>, I<MariaDB>, I<Postgres> and I<SQLite3>.
...
however this process can be suppressed if requested and the SQL command can retrieve (if neeeded, see: C<make_only> option).
...
The controls can initialized by the Options Arguments and/or L<Configuration File>.
...
=head3 Loading Data into Applications
X<Loading Data into Applicatons>

Exists two different process to manipulate the application data with the module.
The first put data into the database (C<Insert> and C<Update>) and the second get data from the database (C<Select> and C<SelectCursor>).
...
The Aliases Table is not mandatory, use it you necessary. See B<EXAMPLES>

When mapped the B<Aliases Column> these aliases will be the keys in the C<hashref buffers>

B<Why should I?> Some companies disallow tables using generic names (out of the norms), in this case, you can use aliases to abstract the real name in your the databases.
...
B<NOTE:> This last will not assign the alias names in the SQL Command.

=head2 Escape or Backslash

The C<escape> operand (or C<backslash>) is the format used to force the methods to ignore the translating data as arguments.
...
B<NOTE:> The escape is valid in the 'where' clause of C<Select>, C<SelectCursor>, C<Delete> and C<Update> command.
...
B<NOTE:> The C<on duplicate> is I<MySQL/MariaDB> database engine, for I<SQLite> and I<Postgres> will be C<on conflict>.
...
We belive that the most important situations will was created,
however we understand that some format will not be supported.

The Where Clause was be create based C<array_ref>, no others options exists.
...
using this point of view the best way is this type of struct.

If you any item (or sub-item) specifies C<hash_ref>, the process is aborted with C<SQL_SIMPLE_RC_SYNTAX> as the return code.
...
=item B<commit:> Enable/disable commit after updates.
...
=item B<db:> The name of database name. The value depend of type of interface and driver option. see L<DBI> module.

Defaults: no defaults

=item B<driver:> Sets the name of database engine. see L<DBI> module.

Defaults: no defaults

=item B<interface:> Sets the interface module to use.
The current version support only C<interface=dbi>.

Defaults: The defaul value is C<dbi>.

=item B<interface_options:> See options for L<DBI> module.

Defaults: The default values for C<interface=dbi> are: C<RaiseError=0> and C<PrintError=0>.

=item B<login:> Sets the user/profile login for authenticated connection.
For security reasons and best practices, the module expects that all connections will be authenticated. 

Defaults: no defaults

=item B<message_log:> Sets message log mode, can be:
...
=item B<message_syslog_service:> Sets an identifier string on the System Syslog Messages.
see: L<Sys::Syslg>

Defaults: The default value is C<message_syslog_service=SQL-SimpleOps>

=item B<message_syslog_facility:> Sets the Syslog Facility for the messages on the System Syslog Messages, must be: C<local0> to C<local7>.
see: L<Sys::Syslg>

Defaults: The default value is C<local0>.

=item B<password:> The password/profile for authentication process.

Defaults: no defaults

=item B<port:> This option consists the TCP Port to use for connection.
...
=item B<quote:> Sets the quote caracter on string commands (see C<setQuote> method), must be: apostrophe or quote.

Defaults: apostrophe.

=item B<server:> Sets the hostname or ip address of hosted database server.

Defaults: no defaults.

=item B<sql_save:> Sets the SQL log file mode, can be:
...
=item B<sql_save_bydate:> Sets enable/disable writing log files into distinct folders.
...
=item B<sql_save_dir:> Sets the folder for writes log file process.

Defaults: (unix/linux) F</var/spool/sql/> or (windows) F<c:\windows\temp>

B<NOTE:> You must create the F</var/spool/sql/> as I<mode drwxrwxrwt>.

=item B<sql_save_name:> Sets the filename string  for writes log file process.

Defaults: The default value is C<sql>.

=item B<sql_save_ignore:> Sets enable/disable option to abort if there are errors wrtten to the Log File.

Defaults: The default value is disabled. If errors the return code will be C<SQL_SIMPLE_RC_ERROR>.

=item B<tables:> Sets the list of aliases rules for the tables and fields. See L<Aliases Table>.
...
=item B<commit:> Enable/Disable the commit after a specific update command.

=item B<message_log:> Enable/Disable the written on System Syslog Services.

=item B<quote:> Change the quote character.

=item B<sql_save:> Enable/Disable the written for SQL Log Files.

This Options can be modified with each SQL Command execution.
...
=item B<sql_save_name:> Consists in the C<sql_save_name> option value.

=item B<database_name:> Consists in the C<my_db> database value.

=item B<today:> Current day formated as C<YYYYmmdd> (year+month+day).

=item B<pid:> Current Pid (system process identifier) in execution.

=item B<counter:> Indexer for each SQL Command executed by the Current Pid.
...
F</var/tmp/sql/YYYY/YYYYmm/YYYYmmdd/[sql_save_name].[database_name].[today].[pid].[counter]>
...
B<When can i use it?>
The C<commit> option can be used at the end of a long process of updates,
...
=head3 Openning Interface MySQL/MariaDB
...
=head3 Openning Interface SQLite
X<Interface SQLite>
...
=head3 Openning Interface Postgres
...
B<NOTE:> This command force the return code to C<SQL_SIMPLE_RC_OK> if ther is no match, where without it the C<notfound> option must be C<SQL_SIMPLE_RC_EMPTY>.
...
=item B<cursor_command:> Sets the type of command to be executed in the search, must be:

=over

=item B<SQL_SIMPLE_CURSOR_TOP:> Go to the first page of the search.
No C<cursor_info> will be used.

=item B<SQL_SIMPLE_CURSOR_LAST:> Go to the last page search.
No C<cursor_info> will be used.

=item B<SQL_SIMPLE_CURSOR_NEXT:> Go to the next search page.
...
=item B<SQL_SIMPLE_CURSOR_BACK:> Go to the pervious search page.
Will use the 'first' info on C<cursor_info> option.
If 'cursor' is missing (or undef) the module will search as 'SQL_SIMPLE_CURSOR_LAST' command.

=item B<SQL_SIMPLE_CURSOR_RELOAD:> Reload current cursor.
Will use the 'first' info on C<cursor_info> option.

=back

Defaults: C<SQL_SIMPLE_CURSOR_TOP>

=item B<cursor:> Sets an specific starter cursor value for the search.
This options is not mandatory.
...
=item B<cursor_key:> Sets the key that will use to search.
You must use only one key, multiple keys are not supported.

Defaults: no defaults

=item B<cursor_info:> Sets the return buffer to getting the page controls.
...
=item B<rc:> Consists the return code of SQL Command.

=item B<no_lines:> Consists the number of extracted lines.

=item B<first:> Consists the first key value.

=item B<last:> Consists the last key value.

=back

B<BEWARE:> For keys that use I<space values> do not use the I<scalar_ref> format to capture the C<first_key> and C<last_key>.

=item B<limit:> Sets the maximum number of lines to retrieve.
...
=item B<command:> Consists of the SQL Command to be executed (fully formatted)

=item B<buffer:> Consists of the Return Buffer Area, where the method will written the columns and rows, can be:
...
B<flush:> Force the clean up in the return buffer area. The default value is enabled, all buffer area will be initialized.
...
B<NOTE:> This method can be used on B<PLUGINS> to help to translate the aliases names.
...
=head2 005E [I<command>] Table is missing
X<005E>
...
B<Error:> The table alias name omitted or empty.

=item *

B<Action:> Check the Command and validate the C<table> option.
Remember: the you must specify the aliases table name assigned on contents table.
...
=head2 010E [I<command>] Table/Field Index invalid
X<010E>
...
B<Error:> The indexed field is using a non-valid table/field identifier.

=item *

B<Action:> The table must specified on the C<table> option. The C<fields> is omitted, have multiples tables on the C<table> option and one or more tables does not exists on the L<Aliases Table>. Check the aliases of the table names specified in the fields in use of the C<where> option.
...
=head2 021E [I<command>] Field invalid, must be single-value or array
X<021E>
...
L<DBD::mysql>, L<DBD::Pg>, L<DBD::SQLite>, L<DBI>, L<Date::Calc>, L<File::Path>, L<File::Spec>, L<JSON>, L<Sys::Syslog>
