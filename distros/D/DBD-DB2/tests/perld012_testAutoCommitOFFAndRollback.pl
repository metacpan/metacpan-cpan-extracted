####################################################################
# TESTCASE: 		perld012_testAutoCommitOFFAndRollback.pl
# DESCRIPTION: 		Test autocommit off and then Rollback
...
if ($DBI::err == 0)
{
  $stmt = "UPDATE staff SET job = 'Mgr' where id = 400";
  ...
  #
  # Verify that the UPDATE statement is rolled back
  #
