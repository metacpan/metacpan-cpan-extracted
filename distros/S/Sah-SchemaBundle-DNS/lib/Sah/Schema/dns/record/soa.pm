...
This document describes version 0.003 of Sah::Schema::dns::record::soa (from Perl distribution Sah-SchemaBundle-DNS), released on 2024-06-26.
...
=head1 SAH SCHEMA DEFINITION
 ...
 [
   ...
   {
     "keys.restrict"      => 1,
     "merge.add.req_keys" => ["serial", "refresh", "retry", "expire", "email"],
     "merge.normal.keys"  => {
                               name    => ["str", { req => 1, is => "" }],
                               email   => ["email", { req => 1 }],
                               expire  => ["uint", { req => 1 }],
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("dns::record::soa*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("dns::record::soa", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("dns::record::soa", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("dns::record::soa*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"dns::record::soa*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('dns::record::soa*', name=>'DnsRecordSoa')
     );
 }

 use My::Types qw(DnsRecordSoa);
 DnsRecordSoa->assert_valid($data);
