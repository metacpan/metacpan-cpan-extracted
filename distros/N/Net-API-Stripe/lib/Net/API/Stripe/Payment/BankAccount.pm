...
On the other hand External Accounts (C<Net::API::Stripe::Connect::Account::ExternalAccounts> / L<https://stripe.com/docs/api#external_accounts>) are transfer destinations on Account objects for Custom accounts (L<https://stripe.com/docs/connect/custom-accounts>). They can be bank accounts or debit cards as well, and are documented in the links above.

That being said, all the methods are exactly the same as C<Net::API::Stripe::Connect::ExternalAccount::Bank> so this module inherits 100% from it.
