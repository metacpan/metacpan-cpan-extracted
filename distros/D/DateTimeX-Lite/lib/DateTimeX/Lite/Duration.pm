...
sub _normalize_nanoseconds
{
    ...
    return if
        ( $self->{nanoseconds} == DateTimeX::Lite::INFINITY()
          || $self->{nanoseconds} == DateTimeX::Lite::NEG_INFINITY()
          || $self->{nanoseconds} eq DateTimeX::Lite::NAN()
        );
...
sub compare
{
    ...
    $dt ||= DateTimeX::Lite->now;

    return
        DateTimeX::Lite->compare( $dt->clone->add_duration($dur1), $dt->clone->add_duration($dur2) );
}
...
sub _add_overload
{
    ...
    if ( blessed $d2 && $d2->isa( 'DateTimeX::Lite' ) )
    {
        $d2->add_duration($d1);
        return;
    }

    # will also work if $d1 is a DateTimeX::Lite.pm object
...
sub _subtract_overload
{
    ...
    Carp::croak( "Cannot subtract a DateTimeX::Lite object from a DateTimeX::Lite::Duration object" )
        if blessed $d2 && $d2->isa( 'DateTimeX::Lite' );
...
{
    Carp::croak( 'DateTimeX::Lite::Duration does not overload comparison.'
...
objects are used whenever you do date math with DateTimeX::Lite.pm.

See the L<How Date Math is Done|DateTimeX::Lite/"How Date Math is Done">
section of the DateTimeX::Lite.pm documentation for more details.  The short
...
the appropriate subtraction/delta method on a C<DateTimeX::Lite.pm> object.

=head1 METHODS

Like C<DateTimeX::Lite> itself, C<DateTimeX::Lite::Duration> returns the object from
mutator methods in order to make method chaining possible.

C<DateTimeX::Lite::Duration> has the following methods:
...
after taking away the larger units given, so for example:
  ...
  my $dur = DateTimeX::Lite::Duration->new( years => 1, months => 15 );
...
use the C<DateTimeX::Lite::Format::Duration> module.
