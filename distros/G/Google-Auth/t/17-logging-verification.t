...
subtest 'ServiceAccountCredentials Logging' => sub {
  ...
  $log->contains_ok(
qr/Access token is missing or expired for Google::Auth::ServiceAccountCredentials/,
    'info log for missing token check is present'
  );
  $log->contains_ok(
    qr/Signing JWT assertion for service account test-sa\@google.com/,
    'trace log for JWT signing is present');
  $log->contains_ok(qr/Exchanging signed JWT assertion for access token at/,
    'info log for exchange start is present');
  $log->contains_ok(
qr/Successfully refreshed access token for Google::Auth::ServiceAccountCredentials/,
    'info log for exchange completion is present'
  );
};
...
subtest 'RetryHelper and Error Logging' => sub {
  ...
  $log->contains_ok(
    qr/Transient error on attempt 1\/2/,
    'warn log for transient retry is present'
  );
  $log->contains_ok(
    qr/Max retry attempts \(2\/2\) reached/,
    'error log for max retries failure is present'
  );
};
...
'"%s" -e print+qq({\"version\":1,\"success\":true,\"expiration_time\":1234567890,\"id_token\":\"mock_pluggable_token\"})',
  ...
  $log->contains_ok(
    qr/Executing Pluggable credential command/,
    'info log for executable command start is present'
  );
  $log->contains_ok(
    qr/Parsing JSON output from Pluggable command/,
    'trace log for JSON parsing is present'
  );
  $log->contains_ok(
    qr/Pluggable subject token retrieved successfully/,
    'trace log for retrieval completion is present'
  );
};
