package Bot::BasicBot::Pluggable::Module::Zippy;
...
sub said { 
    ...
    return unless $body =~ /^yow[!? ]*$/i or $body =~ /^be zippy\?*$/i;

    return unless ($pri == 2);

    unless (@yows) {
        while (<DATA>) { chomp; next if /^\s*$/; push @yows, $_; }
    }

    return $yows[rand(@yows)];
}
...
Bot::BasicBot::Pluggable::Module::Zippy - module for Zippy the Pinhead quotes
...
based on code by Rich Lafferty (mendel) <rich@vax2.concordia.ca>

and data from fortune-mod-9708
