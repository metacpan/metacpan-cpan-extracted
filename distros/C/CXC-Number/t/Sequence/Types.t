  ...
  )
{
    ...
    like( dies { $type->assert_coerce( $exp ) }, qr/.*/, "bad $type: $exp" );
  ...
  )
{
    ...
    ok( lives { $got = $type->assert_coerce( $exp ) }, "good $type: $exp" )
      or diag $@;
    ...
    [ 2 => [ 2, 0.5 ] ] )
{
    ...
    my $ins = Dumper( $in );
    my $got;
    ok( lives { $got = Alignment->assert_coerce( $in ) }, "coerce: $ins" )
      or diag $@;
...
subtest 'Sequence' => sub {
    ...
    isa_ok(
        dies { Sequence->assert_coerce( [ 10, 8, 9 ] ) },
        ["Error::TypeTiny::Assertion"],
        "out of order"
    );
};
