...
#
package PDLA::Compression;

@EXPORT_OK  = qw( PDLA::PP rice_compress PDLA::PP rice_expand );
...
These routines generally accept some data as a PDLA and compress it
into a smaller PDLA.  Algorithms typically work on a single dimension
...
RICE_1 algorithm used in internal FITS-file compression (see PDLA::IO::FITS).


=head1 SYNOPSIS

 use PDLA::Compression
...
Squishes an input PDLA along the 0 dimension by Rice compression.  In
scalar context, you get back only the compressed PDLA; in list context,
...
separately, and the returned PDLA is squished to the maximum compressed
...
CFITSIO (and is used by PDLA::IO::FITS to load and save compressed FITS
images).
...
sub PDLA::rice_compress {
    ...
    &PDLA::_rice_compress_int( $in, $out, $len, $lbuf, $blocksize  );
...
*rice_compress = \&PDLA::rice_compress;
...
Unsquishes a PDLA that has been squished by rice_compress.
...
sub PDLA::rice_expand {
   ...
   &PDLA::_rice_expand_int( $squished, $out, $lbuf, $blocksize );

   return $out;
}


*rice_expand = \&PDLA::rice_expand;
...
terms than PDLA itself; that notice is present in the file "ricecomp.c".
