...
PPCODE:
    increment_metric_( move(self), move(key), move(value) );
    XSRETURN_EMPTY;
...
PPCODE:
    if ( SVt_NULL != SvTYPE(key) ) {
        ...
        self->invalid_key_counter = 0;
        increment_hash_value_by( move(self->bulk_hv), move(key), move(counter) );
    }
    XSRETURN_EMPTY;
...
PPCODE:
    ...
    if ( is_valid_key_(self, key) )
        is_blocked = is_metric_blocked_(move(self), move(key));
