...
cmp_deeply(
   ...
   [qw(sc config foo DisplayName= Foo type= own start= auto obj= domain\wes)],
   'sc configure auth ok',
);

cmp_deeply(
   [$sm->_sc_configure('foo', {display => 'Foo', user => 'wes', password => 'gnarl'})],
   [qw(sc config foo DisplayName= Foo type= own start= auto obj= wes password= gnarl)],
   'sc configure auth ok',
);

cmp_deeply(
   [$sm->_sc_configure('foo', {display => 'Foo', start => 'delayed-auto'})],
   [qw(sc config foo DisplayName= Foo type= own start= delayed-auto)],
