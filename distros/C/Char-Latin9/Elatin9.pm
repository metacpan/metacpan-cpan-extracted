...
######################################################################
#
# Elatin9 - Run-time routines for Latin9.pm
#
...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Elatin9::index($name, '::') == -1) && (Elatin9::index($name, "'") == -1)) {
            my $pkg;
...
#
# Latin-9 character range per length
#
my %range_tr = ();
...
sub import {
    ...
    *Char::ord           = \&Latin9::ord;
    *Char::ord_          = \&Latin9::ord_;
    *Char::reverse       = \&Latin9::reverse;
    *Char::getc          = \&Latin9::getc;
    *Char::length        = \&Latin9::length;
    *Char::substr        = \&Latin9::substr;
    *Char::index         = \&Latin9::index;
    *Char::rindex        = \&Latin9::rindex;
    *Char::eval          = \&Latin9::eval;
    *Char::escape        = \&Latin9::escape;
    *Char::escape_token  = \&Latin9::escape_token;
    *Char::escape_script = \&Latin9::escape_script;
}
...
sub Elatin9::classic_character_class;
sub Elatin9::capture;
sub Elatin9::chr(;$);
sub Elatin9::chr_();
sub Elatin9::glob($);
...
# ${Elatin9::eS}          = qr{(?>[^\x09\x0A    \x0C\x0D\x20])};
# ${Elatin9::eS}          = qr{(?>[^\x09\x0A\x0B\x0C\x0D\x20])};
${Elatin9::eS}            = qr{(?>[^\s])};
...
# avoid: Name "Elatin9::foo" used only once: possible typo at here.
${Elatin9::dot}         = ${Elatin9::dot};
${Elatin9::dot_s}       = ${Elatin9::dot_s};
${Elatin9::eD}          = ${Elatin9::eD};
${Elatin9::eS}          = ${Elatin9::eS};
${Elatin9::eW}          = ${Elatin9::eW};
${Elatin9::eH}          = ${Elatin9::eH};
${Elatin9::eV}          = ${Elatin9::eV};
${Elatin9::eR}          = ${Elatin9::eR};
${Elatin9::eN}          = ${Elatin9::eN};
${Elatin9::not_alnum}   = ${Elatin9::not_alnum};
${Elatin9::not_alpha}   = ${Elatin9::not_alpha};
${Elatin9::not_ascii}   = ${Elatin9::not_ascii};
${Elatin9::not_blank}   = ${Elatin9::not_blank};
${Elatin9::not_cntrl}   = ${Elatin9::not_cntrl};
${Elatin9::not_digit}   = ${Elatin9::not_digit};
${Elatin9::not_graph}   = ${Elatin9::not_graph};
${Elatin9::not_lower}   = ${Elatin9::not_lower};
${Elatin9::not_lower_i} = ${Elatin9::not_lower_i};
${Elatin9::not_print}   = ${Elatin9::not_print};
${Elatin9::not_punct}   = ${Elatin9::not_punct};
${Elatin9::not_space}   = ${Elatin9::not_space};
${Elatin9::not_upper}   = ${Elatin9::not_upper};
${Elatin9::not_upper_i} = ${Elatin9::not_upper_i};
${Elatin9::not_word}    = ${Elatin9::not_word};
${Elatin9::not_xdigit}  = ${Elatin9::not_xdigit};
${Elatin9::eb}          = ${Elatin9::eb};
${Elatin9::eB}          = ${Elatin9::eB};
...
#
# Latin-9 lower case first with parameter
#
sub Elatin9::lcfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Elatin9::lc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Elatin9::lc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Elatin9::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}

#
# Latin-9 lower case first without parameter
#
sub Elatin9::lcfirst_() {
    return Elatin9::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
}
...
#
# Latin-9 lower case with parameter
#
sub Elatin9::lc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# Latin-9 lower case without parameter
#
sub Elatin9::lc_() {
    my $s = $_;
    return join '', map {defined($lc{$_}) ? $lc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# Latin-9 upper case first with parameter
#
sub Elatin9::ucfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Elatin9::uc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Elatin9::uc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Elatin9::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}

#
# Latin-9 upper case first without parameter
#
sub Elatin9::ucfirst_() {
    return Elatin9::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
}
...
#
# Latin-9 upper case with parameter
#
sub Elatin9::uc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# Latin-9 upper case without parameter
#
sub Elatin9::uc_() {
    my $s = $_;
    return join '', map {defined($uc{$_}) ? $uc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# Latin-9 fold case with parameter
#
sub Elatin9::fc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# Latin-9 fold case without parameter
#
sub Elatin9::fc_() {
    my $s = $_;
    return join '', map {defined($fc{$_}) ? $fc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# prepare Latin-9 characters per length
#
...
#
# Latin-9 open character list for tr
#
sub _charlist_tr {
...
#
# Latin-9 open character list for qr and not qr
#
sub _charlist {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '${Elatin9::not_lower_i}',
                '[:^upper:]'  => '${Elatin9::not_upper_i}',
...
#
# Latin-9 hexadecimal escape sequence
#
sub hexchr {
    my($hexdigit) = @_;
...
#
# Latin-9 open character list for qr
#
sub charlist_qr {
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Elatin9::uc($char);
                        my $fc = Elatin9::fc($char);
                        if ($uc eq $fc) {
...
#
# Latin-9 open character list for not qr
#
sub charlist_not_qr {
...
#
# Latin-9 order to character (with parameter)
#
sub Elatin9::chr(;$) {
...
#
# Latin-9 order to character (without parameter)
#
sub Elatin9::chr_() {
...
#
# Latin-9 path globbing (with parameter)
#
sub Elatin9::glob($) {
...
#
# Latin-9 path globbing (without parameter)
#
sub Elatin9::glob_() {
...
#
# Latin-9 path globbing via File::DosGlob 1.10
#
...
#
# Latin-9 path globbing subroutine
#
sub _do_glob {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $fc = Elatin9::fc($char)) ne $char) {
                $pattern .= $fc;
            }
            else {
        ...
        my $matchsub = sub { Elatin9::fc($_[0]) =~ /\A $pattern \z/xms };
...
INNER:
        for my $leaf (@leaf) {
            ...
            if (Elatin9::index($leaf,'.') == -1 and   # if name does not have a dot in it *and*
                CORE::length($leaf) <= 8 and        # name is shorter than or equal to 8 chars *and*
                Elatin9::index($pattern,'\\.') != -1  # pattern has a dot.
            ) {
                if (&$matchsub("$leaf.")) {
                    push @matched, "$head$leaf";
                    next INNER;
...
#
sub Elatin9::POSTMATCH {
    return $';
}

#
# Latin-9 character to order (with parameter)
#
sub Latin9::ord(;$) {
...
#
# Latin-9 character to order (without parameter)
#
sub Latin9::ord_() {
...
#
# Latin-9 getc (with parameter, without parameter)
#
sub Latin9::getc(;*@) {

    my($package) = caller;
    my $fh = @_ ? qualify_to_ref(shift,$package) : \*STDIN;
    croak 'Too many arguments for Latin9::getc' if @_ and not wantarray;
    ...
    for my $length ($length[0] .. $length[-1]) {
        ...
        if (exists $range_tr{CORE::length($getc)}) {
            if ($getc =~ /\A ${Elatin9::dot_s} \z/oxms) {
...
#
# Latin-9 index by character
#
sub Latin9::index($$;$) {

    my $index;
    if (@_ == 3) {
        $index = Elatin9::index($_[0], $_[1], CORE::length(Latin9::substr($_[0], 0, $_[2])));
    }
    else {
        $index = Elatin9::index($_[0], $_[1]);
    }
    ...
    if ($index == -1) {
        ...
        return Latin9::length(CORE::substr $_[0], 0, $index);
    }
}

#
# Latin-9 rindex by character
#
sub Latin9::rindex($$;$) {

    my $rindex;
    if (@_ == 3) {
        $rindex = Elatin9::rindex($_[0], $_[1], CORE::length(Latin9::substr($_[0], 0, $_[2])));
    }
    else {
        $rindex = Elatin9::rindex($_[0], $_[1]);
    }
    ...
    if ($rindex == -1) {
        ...
        return Latin9::length(CORE::substr $_[0], 0, $rindex);
    }
}
...
my $qq_substr  = qr{(?> Char::substr | Latin9::substr | CORE::substr | substr ) (?>\s*) \( $qq_paren \)
                 }xms;
...
#
# escape Latin-9 token of script
#
sub Latin9::escape_token {
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Elatin9::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ (?>\s*) PREMATCH  \b | \$ (?>\s*) \{ (?>\s*) PREMATCH  (?>\s*) \} | \$ (?>\s*) \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin9::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Elatin9::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ (?>\s*) MATCH     \b | \$ (?>\s*) \{ (?>\s*) MATCH     (?>\s*) \} | \$ (?>\s*) \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin9::MATCH()};
    }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Elatin9::POSTMATCH()
    elsif (/\G (                 \$ (?>\s*) POSTMATCH \b | \$ (?>\s*) \{ (?>\s*) POSTMATCH (?>\s*) \} | \$ (?>\s*) \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin9::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Elatin9::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while (?>\s*) \( (?>\s*) glob (?>\s*) \) /oxgc) {
        return 'while ($_ = Elatin9::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while (?>\s*) \( (?>\s*) glob \b /oxgc) {
        return 'while ($_ = Elatin9::glob';
    }
...
# subroutines of package Elatin9
    elsif (/\G \b (CORE:: | ->(>?\s*) (?: atan2 | [a-z]{2,})) \b       /oxgc) { $slash = 'm//'; return $1;                  }
    elsif (/\G \b Char::eval       (?= (?>\s*) \{ )                    /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Latin9::eval       (?= (?>\s*) \{ )                    /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval Char::escape'; }
    elsif (/\G \b Latin9::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval Latin9::escape'; }
    elsif (/\G \b bytes::substr \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'substr';            }
    elsif (/\G \b chop \b          (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Elatin9::chop';       }
    elsif (/\G \b bytes::index \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'index';             }
    elsif (/\G \b Char::index \b   (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Char::index';       }
    elsif (/\G \b Latin9::index \b   (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Latin9::index';       }
    elsif (/\G \b index \b         (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Elatin9::index';      }
    elsif (/\G \b bytes::rindex \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'rindex';            }
    elsif (/\G \b Char::rindex \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Char::rindex';      }
    elsif (/\G \b Latin9::rindex \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Latin9::rindex';      }
    elsif (/\G \b rindex \b        (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Elatin9::rindex';     }
    elsif (/\G \b lc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin9::lc';         }
    elsif (/\G \b lcfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin9::lcfirst';    }
    elsif (/\G \b uc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin9::uc';         }
    elsif (/\G \b ucfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin9::ucfirst';    }
    elsif (/\G \b fc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin9::fc';         }
    ...
    elsif (/\G \b chr           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin9::chr';               }
    elsif (/\G \b bytes::ord    (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin9::glob';              }
    elsif (/\G \b lc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin9::lc_';               }
    elsif (/\G \b lcfirst \b       (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin9::lcfirst_';          }
    elsif (/\G \b uc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin9::uc_';               }
    elsif (/\G \b ucfirst \b       (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin9::ucfirst_';          }
    elsif (/\G \b fc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin9::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin9::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! (?>\s*) => )                          /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! (?>\s*) => )                          /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Elatin9::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Elatin9::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Elatin9::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq           (\#) [ ] (\#) /oxgc) { return 'Elatin9::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq ((?>\s*)) (\() [ ] (\)) /oxgc) { return 'Elatin9::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\{) [ ] (\}) /oxgc) { return 'Elatin9::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\[) [ ] (\]) /oxgc) { return 'Elatin9::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\<) [ ] (\>) /oxgc) { return 'Elatin9::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\S) [ ] (\2) /oxgc) { return 'Elatin9::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q            (\#) [ ] (\#) /oxgc) { return 'Elatin9::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  ((?>\s*)) (\() [ ] (\)) /oxgc) { return 'Elatin9::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\{) [ ] (\}) /oxgc) { return 'Elatin9::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\[) [ ] (\]) /oxgc) { return 'Elatin9::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\<) [ ] (\>) /oxgc) { return 'Elatin9::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\S) [ ] (\2) /oxgc) { return 'Elatin9::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                    ' [ ] '    /oxgc) { return 'Elatin9::split' . $e . qq     {' '};  }
        elsif (/\G                    " [ ] "    /oxgc) { return 'Elatin9::split' . $e . qq     {" "};  }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Elatin9::PREMATCH()]}
            ...
            $e_string .= q{Elatin9::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Elatin9::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ (?>\s*) MATCH     \b | \$ (?>\s*) \{ (?>\s*) MATCH     (?>\s*) \} | \$ (?>\s*) \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Elatin9::MATCH()};
            $slash = 'div';
        }
...
# subroutines of package Elatin9
        ...
        elsif ($string =~ /\G \b Latin9::eval       (?= (?>\s*) \{ )                    /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::eval \b                                        /oxgc) { $e_string .= 'eval Char::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b Latin9::eval \b                                        /oxgc) { $e_string .= 'eval Latin9::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::substr \b                                     /oxgc) { $e_string .= 'substr';            $slash = 'm//'; }
        elsif ($string =~ /\G \b chop \b                                              /oxgc) { $e_string .= 'Elatin9::chop';       $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                                      /oxgc) { $e_string .= 'index';             $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::index \b                                       /oxgc) { $e_string .= 'Char::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b Latin9::index \b                                       /oxgc) { $e_string .= 'Latin9::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                             /oxgc) { $e_string .= 'Elatin9::index';      $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b Latin9::rindex \b                                      /oxgc) { $e_string .= 'Latin9::rindex';      $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                            /oxgc) { $e_string .= 'Elatin9::rindex';     $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin9::lc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin9::lcfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin9::uc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin9::ucfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin9::fc';         $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin9::chr';           $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin9::glob';          $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                                      /oxgc) { $e_string .= 'Elatin9::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                                 /oxgc) { $e_string .= 'Elatin9::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                                      /oxgc) { $e_string .= 'Elatin9::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                                 /oxgc) { $e_string .= 'Elatin9::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                                      /oxgc) { $e_string .= 'Elatin9::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                                     /oxgc) { $e_string .= 'Elatin9::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                              /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                                     /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                                    /oxgc) { $e_string .= 'Elatin9::glob_';             $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Elatin9::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Elatin9::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq           (\#) [ ] (\#) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\() [ ] (\)) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\{) [ ] (\}) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\[) [ ] (\]) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\<) [ ] (\>) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\S) [ ] (\2) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q            (\#) [ ] (\#) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\() [ ] (\)) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\{) [ ] (\}) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\[) [ ] (\]) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\<) [ ] (\>) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\S) [ ] (\2) /oxgc) { $e_string .= 'Elatin9::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                    ' [ ] '    /oxgc) { $e_string .= 'Elatin9::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                    " [ ] "    /oxgc) { $e_string .= 'Elatin9::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        ...
        return Elatin9::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Elatin9::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Elatin9::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Elatin9::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Elatin9::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Elatin9::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Elatin9::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Elatin9::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Elatin9::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Elatin9::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Elatin9::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Elatin9::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Elatin9::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Elatin9::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Elatin9::uc($char[$i]) ne Elatin9::fc($char[$i]))) {
            if (CORE::length(Elatin9::fc($char[$i])) == 1) {
                $char[$i] = '['   . Elatin9::uc($char[$i])       . Elatin9::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Elatin9::uc($char[$i]) . '|' . Elatin9::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin9::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ (?>\s*) 0 (?>\s*) \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Elatin9::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin9::ignorecase(Elatin9::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin9::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Elatin9::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin9::ignorecase(Elatin9::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin9::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Elatin9::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin9::ignorecase(Elatin9::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin9::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Latin9::re_t=%s; while($Latin9::re_t =~ %s){%s local $^W=0; local $Latin9::re_r=%s; %s$Latin9::re_t="$`$Latin9::re_r$'"; pos($Latin9::re_t)=length "$`$Latin9::re_r"; } return $Latin9::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Latin9::re_r=CORE::eval $Latin9::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Latin9::re_r=%s; %s"%s$Latin9::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Latin9::re_n=0; while(%s =~ %s){%s local $^W=0; local $Latin9::re_r=%s; %s%s="$`$Latin9::re_r$'"; pos(%s)=length "$`$Latin9::re_r"; $Latin9::re_n++} return %s$Latin9::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Latin9::re_r=%s; %s%s="%s$Latin9::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Latin9::re_r=%s; %s%s="%s$Latin9::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Latin9::re_r=CORE::eval $Latin9::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Elatin9::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Elatin9::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
This Elatin9 module first appeared in ActivePerl Build 522 Built under
...
=item Split string
  ...
  @split = Elatin9::split(/pattern/,$string,$limit);
  @split = Elatin9::split(/pattern/,$string);
  @split = Elatin9::split(/pattern/);
  @split = Elatin9::split('',$string,$limit);
  @split = Elatin9::split('',$string);
  @split = Elatin9::split('');
  @split = Elatin9::split();
  @split = Elatin9::split;
  ...
  at all, Elatin9::split returns the original string as a single substring, If it
  ...
  @chars  = Elatin9::split(//,  $word);
  @fields = Elatin9::split(/:/, $line);
  @words  = Elatin9::split(" ", $paragraph);
  @lines  = Elatin9::split(/^/, $buffer);
  ...
  print join(":" => Elatin9::split(/ */, "hi there"));
  ...
  Elatin9::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Elatin9::split(/:/, $_, 3);
  ...
  splits on whitespace just as Elatin9::split with no arguments does. Thus,
  Elatin9::split(" ") can be used to emulate awk's default behavior. In contrast,
  Elatin9::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Elatin9::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Elatin9::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Elatin9::split()) {
          $count{$word}++;
      }
  }

  The inverse of Elatin9::split is join, except that join can only join with the
...
=item Transliteration
  ...
  $tr = Elatin9::tr($variable,$bind_operator,$searchlist,$replacementlist,$modifier);
  $tr = Elatin9::tr($variable,$bind_operator,$searchlist,$replacementlist);
  ...
  This subroutine scans a Latin-9 string character by character and replaces all
  occurrences of the characters found in $searchlist with the corresponding character
  in $replacementlist. It returns the number of characters replaced or deleted.
  If no Latin-9 string is specified via =~ operator, the $_ variable is translated.
  $modifier are:
  ...
  print Elatin9::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'

=item Chop string

  $chop = Elatin9::chop(@list);
  $chop = Elatin9::chop();
  $chop = Elatin9::chop;

  This subroutine chops off the last character of a string variable and returns the
  character chopped. The Elatin9::chop subroutine is used primary to remove the newline
  ...
  since Elatin9::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Elatin9::chop a literal, only a variable. If you Elatin9::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Elatin9::chop(@lines);

  You can Elatin9::chop anything that is an lvalue, including an assignment:

  Elatin9::chop($cwd = `pwd`);
  Elatin9::chop($answer = <STDIN>);

  This is different from:

  $answer = Elatin9::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Elatin9::chop returns the character
  ...
  In the most general case, Elatin9::chop can be expressed using substr:

  $last_code = Elatin9::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Elatin9::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Elatin9::chop;
...
=item Index string
  ...
  $byte_pos = Elatin9::index($string,$substr,$byte_offset);
  $byte_pos = Elatin9::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Elatin9::index($string, $lookfor, $byte_pos)) > -1) {
...
=item Reverse index string
  ...
  $byte_pos = Elatin9::rindex($string,$substr,$byte_offset);
  $byte_pos = Elatin9::rindex($string,$substr);

  This subroutine works just like Elatin9::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Elatin9::index).
  ...
  while (($byte_pos = Latin9::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item Lower case string
  ...
  $lc = Elatin9::lc($string);
  $lc = Elatin9::lc_;

  This subroutine returns a lowercased version of Latin-9 $string (or $_, if
  $string is omitted). This is the internal subroutine implementing the \L escape
  in double-quoted strings.

  You can use the Elatin9::fc subroutine for case-insensitive comparisons via Latin9
  software.

=item Lower case first character of string

  $lcfirst = Elatin9::lcfirst($string);
  $lcfirst = Elatin9::lcfirst_;

  This subroutine returns a version of Latin-9 $string with the first character
  lowercased (or $_, if $string is omitted). This is the internal subroutine
  implementing the \l escape in double-quoted strings.

=item Upper case string

  $uc = Elatin9::uc($string);
  $uc = Elatin9::uc_;

  This subroutine returns an uppercased version of Latin-9 $string (or $_, if
  $string is omitted). This is the internal subroutine implementing the \U escape
  in interpolated strings. For titlecase, use Elatin9::ucfirst instead.
...
=item Upper case first character of string
  ...
  $ucfirst = Elatin9::ucfirst($string);
  $ucfirst = Elatin9::ucfirst_;
  ...
  $titlecase = Elatin9::ucfirst(substr($word,0,1)) . Elatin9::lc(substr($word,1));
  ...
  $do_not_use = Elatin9::ucfirst(Elatin9::lc($word));
...
=item Fold case string
  ...
  $fc = Elatin9::fc($string);
  $fc = Elatin9::fc_;

  New to Latin9 software, this subroutine returns the full Unicode-like casefold of
  Latin-9 $string (or $_, if omitted). This is the internal subroutine implementing
  ...
  Elatin9::fc($a) eq Elatin9::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because Latin9 software has always used casefolding semantics for
...
=item Make character
  ...
  $chr = Elatin9::chr($code);
  $chr = Elatin9::chr_;

  This subroutine returns a programmer-visible character, character represented by
  that $code in the character set. For example, Elatin9::chr(65) is "A" in either
  ASCII or Latin-9, not Unicode. For the reverse of Elatin9::chr, use Latin9::ord.

=item Filename expansion (globbing)

  @glob = Elatin9::glob($string);
  @glob = Elatin9::glob_;
  ...
  The Elatin9::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Elatin9::glob('"*e f*"');
  @spacies = Elatin9::glob(q("*e f*"));

  If you had to get a variable through, you could do this:

  @spacies = Elatin9::glob("'*${var}e f*'");
  @spacies = Elatin9::glob(qq("*${var}e f*"));
