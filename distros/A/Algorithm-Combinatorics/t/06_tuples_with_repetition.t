...
eval { tuples_with_repetition() };
ok($@, '');

eval { tuples_with_repetition([1]) };
ok($@, '');

eval { tuples_with_repetition(0, 0) };
ok($@, '');
...
# n^k
my $ncomb = 0;
my $iter = tuples_with_repetition([1..7], 5);
...
$ncomb = 0;
$iter = tuples_with_repetition([1..4], 7);
while (my $c = $iter->next) {
    ++$ncomb;
}
is($ncomb, 16384, "");
