...
We have tested tmi.pm, a component of Bigram Statistics Package version
0.5. File tmi.pm implements the true mutual information measure of
association. Note that tmi.pm is not a program by itself, but is a
statistical library package that is plugged into statistic.pl. We test
here the features of tmi.pm by running statistic.pl.

Following is a description of the aspects of tmi.pm that we have
tested. We provide the scripts and files used for testing so that
later versions of tmi.pm can be tested for backward compatibility.
...
Phase 1: Testing tmi.pm's response to erroneous conditions. 
Phase 2: Testing tmi.pm's behaviour under normal conditions. 
...
tmi.pm is meant only for bigrams. This test checks if tmi.pm throws
an error when provided with larger n-grams.

2.1.2. Subtest 2: 
-----------------

tmi.pm requires three frequency values: the frequency of the bigram,
...
not provided, tmi.pm should throw an error. This test checks for this
error. 
...
checks if tmi.pm does indeed throw an error when passed a total
bigrams value that is less than or equal to zero.

2.1.4. Subtest 4: 
-----------------

The numbers passed to tmi.pm should be "valid" in that they should
represent a possible two-by-two table. If this is not the case,
various warnings are thrown by tmi.pm. This test checks for these
warnings. 
...
2.2. Phase 2: Testing tmi.pm's behaviour under normal conditions:
------------------------------------------------------------------
...
First, it checks if tmi.pm works when the frequencies are given in a
...
Second it checks the actual calculations of tmi.pm. The output target
...
Statistical library package tmi.pm has been tested for erroneous
