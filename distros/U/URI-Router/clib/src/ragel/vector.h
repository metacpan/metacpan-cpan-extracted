		...
		upResize(long len)
{
	...
	if ( newLen > BaseTable::allocLen ) {
		if ( BaseTable::data != 0 ) {
			...
			BaseTable::data = (T*) anti_leak_pool->reallocate( BaseTable::data, sizeof(T) * BaseTable::allocLen, sizeof(T) * newLen );
			if ( BaseTable::data == 0 )
				throw std::bad_alloc();
		}
		else {
			/* Create the data. */
			BaseTable::data = (T*) anti_leak_pool->allocate( sizeof(T) * newLen );
			if ( BaseTable::data == 0 )
				throw std::bad_alloc();
		}
		...
		downResize(long len)
{
	...
	if ( newLen < BaseTable::allocLen ) {
		if ( newLen == 0 ) {
		    ...
		    anti_leak_pool->deallocate(BaseTable::data, BaseTable::allocLen);
			BaseTable::data = 0;
		}
		else {
		...
		Vector(const Vector<T, Resize> &v)
{
	...
	if ( BaseTable::allocLen > 0 ) {
		...
		BaseTable::data = (T*) anti_leak_pool->allocate(sizeof(T) * BaseTable::allocLen);
		if ( BaseTable::data == 0 )
			throw std::bad_alloc();
