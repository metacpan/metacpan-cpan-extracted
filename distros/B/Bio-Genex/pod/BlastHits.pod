...
Bio::Genex::BlastHits - Methods for processing data from the GeneX DB
 table: BlastHits
...
=head1 SYNOPSIS
  ...
  my $BlastHits = Bio::Genex::BlastHits->new(id=>47);

  # retrieve data from the DB for all columns
  $BlastHits->fetch();

  # creating an instance, without pre-fetching all columns
  my $BlastHits = new Bio::Genex::BlastHits(id=>47);

  # creating an instance with pre-fetched data
  my $BlastHits = new Bio::Genex::BlastHits(id=>47, 'fetch_all'=>1);

  # retrieving multiple instances via primary keys
  my @objects = Bio::Genex::BlastHits->get_objects(23,57,98)


  # retrieving all instances from a table
  my @objects = Bio::Genex::BlastHits->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $BlastHits->id();

  # or specifically
  my $bh_pk_val = $BlastHits->bh_pk();

  # retreving other DB column attributes
  my $usf_fk_val = $BlastHits->usf_fk();
  $BlastHits->usf_fk($value);

  my $match_accession_val = $BlastHits->match_accession();
  $BlastHits->match_accession($value);

  my $expect_value_val = $BlastHits->expect_value();
  $BlastHits->expect_value($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::BlastHits is
BlastHits).
...
Bio::Genex::BlastHits can access: I<raw> foreign key attributes,
...
Class Bio::Genex::BlastHits defines the following utility variables for assisting
programmers to access the BlastHits table.

=over 4

=item $Bio::Genex::BlastHits::LIMIT
...
=item $Bio::Genex::BlastHits::USE_CACHE
...
instance of the class via the Bio::Genex::BlastHits->methodname() syntax.
...
linking table class. For Bio::Genex::BlastHits it returns 0, since it is
...
table classes, and for Bio::Genex::BlastHits it returns the value 'bh_pk';


=item table_name()

Returns the name of the DB table represented by this class. For
Bio::Genex::BlastHits it returns 'BlastHits';
...
C<insert_db()>.
  ...
  my BlastHits = Bio::Genex::BlastHits->new();
  BlastHits->usf_fk('some_value');
  BlastHits->insert_db($dbh);
...
C<update_db()>.
  ...
  my $dbh = Bio::Genex::current_connection(USER=>$SU_USERNAME,
                                      PASSWORD=>$SU_PASSWORD);
  my BlastHits = Bio::Genex::BlastHits->new(id=>43);
  BlastHits->usf_fk('some_value');
  BlastHits->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('bh_pk' for module Bio::Genex::BlastHits).
...
This method is used to retrieve multiple instances of class Bio::Genex::BlastHits
...
list as a primary key for the BlastHits table and returns a single
...
instance of class Bio::Genex::BlastHits.
...
(and for class Bio::Genex::BlastHits it is synonomous with the
C<bh_pk()>method). The C<id()> method can be useful in writing
generic methods because it avoids having to know the name of the
primary key column. 

=item bh_pk()

This is the primary key attribute for Bio::Genex::BlastHits. It has no setter method. 
...
=item $value = match_accession();

=item match_accession($value);

Methods for the match_accession attribute.


=item $value = expect_value();

=item expect_value($value);

Methods for the expect_value attribute.
...
on Tue Jan 23 22:53:43 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=BlastHits
