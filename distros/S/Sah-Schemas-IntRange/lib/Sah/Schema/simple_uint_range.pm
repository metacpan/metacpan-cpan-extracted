...
our $schema = [str => {
    ...
    examples => [
        ...
        {data=>'1', valid=>0, summary=>'Not a range, but a single positive integer'},
        {data=>'-2', valid=>0, summary=>'Not a range, but a single positive integer'},
...
This document describes version 0.004 of Sah::Schema::simple_uint_range (from Perl distribution Sah-Schemas-IntRange), released on 2021-07-17.
...
Sample data:
 ...
 1  # INVALID (Not a range, but a single positive integer)

 -2  # INVALID (Not a range, but a single positive integer)
