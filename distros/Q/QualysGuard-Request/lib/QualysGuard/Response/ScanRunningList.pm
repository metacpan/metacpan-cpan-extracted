package QualysGuard::Response::ScanRunningList;
...
sub new {
    ...
    if ( $self->exists('/SCAN_RUNNING_LIST/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/SCAN_RUNNING_LIST/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/SCAN_RUNNING_LIST/ERROR');
...
QualysGuard::Response::ScanRunningList
