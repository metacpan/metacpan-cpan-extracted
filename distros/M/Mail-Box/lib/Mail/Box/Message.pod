...
The C<$folder> where this message appeared in.  The argument is
...
Copy the message to the indicated opened C<$folder>, without deleting the
...
Move the message from this folder to the C<$folder> specified.  This will
...
is returned.  All C<%options> are passed to L<copyTo()|Mail::Box::Message/"The message">
...
this must be set to C<false>.  Otherwise a shallow clone will be made, which
...
Read the body of one message.  The C<$parser> gives access to the folder file.
The C<$head> has been read with L<readHead()|Mail::Message/"Internals">.  The optional C<$bodytype> supplies
...
By default, the C<$bodytype> will call L<Mail::Box::determineBodyType()|Mail::Box/"Internals">
