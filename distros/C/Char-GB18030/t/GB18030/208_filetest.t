# encoding: GB18030
# This file is encoded in GB18030.
die "This file is not encoded in GB18030.\n" if q{偁} ne "\x82\xa0";

my $__FILE__ = __FILE__;

use GB18030;
print "1..6\n";

my $chcp = '';
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    $chcp = `chcp`;
}
if ($chcp !~ /932|936/oxms) {
    print "ok - 1 # SKIP $^X $__FILE__\n";
    print "ok - 2 # SKIP $^X $__FILE__\n";
    print "ok - 3 # SKIP $^X $__FILE__\n";
    print "ok - 4 # SKIP $^X $__FILE__\n";
    print "ok - 5 # SKIP $^X $__FILE__\n";
    print "ok - 6 # SKIP $^X $__FILE__\n";
    exit;
}

open(FILE,'>F婡擻') || die "Can't open file: F婡擻\n";
print FILE "1\n";
close(FILE);
mkdir('D婡擻', 0777);
open(FILE,'>D婡擻/a.txt') || die "Can't open file: D婡擻/a.txt\n";
print FILE "1\n";
close(FILE);
open(FILE,'>D婡擻/b.txt') || die "Can't open file: D婡擻/b.txt\n";
print FILE "1\n";
close(FILE);
open(FILE,'>D婡擻/c.txt') || die "Can't open file: D婡擻/c.txt\n";
print FILE "1\n";
close(FILE);
open(FILE,'>D婡擻/F婡擻') || die "Can't open file: D婡擻/F婡擻\n";
print FILE "1\n";
close(FILE);
mkdir('D婡擻/D婡擻', 0777);

my $filetest;

$_ = 'F婡擻';

$filetest = -d $_;
if ($filetest) {
    print "not ok - 1 $^X $__FILE__\n";
}
else {
    print "ok - 1 $^X $__FILE__\n";
}

$filetest = -f $_;
if ($filetest) {
    print "ok - 2 $^X $__FILE__\n";
}
else {
    print "not ok - 2 $^X $__FILE__\n";
}

$filetest = -e $_;
if ($filetest) {
    print "ok - 3 $^X $__FILE__\n";
}
else {
    print "not ok - 3 $^X $__FILE__\n";
}

$_ = 'D婡擻';

$filetest = -d $_;
if ($filetest) {
    print "ok - 4 $^X $__FILE__\n";
}
else {
    print "not ok - 4 $^X $__FILE__\n";
}

$filetest = -f $_;
if ($filetest) {
    print "not ok - 5 $^X $__FILE__\n";
}
else {
    print "ok - 5 $^X $__FILE__\n";
}

$filetest = -e $_;
if ($filetest) {
    print "ok - 6 $^X $__FILE__\n";
}
else {
    print "not ok - 6 $^X $__FILE__\n";
}

unlink('F婡擻');
rmdir('D婡擻/D婡擻');
unlink('D婡擻/a.txt');
unlink('D婡擻/b.txt');
unlink('D婡擻/c.txt');
unlink('D婡擻/F婡擻');
rmdir('D婡擻');

__END__

Perl儊儌/Windows偱偺僼傽僀儖僷僗
http://digit.que.ne.jp/work/wiki.cgi?Perl%E3%83%A1%E3%83%A2%2FWindows%E3%81%A7%E3%81%AE%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB%E3%83%91%E3%82%B9

僼傽僀儖娭楢僐儅儞僪偺摦嶌妋擣
乽婡擻乿偲偄偆暥帤楍傪曄悢$_偵擖傟丄僼傽僀儖墘嶼巕-d丄-f丄-e摍偱僠僃僢僋偡傞偲丄偄偢傟傕忢偵undef傪曉偡

