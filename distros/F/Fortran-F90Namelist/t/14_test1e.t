...
my $hash;
{ local $/;
  open(HASH,"< t/files/test1e.hash")
    or die "Couldn't open reference file t/files/test1e.hash";
  $hash = <HASH>;
  close(HASH);
}
...
die "$@\n" if ($@);
#
my $parseresp = $nl->parse(file => "t/files/test1e.nml");
