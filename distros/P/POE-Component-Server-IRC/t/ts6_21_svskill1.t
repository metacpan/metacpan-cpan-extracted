...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^bobbins' && $params->[1] eq '+i' ) {
    ...
    $poe_kernel->post('groucho', 'send_to_server', { prefix => '4AK', command => 'SVSKILL', params => [ $uid, $ts, q{It's not you, it's me} ] } );
    return;
  }
  if ( $cmd eq 'ERROR' ) {
    pass($cmd);
...
sub groucho_input {
  ...
  if ( $cmd eq 'QUIT' ) {
    ...
    is( $params->[0], q{SVSKilled: It's not you, it's me}, 'Correct QUIT message' );
    return;
  }
  if ( $cmd eq 'SQUIT' ) {
    pass($cmd);
