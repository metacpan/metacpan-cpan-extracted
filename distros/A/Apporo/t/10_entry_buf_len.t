...
my $index_path = "/tmp/p5_apporo_index_10.tsv";
my $out_index;
...
my $conf_path = "/tmp/p5_apporo_conf_10.tsv";
my $out_conf;
{
...
is_utf8	true
dist_threshold	0.0
index_path	/tmp/p5_apporo_index_10.tsv
dist_func	edit
entry_buf_len	1024
engine	tsubomi
result_num	10
...
__CONF__
    ...
    my $file_name = "configure file(UTF-8, 2-gram, entry_buf_len=1024, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of entry_buf_len:1024 from the indexes using ".encode_utf8($query)." query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(UTF-8, 2-gram, entry_buf_len=97, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    my @arr = @{$app->retrieve($query)};
    my @res = (
        "0.666667	佐藤勉	さとうつとむ	自民党	北関東比例	衆2-408号室	電話03-3508-7408	FAX03-3",
        "0.5	佐藤公治	さとうこうじ	民主党	広島	参-305号室	電話03-3508-8305	FAX03-5512-230",
        "0.5	佐藤夕子	さとうゆうこ	民主党	愛知01区	衆1-537号室	電話03-3508-7902	FAX03-35",
        "0.5	佐藤昭郎	さとうあきお	自民党	比例	参-316号室	電話03-3508-8316	FAX03-5512-231",
        "0.5	佐藤信秋	さとうのぶあき	自民党	比例	参-503号室	電話03-3508-8503	FAX03-3262-",
        "0.5	佐藤茂樹	さとうしげき	公明党	近畿比例	衆1-510号室	電話03-3508-7200	FAX03-3",
        "0.5	佐藤正久	さとうまさひさ	自民党	比例	参-507号室	電話03-3508-8507	FAX03-5512-",
        "0.5	佐藤泰介	さとうたいすけ	民主党	愛知	参-411号室	電話03-3508-8411	FAX03-5512-",
        "0.2	佐々木憲昭	ささきけんしょう	共産党	東海比例	衆1-730号室	電話03-3508-373",
        "0.2	佐々木隆博	ささきたかひろ	民主党	北海道06区	衆2-325号室	電話03-3508-7075",
    );
    my %hash_res = ();
    for (my $i = 0; $i <= $#res; $i++) {
        $res[$i] = encode_utf8($res[$i]);
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of entry_buf_len:97 from the indexes using ".encode_utf8($query)." query");
}
