package Data::Sah::Coerce::perl::To_date::From_str::flexible_jakarta;
...
sub meta {
    +{
        v => 4,
        summary => 'Coerce date from string parsed by DateTime::Format::Flexible (Asia/Jakarta time zone)',
        might_fail => 1,
        prio => 60, # a bit lower than normal
        precludes => [qr/\A(From_str::alami(_.+)?|From_str::natural(_.+)?)\z/],
    };
}
...
sub coerce {
    ...
    $res->{expr_coerce} = join(
        "",
        "do { my \$datetime; eval { \$datetime = DateTime::Format::Flexible->parse_datetime($dt)->set_time_zone('Asia/Jakarta') }; ",
...
1;
# ABSTRACT: Coerce date from string parsed by DateTime::Format::Flexible (Asia/Jakarta time zone)
...
Data::Sah::Coerce::perl::To_date::From_str::flexible_jakarta - Coerce date from string parsed by DateTime::Format::Flexible (Asia/Jakarta time zone)

=head1 VERSION

This document describes version 0.009 of Data::Sah::Coerce::perl::To_date::From_str::flexible_jakarta (from Perl distribution Data-Sah-Coerce-perl-To_date-From_str-flexible), released on 2021-09-07.

=head1 SYNOPSIS

To use in a Sah schema:

 ["date",{"x.perl.coerce_rules"=>["From_str::flexible_jakarta"]}]
