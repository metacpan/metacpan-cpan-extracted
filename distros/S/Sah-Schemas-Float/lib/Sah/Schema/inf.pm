...
This document describes version 0.013 of Sah::Schema::inf (from Perl distribution Sah-Schemas-Float), released on 2022-10-20.
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("inf", {return_type=>'str_errmsg'});
 ...
 $data = "NaN";
 my $errmsg = $validator->($data); # => "Must an infinity"

Often a schema has coercion rule or default value, so after validation the
validated value is different. To return the validated (set-as-default, coerced,
prefiltered) value:

 my $validator = gen_validator("inf", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = "NaN";
 my $res = $validator->($data); # => ["Must an infinity","NaN"]
...
To create a type constraint and type library from a schema:
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('$sch_name*', name=>'Inf')
     );
 }

 use My::Types qw(Inf);
 Inf->assert_valid($data);
