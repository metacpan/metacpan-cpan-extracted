...
sub fetch_access_token {
  ...
  Google::Auth::Error->throw(
    'fetch_access_token must be implemented by subclasses');
}
...
sub is_expired {
  ...
  $log->tracef(
'Token expiration check for %s: expires_at=%s (epoch=%s), skew=%d, now=%s, expired=%d',
    ref $self, $expires, $expiry_epoch, $skew, $now, $expired);
  return $expired;
}
...
sub get_token {
  ...
  if ($self->is_refreshing) {
    $log->debugf(
'Token refresh is already active in another thread/coroutine for %s. Yielding...',
      ref $self
    );
    my $limit = 30;
    ...
    $log->debugf('Resume from lock for %s. Access token is: %s',
      ref $self, defined $self->access_token ? 'PRESENT' : 'MISSING');
    return $self->access_token;
  }

  if ($self->is_expired(%options) || !defined $self->access_token) {
    $log->infof(
      'Access token is missing or expired for %s. Initiating refresh...',
      ref $self);
    $self->is_refreshing(1);
    eval { $self->fetch_access_token(%options); };
    my $err = $@;
...
sub apply {
  ...
  if (ref $req_or_headers eq 'HASH') {
    ...
    $log->errorf(
      ...
      ref $req_or_headers || $req_or_headers
    );
    Google::Auth::Error->throw(
      'apply expected a HASH reference or HTTP::Request object');
  }
