...
for my $eq (
  ...
  for my $where (
    ...
    push @tests, {
      ...
      sql => ( ref $eq eq 'HASH' && $eq->{'='} && ref $eq->{'='} eq 'HASH' && $eq->{'='}{'-ident'} ? 'WHERE "0" = "baz" AND "bizz" = ? AND "foo" = ?' : 'WHERE "0" = baz AND "bizz" = ? AND "foo" = ?' ),
    };
    ...
    push @tests, {
      ...
      efcc_result => {},
      sql => ( ref $eq eq 'HASH' && $eq->{'='} && ref $eq->{'='} eq 'HASH' && $eq->{'='}{'-ident'} ? 'WHERE "0" = "baz" OR "bizz" = ? OR "foo" = ?' : 'WHERE "0" = baz OR "bizz" = ? OR "foo" = ?' ),
    }
  ...
  for my $where (
    ...
    push @tests, {
      ...
      efcc_result => {},
      sql => ( ref $eq eq 'HASH' && $eq->{'='} && ref $eq->{'='} eq 'HASH' && $eq->{'='}{'-ident'} ? 'WHERE "foo" = ? OR "0" = "baz" OR "bizz" = ?' : 'WHERE "foo" = ? OR "0" = baz OR "bizz" = ?' ),
      collapsed_sql => ( ref $eq eq 'HASH' && $eq->{'='} && ref $eq->{'='} eq 'HASH' && $eq->{'='}{'-ident'} ? 'WHERE "0" = "baz" OR "bizz" = ? OR "foo" = ?' : 'WHERE "0" = baz OR "bizz" = ? OR "foo" = ?' ),
    }
  }
