...
# now do the tests
SKIP: {
  skip "no database for testing available", 123 if $has_db == 0;

  my $r;
  my %vars;
  my %svars;

  my $dbh = DBI->connect( $DBCONN, $DBUSER, $DBPWD, 
			  { PrintError => 0,AutoCommit => 1,ChopBlanks =>1 } );
  ok( defined $dbh, "get dbh object" );
  ...
  ########################################################################
  $fs = new DBIx::FileSystem( rdbh => \$dbh,
			      progdbver => "42abc" );
  ...
  is( $r, OK, "wrong db version + query" );
