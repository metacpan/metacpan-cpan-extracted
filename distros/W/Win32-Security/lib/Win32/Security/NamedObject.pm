...
$Win32::Security::VERSION = '0.60';
$Win32::Security::NamedObject::VERSION = '0.60';
	...
	dacl => sub {
		...
		unless (scalar(@_)) {
			unless (exists $self->{dacl}) {
				$self->getSecurityInfo('DACL_SECURITY_INFORMATION');
			}
			return bless(\(my $o = ${$self->{dacl}}), ref($self->{dacl}));
		} else {
			my($dacl, $SecurityInfo) = @_;

			$self->enablePrivileges();

			eval {
				delete $self->{dacl};

				my $objectName = $self->{objectName} || $self->objectName();
				...
				Win32::Security::Raw::SetNamedSecurityInfo(
						$objectName, $objectType, $SecurityInfo,
	...
	dacl_noprop => sub {
		...
		if (scalar(@_)) {
			...
			eval {
				...
				$pSecurityDescriptor = Win32::Security::Raw::InitializeSecurityDescriptor();
	...
	ownerTrustee => sub {
		...
		unless (scalar(@_)) {
			unless ($self->{ownerTrustee}) {
				my $ownerSid = $self->{ownerSid} || $self->ownerSid();
				$self->{ownerTrustee} = eval { Win32::Security::SID::ConvertSidToName($ownerSid) };
				$@ and $self->_cleansedCroak($@);
			}
			return $self->{ownerTrustee};
		} else {
			my($ownerTrustee) = @_;
			my $ownerSid = eval { Win32::Security::SID::ConvertNameToSid($ownerTrustee) };
			$@ and $self->_cleansedCroak($@);
...
Win32::Security::NamedObject->reflect->addSlot(
	ownerSid => sub {
		...
		unless (scalar(@_)) {
			unless (exists $self->{ownerSid}) {
				$self->getSecurityInfo('OWNER_SECURITY_INFORMATION');
			}
...
=head2 C<getSecurityInfo>

Internal method for retrieving one or more types of SecurityInfo with a
single call to C<GetNamedSecurityInfo>.  Used internally to retrieve info
for C<dacl>, C<ownerSid> (and by extension C<ownerTrustee>), and C<control>.

Can be used by external code (and is used by C<Win32::Security::Recursor>)
to improve performance by specifying the complete set of information needed
in a single call.

Mandatory parameter C<$SecurityInfo> takes a C<SECURITY_INFORMATION> mask (i.e.
C<'DACL_SECURITY_INFORMATION|OWNER_SECURITY_INFORMATION'>).
	...
	getSecurityInfo => sub {
		...
		eval {
			($psidOwner, $psidGroup, $pDacl, $pSacl, $pSecurityDescriptor) =
				Win32::Security::Raw::GetNamedSecurityInfo( $objectName, $objectType, $SecurityInfo);
		};
		$temperr = $@;

		if ($temperr) {
			if ( !$self->enablePrivileges() && $self->enablePrivileges() ) {
				eval {
					($psidOwner, $psidGroup, $pDacl, $pSacl, $pSecurityDescriptor) =
						Win32::Security::Raw::GetNamedSecurityInfo( $objectName, $objectType, $SecurityInfo);
				};
		...
		unless ($temperr) {
			eval {
				my $SecurityInfoHash = &Win32::Security::SECURITY_INFORMATION->break_mask($SecurityInfo);

				if ( $SecurityInfoHash->{DACL_SECURITY_INFORMATION}) {
					$self->{control} = (Win32::Security::Raw::GetSecurityDescriptorControl($pSecurityDescriptor))[0];

					if ($pDacl) {
						my($AceCount, $AclBytesInUse, $AclBytesFree) =
							Win32::Security::Raw::GetAclInformation($pDacl, 'AclSizeInformation');

						$self->{dacl} = "Win32::Security::ACL::$objectType"->new(
								Win32::Security::Raw::CopyMemory_Read($pDacl, $AclBytesInUse)
							);
					} else {
				...
				if ($SecurityInfoHash->{OWNER_SECURITY_INFORMATION}) {
					my $sidLength = Win32::Security::Raw::GetLengthSid($psidOwner);
					my $Sid = Win32::Security::Raw::CopyMemory_Read($psidOwner, $sidLength);
					$self->{ownerSid} = $Sid;
				}
			};
			$temperr = $@;
...
=head2 C<enablePrivileges>

Internal method for attempting to enable elevated privileges that make it
possible to read files to which access is denied, set ownership to another
user, etc.

C<enablePrivileges> returns the number of privileges that were successfully
...
of privileges enabled during the original call.  To determine whether
a call has already been made, use C<enablePrivilegesAttempted>.
...
Win32::Security::NamedObject->reflect->addSlot(
	enablePrivileges => sub {
		unless (defined $enablePrivileges) {
			my $th = Win32::Security::Raw::OpenProcessToken(
					Win32::Security::Raw::GetCurrentProcess(),
					"TOKEN_ADJUST_PRIVILEGES|TOKEN_QUERY"
				);

			my(@privs) = qw(SeBackupPrivilege SeChangeNotifyPrivilege
							SeRestorePrivilege SeTakeOwnershipPrivilege);
			my(@luids) = map { Win32::Security::Raw::LookupPrivilegeValue(undef, $_) } @privs;

			my($ps, $err) = Win32::Security::Raw::AdjustTokenPrivileges(
					$th, 0, [ map { [$_, 'SE_PRIVILEGE_ENABLED'] } @luids ]
				);

			$enablePrivileges = scalar(@$ps);
		}
...
=head2 C<enablePrivilegesAttempted>

Returns C<1> if C<enablePrivileges> has been previously attempted.

=cut

Win32::Security::NamedObject->reflect->addSlot(
	enablePrivilegesAttempted => sub {
		return defined $enablePrivileges;
	},
);
