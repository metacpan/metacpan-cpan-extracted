...
BEGIN {
    use_ok('Locales::DB::Language::jmc');
    use_ok('Locales::DB::Territory::jmc');
}

diag("Sanity checking Locales::DB::Language::jmc $Locales::DB::Language::jmc::VERSION DB");
...
my @my_lang_codes = sort( keys %Locales::DB::Language::jmc::code_to_name );
my @my_terr_codes = sort( keys %Locales::DB::Territory::jmc::code_to_name );
my %lang_lu;
my %terr_lu;
@lang_lu{@my_lang_codes} = ();
@terr_lu{@my_terr_codes} = ();
ok( $Locales::DB::Language::jmc::cldr_version eq $Locales::cldr_version,  'CLDR version is correct' );
ok( $Locales::DB::Language::jmc::VERSION eq ( $Locales::VERSION - 0.25 ), 'VERSION is correct' );

ok( !( grep { !exists $lang_lu{$_} } @en_lang_codes ), 'jmc languages contains en' );
ok( !( grep { !exists $terr_lu{$_} } @en_terr_codes ), 'jmc territories contains en' );

my %uniq = ();
grep { not $uniq{$_}++ } @{ $Locales::DB::Language::jmc::misc_info{'plural_forms'}->{'category_list'} };
is_deeply(
    [ sort @{ $Locales::DB::Language::jmc::misc_info{'plural_forms'}->{'category_list'} } ],
    [ sort keys %uniq ],
    "'category_list' contains no duplicates"
);

ok( grep( m/^other$/, @{ $Locales::DB::Language::jmc::misc_info{'plural_forms'}->{'category_list'} } ), "'category_list' has 'other'" );

is_deeply(
    [ grep !m/^other$/, sort @{ $Locales::DB::Language::jmc::misc_info{'plural_forms'}->{'category_list'} } ],
    [ grep !m/^other$/, sort keys %{ $Locales::DB::Language::jmc::misc_info{'plural_forms'}->{'category_rules'} } ],
    "'category_rules' has necessary 'category_list' items"
);

is_deeply(
    [ sort keys %{ $Locales::DB::Language::jmc::misc_info{'plural_forms'}->{'category_rules'} } ],
    [ sort keys %{ $Locales::DB::Language::jmc::misc_info{'plural_forms'}->{'category_rules_compiled'} } ],
    "each 'category_rules' has a 'category_rules_compiled'"
);
my $ok_rule_count = 0;
my $error         = '';
for my $rule ( keys %{ $Locales::DB::Language::jmc::misc_info{'plural_forms'}->{'category_rules_compiled'} } ) {
    if ( ref( $Locales::DB::Language::jmc::misc_info{'plural_forms'}->{'category_rules_compiled'}{$rule} ) eq 'CODE' ) {
        $ok_rule_count++;
        next;
    }
    eval $Locales::DB::Language::jmc::misc_info{'plural_forms'}->{'category_rules_compiled'}{$rule};
    if ($@) {
        $error .= $@;
        next;
    }
...
ok( $ok_rule_count == keys %{ $Locales::DB::Language::jmc::misc_info{'plural_forms'}->{'category_rules_compiled'} }, "each 'category_rules_compiled' eval without error - count" );
is( $error, '', "each 'category_rules_compiled' is a code ref or evals without error - errors" );

my $self_obj = Locales->new('jmc');
ok( ref($self_obj), 'jmc object created OK' );
