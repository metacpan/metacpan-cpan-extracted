...
sub BUILD {
    ...
    my $client = Google::gRPC::Client->new(
        target     => 'datafusion.googleapis.com:443',
        auth_token => $token,
    );
...
Google::Cloud::DataFusion::V1 - Google Cloud Data Fusion V1 API Client

=head1 SYNOPSIS

    use Google::Cloud::DataFusion::V1;
    use Google::Auth;

    my $auth = Google::Auth->default();
    my $client = Google::Cloud::DataFusion::V1->new(credentials => $auth);

=head1 DESCRIPTION

Google Cloud Data Fusion V1 API Client over high-performance gRPC transport.
