...
sub _gauss2D {
    ...
    my $g0
      = exp( -( ( $x - $c->[0] )**2 / $s->[0]**2 + ( $y - $c->[1] )**2 / $s->[1]**2 ) / 2 )
      / ( 2 * pi )**( 2 / 2 )
      / ( $s->[0] * $s->[1] );
...
# test 2D Gaussian, correlated
{
    ...
    my $g0 = exp( -( $a * $x**2 + 2 * $b * $x * $y + $c * $y**2 ) / 2 ) / ( 2 * pi )**( 2 / 2 ) /
      sqrt( determinant( $cov ) );
...
# test 3D Gaussian, uncorellated
{
    ...
    my $g0
      = exp(
        -( ( $x - $c[0] )**2 / $s[0]**2 + ( $y - $c[1] )**2 / $s[1]**2 + ( $z - $c[2] )**2 / $s[2]**2 )
          / 2 )
      / ( 2 * pi )**( 3 / 2 )
      / ( $s[0] * $s[1] * $s[2] );
