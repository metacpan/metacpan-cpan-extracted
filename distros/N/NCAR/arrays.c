...
void* pack1D ( SV* arg, char packtype, long* maxlen ) {
   ...
   */
   ...
   if (SvTYPE(arg)==SVt_PVGV || (SvROK(arg) && SvTYPE(SvRV(arg))==SVt_PVAV)) {
      ...
      if (packtype=='c') {
          ...
          *maxlen =  *maxlen - 1;
	  SvGROW( work, sizeof(char)*(n+1)*( *maxlen ) );
      }
      ...
      for(i=0; i<=n; i++) {
	    ...
	    if (packtype=='u') {
            ...
            if (packtype=='c') { /* 
                                     ...
                                     eg :  
                                     character*10 chrs(*)
                                     is a char* with all strings
                                     extended to 10 and concatenated
                                     Fortran expects the length passed
                                     by value as an extra argument
                                 */
                int i;
                i = (int)( *maxlen ) - strlen( cval );

	        sv_catpvn( work, cval, strlen( cval ) );
...
void* pack2D ( SV* arg, char packtype, long* maxlen ) {
...
void* packND ( SV* arg, char packtype, long* maxlen ) {
