...
use Storable::Improved qw (store retrieve freeze thaw nstore nfreeze);
require 'testlib.pl';
our $file;
...
sub test_things {
  ...
  my $header = Storable::Improved::read_magic ($contents);
  ...
  test_corrupt ($copy, $sub,
                ...
                "/^Corrupted storable $what \\(binary v\\d+.\\d+\\)/",
                "bogus tag");
  ...
  test_corrupt ($copy, $sub,
                ...
                "/^Storable binary image v\\d+.\\d+ contains data of type 255. This Storable is v\\d+.\\d+ and can only handle data types up to \\d+/",
