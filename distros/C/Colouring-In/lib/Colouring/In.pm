...
BEGIN {
	%TOOL = (
		...
		round => sub {
			my $precise = '%.' . ( $_[1] // 0 ) . 'f';
			return sprintf $precise, $_[0];
		},
		num       => sub { return $_[0] + 0; },
		numIs     => sub { return $_[0] // '' =~ /^\d+$/ },
...
sub rgb {
	return $_[0]->rgba( $_[1], $_[2], $_[3], 1.0 );
}

sub rgba {
	my $rgb = [ map { $TOOL{scaled}( $_, 255 ) } ( $_[1], $_[2], $_[3] ) ];
	return Colouring::In->new( $rgb, $_[4] );
}

sub hsl {
	return $_[0]->hsla( $_[1], $_[2], $_[3], 1.0 );
}
...
sub hsla {
	...
	return $self->rgba(
		$TOOL{scaled}(($TOOL{hue}( $h + 1 / 3, $m1, $m2 ) * 255)),
		$TOOL{scaled}(($TOOL{hue}( $h,		 $m1, $m2 ) * 255)),
		$TOOL{scaled}(($TOOL{hue}( $h - 1 / 3, $m1, $m2 ) * 255)), 
		$a
	);
}
...
sub asHSL {
	my ( $r, $g, $b, $max, $min, $d, $h, $s, $l ) =
	  $TOOL{rgb2hs}( $_[0]->colour );
...
sub toHSL {
	...
	sprintf( "hsl(%s,%s,%s)", $hsl->{h}, $TOOL{percent}($hsl->{s}), $TOOL{percent}($hsl->{l}), );
}

sub toHSV {
	my ( $r, $g, $b, $max, $min, $d, $h, $s, $v ) =
	  $TOOL{rgb2hs}( $_[0]->colour );
