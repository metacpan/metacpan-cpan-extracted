...
#
package SQL::QueryBuilder::WebApi::ApiClient;
...
sub _new_instance
{
    ...
    my (%args) = (
        ...
        'base_url' => 'https://webapi.activequerybuilder.com',
        @_
    );
...
sub _cfg {'SQL::QueryBuilder::WebApi::Configuration'}
...
sub call_api {
    ...
    $self->{ua}->timeout($self->{http_timeout} || $SQL::QueryBuilder::WebApi::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $SQL::QueryBuilder::WebApi::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("SQL::QueryBuilder::WebApi::Object::$class")->new;
        if (ref $data eq "HASH") {
...
sub get_api_key_with_prefix
{
	...
	my $api_key = $SQL::QueryBuilder::WebApi::Configuration::api_key->{$key_name};
	
	return unless $api_key;
	
	my $prefix = $SQL::QueryBuilder::WebApi::Configuration::api_key_prefix->{$key_name};
