This is the Security Policy for the Perl DBIO-MySQL-EV distribution.
...
[DBIO-MySQL-EV on CPAN](https://metacpan.org/dist/DBIO-MySQL-EV) or the main branch of the
[DBIO-MySQL-EV git repository](https://codeberg.org/dbio/dbio-mysql-async).
...
DBIO-MySQL-EV is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO-MySQL-EV are covered by this policy.
...
they affect DBIO-MySQL-EV, or DBIO-MySQL-EV can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO-MySQL-EV, or plugins to it that are not included with the DBIO-MySQL-EV
distribution) are not covered by this policy.

## Supported Versions of DBIO-MySQL-EV

Only the latest release of DBIO-MySQL-EV will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO-MySQL-EV will run on older versions of Perl. If a
...
that are required for DBIO-MySQL-EV to work. However, some of these
