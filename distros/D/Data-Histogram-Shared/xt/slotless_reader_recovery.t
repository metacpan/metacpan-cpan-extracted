...
# compiling hist.h with HIST_READER_SLOTS=1 so the 2nd reader goes slotless.

plan skip_all => 'Linux only' unless $^O eq 'linux';
my $cc = $Config{cc} || 'cc';
system("$cc --version >/dev/null 2>&1") == 0
    or plan skip_all => "no working C compiler ($cc)";
-f 'hist.h' or plan skip_all => 'run from the distribution root (hist.h not found)';
...
static struct flags *F; static HistHandle *L;
int main(void) {
    char eb[256];
    L = hist_create(NULL, 1, 1000000, 2, 0600, eb);   /* mode arg (ignored for anonymous) */
    if (!L) { fprintf(stderr, "create: %s\n", eb); return 2; }
    F = mmap(NULL, sizeof *F, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0);
    memset(F, 0, sizeof *F);
    pid_t a = fork();
    if (a == 0) { hist_rwlock_rdlock(L);
    ...
    if (b == 0) { hist_rwlock_rdlock(L);
        ...
        __atomic_sub_fetch(&F->reader_active,1,__ATOMIC_RELEASE); hist_rwlock_rdunlock(L); _exit(0); }
    ...
    if (c == 0) { hist_rwlock_wrlock(L);
        ...
        __atomic_store_n(&F->writer_active,0,__ATOMIC_RELEASE); hist_rwlock_wrunlock(L); _exit(0); }
...
my $bin = "$dir/repro";
my $log = `$cc -O2 -D_GNU_SOURCE -DHIST_READER_SLOTS=1 -I. -o $bin $src -lpthread -lm 2>&1`;
