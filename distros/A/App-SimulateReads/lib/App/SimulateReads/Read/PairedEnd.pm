...
sub BUILD {
	...
	unless (($self->fragment_mean - $self->fragment_stdd) >= $self->read_size) {
		croak sprintf "fragment_mean (%d) minus fragment_stdd (%d) must be greater or equal to read_size (%d)\n"
...
sub gen_read {
	...
	if ($seq_size < $self->fragment_mean) {
		croak sprintf "seq_size (%d) must be greater or equal to fragment_mean (%d)\n"
