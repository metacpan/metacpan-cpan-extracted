...
L<teip(1)>'in B<--offload> seçeneği farklı bir modül olan B<-Mline>'da uygulanmaktadır.

    greple -Mtee cat -n -- -Mline --offload 'seq 10 20'

B<line> modülünü sadece çift sayılı satırları işlemek için de aşağıdaki gibi kullanabilirsiniz.
