...
use constant _eg => cwd()->child('examples/multi-lib')->stringify;
use lib _eg;
...
ok( $plugin = $tzil->plugin_named("=Bundled::Plugin"), "bundled plugin loaded in dzil" )
  or do {
  diag "All plugins:";
  for my $plugin ( @{ $tzil->plugins } ) {
    diag "- $plugin => " . $plugin->plugin_name;
...
$plugin and (
  is( $plugin->property, "bar", "bundled plugins have working attributes" )
  or do {
    diag "All attributes";
    diag explain {
      map { $_ => '' . $plugin->{$_} } keys %{$plugin}
