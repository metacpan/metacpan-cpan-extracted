...
any '/'      => sub {
    ...
    $self->render( text => $self->locale_obj->$method() );
};
...
my %headers = (
    ...
    en_EN   => { 'Accept-Language' => 'en-EN' },
    sk_Latn => { 'Accept-Language' => 'sk-Latn' },
);
...
for my $locale ( sort keys %tests ) {
    ...
    for my $method ( sort keys %{ $tests{$locale} } ) {
        ...
        $t->get_ok( "/?method=$method" => ( $headers{$locale} || $headers{en} ) )
          ->status_is( 200 )
          ->content_is( $check, "test language $locale // $method" );
    }
}
