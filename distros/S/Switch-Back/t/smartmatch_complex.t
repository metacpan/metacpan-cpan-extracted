# THESE TESTS BORROWED FROM THE match::smart DISTRIBUTION...
...
while (<DATA>) {
        SKIP: {
                ...
                if ($] < 5.042) {
                    ...
                    my $expected = $yn =~ /\@/ ? 'exception'
                                 : $yn =~ /!/  ? 'false'
                                 :                 'true';
                    my $op_res = eval qq{ use experimental 'smartmatch'; no warnings; $t_op };
                    die "Incorrect test spec: $t_op claimed to cause exception but doesn't"
                        if $yn =~ /\@/ && !defined $@;
                    die "Incorrect test spec: $t_op claimed to not match but does match"
                        if $yn =~ /!/ && $op_res;
                    die "Incorrect test spec: $t_op claimed to match but doesn't match"
                        if $yn !~ /[!\@]/ && !$op_res;
                }
