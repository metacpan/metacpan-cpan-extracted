...
{ my $b = Data::BitSet::Shared->new($p, $cap); }
my $total = -s $p;
unlink $p;
...
# 1. Recovery: an abandoned all-zero file of exactly $total bytes is re-initialized.
{
    ...
    my $b = eval { Data::BitSet::Shared->new($p, $cap) };
    ok($b, "new() recovers an abandoned mid-init file instead of bricking") or diag $@;
  SKIP: {
        skip "no handle", 4 unless $b;
        ok($b->none,        "recovered bitset starts all-zero (none set)");
        is($b->count, 0,    "recovered bitset count == 0");
        is($b->set(10), 0,  "set(10) on recovered bitset returns old=0");
        ok($b->test(10),    "recovered bitset works (set/test readback)");
    }
    undef $b; unlink $p;
}
...
{
    my $a = Data::BitSet::Shared->new($p, $cap); $a->set(99); undef $a;
    my $r = Data::BitSet::Shared->new($p, $cap);
    ok($r->test(99), "reopening a valid file preserves its data");
    undef $r; unlink $p;
}
