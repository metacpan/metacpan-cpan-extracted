...
sub gen_completion {
    ...
    sub {
        ...
        my $sth;
        $sth = $dbh->prepare(
            "SELECT DISTINCT name,abstract FROM module ORDER BY name");
        $sth->execute;
        my @all_modnames;
        my @all_modabstracts;
        while (my @row = $sth->fetchrow_array) {
            push @all_modnames, $row[0];
            push @all_modabstracts, $row[1];
        }
        return complete_array_elem(
            array     => \@all_modnames,
            summaries => \@all_modabstracts,
            word      => $word,
        );
    }
}
...
Perinci::Sub::XCompletion::lcpan_modname - Generate completion of CPAN module names from local CPAN index

=head1 VERSION

This document describes version 0.003 of Perinci::Sub::XCompletion::lcpan_modname (from Perl distribution Perinci-Sub-XCompletionBundle-lcpan), released on 2022-09-02.
