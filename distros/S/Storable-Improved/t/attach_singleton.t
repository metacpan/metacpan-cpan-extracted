...
# Freeze the struct
my $frozen = Storable::Improved::freeze( $struct );
ok( (defined($frozen) and ! ref($frozen) and length($frozen)), 'freeze returns a string' );

# Thaw the struct
my $thawed = Storable::Improved::thaw( $frozen );
...
$frozen = Storable::Improved::freeze($struct);
$thawed = Storable::Improved::thaw($frozen);
