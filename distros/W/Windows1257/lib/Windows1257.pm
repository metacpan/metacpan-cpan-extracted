...
######################################################################
#
# Windows1257 - Source code filter to escape Windows-1257 script
#
# http://search.cpan.org/dist/Char-Windows1257/
#
...
perl $0 Windows-1257_script.pl > Escaped_script.pl.e
END
    }

    print Windows1257::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Ewindows1257::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
        ...
        my $e_script = Windows1257::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Ewindows1257::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
# escape Windows-1257 script
sub Windows1257::escape_script {
    my($script) = @_;
    my $e_script = '';

    # read Windows-1257 script
    my $fh = gensym();
    Ewindows1257::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Ewindows1257(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ewindows1257 '%s.0'; # 'quote' for perl5.6\n", $Windows1257::VERSION); # require run-time routines version

        # use Windows1257 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Windows1257 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Windows1257::VERSION) {
                    my @file = grep -e, map {qq{$_/Windows1257.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Windows1257.pm $version, but @{[__FILE__]} is version $Windows1257::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Ewindows1257.pm $version, but \\\$Ewindows1257::VERSION is \$Ewindows1257::VERSION" if \$Ewindows1257::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Windows1257::VERSION) {
                    die "Script $0 required Windows1257.pm $version, but @{[__FILE__]} is only version $Windows1257::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Ewindows1257::function_ord     = 'Windows1257::ord'     if /\A ord \z/oxms;
                    $Ewindows1257::function_ord_    = 'Windows1257::ord_'    if /\A ord \z/oxms;
                    $Ewindows1257::function_reverse = 'Windows1257::reverse' if /\A reverse \z/oxms;
                    $Ewindows1257::function_getc    = 'Windows1257::getc'    if /\A getc \z/oxms;

                    # avoid: used only once: possible typo at ...
                    $Ewindows1257::function_ord     = $Ewindows1257::function_ord;
                    $Ewindows1257::function_ord_    = $Ewindows1257::function_ord_;
                    $Ewindows1257::function_reverse = $Ewindows1257::function_reverse;
                    $Ewindows1257::function_getc    = $Ewindows1257::function_getc;
                }
            }
        }
    }

    $e_script .= Windows1257::escape();
...
=item * You'll have to download Windows1257.pm and Ewindows1257.pm and put it in your perl lib directory.

=item * You'll need to write "use Windows1257;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Windows1257;
  use Windows1257 ver.sion;             --- require minimum version
  use Windows1257 ver.sion.0;           --- expects version (match or die)

  # "no Windows1257;" not supported

  or

  $ perl Windows1257.pm Windows-1257_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Windows-1257_script.pl  --- script written in Windows-1257
...
Windows1257 software is "middleware" between perl interpreter and your Perl script
written in Windows-1257.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Windows-1257, Perl3 or later
can treat Windows-1257 as binary data.

By "use Windows1257;", it automatically interpret your script as Windows-1257. The various
functions of perl including a regular expression can treat Windows-1257 now.
...
This software is a source code filter to escape Perl script encoded by Windows-1257
...
=item * Remains One Language Handling Raw Windows-1257, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Windows1257.pm, Ewindows1257.pm
  +---------------------------------------+
...
import of Ewindows1257.pm works well.
   ...
   sub import {
       ...
       if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
           ...
           for (@ARGV) {
               ...
               # has space
               if (/\A (?:$q_char)*? [ ] /oxms) {
                   if (my @glob = Ewindows1257::glob(qq{"$_"})) {
                       push @argv, @glob;
                   }
                   else {
                       push @argv, $_;
               ...
               # has wildcard metachar
                   ...
                   if (my @glob = Ewindows1257::glob($_)) {
                       push @argv, @glob;
                   }
                   else {
                       push @argv, $_;
...
=head1 Software Composition
   ...
   Windows1257.pm               --- source code filter to escape Windows-1257
   Ewindows1257.pm              --- run-time routines for Windows1257.pm
...
You need write 'use Windows1257;' in your script.
...
=head1 Calling 'Ewindows1257::ignorecase()' (Windows1257 software provides)

Windows1257 software applies calling 'Ewindows1257::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Ewindows1257::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Ewindows1257.pm provides)
...
Also POSIX-style character classes.
  ...
  [:^alnum:]    ${Ewindows1257::not_alnum}
  [:^alpha:]    ${Ewindows1257::not_alpha}
  [:^ascii:]    ${Ewindows1257::not_ascii}
  [:^blank:]    ${Ewindows1257::not_blank}
  [:^cntrl:]    ${Ewindows1257::not_cntrl}
  [:^digit:]    ${Ewindows1257::not_digit}
  [:^graph:]    ${Ewindows1257::not_graph}
  [:^lower:]    ${Ewindows1257::not_lower}
                ${Ewindows1257::not_lower_i}    (/i modifier)
  [:^print:]    ${Ewindows1257::not_print}
  [:^punct:]    ${Ewindows1257::not_punct}
  [:^space:]    ${Ewindows1257::not_space}
  [:^upper:]    ${Ewindows1257::not_upper}
                ${Ewindows1257::not_upper_i}    (/i modifier)
  [:^word:]     ${Ewindows1257::not_word}
  [:^xdigit:]   ${Ewindows1257::not_xdigit}
  ---------------------------------------------------------------
...
Definitions in Ewindows1257.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Ewindows1257::dot}            qr{(?>[^\x0A])};
  ${Ewindows1257::dot_s}          qr{(?>[\x00-\xFF])};
  ${Ewindows1257::eD}             qr{(?>[^0-9])};
  ${Ewindows1257::eS}             qr{(?>[^\s])};
  ${Ewindows1257::eW}             qr{(?>[^0-9A-Z_a-z])};
  ${Ewindows1257::eH}             qr{(?>[^\x09\x20])};
  ${Ewindows1257::eV}             qr{(?>[^\x0A\x0B\x0C\x0D])};
  ${Ewindows1257::eR}             qr{(?>\x0D\x0A|[\x0A\x0D])};
  ${Ewindows1257::eN}             qr{(?>[^\x0A])};
  ${Ewindows1257::not_alnum}      qr{(?>[^\x30-\x39\x41-\x5A\x61-\x7A])};
  ${Ewindows1257::not_alpha}      qr{(?>[^\x41-\x5A\x61-\x7A])};
  ${Ewindows1257::not_ascii}      qr{(?>[^\x00-\x7F])};
  ${Ewindows1257::not_blank}      qr{(?>[^\x09\x20])};
  ${Ewindows1257::not_cntrl}      qr{(?>[^\x00-\x1F\x7F])};
  ${Ewindows1257::not_digit}      qr{(?>[^\x30-\x39])};
  ${Ewindows1257::not_graph}      qr{(?>[^\x21-\x7F])};
  ${Ewindows1257::not_lower}      qr{(?>[^\x61-\x7A])};
  ${Ewindows1257::not_lower_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Ewindows1257::not_lower_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
  ${Ewindows1257::not_print}      qr{(?>[^\x20-\x7F])};
  ${Ewindows1257::not_punct}      qr{(?>[^\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E])};
  ${Ewindows1257::not_space}      qr{(?>[^\s\x0B])};
  ${Ewindows1257::not_upper}      qr{(?>[^\x41-\x5A])};
  ${Ewindows1257::not_upper_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Ewindows1257::not_upper_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
  ${Ewindows1257::not_word}       qr{(?>[^\x30-\x39\x41-\x5A\x5F\x61-\x7A])};
  ${Ewindows1257::not_xdigit}     qr{(?>[^\x30-\x39\x41-\x46\x61-\x66])};
  
  # This solution is not perfect. I beg better solution from you who are reading this.
  ${Ewindows1257::eb}             qr{(?:\A(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[0-9A-Z_a-z])|(?<=[0-9A-Z_a-z])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]|\z))};
  ${Ewindows1257::eB}             qr{(?:(?<=[0-9A-Z_a-z])(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]))};
  ---------------------------------------------------------------------------------------------------------------------------------------------------------

=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Windows1257 software provides)

Windows1257 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Windows1257 software provides)

Insert 'Ewindows1257::' at head of function name. Ewindows1257.pm provides your script Ewindows1257::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Ewindows1257::lc        Character
  lcfirst     Ewindows1257::lcfirst   Character
  uc          Ewindows1257::uc        Character
  ucfirst     Ewindows1257::ucfirst   Character
  fc          Ewindows1257::fc        Character
  chr         Ewindows1257::chr       Character
  glob        Ewindows1257::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Windows1257 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Windows1257 software provides)

Windows1257 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Ewindows1257.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Windows1257 software and
...
exist at time of be written this document and JPerl did exist.
                        ...
                        (a)     (b)     (c)     (d)     (e)
                                      JPerl,japerl    Encode,Windows1257
      +--------------+-------+-------+-------+-------+-------+
      | data         |  Old  |  Old  |  New  |  Old  |  New  |
      +--------------+-------+-------+-------+-------+-------+
      | script       |  Old  |      Old      |      New      |
...
Windows1257 software remains one language and one interpreter.
