...
bool fully_contains_node_rect(QuadTreeNode *node, double xmin, double ymin, double xmax, double ymax)
{
	return false;
}
...
bool fully_contains_node_circ(QuadTreeNode *node, double x, double y, double radius)
{
	return x + radius >= node->xmax
		&& x - radius <= node->xmin
		&& y + radius >= node->ymax
		&& y - radius <= node->ymin;
}
...
bool fully_contains_node(QuadTreeNode *node, Shape *param)
{
	switch (param->type) {
		case shape_rectangle:
			return fully_contains_node_rect(node, param->dimensions[0], param->dimensions[1], param->dimensions[2], param->dimensions[3]);
		case shape_circle:
			return fully_contains_node_circ(node, param->dimensions[0], param->dimensions[1], param->dimensions[2]);
	}
}

void found_node(QuadTreeNode *node, AV *ret)
{
	...
	if (node->values != NULL) {
		...
		for (i = 0; i < CHILDREN_PER_NODE; ++i) {
			found_node(&node->children[i], ret);
		}
	}
}
...
{
	if (!node->has_objects) return;
	if (fully_contains_node(node, param)) {
		found_node(node, ret);
		return;
	}
