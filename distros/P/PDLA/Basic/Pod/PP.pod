...
Here is a quick reference list of the functions provided by PDLA::PP.
...
Sets the package to which the XS code is added (default is PDLA)
...
Control state of PDLA bounds checking activity
...
Specify what is imported from PDLA::Core
...
Mark the end of PDLA::PP definitions in the file
...
Add line number information to simplify debugging of PDLA::PP code
...
generate subroutine code for each of the PDLA datatypes (PDLA_Byte,
PDLA_Short,. etc).  AUTOMATICALLY.  Secondly, when referring to slices
of PDLA arrays in Perl (e.g. C<< $a->slice('0:10:2,:') >> or other things such
...
calling of the same routine for multiple slices, see L<PDLA::Indexing>)
and dataflow (see L<PDLA::Dataflow>) which use of PP allows.
...
within PDLA. If you have not yet heard of these concepts or are not very
comfortable with them it is time to check L<PDLA::Indexing>.

As you may appreciate from its name PDLA::PP is a Pre-Processor, i.e.
...
L<PDLA::Indexing>).  In this case the
...
PP Code sections. PDLA::PP uses C<$var()> for its parsing and if you don't
...
when you could have used the PP C<loop> construct since PDLA::PP checks
...
1D row (again check L<PDLA::Indexing> for the details of threading).
...
one of the best features of PDLA making writing
new C subroutines for PDLA an amazingly concise exercise. A second reason is
...
case you want PDLA::PP certainly to generate code that takes advantage of
...
should still expand to working XS code even if the internals of PDLA
changed.
...
PDLA to some libraries. This is handled with the C<$P(var)> directive,
see below.
...
number with the error. However, a recent addition to PDLA::PP helps report
...
PDLA routines using PDLA::PP let's summarise in which circumstances you
should actually use this preprocessor/precompiler. You should use PDLA::PP
if you want to
...
interface PDLA to some external library
...
be a PDLA developer (and even then it's not obligatory)

=back

=head1 WARNING

Because of its architecture, PDLA::PP can be both flexible and easy to use
...
Now that you have some idea how to use C<pp_def> to define new PDLA functions
...
C<pp_done> (see example in the SYNOPSIS) is used to tell PDLA::PP that there
...
in PDLA very well yet; it is still very much experimental).
...
that is part of the PDLA distribution; but beware, this is not introductory
level stuff).

If PDLA was compiled with support for bad values (i.e. C<WITH_BADVAL =E<gt> 1>),
...
for PDLA (well, actually it isn't; you additionally need to write a
...
tells PDLA::PP that this is an output argument. So the above call to
...
haven't used the value C<undef> to flag this instead of the PDLA
...
about the reasons why PDLA::PP supports this style where output arguments
...
temporary.  What does that mean? You tell PDLA::PP that this pdl is only
...
why should PDLA::PP want to know about this in the first place?  The reason
...
where $a is of type C<PDLA_Float> and $b of type C<PDLA_Short>? With the signature
...
C<PDLA_Long> which makes sense since C<b> will represent an index into
...
therefore contains a switch like C<switch (type) {case PDLA_Byte: ... case
PDLA_Double: ...}> that selects a case based on the runtime
datatype of the function (it's called a type ``loop''
because there is a loop in PP code that generates the cases).
In any case your code is inserted once for each PDLA type
...
statement, e.g., in the C<case PDLA_Byte> section C<cur> will expand to
C<PDLA_Byte> and so on for the other case statements. I guess you
...
function with an C<a> of type PDLA_Short the output pdl will be of type
PDLA_Long (just as would have been the case with the C<int>
...
in the latest PDLA versions you can use it to define threaded functions
from within Perl, i.e. what we call I<Perl level threading>. Please check
L<PDLA::Indexing> for details.
...
you still haven't read L<PDLA::Indexing>; do it now ;) after any PP macros
...
the matrix sense that A^T = A where ^T means matrix transpose, or in PDLA
...
are here interpreted in the way that PDLA prints 2D pdls). If this explanation
still sounds a bit strange just go ahead, make a little file into which you
write this definition, build the new PDLA extension (see section on Makefiles
...
in the signature of C<symm>. This tells PDLA::PP that the first and
second dimension of these arguments should have the same
size. Otherwise the generated function will raise a runtime error.
However, now in an access to C<$a> and C<$c> PDLA::PP cannot figure out
...
other keys that are recognised by PDLA::PP and we will hear about some
...
PDLA::PP is not a completely static, well designed set of routines (as
Tuomas puts it: "stop thinking of PP as a set of routines carved in
stone") but rather a collection of things that the PDLA::PP author
(Tuomas J. Lukka) considered he would have to write often into his PDLA
...
you want to learn more on why you might want to change PDLA::PP and how
to do it check the section on PDLA::PP internals.

=head2 Handling bad values

If you do not have bad-value support compiled into PDLA you can
ignore this section and the related keys: C<BadCode>, C<HandleBad>, ...
(try printing out the value of C<$PDLA::Bad::Status> - if it equals 0 
...
This causes PDLA::PP to write extra code that ensures the BadCode
...
bad values are not present (i.e. the L<badflag|PDLA::Bad/badflag> routine
returns 0).

The C preprocessor symbol C<PDLA_BAD_CODE> is defined when the bad code
...
You can reduce code repition using the C C<PDLA_BAD_CODE> macro,
...
piddle, you can use the PDLA STATE macros:

=over 4

=item $ISPDLASTATEBAD(pdl)

=item $ISPDLASTATEGOOD(pdl)

=item $SETPDLASTATEBAD(pdl)

=item $SETPDLASTATEGOOD(pdl)
...
(that may in fact be part of some library you want to interface to PDLA)
which takes as arguments two pointers to vectors of double:

	void myfunc(int n,double *v1,double *v2);

The correct way of defining the PDLA function is
...
recognised by PDLA::PP : the C<GenericTypes> declaration tells PDLA::PP
...
are passed to C<myfunc()> PDLA will automatically convert them to
...
interface in the C<Lib> tree of the PDLA distribution. If you want to check
...
C<Code> section of PDLA::PP along with examples of their usage.
...
double pointers. You could interface them to PDLA by defining a generic
...
I<physical> refers to some internal optimisations of PDLA (for those who
are familiar with the PDLA core we are talking about the vaffine
...
name is derived from the implementation of transformations in PDLA. The
variables you can refer to using C<$COMP> are members
of the ``compiled'' structure that represents the PDLA transformation in question
but does not yet contain any information about dimensions
(for further details check L<PDLA::Internals>). However, you can treat
C<$COMP> just as a black box without knowing anything about the
implementation of transformations in PDLA. So when would you use this
...
to explicitly specify the size of C<n> since PDLA::PP can figure it out
...
and L<PDLA::Indexing>). Assuming you have some notion of this concept you
...
use the C<threadloop> function, PDLA::PP recognises this and doesn't
...
This code works as follows: The macro $PDLA(p) expands to a
...
rules. Here is an example from PDLA::Math
...
crucial when you interface external libraries with PDLA. However in
...
Please speak up on one of the PDLA mailing lists. Most other keys recognised
...
save time we make sure that we call C<PDLA-E<gt>propagate_badgflag> only
...
=head2 Other PDLA::PP functions to support concise package definition

So far, we have described the C<pp_def> and C<pp_done> functions. PDLA::PP
exports a few other functions to aid you in writing concise PDLA extension
package definitions.
...
By including these functions here you make sure that PDLA::PP inserts your
...
A typical call would be
  ...
  static void do_the real_work(PDLA_Byte * in, PDLA_Byte * out, int n)
  {
...
generated B<pm>-file. This is easily achieved with the C<pp_addpm> command:
   ...
   PDLA::Lib::Mylib -- a PDLA interface to the Mylib library

   =head1 DESCRIPTION

   This package implements an interface to the Mylib package with full
   threading and indexing support (see L<PDLA::Indexing>).
...
C<pp_bless('PDLA::MyClass')>. The default is C<PDLA> if this is
omitted.
...
PDLA::Exporter, hence letting PP export your function means that they get added
...
However, don't use this if you use L<Module::Build::PDLA>. See that
...
notice. The replacement module can be passed as an argument like this:
 ...
 pp_deprecate_module( infavor => "PDLA::NewNonDeprecatedModule" );

Note that function affects I<only> the runtime warning and the POD.

=head1 Making your PP function "private"

Let's say that you have a function in your module called PDLA::foo that
...
accessible via PDLA::bar_pp due to Perl's module design. But making
...
of PDLA internals than the data operations. Furthermore, the complexity
...
take a look at the F<Basic/Slices/slices.pd> file in the PDLA
distribution :-). Nevertheless,
functions generated using the slice operations are at the heart of the
index manipulation and dataflow capabilities of PDLA.
...
is compiled into PDLA). The easiest thing to do is look at
...
macros to C<< PDLA->barf >> and C<< PDLA->warn >>. These redefinitions are in place to keep
...
PDLA's own versions of C<barf> and C<warn> will queue-up warning or barf messages until after pthreading
...
The PDLA C<Core> structure, defined in F<Basic/Core/pdlcore.h.PL>, contains
...
=item PDLA->qsort_B( PDLA_Byte *xx, int a, int b )

Sort the array C<xx> between the indices C<a> and C<b>.
There are also versions for the other PDLA datatypes,
with postfix C<_S>, C<_U>, C<_L>, C<_N>, C<_Q>, C<_F>, and C<_D>.
Any module using this must ensure that C<PDLA::Ufunc>
is loaded.

=item PDLA->qsort_ind_B( PDLA_Byte *xx, int *ix, int a, int b )

As for C<PDLA-E<gt>qsort_B>, but this time sorting the indices
...
the PDLA::Core::Dev package. We just have to tell MakeMaker to use
it.
...
In most cases you can define your Makefile like
  ...
  use PDLA::Core::Dev;            # Pick up development utilities
  use ExtUtils::MakeMaker;

  $package = ["mylib.pd",Mylib,PDLA::Lib::Mylib];
...
needs:
  ...
  WriteMakefile(
   ...
   'INC'          => &PDLA_INCLUDE(), # add include dirs as required by your lib
  ...
  # Add genpp rule; this will invoke PDLA::PP on our PP file
  ...
  sub MY::postamble { pdlpp_postamble(["mylib.pd",Mylib,PDLA::Lib::Mylib]); }

To make life even easier PDLA::Core::Dev defines the function C<pdlpp_stdargs>
...
=head1 Appendix A: Some keys recognised by PDLA::PP

Unless otherwise specified, the arguments are strings. Keys marked with
(bad) are only used if bad-value support is compiled into PDLA.
...
Used to specify a documentation string in Pod format. See PDLA::Doc
for information on PDLA documentation conventions. Note: in
...
created by PDLA::PP. However, as befits computer-generated text, it's
rather stilted; it may be much better to do it yourself!

=item NoPthread

Optional flag to indicate the PDLA function should B<not> use processor threads (i.e.
pthreads or POSIX threads) to split up work across mutliple CPU cores. This option
is typically set to 1 if the underlying PDLA function is not threadsafe. If this option
isn't present, then the function is assumed to be threadsafe. This option only
applies if PDLA has been compiled with POSIX threads enabled. 
...
PDLA functions allow you to pass in a piddle into which you want the output
saved. This is handy because you can allocate an output piddle once and
reuse it many times; the alternative would be for PDLA to create a new piddle
each time, which may waste compute cycles or, more likely, RAM. This added
flexibility comes at the cost of more complexity: PDLA::PP has to write
...
PDLA::PP is smart enough to do that, but there are restrictions on argument
...
When pp_def generates functions, it typically defines them in the PDLA
...
a pointer access to the PDLA named C<a> in the signature. Useful for
...
=head1 Appendix C: Functions imported by PDLA::PP

A number of functions are imported when you C<use PDLA::PP>. These include
...
PDLA::PP's main purpose is to make it easy for you to wrap the threading
...
Indicates you are done with PDLA::PP and that it should generate its .xs
...
PDLA normally checks the bounds of your accesses before making them. You can
...
Many functions imported when you use PDLA::PP allow you to modify the
...
Adds Perl code to the generated .pm file. PDLA::PP actually keeps track of
...
PDLA::PP usually exports all functions generated using pp_def, and usually
installs them into the PDLA symbol table. However, you can modify this
...
For the concepts of threading and slicing check L<PDLA::Indexing>.

L<PDLA::Internals>

L<PDLA::BadValues> for information on bad values
...
Although PDLA::PP is quite flexible and thoroughly used, there are surely
