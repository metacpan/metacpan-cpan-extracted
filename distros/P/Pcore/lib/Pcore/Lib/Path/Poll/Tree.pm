package Pcore::Lib::Path::Poll::Tree;
...
sub DESTROY ($self) {
    delete $Pcore::Lib::Path::Poll::POLL->{ $self->{id} } if ${^GLOBAL_PHASE} ne 'DESTRUCT';
...
sub scan ($self) {
    ...
    for my $stat ( values $stats->%* ) {
        ...
        if ( my $old_stat = $old_stats->{ $stat->[$STAT_ID]->$* } ) {
            ...
            if ( $old_stat->[$STAT_DATA]->[9] != $stat->[$STAT_DATA]->[9] ) {
                push @changes, [ $stat->[$STAT_PATH], $Pcore::Lib::Path::Poll::POLL_MODIFIED ];
        ...
        # new path was created
            ...
            push @changes, [ $stat->[$STAT_PATH], $Pcore::Lib::Path::Poll::POLL_CREATED ];
    ...
    for my $old_stat ( values $old_stats->%* ) {
        ...
        if ( !exists $stats->{ $old_stat->[$STAT_ID]->$* } ) {
            push @changes, [ $old_stat->[$STAT_PATH], $Pcore::Lib::Path::Poll::POLL_REMOVED ];
