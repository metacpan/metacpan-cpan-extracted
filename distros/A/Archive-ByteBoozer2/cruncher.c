...
void findMatches() {
  ...
  while (get >= 0) {
	...
	scn = _link[scn]; // Table65535 lookup
      };

      first[cur] = _link[first[cur]]; // Waste first entry
...
int writeOutput() {
  ...
  _bool needCopyBit = _true;

  for (i = 0; i < ibufSize;) {

    uint _link = context[i].next;
	    ...
	    _bool isRelocated)
{
  ...
  _link = (uint*)malloc(sizeof(uint) * ibufSize);
