...
# {{{ num2spa_ordinal             standalone ordinal conversion

sub num2spa_cardinal :Export {
    my $number = shift;
    my $obj = Lingua::SPA::Num2Word->new();
    return $obj->cardinal($number);
}

sub num2spa_ordinal :Export {
    my $number = shift;

    croak 'You should specify a positive integer'
        unless defined $number and $number =~ /^\d+$/ and $number > 0;

    my $obj = Lingua::SPA::Num2Word->new( GENERO => 'o' );
    return $obj->ordinal($number);
}
...
Lingua::SPA::Num2Word - Translates numbers to spanish text
...
=head1 SYNOPSIS
   ...
   use Lingua::SPA::Num2Word

   $obj = new Lingua::SPA::Num2Word ('MAYUSCULAS' => 1)
...
Currently Lingua::SPA::Num2Word handles numbers up to 1e127-1 (999999 vigintillions).

=head1 METHODS

=head2 CONSTRUCTOR: new

To create a new Lingua::SPA::Num2Word, use the B<new> class method. This method can
...
Examples:
      ...
      $obj = new Lingua::SPA::Num2Word; 
      
      # Specifies the values of some of them
      $obj = Lingua::SPA::Num2Word->new( ACENTOS => 0, 
...
=head2 num2spa_ordinal

SYNOPSIS:
  use Lingua::SPA::Num2Word qw(num2spa_ordinal);
  my $text = num2spa_ordinal(21);  # "vigésimo primero"
...
Standalone (non-OO) function that converts a positive integer to its
Spanish ordinal text representation using masculine gender and default
settings.  This function is exported via L<Export::Attrs>.

=head3 Return Value

Textual representation of the ordinal number as a string.
