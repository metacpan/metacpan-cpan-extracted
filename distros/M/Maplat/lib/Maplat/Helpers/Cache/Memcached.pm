...
package Maplat::Helpers::Cache::Memcached;
...
my $HAVE_XS = eval "use Maplat::Helpers::Cache::Memcached::GetParserXS; 1;";
$HAVE_XS = 0 if $ENV{NO_XS};

my $parser_class = $HAVE_XS ? "Maplat::Helpers::Cache::Memcached::GetParserXS" : "Maplat::Helpers::Cache::Memcached::GetParser";
if ($ENV{XS_DEBUG}) {
...
sub new {
    my Maplat::Helpers::Cache::Memcached $self = shift;
    ...
    # TODO: undocumented
    $self->{'connect_timeout'} = $args->{'connect_timeout'} || 0;
...
#   FIXME! This seems broken in the current version of Perl
#   and/or IO::Handle ... ask Rene for Details
#    if ($timeout) {
...
sub sock_to_host { # (host)
    my Maplat::Helpers::Cache::Memcached $self = ref $_[0] ? shift : undef;
    my $host = $_[0];
    ...
    if ( index($host, '/') != 0 )
    {
        ...
        unless ($connected) {
            ...
            my $timeout = $self ? $self->{connect_timeout} : 0;
...
sub get_sock { # (key)
    my Maplat::Helpers::Cache::Memcached $self = $_[0];
    my $key = $_[1];
...
sub _load_multi {
    use bytes; # return bytes from length()
    my Maplat::Helpers::Cache::Memcached $self;
    my ($sock_keys, $ret);
    ...
    my %parser;  # $sockstr -> Maplat::Helpers::Cache::Memcached::GetParser
...
This module is basically a copy of Cache::Memcached with some workaround hacks
build in by Rene Schickbauer. It is used as a last resort if Cache::Memcached::Fast
and Cache::Memcached are either unavailable or don't work properly (There are some
problems with at least one recent release of ActiveState Perl for Windows).

Unless you are experiencing problems with Cache::Memcached, don't use this module. I (Rene
Schickbauer) didn't submit the updated version to the original author because my changes are
mostly untested, ugly hacks (i just poked the source until it worked on MY machine). Anyway,
the problem is has its root not in Cache::Memcached but in one build of ActiveState's perl
for windows.
