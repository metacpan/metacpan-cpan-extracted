package Faker::Plugin::InternetDomainName;
...
method execute() {
  ...
  return join '.',
    $faker->internet_domain_word,
    $faker->internet_root_domain;
}
...
Faker::Plugin::InternetDomainName

=cut

=head1 ABSTRACT

Internet Domain Name Plugin for Faker
...
=head1 SYNOPSIS
  ...
  use Faker;
  use Faker::Plugin::InternetDomainName;

  my $f = Faker->new;
  my $p = Faker::Plugin::InternetDomainName->new(faker => $f);
...
This package provides methods for generating fake internet domain name data.
...
The execute method returns a random fake internet domain name.
