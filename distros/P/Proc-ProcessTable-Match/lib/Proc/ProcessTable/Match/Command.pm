package Proc::ProcessTable::Match::Command;
...
Proc::ProcessTable::Match::Command - Check if the fname or cmndline of a process matches via regexp.
...
=head1 SYNOPSIS
    ...
    my $checker=Proc::ProcessTable::Match::Command->new( \%args );
...
that is 'commands', which is a array of commands to match.

The matching is done via regexp.
...
sub new{
	...
	my $regex_int=0;
	while ( defined( $args{commands}[$regex_int] ) ){
		if ( ! eval{ qr/$args{commands}[$regex_int]/ } ){
			die (q{The value "}.$args{commands}[$regex_int].q{" in the commands array does not compile as a regex... }.$@);
		}
		$regex_int++;
	}
