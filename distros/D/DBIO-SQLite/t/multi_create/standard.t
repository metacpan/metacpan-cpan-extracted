...
lives_ok ( sub {
  ...
  isa_ok($cd, 'DBIO::Test::CD', 'Created CD object');
  isa_ok($cd->artist, 'DBIO::Test::Artist', 'Created related Artist');
...
lives_ok ( sub {
  ...
  isa_ok($bookmark, 'DBIO::Test::Bookmark', 'Created Bookrmark object');
  isa_ok($bookmark->link, 'DBIO::Test::Link', 'Created related Link');
  is (
    $bm_rs->search (
...
lives_ok ( sub {
  ...
  isa_ok($cd, 'DBIO::Test::CD', 'Created CD');
...
lives_ok ( sub {
  ...
  isa_ok ($c2p, 'DBIO::Test::CD_to_Producer', 'Linker object created');
  my $prod = $schema->resultset ('Producer')->find ({ name => 'Lehman Bros.' });
  isa_ok ($prod, 'DBIO::Test::Producer', 'Producer row found');
...
lives_ok ( sub {
  ...
  isa_ok ($t2->cd_single, 'DBIO::Test::CD', 'Created a single for 2nd track');
...
lives_ok ( sub {
  ...
  isa_ok ($single, 'DBIO::Test::CD', 'Created a single with the track');
...
lives_ok ( sub {
  ...
  isa_ok ($cd->artwork, 'DBIO::Test::Artwork', 'Artwork created');
...
lives_ok ( sub {
  ...
  isa_ok ($track->lyrics, 'DBIO::Test::Lyrics', 'Lyrics created');
...
lives_ok ( sub {
  ...
  isa_ok( $cd_result, 'DBIO::Test::CD', "Got Good CD Class");
  ...
  {
    isa_ok( $track, 'DBIO::Test::Track', 'Got Expected Track Class');
  }
