...
my $p   = "$dir/recover.stk";

# Learn the on-disk size for this geometry.
{ my $s = Data::Stack::Shared::Int->new($p, 16); }
my $total = -s $p;
unlink $p;
...
# 1. Recovery: an abandoned all-zero file of exactly $total bytes is re-initialized.
{
    ...
    my $s = eval { Data::Stack::Shared::Int->new($p, 16) };
    ok($s, "new() recovers an abandoned mid-init file instead of bricking") or diag $@;
  SKIP: {
        skip "no handle", 4 unless $s;
        is($s->size, 0, "recovered stack starts empty");
        ok($s->push(42), "recovered stack accepts a push");
        is($s->size, 1, "size reflects the push");
        is($s->pop, 42, "pop returns what was pushed (readback works)");
    }
    undef $s; unlink $p;
}
...
{
    my $a = Data::Stack::Shared::Int->new($p, 16);
    $a->push(7); $a->push(9);
    undef $a;
    my $r = Data::Stack::Shared::Int->new($p, 16);
    is($r->size, 2, "reopening a valid file preserves its size");
    is($r->pop, 9, "reopening a valid file preserves its data (LIFO top)");
    undef $r; unlink $p;
}
