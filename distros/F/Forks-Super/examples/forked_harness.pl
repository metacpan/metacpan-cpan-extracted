...
sub process_test_output {
	...
	# ExtUtils::MM::test_harness output
        ...
        elsif ($s =~ /Failed \(TODO\) test/) {
            # oops, that failure wasn't so bad
            if ($last_fail) {
                $fail{$test_file}{"$last_fail (TODO)"} +=
                    delete $fail{$test_file}{$last_fail};
            }
        }
    }
...
of what they do. After displaying help, causes the program
to exit without running any tests.
