package Dist::Zilla::Plugin::Codeberg::Create 2.0102;
...
sub after_mint {
   ...
   $self->log( [ 'Creating new Codeberg repository \'%s\'', $repo_name ] );
...
the newly created Codeberg repository's private URL. See L</"ADDING REMOTE"> for
more info.

=item C<repo>

Specifies the name of the Codeberg repository to be created (by default the name
...
By default C<Codeberg::Create> adds a new git remote pointing to the newly created
Codeberg repository's private URL B<if, and only if,> a git repository has already
been initialized, and if the remote doesn't already exist in that repository.

To take full advantage of this feature you should use, along with C<Codeberg::Create>,
...
C<Codeberg::Create> plugin, as follows:

    [Git::Init]
    [Codeberg::Create]
    remote = myremote

Remember to put C<[Git::Init]> B<before> C<[Codeberg::Create]>.
...
# ABSTRACT: Create a new Codeberg repo on dzil new
