package Proc::ProcessTable::Match::WChan;
...
Proc::ProcessTable::Match::WChan - Check if the wait channel of a process matches via regexp.
...
=head1 SYNOPSIS
    ...
    my $checker=Proc::ProcessTable::Match::WChan->new( \%args );
...
that is 'wchans', which is a array of wait channels to match.

The matching is done via regexp.

Atleast one wchan must be specified.
...
sub match{
	...
	my $proc_wchan;
	eval{
		$proc_wchan=$object->wchan;
	};

	# don't bother proceeding, the object won't match ever
	# as it does not have a wchan
	if ( ! defined( $proc_wchan ) ){
		return 0;
	}

	foreach my $wchan ( @{ $self->{wchans} } ){
		if ( $proc_wchan =~ /$wchan/ ){
			return 1;
		}
	}
