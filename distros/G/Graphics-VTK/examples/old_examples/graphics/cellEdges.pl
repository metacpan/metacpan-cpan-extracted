...
$aWedgeActor->GetProperty->SetDiffuseColor(0,1,0);
$ren1->AddActor('aPyramidActor');
$aPyramidActor->GetProperty->SetDiffuseColor(0,1,0);
...
$ball->SetRadius('.2');
foreach $cell ( qw/aVoxel aHexahedron aTetra aWedge aPyramid aPixel aQuad aTriangle aPolygon aTriangleStrip/)
 {
  $Edges{$cell} = Graphics::VTK::ExtractEdges->new;
  $Edges{$cell}->SetInput($Grid{$cell});
  $Tubes{$cell} = Graphics::VTK::TubeFilter->new;
  $Tubes{$cell}->SetInput($Edges{$cell}->GetOutput);
  $Tubes{$cell}->SetRadius('.1');
  $EdgesMapper{$cell} = Graphics::VTK::PolyDataMapper->new;
  $EdgesMapper{$cell}->SetInput($Tubes{$cell}->GetOutput);
  $EdgesActor{$cell} = Graphics::VTK::Actor->new;
  $EdgesActor{$cell}->SetMapper($EdgesMapper{$cell});
  $EdgesActor{$cell}->SetPosition($Actor{$cell}->GetPosition);
  $ren1->AddActor($EdgesActor{$cell});
