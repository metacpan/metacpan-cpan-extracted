...
sub client_input {
  ...
  if ( $cmd eq 'NICK' ) {
    ...
    is($params->[0],'mustard','Got mustard');
    return;
  }
  return;
}
...
sub client2_input {
  ...
  if ($cmd eq 'ERROR') {
    pass($cmd);
    is($params->[0],'Closing Link: 127.0.0.1 (SVSNICK Override)','Closing Link: 127.0.0.1 (SVSNICK Override)');
    return;
  }
  return;
}
...
sub groucho_input {
  ...
  # :1FUAAAAAA NICK mustard :1530209900
  if ( $cmd eq 'NICK' ) {
    pass($cmd);
    is($prefix,'1FUAAAAAA','Prefix okay');
    is($params->[0],'mustard','We got mustard');
    return;
  }
  return;
}
