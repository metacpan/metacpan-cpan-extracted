...
BEGIN { CORE::require Einformixv6als; }
...
# regexp of character
my $qq_char   = qr/(?> \\c[\x40-\x5F] | \\? (?:[\x81-\x9F\xE0-\xFC][\x00-\xFF]|\xFD[\xA1-\xFE][\x00-\xFF] | [\x00-\xFF]) )/oxms;
my  $q_char   = qr/(?> [\x81-\x9F\xE0-\xFC][\x00-\xFF]|\xFD[\xA1-\xFE][\x00-\xFF] | [\x00-\xFF] )/oxms;
...
sub import {
    ...
    if (Einformixv6als::e("$filename.e")) {
        if (exists $ENV{'CHAR_DEBUG'}) {
        ...
        elsif (Einformixv6als::z("$filename.e")) {
            Einformixv6als::unlink "$filename.e";
        }
        else {
            ...
            my $mtime_filter = (Einformixv6als::stat(__FILE__     ))[9];
            my $mtime_source = (Einformixv6als::stat($filename    ))[9];
            my $mtime_escape = (Einformixv6als::stat("$filename.e"))[9];
    ...
    if (not Einformixv6als::e("$filename.e")) {
        ...
        if (CORE::eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Einformixv6als::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
        }
    ...
    my $fh = gensym();
    Einformixv6als::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub INFORMIXV6ALS::escape_script {
    ...
    my $fh = gensym();
    Einformixv6als::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script\n";

    if (/^ use Einformixv6als(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Einformixv6als '%s.0'; # 'quote' for perl5.6\n", $INFORMIXV6ALS::VERSION); # require run-time routines version

        # use INFORMIXV6ALS version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | INFORMIXV6ALS ) (?>\s*) ([^\x81-\x9F\xE0-\xFD;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $INFORMIXV6ALS::VERSION) {
                    my @file = grep -e, map {qq{$_/INFORMIXV6ALS.pm}} @INC;
...
****************************************************
              ...
              CONFLICT INFORMIXV6ALS.pm FILE
...
END
                    ...
                    die "Script $0 expects INFORMIXV6ALS.pm $version, but @{[__FILE__]} is version $INFORMIXV6ALS::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Einformixv6als.pm $version, but \\\$Einformixv6als::VERSION is \$Einformixv6als::VERSION" if \$Einformixv6als::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $INFORMIXV6ALS::VERSION) {
                    die "Script $0 required INFORMIXV6ALS.pm $version, but @{[__FILE__]} is only version $INFORMIXV6ALS::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Einformixv6als::function_ord     = 'INFORMIXV6ALS::ord'     if /\A ord \z/oxms;
                    $Einformixv6als::function_ord_    = 'INFORMIXV6ALS::ord_'    if /\A ord \z/oxms;
                    $Einformixv6als::function_reverse = 'INFORMIXV6ALS::reverse' if /\A reverse \z/oxms;
                    $Einformixv6als::function_getc    = 'INFORMIXV6ALS::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= INFORMIXV6ALS::escape();
...
=item * You'll have to download INFORMIXV6ALS.pm and Einformixv6als.pm and put it in your perl lib directory.

=item * You'll need to write "use INFORMIXV6ALS;" at head of the script.
...
=head1 SYNOPSIS
  ...
  $ perl INFORMIXV6ALS.pm INFORMIX V6 ALS_script.pl > Escaped_script.pl.e
...
INFORMIXV6ALS software is "middleware" between perl interpreter and your Perl script
written in INFORMIX V6 ALS.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain INFORMIX V6 ALS, Perl3 or later
can treat INFORMIX V6 ALS as binary data.

By "use INFORMIXV6ALS;", it automatically interpret your script as INFORMIX V6 ALS. The various
functions of perl including a regular expression can treat INFORMIX V6 ALS now.
...
=item * Remains One Language Handling Raw INFORMIX V6 ALS, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. INFORMIXV6ALS.pm, Einformixv6als.pm
  +---------------------------------------+
...
import of Einformixv6als.pm works well.

   in Einformixv6als.pm
   #
...
=head1 Multiple-Octet Anchoring of Regular Expression (INFORMIXV6ALS software provides)
...
=head1 Escaping Second Octet (INFORMIXV6ALS software provides)

INFORMIXV6ALS software escapes second octet of multiple-octet character in regular
expression.
...
=head1 Multiple-Octet Character Regular Expression (INFORMIXV6ALS software provides)

INFORMIXV6ALS software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Einformixv6als::ignorecase()' (INFORMIXV6ALS software provides)
...
Definitions in Einformixv6als.pm.
  ...
  ${Einformixv6als::anchor}         qr{\G(?>[^\x81-\x9F\xE0-\xFD]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|\xFD[\xA1-\xFE][\x00-\xFF])*?}
                           ...
                           qr{\G(?(?=.{0,32766}\z)\G(?>[^\x81-\x9F\xE0-\xFD]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|\xFD[\xA1-\xFE][\x00-\xFF])*?|(?(?=[\x00-\x80\xA0-\xDF\xFE]+\z).*?|(?:.*?[\x00-\x80\xA0-\xDF\xFF](?>[\x81-\x9F\xE0-\xFC][\x00-\xFF]|\xFD[\xA1-\xFE][\x40-\x7E\x80-\xFC]|\xFD[\xA1-\xFE][\xA1-\xFE]|\xFE)*?)))}oxms
                           for over 32766 octets string on ActivePerl5.6 and Perl5.10 or later
                           qr{\G(?(?=.{0,32766}\z)\G(?>[^\x81-\x9F\xE0-\xFD]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|\xFD[\xA1-\xFE][\x00-\xFF])*?|(?(?=[$sbcs]+\z).*?|(?:.*?[$sbcs](?:[^$sbcs][^$sbcs])*?)))}oxms
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (INFORMIXV6ALS software provides)

INFORMIXV6ALS software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=item * Ordinal Value of Character
  ...
  This subroutine returns the numeric value (ASCII or INFORMIX V6 ALS character) of the
  first character of $string, not Unicode. If $string is omitted, it uses $_.
  The return value is always unsigned.

  If you import ord "use INFORMIXV6ALS qw(ord);", ord of your script will be rewritten in
  INFORMIXV6ALS::ord. INFORMIXV6ALS::ord is not compatible with ord of JPerl.
...
=item * Reverse List or String
  ...
  If you import reverse "use INFORMIXV6ALS qw(reverse);", reverse of your script will be
  rewritten in INFORMIXV6ALS::reverse. INFORMIXV6ALS::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  $getc = INFORMIXV6ALS::getc(FILEHANDLE);
  $getc = INFORMIXV6ALS::getc($filehandle);
  $getc = INFORMIXV6ALS::getc;
  ...
  If you import getc "use INFORMIXV6ALS qw(getc);", getc of your script will be rewritten
  in INFORMIXV6ALS::getc. INFORMIXV6ALS::getc is not compatible with getc of JPerl.
...
=item * Length by INFORMIX V6 ALS Character
  ...
  of the scalar value $string. If $string is omitted, it returns the INFORMIXV6ALS::length
  of $_.

  Do not try to use INFORMIXV6ALS::length to find the size of an array or hash. Use scalar
...
=item * Substr by INFORMIX V6 ALS Character
  ...
  my $end    = INFORMIXV6ALS::substr $s, 14;        # climbed the green tree
  my $tail   = INFORMIXV6ALS::substr $s, -4;        # tree
  my $z      = INFORMIXV6ALS::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the INFORMIXV6ALS::substr() subroutine as an
  ...
  end of the string, INFORMIXV6ALS::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  INFORMIXV6ALS::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = INFORMIXV6ALS::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = INFORMIXV6ALS::substr $name, 7;      # returns undef, with warning
  INFORMIXV6ALS::substr($name, 7) = 'gap';        # raises an exception
  ...
  my $s = "The black cat climbed the green tree";
  my $z = INFORMIXV6ALS::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of INFORMIXV6ALS::substr() acts
  ...
  $x = '1234';
  for (INFORMIXV6ALS::substr($x,1,2)) {
      $_ = 'a';   print $x,"\n";    # prints 1a4
      $_ = 'xyz'; print $x,"\n";    # prints 1xyz4
      $x = '56789';
      $_ = 'pq';  print $x,"\n";    # prints 5pq9
  ...
  $x = '1234';
  for (INFORMIXV6ALS::substr($x, -3, 2)) {
      $_ = 'a';   print $x,"\n";    # prints 1a4, as above
      $x = 'abcdefg';
      print $_,"\n";                # prints f
  }
...
=item * Rindex by INFORMIX V6 ALS Character
  ...
  $rindex = INFORMIXV6ALS::rindex($string,$substring,$offset);
  $rindex = INFORMIXV6ALS::rindex($string,$substring);

  This subroutine works just like INFORMIXV6ALS::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse INFORMIXV6ALS::index).
...
=item * Eval INFORMIX V6 ALS Script

  $eval = INFORMIXV6ALS::eval { block };
  $eval = INFORMIXV6ALS::eval $expr;
  $eval = INFORMIXV6ALS::eval;

  The INFORMIXV6ALS::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, INFORMIXV6ALS::eval { block }
  and INFORMIXV6ALS::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, INFORMIXV6ALS::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of INFORMIXV6ALS::eval, the value returned from an INFORMIXV6ALS::eval is
  ...
  INFORMIXV6ALS::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  INFORMIXV6ALS::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      INFORMIXV6ALS::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The INFORMIXV6ALS::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow INFORMIXV6ALS::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  INFORMIXV6ALS::eval cannot trap syntax errors.

  The INFORMIXV6ALS::eval $expr form can trap syntax errors because it parses the code
  ...
  after the INFORMIXV6ALS::eval is complete, as do any subroutine or format definitions.
  The code of the INFORMIXV6ALS::eval is treated as a block, so any locally scoped
  variables declared within the INFORMIXV6ALS::eval last only until the INFORMIXV6ALS::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  INFORMIXV6ALS::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  INFORMIXV6ALS::eval { block }    eval { block }
  INFORMIXV6ALS::eval $expr        eval INFORMIXV6ALS::escape $expr
  INFORMIXV6ALS::eval              eval INFORMIXV6ALS::escape
  -------------------------------------------------

  To tell the truth, the subroutine INFORMIXV6ALS::eval does not exist. If it exists,
  you will troubled, when INFORMIXV6ALS::eval has a parameter that is single quoted
  string included my variables. INFORMIXV6ALS::escape is a subroutine that makes Perl
  ...
  Since INFORMIXV6ALS::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because INFORMIXV6ALS::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  INFORMIXV6ALS::eval $expr and INFORMIXV6ALS::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      INFORMIXV6ALS::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      INFORMIXV6ALS::eval { $answer = }; # WRONG

      # a runtime syntax error
      INFORMIXV6ALS::eval '$answer =';   # sets $@
  ...
  Using the INFORMIXV6ALS::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      INFORMIXV6ALS::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          INFORMIXV6ALS::eval { die "foo lives here" };
  ...
  With an INFORMIXV6ALS::eval, you should be especially careful to remember what's being
  looked at when:

      INFORMIXV6ALS::eval $x;        # CASE 1
      INFORMIXV6ALS::eval "$x";      # CASE 2

      INFORMIXV6ALS::eval '$x';      # CASE 3
      INFORMIXV6ALS::eval { $x };    # CASE 4

      INFORMIXV6ALS::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  a temporary is required if you want to mask some but not all errors:
      ...
      # alter $@ on nefarious repugnancy only
      {
          my $e;
          {
              ...
              INFORMIXV6ALS::eval { test_repugnancy() };
  ...
  The block of INFORMIXV6ALS::eval { block } does not count as a loop, so the loop
...
=item * Filename Globbing
  ...
  This operator is implemented via the Einformixv6als::glob() subroutine. See Einformixv6als::glob
  of Einformixv6als.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use INFORMIXV6ALS qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use INFORMIXV6ALS qw(reverse);" exists or not.
...
=head1 Yada Yada Operator (INFORMIXV6ALS software provides)
...
=head1 Un-Escaping bytes::* Subroutines (INFORMIXV6ALS software provides)
...
some bugs. Thus, if you happen to find a bug that's in INFORMIXV6ALS software and
...
If Perl version is older than 5.14, INFORMIXV6ALS::substr differs from CORE::substr, and
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Einformixv6als::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${`}            Einformixv6als::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Einformixv6als::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Einformixv6als::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Einformixv6als::MATCH()       $1
  ${&}            Einformixv6als::MATCH()       $1
  $MATCH          Einformixv6als::MATCH()       $1
  ${^MATCH}       Einformixv6als::MATCH()       $1
  $'              $'                   $'
  ${'}            ${'}                 $'
  $POSTMATCH      Einformixv6als::POSTMATCH()   $'
  ${^POSTMATCH}   Einformixv6als::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
Even if ${^WIN32_SLOPPY_STAT} is set to a true value, file test functions Einformixv6als::*(),
Einformixv6als::lstat(), and Einformixv6als::stat() on Microsoft Windows open the file for the path
...
exist at time of be written this document and JPerl did exist.
                        ...
                        (a)     (b)     (c)     (d)     (e)
                                      JPerl,japerl    Encode,INFORMIXV6ALS
      +--------------+-------+-------+-------+-------+-------+
      | data         |  Old  |  Old  |  New  |  Old  |  New  |
      +--------------+-------+-------+-------+-------+-------+
      | script       |  Old  |      Old      |      New      |
...
INFORMIXV6ALS software remains one language and one interpreter.
