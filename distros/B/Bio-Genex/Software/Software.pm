...
# Bio::Genex::Software
#
# created on Mon Feb  5 21:23:57 2001 by /home/jasons/work/GeneX-Server/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-Server/Genex --target=Software
#
# cvs id: $Id: Software.pm,v 1.17 2001/02/06 18:58:52 jes Exp $ 
#
##############################
...
attributes (no_lookup=>['fetched', 'fetch_all', 'fetched_attr', 'id'], lookup=>['sw_pk', 'name', 'version', 'type', 'con_fk', 'con_obj']);

sub table_name {return 'Software';} # probably unnecessary
...
sub insert_db {
  ...
  # execute the INSERT
  my $sql = create_insert_sql($dbh,'Software',\%values);
  $dbh->do($sql);
...
sub update_db {
  ...
  # on error
  if ($dbh->err) {
    warn "Bio::Genex::Software::update_db: SQL=<$sql>, DBI=<$DBI::errstr>";
    return undef;
  }
  return 1;
}
...
sub get_all_objects {
  ...
  if (ref($_[0]) eq 'HASH') {
    ...
    die "Bio::Genex::Software::get_all_objects: Must define both 'column' and 'value'" 
  ...
  my $sth = $dbh->prepare($sql) 
    or die "Bio::Genex::Software::get_all_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
  $sth->execute() 
    or die "Bio::Genex::Software::get_all_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>";
  ...
  die "Bio::Genex::Software::get_all_objects:\nSQL=<$sql>,\nDBI=<$DBI::errstr>" 
    if $sth->err;
...
sub get_objects {
  ...
  if (ref($_[0]) eq 'HASH' || scalar @_ == 0) {
    croak("Bio::Genex::Software::get_objects called with no ID's, perhaps you meant to use Bio::Genex::Software::get_all_objects
");
  } 
  my @ids = @_;
  my $obj;
...
sub initialize {
  ...
  if (defined $id) {
    ...
    die "Bio::Genex::Software::initialize: $DBI::errstr" if $dbh->err;
  
    # if there was a problem, return an error to new(), so that 
    # new will return undef to the calling function
    if ($count < 1) {
      warn("Bio::Genex::Software::initialize: no DB entries for id: $id");
...
sub fetch {
  ...
  my $sql = create_select_sql($dbh,
                    COLUMNS=>['sw_pk', 'name', 'version', 'type', 'con_fk'],
                    FROM=>[$self->table_name()],
                    WHERE=>$self->pkey_name() . " = '$pkey'",
                              );
  my $sth = $dbh->prepare($sql) || die "Bio::Genex::Software::initialize: $DBI::errstr";
  $sth->execute() || die "Bio::Genex::Software::initialize: $DBI::errstr";

  # sanity check to see if bogus id
  my $ref = $sth->fetchrow_hashref();
  die "Software: ", $self->pkey_name(), " $pkey, not in DB"
    unless defined $ref;
...
Bio::Genex::Software - Methods for processing data from the GeneX DB
 table: Software
...
of the same name (I<i.e.> the corresponding table for Bio::Genex::Software is
Software).
...
Bio::Genex::Software can access: I<raw> foreign key attributes,
...
Class Bio::Genex::Software defines the following utility variables for assisting
programmers to access the Software table.

=over 4

=item $Bio::Genex::Software::LIMIT
...
=item $Bio::Genex::Software::USE_CACHE
...
instance of the class via the Bio::Genex::Software->methodname() syntax.
...
linking table class. For Bio::Genex::Software it returns 0, since it is
...
table classes, and for Bio::Genex::Software it returns the value 'sw_pk';


=item table_name()

Returns the name of the DB table represented by this class. For
Bio::Genex::Software it returns 'Software';
...
C<insert_db()>.
  ...
  my Software = Bio::Genex::Software->new();
  Software->name('some_value');
  Software->insert_db($dbh);
...
C<update_db()>.
  ...
  my $dbh = Bio::Genex::current_connection(USER=>$SU_USERNAME,
                                      PASSWORD=>$SU_PASSWORD);
  my Software = Bio::Genex::Software->new(id=>43);
  Software->name('some_value');
  Software->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('sw_pk' for module Bio::Genex::Software).
...
This method is used to retrieve multiple instances of class Bio::Genex::Software
...
list as a primary key for the Software table and returns a single
instance for each entry.

B<WARNING>: Passing incorrect id values to C<get_objects()> will cause
a warning from C<Bio::Genex::Software::initialize()>. Objects will be
...
instance of class Bio::Genex::Software.
...
(and for class Bio::Genex::Software it is synonomous with the
C<sw_pk()>method). The C<id()> method can be useful in writing
generic methods because it avoids having to know the name of the
primary key column. 

=item sw_pk()

This is the primary key attribute for Bio::Genex::Software. It has no setter method. 
...
Methods for the version attribute.
...
on Mon Feb  5 21:23:57 2001 by /home/jasons/work/GeneX-Server/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-Server/Genex --target=Software
