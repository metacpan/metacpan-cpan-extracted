...
----------------------------------------------------------------------
    ...
    src/ppport.h -- Perl/Pollution/Portability Version 3.68

    Automatically created by Devel::PPPort running under perl 5.036001.

    Do NOT edit this file directly! -- Edit PPPort_pm.PL and the
    includes in parts/inc/ instead.

    Use 'perldoc src/ppport.h' to view the documentation below.
...
=head1 SYNOPSIS
  ...
  perl src/ppport.h [options] [source files]
...
This version of F<src/ppport.h> is designed to support operation with Perl
...
Display the version of F<src/ppport.h>.
...
Tell F<src/ppport.h> to check for compatibility with the given
Perl version. The default is to check for compatibility with Perl
version 5.003_07. You can use this option to reduce the output
of F<src/ppport.h> if you intend to be backward compatible only
down to a certain Perl version.

=head2 --cplusplus

Usually, F<src/ppport.h> will detect C++ style comments and
replace them with C style comments for portability reasons.
Using this option instructs F<src/ppport.h> to leave C++
comments untouched.
...
Strip all script and documentation functionality from F<src/ppport.h>.
This reduces the size of F<src/ppport.h> dramatically and may be useful
if you want to include F<src/ppport.h> in smaller modules without
increasing their distribution size too much.

The stripped F<src/ppport.h> will have a C<--unstrip> option that allows
...
F<src/ppport.h>. Also lists if it must be explicitly requested,
if it has dependencies, and if there are hints or warnings for it.

=head2 --list-unsupported

Lists the API elements that are known not to be FULLY supported by F<src/ppport.h>,
...
version of Perl that F<src/ppport.h> supports at all.
...
earlier Perl releases. Use
    ...
    perl src/ppport.h --list-provided

to see which API elements are provided by src/ppport.h.
...
also deprecated. You can safely use the supported API, as F<src/ppport.h>
will provide wrappers for older Perl versions.

=item *

Although the purpose of F<src/ppport.h> is to keep you from having to concern
...
more C<#define>s in your source code before the inclusion of F<src/ppport.h>.
...
macro. Just C<#define> the macro before including C<src/ppport.h>:

    #define DPPP_NAMESPACE MyOwnNamespace_
    #include "src/ppport.h"
...
F<src/ppport.h> on your source code. See the next section for
details.

=head1 EXAMPLES

To verify whether F<src/ppport.h> is needed for your module, whether you
should make any changes to your code, and whether any special defines
should be used, F<src/ppport.h> can be run as a Perl script to check your
...
use something like this:
    ...
    perl src/ppport.h --compat-version=5.6.0 --cplusplus --patch=test.diff

If you only want your code to be scanned without any suggestions
for changes, use:

    perl src/ppport.h --nochanges

You can specify a different C<diff> program or options, using
the C<--diff> option:

    perl src/ppport.h --diff='diff -C 10'

This would output context diffs with 10 lines of context.

If you want to create patched copies of your files instead, use:

    perl src/ppport.h --copy=.new

To display portability information for the C<newSVpvn> function,
use:

    perl src/ppport.h --api-info=newSVpvn

Since the argument to C<--api-info> can be a regular expression,
you can use

    perl src/ppport.h --api-info=/_nomg$/

to display portability information for all C<_nomg> functions or

    perl src/ppport.h --api-info=/./
...
If this version of F<src/ppport.h> is causing failure during
the compilation of this module, please check if newer versions
of either this module or C<Devel::PPPort> are available on CPAN
before sending a bug report.

If F<src/ppport.h> was generated using the latest version of
...
# These are tools that must be included in src/ppport.h.  It doesn't work if given
# a .pl suffix.
#
...
/* Hint: isPSXSPC, isPSXSPC_A, isPSXSPC_L1, isPSXSPC_utf8_safe
    ...
    src/ppport.h SPACE version does match VT in all perl releases.  Since VT's are
    extremely rarely found in real-life files, this difference effectively
    doesn't matter */

/* Hint: isSPACE, isSPACE_A, isSPACE_L1, isSPACE_utf8_safe
    Until Perl 5.18, this did not match the vertical tab (VT).  The src/ppport.h
...
/* Hint: SvPVbyte
 * Does not work in perl-5.6.1, src/ppport.h implements a version
