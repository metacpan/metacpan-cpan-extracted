...
$opts->set_free_stri("[original] [most_recent] [input] [output]");
$opts->set_free_mini(4);
$opts->set_free_maxi(4);
$opts->analyze(\@ARGV);

my($original,$most_recent,$input,$output)=($ARGV[0],$ARGV[1],$ARGV[2],$ARGV[3]);
my($scod)=Meta::Utils::System::system_nodie("fmerge",[$original,$most_recent,$input,"-o",$output,"-c",$output.",C"]);
...
aegis_conf_merge_command.pl - run 3way merge on files.
...
=head1 DETAILS
	...
	MANIFEST: aegis_conf_merge_command.pl
	PROJECT: meta
	VERSION: 0.01

=head1 SYNOPSIS

	aegis_conf_merge_command.pl

=head1 DESCRIPTION

This command is called to 3way merge files.
We use fmerge to get this done.
