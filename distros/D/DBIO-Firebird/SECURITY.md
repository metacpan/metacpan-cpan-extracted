This is the Security Policy for the Perl DBIO-Firebird distribution.
...
[DBIO-Firebird on CPAN](https://metacpan.org/dist/DBIO-Firebird) or the main branch of the
[DBIO-Firebird git repository](https://codeberg.org/dbio/dbio-firebird).
...
DBIO-Firebird is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO-Firebird are covered by this policy.
...
they affect DBIO-Firebird, or DBIO-Firebird can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO-Firebird, or plugins to it that are not included with the DBIO-Firebird
distribution) are not covered by this policy.

## Supported Versions of DBIO-Firebird

Only the latest release of DBIO-Firebird will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO-Firebird will run on older versions of Perl. If a
...
that are required for DBIO-Firebird to work. However, some of these
