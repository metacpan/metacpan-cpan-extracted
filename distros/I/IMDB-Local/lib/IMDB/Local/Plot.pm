...
IMDB::Local::Plot - Object representation of Plot information.
...
use constant DB_TABLE => 'Plots';
use constant DB_KEY   => 'PlotID';
...
sub createNew($%)
{
    ...
    return new IMDB::Local::Plot(imdbdb=>$imdbdb, TableID=>$id);
}

sub findByPlotID($$)
{
    my ($imdbdb, $id)=@_;
    return new IMDB::Local::Plot(imdbdb=>$imdbdb, DB_KEY, $id);
}
