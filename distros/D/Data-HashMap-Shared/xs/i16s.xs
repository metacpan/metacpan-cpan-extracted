    ...
    CODE:
        ...
        if (h->shard_handles) {
            for (int i = 1; i < items; i += 2) {
                ...
                count += shm_i16s_put(h, (int16_t)SvIV(ST(i)), _vs, (uint32_t)_vl, _vu);
            }
        } else {
            WRSEQ_GUARD(h);
            for (int i = 1; i < items; i += 2) {
                ...
                bool _vu = SvUTF8(ST(i+1)) ? 1 : 0;
                if (_vl > SHM_MAX_STR_LEN) croak("value too long (max 1GB)");
                count += shm_i16s_put_inner(h, (int16_t)SvIV(ST(i)), _vs, (uint32_t)_vl, _vu, SHM_TTL_USE_DEFAULT);
            }
        }
        RETVAL = count;
    OUTPUT:
    ...
    CODE:
        ...
        if (h->shard_handles) {
            for (int i = 1; i < items; i++)
                count += shm_i16s_remove(h, (int16_t)SvIV(ST(i)));
        } else {
            WRSEQ_GUARD(h);
            for (int i = 1; i < items; i++)
