...
This document describes version 0.003 of Sah::Schema::dns::record::txt (from Perl distribution Sah-SchemaBundle-DNS), released on 2024-06-26.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("dns::record::txt*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("dns::record::txt", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("dns::record::txt", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("dns::record::txt*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"dns::record::txt*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('dns::record::txt*', name=>'DnsRecordTxt')
     );
 }

 use My::Types qw(DnsRecordTxt);
 DnsRecordTxt->assert_valid($data);
