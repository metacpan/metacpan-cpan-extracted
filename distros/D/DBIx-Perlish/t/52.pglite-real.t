...
is($h{3}{name}, "eh'lo", "fetch all hash unfiltered 3 key name");
...
ok(!$h{"eh'lo"},  "multi-key ehlo not present");
ok($h{1}{hello},   "multi-key 1/hello present");
ok($h{3}{"eh'lo"},   "multi-key 3/ehlo present");
ok(!$h{1}{"eh'lo"},   "multi-key 1/ehlo not present");
...
is($h{3}{"eh'lo"}{id},   3, "multi-key 3/ehlo key id");
is($h{3}{"eh'lo"}{name}, "eh'lo", "multi-key 3/ehlo key name");
...
ok(!exists $h{3}{"eh'lo"}{'$kf-1'}, "multi-key no key field");
ok(!exists $h{3}{"eh'lo"}{'$kf-2'}, "multi-key no key field");

%h = db_fetch { my $t : names; return -k $t->id, $t->name; };
ok(%h, "fetch key flatten");
is($h{1}, "hello",  "key flatten 1 value");
is($h{3}, "eh'lo",  "key flatten 3 value");
...
is($h{3}{"eh'lo"}, "eh'lo",  "key flatten 3/ehlo value");
...
is($h{3}{name}, "eh'lo", "key noflatten 3 key name");
...
is($n[1], "eh'lo", "fetch array val 2");
...
is($n[1]->{name}, "eh'lo", "fetch array all val 2/2");
