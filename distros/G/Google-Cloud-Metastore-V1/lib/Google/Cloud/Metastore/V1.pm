...
sub BUILD {
    ...
    my $client = Google::gRPC::Client->new(
        target     => 'metastore.googleapis.com:443',
        auth_token => $token,
    );
...
Google::Cloud::Metastore::V1 - Google Cloud Dataproc Metastore V1 API Client

=head1 SYNOPSIS

    use Google::Cloud::Metastore::V1;
    use Google::Auth;

    my $auth = Google::Auth->default();
    my $client = Google::Cloud::Metastore::V1->new(credentials => $auth);

=head1 DESCRIPTION

Google Cloud Dataproc Metastore V1 API Client over high-performance gRPC transport.
