...
use strict;
{
    ...
    package OOP::Perlish::Class::Accessor::UnitTests::HashRef;
    use warnings;
    use strict;
    use OOP::Perlish::Class::Accessor::UnitTests::Hash;
    use base 'OOP::Perlish::Class::Accessor::UnitTests::Hash';
    ...
    {
        my ($self) = @_;
        $self->{accessor} = OOP::Perlish::Class::Accessor->new( type => 'HASHREF', name => 'test', self => bless( {}, __PACKAGE__ ) );
    }
    ...
    sub get_value
    {
        ...
        return( %{ $self->{accessor}->value() } ) if(ref($self->{accessor}->value()) eq 'HASH');
    }
    ...
    sub cannot_mutate_reference : Test(1)
    {
        ...
        $self->{accessor}->value(baz => 'quux');
        $self->{accessor}->value()->{foo} = 'bar';
        ok( ! exists($self->{accessor}->value()->{foo}), 'cannot mutate instance reference' ) || diag(Dumper($self->{accessor}->value()));
    }
    ...
    sub can_mutate_explicitely_mutable_reference : Test(1)
    {
        ...
        $self->{accessor}->value(baz => 'quux');
        $self->{accessor}->value()->{foo} = 'bar';
        ok( exists($self->{accessor}->value()->{foo}), 'can mutate mutable instance reference' ) || diag(Dumper($self->{accessor}->value()));
    }
