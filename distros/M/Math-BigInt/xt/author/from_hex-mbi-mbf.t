...
subtest '$x = Math::BigInt -> from_hex("1.2p+2");'
  => sub {
      # this must not upgrade

      my $x = Math::BigInt -> from_hex("1.2p+2");

      is(ref($x), "Math::BigInt", "class");
      is($x, "NaN", "value");
  };

subtest '$x = Math::BigFloat -> from_hex("4");'
  => sub {
      # this must not downgrade

      my $x = Math::BigFloat -> from_hex("4");

      is(ref($x), "Math::BigFloat", "class");
      is($x, "4", "value");
  };

subtest '$x = Math::BigInt -> new("4"); $q = $x -> from_hex("1.2p+2");'
  => sub {
      # this must not upgrade

      my $x = Math::BigInt -> new("4");
      my $q = $x -> from_hex("1.2p+2");                 # = 4.5
...
subtest '$x = Math::BigFloat -> new("4.5"); $q = $x -> from_hex("4");'
  => sub {
      # this must not downgrade
...
subtest '$x = Math::BigInt -> from_hex("1.2p+2");'
  => sub {
      ...
      my $x = Math::BigInt -> from_hex("1.2p+2");       # = 4.5
