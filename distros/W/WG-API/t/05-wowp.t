...
my WG::API::WoWp $wowp = WG::API->new( application_id => $ENV{'WG_KEY'} || 'demo' )->wowp;
...
SKIP: {
    ...
    ok( $wowp->ratings_accounts( type => 1, account_id => $accounts->[0]->{'account_id'} ),
        'get account rating without params' );

    is( $wowp->ratings_neighbors, undef, 'get rating neighbors without params' );
    is( $wowp->ratings_neighbors( type => 1, account_id => 'xxx', rank_field => 'battles_count' ),
        undef, 'get rating neighbors with invalid params' );
    ok( $wowp->ratings_neighbors(
            type       => 1,
            account_id => $accounts->[1]->{'account_id'},
            rank_field => 'battles_count'
        ),
        'get rating neighbors with valid params'
    );
