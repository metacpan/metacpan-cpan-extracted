...
my @io_encoding = (grep( ! /^(cp932|cp932ibm|cp932nec|sjis2004|unicode)$/, @encoding), 'jef9p');

my $fileno = 5001;
for my $INPUT_encoding (@io_encoding) {
    for my $OUTPUT_encoding (@io_encoding) {
        my $filename = sprintf("%04d_${OUTPUT_encoding}_by_${INPUT_encoding}_RT_DBCS.t", $fileno++);
...
END___________________________________________________________________
        ...
        for my $octet1 (0x00 .. 0xFF) {
            for my $octet2 (0x00 .. 0xFF) {
                my $hex_utf8jp = $tr{'cp932x'}{'utf8jp'}{sprintf('%02X%02X',$octet1,$octet2)};
                next if $hex_utf8jp eq '';
        ...
        for my $octet1 (0x00 .. 0xFF) {
            for my $octet2 (0x00 .. 0xFF) {
                my $hex_utf8jp = $tr{'cp932x'}{'utf8jp'}{sprintf('9C5A%02X%02X',$octet1,$octet2)};
                next if $hex_utf8jp eq '';
