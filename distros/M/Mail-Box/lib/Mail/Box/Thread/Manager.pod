...
A thread manager object is usually created by a L<Mail::Box::Manager|Mail::Box::Manager>.
...
=item folder => $folder|\@folders

Specifies which C<@folders> are to be covered by the threads.  You can
...
See L<Mail::Box|Mail::Box>::timespan2seconds() for the possibilities for TIME.
...
=item $obj-E<gt>B<msgById>( $id, [$node] )

Returns the message which the specified C<$id>.
...
Add one or more C<@folders> to the LIST of folders whose messages are
...
Remove one or more C<@folders> from the LIST of folders whose messages are
...
will be sorted on timestamp, But a different C<$compare> method can be
specified.

=item $obj-E<gt>B<thread>($message)

Returns the thread where this C<$message> is the start of.  However, there
...
Based on a C<$message>, and facts from previously detected threads, try
...
Remove the specified C<@messages> in/from the threads managed by
this object, if this folder is maintained by this thread-manager.
...
L<Mail::Box::Manager|Mail::Box::Manager>, using L<Mail::Box::Manager::threads()|Mail::Box::Manager/"Attributes">.
...
=item Error: attempt to include a none folder: $what.

Cast by C<includeFolder()>

=item Error: attempt to remove a none folder: $something.

Cast by C<removeFolder()>
...
=item Error: thread manager needs a folder manager to work with.
