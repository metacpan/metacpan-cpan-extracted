...
package Apache2::AMFLiteDetectionFilter; 
  ...
  my $hostNames = "github.com";
  my @hostName = split(/,/, $hostNames);
  my $numberOfAvaiableHosts=@hostName;
  my $correctHost=0;
  my $correctHostName=$hostName[$correctHost];
  my $hostServer="https://".$correctHostName."/ifuschini/amf/blob/master/repository/";
  my $urlmobile=$hostServer."litemobiledetection.config";
  my $urlTablet=$hostServer."litetabletdetection.config";
  my $urlTouch=$hostServer."litetouchdetection.config";
  my $urlTv=$hostServer."litetvdetection.config";
  my $urlBot=$hostServer."litebotdetection.config";
