...
This returns the names of the message sets that have been loaded.  B<SETi>
...
all message sets, the default is to use B<LOCALE0>, followed by B<LOCALE1>,
B<LOCALE2>, etc.
...
would look for a messages in the B<BUTTONS> set in the locales: 'en',
...
substitutes in B<VALUES> (described below in L</"MESSAGE SUBSTITUTIONS>).  If
B<LOCALE> is present, it only looks up the message in that locale.  By
default, it will look in the search order for that set.

If the message is not found, C<$text> will be the empty string.
...
returns the version of the message in the given locale.  If B<LOCALE> is
...
simple formats (i.e. that start with C<%> with no trailing strings) are
allowed, so B<FORMAT> can be something like C<%.3f> but not C<the number %3d>
or C<%3d foo>.  It must also take exactly one argument, so C<%%> is not
allowed as it does not take any arguments.
...
If you find a bug in Locale::VersionedMessages, please let me know.  As CPAN has removed
...
L<https://github.com/SBECK-github/Locale-VersionedMessages>
...
=item B<Locale::VersionedMessages version> 

Please include the version of Locale::VersionedMessages you are using.  You can get
this by using the script:

   use Locale::VersionedMessages; 
   print $Locale::VersionedMessages::VERSION,"\n";
...
L<Locale::VersionedMessages::Overview>
