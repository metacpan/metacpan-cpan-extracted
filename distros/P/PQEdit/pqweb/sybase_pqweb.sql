...
/* This sybase_pqweb.sql DDL script assumes you have created and sized a database with the name pqweb. */
/* To use this script make sure that the isql program is in your path and configured properly and then run: */
/*      isql -Sserver_name -Usa -i sybase_pqweb.sql */
...
execute sp_addtype T_field_name, 'nvarchar(50)', 'null'
go
...
execute sp_addtype T_pq_result, 'nvarchar(50)', 'null'
go
...
execute sp_addtype T_table_name, 'nvarchar(120)', 'null'
go
...
(
    ...
    constraint PK_RESULT_TYPES primary key clustered (result_enum)
)
go
...
(
    ...
    constraint PK_WEB_ELEMENTS primary key clustered (web_elements)
)
go
...
(
    ...
    constraint PK_WEB_PROPERTY primary key clustered (web_property)
)
go
...
(
    ...
    constraint PK_PQWEB primary key clustered (pq)
)
go
...
(
    ...
    constraint PK_PQ_LABELS primary key clustered (pq, field_name)
)
go
...
(
    ...
    constraint PK_PQ_NOSHOW primary key clustered (noshow_field)
)
go
...
(
    ...
    constraint PK_PQ_WEB primary key clustered (pq, web_elements, web_property)
)
go
