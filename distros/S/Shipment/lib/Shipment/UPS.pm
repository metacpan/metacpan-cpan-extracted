            ...
            foreach my $service (@{$response->get_RatedShipment()}) {
                ...
                $services{$service->get_Service()->get_Code()->get_value} =
                  Shipment::Service->new(
                    ...
                    name => (
                        $self->service_map->{$self->from_address()
                              ->country_code}
                          ->{$service->get_Service()->get_Code()->get_value}
                          || $self->service_map->{$service->get_Service()
                              ->get_Code()->get_value}
                    ),
    ...
    try {
        ...
        if (!$response) {
                ...
                @{$response->get_Shipment()->get_Package()->get_Activity()})
            {
                ...
                {
                    $city = $activity->get_ActivityLocation()->get_Address()
                      ->get_City()->get_value();
                    $state = $activity->get_ActivityLocation()->get_Address()
                      ->get_StateProvinceCode()->get_value();
                    $country = $activity->get_ActivityLocation()->get_Address()
                      ->get_CountryCode()->get_value();
                }
                $self->add_activity(
                    Shipment::Activity->new(
                        description =>
                          $activity->get_Status()->get_Description()
                          ->get_value(),
