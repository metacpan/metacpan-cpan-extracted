...
# Mite keywords
BEGIN {
    my ( $SHIM, $CALLER ) = ( "MyTest::Mite", "MyTest::TestClass::Bool" );
...
sub new {
    ...
    # Attribute attr (type: Bool)
    # has declaration, file lib/MyTest/TestClass/Bool.pm, line 42
    do { my $value = exists( $args->{"attr"} ) ? $args->{"attr"} : $MyTest::TestClass::Bool::__attr_DEFAULT__->( $self ); (!ref $value and (!defined $value or $value eq q() or $value eq '0' or $value eq '1')) or MyTest::Mite::croak "Type check failed in constructor: %s should be %s", "attr", "Bool"; $self->{"attr"} = $value; }; 
