    ...
    our %EXPORT_TAGS =
        (
         ...
         pg_types => [qw($DBDPG_DEFAULT PG_ASYNC
            ...
            PG_REGCOLLATION PG_REGCOLLATIONARRAY PG_REGCONFIG PG_REGCONFIGARRAY PG_REGDATABASE
            PG_REGDATABASEARRAY PG_REGDICTIONARY PG_REGDICTIONARYARRAY PG_REGNAMESPACE PG_REGNAMESPACEARRAY
            ...
            PG_TRIGGER PG_TSMULTIRANGE PG_TSMULTIRANGEARRAY PG_TSM_HANDLER PG_TSQUERY
            PG_TSQUERYARRAY PG_TSRANGE PG_TSRANGEARRAY PG_TSTZMULTIRANGE PG_TSTZMULTIRANGEARRAY
...

{
    ...
    sub data_sources {
        ...
        if ($ENV{DBI_DSN}) {
            ($connstring = $ENV{DBI_DSN}) =~ s/dbi:PgAsync://i;
        }
        if (length $conninfo) {
            $connstring .= ";$conninfo";
        }
        ...
        my @sources = map { "dbi:PgAsync:dbname=$_->[0]$conninfo" } @{$sth->fetchall_arrayref()};
        $dbh->disconnect;
        return @sources;
    }
...

{
    ...
    sub parse_trace_flag {
        return DBD::PgAsync->parse_trace_flag($_[1]);
    }
...
## Driver information:
   ...
   10021 => ['SQL_ASYNC_MODE',                      2                         ], ## SQL_AM_STATEMENT
     ...
     121 => ['SQL_BATCH_SUPPORT',                   3                         ], ## 12 SELECT_PROC + ROW_COUNT_PROC
       2 => ['SQL_DATA_SOURCE_NAME',                sub { sprintf 'dbi:PgAsync:%s', shift->{Name} } ],
     ...
     135 => ['SQL_DRIVER_HDESC',                    0                         ], ## not applicable
      ...
      76 => ['SQL_DRIVER_HLIB',                     0                         ], ## not applicable
       ...
       ## Not clear what should go here. Some things suggest 'Pg', others 'Pg.pm'. We'll use DBD::PgAsync for now
       6 => ['SQL_DRIVER_NAME',                     'DBD::PgAsync'                 ],
...
=head1 SYNOPSIS
  ...
  $dbh = DBI->connect("dbi:PgAsync:dbname=$dbname", '', '', {AutoCommit => 0});
  # The AutoCommit attribute should always be explicitly set

  # For some advanced uses you may need PostgreSQL type values:
  use DBD::PgAsync qw(:pg_types);
...
The following connect statement shows almost all possible parameters:
  ...
  $dbh = DBI->connect("dbi:PgAsync:dbname=$dbname;host=$host;port=$port;options=$options",
                      $username,
                      $password,
...
e.g. C<dbi:PgAsync:dbname='\'spacey\' name';host=$host>.
...
service named "zephyr", you could use:
  ...
  $dbh = DBI->connect("dbi:PgAsync:service=zephyr", '', '');

You could also set C<$ENV{PGSERVICE}> to "zephyr" and connect like this:

  $dbh = DBI->connect("dbi:PgAsync:", '', '');
...
default socket directory from the one used to compile DBD::PgAsync.
Use the complete path to the socket directory as the name of the
host, like this:

  $dbh = DBI->connect('dbi:PgAsync:dbname=foo;host=/var/tmp/socket',
    $username,
    $password,
...
=head3 B<connect_cached>
  ...
  $dbh = DBI->connect_cached("dbi:PgAsync:dbname=$dbname", $username, $password, \%options);
...
flags that DBD::PgAsync uses, see L<parse_trace_flag|/parse_trace_flag and parse_trace_flags>.
...
=head3 B<parse_trace_flag> and B<parse_trace_flags>
  ...
  my $value = DBD::PgAsync->parse_trace_flag('pglibpq');
  DBI->trace($value);
...
DBD::PgAsync currently supports the DBI-specific flag, C<SQL>,
...
the call to C<< DBD::PgAsync->parse_trace_flags >> provides a number than can
be fed to C<< DBI->trace >> before you create a database handle.

DBD::PgAsync supports the following trace flags:
...
as server-side prepared statements are used extensively by DBD::PgAsync.
...
to DBD::PgAsync (all those below which start with the letters 'pg').
...
Outputs the name of each internal DBD::PgAsync function, and other information such as
...
Outputs a simple message at the very end of each internal DBD::PgAsync function. This is also
...
DBD::PgAsync uses the C<func> method to support a variety of functions.
...
Backwards compatible alias for L</pg_lo_lseek>. Since DBD::PgAsync 3.16, that
...
Backwards compatible alias for L</pg_lo_tell>. Since DBD::PgAsync 3.16, that
method handles 64-bit offsets if supported by the Perl and PostgreSQL versions
in use.
...
Backwards compatible alias for L</pg_lo_truncate>. Since DBD::PgAsync 3.16, that
method handles 64-bit offsets if supported by the Perl and PostgreSQL versions
in use.
...
This is only available when DBD::PgAsync is compiled against a Postgres
...
Returns a hash of all private attributes used by DBD::PgAsync, for either
...
Supported by DBD::PgAsync as proposed by DBI. This method is similar to the
...
WARNING: DBD::PgAsync now (as of version 1.40) uses true prepared statements by sending them
...
prepared statements, which enables DBD::PgAsync to only send the query once, and
simply send the arguments for every subsequent call to L</execute>.
DBD::PgAsync can use these server-side prepared statements, or it can
...
PostgreSQL backend only have to prepare the query only once, but DBD::PgAsync no
...
at a low level should not use prepared statements via DBD::PgAsync, or must take
...
include C<SELECT>, C<INSERT>, C<UPDATE>, and C<DELETE>. DBD::PgAsync uses a simple
...
You can force DBD::PgAsync to send your query directly to the server by adding
...
There are three types of placeholders that can be used in DBD::PgAsync. The first is
...
if you do not need compatibility with other database systems. DBD::PgAsync, like
...
syntax is supported by DBD::PgAsync, its use is discouraged in favor of
dollar-sign numbers.
...
there are methods to instruct DBD::PgAsync to not treat some symbols as placeholders. First, you
may simply add a backslash before the start of a placeholder, and DBD::PgAsync will strip the
backslash and not treat the character as a placeholder.

You can also tell DBD::PgAsync to ignore any non-dollar sign placeholders by setting the
...
placeholders, you can tell DBD::PgAsync to ignore just colon placeholders by setting
...
name with optional schema, and DBD::PgAsync will attempt to find the sequence itself.
...
DBD::PgAsync will attempt to return the correct value. To do this, there must be at
...
looking up of things in the system table, so DBD::PgAsync will cache the sequence
...
exported by DBD::PgAsync (such as PG_BYTEA). In addition to string, bytea, char, bool,
...
This is a DBD::PgAsync-specific extension to the L</ping> method. This will check the
...
Four additional fields specific to DBD::PgAsync are returned:
...
not used by DBD::PgAsync. For information on the format of the standard rows returned, please
...
Supported by DBD::PgAsync as proposed by DBI. According to the classification of
...
DBD::PgAsync specific attribute. If true, boolean values will be returned
...
DBD::PgAsync specific attribute. Indicates if DBD::PgAsync should attempt to use server-side
...
DBD::PgAsync specific attribute. Indicates when DBD::PgAsync will internally switch from using
...
Setting pg_switch_prepared to 0 will force DBD::PgAsync to always use PQexecParams.

=head3 B<pg_placeholder_dollaronly> (boolean)

DBD::PgAsync specific attribute. Defaults to false. When true, question marks inside of statements
...
DBD::PgAsync specific attribute. Defaults to false. When true, colons inside of statements
...
DBD::PgAsync specific attribute. The behavior of DBD::PgAsync with regards to this flag has
changed as of version 3.0.0. The default value for this attribute, -1, tells
DBD::PgAsync to UTF8-decode all strings coming back
...
If this attribute is set to 0, then DBD::PgAsync will B<never> UTF8-decode
returned data, regardless of the current client_encoding.

If this attribute is set to 1, then DBD::PgAsync will B<always> UTF8-decode
returned data, regardless of the current client_encoding
...
pg_enable_utf8 to -1 to force DBD::PgAsync to read in the new client_encoding and
act accordingly.

=head3 B<pg_int8_as_string> (integer)

DBD::PgAsync specific attribute. Since version 3.0.0 the processing of SQL_INT8 has
...
DBD::PgAsync specific attribute. By default this is false, and causes prepared statements
...
DBD::PgAsync specific attribute. Sets the amount of information returned by the server's
...
DBD::PgAsync specific attribute. Indicates which version of PostgreSQL that
DBD::PgAsync was compiled against. In other words, which libraries were used.
...
"dbi:PgAsync:" part. Before version 2.0.0, this only returned the bare database name
...
DBD::PgAsync specific attribute. Returns the name of the current database.

=head3 B<pg_user> (string, read-only)

DBD::PgAsync specific attribute. Returns the name of the user that
connected to the server.

=head3 B<pg_host> (string, read-only)

DBD::PgAsync specific attribute. Returns the host of the current
server connection. Locally connected hosts will return an empty
string.

=head3 B<pg_port> (integer, read-only)

DBD::PgAsync specific attribute. Returns the port of the connection to
the server.

=head3 B<pg_socket> (integer, read-only)

DBD::PgAsync specific attribute. Returns the file description number of
the connection socket to the server.

=head3 B<pg_pass> (string, read-only)

DBD::PgAsync specific attribute. Returns the password used to connect
to the server.

=head3 B<pg_options> (string, read-only)

DBD::PgAsync specific attribute. Returns the command-line options passed
to the server. May be an empty string.

=head3 B<pg_default_port> (integer, read-only)

DBD::PgAsync specific attribute. Returns the default port used if none is
specifically given.

=head3 B<pg_pid> (integer, read-only)

DBD::PgAsync specific attribute. Returns the process id (PID) of the
backend server process handling the connection.

=head3 B<pg_prepare_now> (boolean)

DBD::PgAsync specific attribute. Default is off. If true, then the L</prepare> method will
immediately prepare commands, rather than waiting until the first execute.

=head3 B<pg_expand_array> (boolean)

DBD::PgAsync specific attribute. Defaults to true. If false, arrays returned from the server will
...
DBD::PgAsync specific attribute. Returns the version of the PostgreSQL server.
If DBD::PgAsync is unable to figure out the version, it will return a "0". Otherwise,
a "3" is returned.
...
use one of the values provided by DBI or by DBD::PgAsync. To use a SQL value,
...
The current list of Postgres data types exported is:
 ...
 PG_REGCONFIG PG_REGCONFIGARRAY PG_REGDATABASE PG_REGDATABASEARRAY PG_REGDICTIONARY PG_REGDICTIONARYARRAY
 ...
 PG_TIMESTAMPTZARRAY PG_TIMETZ PG_TIMETZARRAY PG_TRIGGER PG_TSMULTIRANGE PG_TSMULTIRANGEARRAY
 PG_TSM_HANDLER PG_TSQUERY PG_TSQUERYARRAY PG_TSRANGE PG_TSRANGEARRAY PG_TSTZMULTIRANGE
 PG_TSTZMULTIRANGEARRAY PG_TSTZRANGE PG_TSTZRANGEARRAY PG_TSVECTOR PG_TSVECTORARRAY PG_TXID_SNAPSHOT
...
data type to something else, DBD::PgAsync will re-prepare the statement for you before
...
DBD::PgAsync also supports passing in arrays to execute: simply pass in an arrayref,
and DBD::PgAsync will flatten it into a string suitable for input on the backend.
...
Supported by DBD::PgAsync. This method is implemented by DBI but not
...
DBD::PgAsync specific method. It returns Oid of table and position in table for
...
DBD::PgAsync specific method. It returns array of original (or canonical) names
...
DBD::PgAsync specific attribute. Returns the number of the tuple (row) that was
most recently fetched. Returns zero before and after fetching is performed.

=head3 B<pg_numbound> (integer, read-only)

DBD::PgAsync specific attribute. Returns the number of placeholders
that are currently bound (via bind_param).

=head3 B<pg_bound> (hashref, read-only)

DBD::PgAsync specific attribute. Returns a hash of all named placeholders. The
...
DBD::PgAsync specific attribute. It returns a reference to an array of integer
...
DBD::PgAsync specific attribute. It returns a reference to an array of strings
for each column. The string shows the name of the data_type.

=head3 B<pg_segments> (arrayref, read-only)

DBD::PgAsync specific attribute. Returns an arrayref of the query split on the
placeholders.

=head3 B<pg_oid_status> (integer, read-only)

DBD::PgAsync specific attribute. It returns the OID of the last INSERT command.

=head3 B<pg_cmd_status> (integer, read-only)

DBD::PgAsync specific attribute. It returns the type of the last
command. Possible types are: "INSERT", "DELETE", "UPDATE", "SELECT".

=head3 B<pg_direct> (boolean)

DBD::PgAsync specific attribute. Default is false. If true, the query is passed
directly to the backend without parsing for placeholders.

=head3 B<pg_prepare_now> (boolean)

DBD::PgAsync specific attribute. Default is off. If true, the query will be immediately
prepared, rather than waiting for the L</execute> call.

=head3 B<pg_prepare_name> (string)

DBD::PgAsync specific attribute. Specifies the name of the prepared statement to use for this
...
DBD::PgAsync specific attribute. Defaults to false. When true, question marks inside of the query
...
Not used by DBD::PgAsync. See the note about L</Cursors> elsewhere in this document.

=head1 FURTHER INFORMATION

=head2 Encoding

DBD::PgAsync has extensive support for a client_encoding of UTF-8, and most
...
rest of the transaction. DBD::PgAsync encourages using the following methods to
control savepoints:
...
DBD::PgAsync allows arrays (as arrayrefs) to be passed in to both
...
DBD::PgAsync allows for quick (bulk) reading and storing of data by using
...
For convenience, DBD::PgAsync can export certain constants representing the limits of
Postgres data types. To use them, just add C<:pg_limits> when DBD::PgAsync is used:

  use DBD::PgAsync qw/:pg_limits/;
...
DBD::PgAsync supports all largeobject functions provided by libpq via the
C<< $dbh->pg_lo* >> methods. Please note that access to a large object, even read-only
large objects, must be put into a transaction.

If DBD::PgAsync is compiled against and connected to PostgreSQL 9.3 or newer, and
...
implementation. When DBD::PgAsync was created, cursors in PostgreSQL could only be
...
choice. DBD::PgAsync therefore translates the result for the C<BOOL> data type in a
...
DBD::PgAsync does not provide explicit support for PostgreSQL schemas.
