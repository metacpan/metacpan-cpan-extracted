...
BEGIN { use_ok("Net::SSH2::Cisco") }

ok($Net::SSH2::Cisco::VERSION,  "\$VERSION set");

SKIP: {
    skip("Won't login to router without a login and password.", 26)
    unless $LOGIN && $PASSWD;

    ok( $S = Net::SSH2::Cisco->new(
            Host   => $ROUTER,
            Input_log  => $input_log,
            Dump_log   => $dump_log,
        ), "new() object"
...
#printf "%i = %i\n", $i+length $out, length $log; exit;
    ...
    $S->cmd(-String => 'show ver', -Timeout => 2);
...
#        && $S->timed_out,       "waitfor() timeout"     );
    ...
    my $ss = Net::SSH2::Cisco->new( Prompt => "/broken_pre1.08/" );
    ok( $ss->prompt eq "/broken_pre1.08/", "new(args) 1.08 bugfix" );
}
...
SKIP: {
    skip("Won't close without a session", 1)
    unless $S;
    ok( $S->close, "close()");
}
...
Net::SSH2::Cisco needs to log into a router to perform 
it\'s full suite of tests. To log in, we need a test 
router, a login, a password, an optional enable password, 
and an optional SecurID/TACACS PASSCODE.
