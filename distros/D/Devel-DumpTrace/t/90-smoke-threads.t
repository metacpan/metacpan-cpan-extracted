...
for my $level (4, 5) {
  ...
  # on <=v5.12, output has 72 lines and 16 refs to file and line no
  # on >=v5.14, there are 68 lines and 15 refs to file and line no

  ok(@xh == 72 || @xh == 68,
     "smoke output has right number of lines level=$level") or $keep++;

  my $separate_line_for_line_and_file = qr{^>> \s+.*:?lib/thread-demo.pl:\d+:};
