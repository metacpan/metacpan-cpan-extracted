...
## THIS THE TEST SCRIPT THAT HANS PROVIDED WITH HMListbox

## HMListbox demonstration application. This is a simple directory browser
## Original Author: Hans J. Helgesen, December 1999.
## Modified by: Rob Seegel, to work in Win32 as well
## Modified by: Jim Turner for Tk::HMListbox, May, 2015.
...
if ($ENV{PERL_MM_USE_DEFAULT} || $ENV{AUTOMATED_TESTING}) {
	...
	print "skipping 4\n";
	print "skipping 5\n";
	print "..done: 1 tests completed, 5 tests skipped.\n";
	exit (0);
}
...
#    -width => 0,
    ...
    -columns=>[
         ...
         [qw/-text ~Link -textwidth 7/, %green, -comparecmd => sub {$_[0] <=> $_[1]}],
         [qw/-text ~UID/, %white],
         [qw/-text ~GID/, %green],
         [qw/-text Si~ze/,%red, -comparecmd => sub {$_[0] <=> $_[1]}],
         [qw/-text M~time/, %green, -comparecmd => \&compareDate],
         [qw/-text ~Name -textwidth 20/, %white]
 ]);

print $ml ? "ok 2\n" : "not ok 2 ($@$? - widget not created?!)\n";

my %extHash = (    #load the icon images (*** YOU WILL LIKELY HAVE TO CHANGE THESE! ***)
      'dir' => $mw->Photo(-file => './dir.xpm'),
      ''    => $mw->Photo(-file => './file.xpm')
);
...
# Put the HMListbox widget on the bottom of the main window.
...
#               $mode = 0777;
         ...
         $ml->insert('end',   #Add each file's info as a new row:
               [$icon, $mode, $st->nlink, $st->uid, $st->gid, $st->size, $mtime, $name]
         );
      }
      $ml->sort($ml->getSortOrder());   #Sort the newly-loaded data):
}

# This callback is called if the user double-clicks one of the rows in
# the HMListbox. If the selected file is a directory, open it.
#
sub openFileOrDir
{
