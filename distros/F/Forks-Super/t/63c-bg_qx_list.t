...
# Calling bg_qx with an array reference
...
my $y = $x;
ok($y == $z, "scalar bg_qx");
okl($t2 >= 2.35 && $t <= 6.5,          ### 10 ### was 5.1 obs 5.57,6.31,2.55
...
$t = Time::HiRes::time();
ok($x eq "$z \n" || $x eq "$z ",     ### 15 ###
   "scalar bg_qx failed but retrieved output (cmd w/meta)")
    or diag("\$x was $x, expected $z\n");
if (!defined $x) {
    diag "(output was: <undef>;target was \"$z \")";
} elsif ($x ne "$z \n" && $x ne "$z ") {
    diag "(output was: $x; target was \"$z \")";
}
$t = Time::HiRes::time() - $t;
okl($t <= 7.5,                          ### 16 ### was 3 obs 3.62,5.88,7.34
    "scalar bg_qx (cmd w/meta) respected timeout, took ${t}s expected ~4s");
