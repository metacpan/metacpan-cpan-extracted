...
file.  In that file, you should find a category with the C<$name>.
...
When there is no specific logger for this C<$domain> (logger with the exact
name of the C<$domain>) the default logger is being used, with the name of
this dispatcher.
...
=item Error: Log4perl level '$level' must be in 0-5
