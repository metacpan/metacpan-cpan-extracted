...
# The $SCHEMA variable in Statistics::Covid::Datum::Table contains all
# this module needs to initialise
use Statistics::Covid::Datum::Table;

print __PACKAGE__ . ": preparing table ".$Statistics::Covid::Datum::Table::SCHEMA->{'tablename'}."\n";

if( ! __PACKAGE__->table($Statistics::Covid::Datum::Table::SCHEMA->{'tablename'}) ){ die "call to table() has failed for table name '".$Statistics::Covid::Datum::Table::SCHEMA->{'tablename'}."'." }
my $sc = $Statistics::Covid::Datum::Table::SCHEMA->{'schema'};
die "there is no schema!" unless defined $sc;

for my $acolname (@{$Statistics::Covid::Datum::Table::SCHEMA->{'column-names'}}) {
...
if( ! __PACKAGE__->set_primary_key(@{$Statistics::Covid::Datum::Table::SCHEMA->{'column-names-for-primary-key'}}) ){ die "call to set_primary_key() has failed for: ".join(',',@{$Statistics::Covid::Datum::Table::SCHEMA->{'column-names-for-primary-key'}})."." }
...
Statistics::Covid::Schema::Result::Datum - Datum is a table in our database of collected Covid statistics and this module initialises this table using DBIx::Class
...
=head1 DESCRIPTION
This module is not to be used directly. It creates the table Datum
...
interact with these tables. Something along these lines:
    ...
    my $resultset = $shemahandle->('Datum')->search({name=>'Samarkand'});
    # ... and act on them
    $resultset->delete();
...
You can find documentation for this module with the perldoc command.
    ...
    perldoc Statistics::Covid::Schema::Result::Datum
