...
=head1 SYNOPSIS
  ...
  use Mail::Mbox::Mbox ();
  my $emails = Mail::Mbox::Mbox->new(folder => $ENV{MAIL});
  print $folder->name;

  print $folder->message(0);    # get the first message.
  $folder->message(3)->delete;  # delete the third message
  say "Folder size = ", $folder->nrMessages;

  # Iterate over the messages.
  foreach my $msg ($folder->messages) {...} # all messages
  foreach my $msg (@$folder) {...}          # all messages

  $folder->addMessage(Mail::Message->new(...));
...
file, directory, and remote folder types.  See L</Different kinds of folders>
...
Open a new folder. A list of labeled C<%options> for the mailbox can be
supplied. Some options pertain to L<Mail::Box|Mail::Box>, and others are added by
sub-classes.
...
means write-protected) by default!  The C<$mode> can be
C<'r'> (read-only, default), C<'a'> (append only), C<'rw'> (read-write),
and C<'d'> (delete only).

This C<$mode> describes the abstract folder access: how the underlaying files are
really accessed depends on the action which needs to take place.

Be warned: writing a MBOX type folder may create a new file to replace
the old folder.  The permissions and owner of the file may get changed
by this.
...
See L<Mail::Box::Parser::Perl::new(fix_header_errors)|Mail::Box::Parser/"METHODS">.
...
“C<< folderdir => '/tmp'>>” and “C<< folder => '=abc' >>”, the name of the
...
option is C<false> (the default) such messages are automatically deleted,
because it is considered useless to store the same message twice.

=item lock_file => $file

The name of the C<$file> which is used to lock.  This must be specified when
locking is to be used.

=item lock_timeout => $seconds

When the lock file is older than the specified number of C<$seconds>, it is
considered a mistake.  The original lock is released, and accepted for
this folder.

=item lock_type => $class|$name|\@names

The type of the locker object.  This may be a C<$class>
which extends L<Mail::Box::Locker|Mail::Box::Locker>, or one of the known locker types by C<$name>:
...
specified list of locker C<@names>.

=item lock_wait => $seconds

How many C<$seconds> to wait before failing on opening this folder.
...
For instance, the message type for L<Mail::Box::POP3|Mail::Box::POP3> is
L<Mail::Box::POP3::Message|Mail::Box::POP3::Message>.
...
Get or set the C<$directory> which is used to store mail-folders by default.
...
slow (depends on the implementation).  All C<%options> passed will also be
...
The C<$name> of the folder to which the messages are to be appended.  The folder
...
One reference to one C<$message> or an ARRAY of C<@messages>, which may
...
How to handle sub-folders.  When C<false> (C<0> or C<undef>), sub-folders
...
of C<true> will select the default.
...
The C<%options> are extra values which are passed to the
...
Checks if the folder, as stored in memory, is modified.  A C<true> value is
...
You may specify a C<$number>, to specify that that message number is to be
selected as current, or a C<$message>/C<$message_id> (as long as you are sure
...
C<$message_id>, returning the corresponding message object.  However, C<find>
...
Find the first message which has this C<$label> with the correct setting. The
C<BOOLEAN> indicates whether any C<true> value or any C<false> value is to be
found in the ARRAY of C<@msgs>.  By default, a C<true> value is searched for.
When a message does not have the requested label, it is taken as C<false>.
...
C<$message_id>. If a reference to a message object is passed as the optional
second argument, the message is first stored in the folder, replacing any
existing message whose message ID is C<$message_id>. (The message ID of C<$message>
need not match C<$message_id>.)
...
The C<$message_id> may still be in angles, which will be stripped.  In that
...
You may also specify a C<$range>: two numbers specifying begin and end
...
For more complex searches, you can specify a C<$filter>, which is
...
The C<%options> are passed to (and explained in) L<messages()|Mail::Box/"The messages">.

=item $obj-E<gt>B<scanForMessages>($message, $message_ids, $timespan, $window)

You start with a C<$message>, and are looking for a set of messages
which are related to it.  For instance, messages which appear in
the 'In-Reply-To' and 'Reference' header fields of that message.
These messages are known by their C<$message_ids> and you want to find
them in the folder.

When all message-ids are known, then looking-up messages is simple:
they are found in a plain hash using L<messageId()|Mail::Box/"The messages">.  But L<Mail::Box|Mail::Box>
...
scanning the folder from the specified C<$message>, back to the front
of the folder.

The C<$timespan> can be used to terminate the search based on the time
enclosed in the message.  When the constant string C<EVER> is used as
C<$timespan>, then the search is not limited by that.  When an integer
...
The C<$window> is used to limit the search in number of messages to be
...
as instance method, or specify a C<$parentname>.
...
folder.  C<%options> overrule the options which where used for the folder
...
Coerce the C<$message> to be of the correct type to be placed in the
folder.  You can specify L<Mail::Internet|Mail::Internet> and MIME::Entity objects
here: they will be translated into L<Mail::Message|Mail::Message> messages first.
...
When the foldername is preceded by a “C<< = >>”, the C<$directory>
...
The C<$foldername> specifies the name of the folder, as is specified by the
application.  You need to specified the C<folder> option when you skip
this first argument.

C<%options> is a list of extra information for the request.  Read
...
to be found in this default C<$directory>.
...
file, optionally after setting it to C<STRING>, or one of the constants.
The first line of the folder sets the default.

UNIX uses a C<LF> character, Mac a C<CR>, and Windows both a C<CR> and a C<LF>.  Each
...
Read messages from the folder into memory.  The C<%options> are folder
...
The C<%options> are C<trusted>, C<head_type>, C<field_type>,
...
Parameter C<$time> is a string, which starts with a float, and then one of the
