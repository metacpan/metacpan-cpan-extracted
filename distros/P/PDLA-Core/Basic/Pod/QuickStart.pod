...
A brief summary of the main PDLA features and how to use them.
...
This has been fully exploited by the PerlDL developers. The 'PDLA' module is a
...
manipulated B<en masse>.  For example  with the PDLA module we can write the
...
PDLA variables (or 'piddles' as they have come to be known)
...
of PDLA new customised datatypes can be derived from them.

As well as the PDLA modules, that can be used by normal Perl programs, PerlDL
comes with a command line Perl shell, called 'perldla', which supports command
line editing. In combination with the various PDLA graphics modules this allows
data to be easily played with and visualised.

=head2 Help

PDLA contains extensive documentation, available both within the
I<perldla> or I<pdla2> shells and from the command line, using the C<pdladoc> program.
...
To find their location, try the following:
 ...
 pdla> foreach ( map{"$_/PDLA/HtmlDocs"}@INC ) { p "$_\n" if -d $_ }  

=head2 Perl Datatypes and how PDLA extends them
...
C<@x> variables as "lists" of generic "things" and PDLA variables like
...
PerlDL is loaded into your Perl script using this command:
 ...
 use PDLA;  # in Perl scripts: use the standard perlDL modules

There are also a lot of extension modules, e.g. 
L<PDLA::Graphics::TriD|PDLA::Graphics::TriD>. 
...
There is also an interactive shell, C<perldla> or C<pdla2>, see I<perldla>
or L<pdla2|PDLA::Perldl2> for details.

=head2 To create a new PDLA variable

Here are some ways of creating a PDLA variable:
 ...
 @x = ( pdl(42), zeroes(3,2,4), rfits($file) ); # Is a LIST of PDLA variables!

The L<pdl()|PDLA::Core/pdl> function is used to initialise a PDLA variable from a scalar,
list, list reference, another PDLA variable, or a properly formatted string.

In addition all PDLA functions automatically convert normal Perl scalars
to PDLA variables on-the-fly.
...
since C<$x E<gt> 3> is a piddle with 4 elements. Rather use 
L<all|PDLA::Ufunc/all> or L<any|PDLA::Ufunc/any>
...
Note: L<transpose()|PDLA::Basic/transpose> 
...
are using L<PDLA::AutoLoader|PDLA::AutoLoader> (see below).

Of course, this function is already available as the 
L<inner|PDLA::Primitive/inner>
function, see L<PDLA::Primitive>.
...
Also double(), short(), ushort(), indx().  
  ...
  is the correct size for a PDLA index value (dimension size,
...
PDLA has very powerful multidimensional slicing and sectioning
operators; see L<the PDLA::Slices(3) man page|PDLA::Slices> for details;
we'll describe the most important one here.

PDLA shows its Perl/C heritage in that arrays are zero-offset.  Thus a
100x100 image has indices C<0..99,0..99>.  (The convention is that the
I<center> of pixel (0,0) is at coordinate (0.0,0.0). All PDLA graphics
...
L<PDLA::NiceSlice|PDLA::NiceSlice>, that allows you to specify subranges
via a null-method modifier to a PDLA:
...
You can put PDLA slices on either side of the element-wise assignment 
operator C<.=>, like so:
...
The C<PDLA::IO> modules implement several useful IO format functions.
It would be too much to give examples of each, but you can find a nice
overview at L<PDLA::IO|PDLA::IO>. Here is a sample of some of the
supported IO formats in PDLA.
...
this allows PDLA to keep up with a rapidly developing field - the
latest PDLA modules provide interfaces to OpenGL and VRML graphics!

=over 4

=item PDLA::Graphics::PGPLOT
...
L<PDLA::Graphics::PGPLOT|PDLA::Graphics::PGPLOT>, which
calls routines in the separately available
PGPLOT top-level module.

=item PDLA::Graphics::PLplot
...
PLplot's C interface. Read more at L<PDLA::Graphics::PLplot|PDLA::Graphics::PLplot>.

=item PDLA::Graphics::IIS
...
The L<PDLA::Graphics::IIS|PDLA::Graphics::IIS> 
...
=item PDLA::Graphics::TriD

See L<PDLA::Graphics::TriD|PDLA::Graphics::TriD>, this is a collection
of 3D routines for OpenGL and (soon) VRML and other 3D formats which
allow 3D point, line, and surface plots from PDLA.

=back

=head2 Autoloading

See L<PDLA::AutoLoader>. This allows one to autoload functions
on demand, in a way perhaps familiar to users of MatLab.

One can also write PDLA extensions as normal Perl modules.

=head2 PDLA shells

The Perl script C<pdla2> (or C<perldla>) provides a simple command line interface
to PDLA.  If the latest Readlines/ReadKey modules have been installed C<pdla2>
...
e.g.:
 ...
 Reading PDLA/default.perldlrc...
 Found docs database /home/pdl/dev/lib/perl5/site_perl/PDLA/pdladoc.db
 Type 'help' for online help
 Type 'demo' for online demos
 Loaded PDLA v2.4.9_003 (supports bad values)
 pdla> $x = rfits 'm51.fits'
 Reading IMAGE data...
 BITPIX =  32  size = 147456 pixels 
 Reading  589824  bytes
 BSCALE =  &&  BZERO = 

 pdla> use PDLA::Graphics::PGPLOT;
 pdla> imag $x
 Displaying 384 x 384 image from 40 to 761, using 84 colors (16-99)...

You can also run it from the Perl debugger (C<perl -MPDLA -d -e 1>)
if you want.
...
and local PDLA code for startup.

=item Help

Type 'help'! One can search the PDLA documentation, and look up documentation
on any function.
...
PDLA operations are available as functions and methods.
Thus one can derive new types of object, to represent
custom data classes.

By using overloading one can make mathematical operators
do whatever you please, and PDLA has some built-in tricks
which allow existing PDLA functions to work unchanged, even
...
the operator L<inplace()|PDLA::Core/inplace> 
...
number rather than a PDLA variable. Simply call the function
L<topdl()|PDLA::Core/topdl> first to make it safe. e.g.:
