...
sub test_new : Test(1) {
    ...
    my $cmd = Prty::FFmpeg->new;
    $self->is(ref($cmd),'Prty::FFmpeg');
}
...
sub test_imagesToVideo : Test(1) {
    ...
    my $cmd = $exa->execute(q|
            Prty::FFmpeg->imagesToVideo('img/*.jpg','img.mp4');
        |,
...
sub test_videoInfo : Test(1) {
    ...
    my $cmd = $exa->execute(q|
            Prty::FFmpeg->videoInfo('video.mp4');
        |,
        -asStringCallback=>sub {
            my ($cmd) = @_;
            return $cmd->command;
