...
sub ircd_daemon_kline {
    ...
    is($args[3], '*bobb*', 'The usermask is right' );
...
sub ircd_daemon_unkline {
    ...
    is($args[2], '*bobb*', 'The usermask is right' );
    is($args[3], '127.0.0.1', 'The hostmask is right' );
...
sub harpo_connected {
  ...
  #$kernel->post( $sender, 'send_to_server', { command => 'CAPAB', params => [ 'KNOCK UNDLN DLN TBURST GLN ENCAP UNKLN KLN CHW IE EX HOPS SVS CLUSTER EOB QS' ], colonify => 1 } );
  $kernel->post( $sender, 'send_to_server', { command => 'CAPAB', params => [ 'KNOCK UNDLN DLN TBURST GLN ENCAP CHW IE EX HOPS SVS EOB QS' ], colonify => 1 } );
...
sub groucho_input {
  ...
  if ( $cmd eq 'KLINE' ) {
    ...
    is( $params->[2], '*bobb*', 'UserMask is correct' );
    is( $params->[3], '127.0.0.1', 'HostMask is correct' );
    is( $params->[4], 'Banhammer', 'A sound reason indeed' );
    return;
  }
  if ( $cmd eq 'UNKLINE' ) {
    pass($cmd);
    is( $prefix, '1FUAAAAAA', 'Correct prefix: 1FUAAAAAAA' );
    is( $params->[0], '*', 'Target is all servers' );
    is( $params->[1], '*NastyBot*', 'Mask is correct' );
    return;
  }
  if ( $cmd eq 'QUIT' ) {
    pass($cmd);
...
sub client_disconnected {
  ...
  $heap->{ircd}->yield('daemon_cmd_unkline','OperServ','*bobb*@127.0.0.1','ON','*');
  return;
}
...
sub ircd_daemon_eob {
  ...
  if ( $heap->{eob} >= 3 ) {
    $heap->{ircd}->yield( 'daemon_cmd_kline', 'OperServ', '10', '*bobb*@127.0.0.1', 'ON', '*', 'Banhammer' );
  }
  return;
}
