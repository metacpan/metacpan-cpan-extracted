...
plan skip_all => 'this test requires a built dist'
    unless -d 'blib/lib/auto/share/dist/Dist-Zilla-PluginBundle-Author-TABULO/profiles';

plan skip_all => 'minting requires perl 5.014' unless "$]" >= 5.013002;

my $tzil = Minter->_new_from_profile(
    [ 'Author::TABULO' => 'github' ],
    { name => 'My-New-Dist', },
...
unlike($dist_ini, qr/\n\n\n/, 'no double blank lines in dist.ini');

# TABULO]: modified the below to look for double (instead of single) blank lines,
# since it was quite challenging to conform because of a HEREDOC that happens
# to have its end-marker there at this time, combined with my text
# text editor's settings which wanted to make sure a file always ends with a new line.
unlike($dist_ini, qr/\n\n\n\z/, 'file does not end with double blank lines');
