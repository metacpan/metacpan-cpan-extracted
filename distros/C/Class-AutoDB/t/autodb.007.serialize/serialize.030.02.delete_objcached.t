########################################
# delete objects when object already in object cache
...
# fetch persistent objects & make sure it worked
...
report_fail($ok,'bad news: fetch of persistent objects failed',__FILE__,__LINE__);

# for sanity, make sure objects in cache
ok_objcache($actual_p0,$p0_oid,'object','Persistent',
	    'p0 cache entry after fetch',__FILE__,__LINE__);
ok_objcache($actual_p1,$p1_oid,'object','Persistent',
	    'p1 cache entry after fetch',__FILE__,__LINE__);
...
ok_objcache($actual_p0,$p0_oid,'OidDeleted','Persistent',
	    'p0 changed to OidDeleted by del',__FILE__,__LINE__);
ok_objcache($actual_p1,$p1_oid,'OidDeleted','Persistent',
	    'p1 changed to OidDeleted by del',__FILE__,__LINE__);
