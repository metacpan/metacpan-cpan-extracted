...
#
#=======================================================================
package PDF::API3::Compat::API2::Basic::TTF::Anchor;
...
sub read
{
    ...
    } elsif ($fmt == 3)
    {
        ...
        {
            $fh->seek($loc + $xoff, 0);
            $self->{'xdev'} = PDF::API3::Compat::API2::Basic::TTF::Delta->new->read($fh);
        }
        if ($yoff)
        {
            $fh->seek($loc + $yoff, 0);
            $self->{'ydev'} = PDF::API3::Compat::API2::Basic::TTF::Delta->new->read($fh);
        }
    }
    $self;
}
