package DBIx::Changeset::App::Command::applied;
...
DBIx::Changeset::App::Command::applied - show the changeset history record db
...
sub options {
	my ($self, $app) = @_;
	return (
		[ 'history_db_dsn=s' => 'DBI DSN for the history db', { default => $app->{'config'}->{'history_db_dsn'} || $app->{'config'}->{'applied'}->{'history_db_dsn'} || undef, required => 1 } ],
		[ 'history_db_user=s' => 'db user for history db', { default => $app->{'config'}->{'history_db_user'} || $app->{'config'}->{'applied'}->{'history_db_user'} || undef } ],
