...
sub _build_label_stock_type {
    my $self = shift;
    return 'STOCK_4X6'
      if $self->printer_type eq 'thermal'
...
has '+package_type' => (isa =>
      Enum [qw( custom envelope tube box pack FEDEX_10KG_BOX FEDEX_25KG_BOX )]
);
...
sub _build_services {
    ...
    push @to_streetlines, $self->to_address()->address2
      if $self->to_address()->address2;

    my @from_streetlines;
    push @from_streetlines, $self->from_address()->address1;
    push @from_streetlines, $self->from_address()->address2
      if $self->from_address()->address2;
    ...
    $options->{SignatureOptionDetail}->{OptionType} =
        ...
        foreach (@{$self->packages}) {
            push @pieces,
              { SequenceNumber => $sequence,
                ...
                Weight => {
                    Value => $_->weight,
                    Units => $units_type_map{$self->weight_unit}
                      || $self->weight_unit,
                },
                Dimensions => {
                    Length => $_->length,
                    Width  => $_->width,
                    Height => $_->height,
                    Units  => $units_type_map{$self->dim_unit}
                      || $self->dim_unit,
                },
    ...
    try {
        ...
        foreach my $service (@{$response->get_RateReplyDetails()}) {
            $services{$service->get_ServiceType()->get_value} =
        ...
        $services{ground} =
          ...
          || $services{'GROUND_HOME_DELIVERY'}
          || $services{'INTERNATIONAL_GROUND'}
          || Shipment::Service->new();
        $services{express} =
...
sub ship {
    ...
    $package_options->{SignatureOptionDetail}->{OptionType} =
      $signature_type_map{$self->signature_type} || $self->signature_type;
    ...
    foreach (@{$self->packages}) {
        ...
        try {
            ...
            $response                    = $interface->processShipment(
                {   WebAuthenticationDetail => {
                    ...
                    RequestedShipment => {
                        ...
                        DropoffType   => $pickup_type_map{$self->pickup_type}
                          || $self->pickup_type,
                        PackagingType => $package_type_map{$self->package_type}
                          || $self->package_type,
                        TotalWeight => {
                            Value => $total_weight,
                            Units => $units_type_map{$self->weight_unit}
                        ...
                        Shipper => {
                            ...
                            Address => {
                                ...
                                CountryCode =>
                                  $self->from_address()->country_code,
                            },
                        },
                        Recipient => {
                            Contact => {
                        ...
                        ShippingChargesPayment => {
                            ...
                            Payor => {
                                ...
                                CountryCode   => ($self->bill_address)
                                ? $self->bill_address->country_code
                                : $self->from_address->country_code,
                            },
                        },
                        SpecialServicesRequested  => $shipment_options,
                        ...
                        LabelSpecification => {
                            ...
                            ImageType => $printer_type_map{$self->printer_type}
                              || $self->printer_type,
            ...
            foreach my $notification (@{$response->get_Notifications()}) {
                warn $notification->get_Message->get_value if $self->debug;
                $self->add_notice(
                    $notification->get_Message->get_value . "\n");
            }

            my $package_details = $response->get_CompletedShipmentDetail
              ->get_CompletedPackageDetails;

            if ($self->count_packages > 1) {
                my $master_tracking =
                  $response->get_CompletedShipmentDetail->get_MasterTrackingId;
                $self->tracking_id(
                    $master_tracking->get_TrackingNumber->get_value);
                $master_tracking_id = {
                    TrackingIdType =>
                      $master_tracking->get_TrackingIdType->get_value,
                    TrackingNumber =>
                      $master_tracking->get_TrackingNumber->get_value,
                };
            }
                ...
                $self->tracking_id(
                    $package_details->get_TrackingIds->get_TrackingNumber
                      ->get_value);
            }
            $_->tracking_id(
                $package_details->get_TrackingIds->get_TrackingNumber
            ...
            if ($package_details->get_PackageRating) {
                $_->cost(
                    Data::Currency->new(
                        $package_details->get_PackageRating
                          ->get_PackageRateDetails->[
                          0]->get_NetCharge->get_Amount->get_value,
                        $package_details->get_PackageRating
                          ->get_PackageRateDetails->[
                          0]->get_NetCharge->get_Currency->get_value,
                    )
                );
            }
            elsif ($response->get_CompletedShipmentDetail->get_ShipmentRating)
            {
                $_->cost(
                    Data::Currency->new(
                        $response->get_CompletedShipmentDetail
                          ->get_ShipmentRating->get_ShipmentRateDetails->[
                          0]->get_TotalNetCharge->get_Amount->get_value,
                        $response->get_CompletedShipmentDetail
                          ->get_ShipmentRating->get_ShipmentRateDetails->[
                          0]->get_TotalNetCharge->get_Currency->get_value,
                    )
                );
            }
            $_->label(
                Shipment::Label->new(
                    {   tracking_id => $package_details->get_TrackingIds
                          ->get_TrackingNumber->get_value,
                        content_type =>
                          $label_content_type_map{$self->printer_type},
                        data => decode_base64(
                            $package_details->get_Label->get_Parts->get_Image
                              ->get_value
                        ),
                        file_name => $package_details->get_TrackingIds
                          ->get_TrackingNumber->get_value . '.'
            ...
            warn $_ if $self->debug;
            try {
                $self->error($response->get_Notifications()->[0]
                      ->get_Message->get_value);
                warn $response->get_Notifications()->[0]
                  ->get_Message->get_value
                  if $self->debug;
            }
            catch {
    ...
    if (!$self->error) {
        ...
        try {
            my $total_charge =
              $response->get_CompletedShipmentDetail->get_ShipmentRating
              ->get_ShipmentRateDetails->[0]->get_TotalNetCharge;
...
#warn $_;
        ...
        $self->service(
            Shipment::Service->new(
                ...
                name => $self->services->{$service_id}->name,
                cost => Data::Currency->new(
                    $total_charge_amount, $total_charge_currency,
                ),
            )
        );
    }
...
sub track {
    ...
    try {
        ...
        if ($response->get_CompletedTrackDetails()
            ->get_TrackDetails()
            ->get_Notification()
            ->get_Severity()
            ->get_value() eq 'ERROR')
        {
            $self->error($response->get_CompletedTrackDetails()
                  ->get_TrackDetails()
                  ->get_Notification()
                  ->get_Message()
                  ->get_value());
