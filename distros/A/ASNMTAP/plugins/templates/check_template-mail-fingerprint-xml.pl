...
# ----------------------------------------------------------------------------------------------------------
# 2011/mm/dd, v3.002.003, check_template-mail-fingerprint-xml.pl
# ----------------------------------------------------------------------------------------------------------
...
my $objectMAIL = ASNMTAP::Asnmtap::Plugins::Mail->new (
  ...
  _mailType         => 1,
  _text             => { SUBJECT => 'uKey=MAIL_'. $environment .'_0002' },
  _mail             => {
...
($returnCode, $numberOfMatches) = $objectMAIL->receiving_fingerprint_mails( custom => \&actionOnMailBody, receivedState => 0, outOfDate => $interval, perfdataLabel => 'email(s) received' );
