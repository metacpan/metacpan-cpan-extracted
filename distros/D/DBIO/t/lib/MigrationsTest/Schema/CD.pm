...
__PACKAGE__->belongs_to( artist => 'MigrationsTest::Schema::Artist', undef, {
    is_deferrable => 1,
    proxy => { artist_name => 'name' },
});
__PACKAGE__->belongs_to( very_long_artist_relationship => 'MigrationsTest::Schema::Artist', 'artist', {
    is_deferrable => 1,
});

# in case this is a single-cd it promotes a track from another cd
__PACKAGE__->belongs_to( single_track => 'MigrationsTest::Schema::Track',
  { 'foreign.trackid' => 'self.single_track' },
  { join_type => 'left'},
);

__PACKAGE__->belongs_to( single_track_opaque => 'MigrationsTest::Schema::Track',
  sub {
...
__PACKAGE__->belongs_to( existing_single_track => 'MigrationsTest::Schema::Track',
  { 'foreign.trackid' => 'self.single_track' },
);

__PACKAGE__->has_many( tracks => 'MigrationsTest::Schema::Track' );
__PACKAGE__->has_many(
    tags => 'MigrationsTest::Schema::Tag', undef,
    { order_by => 'tag' },
);
__PACKAGE__->has_many(
    cd_to_producer => 'MigrationsTest::Schema::CD_to_Producer' => 'cd'
);

__PACKAGE__->has_many( twokeys => 'MigrationsTest::Schema::TwoKeys', 'cd' );


# the undef condition in this rel is *deliberate*
# tests oddball legacy syntax
__PACKAGE__->might_have(
    liner_notes => 'MigrationsTest::Schema::LinerNotes', undef,
    { proxy => [ qw/notes/ ] },
);
__PACKAGE__->might_have(artwork => 'MigrationsTest::Schema::Artwork', 'cd_id');
__PACKAGE__->has_one(mandatory_artwork => 'MigrationsTest::Schema::Artwork', 'cd_id');
...
__PACKAGE__->belongs_to('genre', 'MigrationsTest::Schema::Genre',
    'genreid',
    {
        join_type => 'left',
        on_delete => 'SET NULL',
...
__PACKAGE__->belongs_to('genre_inefficient', 'MigrationsTest::Schema::Genre',
