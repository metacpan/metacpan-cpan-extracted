...
### Webhook Signature Validation (see webhooks.md)
- [ ] `validateWebhookSignature(signingKey, signature, url, rawBody)` public function
- [ ] Scheme A: hex HMAC-SHA1 of `url + rawBody` (RELAY/SWML/SWAIG)
- [ ] Scheme B: base64 HMAC-SHA1 of `url + sortedFormParams` (cXML)
- [ ] Scheme B: `bodySHA256` query-param path for JSON bodies on compat endpoints
- [ ] URL port normalization (try with and without `:443`/`:80`)
- [ ] Repeated form keys concatenated in submission order
- [ ] Timing-safe digest comparison
- [ ] `X-SignalWire-Signature` header recognized; `X-Twilio-Signature` alias on the compat path
- [ ] `SIGNALWIRE_SIGNING_KEY` env var fallback
- [ ] `validateRequest` compat alias (legacy `@signalwire/compatibility-api` signature)
- [ ] Framework adapter: raw-body-capturing middleware, 403 on failure, body forwarded on success
- [ ] AgentBase integration (if applicable): `signingKey` option auto-mounts middleware on `/`, `/swaig`, `/post_prompt`
- [ ] Tests: Scheme A canonical vector (positive + tampered negative), Scheme B canonical form vector, `bodySHA256` canonical vector, port normalization, repeated keys, missing header → false, missing key → error, malformed signature → false, middleware 403 vs 200, AgentBase accepts signed / rejects unsigned
