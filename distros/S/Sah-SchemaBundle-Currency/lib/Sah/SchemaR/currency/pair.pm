...
our $rschema = do{my$var={base=>"str",clsets_after_base=>[{description=>"\nCurrency pair is string in the form of *currency1*/*currency2*, where\n*currency1* is called the base currency while *currency2* is the quote (or\nprice) currency. Both must be known currency codes (e.g. USD, or IDR).\n\nCurrency code is checked against <pm:Locale::Codes::Currency_Codes>.\n\nWill be normalized to uppercase.\n\n",examples=>[{valid=>0,value=>""},{valid=>0,value=>"idr"},{valid=>1,validated_value=>"USD/IDR",value=>"usd/idr"},{valid=>0,value=>"usd idr"},{valid=>0,value=>"usd/foo"}],match=>qr(\A\S+/\S+\z),summary=>"Fiat currency pair, e.g. USD/IDR","x.perl.coerce_rules"=>["From_str::to_currency_pair"]}],clsets_after_type=>['$var->{clsets_after_base}[0]'],"clsets_after_type.alt.merge.merged"=>['$var->{clsets_after_base}[0]'],resolve_path=>["str"],type=>"str",v=>2};$var->{clsets_after_type}[0]=$var->{clsets_after_base}[0];$var->{"clsets_after_type.alt.merge.merged"}[0]=$var->{clsets_after_base}[0];$var};
...
This document describes version 0.009 of Sah::SchemaR::currency::pair (from Perl distribution Sah-SchemaBundle-Currency), released on 2024-06-26.
