...
Bio::Genex::TreatmentLevel - Methods for processing data from the GeneX DB
...
=head1 SYNOPSIS
  ...
  $TreatmentLevel->fetch();

  # creating an instance, without pre-fetching all columns
  my $TreatmentLevel = new Bio::Genex::TreatmentLevel(id=>47);

  # creating an instance with pre-fetched data
  my $TreatmentLevel = new Bio::Genex::TreatmentLevel(id=>47, 'fetch_all'=>1);

  # retrieving multiple instances via primary keys
  my @objects = Bio::Genex::TreatmentLevel->get_objects(23,57,98)


  # retrieving all instances from a table
  my @objects = Bio::Genex::TreatmentLevel->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $TreatmentLevel->id();

  # or specifically
  my $tl_pk_val = $TreatmentLevel->tl_pk();

  # retreving other DB column attributes
  my $es_fk_val = $TreatmentLevel->es_fk();
  $TreatmentLevel->es_fk($value);

  my $ratio_am_fk_val = $TreatmentLevel->ratio_am_fk();
  $TreatmentLevel->ratio_am_fk($value);

  my $name_val = $TreatmentLevel->name();
  $TreatmentLevel->name($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::TreatmentLevel is
TreatmentLevel).
...
Bio::Genex::TreatmentLevel can access: I<raw> foreign key attributes,
...
Class Bio::Genex::TreatmentLevel defines the following utility variables for assisting
programmers to access the TreatmentLevel table.

=over 4

=item $Bio::Genex::TreatmentLevel::LIMIT
...
=item $Bio::Genex::TreatmentLevel::USE_CACHE
...
instance of the class via the Bio::Genex::TreatmentLevel->methodname() syntax.
...
C<update_db()>.
  ...
  TreatmentLevel->es_fk('some_value');
  TreatmentLevel->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('tl_pk' for module Bio::Genex::TreatmentLevel).
...
a warning from C<Bio::Genex::TreatmentLevel::initialize()>. Objects will be
...
instance of class Bio::Genex::TreatmentLevel.
...
=item @id_list = treatment_ams_fk()

=item @obj_list = treatment_ams_obj()

This is an attribute of type ONE_TO_MANY_LINK and refers to class
L<Bio::Genex::Treatment_AMs>. The raw accessor method, C<treatment_ams_fk()> returns a list of
foreign key ids. The OO accessor method, C<treatment_ams_obj()> returns a
list of objects of class Bio::Genex::Treatment_AMs.


=item @id_list = tl_factorvalues_fk()

=item @obj_list = tl_factorvalues_obj()

This is an attribute of type ONE_TO_MANY_LINK and refers to class
L<Bio::Genex::TL_FactorValues>. The raw accessor method, C<tl_factorvalues_fk()> returns a list of
foreign key ids. The OO accessor method, C<tl_factorvalues_obj()> returns a
...
C<tl_pk()>method). The C<id()> method can be useful in writing
...
=item $value = ratio_am_fk();

=item ratio_am_fk($value);
