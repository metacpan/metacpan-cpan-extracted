package Games::GUI::Battleship::AI::Naive;

use v5.38;
use experimental 'signatures';
use feature 'try';
no warnings 'experimental::try';
use parent 'Games::GUI::Battleship::AI';

use Logic::Relational ();
use Logic::Relational::Syntax;
use Logic::Relational::Goal::Call;

our $CURRENT_SHOTS = {};

logic NaiveAITemplate {
    fact shot_hit( 0, 0 );
    fact shot_miss( 0, 0 );
    fact sunk_cell( 0, 0 );

    rule cell_unshot( $x, $y ) {
        guard(
            [ $x, $y ],
            sub ( $vx, $vy ) {
                return 0 if $vx < 1 || $vx > 10 || $vy < 1 || $vy > 10;
                return $Games::GUI::Battleship::AI::Naive::CURRENT_SHOTS->{
                    "$vx,$vy"} ? 0 : 1;
            }
        );
    }

    rule cell_valid( $x, $y ) {
        between( 1, 10, $y );
        between( 1, 10, $x );
        cell_unshot( $x, $y );
    }

    rule adjacent( $x1, $y1, $x2, $y2 ) {
        is_goal( $x2, [$x1], sub ($v) { $v + 1 } );
        unify( $y1, $y2 );
    }
    rule adjacent( $x1, $y1, $x2, $y2 ) {
        is_goal( $x2, [$x1], sub ($v) { $v - 1 } );
        unify( $y1, $y2 );
    }
    rule adjacent( $x1, $y1, $x2, $y2 ) {
        is_goal( $y2, [$y1], sub ($v) { $v + 1 } );
        unify( $x1, $x2 );
    }
    rule adjacent( $x1, $y1, $x2, $y2 ) {
        is_goal( $y2, [$y1], sub ($v) { $v - 1 } );
        unify( $x1, $x2 );
    }

    # Priority 1: Basic Adjacent Probe (No Line Extension)
    rule select_target( $x, $y, $mode ) {
        fresh my ( $hx, $hy );
        shot_hit( $hx, $hy );
        adjacent( $hx, $hy, $x, $y );
        cell_valid( $x, $y );
        unify( $mode, 'Basic Adjacent Probe' );
    }

    # Priority 2: Linear Sequential Scan
    rule select_target( $x, $y, $mode ) {
        between( 1, 10, $y );
        between( 1, 10, $x );
        cell_unshot( $x, $y );
        unify( $mode, 'Linear Sequential Scan' );
    }
}

sub new ( $class, %args ) {
    $args{name}       = 'NaiveAI';
    $args{difficulty} = 'easy';
    return $class->SUPER::new(%args);
}

sub init_program ($self) {
    $self->{program} = $self->clone_program($NaiveAITemplate::PROGRAM);
    return $self->{program};
}

sub select_target ($self) {
    local $CURRENT_SHOTS = $self->{shots_history};

    my $tx = Logic::Relational::variable('x');
    my $ty = Logic::Relational::variable('y');
    my $tm = Logic::Relational::variable('mode');

    my $q = $self->{program}->query(
        Logic::Relational::Goal::Call->new(
            'select_target', [ $tx, $ty, $tm ]
        )
    );

    my $sol = $q->next // return;
    return ( $sol->value($tx), $sol->value($ty), $sol->value($tm) );
}

1;

