...
3. A plain `.yath.rc` (not a symlink to a versioned file) -- the rc file
is used but **no version is captured**; the caller falls through to the
checkout / `@INC` sources above.
...
When an explicit `V#` is given on the CLI, the lookup prefers a
matching versioned rc file but falls back to a plain `.yath.rc` /
`.yath.user.rc` if no versioned file is found.
