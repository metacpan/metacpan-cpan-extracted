...
package PDF::API3::Compat::API2::Basic::PDF::Literal;
...
sub outobjdeep
{
    ...
    if($self->{-isdict}) 
    {
        ...
        {
            next if($k=~m|^[ \-]|o);
            $fh->print('/'.PDF::API3::Compat::API2::Basic::PDF::Name::string_to_name($k).' ');
            ...
            {
                $fh->print('<<'.join(' ', map { '/'.PDF::API3::Compat::API2::Basic::PDF::Name::string_to_name($_).' '.$self->{$k}->{$_} } sort keys %{$self->{$k}})." >>\n");
            } 
