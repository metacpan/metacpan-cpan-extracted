...
######################################################################
#
# Windows1250 - Source code filter to escape Windows-1250 script
#
# http://search.cpan.org/dist/Char-Windows1250/
#
...
perl $0 Windows-1250_script.pl > Escaped_script.pl.e
END
    }

    print Windows1250::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        my $fh = gensym();
        Ewindows1250::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
        ...
        my $e_script = Windows1250::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Ewindows1250::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
# escape Windows-1250 script
sub Windows1250::escape_script {
    my($script) = @_;
    my $e_script = '';

    # read Windows-1250 script
    my $fh = gensym();
    Ewindows1250::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Ewindows1250(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ewindows1250 '%s.0'; # 'quote' for perl5.6\n", $Windows1250::VERSION); # require run-time routines version

        # use Windows1250 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Windows1250 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Windows1250::VERSION) {
                    my @file = grep -e, map {qq{$_/Windows1250.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Windows1250.pm $version, but @{[__FILE__]} is version $Windows1250::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Ewindows1250.pm $version, but \\\$Ewindows1250::VERSION is \$Ewindows1250::VERSION" if \$Ewindows1250::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Windows1250::VERSION) {
                    die "Script $0 required Windows1250.pm $version, but @{[__FILE__]} is only version $Windows1250::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Ewindows1250::function_ord     = 'Windows1250::ord'     if /\A ord \z/oxms;
                    $Ewindows1250::function_ord_    = 'Windows1250::ord_'    if /\A ord \z/oxms;
                    $Ewindows1250::function_reverse = 'Windows1250::reverse' if /\A reverse \z/oxms;
                    $Ewindows1250::function_getc    = 'Windows1250::getc'    if /\A getc \z/oxms;

                    # avoid: used only once: possible typo at ...
                    $Ewindows1250::function_ord     = $Ewindows1250::function_ord;
                    $Ewindows1250::function_ord_    = $Ewindows1250::function_ord_;
                    $Ewindows1250::function_reverse = $Ewindows1250::function_reverse;
                    $Ewindows1250::function_getc    = $Ewindows1250::function_getc;
                }
            }
        }
    }

    $e_script .= Windows1250::escape();
...
=item * You'll have to download Windows1250.pm and Ewindows1250.pm and put it in your perl lib directory.

=item * You'll need to write "use Windows1250;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Windows1250;
  use Windows1250 ver.sion;             --- require minimum version
  use Windows1250 ver.sion.0;           --- expects version (match or die)

  # "no Windows1250;" not supported

  or

  $ perl Windows1250.pm Windows-1250_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Windows-1250_script.pl  --- script written in Windows-1250
...
Windows1250 software is "middleware" between perl interpreter and your Perl script
written in Windows-1250.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Windows-1250, Perl3 or later
can treat Windows-1250 as binary data.

By "use Windows1250;", it automatically interpret your script as Windows-1250. The various
functions of perl including a regular expression can treat Windows-1250 now.
...
This software is a source code filter to escape Perl script encoded by Windows-1250
...
=item * Remains One Language Handling Raw Windows-1250, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Windows1250.pm, Ewindows1250.pm
  +---------------------------------------+
...
import of Ewindows1250.pm works well.
   ...
   sub import {
       ...
       if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
           ...
           for (@ARGV) {
               ...
               # has space
               if (/\A (?:$q_char)*? [ ] /oxms) {
                   if (my @glob = Ewindows1250::glob(qq{"$_"})) {
                       push @argv, @glob;
                   }
                   else {
                       push @argv, $_;
               ...
               # has wildcard metachar
                   ...
                   if (my @glob = Ewindows1250::glob($_)) {
                       push @argv, @glob;
                   }
                   else {
                       push @argv, $_;
...
=head1 Software Composition
   ...
   Windows1250.pm               --- source code filter to escape Windows-1250
   Ewindows1250.pm              --- run-time routines for Windows1250.pm
...
You need write 'use Windows1250;' in your script.
...
=head1 Calling 'Ewindows1250::ignorecase()' (Windows1250 software provides)

Windows1250 software applies calling 'Ewindows1250::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Ewindows1250::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Ewindows1250.pm provides)
...
Also POSIX-style character classes.
  ...
  [:^alnum:]    ${Ewindows1250::not_alnum}
  [:^alpha:]    ${Ewindows1250::not_alpha}
  [:^ascii:]    ${Ewindows1250::not_ascii}
  [:^blank:]    ${Ewindows1250::not_blank}
  [:^cntrl:]    ${Ewindows1250::not_cntrl}
  [:^digit:]    ${Ewindows1250::not_digit}
  [:^graph:]    ${Ewindows1250::not_graph}
  [:^lower:]    ${Ewindows1250::not_lower}
                ${Ewindows1250::not_lower_i}    (/i modifier)
  [:^print:]    ${Ewindows1250::not_print}
  [:^punct:]    ${Ewindows1250::not_punct}
  [:^space:]    ${Ewindows1250::not_space}
  [:^upper:]    ${Ewindows1250::not_upper}
                ${Ewindows1250::not_upper_i}    (/i modifier)
  [:^word:]     ${Ewindows1250::not_word}
  [:^xdigit:]   ${Ewindows1250::not_xdigit}
  ---------------------------------------------------------------
...
Definitions in Ewindows1250.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Ewindows1250::dot}            qr{(?>[^\x0A])};
  ${Ewindows1250::dot_s}          qr{(?>[\x00-\xFF])};
  ${Ewindows1250::eD}             qr{(?>[^0-9])};
  ${Ewindows1250::eS}             qr{(?>[^\s])};
  ${Ewindows1250::eW}             qr{(?>[^0-9A-Z_a-z])};
  ${Ewindows1250::eH}             qr{(?>[^\x09\x20])};
  ${Ewindows1250::eV}             qr{(?>[^\x0A\x0B\x0C\x0D])};
  ${Ewindows1250::eR}             qr{(?>\x0D\x0A|[\x0A\x0D])};
  ${Ewindows1250::eN}             qr{(?>[^\x0A])};
  ${Ewindows1250::not_alnum}      qr{(?>[^\x30-\x39\x41-\x5A\x61-\x7A])};
  ${Ewindows1250::not_alpha}      qr{(?>[^\x41-\x5A\x61-\x7A])};
  ${Ewindows1250::not_ascii}      qr{(?>[^\x00-\x7F])};
  ${Ewindows1250::not_blank}      qr{(?>[^\x09\x20])};
  ${Ewindows1250::not_cntrl}      qr{(?>[^\x00-\x1F\x7F])};
  ${Ewindows1250::not_digit}      qr{(?>[^\x30-\x39])};
  ${Ewindows1250::not_graph}      qr{(?>[^\x21-\x7F])};
  ${Ewindows1250::not_lower}      qr{(?>[^\x61-\x7A])};
  ${Ewindows1250::not_lower_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Ewindows1250::not_lower_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
  ${Ewindows1250::not_print}      qr{(?>[^\x20-\x7F])};
  ${Ewindows1250::not_punct}      qr{(?>[^\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E])};
  ${Ewindows1250::not_space}      qr{(?>[^\s\x0B])};
  ${Ewindows1250::not_upper}      qr{(?>[^\x41-\x5A])};
  ${Ewindows1250::not_upper_i}    qr{(?>[^\x41-\x5A\x61-\x7A])}; # Perl 5.16 compatible
# ${Ewindows1250::not_upper_i}    qr{(?>[\x00-\xFF])};                   # older Perl compatible
  ${Ewindows1250::not_word}       qr{(?>[^\x30-\x39\x41-\x5A\x5F\x61-\x7A])};
  ${Ewindows1250::not_xdigit}     qr{(?>[^\x30-\x39\x41-\x46\x61-\x66])};
  
  # This solution is not perfect. I beg better solution from you who are reading this.
  ${Ewindows1250::eb}             qr{(?:\A(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[0-9A-Z_a-z])|(?<=[0-9A-Z_a-z])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]|\z))};
  ${Ewindows1250::eB}             qr{(?:(?<=[0-9A-Z_a-z])(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]))};
  ---------------------------------------------------------------------------------------------------------------------------------------------------------

=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Windows1250 software provides)

Windows1250 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Windows1250 software provides)

Insert 'Ewindows1250::' at head of function name. Ewindows1250.pm provides your script Ewindows1250::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Ewindows1250::lc        Character
  lcfirst     Ewindows1250::lcfirst   Character
  uc          Ewindows1250::uc        Character
  ucfirst     Ewindows1250::ucfirst   Character
  fc          Ewindows1250::fc        Character
  chr         Ewindows1250::chr       Character
  glob        Ewindows1250::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Windows1250 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Windows1250 software provides)

Windows1250 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Ewindows1250.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Windows1250 software and
...
exist at time of be written this document and JPerl did exist.
                        ...
                        (a)     (b)     (c)     (d)     (e)
                                      JPerl,japerl    Encode,Windows1250
      +--------------+-------+-------+-------+-------+-------+
      | data         |  Old  |  Old  |  New  |  Old  |  New  |
      +--------------+-------+-------+-------+-------+-------+
      | script       |  Old  |      Old      |      New      |
...
Windows1250 software remains one language and one interpreter.
