    ...
    if ($cmdline_mod eq 'Perinci::CmdLine::Inline') {
        ...
        if ($cmdline_mod =~ /\APerinci::CmdLine::(Lite|Any)\z/) {
            if ($cmdline_mod eq 'Perinci::CmdLine::Lite') {
                $cmdline_mod_ver = "1.72";
            } else {
                $extra_modules->{"Perinci::CmdLine::Base"} = "1.72";
            }
        }
...
This document describes version 0.45 of Perinci::CmdLine::Gen (from Perl distribution Perinci-CmdLine-Gen), released on 2017-01-29.

=head1 FUNCTIONS


=head2 gen_pericmd_script(%args) -> [status, msg, result, meta]
