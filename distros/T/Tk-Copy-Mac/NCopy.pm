...
=head1 SYNOPSIS
    ...
    -bufsize specifies the copy buffer size, default = 2,097,152 bytes.

    For Perl/Tk there are three callbacks that you may devise to keep
    Tk events flowing and/or to configure your Perl/Tk application:
    -precopycommand, -duringcopycommand and -postcopycommand.  They
    are called with these arguments:

    -precopycommand:     my ($from, $to, @your_args) = @_;

    -duringcopycommand:  my ($from, $to, $bytes_written, @your_args) = @_;

    -postcopycommand:    my ($from, $to, @your_args) = @_;

    Quite often, @your_args nominally consists of $self, a widget reference.
    As an example, please see Tk::MacCopy.
...
2002/09/27 version 0.33.1
Perl/Tk mods and various bug fixes by John W. Krahn <krahnj@acm.org>
and Steve Lidie <sol0@Lehigh.EDU>.
...
NO_FILE:
    ...
    while(! $this->{test} && 1) {
        $len = sysread(FILE_FROM, $buf, $this->{-bufsize});
        return 0
            unless defined $len;
        last
            unless $len > 0;
	...
	return 0 if $this->{_stop};
	if ($this->{'-duringcopycommand'}) {
	    my ($command, @args) = @{$this->{'-duringcopycommand'}};
	    $command->($file_from, $file_to, $write_n, @args);
	}
    ...
    if ($this->{'-postcopycommand'}) {
	my ($command, @args) = @{$this->{'-postcopycommand'}};
	$command->($file_from, $file_to, @args);
    }
...
sub new(@)
{
	...
	# For Perl/Tk integration.
        ...
        $conf->{'-bufsize'} = $ref->{'-bufsize'}
            if defined $ref->{'-bufsize'};
        $conf->{'-precopycommand'} = $ref->{'-precopycommand'}
            if defined $ref->{'-precopycommand'};
        $conf->{'-duringcopycommand'} = $ref->{'-duringcopycommand'}
            if defined $ref->{'-duringcopycommand'};
        $conf->{'-postcopycommand'} = $ref->{'-postcopycommand'}
            if defined $ref->{'-postcopycommand'};
