#!/usr/bin/env perl
# Tier-0 comment-AST tests for the pure-Perl DMS parser. Mirrors the
# Rust reference's tests/comment_*: leading, blank-line→floating,
# trailing, end-of-block-floating, block-form-as-leading,
# front-matter-prefix, and no-comments-means-empty.
use strict;
use warnings;
use Test::More;
use FindBin;
...
sub parse {
    return DMS::Parser::decode_document($_[0]);
}
...
# 1. Leading: comment immediately precedes a sibling kvpair (no blank gap).
{
    ...
    is($ac->{comment}{content}, '# leading', 'leading: content preserved');
}

# 2. Blank-line gap → floating on enclosing container (root here).
{
    my $doc = parse("# floating\n\nport: 8080\n");
    is(scalar @{$doc->{comments}}, 1, 'floating: one comment');
    my $ac = $doc->{comments}[0];
    is($ac->{position}, 'floating', 'floating: position');
    is_deeply(path_strs($ac->{path}), [], 'floating: empty path = root');
    is($ac->{comment}{content}, '# floating', 'floating: content');
}

# 3. Trailing: same line, after the value.
{
...
# 4. End-of-block: leftover pending floats on the enclosing container.
{
    my $doc = parse("a:\n  x: 1\n  # leftover\n");
    is(scalar @{$doc->{comments}}, 1, 'end-of-block: one comment');
    my $ac = $doc->{comments}[0];
    is($ac->{position}, 'floating', 'end-of-block: position');
    is_deeply(path_strs($ac->{path}), ['a'], 'end-of-block: path = container');
    is($ac->{comment}{content}, '# leftover', 'end-of-block: content');
}

# 5. Block-form `### ... ###` attaches as leading, delimiters preserved.
{
    my $doc = parse("###\nNOTE\n###\nport: 8080\n");
...
# 6. Front-matter comments: path is prefixed with the string "__fm__".
{
    my $doc = parse("+++\n# meta-leading\nauthor: \"x\"\n+++\nbody: 1\n");
...
# 7. No comments → empty arrayref (not undef).
{
    my $doc = parse("a: 1\nb: 2\n");
...
# 8. List item: leading comment attaches with DMS::Parser::Index breadcrumb.
{
    my $doc = parse("+ 1\n# before-second\n+ 2\n");
    # The first item gets no leading (no preceding comment); the
    # second item gets the leading.
