...
my $ok = eval { my $h = Data::Sync::Shared::Semaphore->new(undef, 10); 1 };
...
run_child('embedded NUL path',     sub { Data::Sync::Shared::Semaphore->new("/tmp/a\x00b.shm", 16) });

# --- zero capacity ---
run_child('cap=0',                 sub { Data::Sync::Shared::Semaphore->new(undef, 0) });

# --- astronomically huge capacity (mmap may succeed reserving address space) ---
run_child('huge capacity',         sub { Data::Sync::Shared::Semaphore->new(undef, 2 ** 50) });

# --- new_from_fd with bogus fd ---
SKIP: {
    skip 'no new_from_fd', 2 unless Data::Sync::Shared::Semaphore->can('new_from_fd');
    run_child('new_from_fd(-1)', sub { Data::Sync::Shared::Semaphore->new_from_fd(-1) });
    eval { Data::Sync::Shared::Semaphore->new_from_fd(-1) };
