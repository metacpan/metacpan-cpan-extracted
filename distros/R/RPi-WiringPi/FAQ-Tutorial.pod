...
C<RPi::WiringPi::Constant>, you can also use C<PWM_MODE_MS> or C<PWM_MODE_BAL>.
...
you *must* call C<$self->register_pin($pin_num)> in order to have the cleanup
functionality tidy things up properly. Neglecting to do this will prevent the
cleanup regimen from knowing about these pins, and therefore will be left in an
inconsistent state, possibly causing damage on a different run.
