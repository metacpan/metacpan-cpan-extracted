package Faker::Plugin::InternetRootDomain;
...
method execute() {
  ...
  return $faker->process(['internet', 'root_domain']);
}
...
Faker::Plugin::InternetRootDomain

=cut

=head1 ABSTRACT

Internet Root Domain Plugin for Faker
...
=head1 SYNOPSIS
  ...
  use Faker;
  use Faker::Plugin::InternetRootDomain;

  my $f = Faker->new;
  my $p = Faker::Plugin::InternetRootDomain->new(faker => $f);
...
This package provides methods for generating fake internet root domain data.
...
The execute method returns a random fake internet root domain.
