package Pcore::Lib::Data;

use Pcore -const, -export;
use Pcore::Lib::Text qw[decode_utf8 encode_utf8 escape_perl trim];
use Pcore::Lib::List qw[pairs];
use Sort::Naturally qw[nsort];
use Pcore::Lib::Scalar qw[is_ref is_blessed_ref is_plain_scalarref is_plain_arrayref is_plain_hashref];
...
sub to_uri_query : prototype($) ($data) {
    ...
    $data = $data->get_hash if is_blessed_ref $data && $data->isa('Pcore::Lib::Hash::Multivalue');
