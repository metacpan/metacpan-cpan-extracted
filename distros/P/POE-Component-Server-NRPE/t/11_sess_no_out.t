...
sub _response {
  ...
  cmp_ok( $res->{data}, 'eq', 'NRPE: Unable to read output', 'And the data was cool' ) or diag("Got '$res->{data}', expected 'NRPE: Unable to read output'\n");
  $nrped->shutdown();
  return;
}
...
sub _check_session {
  ...
  $kernel->post( $sender, 'return_result', $id, 0 );
  return;
}
