...
package Sah::SchemaR::unix::username::exists;

our $DATE = '2024-11-15'; # DATE
our $VERSION = '0.022'; # VERSION

our $rschema = do{my$var={base=>"unix::username",clsets_after_base=>[{description=>"\nSupport coercion from UID.\n\n",prefilters=>["Unix::convert_uid_to_unix_user","Unix::check_unix_user_exists"],summary=>"Unix user name that must exist on the system"}],clsets_after_type=>[{description=>"\nThe checking follows POSIX rules: does not begin with a hyphen and only contains\n[A-Za-z0-9._-].\n\nThe above rule allows integers like 1234, which can be confused with UID, so\nthis schema disallows pure integers.\n\nThe maximum length is 32 following libc6's limit.\n\nNote that this schema does not check whether the user name exists (has record in\nthe user database e.g. `/etc/group`). To do that, use the\n`unix::username::exists` schema.\n\n",examples=>[{valid=>0,value=>""},{valid=>1,value=>"foo"},{valid=>0,value=>"-andy"},{valid=>0,value=>1234},{valid=>1,value=>"andy2"},{valid=>0,value=>"an dy"},{valid=>1,value=>"an.dy"},{summary=>"Too long",valid=>0,value=>"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}],match=>qr((?=\A[A-Za-z0-9._][A-Za-z0-9._-]{0,31}\z)(?=.*[A-Za-z._-])),max_len=>32,min_len=>1,prefilters=>["Unix::convert_uid_to_unix_user"],summary=>"Unix account name","x.completion"=>["unix_user_or_uid"]},'$var->{clsets_after_base}[0]'],"clsets_after_type.alt.merge.merged"=>['$var->{clsets_after_type}[0]','$var->{clsets_after_base}[0]'],resolve_path=>["str","unix::username"],type=>"str",v=>2};$var->{clsets_after_type}[1]=$var->{clsets_after_base}[0];$var->{"clsets_after_type.alt.merge.merged"}[0]=$var->{clsets_after_type}[0];$var->{"clsets_after_type.alt.merge.merged"}[1]=$var->{clsets_after_base}[0];$var};
...
Sah::SchemaR::unix::username::exists - Unix user name that must exist on the system

=head1 VERSION

This document describes version 0.022 of Sah::SchemaR::unix::username::exists (from Perl distribution Sah-SchemaBundle-Unix), released on 2024-11-15.
