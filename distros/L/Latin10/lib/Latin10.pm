...
perl $0 Latin-10_script.pl > Escaped_script.pl.e
END
    }

    print Latin10::escape_script($ARGV[0]);
    exit 0;
}
...
sub Latin10::escape_script {
    ...
    # read Latin-10 script
    my $fh = gensym();
    Elatin10::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Elatin10(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Elatin10 '%s.0'; # 'quote' for perl5.6\n", $Latin10::VERSION); # require run-time routines version

        # use Latin10 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | Latin10 ) (?>\s*) ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $Latin10::VERSION) {
                    my @file = grep -e, map {qq{$_/Latin10.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Latin10.pm $version, but @{[__FILE__]} is version $Latin10::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Elatin10.pm $version, but \\\$Elatin10::VERSION is \$Elatin10::VERSION" if \$Elatin10::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $Latin10::VERSION) {
                    die "Script $0 required Latin10.pm $version, but @{[__FILE__]} is only version $Latin10::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Elatin10::function_ord     = 'Latin10::ord'     if /\A ord \z/oxms;
                    $Elatin10::function_ord_    = 'Latin10::ord_'    if /\A ord \z/oxms;
                    $Elatin10::function_reverse = 'Latin10::reverse' if /\A reverse \z/oxms;
                    $Elatin10::function_getc    = 'Latin10::getc'    if /\A getc \z/oxms;

                    # avoid: used only once: possible typo at ...
                    $Elatin10::function_ord     = $Elatin10::function_ord;
                    $Elatin10::function_ord_    = $Elatin10::function_ord_;
                    $Elatin10::function_reverse = $Elatin10::function_reverse;
                    $Elatin10::function_getc    = $Elatin10::function_getc;
                }
            }
        }
    }

    $e_script .= Latin10::escape();
...
=item * You'll have to download Latin10.pm and Elatin10.pm and put it in your perl lib directory.

=item * You'll need to write "use Latin10;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Latin10;
  use Latin10 ver.sion;             --- require minimum version
  use Latin10 ver.sion.0;           --- expects version (match or die)

  # "no Latin10;" not supported

  or

  $ perl Latin10.pm Latin-10_script.pl > Escaped_script.pl.e
...
Latin10 software is "middleware" between perl interpreter and your Perl script
written in Latin-10.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Latin-10, Perl3 or later
can treat Latin-10 as binary data.

By "use Latin10;", it automatically interpret your script as Latin-10. The various
functions of perl including a regular expression can treat Latin-10 now.
...
This software is a source code filter to escape Perl script encoded by Latin-10
...
=item * Remains One Language Handling Raw Latin-10, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Latin10.pm, Elatin10.pm
  +---------------------------------------+
...
import of Elatin10.pm works well.
...
=head1 Software Composition
   ...
   Latin10.pm               --- source code filter to escape Latin-10
   Elatin10.pm              --- run-time routines for Latin10.pm
...
You need write 'use Latin10;' in your script.
...
=head1 Calling 'Elatin10::ignorecase()' (Latin10 software provides)

Latin10 software applies calling 'Elatin10::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Elatin10::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Elatin10.pm provides)
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (Latin10 software provides)

Latin10 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (Latin10 software provides)

Insert 'Elatin10::' at head of function name. Elatin10.pm provides your script Elatin10::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Elatin10::lc        Character
  lcfirst     Elatin10::lcfirst   Character
  uc          Elatin10::uc        Character
  ucfirst     Elatin10::ucfirst   Character
  fc          Elatin10::fc        Character
  chr         Elatin10::chr       Character
  glob        Elatin10::glob      Character
  -------------------------------------------
...
=head1 Yada Yada Operator (Latin10 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (Latin10 software provides)

Latin10 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Elatin10.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in Latin10 software and
...
Latin10 software remains one language and one interpreter.
