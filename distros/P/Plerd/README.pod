...
Plerd also supports IndieWeb technologies, such as L<microformats|https://indieweb.org/microformats> and L<Webmention|https://indieweb.org/Webmention>.
...
=head3 A brief apologia for Plerd
...
L<This 20-minute presentation about my reasons for creating Plerd|http://fogknife.com/2015-06-09-my-yapcna-2015-talk-about-blogging.html> remains as true today as it was when I delivered it to a Perl conference in 2015.
...
I do my best to avoid breaking functionality when I do release updates to Plerd, and to clearly announce when something does need to break. To stay informed about Plerd development, follow L<the Plerd blog|https://plerd.jmac.org>, where I post news about significant updates, including breaking changes.
...
B<To install Plerd from source> instead, set the current working directory to the same directory containing this README file, make sure you have C<cpanm> as described above, and then do this:
...
Run C<plerdall --init> to create a new directory called C<plerd/> in your current working directory, populated with all the special files and directories that Plerd requires. This includes a sample config file in C<plerd/conf/plerd.conf>.
...
C<source>: The directory holding your blog's Markdown-based source files.

=item *

C<templates>: Your blog's template directory. C<plerdall --init> will place a full complement of sample templates in this directory for you.

=item *

C<docroot>: Your blog's actual docroot, ready for serving up by the webserver software of your choice.
...
C<db>: The directory containing metadata about your blog's posts.

=item *

C<run>: A directory for PID files and such.

=item *

C<log>: The C<plerdwatcher> program writes logs here.

=back

You can freely add other files or directories in this directory if you wish (a C<drafts> folder, perhaps?). Plerd happily ignores them.

=item 2.

Edit the C<conf/plerd.conf> file to best suit your needs. The file itself is extensively commented and self-documenting.

You can optionally move or copy the configuration file to C<.plerd> in your home directory. If you do, that new copy of the file becomes the default configuration file that Plerd's command-line programs will refer to.

=item 3.

As noted above, the C<templates> directory that you created in the first step of this process contains sample templates that you can customize as much as you'd like. They are rendered using L<Template Toolkit|http://www.template-toolkit.org>. You can't change these template files' names, but you can add new sub-template files that the main templates will invoke via L<the [% INCLUDE %] directive|http://www.template-toolkit.org/docs/manual/Directives.html#section_INCLUDE>, and so on.
...
B<plerdwatcher> runs a daemon that monitors the source directory for changes, republishing files as necessary.

B<I<This is where the magic happens.>> While I<plerdwatcher> runs, any changes you make to your blog's source directory will instantly update your blog's published static files as appropriate. Pair this with a syncing technology such as Dropbox (see L<"Write locally, publish remotely">, below), and updates to source files on your laptop or other preferred writing machine instantly propagate to your published blog.
...
To start writing a new blog post, create a new Markdown file somewhere I<outside of your blog's source directory>. (Recall that any change to the source directory instantly republishes the blog, something you won't want to do with a half-written entry on top.) You can name this file whatever you like, so long as the filename ends in either C<.markdown> or C<.md>.

You must also give your post a title, sometime before you're ready to publish it. To define the title, have the first line of your entry say C<title: [whatever]>, followed by two newlines, followed in turn by the body of your post.
...
Once it has prepared the source file, Plerd updates the blog. It creates an HTML file for the new entry, and adds a link to it from the C<archive.html> page. If the post's date places it among the blog's more recent posts, then the post also appears in the following places:
...
=head2 Write locally, publish remotely
...
Install Dropbox on both your local machine and your webserver machine.

=item 2.

Place your blog's working directory (the one containing the source, docroot, and template subdirectories) somewhere in your synced Dropbox folder.

=item 3.

Update your server's Plerd configuration to use this Dropbox-based location as its working directory.
...
If you define a list of comma-separated tags under a post attribute named C<tags>, then Plerd will add the post to those linked from a file named C<tags/[tag].html>, relative to the blog's docroot. It will also link to that page from C<tags/index.html>.
...
I<Mind your capitalization with tags!> If faced with inconsistent capitalization within a single tag, e.g. one post claims "boston" for a tag and other one claims "Boston", then Plerd will prefer the first tag containing capital letters to one that contains none, and it will retroactively apply it across all relevant posts.
...
can direct Plerd to add L<Open Graph|http://ogp.me> metadata tags to each of your posts. This allows services like Discord and Slack to present attractive little summaries of your blogposts when displaying links to them.
...
If I<not> present, Plerd does I<not> generate any social-media metadata for any post lacking an I<image> attribute.

=item *

B<image_alt>: A textual description of the image referenced by the C<image> attribute. (Equivalent in usage to the "alt" attribute in an HTML C<<< <img> >>> tag.) Plerd just leaves this blank if you don't define it yourself.
...
B<image>: The URL of an image to associate with this post within social-media links. (This could refer to an image that also appears in your post by way of an HTML C<<< <img> >>> tag, but it doesn't have to.)
...
=head3 GitHub-Flavored Markdown

Plerd processes your posts with L<Markdown::Perl|https://metacpan.org/pod/Markdown::Perl>
...
reference, see L<pmarkdown supported syntax|https://github.com/mkende/pmarkdown/blob/main/Syntax.md>.
...
Plerd supports L<Webmention|https://jmac.org/webmention/>, an open technology that allows websites to send simple "Hey, this page of mine contains a link to that page of yours!" messages to other websites. If the linking page employs L<Microformats2|http://microformats.io> metadata, then the target page can choose to display salient information about the mention, such as its author, or a summary of its content. It can adjust the format of this display depending upon the mention's apparent type -- a "like", a repost, a comment-style response, and so on.

With certain options enabled, Plerd can send webmentions to websites that your blog posts link to. Consult the documentation of the C<plerdwatcher> and C<plerdall> programs for details about Webmention-related options.
...
To keep up to date on Plerd news, follow L<the Plerd blog|https://plerd.jmac.org>.

You can also L<email me, Jason McIntosh|mailto:jmac@jmac.org>, directly. I am always interested to hear about other folks making use of Plerd, and I will do whatever I can to help them with it. All such feedback does tend to make the software that much better, after all!
