# This Makefile is for the DataFax extension to perl.
#
...
#     ABSTRACT => q[an DataFax object]
#     AUTHOR => q[geotiger2001@yahoo.com]
#     DISTNAME => q[DataFax-0.10.tar.gz]
#     NAME => q[DataFax]
#     PREREQ_PM => { Test::More=>q[0.45], Test::Harness=>q[0.1] }
#     VERSION => q[0.1]
#     test => { TESTS=>q[t/DataFax.t] }
...
TO_INST_PM = DataFax.pm

PM_TO_BLIB = DataFax.pm \
	$(INST_LIB)/DataFax.pm
...
DISTNAME = DataFax-0.10.tar.gz
DISTVNAME = DataFax-0.10.tar.gz-0.1
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  DataFax.pm $(INST_MAN3DIR)/DataFax.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  $(MAKEFILE_OLD) $(FIRST_MAKEFILE) $(INST_LIB)/DataFax.pm
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         DataFax-0.10.tar.gz' >> META.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>an DataFax object</ABSTRACT>' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  DataFax.pm $(INST_LIB)/DataFax.pm 
	$(NOECHO) $(TOUCH) $@
