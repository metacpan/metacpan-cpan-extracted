package App::PerlShell::AddOn::ShellCommands;

########################################################
# AUTHOR = Michael Vincent
# www.VinsWorld.com
########################################################

use strict;
use warnings;
use Pod::Usage;
use Pod::Find qw( pod_where );

our $caller = caller;

my $App_PerlShell_AddOn_ShellCommands = "
package $caller;

our \$AUTOLOAD;

sub AUTOLOAD {
    my \$program = \$AUTOLOAD;
    my \$retType = wantarray;

    \$program =~ s/^.*:://;
    my \@rets = `\$program \@_`;

    if ( not defined \$retType ) {
        print \@rets;
        return;
    } elsif ( \$retType ) {
        return \@rets;
    } else {
        return \\\@rets;
    }
}

sub DESTROY { return }

1;
";

eval $App_PerlShell_AddOn_ShellCommands;

use Exporter;

our @EXPORT = qw(
    ShellCommands
);

our @ISA = qw ( Exporter );

sub ShellCommands {
    pod2usage(
        -verbose => 2,
        -exitval => "NOEXIT",
        -input   => pod_where( {-inc => 1}, __PACKAGE__ )
    );
}

1;

__END__

########################################################
# Start POD
########################################################

=head1 NAME

App::PerlShell::AddOn::ShellCommands - Perl Shell Commands from OS Shell

=head1 SYNOPSIS

 plsh> use App::PerlShell::AddOn::ShellCommands;
 plsh> cat('filename.txt');

=head1 DESCRIPTION

B<App::PerlShell::AddOn::ShellCommands> provides an extension to 
B<App::PerlShell> to run commands from the operating system shell 
in the App::PerlShell.

Note:  Parenthesis ()s must be used around arguments to shell commands 
as shown with C<cat> in the above example.

=head1 COMMANDS

=head2 ShellCommands - provide help

Provides help.

=head1 SEE ALSO

L<App::PerlShell>

=head1 LICENSE

This software is released under the same terms as Perl itself.
If you don't know what that means visit L<http://perl.com/>.

=head1 AUTHOR

Copyright (c) 2016 Michael Vincent

L<http://www.VinsWorld.com>

All rights reserved

=cut
