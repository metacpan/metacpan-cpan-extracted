...
TODO: foreach my $hdrs (sort keys %data) {
  local $TODO;
  if($hdrs =~ /gentoo/) { $TODO = "qmail-scanner hack not supported" }
  if($hdrs =~ /gorkcomputer/) { $TODO = "Don't know why this fails" }
  if($hdrs =~ /fetchmail/) { $TODO = "since we work line at a time we can't pop fetchmail->localSMTP headers" }
  if($hdrs =~ /hotmail/) { $TODO = "hotmail is close but not there yet" }
  my $expected = $data{$hdrs};
  $hdrs =~ s/^\s+//ms; $hdrs =~ s/\s+$//ms;
  my $msg = $hdrs."\n\n[no body]\n";
  my $email = Email::Simple->new($msg);
  my @rcvd = $email->header("Received");
  my @stuff = grep { !$_->{reason}} 
            map { parse_received($_) } @rcvd;
  my @expected;
  for ($expected =~ /\[(.*?)\]/g) {
    my $line = {};
    for (split /\s+/, $_) {
  ...
  my @fired;
  for my $i (0..$#stuff) {
      !defined $stuff[$i]->{$_} and $stuff[$i]->{$_} = '' 
        for qw/ip rdns helo ident envfrom auth by id/;
      delete $stuff[$i]->{sub};
      $fired[$i] = delete $stuff[$i]->{rules_fired};
  }
  if(!is_deeply(\@stuff, \@expected)) {
      print "# Input was:\n";
      $hdrs =~ s/^/# /gms;
      print $hdrs;
      print "#\n# Output was:\n";
      my $out = Dumper(\@stuff); use Data::Dumper;
      $out =~ s/^/# /gms;
      print $out;
      print "#\n# Rules fired:\n";
      for my $i (0..$#stuff) {
