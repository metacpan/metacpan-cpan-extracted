package Pcore::Lib::File;

use Pcore;
use Pcore::Lib::Scalar qw[is_glob is_plain_arrayref is_plain_hashref];
...
sub chdir ($path) {    ## no critic qw[Subroutines::ProhibitBuiltinHomonyms]
    if ( defined wantarray ) {
        ...
        require Pcore::Lib::File::ChdirGuard;

        return Pcore::Lib::File::ChdirGuard->new( { dir => $cwd } );
    }
    elsif ( chdir $path ) {
        return;
    }
...
sub umask ($mode) {    ## no critic qw[Subroutines::ProhibitBuiltinHomonyms]
    ...
    if ( defined wantarray ) {
        require Pcore::Lib::File::UmaskGuard;

        return Pcore::Lib::File::UmaskGuard->new( { old_umask => CORE::umask calc_umask($mode) } );
    }
    else {
...
# TEMP
sub tempfile (%args) {
    require Pcore::Lib::File::TempFile;

    return Pcore::Lib::File::TempFile->new(%args);
}

sub tempdir (%args) {
    require Pcore::Lib::File::TempDir;

    return Pcore::Lib::File::TempDir->new( \%args );
}
...
sub temppath {
  ...
  REDO:
    ...
    my $filename = $args{tmpl} =~ s/X/$Pcore::Lib::File::TempFile::TMPL->[rand $Pcore::Lib::File::TempFile::TMPL->@*]/smger . $args{suffix};
