...
my $s = TestSchema->connect( 'dbi:SQLite:t/db/test_database.sqlite3', undef,
    undef, {} );
...
subtest 'Test resultset names when table name is singular' => sub {
    plan tests => 7;
    is( [ $s->humans->result_source->columns ], [qw(id name)],
    ...
    is(
        $s->humans->search( { name => { '-like' => '%Holloway' } } )->count(),
        2, 'Search can find multiple entries'
    );
    is(
        $s->humans->search( { name => { '-like' => '%Hollowai' } } )->count(),
...
subtest 'Test resultset names when table name is plural' => sub {
    ...
    is(
        $s->cars->search( { model => { '-like' => '%C%' } } )->count(),
        2, 'Search can find multiple entries'
    );
    is(
        $s->cars->search( { model => { '-like' => '%Chev%' } } )->count(),
        0, 'Search can find zero entries'
    );
};
...
subtest 'Check resultsets when schema name is plural' => sub {
    ...
    is( $s->box(1)->description, 'Test Box',
        'Singluar term correctly returns result' );
};

subtest 'Check relationships going toward a plural term' => sub {
    plan tests => 1;
    is( $s->human(3)->cars->count(), 3,
        'singular RS to plural relation works' );
};
...
subtest 'Check to make sure term overrides do what they should.' => sub {
    plan tests => 4;
    is(
        [ $s->clocks->result_source->columns ],
        [qw(id created_at SKU)], 'Plural term returns resultset'
    );
    is( $s->block(1)->SKU, 'SKU123',
        'Singluar term correctly returns result' );
    like(
        dies { $s->widgets },
