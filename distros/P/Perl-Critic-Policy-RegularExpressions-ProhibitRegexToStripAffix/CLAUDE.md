...
Perl-Critic-Policy-RegularExpressions-ProhibitRegexToStripAffix is one
Perl::Critic policy.  It reports a match that anchors a run of literal text to
one end of a string and captures the rest, such as `m/\A(\w+)\.pm\z/`, because
`substr` takes a known prefix or suffix off without running a pattern.
...
| | |
|---|---|
| `perldoc Perl::Critic::Policy::RegularExpressions::ProhibitRegexToStripAffix` | what this is for and how a caller uses it |
