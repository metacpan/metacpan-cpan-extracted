package HTML::Widget::JavaScript::Constraint::HTTP;

use warnings;
use strict;

use base 'HTML::Widget::Constraint::HTTP';

=head1 NAME

HTML::Widget::JavaScript::Constraint::HTTP - JavaScript HTTP Constraint
...
See L<HTML::Widget::Constraint::HTTP>.
...
sub emit_javascript {
	...
	for my $param (@{$self->names}) {
		push(@js_constraints, qq[ (${var_name}.${param}.value != '' && $not /^s?https?:\\/\\/[-_.!~*'()a-zA-Z0-9;\\/?:\@&=+\$,%#]+\$/.test(${var_name}.${param}.value)) ]);	
	}
