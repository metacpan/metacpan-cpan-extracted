package 
    XS::Install::FrozenShit::Typemaps::Cmd;
use 5.006001;
use strict;
use warnings;
our $VERSION = '3.57';
...
sub embeddable_typemap {
  ...
  if (@tm_objs == 1) {
    ...
    # multiple, need merge
    $final_tm = XS::Install::FrozenShit::Typemaps->new;
  ...
  my %sources = (
    module => sub {
      ...
      if (/::/) { # looks like FQ module name, try that first
        foreach my $module ($ident, "XS::Install::FrozenShit::Typemaps::$ident") {
        ...
        foreach my $module ("XS::Install::FrozenShit::Typemaps::$ident", "$ident") {
          if (_load_module($module)) {
            eval { $tm = $module->new }
              and return $tm;
          }
    ...
    file => sub {
      ...
      return XS::Install::FrozenShit::Typemaps->new(file => $ident);
    },
  );
