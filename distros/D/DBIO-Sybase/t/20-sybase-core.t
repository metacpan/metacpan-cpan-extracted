...
my ($dsn, $user, $pass) = @ENV{map { "DBIO_TEST_SYBASE_${_}" } qw/DSN USER PASS/};
if (not ($dsn && $user)) {
  plan skip_all => join ' ',
    'Set $ENV{DBIO_TEST_SYBASE_DSN}, _USER and _PASS to run this test.',
...
sub get_schema {
  DBIO::Test::Schema->connect($dsn, $user, $pass, {
    on_connect_call => [
...
my $ping_count = 0;
{
  require DBIO::Sybase::Storage::ASE;
  my $ping = DBIO::Sybase::Storage::ASE->can('_ping');
  *DBIO::Sybase::Storage::ASE::_ping = sub {
    $ping_count++;
    goto $ping;
  };
}
...
for my $storage_type (@storage_types) {
  ...
  unless ($storage_type eq 'DBI::Sybase::ASE') { # autodetect
    DBIO::Test::Schema->storage_type("::$storage_type");
  }
  ...
  if ($storage_idx == 0 &&
      $schema->storage->isa('DBIO::Sybase::Storage::ASE::NoBindVars')) {
  ...
  isa_ok( $schema->storage, "DBIO::Sybase::Storage::ASE" );
...
# check redispatch to storage-specific insert when auto-detected storage
  if ($storage_type eq 'DBI::Sybase::ASE') {
    DBIO::Test::Schema->storage_type('::DBI');
    $schema = get_schema();
  }
...
#
  SKIP: {
    # Under FreeTDS BCP is disabled (#20), so _insert_bulk falls back to
    # regular array inserts. The "full column set required" semantic is
    # BCP-only: a regular insert of {charfield=>'foo'} succeeds because
    # artist.name is NULLable and rank has DEFAULT 13. There is nothing to
    # throw on this path. We must also NOT run the insert here, or the extra
    # row would perturb the correlated-subquery count below.
    skip 'missing-required-column throw is BCP-only; no-op on FreeTDS fallback', 1
...
# test insert transaction when there's an active cursor
  SKIP: {
    # FreeTDS cannot run a second statement on a connection that still has an
    # active result set: any new prepare on the cursor's connection fails with
    # "Server name not found in configuration files" (an empty-server-name OC
    # error, not a real DSN lookup). The insert itself succeeds on the writer
    # storage, but the subsequent $row->delete runs on the cursor's connection.
    # This works with the native Sybase OpenClient libraries.
    skip 'active-cursor multiplexing does not work with FreeTDS', 1
      if $schema->storage->_using_freetds;
