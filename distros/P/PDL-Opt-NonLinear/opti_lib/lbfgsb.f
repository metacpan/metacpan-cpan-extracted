...
c     vanuxem gregory (2003): modified outputting information
c 
      subroutine setulb(n, m, x, l, u, nbd, f, g, factr, pgtol,gtol,wa,
     ...
     +                 wa(2*m*n+4*n+12*m*m+12*m), dsave(29),gtol
...
c
c     iprint is an integer array of lenght 2 that must be set by the user.
c       It controls the frequency and type of output generated:
c        iprint(1)<0    no output is generated;
c        iprint(1)=0    print only one line at the last iteration;
c        0<iprint(1)<99 print also f and |proj g| every iprint iterations;
c        iprint(1)=99   print details of every iteration except n-vectors;
c        iprint(1)=100  print also the changes of active set and final x;
c        iprint(1)>100  print details of every iteration including x and g;
c       When iprint(2) > 0, the file iterate.dat will be created to
...
c     ************
     ...
     +          lsgo,lwy,lsy,lss,lyy,lwt,lwn,lsnd,lygo
      ...
      call mainlb(n,m,x,l,u,nbd,f,g,factr,pgtol,gtol,
...
c======================= The end of setulb =============================
      ...
      subroutine mainlb(n, m, x, l, u, nbd, f, g, factr, pgtol,gtol,ws,
     +                  wy,sy, ss, yy, wt, wn, snd, z, r, d, t, wa, sg,
     +                  sgo, yg, ygo, index, iwhere, indx2, task,
     +                  iprint, csave, lsave, isave, dsave)
 
      character*60     task, csave
      logical          lsave(4)
      integer          n, m, iprint(2), nbd(n), index(n),
     +                 iwhere(n), indx2(n), isave(23)
      double precision f, factr, pgtol,
     +                 x(n), l(n), u(n), g(n), z(n), r(n), d(n), t(n), 
     +                 wa(8*m), sg(m), sgo(m), yg(m), ygo(m),gtol, 
...
c           'info' records the termination information.
         ...
         if (iprint(1) .ge. 1 .and. iprint(2) .ge. 1) then
...
c     Compute the infinity norm of the (-) projected gradient.
      ...
      if (iprint(1) .ge. 1) then
         ...
         if (iprint(2) .ge. 1) write (itfile,1003) iter,nfgv,sbgnrm,f
      endif
...
c ----------------- the beginning of the loop --------------------------
 ...
 222  continue
      if (iprint(1) .ge. 99) write (6,1001) iter + 1
      iword = -1
c
...
c     Generate the search direction d:=z-x.
 ...
 666  continue
     ...
     +            dtd,gtol,xstep,stpmx,iter,ifun,iback,nfgv,info,task,
...
c                            skip the L-BFGS update.
         nskip = nskip + 1
         updatd = .false.
         if (iprint(1) .ge. 1) write (6,1004) dr, ddum
         goto 888
      endif 
...
c======================= The end of mainlb =============================
     ...
     +                  prjctd, cnstnd, boxed)
      ...
      integer          n, iprint(2), nbd(n), iwhere(n)
...
c                   this variable is always fixed
  ...
  20  continue
     ...
     +      write (6,*) 'This problem is unconstrained.'
      ...
      if (iprint(1) .gt. 0) write (6,1001) nbdd
...
c======================== The end of bmv ===============================
     ...
     +                  v, nint, sg, yg, iprint, sbgnrm, info, epsmch)
      ...
      integer          n, m, head, col, nint, iprint(2), info, 
...
c       the derivative f1 and the vector p = W'd (for theta = 1).
      ...
      if (sbgnrm .le. zero) then
         if (iprint(1) .ge. 0) write (6,*) 'Subgnorm = 0.  GCP = X.'
	 ...
	 return
      ...
      col2 = 2*col
      f1 = zero
      if (iprint(1) .ge. 99) write (6,3010)
...
c                  is a zero vector, return with the initial xcp as GCP.
         if (iprint(1) .gt. 100) write (6,1010) (xcp(i), i = 1, n)
         return
      endif    
...
c           (if iter=2, initialize heap).
      ...
      if (dt .ne. zero .and. iprint(1) .ge. 100) then
...
c       reset the corresponding component of d to zero.
	 ...
	 xcp(ibp) = l(ibp)
      ...
      if (iprint(1) .ge. 100) write (6,*) 'Variable  ',ibp,'  is fixed.'
...
c       which will be used in computing r = Z'(B(x^c - x) + g).
      ...
      if (iprint(1) .gt. 100) write (6,1010) (xcp(i),i = 1,n)
      if (iprint(1) .ge. 99) write (6,2010)
...
c======================= The end of formt ==============================
     ...
     +                 iwhere, wrk, updatd, cnstnd, iprint, iter)
      ...
      integer n, nfree, nenter, ileave, iprint(2), iter, 
...
c====================== The end of hpsolb ==============================
     ...
     +                  z, stp, dnorm, dtd, gtol, xstep, stpmx, iter,
     ...
     +                 nbd(n), isave(2)
      double precision f, fold, gd, gdold, stp, dnorm, dtd, xstep,gtol,
...
c     **********
      ...
      parameter        (ftol=1.0d-3,xtol=0.1d0)
...
c======================= The end of prn1lb =============================
     ...
     +                  sbgnrm, nint, word, iword, iback, stp, xstep)
      ...
      character*3      word
      integer          n, iprint(2), itfile, iter, nfgv, nact, nint,
     +                 iword, iback
...
c                             the truncated Newton step has been used.
	 word = 'TNT'
      ...
      else if (iprint(1) .gt. 0) then 
         imod = mod(iter,iprint(1))
         if (imod .eq. 0) write (6,2001) iter,f,sbgnrm
      endif
      if (iprint(1) .ge. 1 .and. iprint(2) .ge. 1) write (itfile,3001)
...
c======================= The end of prn2lb =============================
     ...
     +                  cachyt, sbtime, lnscht)
      ...
      character*60     task
      character*3      word
      integer          n, iprint(2), info, itfile, iter, nfgv, nintol,
...
c     ************
      ...
      if (iprint(1) .ge. 0) then
         ...
         if (iprint(1) .ge. 100) then
            write (6,1004) 'X =',(x(i),i = 1,n)
         endif  
         if (iprint(1) .ge. 1) write (6,*) ' F =',f
      endif 
 999  continue
      if (iprint(1) .ge. 0) then
         write (6,3009) task
     ...
     +	       write (6,*)' l(',k,') > u(',k,').  No feasible solution.'
         ...
         if (iprint(1) .ge. 1) write (6,3007) cachyt,sbtime,lnscht
         write (6,3008) time
         if (iprint(1) .ge. 1 .and. iprint(2) .ge. 1) then
...
c======================= The end of projgr =============================
     ...
     +                 col, head, iword, wv, wn, iprint, info)
      ...
      integer          n, m, nsub, col, head, iword, iprint(2), info, 
     +                 ind(nsub), nbd(n)
      double precision theta, 
     +                 l(n), u(n), x(n), d(n), 
     +                 ws(n, m), wy(n, m), 
...
c     ************
      ...
      if (nsub .le. 0) return
      if (iprint(1) .ge. 99) write (6,1001)
...
c     Backtrack to the feasible region.
  ...
  70  continue
	 ...
	 if (iprint(1) .gt.100) write (6,1003) (x(i),i=1,n)
      endif
      ...
      if (iprint(1) .ge. 99) write (6,1004)
