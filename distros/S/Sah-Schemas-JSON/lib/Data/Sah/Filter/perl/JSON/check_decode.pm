...
1;
# ABSTRACT: Check that value can be decoded as JSON
...
Data::Sah::Filter::perl::JSON::check_decode - Check that value can be decoded as JSON

=head1 VERSION

This document describes version 0.007 of Data::Sah::Filter::perl::JSON::check_decode (from Perl distribution Sah-Schemas-JSON), released on 2022-11-15.

=head1 SYNOPSIS

=head2 Using in Sah schema's C<prefilters> (or C<postfilters>) clause

 ["str","prefilters",[["JSON::check_decode"]]]

=head2 Using with L<Data::Sah>:

 use Data::Sah qw(gen_validator);
 
 my $schema = ["str","prefilters",[["JSON::check_decode"]]];
...
=head2 Using with L<Data::Sah:Filter> directly:
 ...
 my $filter = gen_filter([["JSON::check_decode"]]);
...
=head2 Sample data and filtering results
 ...
 "" # INVALID (String is not a valid JSON: malformed JSON string, neither tag, array, object, number, string or atom, at character offset 0 (before "(end of string)") at (eval 2336) line 8. ), unchanged (Empty string is not valid JSON)
 "null" # valid, unchanged
 "[1,2,3]" # valid, unchanged
 "\"foo\"" # valid, unchanged
 "\"foo" # INVALID (String is not a valid JSON: unexpected end of string while parsing JSON string, at character offset 4 (before "(end of string)") at (eval 2340) line 8. ), unchanged (Invalid JSON, missing closing quote)
 {} # INVALID (String is not a valid JSON: malformed JSON string, neither tag, array, object, number, string or atom, at character offset 0 (before "HASH(0x559dffce3820)") at (eval 2341) line 8. ), unchanged (Will become something like "HASH(0x560142d4e5e8)" and fail because it is not valid JSON)
