...
deleted (with C<delete> or C<clear>).
...
An entity's GUID is valid from the time it is created with C<create> until the
...
only ever have one component of any one type (as identified by C<ref>).
...
view for components C<A>, C<B>, and C<C> will have I<at least> all those
...
Returns the number of entities that have been created. Calling C<clear>
resets this number.
...
can use the global C<$a> and C<$b> variables as with regular
L<sort|https://perldoc.perl.org/functions/sort>, which will be assigned to
...
representing a I<view> for that specific set of components. Entities in the
