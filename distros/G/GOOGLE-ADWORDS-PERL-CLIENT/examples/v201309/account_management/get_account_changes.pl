...
use Google::Ads::AdWords::v201309::CustomerSyncSelector;
use Google::Ads::AdWords::v201309::DateTimeRange;
...
sub get_account_changes {
  ...
  my $min_date_time = sprintf("%d%02d%02d %02d%02d%02d", ($year + 1900),
      ($mon + 1), $mday, $hour, $min, $sec);
  ($sec, $min, $hour, $mday, $mon, $year) = localtime(time);
  my $max_date_time = sprintf("%d%02d%02d %02d%02d%02d", ($year + 1900),
      ($mon + 1), $mday, $hour, $min, $sec);

  # Get a list of all campaign ids.
  my @campaign_ids = ();
  my $campaigns = $client->CampaignService()->get({
    serviceSelector => Google::Ads::AdWords::v201309::Selector->new({
      fields => ["Id"]
    })
  });
  ...
  my $date_time_range = Google::Ads::AdWords::v201309::DateTimeRange->new({
    min => $min_date_time,
    max => $max_date_time
  });

  # Create selector.
  my $selector = Google::Ads::AdWords::v201309::CustomerSyncSelector->new({
    dateTimeRange => $date_time_range,
    campaignIds => \@campaign_ids
  });

  # Get all account changes for campaign.
  my $account_changes = $client->CustomerSyncService()->get({
    selector => $selector
  });
