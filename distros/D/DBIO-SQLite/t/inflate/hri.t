...
# So we create a scope here cos I'm lazy
{
    ...
    eval "package DBIO::Test::CDSubclass; use base '$orig_resclass'";

# override on a specific $rs object, should not chain
    $rs->result_class ('DBIO::Test::CDSubclass');

    my $cd = $rs->find ({cdid => 1});
    is (ref $cd, 'DBIO::Test::CDSubclass', 'result_class override propagates to find');
...
# set as attr - should propagate
    ...
    is ($rs->result_class, 'DBIO::Test::CDSubclass', 'original class unchanged');
    ...
    my $hri_rs_no_cursor = $rs->search ({}, { result_class => 'DBIO::ResultClass::HashRefInflator' });
    $hri_rs_no_cursor->result_class ('DBIO::Row'); # something bogus
    is(
        $hri_rs_no_cursor->search->result_class, 'DBIO::ResultClass::HashRefInflator',
...
# test result class auto-loading
    ...
    is ($rs->result_class, 'DBIO::Test::CDSubclass', 'class unchanged');
