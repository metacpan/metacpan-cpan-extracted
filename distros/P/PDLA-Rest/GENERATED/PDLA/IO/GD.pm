...
@EXPORT_OK  = qw( PDLA::PP write_png PDLA::PP write_png_ex PDLA::PP write_true_png PDLA::PP write_true_png_ex  write_png_best write_true_png_best  recompress_png_best  load_lut read_png read_true_png PDLA::PP _read_true_png PDLA::PP _read_png PDLA::PP _gd_image_to_pdl_true PDLA::PP _gd_image_to_pdl PDLA::PP _pdl_to_gd_image_true PDLA::PP _pdl_to_gd_image_lut  read_png_lut PDLA::PP _read_png_lut PDLA::PP _gdImageColorAllocates PDLA::PP _gdImageColorAllocateAlphas PDLA::PP _gdImageSetPixels PDLA::PP _gdImageLines PDLA::PP _gdImageDashedLines PDLA::PP _gdImageRectangles PDLA::PP _gdImageFilledRectangles PDLA::PP _gdImageFilledArcs PDLA::PP _gdImageArcs PDLA::PP _gdImageFilledEllipses  gdAlphaBlend   gdTrueColor   gdTrueColorAlpha   gdFree   gdFontGetLarge   gdFontGetSmall   gdFontGetMediumBold   gdFontGetGiant   gdFontGetTiny  );
...
*write_png_ex = \&PDLA::write_png_ex;
...
*write_true_png = \&PDLA::write_true_png;
...
*write_true_png_ex = \&PDLA::write_true_png_ex;
...
*_read_true_png = \&PDLA::_read_true_png;





*_read_png = \&PDLA::_read_png;





*_gd_image_to_pdl_true = \&PDLA::_gd_image_to_pdl_true;





*_gd_image_to_pdl = \&PDLA::_gd_image_to_pdl;





*_pdl_to_gd_image_true = \&PDLA::_pdl_to_gd_image_true;





*_pdl_to_gd_image_lut = \&PDLA::_pdl_to_gd_image_lut;
...
*_read_png_lut = \&PDLA::_read_png_lut;





*_gdImageColorAllocates = \&PDLA::_gdImageColorAllocates;





*_gdImageColorAllocateAlphas = \&PDLA::_gdImageColorAllocateAlphas;





*_gdImageSetPixels = \&PDLA::_gdImageSetPixels;





*_gdImageLines = \&PDLA::_gdImageLines;





*_gdImageDashedLines = \&PDLA::_gdImageDashedLines;





*_gdImageRectangles = \&PDLA::_gdImageRectangles;





*_gdImageFilledRectangles = \&PDLA::_gdImageFilledRectangles;





*_gdImageFilledArcs = \&PDLA::_gdImageFilledArcs;





*_gdImageArcs = \&PDLA::_gdImageArcs;





*_gdImageFilledEllipses = \&PDLA::_gdImageFilledEllipses;
...
If the hash has:
 ...
 pdl => $pdl_var (lut => $lut_piddle)
    Then a new GD is created from that PDLA variable.
...
Example:
 ...
 my $gd = PDLA::IO::GD->new({ pdl => $pdl_var });
    
 my $gd = PDLA::IO::GD->new({ pdl => $pdl_var, lut => $lut_piddle });
 
 my $gd = PDLA::IO::GD->new({ filename => "image.png" });
 
 my $gd = PDLA::IO::GD->new({ x => 100, y => 100 });
 
 my $gd = PDLA::IO::GD->new({ x => 100, y => 100, true_color => 1 });
 
 my $gd = PDLA::IO::GD->new({ data => $imageData });
 
 my $gd = PDLA::IO::GD->new({ data => $imageData, type => 'wbmp' });
...
sub new
{
    ...
    if( !defined $options ) {
      die <<EOF;
PDLA::IO::GD::new couldn't parse its arguments.
...
EOF
    ...
    {   # Create it from a PDLA variable:
        my $pdl = $options->{pdl};
        $pdl->make_physical();
...
#	    print STDERR "in new (ndims=3), setting IMG_PTR to " . $ptr->at(0) . "\n";
        ...
        {
            die "Can't create a PDLA::IO::GD from a PDLA with bad dims!\n";
        }
    }
    ...
    {   # Create it from a file:
        ...
        if( !defined $options->{filename} ) {
          die "PDLA::IO::GD::new got an undefined filename. Giving up.\n";
        }
...
sub apply_lut
{
    ...
    # Let the PDLA threading engine sort this out:
...
gdImageCopy ( $dst(PDLA::IO::GD), $src(PDLA::IO::GD), $dstX, $dstY, $srcX, $srcY, $w, $h )
...
gdImageCopyMerge ( $dst(PDLA::IO::GD), $src(PDLA::IO::GD), $dstX, $dstY, $srcX, $srcY, $w, $h, $pct )
...
gdImageCopyMergeGray ( $dst(PDLA::IO::GD), $src(PDLA::IO::GD), $dstX, $dstY, $srcX, $srcY, $w, $h, $pct )
...
gdImageCopyResized ( $dst(PDLA::IO::GD), $src(PDLA::IO::GD), $dstX, $dstY, $srcX, $srcY, $dstW, $dstH, $srcW, $srcH )
...
gdImageCopyResampled ( $dst(PDLA::IO::GD), $src(PDLA::IO::GD), $dstX, $dstY, $srcX, $srcY, $dstW, $dstH, $srcW, $srcH )
...
gdImageCompare ( $im1(PDLA::IO::GD), $im2(PDLA::IO::GD) )
...
gdImagePaletteCopy ( $dst(PDLA::IO::GD), $src(PDLA::IO::GD) )
