...
# fake up $m and $r
my $r = FakeRequest->new( { header_in => 'deflate,flarb,fleeble' } );
...
like( $r->content_encoding, qr(^deflate) );

is_deeply( $r->header_out, [ Vary => 'Accept-Encoding' ] );

my ( $inf, undef ) = Compress::Zlib::inflateInit();

like( $inf->inflate( $m->request_buffer ), qr($output2) );
