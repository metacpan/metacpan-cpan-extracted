...
my $pocosi = POE::Component::Server::IRC->spawn(
    ...
    antiflood    => 0,
    plugin_debug => 1,
    config => { servername => 'listen.server.irc', sid => '1FU', anti_spam_exit_message_time => 0, anti_spam_exit_message_time => 0 },
);
...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^bobbins' && $params->[1] eq '+i' ) {
    pass($cmd);
    $poe_kernel->post('groucho', 'send_to_server', { prefix => '4AK', command => 'SVSJOIN', params => [ '1FUAAAAAA', '#smurfs' ] } );
    return;
  }
  # :listen.server.irc 366 bobbins #smurfs :End of NAMES list
  if ( $cmd eq 'JOIN' ) {
    pass($cmd);
    is( $prefix, 'bobbins!~bobbins@listen.server.irc', 'It is I' );
    is( $params->[0], '#smurfs', 'And there is a waterpipe' );
    return;
  }
  if ( $cmd eq 'MODE' ) {
    pass($cmd);
    is( $prefix, 'listen.server.irc', 'Server is setting MODE' );
    is( $params->[0], '#smurfs', 'And there is a waterpipe' );
    is( $params->[1], '+nt', 'MODE is expected' );
    return;
  }
  if ( $cmd eq '353' ) {
    pass("IRC_$cmd");
    is( $params->[2], '#smurfs', 'And there is a waterpipe' );
    is( $params->[3], '@bobbins', 'I am chanop' );
    return;
  }
  if ( $cmd eq '366' ) {
    pass("IRC_$cmd");
    is( $params->[1], '#smurfs', 'And there is a waterpipe' );
