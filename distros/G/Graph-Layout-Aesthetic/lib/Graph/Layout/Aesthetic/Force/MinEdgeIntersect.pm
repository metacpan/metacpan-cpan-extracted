package Graph::Layout::Aesthetic::Force::MinEdgeIntersect;
use 5.006001;
use strict;
use warnings;
...
=head1 SYNOPSIS
  ...
  $aglo->add_force("MinEdgeIntersect", 1);
...
constant force. The code only works in two dimensions, so
it will throw an exception if you try to
L<associate|Graph::Layout::Aesthetic/add_force> this force with a layout that's
not two-dimensional.

The old aglo documentation describes this aesthetic as using a inverse
distance force, but that's actually not what the code does.
