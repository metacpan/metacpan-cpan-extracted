...
static inline void it_init_header(void *base, uint32_t capacity, uint64_t total) {
    ...
    /* Publish magic LAST, as a release store: it is the commit point, so a
       ...
       hand. */
    __atomic_store_n(&hdr->magic, IT_MAGIC, __ATOMIC_RELEASE);
    ...
    if (anonymous) {
        ...
        if (!is_new) {
            if (!it_validate_header((ItHeader *)base, (uint64_t)st.st_size)) {
                ...
                if (((ItHeader *)base)->magic == 0 && (uint64_t)st.st_size == total
                    && st.st_uid == geteuid()) {
                    IT_ERR("%s: incomplete interval tree file left by an interrupted create; remove it and retry", path);
                    munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
                }
                IT_ERR("invalid interval tree file"); munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
            }
            if (((ItHeader *)base)->sealed) {
                IT_ERR("%s is frozen (read-only); open it with new_readonly", path);
                munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
            }
            flock(fd, LOCK_UN); close(fd);
            return it_setup(base, map_size, path, -1);
...
static ItHandle *it_open_fd(int fd, char *errbuf) {
    ...
    if (((ItHeader *)base)->sealed) {
        IT_ERR("this interval tree is frozen (read-only); open it with new_readonly");
 ...
 * file). */
static ItHandle *it_open_readonly(const char *path, char *errbuf) {
    if (errbuf) errbuf[0] = '\0';
    int fd = open(path, O_RDONLY|O_NOFOLLOW|O_CLOEXEC);
    if (fd < 0) { IT_ERR("open %s: %s", path, strerror(errno)); return NULL; }
    struct stat st;
    ...
    if (base == MAP_FAILED) { IT_ERR("mmap %s: %s", path, strerror(errno)); return NULL; }
    if (!it_validate_header((ItHeader *)base, (uint64_t)st.st_size)) {
        IT_ERR("%s: invalid interval tree file", path); munmap(base, ms); return NULL;
    }
    if (!((ItHeader *)base)->sealed) {
        IT_ERR("%s is not frozen: call ->freeze on the producer before opening read-only", path);
        munmap(base, ms); return NULL;
    }
    ItHandle *h = it_setup(base, ms, path, -1);   /* munmaps on OOM */
    if (!h) { IT_ERR("out of memory"); return NULL; }
    h->readonly = 1;
    return h;
}
...
/* forward decl: it_freeze (below) must force a pending build to completion
 * before sealing; it_build_locked itself is defined further down, alongside
 ...
 * the read-only query path (it_open_readonly handles) takes no lock and never
