...
sub Populate {
    ...
    $w->{_BE_Style} = delete $args->{-style} || $Tcl::pTk::platform;
...
#      }
    ...
    my $b = $w->$Button(-bitmap => '@' . Tcl::pTk->findINC($w->{_BE_Style} eq 'MSWin32' ? 'arrowdownwin.xbm' : 'cbxarrow.xbm'));
    $w->Advertise('entry' => $e);
    $w->Advertise('arrow' => $b);
    ...
    if ($w->{_BE_Style} eq 'MSWin32' and $Tcl::pTk::platform eq 'MSWin32') {
...
sub SetBindings {
    ...
    # set bind tags
    $w->bindtags([$w, 'Tcl::pTk::BrowseEntry', $w->toplevel, 'all']);
...
sub PopupChoices {
	...
	my $rootheight;
	if ($Tcl::pTk::platform eq 'MSWin32' and $^O eq 'MSWin32') {
	    eval {
	...
	eval{ $w->grabGlobal; }; # sometimes will fail (if another mainwindow has been ->Busy'ed. But is ok to fail
                                 #  in these cases.
    }
}
...
sub Popdown {
    my ($w) = @_;
    if ($w->{'_BE_savefocus'} && Tcl::pTk::Exists($w->{'_BE_savefocus'})) {
...
sub ButtonHack {
    ...
    my $b = $w->Subwidget('arrow');
    if ($w->{'_BE_buttonHack'}) {
	#$b->butUp; # This function not available for Tcl::pTk
    }
}
...
sub _set_edit_state {
	...
	if (exists $w->{_BE_OriginalCursor}) {
        ...
        # Button1 method of Entry isn't there in Tcl::pTk Is it really needed?
	#if ($w->{_BE_Style} eq 'MSWin32') { 
	#    $entry->bind('<1>',['Button1',Ev('x')]);
	#}
    }
}
