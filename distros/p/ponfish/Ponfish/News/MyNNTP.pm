# Net::NNTP.pm
#
# NOTICE: I grabbed this from the perl distro... instead of subclassing from it,
# which I _probably_ should, to insulate myself from any possible (though highly
# unlikely) changes that might occur.
#
# Copyright (c) 1995-1997 Graham Barr <gbarr@pobox.com>. All rights reserved.
# This program is free software; you can redistribute it and/or
# modify it under the same terms as Perl itself.

package Ponfish::News::MyNNTP;
...
sub sync_headers_file {
  ...
  # I have wrapped the following block of text in a do...until loop
  # because sometimes a "." is sent but before the actual last header
  # requested.
  warn "#"x66,"\n";
  warn "Trying for: '$last'\n";
  warn "#"x66,"\n";
  do {
    $self->_XOVER( "$first-$last" ) || return undef;

    # Taken from sub read_until_dot.
    while(1) {
      my $str = $self->getline() or return undef;
	...
	if ($self->debug & 4);
      ...
      # add the article header!
      $actual_last = $AJ->add( $str );
      $cnt++;
    }
    $first	= $actual_last + 1;	# Set up for next _XOVER
    warn "#"x66,"\n";
    warn "  ---> got up to $first (trying for: $last) <---\n";
    warn "#"x66,"\n";
    if( $max_retries-- <= 0 ) {	# Make sure we don't loop infinitely
      last;
    }
  } until( $actual_last >= $last - 2000 );	# allow for cancellations, other errors...
...
sub append_headers {
  my $self	= shift;
  my $newsgroup	= $self->get_choodat()->{newsgroup};

  my $SOCK	= $self->{SOCK};

#  my $CACHE_FH	= IO::File->new( ">newnews.cache" ) || die "Can't open cache file for write!";
  my $AJ	= $self->get_AJ || die "No ArticleJoiner set!";

  while( <$SOCK> ) {
    s/\015?\012$/$self->{EOL}/; # Change termination
    last if $_ eq ".$self->{EOL}";
    s/^\.\././;			# Fix up escaped dots.
    # print $CACHE_FH $_;		# Cache the original data... for later use
    $AJ->add( $_ );
#    print $_;
  }
  #$CACHE_FH->close;
...
I<$Id: MyNNTP.pm,v 1.2 2006/10/22 18:53:42 dclee Exp $>
