...
PDLA has support (currently experimental) for splitting up numerical processing
...
The use of the term I<threading> can be confusing with PDLA, because it can refer to I<PDLA threading>,
as defined in the L<PDLA::Threading> docs, or to I<processor multi-threading>. 

To reduce confusion with the existing PDLA threading terminology, this document uses 
...
=head1 Functions that control PDLA PThreads

This is a brief listing and description of the PDLA pthreading functions, see the L<PDLA::Core> docs
...
See L<PDLA::Core|set_autopthread_targ> for details.

=item set_autopthread_size

Set the minimum size (in Meg-elements or 2**20 elements) of the largest PDLA involved in a function where auto-pthreading will
...
See L<PDLA::Core|set_autopthread_size> for details.

=item get_autopthread_actual

Get the actual number of pthreads executed for the last pdl processing function.

See L<PDLA::get_autopthread_actual> for details.

=back

=head1 Global Control of PDLA PThreading using Environment Variables

PDLA PThreading can be globally turned on, without modifying existing code by setting 
environment variables B<PDLA_AUTOPTHREAD_TARG> and B<PDLA_AUTOPTHREAD_SIZE> before running a PDLA script.
These environment variables are checked when PDLA starts up and calls to I<set_autopthread_targ> and
I<set_autopthread_size> functions made with the environment variable's values.

For example, if the environment var B<PDLA_AUTOPTHREAD_TARG> is set to 3, and B<PDLA_AUTOPTHREAD_SIZE> is
...
The auto-pthreading process works by analyzing threaded array dimensions in PDLA operations
and splitting up processing based on the thread dimension sizes and desired number of 
pthreads (i.e. the pthread target or pthread_targ). The offsets and increments that PDLA uses to step
...
Auto-PThreading only works if your PDLA installation was compiled with POSIX threads enabled. This is normally
the case if you are running on linux, or other unix variants.

=head2 Non-Threadsafe Code

Not all the libraries that PDLA intefaces to are thread-safe, i.e. they aren't written to operate
in a multi-threaded environment without crashing or causing side-effects. Some examples in the PDLA
...
a function as I<not> being pthread-safe. See L<PDLA::PP> docs for details.

=head2 Size of PDLA Dimensions and PThread Target

Due to the way a PDLA is split-up for operation using multiple pthreads, the size of a dimension
must be evenly divisible by the pthread target. For example, if a PDLA has threaded dimension sizes
...
threaded dimensions. For example, if a PDLA has threaded dimension sizes of (9,2,2) and the
