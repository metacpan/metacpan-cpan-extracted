...
this check must be disabled.  In that case, specify C<true> for
this option.

=item realize => $method|CODE

How will transform.  If you specify a CODE reference, then this will be
called with the lazy-object as first argument, and the requested C<$method>
as second.
...
if the C<$class> (a package) is included in a file (module) with a different
...
=item $any-E<gt>B<can>($method)

Is the specified C<$method> available for the lazy or the realized version
...
=item $class-E<gt>B<isa>($class)

Is this object a (sub-)class of the specified C<$class> or can it become a
(sub-)class of C<$class>.
...
=item $class-E<gt>B<import>(%options)

The C<%options> used for C<import> are the values after the class name
...
=item $class-E<gt>B<realizationOf>( $object, [$realized] )

Returns the C<$realized> version of C<$object>, optionally after setting it
...
=item $class-E<gt>B<realize>(%options)
...
This module is part of Object-Realize-Later version 3.00,
built on November 19, 2025. Website: F<http://perl.overmeer.net/CPAN/>
