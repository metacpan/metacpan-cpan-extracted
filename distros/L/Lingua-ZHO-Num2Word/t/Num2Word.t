...
for my $test (@{$nw}) {
    my $got = Lingua::ZHO::Num2Word::number_to_zh($test->[0]);
    my $exp = $test->[1];
    is($got, $exp, $test->[2]);
    $tests++;
}

dies_ok( sub { Lingua::ZHO::Num2Word::number_to_zh(undef); }, 'undef args (like 0)' );
$tests++;
...
dies_ok( sub { Lingua::ZHO::Num2Word::number_to_zh(undef); }, 'undef args' );
$tests++;

Lingua::ZHO::Num2Word->charset('simplified');
...
Lingua::ZHO::Num2Word->charset('traditional');
