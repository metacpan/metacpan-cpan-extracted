...
my %tests = (
              ...
              'line-unindexed.xml'         => undef, # supposed to pass
              'line-partially-indexed.xml' => 'Line id B is partially indexed but not completely', # supposed to fail
              'line-duplicate-index.xml'   => 'Line id B has non-unique indices: 3',               # supposed to fail
            );
...
for my $name ( sort keys %tests ) {
    ...
    my( $ok, @messages ) = ok_lines_indexed( $map, { name => $name } );
    if ( $tests{$name} ) {
