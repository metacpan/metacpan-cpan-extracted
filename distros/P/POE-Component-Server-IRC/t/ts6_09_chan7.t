...
sub client3_input {
  ...
  if ( $cmd eq '366' ) {
    $poe_kernel->post($sender, 'send_to_server', { command => 'PRIVMSG', params => [ '#floobar', $_ ], colonify => 1 } ) for
      (
          'You are all losers',
  ...
  if ( $cmd eq 'NOTICE' ) {
    pass($cmd);
    is($params->[1],'*** Message to #floobar throttled due to flooding',
      '*** Message to #floobar throttled due to flooding');
