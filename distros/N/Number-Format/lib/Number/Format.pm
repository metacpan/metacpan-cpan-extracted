...
#pod =head1 SYNOPSIS
#pod
#pod   use Number::Format;
#pod   my $x = Number::Format->new(%args);
#pod   $formatted = $x->round($number, $precision);
#pod   $formatted = $x->format_number($number, $precision, $trailing_zeroes);
#pod   $formatted = $x->format_negative($number, $picture);
#pod   $formatted = $x->format_picture($number, $picture);
#pod   $formatted = $x->format_price($number, $precision, $symbol);
#pod   $formatted = $x->format_bytes($number, $precision);
#pod   $number    = $x->unformat_number($formatted);
#pod
#pod   use Number::Format qw(:subs);
#pod   $formatted = round($number, $precision);
#pod   $formatted = format_number($number, $precision, $trailing_zeroes);
#pod   $formatted = format_negative($number, $picture);
#pod   $formatted = format_picture($number, $picture);
#pod   $formatted = format_price($number, $precision, $symbol);
#pod   $formatted = format_bytes($number, $precision);
#pod   $number    = unformat_number($formatted);
#pod
#pod =head1 DESCRIPTION
#pod
#pod These functions provide an easy means of formatting numbers in a
#pod manner suitable for displaying to the user.
#pod
#pod There are two ways to use this package.  One is to declare an object
#pod of type Number::Format, which you can think of as a formatting engine.
#pod The various functions defined here are provided as object methods.
#pod The constructor C<new()> can be used to set the parameters of the
#pod formatting engine.  Valid parameters are:
#pod
#pod   THOUSANDS_SEP     - character inserted between groups of 3 digits
#pod   DECIMAL_POINT     - character separating integer and fractional parts
#pod   MON_THOUSANDS_SEP - like THOUSANDS_SEP, but used for format_price
#pod   MON_DECIMAL_POINT - like DECIMAL_POINT, but used for format_price
#pod   INT_CURR_SYMBOL   - character(s) denoting currency (see format_price())
#pod   DECIMAL_DIGITS    - number of digits to the right of dec point (def 2)
#pod   DECIMAL_FILL      - boolean; whether to add zeroes to fill out decimal
#pod   NEG_FORMAT        - format to display negative numbers (def ``-x'')
#pod   KILO_SUFFIX       - suffix to add when format_bytes formats kilobytes (trad)
#pod   MEGA_SUFFIX       -    "    "  "    "        "         "    megabytes (trad)
#pod   GIGA_SUFFIX       -    "    "  "    "        "         "    gigabytes (trad)
#pod   TERA_SUFFIX       -    "    "  "    "        "         "    terabytes (trad)
#pod   KIBI_SUFFIX       - suffix to add when format_bytes formats kibibytes (iec)
#pod   MEBI_SUFFIX       -    "    "  "    "        "         "    mebibytes (iec)
#pod   GIBI_SUFFIX       -    "    "  "    "        "         "    gibibytes (iec)
#pod   TEBI_SUFFIX       -    "    "  "    "        "         "    tebibytes (iec)
#pod
#pod They may be specified in upper or lower case, with or without a
#pod leading hyphen ( - ).
#pod
#pod If C<THOUSANDS_SEP> is set to the empty string, format_number will not
#pod insert any separators.
#pod
#pod The defaults for C<THOUSANDS_SEP>, C<DECIMAL_POINT>,
#pod C<MON_THOUSANDS_SEP>, C<MON_DECIMAL_POINT>, and C<INT_CURR_SYMBOL>
#pod come from the POSIX locale information (see L<perllocale>).  If your
#pod POSIX locale does not provide C<MON_THOUSANDS_SEP> and/or
#pod C<MON_DECIMAL_POINT> fields, then the C<THOUSANDS_SEP> and/or
#pod C<DECIMAL_POINT> values are used for those parameters.  Formerly,
#pod POSIX was optional but this caused problems in some cases, so it is
#pod now required.  If this causes you hardship, please contact the author
#pod of this package at <SwPrAwM@cpan.org> (remove "SPAM" to get correct
...
#pod   THOUSANDS_SEP     = ','
#pod   DECIMAL_POINT     = '.'
#pod   MON_THOUSANDS_SEP = ','
#pod   MON_DECIMAL_POINT = '.'
#pod   INT_CURR_SYMBOL   = 'USD'
#pod   DECIMAL_DIGITS    = 2
#pod   DECIMAL_FILL      = 0
#pod   NEG_FORMAT        = '-x'
#pod   KILO_SUFFIX       = 'K'
#pod   MEGA_SUFFIX       = 'M'
...
#pod   TEBI_SUFFIX       = 'TiB'
#pod
#pod Note however that when you first call one of the functions in this
#pod module I<without> using the object-oriented interface, further setting
#pod of those global variables will have no effect on non-OO calls.  It is
#pod recommended that you use the object-oriented interface instead for
#pod fewer headaches and a cleaner design.
#pod
#pod The C<DECIMAL_FILL> and C<DECIMAL_DIGITS> values are not set by the
#pod Locale system, but are definable by the user.  They affect the output
#pod of C<format_number()>.  Setting C<DECIMAL_DIGITS> is like giving that
#pod value as the C<$precision> argument to that function.  Setting
#pod C<DECIMAL_FILL> to a true value causes C<format_number()> to append
#pod zeroes to the right of the decimal digits until the length is the
#pod specified number of digits.
#pod
#pod C<NEG_FORMAT> is only used by C<format_negative()> and is a string
#pod containing the letter 'x', where that letter will be replaced by a
#pod positive representation of the number being passed to that function.
#pod C<format_number()> and C<format_price()> utilize this feature by
#pod calling C<format_negative()> if the number was less than 0.
#pod
#pod C<KILO_SUFFIX>, C<MEGA_SUFFIX>, C<GIGA_SUFFIX>, and C<TERA_SUFFIX> are
#pod used by C<format_bytes()> when the value is over 1024, 1024**2, 1024**3,
#pod or 1024**4, respectively.  The default values are "K", "M", "G", and
#pod "T".  These apply in the default "traditional" mode only.
#pod
#pod C<KIBI_SUFFIX>, C<MEBI_SUFFIX>, C<GIBI_SUFFIX>, and C<TEBI_SUFFIX> are
#pod used by C<format_bytes()> when the value is over 1024, 1024**2,
#pod 1024**3, or 1024**4, respectively.  The default values are "KiB", "MiB",
#pod "GiB", "TiB".  These apply in the "iso60027" mode only.
#pod
#pod The only restrictions on C<DECIMAL_POINT> and C<THOUSANDS_SEP> are that
#pod they must not be digits and must not be identical.  There are no
#pod restrictions on C<INT_CURR_SYMBOL>.
#pod
#pod For example, a German user might include this in their code:
#pod
#pod   use Number::Format;
#pod   my $de = Number::Format->new(-thousands_sep   => '.',
#pod                                -decimal_point   => ',',
...
# giga, and tera (un-)formatting.  Used in format_bytes and unformat_number.
...
sub _get_multipliers
{
    ...
    {
        # the "tera" value is not portable, but that's a matter for the caller
...
#pod =method new
#pod
#pod   my $formatter = Number::Format->new( %args );
#pod
#pod Creates a new Number::Format object.  Valid keys for %args are any of
#pod the parameters described above.  Keys may be in all uppercase or all
#pod lowercase, and may optionally be preceded by a hyphen (-) character.
#pod Example:
#pod
#pod   my $de = Number::Format->new(-thousands_sep   => '.',
#pod                                -decimal_point   => ',',
...
#pod =method round
#pod
#pod   my $str = $formatter->($number, $precision);
#pod
#pod Rounds the number to the specified precision.  If C<$precision> is
#pod omitted, the value of the C<DECIMAL_DIGITS> parameter is used (default
#pod value 2).  Both input and output are numeric (the function uses math
#pod operators rather than string manipulation to do its job), The value of
#pod C<$precision> may be any integer, positive or negative. Examples:
#pod
#pod   round(3.14159)       yields    3.14
#pod   round(3.14159, 4)    yields    3.1416
#pod   round(42.00, 4)      yields    42
#pod   round(1234, -2)      yields    1200
#pod
#pod Since this is a mathematical rather than string oriented function,
#pod there will be no trailing zeroes to the right of the decimal point,
#pod and the C<DECIMAL_POINT> and C<THOUSANDS_SEP> variables are ignored.
#pod To format your number using the C<DECIMAL_POINT> and C<THOUSANDS_SEP>
#pod variables, use C<format_number()> instead.
#pod
#pod =cut
...
#pod =method format_number
#pod
#pod   my $str = $formatter->format_number($number, $precision, $trailing_zeroes);
#pod
#pod Formats a number by adding C<THOUSANDS_SEP> between each set of 3
#pod digits to the left of the decimal point, substituting C<DECIMAL_POINT>
#pod for the decimal point, and rounding to the specified precision using
#pod C<round()>.  Note that C<$precision> is a I<maximum> precision
#pod specifier; trailing zeroes will only appear in the output if
#pod C<$trailing_zeroes> is provided, or the parameter C<DECIMAL_FILL> is
#pod set, with a value that is true (not zero, undef, or the empty string).
#pod If C<$precision> is omitted, the value of the C<DECIMAL_DIGITS>
#pod parameter (default value of 2) is used.
#pod
#pod If the value is too large or great to work with as a regular number,
#pod but instead must be shown in scientific notation, returns that number
#pod in scientific notation without further formatting.
#pod
#pod Examples:
#pod
#pod   format_number(12345.6789)             yields   '12,345.68'
#pod   format_number(123456.789, 2)          yields   '123,456.79'
#pod   format_number(1234567.89, 2)          yields   '1,234,567.89'
#pod   format_number(1234567.8, 2)           yields   '1,234,567.8'
#pod   format_number(1234567.8, 2, 1)        yields   '1,234,567.80'
#pod   format_number(1.23456789, 6)          yields   '1.234568'
#pod   format_number("0.000020000E+00", 7);' yields   '2e-05'
#pod
#pod Of course the output would have your values of C<THOUSANDS_SEP> and
#pod C<DECIMAL_POINT> instead of ',' and '.' respectively.
#pod
#pod =cut
...
#pod =method format_negative
#pod
#pod   my $str = $formatter->($number, $picture);
#pod
#pod Formats a negative number.  Picture should be a string that contains
#pod the letter C<x> where the number should be inserted.  For example, for
#pod standard negative numbers you might use ``C<-x>'', while for
#pod accounting purposes you might use ``C<(x)>''.  If the specified number
#pod begins with a ``-'' character, that will be removed before formatting,
#pod but formatting will occur whether or not the number is negative.
#pod
#pod =cut
...
#pod =method format_picture
#pod
#pod   my $str = $formatter->format_picture($number, $picture);
#pod
#pod Returns a string based on C<$picture> with the C<#> characters
#pod replaced by digits from C<$number>.  If the length of the integer part
#pod of $number is too large to fit, the C<#> characters are replaced with
#pod asterisks (C<*>) instead.  Examples:
#pod
#pod   format_picture(100.023, 'USD ##,###.##')   yields   'USD    100.02'
#pod   format_picture(1000.23, 'USD ##,###.##')   yields   'USD  1,000.23'
#pod   format_picture(10002.3, 'USD ##,###.##')   yields   'USD 10,002.30'
#pod   format_picture(100023,  'USD ##,###.##')   yields   'USD **,***.**'
#pod   format_picture(1.00023, 'USD #.###,###')   yields   'USD 1.002,300'
#pod
#pod The comma (,) and period (.) you see in the picture examples should
#pod match the values of C<THOUSANDS_SEP> and C<DECIMAL_POINT>,
#pod respectively, for proper operation.  However, the C<THOUSANDS_SEP>
#pod characters in C<$picture> need not occur every three digits; the
#pod I<only> use of that variable by this function is to remove leading
#pod commas (see the first example above).  There may not be more than one
#pod instance of C<DECIMAL_POINT> in C<$picture>.
#pod
#pod The value of C<NEG_FORMAT> is used to determine how negative numbers
#pod are displayed.  The result of this is that the output of this function
#pod my have unexpected spaces before and/or after the number.  This is
#pod necessary so that positive and negative numbers are formatted into a
#pod space the same size.  If you are only using positive numbers and want
#pod to avoid this problem, set NEG_FORMAT to "x".
#pod
#pod =cut
...
#pod =method format_price
#pod
#pod   my $str = $formatter->format_price($number, $precision, $symbol);
#pod
#pod Returns a string containing C<$number> formatted similarly to
#pod C<format_number()>, except that the decimal portion may have trailing
#pod zeroes added to make it be exactly C<$precision> characters long, and
#pod the currency string will be prefixed.
#pod
#pod The C<$symbol> attribute may be one of "INT_CURR_SYMBOL" or
#pod "CURRENCY_SYMBOL" (case insensitive) to use the value of that
#pod attribute of the object, or a string containing the symbol to be used.
#pod The default is "INT_CURR_SYMBOL" if this argument is undefined or not
...
#pod Examples:
#pod
#pod   format_price(12.95)   yields   'USD 12.95'
#pod   format_price(12)      yields   'USD 12.00'
#pod   format_price(12, 3)   yields   '12.000'
#pod
#pod The third example assumes that C<INT_CURR_SYMBOL> is the empty string.
#pod
#pod =cut
...
#pod =method format_bytes
#pod
#pod   my $str = $formatter->format_bytes($number, %options);
#pod
#pod   my $str = $formatter->format_bytes($number, $precision); # deprecated
#pod
#pod Returns a string containing C<$number> formatted similarly to
#pod C<format_number()>, except that large numbers may be abbreviated by
#pod adding a suffix to indicate 1024, 1,048,576, or 1,073,741,824 bytes.
#pod Suffix may be the traditional K, M, or G (default); or the IEC
...
#pod warning; early versions of Number::Format only allowed a numeric
#pod value.  A future release of Number::Format will change this warning to
#pod an error.  New code should use a hash instead to set options.  If it
#pod is a number this sets the value of the "precision" option.
#pod
...
#pod =item precision
#pod
#pod Set the precision for displaying numbers.  If not provided, a default
#pod of 2 will be used.  Examples:
#pod
#pod   format_bytes(12.95)                   yields   '12.95'
#pod   format_bytes(12.95, precision => 0)   yields   '13'
#pod   format_bytes(2048)                    yields   '2K'
#pod   format_bytes(2048, mode => "iec")     yields   '2KiB'
#pod   format_bytes(9999999)                 yields   '9.54M'
#pod   format_bytes(9999999, precision => 1) yields   '9.5M'
#pod
#pod =item unit
#pod
#pod Sets the default units used for the results.  The default is to
#pod determine this automatically in order to minimize the length of the
#pod string.  In other words, numbers greater than or equal to 1024 (or
#pod other number given by the 'base' option, q.v.) will be divided by 1024
#pod and C<$KILO_SUFFIX> or C<$KIBI_SUFFIX> added; if greater than or equal
#pod to 1048576 (1024*1024), it will be divided by 1048576 and
#pod C<$MEGA_SUFFIX> or C<$MEBI_SUFFIX> appended to the end; etc.
#pod
#pod However if a value is given for C<unit> it will use that value
#pod instead.  The first letter (case-insensitive) of the value given
#pod indicates the threshhold for conversion; acceptable values are T (for
#pod tera/tebi), G (for giga/gibi), M (for mega/mebi), K (for kilo/kibi), or
#pod A (for automatic, the default).  For example:
#pod
#pod   format_bytes(1048576, unit => 'K') yields     '1,024K'
#pod                                      instead of '1M'
#pod
#pod Note that the valid values to this option do not vary even when the
#pod suffix configuration variables have been changed.
#pod
#pod =item base
#pod
#pod Sets the number at which the C<$KILO_SUFFIX> is added.  Default is
#pod 1024.  Set to any value; the only other useful value is probably 1000,
#pod as hard disk manufacturers use that number to make their disks sound
#pod bigger than they really are.
#pod
#pod If the mode (see below) is set to "iec" or "iec60027" then setting the
#pod base option results in an error.
#pod
#pod =item mode
#pod
#pod Traditionally, bytes have been given in SI (metric) units such as
#pod "kilo" and "mega" even though they represent powers of 2 (1024, etc.)
#pod rather than powers of 10 (1000, etc.)  This "binary prefix" causes
#pod much confusion in consumer products where "GB" may mean either
#pod 1,048,576 or 1,000,000, for example.  The International
#pod Electrotechnical Commission has created standard IEC 60027 to
#pod introduce prefixes Ki, Mi, Gi, etc. ("kibibytes," "mebibytes,"
#pod "gibibytes," etc.) to remove this confusion.  Specify a mode option
#pod with either "traditional" or "iec60027" (or abbreviate as "trad" or
#pod "iec") to indicate which type of binary prefix you want format_bytes
#pod to use.  For backward compatibility, "traditional" is the default.
#pod See http://en.wikipedia.org/wiki/Binary_prefix for more information.
#pod
#pod =back
#pod
#pod =cut
...
sub format_bytes
{
    ...
    # kilo, mega, giga, and tera values.  Useful values of "base" are 1024 or
    # 1000, but any number can be used.  Larger numbers may cause overflows at
    # some threshholds, of course.
...
#pod =method unformat_number
#pod
#pod   my $str = $formatter->format_number($formatted);
#pod
#pod Converts a string as returned by C<format_number()>,
#pod C<format_price()>, or C<format_picture()>, and returns the
#pod corresponding value as a numeric scalar.  Returns C<undef> if the
#pod number does not contain any digits.  Examples:
#pod
#pod   unformat_number('USD 12.95')   yields   12.95
#pod   unformat_number('USD 12.00')   yields   12
#pod   unformat_number('foobar')      yields   undef
#pod   unformat_number('1234-567@.8') yields   1234567.8
#pod
#pod The value of C<DECIMAL_POINT> is used to determine where to separate
#pod the integer and decimal portions of the input.  All other non-digit
#pod characters, including but not limited to C<INT_CURR_SYMBOL> and
#pod C<THOUSANDS_SEP>, are removed.
#pod
#pod If the number matches the pattern of C<NEG_FORMAT> I<or> there is a C<->
#pod character before any of the digits, then a negative number is returned.
#pod
#pod If the number ends with the C<KILO_SUFFIX>, C<KIBI_SUFFIX>,
#pod C<MEGA_SUFFIX>, C<MEBI_SUFFIX>, C<GIGA_SUFFIX>, C<GIBI_SUFFIX>,
#pod C<TERA_SUFFIX>, or C<TEBI_SUFFIX> characters, then the number returned
#pod will be multiplied by the appropriate multiple of 1024 (or if the base
#pod option is given, by the multiple of that value) as appropriate.  Examples:
#pod
#pod   unformat_number("4K", base => 1024)   yields  4096
#pod   unformat_number("4K", base => 1000)   yields  4000
#pod   unformat_number("4KiB", base => 1024) yields  4096
#pod   unformat_number("4G")                 yields  4294967296
#pod
#pod =cut
...
#pod =head1 CAVEATS
#pod
#pod Some systems, notably OpenBSD, may have incomplete locale support.
#pod Using this module together with L<setlocale(3)> in OpenBSD may therefore
#pod not produce the intended results.
#pod
#pod This library takes some pain to cope with numeric precision, but it doesn't
#pod solve every problem.  Working with terabyte sizes on a 32-bit perl may have
#pod imprecise results.
#pod
#pod =cut
...
C<KILO_SUFFIX>, C<MEGA_SUFFIX>, C<GIGA_SUFFIX>, and C<TERA_SUFFIX> are
used by C<format_bytes()> when the value is over 1024, 1024**2, 1024**3,
or 1024**4, respectively.  The default values are "K", "M", "G", and
"T".  These apply in the default "traditional" mode only.

C<KIBI_SUFFIX>, C<MEBI_SUFFIX>, C<GIBI_SUFFIX>, and C<TEBI_SUFFIX> are
used by C<format_bytes()> when the value is over 1024, 1024**2,
1024**3, or 1024**4, respectively.  The default values are "KiB", "MiB",
"GiB", "TiB".  These apply in the "iso60027" mode only.
...
If the number matches the pattern of C<NEG_FORMAT> I<or> there is a C<->
...
William R. Ward <wrw@cpan.org>

=head1 CONTRIBUTORS

=for stopwords Karl Williamson Ricardo Signes
...
This software is copyright (c) 1997 by William R. Ward <wrw@cpan.org>.
