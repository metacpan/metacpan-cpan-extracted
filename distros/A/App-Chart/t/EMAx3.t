...
sub ref_ema_3_omitted {
  my ($f, $k) = @_;
  my $total;

  for (;;) {
    $k++;
    my $term = triangular($k+1) * $f**$k;
    $total += $term;
...

{
  ...
  foreach my $k (1 .. 50) {
    my $ref = ref_ema_3_omitted ($f, $k);
    my $got = App::Chart::Series::Derived::EMAx3::ema3_omitted ($f, $k);
    ok (nearly_eq ($ref, $got),
        "ema3_omitted $k  (ref=$ref got=$got)");
  }
}
...
{ my $N = 1;
  is (App::Chart::Series::Derived::EMAx3->warmup_count($N),
      0,
      "warmup_count($N)");
}

{ my $N = 2;
  my $got = App::Chart::Series::Derived::EMAx3->warmup_count($N);
  ok ($got > 0,
