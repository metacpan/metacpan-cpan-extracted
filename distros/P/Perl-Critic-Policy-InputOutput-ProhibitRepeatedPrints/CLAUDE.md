...
Perl-Critic-Policy-InputOutput-ProhibitRepeatedPrints is one Perl::Critic
policy.  It reports consecutive prints to the same filehandle, on the grounds
that a run of them is one message written as several statements -- so the shape
of what the reader will see is in none of them, and a line added in the middle
of the run changes a message nobody can see whole.

The rule lives in two of them.
`lib/Perl/Critic/Policy/InputOutput/ProhibitRepeatedPrints.pm` is the policy and
its POD is the specification; `t/main.t` is tables of snippets that must be
reported and snippets that must not, which is where a change to the rule is
argued and settled.  Change one without the other and you have changed nothing
anybody can rely on.
...
| | |
|---|---|
| `perldoc Perl::Critic::Policy::InputOutput::ProhibitRepeatedPrints` | what it reports, what it leaves alone, and what it cannot tell apart |
| `t/main.t` | the tables that decide it: reported, left alone, and under each configuration |
| `.perlcriticrc` | the policies this distribution is itself held to |
| `Changes` | what changed and when, per release |
| `dist.ini` | the build, and what has to be installed to run it |

Read the POD's CAVEATS before changing the rule.  Two consecutive prints are not
always one message, nothing in the source distinguishes the two cases, and the
answer where they differ is an annotation rather than a cleverer policy.
