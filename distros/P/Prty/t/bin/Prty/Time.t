...
sub test_unitTest : Test(59) {
    ...
    $ti = Prty::Time->new(2005,1,31,1,7,2);
    ...
    $ti = Prty::Time->new(2005,1,1,1,0,1);
    ...
    $ti = Prty::Time->new(parse=>'27.7.2005');
    $self->is($ti->dump,'2005-07-27-00-00-00','Prty::Time: deutsches Datum');

    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

    $ti = Prty::Time->new(parse=>'27.7.2005 17:23:56');
    $self->is($ti->dump,'2005-07-27-17-23-56','Prty::Time: deutsche Zeit');

    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

    $ti = Prty::Time->new(parse=>'7/27/2005');
    $self->is($ti->dump,'2005-07-27-00-00-00','Prty::Time: amerikanisches Datum');

    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

    $ti = Prty::Time->new(parse=>'7/27/2005 17:23:56');
    $self->is($ti->dump,'2005-07-27-17-23-56','Prty::Time: amerikanische Zeit');

    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

    $ti = Prty::Time->new(parse=>'2005-07-27');
    $self->is($ti->dump,'2005-07-27-00-00-00','Prty::Time: ISO Datum');

    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

    $ti = Prty::Time->new(parse=>'2005-07-27 17:23:56');
    $self->is($ti->dump,'2005-07-27-17-23-56','Prty::Time: ISO Zeit');

    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

    $ti = eval { Prty::Time->new(parse=>'1 4 2001') };
    $self->ok($@,'Prty::Time: Datum kann nicht geparsed werden');

    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

    $ti = Prty::Time->new(2005,1,31,12,27,9);
...
sub test_year : Test(1) {
    ...
    my $val = Prty::Time->new(2005,1,1)->year;
...
sub test_month : Test(1) {
    ...
    my $val = Prty::Time->new(2005,5,3)->month;
    $self->is($val,5,'month');
}
...
sub test_day : Test(1) {
    ...
    my $val = Prty::Time->new(2005,5,3)->day;
    $self->is($val,3,'day');
}
...
sub test_dayOfYear : Test(3) {
    ...
    my $val = Prty::Time->new(2005,1,1)->dayOfYear;
    $self->is($val,1);

    $val = Prty::Time->new(2005,12,31)->dayOfYear;
    $self->is($val,365);

    $val = Prty::Time->new(2004,12,31)->dayOfYear;
    $self->is($val,366);
}
...
sub test_weekOfYear : Test(16) {
    ...
    my ($year,$week) = Prty::Time->new(2017,1,1)->weekOfYear;
    $self->is($year,2016);
    $self->is($week,53);

    ($year,$week) = Prty::Time->new(2017,1,2)->weekOfYear;
    $self->is($year,2017);
    $self->is($week,1);

    ($year,$week) = Prty::Time->new(2017,1,11)->weekOfYear;
    $self->is($year,2017);
    $self->is($week,2);

    ($year,$week) = Prty::Time->new(2016,1,1)->weekOfYear;
    $self->is($year,2015);
    $self->is($week,53);

    ($year,$week) = Prty::Time->new(2016,1,4)->weekOfYear;
    $self->is($year,2016);
    $self->is($week,1);

    ($year,$week) = Prty::Time->new(2016,1,11)->weekOfYear;
    $self->is($year,2016);
    $self->is($week,2);

    ($year,$week) = Prty::Time->new(2015,1,1)->weekOfYear;
    $self->is($year,2015);
    $self->is($week,1);

    ($year,$week) = Prty::Time->new(2015,1,5)->weekOfYear;
    $self->is($year,2015);
    $self->is($week,2);
}
...
sub test_strftime : Test(1) {
    ...
    my $val = Prty::Time->new(utc=>1)->strftime('%F %H:%M:%S');
...
sub test_yyyymmddhhmmss : Test(2) {
    ...
    my $val = Prty::Time->new(utc=>0)->yyyymmddhhmmss;
    $self->is($val,'1970-01-01 00:00:00','yyyymmddhhmmss: ohne Trenner');

    $val = Prty::Time->new(utc=>0)->yyyymmddhhmmss('+');
...
sub test_yyyymmddxhhmmss : Test(1) {
    ...
    my $val = Prty::Time->new(utc=>1)->yyyymmddxhhmmss;
...
sub test_monthAbbrToNum : Test(3) {
    ...
    my $n = Prty::Time->monthAbbrToNum('May');
    $self->is($n,5);

    $n = eval{Prty::Time->monthAbbrToNum('May','de')};
    $self->ok($@);

    $n = Prty::Time->monthAbbrToNum('Mai','de');
    $self->is($n,5);
