...
=item * B<C<LiveStruct[ ... ]>>: Returns an C<Affix::Live> tied hash. Modifying keys in this hash updates C memory immediately.

=item * B<C<LiveArray[ $type, $count ]>>: Returns an C<Affix::Pointer> tied array. C<< $arr->[0] = 5 >> writes directly to memory.
