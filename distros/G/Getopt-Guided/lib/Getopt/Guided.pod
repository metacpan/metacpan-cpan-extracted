...
This module adheres to the L<Semantic
Versioning|http://semver.org/spec/v2.0.0.html> scheme.  All of its
public functions (C<getopts()>, C<processopts()>, C<readopts()>,
C<print_version_info()>) and constants (EOOD, EXIT_SUCCESS, EXIT_FAILURE,
EXIT_USAGE) are exported on request.
...
=head1 SYNOPSIS
  ...
  getopts( 'a:e=bcd,v+', my %opts, @argv );

  # Parse result
  # %opts == ( a => 'av2', c => !!1, d => [ qw( dv1 dv2 -- ) ], e => { ek1 => 'ev1', ek2 => 'ev2' }, v => 3 )
  # @argv == ( 'v1', 'v2' )
...
If an option name ("a") is followed by a colon (C<:>) character, a comma
(C<,>) character, or an equal sign (C<=>) character, the option should
have an option-argument which either shall be supplied as a separate
argument ("-a foo") or shall be part of the same argument string as the
option name ("-afoo").  We call an option whose option name is followed by a
colon/comma/equal sign "option-argument indicator" a common/list/map option. A
list option is sometimes called "multi-value option".  The option-argument
of a map option is a string of the form "key=value". The equal sign is
required. Both the key and the value can be empty.  The POSIX standard knows
only common options.
...
The hash value of a map option is a hash reference.
...
option-arguments are pushed onto a list. If a map option is repeated, its
option-arguments are parsed into key-value pairs and are added to a map.
...
that returns the end of options delimiter. If the callback function dies the
C<processopts()> function catches the error, warns and returns false.
...
The C<readopts( \@;@ )> function reads default command-line options from an
rcfile and prepends them to the command-line arguments list C<@argv>. This
list is the first parameter of the C<readopts()> function. The remaining
optional parameters of the C<readopts()> function, represent hard-coded
default command-line options that if present will be prepended even before
the default command-line options read from the rcfile. The C<readopts()>
