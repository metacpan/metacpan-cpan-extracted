...
The following functions are exportable:
    ...
    :ntheory
        ...
        is_prime is_coprime next_prime rising_factorial
        falling_factorial
...
Raises 2 to the power of C<x>. (i.e.: C<2**x>)

=head2 exp10

    exp10(x)                                      #=> Any

Raises 10 to the power of C<x>. (i.e.: C<10**x>)
...
=head2 rising_factorial
    ...
    rising_factorial(n, k)                        #=> Int | NaN

Rising factorial, C<n * (n + 1) * ... * (n + k - 1)>, defined as:

    binomial(n + k - 1, k) * k!

Returns NaN for negative values of C<k>.

=head2 falling_factorial

    falling_factorial(n, k)                       #=> Int | NaN
...
Returns the integer part of C<x>. Returns C<NaN> when C<x> cannot be truncated to an integer.
...
Converts C<x> to a rational number. Returns C<NaN> when this conversion is not possible.

When the given argument is a decimal expansion string, it will be specially parsed as a fraction.
...
it tries to convert it to a rational. Returns C<NaN> when this conversion is not possible.
