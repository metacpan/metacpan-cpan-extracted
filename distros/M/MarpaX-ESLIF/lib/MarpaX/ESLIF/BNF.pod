...
Hook that is disabling C<:discard> rule for the current recognizer. Equivalent to a call to C<MarpaX::ESLIF::recognizer_hook_discardb(1)>. Not propagated.

=item C<:discard[off]>

Hook that is enabling C<:discard> rule for the current recognizer. Equivalent to a call to C<MarpaX::ESLIF::recognizer_hook_discardb(0)>. Not propagated.

=back

Please note that the C<:discard[on]> and C<:discard[off]> events will I<never> happen if you use the grammar's C<parse()> method, the later is I<always> disabling any event. Therefore, if you grammar relies on recognizer's hooks, you will have to use the following workflow:
...
Please remark that if you grammar I<only> have internal events, then there is no need to call C<$eslifRecognizer->resume()> -;
