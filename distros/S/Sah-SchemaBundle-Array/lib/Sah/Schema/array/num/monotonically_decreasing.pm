...
1;
# ABSTRACT: An array of numbers with monotonically decreasing elements
...
Sah::Schema::array::num::monotonically_decreasing - An array of numbers with monotonically decreasing elements

=head1 VERSION

This document describes version 0.004 of Sah::Schema::array::num::monotonically_decreasing (from Perl distribution Sah-SchemaBundle-Array), released on 2024-02-13.
...
Used prefilters: L<Array::check_elems_numeric_monotonically_decreasing|Data::Sah::Filter::perl::Array::check_elems_numeric_monotonically_decreasing>
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("array::num::monotonically_decreasing*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("array::num::monotonically_decreasing", {return_type=>'str_errmsg'});
 ...
 $data = [1,"a"];
 my $errmsg = $validator->($data); # => "\@[1]: Not of type number"

Often a schema has coercion rule or default value rules, so after validation the
validated value will be different from the original. To return the validated
(set-as-default, coerced, prefiltered) value:

 my $validator = gen_validator("array::num::monotonically_decreasing", {return_type=>'str_errmsg+val'});
 my $res = $validator->($data); # [$errmsg, $validated_val]
 
 # a sample valid data
 $data = [3,2.9,1];
 my $res = $validator->($data); # => ["",[3,2.9,1]]
 
 # a sample invalid data
 $data = [1,"a"];
 my $res = $validator->($data); # => ["\@[1]: Not of type number",[1,"a"]]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("array::num::monotonically_decreasing*");
     $validator->(\@args);
     ...
 }
 ...
 $SPEC{myfunc} = {
     ...
     args => {
         arg1 => {
             ...
             schema => ['array::num::monotonically_decreasing*'],
         },
         ...
     },
 };
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"array::num::monotonically_decreasing*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('array::num::monotonically_decreasing*', name=>'ArrayNumMonotonicallyDecreasing')
     );
 }

 use My::Types qw(ArrayNumMonotonicallyDecreasing);
 ArrayNumMonotonicallyDecreasing->assert_valid($data);
