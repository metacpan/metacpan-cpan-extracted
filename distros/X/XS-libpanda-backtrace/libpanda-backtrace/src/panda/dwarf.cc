...
DwarfBackend::DwarfBackend(const Backtrace& raw_traces_) noexcept: raw_traces{raw_traces_} {
    SharedObjectMap so_map;
    gather_info(so_map);
    info_map = load_dwarf_info(so_map);
}

bool DwarfBackend::produce_frame(StackFrames& frames, size_t i) {
...
DieSP iterate(DieSP root_node, CheckFN&& fn) noexcept {
    using Queue = std::list<DieSP>;
    Dwarf_Error error;
    ...
    while (!queue.empty()) {
        ...
        Scan scan = fn(n);
        switch (scan) {
            case Scan::found: return n;
            case Scan::dead_end: break;
            case Scan::not_found: {
                Dwarf_Die child_die;

                int res = dwarf_siblingof_b(n->debug, n->die, true, &child_die, &error);
                if (res == DW_DLV_OK) {
                    DieSP child = new DieRC(child_die, n->debug, n->parent);
                    queue.emplace_back(child);
                }

                res = dwarf_child(n->die, &child_die, &error);
                if (res == DW_DLV_OK) {
                    DieSP child = new DieRC(child_die, n->debug, n);
                    queue.emplace_back(child);
                }
            }
        }
