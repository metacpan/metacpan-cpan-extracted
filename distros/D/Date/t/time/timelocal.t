...
subtest 'past within first transition' => sub {
...
subtest 'forcing earlier/later time doesn\'t matter when no ambiguity' => sub {
    is(timelocal(2, 1, 0, 3, 6, 1916, 0), -1688265017);
    is(timelocal(3, 1, 0, 3, 6, 1916, 1), -1688265016);
};

subtest 'transitions' => sub {
...
subtest 'normalize impossible time (should be 3:30:00)' => sub {
...
subtest64 'normalize' => sub {
    my ($isdst,$Y,$M,$D,$h,$m,$s) = (1,2070,-123,-1234,-12345,-123456,133456789);
    is(timelocal($s,$m,$h,$D,$M,$Y,$isdst), 2807084629);
    is(timelocaln($s,$m,$h,$D,$M,$Y,$isdst), 2807084629);
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2058,11,14,12,43,49]);
};
