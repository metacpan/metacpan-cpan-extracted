...
my $ok = eval { my $h = Data::Buffer::Shared::I64->new(scalar(tmpnam()), 64); 1 };
...
run_child('embedded NUL path',     sub { Data::Buffer::Shared::I64->new("/tmp/a\x00b.shm", 16) });

# --- zero capacity ---
run_child('cap=0',                 sub { Data::Buffer::Shared::I64->new(undef, 0) });

# --- astronomically huge capacity (mmap may succeed reserving address space) ---
run_child('huge capacity',         sub { Data::Buffer::Shared::I64->new(undef, 2 ** 50) });

# --- new_from_fd with bogus fd ---
SKIP: {
    skip 'no new_from_fd', 2 unless Data::Buffer::Shared::I64->can('new_from_fd');
    run_child('new_from_fd(-1)', sub { Data::Buffer::Shared::I64->new_from_fd(-1) });
    eval { Data::Buffer::Shared::I64->new_from_fd(-1) };
