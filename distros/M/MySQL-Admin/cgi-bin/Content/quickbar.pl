...
<option value="EditTable" '
  . ( param('action') eq 'EditTable' ? 'selected="selected"' : '' ) . '>' . translate('edit') . '</option>
<option value="ShowDumpTable" '
  . ( param('action') eq 'ShowDumpTable' ? 'selected="selected"' : '' ) . '>' . translate('Export') . '</option>
<option value="AnalyzeTable" '
  . ( param('action') eq 'AnalyzeTable' ? 'selected="selected"' : '' ) . '>' . translate('AnalyzeTable') . '</option>
<option value="OptimizeTable" '
  . ( param('action') eq 'OptimizeTable' ? 'selected="selected"' : '' ) . '>' . translate('OptimizeTable') . '</option>
<option value="RepairTable" '
  . ( param('action') eq 'RepairTable' ? 'selected="selected"' : '' ) . '>' . translate('RepairTable') . '</option>
