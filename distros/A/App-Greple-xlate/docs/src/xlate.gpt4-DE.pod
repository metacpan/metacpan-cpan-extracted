...
Konfliktmarker-Formatdaten können im Side-by-Side-Stil mit dem C<sdif>-Befehl und der Option C<-V> angezeigt werden. Da ein Vergleich auf Zeichenkettenbasis keinen Sinn ergibt, wird die Option C<--no-cdif> empfohlen. Wenn Sie den Text nicht einfärben möchten, geben Sie C<--no-textcolor> (oder C<--no-tc>) an.
