...
my $pocosi = POE::Component::Server::IRC->spawn(
    ...
    antiflood    => 0,
    plugin_debug => 1,
    config => { servername => 'listen.server.irc', sid => '1FU', anti_spam_exit_message_time => 0, pace_wait => 0 },
);
...
sub groucho_input {
  ...
  if ( $cmd eq 'LINKS' ) {
    pass($cmd);
    is( $prefix, '1FUAAAAAA', 'LINKS cmd from 1FUAAAAAA' );
    is( $params->[0], '4AK', 'Target is fake.server.irc' );
    is( $params->[1], '*', 'The mask is for everything' );
    my $sid = '4AK';
    my @replies = (
      [ '364', 'harpo.server.irc', 'listen.server.irc', '3 Open the door and come in!!!!!!' ],
      [ '364', 'listen.server.irc', 'groucho.server.irc', '2 Poco? POCO? POCO!' ],
      [ '364', 'groucho.server.irc', 'fake.server.irc', '1 Open the door and come in!!!!!!' ],
      [ '364', 'fake.server.irc', 'fake.server.irc', '0 This is a fake server' ],
      [ '365', $params->[1], 'End of /LINKS list.' ],
    );
    foreach my $reply ( @replies ) {
        my $numeric = shift @$reply;
        unshift @$reply, $prefix;
        $poe_kernel->post( $sender, 'send_to_server', { prefix => $sid, command => $numeric, params => $reply, colonify => 1 } );
    }
    return;
  }
  return;
