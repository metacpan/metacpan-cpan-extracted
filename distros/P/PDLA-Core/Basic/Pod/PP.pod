...
of PDLA arrays in Perl (e.g. C<< $x->slice('0:10:2,:') >> or other things such
...
where $x is of type C<PDLA_Float> and $y of type C<PDLA_Short>? With the signature
...
C<PDLA_Indx> which makes sense since C<b> will represent an index into
C<a>.

Note that 'curind' is declared as type C<PDLA_Indx> and not C<indx>.
...
to handle PDLA indexing operations.  For 32-bit installs, it can be
...
in the signature. Instead you write
  ...
  pp_def(
      ...
      RedoDimsCode => '
	PDLA_Indx im = $PDLA(p)->dims[0];
	PDLA_Indx in = $PDLA(x)->dims[0];
	PDLA_Indx min = in + im * im;
	PDLA_Indx inw = $PDLA(work)->dims[0];
...
rules. Here is an example from PDLA::Math
 ...
 pp_def("polyroots",
      ...
      RedoDimsCode => 'PDLA_Indx sn = $PDLA(cr)->dims[0]; $SIZE(m) = sn-1;',
...
=item PDLA->qsort_B( PDLA_Byte *xx, PDLA_Indx a, PDLA_Indx b )
...
=item PDLA->qsort_ind_B( PDLA_Byte *xx, PDLA_Indx *ix, PDLA_Indx a, PDLA_Indx b )
...
Contains the text returned by the C<badinfo> command (in C<perldla>) or
the C<-b> switch to the C<pdladoc> shell script. In many cases, you will
...
$CROAK()
    PDLA::PP::Rule::Substitute::dosubst_private()
...
$DOPRIVDIMS()
    Used in slices.pd, defined where?
    Code comes from PDLA::PP::CType::get_malloc, which is called by PDLA::PP::CType::get_copy, which is called by PDLA::PP::CopyOtherPars, PDLA::PP::NT2Copies__, and PDLA::PP::make_incsize_copy.  But none of those three at first glance seem to have anything to do with $DOPRIVDIMS
