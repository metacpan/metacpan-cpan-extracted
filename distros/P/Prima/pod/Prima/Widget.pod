...
To successfully exchange data with other applictions, one may investigate
...
one needs to register that format first using C<Clipbpard::register_format>,
once, and then it is ready for exachange.
...
On X11, it is the sender that contols that aspect, and tells the receiver what
...
If C<TEXT> is a reference to a string, it is translated as a markup string,
...
continue to handle C<MouseDown> event as if no drag sesssion was ever started.
...
=item DragQuery MOD, ANSWERS, COUNTERPART

Triggered on a sender DND widget when there was detected a change in mouse or
modifier buttons, or the user pressed C<Escape> key to cancel the DND session.
The combination of mouse and modifier buttons is stored in C<MOD> integer,
together with a special C<km::Escape> constant for the C<Escape> key.
...
on the C<MOD> value (f.ex. a C<dnd::Move> if a control modifier was pressed). 

Note: This action will only forward the change to the receiver on X11, but
it is advised to implement it anyway for portability.

C<COUNTERPART> is the Prima DND receiver widget, if within
the same program.
...
Triggered on a sender DND widget when there was detected a change in mouse or
modifier buttons, or the mouse was moved from one DND target to another.
The sender event is then presented with the new input, collected from
interaction with the new target; there, C<ALLOW> is set to a boolean
...
event, the handle should update the pointer in this callback. It is not
needed though to save and restore mouse pointers before and after the DND
session.
