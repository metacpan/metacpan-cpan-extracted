...
=item tie B<@array>, 'Mail::Box::Tie::ARRAY', FOLDER
...
Add C<@messages> to the end of the folder.
...
Sets all messages behind from C<$length> to the end of folder to be deleted.
...
=item Error: cannot simply replace messages in a folder: use delete old, then push new.
