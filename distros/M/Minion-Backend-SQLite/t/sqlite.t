...
subtest 'Migrate up and down' => sub {
  is $minion->backend->sqlite->migrations->active, 11, 'active version is 11';
  is $minion->backend->sqlite->migrations->migrate(0)->active, 0, 'active version is 0';
  is $minion->backend->sqlite->migrations->migrate->active, 11, 'active version is 11';
};
...
subtest 'Dispatch schedules' => sub {
  ...
  $minion->backend->sqlite->db->query(q{update minion_schedules set next_run = datetime('now', '-1 hours') where id = ?},
    $sid);
...
subtest 'Dispatch schedules with concurrent dispatchers' => sub {
  ...
  my $blocker = $minion->backend->sqlite->db;
  my $tx      = $blocker->begin('immediate');
