...
충돌 마커 형식 데이터는 `sdif` 명령과 `-V` 옵션을 사용하여 나란히 정렬된 스타일로 볼 수 있습니다. 문자열 단위로 비교하는 것은 의미가 없으므로 `--no-cdif` 옵션을 사용하는 것이 좋습니다. 텍스트에 색상을 지정할 필요가 없는 경우 `--no-color` 또는 `--cm 'TEXT*='`를 지정합니다.
