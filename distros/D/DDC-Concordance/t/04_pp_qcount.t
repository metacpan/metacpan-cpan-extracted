...
##-- 1..3: sample, sort
my ($q);
$q = DDC::PP->parse('count(foo) #sample[10] #by[textClass~s/::.*$//,date/10] #asc_key');
ok($q, "qcount:parse");
is($q->getSample, 10, "qcount:sample");
is($q->getSort, DDC::PP::LessByCountKey, "qcount:sort");
...
my $key = DDC::PP::CQCountKeyExprConstant->new('0');
$keys->PushKey($key);
SKIP: {
  skip('no refcnt hackery required for DDC::PP family', 1);
  ok($key && $key->refcnt==$keys->refcnt+1, "qcount:PushKey:refcnt");
}
undef $key; ##-- segfaults if refcnt failed

##-- 13..18: context-count
$q = DDC::PP->parse('count($l=foo =1) #by[$w =1] #desc_count');
