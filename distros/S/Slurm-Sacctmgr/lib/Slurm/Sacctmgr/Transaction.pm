...
package Slurm::Sacctmgr::Transaction;
use strict;
use warnings;
...
#Fields common to all versions of slurm
...
my @all_accessors = @common_accessors;
...
#Transactions cannot be specified with a single field
...
#Overloaded to match on action, actor, timestamp, info and where
#Sort of pointless; as must match everything.
{	my $obj = shift;
	croak "Must be called as an instance method at "
		unless $obj && ref($obj);

	#Do everythin, because I don't think anything else is necessarily unique
	my @fields = @all_accessors;
	my ($fld, $val, $meth);
	my $where = {};
	foreach $fld (@fields)
	{	$meth = $fld;
...
=head1 SYNOPSIS
  ...
  my $list = Slurm::Sacctmgr::Transaction->sacctmgr_list($sacctmgr);

  #List all transactions known to sacctmgr
  foreach $transaction (@$list)
  {	#transaction is a Slurm::Sacctmgr::Transaction object
  	$uname = $transaction->transaction;
	$defacct = $transaction->defaultaccount;
	$adminlevel = $transaction->adminlevel;
	print "$uname ($defacct) [$adminlevel]\n";
  }

  #Get a single transaction 
  $uname = 'payerle';
  $transaction = Slurm::Sacctmgr::Transaction->new_from_sacctmgr_by_name($sacctmgr,$uname);
  $defacct = $transaction->defaultaccount;
  print "Default account for transaction '$uname' is '$defacct'\n";
...
B<sacctmgr> command.
(Sorry, the above fields are NOT described in sacctmgr man page).
...
all have a very similar transaction interface (basically the differences are what
operations sacctmgr allows on different entity types, and how the results
of list/show operations get parsed).  

Because sacctmgr only allows the show/list command on transaction entities,
the only real method provided by the class is  B<sacctmgr_list($sacctmgr, %where)>.
This will return a list reference of all objects of entity type "transaction"
