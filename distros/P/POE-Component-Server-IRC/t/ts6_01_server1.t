...
sub ircd_listener_add {
    ...
    $heap->{ircd}->add_peer(
        ...
        pass  => '$2a$06$UhDqaEXfRUHlLxezakDEWeqtyeqWRIie.584yVLBawZmydqa11VvO',
        rpass => 'foo',
        type  => 'c',
        zip   => 1,
    );
...
sub testc_connected {
  ...
  pass($_[STATE]);
  $kernel->post( $sender, 'send_to_server', { command => 'PASS', params => [ 'foo', 'TS', '6', 'TFU' ], } );
...
sub testc_connected2 {
  my ($kernel,$heap,$sender) = @_[KERNEL,HEAP,SENDER];
  pass($_[STATE]);
  $kernel->post( $sender, 'send_to_server', { command => 'PASS', params => [ 'foo', 'TS', '6', '1FU' ], } );
  $kernel->post( $sender, 'send_to_server', { command => 'CAPAB', params => [ 'KNOCK UNDLN DLN TBURST GLN ENCAP UNKLN KLN CHW IE EX HOPS SVS CLUSTER EOB QS' ], colonify => 1 } );
  $kernel->post( $sender, 'send_to_server', { command => 'SERVER', params => [ 'connect.server.irc', '1', 'Open the door and come in!!!!!!' ], colonify => 1 } );
  $kernel->post( $sender, 'send_to_server', { command => 'SVINFO', params => [ '6', '6', '0', time() ], colonify => 1 } );
  $uidts = time() - 20;
...
sub testc_input {
  ...
  is($input->{params}[0], 'Closing Link: 127.0.0.1 (Bogus server ID introduced)', 'Closing Link: 127.0.0.1 (Bogus server ID introduced)');
  return;
}
...
sub testc_input2 {
  ...
  #diag($input->{raw_line}, "\n");
  is($input->{command}, 'ERROR', 'ERROR! ERROR! ERROR!');
  is($input->{params}[0], 'Closing Link: 127.0.0.1 (Server ID already exists)', 'Closing Link: 127.0.0.1 (Server ID already exists)');
  return;
}
...
sub testc_disconnected2 {
  my ($heap,$state) = @_[HEAP,STATE];
  pass($state);
  $poe_kernel->state('testc_connected','main','testc_connected3');
  $poe_kernel->state('testc_input','main','testc_input3');
  $poe_kernel->state('testc_disconnected','main','testc_disconnected3');
  $poe_kernel->post( $_[SENDER], 'connect' );
  return;
}

sub testc_disconnected3 {
  my ($heap,$state) = @_[HEAP,STATE];
  pass($state);
  $heap->{testc}->shutdown();
  $heap->{ircd}->yield('shutdown');
