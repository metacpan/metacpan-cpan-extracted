...
use Net::XMPP3::Roster;
use Net::XMPP3::PrivacyLists;
use strict;
use Carp;
...
#-----------------------------------------------------------------------------
$NEWOBJECT{'iq'}       = "Net::XMPP3::IQ";
$NEWOBJECT{'message'}  = "Net::XMPP3::Message";
$NEWOBJECT{'presence'} = "Net::XMPP3::Presence";
$NEWOBJECT{'jid'}      = "Net::XMPP3::JID";
##############################################################################
...
###############################################################################
#
# Send - Takes either XML or a Net::XMPP3::xxxx object and sends that
...
###############################################################################
#
# SendWithID - Take either XML or a Net::XMPP3::xxxx object and send it
...
###############################################################################
#
# SendAndReceiveWithID - Take either XML or a Net::XMPP3::xxxxx object and
...
###############################################################################
#
# GetID - Return the Net::XMPP3::xxxxx object that is stored in the LIST
...
###############################################################################
#
# GotID - Callback to store the Net::XMPP3::xxxxx object in the LIST at
...
{
    my $self = shift;
    &Net::XMPP3::Namespaces::add_ns(@_);
}


###############################################################################
#
# MessageSend - Takes the same hash that Net::XMPP3::Message->SetMessage
...
sub PresenceDBDelete
{
    ...
    my $indexJID = $jid;
    $indexJID = $jid->GetJID() if $jid->isa("Net::XMPP3::JID");
...
###############################################################################
#
# PresenceDBQuery - retrieve the last Net::XMPP3::Presence received with
...
sub AuthIQAuth
{
    ...
    #--------------------------------------------------------------------------
    # Create a Net::XMPP3::IQ object to send to the server
    #--------------------------------------------------------------------------
    my $iqLogin = $self->_iq();
...
sub AuthSASL
{
    ...
    my $xhost = defined $args{host}? $args{host} : $self->{SERVER}->{hostname};
    my $status =
        SASLClient($self->{STREAM},  $sid,
                                    $args{username},
                                    $args{password},
                                    $xhost
                                   );
...
##############################################################################
#
# PrivacyLists - Initialize a Net::XMPP3::PrivacyLists object and return it.
#
##############################################################################
sub PrivacyLists
{
    ...
    return new Net::XMPP3::PrivacyLists(connection=>$self);
}
...
##############################################################################
#
# Roster - Initialize a Net::XMPP3::Roster object and return it.
#
##############################################################################
sub Roster
{
    ...
    return new Net::XMPP3::Roster(connection=>$self);
}
...
sub SASLClient
{
    ...
    my $sasl = new Authen::SASL(mechanism=>join(" ",@{$mechanisms}),
                                callback=>{ authname => $username."@".$self->{SIDS}->{$sid}->{hostname},
                                            user => $username,
                                            pass => $password
                                          }
                               );

    $self->{SIDS}->{$sid}->{sasl}->{client} = $sasl->client_new('xmpp', $host);
