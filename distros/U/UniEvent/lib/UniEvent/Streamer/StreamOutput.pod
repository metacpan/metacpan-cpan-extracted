=head1 NAME

UniEvent::Streamer::StreamOutput - stream input for streamer

=head1 DESCRIPTION

Pulls in data from L<UniEvent::Streamer> stream and pushes it furthe into
the steam handle. The underlying stream can be L<UniEvent::Tcp>, L<UniEvent::Pipe>
or L<UniEvent::Tty>.

L<UniEvent::Streamer::StreamOutput> is derived from L<UniEvent::Streamer::IOutput>.

It is implemented in C++ for the speed, so it meant not to be
derived from; in other words it is use only class.

=head1 METHODS

=head2 new($stream_handle)

Constructs new output stream for from the L<UniEvent::Stream> handle.

=head1 REFERENCES

L<UniEvent::Stream>

L<UniEvent::Streamer>

L<UniEvent::Streamer::IOutput>

L<UniEvent::Streamer::StreamInput>

=cut
