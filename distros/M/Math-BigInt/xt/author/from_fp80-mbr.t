...
for my $entry (@$data) {
    ...
    $got = Math::BigRat -> from_fp80($bin);
    $test = qq|Math::BigRat->from_fp80("$bin")|;
    is($got -> bnstr(), $expected -> bnstr(), $test);

    $got = Math::BigRat -> from_fp80($hex);
    $test = qq|Math::BigRat->from_fp80("$hex")|;
    is($got -> bnstr(), $expected -> bnstr(), $test);

    $got = Math::BigRat -> from_fp80($bytes);
    $test = qq|Math::BigRat->from_fp80("$str")|;
    is($got -> bnstr(), $expected -> bnstr(), $test);
}
...

{
    ...
    my $x = Math::BigRat -> from_fp80($hex);
    is($x, 2, "class method");
}
