...
use Data::Resolver qw< resolver_from_dir >;

my $dir_path = __FILE__ =~ s{t\z}{d}rmxs;
my @exp_list = qw< ciao.txt foo/bar.txt >;

subtest list => sub {
   my $resolver = resolver_from_dir(root => $dir_path);
   ...
   {
      my ($list, $type) = $resolver->('foo', 'list');
      isa_ok $list, 'ARRAY';
      is_deeply sorted($list), ['foo/bar.txt'], 'directory contents'
         or diag(Dumper($list));
   }
};
...
subtest content => sub {
...

   {
      ...
      like $file, qr{(?mxs: ciao\.txt \z)}, 'filename is correct';
   }
...
subtest 'errors (loud setting, errors thrown as exceptions)' => sub {
   my $resolver = resolver_from_dir(root => $dir_path, throw => 1);
   my $stuff;
   ...
   is $@->{message}, 'Not a container', 'it has the right message';
   is $stuff, undef, 'nothing came out';
   ...
   isa_ok $@, 'HASH';
   is $@->{code}, 400, 'it has the right code';
   is $@->{message}, 'Not a file', 'it has the right message';
   is $stuff, undef, 'nothing came out';
