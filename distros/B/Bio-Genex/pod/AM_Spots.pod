...
Bio::Genex::AM_Spots - Methods for processing data from the GeneX DB
 table: AM_Spots
...
=head1 SYNOPSIS
  ...
  my $AM_Spots = Bio::Genex::AM_Spots->new(id=>47);

  # retrieve data from the DB for all columns
  $AM_Spots->fetch();

  # creating an instance, without pre-fetching all columns
  my $AM_Spots = new Bio::Genex::AM_Spots(id=>47);

  # creating an instance with pre-fetched data
  my $AM_Spots = new Bio::Genex::AM_Spots(id=>47, 'fetch_all'=>1);

  # retrieving multiple instances via primary keys
  my @objects = Bio::Genex::AM_Spots->get_objects(23,57,98)


  # retrieving all instances from a table
  my @objects = Bio::Genex::AM_Spots->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $AM_Spots->id();

  # or specifically
  my $ams_pk_val = $AM_Spots->ams_pk();

  # retreving other DB column attributes
  my $am_fk_val = $AM_Spots->am_fk();
  $AM_Spots->am_fk($value);

  my $usf_fk_val = $AM_Spots->usf_fk();
  $AM_Spots->usf_fk($value);

  my $spot_value_val = $AM_Spots->spot_value();
  $AM_Spots->spot_value($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::AM_Spots is
AM_Spots).
...
Bio::Genex::AM_Spots can access: I<raw> foreign key attributes,
...
Class Bio::Genex::AM_Spots defines the following utility variables for assisting
programmers to access the AM_Spots table.

=over 4

=item $Bio::Genex::AM_Spots::LIMIT
...
=item $Bio::Genex::AM_Spots::USE_CACHE
...
instance of the class via the Bio::Genex::AM_Spots->methodname() syntax.
...
linking table class. For Bio::Genex::AM_Spots it returns 0, since it is
...
table classes, and for Bio::Genex::AM_Spots it returns the value 'ams_pk';


=item table_name()

Returns the name of the DB table represented by this class. For
Bio::Genex::AM_Spots it returns 'AM_Spots';
...
C<insert_db()>.
  ...
  my AM_Spots = Bio::Genex::AM_Spots->new();
  AM_Spots->am_fk('some_value');
  AM_Spots->insert_db($dbh);
...
C<update_db()>.
  ...
  my $dbh = Bio::Genex::current_connection(USER=>$SU_USERNAME,
                                      PASSWORD=>$SU_PASSWORD);
  my AM_Spots = Bio::Genex::AM_Spots->new(id=>43);
  AM_Spots->am_fk('some_value');
  AM_Spots->update_db($dbh);

B<NOTE:> You must log into the DB with a user/password that has INSERT
priveleges in the DB, otherwise you will get a DBI error.

B<NOTE:> Any modification of the primary key value will be discarded
('ams_pk' for module Bio::Genex::AM_Spots).
...
This method is used to retrieve multiple instances of class Bio::Genex::AM_Spots
...
list as a primary key for the AM_Spots table and returns a single
...
instance of class Bio::Genex::AM_Spots.
...
(and for class Bio::Genex::AM_Spots it is synonomous with the
C<ams_pk()>method). The C<id()> method can be useful in writing
generic methods because it avoids having to know the name of the
primary key column. 

=item ams_pk()

This is the primary key attribute for Bio::Genex::AM_Spots. It has no setter method. 
...
Methods for the am_fk attribute.
...
=item $value = spot_value();

=item spot_value($value);

Methods for the spot_value attribute.
...
on Tue Jan 23 22:53:41 2001 by /home/jasons/work/GeneX-WWW-Installer/Genex/scripts/create_genex_class.pl --dir=/home/jasons/work/GeneX-WWW-Installer/Genex --target=AM_Spots --support=SpotLink
