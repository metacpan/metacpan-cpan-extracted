...
eval {
    my $filter = HTTPD::Log::Filter->new(
        exclusions_file     => '/tmp/exclusions_elf',
        request_re          => "GET.*?",
        format              => 'ELF',
    );
    open( LOG, 'sample/access_log_elf' ) or die $!;
    open( OUT, '>/tmp/inclusions_elf' ) or die $!;
    while ( <LOG> )
    {
    ...
    if ( -x $diff )
    {
        for ( qw( inclusions_elf exclusions_elf ) )
        {
            my @diff = `$diff sample/$_ /tmp/$_`;
            die "sample/$_ and /tmp/$_ differ:\n@diff" if @diff;
        }
    }
    unlink( '/tmp/exclusions_elf', '/tmp/inclusions_elf' );
};
print STDERR $@ if $@;
print $@ ? "not " : "", "ok 2\n";
