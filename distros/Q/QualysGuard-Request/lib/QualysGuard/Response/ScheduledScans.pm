package QualysGuard::Response::ScheduledScans;
...
sub new {
    ...
    if ( $self->exists('/SCHEDULEDSCANS/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/SCHEDULEDSCANS/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/SCHEDULEDSCANS/ERROR');
...
QualysGuard::Response::ScheduledScans
