...
An instance of
L<DBIx::DataModel::Meta::Schema|DBIx::DataModel::Meta::Schema>, 
corresponding to a given schema subclass
(a subclass of 
L<DBIx::DataModel::Schema|DBIx::DataModel::Schema>),
that holds information about the tables, associations, types, etc.
defined in that schema subclass.
...
In L</SQL>, placeholders are special marks positioned at specific places
in a SQL request, to denote that these places will be replaced by
actual values in a later I<binding> operation. SQL placeholders are usually
identified by a question mark C<'?'>; their order of appearance
must match the order in which values are bound.

At the C<DBIx::DataModel> level, there is an additional mechanism of
I<named parameters>, identified by a I<placeholder_prefix> defined
in the schema, which defaults to C<?:>. This gives additional
flexibility for binding values either I<before> or I<after> the statement
is composed. Examples are given in the
...
encapsulates a L</dbh> connexion to the database.
...
An acronym for L<Structured Query Language|https://en.wikipedia.org/wiki/SQL>,
a standard adopted by most database vendors to express requests to a
