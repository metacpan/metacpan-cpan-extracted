...
is( $blue->values('HSL', 'hash')->{'saturation'},                             100, 'blue has full saturation' );
is( $blue->set(  saturation => 50 )->values('HSL', 'hash')->{'saturation'},    50, 'could set it to half' );


warning_like {$red->add()}                       {carped => qr/need arguments as hash/}, "add method needs arguments";
warning_like {$red->add(r => 4, g => 2, t=> 3)}  {carped => qr/not match any known/},    "fantasy value keys detected";
warning_like {$red->add({r => 4, g => 2, t=> 3})}{carped => qr/not match any known/},    "fantasy value keys detected in hash syntax";
...
is( $black->blend( with => $white )->name,                 'gray50',   "blend black + white = gray");
