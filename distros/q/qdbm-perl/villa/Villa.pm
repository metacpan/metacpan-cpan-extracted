...
use constant OZCOMP => 1 << 5;           # compress leaves
...
use constant DDUP => 2;                  # allow duplication of records
...
##
# use Villa;
# Module `Villa' should be included in application codes.
...
# in the database are compressed.  Both of `Villa::OREADER' and `Villa::OWRITER' can be added to
# by bitwise or: `Villa::ONOLCK', which means it opens a database file without file locking.  If
# it is undef, `Villa::OREADER' is specified.
...
# blocks until the lock is achieved.  `Villa::OZCOMP' is available only if QDBM was built with
# ZLIB enabled.  If `Villa::ONOLCK' is used, the application is responsible for exclusion control.
#
sub new {
    my($class) = shift;
    my($name) = shift;
...
# existing value is kept, `Villa::DDUP', which means duplication of keys is allowed.  If it is
# undef, `Villa::DOVER' is specified.
# If successful, the return value is true, else, it is false.
...
# Method: Move the cursor to position around a record.
# `$key' specifies a key.  If it is undef, this method has no effect.
