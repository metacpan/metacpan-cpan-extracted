...
ok(
  do {
    eval { $schedule = PDK::Firewall::Element::Schedule::Srx->new(fwId => 1, schName => 'a', startDate => 'b', endDate => 'c'); };
    warn $@ if !!$@;
    $schedule->isa('PDK::Firewall::Element::Schedule::Srx');
  },
  ' 生成 PDK::Firewall::Element::Schedule::Srx 对象'
);
...
ok(
  do {
    eval {
      $schedule = PDK::Firewall::Element::Schedule::Srx->new(
        fwId      => 1,
        schName   => 'a',
