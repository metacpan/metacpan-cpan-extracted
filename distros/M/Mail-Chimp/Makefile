# This Makefile is for the Mail::Chimp extension to perl.
#
...
#     ABSTRACT => q[Perl wrapper around the Mailchimp v1.2 API]
#     AUTHOR => q[Dave Pirotte (dpirotte@gmail.com)]
#     NAME => q[Mail::Chimp]
#     PREREQ_PM => { XMLRPC::Lite=>q[0.710.06], MooseX::Types::DateTimeX=>q[0], Test::Simple=>q[0.44], Moose=>q[0] }
#     VERSION_FROM => q[lib/Mail/Chimp.pm]
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Mail/Chimp.pm \
	lib/Mail/Chimp/API.pm
...
TO_INST_PM = lib/Mail/Chimp.pm \
	lib/Mail/Chimp/API.pm \
	lib/Mail/Chimp/AbuseReport.pm \
	lib/Mail/Chimp/List.pm

PM_TO_BLIB = lib/Mail/Chimp.pm \
	blib/lib/Mail/Chimp.pm \
	lib/Mail/Chimp/List.pm \
	blib/lib/Mail/Chimp/List.pm \
	lib/Mail/Chimp/AbuseReport.pm \
	blib/lib/Mail/Chimp/AbuseReport.pm \
	lib/Mail/Chimp/API.pm \
	blib/lib/Mail/Chimp/API.pm
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Mail/Chimp.pm $(INST_MAN3DIR)/Mail::Chimp.$(MAN3EXT) \
	  lib/Mail/Chimp/API.pm $(INST_MAN3DIR)/Mail::Chimp::API.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               Mail-Chimp' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.50' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           Perl wrapper around the Mailchimp v1.2 API' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Dave Pirotte (dpirotte@gmail.com)' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    MooseX::Types::DateTimeX:  0' >> META_new.yml
	$(NOECHO) $(ECHO) '    Test::Simple:         0.44' >> META_new.yml
	$(NOECHO) $(ECHO) '    XMLRPC::Lite:         0.710.06' >> META_new.yml
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,50,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl wrapper around the Mailchimp v1.2 API</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Dave Pirotte (dpirotte@gmail.com)</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Moose" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="MooseX-Types-DateTimeX" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Test-Simple" VERSION="0,44,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="XMLRPC-Lite" VERSION="0,710,06,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/Mail/Chimp.pm blib/lib/Mail/Chimp.pm \
	  lib/Mail/Chimp/List.pm blib/lib/Mail/Chimp/List.pm \
	  lib/Mail/Chimp/AbuseReport.pm blib/lib/Mail/Chimp/AbuseReport.pm \
	  lib/Mail/Chimp/API.pm blib/lib/Mail/Chimp/API.pm 
