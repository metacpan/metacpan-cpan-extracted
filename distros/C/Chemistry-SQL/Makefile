# This Makefile is for the Chemistry::SQL extension to perl.
#
...
#     NAME => q[Chemistry::SQL]
#     PREREQ_PM => { Chemistry::File::SMILES=>q[0.42], Chemistry::Mol=>q[0.26] }
#     VERSION_FROM => q[lib/Chemistry/SQL.pm]
...
FULLEXT = Chemistry/SQL
BASEEXT = SQL
PARENT_NAME = Chemistry
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Chemistry/SQL.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DIRFILESEP)Config.pm $(PERL_INC)$(DIRFILESEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Chemistry
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Chemistry
...
TO_INST_PM = lib/Chemistry/SQL.pm

PM_TO_BLIB = lib/Chemistry/SQL.pm \
	blib/lib/Chemistry/SQL.pm
...
DISTNAME = Chemistry-SQL
DISTVNAME = Chemistry-SQL-0.01
...
manifypods : pure_all  \
	lib/Chemistry/SQL.pm \
	lib/Chemistry/SQL.pm
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW)\
	  lib/Chemistry/SQL.pm $(INST_MAN3DIR)/Chemistry::SQL.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  $(MAKEFILE_OLD) $(FIRST_MAKEFILE) blib/lib/Chemistry/SQL.pm
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         Chemistry-SQL' >> META.yml
	$(NOECHO) $(ECHO) 'version:      0.01' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: lib/Chemistry/SQL.pm' >> META.yml
	$(NOECHO) $(ECHO) 'installdirs:  site' >> META.yml
	$(NOECHO) $(ECHO) 'requires:' >> META.yml
	$(NOECHO) $(ECHO) '    Chemistry::File::SMILES:       0.42' >> META.yml
	$(NOECHO) $(ECHO) '    Chemistry::Mol:                0.26' >> META.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Chemistry-File-SMILES" VERSION="0,42,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Chemistry-Mol" VERSION="0,26,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  lib/Chemistry/SQL.pm blib/lib/Chemistry/SQL.pm 
	$(NOECHO) $(TOUCH) $@
