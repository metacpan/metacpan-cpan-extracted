...
change.  A C<$name> may be specified as first argument, but also
...
See isMale(): return C<true> if we are sure the user is a woman.

=item $obj-E<gt>B<isMale>()

Returns C<true> if we are sure that the user is male.  This is specified as
gender at instantiation, or derived from the courtesy value.  Methods
isMale and isFemale are not complementatory: they can both return C<false>
