#!/usr/bin/env perl
##----------------------------------------------------------------------------
## WebAuthn - scripts/webauthn-using-class-functions.psgi
##----------------------------------------------------------------------------
BEGIN
{
    use strict;
...
#   plackup -l http://localhost:5000 scripts/webauthn-using-class-functions.psgi
#
...
my $app = sub
{
    ...
    if( $path eq '/webauthn/register/begin' && $method eq 'POST' )
    {
        ...
        my $opts = generate_registration_options(
            ...
            attestation => 'none',
        ) || return( json_res( 500, { error => '' . Web::Authn->error } ) );
        $session->{webauthn_challenge} = $opts->{challenge};
        return( json_res( 200, options_to_json( $opts ) ) );
    }
    ...
    if( $path eq '/webauthn/register/complete' && $method eq 'POST' )
    {
        ...
        unless( $reg )
        {
            return( json_res( 400, { error => 'registration failed', detail => '' . Web::Authn->error } ) );
        }
        dbh()->do(
    ...
    if( $path eq '/webauthn/login/complete' && $method eq 'POST' )
    {
        ...
        unless( $ok )
        {
            return( json_res( 401, { error => 'authentication failed', detail => '' . Web::Authn->error } ) );
        }
        dbh()->do(
