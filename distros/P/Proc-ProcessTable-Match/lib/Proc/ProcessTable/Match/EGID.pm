...
Proc::ProcessTable::Match::EGID - Check if the EGID of a process matches.
...
=head1 SYNOPSIS
    ...
    use Proc::ProcessTable::Match::EGID;
...
that is 'egids', which is a array of EGIDs to match.

The EGID values can be prefixed with the equalities below for doing
...
sub match{
	...
	my $egid_int=0;
	while (defined( $self->{egids}[$egid_int] )){
