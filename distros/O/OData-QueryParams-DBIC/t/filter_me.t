...
my %tests = (
    ...
    "filter=Price le 3.5 or Price gt 200"                             => { -or  => [ { 'me.Price' => { '<=' => 3.5 } }, { 'me.Price' => { '>' => 200 } } ] },
    "filter=Price le 200 and Price gt 3.5"                            => { -and => [ { 'me.Price' => { '<=' => 200 } }, { 'me.Price' => { '>' => 3.5 } } ] },
    "filter=Price le 100"                                             => { 'me.Price' => { '<=' => 100 } },
    "filter=Price lt 20"                                              => { 'me.Price' => { '<' => 20 } },
    "filter=Price ge 10"                                              => { 'me.Price' => { '>=' => 10 } },
    "filter=Price gt 20"                                              => { 'me.Price' => { '>' => 20 } },
    "filter=Address/City ne 'London'"                                 => { 'Address.City' => { '!=' => 'London' } },
    "filter=Address/City eq 'Redmond'"                                => { 'Address.City' => { '=' => 'Redmond' } },
    "filter=((name eq 'Serena') and (age lt 5))"                      => { -and => [ { 'me.name' => { '=' => 'Serena' } }, { 'me.age' => { '<' => 5 } } ] },
    "filter=user_id gt 20"                                            => { 'me.user_id' => { '>' => 20 } },
    "filter="                                                         => {},
);
...
for my $query_string ( sort keys %tests ) {
    ...
    if ( !ref $expected && $expected eq 'FAIL' ) {
        dies_ok {
            params_to_dbic( $query_string, me => 1 );
        };
        next QUERYSTRING;
    }

    my ($where,$opts) = params_to_dbic( $query_string, me => 1 );
