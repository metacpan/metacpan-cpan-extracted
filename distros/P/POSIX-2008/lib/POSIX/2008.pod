...
POSIX::2008 provides many of the interfaces specified by
L<POSIX.1-2008|https://pubs.opengroup.org/onlinepubs/9699919799/> I<and later>
that the core L<POSIX> module withholds, implements in Perl or fucked up.

It also ships a number of nonstandard interfaces and constants present in
Linux, BSD and Solaris that are extensions of POSIX.
...
Since version 0.22, returned handles support L<IO::Handle> methods.

=head1 SYSTEM CALL RETURN VALUES

A system call return value of -1 meaning "error" is mapped to undef except for
L<C<poll()>|"poll"> and L<C<ppoll()>|"ppoll"> which return -1 to be consistent
with L<C<select()>|perlfunc/select>.
...
unchanged. These are L<C<creat()>|"creat">, L<C<dup3()>|"dup3">,
L<C<open()>|"open">, L<C<read()>|"read">, L<C<write()>|"write">,
L<C<readv()>|"readv">, L<C<writev()>|"writev">, L<C<pread()>|"pread">,
L<C<pwrite()>|"pwrite">, L<C<preadv()>|"preadv">, L<C<pwritev()>|"pwritev">,
L<C<preadv2()>|"preadv2">, L<C<pwritev2()>|"pwritev2">,
L<C<getpriority()>|"getpriority">,
...
=item C<accept4>
 ...
 conn = accept4(sock, flags);

C<accept4()> is similar to L<C<accept>|perlfunc/accept> except for the
following differences.

I<sock> is a numeric file descriptor or a Perl file handle referring to a
listening socket.

I<flags> is the bitwise OR of zero or more of C<SOCK_CLOEXEC>,
C<SOCK_CLOFORK>, C<SOCK_NONBLOCK>. Your system might support a different set
of flags.

C<accept4()> returns a numeric file descriptor (which may be 0) or a Perl file
handle for the accepted socket. The type of the return value is the same as
that of the I<sock> argument.

On error, undef is returned.

C<accept4()> was standardized with POSIX.1-2024.

I<New in version 0.29.>
...
=item C<dlopen>
 ...
 dlhandle = dlopen(filename, flags);
...
C<dup3()> is similar to L<C<POSIX::dup2()>|POSIX/dup2> except for the
following differences.

I<oldfd> and I<newfd> may be numeric file descriptors or Perl file handles. If
I<oldfd> equals I<newfd>, then C<dup3()> fails with the error C<EINVAL>.

The I<flags> parameter forces the file descriptor flags of I<newfd> to be the
bitwise OR of zero or more of C<O_CLOEXEC>, C<O_CLOFORK>. Your system might
support a different set of flags.

C<dup3()> returns the new numeric file descriptor (which may be 0).

On error, undef is returned.

C<dup3()> was standardized with POSIX.1-2024.
...
In list context, it returns the above list on success or the empty list on
error. In scalar context, it returns a (non-undef) boolean value indicating
success or failure.
...
C<getdate_err()> returns the value of the getdate_err variable.
...
Returns a string of I<length> bytes of "unpredictable high-quality random
data, generated by a cryptographically secure pseudo-random number generator",
or undef on error.

The maximum permitted value for I<length> is C<GETENTROPY_MAX> (usually 256).
If more bytes are requested, C<$!> is set to C<EINVAL> as decreed by POSIX,
even on systems that believe C<EIO> is right.

If your system does not provide C<getentropy()>, it is emulated via
C<getrandom()>, C<arc4random_buf()>, C<BCryptGenRandom()> (Windows), C<RDRAND>
(Intel/AMD), C<RNDR> (ARM64), or C<DARN> (Power9) if available (picked in this
order at build time).

C<getentropy()> was standardized with POSIX.1-2024.

I<New in version 0.27.>

I<Changed in version 0.29:> Added DARN as a fallback on Power9.
...
C<getresgid()> was standardized with POSIX.1-2024.
...
C<getresuid()> was standardized with POSIX.1-2024.
...
From the countless C<l*round*()> interfaces it chooses the one matching your
Perl's NV type, preferring the "long long" return value variant.

If the rounded value is outside Perl's internal signed integer range, it is
returned as a string.

I<Changed in version 0.27:> No longer returns undef on error.
...
I<flags> is a bitwise OR of zero or more C<open()> flags. For the exact set of
supported flags and their restrictions see the manpage of your system.

C<mkostemp()> was standardized with POSIX.1-2024.
...
=item C<pipe2>
 ...
 (read_fd, write_fd) = pipe2(flags);

 (read_fh, write_fh) = pipe2(\flags);

C<pipe2()> is similar to L<C<POSIX::pipe()>|POSIX/pipe> except for the
following differences.

I<flags> is the bitwise OR of zero or more of C<O_CLOEXEC>, C<O_CLOFORK>,
C<O_NONBLOCK>. Your system might support a different set of flags.

If I<flags> is an ordinary scalar, then two numeric file descriptors are
returned. If I<flags> is a reference to a scalar, then two Perl file handles
are returned.

If I<flags> is 0 then C<pipe2()> is the same as C<pipe()>.

On error, the empty list is returned.

C<pipe2()> was standardized with POSIX.1-2024.
...
If I<fd> is negative or anything L<C<sv_2io()>|perlapi/sv_2io> doesn't consider
a file handle, then the corresponding events field is ignored and the revents
field returns zero.

Returns the number of elements in I<pollfds> whose I<revents> have been set to
a non-zero value, zero if the call timed out. On error, C<poll()> returns -1
and sets C<$!> just like L<C<select()>|perlfunc/select>.
...
C<ppoll()> is similar to L<C<poll()>|"poll"> with the following differences:

I<timeout> can be an integer or floating point value in seconds or an array
reference of the form I<[seconds, nanoseconds]>. An omitted or undef
I<timeout> value means an infinite timeout.

I<sigmask> must be a L<C<POSIX::SigSet>|POSIX/POSIX::SigSet>. If I<sigmask> is
omitted or undef, no signal mask is applied and C<ppoll()> behaves like
L<C<poll()>|"poll"> with a more precise timeout.

C<ppoll()> was standardized with POSIX.1-2024.
...
I<flags> is the bitwise OR of zero or more of C<O_RDWR>, C<O_NOCTTY>,
C<O_CLOEXEC>, C<O_CLOFORK>.

C<O_CLOEXEC>, C<O_CLOFORK> were standardized with POSIX.1-2024.
...
C<preadv2()> is similar to L<C<preadv()>|"preadv"> but adds an optional
I<flags> argument, which is a bitwise OR of zero or more of the C<RWF_*> flags
(see the manpage for details). I<flags> is set to 0 if omitted or undef.

Unlike C<preadv()>, if the I<offset> argument is -1, then the current file
...
Unlike C<pwritev()>, if the I<offset> argument is -1, then the current file
offset is used and updated.
...
I<Changed in version 0.27:> No longer returns the empty list on error.
...
From the countless C<scalb*n*()> interfaces it chooses the one matching your
Perl's NV type, preferring the long int exp variant.
...
C<setresgid()> was standardized with POSIX.1-2024.
...
C<setresuid()> was standardized with POSIX.1-2024.
...
up to 11 separate arguments or with a single array reference after the format.
...
In list context it returns the broken-down time or the empty list on error.

Fields not affected by the format string may be returned as initialized, set to
default values, or recomputed depending on your OS (e.g. glibc "recomputes"
I<wday> and I<yday> if any of the year, month, or day elements changed).

In scalar context it returns the index of the first byte in I<s> that was not
...
I<gmtoff> and I<zone> were standardized with POSIX.1-2024. They might not be
supported by your system, and I<zone> might be ignored entirely.

I<New in version 0.02.>

I<Changed in version 0.24:> Accept an array reference as the third argument
(L<RT#66519|https://rt.cpan.org/Public/Bug/Display.html?id=66519>).

I<Changed in version 0.27:> Support gmtoff and zone.
...
Each buffer can be simple scalar (usually a string) or an array reference of
the form C<[scalar, offset, length]> to write I<length> bytes of I<scalar>
from position I<offset>. A negative I<offset> counts from the end of
I<scalar>. If omitted, I<offset> defaults to 0, I<length> defaults to the
string length of I<scalar> starting at I<offset>. An I<offset> beyond either
end of I<scalar> causes the call to croak. I<length> is capped so that it
doesn't exceed the end of I<scalar>.
...
I<Changed in version 0.27:> Buffers may be array references.
...
I<New in version 0.24:> :socket_h export tag.

I<New in version 0.27:> :errno_h export tag.
...
C<BOOT_TIME> C<NEW_TIME> C<OLD_TIME> C<RUN_LVL> C<DEAD_PROCESS>
C<INIT_PROCESS> C<LOGIN_PROCESS> C<USER_PROCESS> C<ACCOUNTING> C<EMPTY>
C<UT_UNKNOWN>
...
C<CLOCK_MONOTONIC_RAW> C<CLOCK_PROCESS_CPUTIME_ID> C<CLOCK_PROF>
C<CLOCK_REALTIME> C<CLOCK_REALTIME_ALARM> C<CLOCK_REALTIME_COARSE>
C<CLOCK_REALTIME_FAST> C<CLOCK_REALTIME_PRECISE> C<CLOCK_SECOND>
C<CLOCK_SOFTTIME> C<CLOCK_TAI> C<CLOCK_THREAD_CPUTIME_ID> C<CLOCK_UPTIME>
C<CLOCK_UPTIME_FAST> C<CLOCK_UPTIME_PRECISE> C<CLOCK_VIRTUAL>

C<E2BIG> C<EACCES> C<EADDRINUSE> C<EADDRNOTAVAIL> C<EAFNOSUPPORT> C<EAGAIN>
C<EALREADY> C<EBADF> C<EBADMSG> C<EBUSY> C<ECANCELED> C<ECHILD>
C<ECONNABORTED> C<ECONNREFUSED> C<ECONNRESET> C<EDEADLK> C<EDESTADDRREQ>
C<EDOM> C<EDQUOT> C<EEXIST> C<EFAULT> C<EFBIG> C<EHOSTUNREACH> C<EIDRM>
C<EILSEQ> C<EINPROGRESS> C<EINTR> C<EINVAL> C<EIO> C<EISCONN> C<EISDIR>
C<ELOOP> C<EMFILE> C<EMLINK> C<EMSGSIZE> C<EMULTIHOP> C<ENAMETOOLONG>
C<ENETDOWN> C<ENETRESET> C<ENETUNREACH> C<ENFILE> C<ENOBUFS> C<ENODEV>
C<ENOENT> C<ENOEXEC> C<ENOLCK> C<ENOLINK> C<ENOMEM> C<ENOMSG> C<ENOPROTOOPT>
C<ENOSPC> C<ENOSYS> C<ENOTCONN> C<ENOTDIR> C<ENOTEMPTY> C<ENOTRECOVERABLE>
C<ENOTSOCK> C<ENOTSUP> C<ENOTTY> C<ENXIO> C<EOPNOTSUPP> C<EOVERFLOW>
C<EOWNERDEAD> C<EPERM> C<EPIPE> C<EPROTO> C<EPROTONOSUPPORT> C<EPROTOTYPE>
C<ERANGE> C<EROFS> C<ESOCKTNOSUPPORT> C<ESPIPE> C<ESRCH> C<ESTALE>
C<ETIMEDOUT> C<ETXTBSY> C<EWOULDBLOCK> C<EXDEV>

C<F_DUPFD> C<F_DUPFD_CLOEXEC> C<F_DUPFD_CLOFORK> C<F_GETFD> C<F_SETFD>
C<F_GETFL> C<F_SETFL> C<F_GETLK> C<F_SETLK> C<F_SETLKW> C<F_OFD_GETLK>
C<F_OFD_SETLK> C<F_OFD_SETLKW> C<F_GETOWN> C<F_GETOWN_EX> C<F_SETOWN>
C<F_SETOWN_EX> C<F_RDLCK> C<F_UNLCK> C<F_WRLCK> C<FD_CLOEXEC> C<FD_CLOFORK>
C<F_OWNER_PID> C<F_OWNER_PGRP>

C<FE_TONEAREST> C<FE_TOWARDZERO> C<FE_UPWARD> C<FE_DOWNWARD> C<FE_DIVBYZERO>
C<FE_INEXACT> C<FE_INVALID> C<FE_OVERFLOW> C<FE_UNDERFLOW> C<FE_ALL_EXCEPT>

C<FNM_CASEFOLD> C<FNM_FILE_NAME> C<FNM_IGNORECASE> C<FNM_LEADING_DIR>
C<FNM_NOESCAPE> C<FNM_NOMATCH> C<FNM_PATHNAME> C<FNM_PERIOD>

C<FLT_EVAL_METHOD> C<FLT_RADIX> C<FP_INFINITE> C<FP_NAN> C<FP_NORMAL>
C<FP_SUBNORMAL> C<FP_ZERO> C<FP_FAST_FMA> C<FP_FAST_FMAF> C<FP_FAST_FMAL>
C<FP_ILOGB0> C<FP_ILOGBNAN> C<MATH_ERRNO> C<MATH_ERREXCEPT>

C<GETENTROPY_MAX> C<NSIG_MAX> C<NZERO>

C<HUGE_VAL> C<INFINITY>

C<ITIMER_PROF> C<ITIMER_REAL> C<ITIMER_VIRTUAL>

C<MSG_CMSG_CLOEXEC> C<MSG_CMSG_CLOFORK> C<MSG_CTRUNC> C<MSG_DONTROUTE>
C<MSG_EOR> C<MSG_NOSIGNAL> C<MSG_OOB> C<MSG_PEEK> C<MSG_TRUNC> C<MSG_WAITALL>

C<O_ACCMODE> C<O_APPEND> C<O_ASYNC> C<O_CLOEXEC> C<O_CLOFORK> C<O_CREAT>
C<O_DIRECT> C<O_DIRECTORY> C<O_DSYNC> C<O_EMPTY_PATH> C<O_EXCL> C<O_EXEC>
C<O_EXLOCK> C<O_LARGEFILE> C<O_NDELAY> C<O_NOATIME> C<O_NOCTTY> C<O_NOFOLLOW>
C<O_NONBLOCK> C<O_NOSIGPIPE> C<O_NOSTDFD> C<O_PATH> C<O_RDONLY> C<O_RDWR>
C<O_REGULAR> C<O_RESOLVE_BENEATH> C<O_RSYNC> C<O_SEARCH> C<O_SHLOCK> C<O_SYNC>
...
C<RESOLVE_NO_XDEV> C<RESOLVE_CACHED>

C<RTLD_DEFAULT> C<RTLD_GLOBAL> C<RTLD_LAZY> C<RTLD_LOCAL> C<RTLD_NEXT>
C<RTLD_NOW>

C<RTLD_DEEPBIND> C<RTLD_MEMBER> C<RTLD_NOAUTODEFER> C<RTLD_NODELETE>
C<RTLD_NOLOAD>

C<RWF_APPEND> C<RWF_ATOMIC> C<RWF_DONTCACHE> C<RWF_DSYNC> C<RWF_HIPRI>
C<RWF_NOAPPEND> C<RWF_NOSIGNAL> C<RWF_NOWAIT> C<RWF_SUPPORTED> C<RWF_SYNC>
...
C<SEEK_SET> C<SEEK_CUR> C<SEEK_END> C<SEEK_DATA> C<SEEK_HOLE>

C<SHUT_RD> C<SHUT_RDWR> C<SHUT_WR>

C<SOCK_DGRAM> C<SOCK_RAW> C<SOCK_SEQPACKET> C<SOCK_STREAM> C<SOCK_NDELAY>
C<SOCK_NONBLOCK> C<SOCK_CLOEXEC> C<SOCK_CLOFORK>

C<SOL_SOCKET> C<SOMAXCONN>

C<SO_ACCEPTCONN> C<SO_BROADCAST> C<SO_DEBUG> C<SO_DOMAIN> C<SO_DONTROUTE>
C<SO_ERROR> C<SO_KEEPALIVE> C<SO_LINGER> C<SO_OOBINLINE> C<SO_PROTOCOL>
C<SO_RCVBUF> C<SO_RCVLOWAT> C<SO_RCVTIMEO> C<SO_REUSEADDR> C<SO_SNDBUF>
C<SO_SNDLOWAT> C<SO_SNDTIMEO> C<SO_TYPE>

C<TIMER_ABSTIME> C<UTIME_NOW> C<UTIME_OMIT>

C<F_OK> C<R_OK> C<W_OK> C<X_OK>

C<POLLERR> C<POLLHUP> C<POLLIN> C<POLLNVAL> C<POLLOUT> C<POLLPRI> C<POLLRDBAND>
C<POLLRDNORM> C<POLLWRBAND> C<POLLWRNORM>
...
C<FILESIZEBITS> C<LINK_MAX> C<MAX_CANON> C<MAX_INPUT> C<NAME_MAX> C<PATH_MAX>
C<PIPE_BUF> C<POSIX2_SYMLINKS> C<POSIX_ALLOC_SIZE_MIN>
C<POSIX_REC_INCR_XFER_SIZE> C<POSIX_REC_MAX_XFER_SIZE>
C<POSIX_REC_MIN_XFER_SIZE> C<POSIX_REC_XFER_ALIGN> C<SYMLINK_MAX>
C<TEXTDOMAIN_MAX>

C<AIO_LISTIO_MAX> C<AIO_MAX> C<AIO_PRIO_DELTA_MAX> C<ARG_MAX> C<ATEXIT_MAX>
C<BC_BASE_MAX> C<BC_DIM_MAX> C<BC_SCALE_MAX> C<BC_STRING_MAX> C<CHILD_MAX>
C<COLL_WEIGHTS_MAX> C<DELAYTIMER_MAX> C<EXPR_NEST_MAX> C<HOST_NAME_MAX>
C<IOV_MAX> C<LINE_MAX> C<LOGIN_NAME_MAX> C<MQ_OPEN_MAX> C<MQ_PRIO_MAX>
C<NGROUPS_MAX> C<OPEN_MAX> C<PAGESIZE> C<PAGE_SIZE>
C<PTHREAD_DESTRUCTOR_ITERATIONS> C<PTHREAD_KEYS_MAX> C<PTHREAD_THREADS_MAX>
C<RE_DUP_MAX> C<RTSIG_MAX> C<SEM_NSEMS_MAX> C<SEM_VALUE_MAX> C<SIGQUEUE_MAX>
C<STREAM_MAX> C<SYMLOOP_MAX> C<TIMER_MAX> C<TTY_NAME_MAX> C<TZNAME_MAX>

C<CHAR_BIT> C<CHAR_MAX> C<UCHAR_MAX> C<SCHAR_MAX> C<CHAR_MIN> C<SCHAR_MIN>
C<INT_MAX> C<INT_MIN> C<LLONG_MAX> C<LLONG_MIN> C<LONG_BIT> C<LONG_MAX>
C<LONG_MIN> C<MB_LEN_MAX> C<SHRT_MAX> C<SHRT_MIN> C<SSIZE_MAX> C<UINT_MAX>
C<ULLONG_MAX> C<ULONG_MAX> C<USHRT_MAX> C<WORD_BIT>

C<_POSIX_ASYNC_IO> C<_POSIX_CHOWN_RESTRICTED> C<_POSIX_CPUTIME>
C<_POSIX_FALLOC> C<_POSIX_NO_TRUNC> C<_POSIX_PRIO_IO> C<_POSIX_SYNC_IO>
C<_POSIX_THREAD_CPUTIME> C<_POSIX_TIMESTAMP_RESOLUTION> C<_POSIX_VDISABLE>

C<_POSIX_ADVISORY_INFO> C<_POSIX_ASYNCHRONOUS_IO> C<_POSIX_BARRIERS>
C<_POSIX_CLOCK_SELECTION> C<_POSIX_DEVICE_CONTROL> C<_POSIX_FSYNC>
C<_POSIX_IPV6> C<_POSIX_JOB_CONTROL> C<_POSIX_MAPPED_FILES> C<_POSIX_MEMLOCK>
C<_POSIX_MEMLOCK_RANGE> C<_POSIX_MEMORY_PROTECTION> C<_POSIX_MESSAGE_PASSING>
C<_POSIX_MONOTONIC_CLOCK> C<_POSIX_PRIORITIZED_IO>
C<_POSIX_PRIORITY_SCHEDULING> C<_POSIX_RAW_SOCKETS>
C<_POSIX_READER_WRITER_LOCKS> C<_POSIX_REALTIME_SIGNALS> C<_POSIX_REGEXP>
C<_POSIX_SAVED_IDS> C<_POSIX_SEMAPHORES> C<_POSIX_SHARED_MEMORY_OBJECTS>
C<_POSIX_SHELL> C<_POSIX_SPAWN> C<_POSIX_SPIN_LOCKS> C<_POSIX_SPORADIC_SERVER>
C<_POSIX_SS_REPL_MAX> C<_POSIX_SYNCHRONIZED_IO> C<_POSIX_THREADS>
C<_POSIX_THREAD_ATTR_STACKADDR> C<_POSIX_THREAD_ATTR_STACKSIZE>
C<_POSIX_THREAD_PRIORITY_SCHEDULING> C<_POSIX_THREAD_PRIO_INHERIT>
C<_POSIX_THREAD_PRIO_PROTECT> C<_POSIX_THREAD_PROCESS_SHARED>
C<_POSIX_THREAD_ROBUST_PRIO_INHERIT> C<_POSIX_THREAD_ROBUST_PRIO_PROTECT>
C<_POSIX_THREAD_SAFE_FUNCTIONS> C<_POSIX_THREAD_SPORADIC_SERVER>
C<_POSIX_TIMEOUTS> C<_POSIX_TIMERS> C<_POSIX_TRACE>
C<_POSIX_TRACE_EVENT_FILTER> C<_POSIX_TRACE_EVENT_NAME_MAX>
C<_POSIX_TRACE_INHERIT> C<_POSIX_TRACE_LOG> C<_POSIX_TRACE_NAME_MAX>
C<_POSIX_TRACE_SYS_MAX> C<_POSIX_TRACE_USER_EVENT_MAX>
C<_POSIX_TYPED_MEMORY_OBJECTS> C<_POSIX_V6_ILP32_OFF32>
C<_POSIX_V6_ILP32_OFFBIG> C<_POSIX_V6_LP64_OFF64> C<_POSIX_V6_LPBIG_OFFBIG>
C<_POSIX_V7_ILP32_OFF32> C<_POSIX_V7_ILP32_OFFBIG> C<_POSIX_V7_LP64_OFF64>
C<_POSIX_V7_LPBIG_OFFBIG> C<_POSIX_V8_ILP32_OFF32> C<_POSIX_V8_ILP32_OFFBIG>
C<_POSIX_V8_LP64_OFF64> C<_POSIX_V8_LPBIG_OFFBIG> C<_POSIX_VERSION>
...
C<_CS_POSIX_V8_ILP32_OFF32_CFLAGS> C<_CS_POSIX_V8_ILP32_OFF32_LDFLAGS>
C<_CS_POSIX_V8_ILP32_OFF32_LIBS> C<_CS_POSIX_V8_ILP32_OFFBIG_CFLAGS>
C<_CS_POSIX_V8_ILP32_OFFBIG_LDFLAGS> C<_CS_POSIX_V8_ILP32_OFFBIG_LIBS>
C<_CS_POSIX_V8_LP64_OFF64_CFLAGS> C<_CS_POSIX_V8_LP64_OFF64_LDFLAGS>
C<_CS_POSIX_V8_LP64_OFF64_LIBS> C<_CS_POSIX_V8_LPBIG_OFFBIG_CFLAGS>
C<_CS_POSIX_V8_LPBIG_OFFBIG_LDFLAGS> C<_CS_POSIX_V8_LPBIG_OFFBIG_LIBS>
C<_CS_POSIX_V8_THREADS_CFLAGS> C<_CS_POSIX_V8_THREADS_LDFLAGS>
C<_CS_POSIX_V8_WIDTH_RESTRICTED_ENVS> C<_CS_V8_ENV>

C<_CS_POSIX_V7_ILP32_OFF32_CFLAGS> C<_CS_POSIX_V7_ILP32_OFF32_LDFLAGS>
C<_CS_POSIX_V7_ILP32_OFF32_LIBS> C<_CS_POSIX_V7_ILP32_OFF32_LINTFLAGS>
C<_CS_POSIX_V7_ILP32_OFFBIG_CFLAGS> C<_CS_POSIX_V7_ILP32_OFFBIG_LDFLAGS>
C<_CS_POSIX_V7_ILP32_OFFBIG_LIBS> C<_CS_POSIX_V7_ILP32_OFFBIG_LINTFLAGS>
C<_CS_POSIX_V7_LP64_OFF64_CFLAGS> C<_CS_POSIX_V7_LP64_OFF64_LDFLAGS>
C<_CS_POSIX_V7_LP64_OFF64_LIBS> C<_CS_POSIX_V7_LP64_OFF64_LINTFLAGS>
C<_CS_POSIX_V7_LPBIG_OFFBIG_CFLAGS> C<_CS_POSIX_V7_LPBIG_OFFBIG_LDFLAGS>
C<_CS_POSIX_V7_LPBIG_OFFBIG_LIBS> C<_CS_POSIX_V7_LPBIG_OFFBIG_LINTFLAGS>
C<_CS_POSIX_V7_WIDTH_RESTRICTED_ENVS> C<_CS_V7_ENV>
C<_CS_V7_WIDTH_RESTRICTED_ENVS>
...
C<_CS_POSIX_V6_WIDTH_RESTRICTED_ENVS> C<_CS_V6_ENV>
C<_CS_V6_WIDTH_RESTRICTED_ENVS>

C<_CS_POSIX_V5_WIDTH_RESTRICTED_ENVS> C<_CS_V5_WIDTH_RESTRICTED_ENVS>

C<_CS_XBS5_ILP32_OFF32_CFLAGS> C<_CS_XBS5_ILP32_OFF32_LDFLAGS>
C<_CS_XBS5_ILP32_OFF32_LIBS> C<_CS_XBS5_ILP32_OFF32_LINTFLAGS>
C<_CS_XBS5_ILP32_OFFBIG_CFLAGS> C<_CS_XBS5_ILP32_OFFBIG_LDFLAGS>
C<_CS_XBS5_ILP32_OFFBIG_LIBS> C<_CS_XBS5_ILP32_OFFBIG_LINTFLAGS>
C<_CS_XBS5_LP64_OFF64_CFLAGS> C<_CS_XBS5_LP64_OFF64_LDFLAGS>
C<_CS_XBS5_LP64_OFF64_LIBS> C<_CS_XBS5_LP64_OFF64_LINTFLAGS>
C<_CS_XBS5_LPBIG_OFFBIG_CFLAGS> C<_CS_XBS5_LPBIG_OFFBIG_LDFLAGS>
C<_CS_XBS5_LPBIG_OFFBIG_LIBS> C<_CS_XBS5_LPBIG_OFFBIG_LINTFLAGS>

C<_PC_2_SYMLINKS> C<_PC_ALLOC_SIZE_MIN> C<_PC_ASYNC_IO>
C<_PC_CHOWN_RESTRICTED> C<_PC_FALLOC> C<_PC_FILESIZEBITS> C<_PC_LINK_MAX>
C<_PC_MAX_CANON> C<_PC_MAX_INPUT> C<_PC_NAME_MAX> C<_PC_NO_TRUNC>
C<_PC_PATH_MAX> C<_PC_PIPE_BUF> C<_PC_PRIO_IO> C<_PC_REC_INCR_XFER_SIZE>
C<_PC_REC_MAX_XFER_SIZE> C<_PC_REC_MIN_XFER_SIZE> C<_PC_REC_XFER_ALIGN>
C<_PC_SOCK_MAXBUF> C<_PC_SYMLINK_MAX> C<_PC_SYNC_IO> C<_PC_TEXTDOMAIN_MAX>
C<_PC_TIMESTAMP_RESOLUTION> C<_PC_VDISABLE>
...
C<_SC_DELAYTIMER_MAX> C<_SC_DEVICE_CONTROL> C<_SC_DEVICE_IO>
C<_SC_DEVICE_SPECIFIC> C<_SC_DEVICE_SPECIFIC_R> C<_SC_EQUIV_CLASS_MAX>
C<_SC_EXPR_NEST_MAX> C<_SC_FD_MGMT> C<_SC_FIFO> C<_SC_FILE_ATTRIBUTES>
C<_SC_FILE_LOCKING> C<_SC_FILE_SYSTEM> C<_SC_FSYNC> C<_SC_GETGR_R_SIZE_MAX>
...
C<_SC_NPROCESSORS_ONLN> C<_SC_NSIG> C<_SC_NZERO> C<_SC_OPEN_MAX>
C<_SC_PAGESIZE> C<_SC_PAGE_SIZE> C<_SC_PASS_MAX> C<_SC_PHYS_PAGES> C<_SC_PII>
...
Warnings about C<embedded null character in path> can (but should not) be
suppressed with C<no warnings 'syscalls';> or, before Perl 5.20.0,
C<no warnings 'misc';>.
