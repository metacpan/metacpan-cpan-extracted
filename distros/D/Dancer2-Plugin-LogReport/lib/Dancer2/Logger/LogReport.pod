...
L<Log::Report|Log::Report> in order to use the additional levels; if doing so directly within
a Dancer2 application (not a sub-module), then you will either need to load
L<Log::Report|Log::Report> with C<syntax, 'LONG'> or use L<Dancer2::Plugin::LogReport|Dancer2::Plugin::LogReport> to
...
another module using L<Log::Report|Log::Report>, the plugin, or Dancer itself). In this case,
...
This module is part of Dancer2-Plugin-LogReport version 2.02,
built on November 10, 2025. Website: F<http://perl.overmeer.net/CPAN/>
