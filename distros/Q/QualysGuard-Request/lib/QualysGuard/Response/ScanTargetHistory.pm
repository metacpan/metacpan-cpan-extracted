package QualysGuard::Response::ScanTargetHistory;
...
sub new {
    ...
    if ( $self->exists('/SCAN_TARGET_HISTORY_OUTPUT/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/SCAN_TARGET_HISTORY_OUTPUT/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/SCAN_TARGET_HISTORY_OUTPUT/ERROR');
...
QualysGuard::Response::ScanTargetHistory
