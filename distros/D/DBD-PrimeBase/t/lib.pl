...
#
my $file;
if (-f ($file = "t/$dbdriver.env")  ||
    -f ($file = "$dbdriver.env")    ||
    -f ($file = "../tests/$dbdriver.env")  ||
    -f ($file = "tests/$dbdriver.env")) {
    eval { require $file; };
    if ($@) {
...
$test_dsn      = $ENV{'DBI_DSN'}   ||  'DBI:PrimeBase:PrimeServer;localhost;demo';
$test_user     = $ENV{'DBI_USER'}  ||  'Administrator';
...
#
{
	...
	if (UNIVERSAL::isa($dbh, "PrimeBase")) {
	    $dbh = $dbh->{'dbh'};
	}
