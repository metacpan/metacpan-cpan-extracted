...
typedef struct {
	SV *meta;           /* the Enum::Declare::Meta object this set type is bound to */
	const char *set_name;
	SV *set_instance;   /* the singleton Set used as the type constraint */
} EnumSetTypeData;

static bool enumset_type_check(pTHX_ SV *val, void *data_ptr) {
	EnumSetTypeData *estd = (EnumSetTypeData *)data_ptr;
	SV *result_sv;
	bool result;
	dSP;
	if (sv_isobject(val))
...
static SV *enumset_type_coerce(pTHX_ SV *val, void *data_ptr) {
	PERL_UNUSED_ARG(data_ptr);
	return val;
}

XS_INTERNAL(xs_enumset_constructor) {
	dXSARGS;
	SV *meta_sv = (SV*)XSANY.any_ptr;
	SV *set;
	AV *vals_av;
	I32 i;
...
static SV * lookup_enum_meta(pTHX_ const char *pkg, const char *enum_name) {
	HV *registry = get_hv("Enum::Declare::_registry", 0);
	SV *key;
	HE *he;
	if (!registry) return NULL;
	key = newSVpvf("%s::%s", pkg, enum_name);
	he = hv_fetch_ent(registry, key, 0, 0);
	SvREFCNT_dec(key);
	if (!he) return NULL;
	return HeVAL(he);
}

static AV *lex_read_set_members(pTHX) {
	AV *av = newAV();
	I32 c;
	lex_read_space(0);
	c = lex_peek_unichar(0);
...
static SV *split_enum_from_attrs(pTHX_ SV **attr_sv_ptr) {
	SV *attr_sv = *attr_sv_ptr;
	SV *enum_name_sv;
	const char *attr_pv;
	STRLEN attr_len;
	char *last_comma;
	...
	if (attr_sv == &PL_sv_undef)
		croak("Expected ':EnumName' binding in enumSet declaration");

	attr_pv = SvPV(attr_sv, attr_len);
	last_comma = strrchr(attr_pv, ',');

	if (last_comma) {
		enum_name_sv = newSVpv(last_comma + 1, 0);
		SvCUR_set(attr_sv, last_comma - attr_pv);
		*SvEND(attr_sv) = '\0';
	} else {
		enum_name_sv = newSVsv(attr_sv);
		SvREFCNT_dec(attr_sv);
		*attr_sv_ptr = &PL_sv_undef;
	}
	return enum_name_sv;
}

static SV *build_frozen_set(pTHX_ AV *member_names, SV *meta, SV *set_name_sv, SV *enum_name_sv) {
	AV *vals_av = newAV();
	SV *set_obj;
	I32 i;
	dSP;
	for (i = 0; i <= av_len(member_names); i++) {
		SV **name_p = av_fetch(member_names, i, 0);
		if (name_p) {
			SV *val, *val_copy;
			const char *vname = SvPV_nolen(*name_p);
			ENTER; SAVETMPS;
			PUSHMARK(SP);
			XPUSHs(meta);
			...
			if (!SvOK(val)) {
				FREETMPS; LEAVE;
				croak("enumSet %s: '%s' is not a variant of %s",
					SvPV_nolen(set_name_sv), vname, SvPV_nolen(enum_name_sv));
			}
			val_copy = newSVsv(val);
			FREETMPS; LEAVE;
			av_push(vals_av, val_copy);
		}
	}
	ENTER; SAVETMPS;
	PUSHMARK(SP);
	...
	call_method("new", G_SCALAR);
	SPAGAIN;
	set_obj = SvREFCNT_inc(POPs);
	PUTBACK;
	FREETMPS; LEAVE;
	return set_obj;
}

static SV *build_mutable_set(pTHX_ SV *meta, SV *set_name_sv) {
	SV *set_obj;
	dSP;
	ENTER; SAVETMPS;
	PUSHMARK(SP);
...
static void register_enumset_type(pTHX_ const char *pkg, SV *set_name_sv, SV *meta) {
	EnumSetTypeData *estd;
	SV *fqn, *singleton;
	GV *gv;
	dSP;
	Newxz(estd, 1, EnumSetTypeData);
	estd->meta = SvREFCNT_inc(meta);
	estd->set_name = savepv(SvPV_nolen(set_name_sv));
	fqn = newSVpvf("%s::%s", pkg, SvPV_nolen(set_name_sv));
	gv = gv_fetchpvn_flags(SvPV_nolen(fqn), SvCUR(fqn), 0, SVt_PV);
	SvREFCNT_dec(fqn);
	if (gv && GvCV(gv)) {
		ENTER; SAVETMPS;
		PUSHMARK(SP);
		...
		SPAGAIN;
		singleton = SvREFCNT_inc(POPs);
		PUTBACK;
		FREETMPS; LEAVE;
	} else {
		croak("enumSet %s: failed to retrieve singleton", SvPV_nolen(set_name_sv));
	}
	estd->set_instance = singleton;
	object_register_type_xs_ex(aTHX_ SvPV_nolen(set_name_sv), enumset_type_check, enumset_type_coerce, (void*)estd);
}

static OP *enumset_parser_callback(pTHX_ GV *namegv, SV *psobj, U32 *flagsp) {
	SV *set_name_sv;
	SV *attr_sv;
	SV *enum_name_sv;
	SV *meta;
	const char *pkg;
	STRLEN pkg_len;
	int is_export, is_type;
	PERL_UNUSED_ARG(namegv);
	PERL_UNUSED_ARG(psobj);
	lex_read_space(0);
	set_name_sv = lex_read_ident(aTHX);
	if (!set_name_sv) croak("Expected enumSet name");
	lex_read_space(0);
	attr_sv = lex_read_attrs(aTHX);
	enum_name_sv = split_enum_from_attrs(aTHX_ &attr_sv);
	pkg     = HvNAME(PL_curstash);
	pkg_len = strlen(pkg);
	is_export = has_attr(aTHX_ attr_sv, "Export");
	is_type   = has_attr(aTHX_ attr_sv, "Type");
	meta = lookup_enum_meta(aTHX_ pkg, SvPV_nolen(enum_name_sv));
	if (!meta)
		croak("enumSet %s: unknown enum '%s' in package %s",
			SvPV_nolen(set_name_sv), SvPV_nolen(enum_name_sv), pkg);
	lex_read_space(0);
	if (lex_peek_unichar(0) == '{') {
		AV *member_names = lex_read_set_members(aTHX);
		SV *set_obj = build_frozen_set(aTHX_ member_names, meta, set_name_sv, enum_name_sv);
		newCONSTSUB(gv_stashpvn(pkg, pkg_len, GV_ADD),
			SvPV_nolen(set_name_sv), set_obj);
		SvREFCNT_dec((SV*)member_names);
	} else {
		SV *set_obj = build_mutable_set(aTHX_ meta, set_name_sv);
		newCONSTSUB(gv_stashpvn(pkg, pkg_len, GV_ADD),
			SvPV_nolen(set_name_sv), set_obj);
	}
	if (is_export) {
		AV *name_list = newAV();
		av_push(name_list, newSVsv(set_name_sv));
		setup_exports(aTHX_ pkg, pkg_len, name_list, NULL);
		SvREFCNT_dec((SV*)name_list);
	}
	if (is_type)
		register_enumset_type(aTHX_ pkg, set_name_sv, meta);
	if (attr_sv != &PL_sv_undef) SvREFCNT_dec(attr_sv);
	SvREFCNT_dec(set_name_sv);
	SvREFCNT_dec(enum_name_sv);
...
void import(...)
	CODE: 
	{
		...
		SvREFCNT_dec(fqn);
		fqn = newSVpvf("%s::enumSet", caller);
		cv = newXS(SvPV_nolen(fqn), xs_enum_stub, __FILE__);
		cv_set_call_parser(cv, enumset_parser_callback, &PL_sv_undef);
		SvREFCNT_dec(fqn);
	}
