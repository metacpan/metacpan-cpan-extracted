...
I<Note: All code examples use modern Perl with subroutine signatures and B<>``B<> for clarity. This is a documentation preference and B<>notB<> a requirement of the PAGI specification.>
...
Perl does not natively support Python's C<async/await> pattern, but we can achieve similar functionality using B<Futures> (from C<Future.pm>) and B<IO::Async> (from C<IO::Async::Loop>). Below is how we translate key ASGI components into Perl.
...
Applications are written as asynchronous subroutines using C<Future> and C<IO::Async>. They receive a C<scope> describing the connection, and two coderefs, C<$recv> and C<$send>, which return Futures representing event input and output.
...
C<<< spec_version => '0.1' >>> (optional; protocol-specific override if it diverges)


=item -

C<features> (optional): a hashref of server-reported capabilities such as:

=over

=item -

C<supports_streaming> => 1 if streaming responses are supported for this scope


=item -

C<max_request_body_size> => maximum accepted request body size (in bytes)


=item -

C<max_concurrent_streams> => maximum streams per HTTP/2/HTTP/3 connection


=item -

C<supports_trailers> => 1 if trailer frames are accepted for the response
...
- C<spec_version>: Indicates the version for the specific protocol (such as HTTP or WebSocket). Servers may omit it, in which case clients should assume it matches C<version>.
- C<features>: Server-defined key/value pairs that describe optional behaviors for the connection. Values should use the permitted data types from this specification.
...
C<http> â standard HTTP events


=item -

C<websocket> â WebSocket events


=item -

C<lifespan> â process lifecycle events


=item -

C<pagi> â reserved for future PAGI-defined system events


=item -

C<ext.> â reserved for experimental or nonstandard events
...
B<Note:> Booleans are intentionally omitted from the core spec to avoid ambiguity in Perl, which lacks a native boolean type. In contexts where ASGI would use C<true>/C<false>, PAGI protocol specifications will define expected values explicitly â usually C<0> or C<1>, or by the presence/absence of a defined value.
...
C<$recv>: Coderef returning a C<Future> resolving to the next event
...
Applications B<must> await all C<$send> calls before their Future completes:
 ...
 # WRONG: sends happen after app Future resolves
 async sub app ($scope, $recv, $send) {
     $loop->delay_future(after => 1)->then(sub {
         $send->({ type => 'http.response.body', body => 'Hello' });
     })->retain;
     return;  # App Future resolves immediately, but send hasn't happened!
 }
...
Event types for C<$recv> and C<$send>
...
Extra fields in events B<must not> cause errors â this supports forward-compatible upgrades.

If C<$send> is called after connection closure, it should be a no-op unless specified otherwise.
