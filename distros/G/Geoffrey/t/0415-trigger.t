...
throws_ok { $object->add(); } 'Geoffrey::Exception::RequiredValue::TriggerName',
  'Throw missing trigger name on add';

throws_ok { $object->alter(); }
'Geoffrey::Exception::NotSupportedException::ConverterType',
  'Alter trigger not supportet thrown';

throws_ok { $object->drop(); }
'Geoffrey::Exception::RequiredValue::TriggerName',
  'Throw missing trigger name on drop';
...
'Geoffrey::Exception::RequiredValue::TableName',
  'Required event_object_table value thrown';
...
is(
    $object->dryrun(1)->drop( 'trigger_name', 'test_table' ),
    'DROP TRIGGER IF EXISTS test_table',
    'Drop trigger test'
);
