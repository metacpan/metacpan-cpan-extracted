...
subtest '1. read_all with range (closed interval, min only, max only, keys_only)' => sub {
    ...
    my @p_ids = $adb->read_all( 'products', { filter => { 4 => { min => 1000, max => 2000 } }, keys_only => 1 } );
    ...
    my @p_recs = $adb->read_all( 'products', { filter => { 4 => { min => 1000, max => 2000 } } } );
    ...
    my @min_ids = $adb->read_all( 'products', { filter => { 4 => { min => 1800 } }, keys_only => 1 } );
    ...
    my @max_ids = $adb->read_all( 'products', { filter => { 4 => { max => 500 } }, keys_only => 1 } );
    ...
    my @named_ids = $adb->read_all( 'products', { filter => { price => { min => 1000, max => 2000 } }, keys_only => 1 } );
    ...
    my ( $total_cnt, @page_ids ) = $adb->read_all( 'products', { filter => { 4 => { min => 1000, max => 2000 } }, offset => 0, limit => 2, keys_only => 1 } );
...
subtest '2. field_fetch with range' => sub {
    ...
    my @smartphones = $adb->field_fetch( 'products', 2, 'Smartphones', { filter => { 4 => { min => 1150 } } } );
    ...
    # With keys_only => 1
    my @fetch_ids = $adb->field_fetch( 'products', 2, 'Smartphones', { filter => { price => { max => 1150 } }, keys_only => 1 } );
...
subtest '3. search_table with range' => sub {
    ...
    my @results = $adb->search_table( 'products', 'Apple', { filter => { price => { min => 500, max => 2000 } }, keys_only => 1 } );
    ...
    my @high_results = $adb->search_table( 'products', 'Apple', { filter => { 4 => { min => 2000 } }, keys_only => 1 } );
    is_deeply( \@high_results, [ 3 ], 'search_table with min => 2000 only matches ID 3' );

    # Search for 'Apple' with max => 300 -> only AirPods (ID 6, 250)
    my @low_results = $adb->search_table( 'products', 'Apple', { filter => { 4 => { max => 300 } }, keys_only => 1 } );
...
subtest '4. field_filter with range' => sub {
    ...
    my $flt_res = $adb->field_filter( 'products', { filter => { 3 => 'Apple', 4 => { min => 1000 } } } );
...
subtest '5. facet_menu with range' => sub {
    ...
    my $menu = $adb->facet_menu( 'products', { filter => { price => { min => 1000 } } } );
...
subtest '6. field_fltkeys and field_allfltkeys with range' => sub {
    ...
    my $single_flt = $adb->field_fltkeys( 'products', { target_block => 2, filter => { 4 => { min => 1500 } } } );
    is_deeply( $single_flt, { 'Laptops' => 2 }, 'field_fltkeys respects range filter' );

    # field_allfltkeys with range
    my $all_flt = $adb->field_allfltkeys( 'products', { target_blocks => [ 2 ], filter => { 4 => { min => 1500 } } } );
...
subtest '7. Edge cases (empty results, out-of-range bounds)' => sub {
    ...
    my @empty_res = $adb->read_all( 'products', { filter => { 4 => { min => 100000 } }, keys_only => 1 } );
    is( scalar(@empty_res), 0, 'Out-of-range min returns empty list' );

    # Non-existent block fails gracefully without crashing
    my @safe_res = $adb->read_all( 'products', { filter => { 999 => { min => 10 } }, keys_only => 1 } );
