        ...
        constructors => [
            ...
            anon => {
                code => sub {
                    ...
                    my $anon  = $self;
                    $anon =~ s/[^[:word:]]/_/g;
                    my $anon_class = "${class}::$anon";
                    no strict 'refs';
                    @{"${anon_class}::ISA"} = $class;
                    return bless $self, $anon_class;
                }
            }
        ],
        attributes => [
...
# XXX for some reason, the anonymous things don't share the behavior of the
# base class, even though it shares the methods.  Doing a "diag
# $anon_thing->name" can reveal the problem.  Still, I think it's a bug in my
# example code and not in Class::Meta or Class::Meta::Declare.

can_ok $CLASS, 'anon';
ok my $anon_thing = $CLASS->anon,
  '... and we should be able to call the extra constructor';
isa_ok $anon_thing, $CLASS, '... and the object it returns';
cmp_ok ref($anon_thing), 'ne', $CLASS, '... but it should be its own subclass';
...
is_deeply \@constructors, ['anon', 'new'],
