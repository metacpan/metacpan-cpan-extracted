...
Sah::Schema::dns::record::sshfp - DNS SSHFP record

=head1 VERSION

This document describes version 0.003 of Sah::Schema::dns::record::sshfp (from Perl distribution Sah-SchemaBundle-DNS), released on 2024-06-26.
...
=head1 SAH SCHEMA DEFINITION
 ...
 [
   ...
   {
     "keys.restrict"      => 1,
     "merge.add.req_keys" => ["algo", "fptype", "fp"],
     "merge.normal.keys"  => {
                               name   => ["dns::record_field::name::disallow_underscore"],
                               algo   => ["int", { req => 1, in => [1 .. 4] }],
                               fp     => ["hexstr", { req => 1 }],
                               fptype => ["int", { req => 1, in => [1, 2] }],
                               type   => ["str", { req => 1, is => "SSHFP" }],
                             },
   },
 ]

Base schema: L<dns::record|Sah::Schema::dns::record>
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("dns::record::sshfp*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("dns::record::sshfp", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("dns::record::sshfp", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("dns::record::sshfp*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"dns::record::sshfp*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('dns::record::sshfp*', name=>'DnsRecordSshfp')
     );
 }

 use My::Types qw(DnsRecordSshfp);
 DnsRecordSshfp->assert_valid($data);
