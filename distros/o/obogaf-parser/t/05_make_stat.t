...
use Test::Files;
use Test::Exception;
use obogaf::parser qw(make_stat build_edges build_subonto make_stat);
...
lives_ok( sub { my $file = make_stat($goedges,  $parentIndex, $childIndex) }, 'file opened' );
dies_ok ( sub { my $file = make_stat($fakedges, $parentIndex, $childIndex) }, 'die: wrong file name');
...
$res= make_stat($gobp, $parentIndex, $childIndex);
is($res, "#oboterm <tab> degree <tab> indegree <tab> outdegree\nGO:0007260\t2\t2\t0\nGO:0007259\t1\t0\t1\nGO:0018108\t1\t0\t1\n\n~summary stat~\nnodes: 3\nedges: 2\nmax degree: 2\nmin degree: 1\nmedian degree: 1.0000\naverage degree: 0.6667\ndensity: 3.3333e-01\n", "test that make_stat works on whole ontology");
...
($parentIndex, $childIndex)= (1,2);
$res= make_stat($goedges_stat, $parentIndex, $childIndex);
