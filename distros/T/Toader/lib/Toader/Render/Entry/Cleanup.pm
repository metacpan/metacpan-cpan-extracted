package Toader::Render::Entry::Cleanup;
...
Toader::Render::Entry::Cleanup - This is used for cleaning up the output directory prior to rendering.
...
sub cleanup{
	...
	if ( ref( $obj) ne 'Toader::Entry' ){
		$self->{error}=4;
		$self->{errorString}='"'.ref( $obj ).'" is not a Toader::Entry object';
		$self->warn;
		return $self;
	}
	...
	$dir=$dir.'.entries/'.$obj->entryNameGet.'/';

	# if it does not exist, there is no reason to clean it
	if ( ! -e $dir ){
		return 1;
	}
	...
	if ( ! -e $filesdir ){
			...
			#make sure there are no surprises in the file directory
			$int=0;
			my $rmObjDir=1;
			while ( defined( $files[$int] ) ){
				if (
...
1; # End of Toader::Render::Entry::Cleanup
