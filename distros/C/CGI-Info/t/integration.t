...
subtest 'CGI::Info + File::Spec: script_dir usable with File::Spec->catfile' => sub {
    reset_env();
    if($^O eq 'MSWin32') {
        # Unix-style SCRIPT_FILENAME paths are not split correctly by the
        # Windows branch of script_dir() which uses qr{(.+)\\.+?$}
        pass('File::Spec script_dir test skipped on Windows (Unix paths not applicable)');
        return;
    }
    $ENV{SCRIPT_FILENAME} = '/var/www/cgi-bin/myapp.cgi';
    ...
    $spec_dir =~ s{[/\\]+$}{};   # strip trailing separator(s)
    is($spec_dir, $dir,
