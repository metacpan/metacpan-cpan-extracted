...
A lexeme or a terminal can have a C<if-action> setting, that applies I<only> when the lexeme or a terminal is discovered via the automatic scan. The C<if-action> must refers to a callback in user-land's recognizer interface, and must return a boolean:

  :lexeme   ::= symbolname if-action => Custom_action
