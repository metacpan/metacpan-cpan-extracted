package Mojolicious::Plugin::QuickMy;
...
sub register {
    ...
    $plugin->pg(Mojo::mysql->new($conf->{dsn}));
    $Debug = $conf->{debug};
    ...
    $app->log->info("MySQL version: $version->{'version()'}") if ($conf->{debug});
...
sub _quick_query {
    ...
    if ($type eq 'SELECT') {
        ...
        if ($type eq 'INSERT') {
            eval {
            return $self->pg->db->query($sql, @bind_params)->last_insert_id;
            };
        } else {
        # delete/update
            eval {
...
L<Mojolicous::Plugin::QuickMy> is a plugin for Mojolicious apps thas provide simple access to L<Mojo::mysql>.
The most part of the code for plugin is taken from L<Dancer::Plugin::Database::Core::Handle> (under Artistic License)

=head1 HELPERS

L<Mojolicious::Plugin::QuickMy> contains next helpers: I<qselect>, I<qinsert>, I<qupdate>, I<qdelete>, I<qcustom>, I<qerror>,
I<qcount>.
...
L<Mojolicious::Plugin::QuickMy> configuration support two keys.
...
=item * debug
    ...
    plugin 'QuickMy' => {dsn => 'mysql://username@/test', debug => 1};
...
L<Mojo::mysql> L<Mojolicious> L<Mojolicious::Guides> L<http://mojolicious.org>.
...
Jan Henning Thorse <jhthorsen@cpan.org>

=head1 BUGS

Please report any bugs or feature requests to C<bug-mojolicious-plugin-quickmy at rt.cpan.org>, or through
the web interface at L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=Mojolicious-Plugin-QuickMy>.  We will be notified, and then you'll
...
L<https://bitbucket.org/pkuptsov/mojolicious-plugin-quickmy>

=item * RT: CPAN's request tracker

L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=Mojolicious-Plugin-QuickMy>

=item * AnnoCPAN: Annotated CPAN documentation

L<http://annocpan.org/dist/Mojolicious-Plugin-QuickMy>

=item * CPANTS: CPAN Testing Service

L<http://cpants.perl.org/dist/overview/Mojolicious-Plugin-QuickMy>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/Mojolicious-Plugin-QuickMy>

=item * Search CPAN

L<http://search.cpan.org/dist/Mojolicious-Plugin-QuickMy>
