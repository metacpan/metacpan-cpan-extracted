...
# shell program to test phi.pm's responses to various erroneous
# conditions. note of course that phi never halts on errors/warnings
# but sets error codes that statistic.pl can request later on.

# Subtest 1: what happens when ngram > 2 is provided to phi
...
# test-1.sub-1.cnt has trigrams in it. so phi.pm should complain
# during the initialization step, since phi is for bigrams only!
...
# now the test!
echo "Test:   statistic.pl --ngram 3 phi test-1.out $TESTFILE" 
statistic.pl --ngram 3 phi test-1.out $TESTFILE >& error1.out
...
statistic.pl --set_freq_combo $FREQCOMBFILE phi test-2.out $TESTFILE >& error2.out
...
# now the test!
echo "Test:   statistic.pl phi test-3.out $TESTFILE"
statistic.pl phi test-3.out $TESTFILE >& error3.out
...
# now the test!
echo "Test:   statistic.pl phi test-4.out $TESTFILE"
statistic.pl phi test-4.out $TESTFILE >& error4.out
