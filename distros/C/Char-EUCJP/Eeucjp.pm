...
# regexp of character
BEGIN { CORE::eval q{ use vars qw($your_char) } } $your_char = q{[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF]|[\x00-\xFF]};
...
sub import {
    ...
    *Char::ord           = \&EUCJP::ord;
    *Char::ord_          = \&EUCJP::ord_;
    *Char::reverse       = \&EUCJP::reverse;
    *Char::getc          = \&EUCJP::getc;
    *Char::length        = \&EUCJP::length;
    *Char::substr        = \&EUCJP::substr;
    *Char::index         = \&EUCJP::index;
    *Char::rindex        = \&EUCJP::rindex;
    *Char::eval          = \&EUCJP::eval;
    *Char::escape        = \&EUCJP::escape;
    *Char::escape_token  = \&EUCJP::escape_token;
    *Char::escape_script = \&EUCJP::escape_script;
}
...
${Eeucjp::anchor} = qr{\G(?>[^\x8E\x8F\xA1-\xFE]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])*?}oxms;
...
if (($] >= 5.010001) or
    ...
    # EUC-JP encoding
        ...
        ${Eeucjp::q_char_SADAHIRO_Tomoyuki_2002_01_17} = qr{.*?[^\x8E\x8F\xA1-\xFE] (?>            [\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\xA1-\xFE] )*?}oxms;
    ...
    # other encoding
        ...
        ${Eeucjp::q_char_SADAHIRO_Tomoyuki_2002_01_17} = qr{.*?[$sbcs](?:[^$sbcs][^$sbcs])*?}oxms;
        #                                                     ******* octets not in multiple octet char (always char boundary)
        #                                                               **************** 2 octet chars
    }

    ${Eeucjp::anchor_SADAHIRO_Tomoyuki_2002_01_17} =
    qr{\G(?(?=.{0,32766}\z)(?:[^\x8E\x8F\xA1-\xFE]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])*?|(?(?=[$sbcs]+\z).*?|(?:${Eeucjp::q_char_SADAHIRO_Tomoyuki_2002_01_17})))}oxms;
#   qr{
#      \G # (1), (2)
#        (? # (3)
#          (?=.{0,32766}\z) # (4)
#                          (?:[^\x8E\x8F\xA1-\xFE]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])*?| # (5)
#                                                                                      (?(?=[$sbcs]+\z) # (6)
#                                                                                                      .*?| #(7)
#                                                                                                          (?:${Eeucjp::q_char_SADAHIRO_Tomoyuki_2002_01_17}) # (8)
#                                                                                                                                           ))}oxms;
    ...
    if (((('A' x 32768).'B') !~ / ${Eeucjp::anchor}                              B /oxms) and
        ...
        undef ${Eeucjp::q_char_SADAHIRO_Tomoyuki_2002_01_17};
    }
}
...
# $PadA  = '  (?:\A|                                           [\x00-\x80\xA0-\xDF])(?:[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE]{2})*?';
# $PadG  = '\G(?:                                |[\x00-\xFF]*?[\x00-\x80\xA0-\xDF])(?:[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE]{2})*?';
# $PadGA = '\G(?:\A|(?:[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE]{2})+?|[\x00-\xFF]*?[\x00-\x80\xA0-\xDF] (?:[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE]{2})*?)';
...
# ${Eeucjp::eS}          = qr{(?>[^\x8E\x8F\xA1-\xFE\x09\x0A    \x0C\x0D\x20]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
# ${Eeucjp::eS}          = qr{(?>[^\x8E\x8F\xA1-\xFE\x09\x0A\x0B\x0C\x0D\x20]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])};
...
sub Eeucjp::split(;$$$) {
    ...
    if (CORE::length($string) > 32766) {
        ...
        elsif (defined ${Eeucjp::q_char_SADAHIRO_Tomoyuki_2002_01_17}) {
            $q_char = ${Eeucjp::q_char_SADAHIRO_Tomoyuki_2002_01_17};
        }
    }
...
sub _last_subexpression_offsets {
    ...
    if ($modifier =~ /x/oxms) {
        @char = $pattern =~ /\G((?>
            ...
            \\ $q_char      |
            \# (?>[^\n]*) $ |
            \[ (?>(?:[^\x8E\x8F\xA1-\xFE\\\]]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF]|\\\\|\\\]|$q_char)+) \] |
            \(\?            |
                $q_char
        ))/oxmsg;
    }
...
sub _charlist {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # \P{PROPERTY} --> P\{PROPERTY}
        elsif ($char[$i] =~ /\A \\ ([bBNpP]) ( \{ ([^\x8E\x8F\xA1-\xFE0-9\}][^\x8E\x8F\xA1-\xFE\}]*) \} ) \z/oxms) {
            $char[$i] = $1 . '\\' . $2;
        }
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '${Eeucjp::not_lower_i}',
                '[:^upper:]'  => '${Eeucjp::not_upper_i}',
...
sub charlist_qr {
    ...
    if (scalar(@singleoctet) >= 1) {
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Eeucjp::uc($char);
                        my $fc = Eeucjp::fc($char);
                        if ($uc eq $fc) {
        ...
        my $not_anchor = '';
        $not_anchor = '(?![\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE])';
...
sub charlist_not_qr {
    ...
    if (scalar(@multipleoctet) >= 1) {
        if (scalar(@singleoctet) >= 1) {
            ...
            return '(?!' . join('|', @multipleoctet) . ')(?:[^\x8E\x8F\xA1-\xFE' . join('', @singleoctet) . ']|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])';
        }
        else {
        ...
        if (scalar(@singleoctet) >= 1) {
            ...
            return                                      '(?:[^\x8E\x8F\xA1-\xFE' . join('', @singleoctet) . ']|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])';
        }
        else {
...
#
# EUC-JP path globbing via File::DosGlob 1.10
#
...
sub _DOS_like_glob {
    ...
    # UNIX-like system
        ...
        $expr =~ s{ \A ~ ( (?:[^\x8E\x8F\xA1-\xFE/]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF])* ) }
...
OUTER:
    for my $expr (@expr) {
        ...
        if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
            if ($expr =~ s# \A ((?:[A-Za-z]:)?) ([^\x8E\x8F\xA1-\xFE/\\]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF]) #$1./$2#oxms) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
        ...
        my $matchsub = sub { Eeucjp::fc($_[0]) =~ /\A $pattern \z/xms };
...
sub _parse_line {
    ...
    my @piece = ();
    while ($line =~ /
        " ( (?>(?: [^\x8E\x8F\xA1-\xFE"]  |[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF] )* ) ) " (?>\s+) |
          ( (?>(?: [^\x8E\x8F\xA1-\xFE"\s]|[\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF] )* ) )   (?>\s+)
        /oxmsg
    ) {
...
#
# EUC-JP getc (with parameter, without parameter)
#
sub EUCJP::getc(;*@) {

    my($package) = caller;
    my $fh = @_ ? qualify_to_ref(shift,$package) : \*STDIN;
    croak 'Too many arguments for EUCJP::getc' if @_ and not wantarray;
...
my $qq_paren   = qr{(?{local $nest=0}) (?>(?:
                       [^\x8E\x8F\xA1-\xFE\\()] |
                           \(  (?{$nest++}) |
                           \)  (?(?{$nest>0})(?{$nest--})|(?!)))*) (?(?{$nest!=0})(?!)) |
                       [\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF] |
                    \\ [^\x8E\x8F\xA1-\xFEc] |
                    \\c[\x40-\x5F] |
                    \\ [\x8E\xA1-\xFE][\xA1-\xFE]|\x8F[\xA1-\xFE][\x00-\xFF] |
                       [\x00-\xFF]
...
my $qq_substr  = qr{(?> Char::substr | EUCJP::substr | CORE::substr | substr ) (?>\s*) \( $qq_paren \)
                 }xms;
...
#
# escape EUC-JP token of script
#
sub EUCJP::escape_token {
...
sub e_string {
    ...
    my @char = $string =~ / \G (?>[^\x8E\x8F\xA1-\xFE\\]|\\$q_char|$q_char) /oxmsg;
...
# subroutines of package Eeucjp
        ...
        elsif ($string =~ /\G \b EUCJP::eval       (?= (?>\s*) \{ )                    /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::eval \b                                        /oxgc) { $e_string .= 'eval Char::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b EUCJP::eval \b                                        /oxgc) { $e_string .= 'eval EUCJP::escape'; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b EUCJP::index \b                                       /oxgc) { $e_string .= 'EUCJP::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                             /oxgc) { $e_string .= 'Eeucjp::index';      $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b EUCJP::rindex \b                                      /oxgc) { $e_string .= 'EUCJP::rindex';      $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                            /oxgc) { $e_string .= 'Eeucjp::rindex';     $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Eeucjp::lc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Eeucjp::lcfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Eeucjp::uc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Eeucjp::ucfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Eeucjp::fc';         $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Eeucjp::chr';           $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Eeucjp::glob';          $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                                      /oxgc) { $e_string .= 'Eeucjp::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                                 /oxgc) { $e_string .= 'Eeucjp::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                                      /oxgc) { $e_string .= 'Eeucjp::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                                 /oxgc) { $e_string .= 'Eeucjp::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                                      /oxgc) { $e_string .= 'Eeucjp::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                                     /oxgc) { $e_string .= 'Eeucjp::chr_';              $slash = 'm//'; }
...
sub e_qr {
    ...
    my @char = $string =~ /\G((?>
        ...
        \\x\{ (?>[0-9A-Fa-f]+) \}  |
        \\o\{ (?>[0-7]+)       \}  |
        \\[bBNpP]\{ (?>[^\x8E\x8F\xA1-\xFE0-9\}][^\x8E\x8F\xA1-\xFE\}]*) \} |
        \\  $q_char                |
        \$` | \$\{`\} | \$ (?>\s*) PREMATCH  | \$ (?>\s*) \{ (?>\s*) PREMATCH  (?>\s*) \} | \$ (?>\s*) \{\^PREMATCH\}  |
        \$& | \$\{&\} | \$ (?>\s*) MATCH     | \$ (?>\s*) \{ (?>\s*) MATCH     (?>\s*) \} | \$ (?>\s*) \{\^MATCH\}     |
                        \$ (?>\s*) POSTMATCH | \$ (?>\s*) \{ (?>\s*) POSTMATCH (?>\s*) \} | \$ (?>\s*) \{\^POSTMATCH\} |
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Eeucjp::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Eeucjp::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Eeucjp::ignorecase(Eeucjp::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Eeucjp::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Eeucjp::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Eeucjp::ignorecase(Eeucjp::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Eeucjp::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Eeucjp::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Eeucjp::ignorecase(Eeucjp::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Eeucjp::POSTMATCH()]}';
            }
...
sub e_s2_q {
    ...
    my @char = $string =~ / \G (?>[^\x8E\x8F\xA1-\xFE\\]|\\\\|$q_char) /oxmsg;
    for (my $i=0; $i <= $#char; $i++) {
        if (0) {
        }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                                              2   3                                 4   5
                q<CORE::eval{local $EUCJP::re_t=%s; local $EUCJP::re_a=''; while($EUCJP::re_t =~ %s){%s local $^W=0; local $EUCJP::re_r=%s; %s$EUCJP::re_t="$EUCJP::re_a${1}$EUCJP::re_r$'"; pos($EUCJP::re_t)=length "$EUCJP::re_a${1}$EUCJP::re_r"; $EUCJP::re_a=substr($EUCJP::re_t,0,pos($EUCJP::re_t)); } return $EUCJP::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$EUCJP::re_r=CORE::eval $EUCJP::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $EUCJP::re_t=%s; while($EUCJP::re_t =~ %s){%s local $^W=0; local $EUCJP::re_r=%s; %s$EUCJP::re_t="$`$EUCJP::re_r$'"; pos($EUCJP::re_t)=length "$`$EUCJP::re_r"; } return $EUCJP::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $EUCJP::re_r=%s; %s"%s$EUCJP::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<CORE::eval{local $EUCJP::re_n=0; local $EUCJP::re_a=''; while(%s =~ %s){%s local $^W=0; local $EUCJP::re_r=%s; %s%s="$EUCJP::re_a${1}$EUCJP::re_r$'"; pos(%s)=length "$EUCJP::re_a${1}$EUCJP::re_r"; $EUCJP::re_a=substr(%s,0,pos(%s)); $EUCJP::re_n++} return %s$EUCJP::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $EUCJP::re_n=0; while(%s =~ %s){%s local $^W=0; local $EUCJP::re_r=%s; %s%s="$`$EUCJP::re_r$'"; pos(%s)=length "$`$EUCJP::re_r"; $EUCJP::re_n++} return %s$EUCJP::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $EUCJP::re_r=%s; %s%s="%s$EUCJP::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $EUCJP::re_r=%s; %s%s="%s$EUCJP::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$EUCJP::re_r=CORE::eval $EUCJP::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Eeucjp::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
...
This module has run-time routines for use EUCJP software automatically, you
do not have to use.
...
=item Chop string
  ...
  character chopped. The Eeucjp::chop subroutine is used primary to remove the newline
...
=item Reverse index string
  ...
  This subroutine works just like Eeucjp::index except that it returns the byte
...
=item Lower case string
  ...
  You can use the Eeucjp::fc subroutine for case-insensitive comparisons via EUCJP
  software.
...
=item Fold case string
  ...
  New to EUCJP software, this subroutine returns the full Unicode-like casefold of
...
=item Filename expansion (globbing)
  ...
  The Eeucjp::glob subroutine grandfathers the use of whitespace to separate multiple
