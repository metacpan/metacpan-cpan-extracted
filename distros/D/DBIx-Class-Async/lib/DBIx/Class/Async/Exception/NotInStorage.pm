...
$DBIx::Class::Async::Exception::NotInStorage::VERSION   = '0.64';
...
DBIx::Class::Async::Exception::NotInStorage - Exception for operations on
un-inserted row objects
...
=head1 SYNOPSIS
    ...
    DBIx::Class::Async::Exception::NotInStorage->throw(
        message   => "[DBIx::Class::Async] Cannot update a row that has not been inserted",
        row_class => 'My::Schema::Result::Operation',
        operation => 'update',
        hint      => "Call insert() or create() before calling update().",
    );

    use Try::Tiny;
    try { ... } catch {
        if ( ref $_ && $_->isa('DBIx::Class::Async::Exception::NotInStorage') ) {
            warn "Row not saved yet (" . $_->row_class . "): " . $_->hint;
        }
    };

=head1 DESCRIPTION

Thrown when C<update()> or C<delete()> is called on a L<DBIx::Class::Row>
object whose C<in_storage> flag is false - meaning the row has not yet been
...
=head2 row_class
    ...
    my $class = $exception->row_class;

The fully-qualified result class of the row object that was not in storage,
e.g. C<'My::Schema::Result::Operation'>. May be undef if not determinable.

=cut

sub row_class { $_[0]->{row_class} }
...
You can find documentation for this module with the perldoc command.
    ...
    perldoc DBIx::Class::Async::Exception::NotInStorage
...
1; # End of DBIx::Class::Async::Exception::NotInStorage
