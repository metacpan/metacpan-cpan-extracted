...
use Google::Ads::AdWords::v201710::BatchJob;
use Google::Ads::AdWords::v201710::BatchJobOperation;
...
sub add_keywords_using_incremental_batch_job {
  ...
  my $operation = Google::Ads::AdWords::v201710::BatchJobOperation->new({
      operator => 'ADD',
      operand  => Google::Ads::AdWords::v201710::BatchJob->new({})});
...
sub _create_keyword_operations {
  ...
  for (my $i = 1 ; $i < KEYWORD_COUNT ; $i++) {
    # Create keyword.
    my $keyword =
      Google::Ads::AdWords::v201710::Keyword->new({matchType => "BROAD"});
...
sub _verify_operations {
  ...
  my $predicate = Google::Ads::AdWords::v201710::Predicate->new({
      field    => "Id",
      operator => "IN",
      values   => [$job_id]});
