...
However you can write our own XS code using these functions or C++ Date class (from L<Panda::Date> module).
...
=head4 void tzset (const std::string_view& zone)

=head4 void tzset (const Timezone* zone)

See L</tzset([$zone])>.

=head4 const Timezone* tzget (const std::string_view& zone)

Returns timezone object pointer which contains info about timezone 'zone' (or about server's local zone if zone == "").
...
=head4 Timezone* tzlocal ()

Same as C<tzget("")>.

=head4 const panda::string& tzdir ()

Returns current timezone DB directory.

=head4 bool tzdir (const panda::string& newdir)

See L</tzdir([$newdir])>. C<tzdir("")> sets tzdir to tzsysdir().

=head4 const panda::string& tzsysdir ()

Returns system timezones dir if any (usually /usr/share/zoneinfo), otherwise returns empty string.
...
=head4 void dt2tm (tm& to, const datetime& from), void tm2dt (datetime& to, const tm& from)
...
wrong results in case of certain dates (actually rare dates). Therefore in such cases the result of panda::time::* functions
won't match with POSIX functions because panda::time handles all these cases correctly.
