...
eval {
    my $filter = HTTPD::Log::Filter->new(
        exclusions_file     => '/tmp/exclusions_xlf',
        request_re          => "GET.*?",
        format              => 'XLF',
    );
    open( LOG, 'sample/access_log_xlf' ) or die $!;
    open( OUT, '>/tmp/inclusions_xlf' ) or die $!;
    while ( <LOG> )
    {
    ...
    if ( -x $diff )
    {
        for ( qw( inclusions_xlf exclusions_xlf ) )
        {
            my @diff = `$diff sample/$_ /tmp/$_`;
            die "sample/$_ and /tmp/$_ differ:\n@diff" if @diff;
        }
    }
    unlink( '/tmp/exclusions_xlf', '/tmp/inclusions_xlf' );
};
print STDERR $@ if $@;
print $@ ? "not " : "", "ok 2\n";
