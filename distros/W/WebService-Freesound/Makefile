# This Makefile is for the WebService::Freesound extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/WebService/Freesound.pm]
#     AUTHOR => [q[Andy Cragg <andyc@caesuramedia.com>]]
#     BUILD_REQUIRES => { HTTP::Response=>q[0], JSON=>q[0], LWP::UserAgent=>q[0], Test::Exception=>q[0], Test::MockModule=>q[0], Test::More=>q[0] }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker=>q[0] }
#     LICENSE => q[artistic_2]
#     MIN_PERL_VERSION => q[5.006]
#     NAME => q[WebService::Freesound]
#     PL_FILES => {  }
#     PREREQ_PM => { Carp=>q[0], Data::Dumper=>q[0], HTTP::Response=>q[0], JSON=>q[0], LWP::Simple=>q[0], LWP::UserAgent=>q[0], Test::Exception=>q[0], Test::MockModule=>q[0], Test::More=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/WebService/Freesound.pm]
#     clean => { FILES=>q[WebService-Freesound-*] }
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/WebService/Freesound.pm
...
TO_INST_PM = lib/WebService/Freesound.pm
...
DISTNAME = WebService-Freesound
DISTVNAME = WebService-Freesound-0.02
...
manifypods : pure_all config  \
	lib/WebService/Freesound.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) -u \
	  lib/WebService/Freesound.pm $(INST_MAN3DIR)/WebService::Freesound.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Perl wrapper around Freesound OAuth2 API!'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Andy Cragg <andyc@caesuramedia.com>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: WebService-Freesound' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Perl wrapper around Freesound OAuth2 API!",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Andy Cragg <andyc@caesuramedia.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "WebService-Freesound",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="WebService-Freesound" VERSION="0.02">' > WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl wrapper around Freesound OAuth2 API!</ABSTRACT>' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Andy Cragg &lt;andyc@caesuramedia.com&gt;</AUTHOR>' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Carp::" />' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Data::Dumper" />' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="JSON::" />' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="LWP::Simple" />' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="LWP::UserAgent" />' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.22" />' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> WebService-Freesound.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> WebService-Freesound.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/WebService/Freesound.pm' 'blib/lib/WebService/Freesound.pm' 
