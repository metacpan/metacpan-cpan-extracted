...
Data::SIprefixes::deci - This provides deci matching for Data::SIprefixes.
...
=head1 SYNOPSIS
    ...
    use Data::SIprefixes::deci;
    
    my $prefix=Data::SIprefixes::deci->new;

    my $origMeasure='decimeter';
...
sub longMatch{
	...
	if ( $measure=~/^deci/ ){
		...
		if ( $measure =~ /^ / ){
			$self->{error}=2;
			$self->{errorString}='Space found after prefix, /^deci/, in "'.$origMeasure.'"' ;
			$self->warn;
			return undef;
		}
...
sub shortMatch{
    ...
    if ( $measure=~/^d/ ){
        ...
        if ( $measure =~ /^ / ){
            $self->{error}=2;
            $self->{errorString}='Space found after prefix, /^d/, in "'.$origMeasure.'"' ;
            $self->warn;
            return undef;
        }
