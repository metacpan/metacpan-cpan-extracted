...
perl $0 KPS9566_script.pl > Escaped_script.pl.e
END
    }

    print KPS9566::escape_script($ARGV[0]);
    exit 0;
}
...
sub KPS9566::escape_script {
    ...
    # read KPS9566 script
    my $fh = gensym();
    Ekps9566::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Ekps9566(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ekps9566 '%s.0'; # 'quote' for perl5.6\n", $KPS9566::VERSION); # require run-time routines version

        # use KPS9566 version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | KPS9566 ) (?>\s*) ([^\x81-\xFE;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                ...
                if ($version ne $KPS9566::VERSION) {
                    my @file = grep -e, map {qq{$_/KPS9566.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects KPS9566.pm $version, but @{[__FILE__]} is version $KPS9566::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Ekps9566.pm $version, but \\\$Ekps9566::VERSION is \$Ekps9566::VERSION" if \$Ekps9566::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $KPS9566::VERSION) {
                    die "Script $0 required KPS9566.pm $version, but @{[__FILE__]} is only version $KPS9566::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Ekps9566::function_ord     = 'KPS9566::ord'     if /\A ord \z/oxms;
                    $Ekps9566::function_ord_    = 'KPS9566::ord_'    if /\A ord \z/oxms;
                    $Ekps9566::function_reverse = 'KPS9566::reverse' if /\A reverse \z/oxms;
                    $Ekps9566::function_getc    = 'KPS9566::getc'    if /\A getc \z/oxms;

                    # avoid: used only once: possible typo at ...
                    $Ekps9566::function_ord     = $Ekps9566::function_ord;
                    $Ekps9566::function_ord_    = $Ekps9566::function_ord_;
                    $Ekps9566::function_reverse = $Ekps9566::function_reverse;
                    $Ekps9566::function_getc    = $Ekps9566::function_getc;
                }
            }
        }
    }

    $e_script .= KPS9566::escape();
...
=item * You'll have to download KPS9566.pm and Ekps9566.pm and put it in your perl lib directory.

=item * You'll need to write "use KPS9566;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use KPS9566;
  use KPS9566 ver.sion;             --- require minimum version
  use KPS9566 ver.sion.0;           --- expects version (match or die)
  use KPS9566 qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use KPS9566 ver.sion qw(ord reverse getc);
  use KPS9566 ver.sion.0 qw(ord reverse getc);

  # "no KPS9566;" not supported

  or

  $ perl KPS9566.pm KPS9566_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  KPS9566_script.pl  --- script written in KPS9566
...
KPS9566 software is "middleware" between perl interpreter and your Perl script
written in KPS9566.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain KPS9566, Perl3 or later
can treat KPS9566 as binary data.

By "use KPS9566;", it automatically interpret your script as KPS9566. The various
functions of perl including a regular expression can treat KPS9566 now.
...
This software is a source code filter to escape Perl script encoded by KPS9566
...
=item * Remains One Language Handling Raw KPS9566, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. KPS9566.pm, Ekps9566.pm
  +---------------------------------------+
...
import of Ekps9566.pm works well.
...
=head1 Software Composition
   ...
   KPS9566.pm               --- source code filter to escape KPS9566
   Ekps9566.pm              --- run-time routines for KPS9566.pm
...
You need write 'use KPS9566;' in your script.
...
=head1 Escaping Multiple-Octet Code (KPS9566 software provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (KPS9566 software provides)

KPS9566 software applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Ekps9566::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Escaping Second Octet (KPS9566 software provides)

KPS9566 software escapes second octet of multiple-octet character in regular
expression.
...
=head1 Multiple-Octet Character Regular Expression (KPS9566 software provides)

KPS9566 software clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Ekps9566::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Ekps9566::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Ekps9566::ignorecase()' (KPS9566 software provides)

KPS9566 software applies calling 'Ekps9566::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Ekps9566::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Ekps9566::matched/
  m/regexp/               m/ditto$Ekps9566::matched/
  ?regexp?                m?ditto$Ekps9566::matched?
  m?regexp?               m?ditto$Ekps9566::matched?
 
  $_ =~                   ($_ =~ m/ditto$Ekps9566::matched/) ?
  s/regexp/replacement/   CORE::eval{ Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Ekps9566::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Ekps9566::split(qr/regexp/)
  split(m/regexp/)        Ekps9566::split(qr/regexp/)
  split(qr/regexp/)       Ekps9566::split(qr/regexp/)
  qr/regexp/              qr/ditto$Ekps9566::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Ekps9566::matched/
  m/regexp/b              m/(?:regexp)$Ekps9566::matched/
  ?regexp?b               m?regexp$Ekps9566::matched?
  m?regexp?b              m?regexp$Ekps9566::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Ekps9566::matched/) ?
  s/regexp/replacement/b  CORE::eval{ Ekps9566::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Ekps9566::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Ekps9566.pm provides)
...
Definitions in Ekps9566.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Ekps9566::anchor}         qr{\G(?>[^\x81-\xFE]|[\x81-\xFE][\x00-\xFF])*?}
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (KPS9566 software provides)

KPS9566 software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (KPS9566 software provides)

Insert 'Ekps9566::' at head of function name. Ekps9566.pm provides your script Ekps9566::*
subroutines.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Ekps9566::split     Character
  tr///       Ekps9566::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Ekps9566::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Ekps9566::chop      Character
  index       Ekps9566::index     Character
  rindex      Ekps9566::rindex    Character
  lc          Ekps9566::lc        Character
  lcfirst     Ekps9566::lcfirst   Character
  uc          Ekps9566::uc        Character
  ucfirst     Ekps9566::ucfirst   Character
  fc          Ekps9566::fc        Character
  chr         Ekps9566::chr       Character
  glob        Ekps9566::glob      Character
  lstat       Ekps9566::lstat     Character
  opendir     Ekps9566::opendir   Character
  stat        Ekps9566::stat      Character
  unlink      Ekps9566::unlink    Character
  chdir       Ekps9566::chdir     Character
  do          Ekps9566::do        Character
  require     Ekps9566::require   Character
  -------------------------------------------

  ------------------------------------------------------------------------------------------------------------------------
  Before                   After
  ------------------------------------------------------------------------------------------------------------------------
  use Perl::Module;        BEGIN { Ekps9566::require 'Perl/Module.pm'; Perl::Module->import() if Perl::Module->can('import'); }
  use Perl::Module @list;  BEGIN { Ekps9566::require 'Perl/Module.pm'; Perl::Module->import(@list) if Perl::Module->can('import'); }
  use Perl::Module ();     BEGIN { Ekps9566::require 'Perl/Module.pm'; }
  no Perl::Module;         BEGIN { Ekps9566::require 'Perl/Module.pm'; Perl::Module->unimport() if Perl::Module->can('unimport'); }
  no Perl::Module @list;   BEGIN { Ekps9566::require 'Perl/Module.pm'; Perl::Module->unimport(@list) if Perl::Module->can('unimport'); }
  no Perl::Module ();      BEGIN { Ekps9566::require 'Perl/Module.pm'; }
  ------------------------------------------------------------------------------------------------------------------------

=head1 Escaping File Test Operators (KPS9566 software provides)

Insert 'Ekps9566::' instead of '-' of operator.

  Available in MSWin32, MacOS, and UNIX-like systems
  --------------------------------------------------------------------------
  Before   After      Meaning
  --------------------------------------------------------------------------
  -r       Ekps9566::r   File or directory is readable by this (effective) user or group
  -w       Ekps9566::w   File or directory is writable by this (effective) user or group
  -e       Ekps9566::e   File or directory name exists
  -x       Ekps9566::x   File or directory is executable by this (effective) user or group
  -z       Ekps9566::z   File exists and has zero size (always false for directories)
  -f       Ekps9566::f   Entry is a plain file
  -d       Ekps9566::d   Entry is a directory
  -t       -t         The filehandle is a TTY (as reported by the isatty() system function;
                      filenames can't be tested by this test)
  -T       Ekps9566::T   File looks like a "text" file
  -B       Ekps9566::B   File looks like a "binary" file
  -M       Ekps9566::M   Modification age (measured in days)
  -A       Ekps9566::A   Access age (measured in days)
  -C       Ekps9566::C   Inode-modification age (measured in days)
  -s       Ekps9566::s   File or directory exists and has nonzero size
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -R       Ekps9566::R   File or directory is readable by this real user or group
  -W       Ekps9566::W   File or directory is writable by this real user or group
  -X       Ekps9566::X   File or directory is executable by this real user or group
  -l       Ekps9566::l   Entry is a symbolic link
  -S       Ekps9566::S   Entry is a socket
  --------------------------------------------------------------------------
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -o       Ekps9566::o   File or directory is owned by this (effective) user
  -O       Ekps9566::O   File or directory is owned by this real user
  -p       Ekps9566::p   Entry is a named pipe (a "fifo")
  -b       Ekps9566::b   Entry is a block-special file (like a mountable disk)
  -c       Ekps9566::c   Entry is a character-special file (like an I/O device)
  -u       Ekps9566::u   File or directory is setuid
  -g       Ekps9566::g   File or directory is setgid
  -k       Ekps9566::k   File or directory has the sticky bit set
  --------------------------------------------------------------------------
...
You need write 'KPS9566::' at head of function name when you want character-
oriented subroutine. See 'Character-Oriented Subroutines'.
  ...
  --------------------------------------------------------
  ord        KPS9566::ord
  reverse    KPS9566::reverse
  getc       KPS9566::getc
  length     KPS9566::length
  substr     KPS9566::substr
  index      KPS9566::index          See 'About Indexes'
  rindex     KPS9566::rindex         See 'About Rindexes'
  eval       KPS9566::eval
  --------------------------------------------------------
  ...
  (same as Ekps9566::index)
  KPS9566::index    Character   Character    Character-oriented semantics
  ...
  (same as Ekps9566::rindex)
  KPS9566::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  $ord = KPS9566::ord($string);

  This subroutine returns the numeric value (ASCII or KPS9566 character) of the
  first character of $string, not Unicode. If $string is omitted, it uses $_.
  The return value is always unsigned.

  If you import ord "use KPS9566 qw(ord);", ord of your script will be rewritten in
  KPS9566::ord. KPS9566::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = KPS9566::reverse(@list);
  $reverse = KPS9566::reverse(@list);
  ...
  If you import reverse "use KPS9566 qw(reverse);", reverse of your script will be
  rewritten in KPS9566::reverse. KPS9566::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  If you import getc "use KPS9566 qw(getc);", getc of your script will be rewritten
  in KPS9566::getc. KPS9566::getc is not compatible with getc of JPerl.

=item * Length by KPS9566 Character

  $length = KPS9566::length($string);
  $length = KPS9566::length();

  This subroutine returns the length in characters (programmer-visible characters)
  of the scalar value $string. If $string is omitted, it returns the KPS9566::length
  of $_.

  Do not try to use KPS9566::length to find the size of an array or hash. Use scalar
...
=item * Substr by KPS9566 Character

  $substr = KPS9566::substr($string,$offset,$length,$replacement);
  $substr = KPS9566::substr($string,$offset,$length);
  $substr = KPS9566::substr($string,$offset);
  ...
  my $color  = KPS9566::substr $s, 4, 5;      # black
  my $middle = KPS9566::substr $s, 4, -11;    # black cat climbed the
  my $end    = KPS9566::substr $s, 14;        # climbed the green tree
  my $tail   = KPS9566::substr $s, -4;        # tree
  my $z      = KPS9566::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the KPS9566::substr() subroutine as an
  ...
  end of the string, KPS9566::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  KPS9566::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = KPS9566::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = KPS9566::substr $name, 7;      # returns undef, with warning
  KPS9566::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using KPS9566::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = KPS9566::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of KPS9566::substr() acts
...
=item * Index by KPS9566 Character

  $index = KPS9566::index($string,$substring,$offset);
  $index = KPS9566::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = KPS9566::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }

=item * Rindex by KPS9566 Character

  $rindex = KPS9566::rindex($string,$substring,$offset);
  $rindex = KPS9566::rindex($string,$substring);

  This subroutine works just like KPS9566::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse KPS9566::index).
  ...
  $pos = KPS9566::length($string);
  while (($pos = KPS9566::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }

=item * Eval KPS9566 Script

  $eval = KPS9566::eval { block };
  $eval = KPS9566::eval $expr;
  $eval = KPS9566::eval;

  The KPS9566::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, KPS9566::eval { block }
  and KPS9566::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, KPS9566::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of KPS9566::eval, the value returned from an KPS9566::eval is
  ...
  KPS9566::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  KPS9566::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      KPS9566::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The KPS9566::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow KPS9566::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  KPS9566::eval cannot trap syntax errors.

  The KPS9566::eval $expr form can trap syntax errors because it parses the code
  ...
  after the KPS9566::eval is complete, as do any subroutine or format definitions.
  The code of the KPS9566::eval is treated as a block, so any locally scoped
  variables declared within the KPS9566::eval last only until the KPS9566::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  KPS9566::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  KPS9566::eval { block }    eval { block }
  KPS9566::eval $expr        eval KPS9566::escape $expr
  KPS9566::eval              eval KPS9566::escape
  -------------------------------------------------

  To tell the truth, the subroutine KPS9566::eval does not exist. If it exists,
  you will troubled, when KPS9566::eval has a parameter that is single quoted
  string included my variables. KPS9566::escape is a subroutine that makes Perl
  ...
  Since KPS9566::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because KPS9566::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  KPS9566::eval $expr and KPS9566::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      KPS9566::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      KPS9566::eval { $answer = }; # WRONG

      # a runtime syntax error
      KPS9566::eval '$answer =';   # sets $@
  ...
  Using the KPS9566::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      KPS9566::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          KPS9566::eval { die "foo lives here" };
  ...
  With an KPS9566::eval, you should be especially careful to remember what's being
  looked at when:

      KPS9566::eval $x;        # CASE 1
      KPS9566::eval "$x";      # CASE 2

      KPS9566::eval '$x';      # CASE 3
      KPS9566::eval { $x };    # CASE 4

      KPS9566::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  The block of KPS9566::eval { block } does not count as a loop, so the loop
...
=item * Filename Globbing
  ...
  This operator is implemented via the Ekps9566::glob() subroutine. See Ekps9566::glob
  of Ekps9566.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use KPS9566 qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use KPS9566 qw(reverse);" exists or not.
...
=head1 Yada Yada Operator (KPS9566 software provides)
...
=head1 Un-Escaping bytes::* Subroutines (KPS9566 software provides)

KPS9566 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Ekps9566.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in KPS9566 software and
...
=item * KPS9566::substr as Lvalue

If Perl version is older than 5.14, KPS9566::substr differs from CORE::substr, and
...
KPS9566::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Ekps9566::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${`}            Ekps9566::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Ekps9566::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Ekps9566::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Ekps9566::MATCH()       $1
  ${&}            Ekps9566::MATCH()       $1
  $MATCH          Ekps9566::MATCH()       $1
  ${^MATCH}       Ekps9566::MATCH()       $1
  $'              $'                   $'
  ${'}            ${'}                 $'
  $POSTMATCH      Ekps9566::POSTMATCH()   $'
  ${^POSTMATCH}   Ekps9566::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
KPS9566 software remains one language and one interpreter.
