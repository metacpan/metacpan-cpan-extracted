...
$gfx->transform(-translate => [10, 10],
		...
		-skew      => [5, -10],
	        'repeat'   => 1);
like($pdf->to_string, qr/1.3854 0.78142 -1.0586 2.8596 20 50 cm 1.7193 4.0475 -5.7318 10.684 30 60 cm/, q{transform + transform_repeat});
...
$gfx->line_width(9.125);
like($pdf->to_string, qr/9.125 w/, q{line_width(9.125)});
...
$gfx->line_cap(2);
like($pdf->to_string, qr/2 J/, q{line_cap(2)});
...
$gfx->line_join(2);
like($pdf->to_string, qr/2 j/, q{line_join(2)});
...
$gfx->miter_limit(3.5);
like($pdf->to_string, qr/3.5 M/, q{miter_limit(3.5)});
...
$gfx->linedash();
like($pdf->to_string, qr/\[ \] 0 d/, q{linedash()});
...
like($pdf->to_string, qr/\[ 15 5 3 2 \] 8 d/, q{linedash(-pattern => [15, 5, 3, 2], -shift => 8)});
...
like($pdf->to_string, qr/7 i/, q{flatness_tolerance(7)});
...
$gfx->move(0, 0);
$gfx->polyline(72, 144, 216, 288, 100, 200);
$gfx->stroke();
like($pdf->to_string, qr/0 0 m 72 144 l 216 288 l 100 200 l S/, q{polyline, six arguments});
...
$gfx->stroke();
like($pdf->to_string, qr/30 60 m 70 100 110 140 150 180 c S/, q{qbspline});
...
$gfx->stroke();
like($pdf->to_string, qr/216 288 m 302.6 328 l 297.5 335.07 291.08 341.47 283.56 346.98 c 276.04 352.5 267.51 357.06 258.26 360.5 c 249.02 363.95 239.17 366.25 229.05 367.32 c 218.94 368.38 208.68 368.2 198.64 366.78 c h S/,
...
like($pdf->to_string, qr/4 Tc/, q{character_spacing(4)});
...
$gfx->word_spacing(4);
like($pdf->to_string, qr/4 Tw/, q{word_spacing(4)});
...
$gfx->nl(0);
like($pdf->to_string, qr/T\*/, q{nl(0)});
...
$gfx->nl(300);
like($pdf->to_string, qr/T\* \[-3000\] TJ/, q{nl(300)});
