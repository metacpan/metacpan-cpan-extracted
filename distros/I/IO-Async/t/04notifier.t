...

{
   ...
   ok( !dies { $notifier->configure; },
      '$notifier->configure no params succeeds' );

   ok( dies { $notifier->configure( oranges => 1 ) },
   ...
   ok( !dies { $notifier->configure( oranges => 3 ) },
       '$notifier->configure with configure_unknown succeeds' );

   is( \%other, { oranges => 3 }, '%other after configure_unknown' );
}
...
# weaseling
{
   ...
   $mref->( 123 );
   is( \@args, [ $notifier, 123 ], '@args after invoking $mref' );
   ...
   $mref->( $outerself, 456 );
   is( \@args, [ $notifier, 456 ], '@args after invoking replacer $mref' );

   isa_ok( $outerself, [ "OtherClass" ], '$outerself unchanged' );

   ok( dies { $notifier->_capture_weakself( 'cannotdo' ) },
...
# Subclass
{
   ...
   $mref->( 456 );
   is( \@subargs, [ $subn, 456 ], '@subargs after invoking $mref on named method' );
   ...
   # Method capture
   {
      ...
      is( \@subargs, [], '@subargs empty after TestNotifier::frobnicate replacement' );
      is( \@newargs, [ $subn, 321 ], '@newargs after TestNotifier::frobnicate replacement' );
   }

   undef @subargs;

   $subn->invoke_event( 'frobnicate', 78 );
   is( \@subargs, [ $subn, 78 ], '@subargs after ->invoke_event' );

   undef @subargs;

   is( $subn->maybe_invoke_event( 'frobnicate', 'a'..'c' ),
              [ $subn, 'a'..'c' ],
   ...
   $cb->( 90 );
   is( \@subargs, [ $subn, 90 ], '@subargs after ->make_event_cb->()' );

   is( ref $subn->maybe_make_event_cb( 'frobnicate' ), "CODE", '->maybe_make_event_cb yields CODE ref' );
...
# parent/child
{
   ...
   ref_is( $child->parent, $parent, '$child->parent is $parent' );
   is( [ $parent->children ], [ exact_ref($child) ], '$parent->children' );

   is_oneref( $parent, '$parent has refcount 1 after add_child' );
   is_refcount( $child, 2, '$child has refcount 2 after add_child' );

   ok( dies { $parent->add_child( $child ) }, 'Adding child again fails' );

   $parent->remove_child( $child );

   is_oneref( $child, '$child has refcount 1 after remove_child' );
   is( [ $parent->children ], [], '$parent->children now empty' );
}
...
# invoke_error
{
   ...
   ok( dies { $parent->invoke_error( "It went wrong", wrong => ) },
   ...
   ok( !dies { $parent->invoke_error( "It's still wrong", wrong => ) },
       'Exception not thrown from ->invoke_error with handler' );
   is( $err, "It's still wrong", '$message to on_error' );

   ok( !dies { $child->invoke_error( "Wrong on child", wrong => ) },
