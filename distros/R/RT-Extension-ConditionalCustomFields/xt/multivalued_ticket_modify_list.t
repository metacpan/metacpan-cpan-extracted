...
$cf_condition->Create(Name => 'Condition', Type => 'Select', MaxValues => 0, Queue => 'General', RenderType => 'List');
...
my $ticket_cf_condition_passed = $mjs->by_id('Object-RT::Ticket-' . $ticket->id . '-CustomField:Groupone-' . $cf_condition->id . '-Values-' . $cf_values->[0]->id, single => 1);
...
my $ticket_cf_condition_failed = $mjs->by_id('Object-RT::Ticket-' . $ticket->id . '-CustomField:Groupone-' . $cf_condition->id . '-Values-' . $cf_values->[1]->id, single => 1);
...
$ticket_cf_condition_passed = $mjs->by_id('Object-RT::Ticket-' . $ticket->id . '-CustomField:Groupone-' . $cf_condition->id . '-Values-' . $cf_values->[0]->id, single => 1);
$mjs->click($ticket_cf_condition_passed);
...
$ticket_cf_condition_passed = $mjs->by_id('Object-RT::Ticket-' . $ticket->id . '-CustomField:Groupone-' . $cf_condition->id . '-Values-' . $cf_values->[2]->id, single => 1);
$mjs->click($ticket_cf_condition_passed);
