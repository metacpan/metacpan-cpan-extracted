...
Sah::Schema::date::year - Year number (AD, starting from 1), e.g. 2022

=head1 VERSION

This document describes version 0.019 of Sah::Schema::date::year (from Perl distribution Sah-Schemas-Date), released on 2023-12-09.
...
=head2 Sample data and validation results against this schema
 ...
 0  # INVALID (There is no AD 0; AD starts from 1)
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("date::year*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("date::year", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("date::year", {return_type=>'str_errmsg+val'});
 my $res = $validator->($data); # [$errmsg, $validated_val]
 
 # a sample valid data
 $data = 2021;
 my $res = $validator->($data); # => ["",2021]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("date::year*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"date::year*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('date::year*', name=>'DateYear')
     );
 }

 use My::Types qw(DateYear);
 DateYear->assert_valid($data);
