...
test 'MySQL STRSTARTS SARG with string literal' => sub {
	...
	like($sql, qr#SELECT `t0`.`subject` AS `s`, `t0`.`predicate` AS `p`, `t0`.`object` AS `o` FROM quad t0, term (\S+) WHERE [(]t0.graph IN [(][?][)][)] AND [(]`t0`.`object` = \1.term_id[)] AND [(]\1.`type` = [?][)] AND [(]LOCATE[(][?], \1.value[)] = [?][)] AND [(]\1.datatype_id IN [(][?], [?][)][)]#, 'generated SQL');
