...
# https://metacpan.org/release/MLEHMANN/Canary-Stability-2013/source/Stability.pm#L146
# 5.022 is the last supported perl for JSON::XS.
my $jx = $] >= 5.022000 ? 0 : eval {require JSON::XS;};
...
my @tests = (
    ['undef',
      undef,       SQL_INTEGER, SV_IS_UNDEF,     -1,                q{[null]}],
    ['invalid sql type',
     "99",         123456789,   0,               INVALID_TYPE,      q{["99"]}],
    ['non numeric cast to int',
     "aa",         SQL_INTEGER, 0,               NO_CAST_NO_STRICT, q{["aa"]}],
    ['non numeric cast to int (strict)',
     "aa",         SQL_INTEGER, DBIstcf_STRICT,  NO_CAST_STRICT,    q{["aa"]}],
...
unless ($pp) {
    ...
    push @tests, (
        ['very large int cast to int',
         "99999999999999999999", SQL_INTEGER, 0,              NO_CAST_NO_STRICT, q{["99999999999999999999"]}],
        ['very large int cast to int (strict)',
         "99999999999999999999", SQL_INTEGER, DBIstcf_STRICT, NO_CAST_STRICT,    q{["99999999999999999999"]}],
        ['float cast to int',
         "99.99",                SQL_INTEGER, 0,              NO_CAST_NO_STRICT, q{["99.99"]}],
        ['float cast to int (strict)',
         "99.99",                SQL_INTEGER, DBIstcf_STRICT, NO_CAST_STRICT,    q{["99.99"]}],
        ['float cast to double',
         "99.99",                SQL_DOUBLE,  0,              CAST_OK,           q{["99.99"]}]
       );
    if ($Config{ivsize} == 4) {
        push @tests,
            ['4 byte max unsigned int cast to int (ivsize=4)',
             "4294967296",       SQL_INTEGER, 0,              NO_CAST_NO_STRICT, q{["4294967296"]}];
    } elsif ($Config{ivsize} >= 8) {
        push @tests,
            ['4 byte max unsigned int cast to int (ivsize>8)',
             "4294967296",       SQL_INTEGER, 0,              CAST_OK,           q{["4294967296"]}];
    }
}
...
foreach my $test(@tests) {
    ...
    if ($dp) {
        my ($pv, $iv, $nv, $rv, $hm);
        ($pv, $iv, $nv, $rv, $hm) = Data::Peek::DDual($val);

        if ($test->[3] & DBIstcf_DISCARD_STRING) {
            #diag("D::P ",neat($pv), ",", neat($iv), ",", neat($nv),
            #     ",", neat($rv));
          SKIP: {
                skip 'DiscardString not supported in PurePerl', 1 if $pp;

                ok(!defined($pv), "dp: discard works, $test->[0]");
            };
        }
        if ($test->[2] == SQL_DOUBLE) {
            #diag("D::P ", neat($pv), ",", neat($iv), ",", neat($nv),
            #     ",", neat($rv));
            if ($test->[4] == CAST_OK) {
                ok(defined($nv), "dp: nv defined $test->[0]");
            } else {
                ok(!defined($nv) || !$nv, "dp: nv not defined $test->[0]");
            }
        }
    }
}
