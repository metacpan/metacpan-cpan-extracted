...
sub poll_device_token_p {
    ...
    Carp::croak('InternetData::oauth->poll_device_token: expected the device authorization hash')
...
sub _poll_p {
    ...
    return $self->{sleep_p}->($wait)->then(sub {
        ...
        return $self->_exchange_p({
            ...
            my $error = shift;
            die $error unless Scalar::Util::blessed($error) && $error->isa('InternetData::OauthError');
...
sub _refusal {
    ...
    my $body = $res->json;
    my $error = InternetData::Error->from_response($res->code, $res->headers, $body);
    die $error if $res->code >= 500 || ref $body ne 'HASH' || !_is_string($body->{error});
    die InternetData::OauthError->_from(
...
sub _assert_argument {
    ...
    for my $name (sort keys %arguments) {
        ...
        Carp::croak("InternetData::oauth->$method: expected $name as a string")
...
A refusal dies with a L<InternetData::OauthError>; anything else, a transport
failure or a server error, with the ordinary L<InternetData::Error>.
...
L<InternetData::OauthError> coded C<authorization_pending>. Never retried.
