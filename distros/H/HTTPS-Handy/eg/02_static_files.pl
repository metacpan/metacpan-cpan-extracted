######################################################################
# 02_static_files.pl - Static file serving over HTTPS with HTTPS::Handy
#
# Usage: perl eg/02_static_files.pl [port]
#
# Demonstrates:
#   - serve_static: serve files from a document root, over TLS
...
<body>
<h1>HTTPS::Handy Static File Demo</h1>
<p>This page is served over TLS from <code>htdocs/index.html</code>.</p>
<ul>
...
my $app = sub {
    ...
    return HTTPS::Handy->serve_static($env, $docroot, cache_max_age => $age);
};
...
sub _api {
    ...
    if ($path eq '/api/time') {
        ...
        my $json = "{\"time\":\"$ts\",\"secure\":$env->{'psgi.ssl'}}";
        return HTTPS::Handy->response_json($json);
    }

    if ($path eq '/api/mime') {
        my $ext  = HTTPS::Handy->url_decode($env->{QUERY_STRING});
        $ext     =~ s/^ext=//;
        my $mime = HTTPS::Handy->mime_type($ext);
        return HTTPS::Handy->response_text("$ext => $mime\n");
    }
...
print "Starting on https://127.0.0.1:$port/\n";
