...
that will be concatenated to the method in the span's name.
...
The span will be created with a temporary generic name that will only contain
the method of the request. This name will later be extended to include the
matching route, but this will happen I<after> any samplers have had a chance
