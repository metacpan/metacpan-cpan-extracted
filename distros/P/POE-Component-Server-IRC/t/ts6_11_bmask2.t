...
sub _bobbins_quit {
   $poe_kernel->post( $_[ARG0], 'send_to_server', { command => 'QUIT', params => [ 'Connection reset by fear' ] } );
   return;
}
...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix eq 'listen.server.irc' ) {
    ...
    if ( $params->[1] eq '+o' && $params->[2] eq 'harpo' ) {
      $poe_kernel->delay('_bobbins_quit', 7, $sender->ID() );
    }
    return;
  }
  return;
