...
use Google::Ads::AdWords::v201806::ExemptionRequest;
...
sub handle_policy_violation_error {
  ...
  my $exemptable_text_ad = Google::Ads::AdWords::v201806::ExpandedTextAd->new({
  ...
  my $exemptable_text_ad_group_ad =
    Google::Ads::AdWords::v201806::AdGroupAd->new({
      ...
      if ($error->get_ApiError__Type() =~ "PolicyViolationError") {
        ...
        if ($error->get_isExemptable()) {
          ...
          $operation->set_exemptionRequests([
              new Google::Ads::AdWords::v201806::ExemptionRequest(
                {key => $error->get_key()})]);
        } else {
