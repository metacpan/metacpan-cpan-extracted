package UI::Various::RichTerm::Window;

# Author, Copyright and License: see end of file

=head1 NAME

UI::Various::RichTerm::Window - concrete implementation of L<UI::Various::Window>
...
require Exporter;
our @ISA = qw(UI::Various::Window UI::Various::RichTerm::container);
our @EXPORT_OK = qw();
...
Handle the action of the UI element.  For a C<RichTerm>'s window this means
a loop of printing the window's elements and allowing to select one of the
active ones for processing until the window is exited, changed or destroyed.

=head3 returns:

C<+>/C<-> for next/previous window, C<0> for simple exit and C<undef> after
destruction
