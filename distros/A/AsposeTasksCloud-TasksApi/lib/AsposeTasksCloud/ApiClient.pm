package AsposeTasksCloud::ApiClient;
...
sub new
{
	...
	if(not defined $AsposeTasksCloud::Configuration::app_sid or $AsposeTasksCloud::Configuration::app_sid eq ''){
		croak("Aspose Cloud App SID key is missing.");
    }
	
	if(not defined $AsposeTasksCloud::Configuration::api_key or $AsposeTasksCloud::Configuration::api_key eq ''){
    ...
    my (%args) = (
        ...
        'base_url' => $AsposeTasksCloud::Configuration::api_server,
        @_
    );
...
sub call_api {
	...
	$resource_path =~ s/\Q{appSid}\E/$AsposeTasksCloud::Configuration::app_sid/g;
  
    my $_url = $self->{base_url} . $resource_path;
  
    # update signature
    my $signature = $self->sign($_url, $AsposeTasksCloud::Configuration::app_sid, $AsposeTasksCloud::Configuration::api_key);
    ...
    $self->{ua}->timeout($self->{http_timeout} || $AsposeTasksCloud::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $AsposeTasksCloud::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("AsposeTasksCloud::Object::$class")->new;
        if (ref $data eq "HASH") {
