...
is($entry->in_reply_to->[0]->attr('href'),
   'http://sojolicious.example/blog/1.html',
   'Get in_reply_to'
);
...
ok($entry->in_reply_to('urn:entry:2' => {
  href => 'http://sojolicious.example/blog/2.atom',
...
is($entry->in_reply_to->[1]->attr('href'),
   'http://sojolicious.example/blog/2.atom',
   'Get in_reply_to'
);

ok($atom->in_reply_to('urn:feed:1' => {
  href => 'http://sojolicious.example/blog-replies.html',
  }), 'Add entry');

is($atom->in_reply_to->[0]->attr('href'),
   'http://sojolicious.example/blog-replies.html',
   'Get in_reply_to'
);

ok($entry->replies('http://sojolicious.example/entry/1/replies' => {
  count   => 5,
...
ok($atom->replies('http://sojolicious.example/feed/replies' => {
  count   => 18
}), 'Set replies');
