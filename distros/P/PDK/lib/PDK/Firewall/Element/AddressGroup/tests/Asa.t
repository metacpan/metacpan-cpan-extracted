...
ok(
  do {
    eval {
      $addrGroup
        = PDK::Firewall::Element::AddressGroup::Asa->new(fwId => 1, addrGroupName => 'zzz', zone => 'xxx', description => 'yyy');
    };
    warn $@ if !!$@;
    $addrGroup->isa('PDK::Firewall::Element::AddressGroup::Asa');
  },
  ' 生成 PDK::Firewall::Element::AddressGroup::Asa 对象'
);
  ...
  do {
    ...
    eval {
      ...
      $addrGroup
        = PDK::Firewall::Element::AddressGroup::Asa->new(fwId => 1, addrGroupName => 'zzz', zone => 'xxx', description => 'yyy');
      $addrGroup1
        = PDK::Firewall::Element::AddressGroup::Asa->new(fwId => 1, addrGroupName => 'mmm', zone => 'nnn', description => 'jjj');
    };
    warn $@ if !!$@;
    $addrGroup->addAddrGroupMember("member1", $address);
