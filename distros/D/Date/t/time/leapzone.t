...
subtest 'near leap second time' => sub {
    tzset('right/UTC');
    is_deeply([&localtime(1230768022)], [59, 59, 23, 31, 11, 2008, 3, 365, 0]);
    is_deeply([&localtime(1230768023)], [60, 59, 23, 31, 11, 2008, 3, 365, 0]);
    is_deeply([&localtime(1230768024)], [0, 0, 0, 1, 0, 2009, 4, 0, 0]);
    my ($isdst,$Y,$M,$D,$h,$m,$s) = (1,2008,11,31,23,59,59);
    is(timelocal($s,$m,$h,$D,$M,$Y,$isdst), 1230768022);
    is(timelocaln($s,$m,$h,$D,$M,$Y,$isdst), 1230768022);
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2008,11,31,23,59,59]);
    ...
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2008,11,31,23,59,60]);
    ($isdst,$Y,$M,$D,$h,$m,$s) = (1,2009,0,1,0,0,0);
    is(timelocal($s,$m,$h,$D,$M,$Y,$isdst), 1230768024);
    is(timelocaln($s,$m,$h,$D,$M,$Y,$isdst), 1230768024);
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2009,0,1,0,0,0]);
};

subtest 'leap second inside transitions' => sub {
    tzset('right/Europe/Moscow');
    is_deeply([&localtime(1230768021)], [58, 59, 2, 1, 0, 2009, 4, 0, 0]);
    is_deeply([&localtime(1230768022)], [59, 59, 2, 1, 0, 2009, 4, 0, 0]);
    is_deeply([&localtime(1230768023)], [60, 59, 2, 1, 0, 2009, 4, 0, 0]);
    is_deeply([&localtime(1230768024)], [0, 0, 3, 1, 0, 2009, 4, 0, 0]);
    is_deeply([&localtime(1230768025)], [1, 0, 3, 1, 0, 2009, 4, 0, 0]);
    my ($isdst,$Y,$M,$D,$h,$m,$s) = (1,2009,0,1,2,59,58);
    is(timelocal($s,$m,$h,$D,$M,$Y,$isdst), 1230768021);
    is(timelocaln($s,$m,$h,$D,$M,$Y,$isdst), 1230768021);
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2009,0,1,2,59,58]);
    ($isdst,$Y,$M,$D,$h,$m,$s) = (1,2009,0,1,2,59,59);
    is(timelocal($s,$m,$h,$D,$M,$Y,$isdst), 1230768022);
    is(timelocaln($s,$m,$h,$D,$M,$Y,$isdst), 1230768022);
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2009,0,1,2,59,59]);
    ($isdst,$Y,$M,$D,$h,$m,$s) = (1,2009,0,1,2,59,60);
    is(timelocal($s,$m,$h,$D,$M,$Y,$isdst), 1230768023);
    is(timelocaln($s,$m,$h,$D,$M,$Y,$isdst), 1230768023);
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2009,0,1,2,59,60]);
    ($isdst,$Y,$M,$D,$h,$m,$s) = (1,2009,0,1,3,0,0);
    is(timelocal($s,$m,$h,$D,$M,$Y,$isdst), 1230768024);
    is(timelocaln($s,$m,$h,$D,$M,$Y,$isdst), 1230768024);
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2009,0,1,3,0,0]);
    ...
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2009,0,1,3,0,1]);
};

subtest 'when last transition is leap second' => sub {
    is_deeply([&localtime(1341100822)], [58, 59, 3, 1, 6, 2012, 0, 182, 0]);
    is_deeply([&localtime(1341100823)], [59, 59, 3, 1, 6, 2012, 0, 182, 0]);
    is_deeply([&localtime(1341100824)], [60, 59, 3, 1, 6, 2012, 0, 182, 0]);
    is_deeply([&localtime(1341100825)], [0, 0, 4, 1, 6, 2012, 0, 182, 0]);
    is_deeply([&localtime(1341100826)], [1, 0, 4, 1, 6, 2012, 0, 182, 0]);
    my ($isdst,$Y,$M,$D,$h,$m,$s) = (1,2012,6,1,3,59,58);
    is(timelocal($s,$m,$h,$D,$M,$Y,$isdst), 1341100822);
    is(timelocaln($s,$m,$h,$D,$M,$Y,$isdst), 1341100822);
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2012,6,1,3,59,58]);
    ($isdst,$Y,$M,$D,$h,$m,$s) = (1,2012,6,1,3,59,59);
    is(timelocal($s,$m,$h,$D,$M,$Y,$isdst), 1341100823);
    is(timelocaln($s,$m,$h,$D,$M,$Y,$isdst), 1341100823);
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2012,6,1,3,59,59]);
    ($isdst,$Y,$M,$D,$h,$m,$s) = (1,2012,6,1,3,59,60);
    is(timelocal($s,$m,$h,$D,$M,$Y,$isdst), 1341100824);
    is(timelocaln($s,$m,$h,$D,$M,$Y,$isdst), 1341100824);
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2012,6,1,3,59,60]);
    ($isdst,$Y,$M,$D,$h,$m,$s) = (1,2012,6,1,4,0,0);
    is(timelocal($s,$m,$h,$D,$M,$Y,$isdst), 1341100825);
    is(timelocaln($s,$m,$h,$D,$M,$Y,$isdst), 1341100825);
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2012,6,1,4,0,0]);
    # check normalization (120 != 60+60)
    ...
    is_deeply([$isdst,$Y,$M,$D,$h,$m,$s], [0,2012,6,1,4,0,1]);
};
