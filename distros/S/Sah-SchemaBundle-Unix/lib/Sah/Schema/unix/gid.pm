...
MARKDOWN
    ...
    'prefilters' => ['Unix::convert_unix_group_to_gid'],
...
Sah::Schema::unix::gid - Group identifier (GID)

=head1 VERSION

This document describes version 0.022 of Sah::Schema::unix::gid (from Perl distribution Sah-SchemaBundle-Unix), released on 2024-11-15.
...
Used prefilters: L<Unix::convert_unix_group_to_gid|Data::Sah::Filter::perl::Unix::convert_unix_group_to_gid>
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("unix::gid*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("unix::gid", {return_type=>'str_errmsg'});
 ...
 $data = -1;
 my $errmsg = $validator->($data); # => "Unknown Unix group '-1'"

Often a schema has coercion rule or default value rules, so after validation the
validated value will be different from the original. To return the validated
(set-as-default, coerced, prefiltered) value:

 my $validator = gen_validator("unix::gid", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = -1;
 my $res = $validator->($data); # => ["Unknown Unix group '-1'",-1]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("unix::gid*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"unix::gid*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('unix::gid*', name=>'UnixGid')
     );
 }

 use My::Types qw(UnixGid);
 UnixGid->assert_valid($data);
