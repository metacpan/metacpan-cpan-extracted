...
=item   getAttachments (default 0)

If Attachment fields are specified on the 'Fields' option, the field will be represented in the returned
datastructure as a hash reference with these keys: 'compSize', 'origSize', 'name' and 'value'. If getAttachments
is set to 0, the 'value' field will contain the API message "Use ars_GetEntryBLOB or OO->getAttachment to extract the attachment".

If getAttachments is set to 1, the Query function will fetch the attachment content and insert it on the
'buffer' hash key
