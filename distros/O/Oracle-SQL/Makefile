# This Makefile is for the Oracle::SQL extension to perl.
#
...
#     ABSTRACT => q[Perl extension for building SQL statements.]
#     AUTHOR => q[geotiger2001@yahoo.com]
#     DISTNAME => q[Oracle-SQL-0.01.tar.gz]
#     NAME => q[Oracle::SQL]
#     PREREQ_PM => { Test::More=>q[0.45], Test::Harness=>q[0.1] }
#     VERSION => q[0.01]
#     test => { TESTS=>q[t/SQL.t] }
...
# These definitions are from config.sh (via /usr/local/lib/perl5/5.8.7/sun4-solaris/Config.pm)
...
INSTALLARCHLIB = $(PERLPREFIX)/lib/perl5/5.8.7/sun4-solaris
DESTINSTALLARCHLIB = $(DESTDIR)$(INSTALLARCHLIB)
INSTALLSITEARCH = $(SITEPREFIX)/lib/perl5/site_perl/5.8.7/sun4-solaris
...
PERL_ARCHLIB = /usr/local/lib/perl5/5.8.7/sun4-solaris
LIBPERL_A = libperl.a
FIRST_MAKEFILE = Makefile
MAKEFILE_OLD = $(FIRST_MAKEFILE).old
MAKE_APERL_FILE = $(FIRST_MAKEFILE).aperl
PERLMAINCC = $(CC)
PERL_INC = /usr/local/lib/perl5/5.8.7/sun4-solaris/CORE
...
INST_LIBDIR      = $(INST_LIB)/Oracle
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Oracle
...
DISTNAME = Oracle-SQL-0.01.tar.gz
DISTVNAME = Oracle-SQL-0.01.tar.gz-0.01
...
$(INST_AUTODIR)/.exists :: /usr/local/lib/perl5/5.8.7/sun4-solaris/CORE/perl.h
	$(NOECHO) $(MKPATH) $(INST_AUTODIR)
	$(NOECHO) $(EQUALIZE_TIMESTAMP) /usr/local/lib/perl5/5.8.7/sun4-solaris/CORE/perl.h $(INST_AUTODIR)/.exists

	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_AUTODIR)

$(INST_LIBDIR)/.exists :: /usr/local/lib/perl5/5.8.7/sun4-solaris/CORE/perl.h
	$(NOECHO) $(MKPATH) $(INST_LIBDIR)
	$(NOECHO) $(EQUALIZE_TIMESTAMP) /usr/local/lib/perl5/5.8.7/sun4-solaris/CORE/perl.h $(INST_LIBDIR)/.exists

	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_LIBDIR)

$(INST_ARCHAUTODIR)/.exists :: /usr/local/lib/perl5/5.8.7/sun4-solaris/CORE/perl.h
	$(NOECHO) $(MKPATH) $(INST_ARCHAUTODIR)
	$(NOECHO) $(EQUALIZE_TIMESTAMP) /usr/local/lib/perl5/5.8.7/sun4-solaris/CORE/perl.h $(INST_ARCHAUTODIR)/.exists
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  SQL.pm $(INST_MAN3DIR)/Oracle::SQL.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  $(INST_LIB)/Oracle/SQL.pm $(MAKEFILE_OLD) $(FIRST_MAKEFILE)
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         Oracle-SQL-0.01.tar.gz' >> META.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl extension for building SQL statements.</ABSTRACT>' >> $(DISTNAME).ppd
