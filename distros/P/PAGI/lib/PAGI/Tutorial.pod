...
PAGI (Perl Asynchronous Gateway Interface) is a B<protocol>, not a framework.
It is a small, well-defined contract for hooking an asynchronous Perl
application up to an asynchronous server: your application is an C<async sub>
that receives a connection scope and two coderefs (one to receive events from
the client, one to send events back), and that is essentially the whole
specification. PAGI also defines a loop-agnostic way to model custom
asynchronous events (HTTP, WebSocket, Server-Sent Events, lifespan, and your
own), so the same application code runs unchanged on any conforming server,
under any event loop.

This tutorial teaches the protocol itself. To run the examples you need a
server, and we use L<PAGI::Server> (the reference implementation, from the
C<PAGI-Server> distribution) because it is a complete, fully-featured PAGI
server. Nothing in the protocol depends on it; any conforming server would do.

The only hard requirement for a PAGI application is L<Future::AsyncAwait> for
async/await syntax. The event loop (L<IO::Async>, in PAGI::Server's case) is an
implementation detail of the server, not of your application.

Along the way we occasionally point at the C<PAGI-Tools> distribution, which
provides convenience helpers (middleware, routers, request/response sugar,
ready-made apps). Those are B<entirely optional>: ordinary PAGI applications
and helpers built I<on top of> the protocol, never required to write a complete
PAGI application. For a guide to them, see L<PAGI::Tools::Tutorial>.
...
PAGI (Perl Asynchronous Gateway Interface) is a B<protocol> for building
asynchronous web applications in Perl: a specification for how an async
application and an async server talk to each other. It is a spiritual successor
to PSGI that embraces modern async/await patterns using L<Future::AsyncAwait>
and provides first-class support for WebSockets, Server-Sent Events, and
HTTP/1.1.
...
Many applications benefit from both the async model (for real-time endpoints like WebSocket and SSE) and pre-forking (for CPU-bound request/response work). The reference server, L<PAGI::Server>, combines them with its worker mode (C<--workers N>): several pre-forked workers, each running the async event loop. That is a feature of the server, not of the PAGI protocol.
...
=item * B<Perl 5.18 or later> - the floor for the PAGI distributions (the specification, the reference server, and the toolkit)
...
B<Note:> A bare PAGI application's only hard dependency is C<Future::AsyncAwait>
(for async/await syntax), which supports Perl back to 5.16; the event loop
(L<IO::Async>) is an implementation detail of the server, and using C<Future::IO>
for timers and I/O keeps your code portable across servers. In practice, though,
you run on something: the specification, the reference server L<PAGI::Server>,
and the L<PAGI-Tools|PAGI::Tools> toolkit all require B<Perl 5.18 or later>, and
another server or a higher-level framework may require a newer Perl still. The
minimum version you need is the floor of whatever you build on.
...
Installing C<PAGI> currently also pulls in the C<PAGI-Server> reference server
and the C<PAGI-Tools> helpers for backward compatibility during the transition;
see L<PAGI/INSTALLATION AND BACKWARD COMPATIBILITY>.

=head3 About pagi-server

Throughout this tutorial, we'll use C<pagi-server> to run our examples. L<PAGI::Server>
is the reference implementation of a PAGI server - it's a fully-featured async HTTP
server built on L<IO::Async> that supports HTTP/1.1, WebSockets, Server-Sent Events,
TLS, and multi-worker mode.

At the time of writing, C<pagi-server> is the only PAGI-compatible server available.
However, the PAGI specification is designed to be server-agnostic, so future servers
(perhaps built on different event loops like L<Mojo::IOLoop> or L<AnyEvent>) could
run the same application code without modification.
...
=item 3. C<die "Expected http scope" unless $scope-E<gt>{type} eq 'http'> - The server calls your app for different event types (HTTP requests, WebSocket, lifespan). Applications must throw an exception for unrecognized scope types to prevent silent protocol mismatches.
...
specification violations early and provides helpful error messages. Note that
Lint lives in the C<PAGI-Tools> distribution: pagi-server applies it only when
PAGI-Tools is installed, and otherwise prints a reminder that you can install
PAGI-Tools to enable it and runs without it.
...
or any non-TTY environment. Use C<--access-log FILE> or C<--access-log FILE>
to enable logging.
...
After determining the mode, pagi-server sets C<PAGI_ENV> to the resolved
...
=item * C<Future::IO-E<gt>sleep($secs)> instead of C<sleep()> - loop-agnostic!
...
=item 2. B<Use a Worker Pool for CPU-bound Work>

For CPU-intensive operations (password hashing, image processing), offload to
a subprocess. See L<PAGI::Cookbook> for patterns using L<IO::Async::Function>
or external job queues.
...
B<Important:> Worker mode does NOT fix blocking operations. Each worker still runs a single-threaded event loop. If you do a blocking operation in a worker, ALL connections on that worker freeze (including any WebSocket or SSE connections). To avoid blocking, use async libraries as described in section 1.4.
...
Use a shared backend for state. For example with Redis (see L<PAGI::Cookbook>
for complete Redis examples):
...
=item * End the session with C<websocket.close>, or leave the loop on C<websocket.disconnect>; returning from an accepted socket with neither is an incomplete response and the server closes it with code C<1011>
...
To reject a handshake, send an ordinary HTTP response (C<http.response.start>/C<.body> before
C<websocket.accept>) such as a C<401>. See the cookbook's
"Rejecting a handshake with an HTTP response" and
...
=item * Send C<sse.start> first with headers

=item * Send C<sse.send> events with C<data> field

=item * Optional fields: C<event> (event type), C<id> (event ID), C<retry> (reconnect interval)

=item * End a finite stream with C<sse.close>; returning without it is an incomplete response
...
Streaming raises a question: what happens when you produce faster than the
client can read? The server buffers the overflow, and PAGI lets you B<see and
act on> that backlog through C<$scope-E<gt>{'pagi.transport'}> -- the
server-side analogue of the browser's C<WebSocket.bufferedAmount>.
C<buffered_amount> reports the bytes queued but not yet on the wire;
C<high_water_mark> and C<low_water_mark> bound the backpressure band; and the
C<on_high_water>/C<on_drain> callbacks let an event-driven producer pause and
resume. Use it to conflate a live feed -- skipping stale frames so a slow client
stays current -- or to throttle a source you cannot pace by awaiting. The handle
is optional: a server that cannot measure its buffer omits it, so guard for
C<undef>.

For a runnable demonstration see F<examples/13-flow-control>, the B<Flow Control>
recipes in L<PAGI::Cookbook>, and L<PAGI::Spec::Www/"Transport Flow Control"> for
the full interface.

=head2 2.7 Lifespan Protocol (Application Lifecycle)

The lifespan protocol allows your application to initialize resources on startup and clean them up on shutdown. The key feature is C<$scope-E<gt>{state}> - a hash for sharing resources (database pools, caches) from startup with every request in a worker process.

Each request scope receives a B<shallow copy> of this hash: the I<values> you store -- a database pool, a cache handle -- are shared by reference with every request, but the top-level keys are private to each copy. So store your shared resources B<once> at startup and use them through their references; do not expect a top-level key you assign B<in a request> (or reassign in the lifespan once requests are running) to be seen elsewhere. See L<PAGI::Spec::Lifespan/Lifespan State>.
...
=head2 2.8 Middleware: Wrapping an Application

Middleware is not a PAGI-Tools feature -- it is pure protocol. A middleware is
just a function that takes an application and returns a new application with the
same C<($scope, $receive, $send)> signature. Because the wrapper I<is> an
application, you can stack middleware as deeply as you like, and the server
cannot tell the difference between a bare app and one wrapped in ten layers.

=head3 The Basic Shape
  ...
  # Wrap a bare app:
  my $wrapped = with_logging($app);

The outer C<sub> runs once, when you build the application; the inner
C<async sub> runs once per connection. Anything before C<< await $app->(...) >>
happens on the way in, anything after happens on the way out.

=head3 Modifying the Scope (Downward Only)

A middleware may add to C<$scope> for the layers below it, but it B<must not>
mutate the original in place. Clone first:
  ...
  sub with_request_id {
      ...
      return async sub {
          my ($scope, $receive, $send) = @_;
          my $inner = { %$scope };               # shallow clone
          $inner->{'myapp.request_id'} = ++$counter;
          await $app->($inner, $receive, $send);
      };
  }

The clone is B<shallow>, and deliberately so. Top-level keys you set on the
clone are private to this layer and the layers below it; they never leak upward
to the parent layers or the server. But values reached I<through> a reference --
the C<pagi.connection> object, the lifespan C<state> namespace -- are shared by
every layer, because the clone copies the reference, not what it points at. The
rule of thumb: B<scope edits flow downward only; shared state lives behind a
reference.> See L<PAGI::Building/MIDDLEWARE> for the full sharing model and why
the clone is shallow rather than deep.

=head3 Wrapping C<$send> to Rewrite the Response

To observe or change what the inner app sends, wrap C<$send> and pass the
wrapper down:
...
=head2 2.10 Async Patterns and Pitfalls
...
=item 3. The C<sleep> callback runs 1 second later
...
=head1 BEYOND THE PROTOCOL: OPTIONAL CONVENIENCES

Everything above is the PAGI protocol: the complete contract between an async
application and an async server. You can build any PAGI application with
nothing more than what you have seen here: an C<async sub>, C<$scope>,
C<$receive>, C<$send>, and L<Future::AsyncAwait>.

The C<PAGI-Tools> distribution provides B<optional> convenience helpers on top
of the protocol: middleware (logging, sessions, CORS, compression, and more),
request and response sugar (L<PAGI::Request>, L<PAGI::Response>), WebSocket and
SSE helpers, routers, and ready-made applications. None of it is required; it
exists to save boilerplate when you want it. Every piece is an ordinary PAGI
application or a plain wrapper around the C<$scope>/C<$receive>/C<$send>
protocol, so you can read, replace, or ignore any of it.

For a guide to those helpers, see L<PAGI::Tools::Tutorial>.

=head1 PART 3: REFERENCE

=head2 3.1 Scope Reference

The C<$scope> hashref contains connection metadata:

=head3 Async Operations Without Loop Access

For async operations like delays, use L<Future::IO> which is loop-agnostic:

  use Future::IO;
  await Future::IO->sleep(5);  # Works with any event loop

This keeps your application code portable across different PAGI server
implementations.
...
Send:
  ...
  { type => 'sse.send', data => $text, event => 'name', id => '1' }
  { type => 'sse.close', reason => 'done' }
...
=item * L<PAGI::PSGI> - if you're coming from PSGI

=item * L<PAGI::Building> - if you're building a framework on PAGI

=item * Browse F<examples/> for complete working applications, including a small web framework built on PAGI

=item * Read the specification: L<PAGI::Spec> and the protocol documents L<PAGI::Spec::Www>, L<PAGI::Spec::Lifespan>, L<PAGI::Spec::Server>.

=item * L<PAGI::Tools::Tutorial> - Guide to the optional convenience helpers (middleware, routers, request/response sugar, ready-made apps)
...
=item * L<PAGI> - The PAGI specification distribution and overview

=item * L<PAGI::Cookbook> - Worked, runnable recipes for the protocol

=item * L<PAGI::PSGI> - Coming to PAGI from PSGI

=item * L<PAGI::Building> - Building frameworks on PAGI

=item * L<PAGI::Spec> - The full PAGI specification

=item * L<PAGI::Server> - The reference server (C<PAGI-Server> distribution)

=item * L<PAGI::Tools::Tutorial> - Guide to optional helpers (C<PAGI-Tools> distribution)
