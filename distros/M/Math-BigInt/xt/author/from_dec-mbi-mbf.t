...
subtest '$x = Math::BigInt -> from_dec("4.5");'
  => sub {
      # this must not upgrade

      my $x = Math::BigInt -> from_dec("4.5");

      is(ref($x), "Math::BigInt", "class");
      is($x, "NaN", "value");
  };

subtest '$x = Math::BigFloat -> from_dec("4");'
  => sub {
      # this must not downgrade

      my $x = Math::BigFloat -> from_dec("4");

      is(ref($x), "Math::BigFloat", "class");
      is($x, "4", "value");
  };

subtest '$x = Math::BigInt -> new("4"); $q = $x -> from_dec("4.5");'
  => sub {
      # this must not upgrade
...
subtest '$x = Math::BigFloat -> new("4.5"); $q = $x -> from_dec("4");'
  => sub {
      # this must not downgrade
