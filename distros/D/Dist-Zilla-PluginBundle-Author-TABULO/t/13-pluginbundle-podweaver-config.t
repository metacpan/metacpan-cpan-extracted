...
cmp_deeply(
    $tzil->plugin_named('@Author::TABULO/PodWeaver'),
    noclass(superhashof({
            ...
            plugins => supersetof(
                {
                    ...
                    config => superhashof({
                        'Dist::Zilla::Plugin::PodWeaver' => superhashof({
                            config_plugins => [ '@Author::TABULO' ],
                            # check that all plugins came from '@Author::TABULO'
                            plugins => array_each(
                                ...
                                code(sub {
                                    ...
                                    $_[0]->{name} =~ m{^\@(CorePrep|Author::TABULO)/}
...
like(
    $module,
    qr/=head1 COPYRIGHT AND LICENSE/,
...
SKIP: {
    ...
    foreach my $bundle_plugin_config (Pod::Weaver::PluginBundle::Author::TABULO->mvp_bundle_config)
    {
