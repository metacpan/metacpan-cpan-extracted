...
Perl-Critic-Policy-RegularExpressions-PreventUselessMetacharacterEscapes is one
Perl::Critic policy, which says that C<\Q...\E> in a regex should wrap the value
being interpolated and nothing else.  The whole of it is
F<lib/Perl/Critic/Policy/RegularExpressions/PreventUselessMetacharacterEscapes.pm>
and F<t/main.t>; read the POD first, since it carries the argument for the rule
and the cases either side of it.
...
| | |
|---|---|
| `perldoc Perl-Critic-Policy-RegularExpressions-PreventUselessMetacharacterEscapes` | what this is for and how a caller uses it |
...
There is nothing else to read.  A policy is small enough that the module and its
test are the documentation, and anything true about the rule belongs in the POD
where `perldoc` and the generated README both find it.
