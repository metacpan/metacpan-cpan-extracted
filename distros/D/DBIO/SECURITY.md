This is the Security Policy for the Perl DBIO distribution.
...
[DBIO on CPAN](https://metacpan.org/dist/DBIO) or the main branch of the
[DBIO git repository](https://codeberg.org/dbio/dbio).
...
DBIO is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO are covered by this policy.
...
they affect DBIO, or DBIO can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO, or plugins to it that are not included with the DBIO
distribution) are not covered by this policy.

## Supported Versions of DBIO

Only the latest release of DBIO will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO will run on older versions of Perl. If a
...
that are required for DBIO to work. However, some of these
