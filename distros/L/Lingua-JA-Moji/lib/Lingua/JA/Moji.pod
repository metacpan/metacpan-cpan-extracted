...
(This example is included as L<F<synopsis.pl>|https://fastapi.metacpan.org/source/BKB/Lingua-JA-Moji-0.63/examples/synopsis.pl> in the distribution.)
...
(This example is included as L<F<syn-kana.pl>|https://fastapi.metacpan.org/source/BKB/Lingua-JA-Moji-0.63/examples/syn-kana.pl> in the distribution.)


=head1 VERSION

This document describes Lingua::JA::Moji version 0.63
corresponding to git commit L<8cd0fa037206a9fa1a2cd6e648ccb36d0ec6eefc|https://github.com/benkasminbullock/Lingua-JA-Moji/commit/8cd0fa037206a9fa1a2cd6e648ccb36d0ec6eefc> made on Wed Aug 12 09:31:10 2026 +0900.
...
=head2 is_romaji_semistrict 
    ...
    # The following line returns a canonical version of the romaji
...
stuff which "is_romaji" allows. The return value is a canonical form
of the romaji.
...
=item truck
     ...
     kana2romaji ("じょうほう", {style => 'common', truck => 1});

Make じょう into jyo rather than jo. This replicates the romanisation
used by many Japanese commercial vehicles.

=item wikipedia

     kana2romaji ("しいたけ", {wikipedia => 1});

The "shii" in "shiitake" becomes shītake with a macron over the
i. This incorrect romanization is used in Wikipedia articles.
...
(This example is included as L<F<onbiki.pl>|https://fastapi.metacpan.org/source/BKB/Lingua-JA-Moji-0.63/examples/onbiki.pl> in the distribution.)
...
Added L</is_small> and L</is_katakana> functions.

=item 0.61

L</kana2romaji> now supports "Wikipedia romanisation", the false
format used by the English-language Wikipedia to romanise Japanese.

A bug was fixed in L</kana2romaji>'s kunrei romanisation.

=item 0.62

L</is_romaji_semistrict> now returns a canonical form as a positive
result rather than just "1".

=item 0.63

A bug in the Perl regexp classes C<InHiragana> and C<Katakana> was
found where they would match the punctuation characters C<U+3000> to
C<U+3040>, so the regexp class L</InKana> from this module was not
working correctly. This block of false matches was removed.
