...
=cut
 ...
 sub binding_index_aref {
	...
	$index[1] = 7; # inbound/outbound item is bound
...
=cut
 ...
 sub get_binding_index_aref{
	...
	if ($segywrite_spec->{_binding_index_aref} ) {
		...
		print("segywrite_spec, get_binding_index_aref, missing binding_index_aref\n");
		return();
	}
...
=cut
 ...
 sub get_file_dialog_type_aref{
	...
	if ($segywrite_spec->{_file_dialog_type_aref} ) {
		...
		print("segywrite_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return();
	}
 ...
 sub get_incompatibles{
	...
	if ($len_1_needed >= 1) {
		...
		for (my $i=0; $i < $len_1_needed; $i++) {
			...
			print("segywrite, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
=cut
 ...
 sub prefix_aref {
	...
	# label 8 in GUI is input/output xx_file and needs a home directory
...
=cut
 ...
 sub suffix_aref {
	...
	# label 8 in GUI is input/ouput xx_file and needs a home directory
