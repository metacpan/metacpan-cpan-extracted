...
sub client_input {
  ...
  if ( $cmd eq '604' ) {
    pass("IRC_$cmd");
    $poe_kernel->post( 'client2', 'send_to_server', { command => 'NICK', params => [ 'hubbarb' ], colonify => 0 } );
    return;
  }
  if ( $cmd eq '603' ) {
    pass("IRC_$cmd");
  ...
  if ( $cmd eq '607' ) {
    ...
    $poe_kernel->post( 'client3', 'send_to_server', { command => 'QUIT', params => [ 'Buh-bye' ] } );
    return;
  }
  if ( $cmd eq '601' ) {
    pass("IRC_$cmd");
    $poe_kernel->post( 'client2', 'send_to_server', { command => 'QUIT', params => [ 'Ta ta' ] } );
    return;
  }
  return;
}
