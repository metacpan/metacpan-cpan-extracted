...
=item B<--group E<lt>name=queryE<gt>>
...
Contains a value of either 'bin' or 'lib', for pods found in the bin directories or lib directories.
...
groups in ':std' can be used separately, e.g. as ':std-core'.
...
Generally the pods named 'perlxxx' (in top and pods:: namespace).
...
All pods found in the bin directories, plus the 'pp' module.
