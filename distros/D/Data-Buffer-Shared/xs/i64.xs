...
new(char* class, SV* path, UV capacity, UV file_mode = 0600)
    CODE:
        ...
        BufHandle* buf = buf_i64_create(p, (uint64_t)capacity, (mode_t)file_mode, errbuf);
...
DESTROY(SV* self_sv)
    CODE:
        if (!sv_isobject(self_sv) || !sv_derived_from(self_sv, "Data::Buffer::Shared::I64")) return;
...
set_slice(SV* self_sv, UV from, ...)
    CODE:
        ...
        REEXTRACT_BUF("Data::Buffer::Shared::I64", self_sv);
...
unlink(SV* self_or_class, ...)
    CODE:
        const char *p;
        if (sv_isobject(self_or_class) && sv_derived_from(self_or_class, "Data::Buffer::Shared::I64")) {
...
get_raw(SV* self_sv, UV byte_off, UV nbytes)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I64", self_sv);
        if (!buf_i64_raw_in_bounds(h, (uint64_t)byte_off, (uint64_t)nbytes))
...
new_memfd(char* class, SV* name, UV capacity)
    CODE:
        ...
        BufHandle* buf = buf_i64_create_memfd(nm, (uint64_t)capacity, errbuf);
        if (!buf) croak("Data::Buffer::Shared::I64: %s", errbuf[0] ? errbuf : "unknown error");
        REREAD_CLASS();
        RETVAL = sv_setref_pv(newSV(0), class, (void*)buf);
    OUTPUT:
...
sync(SV* self_sv)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I64", self_sv);
        if (buf_msync(h) != 0) croak("Data::Buffer::Shared::I64 sync: %s", strerror(errno));
