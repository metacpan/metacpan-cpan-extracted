...
#
# The module holds the 2048 words as data in this file, one word per
# line, so a person can diff them against share/fuguseed/english.txt
...
# exist in it.
#
# Every method is a class method, and the module keeps no state.

# DIGEST:
#	The SHA-256 of the source list (LIST-SOURCE-1), as lower-case
#	hex. t/fuguseed/list.t holds the embedded words and the share
#	file to this value, so both copies stay the source list.
use constant DIGEST =>
...
# $class->word($index):
#	The word of the 0-based index $index, or undef outside 0 to
#	2047.
sub word ( $, $index )
{
	return unless defined $index;
	return unless $index =~ /\A[0-9]+\z/ && $index <= $#WORDS;
	return $WORDS[$index];
}

# $class->index($word):
#	The 0-based index of $word, or undef for an unknown word. The
#	name repeats a built-in function of perl, so a caller reaches
#	this one through the arrow only.
sub index ( $, $word )
{
