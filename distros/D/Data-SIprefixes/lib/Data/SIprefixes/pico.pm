...
Data::SIprefixes::pico - This provides micro matching for Data::SIprefixes.
...
=head1 SYNOPSIS
    ...
    use Data::SIprefixes::pico;
    
    my $prefix=Data::SIprefixes::pico->new;

    my $origMeasure='picometer';
     
    my $measure=$prefix->longMatch( $origMeasure );
    my $long;
    if ( $prefix->error ){
        warn('error:'.$prefix->error.': '.$prefix->errorString);
...
sub longMatch{
	...
	if ( $measure=~/^pico/ ){
		...
		if ( $measure =~ /^ / ){
			$self->{error}=2;
			$self->{errorString}='Space found after prefix, /^pico/, in "'.$origMeasure.'"' ;
			$self->warn;
			return undef;
		}
...
sub shortMatch{
	...
	# avoids possible collision
	if ( $measure eq 'pascal' ){
		return $measure;
	}
    ...
    if ( $measure=~/^p/ ){
        ...
        if ( $measure =~ /^ / ){
            $self->{error}=2;
            $self->{errorString}='Space found after prefix, /^p/, in "'.$origMeasure.'"' ;
            $self->warn;
            return undef;
        }
