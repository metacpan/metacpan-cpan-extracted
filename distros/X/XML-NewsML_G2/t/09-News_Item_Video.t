...
sub remotes_checks {
    ...
    like(
        $xpc->findvalue('//nar:contentSet/nar:remoteContent/@audiochannels'),
...
my $icon1 = XML::NewsML_G2::Icon->new(
    rendition => 'highRes',
    href      => 'file:///tmp/123.jpg',
    width     => 1920,
    height    => 1080
);
my $icon2 = XML::NewsML_G2::Icon->new(
    rendition => 'thumbnail',
    href      => 'file:///tmp/456.jpg',
    width     => 1280,
    height    => 720
);
$ni->add_icon($icon1);
...
foreach my $version (qw(2.12 2.15 2.18 2.28)) {
    ...
    $xpc->registerNs( 'xhtml', 'http://www.w3.org/1999/xhtml' );
    remotes_checks( $dom, $xpc );
    validate_g2( $dom, $version, "NewsItemVideo_$version" );
