package Pcore::Lib::Src::Filter::html;

use Pcore -class, -res;
use Pcore::Lib::Text qw[trim];
use Pcore::Lib::Src::Filter::js;
use Pcore::Lib::Src::Filter::css;
...
sub compress ($self) {
    ...
    for my $i ( 0 .. $#script ) {
        if ( $script[$i] =~ m[\A</script]sm && $script[ $i - 1 ] ) {
            Pcore::Lib::Src::Filter::js->new( { data => \$script[ $i - 1 ] } )->compress;
    ...
    for my $i ( 0 .. $#css ) {
        if ( $css[$i] =~ m[\A</style]sm && $css[ $i - 1 ] ) {
            Pcore::Lib::Src::Filter::css->new( { data => \$css[ $i - 1 ] } )->compress;
        }
    }
