...
#
package PDLA::ImageND;

@EXPORT_OK  = qw(  kernctr PDLA::PP convolve  ninterpol PDLA::PP rebin  circ_mean circ_mean_p PDLA::PP convolveND );
...
These routines act on PDLAs as N-dimensional objects, not as threaded 
...
ImageND is not a part of the PDLA core (v2.4) and hence must be explicitly
loaded.
...
for most purposes you want L<convolveND|PDLA::ImageND/convolveND> instead:
...
sub PDLA::convolve{
    ...
    $c = PDLA->null if $#_<2;
    &PDLA::_convolve_int( $x->clump(-1), $y->clump(-1),
...
See also L<interpND|PDLA::Primitive/interpND>, which includes boundary 
...
*ninterpol = \&PDLA::ninterpol;

sub PDLA::ninterpol {
    use PDLA::Math 'floor';
    use PDLA::Primitive 'interpol';
...
If you want different behavior, use L<PDLA::Transform::map|PDLA::Transform/map>
instead.  PDLA::Transform::map runs slower but is more flexible.
...
sub PDLA::rebin {
    ...
    foreach $i (0..$#odims) {
      ...
      # 1->n and n->1 separately for speed
         ...
         if (!($odims[$i] % $idims[$i])) {      # Cells map 1 -> n
               ...
               $x = $x->mv($i,0);
               # -> copy so won't corrupt input PDLA
             ...
             &PDLA::_rebin_int($x->mv($i,0), $y = null, $odims[$i]);
         }
         $x = $y->mv(0,$i);
      }
    }
    if (exists $opts->{Norm} and $opts->{Norm}) {
      ...
      # Explicit copy so i) can't corrupt input PDLA through this link
...
sub circ_mean_p {
 ...
 if (defined $opt) {
   $rad = long PDLA::rvals($x,$opt);
 }
 else {
   $rad = long rvals $x;
 }
 $sum = zeroes($rad->max+1);
 PDLA::indadd $x->clump(-1), $rad->clump(-1), $sum; # this does the real work
 $norm = zeroes($rad->max+1);
 PDLA::indadd pdl(1), $rad->clump(-1), $norm;       # equivalent to get norm
 $sum /= $norm;
 return $sum;
}
...
L<fftconvolve|PDLA::FFT/fftconvolve()> routines, so convolveND is a drop-in
...
L<range|PDLA::Slices/range>, because this option is passed directly
into L<range|PDLA::Slices/range>.  Useful options are 'truncate' (the
default), 'extend', and 'periodic'.  You can select different boundary 
conditions for different axes -- see L<range|PDLA::Slices/range> for more 
detail.

The (default) truncate option marks all the near-boundary pixels as BAD if
you have bad values compiled into your PDLA and the array's badflag is set. 
...
sub PDLA::convolveND {
  ...
  # PDLA code choose what type is best.
  my $type;
  if($inplace) {
  ...
  my $opt = $def->options(PDLA::Options::ifhref($opt0));
  ...
  if($fft) {
    ...
    &PDLA::_convolveND_int( $k->flat->index(0), $k, $aa, $x );
