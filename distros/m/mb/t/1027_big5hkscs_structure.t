...
# Big5-HKSCS multibyte structure, verified directly through the runtime API. mb
# is loaded with require (no source filter), so this runs on every perl from
# 5.005_03 up. All multibyte bytes are written with \x escapes so the source
# stays US-ASCII.
#
# Big5-HKSCS extends Big5 with characters in the low-lead area \x81-\xA0 (as
# well as the usual Big5 leads). It is still a two-octet encoding: a lead
# \x81-\xFE reads one more octet; \x00-\x80 and \xFF are one-octet units. As in
# Big5 the trailing octet may be a low byte, so \x40 ("@") and \x5C ("\") can
# sit INSIDE a character and must not be seen as boundaries.
#
# Big5-HKSCS characters (low-lead \x88 HKSCS area):
#   A   US-ASCII single octet   "Z"          (\x5A)
#   C1  two octets              \x88\x40      (low lead \x88; trailing "@")
#   C2  two octets              \x88\x5C      (trailing "\")
#   C3  two octets              \x88\xA1      (trailing \xA1)
...
mb::set_script_encoding('big5hkscs');

my $A  = "\x5A";         # "Z"
my $C1 = "\x88\x40";     # 2-octet, low lead \x88, trailing "@" (\x40)
my $C2 = "\x88\x5C";     # 2-octet, trailing "\" (\x5C)
my $C3 = "\x88\xA1";     # 2-octet
my $s  = $A . $C1 . $C2 . $C3 . $A;  # 1+2+2+2+1 octets, 5 characters

BEGIN { open(GETCFH,">@{[__FILE__]}.getc.tmp") or die $!; binmode GETCFH; print GETCFH "\x5A\x88\x40\x88\x5C\x88\xA1\x5A"; close(GETCFH); }
END   { unlink("@{[__FILE__]}.getc.tmp") }

@test = (

# get/set round trip
    sub { mb::get_script_encoding() eq 'big5hkscs' },
...
# substr walks by character, keeping each multibyte char whole
    ...
    sub { mb::substr($s, 1, 1) eq $C1 },   # low lead \x88 reads two octets
...
# getc-style lead dispatch: \x81-\xFE are leads (incl. the low-lead \x88 area),
# \x00-\x80 and \xFF single
    sub { ("\x88" =~ /\A [\x81-\xFE] \z/x) ? 1 : 0 },
...
# (no -w). $w must stay empty.
    sub { open(TRUNCFH,">@{[__FILE__]}.trunc.tmp") or die $!;
          binmode TRUNCFH; print TRUNCFH "\x88"; close(TRUNCFH);
          ...
          ($c1 eq "\x88") and (not defined $c2) and ($w eq '') },
