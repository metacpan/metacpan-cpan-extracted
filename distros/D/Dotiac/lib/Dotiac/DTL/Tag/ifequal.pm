...
package Dotiac::DTL::Tag::ifequal;
...
sub new {
	...
	my $pos=shift;
	my $found="";
	$self->{true}=$obj->parse($data,$pos,\$found,"else","endifequal");
	if ($found eq "else") {
		$self->{false}=$obj->parse($data,$pos,\$found,"endifequal");
	}
...
sub perlprint {
	...
	print $fh "\t" x $level,"if (Dotiac::DTL::devar_raw(\$vara$id,\$vars,\$escape.\@_)->true() eq Dotiac::DTL::devar_raw(\$varb$id,\$vars,\$escape.\@_)->true()) {\n";
