...
Bio::Genex::CitationLink - Methods for processing data from the GeneX DB
linking table: CitationLink

=head1 SYNOPSIS

  use Bio::Genex::CitationLink;

  # instantiating a linking table instance
  my $CitationLink = Bio::Genex::CitationLink->new(id=>47,pkey_link=>'cit_fk');
  # or 
  my $CitationLink = Bio::Genex::CitationLink->new(id=>47,pkey_link=>'es_fk');

  # retrieve data from the DB for all columns
  $CitationLink->fetch();

  # creating an instance, without pre-fetching all columns
  my $CitationLink = new Bio::Genex::CitationLink(id=>47,
                                              'pkey_link'=>'cit_fk');
  ...
  my @objects = Bio::Genex::CitationLink->get_objects('pkey_link'=>'cit_fk',
                                                  23,57,98);


  # retrieving all instances from a table
  my @objects = Bio::Genex::CitationLink->get_objects('ALL');

  # retrieving the primary key for an object, generically
  my $primary_key = $CitationLink->id();

  # retrieving other DB column attributes
  my $cit_fk_val = $CitationLink->cit_fk();
  $CitationLink->cit_fk($value);

  my $es_fk_val = $CitationLink->es_fk();
  $CitationLink->es_fk($value);


=head1 DESCRIPTION

Each Genex class has a one to one correspondence with a GeneX DB table
of the same name (I<i.e.> the corresponding table for Bio::Genex::CitationLink is
CitationLink).


B<NOTE>: Class Bio::Genex::CitationLink is linking table class. This means
...
value from the database. In Bio::Genex::CitationLink, those two columns are
'cit_fk' and 'cit_fk'. When an instance of
Bio::Genex::CitationLink is instantiated, either via C<new()> or
...
Most applications will first create an instance of Bio::Genex::CitationLink
...
Bio::Genex::CitationLink can access: I<raw> foreign key attributes,
...
Class Bio::Genex::CitationLink defines the following utility variables for assisting
programmers to access the CitationLink table.

=over 4

=item $Bio::Genex::CitationLink::LIMIT
...
=item $Bio::Genex::CitationLink::USE_CACHE
...
For example:
  ...
  my $CitationLink = Bio::Genex::CitationLink->new(id=>47);
  my $val = $CitationLink->cit_fk();
...
instance of the class via the Bio::Genex::CitationLink->methodname() syntax.
...
DB. For class Bio::Genex::CitationLink, 'C<pkey_link>' can be either 'C<cit_fk>' or 'C<es_fk>'.
...
Bio::Genex::CitationLink if a method in class C<Citation> invokes its
foreign key accessor function, the column 'es_fk' will be
used to fetch values from the DB.

=item linking_table()

Used by generic functions to determine if a specified class is a
linking table class. For Bio::Genex::CitationLink it returns 1, since it I<is>
...
Bio::Genex::CitationLink it returns 'CitationLink';
...
=item get_objects({pkey_link=>'cit_fk'}, @id_list)
...
Bio::Genex::CitationLink simultaneously. For linking tables, there are three
different ways to invoke this method.

By passing in an C<@id_list>, get_objects() uses each element of the
list as a primary key for the CitationLink table and returns a single
...
a warning from C<Bio::Genex::CitationLink::initialize()>. Objects will be
...
instance of class Bio::Genex::CitationLink.
...
instance. Class Bio::Genex::CitationLink is a linking table and therefore it
can I<two> possible columns that represent the 'id'
('C<cit_fk>' and 'C<es_fk>'). When an instance of
class Bio::Genex::CitationLink is created using either C<new()> or
...
Methods for the cit_fk attribute. This attribute is the first
...
Methods for the es_fk attribute. This attribute is the second
possible value for the 'C<pkey_link>' attribute at object creation
time.
...
on Thu Dec  7 18:35:48 2000 by /home/jasons/Genex/scripts/create_genex_class.pl --target=CitationLink
