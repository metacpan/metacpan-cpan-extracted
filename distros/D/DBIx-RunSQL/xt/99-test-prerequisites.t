...
my $minimum_perl = $meta->{prereqs}->{runtime}->{perl} || 5.006;

# Find what we distribute:
my @modules;
dir('blib')->recurse( callback => sub {
    my( $child ) = @_;
    if( !$child->is_dir and $child =~ /\.pm$/) {
        push @modules, ((scalar $child->slurp()) =~ m/^\s*package\s+(\w+(?:::\w+)*)\b/msg);
    }
});
my %distribution = map { $_ => $_ } @modules;
