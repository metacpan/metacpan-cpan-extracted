package QualysGuard::Response::ScanOptions;
...
sub new {
    ...
    if ( $self->exists('/SCANNEROPTIONS/ERROR') ) { 
        $self->{error_code} = $self->findvalue('/SCANNEROPTIONS/ERROR/@number');
        $self->{error_text} = $self->getNodeText('/SCANNEROPTIONS/ERROR');
...
QualysGuard::Response::ScanOptions 
