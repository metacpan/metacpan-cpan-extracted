...
subtest 'GetRound' => sub {
  ...
  is_deeply( $M->GetRound( $active, 1 )->RawCount(),
    $rtc, 'GetRound method with topcount droprule' );
  $M->{'DropRule'} = 'approval';
  is_deeply( $M->GetRound( $active, 1 )->RawCount(),
    $rappr, 'GetRound method with approval droprule' );

  $M->{'DropRule'} = 'borda';
  is_deeply( $M->GetRound( $active, 1 )->RawCount(),
    $rborda, 'GetRound method with borda droprule' );
  ...
  is_deeply( $M->GetRound( $rgl, 1 )->RawCount(),
    $rgl, 'GetRound method with greatestloss droprule' );
