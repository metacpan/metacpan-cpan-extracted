...
BEGIN {
  use_ok( 'Encoding::HandleUtf8', qw( fix_encoding ) );
}
...
subtest 'ue string input' => sub {
  ...
  is(
    fix_encoding( 'input', $utf8_input ),
    $unicode_orig,
    q{ue string input UTF-8 > Unicode},
  );

  is(
    fix_encoding( 'input', $unicode_input ),
    $unicode_orig,
...
subtest 'ue string output' => sub {
  ...
  is(
    fix_encoding( 'output', $utf8_input ),
    $utf8_orig,
    q{ue string output UTF-8 > UTF-8},
  );

  is(
    fix_encoding( 'output', $unicode_input ),
    $utf8_orig,
...
subtest 'reference and return value' => sub {
  ...
  my $foo = fix_encoding( 'input', $utf8_input );
