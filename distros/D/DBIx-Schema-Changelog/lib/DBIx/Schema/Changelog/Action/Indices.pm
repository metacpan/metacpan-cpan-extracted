package DBIx::Schema::Changelog::Action::Indices;

=head1 NAME

DBIx::Schema::Changelog::Action::Indices - Action handler for indices
...
sub list_from_schema {
    my ( $self, $schema ) = @_;
    my $indices = [];
    foreach ( @{ $self->do( $self->driver->actions->{list_indices}, [] ) } ) {
        push(
            @$indices,
            {
                type    => 'createindex',
