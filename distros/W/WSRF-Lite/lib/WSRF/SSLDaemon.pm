...
package WSRF::SSLDaemon;

=head1 NAME

SSLDaemon - a simple https server class
...
{
    my $self = shift;
    my $pkg = shift || "SSLDaemon::ClientConn";
...
sub product_tokens
{
    "libwww-perl-daemon/$SSLDaemon::VERSION";
}


package SSLDaemon::ClientConn;

use vars qw(@ISA $DEBUG);
use IO::Socket ();
@ISA=qw(IO::Socket::SSL);
*DEBUG = \$SSLDaemon::DEBUG;
...
The I<SSLDaemon::ClientConn> is also a I<IO::Socket::INET>
...
sub generic_read {
    ...
    #print "Calling read_func....$read_func for $length\n";
    #my $data = $read_func->($ssl, $length);
    my $data = Net::SSLeay::read($ssl, $length);
    #print "Called read_func...\n";
...
sub get_request
{
    ...
    #print "Connection Acepted From:\n";
    #my $cert = $self->dump_peer_certificate();
  #print $cert;
  #my($DN,$issuer) = split( /\n/, $cert);
  #$DN =~ s/Subject Name: //o;
...
sub send_status_line
{
    ...
    $proto   ||= $SSLDaemon::PROTO || "HTTP/1.1";
