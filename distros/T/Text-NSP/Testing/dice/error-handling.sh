...
# shell program to test dice.pm's responses to various erroneous
# conditions. note of course that dice never halts on errors/warnings
# but sets error codes that statistic.pl can request later on.

# Subtest 1: what happens when ngram > 2 is provided to dice
...
# test-1.sub-1.cnt has trigrams in it. so dice.pm should complain
# during the initialization step, since dice is for bigrams only!
...
# now the test!
echo "Test:   statistic.pl --ngram 3 dice test-1.out $TESTFILE" 
statistic.pl --ngram 3 dice test-1.out $TESTFILE >& error.out
...
# now the test!
echo "Test:   statistic.pl --set_freq_combo $FREQCOMBFILE dice test-1.out $TESTFILE" 
statistic.pl --set_freq_combo $FREQCOMBFILE dice test-1.out $TESTFILE >& error.out
...
# now the test!
echo "Test:   statistic.pl dice test-1.out $TESTFILE"
statistic.pl dice test-1.out $TESTFILE >& error.out
