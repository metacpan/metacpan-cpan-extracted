...
sub groucho_input {
  ...
  if ( $cmd eq 'NOTICE' ) {
    ...
    if ( $heap->{unresv} ) {
      is( $params->[1], 'RESV for [bobbins] is removed', 'RESV for [bobbins] is removed' );
    }
    else {
...
sub ircd_daemon_eob {
  ...
  if ( $heap->{eob} >= 3 ) {
    $kernel->post( 'groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'RESV', params => [ '*', '600', 'bobbins', 'Banhammer'] } );
  }
  return;
}
