...
(it defaults to C<irc_css_highlighter>) will receive input
...
If an error occurred during the retrieval of CSS code then the C<error> key will be present
and its value will be a human parseable error message explaining the failure.
