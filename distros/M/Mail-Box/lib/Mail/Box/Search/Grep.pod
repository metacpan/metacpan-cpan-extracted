...
Improves base, see L<Mail::Box::Search/"METHODS">

 -Option    --Defined in       --Default
  binaries    Mail::Box::Search  <C<false>>
  decode      Mail::Box::Search  <C<true>>
...
=item field => undef|$field|REGEX|CODE

Not valid in combination with C<in> set to C<BODY>.
Pass one full C<$field> name (case-insensitive).  Use a REGEX to select more
...
first, and the field as second argument.  If the CODE returns C<true>,
...
When the result is C<true>, the details are delivered.  The call formats are
...
Extends L<"Attributes" in Mail::Box::Search|Mail::Box::Search/"Attributes">.

=over 4

=item $obj-E<gt>B<deliver>()

Inherited, see L<Mail::Box::Search/"Attributes">
...
=item Error: illegal field selector $take

Cast by C<new()>

=item Error: illegal match pattern $match

Cast by C<new()>

=item Error: no match pattern specified
...
This module is part of Mail-Box version 3.012,
built on November 27, 2025. Website: F<http://perl.overmeer.net/CPAN/>
