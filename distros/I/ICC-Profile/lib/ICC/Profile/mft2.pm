...
sub new {
	...
	if (@_) {
		...
		if (@_ == 1 && ref($_[0]) eq 'HASH') {
			...
			# make new mft2 tag from attribute hash
...
# create mft2 tag object from ICC profile
...
# writes mft2 tag object to ICC profile
# parameters: (ref_to_parent_object, file_handle, ref_to_tag_table_entry)
sub write_fh {
	...
	# write mft2 data to profile
	_writeICCmft2($self, @_);
...
sub size {
	...
	$size += 2 * $self->[1][1]->cin() * @{$self->[1][1]->array->[0]->array()};

	# add size of clut
	$size += $self->[1][2]->_clut_size(2);

	# add size of output tables (assumes 'curv' objects)
	$size += 2 * $self->[1][3]->cin() * @{$self->[1][3]->array->[0]->array()};
	...
	if (defined($pcs)) {
		...
		if (defined($self->[0]{'pcs_encoding'})) {
			...
			if (($self->[0]{'input_cs'} eq 'Lab ' && $self->[0]{'output_cs'} ne 'XYZ ') || ($self->[0]{'input_cs'} ne 'XYZ ' && $self->[0]{'output_cs'} eq 'Lab ')) {
				...
				# return PCS type (16-bit ICC legacy, but could be Monaco)
				return(1);
...
sub _check {
	...
	($#{$self->[1]} == 3) || croak('\'mft2\' object has wrong number of processing elements');
	...
	for my $i (0 .. 3) {
		...
		if (defined($self->[1][$i])) {
			...
			(ref($self->[1][$i]) eq $class[$i]) || croak("'mft2' processing element $i is wrong object type");
			...
			if ($i) {
				...
				if (defined($co)) {
					...
					($self->[1][$i]->cin() == $co) || croak("'mft2' processing element $i has wrong number of input channels");
				...
				($self->[1][0]->cin() == 3 && $self->[1][0]->cout() == 3) || croak("'mft2' matrix processing element wrong size");
				
				# warn if matrix has non-zero offset values
				(defined($self->[1][0]->offset) && grep {$_} @{$self->[1][0]->offset}) && carp("'mft2' matrix processing element has non-zero offset values")
		...
		# if not matrix processing element
			...
			# error
			croak("'mft2' processing element $i is missing");
	...
	(@{$gsa} == grep {$_ == $gsa->[0]} @{$gsa}) || croak("'mft2' clut processing element grid points vary by channel");

	# verify input 'cvst' elements are 'curv' objects
	(@{$self->[1][1]->array} == map {UNIVERSAL::isa($_, 'ICC::Profile::curv')} @{$self->[1][1]->array}) || croak("'mft2' input processing element has wrong curve type");

	# get number of entries from first input 'curv' object
	$n = @{$self->[1][1]->array->[0]->array};

	# verify number of entries is between 2 and 4096
	($n >= 2 && $n <= 4096) || croak("'mft2' input processing element has wrong number of curve entries");

	# verify input 'curv' objects have same number of entries
	(@{$self->[1][1]->array} == grep {@{$_->array} == $n} @{$self->[1][1]->array}) || croak("'mft2' input processing elements have different numbers of curve entries");

	# verify output 'cvst' elements are 'curv' objects
	(@{$self->[1][3]->array} == map {UNIVERSAL::isa($_, 'ICC::Profile::curv')} @{$self->[1][3]->array}) || croak("'mft2' output processing element has wrong curve type");

	# get number of entries from first output 'curv' object
	$n = @{$self->[1][3]->array->[0]->array};

	# verify number of entries is between 2 and 4096
	($n >= 2 && $n <= 4096) || croak("'mft2' output processing element has wrong number of curve entries");

	# verify output 'curv' objects have same number of entries
	(@{$self->[1][3]->array} == grep {@{$_->array} == $n} @{$self->[1][3]->array}) || croak("'mft2' output processing elements have different numbers of curve entries");
...
# read mft2 tag from ICC profile
...
sub _readICCmft2 {
	...
	($mft[0] eq 'mft2') or croak('wrong tag type');
	...
	# set signature
	$self->[1][0][0]{'signature'} = 'mft2';

	# read size of input and output tables
	read($fh, $buf, 4);

	# unpack values
	@mft[4, 5] = unpack('n2', $buf);
	...
	for my $i (0 .. $mft[1] - 1) {
		...
		# make 'curv' object
		$input->[$i] = ICC::Profile::curv->new([map {$_/65535} unpack('n*', $buf)]);
	...
	# read 'clut' data
	$self->[1][2]->_read_clut($fh, $mft[2], $gsa, 2);
	
	# save 'clut' gsa
	$self->[1][2][2] = $gsa;

	# save CLUT byte width
	$self->[1][2][0]{'clut_bytes'} = 2;
	...
	for my $i (0 .. $mft[2] - 1) {
		...
		# make 'curv' object
		$output->[$i] = ICC::Profile::curv->new([map {$_/65535} unpack('n*', $buf)]);
...
sub _writeICCmft2 {
	...
	# set size of input tables
	$mft[13] = @{$self->[1][1]->array->[0]->array};

	# set size of output tables
	$mft[14] = @{$self->[1][3]->array->[0]->array};

	# seek start of tag
	seek($fh, $tag->[1], 0);

	# write mft tag header
	print $fh pack('a4 x4 C3 x N9 n2', @mft);

	# for each input channel
	for my $i (0 .. $mft[1] - 1) {
		
		# write table values
		print $fh pack('n*', map {$_ < 0 ? 0 : ($_ > 1 ? 65535 : $_ * 65535 + 0.5)} @{$self->[1][1]->array->[$i]->array});
		
	}

	# write clut
	$self->[1][2]->_write_clut($fh, $self->[1][2]->gsa(), 2);

	# for each output channel
	for my $i (0 .. $mft[2] - 1) {
		
		# write table values
		print $fh pack('n*', map {$_ < 0 ? 0 : ($_ > 1 ? 65535 : $_ * 65535 + 0.5)} @{$self->[1][3]->array->[$i]->array});
