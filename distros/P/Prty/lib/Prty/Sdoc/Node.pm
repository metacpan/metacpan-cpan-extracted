...
sub nextType {
    ...
    if ($type eq 'Item') {
        ...
        if (!$self->isa('Prty::Sdoc::KeyValTable')) {
            $type = 'KeyValTable';
        }
    }

    return ($type,Prty::Array->new($arr));
}
...
sub expand {
    ...
    if ($format =~ /^e?html/) {
        ...
        if ($inlineSegments) {
            ...
            $val =~ s|B\{([^}]+)\}|Prty::Pod::Generator->fmt('B',$1)|gse;
            $val =~ s|C\{([^}]+)\}|Prty::Pod::Generator->fmt('C',$1)|gse;
            $val =~ s|E\{([^}]+)\}|Prty::Pod::Generator->fmt('I',$1)|gse;
            $val =~ s|I\{([^}]+)\}|Prty::Pod::Generator->fmt('I',$1)|gse;
            $val =~ s|Q\{([^}]+)\}|"$1"|gs;
        }
    }
    elsif ($format eq 'man') {
