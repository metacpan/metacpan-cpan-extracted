...
#
package AgentPushKit::Client::ApiClient;
...
use AgentPushKit::Client::Configuration;

sub new {
    my $class = shift;

    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'AgentPushKit::Client::Configuration' ) {
        $config = $_[0];
    } else {
        $config = AgentPushKit::Client::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("AgentPushKit::Client::Object::$class")->new;
        if (ref $data eq "HASH") {
...
sub update_params_for_auth {
    ...
    foreach my $auth (@$auth_settings) {
        ...
        elsif ($auth eq 'UserOrAgent') {
            # this endpoint requires Bearer (Agent Push Kit JWT or apt_ token) authentication (access token)
            if ($self->{config}{access_token}) {
                $header_params->{'Authorization'} = 'Bearer ' . $self->{config}{access_token};
            }
        }
        elsif ($auth eq 'AgentToken') {
            # this endpoint requires Bearer (apt_) authentication (access token)
            if ($self->{config}{access_token}) {
                $header_params->{'Authorization'} = 'Bearer ' . $self->{config}{access_token};
            }
        }
        elsif ($auth eq 'OrganizationApiKey') {
            # this endpoint requires Bearer (apk_) authentication (access token)
            if ($self->{config}{access_token}) {
                $header_params->{'Authorization'} = 'Bearer ' . $self->{config}{access_token};
            }
        }
