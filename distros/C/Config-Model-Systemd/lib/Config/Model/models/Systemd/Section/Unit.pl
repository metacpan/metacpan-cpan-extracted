...
",
    ...
    'description' => {
      'After' => 'These two settings expect a space-separated list of unit names. They may be specified
...
C<false>.',
      'AssertACPower' => "Similar to the C<ConditionArchitecture>,
...
into.",
      'AssertArchitecture' => '*AssertACPower',
      'AssertCPUFeature' => '*AssertACPower',
      'AssertCPUPressure' => '*AssertACPower',
      'AssertCPUs' => '*AssertACPower',
      'AssertCapability' => '*AssertACPower',
      'AssertControlGroupController' => '*AssertACPower',
      'AssertCredential' => '*AssertACPower',
      'AssertDirectoryNotEmpty' => '*AssertACPower',
      'AssertEnvironment' => '*AssertACPower',
      ...
      'AssertOSRelease' => '*AssertACPower',
      'AssertPathExists' => '*AssertACPower',
      'AssertPathExistsGlob' => '*AssertACPower',
      'AssertPathIsDirectory' => '*AssertACPower',
      'AssertPathIsEncrypted' => '*AssertACPower',
      'AssertPathIsMountPoint' => '*AssertACPower',
      'AssertPathIsReadWrite' => '*AssertACPower',
      'AssertPathIsSocket' => '*AssertACPower',
      'AssertPathIsSymbolicLink' => '*AssertACPower',
      'AssertSecurity' => '*AssertACPower',
      'AssertUser' => '*AssertACPower',
      'AssertVersion' => '*AssertACPower',
      'AssertVirtualization' => '*AssertACPower',
      'Before' => '*After',
...
dependency cannot be specified directly.",
      'CollectMode' => "Tweaks the \"garbage collection\" algorithm for this unit. Takes one of C<inactive>
...
mark.',
      'ConditionCPUFeature' => 'Verify that a given CPU feature is available via the C<CPUID>
...
C<@system>.',
      'ConditionHost' => 'C<ConditionHost> may be used to match against the hostname,
...
details about the machine ID. The test may be negated by prepending an exclamation mark.',
      'ConditionIOPressure' => '*ConditionCPUPressure',
      'ConditionKernelCommandLine' => "C<ConditionKernelCommandLine> may be used to check whether a
...
1> is used instead (i.e. C</proc/1/cmdline>).",
      'ConditionKernelModuleLoaded' => 'Test whether the specified kernel module has been loaded and is already fully
initialized.',
      'ConditionKernelVersion' => 'C<ConditionKernelVersion> may be used to check whether the kernel
...
amount of memory assigned to the container instead.',
      ...
      'ConditionNeedsUpdate' => 'Takes one of C</var/> or C</etc/> as argument,
...
happen.',
      'ConditionOSRelease' => 'Verify that a specific C<key=value> pair is set in the host\'s
L<os-release(5)>.
...
exist.',
      'ConditionPathExistsGlob' => 'C<ConditionPathExistsGlob> is similar to
...
directory matching the specified globbing pattern.',
      'ConditionPathIsDirectory' => 'C<ConditionPathIsDirectory> is similar to
C<ConditionPathExists> but verifies that a certain path exists and is a
directory.',
      'ConditionPathIsEncrypted' => 'C<ConditionPathIsEncrypted> is similar to
...
detected. It is not detected whether the file system backing the loopback block device is encrypted.',
      'ConditionPathIsMountPoint' => 'C<ConditionPathIsMountPoint> is similar to
C<ConditionPathExists> but verifies that a certain path exists and is a mount
point.',
      'ConditionPathIsReadWrite' => 'C<ConditionPathIsReadWrite> is similar to
C<ConditionPathExists> but verifies that the underlying file system is readable
and writable (i.e. not mounted read-only).',
      'ConditionPathIsSocket' => 'C<ConditionPathIsSocket> is similar to
C<ConditionPathExists> but verifies that a certain path exists and is a
socket.',
      'ConditionPathIsSymbolicLink' => 'C<ConditionPathIsSymbolicLink> is similar to
C<ConditionPathExists> but verifies that a certain path exists and is a symbolic
link.',
      'ConditionSecurity' => 'C<ConditionSecurity> may be used to check whether the given
...
The test may be negated by prepending an exclamation mark.',
      'ConditionUser' => 'C<ConditionUser> takes a numeric C<UID>, a UNIX
...
root user, and thus the test result is constant.',
      'ConditionVersion' => 'C<ConditionVersion> may be used to check whether a software
...
effect.',
      'FailureActionExitStatus' => "Controls the exit status to propagate back to an invoking container manager (in case of a
...
request default behaviour.",
      'IgnoreOnIsolate' => 'Takes a boolean argument. If C<true>, this unit will not be stopped
...
units.',
      'JobRunningTimeoutSec' => 'C<JobTimeoutSec> specifies a timeout for the whole job that starts
...
the unit state to change.',
      'JobTimeoutAction' => 'C<JobTimeoutAction> optionally configures an additional action to
...
call.',
      'JobTimeoutRebootArgument' => '*JobTimeoutAction',
...
the C<failed> state.',
      'OnFailureJobMode' => 'Takes a value of
C<fail>,
C<replace>,
...
dependency cannot be specified directly.",
      'PropagatesReloadTo' => 'A space-separated list of one or more units to which reload requests from this unit
...
all units that are linked to it using these two settings.',
      'PropagatesStopTo' => 'A space-separated list of one or more units to which stop requests from this unit
...
units that are linked to it using these two settings.',
      'RefuseManualStart' => 'Takes a boolean argument. If
...
C<false>.',
      'RefuseManualStop' => '*RefuseManualStart',
      'ReloadPropagatedFrom' => '*PropagatesReloadTo',
      'Requires' => "Similar to C<Wants>, but declares a stronger requirement
...
will be pulled in by this unit.',
      'Requisite' => 'Similar to C<Requires>. However, if the units listed here
...
it. Defaults to C<false>.',
      ...
      'SurviveFinalKillSignal' => 'Takes a boolean argument. Defaults to C<no>. If C<yes>,
...
System|https://systemd.io/ROOT_STORAGE_DAEMONS>, which however continues to be supported.',
      'Upholds' => 'Configures dependencies similar to C<Wants>, but as long as this unit
...
activated.',
      'WantsMountsFor' => 'Same as C<RequiresMountsFor>,
