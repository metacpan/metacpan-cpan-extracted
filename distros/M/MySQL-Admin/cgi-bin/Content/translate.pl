...
foreach my $key ( sort keys %{$m_hrLng} ) {
    ...
    print a(
        {
            href => "javascript:requestURI('$m_hrSettings->{cgi}{serverName}$ENV{SCRIPT_NAME}?action=translate&lang=$key','translate','$key')"
        },
        $key
    ) . '&#160;|&#160;';
} ## end foreach my $key ( sort keys...)
...
print a(
    {
        href => "javascript:requestURI('$m_hrSettings->{cgi}{serverName}$ENV{SCRIPT_NAME}?action=showaddTranslation','translate','translate')"
    },
    $addtranslate
) . '&#160;';
show_form(
...
sub on_valid_form {
    ...
    my $rs            = "javascript:requestURI('$m_hrSettings->{cgi}{serverName}$ENV{SCRIPT_NAME}?action=translate','$m_sAction','$savetranslate')";
    my %parameter     = (
