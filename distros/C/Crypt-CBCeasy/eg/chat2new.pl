...
# UDP support to open_port(), $chat::COPY in expect() added,
# $thishost lookup changed
# socket.ph is not needed but you can do 'use Socket;'
# by Mike Blazer<blazer@mail.nevalink.ru>  last: Dec 22, 1998
...
local($aname, $aaliases, $aaddrtype, $alength, @aaddrs) = gethostbyname ("localhost");
($aname, $aaliases, $aaddrtype, $alength, @aaddrs) = gethostbyname ($aname);
$thishost= join '.', unpack ('C4', $aaddrs[0]);
#print "$thishost\n";
...
sub open_port { ## public
	local($server, $port, $proto) = @_;
        local($af_pf, $stream);

        die "chat::open_port: too few parameters\n" if @_ < 2;

	if ($proto && $proto =~ /^udp$/i) { # UDP
	    $af_pf  = $af_inet;
	    $stream = $sock_dgram;
	    $proto  = $udp_proto;
	} else {		  # TCP
	...
	*S = ++$next;
	...
	unless (socket(S, $af_pf, $stream, $proto)) {
...
#	(@myaddr, $port, $next); # =====old variant
	($port, $next, @myaddr); # returning this
}
...
sub expect { ## public
	...
	local($endtime) = shift;
	local($COPY); # whole copy of what we got in S
...
EDQ
		$cases .= <<'ESQ'; # Single Quote
		else {
			$rmask = "";
			vec($rmask,fileno(S),1) = 1;
			($nfound, $rmask) =
		 		select($rmask, undef, undef, $endtime - time);
#print "\$rmask = $rmask\n"; # time left to grand-timeout (here)
#print "\$nfound = $nfound\n";
			if ($nfound) {
