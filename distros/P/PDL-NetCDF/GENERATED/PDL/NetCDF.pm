...
sub defaultFormat {
	...
	if (defined $format) {
		...
		croak ("Cannot change default format -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;
	} else {
		my $rc = PDL::NetCDF::nc_set_default_format(NC_FORMAT_CLASSIC(), $oldFormat=-999);
		croak ("Cannot get default format -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;
		if ($rc == NC_NOERR) {
			my $temp;
...
sub new {
  ...
  if (-e $file or ($file =~ m{^http://})) {
    ...
    if ($write) {
      ...
      croak ("new:  Cannot open file -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;
      $self->{WR} = 'w';
...
#    croak ("new:  Cannot inquire ndims -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;
#    $rc = PDL::NetCDF::nc_inq_nvars ($self->{NCID}, $nvars=-999);
#    croak ("new:  Cannot inquire nvars -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;

    $rc = PDL::NetCDF::nc_inq ($self->{NCID}, $ndims=-999, $nvars=-999, $ngatts=-999, $unlimid=-999);
    croak ("new:  Cannot inquire -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;
    ...
    for (my $i=0;$i<$ndims;$i++) {
      ...
      croak ("new:  Cannot inquire dim name -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;	
      $self->{DIMIDS}{$name} = $i;
      
      $rc = PDL::NetCDF::nc_inq_dimlen ($self->{NCID}, $i, my $len = -999);
      croak ("new:  Cannot inquire dim length -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;	
    ...
    for (my $i=0;$i<$ngatts; $i++) {
      ...
      $rc = PDL::NetCDF::nc_inq_attname ($self->{NCID},$self->{VARIDS}{GLOBAL} , $i,
                                       my $attname='x' x NC_MAX_NAME()); # Preallocate strings
      croak ("new:  Cannot inquire global att name -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;	
    ...
    croak ("new:  Cannot create netCDF file -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;	
...
sub getFormat {
	...
	croak("Cannot get format -- " . PDL::NetCDF::nc_strerror($rc)) if $rc;
	return $format;
}
...
sub getDeflateShuffle {
    ...
    croak ("getDeflateShuffle: undefined varname $varname") unless exists $self->{VARIDS}{$varnm};
    my ($deflate, $isDeflate, $shuffel);
    my $rc = nc_inq_var_deflate($self->{NCID}, $self->{VARIDS}{$varnm}, $shuffle=-999, $isDeflate=-999, $deflate=-999);
    croak ("getDeflateShuffle: ncerror on $varname -- ".nc_strerror($rc)) if $rc;
...
sub getdimensionnames {
  ...
  if (defined $varnm) {
    ...
    if ($ndims > 0) {
      ...
      croak ("getdimensionnames:  Cannot get info on this var id -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;    
      $dimids = [list $dimids];
    } else {
      $dimids = [];
    }
...
sub sync {
  ...
  croak ("sync:  Cannot sync file -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;    
  return;
}
...
sub put {
  ...
  croak ("Unknown options to put: ". join(",", keys %{$opt})) if (keys %{$opt});
  
  if ($self->{REVERSE_DIMS}) {
      $dims = [ reverse @$dims ];
  }

  croak "Cannot write read-only netCDF file $self->{FILENM}" if ($self->{WR} eq 'r');
  ...
  for (my $i=0;$i<@$dims;$i++) {
	...
	croak ("Cannot put file into define mode") if $rc;
	$self->{DEFINE} = 1;
      }
      ...
      croak ("put:  Cannot define dimension -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;
    ...
    croak ("put:  Attempt to redefine length of dimension $$dims[$i]") 
  ...
  if (!defined($self->{VARIDS}{$varnm})) {
    ...
    unless ($self->{DEFINE}) {
      my $rc = PDL::NetCDF::nc_redef ($self->{NCID});
      croak ("put:  Cannot put file into define mode -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;
      $self->{DEFINE} = 1;
    }
    ...
    croak ("put:  Cannot define variable -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;    
    if ($deflate || $shuffle) {
        my $doDeflate = $deflate > 0 ? 1 : 0;
        my $rc = PDL::NetCDF::nc_def_var_deflate($self->{NCID}, $self->{VARIDS}{$varnm}, $shuffle, $doDeflate, $deflate);
        croak ("put: Cannot shuffle/deflate variable -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;
    }
  ...
  if ($self->{DEFINE}) {
    ...
    croak ("put:  Cannot end define mode -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;    
    $self->{DEFINE} = 0;
  }
  ...
  if (ref($pdl) =~ /Char/) {  # a PDL::Char type PDL--write a netcdf char variable
	...
	$scale_factor = double([$r1/$r2]);
        ...
        croak ("put (compressed):  compression of other than float or double variables not supported");
      }
      croak ("put (compressed):  Cannot write file -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;
      ...
      croak ("put:  Cannot write file -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;    
    }
...
sub putslice {
  ...
  my $deflate = delete $opt->{DEFLATE} || 0;
  my $shuffle = delete $opt->{SHUFFLE} || 0;
  croak ("Unknown options to putslice: ". join(",", keys %{$opt})) if (keys %{$opt});
  ...
  for (my $i=0;$i<@$dims;$i++) {
    ...
    croak ("putslice:  Attempt to redefine length of dimension $$dims[$i]") 
      if ($self->{DIMLENS}{$$dims[$i]} != $dimlens[$i]);
...
sub get {
  ...
  croak ("Cannot find variable $varnm") if (!defined($self->{VARIDS}{$varnm}));
  ...
  if (defined ($count)) {  # Get a hyperslab of the netCDF matrix
    ...
    if (ref($pdl) =~ /Char/) {  # a PDL::Char type PDL--write a netcdf char variable
      ...
      croak ("get:  Cannot get number of variables -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;    	
      my $dimids = ($ndims > 0) ? zeroes (PDL::long, $ndims) : pdl [];
      $rc = PDL::NetCDF::nc_inq_vardimid ($self->{NCID}, $self->{VARIDS}{$varnm}, $dimids);
      croak ("get:  Cannot get info on this var id -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;    
    ...
    croak ("get:  Cannot get data -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;
    ...
    for ($i=0;$i<$ndims;$i++) {
      ...
      croak ("get:  Cannot get info on this dimension -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;    
      croak ("get:  Dimension must be positive") if ($size <= 0);  # Added to address zero length vars. D. Hunnt 4/29/2005
      push (@cnt, $size);
    }
  ...
  #
   unless ($opt->{NOCOMPRESS}) {
	...
	my $scale_factor = $self->getatt('scale_factor', $varnm);
        ...
        my $fill         = eval { $self->getatt('_FillValue',   $varnm); };
  ...
  # convert netcdf missing values to PDL badvals if requested and missing value is available
  if (exists($self->{PDL_BAD})) {
    my $missing = eval { $self->getatt('_FillValue',   $varnm); };
    $missing = eval { $self->getatt('missing_value',   $varnm); } unless (defined($missing));
    $missing = $missing->sclr if (ref($missing) =~ /PDL/); # Convert PDLs to scalars. D. Hunt 2005.10.23
...
sub dimsize {
  ...
  croak ("dimsize: No such dimension -- $dimnm") unless exists ($self->{DIMIDS}{$dimnm});

  my ($dimsz, $name);
  my $rc = nc_inq_dimlen ($self->{NCID}, $self->{DIMIDS}{$dimnm}, $dimsz=-999);
  croak ("dimsize:  Cannot get dimension length -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;    
...
sub putatt {
  ...
  unless ($self->{DEFINE}) {
    my $rc = PDL::NetCDF::nc_redef ($self->{NCID});
    croak ("putatt:  Cannot put file into define mode -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;    
    $self->{DEFINE} = 1;
  }
  ...
  if (ref $att eq 'PDL') {
    ...
    croak ("Attributes can only be 1 dimensional") if ($att->dims != 1);
    ...
    my $rc = &{$typemap5{$att->get_datatype}}($self->{NCID}, $self->{VARIDS}{$varnm}, 
					      ...
					      nelem ($att), $att);
    croak ("putatt:  Cannot put PDL attribute -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;  
...
#
    ...
    croak ("putatt:  Cannot put string attribute -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;  
...
#
    ...
    croak ("Attributes of this type not supported");
...
sub getatt {
  ...
  if(defined $self->{ATTTYPE}{$varnm}{$attnm}){
     ...
     croak ("getatt:  Attribute not found -- $varnm:$attnm");
  } 
#  $rc = PDL::NetCDF::nc_inq_atttype ($self->{NCID}, $self->{VARIDS}{$varnm}, $attnm, my $datatype=-999);
#  croak ("getatt:  Cannot get attribute type -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;  
#
#  $rc = PDL::NetCDF::nc_inq_attlen ($self->{NCID}, $self->{VARIDS}{$varnm}, $attnm, my $attlen=-999);
#  croak ("getatt:  Cannot get attribute length -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;  
  ...
  if ($datatype == NC_CHAR()) {
    ...
    croak ("getatt:  Cannot get text attribute -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;  
  ...
  croak ("getatt:  Cannot get attribute -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;    
...
sub puttext {
  ...
  my $deflate = delete $opt->{DEFLATE} || 0;
  my $shuffle = delete $opt->{SHUFFLE} || 0;
  croak ("Unknown options to puttext: ". join(",", keys %{$opt})) if (keys %{$opt});
...
sub gettext {
  ...
  # Get text attribute into perl string
  croak ("gettext:  Data not of string type") if ($datatype != NC_CHAR());
  ...
  croak ("gettext:  Cannot get text variable -- " . PDL::NetCDF::nc_strerror ($rc)) if $rc;  
