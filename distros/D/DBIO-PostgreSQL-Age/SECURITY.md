This is the Security Policy for the Perl DBIO-PostgreSQL-Age distribution.
...
[DBIO-PostgreSQL-Age on CPAN](https://metacpan.org/dist/DBIO-PostgreSQL-Age) or the main branch of the
[DBIO-PostgreSQL-Age git repository](https://codeberg.org/dbio/dbio-postgresql-age).
...
DBIO-PostgreSQL-Age is hosted on Codeberg, which does not support confidential issue
...
Any security vulnerabilities in DBIO-PostgreSQL-Age are covered by this policy.
...
they affect DBIO-PostgreSQL-Age, or DBIO-PostgreSQL-Age can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that uses
DBIO-PostgreSQL-Age, or plugins to it that are not included with the DBIO-PostgreSQL-Age
distribution) are not covered by this policy.

## Supported Versions of DBIO-PostgreSQL-Age

Only the latest release of DBIO-PostgreSQL-Age will be supported for security fixes.

Only major versions of Perl released in the past ten (10) years will be
supported, even when DBIO-PostgreSQL-Age will run on older versions of Perl. If a
...
that are required for DBIO-PostgreSQL-Age to work. However, some of these
