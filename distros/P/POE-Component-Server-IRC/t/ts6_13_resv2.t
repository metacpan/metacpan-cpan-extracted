...
sub ircd_daemon_nick {
    ...
    $heap->{ircd}->yield( 'daemon_cmd_resv', 'OperServ', '#drones', 'Banhammer' );
...
sub ircd_daemon_resv {
    ...
    is($args[1], '#drones', 'The mask is right' );
...
sub ircd_daemon_unresv {
    ...
    $poe_kernel->post('client', 'send_to_server', { command => 'JOIN', params => [ '#drones' ] } );
    return;
}
...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m!^bobbins! ) {
    pass($cmd);
    $poe_kernel->post( $sender, 'send_to_server', { command => 'JOIN', params => [ '#drones' ] } );
    return;
  }
  if ( $cmd eq '485' ) {
    pass("IRC_$cmd");
    is( $params->[1], '#drones', 'The channel was #drones' );
    is( $params->[2], 'Cannot join channel (Banhammer)', 'Cannot join channel (Banhammer)' );
    $poe_kernel->state('client_input','main','client_input2');
    $heap->{ircd}->yield('daemon_cmd_unresv','OperServ','#drones');
    return;
  }
  return;
}
...
sub client_input2 {
  ...
  pass("IRC_$cmd");
  if ( $cmd eq '366' ) {
    is( $params->[1], '#drones', 'Joined channel' );
