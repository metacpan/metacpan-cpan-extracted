...
Default-Definition genutzt:
    ...
    sub {
        ...
        return Prty::Section::Object->new(@_);
    }
        ...
        while (<$fh>) {
            if ($state == 0) {
                if (/$sectionRegex/ || $defaultSection) {
                    ...
                    $keyA = [];
                    $keyValH = Prty::Hash->new->unlockKeys;
                    $content = $source = '';
                    $lineNumber = $line;
...
C<< Prty::Section::Object >> und liefert eine Referenz auf dieses Objekt
...
als Default-Section definiert wird:
    ...
    $par = Prty::Section::Parser->new(
        defaultSection => $identifier,
        ...
    );

Als Beispiel siehe prty-confluence:
