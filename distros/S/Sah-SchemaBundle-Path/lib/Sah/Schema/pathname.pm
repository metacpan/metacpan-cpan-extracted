...
This document describes version 0.031 of Sah::Schema::pathname (from Perl distribution Sah-SchemaBundle-Path), released on 2024-02-16.
...
Used prefilters: L<Path::expand_tilde_when_on_unix|Data::Sah::Filter::perl::Path::expand_tilde_when_on_unix>, L<Path::strip_slashes_when_on_unix|Data::Sah::Filter::perl::Path::strip_slashes_when_on_unix>
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("pathname*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("pathname", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("pathname", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("pathname*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"pathname*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('pathname*', name=>'Pathname')
     );
 }

 use My::Types qw(Pathname);
 Pathname->assert_valid($data);
