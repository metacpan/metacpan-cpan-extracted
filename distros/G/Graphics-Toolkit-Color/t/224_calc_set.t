...
@colors = $complement->($blue, [], 1, 0, 0, $HSL );
...
@colors = $complement->($blue, [], 2, 0, 0, $HSL);
...
@colors = $complement->($blue, [], 3, 0, 0, $HSL);
...
@colors = $complement->($blue, [], 4, 0, 0, $HSL);
...
@colors = $complement->($midblue, [], 5, 0, 0, $HSL);
...
@colors = $complement->($blue, [], 3, 2, 0, $HSL);
...
@colors = $complement->($blue, [10,-20,30], 4, 1.5, 0,  $HSL);
...
@colors = $complement->($blue, [], 4, 0, 10, $HSL);
is( int @colors,                        4,   'got 4 "tetradic" colors with skew');
$values = $colors[0]->shaped('HSL');
is_tuple( $values, [330, 100, 40], [qw/hue saturation lightness/], 'first complement is pink');
$values = $colors[1]->shaped('HSL');
is_tuple( $values, [ 60, 100, 50], [qw/hue saturation lightness/], 'second complement is yellow');
$values = $colors[2]->shaped('HSL');
is_tuple( $values, [150, 100, 60], [qw/hue saturation lightness/], 'third complement is teal');
$values = $colors[3]->shaped('HSL');
is_tuple( $values, [240, 100, 50], [qw/hue saturation lightness/], 'forth complement is blue (original color)');
