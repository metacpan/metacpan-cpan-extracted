...
sub import {
    ...
    *Char::ord           = \&Big5::ord;
    *Char::ord_          = \&Big5::ord_;
    *Char::reverse       = \&Big5::reverse;
    *Char::getc          = \&Big5::getc;
    *Char::length        = \&Big5::length;
    *Char::substr        = \&Big5::substr;
    *Char::index         = \&Big5::index;
    *Char::rindex        = \&Big5::rindex;
    *Char::eval          = \&Big5::eval;
    *Char::escape        = \&Big5::escape;
    *Char::escape_token  = \&Big5::escape_token;
    *Char::escape_script = \&Big5::escape_script;
}
...
sub Ebig5::classic_character_class;
sub Ebig5::capture;
sub Ebig5::chr(;$);
sub Ebig5::chr_();
sub Ebig5::filetest;
...
if (($] >= 5.010001) or
    ...
    ${Ebig5::anchor_SADAHIRO_Tomoyuki_2002_01_17} =
    qr{\G(?(?=.{0,32766}\z)(?:[^\x81-\xFE]|[\x81-\xFE][\x00-\xFF])*?|(?(?=[$sbcs]+\z).*?|(?:${Ebig5::q_char_SADAHIRO_Tomoyuki_2002_01_17})))}oxms;
#   qr{
#      \G # (1), (2)
#        (? # (3)
#          (?=.{0,32766}\z) # (4)
...
#                                                                                      (?(?=[$sbcs]+\z) # (6)
#                                                                                                      .*?| #(7)
#                                                                                                          (?:${Ebig5::q_char_SADAHIRO_Tomoyuki_2002_01_17}) # (8)
#                                                                                                                                           ))}oxms;
    ...
    if (((('A' x 32768).'B') !~ / ${Ebig5::anchor}                              B /oxms) and
        ((('A' x 32768).'B') =~ / ${Ebig5::anchor_SADAHIRO_Tomoyuki_2002_01_17} B /oxms)
    ) {
        ${Ebig5::anchor} = ${Ebig5::anchor_SADAHIRO_Tomoyuki_2002_01_17};
    }
    else {
        undef ${Ebig5::q_char_SADAHIRO_Tomoyuki_2002_01_17};
    }
}
...
sub Ebig5::split(;$$$) {
    ...
    if (CORE::length($string) > 32766) {
        ...
        elsif (defined ${Ebig5::q_char_SADAHIRO_Tomoyuki_2002_01_17}) {
            $q_char = ${Ebig5::q_char_SADAHIRO_Tomoyuki_2002_01_17};
        }
    }
...
#
# Big5 lower case first with parameter
#
sub Ebig5::lcfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Ebig5::lc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Ebig5::lc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Ebig5::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}

#
# Big5 lower case first without parameter
#
sub Ebig5::lcfirst_() {
    return Ebig5::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
}
...
#
# Big5 lower case with parameter
#
sub Ebig5::lc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# Big5 lower case without parameter
#
sub Ebig5::lc_() {
    my $s = $_;
    return join '', map {defined($lc{$_}) ? $lc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# Big5 upper case first with parameter
#
sub Ebig5::ucfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Ebig5::uc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Ebig5::uc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Ebig5::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}

#
# Big5 upper case first without parameter
#
sub Ebig5::ucfirst_() {
    return Ebig5::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
}
...
#
# prepare Big5 characters per length
#
...
#
# Big5 open character list for tr
#
sub _charlist_tr {
...
#
# Big5 open character list for qr and not qr
#
sub _charlist {
...
#
# Big5 hexadecimal escape sequence
#
sub hexchr {
    my($hexdigit) = @_;
...
#
# Big5 open character list for qr
#
sub charlist_qr {
...
#
# Big5 order to character (with parameter)
#
sub Ebig5::chr(;$) {
...
#
# Big5 order to character (without parameter)
#
sub Ebig5::chr_() {
...
sub Ebig5::filetest {
    ...
    unless (CORE::eval qq{Ebig5::$filetest(\$file)}) {
        return '';
    }
...
#
# Big5 file test -r expr
#
sub Ebig5::r(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -r (Ebig5::r)' if @_ and not wantarray;
    ...
    # (and so on)
        ...
        if (-d "$_/.") {
            ...
            # Even if ${^WIN32_SLOPPY_STAT} is set to a true value, Ebig5::*()
...
#
# Big5 file test -w expr
#
sub Ebig5::w(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -w (Ebig5::w)' if @_ and not wantarray;
...
#
# Big5 file test -x expr
#
sub Ebig5::x(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -x (Ebig5::x)' if @_ and not wantarray;
...
#
# Big5 file test -o expr
#
sub Ebig5::o(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -o (Ebig5::o)' if @_ and not wantarray;
...
#
# Big5 file test -e expr
#
sub Ebig5::e(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -e (Ebig5::e)' if @_ and not wantarray;
    ...
    elsif (defined Ebig5::telldir($fh)) {
        return wantarray ? ('',@_) : '';
    }
...
#
# Big5 file test -z expr
#
sub Ebig5::z(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -z (Ebig5::z)' if @_ and not wantarray;
...
#
# Big5 file test -s expr
#
sub Ebig5::s(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -s (Ebig5::s)' if @_ and not wantarray;
...
#
# Big5 file test -f expr
#
sub Ebig5::f(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -f (Ebig5::f)' if @_ and not wantarray;
...
#
# Big5 file test -d expr
#
sub Ebig5::d(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -d (Ebig5::d)' if @_ and not wantarray;
...
#
# Big5 file test -l expr
#
sub Ebig5::l(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -l (Ebig5::l)' if @_ and not wantarray;
...
#
# Big5 file test -p expr
#
sub Ebig5::p(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -p (Ebig5::p)' if @_ and not wantarray;
...
#
# Big5 file test -b expr
#
sub Ebig5::b(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -b (Ebig5::b)' if @_ and not wantarray;
...
#
# Big5 file test -u expr
#
sub Ebig5::u(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -u (Ebig5::u)' if @_ and not wantarray;
...
#
# Big5 file test -g expr
#
sub Ebig5::g(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -g (Ebig5::g)' if @_ and not wantarray;
...
#
# Big5 file test -k expr
#
sub Ebig5::k(;*@) {

    local $_ = shift if @_;
    croak 'Too many arguments for -k (Ebig5::k)' if @_ and not wantarray;
...
sub Ebig5::T(;*@) {
    ...
    #     croak 'Too many arguments for -T (Ebig5::T)';
...
sub Ebig5::stat(*) {
    ...
    if (defined fileno $fh) {
        ...
        # Even if ${^WIN32_SLOPPY_STAT} is set to a true value, Ebig5::stat()
        # on Windows opens the file for the path which has 5c at end.
        # (and so on)
...
#
# Big5 file stat (without parameter)
#
sub Ebig5::stat_() {
        ...
        # rejected idea ...
            ...
            if (($] =~ /^5\.006/oxms)                     and
                ...
                *{'Dyna'.'Loader::dl_install_xsub'}{'CODE'}->('_SetCurrentDirectoryA', unpack('L', pack 'P', $x86));
                ...
                if (Ebig5::fc($chdir) eq Ebig5::fc($dir)) {
                    return 1;
                }
                else {
                    return 0;
...
#   long filename form.
        ...
        if (qx{ver} =~ /\b(?:Windows NT|Windows 2000)\b/oms) {
            ...
            for my $dirx (sort { CORE::length($a) <=> CORE::length($b) } @dirx) {
                if (Ebig5::fc(CORE::substr $dirx,-CORE::length($subdir[-1]),CORE::length($subdir[-1])) eq Ebig5::fc($subdir[-1])) {
        ...
        elsif (Ebig5::fc($shortdir) eq Ebig5::fc($dir)) {
            return 0;
        }
        return CORE::chdir $shortdir;
    }
...
#
# Big5 chr(0x5C) ended path on MSWin32
#
...
ITER_DO:
    {
        for my $prefix (@INC) {
            ...
            if (Ebig5::f($realfilename)) {

                my $script = '';

                if (Ebig5::e("$realfilename.e")) {
                    my $e_mtime      = (Ebig5::stat("$realfilename.e"))[9];
                    my $mtime        = (Ebig5::stat($realfilename))[9];
                    my $module_mtime = (Ebig5::stat(__FILE__))[9];
                    if (($e_mtime < $mtime) or ($mtime < $module_mtime)) {
                        Ebig5::unlink "$realfilename.e";
                    }
                }
                ...
                if (Ebig5::e("$realfilename.e")) {
                    ...
                    if ($script =~ /^ (?>\s*) use (?>\s+) Big5 (?>\s*) ([^\x81-\xFE;]*) ; (?>\s*) \n? $/oxms) {
                        CORE::require Big5;
                        $script = Big5::escape_script($script);
                        my $fh = gensym();
...
#
# Big5 character to order (with parameter)
#
sub Big5::ord(;$) {
...
#
# Big5 character to order (without parameter)
#
sub Big5::ord_() {
...
#
# Big5 getc (with parameter, without parameter)
#
sub Big5::getc(;*@) {

    my($package) = caller;
    my $fh = @_ ? qualify_to_ref(shift,$package) : \*STDIN;
    croak 'Too many arguments for Big5::getc' if @_ and not wantarray;
...
#
# Big5 index by character
#
sub Big5::index($$;$) {

    my $index;
    if (@_ == 3) {
        $index = Ebig5::index($_[0], $_[1], CORE::length(Big5::substr($_[0], 0, $_[2])));
    }
    else {
        $index = Ebig5::index($_[0], $_[1]);
    }
    ...
    if ($index == -1) {
        ...
        return Big5::length(CORE::substr $_[0], 0, $index);
    }
}

#
# Big5 rindex by character
#
sub Big5::rindex($$;$) {

    my $rindex;
    if (@_ == 3) {
        $rindex = Ebig5::rindex($_[0], $_[1], CORE::length(Big5::substr($_[0], 0, $_[2])));
    }
    else {
        $rindex = Ebig5::rindex($_[0], $_[1]);
    }
    ...
    if ($rindex == -1) {
        ...
        return Big5::length(CORE::substr $_[0], 0, $rindex);
    }
}
...
# ord() to ord() or Big5::ord()
my $function_ord = 'ord';

# ord to ord or Big5::ord_
my $function_ord_ = 'ord';

# reverse to reverse or Big5::reverse
my $function_reverse = 'reverse';

# getc to getc or Big5::getc
...
my $qq_substr  = qr{(?> Char::substr | Big5::substr | CORE::substr | substr ) (?>\s*) \( $qq_paren \)
                 }xms;
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Ebig5::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while (?>\s*) \( (?>\s*) glob (?>\s*) \) /oxgc) {
        return 'while ($_ = Ebig5::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while (?>\s*) \( (?>\s*) glob \b /oxgc) {
        return 'while ($_ = Ebig5::glob';
    }
...
# subroutines of package Ebig5
    ...
    elsif (/\G \b Big5::eval       (?= (?>\s*) \{ )                    /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval Char::escape'; }
    elsif (/\G \b Big5::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval Big5::escape'; }
    elsif (/\G \b bytes::substr \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'substr';            }
    elsif (/\G \b chop \b          (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Ebig5::chop';       }
    elsif (/\G \b bytes::index \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'index';             }
    elsif (/\G \b Char::index \b   (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Char::index';       }
    elsif (/\G \b Big5::index \b   (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Big5::index';       }
    elsif (/\G \b index \b         (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Ebig5::index';      }
    elsif (/\G \b bytes::rindex \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'rindex';            }
    elsif (/\G \b Char::rindex \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Char::rindex';      }
    elsif (/\G \b Big5::rindex \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Big5::rindex';      }
    elsif (/\G \b rindex \b        (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Ebig5::rindex';     }
    elsif (/\G \b lc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ebig5::lc';         }
    elsif (/\G \b lcfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ebig5::lcfirst';    }
    elsif (/\G \b uc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ebig5::uc';         }
    elsif (/\G \b ucfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ebig5::ucfirst';    }
    elsif (/\G \b fc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ebig5::fc';         }
    ...
    # (and so on)
                                                             ...
                                                             (?>\s*) (\") ((?:$qq_char)+?)             (\") /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_qq('',  $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\#) ((?:$qq_char)+?)             (\#) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\() ((?:$qq_paren)+?)            (\)) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\{) ((?:$qq_brace)+?)            (\}) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\[) ((?:$qq_bracket)+?)          (\]) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\<) ((?:$qq_angle)+?)            (\>) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\S) ((?:$qq_char)+?)             (\2) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB] (?>(?:\s+ -[rwxoRWXOezfdlpSbcugkTB])+))
                                                             (?>\s*) (\') ((?:\\\'|\\\\|$q_char)+?)    (\') /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\S) ((?:\\\2|\\\\|$q_char)+?)    (\2) /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB] (?>(?:\s+ -[rwxoRWXOezfdlpSbcugkTB])+)) (\$ (?> \w+ (?: ::\w+)* ) (?: (?: ->)? (?: [\$\@\%\&\*]\* | \$\#\* | \( (?:$qq_paren)*? \) | [\@\%\*]? \{ (?:$qq_brace)+? \} | [\@\%]? \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                                   { $slash = 'm//'; return "Ebig5::filetest(qw($1),$2)"; }
    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB] (?>(?:\s+ -[rwxoRWXOezfdlpSbcugkTB])+)) \( ((?:$qq_paren)*?) \)   /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1),$2)"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) (?= [a-z]+)                                       /oxgc) { $slash = 'm//'; return "Ebig5::filetest qw($1),";    }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) ((?>\w+))                                         /oxgc) { $slash = 'm//'; return "Ebig5::filetest(qw($1),$2)"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC])            (?>\s*) (\") ((?:$qq_char)+?)                (\") /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_qq('',  $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) qq (?>\s*) (\#) ((?:$qq_char)+?)                (\#) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) qq (?>\s*) (\() ((?:$qq_paren)+?)               (\)) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) qq (?>\s*) (\{) ((?:$qq_brace)+?)               (\}) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) qq (?>\s*) (\[) ((?:$qq_bracket)+?)             (\]) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) qq (?>\s*) (\<) ((?:$qq_angle)+?)               (\>) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) qq (?>\s*) (\S) ((?:$qq_char)+?)                (\2) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC])            (?>\s*) (\') ((?:\\\'|\\\\|$q_char)+?)       (\') /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_q ('',  $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) q  (?>\s*) (\#) ((?:\\\#|\\\\|$q_char)+?)       (\#) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) q  (?>\s*) (\() ((?:\\\)|\\\\|$q_paren)+?)      (\)) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) q  (?>\s*) (\{) ((?:\\\}|\\\\|$q_brace)+?)      (\}) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) q  (?>\s*) (\[) ((?:\\\]|\\\\|$q_bracket)+?)    (\]) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) q  (?>\s*) (\<) ((?:\\\>|\\\\|$q_angle)+?)      (\>) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) q  (?>\s*) (\S) ((?:\\\2|\\\\|$q_char)+?)       (\2) /oxgc) { $slash = 'm//'; return "Ebig5::$1(" . e_q ('q', $2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s*) (\$ (?> \w+ (?: ::\w+)* ) (?: (?: ->)? (?: [\$\@\%\&\*]\* | \$\#\* | \( (?:$qq_paren)*? \) | [\@\%\*]? \{ (?:$qq_brace)+? \} | [\@\%]? \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                                   { $slash = 'm//'; return "Ebig5::$1($2)";             }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s*) \( ((?:$qq_paren)*?) \)                              /oxgc) { $slash = 'm//'; return "Ebig5::$1($2)";             }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?= (?>\s+) [a-z]+)                                          /oxgc) { $slash = 'm//'; return "Ebig5::$1";                 }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?>\s+) ((?>\w+))                                            /oxgc) { $slash = 'm//'; return "Ebig5::$1(::"."$2)";        }
    elsif (/\G -(t)                            (?>\s+) ((?>\w+))                                            /oxgc) { $slash = 'm//'; return "-t $2";                     }
    elsif (/\G \b lstat         (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(])                                /oxgc) { $slash = 'm//'; return 'Ebig5::lstat';              }
    elsif (/\G \b stat          (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(])                                /oxgc) { $slash = 'm//'; return 'Ebig5::stat';               }
    ...
    elsif (/\G \b chr           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ebig5::chr';               }
    elsif (/\G \b bytes::ord    (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ebig5::glob';              }
    elsif (/\G \b lc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ebig5::lc_';               }
    elsif (/\G \b lcfirst \b       (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ebig5::lcfirst_';          }
    elsif (/\G \b uc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ebig5::uc_';               }
    elsif (/\G \b ucfirst \b       (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ebig5::ucfirst_';          }
    elsif (/\G \b fc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ebig5::fc_';               }
    elsif (/\G \b lstat \b         (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ebig5::lstat_';            }
    elsif (/\G \b stat \b          (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ebig5::stat_';             }
    elsif (/\G    (-[rwxoRWXOezfdlpSbcugkTB] (?>(?:\s+ -[rwxoRWXOezfdlpSbcugkTB])+))
                     \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return "Ebig5::filetest_(qw($1))"; }
    elsif (/\G    -([rwxoRWXOezsfdlpSbcugkTBMAC])
                     \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return "Ebig5::${1}_";             }
    ...
    elsif (/\G \b chr \b           (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ebig5::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! (?>\s*) => )                          /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! (?>\s*) => )                          /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ebig5::glob_';             }
    elsif (/\G \b reverse \b       (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return $function_reverse;          }
    elsif (/\G \b getc \b          (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return $function_getc;             }
    elsif (/\G \b opendir ((?>\s*) \( (?>\s*)) (?=[A-Za-z_])                 /oxgc) { $slash = 'm//'; return "Ebig5::opendir$1*";        }
    elsif (/\G \b opendir ((?>\s+))            (?=[A-Za-z_])                 /oxgc) { $slash = 'm//'; return "Ebig5::opendir$1*";        }
    elsif (/\G \b unlink \b       (?! (?>\s*) => )                           /oxgc) { $slash = 'm//'; return 'Ebig5::unlink';            }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Ebig5::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Ebig5::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq           (\#) [ ] (\#) /oxgc) { return 'Ebig5::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq ((?>\s*)) (\() [ ] (\)) /oxgc) { return 'Ebig5::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\{) [ ] (\}) /oxgc) { return 'Ebig5::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\[) [ ] (\]) /oxgc) { return 'Ebig5::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\<) [ ] (\>) /oxgc) { return 'Ebig5::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\S) [ ] (\2) /oxgc) { return 'Ebig5::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q            (\#) [ ] (\#) /oxgc) { return 'Ebig5::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  ((?>\s*)) (\() [ ] (\)) /oxgc) { return 'Ebig5::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\{) [ ] (\}) /oxgc) { return 'Ebig5::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\[) [ ] (\]) /oxgc) { return 'Ebig5::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\<) [ ] (\>) /oxgc) { return 'Ebig5::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\S) [ ] (\2) /oxgc) { return 'Ebig5::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                    ' [ ] '    /oxgc) { return 'Ebig5::split' . $e . qq     {' '};  }
        elsif (/\G                    " [ ] "    /oxgc) { return 'Ebig5::split' . $e . qq     {" "};  }
...
# do
    ...
    elsif (/\G \b do (?= (?>\s+) (?: q | qq | qx ) \b)                                        /oxmsgc) { return 'Ebig5::do';         }
    elsif (/\G \b do (?= (?>\s+) (?>\w+))                                                     /oxmsgc) { return 'do';                }
    elsif (/\G \b do (?= (?>\s*) \$ (?> \w+ (?: ::\w+)* ) \( )                                /oxmsgc) { return 'do';                }
    elsif (/\G \b do \b                                                                       /oxmsgc) { return 'Ebig5::do';         }
...
# require else
    elsif (/\G \b require                                       (?>\s*) ;                     /oxmsgc) { return 'Ebig5::require;';   }
    elsif (/\G \b require \b                                                                  /oxmsgc) { return 'Ebig5::require';    }
...
# subroutines of package Ebig5
                                                                            ...
                                                                            (?>\s*) (\") ((?:$qq_char)+?)             (\") /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\#) ((?:$qq_char)+?)             (\#) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\() ((?:$qq_paren)+?)            (\)) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\{) ((?:$qq_brace)+?)            (\}) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\[) ((?:$qq_bracket)+?)          (\]) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\<) ((?:$qq_angle)+?)            (\>) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) qq (?>\s*) (\S) ((?:$qq_char)+?)             (\2) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB] (?>(?:\s+ -[rwxoRWXOezfdlpSbcugkTB])+))
                                                                            (?>\s*) (\') ((?:\\\'|\\\\|$q_char)+?)    (\') /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) q  (?>\s*) (\S) ((?:\\\2|\\\\|$q_char)+?)    (\2) /oxgc) { $e_string .= "Ebig5::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB] (?>(?:\s+ -[rwxoRWXOezfdlpSbcugkTB])+)) (\$ (?> \w+ (?: ::\w+)*) (?: (?: ->)? (?: [\$\@\%\&\*]\* | \$\#\* | \( (?:$qq_paren)*? \) | [\@\%\*]? \{ (?:$qq_brace)+? \} | [\@\%]? \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                                                  { $e_string .= "Ebig5::filetest(qw($1),$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB] (?>(?:\s+ -[rwxoRWXOezfdlpSbcugkTB])+)) \( ((?:$qq_paren)*?) \)   /oxgc) { $e_string .= "Ebig5::filetest(qw($1),$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) (?= [a-z]+)                                       /oxgc) { $e_string .= "Ebig5::filetest qw($1),";    $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB](?>\s+)){2,}) ((?>\w+))                                         /oxgc) { $e_string .= "Ebig5::filetest(qw($1),$2)"; $slash = 'm//'; }
...
sub character_class {
    ...
    if ($char eq '.') {
        ...
        return Ebig5::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Ebig5::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Ebig5::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Ebig5::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ebig5::ignorecase(Ebig5::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Ebig5::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Ebig5::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ebig5::ignorecase(Ebig5::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Ebig5::MATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                               1                                                2   3                                  4   5
                q<CORE::eval{local $Ebig5::re_t=%s; local $Ebig5::re_a=''; while($Ebig5::re_t =~ %s){%s local $^W=0; local $Ebig5::re_r=%s; %s$Ebig5::re_t="$Ebig5::re_a${1}$Ebig5::re_r$'"; pos($Ebig5::re_t)=length "$Ebig5::re_a${1}$Ebig5::re_r"; $Ebig5::re_a=substr($Ebig5::re_t,0,pos($Ebig5::re_t)); } return $Ebig5::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Ebig5::re_r=CORE::eval $Ebig5::re_r; ' x $e_modifier,          #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                               1                         2   3                                  4   5
                q<CORE::eval{local $Ebig5::re_t=%s; while($Ebig5::re_t =~ %s){%s local $^W=0; local $Ebig5::re_r=%s; %s$Ebig5::re_t="$`$Ebig5::re_r$'"; pos($Ebig5::re_t)=length "$`$Ebig5::re_r"; } return $Ebig5::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                  4   5  6                     7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Ebig5::re_r=%s; %s"%s$Ebig5::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<CORE::eval{local $Ebig5::re_n=0; local $Ebig5::re_a=''; while(%s =~ %s){%s local $^W=0; local $Ebig5::re_r=%s; %s%s="$Ebig5::re_a${1}$Ebig5::re_r$'"; pos(%s)=length "$Ebig5::re_a${1}$Ebig5::re_r"; $Ebig5::re_a=substr(%s,0,pos(%s)); $Ebig5::re_n++} return %s$Ebig5::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                        1     2   3                                  4   5 6                          7                                                   8
                q<CORE::eval{local $Ebig5::re_n=0; while(%s =~ %s){%s local $^W=0; local $Ebig5::re_r=%s; %s%s="$`$Ebig5::re_r$'"; pos(%s)=length "$`$Ebig5::re_r"; $Ebig5::re_n++} return %s$Ebig5::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                  5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Ebig5::re_r=%s; %s%s="%s$Ebig5::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                  5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Ebig5::re_r=%s; %s%s="%s$Ebig5::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Ebig5::re_r=CORE::eval $Ebig5::re_r; ' x $e_modifier,          #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Ebig5::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ (?>\s*) use (?>\s+) Big5 (?>\s*) ([^\x81-\xFE;]*) ; (?>\s*) \n? $/oxms) {
                ...
                return qq[BEGIN { Ebig5::require '$expr'; $module->import() if $module->can('import'); }];
            }
            last;
        }
    }
...
sub e_no_noparam {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Ebig5::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ (?>\s*) use (?>\s+) Big5 (?>\s*) ([^\x81-\xFE;]*) ; (?>\s*) \n? $/oxms) {
                return qq[BEGIN { Ebig5::require '$expr'; $module->unimport() if $module->can('unimport'); }];
            }
            last;
        }
    }
...
sub e_use {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Ebig5::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ (?>\s*) use (?>\s+) Big5 (?>\s*) ([^\x81-\xFE;]*) ; (?>\s*) \n? $/oxms) {
                return qq[BEGIN { Ebig5::require '$expr'; $module->import($list) if $module->can('import'); }];
            }
            last;
        }
    }
...
This module has run-time routines for use Big5 software automatically, you
do not have to use.
...
some bugs. Thus, if you happen to find a bug that's in Big5 software and not
...
This Ebig5 module first appeared in ActivePerl Build 522 Built under
...
=item * Split string
  ...
  @split = Ebig5::split(/pattern/,$string,$limit);
  @split = Ebig5::split(/pattern/,$string);
  @split = Ebig5::split(/pattern/);
  @split = Ebig5::split('',$string,$limit);
  @split = Ebig5::split('',$string);
  @split = Ebig5::split('');
  @split = Ebig5::split();
  @split = Ebig5::split;
  ...
  at all, Ebig5::split returns the original string as a single substring, If it
  ...
  @chars  = Ebig5::split(//,  $word);
  @fields = Ebig5::split(/:/, $line);
  @words  = Ebig5::split(" ", $paragraph);
  @lines  = Ebig5::split(/^/, $buffer);
  ...
  print join(":" => Ebig5::split(/ */, "hi there"));
  ...
  Ebig5::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Ebig5::split(/:/, $_, 3);
...
=item * Chop string
  ...
  You cannot Ebig5::chop a literal, only a variable. If you Ebig5::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Ebig5::chop(@lines);

  You can Ebig5::chop anything that is an lvalue, including an assignment:

  Ebig5::chop($cwd = `pwd`);
  Ebig5::chop($answer = <STDIN>);

  This is different from:

  $answer = Ebig5::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Ebig5::chop returns the character
  ...
  In the most general case, Ebig5::chop can be expressed using substr:

  $last_code = Ebig5::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Ebig5::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Ebig5::chop;
...
=item * Index string
  ...
  $byte_pos = Ebig5::index($string,$substr,$byte_offset);
  $byte_pos = Ebig5::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Ebig5::index($string, $lookfor, $byte_pos)) > -1) {
...
=item * Reverse index string
  ...
  $byte_pos = Ebig5::rindex($string,$substr,$byte_offset);
  $byte_pos = Ebig5::rindex($string,$substr);

  This subroutine works just like Ebig5::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Ebig5::index).
  ...
  while (($byte_pos = Big5::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item * File test subroutine Ebig5::X
  ...
  Subroutine and Prototype   Meaning
  ------------------------------------------------------------------------------
  Ebig5::s(*), Ebig5::s_()   File or directory exists and has nonzero size
                             (the value is the size in bytes)
  ------------------------------------------------------------------------------

=item * Filename expansion (globbing)

  @glob = Ebig5::glob($string);
  @glob = Ebig5::glob_;
  ...
  The Ebig5::glob subroutine grandfathers the use of whitespace to separate multiple
...
=item * Do file
  ...
  except that Ebig5::do is more efficient, more concise, keeps track of the current
  filename for error messages, searches all the directories listed in the @INC
  array, and updates %INC if the file is found.
  It also differs in that code evaluated with Ebig5::do FILE can not see lexicals in
  ...
  If Ebig5::do can't read the file, it returns undef and sets $! to the error. If 
  Ebig5::do can read the file but can't compile it, it returns undef and sets an
  ...
  with the use and require operators, which also Ebig5::do error checking and raise
  ...
  But Ebig5::do FILE is still useful for such things as reading program configuration
  files. Manual error checking can be done this way:

  # read in config files: system first, then user
  for $file ("/usr/share/proggie/defaults.rc", "$ENV{HOME}/.someprogrc") {
      unless ($return = Ebig5::do($file)) {
          warn "couldn't parse $file: $@" if $@;
          warn "couldn't Ebig5::do($file): $!" unless defined $return;
  ...
  Ebig5::do to reload that file. This is more tidily accomplished with Ebig5::do than
  with Ebig5::require.
...
=item * Require file
  ...
  Ebig5::require loads and executes the Perl code found in the separate file whose
  name is given by the $file. This is similar to using a Ebig5::do on a file, except
  that Ebig5::require checks to see whether the library file has been loaded already
  ...
  its cousins Ebig5::do, Ebig5::require knows how to search the include path stored
...
=item * Current position of the readdir
  ...
  $telldir = Ebig5::telldir(DIRHANDLE);
