...
Lingua::FRA::Numbers - Converts numeric values into their French string
equivalents
...
=head1 SYNOPSIS
 ...
 # Procedural Style
 use Lingua::FRA::Numbers qw(number_to_fr ordinate_to_fr);
 ...
 my $number = Lingua::FRA::Numbers->new( 123 );
 print $number->get_string;
 print $number->get_ordinate;

 my $other_number = Lingua::FRA::Numbers->new;
...
=head2 ordinate_to_fr( $number )
 ...
 use Lingua::FRA::Numbers qw(ordinate_to_fr);
...
=head2 new( [ $number ] )
 ...
 my $start = Lingua::FRA::Numbers->new( 500 );
 my $end   = Lingua::FRA::Numbers->new( 3000 );
