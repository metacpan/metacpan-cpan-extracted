...
######################################################################
#
# Ekoi8u - Run-time routines for KOI8U.pm
#
...
#
# KOI8-U character range per length
#
my %range_tr = ();

#
# KOI8-U case conversion
#
my %lc = ();
...
sub import {
    ...
    *Char::ord           = \&KOI8U::ord;
    *Char::ord_          = \&KOI8U::ord_;
    *Char::reverse       = \&KOI8U::reverse;
    *Char::getc          = \&KOI8U::getc;
    *Char::length        = \&KOI8U::length;
    *Char::substr        = \&KOI8U::substr;
    *Char::index         = \&KOI8U::index;
    *Char::rindex        = \&KOI8U::rindex;
    *Char::eval          = \&KOI8U::eval;
    *Char::escape        = \&KOI8U::escape;
    *Char::escape_token  = \&KOI8U::escape_token;
    *Char::escape_script = \&KOI8U::escape_script;
}
...
sub Ekoi8u::fc(@);
sub Ekoi8u::fc_();
sub Ekoi8u::ignorecase;
sub Ekoi8u::classic_character_class;
sub Ekoi8u::capture;
sub Ekoi8u::chr(;$);
sub Ekoi8u::chr_();
sub Ekoi8u::glob($);
...
# ${Ekoi8u::eS}          = qr{(?>[^\x09\x0A\x0B\x0C\x0D\x20])};
${Ekoi8u::eS}            = qr{(?>[^\s])};
...
# avoid: Name "Ekoi8u::foo" used only once: possible typo at here.
${Ekoi8u::dot}         = ${Ekoi8u::dot};
${Ekoi8u::dot_s}       = ${Ekoi8u::dot_s};
${Ekoi8u::eD}          = ${Ekoi8u::eD};
${Ekoi8u::eS}          = ${Ekoi8u::eS};
${Ekoi8u::eW}          = ${Ekoi8u::eW};
${Ekoi8u::eH}          = ${Ekoi8u::eH};
...
${Ekoi8u::eR}          = ${Ekoi8u::eR};
${Ekoi8u::eN}          = ${Ekoi8u::eN};
${Ekoi8u::not_alnum}   = ${Ekoi8u::not_alnum};
${Ekoi8u::not_alpha}   = ${Ekoi8u::not_alpha};
${Ekoi8u::not_ascii}   = ${Ekoi8u::not_ascii};
${Ekoi8u::not_blank}   = ${Ekoi8u::not_blank};
${Ekoi8u::not_cntrl}   = ${Ekoi8u::not_cntrl};
${Ekoi8u::not_digit}   = ${Ekoi8u::not_digit};
${Ekoi8u::not_graph}   = ${Ekoi8u::not_graph};
${Ekoi8u::not_lower}   = ${Ekoi8u::not_lower};
${Ekoi8u::not_lower_i} = ${Ekoi8u::not_lower_i};
${Ekoi8u::not_print}   = ${Ekoi8u::not_print};
${Ekoi8u::not_punct}   = ${Ekoi8u::not_punct};
${Ekoi8u::not_space}   = ${Ekoi8u::not_space};
${Ekoi8u::not_upper}   = ${Ekoi8u::not_upper};
${Ekoi8u::not_upper_i} = ${Ekoi8u::not_upper_i};
${Ekoi8u::not_word}    = ${Ekoi8u::not_word};
${Ekoi8u::not_xdigit}  = ${Ekoi8u::not_xdigit};
${Ekoi8u::eb}          = ${Ekoi8u::eb};
${Ekoi8u::eB}          = ${Ekoi8u::eB};
...
#
# KOI8-U lower case first with parameter
#
sub Ekoi8u::lcfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Ekoi8u::lc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Ekoi8u::lc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Ekoi8u::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}

#
# KOI8-U lower case first without parameter
#
sub Ekoi8u::lcfirst_() {
    return Ekoi8u::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
}
...
#
# KOI8-U lower case with parameter
#
sub Ekoi8u::lc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# KOI8-U lower case without parameter
#
sub Ekoi8u::lc_() {
    my $s = $_;
    return join '', map {defined($lc{$_}) ? $lc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# KOI8-U upper case first with parameter
#
sub Ekoi8u::ucfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Ekoi8u::uc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Ekoi8u::uc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Ekoi8u::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}

#
# KOI8-U upper case first without parameter
#
sub Ekoi8u::ucfirst_() {
    return Ekoi8u::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
}
...
#
# KOI8-U upper case with parameter
#
sub Ekoi8u::uc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# KOI8-U upper case without parameter
#
sub Ekoi8u::uc_() {
    my $s = $_;
    return join '', map {defined($uc{$_}) ? $uc{$_} : $_} ($s =~ /\G ($q_char) /oxmsg);
...
#
# KOI8-U fold case with parameter
#
sub Ekoi8u::fc(@) {
    if (@_) {
        my $s = shift @_;
...
#
# KOI8-U regexp ignore case modifier
#
sub Ekoi8u::ignorecase {
    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # with /i modifier
                ...
                my $uc = Ekoi8u::uc($char[$i]);
                my $fc = Ekoi8u::fc($char[$i]);
                if ($uc ne $fc) {
                    if (CORE::length($fc) == 1) {
                        $char[$i] = '['   . $uc       . $fc . ']';
                    }
...
#
# prepare KOI8-U characters per length
#
...
#
# KOI8-U open character list for tr
#
sub _charlist_tr {
...
#
# KOI8-U open character list for qr and not qr
#
sub _charlist {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '${Ekoi8u::not_lower_i}',
                '[:^upper:]'  => '${Ekoi8u::not_upper_i}',
...
#
# KOI8-U hexadecimal escape sequence
#
sub hexchr {
    my($hexdigit) = @_;
...
#
# KOI8-U open character list for qr
#
sub charlist_qr {
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Ekoi8u::uc($char);
                        my $fc = Ekoi8u::fc($char);
                        if ($uc eq $fc) {
...
#
# KOI8-U open character list for not qr
#
sub charlist_not_qr {
...
#
# KOI8-U order to character (with parameter)
#
sub Ekoi8u::chr(;$) {
...
#
# KOI8-U order to character (without parameter)
#
sub Ekoi8u::chr_() {
...
#
# KOI8-U path globbing (with parameter)
#
sub Ekoi8u::glob($) {
...
#
# KOI8-U path globbing (without parameter)
#
sub Ekoi8u::glob_() {
...
#
# KOI8-U path globbing via File::DosGlob 1.10
#
...
#
# KOI8-U path globbing subroutine
#
sub _do_glob {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $fc = Ekoi8u::fc($char)) ne $char) {
                $pattern .= $fc;
            }
            else {
        ...
        my $matchsub = sub { Ekoi8u::fc($_[0]) =~ /\A $pattern \z/xms };
...
INNER:
        for my $leaf (@leaf) {
            ...
            if (Ekoi8u::index($leaf,'.') == -1 and   # if name does not have a dot in it *and*
                CORE::length($leaf) <= 8 and        # name is shorter than or equal to 8 chars *and*
                Ekoi8u::index($pattern,'\\.') != -1  # pattern has a dot.
            ) {
                if (&$matchsub("$leaf.")) {
                    push @matched, "$head$leaf";
                    next INNER;
...
#
sub Ekoi8u::POSTMATCH {
    return $';
}

#
# KOI8-U character to order (with parameter)
#
sub KOI8U::ord(;$) {
...
#
# KOI8-U character to order (without parameter)
#
sub KOI8U::ord_() {
...
#
# KOI8-U getc (with parameter, without parameter)
#
sub KOI8U::getc(;*@) {

    my($package) = caller;
    my $fh = @_ ? qualify_to_ref(shift,$package) : \*STDIN;
    croak 'Too many arguments for KOI8U::getc' if @_ and not wantarray;
...
#
# KOI8-U index by character
#
sub KOI8U::index($$;$) {

    my $index;
    if (@_ == 3) {
        $index = Ekoi8u::index($_[0], $_[1], CORE::length(KOI8U::substr($_[0], 0, $_[2])));
    }
    else {
        $index = Ekoi8u::index($_[0], $_[1]);
    }
    ...
    if ($index == -1) {
        ...
        return KOI8U::length(CORE::substr $_[0], 0, $index);
    }
}

#
# KOI8-U rindex by character
#
sub KOI8U::rindex($$;$) {

    my $rindex;
    if (@_ == 3) {
        $rindex = Ekoi8u::rindex($_[0], $_[1], CORE::length(KOI8U::substr($_[0], 0, $_[2])));
    }
    else {
        $rindex = Ekoi8u::rindex($_[0], $_[1]);
    }
    ...
    if ($rindex == -1) {
        ...
        return KOI8U::length(CORE::substr $_[0], 0, $rindex);
    }
}
...
# ord() to ord() or KOI8U::ord()
my $function_ord = 'ord';

# ord to ord or KOI8U::ord_
my $function_ord_ = 'ord';

# reverse to reverse or KOI8U::reverse
my $function_reverse = 'reverse';

# getc to getc or KOI8U::getc
...
my $qq_substr  = qr{(?> Char::substr | KOI8U::substr | CORE::substr | substr ) (?>\s*) \( $qq_paren \)
                 }xms;
...
#
# escape KOI8-U token of script
#
sub KOI8U::escape_token {
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Ekoi8u::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ (?>\s*) PREMATCH  \b | \$ (?>\s*) \{ (?>\s*) PREMATCH  (?>\s*) \} | \$ (?>\s*) \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Ekoi8u::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Ekoi8u::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ (?>\s*) MATCH     \b | \$ (?>\s*) \{ (?>\s*) MATCH     (?>\s*) \} | \$ (?>\s*) \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Ekoi8u::MATCH()};
    }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Ekoi8u::POSTMATCH()
    elsif (/\G (                 \$ (?>\s*) POSTMATCH \b | \$ (?>\s*) \{ (?>\s*) POSTMATCH (?>\s*) \} | \$ (?>\s*) \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Ekoi8u::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Ekoi8u::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while (?>\s*) \( (?>\s*) glob (?>\s*) \) /oxgc) {
        return 'while ($_ = Ekoi8u::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while (?>\s*) \( (?>\s*) glob \b /oxgc) {
        return 'while ($_ = Ekoi8u::glob';
    }
...
# subroutines of package Ekoi8u
    elsif (/\G \b (CORE:: | ->(>?\s*) (?: atan2 | [a-z]{2,})) \b       /oxgc) { $slash = 'm//'; return $1;                  }
    elsif (/\G \b Char::eval       (?= (?>\s*) \{ )                    /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b KOI8U::eval       (?= (?>\s*) \{ )                    /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval Char::escape'; }
    elsif (/\G \b KOI8U::eval    \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'eval KOI8U::escape'; }
    elsif (/\G \b bytes::substr \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'substr';            }
    elsif (/\G \b chop \b          (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Ekoi8u::chop';       }
    elsif (/\G \b bytes::index \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'index';             }
    elsif (/\G \b Char::index \b   (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Char::index';       }
    elsif (/\G \b KOI8U::index \b   (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'KOI8U::index';       }
    elsif (/\G \b index \b         (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Ekoi8u::index';      }
    elsif (/\G \b bytes::rindex \b (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'rindex';            }
    elsif (/\G \b Char::rindex \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Char::rindex';      }
    elsif (/\G \b KOI8U::rindex \b  (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'KOI8U::rindex';      }
    elsif (/\G \b rindex \b        (?! (?>\s*) => )                    /oxgc) { $slash = 'm//'; return 'Ekoi8u::rindex';     }
    elsif (/\G \b lc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ekoi8u::lc';         }
    elsif (/\G \b lcfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ekoi8u::lcfirst';    }
    elsif (/\G \b uc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ekoi8u::uc';         }
    elsif (/\G \b ucfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ekoi8u::ucfirst';    }
    elsif (/\G \b fc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ekoi8u::fc';         }
    ...
    elsif (/\G \b chr           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ekoi8u::chr';               }
    elsif (/\G \b bytes::ord    (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Ekoi8u::glob';              }
    elsif (/\G \b lc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ekoi8u::lc_';               }
    elsif (/\G \b lcfirst \b       (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ekoi8u::lcfirst_';          }
    elsif (/\G \b uc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ekoi8u::uc_';               }
    elsif (/\G \b ucfirst \b       (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ekoi8u::ucfirst_';          }
    elsif (/\G \b fc \b            (?! (?>\s*) => )                          /oxgc) { $slash = 'm//'; return 'Ekoi8u::fc_';               }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Ekoi8u::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Ekoi8u::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq           (\#) [ ] (\#) /oxgc) { return 'Ekoi8u::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq ((?>\s*)) (\() [ ] (\)) /oxgc) { return 'Ekoi8u::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\{) [ ] (\}) /oxgc) { return 'Ekoi8u::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\[) [ ] (\]) /oxgc) { return 'Ekoi8u::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\<) [ ] (\>) /oxgc) { return 'Ekoi8u::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq ((?>\s*)) (\S) [ ] (\2) /oxgc) { return 'Ekoi8u::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q            (\#) [ ] (\#) /oxgc) { return 'Ekoi8u::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  ((?>\s*)) (\() [ ] (\)) /oxgc) { return 'Ekoi8u::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\{) [ ] (\}) /oxgc) { return 'Ekoi8u::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\[) [ ] (\]) /oxgc) { return 'Ekoi8u::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\<) [ ] (\>) /oxgc) { return 'Ekoi8u::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  ((?>\s*)) (\S) [ ] (\2) /oxgc) { return 'Ekoi8u::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                    ' [ ] '    /oxgc) { return 'Ekoi8u::split' . $e . qq     {' '};  }
        elsif (/\G                    " [ ] "    /oxgc) { return 'Ekoi8u::split' . $e . qq     {" "};  }
...
# subroutines of package Ekoi8u
        ...
        elsif ($string =~ /\G \b KOI8U::eval       (?= (?>\s*) \{ )                    /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::eval \b                                        /oxgc) { $e_string .= 'eval Char::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b KOI8U::eval \b                                        /oxgc) { $e_string .= 'eval KOI8U::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::substr \b                                     /oxgc) { $e_string .= 'substr';            $slash = 'm//'; }
        elsif ($string =~ /\G \b chop \b                                              /oxgc) { $e_string .= 'Ekoi8u::chop';       $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                                      /oxgc) { $e_string .= 'index';             $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::index \b                                       /oxgc) { $e_string .= 'Char::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b KOI8U::index \b                                       /oxgc) { $e_string .= 'KOI8U::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                             /oxgc) { $e_string .= 'Ekoi8u::index';      $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b KOI8U::rindex \b                                      /oxgc) { $e_string .= 'KOI8U::rindex';      $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                            /oxgc) { $e_string .= 'Ekoi8u::rindex';     $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ekoi8u::lc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ekoi8u::lcfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ekoi8u::uc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ekoi8u::ucfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ekoi8u::fc';         $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ekoi8u::chr';           $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= (?>\s+)[A-Za-z_]|(?>\s*)['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Ekoi8u::glob';          $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                                      /oxgc) { $e_string .= 'Ekoi8u::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                                 /oxgc) { $e_string .= 'Ekoi8u::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                                      /oxgc) { $e_string .= 'Ekoi8u::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                                 /oxgc) { $e_string .= 'Ekoi8u::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                                      /oxgc) { $e_string .= 'Ekoi8u::fc_';               $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Ekoi8u::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq           (\#) [ ] (\#) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\() [ ] (\)) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\{) [ ] (\}) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\[) [ ] (\]) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\<) [ ] (\>) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq ((?>\s*)) (\S) [ ] (\2) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q            (\#) [ ] (\#) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\() [ ] (\)) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\{) [ ] (\}) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\[) [ ] (\]) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\<) [ ] (\>) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  ((?>\s*)) (\S) [ ] (\2) /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                    ' [ ] '    /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                    " [ ] "    /oxgc) { $e_string .= 'Ekoi8u::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        ...
        return Ekoi8u::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Ekoi8u::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Ekoi8u::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Ekoi8u::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Ekoi8u::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Ekoi8u::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Ekoi8u::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Ekoi8u::uc qq<';
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Ekoi8u::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Ekoi8u::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Ekoi8u::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Ekoi8u::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Ekoi8u::uc($char[$i]) ne Ekoi8u::fc($char[$i]))) {
            if (CORE::length(Ekoi8u::fc($char[$i])) == 1) {
                $char[$i] = '['   . Ekoi8u::uc($char[$i])       . Ekoi8u::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Ekoi8u::uc($char[$i]) . '|' . Ekoi8u::fc($char[$i]) . ')';
            }
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Ekoi8u::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ekoi8u::ignorecase(Ekoi8u::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Ekoi8u::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Ekoi8u::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ekoi8u::ignorecase(Ekoi8u::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Ekoi8u::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Ekoi8u::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Ekoi8u::ignorecase(Ekoi8u::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Ekoi8u::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                               1                         2   3                                  4   5
                q<CORE::eval{local $Ekoi8u::re_t=%s; while($Ekoi8u::re_t =~ %s){%s local $^W=0; local $Ekoi8u::re_r=%s; %s$Ekoi8u::re_t="$`$Ekoi8u::re_r$'"; pos($Ekoi8u::re_t)=length "$`$Ekoi8u::re_r"; } return $Ekoi8u::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Ekoi8u::re_r=CORE::eval $Ekoi8u::re_r; ' x $e_modifier,          #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                  4   5  6                     7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Ekoi8u::re_r=%s; %s"%s$Ekoi8u::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                        1     2   3                                  4   5 6                          7                                                   8
                q<CORE::eval{local $Ekoi8u::re_n=0; while(%s =~ %s){%s local $^W=0; local $Ekoi8u::re_r=%s; %s%s="$`$Ekoi8u::re_r$'"; pos(%s)=length "$`$Ekoi8u::re_r"; $Ekoi8u::re_n++} return %s$Ekoi8u::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                  5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Ekoi8u::re_r=%s; %s%s="%s$Ekoi8u::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                  5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Ekoi8u::re_r=%s; %s%s="%s$Ekoi8u::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Ekoi8u::re_r=CORE::eval $Ekoi8u::re_r; ' x $e_modifier,          #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Ekoi8u::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Ekoi8u::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
=head1 SYNOPSIS
  ...
  # "no Ekoi8u;" not supported

=head1 ABSTRACT

This module has run-time routines for use KOI8U software automatically, you
do not have to use.
...
some bugs. Thus, if you happen to find a bug that's in KOI8U software and not
...
This Ekoi8u module first appeared in ActivePerl Build 522 Built under
...
=item * Split string
  ...
  @split = Ekoi8u::split(/pattern/,$string,$limit);
  @split = Ekoi8u::split(/pattern/,$string);
  @split = Ekoi8u::split(/pattern/);
  @split = Ekoi8u::split('',$string,$limit);
  @split = Ekoi8u::split('',$string);
  @split = Ekoi8u::split('');
  @split = Ekoi8u::split();
  @split = Ekoi8u::split;
  ...
  at all, Ekoi8u::split returns the original string as a single substring, If it
  ...
  @chars  = Ekoi8u::split(//,  $word);
  @fields = Ekoi8u::split(/:/, $line);
  @words  = Ekoi8u::split(" ", $paragraph);
  @lines  = Ekoi8u::split(/^/, $buffer);
  ...
  print join(":" => Ekoi8u::split(/ */, "hi there"));
  ...
  Ekoi8u::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Ekoi8u::split(/:/, $_, 3);
  ...
  splits on whitespace just as Ekoi8u::split with no arguments does. Thus,
  Ekoi8u::split(" ") can be used to emulate awk's default behavior. In contrast,
  Ekoi8u::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Ekoi8u::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Ekoi8u::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Ekoi8u::split()) {
          $count{$word}++;
      }
  }

  The inverse of Ekoi8u::split is join, except that join can only join with the
...
=item * Transliteration
  ...
  $tr = Ekoi8u::tr($variable,$bind_operator,$searchlist,$replacementlist,$modifier);
  $tr = Ekoi8u::tr($variable,$bind_operator,$searchlist,$replacementlist);
  ...
  This subroutine scans a KOI8-U string character by character and replaces all
  occurrences of the characters found in $searchlist with the corresponding character
  in $replacementlist. It returns the number of characters replaced or deleted.
  If no KOI8-U string is specified via =~ operator, the $_ variable is translated.
  $modifier are:
  ...
  print Ekoi8u::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'

=item * Chop string

  $chop = Ekoi8u::chop(@list);
  $chop = Ekoi8u::chop();
  $chop = Ekoi8u::chop;

  This subroutine chops off the last character of a string variable and returns the
  character chopped. The Ekoi8u::chop subroutine is used primary to remove the newline
  ...
  since Ekoi8u::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Ekoi8u::chop a literal, only a variable. If you Ekoi8u::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Ekoi8u::chop(@lines);

  You can Ekoi8u::chop anything that is an lvalue, including an assignment:

  Ekoi8u::chop($cwd = `pwd`);
  Ekoi8u::chop($answer = <STDIN>);

  This is different from:

  $answer = Ekoi8u::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Ekoi8u::chop returns the character
  ...
  In the most general case, Ekoi8u::chop can be expressed using substr:

  $last_code = Ekoi8u::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Ekoi8u::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Ekoi8u::chop;
...
=item * Index string
  ...
  $byte_pos = Ekoi8u::index($string,$substr,$byte_offset);
  $byte_pos = Ekoi8u::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Ekoi8u::index($string, $lookfor, $byte_pos)) > -1) {
...
=item * Reverse index string
  ...
  $byte_pos = Ekoi8u::rindex($string,$substr,$byte_offset);
  $byte_pos = Ekoi8u::rindex($string,$substr);

  This subroutine works just like Ekoi8u::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Ekoi8u::index).
  ...
  while (($byte_pos = KOI8U::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item * Lower case string
  ...
  This subroutine returns a lowercased version of KOI8-U $string (or $_, if
  $string is omitted). This is the internal subroutine implementing the \L escape
  in double-quoted strings.

  You can use the Ekoi8u::fc subroutine for case-insensitive comparisons via KOI8U
  software.

=item * Lower case first character of string

  $lcfirst = Ekoi8u::lcfirst($string);
  $lcfirst = Ekoi8u::lcfirst_;

  This subroutine returns a version of KOI8-U $string with the first character
...
=item * Upper case string
  ...
  This subroutine returns an uppercased version of KOI8-U $string (or $_, if
  $string is omitted). This is the internal subroutine implementing the \U escape
  in interpolated strings. For titlecase, use Ekoi8u::ucfirst instead.
...
=item * Upper case first character of string
  ...
  $ucfirst = Ekoi8u::ucfirst($string);
  $ucfirst = Ekoi8u::ucfirst_;
  ...
  $titlecase = Ekoi8u::ucfirst(substr($word,0,1)) . Ekoi8u::lc(substr($word,1));
  ...
  $do_not_use = Ekoi8u::ucfirst(Ekoi8u::lc($word));
...
=item * Fold case string
  ...
  New to KOI8U software, this subroutine returns the full Unicode-like casefold of
  KOI8-U $string (or $_, if omitted). This is the internal subroutine implementing
  ...
  Ekoi8u::fc($a) eq Ekoi8u::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because KOI8U software has always used casefolding semantics for
...
=item * Make ignore case string
  ...
  @ignorecase = Ekoi8u::ignorecase(@string);

  This subroutine is internal use to m/ /i, s/ / /i, split / /i, and qr/ /i.

=item * Make capture number

  $capturenumber = Ekoi8u::capture($string);
...
=item * Make character
  ...
  that $code in the character set. For example, Ekoi8u::chr(65) is "A" in either
  ASCII or KOI8-U, not Unicode. For the reverse of Ekoi8u::chr, use KOI8U::ord.

=item * Filename expansion (globbing)

  @glob = Ekoi8u::glob($string);
  @glob = Ekoi8u::glob_;
  ...
  The Ekoi8u::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Ekoi8u::glob('"*e f*"');
  @spacies = Ekoi8u::glob(q("*e f*"));

  If you had to get a variable through, you could do this:

  @spacies = Ekoi8u::glob("'*${var}e f*'");
  @spacies = Ekoi8u::glob(qq("*${var}e f*"));
