...
of PDLA. If you just want to use PDLA for something, you definitely
do not need to read this. Even if you want to interface your C routines
to PDLA or create new L<PDLA::PP|PDLA::PP> functions, you do not need to read this
...
of PDLA. To read this, a good understanding of the C language
...
this document refers to in the PDLA core sources and additionally
struggle to understand L<PDLA::PP|PDLA::PP>, you will be awarded the title
"PDLA Guru" (of course, the current version of this document
...
please inform the PDLA porters email list (pdl-devel@lists.sourceforge.net).
...
the C<PDLA> field containing the scalar reference (this makes overloading
piddles easy, see L<PDLA::Objects|PDLA::Objects>). You can easily find out
...
code below demonstrates how to do it:
   ...
   die "not a piddle" unless UNIVERSAL::isa($pdl, 'PDLA');
   # is it a scalar ref or a hash ref?
   if (UNIVERSAL::isa($pdl, "HASH")) {
     die "not a valid PDLA" unless exists $pdl->{PDLA} &&
	UNIVERSAL::isa($pdl->{PDLA},'PDLA');
     print "This is a hash reference,",
	" the PDLA field contains the scalar ref\n";
   } else {
...
data and structural that makes up a piddle is done by the PDLA typemap.
The functions used in the PDLA typemap are defined pretty much at the top
of the file F<pdlcore.h>. So what does the structure look like:

	struct pdl {
	   unsigned long magicno; /* Always stores PDLA_MAGICNO as a sanity check */
	   ...
	   PDLA_Indx nvals;           /* How many values allocated */
	   int datatype;
	   PDLA_Indx   *dims;      /* Array of data dimensions */
	   PDLA_Indx   *dimincs;   /* Array of data default increments */
...
data items of the data type of this piddle.  PDLA_Indx is either 'long' or
...
double types, see also L<PDLA::Types>.
...
inside the PDLA struct.

   	PDLA_Indx   def_dims[PDLA_NDIMS];   
   	PDLA_Indx   def_dimincs[PDLA_NDIMS]; 
...
C<PDLA_NDIMS>. This is important to note when freeing a piddle struct.
...
is nonzero, the PDLA has some magic attached to it. The implementation
...
This one is generally manipulated through L<sethdr|PDLA::Core/sethdr> and
L<gethdr|PDLA::Core/gethdr> calls.
...
(as mentioned above) which are represented by the type C<pdl_trans> in PDLA.
...
In general, executing a PDLA function on a group of piddles
results in creation of a transformation of the requested
type that links all input and output arguments (at least
those that are piddles). In PDLA functions that support
...
of a flowing function (like L<slice|PDLA::Slices/slice>) requires
...
a dataflow paradigm between PDLA objects necessitates a special
...
with other piddles via PDLA transformations (remember, C<pdl_trans>). 
...
by the C<baddata> method of PDLA::Bad (only available if you
have compiled PDLA with the C<WITH_BADVAL> option set to 1,
but still useful as an example!). 

Consider the following situation:

 pdla> $x = rvals(7,7,{Centre=>[3,4]});
 pdla> $y = $x->slice('2:4,3:5');
 pdla> ? vars
...
then I want the state of C<$y> - it's I<child> - to be changed as
...
C<PDLA_CHILDLOOP_THISCHILD> macro.
...
they are explicitly typed, PDLA functions created with L<PDLA::PP|PDLA::PP>
...
The PDLA core makes sure that these are called as needed when
...
Again, functions built with L<PDLA::PP|PDLA::PP> make sure that copying
...
hand but rather generated by L<PDLA::PP|PDLA::PP> from concise descriptions.
...
no big loss since the whole PDLA core is not reentrant
(Perl threading C<!=> PDLA threading!).

=head2 Signatures: threading over elementary operations

Most of that functionality of PDLA threading (automatic iteration
of elementary operations over multi-dim piddles) is implemented in the
file F<pdlthread.c>.

The L<PDLA::PP|PDLA::PP> generated functions (in particular the
...
trans is performed in agreement with PDLA's threading semantics.

=head2 Defining new PDLA functions -- Glue code generation

Please, see L<PDLA::PP> and examples in the PDLA distribution. Implementation
and syntax are currently far from perfect but it does a good job!

=head2 The Core struct

As discussed in L<PDLA::API|PDLA::API>, PDLA uses a pointer to a structure
to allow PDLA modules access to its core routines. The definition of this
structure (the C<Core> struct) is in F<pdlcore.h> (created by 
F<pdlcore.h.PL> in F<Basic/Core>) and looks something like

 /* Structure to hold pointers core PDLA routines so as to be used by 
  * many modules
  */
 struct Core {
    I32    Version;
...
is placed in the BOOT section of the generated xs code:
 ...
 if (PDLA->Version != PDLA_CORE_VERSION)
   Perl_croak(aTHX_ "Foo needs to be recompiled against the newly installed PDLA");
...
C<PDLA_CORE_VERSION> C macro used to populate the Version field

=item *

add documentation (e.g. to L<PDLA::API|PDLA::API>) if it's a
"useful" function for external module writers (as well as
ensuring the code is as well documented as the rest of PDLA
;)
