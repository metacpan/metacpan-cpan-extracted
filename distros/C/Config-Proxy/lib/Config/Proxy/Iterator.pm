package Config::Proxy::Iterator;
use strict;
use warnings;
use Config::Proxy::Node;
use Carp;
...
Proxy parse tree. The object is returned by the B<iterator> method of
B<Config::Proxy> and B<Config::Proxy::Node> objects. The method takes
...
B<Config::Proxy::Iterator> objects explicitly. Please use the B<iterator>
method of B<Config::Proxy> or B<Config::Proxy::Node> class objects.

    $itr = new Config::Proxy::Iterator($node, %rec);

Returns new iterator object for traversing the tree starting from B<$node>,
which must be a B<Config::Proxy::Node> object or its derivative. Optional
B<%rec> is one of the keywords discussed above, in section B<DESCRIPTION>.
...
L<Proxy::Config>, L<Proxy::Config::Node>, L<Config::HAProxy>,
L<Config::Pound>.
