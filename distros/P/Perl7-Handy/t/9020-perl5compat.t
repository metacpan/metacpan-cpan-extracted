...
#       Perl7::Handy order: use5, use vars, $VERSION, pop @INC,
#                           use strict, warnings stub, use warnings
######################################################################
use strict;
...
for my $pm (@pm_files) {
    ...
    ok($text =~ /\$INC\{'warnings\.pm'\}\s*=.*?if\s+\$\]\s*</s,
    ...
    # P14: Perl7::Handy header order:
    #   use 5.00503  <  use vars  <  $VERSION  <  pop @INC  <
    #   use strict  <  warnings stub  <  use warnings
    my %pos;
    for my $pair (
    ...
    my $p14 = $pos{use5}     < $pos{usevars}
           ...
           && $pos{version}  < $pos{popinc}
           && $pos{popinc}   < $pos{strict}
           && $pos{strict}   < $pos{wstub}
           && $pos{wstub}    < $pos{warnings};
    ok($p14, "P14 - header order "
           . "(use5..usevars..\$VERSION..pop\@INC..strict..warnings): $pm");
}
