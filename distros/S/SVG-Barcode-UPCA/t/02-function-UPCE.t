...
note 'Functions';
my @functions = qw|plot_upce|;
can_ok $package, $_ for @functions;

is_deeply \@SVG::Barcode::UPCE::EXPORT_OK, \@functions, 'All functions exported';

note 'Plot';
my $plot = $package->can('plot_upce');
my $text = '01234565';
ok my $svg = $plot->($text), 'Plot code';
is $svg, slurp("$FindBin::Bin/resources/01234565_black_text.svg"), 'Content is correct';

ok $svg = $plot->($text, foreground => 'red', lineheight => 20, textsize => 0),
  'Plot in red without text';
is $svg, slurp("$FindBin::Bin/resources/01234565_red_notext.svg"), 'Content is correct';

eval { $plot->() };
like $@, qr/Too few arguments for subroutine/, 'Correct error for missing text';

note 'Plot with uncalculated check digit';
my $text_no_check_digit = '0123456';
