...
// Oid lo_truncate (KCIConnection *conn, int fd, size_t len);
// Oid lo_truncate (KCIConnection *conn, int fd, size_t len) {
//     croak ("Cannot use lo_truncate unless compiled against Kingbase 8.3 or later");
// }
// 
// #endif

// #if PGLIBVERSION < 80400
// Oid lo_import_with_oid (KCIConnection *conn, char *filename, unsigned int lobjId);
// Oid lo_import_with_oid (KCIConnection *conn, char *filename, unsigned int lobjId) {
//     croak ("Cannot use lo_import_with_oid unless compiled against Kingbase 8.4 or later");
// }
// 
// #endif
...
#ifndef KB_DIAG_SEVERITY_NONLOCALIZED
#define KB_DIAG_SEVERITY_NONLOCALIZED 'V'
#endif
...
static KCIExecuteStatus _result(pTHX_ imp_dbh_t *imp_dbh, const char *sql);
static void _fatal_sqlstate(pTHX_ imp_dbh_t *imp_dbh);
static KCIExecuteStatus _sqlstate(pTHX_ imp_dbh_t *imp_dbh, KCIResult *result);
...
static KCITransactionStatus pg_db_txn_status (pTHX_ imp_dbh_t *imp_dbh);
...
int dbd_db_login6 (SV * dbh, imp_dbh_t * imp_dbh, char * dbname, char * uid, char * pwd, SV *attr)
{
    ...
    KCIConnectionStatus connstatus;

    if (TSTART_slow) TRC(DBILOGFP, "%sBegin dbd_db_login\n", THEADER_slow);

    /* DBD::KingBase syntax: 'dbname=dbname;host=host;port=port', 'User', 'Pass' */
    /* libkci syntax: 'dbname=dbname host=host port=port user=uid password=pwd' */
    ...
    *dest = '\0';
    ...
    TRACE_PQSTATUS;
    connstatus = KCIConnectionGetStatus(imp_dbh->conn);
    if (CONNECTION_OK != connstatus) {
        TRACE_PQERRORMESSAGE;
        strncpy(imp_dbh->sqlstate, "08006", 6); /* "CONNECTION FAILURE" */
        pg_error(aTHX_ dbh, connstatus, KCIConnectionGetLastError(imp_dbh->conn));
        TRACE_PQFINISH;
        KCIConnectionDestory(imp_dbh->conn);
        sv_free((SV *)imp_dbh->savepoints);
    ...
    (void)KCISetNoticeProcessor(imp_dbh->conn, pg_warn, (void *)SvRV(dbh));
    
    /* Figure out what protocol this server is using (most likely 3) */
    TRACE_PQPROTOCOLVERSION;
    imp_dbh->pg_protocol = KCIConnectionGetProtocolVersion(imp_dbh->conn);

    /* Figure out this particular backend's version */
    TRACE_PQSERVERVERSION;
    imp_dbh->pg_server_version = KCIConnectionGetServerVersion(imp_dbh->conn);

    if (imp_dbh->pg_server_version < 80000) {
        /* 
           Special workaround for KBBouncer, which has the unfortunate habit of modifying 'server_version', 
        ...
        */
        if (NULL != strstr(KCIConnectionGetParameterValue(imp_dbh->conn, "server_version"), "bouncer")) {
    ...
    imp_dbh->last_result       = NULL; /* NULL or the last KCIResult returned by something */
    imp_dbh->sth_result_owner  = 0;
	imp_dbh->fetch_size        = 50000; // 默认：0 fetch-all rows at once. 2026-03-26 王海清根据KingBase官方要求新加新加
...
static void pg_error (pTHX_ SV * h, int error_num, const char * error_msg)
{
    ...
    /*
        We need a special exception for cases in which libkci doesn't know what the error was,
        and Kingbase returns nothing. Probably client_min_messages is boosted too high.
        See CPAN ticket #109591
    */
    if (7 == error_num && 0 == error_len) {
        sv_setpvn(DBIc_ERRSTR(imp_xxh), "No error returned from Kingbase. Perhaps client_min_messages is set too high?", 77);
    }
    else {
...
static KCIExecuteStatus _result(pTHX_ imp_dbh_t * imp_dbh, const char * sql)
{
    ...
    if (0 == imp_dbh->sth_result_owner && NULL != imp_dbh->last_result) {
        TRACE_PQCLEAR;
        KCIResultDealloc(imp_dbh->last_result);
        imp_dbh->last_result = NULL;
    }

    TRACE_PQEXEC;
    imp_dbh->last_result = KCIStatementExecute(imp_dbh->conn, sql);
    imp_dbh->sth_result_owner = 0;
...
static void _fatal_sqlstate(pTHX_ imp_dbh_t * imp_dbh)
{
    ...
    sqlstate = KCIConnectionGetStatus(imp_dbh->conn) == CONNECTION_BAD ?
...
{
    char *sqlstate;
    KCIExecuteStatus status   = EXECUTE_FATAL_ERROR; /* until proven otherwise */

    if (TSTART_slow) TRC(DBILOGFP, "%sBegin _sqlstate\n", THEADER_slow);

    if (result) {
        TRACE_PQRESULTSTATUS;
        status = KCIResultGetStatusCode(result);
    }

    sqlstate = NULL;

    /*
      Because KCIResultGetErrorField may not work completely when an error occurs, and 
    ...
    */
    if (result) {
        ...
        sqlstate = KCIResultGetErrorField(result, KB_DIAG_SQLSTATE);
    }
    
    if (!sqlstate) {
        /* Do our best to map the status result to a sqlstate code */
        switch ((int)status) {
        case EXECUTE_EMPTY_QUERY:
        case EXECUTE_COMMAND_OK:
        case EXECUTE_TUPLES_OK:
        case EXECUTE_COPY_OUT:
        case EXECUTE_COPY_IN:
        case PGRES_COPY_BOTH:
        case EXECUTE_PARTIAL_TUPLES:
            sqlstate = "00000"; /* SUCCESSFUL COMPLETION */
            break;
        case EXECUTE_BAD_RESPONSE:
        case EXECUTE_NONFATAL_ERROR:
            sqlstate = "01000"; /* WARNING */
            break;
        case EXECUTE_FATAL_ERROR:
            /* libkci returns NULL result in case of connection failures */
            if (!result || KCIConnectionGetStatus(imp_dbh->conn) == CONNECTION_BAD) {
...
int dbd_db_ping (SV * dbh)
{
    ...
    if (tstatus >= TRANSACTION_UNKNOWN) { /* Unknown, so we err on the side of "bad" */
    ...
    result = KCIStatementExecute(imp_dbh->conn, "/* DBD::KingBase ping test v3.10.4 */");
    status = KCIResultGetStatusCode(result);
    KCIResultDealloc(result);
    if (EXECUTE_FATAL_ERROR == status) {
        /* Something very bad, usually indicating the backend is gone */
        return -3;
    }

    /* We expect to see an empty query most times */
    if (EXECUTE_EMPTY_QUERY == status) {
        if (TEND_slow) TRC(DBILOGFP, "%sEnd dbd_pg_ping (EXECUTE_EMPTY_QUERY)\n", THEADER_slow);
        return 1+tstatus;
        /* 0=idle 1=active 2=intrans 3=inerror 4=unknown */
    }

    /* As a safety measure, check KCIConnectionGetStatus as well */
    if (CONNECTION_BAD == KCIConnectionGetStatus(imp_dbh->conn)) {
        if (TEND_slow) TRC(DBILOGFP, "%sEnd dbd_pg_ping (KCIConnectionGetStatus returned CONNECTION_BAD)\n", THEADER_slow);
        return -4;
    }
...
static KCITransactionStatus pg_db_txn_status (pTHX_ imp_dbh_t * imp_dbh)
{
    ...
    if (TSTART_slow) TRC(DBILOGFP, "%sBegin KCITransactionStatus\n", THEADER_slow);
    TRACE_PQTRANSACTIONSTATUS;
    return KCIConnectionGetTransactionStatus(imp_dbh->conn);
...
static int pg_db_rollback_commit (pTHX_ SV * dbh, imp_dbh_t * imp_dbh, int action)
{
    ...
    if (TRANSACTION_IDLE == tstatus) { /* Not in a transaction */
    ...
    else if (TRANSACTION_ACTIVE == tstatus) { /* Still active - probably in a COPY */
        if (TRACEWARN_slow)
            TRC(DBILOGFP,"%sCommand in progress, so no done_begin checking!\n", THEADER_slow);
    }
    else if (TRANSACTION_INTRANS == tstatus || TRANSACTION_INERROR == tstatus) { /* In a (possibly failed) transaction */
        if (!imp_dbh->done_begin) {
    ...
    if (EXECUTE_COMMAND_OK != status) {
        TRACE_PQERRORMESSAGE;
        pg_error(aTHX_ dbh, status, KCIConnectionGetLastError(imp_dbh->conn));
...
void dbd_db_destroy (SV * dbh, imp_dbh_t * imp_dbh)
{
    ...
    if (NULL != imp_dbh->async_sth) { /* Just in case */
        if (NULL != imp_dbh->async_sth->result) {
            TRACE_PQCLEAR;
            KCIResultDealloc(imp_dbh->async_sth->result);
...
SV * dbd_db_FETCH_attrib (SV * dbh, imp_dbh_t * imp_dbh, SV * keysv)
{
    ...
    case 5: /* pg_db */
        ...
        if (strEQ("pg_db", key)) {
            TRACE_PQDB;
            retsv = newSVpv(KCIConnectionGetDatabase(imp_dbh->conn),0);
        }
        break;
    ...
    case 6: /* pg_pid */
        ...
        if (strEQ("pg_pid", key)) {
            TRACE_PQBACKENDPID;
            retsv = newSViv((IV)KCIConnectionGetBackendPid(imp_dbh->conn));
        }
        break;
    ...
    case 7: /* pg_user  pg_pass  pg_port  pg_host */
        ...
        if (strEQ("pg_user", key)) {
            TRACE_PQUSER;
            retsv = newSVpv(KCIConnectionGetUser(imp_dbh->conn),0);
        }
        else if (strEQ("pg_pass", key)) {
            TRACE_PQPASS;
            retsv = newSVpv(KCIConnectionGetPassword(imp_dbh->conn),0);
        }
        else if (strEQ("pg_port", key)) {
            TRACE_PQPORT;
            retsv = newSVpv(KCIConnectionGetPort(imp_dbh->conn),0);
        }
        else if (strEQ("pg_host", key)) {
            TRACE_PQHOST;
            retsv = KCIConnectionGetHost(imp_dbh->conn) ? newSVpv(KCIConnectionGetHost(imp_dbh->conn),0) : Nullsv;
        }
        break;
    ...
    case 9: /* pg_socket */
        ...
        if (strEQ("pg_socket", key)) {
            TRACE_PQSOCKET;
            retsv = newSViv((IV)KCIConnectionGetSocket(imp_dbh->conn));
        }
        else if (strEQ("FetchSize", key)) {
            retsv = newSViv((IV)imp_dbh->fetch_size);
        }
        break;
    ...
    case 10: /* AutoCommit  pg_bool_tf  pg_pid_number  pg_options */
        ...
        if (strEQ("AutoCommit", key))
            ...
            TRACE_PQOPTIONS;
            retsv = newSVpv(KCIConnectionGetCommandLineOptions(imp_dbh->conn),0);
        }
        break;
    ...
    case 30: /* pg_standard_conforming_strings */
        ...
        if (strEQ("pg_standard_conforming_strings", key)) {
            if (NULL != KCIConnectionGetParameterValue(imp_dbh->conn, "standard_conforming_strings")) {
                retsv = newSVpv(KCIConnectionGetParameterValue(imp_dbh->conn,"standard_conforming_strings"),0);
            }
        }
        break;
...
int dbd_db_STORE_attrib (SV * dbh, imp_dbh_t * imp_dbh, SV * keysv, SV * valuesv)
{
    ...
    case 9: /* FetchSize */
        if (strEQ("FetchSize", key)) {
            if (SvOK(valuesv)) {
                int fsize = (int)SvIV(valuesv);
                imp_dbh->fetch_size = (fsize < 0) ? 0 : fsize;
            } else {
                imp_dbh->fetch_size = 0;
            }
            retval = 1;
        }
        break;
    ...
    case 13: /* pg_errorlevel */
        ...
        if (strEQ("pg_errorlevel", key)) {
            ...
            (void)KCISetVerbosity(imp_dbh->conn, (KCIErrorVerbosity)imp_dbh->pg_errorlevel);
            if (TRACE5_slow)
    ...
    case 14: /* pg_prepare_now  pg_enable_utf8 */
        ...
        /* 
           ...
           so we only do it here, which allows people to signal DBD::KingBase that something 
...
SV * dbd_st_FETCH_attrib (SV * sth, imp_sth_t * imp_sth, SV * keysv)
{
    ...
    case 4: /* NAME  TYPE */
        ...
        if (strEQ("NAME", key)) {
            ...
            while(--fields >= 0) {
                D_imp_dbh_from_sth;
                TRACE_PQFNAME;
                fieldname = KCIResultGetColumnName(imp_sth->result, fields);
            ...
            /* Need to convert the KingBase type to ANSI/SQL type. */
            sql_type_info_t * type_info;
            AV *av = newAV();
            retsv = newRV_inc(sv_2mortal((SV*)av));
            while(--fields >= 0) {
                TRACE_PQFTYPE;
                type_info = pg_type_data((int)KCIResultGetColumnType(imp_sth->result, fields));
    ...
    case 5: /* SCALE */
        ...
        if (strEQ("SCALE", key)) {
            ...
            while(--fields >= 0) {
                TRACE_PQFTYPE;
                o = KCIResultGetColumnType(imp_sth->result, fields);
                if (PG_NUMERIC == o) {
                    TRACE_PQFMOD;
                    o = KCIResultGetColumnTypmod(imp_sth->result, fields)-4;
    ...
    case 7: /* pg_size  pg_type */
        ...
        if (strEQ("pg_size", key)) {
            ...
            while(--fields >= 0) {
                TRACE_PQFSIZE;
                (void)av_store(av, fields, newSViv(KCIResultGetColumnLength(imp_sth->result, fields)));
            }
        }
    ...
    case 8: /* pg_async  NULLABLE */
        ...
        if (strEQ("pg_async", key))
            ...
            while(--fields >= 0) {
                nullable=2;
                TRACE_PQFTABLE;
                x = KCIResultGetRelationOidOfColumn(imp_sth->result, fields);
                TRACE_PQFTABLECOL;
                y = KCIResultGetRelationOidOfColumncol(imp_sth->result, fields);
                if (InvalidOid != x && y > 0) { /* We know what table and column this came from */
                    ...
                    TRACE_PQEXEC;
                    result = KCIStatementExecute(imp_dbh->conn, statement);
                    TRACE_PQRESULTSTATUS;
                    status = KCIResultGetStatusCode(result);
                    if (EXECUTE_TUPLES_OK == status) {
                        TRACE_PQNTUPLES;
                        if (KCIResultGetRowCount(result)!=0) {
                            TRACE_PQGETVALUE;
                            switch (KCIResultGetColumnValue(result,0,0)[0]) {
                            case 't':
                                nullable = 0;
                                break;
                            case 'f':
    ...
    case 9: /* PRECISION */
        ...
        if (strEQ("PRECISION", key)) {
            ...
            while(--fields >= 0) {
                ...
                case PG_VARCHAR:
                    TRACE_PQFMOD;
                    sz = KCIResultGetColumnTypmod(imp_sth->result, fields);
                    break;
                case PG_NUMERIC:
                    TRACE_PQFMOD;
                    sz = KCIResultGetColumnTypmod(imp_sth->result, fields)-4;
                    if (sz > 0)
                        sz = sz >> 16;
                    break;
                default:
                    TRACE_PQFSIZE;
                    sz = KCIResultGetColumnLength(imp_sth->result, fields);
                    break;
                }
    ...
    case 13: /* pg_oid_status  pg_cmd_status */
        if (strEQ("pg_oid_status", key)) {
            TRACE_PQOIDVALUE;
            retsv = newSVuv((unsigned int)KCIResultInsertRowOid(imp_sth->result));
        }
        else if (strEQ("pg_cmd_status", key)) {
            TRACE_PQCMDSTATUS;
            retsv = newSVpv((char *)KCIResultGetCommandStatus(imp_sth->result), 0);
        }
        break;
...
int pg_db_getfd (imp_dbh_t * imp_dbh)
{
    ...
    TRACE_PQSOCKET;
    return KCIConnectionGetSocket(imp_dbh->conn);
...
SV * pg_db_pg_notifies (SV * dbh, imp_dbh_t * imp_dbh)
{
    ...
    status = KCIConnectionForceRead(imp_dbh->conn);
    if (0 == status) {
        _fatal_sqlstate(aTHX_ imp_dbh);
        
        TRACE_PQERRORMESSAGE;
        pg_error(aTHX_ dbh, EXECUTE_FATAL_ERROR, KCIConnectionGetLastError(imp_dbh->conn));
        if (TEND_slow) TRC(DBILOGFP, "%sEnd pg_db_pg_notifies (error)\n", THEADER_slow);
        return &PL_sv_undef;
    }

    TRACE_PQNOTIFIES;
    notify = KCIGetNextNotification(imp_dbh->conn);
...
int dbd_st_prepare_sv (SV * sth, imp_sth_t * imp_sth, SV * statement_sv, SV * attribs)
{
    ...
    // fragment to extract the result set
    imp_sth->chunk_size = 0;
    imp_sth->fetch_chunked = DBDPG_FALSE;
    ...
    */
    ...
    if (imp_sth->is_dml
        ...
        if (pg_st_prepare_statement(aTHX_ sth, imp_sth)!=0) {
            TRACE_PQERRORMESSAGE;
            croak ("%s", KCIConnectionGetLastError(imp_dbh->conn));
        }
    }
...
static void pg_st_split_statement (pTHX_ imp_sth_t * imp_sth, char * statement)
{
    ...
    */
    ...
    while (1) {
        ...
        if ('\'' == ch || '"' == ch) {
            ...
            if ('\'' == ch && -1 == non_standard_strings) {
                const char * scs = KCIConnectionGetParameterValue(imp_dbh->conn,"standard_conforming_strings");
    ...
    if (PLACEHOLDER_DOLLAR == imp_sth->placeholder_type) {
        /* 
           We follow the KingBase rules: must start with $1, repeats are allowed, 
...
static int pg_st_prepare_statement (pTHX_ SV * sth, imp_sth_t * imp_sth)
{
    ...
    if (!same_result && NULL != imp_sth->result) {
        TRACE_PQCLEAR;
        KCIResultDealloc(imp_sth->result);
        imp_sth->result = NULL;
    }

    TRACE_PQPREPARE;
    imp_dbh->last_result = imp_sth->result = KCIStatementPrepare(imp_dbh->conn, imp_sth->prepare_name, statement, params, imp_sth->PQoids);
    imp_dbh->sth_result_owner = (long int)imp_sth;
    status = _sqlstate(aTHX_ imp_dbh, imp_sth->result);
    if (TRACE6_slow)
        TRC(DBILOGFP, "%sUsing KCIStatementPrepare: %s\n", THEADER_slow, statement);
    ...
    if (EXECUTE_COMMAND_OK != status) {
        ...
        Safefree(imp_sth->prepare_name);
        imp_sth->prepare_name = NULL;
        pg_error(aTHX_ sth, status, KCIConnectionGetLastError(imp_dbh->conn));
...
int dbd_bind_ph (SV * sth, imp_sth_t * imp_sth, SV * ph_name, SV * newvalue, IV sql_type, SV * attribs, int is_inout, IV maxlen)
{
    ...
    if (SvROK(newvalue) &&!IS_DBI_HANDLE(newvalue)) {
        if (sv_isa(newvalue, "DBD::KingBase::DefaultValue")
        ...
        else if (sv_isa(newvalue, "DBD::KingBase::Current")) {
            /* This is a special type */
            Safefree(currph->value);
            currph->value = NULL;
            currph->valuelen = 0;
            ...
            /*
              ...
              because sometimes DBD::KingBase::DefaultValue arrives as one!
            */
    ...
    if (pg_type) {
        if ((currph->bind_type = pg_type_data(pg_type))) {
            if (!currph->bind_type->bind_ok) { /* Re-evaluate with new prepare */
                croak("Cannot bind %s, pg_type %s not supported by DBD::KingBase",
...
static SV * pg_destringify_array(pTHX_ imp_dbh_t *imp_dbh, unsigned char * input, sql_type_info_t * coltype)
{
    ...
    /*
      ...
      the Kingbase backend, and we rely on it to give us a sane and balanced structure
    */
...
static void pg_db_detect_client_encoding_utf8(pTHX_ imp_dbh_t *imp_dbh) {
    ...
    const char * const client_encoding =
        KCIConnectionGetParameterValue(imp_dbh->conn, "client_encoding");
...
long pg_quickexec (SV * dbh, const char * sql, const int asyncflag)
{
    dTHX;
    D_imp_dbh(dbh);
    KCIExecuteStatus          status = EXECUTE_FATAL_ERROR; /* Assume the worst */
    KCITransactionStatus txn_status;
    char *                  cmdStatus = NULL;
    long                    rows = 0;
    ...
    if (NULL == imp_dbh->conn) {
        pg_error(aTHX_ dbh, EXECUTE_FATAL_ERROR, "Database handle has been disconnected");
        return -2;
    }

    /* Abort if we are in the middle of a copy */
    if (imp_dbh->copystate != 0) {
        if (EXECUTE_COPY_IN == imp_dbh->copystate) {
    ...
    if (asyncflag & PG_ASYNC) {
        ...
        TRACE_PQSENDQUERY;
        if (! KCIStatementSend(imp_dbh->conn, sql)) {
            if (TRACE4_slow) TRC(DBILOGFP, "%sKCIStatementSend failed\n", THEADER_slow);
            _fatal_sqlstate(aTHX_ imp_dbh);
    ...
    case EXECUTE_TUPLES_OK:
        TRACE_PQNTUPLES;
        rows = KCIResultGetRowCount(imp_dbh->last_result);
        break;
    case EXECUTE_COMMAND_OK:
        /* non-select statement */
        TRACE_PQCMDSTATUS;
        cmdStatus = KCIResultGetCommandStatus(imp_dbh->last_result);
    ...
    case PGRES_COPY_BOTH:
        ...
        imp_dbh->copybinary = KCIResultIsBinary(imp_dbh->last_result);
        rows = -1;
        break;
    case EXECUTE_EMPTY_QUERY:
    case EXECUTE_BAD_RESPONSE:
    ...
    TRACE_PQTRANSACTIONSTATUS;
    txn_status = KCIConnectionGetTransactionStatus(imp_dbh->conn);

    if (TRANSACTION_IDLE == txn_status) {
        imp_dbh->done_begin = DBDPG_FALSE;
        imp_dbh->copystate=0;
        /* If begin_work has been called, turn AutoCommit back on and BegunWork off */
        if (DBIc_has(imp_dbh, DBIcf_BegunWork)!=0) {
...
long dbd_st_execute (SV * sth, imp_sth_t * imp_sth)
{
    ...
    if (NULL == imp_dbh->conn) {
        pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, "Cannot call execute on a disconnected database handle");
        return -2;
    }
    ...
    if (!imp_sth->all_bound && imp_sth->numphs!=0) {
        for (currph=imp_sth->ph; NULL != currph; currph=currph->nextph) {
            if (NULL == currph->bind_type) {
                pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, "execute called with an unbound placeholder");
    ...
    /*
      ...
      We are using one of KCIStatementExecute, KCIStatementExecutePrepared, or KCIStatementExecuteParams
      Let's figure out which we are going to use and set pqtype
    */

    if (TRACE4_slow) TRC(DBILOGFP,
                    "%sKCIStatementExecute* decision: dml=%d direct=%d server_prepare=%d numbound=%d numphs=%d default=%d current=%d\n",
                    THEADER_slow, 
                    imp_sth->is_dml,
                    imp_sth->direct,
                    imp_sth->server_prepare,
    ...
    */
    ...
    /**
      * FetchSize set, 
      * if sql is select, use pbe, so KCIStatementExecutePrepared2 can do chunked fetch.
      */
    if (DBDPG_FALSE == DBIc_has(imp_dbh, DBIcf_AutoCommit) &&
        NULL != imp_sth->firstword && 
        0 == strcasecmp("SELECT", imp_sth->firstword) && 
        0 < imp_dbh->fetch_size)
    {
        pqtype = PQTYPE_PREPARED;
    }
    ...
    */
    ...
    /* If using plain old KCIStatementExecute, we need to quote each value ourselves */
    ...
    else { /* We are using a server that can handle KCIStatementExecuteParams/KCIStatementExecutePrepared */
    ...
    /* Run one of KCIStatementExecute (or KCIStatementSend), KCIStatementExecuteParams (or KCIStatementSendParams), KCIStatementExecutePrepared (or KCIStatementSendPrepared) */

    if (PQTYPE_EXEC == pqtype) { /* KCIStatementExecute or KCIStatementSend */

        if (TRACE4_slow) TRC(DBILOGFP, "%s%s\n",
                             THEADER_slow,
                             imp_sth->async_flag & PG_ASYNC ? "KCIStatementSend" : "KCIStatementExecute");
        ...
        if (imp_sth->async_flag & PG_ASYNC) {
            TRACE_PQSENDQUERY;
            ret = KCIStatementSend(imp_dbh->conn, statement);
        }
        else {
            ...
            TRACE_PQEXEC;
            imp_dbh->last_result = imp_sth->result = KCIStatementExecute(imp_dbh->conn, statement);
            imp_dbh->sth_result_owner = (long int)imp_sth;
        }
    ...
    else if (PQTYPE_PARAMS == pqtype) { /* KCIStatementExecuteParams or KCIStatementSendParams */

        if (TRACE4_slow) TRC(DBILOGFP, "%s%s\n",
                             THEADER_slow,
                             imp_sth->async_flag & PG_ASYNC ? "KCIStatementSendParams" : "KCIStatementExecuteParams");
        ...
        if (TRACE7_slow) {
            for (x=0,currph=imp_sth->ph; NULL != currph; currph=currph->nextph,x++) {
                TRC(DBILOGFP, "%sKCIStatementExecuteParams item #%d\n", THEADER_slow, (int)x);
        ...
        if (imp_sth->async_flag & PG_ASYNC) {
            ...
            TRACE_PQEXECPARAMS;
            imp_dbh->last_result = imp_sth->result = KCIStatementExecuteParams
                (imp_dbh->conn, statement, imp_sth->numphs,
                 imp_sth->PQoids, imp_sth->PQvals, imp_sth->PQlens, imp_sth->PQfmts, 0);
            imp_dbh->sth_result_owner = (long int)imp_sth;
        }
    ...
    else if (PQTYPE_PREPARED == pqtype) { /* KCIStatementExecutePrepared or KCIStatementSendPrepared */
    
        if (TRACE4_slow) TRC(DBILOGFP, "%s%s\n",
                             THEADER_slow,
                             imp_sth->async_flag & PG_ASYNC ? "KCIStatementSendPrepared" : "KCIStatementExecutePrepared");
        ...
        if (TRACE7_slow) {
            for (x=0,currph=imp_sth->ph; NULL != currph; currph=currph->nextph,x++) {
                TRC(DBILOGFP, "%sKCIStatementExecutePrepared item #%d\n", THEADER_slow, (int)x);
                TRC(DBILOGFP, "%s-> Value: (%s)\n",
        ...
        if (imp_sth->async_flag & PG_ASYNC) {
            ...
            // sync, fetch_size > 0, select single sql, KCIStatementExecutePrepared2
            if (0 < imp_dbh->fetch_size && 
                NULL != imp_sth->firstword && 
                0 == strcasecmp("SELECT", imp_sth->firstword) && 
                NULL != imp_sth->prepare_name)
            {
                TRACE_PQEXECPREPARED2;

                // execute first
                imp_dbh->last_result = imp_sth->result = KCIStatementExecutePrepared2
                    (imp_dbh->conn, imp_sth->prepare_name, imp_sth->numphs,
                     imp_sth->PQvals, imp_sth->PQlens, imp_sth->PQfmts, 
                     0, imp_dbh->fetch_size);
                imp_sth->fetch_chunked = DBDPG_TRUE;
                imp_sth->chunk_size = imp_dbh->fetch_size;
            }
            else
            {
                TRACE_PQEXECPREPARED;

                imp_dbh->last_result = imp_sth->result = KCIStatementExecutePrepared
                    (imp_dbh->conn, imp_sth->prepare_name, imp_sth->numphs,
                     imp_sth->PQvals, imp_sth->PQlens, imp_sth->PQfmts, 0);
                imp_sth->fetch_chunked = DBDPG_FALSE;
                imp_sth->chunk_size = 0;
    ...
    /* Some form of KCIStatementExecute* or PQsend* has been run at this point */
    ...
    if (EXECUTE_TUPLES_OK == status 
        || EXECUTE_PARTIAL_TUPLES == status) {
        TRACE_PQNFIELDS;
        num_fields = KCIResultGetColumnCount(imp_sth->result);
        imp_sth->cur_tuple = 0;
        DBIc_NUM_FIELDS(imp_sth) = num_fields;
        DBIc_ACTIVE_on(imp_sth);
        TRACE_PQNTUPLES;
        ret = KCIResultGetRowCount(imp_sth->result);
        if (TRACE5_slow) TRC(DBILOGFP,
                        "%sStatus was %s, fields=%d, tuples=%ld\n",
                        THEADER_slow, EXECUTE_TUPLES_OK == status ? "EXECUTE_TUPLES_OK" : "EXECUTE_PARTIAL_TUPLES",
                        num_fields, ret);
    }
    else if (EXECUTE_COMMAND_OK == status) {
        /* non-select statement */
        char *cmdStatus = NULL;
        bool gotrows = DBDPG_FALSE;

        if (TRACE5_slow)
            TRC(DBILOGFP, "%sStatus was EXECUTE_COMMAND_OK\n", THEADER_slow);

        if (NULL != imp_sth->result) {
            TRACE_PQCMDSTATUS;
            cmdStatus = KCIResultGetCommandStatus(imp_sth->result);
            if (0 == strncmp(cmdStatus, "INSERT", 6)) {
                /* INSERT(space)oid(space)numrows */
        ...
        if (!gotrows) {
            ...
            TRACE_PQTRANSACTIONSTATUS;
            if (TRANSACTION_IDLE == KCIConnectionGetTransactionStatus(imp_dbh->conn)) {
                imp_dbh->done_begin = DBDPG_FALSE;
                /* If begin_work has been called, turn AutoCommit back on and BegunWork off */
                if (DBIc_has(imp_dbh, DBIcf_BegunWork)!=0) {
                    DBIc_set(imp_dbh, DBIcf_AutoCommit, 1);
    ...
    else if (EXECUTE_COPY_OUT == status || EXECUTE_COPY_IN == status || PGRES_COPY_BOTH == status) {
        if (TRACE5_slow)
            TRC(DBILOGFP, "%sStatus was PGRES_COPY_%s\n",
                THEADER_slow, EXECUTE_COPY_OUT == status ? "OUT" : EXECUTE_COPY_IN == status ? "IN" : "BOTH");
        /* Copy Out/In data transfer in progress */
        imp_dbh->copystate = status;
        imp_dbh->copybinary = KCIResultIsBinary(imp_sth->result);
...
AV * dbd_st_fetch (SV * sth, imp_sth_t * imp_sth)
{
    ...
    if ( !DBIc_ACTIVE(imp_sth) ) {
        pg_error(aTHX_ sth, EXECUTE_NONFATAL_ERROR, "no statement executing\n");    
    ...
    {
        if (DBDPG_TRUE == imp_sth->fetch_chunked 
            && imp_sth->chunk_size == imp_sth->rows)
        {
            KCIResult  *result = NULL;
            KCIExecuteStatus res_status = -1;
            int rows = 0;
            int ret = RET_ERROR;
            int same_result = 0;
            ...
            do
            {
                if (NULL == imp_dbh->conn)
                {
                    pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, "fetch next chunk failed: conn is null");
                    break;
                }
                
                if (NULL == imp_sth->prepare_name)
                {
                    pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, "fetch next chunk failed: prepare stmt is null");
                    break;
                }
                ...
                // execute again
                result = KCIStatementExecutePrepared2(imp_dbh->conn, 
                     imp_sth->prepare_name, -1,
                     imp_sth->PQvals, imp_sth->PQlens, imp_sth->PQfmts, 
                     0, imp_sth->chunk_size);
                if (NULL == result)
                {
                    pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, "KCIStatementExecutePrepared2 next chunk return null");
                    break;
                }
                
                res_status = KCIResultGetStatusCode(result);
                if (EXECUTE_TUPLES_OK != res_status &&
                    EXECUTE_PARTIAL_TUPLES != res_status)
                {
                    pg_error(aTHX_ sth, res_status, KCIConnectionGetLastError(imp_dbh->conn));
                    KCIResultDealloc(result);
                    if (TEND_slow)
                    {
                        TRC(DBILOGFP, "%sEnd dbd_st_fetch (chunk error: bad status)\n", THEADER_slow);
                    }
                    break;
                }
                 ...
                 * executing the same prepared sth one more time. 
                 * must reuse the same"two-step release" pattern,
                 * to preserve dbh->last_result / sth_result_owner / imp_sth->result:
                 *   if (last_result == 0 || == this sth) free(last_result);
                 *   if (imp_sth->result != dbh->last_result) free(imp_sth->result);
                 */
                same_result = (imp_dbh->last_result == imp_sth->result) ? 1 : 0;
                if ((0 == imp_dbh->sth_result_owner || (long int)imp_sth == imp_dbh->sth_result_owner)
                    && NULL != imp_dbh->last_result) {
                ...
                if (0 == rows)
                {
                    if (TEND_slow)
                    {
                        TRC(DBILOGFP, "%sEnd dbd_st_fetch (last chunk)\n", THEADER_slow);
                    }
                    break;
                }
                imp_sth->cur_tuple = 0;
    ...
    if (NULL == imp_sth->type_info) {
        ...
        for (i = 0; i < num_fields; ++i) {
            TRACE_PQFTYPE;
            imp_sth->type_info[i] = pg_type_data((int)KCIResultGetColumnType(imp_sth->result, i));
            if (imp_sth->type_info[i] == NULL) {
                if (TRACEWARN_slow) {
                    TRACE_PQFTYPE;
                    TRC(DBILOGFP, "%sUnknown type returned by Kingbase: %d. Setting to UNKNOWN\n",
                        THEADER_slow, KCIResultGetColumnType(imp_sth->result, i));
                }
    ...
    for (i = 0; i < num_fields; ++i) {
        ...
        TRACE_PQGETISNULL;
        if (KCIResultColumnIsNull(imp_sth->result, imp_sth->cur_tuple, i)!=0) {
            SvROK(sv) ? (void)sv_unref(sv) : (void)SvOK_off(sv);
        }
        else {
            unsigned char * value;
            TRACE_PQGETVALUE;
            value = (unsigned char*)KCIResultGetColumnValue(imp_sth->result, imp_sth->cur_tuple, i); 
...
{    
    ...
    /* Clean up result regardless of ACTIVE state: after all rows are fetched,
     * DBIc_ACTIVE is already off but imp_sth->result still holds the last
     * KCIResult. Without this, the result leaks when finish() is called after
     * natural fetch completion (the common case for chunked/prepared queries). */
    if (imp_sth->result) {
        if (imp_dbh->sth_result_owner == (long int)imp_sth) {
            imp_dbh->sth_result_owner = 0;
        }
        else {
            /* Not the current last_result owner; safe to free immediately. */
            TRACE_PQCLEAR;
            KCIResultDealloc(imp_sth->result);
        }
        imp_sth->result = NULL;
    ...
    // reset and used to fetch batch again
    imp_sth->chunk_size = 0;
    imp_sth->fetch_chunked = DBDPG_FALSE;
...
static int pg_st_deallocate_statement (pTHX_ SV * sth, imp_sth_t * imp_sth)
{
    ...
    if (TRANSACTION_INERROR == tstatus) {
        if (TRACE4_slow)
            TRC(DBILOGFP, "%sIssuing rollback before deallocate\n", THEADER_slow);
        {
            ...
            if (alen > -1) {
                ...
                strncpy(temkingbasestate, imp_dbh->sqlstate, strlen(imp_dbh->sqlstate)+1);
    ...
    Safefree(imp_sth->prepare_name);
    imp_sth->prepare_name = NULL;
    if (temkingbasestate[0]) {
        strncpy(imp_dbh->sqlstate, temkingbasestate, strlen(temkingbasestate)+1);
    }
...
void dbd_st_destroy (SV * sth, imp_sth_t * imp_sth)
{
    ...
    /* Clear the statement's result before DEALLOCATE. If this result is also
     * dbh->last_result, just release ownership so _result("DEALLOCATE ...")
     * can free it before replacing last_result. If it is stale, free it here. */
    if (NULL != imp_sth->result) {
        if ((long int)imp_sth == imp_dbh->sth_result_owner) {
            /* Owned last_result will be freed by _result or dbd_db_destroy. */
            imp_dbh->sth_result_owner = 0;
        } else if (imp_sth->result != imp_dbh->last_result) {
            /* Not the current last_result; safe to free without affecting imp_dbh. */
            TRACE_PQCLEAR;
            KCIResultDealloc(imp_sth->result);
        }
        imp_sth->result = NULL;
...
int pg_db_putline (SV * dbh, SV * svbuf)
{
    ...
    if (EXECUTE_COPY_IN != imp_dbh->copystate && PGRES_COPY_BOTH != imp_dbh->copystate)
    ...
    TRACE_PQPUTCOPYDATA;
    copystatus = KCICopySendData(imp_dbh->conn, buffer, (int)strlen(buffer));
    if (-1 == copystatus) {
        _fatal_sqlstate(aTHX_ imp_dbh);
        ...
        croak("KCICopySendData gave a value of %d\n", copystatus);
    }
...
int pg_db_getline (SV * dbh, SV * svbuf, int length)
{
    ...
    if (EXECUTE_COPY_OUT != imp_dbh->copystate && PGRES_COPY_BOTH != imp_dbh->copystate)
        croak("pg_getline can only be called directly after issuing a COPY TO command\n");

    length = 0; /* Make compilers happy */
    TRACE_PQGETCOPYDATA;
    copystatus = KCICopyReceiveData(imp_dbh->conn, &tempbuf, 0);

    if (-1 == copystatus) {
        *buffer = '\0';
        imp_dbh->copystate=0;
        TRACE_PQENDCOPY;
        KCICopySync(imp_dbh->conn); /* Can't hurt */
...
int pg_db_getcopydata (SV * dbh, SV * dataline, int async)
{
    ...
    TRACE_PQGETCOPYDATA;
    copystatus = KCICopyReceiveData(imp_dbh->conn, &tempbuf, async);
    ...
    if (copystatus > 0) {
        ...
        TRACE_PQCONSUMEINPUT;
        if (!KCIConnectionForceRead(imp_dbh->conn)) {
            _fatal_sqlstate(aTHX_ imp_dbh);
        ...
        imp_dbh->copystate=0;
        TRACE_PQGETRESULT;
        result = KCIConnectionFetchResult(imp_dbh->conn);
...
int pg_db_putcopydata (SV * dbh, SV * dataline)
{
    ...
    TRACE_PQPUTCOPYDATA;
    copystatus = KCICopySendData(imp_dbh->conn, copydata, copylen);

    if (1 == copystatus) {
        if (PGRES_COPY_BOTH == imp_dbh->copystate && KCIConnectionFlush(imp_dbh->conn)) {
            _fatal_sqlstate(aTHX_ imp_dbh);
...
int pg_db_putcopyend (SV * dbh)
{
    ...
    if (EXECUTE_COPY_OUT == imp_dbh->copystate) {
    ...
    /* Must be EXECUTE_COPY_IN or PGRES_COPY_BOTH at this point */

    TRACE_PQPUTCOPYEND;
    copystatus = KCICopySendEOF(imp_dbh->conn, NULL);
...
SV * pg_db_error_field (SV *dbh, char * fieldname)
{
    ...
    if ( 0 == strncmp(fieldname, "KB_DIAG_SEVERITY_NONLOCALIZED", 25) ||
         0 == strncmp(fieldname, "SEVERITY_NONLOCAL", 17)) {
        fieldcode = KB_DIAG_SEVERITY_NONLOCALIZED; // i.e. 'V'
    }
    else if ( 0 == strncmp(fieldname, "KB_DIAG_SEVERITY", 16) ||
              0 == strncmp(fieldname, "SEVERITY", 8)) {
        fieldcode = KB_DIAG_SEVERITY; // i.e. 'S'
    }
    else if ( 0 == strncmp(fieldname, "KB_DIAG_MESSAGE_PRIMARY", 20) ||
              0 == strncmp(fieldname, "MESSAGE_PRIMARY", 13) ||
              0 == strncmp(fieldname, "PRIMARY", 4)) {
        fieldcode = KB_DIAG_MESSAGE_PRIMARY; // i.e. 'M'
    }
    else if ( 0 == strncmp(fieldname, "KB_DIAG_MESSAGE_DETAIL", 22) ||
              0 == strncmp(fieldname, "MESSAGE_DETAIL", 14) ||
              0 == strncmp(fieldname, "DETAIL", 6)) {
        fieldcode = KB_DIAG_MESSAGE_DETAIL; // i.e. 'D'
    }
    else if ( 0 == strncmp(fieldname, "KB_DIAG_MESSAGE_HINT", 20) ||
              0 == strncmp(fieldname, "MESSAGE_HINT", 12) ||
              0 == strncmp(fieldname, "HINT", 4)) {
        fieldcode = KB_DIAG_MESSAGE_HINT; // i.e. 'H'
    }
    else if ( 0 == strncmp(fieldname, "KB_DIAG_STATEMENT_POSITION", 21) ||
              0 == strncmp(fieldname, "STATEMENT_POSITION", 13)) {
        fieldcode = KB_DIAG_STATEMENT_POSITION; // i.e. 'P'
    }
    else if ( 0 == strncmp(fieldname, "KB_DIAG_INTERNAL_POSITION", 20) ||
              0 == strncmp(fieldname, "INTERNAL_POSITION", 12)) {
        fieldcode = KB_DIAG_INTERNAL_POSITION; // i.e. 'p'
    }
    else if ( 0 == strncmp(fieldname, "KB_DIAG_INTERNAL_QUERY", 22) ||
              0 == strncmp(fieldname, "INTERNAL_QUERY", 14)) {
        fieldcode = KB_DIAG_INTERNAL_QUERY; // i.e. 'q'
    }
    else if ( 0 == strncmp(fieldname, "KB_DIAG_CONTEXT", 15) ||
              0 == strncmp(fieldname, "CONTEXT", 7)) {
        fieldcode = KB_DIAG_CONTEXT; // i.e. 'W'
    }
    ...
    else if ( 0 == strncmp(fieldname, "KB_DIAG_SOURCE_FILE", 19) ||
              0 == strncmp(fieldname, "SOURCE_FILE", 11)) {
        fieldcode = KB_DIAG_SOURCE_FILE; // i.e. 'F'
    }
    else if ( 0 == strncmp(fieldname, "KB_DIAG_SOURCE_LINE", 19) ||
              0 == strncmp(fieldname, "SOURCE_LINE", 11)) {
        fieldcode = KB_DIAG_SOURCE_LINE; // i.e. 'L'
    }
    else if ( 0 == strncmp(fieldname, "KB_DIAG_SOURCE_FUNCTION", 19) ||
              0 == strncmp(fieldname, "SOURCE_FUNCTION", 11)) {
        fieldcode = KB_DIAG_SOURCE_FUNCTION; // i.e. 'R'
    }
    else if ( 0 == strncmp(fieldname, "KB_DIAG_SQLSTATE", 16) || 
              0 == strncmp(fieldname, "SQLSTATE", 8) ||
              0 == strncmp(fieldname, "STATE", 5)) {
        fieldcode = KB_DIAG_SQLSTATE; // i.e. 'C'
    }
    else {
        pg_error(aTHX_ dbh, EXECUTE_FATAL_ERROR, "Invalid error field");
    ...
    return NULL == KCIResultGetErrorField(imp_dbh->last_result, fieldcode) ? &PL_sv_undef : 
      sv_2mortal(newSVpv(KCIResultGetErrorField(imp_dbh->last_result, fieldcode), 0));
...
int pg_db_endcopy (SV * dbh)
{
    ...
    if (EXECUTE_COPY_IN == imp_dbh->copystate) {
        ...
        if (-1 == copystatus) {
            ...
            croak("KCICopySendEOF returned a value of %d\n", copystatus);
        ...
        TRACE_PQENDCOPY;
        copystatus = KCICopySync(imp_dbh->conn);
    }
...
int dbd_st_blob_read (SV * sth, imp_sth_t * imp_sth, int lobjId, long offset, long len, SV * destrv, long destoffset)
{
    ...
    /* safety checks */
    if (lobjId <= 0) {
        pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, "dbd_st_blob_read: lobjId <= 0");
        return 0;
    }
    if (offset < 0) {
        pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, "dbd_st_blob_read: offset < 0");
        return 0;
    }
    if (len < 0) {
        pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, "dbd_st_blob_read: len < 0");
        return 0;
    }
    if (! SvROK(destrv)) {
        pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, "dbd_st_blob_read: destrv not a reference");
        return 0;
    }
    if (destoffset < 0) {
        pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, "dbd_st_blob_read: destoffset < 0");
        return 0;
    }
    ...
    if (lobj_fd < 0) {
        TRACE_PQERRORMESSAGE;
        pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, KCIConnectionGetLastError(imp_dbh->conn));
...
long pg_db_result (SV *h, imp_dbh_t *imp_dbh)
{
    dTHX;
    KCIResult *result;
    KCIExecuteStatus status = EXECUTE_FATAL_ERROR;
    long rows = 0;
    char *cmdStatus = NULL;
    bool same_result;
    ...
    if (1 != imp_dbh->async_status) {
        pg_error(aTHX_ h, EXECUTE_FATAL_ERROR, "No asynchronous query is running\n");
    ...
    TRACE_PQGETRESULT;
    while ((result = KCIConnectionFetchResult(imp_dbh->conn)) != NULL) {
        ...
        case EXECUTE_TUPLES_OK:
            ...
            if (NULL != imp_dbh->async_sth) {
                ...
                TRACE_PQNFIELDS;
                DBIc_NUM_FIELDS(imp_dbh->async_sth) = KCIResultGetColumnCount(result);
        ...
        case EXECUTE_COMMAND_OK:
            /* non-select statement */
            TRACE_PQCMDSTATUS;
            cmdStatus = KCIResultGetCommandStatus(result);
            if (0 == strncmp(cmdStatus, "INSERT", 6)) {
                /* INSERT(space)oid(space)numrows */
                for (rows=8; cmdStatus[rows-1] != ' '; rows++) {
                }
        ...
        case PGRES_COPY_BOTH:
            /* Copy Out/In data transfer in progress */
            imp_dbh->copystate = status;
            imp_dbh->copybinary = KCIResultIsBinary(result);
            rows = -1;
            break;
        case EXECUTE_EMPTY_QUERY:
        case EXECUTE_BAD_RESPONSE:
        case EXECUTE_NONFATAL_ERROR:
            rows = -2;
            TRACE_PQERRORMESSAGE;
            pg_error(aTHX_ h, status, KCIConnectionGetLastError(imp_dbh->conn));
            break;
        case EXECUTE_FATAL_ERROR:
        default:
            rows = -2;
...
int pg_db_ready(SV *h, imp_dbh_t *imp_dbh)
{
    ...
    if (!KCIConnectionForceRead(imp_dbh->conn)) {
        ...
        TRACE_PQERRORMESSAGE;
        pg_error(aTHX_ h, EXECUTE_FATAL_ERROR, KCIConnectionGetLastError(imp_dbh->conn));
    ...
    TRACE_PQISBUSY;
    return KCIConnectionIsBusy(imp_dbh->conn) ? 0 : 1;
...
int pg_db_cancel(SV *h, imp_dbh_t *imp_dbh)
{
    ...
    if (-1 == imp_dbh->async_status) {
        pg_error(aTHX_ h, EXECUTE_FATAL_ERROR, "Asychronous query has already been cancelled");
    ...
    TRACE_PQGETCANCEL;
    cancel = KCICancelAlloc(imp_dbh->conn);

    /* This almost always works. If not, free our structure and complain loudly */
    TRACE_PQGETCANCEL;
    if (! KCICancelSend(cancel,errbuf,sizeof(errbuf))) {
        TRACE_PQFREECANCEL;
        KCICancelAlloc(cancel);
        if (TRACEWARN_slow) { TRC(DBILOGFP, "%sKCICancelAlloc failed: %s\n", THEADER_slow, errbuf); }
        _fatal_sqlstate(aTHX_ imp_dbh);
        pg_error(aTHX_ h, EXECUTE_FATAL_ERROR, "KCICancelAlloc failed");
    ...
    status = _sqlstate(aTHX_ imp_dbh, result);
    if (!result) {
        pg_error(aTHX_ h, EXECUTE_FATAL_ERROR, "Failed to get a result after KCICancelAlloc");
...
static int handle_old_async(pTHX_ SV * handle, imp_dbh_t * imp_dbh, const int asyncflag) {
    ...
    if (asyncflag & PG_OLDQUERY_CANCEL) {
        ...
        TRACE_PQISBUSY;
        if (KCIConnectionIsBusy(imp_dbh->conn)) {
            KCICancel *cancel;
            char errbuf[256];
            int cresult;
            ...
            cancel = KCICancelAlloc(imp_dbh->conn);
            TRACE_PQCANCEL;
            cresult = KCICancelSend(cancel,errbuf,255);
            if (! cresult) {
                if (TRACEWARN_slow) { TRC(DBILOGFP, "%sKCICancelSend failed: %s\n", THEADER_slow, errbuf); }
                _fatal_sqlstate(aTHX_ imp_dbh);
                pg_error(aTHX_ handle, EXECUTE_FATAL_ERROR, "Could not cancel previous command");
            ...
            TRACE_PQEXEC;
            KCIStatementExecute(imp_dbh->conn, "rollback");
            imp_dbh->done_begin = DBDPG_FALSE;
        }
    }
        ...
        while ((result = KCIConnectionFetchResult(imp_dbh->conn)) != NULL) {
            ...
            TRACE_PQCLEAR;
            KCIResultDealloc(result);
            if (status == EXECUTE_COPY_IN) { /* In theory, this should be caught by copystate, but we'll be careful */
                TRACE_PQPUTCOPYEND;
                if (-1 == KCICopySendEOF(imp_dbh->conn, NULL)) {
                    TRACE_PQERRORMESSAGE;
                    pg_error(aTHX_ handle, EXECUTE_FATAL_ERROR, KCIConnectionGetLastError(imp_dbh->conn));
                    if (TEND_slow) TRC(DBILOGFP, "%sEnd handle_old_async (error: KCICopySendEOF)\n", THEADER_slow);
                    return -2;
                }
            }
            else if (status == EXECUTE_COPY_OUT) { /* Won't be as nice with this one */
                pg_error(aTHX_ handle, EXECUTE_FATAL_ERROR, "Must finish copying first");
                if (TEND_slow) TRC(DBILOGFP, "%sEnd handle_old_async (error: COPY_OUT status)\n", THEADER_slow);
                return -2;
            }
            else if (status != EXECUTE_EMPTY_QUERY
                     && status != EXECUTE_COMMAND_OK
                     && status != EXECUTE_TUPLES_OK) {
                TRACE_PQERRORMESSAGE;
                pg_error(aTHX_ handle, status, KCIConnectionGetLastError(imp_dbh->conn));
        ...
        pg_error(aTHX_ handle, EXECUTE_FATAL_ERROR, "Cannot execute until previous async query has finished");
...
int dbd_st_cancel(SV *sth, imp_sth_t *imp_sth)
{
    ...
    if (!KCICancelSend(cancel, errbuf, sizeof(errbuf))) {
        TRACE_PQFREECANCEL;
        KCICancelDealloc(cancel);
        if (TRACEWARN_slow) TRC(DBILOGFP, "%sKCICancelDealloc failed: %s\n", THEADER_slow, errbuf);
        pg_error(aTHX_ sth, EXECUTE_FATAL_ERROR, "KCICancelDealloc failed");
...
SV* dbd_st_canonical_ids(SV *sth, imp_sth_t *imp_sth)
{
    dTHX;
    TRACE_PQNFIELDS;
    int fields = KCIResultGetColumnCount(imp_sth->result);
    AV* result = newAV();
    av_extend(result, fields);
    while(fields--){
        int stored = 0;
        TRACE_PQFTABLE;
        int oid = KCIResultGetRelationOidOfColumn(imp_sth->result, fields);
        if(oid != InvalidOid){
            TRACE_PQFTABLECOL;
            int pos = KCIResultGetRelationOidOfColumncol(imp_sth->result, fields);
            if(pos > 0){
                AV * row = newAV();
                av_extend(row, 2);
        ...
        if(oid != InvalidOid) {
            ...
            if(pos > 0){
                ...
                if (EXECUTE_TUPLES_OK == status) {
                    ...
                    if (KCIResultGetRowCount(result)!=0) {
                        TRACE_PQGETLENGTH;
                        int len = KCIResultGetColumnValueLength(result, 0, 0) + 1;
                        TRACE_PQGETLENGTH;
                        len += KCIResultGetColumnValueLength(result, 0, 1) + 1;
                        TRACE_PQGETLENGTH;
                        len += KCIResultGetColumnValueLength(result, 0, 2);
                        SV* table_name = newSV(len);
                        TRACE_PQGETVALUE;
                        char *nsp = KCIResultGetColumnValue(result, 0, 0);
                        TRACE_PQGETVALUE;
                        char *tbl = KCIResultGetColumnValue(result, 0, 1);
                        TRACE_PQGETVALUE;
                        char *col = KCIResultGetColumnValue(result, 0, 2);
...
1    EXECUTE_COMMAND_OK,            // a query command that doesn't return
                                   anything was executed properly by the
                                   backend 
2    EXECUTE_TUPLES_OK,            // a query command that returns tuples was
                                   executed properly by the backend, KCIResult
                                   contains the result tuples 
3    EXECUTE_COPY_OUT,                // Copy Out data transfer in progress 
4    EXECUTE_COPY_IN,                // Copy In data transfer in progress 
5    EXECUTE_BAD_RESPONSE,            // an unexpected response was recv'd from the
                                   backend 
6    EXECUTE_NONFATAL_ERROR,        // notice or warning message 
7    EXECUTE_FATAL_ERROR            // query failed 


8    KCIRES_COPY_BOTH,              // Copying both out and in data
9    KCIRES_SINGLE_TUPLE,           // A single tuple from a larger result set
10   EXECUTE_PARTIAL_TUPLES,        // Retrieving the result set in batches

#ifndef WIN32
11    EXECUTE_SYNC                  // Execute incremental synchronization
#endif
} KCIExecuteStatus;
