...
btool_faq - Frequently-Asked Questions about btparse and LaTeX::BibTeX
...
This section covers questions that users of C<LaTeX::BibTeX>, the Perl
component of B<btOOL>, have asked.

=head2 Why aren't the BibTeX "month" macros defined?

Because they're bibliography-specific, and C<LaTeX::BibTeX> by default
...
style files:
   ...
   $bibfile = new LaTeX::BibTeX::File 'foo.bib'    # open file
      or die "foo.bib: $!\n";
   $entry = new LaTeX::BibTeX::Entry $bibfile;     # parse first entry
...
of fake entries, but now C<LaTeX::BibTeX> provides a direct interface to
...
C<LaTeX::BibTeX>'s object-oriented analogue to style files, called
structure modules.  C<LaTeX::BibTeX> provides a structure module,
C<LaTeX::BibTeX::Bib>, that (partially) emulates the standard style files
...
method on a C<LaTeX::BibTeX::File> object.  It's quite simple to tell
C<LaTeX::BibTeX> that entries from C<$bibfile> are expected to conform to
the C<Bib> structure (which is implemented by the C<LaTeX::BibTeX::Bib>
...
Remember, a C<LaTeX::BibTeX::File> object is mainly a wrapper around an
C<IO::File> object, and the C<LaTeX::BibTeX::File::open> method (and thus
...
C<LaTeX::BibTeX::File>'s C<new> method, then it's treated just like a
filename passed to Perl's builtin C<open>:

   my $append_file = new LaTeX::BibTeX::File ">>$filename"
...
C<LaTeX::BibTeX> distribution for a complete example.
