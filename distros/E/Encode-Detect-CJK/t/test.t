...
ok('iso-8859-1' eq Encode::Detect::CJK::detect(''));
ok(Encode::Detect::CJK::detect(undef) eq '' );

#check det1 ok 
my $str_gbk = Encode::encode('gbk', $str_uni);
ok('cp936' eq Encode::Detect::CJK::detect($str_gbk));

my $str_utf8 = Encode::encode('utf8', $str_uni);
ok('utf8' eq Encode::Detect::CJK::detect($str_utf8));

my $str_sjis = Encode::encode('shiftjis', $str_uni);
ok('shiftjis' eq Encode::Detect::CJK::detect($str_sjis));

my $str_euc_jp = Encode::encode('euc-jp', $str_uni);
ok('euc-jp' eq Encode::Detect::CJK::detect($str_euc_jp));

my $str_iso_2002_jp = Encode::encode('iso-2022-jp', $str_uni);
ok('iso-2022-jp' eq Encode::Detect::CJK::detect($str_iso_2002_jp));

#check trim ok
$str_gbk = Encode::encode('gbk', $str_uni);
ok('iso-8859-1' eq Encode::Detect::CJK::detect($str_gbk,4));
ok('cp936' eq Encode::Detect::CJK::detect('abcd  charset=gb18030',4));

#check det2 ok
my $str_charset = "charset=gb18030";
ok('cp936' eq Encode::Detect::CJK::detect($str_charset)); 

#check det1 && det2
my $str_uni2 = Encode::decode('utf8', 'abcd連動型広告配信過去のプレスリリース一覧 charset=gb18030');
$str_iso_2002_jp = Encode::encode('iso-2022-jp', $str_uni2);
ok('cp936' eq Encode::Detect::CJK::detect($str_iso_2002_jp));
