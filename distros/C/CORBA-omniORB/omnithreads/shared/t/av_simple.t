...
omnithreads->create(sub { $ref->[0] = "thread4"})->join();
ok(31, $ref->[0] eq "thread4", "Check that it works after another thread");
undef($ref);
omnithreads->create(sub { @foo = () })->join();
...
omnithreads->create(sub { delete($foo[0])})->join();
...

{
    ...
    omnithreads->create(sub { @foo[0,1] = ("hej","hop") })->join();
