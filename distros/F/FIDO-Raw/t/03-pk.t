use strict;
use warnings;
use Test::More;

use FIDO::Raw;

sub packit
{
	my $data = join ('', @_);
	$data =~ s/0x//g;
	$data =~ s/\s+//g;

    pack ("H*", $data);
}

my $dummy_es256 = packit ((qw(
	0xcc 0x1b 0x50 0xac 0xc4 0x19 0xf8 0x3a
	0xee 0x0a 0x77 0xd6 0xf3 0x53 0xdb 0xef
	0xf2 0xb9 0x5c 0x2d 0x8b 0x1e 0x52 0x58
	0x88 0xf4 0x0b 0x85 0x1f 0x40 0x6d 0x18
	0x15 0xb3 0xcc 0x25 0x7c 0x38 0x3d 0xec
	0xdf 0xad 0xbd 0x46 0x91 0xc3 0xac 0x30
	0x94 0x2a 0xf7 0x78 0x35 0x70 0x59 0x6f
	0x28 0xcb 0x8e 0x07 0x85 0xb5 0x91 0x96
)));

my $dummy_rs256 = packit ((qw(
	
)));

my $dummy_eddsa = packit ((qw(
	0xfe 0x8b 0x61 0x50 0x31 0x7a 0xe6 0xdf
	0xb1 0x04 0x9d 0x4d 0xb5 0x7a 0x5e 0x96
	0x4c 0xb2 0xf9 0x5f 0x72 0x47 0xb5 0x18
	0xe2 0x39 0xdf 0x2f 0x87 0x19 0xb3 0x02
)));


my $es256 = FIDO::Raw::PublicKey::ES256->new ($dummy_es256);
isa_ok $es256, 'FIDO::Raw::PublicKey::ES256';
ok (!eval {FIDO::Raw::PublicKey::ES256->new (undef)});
ok (!eval {FIDO::Raw::PublicKey::ES256->new ('')});

my $rs256 = FIDO::Raw::PublicKey::RS256->new ($dummy_rs256);
isa_ok $rs256, 'FIDO::Raw::PublicKey::RS256';
ok (!eval {FIDO::Raw::PublicKey::RS256->new (undef)});
ok (!eval {FIDO::Raw::PublicKey::RS256->new ('')});

my $eddsa = FIDO::Raw::PublicKey::EDDSA->new ($dummy_eddsa);
isa_ok $eddsa, 'FIDO::Raw::PublicKey::EDDSA';
ok (!eval {FIDO::Raw::PublicKey::EDDSA->new (undef)});
ok (!eval {FIDO::Raw::PublicKey::EDDSA->new ('')});

done_testing;

