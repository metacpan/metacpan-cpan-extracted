...
The execveat() system call is a nonstandard extension present in Linux.
...
Returns a file handle associated with the I<numeric> file descriptor I<fd>
or undef on error. I<mode> is one of the values C<"r">, C<"w">, C<"a"> with
an optional C<"+"> and/or C<"b">.
...
C<what> can be a path or, if your system supports fpathconf(), a Perl file or
directory handle or a file descriptor.
...
Like C<POSIX::read()> but returns 0 at EOF instead of I<"0 but true">. Croaks
if I<buf> is read-only and I<count> is non-zero.
