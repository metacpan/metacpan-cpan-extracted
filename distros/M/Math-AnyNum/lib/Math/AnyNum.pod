...
=head1 FUNCTIONS
  ...
  :ntheory
    ...
    euler(n,x)                Euler polynomial: E_n(x)
    bernoulli(n,x)            Bernoulli polynomial: B_n(x)
...
Natural logarithm of the Gamma function on C<x>.
...
If C<x> is greater than C<y>, the returned value will be in the range "[y, x)".
...
The original project was called C<Math::BigNum>, but because of some design flaws, the project
