...
ok(path('CODE_OF_CONDUCT.md')->is_file(), 'Policy file CODE_OF_CONDUCT.md exists');

# Read file and remove whitespace from the end.
my $policy = path('CODE_OF_CONDUCT.md')->slurp_utf8 =~ s/[[:space:]]+$//r;
...
do {
    ...
    is($policy, $wanted, 'Policy file CODE_OF_CONDUCT.md is current');
};
...
Software-Policies project a harassment-free experience for everyone,
...
Software-Policies project include:
...
This Code of Conduct applies within all Software-Policies project spaces, and
also applies when an individual is officially representing the
Software-Policies project in public spaces. Examples of representing the
Software-Policies project include using an official email address, posting
