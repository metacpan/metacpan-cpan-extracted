...
1;
# ABSTRACT: File name, defaults to newest file in current directory (if there is one)
...
Sah::Schema::filename::default_newest_file_in_curdir - File name, defaults to newest file in current directory (if there is one)

=head1 VERSION

This document describes version 0.031 of Sah::Schema::filename::default_newest_file_in_curdir (from Perl distribution Sah-SchemaBundle-Path), released on 2024-02-16.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("filename::default_newest_file_in_curdir*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("filename::default_newest_file_in_curdir", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("filename::default_newest_file_in_curdir", {return_type=>'str_errmsg+val'});
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("filename::default_newest_file_in_curdir*");
     $validator->(\@args);
     ...
 }
 ...
 $SPEC{myfunc} = {
     ...
     args => {
         arg1 => {
             ...
             schema => ['filename::default_newest_file_in_curdir*'],
         },
         ...
     },
 };
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"filename::default_newest_file_in_curdir*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('filename::default_newest_file_in_curdir*', name=>'FilenameDefaultNewestFileInCurdir')
     );
 }

 use My::Types qw(FilenameDefaultNewestFileInCurdir);
 FilenameDefaultNewestFileInCurdir->assert_valid($data);
