...
{  # TABULO : Make 'x_static_install' comparison dependent on authority being 'cpan:ETHER', since the plugin-bundle has specific logic for that case.
    my $authority = $tzil->plugin_named('@Author::TABULO/Authority')->authority;
    cmp_deeply(
        $tzil->distmeta,
        superhashof({
            ( ( $authority =~ /^cpan\:ETHER$/) ?  ( x_static_install => 1) : () ),
            prereqs => superhashof({
                develop => superhashof({
            ...
            x_Dist_Zilla => superhashof({
                plugins => supersetof(
                    ...
                    }),
                    {
                        ...
                        name => '@Author::TABULO/copy generated files',
                    ...
                    },
                    {
                        ...
                        name => '@Author::TABULO/@Git::VersionManager/CopyFilesFromRelease',
                        version => Dist::Zilla::Plugin::CopyFilesFromRelease->VERSION,
                    },
                    {
...
my $version = Dist::Zilla::PluginBundle::Author::TABULO->VERSION;
like(
    $contributing,
    qr/^template file originating in Dist-Zilla-PluginBundle-Author-TABULO-$version\.$/m,
...
ok(
    !$tzil->plugin_named('@Author::TABULO/ExecDir'),
