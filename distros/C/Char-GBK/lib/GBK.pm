...
#
# http://search.cpan.org/dist/Char-GBK/
#
...
perl $0 GBK_script.pl > Escaped_script.pl.e
END
    }

    print GBK::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (Egbk::e("$filename.e")) {
        if (exists $ENV{'CHAR_DEBUG'}) {
            Egbk::unlink "$filename.e";
        }
        elsif (Egbk::z("$filename.e")) {
            Egbk::unlink "$filename.e";
        }
        else {
            ...
            my $mtime_filter = (Egbk::stat(__FILE__     ))[9];
            my $mtime_source = (Egbk::stat($filename    ))[9];
            my $mtime_escape = (Egbk::stat("$filename.e"))[9];
    ...
    if (not Egbk::e("$filename.e")) {
        my $fh = gensym();
        Egbk::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e\n";
    ...
    my $fh = gensym();
    Egbk::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e\n";
...
sub GBK::escape_script {
    ...
    # read GBK script
    my $fh = gensym();
    Egbk::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script\n";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die "Can't close file: $script: $!";

    if (/^ use Egbk(?:(?>\s+)(?>[0-9\.]*))?(?>\s*); $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Egbk '%s.0'; # 'quote' for perl5.6\n", $GBK::VERSION); # require run-time routines version

        # use GBK version qw(ord reverse getc);
        if (s/^ (?>\s*) use (?>\s+) (?: Char | GBK ) (?>\s*) ([^\x81-\xFE;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ((?>[0-9]+)\.(?>[0-9]+)) \.0 (?>\s*) //oxms) {
                my $version = $1;
                if ($version ne $GBK::VERSION) {
                    my @file = grep -e, map {qq{$_/GBK.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects GBK.pm $version, but @{[__FILE__]} is version $GBK::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Egbk.pm $version, but \\\$Egbk::VERSION is \$Egbk::VERSION" if \$Egbk::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ((?>[0-9]+)(?>\.[0-9]*)) (?>\s*) //oxms) {
                my $version = $1;
                if ($version > $GBK::VERSION) {
                    die "Script $0 required GBK.pm $version, but @{[__FILE__]} is only version $GBK::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A (?>\s*) \z/oxms) {
                ...
                for (@list) {
                    $Egbk::function_ord     = 'GBK::ord'     if /\A ord \z/oxms;
                    $Egbk::function_ord_    = 'GBK::ord_'    if /\A ord \z/oxms;
                    $Egbk::function_reverse = 'GBK::reverse' if /\A reverse \z/oxms;
                    $Egbk::function_getc    = 'GBK::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
=item * You'll have to download GBK.pm and Egbk.pm and put it in your perl lib directory.

=item * You'll need to write "use GBK;" at head of the script.
...
GBK software is "middleware" between perl interpreter and your Perl script
written in GBK.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain GBK, Perl3 or later
can treat GBK as binary data.

By "use GBK;", it automatically interpret your script as GBK. The various
functions of perl including a regular expression can treat GBK now.
...
This software is a source code filter to escape Perl script encoded by GBK
...
=item * Remains One Language Handling Raw GBK, Doesn't Use UTF8 flag like JPerl
...
=head1 Escaping Multiple-Octet Code (GBK software provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (GBK software provides)
...
=head1 Escaping Second Octet (GBK software provides)

GBK software escapes second octet of multiple-octet character in regular
expression.
...
=head1 Multiple-Octet Character Regular Expression (GBK software provides)

GBK software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Egbk::ignorecase()' (GBK software provides)
...
=head1 Escaping Character Classes (Egbk.pm provides)
...
=head1 Un-Escaping \ of \b{}, \B{}, \N{}, \p{}, \P{}, and \X (GBK software provides)

GBK software removes '\' at head of alphanumeric regexp metasymbols \b{}, \B{},
...
=head1 Escaping Built-in Functions (GBK software provides)

Insert 'Egbk::' at head of function name. Egbk.pm provides your script Egbk::*
subroutines.
...
=head1 Escaping File Test Operators (GBK software provides)
...
You need write 'GBK::' at head of function name when you want character-
...
=head1 Yada Yada Operator (GBK software provides)
...
=head1 Un-Escaping bytes::* Subroutines (GBK software provides)
...
some bugs. Thus, if you happen to find a bug that's in GBK software and
...
=item * GBK::substr as Lvalue

If Perl version is older than 5.14, GBK::substr differs from CORE::substr, and
...
Even if ${^WIN32_SLOPPY_STAT} is set to a true value, file test functions Egbk::*(),
Egbk::lstat(), and Egbk::stat() on Microsoft Windows open the file for the path
