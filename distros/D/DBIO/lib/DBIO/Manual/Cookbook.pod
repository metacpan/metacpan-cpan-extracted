...
In case you use this "trick" together with L<DBIO::Schema/deploy>, a table
called "dual" would be created in your schema. This would overlap "sys.dual" and
you could no longer fetch "sysdate" or "sequence.nextval" from it. To avoid the
problem, keep the C<Dual> result class out of the set of sources you deploy - for
example by registering it on a separate schema that you never call C<deploy> on.
...
B<Legacy path.> C<sqlt_deploy_hook> is a deprecated, SQL::Translator-era hook
kept for backwards compatibility. With the native Deploy classes, declare
indexes through the driver's own facilities instead - C<< __PACKAGE__->indices >>
(see L<DBIO::ResultSourceProxy/indices>) or the L<DBIO::Cake/idx> DSL, which also
emit native C<pg_indexes> for the PostgreSQL Deploy class. The hook below still
fires for code that has not yet been ported.
...
The following example shows how you might use DBIO to deploy and upgrade
versioned schemas on your customers' databases. DBIO tracks the deployed
version in a C<__VERSION> table via L<DBIO::DeploymentHandler>, and the native
driver Deploy class reconciles the live database against your current schema in
one shot - there are no pre-generated SQL diff files to ship. The basic process
is as follows:
...
Create a DBIO schema and give the schema class a C<$VERSION>
...
Deploy the update to customers

=back

B<Create and version a DBIO schema>

Build the schema manually or generate it from an existing database as described
under L</Creating Schemas From An Existing Database>, and set a C<$VERSION> on
the schema class so the deployment handler can track it:
...
Install the schema and record its version:

  my $schema = My::Schema->connect($dsn, $user, $pass);
  my $dh = DBIO::DeploymentHandler->new({ schema => $schema });
  $dh->prepare_install;   # create the __VERSION table
  $dh->install;           # deploy the schema via the native Deploy class

B<Modify the schema to change functionality>

As your application evolves, edit your Result classes and bump the schema
class C<$VERSION>. There are no conversion scripts to generate: the native
Deploy class computes the diff between the live database and your current
schema when you upgrade.

B<Deploy the update to customers>

Run the upgrade. The deployment handler reconciles the live database against
the current schema and bumps the recorded version:

  my $schema = My::Schema->connect($dsn, $user, $pass);
  my $dh = DBIO::DeploymentHandler->new({ schema => $schema });
  $dh->upgrade;

For data migrations that a DDL diff cannot express (column backfills, value
normalisation, and so on) pass C<upgrade_hooks> to
C<< DBIO::DeploymentHandler->new >>; see L<DBIO::DeploymentHandler> for details.
The L<dbioadmin> CLI wraps the same deploy and upgrade operations for
command-line use.
...
If you build your classes dynamically based on the database schema at
startup then there will be a significant startup delay.

For production use a statically defined schema, which can be generated
using the L<dbiogen> utility (powered by L<DBIO::Generate>) to dump the
database schema once - see L<DBIO::Generate/dump_directory> for more
