...
1;
# ABSTRACT: A string that starts with [A-Za-z_] followed by zero of more [A-Za-z0-9_]
...
Sah::Schema::identifier - A string that starts with [A-Za-z_] followed by zero of more [A-Za-z0-9_]

=head1 VERSION

This document describes version 0.002 of Sah::Schema::identifier (from Perl distribution Sah-SchemaBundle-Identifier), released on 2025-11-11.

=head1 SAH SCHEMA DEFINITION

 [
   "str",
   { min_len => 1, match => qr/\A[A-Za-z_][A-Za-z0-9_]*\z/ },
 ]
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("identifier*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("identifier", {return_type=>'str_errmsg'});
...
(set-as-default, coerced, prefiltered) value:
 ...
 my $validator = gen_validator("identifier", {return_type=>'str_errmsg+val'});
 my $res = $validator->($data); # [$errmsg, $validated_val]
 
 # a sample valid data
 $data = "a_";
 my $res = $validator->($data); # => ["","a_"]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("identifier*");
     $validator->(\@args);
     ...
 }
...
to validate as the second argument (encoded in Perl syntax):
 ...
 % validate-with-sah '"identifier*"' '"data..."'
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('identifier*', name=>'Identifier')
     );
 }

 use My::Types qw(Identifier);
 Identifier->assert_valid($data);
