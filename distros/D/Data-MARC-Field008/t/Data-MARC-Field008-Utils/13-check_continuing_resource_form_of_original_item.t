...
use Data::MARC::Field008::Utils qw(check_continuing_resource_form_of_original_item);
...
my $ret = check_continuing_resource_form_of_original_item($self, 'key');
is($ret, undef, 'Right continuing resource form of original item is present (a).');

# Test.
$self = {
	'key' => '|',
};
$ret = check_continuing_resource_form_of_original_item($self, 'key');
is($ret, undef, 'Right continuing resource form of original item is present (|).');
