...
sub load_mecab {
    local $@;
    eval { require MeCab; };
    plan skip_all => "MeCab.pm is not available." if $@;
    my $mecab;
    eval {
        $mecab = MeCab::Tagger->new(@_);
    };
    plan skip_all => "MeCab::Tagger is not available. $@" unless ref $mecab;
	undef;
}
# ----------------------------------------------------------------
sub read_data {
...
#   ok( utf8::is_utf8($t->{phrase2}), "source: phrase2 utf8 flaged" );
    ...
    like( $t4[0][1], qr/(^|\W)(yuu|u|tamotsu)(\W|$)/, "string: okuri-nashi yuu or tamotsu" );
