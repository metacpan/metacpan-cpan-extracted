...
plugin 'FormFieldsFromJSON' => {
  ...
  template => '<label for="<%= $id %>"><%= $label %>:(<%= $message %>)</label><div><%= $field %></div>',
};
...
$t->get_ok('/')
  ->status_is(200)
  ->content_is(qq~<label for="name">Name:(Your name)</label><div><input id="name" name="name" type="text" value=""$close></div>\n~);
