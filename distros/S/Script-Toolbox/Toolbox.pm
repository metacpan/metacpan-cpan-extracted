...
Return undef in case of an error otherwise 1;
...
=item setReadOnlys(<name>,<readOnly>)

This method provides setting of the readOnly fields of options.
<readOnly> is a hash like {<label> => <readOnly>}. Only updates on existing
labels are allowed.

=item setJumps(<name>,<jump>)

This method provides setting of the jump fields of options.
<jump> is a hash like {<label> => <jump>}. Only updates on existing
labels are allowed.
...
=item CheckBox(<head>,<opts>[,<preSel>[,<cont>[,<menName>]]]])
...
=item RadioButton(<head>,<opts>[<preSel>[,<cont>[,<menName>]]]])

Display the options <opts> as menu. Each of them has an autogenerated 
value field with possible values of '' (means: OFF) or 'x' (means: ON).
Only one option at the same time can be ON.
<preSel> is a regular expression matching one of the labels.
The option with this label gets in ON state. All other options are switched OFF.
Selecting an option switch it ON and all other OFF.
