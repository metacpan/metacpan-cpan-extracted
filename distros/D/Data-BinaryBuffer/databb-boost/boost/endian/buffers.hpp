...
//----------------------------------  end synopsis  ------------------------------------//
  ...
  namespace detail
  {
    ...
    template <typename T, std::size_t n_bytes,
      bool sign=databb_boost::is_signed<T>::value >
    struct unrolled_byte_loops
    {
      ...
      typedef typename databb_boost::conditional<
        databb_boost::is_integral<T>::value,
        databb_boost::make_unsigned<T>, databb_boost::type_identity<T> >::type::type U;
...
#   if defined(__x86_64__) || defined(_M_X64) || defined(__i386) || defined(_M_IX86)
      ...
      if (sizeof(T) == n_bytes)
      {
        ...
        return databb_boost::endian::big_to_native(t);
      }
#   endif
...
#     if defined(__x86_64__) || defined(_M_X64) || defined(__i386) || defined(_M_IX86)
      ...
      if (sizeof(T) == n_bytes)
      {
        databb_boost::endian::native_to_big_inplace(value);
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "big, aligned, " << n_bits
              << "-bits, construct(" << val << ")\n";
#       endif
          m_value = ::databb_boost::endian::native_to_big(val);
        }
...
#     endif
        ...
        //{
        //  return ::databb_boost::endian::big_to_native(m_value);
        //}
        value_type value() const BOOST_NOEXCEPT
        {
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "big, aligned, " << n_bits << "-bits, convert("
              << ::databb_boost::endian::big_to_native(m_value) << ")\n";
#       endif
          return ::databb_boost::endian::big_to_native(m_value);
        }
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "little, aligned, " << n_bits
              << "-bits, construct(" << val << ")\n";
#       endif
          m_value = ::databb_boost::endian::native_to_little(val);
        }
...
#       ifdef BOOST_ENDIAN_LOG
          if ( endian_log )
            std::cout << "little, aligned, " << n_bits << "-bits, convert("
              << ::databb_boost::endian::little_to_native(m_value) << ")\n";
#       endif
          return ::databb_boost::endian::little_to_native(m_value);
        }
        const char* data() const BOOST_NOEXCEPT
          {return reinterpret_cast<const char*>(&m_value);}
      protected:
