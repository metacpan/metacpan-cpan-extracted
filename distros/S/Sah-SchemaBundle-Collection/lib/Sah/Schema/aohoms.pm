...
Sah::Schema::aohoms - Array of (defined-)hash-of-maybe-strings

=head1 VERSION

This document describes version 0.009 of Sah::Schema::aohoms (from Perl distribution Sah-SchemaBundle-Collection), released on 2024-06-13.
...
To check data against this schema (requires L<Data::Sah>):
 ...
 my $validator = gen_validator("aohoms*");
...
valid, a non-empty error message otherwise):
 ...
 my $validator = gen_validator("aohoms", {return_type=>'str_errmsg'});
 ...
 $data = [[]];
 my $errmsg = $validator->($data); # => "\@[0]: Not of type hash"

Often a schema has coercion rule or default value rules, so after validation the
validated value will be different from the original. To return the validated
(set-as-default, coerced, prefiltered) value:

 my $validator = gen_validator("aohoms", {return_type=>'str_errmsg+val'});
 ...
 # a sample invalid data
 $data = [[]];
 my $res = $validator->($data); # => ["\@[0]: Not of type hash",[[]]]
...
To validate function parameters against this schema (requires L<Params::Sah>):
 ...
 sub myfunc {
     my @args = @_;
     state $validator = gen_validator("aohoms*");
     $validator->(\@args);
     ...
 }
...
L<Type::Tiny> as well as L<Type::FromSah>):
 ...
 package My::Types {
     ...
     __PACKAGE__->add_type(
         sah2type('aohoms*', name=>'Aohoms')
     );
 }

 use My::Types qw(Aohoms);
 Aohoms->assert_valid($data);
