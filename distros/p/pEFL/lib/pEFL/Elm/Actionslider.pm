...
require XSLoader;
XSLoader::load('pEFL::Elm::Actionslider');
...
pEFL::Elm:Actionslider

=head1 SYNOPSIS

  use pEFL::Elm;
  [...]
  my $actionslider = pEFL::Elm::Actionslider->add($parent);
...
For instructions, how to use pEFL::Elm::Actionslider, please study this API reference for now. A perl-specific documentation will perhaps come in later versions. But applying the C documentation should be no problem. pEFL::Elm::Actionslider gives you a nice object-oriented interface that is kept close to the C API. But Please note, that the perl method names remove the "elm_actionslider_" at the beginning of the c functions.
