...
Like all vectorized languages, PDLA automates looping over arrays using
...
that apply to PDLAs.  From the L<perldla|perldla> (or L<pdla2|PDLA::Perldl2>) shell, you
...
=item nelem - the total number of elements in a PDLA

=item ndims - returns the number of dimensions in a PDLA

=item dims - returns the dimension list of a PDLA as a Perl list

=item dim - returns the size of a particular dimension of a PDLA
...
child and parent PDLA:

=over 3

=item copy - forces an explicit copy of a PDLA

=item sever - breaks the dataflow connection between a PDLA and its parents (if any)
...
looped over.  There are several ways to transpose or re-order the dimension list of a PDLA.
...
'magically') generated by PDLA based on the dimensions of the pdls
...
L<PDLA::PP|PDLA::PP> compiled functions) and that the pdls are "threaded" over. 
...
from each of its pdl arguments. The L<inner|PDLA::Primitive/inner> function 
