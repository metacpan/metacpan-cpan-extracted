...
#
# (July 23, 2001) Modified by Barry Leslie to support PrimeBase.
#
...
$opt_server="PrimeBase"; 
my $file;
if (-f ($file = "$opt_server.env")) {
    eval { require $file; };
    if ($@) {
	...
	$opt_user=$ENV{'DBI_USER'};
	$opt_password=$ENV{'DBI_PASS'};
	$opt_host="$ENV{'DBI_SERVER'};$ENV{'DBI_HOST'}";
	$opt_database="$ENV{'DBI_DATABASE'}";
} else {
	$opt_user="Administrator";
	$opt_password="";
	$opt_host="PrimeServer;localhost'}";
	$opt_database="demo";
}
