...
SV * new_from_fd(const char * class, int fd, STRLEN maxlen = 0, SV * flags = &PL_sv_undef)
  ...
  for (;;) {
    if (bytes_total == pmsize) {
      if (pmsize > SIZE_MAX / 2)
        croak("new_from_fd: Out of memory");
      pmsize *= 2;
      if (maxlen && maxlen < pmsize)
        pmsize = maxlen;
...
SV * _overload_mult(SV * self, SV * other, SV * swapped)
  ...
  self_pm = protmem_get(aTHX_ self, MEMVAULT_CLASS);
  if (self_pm->size > SIZE_MAX / count)
    croak("_overload_mult: Out of memory");
...
void concat(SV * self, SV * other, SV * swapped = &PL_sv_undef)
  ...
  if (self_pm->size > SIZE_MAX - buf_len)
    croak("concat: Out of memory");
...
SV * pad(SV * self, STRLEN blocksize)
  ...
  if (pad_len > SIZE_MAX - buf_len)
    croak("pad: Out of memory");
...
SV * to_bytes(SV * self, ...)
  ...
  if (!(self_pm->flags & PROTMEM_FLAG_LOCK_UNLOCKED))
    croak("to_bytes: Unlock MemVault object before stringifying");

  if (protmem_grant(aTHX_ self_pm, PROTMEM_FLAG_MPROTECT_RO) != 0)
    croak("to_bytes: Failed to grant protmem RO");

  RETVAL = newSVpvn((char *)self_pm->pm_ptr, self_pm->size);

  if (protmem_release(aTHX_ self_pm, PROTMEM_FLAG_MPROTECT_RO) != 0)
    croak("to_bytes: Failed to release protmem RO");
...
SV * to_hex(SV * self, SV * flags = &PL_sv_undef)
  ...
  if (SvOK(flags))
    new_flags = SvUV_nomg(flags);
  if (self_pm->size > SIZE_MAX / 2 - 1)
    croak("to_hex: Out of memory");
