...
#### EDIT all these inside this current file: they are in string vars
...
#
# You will end up with a 'whitelist.txt.final' file.
# This file will be read here and update the
#    %function_names_whitelist
# automatically, you do not need to do anything with its contents.

# Also, don't do anything with the blacklist. There are already
# blacklisted functions below and if some more exist in file
# 'blacklist.txt.final' (just produced) then they will be read automatically in.
# Only add to the blacklist if GCC adds more builtins and some of them need to
# be blacklisted.

# The blacklisted functions are those returning weird return types which
# I did not know how to express them in XS (check file 'typemap' for what
# types are already understood).

# For a function to be added must exist in whitelist and not exist in blacklist.
#
# When sbin/build-gcc-builtins-package.sh finishes OK, you
# need to run:
#   sbin/build-gcc-builtins-package.pl
#
# (Note the .sh and .pl difference)
# And if that runs OK, then do:
#
#   perl Makefile.PL && make all && make test
#
# The test files will tell you what value was expected.
# (search in the test file output for e.g.:
#    copy-this-expected-value 'powil' => '4',
# )
# Normally, nothing needs to be changed.
# However, if a test fails it is likely that expected values need updating.
# To do this observe expected values and copy them, e.g. 'powil' => '4',
# in the hash
#    %T_EXPECTED_RESULTS
# below (in this file).
...
# Also, in some Perl's (5.12.5, e.g. see http://www.cpantesters.org/report/7a6d0bce-fdcd-11ee-892f-69b3be5e1c1b)
# expected 'Inf' and yielded 'inf' failed some tests. But that is fixed with v0.6
#
...
# Add here functions to be blacklisted.
# Blacklisted functions are either unknown or depend on unknown symbols
# they will be excluded from our package.
# These functions here are in addition to whatever 'blacklist.txt.final'
# file may contain which are read and appended to this later on.
# As it is, the list of functions here works fine and needs not be changed.
# If GCC adds more builtins and any of them needs to be blacklisted add it
# here:
my %function_names_blacklist = map { $_ => 1 } qw/
__builtin_alloca
...
# the whitelist is read from the file mentioned below, there is no need to add
# any contents of that file here unless you want to override something.
my %function_names_whitelist;
my $bs = 'whitelist.txt.final';
if( -f $bs ){
	my $xx;
...
# read the blacklisted functions from this file (if any) and append
# to already specified blacklisted functions, above:
$bs = 'blacklist.txt.final';
if( -f $bs ){
	my $xx;
	open($xx, '<', $bs) or die "failed to open $bs: $!";
	while( my $al = <$xx> ){
