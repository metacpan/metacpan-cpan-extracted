...
sub client_input {
  ...
  if ( $cmd eq 'MODE' && $prefix =~ m'^bobbins' && $params->[1] eq '+i' ) {
    $heap->{ircd}->_state_do_change_hostmask('1FUAAAAAB', 'magic.super.mask');
    return;
  }
  if ( $cmd eq '396' ) {
    pass("IRC_$cmd");
    is( $params->[1], 'magic.super.mask', 'Hostmask is: "magic.super.mask"' );
    is( $params->[2], 'is now your visible host', 'is now your visible host' );
    $poe_kernel->post($sender,'send_to_server',{ command => 'QUIT', params => [ 'Bye Bye' ] });
    return;
  }
  if ( $cmd eq 'ERROR' ) {
