...
Data::SIprefixes::deca - This provides deca matching for Data::SIprefixes.
...
=head1 SYNOPSIS
    ...
    use Data::SIprefixes::deca;
    
    my $prefix=Data::SIprefixes::deca->new;

    my $origMeasure='decameter';
...
sub longMatch{
	...
	if ( $measure=~/^deca/ ){
		...
		if ( $measure =~ /^ / ){
			$self->{error}=2;
			$self->{errorString}='Space found after prefix, /^deca/, in "'.$origMeasure.'"' ;
			$self->warn;
			return undef;
		}
...
sub shortMatch{
    ...
    if ( $measure=~/^da/ ){
        ...
        if ( $measure =~ /^ / ){
            $self->{error}=2;
            $self->{errorString}='Space found after prefix, /^da/, in "'.$origMeasure.'"' ;
            $self->warn;
            return undef;
        }
