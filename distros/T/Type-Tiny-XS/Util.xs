...
#define MY_CXT_KEY "Type::Tiny::XS::Util::_guts" XS_VERSION
typedef struct {
    HV* metas;
} my_cxt_t;
START_MY_CXT
...
AV*
typetiny_mro_get_linear_isa(pTHX_ HV* const stash){
    ...
    get_linear_isa = get_cv("Type::Tiny::XS::Util::get_linear_isa", TRUE);

    {
        ...
        if(IsArrayRef(avref)){
            ...
            Perl_croak(aTHX_ "Type::Tiny::XS::Util::get_linear_isa() didn't return an ARRAY reference");
        }
...
#ifdef DEBUGGING
SV*
typetiny_av_at_safe(pTHX_ AV* const av, I32 const ix){
    assert(av);
...
typetiny_throw_error(SV* const metaobject, SV* const data /* not used */, const char* const fmt, ...){
...

    {
        ...
        if(SvOK(metaobject)) {
            ...
            call_pv("Type::Tiny::XS::Util::throw_error", G_VOID);
        }
...
I32
typetiny_call_sv_safe(pTHX_ SV* const sv, I32 const flags) {
    I32 count;
    ENTER;
    /* Don't do SAVETMPS */
...
void
typetiny_must_defined(pTHX_ SV* const value, const char* const name) {
    assert(value);
    assert(name);
...
void
typetiny_must_ref(pTHX_ SV* const value, const char* const name, svtype const t) {
    assert(value);
    assert(name);
...
bool
typetiny_is_class_loaded(pTHX_ SV * const klass){
    HV *stash;
    GV** gvp;
    HE* he;
...
SV*
typetiny_call0 (pTHX_ SV* const self, SV* const method) {
    dSP;
    SV *ret;
...
SV*
typetiny_call1 (pTHX_ SV* const self, SV* const method, SV* const arg1) {
    dSP;
    SV *ret;
...
int
typetiny_predicate_call(pTHX_ SV* const self, SV* const method) {
    return sv_true( mcall0(self, method) );
}

SV*
typetiny_get_metaclass(pTHX_ SV* metaclass_name){
    dMY_CXT;
    HE* he;
...
MAGIC*
typetiny_mg_find(pTHX_ SV* const sv, const MGVTBL* const vtbl, I32 const flags){
    MAGIC* mg;
    ...
    if(flags & TYPETINYf_DIE_ON_FAIL){
        croak("typetiny_mg_find: no MAGIC found for %"SVf, sv_2mortal(newRV_inc(sv)));
    }
    return NULL;
}

GV*
typetiny_stash_fetch(pTHX_ HV* const stash, const char* const name, I32 const namelen, I32 const create) {
...
void
typetiny_install_sub(pTHX_ GV* const gv, SV* const code_ref) {
    CV* cv;
...
MODULE = Type::Tiny::XS::Util  PACKAGE = Type::Tiny::XS::Util
...
PPCODE:
{
    ...
    if(ix == 0){
        xsub = typetiny_generate_isa_predicate_for(aTHX_ arg, name_pv);
    }
    else{
        xsub = typetiny_generate_can_predicate_for(aTHX_ arg, name_pv);
    }
...
CODE:
{
    ...
    for(i = 1; i < items; i += 2) {
        ...
        typetiny_install_sub(aTHX_ gv, code);
    }
}
