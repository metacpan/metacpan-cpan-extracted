...
static int parse_nat_range(char *string, struct ip_nat_range *range,
	...
	if(sep) {
		if(entry->ip.proto != IPPROTO_TCP && entry->ip.proto != IPPROTO_UDP) {
			SET_ERRSTR("to-source: Protocol must be TCP or UDP to specify "
							"ports");
			return(FALSE);
		}
		...
		if(extent < (psep ? psep : (ptext + strlen(ptext))) || port == 0 ||
						port > USHRT_MAX)
			return(FALSE);
		...
		if(psep) {
			...
			if(extent < (ptext + strlen(ptext)) || port == 0 ||
							port > USHRT_MAX || port < range->min.tcp.port)
				return(FALSE);
		...
		for(i = 0; i <= av_len(av); i++) {
			...
			if(info->rangesize) {
				...
				*targinfo = realloc(*targinfo, (*targinfo)->u.target_size);
				info = (MODULE_DATATYPE *)(*targinfo)->data;
...
static void get_fields(HV *ent_hash, void *myinfo, struct ipt_entry *entry) {
	...
	hv_store(ent_hash, "to-source", 9, sv, 0);
}

static int final_check(void *myinfo, int flags) {
	if(!flags) {
		SET_ERRSTR("SNAT target requires 'to-source'");
		return(FALSE);
	}
