...
sub groucho_input {
  ...
  if ( $cmd eq 'NOTICE' ) {
    ...
    if ( $heap->{undline} ) {
      is( $params->[1], 'D-Line for [127.0.0.1/32] is removed', 'D-Line for [127.0.0.1/32] is removed' );
    }
    else {
...
sub ircd_daemon_eob {
  ...
  if ( $heap->{eob} >= 3 ) {
    ...
    $kernel->post( 'groucho', 'send_to_server', { prefix => '7UPAAAAAA', command => 'DLINE', params => [ '*', '600', '127.0.0.1/32', 'Banhammer'] } );
  }
  return;
}
