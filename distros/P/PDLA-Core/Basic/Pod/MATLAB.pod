...
C<perldla> or C<pdla2> shell) you must start the program with C<use PDLA;>.
...
works over the entire piddle.
  ...
  pdla> p $A->max
  5
  pdla> p ones(2,2,2)->max
  1
  pdla> p ones(2,2,2,2)->max
  1
...
documentation for L<PDLA::IO>, e.g., C<help PDLA::IO> in the I<pdla2>
shell or C< pdladoc PDLA::IO > from the shell command line.
