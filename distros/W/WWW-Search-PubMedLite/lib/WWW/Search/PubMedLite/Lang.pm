package WWW::Search::PubMedLite::Lang;
...
Given a three-character ISO 639-2 language code, returns the English
...
sub abbr2name {
  my( $pkg, $abbr3 ) = @_;
  $pkg->_load unless $loaded;
  return $lang_table->{$abbr3};
}
...
sub _load {
  ...
  while( <DATA> ) {
    chomp;
    my( $abbr3, $abbr3t, $abbr2, $eng, $fr )  = split '\|';
    $lang_table->{$abbr3} = $eng;
  }
  $loaded = 1;
}
