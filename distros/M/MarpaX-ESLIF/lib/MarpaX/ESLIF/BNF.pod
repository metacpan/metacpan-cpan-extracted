...
In any of these cases, the call must be successful a return a string that is a new grammar, that ESLIF will compile and execute as a lexeme. ESLIF will be try to clever to not recompile grammars everytime, though it will use a very basic check: the whole UTF-8 version of the grammar will be kept in memory, and only grammars with a different UTF-8 version will be recompiled.
...
A concrete lua example is e.g.:
  ...
  local recognizerInterface = {
     ...
     ["parameterizedRhs"]       = function(self, parameter)
        ...
        elseif (self._nbParameterizedRhsCalls > 5) then
          ...
          output = "start ::= . => parameterizedRhs->("..parameter..")"
        end
        return output
     end
  }
  ...
  top ::= . => parameterizedRhs->(1)
        ...
        | . => parameterizedRhs->(4)
        | . => ::luac->function(x)
                         return "thisgrammar ::= 'will not match'"
                       end
               ->(5)
        | . => ::luac->function(x)
