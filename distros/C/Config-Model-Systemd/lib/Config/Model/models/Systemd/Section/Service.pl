...
Defaults to C<yes>.',
      'NonBlocking' => 'Set the C<O_NONBLOCK> flag for all file descriptors passed via
...
',
      'TimeoutStartFailureMode' => 'These options configure the action that is taken in case a daemon service does not signal
...
but will be logged and will cause e.g. systemctl reload to fail.",
      'TimeoutStopFailureMode' => '*TimeoutStartFailureMode',
