...
sub assert_lib {
  ...
  @libpaths
   ...
   if $args{libpath};
  @headers
   = ( ref( $args{header} ) ? @{ $args{header} } : $args{header} )
   if $args{header};
  @incpaths
    ...
    if ( $Config{cc} eq 'cl' ) {    # Microsoft compiler
      ...
      @sys_cmd
       = ( @cc, ( map { "-I$_" } @incpaths ), "-o$exefile", $cfile );
          ...
          # gcc, Sun, AIX (gcc, cc)
      @sys_cmd = ( @cc, $cfile, ( map { "-I$_" } @incpaths ), "-o",
        "$exefile" );
    }
    warn "# @sys_cmd\n" if $args{debug};
    my $rv
     = $args{debug} ? system( @sys_cmd ) : _quiet_system( @sys_cmd );
  ...
  for my $lib ( @libs ) {
    ...
    if ( $Config{cc} eq 'cl' ) {    # Microsoft compiler
      ...
      @sys_cmd = ( @cc, $cfile, "${lib}.lib", "/Fe$exefile", "/link",
        @libpath );
    }
