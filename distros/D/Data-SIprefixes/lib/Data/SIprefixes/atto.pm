...
Data::SIprefixes::atto - This provides femto matching for Data::SIprefixes.
...
=head1 SYNOPSIS
    ...
    use Data::SIprefixes::atto;
    
    my $prefix=Data::SIprefixes::atto->new;

    my $origMeasure='attometer';
...
sub longMatch{
	...
	if ( $measure=~/^atto/ ){
		...
		if ( $measure =~ /^ / ){
			$self->{error}=2;
			$self->{errorString}='Space found after prefix, /^atto/, in "'.$origMeasure.'"' ;
			$self->warn;
			return undef;
		}
...
sub shortMatch{
    ...
    if ( $measure=~/^a/ ){
        ...
        if ( $measure =~ /^ / ){
            $self->{error}=2;
            $self->{errorString}='Space found after prefix, /^a/, in "'.$origMeasure.'"' ;
            $self->warn;
            return undef;
        }
