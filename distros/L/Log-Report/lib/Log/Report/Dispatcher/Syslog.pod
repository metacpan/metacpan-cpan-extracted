...
Translate the text-strings into the specified C<$charset>, otherwise the
...
Any combination of C<$flags> as defined by Sys::Syslog, as comma-separated
...
Returns the C<CODE> ref which formats the syslog line.
...
=item $obj-E<gt>B<log>(\%options, $reason, $message, $domain)
...
=item Error: max one active syslog dispatcher, attempt for $new have $old

Cast by C<new()>

=item Error: syslog level '$level' not understood
