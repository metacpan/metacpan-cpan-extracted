...
#pod
#pod =head2 Version Handling
#pod
#pod I want the version of my distribution is bumped automatically after each release, and automatically
#pod assigned version should be alpha (trial) one.
#pod
#pod For example: if I released version "0.001", the version of the next release should be "0.001_01".
#pod When I release "0.001_01", the next should be "0.001_02", the next one — "0.001_03" and so on.
#pod When I decide it is time to non-trial release, I will reset the version to "0.002" manually,
#pod release it, and will have automatically bumped version "0.002_01".
#pod
#pod This is implemented with two plugins: C<Hook:VersionProvider> and C<Hook::AfterRelease>. The first
#pod one reads version from external file F<VERSION> which contains only version and nothing more (ok,
#pod trailing newline is allowed) — it simplifies implementation, because there is no need in parsing
#pod F<dist.ini> file. The second plugin bumps the version after release with help from C<Perl::Version>
#pod module, and writes bumped version back to the F<VERSION> file.
#pod
#pod F<dist.ini> file:
#pod
#pod     name     = VersionHandling
#pod     abstract = Hook demo: Bump version after release
#pod     [Hook/prologue]                     ; <<<=== Look at this
#pod         . = use Path::Tiny;
#pod         . = use Perl::Version;
#pod     [Hook::VersionProvider]             ; <<<=== Look at this
#pod         . = path( $zilla->root )->child( 'VERSION' )->slurp =~ s{\s*\z}{}r;
#pod         . =     #       Strip trailing whitespaces, if any. ^^^^^^^^^^^^^^
#pod     [GenerateFile/Assa.pm]
#pod         filename = lib/Assa.pm
#pod         content  = package Assa; 1;
#pod     [MetaJSON]
...
#pod         . = my $ver = Perl::Version->new( $dist->version );
#pod         . = $ver->inc_alpha;
#pod         . = path( $zilla->root )->child( 'VERSION' )->spew( $ver );
#pod         . = $self->log( [ 'Next release will be %s', "$ver" ] );
#pod
#pod F<VERSION> file:
#pod
#pod     0.001
...
=head2 Version Handling

I want the version of my distribution is bumped automatically after each release, and automatically
assigned version should be alpha (trial) one.

For example: if I released version "0.001", the version of the next release should be "0.001_01".
When I release "0.001_01", the next should be "0.001_02", the next one — "0.001_03" and so on.
When I decide it is time to non-trial release, I will reset the version to "0.002" manually,
release it, and will have automatically bumped version "0.002_01".

This is implemented with two plugins: C<Hook:VersionProvider> and C<Hook::AfterRelease>. The first
one reads version from external file F<VERSION> which contains only version and nothing more (ok,
trailing newline is allowed) — it simplifies implementation, because there is no need in parsing
F<dist.ini> file. The second plugin bumps the version after release with help from C<Perl::Version>
module, and writes bumped version back to the F<VERSION> file.
...
F<dist.ini> file:
    ...
    [Hook::VersionProvider]             ; <<<=== Look at this
        . = path( $zilla->root )->child( 'VERSION' )->slurp =~ s{\s*\z}{}r;
    ...
    [Hook::AfterRelease/bump version]   ; <<<=== Look at this
        ...
        . = $ver->inc_alpha;
        . = path( $zilla->root )->child( 'VERSION' )->spew( $ver );
