...
C<EPOLLERR>. I< Optional. Type enum. choice: 'inactive', 'inactive-or-failed'.  > 
...
=head2 ConditionFraction

C<ConditionFraction> may be used to enable a unit on a stable,
...
complementary fraction of machines (e.g. C<!myrollout 30%> matches the other ≈70%).
If the machine ID cannot be determined, the condition fails. I< Optional. Type list of uniline.  > 
...
=head2 ConditionPathIsSocket

C<ConditionPathIsSocket> is similar to
C<ConditionPathExists> but verifies that a certain path exists and is a
socket. I< Optional. Type list of uniline.  > 
...
=head2 ConditionMachineTag

C<ConditionMachineTag> may be used to match against the tags
...
tags matches. I< Optional. Type list of uniline.  > 
...
consequence, C<Condition…=> and C<Assert…=> directives that fail do
